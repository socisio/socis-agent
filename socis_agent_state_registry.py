...
def _stat_db_file_identity(path: Path) -> Optional[Tuple[int, int]]:
    ...
    Mirrors the socis_agent_state helper of the same name; kept local so this
    module has no import-time dependency on socis_agent_state (which imports
...
def acquire(db_path: Optional[Path] = None) -> "SessionDB":
    ...
    shared generation was opened — e.g. by ``socis sessions recover`` or
...
# ── Backwards-compatible aliases (socis_agent_state re-exports) ──
# Kept so call sites and tests can import either from socis_agent_state
