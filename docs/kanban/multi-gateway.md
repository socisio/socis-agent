...
SOCIS supports multiple gateway processes running concurrently — one per profile
...
needed. On every other profile gateway, add to `~/.socis-agent/config.yaml`:
...
Or set the env var: `SOCIS_AGENT_KANBAN_DISPATCH_IN_GATEWAY=false`
