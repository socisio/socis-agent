...
_profile_runtime_scope(profile_home)           (gateway/run.py)
   ├── set_socis_agent_home_override(home)          config / state.db / skills /
...
- `get_secret(name)` resolves: global allowlist → active scope → fallback.
  ...
  - multiplexing **on**, no scope installed: **raises `UnscopedSecretError`**
    ...
    value.
- A small allowlist (`SOCIS_AGENT_HOME`, `SOCIS_AGENT_PROFILE`, proxy settings,
...
reload helper: `socis_cli.env_loader.load_socis_dotenv` skips the
...
## The SOCIS_AGENT_HOME override

`socis_agent_constants.py` holds a context-local override consulted by
`get_socis_agent_home()` before the `SOCIS_AGENT_HOME` env var. Everything that resolves
...
automatically. `get_process_socis_agent_home()` exists for the few machine-level
assets that must not follow the override. `socis_agent_home_key()` gives
...
`profiles_to_serve(multiplex, profile_allowlist)` in `socis_cli/profiles.py`
...
DB handles are resolved at call time through the active SOCIS_AGENT_HOME override —
...
SOCIS_AGENT_HOME override. Asset writes are atomic, type- and size-capped.
...
| Built-in tool registry | Built-ins are process-global; plugin-registered tools are overlaid per profile via `socis_agent_home_key()`. |
...
- `agent/secret_scope.py`, `socis_agent_constants.py`, `gateway/profile_routing.py`,
  `gateway/run.py` (`_profile_runtime_scope`), `socis_cli/profiles.py`
