...
SOCIS includes NeMo Relay as a normal runtime dependency on platforms for
which Relay publishes a native wheel. The shared-metrics integration is built
into SOCIS and does not require a SOCIS observability plugin. SOCIS remains
importable without Relay on other native targets. Those targets use an
explicit reduced-capability no-op host:
SOCIS execution remains available, while Relay scopes, middleware, plugins,
and subscribers are unavailable. The `socis-agent[nemo-relay]` extra remains
as a no-op compatibility alias for existing installation commands.

> [!WARNING]
> This removes the SOCIS `observability/nemo_relay` plugin. Existing users
...
> `plugins.toml` selected with `SOCIS_AGENT_NEMO_RELAY_PLUGINS_TOML`. The legacy
> `SOCIS_AGENT_NEMO_RELAY_ATOF_*` and `SOCIS_AGENT_NEMO_RELAY_ATIF_*` variables no
> longer activate exporters. Without the new variable, SOCIS does not run
> Relay plugin discovery, configuration layering, middleware, or exporters.

SOCIS requires NeMo Relay 0.8.3 or later within the 0.8 release line. That
line provides the provider-codec and canonical tool-result contracts SOCIS
uses for managed provider and tool calls.

## Runtime Dependency and Data Boundary

SOCIS installs the platform-specific `nemo-relay` native wheel from the
...
through that native module in the SOCIS process so configured interceptors can
...
Collection remains off unless SOCIS policy enables it:
...
opt-in. Set `SOCIS_AGENT_NEMO_RELAY_PLUGINS_TOML` to a selected `plugins.toml` to
activate configured middleware, exporters, or dynamic plugins. When the
variable is unset, SOCIS does not invoke Relay's plugin initializer, so Relay
...
selected file cannot be loaded, SOCIS reports the error and does not invoke
...
retain the same `session_id` and add `socis.session.segment` plus
`socis.session.segment_reason` (`compaction` or `max_turns`).

## Process-Wide Plugin Policy and Profile Isolation

Relay plugin configuration is a process-level deployment choice, not a SOCIS
profile setting. The first hosted profile triggers lazy initialization, and
every additional profile hosted by that SOCIS process shares the resulting
static middleware, dynamic plugins, subscribers, exporters, and guardrail
policy. After initialization succeeds, SOCIS logs:

```text
Relay plugins are active process-wide and apply to all profiles hosted by this SOCIS process.
```
...
profile's Relay scope stack. Native dynamic plugins are loaded into the SOCIS
process and share the same policy boundary.

Run profiles in separate SOCIS processes when they require different trust
...
SOCIS core owns one Relay host and one isolated Relay session scope per SOCIS
session. Core lifecycle producers use
`socis_cli.observability.relay_runtime` to obtain the shared session handle or
run Relay scope, LLM, tool, and mark APIs in that session context. New product
marks do not require SOCIS plugin registration. Shared-metrics marks must
...
```text
SOCIS turn, API, tool, and approval hooks
  -> Relay session, task, LLM, tool, and mark lifecycle
  -> SOCIS shared-metrics subscriber
  -> SQLite counters
  -> immutable JSON delta package
```

SOCIS sends an empty `LLMRequest` into the metrics-owned lifecycle. This does
...
provider route that SOCIS used for the logical call, such as
...
New calls use `socis.model_route.count`. The previous
`socis.model_call.count` contract remains readable only so pending local
counters created by older builds can be exported without losing data.

The first consented session start emits an empty `socis.client.active` Relay
...
counter in any rolling 24-hour window. The metric has no dimensions; SOCIS
version, OS family, architecture, and install method remain bounded package
resources. Concurrent SOCIS processes share the SQLite latch, so simultaneous
...
Each task run is a Relay `Function` scope named `socis.task_run`, parented to
the owning SOCIS session. The start counter contains only bounded execution
...
provider attempts for the same SOCIS API request ID; they do not inflate the
logical model-call count. Tool calls are deduplicated by their SOCIS tool-call
...
cancellations. Active task ownership follows the task ID if SOCIS rotates its
conversation session during context compression.

Each tool invocation is represented by a Relay tool lifecycle named
`socis.tool_call`. The terminal counter contains only bounded tool category,
outcome, approval outcome, latency, and explicit retry-count buckets. SOCIS
...
tool or plugin names. SOCIS does not infer retries from repeated tool names or
adjacent calls; when the
hook does not provide an explicit retry relationship, the retry bucket is
`unknown`. Approval decisions are emitted as `socis.tool_approval` marks and
...
Successful skill mutations emit `socis.skill.lifecycle` marks with only a
bounded action and provenance. Successful loads emit `socis.skill.load`
marks with bounded provenance, first-use or reuse state, reuse-after-patch
state, and a use-count bucket. SOCIS derives reuse and patch-generation
...
```text
$SOCIS_AGENT_HOME/telemetry/shared_metrics/metrics.sqlite3
$SOCIS_AGENT_HOME/telemetry/shared_metrics/outbox/*.json
```
...
are written with atomic replacement. Each package records the SOCIS version,
...
schema field name, its current scope is one `SOCIS_AGENT_HOME`, so it is more
...
Deleting `$SOCIS_AGENT_HOME/telemetry/shared_metrics` resets the identifier together
...
The install identity is scoped to one `SOCIS_AGENT_HOME`. To reset it, stop SOCIS
processes and remove `$SOCIS_AGENT_HOME/telemetry/shared_metrics`. This deliberately
...
Run a real SOCIS CLI turn against the deterministic local model server:
...
`$SOCIS_AGENT_HOME/telemetry/shared_metrics/outbox/` to the SOCIS telemetry ingest
...
Removing `$SOCIS_AGENT_HOME/telemetry/shared_metrics` still resets local identity,
