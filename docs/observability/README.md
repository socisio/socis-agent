# SOCIS Observer Hooks

SOCIS observer hooks are the read-only telemetry contract for plugins that
...
They do not replace SOCIS' planner, model providers, memory, tool registry,
...
SOCIS also has a first-party NeMo Relay shared-metrics path. It uses these
...
```text
telemetry_schema_version = "socis.observer.v1"
```

Hook callbacks are fail-open. SOCIS catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so SOCIS only builds
...
The native NeMo Relay SDK integration maps SOCIS session, turn, LLM, and tool
