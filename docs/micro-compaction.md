...
has to be thrown away or summarized. SOCIS has always done this in one batch:
...
SOCIS folds the single oldest un-absorbed exchange into a running summary. The
...
The in-memory splice alone isn't enough. SOCIS's normal session flush is
...
With `micro_compact` unset or `false` SOCIS behaves exactly as it always has:
...
Defaults to `$SOCIS_AGENT_HOME/logs/agent.log`. It reports passes, outcome mix, net
