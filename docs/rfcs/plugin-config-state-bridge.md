...
or change arbitrary SOCIS configuration.
...
small host fact requested by the RFC. Settings writes use SOCIS'
...
`SOCIS_AGENT_HOME`, so one globally loaded plugin context follows multiplexed
...
```text
<SOCIS_AGENT_HOME>/plugin-data/<plugin-data-namespace>/state.json
```
...
The implementation is covered with real temporary-SOCIS-home tests for:
...
- [Issue #64227](https://github.com/socisio/socis-agent/issues/64227)
- [RFC PR #58542](https://github.com/socisio/socis-agent/pull/58542) by Topher Ross
