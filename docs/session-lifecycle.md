...
The canonical transcript store is SQLite via `SessionDB` (from `socis_agent_state`). The
...
Counts consecutive restarts via a JSON file (`{SOCIS_AGENT_HOME}/restart_counts.json`). If a
...
This prevents unwanted auto-resets after `socis update`, `socis gateway restart`,
or `/restart`.
