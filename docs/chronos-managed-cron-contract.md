...
Chronos lets a hosted SOCIS gateway **scale to zero** while idle and still
...
|---|---|---|---|
| 1 | agent → NAS (`provision`/`cancel`/`list`) | the agent's existing **Nous Portal access token** (Bearer) — for a hosted agent this is the **bootstrap-session token** NAS planted in `auth.json` (client `socis-cli-vps`), NOT an `agent:*` client token | NAS (its normal agent-token path) |
...
> minted under the bootstrap-only client `socis-cli-vps` and seeded into the
...
1. **Dashboard app** (`socis_cli/web_server.py`) — the agent's only public
...
- **Multi-replica agents:** the store CAS makes the fire at-most-once across N
  gateway replicas sharing one `SOCIS_AGENT_HOME` — exactly one replica runs each
  fire.
