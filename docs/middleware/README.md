...
SOCIS middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing SOCIS'
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before SOCIS calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving SOCIS retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving SOCIS guardrails,
...
- `telemetry_schema_version`: currently `socis.observer.v1`
- `middleware_schema_version`: currently `socis.middleware.v1`
...
SOCIS stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, SOCIS runs
them as a nested chain in registration order. Middleware failures are fail-open:
SOCIS logs a warning and continues with the next middleware or the base
runtime path.
...
For each provider request, SOCIS applies middleware in this order:
...
For each tool call, SOCIS applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal SOCIS pre-execution path against the effective arguments:
...
For isolated local testing, use one `SOCIS_AGENT_HOME` for plugin enablement and the
agent run:

```bash
export SOCIS_AGENT_HOME=/tmp/socis-middleware-test
mkdir -p "$SOCIS_AGENT_HOME"
socis plugins enable <plugin-name>
socis chat --query 'Reply exactly ok'
```
...
```bash
uv sync
uv run socis plugins enable <plugin-name>
uv run socis chat --query 'Reply exactly ok'
```
...
def tag_llm_request(**kwargs):
    ...
    extra_body.setdefault("metadata", {})["socis_middleware_demo"] = True
...
def normalize_terminal_workdir(**kwargs):
    ...
    args.setdefault("workdir", "/tmp/socis-middleware-demo")
    return {
        "args": args,
        "source": "middleware-demo",
...
Return the same response shape SOCIS expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, SOCIS treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, SOCIS preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. SOCIS does not convert downstream
