...
This document is the formal interface between the **SOCIS gateway** (Python,
`gateway/relay/`) and the **connector** (Node/TypeScript,
`SOCIS/gateway-gateway`). The connector implementer's first action is to
read this file.
...
- **Registration.** The gateway registers a **wake URL** at enroll/provision —
  ...
  a dashboard host). Self-hosted: `socis gateway enroll --wake-url <url>` (or
...
**per-gateway secret** (`socis gateway enroll` → connector `/relay/enroll`, or
...
(`SOCIS/gateway-gateway`); this section is the gateway-facing summary.
...
driven by the managed Portal / `socis` CLI.
