...
# IMPORTANT: socis_agent_bootstrap must be the very first import — UTF-8 stdio
# on Windows.  No-op on POSIX.  See socis_agent_bootstrap.py for full rationale.
try:
    import socis_agent_bootstrap  # noqa: F401
except ModuleNotFoundError:
    # Graceful fallback when socis_agent_bootstrap isn't registered in the venv
    # yet — happens during partial ``socis update`` where git-reset landed
...
def _launch_cwd_for_session(source: str) -> Optional[str]:
    ...
    launched from is meaningful for ``socis -c`` / ``--resume`` (relaunch
...
def _session_source_for_agent(platform: Optional[str]) -> str:
    try:
        ...
        source = get_session_env("SOCIS_AGENT_SESSION_SOURCE", "")
    except Exception:
        source = os.environ.get("SOCIS_AGENT_SESSION_SOURCE", "")
...
from socis_cli.timeouts import (
...
_loaded_env_paths = load_socis_dotenv(socis_agent_home=_socis_agent_home, project_env=_project_env)
...
# (socis_agent_state._rows_to_conversation), so any code that mutates a loaded or
...
def _safe_session_filename_component(session_id: str) -> str:
    ...
    ``X-SOCIS-Session-Id`` API header) and are otherwise interpolated raw
    into on-disk artifact filenames under ``~/.socis-agent/sessions/``.  Without
...
class AIAgent:
    ...
    _TOOL_CALL_ARGUMENTS_CORRUPTION_MARKER = (
        "[socis-agent: tool call arguments were corrupted in this session and "
    ...
    def _ensure_db_session(self) -> None:
        ...
        try:
            ...
            # `socis --resume`. set_session_yolo() no-ops on a missing row, so
    ...
    def _ensure_lmstudio_runtime_loaded(
        ...
        from socis_cli.models import ensure_lmstudio_model_loaded
    ...
    def _vprint(self, *args, force: bool = False, **kwargs):
        ...
        parseable single-query flows such as ``socis chat -q``. In that mode,
    ...
    def _should_emit_quiet_tool_messages(self) -> bool:
        ...
        ``socis chat -Q``) always wins: those flows neutralize the rendering
    ...
    def _resolved_api_call_timeout(self) -> float:
        ...
        Priority:
          ...
          3. ``SOCIS_AGENT_API_TIMEOUT`` env var (legacy escape hatch)
          4. 1800.0s default
        ...
        helper, the hardcoded ``SOCIS_AGENT_API_TIMEOUT`` fallback would always be
        ...
        if cfg is not None:
            return cfg
        return env_float("SOCIS_AGENT_API_TIMEOUT", 1800.0)
    ...
    def _resolved_api_call_stale_timeout_base(self) -> tuple[float, bool]:
        ...
        Priority:
          ...
          3. ``SOCIS_AGENT_API_CALL_STALE_TIMEOUT`` env var
        ...
        env_timeout = os.getenv("SOCIS_AGENT_API_CALL_STALE_TIMEOUT")
    ...
    def _stale_timeout_is_explicit(self) -> bool:
        ...
        the ``SOCIS_AGENT_API_CALL_STALE_TIMEOUT`` env var. Reasoning-model floors
        ...
        if get_provider_stale_timeout(self.provider, self.model) is not None:
            return True
        return os.getenv("SOCIS_AGENT_API_CALL_STALE_TIMEOUT") is not None
    ...
    def _codex_silent_hang_hint(self, model: Optional[str] = None) -> Optional[str]:
        ...
        socis-agent #21444 for the symptom history.  The upstream backend
        ...
        return (
            ...
            "See socis-agent#21444 for symptom history."
        )
    ...
    def _decorate_xai_entitlement_error(detail: str) -> str:
        ...
        SuperGrok subscribers can use SOCIS against xai-oauth.  Lots of
    ...
    def _summarize_api_error(error: Exception) -> str:
        ...
        while current is not None and id(current) not in seen:
            ...
            if any(
                ...
                return (
                    "SOCIS can't reach the model provider. You may be offline. "
    ...
    def _hook_payload_max_chars() -> int:
        raw = os.getenv("SOCIS_AGENT_PLUGIN_PAYLOAD_MAX_CHARS", "50000")
        try:
    ...
    def _invoke_api_request_error_hook(
        ...
        *,
        ...
        try:
            from socis_cli import lifecycle as _lifecycle
    ...
    def _redact_message_content(content):
        ...
        Respects ``SOCIS_AGENT_REDACT_SECRETS`` via ``redact_sensitive_text`` —
    ...
    def _save_session_log(self, messages: List[Dict[str, Any]] = None):
        ...
        whose external tooling consumes ``~/.socis-agent/sessions/session_{sid}.json``
        ...
        # untrusted input (X-SOCIS-Session-Id header) and must not escape
        ...
        try:
            ...
            for msg in messages:
                ...
                # output, or user paste. Respects SOCIS_AGENT_REDACT_SECRETS via
    ...
    def interrupt(
        ...
        *,
        ...
        # interrupt event rather than SOCIS' per-thread flag.
    ...
    def redirect(self, text: str) -> bool:
        ...
        During a normal SOCIS model request this cancels only that request;
    ...
    def _record_file_mutation_result(
        ...
        if landed:
            ...
            # can tell SOCIS-authored content from later user hand-edits.
    ...
    def _file_mutation_verifier_enabled(self) -> bool:
        ...
        ``SOCIS_AGENT_FILE_MUTATION_VERIFIER`` env var overrides config.  Exposed
        ...
        try:
            import os as _os
            env = _os.environ.get("SOCIS_AGENT_FILE_MUTATION_VERIFIER")
            if env is not None:
    ...
    def _format_file_mutation_failure_footer(cls, failed: Dict[str, Dict[str, Any]]) -> str:
        ...
        (e.g. ``~/.socis-agent/config.yaml``) to a messaging channel (#35584).
        """
        if not failed:
            return ""
        lines = [
    ...
    def _turn_completion_explainer_enabled(self) -> bool:
        ...
        True).  ``SOCIS_AGENT_TURN_COMPLETION_EXPLAINER`` env var overrides
        ...
        try:
            import os as _os
            env = _os.environ.get("SOCIS_AGENT_TURN_COMPLETION_EXPLAINER")
            if env is not None:
                return env.strip().lower() not in {"0", "false", "no", "off"}
    ...
    def _format_turn_completion_explanation(
        ...
        if reason == "session_persistence_failed":
            ...
            if cause == "turn_lease":
                return (
                    prefix
                    + "the turn was stopped because another SOCIS process "
            ...
            if cause == "locked":
                return (
                    ...
                    "(another SOCIS process was writing to the state "
            ...
            if cause == "replaced":
                return (
                    ...
                    "`socis doctor --fix` or in-place FTS repair — stop "
            ...
            if cause == "corrupt":
                ...
                # SOCIS_AGENT_HOME do not live under ~/.socis-agent).
                ...
                return (
                    ...
                    "further\n"
                    "3. Restore from a backup in ~/.socis-agent/backups/\n"
            ...
            return (
                ...
                "Check the state database health (`socis doctor`), then "
    ...
    def _touch_activity(
        ...
        *,
        ...
        process is a dispatcher-spawned worker (SOCIS_AGENT_KANBAN_TASK set),
    ...
    def _capture_credits(self, http_response: Any) -> None:
        ...
        """
        # Dev test fixture (SOCIS_AGENT_DEV_CREDITS_FIXTURE): inject a chosen notice state
        ...
        if _fixture is not None:
            ...
            logger.info(
                ...
                "(real headers bypassed — `echo clear` / unset SOCIS_AGENT_DEV_CREDITS_FIXTURE to restore)",
        ...
        if not headers:
            return
        _dev = is_truthy_value(os.environ.get("SOCIS_AGENT_DEV_CREDITS"))
        ...
        if _dev:
            # SOCIS_AGENT_DEV_CREDITS: stream each capture to agent.log — watch live with
            # `socis logs -f` (grep 'credits ▸'). Dev-only; silent for normal users.
    ...
    def close(self) -> None:
        ...
        # a private embedded daemon when SOCIS YOLO selected unrestricted
    ...
    def _try_refresh_codex_client_credentials(self, *, force: bool = True) -> bool:
        ...
        # pool entry (``socis auth add xai-oauth``) whose tokens belong to
    ...
    def _try_refresh_env_client_credentials(self) -> bool:
        """Adopt ~/.socis-agent/.env credential/base-url edits at the turn boundary.

        A Settings save (desktop ``PUT /api/env``, ``socis setup``) updates
        ...
        if (
            ...
            if self.provider == "kimi-coding":
                from socis_cli.auth import _resolve_kimi_base_url

                base_url = _resolve_kimi_base_url(
                    api_key, pconfig.inference_base_url, env_url
                ).rstrip("/")
            elif self.provider == "zai":
                from socis_cli.auth import _resolve_zai_base_url
    ...
    def _apply_client_headers_for_base_url(
        ...
        *,
        ...
        if base_url_host_matches(base_url, "openrouter.ai"):
            ...
            self._client_kwargs["default_headers"] = socis_xai_default_headers()
        else:
    ...
    def _supports_reasoning_extra_body(self) -> bool:
        ...
        """
        if base_url_host_matches(self._base_url_lower, "socis.io"):
            return True
        ...
        if (
            ...
            try:
                from socis_cli.models import github_model_reasoning_efforts
    ...
    def _lmstudio_reasoning_options_cached(self) -> list[str]:
        ...
        if cached is not None:
            ...
            if opts or (_time.monotonic() - ts) < 60:
                return opts
        try:
            from socis_cli.models import lmstudio_model_reasoning_options
    ...
    def _ollama_supports_thinking_cached(self) -> bool:
        ...
        try:
            from socis_cli.models import ollama_model_supports_thinking
    ...
    def _compress_context(
        ...
        *,
        ...
        try:
            ...
            # id rotation updated socis_agent_logging._session_context (a
            ...
            try:
                from socis_agent_logging import set_session_context
            ...
            # SOCIS_AGENT_SESSION_ID to the child id after an out-of-place
    ...
    def run_conversation(
        ...
        from agent.prompt_cache_scope import declared_conversation_scope_safe
        from socis_cli.observability.relay_shared_metrics import (
            finish_task_run,
            start_task_run,
        )
        ...
        try:
            ...
            # Serialize the full load -> run -> flush region across SOCIS
            ...
            if (
                ...
                def _on_session_turn_lease_wait(elapsed: float) -> None:
                    ...
                    if elapsed < 1.0:
                        self._emit_status(
                            "⏳ Another SOCIS process is using this session; "
                            "waiting for it to finish before starting your turn..."
                        )
                    else:
                        self._emit_status(
                            "⏳ Still waiting for the other SOCIS process on "
                ...
                if not _turn_db.acquire_session_turn_lease(
                    ...
                    if getattr(self, "_interrupt_requested", False):
                        ...
                        interrupt_msg = (
                            "Stopped waiting for another SOCIS process on "
                    ...
                    timeout_msg = (
                        "⏳ Another SOCIS process kept this session busy too "
