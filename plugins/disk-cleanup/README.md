...
Auto-tracks and cleans up ephemeral files created during SOCIS Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$SOCIS_AGENT_HOME` and `/tmp/socis-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `SOCIS_AGENT_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `SOCIS_AGENT_HOME` or `/tmp/socis-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$SOCIS_AGENT_HOME/disk-cleanup/` is itself excluded
- `$SOCIS_AGENT_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
