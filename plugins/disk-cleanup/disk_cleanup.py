"""disk_cleanup — ephemeral file cleanup for SOCIS Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under SOCIS_AGENT_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly SOCIS_AGENT_HOME and /tmp/socis-*
Never touches: ~/.socis-agent/logs/ or any system directory.
"""
...
try:
    ...
    def get_socis_agent_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("SOCIS_AGENT_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".socis-agent").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$SOCIS_AGENT_HOME/logs/``."""
    return get_socis_agent_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$SOCIS_AGENT_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under SOCIS_AGENT_HOME or ``/tmp/socis-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    socis_agent_home = get_socis_agent_home()
    try:
        path.resolve().relative_to(socis_agent_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/socis-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("socis-"):
        return True
    return False
...
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    ...
    "socis-agent", "backups", "profiles", ".worktrees",
...
# Paths under $SOCIS_AGENT_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    history at once.
    """
    # Lazily build the set once per process so SOCIS_AGENT_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        socis_agent_home = get_socis_agent_home()
        for parent in ("cron", "cronjobs"):
            base = socis_agent_home / parent
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside SOCIS_AGENT_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under SOCIS_AGENT_HOME, but never recurse into known
    # durable state trees.  Some installs place the SOCIS checkout, venv,
    # and desktop build under SOCIS_AGENT_HOME; a full rglob over that tree can
    # stall the gateway event loop for minutes.
    socis_agent_home = get_socis_agent_home()
    empty_removed = 0
    sweep_stack: List[Tuple[Path, bool]] = []
    try:
        for top in socis_agent_home.iterdir():
            if (
                top.is_dir()
...
def guess_category(path: Path) -> Optional[str]:
    ...
    socis_agent_home = get_socis_agent_home()
    try:
        rel = path.resolve().relative_to(socis_agent_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "socis-agent",
        ...
        # Path isn't under SOCIS_AGENT_HOME (e.g. /tmp/socis-*) — fall through.
        pass
