...
# ``model_tools`` import time, so ``socis status``, ``socis chat``, etc.
...
# ``socis_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "SOCIS is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a socis MessageType.
...
def card_spec_to_cards_v2(card_spec: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {"cardId": str(card_spec.get("card_id") or "socis-card"), "card": card}
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${SOCIS_AGENT_HOME}/google_chat_thread_counts.json so
        ...
        try:
            from socis_agent_constants import get_socis_agent_home as _get_socis_agent_home
            _socis_agent_home = _get_socis_agent_home()
        except (ModuleNotFoundError, ImportError):
            _socis_agent_home = _Path.home() / ".socis-agent"
        self._thread_count_store = _ThreadCountStore(
            _socis_agent_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "SOCIS is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        return _get_socis_agent_home() / "google_chat_bot_id.json"
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${SOCIS_AGENT_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a socis MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'SOCIS is thinking…' marker message.
        ...
        "SOCIS is thinking…" stuck in chat forever, plus one card that
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "SOCIS is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "SOCIS is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "SOCIS is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``socis setup``.

    The setup wizard at ``socis_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.socis-agent/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. socis-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.socis-agent/.env")
    print_info("Restart the gateway: socis gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``socis cron`` running as a
    separate process from ``socis gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the SOCIS plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="Run `socis setup` to install Google Chat support.",
        ...
        # the "SOCIS is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'SOCIS is thinking…' marker message "
