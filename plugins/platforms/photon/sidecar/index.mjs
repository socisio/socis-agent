// SOCIS Agent — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> SOCIS): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (SOCIS -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-SOCIS-Sidecar-Token: ${TOKEN}`):
...
//                          default off — toggle with `socis photon telemetry`)
  ...
  streamRestartTimer = setTimeout(() => {
    ...
    console.error(
      `photon-sidecar: upstream stream degraded for ${degradedForMs}ms; ` +
        "exiting so SOCIS can restart the Photon adapter"
    );
    process.exit(75);
...
const PROBE_MSG_PREFIX = "socis-liveness-probe-";
...
// instead of a cryptic module-resolution error during import. Apply SOCIS'
...
function inboundStreamErrorMessage(e) {
  ...
  // iMessage internal error. Local SOCIS allowlists cannot cause or fix this:
  ...
  if (
    ...
    out +=
      ...
      "this is upstream of SOCIS, so inbound iMessages may not be delivered " +
...
const server = http.createServer(async (req, res) => {
  if (!tokenOk(req.headers["x-socis-sidecar-token"])) {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when SOCIS supplied them so a known-good
