...
// `message.content.text` before SOCIS can see it. We rewrite the two inbound
...
const MARKER = "SOCIS patch: Preserve mixed text + attachment iMessage payloads";
...
export function patchSpectrumTs(root = scriptDir()) {
  ...
  for (const file of files) {
    ...
    // SOCIS to patch; keep the legacy v8 path below for older pinned installs.
    if (
