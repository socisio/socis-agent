...
Small Node helper that bridges SOCIS Agent to Photon's Spectrum SDK
(`spectrum-ts`).  SOCIS is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-SOCIS-Sidecar-Token`)
- drains the inbound message stream so `spectrum-ts` keeps its
  reconnect/heartbeat machinery alive and SOCIS can receive inbound messages
...
The SOCIS plugin's `socis photon setup` command runs `npm install`
here automatically.
...
`Space.send(...)` API. SOCIS is Python, so replies go through this sidecar
