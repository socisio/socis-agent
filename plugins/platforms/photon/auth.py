...
Credential storage mirrors every other SOCIS channel:

    * runtime SDK creds  -> ``~/.socis-agent/.env``  (``PHOTON_PROJECT_ID`` =
      project id, ``PHOTON_PROJECT_SECRET``) via ``save_env_value``
    * management metadata -> ``~/.socis-agent/auth.json`` under
...
try:
    import httpx
except ImportError:  # pragma: no cover - httpx is a socis dependency
...
# registers SOCIS as its own client_id.
...
# Default name of the project SOCIS provisions for the operator.
DEFAULT_PROJECT_NAME = "SOCIS Agent"
...
# ---------------------------------------------------------------------------
# auth.json helpers — share the file with the rest of socis-agent.

def _auth_json_path() -> Path:
    """Resolve ``~/.socis-agent/auth.json`` honouring the active SOCIS profile."""
    try:
        from socis_agent_constants import get_socis_agent_home
        return Path(get_socis_agent_home()) / "auth.json"
    except Exception:
        return Path(os.path.expanduser("~/.socis-agent")) / "auth.json"
...
def _save_auth(data: Dict[str, Any]) -> None:
    ...
    # socis_cli/auth.py:_save_auth_store (#19673, #21148).
    fd = os.open(
        str(tmp),
...
def store_photon_token(token: str) -> None:
    ...
    from socis_cli.auth import _auth_store_lock
...
def load_project_credentials() -> Tuple[Optional[str], Optional[str]]:
    ...
    Precedence: process env (``~/.socis-agent/.env`` is loaded into the gateway's
...
def store_project_credentials(
    *,
    ...
    The runtime SDK creds land in ``~/.socis-agent/.env`` via the same
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.socis-agent/.env`` (canonical runtime store).
    ...
    try:
        ...
        logger.warning("photon: socis_cli.config unavailable — skipping .env write")
        return
    try:
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    labels["device_token"] = (
        ...
        else "✗ missing (run `socis photon setup`)"
    )
    ...
    labels["phone_number"] = (
        phone if phone else "✗ missing (run `socis photon setup --phone ...`)"
    )
    labels["assigned_phone_number"] = (
        assigned if assigned else "✗ missing (run `socis photon setup`)"
    )
...
def credential_summary() -> Dict[str, str]:
    ...
    def _present_phone() -> str:
        ...
        return phone or "✗ missing (run `socis photon setup --phone ...`)"

    def _present_assigned_phone() -> str:
        _phone, assigned = load_user_numbers()
        return assigned or "✗ missing (run `socis photon setup`)"
