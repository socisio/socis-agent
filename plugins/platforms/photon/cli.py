"""
``socis photon ...`` CLI subcommands — registered by the plugin via
...
there is no standalone ``login`` verb (matching how every other SOCIS
...
# Writable sidecar runtime dir (mirrors to SOCIS_AGENT_HOME on immutable
...
# wiring imports this module for `socis --help`).
...
# when called later (gateway start, socis status). Cleared on success.
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire up `socis photon ...` subcommands."""
    ...
    p_setup.add_argument("--project-name", default=None,
                         help="Project name (default: 'SOCIS Agent')")
...
def _run_device_login(args: argparse.Namespace) -> int:
    ...
    no standalone ``socis photon login`` command; Photon onboards
...
def _cmd_setup(args: argparse.Namespace) -> int:
    ...
    # 2. Find or create the "SOCIS Agent" project.
    ...
    # 3. Provision Spectrum credentials (runtime -> ~/.socis-agent/.env,
    ...
    try:
        from socis_cli.config import write_platform_config_field
    ...
    print("  Start the gateway:  socis gateway start")
    return 0
...
def _cmd_status(_args: argparse.Namespace) -> int:
    ...
    print(f"  sidecar deps        : {'✓ installed' if sidecar_installed else '✗ run `socis photon install-sidecar`'}")
    print(f"  telemetry           : {'on' if _telemetry_enabled() else 'off'} (`socis photon telemetry on|off`)")
    return 0
...
def _telemetry_enabled() -> bool:
    """Read PHOTON_TELEMETRY from the env / ~/.socis-agent/.env.
...
def _cmd_telemetry(args: argparse.Namespace) -> int:
    ...
    if state is None:
        ...
        print("  Toggle with `socis photon telemetry on` / `socis photon telemetry off`.")
        return 0
    try:
        ...
        return 1
    print(f"✓ Spectrum telemetry turned {state} (PHOTON_TELEMETRY in ~/.socis-agent/.env)")
    print("  Restart the gateway for the sidecar to pick it up:  socis gateway restart")
    return 0
...
#
# `socis gateway setup` discovers platforms via the registry and calls each
...
# project + user + sidecar flow as ``socis photon setup`` with interactive
# defaults (phone is prompted when stdin is a TTY).

def gateway_setup() -> None:
    """Run Photon first-time setup from the `socis gateway setup` wizard."""
