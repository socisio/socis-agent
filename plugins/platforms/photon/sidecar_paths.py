...
under an immutable ``/opt/socis`` (read-only for the socis user), which
...
4. Source dir read-only and deps missing or stale → mirror the sidecar
   source files to ``$SOCIS_AGENT_HOME/photon/sidecar`` (the durable data volume,
...
def resolve_sidecar_dir(source_dir: Optional[Path] = None) -> Path:
    ...
    mirror = get_socis_agent_home() / "photon" / "sidecar"
    try:
