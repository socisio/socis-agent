# A2A — Agent-to-Agent protocol for SOCIS
...
peer (another SOCIS, LangChain, CrewAI, Google ADK, OpenClaw, …). Stdlib only —
no `a2a-sdk` dependency.

## Enable

```bash
socis gateway setup      # pick A2A, or:
```
...
When the `a2a` platform is enabled, SOCIS serves a v1.0 Agent Card at
...
- Every exchange is logged to `~/.socis-agent/a2a_audit.jsonl`.
- Conversations persist to `~/.socis-agent/a2a_conversations/` — they survive context
