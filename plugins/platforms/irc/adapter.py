"""
IRC Platform Adapter for SOCIS Agent.

A plugin-based gateway adapter that connects to an IRC server and relays
messages to/from the SOCIS agent.  Zero external dependencies — uses
...
class IRCAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config, **kwargs):
        ...
        try:
            ...
            self.port = 6697
        self.nickname = _get_scoped_secret("IRC_NICKNAME") or extra.get("nickname", "socis-bot")
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        await self._send_raw(f"USER {self.nickname} 0 * :SOCIS Agent")
    ...
    async def disconnect(self) -> None:
        ...
        if self._writer and not self._writer.is_closing():
            try:
                await self._send_raw("QUIT :SOCIS Agent shutting down")
    ...
    async def _handle_line(self, raw: str) -> None:
        ...
        if command == "433":
            # Retry with incrementing suffix: socis_, socis_1, socis_2...
...
def check_requirements() -> bool:
    ...
    # socis setup / requirements check path.
...
def interactive_setup() -> None:
    """Interactive `socis gateway setup` flow for the IRC platform.
    ...
    print_info("Connect SOCIS to an IRC network. Uses Python stdlib — no extra packages needed.")
    ...
    channel = prompt(
        "Channel to join (e.g. #socis — comma-separate for multiple)",
    ...
    print()
    print_success("IRC configuration saved to ~/.socis-agent/.env")
...
async def _standalone_send(
    ...
    *,
    ...
    nickname = _get_scoped_secret("IRC_NICKNAME") or extra.get("nickname", "socis-bot")
    ...
    nick_base = nickname.rstrip("_0123456789-")[:24] or "socis-bot"
    ...
    try:
        ...
        await _raw(f"USER {standalone_nick} 0 * :SOCIS Agent (cron)")
