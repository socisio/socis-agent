"""SimpleX Chat platform adapter (SOCIS plugin).
...
This adapter ships as a SOCIS platform plugin under
``plugins/platforms/simplex/``. The SOCIS plugin loader scans the
...
Optional environment variables:
    ...
    SOCIS_AGENT_SIMPLEX_TEXT_BATCH_DELAY
...
discoverable and ``socis setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside SOCIS — no
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(
            os.getenv("SOCIS_AGENT_SIMPLEX_TEXT_BATCH_DELAY", "0.8")
        )
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``socis setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.socis-agent/.env`` via
    ``socis_cli.config``.
    """
    print()
    ...
    try:
        ...
        print(
            "socis_cli.config not available; set SIMPLEX_* vars manually in "
            "~/.socis-agent/.env"
        )
        return
