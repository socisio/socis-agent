"""
LINE Messaging API platform adapter for SOCIS Agent.
...
to SOCIS Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``SOCIS_AGENT_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            socis_agent_home = Path(get_socis_agent_home()).resolve()
        except Exception:
            socis_agent_home = Path.home().joinpath(".socis-agent").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``socis status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``socis status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``socis setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.socis-agent/.env`` via ``socis_cli.config``.
    """
    print()
    ...
    try:
        from socis_cli.config import get_env_value as _get_env, save_env_value as _set_env
    except ImportError:
        print("socis_cli.config not available; set LINE_* vars manually in ~/.socis-agent/.env")
        return
