"""ntfy platform adapter (SOCIS plugin).
...
``plugins/platforms/ntfy/``. The SOCIS plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "socis-in"              # subscribe topic (incoming)
          publish_topic: "socis-out"     # optional — defaults to topic
...
_ECHO_TAG = "socis-agent"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``socis cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a SOCIS dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `socis gateway status` without
