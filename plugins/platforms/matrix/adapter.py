...
def _resolve_matrix_bang_command(name: str) -> str | None:
    """Resolve a ``!command`` token to a dispatchable SOCIS command token.

    Matrix clients often reserve leading ``/`` for local client commands.
    SOCIS accepts ``!command`` as a Matrix-friendly alias, but only for
...
def _normalize_matrix_bang_command(text: str) -> str:
    """Convert Matrix ``!command`` aliases to normal SOCIS ``/command`` text."""
...
# would pin the root SOCIS_AGENT_HOME for every profile and all bots' Olm
# identities would collide in one crypto.db (#89168). Mirrors the
# pairing-store fix (a6397c379).
from socis_agent_constants import get_socis_dir as _get_socis_dir
...
class MatrixAdapter(BasePlatformAdapter):
    ...
    # the adapter accepts "!command" as the alias that always reaches SOCIS
    ...
    def _resolve_store_dir(self) -> Path:
        ...
        ``_profile_runtime_scope`` -- ``get_socis_dir`` honors that
        context-local SOCIS_AGENT_HOME, so each profile's adapter gets its own
        ...
        """
        self._store_dir = _get_socis_dir("platforms/matrix/store", "matrix/store")
        return self._store_dir

    @property
    def _crypto_db_path(self) -> Path:
        store_dir = self._store_dir or _get_socis_dir("platforms/matrix/store", "matrix/store")
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._text_batch_delay_seconds = float(
            os.getenv("SOCIS_AGENT_MATRIX_TEXT_BATCH_DELAY_SECONDS", "0.6")
        )
        self._text_batch_split_delay_seconds = float(
            os.getenv("SOCIS_AGENT_MATRIX_TEXT_BATCH_SPLIT_DELAY_SECONDS", "2.0")
        )
        ...
        if self._encryption:
            ...
            if self._encryption:
                try:
                    ...
                    _acct_id = self._user_id or "socis"
    ...
    def _strip_mention(self, body: str) -> str:
        ...
        otherwise normal phrases like "SOCIS Agent" become "Agent".
        """
        if not body:
            return ""
...
# + _PLATFORMS["matrix"] static dict in socis_cli/{setup,gateway}.py, and the
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        txn_id = f"socis_{int(time.time() * 1000)}_{os.urandom(4).hex()}"
...
def interactive_setup() -> None:
    """Configure Matrix credentials. Replaces socis_cli/setup.py::_setup_matrix
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print_info("📬 Home Room: where SOCIS delivers cron job results and notifications.")
...
def _is_connected(config) -> bool:
    ...
    configured. Read via socis_cli.gateway.get_env_value so setup-status
