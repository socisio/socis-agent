"""
Email platform adapter for the SOCIS gateway.

Allows users to interact with SOCIS by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        try:
            from socis_cli import __version__ as _socis_version
        except Exception:  # noqa: BLE001 — keep ID best-effort if import fails
            _socis_version = "0"
        imap.xatom(
            "ID",
            f'("name" "socis-agent" "version" "{_socis_version}" '
            '"vendor" "SOCIS" '
            '"support-email" "noreply@socis.io")',
        )
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send-helper, and `socis config show` already read. Without the
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if missing:
            message = (
                ...
                + ", ".join(missing)
                + ". Set it via `socis gateway setup` (env) or platforms.email "
                "in config.yaml."
            )
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "SOCIS Agent")
        ...
        msg_id = f"<socis-{uuid.uuid4().hex[:12]}@{self._message_id_domain()}>"
...
# socis_cli/gateway.py, and the _send_email dispatch in
