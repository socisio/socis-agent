...
:func:`socis_cli.models._fetch_deepinfra_models_by_tag` so newly added
models show up in ``socis tools`` automatically. No model ids are
hardcoded in this file — if a model is retired upstream, it disappears
from socis the next time the catalog is fetched, no patch required.
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not api_key:
            return error_response(
                error=(
                    "DEEPINFRA_API_KEY not set. Run `socis tools` → Image "
                    "Generation → DeepInfra to configure, or `socis setup` "
                    "to add the key."
                ),
        ...
        from socis_cli.models import deepinfra_base_url
