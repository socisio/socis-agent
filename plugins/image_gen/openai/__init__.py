...
three virtual model IDs so the ``socis tools`` model picker and the
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not api_key:
            return error_response(
                error=(
                    "OPENAI_API_KEY not set. Run `socis tools` → Image "
                    "Generation → OpenAI to configure, or `socis setup` "
                    "to add the key."
                ),
