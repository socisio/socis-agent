"""Meta Model API (Muse Spark) provider plugin for SOCIS Agent.
...
Bundled from https://github.com/albertodepaola/hermes-meta-provider. SOCIS'
...
* **Zero core edits.** Everything rides on ``ProviderProfile`` hooks. No changes
  to socis' ``model_metadata.py`` / ``models.py`` / ``run_agent.py`` are needed:
...
def _resolve_effort(reasoning_config: dict | None) -> str:
    """Map SOCIS' reasoning_config to a Meta-safe ``reasoning_effort`` value.
...
meta_ai = MetaAIProfile(
    ...
    # in socis_cli/providers.py and the retention hint in
