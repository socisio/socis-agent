...
request omits ``thinking``.  SOCIS' ``reasoning_config = {"enabled": False}``
...
:meth:`ZaiProfile.build_api_kwargs_extras` translates the SOCIS reasoning
...
(per Z.AI / BigModel docs).  SOCIS' richer effort scale is collapsed onto
...
def _glm_5_2_reasoning_effort(
    ...
    """Map SOCIS reasoning effort onto GLM's native vocabulary.
