...
def _missing_key_error(action: str) -> str:
    return (
        ...
        f"or select Tavily in `socis tools` for opt-in keyless {action}."
    )
...
class TavilyWebSearchProvider(WebSearchProvider):
    ...
    def is_keyless_available(self) -> bool:
        ...
        ``web.backend: tavily`` (or ``socis tools`` pick) works without a
