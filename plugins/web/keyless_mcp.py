...
two endpoints so a fresh SOCIS install with **zero web credentials** still
...
def provider_tier(name: str) -> str:
    ...
    ``socis tools`` picker's Free/Paid rows). ``free`` forces the keyless
    ...
    try:
        ...
        return {
            ...
            "error": (
                ...
                "backend via `socis tools` for reliable service."
            ),
        }
    ...
    try:
        ...
        return {
            ...
            "error": (
                ...
                "via `socis tools` for reliable service."
            ),
        }
