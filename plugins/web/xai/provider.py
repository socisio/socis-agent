...
``{title, url, description, position}`` rows every other SOCIS web
provider produces.
...
prefers SOCIS-managed xAI Grok OAuth (via ``socis auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.socis-agent/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``socis tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `socis auth` to sign in with "
