...
```bash
socis memory setup honcho   # configure Honcho directly (works on a fresh install)
socis memory setup          # generic picker, choose Honcho from the list
```
...
Or manually:
```bash
socis config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.socis-agent/.env
```

> `socis honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `socis memory setup honcho`.
...
cycle (not per pass). Select a fast, inexpensive model under `socis model`
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$SOCIS_AGENT_HOME/honcho.json` | Profile-local (isolated SOCIS instances) |
| 2 | `~/.socis-agent/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active SOCIS profile: `socis` (default) or `socis_<profile>`.
...
> **Deprecated:** `pinPeerName` is a legacy alias for `pinUserPeer`, still read for back-compat (`pinUserPeer` wins where both are set). `socis honcho setup` migrates it onto `pinUserPeer` on touch and never writes it.
...
| 4 | `per-session` strategy | SOCIS session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`socis-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`socis`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `alice-socis-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening socis in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — SOCIS session ID (timestamp + hex). Every `socis` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple SOCIS profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`socis`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active SOCIS profile: `socis` (default) or `socis_<profile>` (e.g. `socis -p coder` -> host key `socis_coder`). Older `socis.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `SOCIS_AGENT_HONCHO_HOST` | Host key override |
...
| `HONCHO_OAUTH_CLIENT_ID` | OAuth client (default `socis-agent`) |
...
|---------|-------------|
| `socis memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `socis honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `socis memory setup`) |
| `socis honcho status` | Show resolved config for active profile |
| `socis honcho enable` / `disable` | Toggle Honcho for active profile |
| `socis honcho mode <mode>` | Change recall or observation mode |
| `socis honcho peer --user <name>` | Update user peer name |
| `socis honcho peer --ai <name>` | Update AI peer name |
| `socis honcho tokens --context <N>` | Set context token budget |
| `socis honcho tokens --dialectic <N>` | Set dialectic max chars |
| `socis honcho map <name>` | Map current directory to a session name |
| `socis honcho sync` | Create host blocks for all SOCIS profiles |
