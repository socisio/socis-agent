...
Resolution order for config file:
  1. $SOCIS_AGENT_HOME/honcho.json  (instance-local, enables isolated SOCIS instances)
...
from socis_cli.profiles import _get_default_socis_agent_home
...
def profile_host_key(profile: str | None) -> str:
    """Return the safe Honcho host key for a SOCIS profile."""
...
def resolve_active_host() -> str:
    """Derive the Honcho host key from the active SOCIS profile.

    Resolution order:
      1. SOCIS_AGENT_HONCHO_HOST env var (explicit override)
      2. Active profile name via profiles system -> ``socis_<profile>``
      3. defaultHost from the active config, but only for the default profile
      4. Fallback: ``"socis"`` (default profile)
    """
    explicit = os.environ.get("SOCIS_AGENT_HONCHO_HOST", "").strip()
    if explicit:
        return explicit
    ...
    # but applying it before profile resolution makes every named SOCIS
    # profile share that same host.  Keep named profiles isolated; only the
    # default SOCIS profile may opt into the config's default host.
...
def resolve_config_path() -> Path:
    ...
    Resolution order:
      1. $SOCIS_AGENT_HOME/honcho.json      (profile-local, if it exists)
      2. ~/.socis-agent/honcho.json          (default profile — shared host blocks live here)
      3. ~/.honcho/config.json          (global, cross-app interop)

    Returns the global path if none exist (for first-time setup writes).
    """
    local_path = get_socis_agent_home() / "honcho.json"
    if local_path.exists():
        return local_path

    # Default profile's config — host blocks accumulate here via setup/clone
    default_path = _get_default_socis_agent_home() / "honcho.json"
...
class HonchoClientConfig:
    ...
    # True when Honcho was explicitly configured for this host (hosts.socis
    ...
    # resolve_config_path()/get_socis_agent_home() later — those resolvers read a
    ...
    def from_global_config(
        ...
        Resolution: $SOCIS_AGENT_HOME/honcho.json -> ~/.honcho/config.json -> env vars.
        When host is None, derives it from the active SOCIS profile.
        """
        resolved_host = host or resolve_active_host()
        ...
        # A hosts.socis block or explicit enabled flag means the user
        ...
        # baseUrl / base_url keys below are the SOCIS-specific spelling.
    ...
    def resolve_session_name(
        ...
        Resolution order:
          ...
          2. per-session strategy — SOCIS session_id ({timestamp}_{hex}); authoritative,
             so a generated title never remaps a live conversation
          3. Manual directory override from sessions map
          4. SOCIS session title (from /title command; non-per-session)
...
def spawn_context_thread(
    ...
    *,
    ...
    (set_socis_agent_home_override); plain threading.Thread targets start with an
    EMPTY context, so any ambient resolution on the thread
    (resolve_config_path, resolve_active_host, get_socis_agent_home) silently
...
def _credential_fingerprint(config: HonchoClientConfig | None) -> str:
    ...
    makes 'socis honcho setup' account switches produce a NEW cache identity
...
def _client_cache_key(config: HonchoClientConfig | None) -> tuple:
    ...
    if config is not None:
        return (
            ...
            str(config.socis_agent_home) if config.socis_agent_home is not None else "",
...
def _slot_for(key: tuple) -> SingletonSlot:
    ...
    When a (kind, host, config_path/socis_agent_home) identity reappears with a
...
def get_honcho_client(config: HonchoClientConfig | None = None) -> Honcho:
    ...
    if not config.api_key and not config.base_url:
        raise ValueError(
            ...
            "then run 'socis honcho setup' or set HONCHO_API_KEY. "
    ...
    def _build() -> "Honcho":
        ...
        try:
            ...
            raise ImportError(
                ...
                "(or run `socis honcho setup` to configure)."
            )
        ...
        if not resolved_base_url or resolved_timeout is None:
            try:
                ...
                socis_cfg = load_config()
                honcho_cfg = socis_cfg.get("honcho", {})
                if isinstance(honcho_cfg, dict):
