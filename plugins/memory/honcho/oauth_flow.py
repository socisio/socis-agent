...
``socis://`` handler. Endpoints are env-overridable with local-dev defaults
...
# The loopback redirect registered for the SOCIS OAuth client. IP-literal so
...
_DEFAULT_CLIENT_ID = "socis-agent"
...
def begin_authorization(
    ...
    *,
    ...
    (``socis-desktop`` / ``socis-cli``) so the consent side can attribute
...
_CALLBACK_HTML = (
    ...
    b"<div>Connected to Honcho. You can close this tab and return to SOCIS.</div>"
)
...
def start_loopback_flow_background(
    *,
    ...
    # the request, where a context-local SOCIS_AGENT_HOME override can't reach.
