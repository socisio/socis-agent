...
- OpenViking server running and reachable from SOCIS

OpenViking 0.2.10 or newer is recommended. For backward compatibility,
SOCIS can identify older servers that expose the legacy status-only health
...
connection values into SOCIS, or create a minimal `ovcli.conf` when one does
not exist.
...
default profile that is `~/.socis-agent/.env`; for a named profile use
...
OpenViking's server config is separate from SOCIS:
...
SOCIS-side provider config is read from environment variables in the active
profile's `.env`:
...
When `OPENVIKING_API_KEY` is set, SOCIS lets OpenViking derive account/user
identity from the key. In local or trusted deployments without an API key,
SOCIS sends `OPENVIKING_ACCOUNT` and `OPENVIKING_USER` as identity headers.
SOCIS also sends `User-Agent: openviking-memory-socis/<version>` on
OpenViking requests. This standard harness identifier contains the SOCIS
...
does not ask for a peer ID. Without a configured peer, SOCIS sends neither
...
order remains environment, linked OpenViking config, then SOCIS YAML. To use
...
on the old implicit `socis` peer now use user memory for new writes. Without
...
Set `agent: socis` to restore peer-scoped writes. Memories written at user
...
`viking_remember` creates a one-shot `socis-remember-<random>` OpenViking
...
not advertised or used. The one-shot session is separate from the live SOCIS
...
Use the same OpenViking profile and credentials as SOCIS for manual recovery.
...
SOCIS built-in `memory` tool additions are mirrored to OpenViking after the
local memory operation succeeds:

| SOCIS action | OpenViking operation |
|---------------|----------------------|
| `add` | `content/write` with `mode=create` under user memory, or the configured peer memory directory |

Built-in `replace` and `remove` operations are not mirrored because SOCIS
...
file URIs, such as
`viking://user/default/peers/socis/memories/preferences/mem_abc123.md` (any
