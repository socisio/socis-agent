...
_DEFAULT_AGENT = ""
_OPENVIKING_USER_AGENT = f"openviking-memory-socis/{_SOCIS_AGENT_VERSION}"
...
_SYNC_TRACE_ENV = "SOCIS_AGENT_OPENVIKING_SYNC_TRACE"
...
def _derive_openviking_user_text(content: Any) -> str:
    """Strip SOCIS slash-skill scaffolding before sending content to OpenViking.
...
def _normalize_openviking_url(url: str) -> str:
    ...
    # could attach SOCIS to an unrelated deployment and forward credentials to
    ...
    try:
        ...
        raise _OpenVikingEndpointError(
            "OpenViking endpoint safety validation failed; SOCIS refused the connection."
        ) from exc
...
def _load_socis_openviking_config() -> dict:
    try:
...
def _openviking_server_log_path() -> Path:
    try:
        ...
        home = Path(os.environ.get("SOCIS_AGENT_HOME", "")).expanduser() if os.environ.get("SOCIS_AGENT_HOME") else Path.home() / ".socis-agent"
...
def _start_local_openviking_server(endpoint: str) -> tuple[str, str]:
    ...
    if _local_openviking_port_is_open(host, port):
        ...
        return (
            ...
            f"Port {host}:{port} is occupied by {listener}. SOCIS did not start "
    ...
    try:
        ...
        # The SOCIS Agent Desktop backend can include the SOCIS venv's
        # site-packages in PYTHONPATH. If inherited, openviking-server would
        # import aiohttp and friends from the SOCIS venv instead of its own
        # (its venv's site-packages are shadowed because PYTHONPATH precedes
        # them) —
        # and on Windows the loaded DLLs then lock the SOCIS venv,
        # aborting `socis update` with access-denied on .pyd files.
...
def _runtime_openviking_timeout_message(endpoint: str) -> str:
    return (
        ...
        "OpenViking memory is temporarily unavailable; SOCIS will retry on a later access or when "
        "the config changes."
    )
...
def _print_openviking_ready(message: str, path: Optional[Path] = None) -> None:
    ...
    print("  Start a new SOCIS session to activate.\n")
...
def _run_create_profile_setup(
    *,
    ...
    save_choice = select(
        ...
        [
            ("Keep in SOCIS only", "write values only to SOCIS .env"),
    ...
    _save_socis_only_config(
        config=config,
        provider_config=provider_config,
        env_path=env_path,
        values=values,
    )
    _print_openviking_ready("Connection saved to SOCIS .env.")
    return True
...
class OpenVikingMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        # against a concurrent increment. See socis-agent#28296 review.
    ...
    def is_available(self) -> bool:
        ...
        if os.environ.get("OPENVIKING_ENDPOINT"):
            return True
        provider_config = _load_socis_openviking_config()
    ...
    def save_config(self, values: Dict[str, Any], socis_agent_home: str) -> None:
    ...
    def post_setup(self, socis_agent_home: str, config: dict) -> None:
        ...
        from socis_cli.memory_setup import _CANCELLED, _curses_select, _print_cancelled_setup, _prompt

        socis_agent_home_path = Path(socis_agent_home)
        env_path = socis_agent_home_path / ".env"
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        connection_error = ""
        try:
            settings = _resolve_connection_settings(_load_socis_openviking_config())
        except _OpenVikingEndpointError as exc:
        ...
        self._turn_count = 0
        socis_agent_home = str(kwargs.get("socis_agent_home") or "").strip()
        if not socis_agent_home:
            try:
                from socis_agent_constants import get_socis_agent_home
                socis_agent_home = str(get_socis_agent_home())
            except Exception:
                socis_agent_home = str(Path.home() / ".socis-agent")
        self._socis_agent_home = socis_agent_home
            ...
            try:
                ...
                if health_state == "unreachable":
                    ...
                    _emit_runtime_warning(
                        ...
                        "SOCIS will retry on a later access or when the config changes.",
                        warning_callback,
                    )
                    self._client = None
            except ImportError:
    ...
    def _ensure_client(self) -> Optional["_VikingClient"]:
        ...
        ``~/.socis-agent/.env`` after startup never reach the live client and tools
        keep running against stale auth until the user restarts socis (#21130).
    ...
    def _ensure_client_locked(self) -> Optional["_VikingClient"]:
        ...
        if health_state == "responded":
            logger.warning(
                "%s OpenViking memory is temporarily unavailable; SOCIS will retry on a "
    ...
    def _pending_session_dir(self) -> Optional[Path]:
        if not self._socis_agent_home:
            return None
        return Path(self._socis_agent_home) / _PENDING_SESSIONS_RELATIVE_DIR
    ...
    def _run_lock_dir(self) -> Optional[Path]:
        if not self._socis_agent_home:
            return None
        return Path(self._socis_agent_home) / _RUN_LOCKS_RELATIVE_DIR
    ...
    def _profile_token_budget(self) -> int:
        cfg = _load_socis_openviking_config()
    ...
    def _extract_current_turn_messages(
        ...
        """Slice the completed turn out of SOCIS' full canonical transcript."""
        if not messages:
            return []
    ...
    def _messages_to_openviking_batch(
        ...
        *,
        ...
        """Convert SOCIS canonical messages into OpenViking batch payloads."""
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        and memory extraction never fires for it. See socis-agent#28296.
    ...
    def _read_uri_payload(
        ...
        *,
        ...
        # (e.g. viking://user/socis/.overview.md).
    ...
    def _tool_remember(self, args: dict) -> str:
        ...
        session_id = f"socis-remember-{uuid.uuid4().hex[:12]}"
        ...
        recovery_note = (
            ...
            "SOCIS. Otherwise, do not resubmit automatically; report the uncertain "
            "state to the user."
        )
        ...
        # otherwise alter the live SOCIS conversation session.
        try:
