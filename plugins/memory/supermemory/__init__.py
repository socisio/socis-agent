...
def _load_supermemory_config(socis_agent_home: str) -> dict:
    config = _default_config()
    config_path = Path(socis_agent_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, socis_agent_home: str) -> None:
    config_path = Path(socis_agent_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class _SupermemoryClient:
    def __init__(self, api_key: str, timeout: float, container_tag: str,
        ...
        self._client = Supermemory(
            ...
            timeout=timeout,
            max_retries=0,
            default_headers={"x-sm-source": "socis"},
        )

    def _merge_metadata(self, metadata: Optional[dict]) -> dict:
        # sm_source routes SOCIS writes into the "SOCIS" Space in the Supermemory
        ...
        merged = {"sm_source": "socis", **(metadata or {})}
...
def _resolve_container_tag_for_setup(socis_agent_home: str, *, identity: str = "default") -> str:
    config = _load_supermemory_config(socis_agent_home)
...
def _probe_supermemory_connection(api_key: str, socis_agent_home: str, *, identity: str = "default") -> dict:
    config = _load_supermemory_config(socis_agent_home)
    base_url = _resolve_base_url(config["base_url"])
    status = {
        "ok": False,
        "error": "",
        "container_tag": _resolve_container_tag_for_setup(socis_agent_home, identity=identity),
        "profile_facts": 0,
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `socis memory setup`.
        # All other options are documented for $SOCIS_AGENT_HOME/supermemory.json
    ...
    def save_config(self, values, socis_agent_home):
        ...
        _save_supermemory_config(sanitized, socis_agent_home)
    ...
    def get_status_config(self, provider_config: dict) -> dict:
        ...
        status = _probe_supermemory_connection(api_key, socis_agent_home)
    ...
    def post_setup(self, socis_agent_home: str, config: dict) -> None:
        ...
        from socis_cli.memory_setup import _prompt, _write_env_vars
        ...
        if env_writes:
            _write_env_vars(env_writes, socis_agent_home=socis_agent_home)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        from socis_agent_constants import get_socis_agent_home
        self._socis_agent_home = kwargs.get("socis_agent_home") or str(get_socis_agent_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._socis_agent_home)
