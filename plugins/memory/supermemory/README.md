...
- Hosted: API key from [app.supermemory.ai/integrations?connect=socis](http://app.supermemory.ai/integrations?connect=hermes)
...
Before running `socis memory setup`, add the local endpoint to
...
Then run `socis memory setup` and enter the local server's API key. Configuring
the endpoint first ensures the setup connection probe also stays local.

## Config

Config file: `$SOCIS_AGENT_HOME/supermemory.json`
...
| `container_tag` | `socis` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `socis-{identity}` → `socis-coder`). |
...
`https://api.supermemory.ai`. SOCIS resolves it once and uses the same endpoint
...
All Supermemory API calls send `x-sm-source: socis`, and document writes stamp
`metadata.sm_source: socis`. This is a **functional routing key, not telemetry**:
it groups SOCIS-written memories into a dedicated "SOCIS" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per SOCIS profile:

```json
{
  "container_tag": "socis-{identity}"
}
```

For a profile named `coder`, this resolves to `socis-coder`. The default profile resolves to `socis-default`. Without `{identity}`, all profiles share the same container.
