...
```bash
socis memory setup    # select "mem0"
```
...
Behavioral settings live in `$SOCIS_AGENT_HOME/mem0.json` (set them via `socis memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.socis-agent/.env`.
...
| `user_id` | `socis-user` | User identifier on Mem0 |
...
2. Point the plugin at it — via the setup wizard:
   ...
   ```
   or in `$SOCIS_AGENT_HOME/mem0.json`:
   ```json
   {
   ...
   ```
3. Start a fresh SOCIS session and call `mem0_search` — it connects to your server.
...
```bash
socis memory setup mem0 --mode oss \
...
```bash
socis memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$SOCIS_AGENT_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.socis-agent/mem0_qdrant"}}
  }
}
```
...
```bash
socis memory setup mem0 --mode platform --api-key sk-...
```
...
- Check `user_id` matches between sessions (`$SOCIS_AGENT_HOME/mem0.json`).
