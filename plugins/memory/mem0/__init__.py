...
Configuration
-------------
Secret (lives in $SOCIS_AGENT_HOME/.env or the environment):
...
Behavioral settings (live in $SOCIS_AGENT_HOME/mem0.json, set via `socis memory
setup`):
  ...
  agent_id           — Agent identifier (default: socis)
...
def _load_config() -> dict:
    """Load config from env vars, with $SOCIS_AGENT_HOME/mem0.json overrides.
    ...
    config = {
        ...
        "agent_id": os.environ.get("MEM0_AGENT_ID", "socis"),
        "oss": {},
    }
    ...
    config_path = get_socis_agent_home() / "mem0.json"
...
class Mem0MemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, socis_agent_home):
        """Write config to $SOCIS_AGENT_HOME/mem0.json."""
        import json
    ...
    def post_setup(self, socis_agent_home: str, config: dict) -> None:
        ...
        post_setup(socis_agent_home, config)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        #   1. Operator-configured MEM0_USER_ID (env or $SOCIS_AGENT_HOME/mem0.json) —
        ...
        self._agent_id = self._config.get("agent_id", "socis")
