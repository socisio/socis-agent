...
the ``socis`` integration, and applies a chosen manifest to the user's bank
...
def fetch_socis_templates(url: str | None = None) -> list[dict]:
    """Return catalog entries tagged for the ``socis`` integration."""
    catalog = _get_json(url or catalog_url())
    entries = catalog.get("templates", []) if isinstance(catalog, dict) else []
    return [e for e in entries if "socis" in (e.get("integrations") or [])]
