"""socis-memory-store — holographic memory plugin using MemoryProvider interface.
...
Config in $SOCIS_AGENT_HOME/config.yaml (profile-scoped):
  plugins:
    socis-memory-store:
      db_path: $SOCIS_AGENT_HOME/memory_store.db   # omit to use the default
      auto_extract: false
      default_trust: 0.5
...
def _load_plugin_config() -> dict:
    try:
        ...
        return cfg_get(all_config, "plugins", "socis-memory-store", default={}) or {}
    except Exception:
        return {}
...
class HolographicMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, socis_agent_home):
        """Write config to config.yaml under plugins.socis-memory-store."""
        from pathlib import Path
        config_path = Path(socis_agent_home) / "config.yaml"
        try:
            import yaml
            ...
            existing["plugins"]["socis-memory-store"] = values
    ...
    def get_config_schema(self):
        ...
        _default_db = f"{display_socis_agent_home()}/memory_store.db"
        return [
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        _socis_agent_home = str(get_socis_agent_home())
        _default_db = _socis_agent_home + "/memory_store.db"
        db_path = self._config.get("db_path", _default_db)
        # Expand $SOCIS_AGENT_HOME in user-supplied paths so config values like
        # "$SOCIS_AGENT_HOME/memory_store.db" or "~/.socis-agent/memory_store.db" both
        # resolve to the active profile's directory.
        if isinstance(db_path, str):
            db_path = db_path.replace("$SOCIS_AGENT_HOME", _socis_agent_home)
            db_path = db_path.replace("${SOCIS_AGENT_HOME}", _socis_agent_home)
