/**
 * SOCIS Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.socis-agent/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
 * and tails task_events over a WebSocket for live updates.
 *
 * Plain IIFE, no build step. Uses window.__SOCIS_AGENT_PLUGIN_SDK__ for React +
...
(function () {
  ...
  const SDK = window.__SOCIS_AGENT_PLUGIN_SDK__;
  if (!SDK) return;
  ...
  const COLUMN_DOT = {
    triage: "socis-kanban-dot-triage",
    todo: "socis-kanban-dot-todo",
    ready: "socis-kanban-dot-ready",
    running: "socis-kanban-dot-running",
    blocked: "socis-kanban-dot-blocked",
    review: "socis-kanban-dot-review",
    done: "socis-kanban-dot-done",
    archived: "socis-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-socis-task";
  ...
  const DOCS_URL = "https://agent.socis.io/docs/user-guide/features/kanban";
  const DOCS_TUTORIAL_URL = "https://agent.socis.io/docs/user-guide/features/kanban-tutorial";
  ...
  const LS_BOARD_KEY = "socis.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="socis-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function sanitizeMarkdownAttrs(tag, attrs) {
    ...
    if (tag === "pre" && /\sclass=(["'])socis-kanban-md-code\1/i.test(attrs)) {
      return ' class="socis-kanban-md-code"';
    }
    return "";
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "socis-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "socis-kanban-md",
  ...
  // standard `drop` event and our `socis-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("socis-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("socis-kanban-column--drop");
          if (target) target.classList.add("socis-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("socis-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("socis-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("socis-kanban:drop", {
  ...
  function KanbanPage() {
    ...
    useEffect(function () {
      ...
      function openWs() {
        ...
        // loopback. Reading window.__SOCIS_AGENT_SESSION_TOKEN__ directly (the old
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "socis-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "socis-kanban-attention",
        "socis-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "socis-kanban-attention-bar" },
        h("span", { className: "socis-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "socis-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "socis-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "socis-kanban-attention-row",
                  "socis-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "socis-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "socis-kanban-attention-row-id" }, task.id),
                h("span", { className: "socis-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "socis-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "socis-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "socis-kanban-diag-action-btn",
      action.suggested ? "socis-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " socis-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".socis-kanban-drawer-comment-row input, .socis-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "socis-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("socis-kanban-diag", sevClass) },
      h("div", { className: "socis-kanban-diag-header" },
        h("span", { className: "socis-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "socis-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "socis-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "socis-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "socis-kanban-diag-data-row" },
                  h("span", { className: "socis-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "socis-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "socis-kanban-diag-data-row" },
                h("span", { className: "socis-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "socis-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "socis-kanban-diag-reassign-row" },
            h("span", { className: "socis-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "socis-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "socis-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "socis-kanban-diag-msg",
              msg.ok ? "socis-kanban-diag-msg--ok" : "socis-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "socis-kanban-section" },
      h("div", { className: "socis-kanban-section-head-row" },
        h("span", { className: "socis-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "socis-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "socis-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "socis-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "socis-kanban-docs-link",
      title: "Open SOCIS Kanban docs in a new tab",
      "aria-label": "SOCIS Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `socis kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "socis-kanban-msg-ok" : "socis-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "socis-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "socis-kanban-boardswitcher" },
      h("div", { className: "socis-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "socis-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "socis-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "socis-kanban-dialog-title" },
        ...
        h("div", { className: "socis-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned SOCIS profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "socis-kanban-bulk" },
      h("span", { className: "socis-kanban-bulk-count" },
      ...
      h("div", { className: "socis-kanban-bulk-priority",
      ...
      h("div", { className: "socis-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different SOCIS profile. Pick a profile (or unassign) and click Apply." },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
      ...
      h("label", { className: "socis-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h(Checkbox, {
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("socis-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("socis-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "socis-kanban-trash",
        dragOver ? "socis-kanban-trash--drop" : "",
        props.draggingTaskId ? "socis-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "socis-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "socis-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    ...
    const isPanBlockedTarget = useCallback(function (target) {
      ...
      if (target.closest && target.closest(".socis-kanban-card")) return true;
      if (target.closest && target.closest(".socis-kanban-column-add")) return true;
      if (target.closest && target.closest(".socis-kanban-col-check")) return true;
    ...
    const stopPan = useCallback(function () {
      ...
      if (el) {
        ...
        el.classList.remove("socis-kanban-columns--panning");
        el.style.userSelect = "";
      }
    ...
    const handleMouseDown = useCallback(function (e) {
      ...
      setIsPanning(true);
      el.classList.add("socis-kanban-columns--panning");
    ...
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".socis-kanban-card");
      if (!card) return;
    ...
    return h("div", {
      ref: columnsRef,
      className: cn(
        "socis-kanban-columns",
        isScrollable ? "socis-kanban-columns--scrollable" : "",
        isPanning ? "socis-kanban-columns--panning" : "",
      ),
      onDragStart: handleDragStart,
      onDragEnd: handleDragEnd,
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("socis-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("socis-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "socis-kanban-column",
        dragOver ? "socis-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "socis-kanban-column-header",
                 title: colHelp || "" },
        h(Checkbox, {
          className: "socis-kanban-col-check",
        ...
        h("span", { className: cn("socis-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "socis-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "socis-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "socis-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "socis-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "socis-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "socis-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "socis-kanban-lane" },
                  h("div", { className: "socis-kanban-lane-head" },
                    h("span", { className: "socis-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "socis-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "socis-kanban-card--stale-red";
    if (age >= tier.amber) return "socis-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".socis-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "socis-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      className: cn(
        "socis-kanban-card",
        props.selected ? "socis-kanban-card--selected" : "",
        props.failed ? "socis-kanban-card--failed" : "",
        props.draggingSource ? "socis-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "socis-kanban-card-content" },
          h("div", { className: "socis-kanban-card-row" },
            h("label", {
              className: "socis-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h(Checkbox, {
                className: "socis-kanban-card-check",
            ...
            h("span", { className: "socis-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or socis kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "socis-kanban-warning-badge",
                    "socis-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "socis-kanban-needs-assignee",
          ...
          h("div", { className: "socis-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "socis-kanban-card-row socis-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "socis-kanban-assignee",
                            title: `Assigned to SOCIS profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "socis-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "socis-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "socis-kanban-ago",
  ...
  // centered modal reusing the socis-kanban-dialog chrome so the form is
  ...
  function InlineCreate(props) {
    ...
    return h("div", {
      ...
      h("form", {
        className: "socis-kanban-dialog socis-kanban-create-dialog",
            ...
            h("div", { className: "flex flex-col gap-1 flex-1" },
              ...
              h(Input, {
                ...
                title: props.columnName === "triage"
                  ? "SOCIS profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
                  : "SOCIS profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "socis-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "socis-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "socis-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "socis-kanban-drawer-close",
        ...
        data ? h("div", { className: "socis-kanban-drawer-comment-foot" },
          h("div", {
            className: "socis-kanban-comment-hint text-xs text-muted-foreground",
          ...
          h("div", { className: "socis-kanban-drawer-comment-row" },
            h(Input, {
              value: newComment,
  ...
  function AttachmentsSection(props) {
    ...
    return h("div", { className: "socis-kanban-section" },
      h("div", { className: "socis-kanban-section-head" },
      ...
      atts.length === 0
        ...
        : atts.map(function (a) {
            return h("div", {
              ...
              h("button", {
                type: "button",
                className: "socis-kanban-attachment-link truncate",
                title: a.filename,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "socis-kanban-drawer-body" },
      h("div", { className: "socis-kanban-drawer-title" },
        h("span", { className: cn("socis-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
          ...
          : h("span", {
              className: "socis-kanban-drawer-title-text",
              title: tx(i18n, "clickToEdit", "Click to edit"),
              onClick: function () { props.setEditing(true); },
            }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "socis-kanban-drawer-meta" },
      ...
      (function () {
        ...
        if (finalResult) {
          ...
          return h("div", { className: "socis-kanban-section" },
            h("div", { className: "socis-kanban-section-head" }, label),
            h(MarkdownBlock, { source: finalResult, enabled: props.renderMarkdown }),
          );
        }
        if (isDone && isParent) {
          return h("div", { className: "socis-kanban-section" },
            h("div", { className: "socis-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "socis-kanban-done-no-result socis-kanban-done-parent-note" },
        ...
        if (isDone) {
          return h("div", { className: "socis-kanban-section" },
            h("div", { className: "socis-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "socis-kanban-done-no-result" },
      ...
      childResults.length > 0 ? h("div", { className: "socis-kanban-section" },
        h("div", { className: "socis-kanban-section-head" },
        ...
        childResults.map(function (child) {
          ...
          return h("div", { key: child.id, className: "socis-kanban-comment" },
            h("div", { className: "socis-kanban-comment-head" },
              h("span", { className: "socis-kanban-comment-author" },
                `${child.id} · ${child.title || tx(i18n, "untitled", "(untitled)")}`),
              h(Badge, { variant: "outline" }, child.status),
              h("button", {
                type: "button",
                className: "socis-kanban-diag-action-btn",
      ...
      h("div", { className: "socis-kanban-section" },
        h("div", { className: "socis-kanban-section-head" },
        ...
        comments.length === 0
          ...
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "socis-kanban-comment" },
                h("div", { className: "socis-kanban-comment-head" },
                  h("span", { className: "socis-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "socis-kanban-comment-ago" },
      ...
      h("div", { className: "socis-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            ...
            className: cn(
              "socis-kanban-event",
              isDiag ? "socis-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "socis-kanban-event-header" },
                  h("span", { className: "socis-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "socis-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "socis-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "socis-kanban-event-header-plain" },
                  h("span", { className: "socis-kanban-event-kind" }, e.kind),
                  h("span", { className: "socis-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "socis-kanban-event-phantom-row" },
                  h("span", { className: "socis-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "socis-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "socis-kanban-event-payload" },
    ...
    return h("div", { className: "socis-kanban-section" },
      h("div", { className: "socis-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "socis-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `socis-kanban-run--${r.outcome || r.status || "ended"}`
          : "socis-kanban-run--active";
        return h("div", { key: r.id, className: cn("socis-kanban-run", outcomeClass) },
          h("div", { className: "socis-kanban-run-head" },
            h("span", { className: "socis-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "socis-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "socis-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "socis-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "socis-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "socis-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "socis-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "socis-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "socis-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "socis-kanban-pre socis-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "socis-kanban-meta-row" },
      h("span", { className: "socis-kanban-meta-label" }, props.label),
      h("span", { className: "socis-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "socis-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "socis-kanban-meta-row" },
        h("span", { className: "socis-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "socis-kanban-meta-value socis-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "socis-kanban-meta-row" },
        h("span", { className: "socis-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "socis-kanban-meta-value socis-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function ModelEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "socis-kanban-meta-row" },
        h("span", { className: "socis-kanban-meta-label" }, tx(t, "model", "Model")),
        h("span", {
          className: cn(
            "socis-kanban-meta-value socis-kanban-editable",
    ...
    return h("div", { className: "socis-kanban-meta-row" },
      h("span", { className: "socis-kanban-meta-label" }, tx(t, "model", "Model")),
      loading
        ? h("span", { className: "socis-kanban-meta-value text-muted-foreground" },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "socis-kanban-section" },
      h("div", { className: "socis-kanban-section-head-row" },
        h("span", { className: "socis-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
            className: "socis-kanban-textarea",
            value: v,
            rows: 8,
            onChange: function (e) { setV(e.target.value); },
          })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "socis-kanban-section" },
      h("div", { className: "socis-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "socis-kanban-deps-row" },
        h("span", { className: "socis-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "socis-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "socis-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "socis-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "socis-kanban-dep-chip-x",
      ...
      h("div", { className: "socis-kanban-deps-row" },
        h("span", { className: "socis-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "socis-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "socis-kanban-actions" },
        specifyButton,
        decomposeButton,
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "socis-kanban-section" },
      ...
      h("div", { className: "socis-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "socis-kanban-home-sub socis-kanban-home-sub--on"
              : "socis-kanban-home-sub",
          }, label);
        })
      )
    );
  ...
  if (window.__SOCIS_AGENT_PLUGINS__ && typeof window.__SOCIS_AGENT_PLUGINS__.register === "function") {
    window.__SOCIS_AGENT_PLUGINS__.register("kanban", KanbanPage);
  }
})();
