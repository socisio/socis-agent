...
Config surface is deliberately env-only (``SOCIS_AGENT_STARTUP_WATCHDOG=0`` to
disable, ``SOCIS_AGENT_STARTUP_WATCHDOG_TIMEOUT_S`` to tune): the watchdog must be
...
ENV_STARTUP_WATCHDOG = "SOCIS_AGENT_STARTUP_WATCHDOG"
ENV_STARTUP_WATCHDOG_TIMEOUT_S = "SOCIS_AGENT_STARTUP_WATCHDOG_TIMEOUT_S"
...
# Module-level singleton: the arm sites (socis_cli.main / socis_cli.gateway
# / gateway.run.main / cli.py --gateway / scripts/socis-gateway) and the
...
def _process_socis_agent_home() -> Path:
    """SOCIS_AGENT_HOME for process-level diagnostic files.

    Stdlib-only replica of ``socis_agent_constants``' platform default — this
    module must not import application code (see module docstring). Hosted
    images always set ``SOCIS_AGENT_HOME`` explicitly.
    """
...
def get_startup_watchdog_dump_path(home: Optional[Path] = None) -> Path:
    """Return ``<SOCIS_AGENT_HOME>/logs/gateway-startup-watchdog.log``."""
    base = home if home is not None else _process_socis_agent_home()
    return base.joinpath(*_DUMP_RELATIVE)


def startup_watchdog_disabled() -> bool:
    """True when ``SOCIS_AGENT_STARTUP_WATCHDOG`` opts out explicitly."""
...
def arm_startup_watchdog(
    ...
    *,
    ...
    ``SOCIS_AGENT_STARTUP_WATCHDOG=0`` or when the thread could not be started.
    """
    global _handle
    try:
