...
class SessionSearchMixin:
    ...
    def search_messages(
        ...
        Threshold: SOCIS_AGENT_SEARCH_SLOW_MS (default 1000; 0 logs every call).
        """
        started = time.time()
        rows = None
        try:
            ...
            try:
                threshold = float(os.getenv("SOCIS_AGENT_SEARCH_SLOW_MS", "1000"))
    ...
    def search_sessions_by_id(
        ...
        a session id from logs, CLI output, or another SOCIS surface and jump
    ...
    def optimize_fts(self) -> int:
        ...
        disabled via ``SOCIS_AGENT_DISABLE_FTS_TRIGRAM`` or not yet created), so
