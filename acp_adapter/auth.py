"""ACP auth helpers — detect and advertise SOCIS authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "socis-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active SOCIS runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if SOCIS can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for SOCIS.
    ...
    not have SOCIS provider credentials configured yet, so SOCIS always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate SOCIS using the currently configured "
    ...
    methods.append(
        TerminalAuthMethod(
            ...
            name="Configure SOCIS provider",
            description=(
                "Open SOCIS' interactive model/provider setup in a terminal. "
                "Use this when SOCIS has not been configured on this machine yet."
            ),
            type="terminal",
            args=["--setup"],
        )
