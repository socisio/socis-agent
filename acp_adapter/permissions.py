"""ACP permission bridging for SOCIS dangerous-command approvals."""
...
# Maps ACP permission option ids to SOCIS approval result strings.
...
def _build_permission_options(
    *, allow_permanent: bool, allow_session: bool = True,
    ...
    """Return ACP options that match SOCIS approval semantics."""
    ...
    # Smart DENY override — the editor must not offer a scope SOCIS
    ...
    if not once_only:
        options.append(PermissionOption(
            ...
            # hint while keeping SOCIS semantics in the option id.
            kind="allow_always",
...
def _map_outcome_to_socis(outcome: object, *, allowed_option_ids: set[str]) -> str:
    """Map an ACP permission outcome into SOCIS approval strings."""
    ...
    if option_id not in allowed_option_ids:
        ...
        return "deny"
    return _OPTION_ID_TO_SOCIS.get(option_id, "deny")
...
def make_approval_callback(
    ...
    """
    Return a SOCIS-compatible approval callback that bridges to ACP.
