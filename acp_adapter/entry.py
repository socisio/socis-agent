"""CLI entry point for the socis-agent ACP adapter.

Loads environment variables from ``~/.socis-agent/.env``, configures logging
...
try:
    ...
    # shadowing SOCIS's own modules — ``socis acp`` can be started from any
...
def _load_env() -> None:
    """Load .env from SOCIS_AGENT_HOME (default ``~/.socis-agent``)."""
    from socis_cli.env_loader import load_socis_dotenv

    socis_agent_home = get_socis_agent_home()
    loaded = load_socis_dotenv(socis_agent_home=socis_agent_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", socis_agent_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="socis-acp",
        description="Run SOCIS Agent as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print SOCIS version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive SOCIS provider/model setup for ACP terminal auth",
    )
    parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.socis-agent/node/ "
...
def _run_check() -> None:
    ...
    from acp_adapter.server import SOCISACPAgent  # noqa: F401

    print("SOCIS ACP check OK")


def _run_setup() -> None:
    from socis_cli.main import main as socis_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "socis", "model"]
        socis_main()
    finally:
        sys.argv = old_argv
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting socis-agent ACP adapter")
    ...
    import acp
    from .server import SOCISACPAgent
    ...
    if os.environ.get("SOCIS_AGENT_ACP_SKIP_CONFIGURED_MCP", "").strip() != "1":
        try:
