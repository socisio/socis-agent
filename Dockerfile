...
# node:26 image instead (SOCIS pins its toolchain to Node 26 everywhere).
...
# Do not write .pyc files at runtime: /opt/socis is immutable in the
...
ENV PLAYWRIGHT_BROWSERS_PATH=/opt/socis/.playwright

# Install system dependencies in one layer, clear APT cache.
# tini was previously PID 1 to reap orphaned zombie processes (MCP stdio
# subprocesses, git, bun, etc.) that would otherwise accumulate when socis
# ran as PID 1. See #15012. Phase 2 of the s6-overlay supervision plan
# replaces tini with s6-overlay's /init (PID 1 = s6-svscan), which reaps
# zombies non-blockingly on SIGCHLD and additionally supervises the main
# socis process, the dashboard, and per-profile gateways.
RUN apt-get -o Acquire::Retries=3 update && \
    apt-get -o Acquire::Retries=3 install -y --no-install-recommends \
...
db.execute(\"INSERT INTO docs VALUES ('socis')\"); \
...
# ---------- s6-overlay install ----------
# s6-overlay provides supervision for the main socis process, the dashboard,
...
# 'SOCIS WebUI' catalog, NAS compose projects that preserve an old
...
# Non-root user for runtime; UID can be overridden via SOCIS_AGENT_UID at runtime
RUN useradd -u 10000 -m -d /opt/data socis
...
#
# ui-tui/packages/socis-ink/ is copied IN FULL (not just its manifests)
...
COPY ui-tui/packages/socis-ink/ ui-tui/packages/socis-ink/
# apps/shared/ is copied IN FULL because web/package.json references it as a
# `file:` workspace dependency (same pattern as socis-ink above).
...
# lock on the @socis/ink entry, tripped the TUI launcher's
...
#
# The [otlp] extra contains the SDK/exporter imported by SOCIS when Gateway
# Health export is enabled. Collector and observability-backend dependencies
# remain external and are not part of the SOCIS production image.
#
# The hindsight memory provider's client (hindsight-client) is baked in
# for the same reason: it lazy-installs into /opt/socis/.venv at first
...
# gives the non-root socis user read + traverse but no write; root retains
# write so the build steps below don't need chmod u+w dances.
COPY --link --chmod=a+rX,go-w . .

# ---------- Permissions ----------
# Link socis-agent itself (editable). Deps are already installed in the
# cached layer above; `--no-deps` makes this a fast egg-link creation with no
# resolution or downloads.
RUN uv pip install --no-cache-dir --no-deps -e "."

# Wire the exec shim and install-method stamp.  Files under /opt/socis are
# already root-owned (COPY, uv sync, npm install all run as root) and
# read-only for the socis user (go-w from the --chmod above).

USER root
RUN mkdir -p /opt/socis/bin && \
    cp /opt/socis/docker/socis-exec-shim.sh /opt/socis/bin/socis && \
    chmod 0755 /opt/socis /opt/socis/bin/socis && \
    printf 'docker\n' > /opt/socis/.install_method
# The ``.install_method`` stamp is baked next to the running code (the install
# tree), NOT into $SOCIS_AGENT_HOME. $SOCIS_AGENT_HOME (/opt/data) is a shared data
# volume that is commonly bind-mounted from the host and even shared with a
# host-side Desktop/CLI install; stamping it at boot used to clobber that
# host install's marker and wrongly block its ``socis update``. A code-scoped
# stamp is read first by detect_install_method() and is immune to the share.
# Start as root so the s6-overlay stage2 hook can usermod/groupmod and chown
# the data volume. Each supervised service then drops to the socis user via
# `s6-setuidgid socis` in its run script. If SOCIS_AGENT_UID is unset, services
# run as the default socis user (UID 10000).
...
# lives outside both /opt/socis (which operators sometimes bind-mount as a
# checkout) and /opt/data (the mutable SOCIS_AGENT_HOME volume).
# .dockerignore excludes .git, so `git rev-parse HEAD` from inside the
# container always returns nothing — meaning `socis dump` reports
# "(unknown)" and the startup banner drops its `· upstream <sha>` suffix.
# That makes support triage from container bug reports impossible:
# we can't tell which commit the user is actually running.
#
# Fix: write the commit SHA passed via the SOCIS_AGENT_GIT_SHA build-arg to
# /opt/socis/.socis_build_sha at build time, and have
# socis_cli/build_info.py read it at runtime.  Both `socis dump` and
...
ARG SOCIS_AGENT_GIT_SHA=
RUN set -eu; \
    if [ -n "${SOCIS_AGENT_GIT_SHA}" ]; then \
        printf '%s\n' "${SOCIS_AGENT_GIT_SHA}" > /opt/socis/.socis_build_sha; \
    fi; \
    mkdir -p /etc/socis; \
    SOCIS_AGENT_GIT_SHA="${SOCIS_AGENT_GIT_SHA}" python3 -c 'import json, os, pathlib, tomllib; project = tomllib.loads(pathlib.Path("/opt/socis/pyproject.toml").read_text(encoding="utf-8"))["project"]; marker = pathlib.Path("/etc/socis/image-provenance.json"); marker.write_text(json.dumps({"schema": 1, "deployment_kind": "image", "manager": "docker", "image": "socis/socis-agent", "version": project["version"], "revision": os.environ.get("SOCIS_AGENT_GIT_SHA") or None}, sort_keys=True, separators=(",", ":")) + "\n", encoding="utf-8"); marker.chmod(0o444)'

# ---------- s6-overlay service wiring ----------
# Static services declared at build time: main-socis + dashboard.
...
# `exec socis`. Wired in as cont-init.d/01- so it
# runs before user services start.
#
# 02-reconcile-profiles re-creates per-profile gateway s6 service
# slots from $SOCIS_AGENT_HOME/profiles/<name>/ after a container restart
# (the /run/service/ scandir is tmpfs and wiped on restart). Phase 4.
RUN mkdir -p /etc/cont-init.d && \
    printf '#!/command/with-contenv sh\nexec /opt/socis/docker/stage2-hook.sh\n' \
        > /etc/cont-init.d/01-socis-setup && \
    chmod +x /etc/cont-init.d/01-socis-setup
COPY --chmod=0755 docker/cont-init.d/015-supervise-perms /etc/cont-init.d/015-supervise-perms
COPY --chmod=0755 docker/cont-init.d/02-reconcile-profiles /etc/cont-init.d/02-reconcile-profiles

# ---------- Runtime ----------
ENV SOCIS_AGENT_WEB_DIST=/opt/socis/socis_cli/web_dist
# Point the TUI launcher at the prebuilt bundle baked at build time (Layer 8:
# `ui-tui && npm run build`). This makes _make_tui_argv take the prebuilt-bundle
# fast path (`node --expose-gc /opt/socis/ui-tui/dist/entry.js`) and skip the
...
ENV SOCIS_AGENT_TUI_DIR=/opt/socis/ui-tui
ENV SOCIS_AGENT_HOME=/opt/data
ENV SOCIS_AGENT_WRITE_SAFE_ROOT=/opt/data
ENV SOCIS_AGENT_DISABLE_LAZY_INSTALLS=1
# The published image seals /opt/socis (root-owned, read-only) so a runtime
...
# is seeded + chowned to the socis user by docker/stage2-hook.sh and lives
...
ENV SOCIS_AGENT_LAZY_INSTALL_TARGET=/opt/data/lazy-packages

# `docker exec` privilege-drop shim. When operators run
# `docker exec <c> socis ...` they default to root, and any file the
# command writes under $SOCIS_AGENT_HOME (auth.json, .env, config.yaml) ends
# up root-owned and unreadable to the supervised gateway (UID 10000).
# The shim lives at /opt/socis/bin/socis, sits earliest on PATH, and
# transparently re-exec's the real venv binary via `s6-setuidgid socis`
# when invoked as root. Non-root callers (supervised processes,
# `--user socis`, etc.) hit the short-circuit path with no overhead.
# Recursion is impossible because the shim exec's the venv binary by
# absolute path (/opt/socis/.venv/bin/socis). See the shim source for
# the opt-out env var (SOCIS_AGENT_DOCKER_EXEC_AS_ROOT=1).
COPY --chmod=0755 docker/socis-exec-shim.sh /opt/socis/bin/socis
COPY --chmod=0755 docker/entrypoint-dispatch.sh /opt/socis/docker/entrypoint-dispatch.sh

# Pre-s6 entrypoint.sh did `source .venv/bin/activate` which exported
# the venv bin onto PATH; Architecture B's main-wrapper.sh does the
# same for the container's main process, but `docker exec` and our
# cont-init.d scripts don't pass through the wrapper. Expose the venv
# bin globally so `docker exec <container> socis ...` and any
# subprocess that doesn't activate the venv first still find socis.
#
# /opt/socis/bin is prepended ahead of the venv so the privilege-drop
# shim wins PATH resolution. The shim's last act is to exec the venv
# binary by absolute path, so this PATH ordering is transparent to
# every other consumer.
ENV PATH="/opt/socis/bin:/opt/socis/.venv/bin:/opt/data/.local/bin:${PATH}"
...
#
# main-wrapper.sh handles arg routing (bare-exec vs. socis
# subcommand vs. no-args), drops to the socis user via s6-setuidgid,
...
ENTRYPOINT [ "/opt/socis/docker/entrypoint-dispatch.sh" ]
CMD [ ]
