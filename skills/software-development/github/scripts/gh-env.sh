#!/usr/bin/env bash
# GitHub environment detection helper for SOCIS Agent skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _socis_env="${SOCIS_AGENT_HOME:-$HOME/.socis-agent}/.env"; [ -f "$_socis_env" ] && grep -q "^GITHUB_TOKEN=" "$_socis_env" 2>/dev/null; then
