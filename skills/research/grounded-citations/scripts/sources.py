...
Ledger path resolution (first wins):
  --ledger PATH
  $SOCIS_AGENT_CITATION_LEDGER
  $SOCIS_AGENT_HOME/cache/citations/ledger.json
"""
...
from _socis_agent_home import get_socis_agent_home  # noqa: E402
...
def resolve_ledger_path(explicit: str | None = None) -> Path:
    ...
    env = os.environ.get("SOCIS_AGENT_CITATION_LEDGER", "").strip()
    if env:
        return Path(env).expanduser()
    return get_socis_agent_home() / "cache" / "citations" / "ledger.json"
