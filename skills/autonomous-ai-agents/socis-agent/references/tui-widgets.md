...
Author widget apps for the SOCIS TUI (`socis --tui`): glanceable ambient
...
- The TUI must be in use (`socis --tui`). Widgets do not render in the
...
1. Use `write_file` to create `~/.socis-agent/tui-widgets/<name>.mjs` (see
...
- A thrown `register()` is logged and skipped; check
  `~/.socis-agent/logs/tui_gateway_crash.log` if a widget never appears.
