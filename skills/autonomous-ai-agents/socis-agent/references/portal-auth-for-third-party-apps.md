...
## Layer 1 — Is this thing a SOCIS plugin, or a separate app?
...
|---|---|---|
| **OpenViking memory plugin** (`plugins/memory/openviking/`) | Code that runs **inside the SOCIS process**. Its LLM calls go through SOCIS's already-configured provider. | Already uses Portal if user's SOCIS is configured for Portal. Nothing extra needed. `OPENVIKING_API_KEY` is the OpenViking *server's* own auth, not LLM auth. |
...
plugin that already runs inside SOCIS. That LLM call is already authenticated
via SOCIS's provider config. The plugin's own server auth (e.g.
...
What SOCIS Agent has that *feels* like OAuth — `socis login --provider nous`
opening a browser, user signs in, token lands in `~/.socis-agent/auth.json` — is a
**SOCIS-specific browser flow**. Under the hood it produces a credential
SOCIS uses as a bearer. It is not a public OAuth provider that Karakeep et al.
...
1. Read SOCIS's existing Portal credential out of `~/.socis-agent/auth.json`.
...
rides on the credential SOCIS already holds.

Where this could live in SOCIS:
...
- **Per-key rate limits** so a noisy app can be capped without eating the
  user's headroom for SOCIS itself.
...
1. First decide: SOCIS plugin (runs inside SOCIS) or separate app? If plugin,
   it already uses Portal via SOCIS's provider config — done.
...
4. Mixed setup ("Portal for some things, OpenRouter/Ollama Cloud for the
   SOCIS agent itself") is fully supported. SOCIS treats agent
...
pitch in the Tool Gateway announcement is specifically about SOCIS Agent's
*tools* (web search, browser, image gen, TTS) flowing through the Portal
subscription when SOCIS is configured to use Portal as its provider. It is
