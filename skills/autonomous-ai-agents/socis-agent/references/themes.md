# Themes / Skins — Author a SOCIS Color Theme

Author a SOCIS **skin** — one YAML file that themes the CLI, the TUI, and the
desktop GUI at once. The skin engine (`socis_cli/skin_engine.py`) resolves the
active skin and the gateway pushes it to every surface, so a file dropped in
`~/.socis-agent/skins/` is the theme analogue of a plugin: no code, all surfaces. This
...
- The user asks for a custom look ("make me a synthwave theme", "dark forest
  vibes", "match my brand colors") for SOCIS itself.
...
- Write access to the SOCIS home dir — `~/.socis-agent` by default, or `$SOCIS_AGENT_HOME`
  / the active profile's dir. Skins live in `<socis-home>/skins/`.
- Native tools: `write_file` (create the YAML), `read_file` / `search_files`
  (inspect existing skins), `terminal` (activate via `socis config set`).
...
3. `write_file` it to `<socis-home>/skins/<name>.yaml`.
...
4. **Confirm the new look landed** and tell the user how to revert: run
   `socis config set display.skin default` (or they can `/skin default`).
...
```
socis skin set <key> <hex>      # e.g. socis skin set ui_tool "#00FFFF"
```
...
palette and resets the background. `socis skin set background "#08201f"` changes
only the background; `socis skin use <name>` / `socis skin list` switch and
enumerate.

## Pitfalls

- **Don't hardcode `~/.socis-agent`** when a profile is active — resolve the real home
  from `$SOCIS_AGENT_HOME` first, falling back to `~/.socis-agent`.
...
- **Never hand-edit `config.yaml` to activate.** Use `socis config set
...
- **You apply it, not the user.** `socis config set display.skin <name>` is
...
- `read_file` the written `<socis-home>/skins/<name>.yaml` and confirm valid
  YAML with the intended `name` and `colors`.
- Run `socis config get display.skin` and confirm it reports `<name>`.
