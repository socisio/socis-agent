...
Write plugins for the SOCIS desktop app: statusbar items, layout panes,
...
system (`~/.socis-agent/plugins/`) is otherwise documented separately.

There are TWO on-disk doors, same contract and hot reload:

- `$SOCIS_AGENT_HOME/desktop-plugins/<id>/plugin.js` — standalone desktop plugin.
  Loads enabled by default.
- `$SOCIS_AGENT_HOME/plugins/<id>/desktop/plugin.js` — the desktop HALF of a
...
- The SOCIS desktop app (it loads plugins; the CLI/gateway alone does not).
- Write access to `$SOCIS_AGENT_HOME/desktop-plugins/` (usually
  `~/.socis-agent/desktop-plugins/`).

## How to Run

1. Create `$SOCIS_AGENT_HOME/desktop-plugins/<name>/plugin.js` from
   `templates/plugin.js` (in this skill directory) — that's
   `~/.socis-agent/...` by default, or `~/.socis-agent/profiles/<profile>/...` under a
...
The ONLY import surface is `@socis/plugin-sdk` (plus `react` /
...
- `ctx.os` — the curated OS door, attributed to your plugin:
  ...
  posts a native OS notification. Fires only while the user is away from SOCIS
  ...
  (`socis://index-network/intent/1`), a hash path (`/index-network/intent/1`),
...
- Backend: if the plugin ships a Python `plugin_api.py` (under
  `~/.socis-agent/plugins/<id>/dashboard/`, manifest `"api": "plugin_api.py"`), reach
...
- Do not import anything except `@socis/plugin-sdk`, `react`, and
