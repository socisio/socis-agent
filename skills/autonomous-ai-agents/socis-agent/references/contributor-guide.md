...
For occasional contributors and PR authors. Full developer docs: https://agent.socis.io/docs/developer-guide/
...
Config: `~/.socis-agent/config.yaml` (settings), `~/.socis-agent/.env` (API keys) — both under `$SOCIS_AGENT_HOME` when it is set.
...
`_SOCIS_AGENT_CORE_TOOLS` (every platform) or to a specific toolset.

All handlers must return JSON strings. Use `get_socis_agent_home()` for paths,
never hardcode `~/.socis-agent`. For custom/local-only tools, write a plugin in
`~/.socis-agent/plugins/` instead of editing core — see the developer docs.

### Adding a Slash Command

1. Add `CommandDef` to `COMMAND_REGISTRY` in `socis_cli/commands.py`
...
- Tests auto-redirect `SOCIS_AGENT_HOME` to temp dirs — never touch real `~/.socis-agent/`.
...
- POSIX file modes (0o600, etc.) → `@pytest.mark.skipif(sys.platform.startswith("win"), reason="POSIX mode bits not enforced on Windows")` (see `tests/socis_cli/test_auth_toctou_file_modes.py`)
...
- Use `get_socis_agent_home()` from `socis_agent_constants` for all paths (profile-safe)
