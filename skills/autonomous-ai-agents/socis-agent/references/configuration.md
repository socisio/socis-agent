...
Edit with `socis config edit` or `socis config set section.key value`.
Full reference: https://agent.socis.io/docs/user-guide/configuration
...
`socis config check` reports sections missing from an older config.

### Toolsets

Enable/disable via `socis tools` (interactive) or `socis tools enable/disable NAME`.
Full enumeration: `TOOLSETS` dict in `toolsets.py` (`_SOCIS_AGENT_CORE_TOOLS` is the default bundle most platforms inherit).
