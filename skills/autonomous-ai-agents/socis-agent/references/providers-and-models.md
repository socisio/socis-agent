...
Set via `socis model` (picker) or `socis setup`. 35+ provider profiles ship as
plugins under `plugins/model-providers/`; user plugins of the same name override.
Full docs: https://agent.socis.io/docs/integrations/providers
...
| nous | OAuth device code | `socis auth add nous` (or `NOUS_API_KEY`) |
| openai-codex | OAuth | `socis auth add openai-codex` |
| qwen-oauth | OAuth | `socis auth add qwen-oauth` |
| minimax-oauth | OAuth | `socis auth add minimax-oauth` |
...
Multiple credentials per provider pool and rotate automatically (`socis auth`).
Fallback chain when the primary fails: `socis fallback add|remove|list`.

### User-defined model aliases

Work with `/model <name>` in CLI and every gateway platform. Resolved by
`socis_cli/model_switch.py::resolve_alias()`; user aliases are checked BEFORE
...
#   socis config set model.aliases.fav openrouter/anthropic/claude-sonnet-4.6
model:
  aliases:
