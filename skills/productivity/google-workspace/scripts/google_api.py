#!/usr/bin/env python3
"""Google Workspace API CLI for SOCIS Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing SOCIS-facing JSON contract and falls back to the Python client
...
from _socis_agent_home import get_socis_agent_home
...
def _gws_binary() -> str | None:
    override = os.getenv("SOCIS_AGENT_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for SOCIS Agent")
