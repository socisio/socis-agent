# Contributing to SOCIS Agent

Thank you for contributing to SOCIS Agent! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — macOS, different Linux distros, and WSL2 on Windows. We want SOCIS to work everywhere.
...
Related: #38284 covers the agent-side analog — SOCIS itself checking existing issues and PRs before deep self-troubleshooting. This section is the human-contributor complement.
...
Bundled skills (in `skills/`) ship with every SOCIS install. They should be **broadly useful to most users**:
...
**We are no longer accepting new memory providers into this repo.** The set of built-in providers under `plugins/memory/` (honcho, mem0, supermemory, byterover, hindsight, holographic, openviking, retaindb) is closed. If you want to add a new memory backend, publish it as a **standalone plugin repo** that users install into `~/.socis-agent/plugins/` (or via a pip entry point).

Standalone memory plugins:

- Implement the same `MemoryProvider` ABC (`agent/memory_provider.py`) — `sync_turn`, `prefetch`, `shutdown`, and optionally `post_setup(socis_agent_home, config)` for setup-wizard integration
...
- Integrate with `socis memory setup` via `post_setup()` — no need to touch core code
...
The installer creates the SOCIS venv, wires the `socis` command, stamps the
install method for `socis update`, and clones the full git project into
`$SOCIS_AGENT_HOME/socis-agent` (usually `~/.socis-agent/socis-agent`). That keeps your
...
Use this only if you intentionally do not want SOCIS' managed install layout
(for example, a throwaway clone inside a container or CI job). If you install
this way, make sure you run the `socis` entrypoint from this venv; running the
system `python3 -m socis_cli.main` can pick up unrelated system Python
packages.
...
```bash
mkdir -p ~/.socis-agent/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.socis-agent/config.yaml
touch ~/.socis-agent/.env

# Add at minimum an LLM provider key:
echo "OPENROUTER_API_KEY=***" >> ~/.socis-agent/.env
```

### Run

```bash
# The standard installer already put `socis` on PATH.
socis doctor
socis chat -q "Hello"
```

If you used the manual clone fallback, run `./socis` from the checkout or
symlink this clone's venv explicitly:

```bash
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/socis" ~/.local/bin/socis
```
...
├── cli.py                    # SOCISCLI class — interactive TUI, prompt_toolkit integration
├── model_tools.py            # Tool orchestration (thin layer over tools/registry.py)
├── toolsets.py               # Tool groupings and presets (socis-cli, socis-telegram, etc.)
├── socis_agent_state.py           # SQLite session database with FTS5 full-text search, session titles
...
│
├── socis_cli/               # CLI command implementations
...
│
├── skills/                   # Bundled skills (copied to ~/.socis-agent/skills/ on install)
├── optional-skills/          # Official optional skills (discoverable via hub, not activated by default)
├── tests/                    # Test suite
├── website/                  # Documentation site (agent.socis.io)
│
├── cli-config.yaml.example   # Example configuration (copied to ~/.socis-agent/config.yaml)
└── AGENTS.md                 # Development guide for AI coding assistants
```

### User configuration (stored in `~/.socis-agent/`)

| Path | Purpose |
|------|---------|
| `~/.socis-agent/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.socis-agent/.env` | API keys and secrets |
| `~/.socis-agent/auth.json` | OAuth credentials (Nous Portal) |
| `~/.socis-agent/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.socis-agent/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.socis-agent/state.db` | SQLite session database |
| `~/.socis-agent/sessions/` | Gateway routing index (`sessions.json`), request-dump breadcrumbs, gateway `*.jsonl` transcripts, and (optionally) per-session JSON snapshots when `sessions.write_json_snapshots: true` is set. The per-session snapshots are off by default; state.db is canonical. |
| `~/.socis-agent/cron/` | Scheduled job data |
| `~/.socis-agent/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`socis_agent_state.py`) with full-text search and unique session titles. Per-session JSON snapshots in `~/.socis-agent/sessions/` were superseded by the SQLite store and are off by default; opt back in with `sessions.write_json_snapshots: true` if you have external tooling that consumes the JSON files directly.
...
(for example `_SOCIS_AGENT_CORE_TOOLS` or a dedicated toolset); otherwise the tool
...
Four fields are supported under `metadata.socis`:
...
The user may skip setup and keep loading the skill. SOCIS only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `socis setup` or update `~/.socis-agent/.env` locally.
...
2. **Tools referenced in SKILL.md prose must be native SOCIS tools or MCP servers the skill explicitly expects.** When the skill needs a capability, point at the proper tool by name in backticks: `` `terminal` ``, `` `web_extract` ``, `` `web_search` ``, `` `read_file` ``, `` `write_file` ``, `` `patch` ``, `` `search_files` ``, `` `vision_analyze` ``, `` `browser_navigate` ``, `` `delegate_task` ``, `` `image_generate` ``, `` `text_to_speech` ``, `` `cronjob` ``, `` `memory` ``, `` `skill_view` ``, `` `todo` ``, `` `execute_code` ``.
...
4. **`author` credits the human contributor first.** For external contributions, the contributor's real name + GitHub handle goes first (`Jane Doe (jane-doe)`); "SOCIS Agent" is the secondary collaborator. If the contributor's commit shows "SOCIS Agent" as author because they used SOCIS to draft the skill, replace it with their actual name — credit the human, not the tool.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing SOCIS tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `socis --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

SOCIS uses a data-driven skin system — no code changes needed to add a new skin.

**Option A: User skin (YAML file)**

Create `~/.socis-agent/skins/<name>.yaml`:
...
Add to `_BUILTIN_SKINS` dict in `socis_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `socis_cli/skin_engine.py` for the full schema and existing skins as examples.

---

## Cross-Platform Compatibility

SOCIS runs on Linux, macOS, and native Windows (plus WSL2). When writing code
...
1. **Never call `os.kill(pid, 0)` for liveness checks.** `os.kill(pid, 0)`
   ...
   **Preferred:** use `psutil` (a core dependency — always available):
   ...
   If you specifically need the socis wrapper (it has a stdlib fallback
...
2. **Use `shutil.which()` before shelling out — don't assume Windows has
   ...
   `socis_cli/gateway.py::_scan_gateway_pids` for the pattern.
   ```
...
9. **Detached background daemons on Windows need `pythonw.exe`, NOT
    ...
    See `socis_cli/gateway_windows.py::_spawn_detached` for the reference
    implementation.
...
15. **Two different quoting schemes in one command line.** `subprocess.run
    ...
    helpers and never cross them. See `socis_cli/gateway_windows.py::
...
SOCIS has terminal access. Security matters.
...
2. **Test manually**: Run `socis` and exercise the code path you changed
...
- Use [GitHub Issues](https://github.com/socisio/socis-agent/issues)
- Include: OS, Python version, SOCIS version (`socis --version`), full error traceback
