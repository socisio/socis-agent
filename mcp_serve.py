"""
SOCIS MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (SOCIS-specific extra)

Usage:
    socis mcp serve
    socis mcp serve --verbose
...
logger = logging.getLogger("socis.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using SOCIS_AGENT_HOME."""
    try:
        from socis_agent_constants import get_socis_agent_home
        return get_socis_agent_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("SOCIS_AGENT_HOME", Path.home() / ".socis-agent")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from socis_agent_constants import get_socis_agent_home
        directory_file = get_socis_agent_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("SOCIS_AGENT_HOME", Path.home() / ".socis-agent")
...
class EventBridge:
    ...
    This is the SOCIS equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _establish_baseline_with_db(self, db) -> None:
        ...
        try:
            from socis_agent_constants import get_socis_agent_home
            db_file = get_socis_agent_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("SOCIS_AGENT_HOME", Path.home() / ".socis-agent")) / "state.db"
        try:
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "MCPServer":
    """Create and return the SOCIS MCP server with all tools registered."""
    ...
    mcp = MCPServer(
        "socis",
        instructions=(
            "SOCIS Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the SOCIS MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
