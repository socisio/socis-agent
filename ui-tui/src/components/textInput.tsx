import type { InputEvent, Key } from '@socis/ink'
import * as Ink from '@socis/ink'
 ...
 * but that accepts `key.meta` on macOS — and socis-ink reports Option as
...
export function supportsFastEchoTerminal(env: NodeJS.ProcessEnv = process.env): boolean {
  ...
  if (isTermuxTuiMode(env)) {
    const override = String(env.SOCIS_AGENT_TUI_TERMUX_FAST_ECHO ?? '')
      .trim()
      .toLowerCase()
  ...
  useInput(
    (inp: string, k: Key, event: InputEvent) => {
      ...
      if (actionHome) {
        ...
        // ESC d, which socis-ink decodes as meta+'d'; without this branch it
