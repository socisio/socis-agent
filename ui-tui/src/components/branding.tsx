import { Box, Text, useStdout } from '@socis/ink'
...
const TAG_FULL = 'SOCIS · The Self-Improving AI Agent'
const TAG_MID = 'The Self-Improving AI Agent'
const TAG_TINY = 'SOCIS'
const HIDE_BELOW = 34
...
export function SessionPanel({ info, maxWidth, sid, t }: SessionPanelProps) {
  ...
  const leftW = Math.min((artWidth(heroLines) || HERO_MARK_WIDTH) + 4, Math.floor(cols * 0.4))
  ...
  // socis_cli/banner.py) and the "connected" label on the collapse toggle.
  ...
  const heroColumn = wide ? (
    <Box flexDirection="column" width="100%">
      ...
      <Text color={t.color.accent}>
        ...
        <Text color={t.color.muted}> · SOCIS</Text>
      </Text>
    ...
    <Box flexDirection="column" width="100%">
      ...
      {typeof info.update_behind === 'number' && info.update_behind > 0 && (
        <Text bold color={t.color.warn}>
          ...
          <Text bold color={t.color.warn}>
            {info.update_command || 'socis update'}
          </Text>
          <Text bold={false} color={t.color.warn} dimColor>
            {' '}
            to update
