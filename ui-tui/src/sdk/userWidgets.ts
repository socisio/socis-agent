...
import { Box, Text } from '@socis/ink'
...
/**
 * User widget apps — SOCIS authors its own TUI widgets, mirroring the
 * Python plugin contract: drop `<name>.mjs` into `$SOCIS_AGENT_HOME/tui-widgets/`,
 ...
 * React.createElement.
 *
 * Trust model matches `~/.socis-agent/plugins/`: files under SOCIS_AGENT_HOME execute
...
const widgetsDir = () => join(process.env.SOCIS_AGENT_HOME?.trim() || join(homedir(), '.socis-agent'), 'tui-widgets')
 ...
 *  every change, so a widget SOCIS writes appears within ~a second — no
...
export function watchUserWidgets(dir = widgetsDir()): void {
  ...
  if (!attach()) {
    ...
    // user (or SOCIS) ever writes must hot-load too — a 10s poll here read
