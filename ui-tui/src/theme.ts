import type { SkinBranding, SkinColors } from '@socis/shared/skin'
...
// Mirrors @socis/ink's colorize.ts. Keep local: app code compiles from
// ui-tui/src, while @socis/ink is bundled separately from packages/.
...
export const DARK_SEEDS: ThemeSeeds = {
  accent: '#FF3366',
  // The SOCIS navy surfaces are IDENTITY, not derivation drift — keep them
  // as explicit fill seeds (the ladder derives them for skins that don't
  // care). Sourced from the SOCIS brand mark: bg #070A0F (canvas),
  // border #161F30 (divider lines), accent #FF3366 ("Kinetic Coral").
  activeRow: '#2A1420',
  bg: '#070A0F',
  border: '#161F30',
  error: '#ef5350',
...
// Light-terminal seeds: darker corals that stay legible on white, computed
// via liftForContrast(dark, '#ffffff', 4.5) — the same derivation the
// original gold theme used (hue/saturation kept, luminance lifted until
// 4.5 contrast is cleared). Recompute these if DARK_SEEDS.accent/text/
// primary ever change; they are not independently authored.
 ...
 *  answer when known (cached in SOCIS_AGENT_TUI_BACKGROUND), else the mode's
 *  assumed pole. */
function referenceBackground(isLight: boolean, env: NodeJS.ProcessEnv = process.env): string {
  const cached = (env.SOCIS_AGENT_TUI_BACKGROUND ?? '').trim()
...
// Terminal. Explicit SOCIS_AGENT_TUI_THEME / COLORFGBG signals above still win,
...
// `SOCIS_AGENT_TUI_BACKGROUND` is intentionally generic so a future OSC11
...
//
//   1. `SOCIS_AGENT_TUI_LIGHT` boolean — `1`/`true`/`yes`/`on` → light;
//      `0`/`false`/`no`/`off` → dark.  Either explicit value wins
//      regardless of any later signal.
//   2. `SOCIS_AGENT_TUI_THEME` named override — `light` / `dark` win over
//      every signal below.
//   3. `SOCIS_AGENT_TUI_BACKGROUND` hex hint (3- or 6-digit) — luminance
...
//
// Anything we can't decide stays dark — the default SOCIS palette
// is the dark one.
export function detectLightMode(
  ...
  const lightFlag = (env.SOCIS_AGENT_TUI_LIGHT ?? '').trim().toLowerCase()
  ...
  const themeFlag = (env.SOCIS_AGENT_TUI_THEME ?? '').trim().toLowerCase()
  ...
  const bgHint = backgroundLuminance(env.SOCIS_AGENT_TUI_BACKGROUND ?? '')
 ...
 * the OSC-11 background answer cached into SOCIS_AGENT_TUI_BACKGROUND after
...
export function fromSkin(
  ...
  // itself into SOCIS_AGENT_TUI_BACKGROUND. See #applySkin / syncThemeToTerminalBackground).
  ...
  //    base seeds for this polarity. The base's IDENTITY FILLS (SOCIS navy
  ...
  // the derived value already carries the identity seeds (e.g. SOCIS navy).
  const activeRow =
