...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.SOCIS_AGENT_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep SOCIS on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via SOCIS_AGENT_TUI_TRUECOLOR=1.
