...
const logDir = join(process.env.SOCIS_AGENT_HOME?.trim() || join(homedir(), '.socis-agent'), 'logs')
...
// a real ~/.socis-agent (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('socis-tui: parent lifecycle log unavailable\n')
    }
  }
}
