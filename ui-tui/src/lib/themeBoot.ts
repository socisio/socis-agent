 ...
 * `socis-boot-background` / `socis-boot-color-scheme` localStorage keys.
 *
...
// Profile-aware: the Python launcher exports SOCIS_AGENT_HOME (set by
// _apply_profile_override) before spawning the TUI. Falling back to
// ~/.socis-agent matches get_socis_agent_home()'s default.
const bootFilePath = () => join(process.env.SOCIS_AGENT_HOME ?? join(homedir(), '.socis-agent'), 'tui-theme-boot.json')

// Never touch the user's real ~/.socis-agent from test runs (the TS suite has no
// SOCIS_AGENT_HOME isolation fixture).
...
export interface BootSeedResult {
  ...
  /** True when the cached config pin was seeded into SOCIS_AGENT_TUI_THEME. */
  seededPin: boolean
}
...
export function seedBootEnvironment(boot: BootTheme | null, env: NodeJS.ProcessEnv): BootSeedResult {
  ...
  if (!boot || env.SOCIS_AGENT_TUI_THEME || env.SOCIS_AGENT_TUI_LIGHT) {
    return result
  }
  ...
  if (boot.mode) {
    env.SOCIS_AGENT_TUI_THEME = boot.mode
  ...
  if (
    ...
    !env.SOCIS_AGENT_TUI_BACKGROUND
  ) {
    env.SOCIS_AGENT_TUI_BACKGROUND = boot.background
...
export function invalidateBootBackground(env: NodeJS.ProcessEnv = process.env): boolean {
  if (!seeded.seededBackground || env.SOCIS_AGENT_TUI_BACKGROUND !== seeded.seededBackground) {
    return false
  }

  delete env.SOCIS_AGENT_TUI_BACKGROUND
...
/** True when this boot replayed a cached config pin into SOCIS_AGENT_TUI_THEME.
