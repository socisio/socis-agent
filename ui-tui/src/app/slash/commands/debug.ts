...
import { terminalBackgroundHex } from '@socis/ink'
...
export const debugCommands: SlashCommand[] = [
  ...
  {
    help: 'rescan $SOCIS_AGENT_HOME/tui-widgets and (re)register user widget apps',
    name: 'widgets-reload',
    run: (_arg, ctx) => {
  ...
  {
    help: 'write a V8 heap snapshot + memory diagnostics (see SOCIS_AGENT_HEAPDUMP_DIR)',
    name: 'heapdump',
    run: (_arg, ctx) => {
      ...
      ctx.transcript.panel('Theme', [
        {
          rows: [
            ...
            ['SOCIS_AGENT_TUI_BACKGROUND', process.env.SOCIS_AGENT_TUI_BACKGROUND ?? '(unset)'],
            ['SOCIS_AGENT_TUI_THEME', process.env.SOCIS_AGENT_TUI_THEME ?? '(unset)'],
