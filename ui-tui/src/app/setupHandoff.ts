import type { RunExternalProcess } from '@socis/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`socis ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching socis: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`socis ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
