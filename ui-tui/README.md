...
React + Ink terminal UI for SOCIS. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `SOCIS_AGENT_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- `src/app/setupHandoff.ts` — launches external `socis setup`, suspends Ink while it runs, opens a new session on success
...
- Input history is stored in `~/.socis-agent/.socis_history` or under `SOCIS_AGENT_HOME`.
...
### Setup (`setup.ts`)
`/setup` — launches external `socis setup` wizard, suspends Ink while it runs
...
```text
ui-tui/
  packages/socis-ink/   forked Ink renderer (local dep)
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external socis setup, suspends Ink while it runs
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.socis-agent/tui-parent.log
    ...
    types/
      socis-ink.d.ts            type declarations for @socis/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent SOCISCLI subprocess for slash commands
```
