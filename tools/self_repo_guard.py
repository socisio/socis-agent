...
def _block_message(operation: str, root: Path) -> str:
    ...
    return (
        f"Blocked: `{operation}` would rewrite SOCIS's live source checkout "
        ...
        "checkout, stop SOCIS, run the command externally, then restart "
        "SOCIS."
    )
...
def _scratch_dir_hint() -> str:
    ...
    base = Path(socis_agent_home).expanduser() if socis_agent_home else Path.home() / ".socis-agent"
