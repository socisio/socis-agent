...
def nous_tool_gateway_unavailable_message(
    ...
    *,
    ...
    try:
        ...
        pass
    return (
        f"{capability} is unavailable. Run `socis model` to refresh your "
...
def resolve_provider_secret(
    ...
    Resolution order (fixes #68003 — keys added via ``socis auth add
    ...
    2. The environment / ``~/.socis-agent/.env``. Under a multiplexed gateway turn
       ...
       ``socis_cli.config.get_env_value`` (os.environ, then ``.env``),
       matching the tools' historical behaviour exactly.
    3. The credential pool / auth store for ``provider_id`` (``socis auth
    ...
    when omitted, ``socis_cli.config.get_env_value`` is used directly.
    """
    ...
    try:
        ...
        # `socis auth add <provider>` keys a registry provider by its plain
...
def resolve_openai_audio_api_key() -> str:
    ...
    the credential pool (``socis auth add openai-api``) via
...
def read_selection(section: str) -> str | None:
    """Return the stored `socis tools` provider string for a config section.
...
# config check (socis_cli.config.validate_config_structure) and
...
def selection_error(section: str, selection_name: str, failure: str) -> str:
    ...
    if note:
        failure = note
    return (
        f"{section} is configured to use {selection_name} (set via socis "
        f"tools), but {failure}. Run 'socis tools' to change it."
    )
...
def fal_key_is_configured() -> bool:
    ...
    Consults both ``os.environ`` and ``~/.socis-agent/.env`` (via
    ``socis_cli.config.get_env_value`` when available) so tool-side
