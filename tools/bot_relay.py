...
SOCIS Cloud, docker) is a persistent line. This module is the gateway-side
...
file plumbing on the gateway's own SOCIS root — no network. The public
...
# ``bot_mode.envelope_ttl_seconds`` default in socis_cli/config_defaults.py.
...
def _normalize_roster_row(row: Any) -> Optional[dict]:
    ...
    if not profile or not connection_id:
        return None
    if not handle:
        handle = "socis" if profile == "default" else profile
    if (
...
def _socis_cli() -> str:
    """Resolve the socis CLI beside this gateway's own interpreter.

    The deliver RPC runs on the target gateway, whose process is the venv
    python — its bin/Scripts directory holds the matching ``socis``
    entrypoint. A bare ``"socis"`` relies on PATH, which is exactly what
    ...
    sibling = exe.parent / ("socis.exe" if sys.platform == "win32" else "socis")
    if sibling.is_file():
        return str(sibling)
    found = shutil.which("socis")
    if found:
        return found
    return "socis"
...
# turns concurrently: deliveries spawn separate ``socis`` subprocesses, so
