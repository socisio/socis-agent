...
_PRIVATE_BROWSER_SENTINEL = "_SOCIS_AGENT_BU_PRIVATE_BROWSER"
...
_OWN_TAB_PREAMBLE = """\
# socis: pin this named session to its own tab (once per daemon process)
def _socis_ensure_own_tab():
    ...
    _marker = _os.path.join(
        _tf.gettempdir(), "socis-bu-owntab-%s-%s-%s" % (_uid, _name, _dpid)
    )
    ...
    try:
        ...
        pass
_socis_ensure_own_tab()
del _socis_ensure_own_tab
"""
...
def _base_subprocess_env() -> dict:
    ...
    # may differ from SOCIS's venv Python. PYTHONPATH/PYTHONHOME inherited
    # from the agent process point at SOCIS's venv site-packages, and a
...
def default_downgrade_notice() -> Optional[str]:
    ...
    try:
        ...
        stamp = Path(get_socis_agent_home()) / "cache" / _NOTICE_STAMP_NAME
        try:
        ...
        return (
            ...
            "Run `socis tools` (Browser Automation → Browser Use) to install it, "
...
def _managed_bin_dir() -> Optional[str]:
    """SOCIS' own bin dir ($SOCIS_AGENT_HOME/bin) — where install.sh puts uv/uvx
    and where install_cli() links the browser-use binary."""
    try:
        from socis_agent_constants import get_socis_agent_home

        return str(Path(get_socis_agent_home()) / "bin")
...
def _find_cli() -> Optional[List[str]]:
    ...
    MANAGED-FIRST resolution: SOCIS' own ``$SOCIS_AGENT_HOME/bin`` copy — the
    one every browser backend selection installs and updates via
    ``install_cli()`` — always wins, so all sessions drive one canonical,
    SOCIS-controlled binary. PATH and the user-level tool dir
...
def install_cli(timeout_s: int = 600) -> Tuple[bool, str]:
    ...
    Resolution order for uv: SOCIS' managed uv (bootstrapped on demand via
    ``socis_cli.managed_uv.ensure_uv``) → uv on PATH. The binary is linked
    into ``$SOCIS_AGENT_HOME/bin`` (``UV_TOOL_BIN_DIR``) so ``_find_cli()``
    ...
    # prevent provisioning the canonical SOCIS-managed copy, or resolution
...
def _workspace_dir(task_id: Optional[str]) -> Optional[str]:
    ...
    try:
        ...
        path = Path(get_socis_agent_home()) / "cache" / "browser-use" / "workspace" / safe
...
def _resolve_lightpanda_cdp(
    ...
    """Point the harness at a SOCIS-spawned ``lightpanda serve``.
    ...
    try:
        ...
        return (
            ...
            "to use local Chrome, or switch backends via `socis tools` → "
            "Browser Automation."
        )
...
def _resolve_backend_cdp(
    ...
    4. ``browser.engine: lightpanda``: a SOCIS-spawned ``lightpanda serve``
    ...
    try:
        ...
        return (
            ...
            "switch backends via `socis tools` → Browser Automation."
        )
    cdp = str((session_info or {}).get("cdp_url") or "")
    if not cdp:
        return (
...
def _resolve_real_profile_cdp(env: dict, force_local: bool) -> Optional[str]:
    ...
    default Chromium with their logins — a browser SOCIS launches on a
    SNAPSHOT of their real profile (see socis_cli.browser_connect). Two ways
    in:
...
def _dynamic_schema_overrides() -> dict:
    ...
    if _real_profile_consented():
        ...
        props["local"] = {
            "type": "boolean",
            "description": (
                "Drive the user's own local browser (a SOCIS-managed copy of "
