#!/usr/bin/env python3
"""Run a guided tour (highlight + narrate UI elements) in the SOCIS desktop GUI.
...
- ``surface="app"`` — the SOCIS desktop app's own DOM (tours of SOCIS itself).
...
call fail would leave SOCIS offering walkthroughs it cannot give.
"""
...
def tour_tool(
    ...
    if callback is None:
        return tool_error("tour is only available in the SOCIS desktop app.")
...
TOUR_SCHEMA = {
    ...
    "description": (
        ...
        "element, attach a titled popover. Surfaces: 'app' (SOCIS itself) "
