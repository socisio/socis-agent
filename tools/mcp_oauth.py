...
URL of SOCIS' published Client ID Metadata Document (CIMD) as the
...
This module provides the glue:
    - ``SOCISTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "SOCIS Agent"
...
from socis_agent_constants import secure_parent_dir
...
_USER_SKIPPED_SENTINEL = "__socis_user_skipped__"
...
def _get_token_dir(socis_agent_home: str | Path | None = None) -> Path:
    """Return the directory for MCP OAuth token files.

    Uses SOCIS_AGENT_HOME so each profile gets its own OAuth tokens.
    Layout: ``SOCIS_AGENT_HOME/mcp-tokens/``
    """
    from socis_agent_constants import get_socis_agent_home

    base = Path(socis_agent_home) if socis_agent_home is not None else Path(get_socis_agent_home())
...
def _cached_redirect_port(storage: "SOCISTokenStorage | None") -> int | None:
    ...
    redirect URI that was registered with it. If SOCIS restarts and chooses a
...
def _cached_redirect_uri(storage: "SOCISTokenStorage | None") -> str | None:
...
def _raise_if_non_interactive(lead: str) -> None:
    ...
    shared, actionable ``socis mcp login`` next-step so the guidance wording
    lives in one place across every non-interactive OAuth boundary (#57836).
    """
    if not _is_interactive():
        raise OAuthNonInteractiveError(
            f"{lead} "
            "Run `socis mcp login <server>` interactively to (re)authorize, "
...
# ---------------------------------------------------------------------------
# SOCISTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class SOCISTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        SOCIS_AGENT_HOME/mcp-tokens/<server_name>.json         -- tokens
        SOCIS_AGENT_HOME/mcp-tokens/<server_name>.client.json   -- client info
        SOCIS_AGENT_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
        SOCIS_AGENT_HOME/mcp-tokens/<server_name>.cimd-off      -- CIMD refused here
    """

    def __init__(self, server_name: str, *, socis_agent_home: str | Path | None = None):
        self._server_name = _safe_filename(server_name)
        self._socis_agent_home = Path(socis_agent_home) if socis_agent_home is not None else None

    def _tokens_path(self) -> Path:
        return _get_token_dir(self._socis_agent_home) / f"{self._server_name}.json"

    def _client_info_path(self) -> Path:
        return _get_token_dir(self._socis_agent_home) / f"{self._server_name}.client.json"

    def _meta_path(self) -> Path:
        return _get_token_dir(self._socis_agent_home) / f"{self._server_name}.meta.json"

    def _cimd_rejected_path(self) -> Path:
        return _get_token_dir(self._socis_agent_home) / f"{self._server_name}.cimd-off"
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if OAuthToken is None and not _ensure_sdk_loaded():
            return None
        # SOCIS records an absolute wall-clock ``expires_at`` alongside the
    ...
    def mark_cimd_rejected(self) -> None:
        ...
        refused. Cleared by ``remove()``, i.e. by ``socis mcp login`` /
        ``socis mcp remove``, so a fixed document gets another chance.
        """
    ...
    def restore(self, snapshot: dict[str, bytes], *, only_if_absent: bool = False) -> None:
        ...
        if not snapshot:
            return
        token_dir = _get_token_dir(self._socis_agent_home)
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to SOCIS.</p></body></html>"
            ) if code else (
    ...
    async def _redirect_handler(authorization_url: str) -> None:
        ...
        if on_ssh and redirect_uri:
            ...
            print(
                ...
                f"\n"
                f"  See: https://agent.socis.io/docs/guides/oauth-over-ssh\n",
                file=sys.stderr,
            )
        ...
        if result["auth_code"] is None:
            ...
            if cimd_url:
                hint = (
                    ...
                    f"SOCIS' Client ID Metadata Document ({cimd_url}); set "
...
def _paste_callback_reader(result: dict) -> None:
    ...
    if line.lower() in _SKIP_TOKENS:
        ...
        print(
            "  OAuth skipped. Run `socis mcp login <server>` later to "
...
SOCISOAuthClientProvider: Any = None


def _get_socis_oauth_provider_class() -> type | None:
    global SOCISOAuthClientProvider
    if SOCISOAuthClientProvider is not None:
        return SOCISOAuthClientProvider
    if not _ensure_sdk_loaded():
        return None

    class _SOCISOAuthClientProvider(OAuthClientProvider):
    ...
    _SOCISOAuthClientProvider.__name__ = "SOCISOAuthClientProvider"
    _SOCISOAuthClientProvider.__qualname__ = "SOCISOAuthClientProvider"
    SOCISOAuthClientProvider = _SOCISOAuthClientProvider
    return SOCISOAuthClientProvider
...
def remove_oauth_tokens(
    ...
    *,
    ...
    storage = SOCISTokenStorage(server_name, socis_agent_home=socis_agent_home)
    storage.remove()
...
# 2026-07-28. The SDK does the protocol work; SOCIS only decides whether a
...
# (section 4.2), so a CIMD flow cannot use the ephemeral port SOCIS picks
...
def _has_cached_client_info(storage: "SOCISTokenStorage | None") -> bool:
...
def _server_declined_cimd(storage: "SOCISTokenStorage | None") -> bool:
    ...
    SOCIS has to fix the redirect URI. Cached authorization-server metadata
...
def _maybe_use_cimd(
    cfg: dict,
    storage: "SOCISTokenStorage | None" = None,
) -> "tuple[str, int] | None":
    """Return ``(client_id URL, pinned callback port)``, or None to use DCR.

    Every early return below is a case where the redirect URI SOCIS would
...
#   "Claude Code" → 200
#   "Codex"       → 200
#   "SOCIS Agent" / "SOCIS" / "Cursor" / "VS Code" / … → 403
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    if OAuthClientMetadata is None:
        _ensure_sdk_loaded()
    client_name = cfg.get("client_name", "SOCIS Agent")
    scope = cfg.get("scope")
    ...
    metadata_kwargs: dict[str, Any] = {
        ...
        # rejecting loopback redirect_uris. SOCIS is a CLI/desktop app
...
def _invalidate_tokens_on_client_change(
    ...
    if removed:
        logger.warning(
            ...
            "Re-authorize with: socis mcp login %s",
...
def humanize_oauth_registration_error(
    ...
    *,
    ...
    Figma's remote MCP gates DCR on exact ``client_name``. SOCIS auto-sets
    ``Claude Code`` (known-good); this message fires when the user overrode
    that with something Figma still rejects, or an older SOCIS is running.
    """
    msg = str(exc)
    lowered = msg.lower()
...
def build_oauth_auth(
    ...
    storage = SOCISTokenStorage(server_name)
    ...
    if not _is_interactive() and not storage.has_cached_tokens():
        raise OAuthNonInteractiveError(
            ...
            f"`socis mcp login {server_name}` interactively first to complete "
    ...
    provider_class = _get_socis_oauth_provider_class()
