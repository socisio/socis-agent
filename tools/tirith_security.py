...
it is automatically downloaded from GitHub releases to $SOCIS_AGENT_HOME/bin/tirith.
...
def _get_socis_agent_home() -> str:
    """Return the SOCIS home directory, respecting SOCIS_AGENT_HOME env var."""
    return str(get_socis_agent_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_socis_agent_home(), ".tirith-install-failed")
...
def _socis_bin_dir() -> str:
    """Return $SOCIS_AGENT_HOME/bin, creating it if needed."""
    d = os.path.join(_get_socis_agent_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $SOCIS_AGENT_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_socis_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $SOCIS_AGENT_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $SOCIS_AGENT_HOME/bin/tirith
    ...
    socis_bin = os.path.join(_socis_bin_dir(), "tirith")
    if os.path.isfile(socis_bin) and os.access(socis_bin, os.X_OK):
        _resolved_path = socis_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return socis_bin
