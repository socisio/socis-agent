...
_FORCE_SYNC_ENV = "SOCIS_AGENT_FORCE_FILE_SYNC"
...
class FileSyncManager:
    ...
    def sync(self, *, force: bool = False) -> None:
        ...
        or ``SOCIS_AGENT_FORCE_FILE_SYNC=1`` is set.
    ...
    def sync_back(self, socis_agent_home: Path | None = None) -> None:
        """Pull remote changes back to the host filesystem.

        Downloads the remote ``.socis-agent/`` directory as a tar archive,
        ...
        """
        with self._transaction_lock:
            self._sync_back_transaction(socis_agent_home=socis_agent_home)

    def _sync_back_transaction(self, socis_agent_home: Path | None = None) -> None:
        ...
        # against an uninitialized remote .socis-agent/ directory.
        if not self._pushed_hashes and not self._synced_files:
            logger.debug("sync_back: no prior push state — skipping")
            return

        lock_path = (socis_agent_home or get_socis_agent_home()) / ".sync.lock"
    ...
    def _sync_back_impl(self) -> None:
        ...
        with tempfile.NamedTemporaryFile(suffix=".tar") as tf:
            ...
            with tempfile.TemporaryDirectory(prefix="socis-sync-back-") as staging:
    ...
    def _infer_host_path(self, remote_path: str,
        ...
        For example, if the mapping has ``/root/.socis-agent/skills/a.md`` →
        ``~/.socis-agent/skills/a.md``, a new remote file at
        ``/root/.socis-agent/skills/b.md`` maps to ``~/.socis-agent/skills/b.md``.
        """
        mapping = file_mapping if file_mapping is not None else []
