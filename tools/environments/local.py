...
from socis_agent_constants import get_process_socis_agent_home
...
#
# get_temp_dir() now defaults to SOCIS_AGENT_HOME/cache/terminal (real storage)
...
#
# Background-process artifacts come in triplets (socis_bg_<id>.log/.pid/
...
_BG_GROUP_RE = re.compile(r"^(socis_bg_[A-Za-z0-9_-]+)\.(log|pid|exit)$")


def _default_terminal_temp_dir() -> "Path | None":
    """Return SOCIS_AGENT_HOME/cache/terminal, or None if unresolvable."""
    try:
        from socis_agent_constants import get_socis_agent_home
        return get_socis_agent_home() / "cache" / "terminal"
    except Exception:
        return None
...
def cleanup_terminal_temp_cache(
    ...
    Only prunes the managed default dir (``SOCIS_AGENT_HOME/cache/terminal``).
    ...
    # Newest mtime per socis_bg_<id> group, so a live server's fresh .log
...
def _resolve_local_initial_cwd(cwd: str) -> str:
    ...
    directory SOCIS was already launched from (for example ``socis-agent``
    while the process cwd is ``~/.socis-agent/socis-agent``), passing it through
    unchanged makes the wrapper run ``cd socis-agent`` *inside* the project
    ...
    # Common recovery for config values like ``socis-agent`` when SOCIS was
    # launched from that directory already.  ``os.path.abspath`` would point at
    # a nonexistent nested ``./socis-agent``; use the current directory instead.
...
# SOCIS-internal env vars that should NOT leak into terminal subprocesses.
_SOCIS_AGENT_PROVIDER_ENV_FORCE_PREFIX = "_SOCIS_AGENT_FORCE_"

# SOCIS-managed AWS *inference* credentials for ``auth_type="aws_sdk"``
# providers (Bedrock).  Scoped DELIBERATELY NARROW: this lists only the
# Bedrock-specific bearer token, which is a SOCIS inference secret exactly
...
def _build_provider_env_blocklist() -> frozenset:
    ...
    try:
        from socis_cli.config import OPTIONAL_ENV_VARS
    ...
    blocked.update({
        ...
        "SOCIS_AGENT_DASHBOARD_SESSION_TOKEN",
    ...
    # SOCIS-managed inference credential — Claude subscription auth is not a
    # working SOCIS provider path.  Stripping it broke agent-spawned
    ...
    # the :func:`socis_subprocess_env` Tier-2 strip (browser / TUI host /
...
# ``buzz`` (the gateway's ``SOCIS_AGENT_SESSION_PLATFORM`` ContextVar; concurrency
...
# :func:`socis_subprocess_env` (browser / TUI host / copilot-executor
...
# from the blocklist entirely because it is NOT a SOCIS credential; these ARE
# SOCIS-managed first-party platform credentials, so they stay IN the
...
def _buzz_terminal_context_active() -> bool:
    ...
    * ``BUZZ_MANAGED_AGENT`` in the process env — set exclusively by Buzz
      Desktop's buzz-acp harness when it spawns ``socis acp`` (#76243).
      Gateway / CLI / cron / kanban processes never carry it.
    * The live session's platform is ``buzz`` — the gateway's
      ``SOCIS_AGENT_SESSION_PLATFORM`` ContextVar via
    ...
    try:
        ...
        return get_session_env("SOCIS_AGENT_SESSION_PLATFORM", "").strip().lower() == "buzz"
    except Exception:
        return False
...
# project's dependencies into the SOCIS venv path instead of the project's own
# ``.venv`` — silently clobbering the SOCIS environment (e.g. a project pinned
# to a different Python version overwrites it and breaks the gateway). The
# SOCIS venv stays reachable via PATH (its bin dir is first), so stripping
...
# system/venv Pythons — to the SOCIS venv's stdlib, which crashes with
# version-mismatch errors before a child script even imports a package
# (#75018). SOCIS itself treats PYTHONHOME as contamination in its own
...
# _strip_socis_owned_pythonpath() which removes only SOCIS-owned entries,
# preserving user-set paths.
_ACTIVE_VENV_MARKER_VARS = ("VIRTUAL_ENV", "CONDA_PREFIX", "PYTHONHOME")


def _is_socis_internal_secret(key: str) -> bool:
    """Return True for SOCIS-internal secrets injected under *dynamic* names.

    ``_SOCIS_AGENT_PROVIDER_ENV_BLOCKLIST`` is name-based and derived from the
    ...
    This is the single source of truth for "SOCIS-internal dynamic secret"
    across every spawn path — the terminal ``_make_run_env`` /
    ``_sanitize_subprocess_env`` filters, the Docker passthrough filter, and the
    non-terminal :func:`socis_subprocess_env` helper all call it, so the
...
def _inject_context_socis_agent_home(env: dict) -> None:
    """Bridge the context-local SOCIS home override into subprocess env."""
    try:
        from socis_agent_constants import get_socis_agent_home_override

        value = get_socis_agent_home_override()
        if value:
            env["SOCIS_AGENT_HOME"] = value
    except Exception:
        pass
...
def _inject_session_context_env(env: dict) -> None:
    ...
    (SOCIS_AGENT_SESSION_*) are bridged onto the child env here.
...
def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    """Filter SOCIS-managed secrets from a subprocess environment."""
    try:
    ...
    for key, value in (base_env or {}).items():
        if key.startswith(_SOCIS_AGENT_PROVIDER_ENV_FORCE_PREFIX):
            continue
        if _is_socis_internal_secret(key):
            continue
    ...
    for key, value in (extra_env or {}).items():
        if key.startswith(_SOCIS_AGENT_PROVIDER_ENV_FORCE_PREFIX):
            real_key = key[len(_SOCIS_AGENT_PROVIDER_ENV_FORCE_PREFIX):]
            if _is_socis_internal_secret(real_key):
                continue
            sanitized[real_key] = value
        elif _is_socis_internal_secret(key):
            continue
    ...
    _inject_context_socis_agent_home(sanitized)

    from socis_agent_constants import apply_subprocess_home_env
    ...
    # the separate SOCIS runtime venv.  The filter validates that relationship
    # against the repo layout before trusting it.
    _strip_socis_owned_pythonpath_and_runtime_markers(sanitized)

    # Keep bare ``socis`` invocations available to child jobs even when the
    ...
    if path_key is not None:
        sanitized[path_key] = _prepend_socis_bin_dir(sanitized.get(path_key, ""))
...
# legitimate child SOCIS spawns needs them, and they are the highest-value
...
# narrow subset of _SOCIS_AGENT_PROVIDER_ENV_BLOCKLIST; provider keys are handled by
# the conditional Tier-2 strip in socis_subprocess_env().
_ALWAYS_STRIP_KEYS: frozenset[str] = frozenset({
    ...
    # _is_socis_internal_secret, but _ID has no secret suffix, so it must be
...
def socis_subprocess_env(*, inherit_credentials: bool = False) -> dict[str, str]:
    ...
    single source of truth (``_SOCIS_AGENT_PROVIDER_ENV_BLOCKLIST``).  The terminal
    ...
    * **Tier 1 (always):** ``_ALWAYS_STRIP_KEYS`` — gateway bot tokens, GitHub
      ...
      ``inherit_credentials``.  No child SOCIS spawns legitimately needs them.
    * **Tier 2 (conditional):** the rest of ``_SOCIS_AGENT_PROVIDER_ENV_BLOCKLIST``
    ...
    for key in _plugin_terminal_env_strip_keys():
        env.pop(key, None)
    # Internal routing hints and SOCIS-internal dynamic secrets
    ...
    # legitimate use for them. See :func:`_is_socis_internal_secret`.
    for key in list(env):
        if key.startswith(_SOCIS_AGENT_PROVIDER_ENV_FORCE_PREFIX):
            env.pop(key, None)
        elif _is_socis_internal_secret(key):
    ...
    _inject_context_socis_agent_home(env)
    from socis_agent_constants import apply_subprocess_home_env
    apply_subprocess_home_env(env)

    _strip_socis_owned_pythonpath_and_runtime_markers(env)

    _apply_windows_msys_bash_env_defaults(env)

    # Cross-session leak guard, same as the terminal spawn paths: this helper
    # copies os.environ, whose SOCIS_AGENT_SESSION_* mirror is a last-writer-wins
    ...
    # still see the parent's SOCIS_AGENT_HOME but lose the DB mutation guard.
...
def build_subprocess_env(
    ...
    *,
    ...
    function (or :func:`socis_subprocess_env` for the model-driving-CLI
    surface) instead of copying ``os.environ`` directly, so profile-home
    propagation (``SOCIS_AGENT_HOME`` / subprocess ``HOME`` contract) and the
    SOCIS secret-scrub policy have a single owner.  History: ~11 separate
    ...
    * ``scrub_secrets=True`` (default) — delegate to
      ...
      list (provider blocklist + ``_is_socis_internal_secret`` dynamic
      patterns + kanban/venv-marker/session-context guards) **and** of
      ``SOCIS_AGENT_HOME`` / subprocess-HOME propagation.  On this path profile
    ...
    * ``inherit_profile_home`` — on the non-scrub path, when True, bridge the
      context-local SOCIS home override into ``SOCIS_AGENT_HOME`` and apply the
      subprocess HOME contract (``socis_agent_constants.apply_subprocess_home_env``).
    ...
    * ``extra`` — applied **last** on the non-scrub path so explicit caller
      overrides (e.g. a session-scoped ``SOCIS_AGENT_HOME``) always win.  On the
      scrub path it is forwarded as ``_sanitize_subprocess_env``'s
      ``extra_env`` (same force-prefix / blocklist handling as today).
    """
    if scrub_secrets:
        # _sanitize_subprocess_env already performs SOCIS_AGENT_HOME override
...
def _find_bash() -> str:
    ...
    custom = os.environ.get("SOCIS_AGENT_GIT_BASH_PATH")
    if custom and os.path.isfile(custom):
        candidates.append(custom)

    # Prefer our own portable Git install — a broken or partially-uninstalled
    # system Git (or a stale SOCIS_AGENT_GIT_BASH_PATH pointing at one) must not
    ...
    #   PortableGit: %LOCALAPPDATA%\socis\git\bin\bash.exe   (primary)
    #   MinGit:      %LOCALAPPDATA%\socis\git\usr\bin\bash.exe (legacy/32-bit fallback)
    _local_appdata = os.environ.get("LOCALAPPDATA", "")
    _socis_portable_git = os.path.join(_local_appdata, "socis", "git") if _local_appdata else ""
    if _socis_portable_git:
        for candidate in (
            os.path.join(_socis_portable_git, "bin", "bash.exe"),        # PortableGit (primary)
            os.path.join(_socis_portable_git, "usr", "bin", "bash.exe"), # MinGit fallback
        ):
    ...
    # SOCIS_AGENT_GIT_BASH_PATH pointing at a broken Git-for-Windows install
    # (``Directory \\drivers\\etc does not exist``) must not win over a
    # healthy portable Git under %LOCALAPPDATA%\\socis\\git.
    for candidate in candidates:
        if _bash_starts(candidate):
            if candidate != custom and custom and os.path.isfile(custom):
                logger.warning(
                    "SOCIS_AGENT_GIT_BASH_PATH=%s fails to start; using %s instead",
                    custom,
                    candidate,
                )
            return candidate
    ...
    raise RuntimeError(
        "Git Bash not found. SOCIS Agent requires Git for Windows on Windows.\n"
        "Install it from: https://git-scm.com/download/win\n"
        "Or set SOCIS_AGENT_GIT_BASH_PATH to your bash.exe location."
    )
...
def _git_bash_aslr_help(bash: str, details: str = "") -> str:
    ...
    return (
        ...
        "Then restart SOCIS. If the override is blocked or later re-applied, "
...
# Cached directory containing the ``socis`` console-script.
# ``_SENTINEL`` distinguishes "not resolved yet" from a resolved ``None``.
_SENTINEL = object()
_SOCIS_AGENT_BIN_DIR: "str | None | object" = _SENTINEL


def _resolve_socis_bin_dir() -> str | None:
    """Return the directory holding the ``socis`` console-script, or None.
    ...
    socis install dir (``~/.local/bin``, the venv ``bin``/``Scripts``, pipx,
    nix) is absent from that PATH, so plugins shelling out to bare ``socis``
    via the terminal tool hit ``command not found`` (exit 127) even though
    ``socis`` works fine in the user's own interactive terminal.

    We resolve the install dir once (it never changes within a process) and
    prepend-if-missing it to the subshell PATH so bare ``socis`` resolves
    regardless of how the gateway was started.

    Resolution order (cheap, no heavy imports):
      1. ``shutil.which("socis")`` — normal PATH-installed shim.
      2. The directory of ``sys.argv[0]`` when it's an absolute path to a
         real ``socis`` executable (covers nix-store / venv wrappers).
      3. The directory of ``sys.executable`` — the running interpreter's
         venv ``bin``/``Scripts`` is where its console-scripts live.
    """
    global _SOCIS_AGENT_BIN_DIR
    if _SOCIS_AGENT_BIN_DIR is not _SENTINEL:
        return _SOCIS_AGENT_BIN_DIR  # type: ignore[return-value]

    candidate: str | None = None

    which = shutil.which("socis")
    if which:
    ...
    if candidate is None:
        ...
        if (
            os.path.isabs(argv0)
            and (base == "socis" or base.startswith("socis."))
    ...
    if candidate is None:
        ...
        if exe_dir:
            shim = "socis.exe" if _IS_WINDOWS else "socis"
    ...
    _SOCIS_AGENT_BIN_DIR = candidate
    return candidate


def _prepend_socis_bin_dir(existing_path: str) -> str:
    """Prepend the socis install dir to ``existing_path`` if it's missing.
    ...
    """
    bin_dir = _resolve_socis_bin_dir()
    if not bin_dir:
        return existing_path
    sep = os.pathsep
    entries = [e for e in existing_path.split(sep) if e] if existing_path else []
...
def _managed_runtime_path_entries() -> list[str]:
    """Return existing SOCIS-managed runtime dirs for the terminal subshell PATH.

    The terminal tool spawns a subshell whose PATH is the agent process's PATH
    plus ``_SANE_PATH``. Neither carries the runtimes SOCIS installs for
    itself, so on a machine where SOCIS provisioned its own toolchain a
    command the agent runs resolves a system copy instead — or nothing at all:

    - ``$SOCIS_AGENT_HOME/node`` (+ ``/bin``) — installed to satisfy the desktop and
      browser toolchain. ``tools/browser_tool.py`` already does this for its own
      subprocesses; the agent's shell deserves the same.
    - ``$SOCIS_AGENT_HOME/bin`` — the managed ``uv``. ``install.sh`` writes it there
    ...
    ``get_socis_agent_home()`` is profile-scoped and a managed tree can appear
    mid-process (``heal_socis_managed_node``, a first browser install).
    """
    try:
        from socis_agent_constants import get_socis_agent_home, iter_socis_node_dirs

        candidates = [*iter_socis_node_dirs(), get_socis_agent_home() / "bin"]
...
def _append_missing_sane_path_entries(existing_path: str) -> str:
    ...
    SOCIS-managed runtime dirs are appended alongside the sane entries, not
...
def _apply_windows_msys_bash_env_defaults(env: dict) -> None:
    ...
    Windows commands such as ``tasklist``, ``schtasks``, and ``wmic``.  SOCIS
...
def _make_run_env(env: dict) -> dict:
    ...
    for k, v in merged.items():
        if k.startswith(_SOCIS_AGENT_PROVIDER_ENV_FORCE_PREFIX):
            real_key = k[len(_SOCIS_AGENT_PROVIDER_ENV_FORCE_PREFIX):]
            if _is_socis_internal_secret(real_key):
                continue
            run_env[real_key] = v
        elif _is_socis_internal_secret(k):
            continue
        else:
    ...
    if path_key is not None:
        ...
        # Ensure the socis install dir is reachable so plugins can shell out
        # to bare ``socis`` via the terminal tool even when the gateway was
        # launched without it on PATH (systemd, service managers, cron, etc.).
        run_env[path_key] = _prepend_socis_bin_dir(new_path)

    _inject_context_socis_agent_home(run_env)
    ...
    _strip_socis_owned_pythonpath_and_runtime_markers(run_env)
...
def _build_socis_repo_root_aliases(
    resolved_root: Path,
    lexical_root: Path,
    configured_home: Path,
) -> tuple[Path, ...]:
    """Return exact repo-root spellings emitted by SOCIS launchers.

    ``gateway_windows._preserve_socis_agent_home_path`` maps a physical path under
    the resolved SOCIS_AGENT_HOME back onto the configured SOCIS_AGENT_HOME spelling.
    Mirror that producer contract here so a junction-backed install is matched
    without treating arbitrary descendants of SOCIS_AGENT_HOME as SOCIS-owned.
    ...
    # lexically the same way get_default_socis_root() does (parent of a
    ...
    # while still never matching arbitrary descendants of SOCIS_AGENT_HOME.
    ...
    # junction/symlink under the configured root (e.g. D:\socis\socis-agent
    # -> C:\...\socis-agent) while the import spelling (editable install)
    ...
    # strict resolve before accepting it as SOCIS-owned.  Fail-closed: a
...
# --- SOCIS venv / repo-root detection (module-level, computed once) ---

#: The SOCIS repository root - three levels up from this file
#: (``tools/environments/local.py`` -> ``tools/environments`` -> ``tools``
#: -> repo root).  This is the directory the Electron app prepends to
#: PYTHONPATH so the backend can do ``import tools``, ``import socis_cli``,
#: etc.  Subprocesses that are NOT the SOCIS backend don't need it and it
#: can shadow local packages.
_socis_repo_root: Path = Path(__file__).resolve().parents[2]

#: Alternate spellings of the repo root that SOCIS launchers may emit.
#: ``Path(__file__).resolve()`` canonicalizes symlinks/junctions, but the
#: Windows gateway launcher deliberately renders SOCIS-owned paths under
#: the configured SOCIS_AGENT_HOME spelling (which may be a junction to another
#: drive — see ``socis_cli/gateway_windows.py::_preserve_socis_agent_home_path``).
...
_socis_repo_root_aliases: tuple[Path, ...] = _build_socis_repo_root_aliases(
    _socis_repo_root,
...
#: subprocess spawn.
_socis_site_packages: list[Path] | None = None
...
def _validated_runtime_venv(env: dict) -> Path | None:
    ...
    ``<SOCIS repo>/venv`` layout and a real venv marker; require both before
    ...
    if not any(_same_path(candidate, repo_root / "venv") for repo_root in _socis_repo_root_aliases):
        return None
...
def _get_socis_site_packages(env: dict) -> list[Path]:
    """Return exact site-packages dirs owned by the SOCIS runtime.
    ...
    """
    global _socis_site_packages
    if _socis_site_packages is not None:
        result = list(_socis_site_packages)
    else:
        result = []
        if _in_venv:
            try:
        ...
        _socis_site_packages = list(result)
...
def _strip_socis_owned_pythonpath_and_runtime_markers(env: dict) -> None:
    """Strip SOCIS-owned PYTHONPATH entries, then the runtime marker vars.
    ...
    """
    _strip_socis_owned_pythonpath(env)
    for _marker in _ACTIVE_VENV_MARKER_VARS:
        env.pop(_marker, None)


def _strip_socis_owned_pythonpath(env: dict) -> None:
    """Remove SOCIS-owned PYTHONPATH entries from subprocess environments.

    Launchers prepend the SOCIS repo root and the SOCIS venv's
    ...
    user entries, so only entries proven SOCIS-owned are removed:
    ...
    socis_site_packages = _get_socis_site_packages(env)
    ...
    for entry in pp.split(os.pathsep):
        ...
        # Preserve raw spelling unless the exact component is SOCIS-owned.
        if entry == "":
            kept.append(entry)
            continue
        ...
        # --- Check 1: SOCIS venv site-packages ---
        ...
        # resolved and unresolved (SOCIS_AGENT_HOME/junction) spellings count as
        # SOCIS-owned.
        if not should_strip:
            should_strip = any(
                _same_path(entry_path, repo_root)
                for repo_root in _socis_repo_root_aliases
            )
    ...
    if stripped:
        logger.debug(
            "Stripped SOCIS-owned entries from PYTHONPATH: %s",
            stripped,
        )
...
def _resolve_shell_init_files() -> list[str]:
    ...
    an explicit list — once they have, SOCIS trusts them.
    """
...
class LocalEnvironment(BaseEnvironment):
    ...
    # Commands run on the SOCIS host itself — controller-side platform
    ...
    def _additional_profile_scoped_passthrough_names(self) -> tuple[str, ...]:
        ...
        ``socis-snap-<id>.sh`` and a later command from profile B sharing
    ...
    def get_temp_dir(self) -> str:
        ...
        **Default (no override set):** a dedicated cache dir under
        ``SOCIS_AGENT_HOME`` (``~/.socis-agent/cache/terminal``) rather than ``/tmp``.
        On several distros (Arch and friends) ``/tmp`` is a small RAM-backed
        tmpfs, and SOCIS session artifacts — background-process logs,
        ...
        **Windows:** hardcoded ``/tmp`` is wrong in two ways — native Python
        ...
        ``SOCIS_AGENT_HOME`` instead — single-word path, guaranteed to exist, same
        string resolves in both Git Bash and native Python.
        """
        if _IS_WINDOWS:
            # Derive a Windows-safe temp dir under SOCIS_AGENT_HOME.  Using
            ...
            try:
                from socis_agent_constants import get_socis_agent_home
                cache_dir = get_socis_agent_home() / "cache" / "terminal"
            except Exception:
                cache_dir = Path(tempfile.gettempdir()) / "socis_terminal"
        ...
        # Honored ahead of the generic TMPDIR so users can redirect SOCIS' temp
        ...
        # Default: SOCIS_AGENT_HOME/cache/terminal — real storage, mirroring the
        # Windows branch above. /tmp is only a last-resort fallback now
        # because RAM-backed tmpfs /tmp fills up under SOCIS load.
        try:
            from socis_agent_constants import get_socis_agent_home
            cache_dir = get_socis_agent_home() / "cache" / "terminal"
            cache_dir.mkdir(parents=True, exist_ok=True)
    ...
    def _run_bash(self, cmd_string: str, *, login: bool = False,
        ...
        if not _IS_WINDOWS:
            try:
                proc._socis_pgid = os.getpgid(proc.pid)
        ...
        try:
            if _IS_WINDOWS:
                ...
                try:
                    ...
                    pgid = getattr(proc, "_socis_pgid", None)
                    if pgid is None:
                        raise
