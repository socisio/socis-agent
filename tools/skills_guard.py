...
Trust levels:
  - builtin:   Ships with SOCIS. Never scanned, always trusted.
...
_SOCIS_AGENT_CONFIG_FILES = r'\.socis-agent/(?:config\.yaml|SOUL\.md)'
...
THREAT_PATTERNS = [
    ...
    (r'\$HOME/\.socis-agent/\.env|\~/\.socis-agent/\.env',
     "socis_env_access", "critical", "exfiltration",
     "directly references SOCIS secrets file"),
    ...
    #     For SOCIS/other config files it is high (caution) — legitimate
    ...
    (_prose_modify_re(_SOCIS_AGENT_CONFIG_FILES),
     "socis_config_mod", "high", "persistence",
     "modification language aimed at SOCIS configuration files (verify intent)"),
    (_shell_write_re(_SOCIS_AGENT_CONFIG_FILES),
     "socis_config_mod_shell", "critical", "persistence",
     "shell write (redirect/sed -i/tee/cp/mv) targeting SOCIS configuration files"),
    (r'\.socis-agent/config\.yaml|\.socis-agent/SOUL\.md',
     "socis_config_ref", "low", "persistence",
     "references SOCIS configuration files (informational; only modification intent is scored)"),
...
# scan. `.skillignore` is the SOCIS-native name; `.clawhubignore` is honored
