...
`socis skills publish` for sharing. No new source type, no new store, no new
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.socis.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.socis.blueprint`` is a mapping containing
    ...
    socis = meta.get("socis") if isinstance(meta, dict) else None
    blueprint = socis.get("blueprint") if isinstance(socis, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.socis.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.socis.blueprint`` block) they can hand
    to ``socis skills publish`` to share. ``body`` is the plain-language
