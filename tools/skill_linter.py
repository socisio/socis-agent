...
Design contract (matches the SOCIS "no lazy-reading escape hatches / don't
...
def _check_metadata_block(frontmatter: Dict[str, Any]) -> List[LintFinding]:
    ...
    socis_meta = meta.get("socis") if isinstance(meta, dict) else None
    if not isinstance(socis_meta, dict):
        findings.append(
            LintFinding(
                WARNING,
                "missing-metadata",
                "frontmatter is missing metadata.socis.{tags, related_skills}.",
            )
        )
    else:
        if "tags" not in socis_meta:
            findings.append(
                LintFinding(
                    WARNING, "missing-metadata", "metadata.socis.tags is missing."
                )
            )
    author = str(frontmatter.get("author", ""))
    if author and author.strip().lower() in ("socis", "agent", "socis agent") and (
        author != "SOCIS Agent"
    ):
        findings.append(
            LintFinding(
                WARNING,
                "author-caps",
                f"author '{author}' should be 'SOCIS Agent' (proper caps) "
