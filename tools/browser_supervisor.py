...
One ``CDPSupervisor`` runs per SOCIS ``task_id`` that has a reachable CDP
...
def _redact_cdp_error_text(exc: object) -> str:
    ...
    those credentials never reach SOCIS logs or tracebacks. Falls back to a
...
DIALOG_BRIDGE_HOST = "socis-dialog-bridge.invalid"
...
(() => {
  if (window.__socisDialogBridgeInstalled) return;
  window.__socisDialogBridgeInstalled = true;
