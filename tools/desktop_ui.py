#!/usr/bin/env python3
"""Bridge desktop-only tools to SOCIS-desktop renderer events.
...
``SOCIS_AGENT_UI_SESSION_ID`` so the event lands on the window that owns the turn
...
def user_enabled(setting: str, default: bool) -> bool:
    ...
    SOCIS should not be told about a surface it isn't allowed to use.
...
def emit(event: str, payload: dict) -> bool:
    ...
    if fn is None:
        return False
    fn(get_session_env("SOCIS_AGENT_UI_SESSION_ID", ""), event, payload)
    return True
