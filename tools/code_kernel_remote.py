...
Closes the gap tracked in socis-agent#96873: local execute_code holds a
...
"""Auto-generated SOCIS REMOTE session-kernel runner (file cell protocol)."""
import contextlib
import io
import json
import os
...
KDIR = os.environ["SOCIS_AGENT_KERNEL_DIR"]
...
class RemoteKernel:
    ...
    # poll loop (same guard as tools.code_kernel, socis-agent#101861).
    attached: int = 0
...
def _spawn_remote_kernel(env, env_type: str, owner: str, task_env_id: str,
    ...
    kernel_dir = f"{_env_temp_dir(env)}/socis_rkernel_{uuid.uuid4().hex[:12]}"
    ...
    try:
        ...
        _ship_file_to_remote(env, f"{kernel_dir}/socis_tools.py", tools_src)

        env_prefix = (
            f"SOCIS_AGENT_KERNEL_DIR={q_dir} "
            f"SOCIS_AGENT_RPC_DIR={shlex.quote(kernel_dir + '/rpc')} "
