...
machine as SOCIS itself), we surface a single deterministic line about
Python tooling state so models don't have to discover it by hitting
walls.  Common failure modes this addresses:

* SOCIS ships under one Python (e.g. 3.11 in a bundled venv) while the
...
def _build_probe_line() -> str:
    ...
    # Bare which() is correct here, unlike SOCIS's own uv call sites: this
    ...
    # local.py puts the SOCIS-managed $SOCIS_AGENT_HOME/bin there, so a managed-only
