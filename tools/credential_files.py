...
def _resolve_socis_agent_home() -> Path:
...
def register_credential_file(
    ...
    *relative_path* is relative to ``SOCIS_AGENT_HOME`` (e.g. ``google_token.json``).
    ...
    The resolved host path must remain inside SOCIS_AGENT_HOME so that a malicious
    skill cannot declare ``required_credential_files: ['../../.ssh/id_rsa']``
    and exfiltrate sensitive host files into a container sandbox.

    Containment alone is not sufficient, because SOCIS_AGENT_HOME is exactly where
    ...
    """
    socis_agent_home = _resolve_socis_agent_home()

    # Reject absolute paths — they bypass the SOCIS_AGENT_HOME sandbox entirely.
    if os.path.isabs(relative_path):
        logger.warning(
            "credential_files: rejected absolute path %r (must be relative to SOCIS_AGENT_HOME)",
            relative_path,
        )
        return False

    host_path = socis_agent_home / relative_path

    # Resolve symlinks and normalise ``..`` before the containment check so
    # that traversal like ``../. ssh/id_rsa`` cannot escape SOCIS_AGENT_HOME.
    from tools.path_security import validate_within_dir

    containment_error = validate_within_dir(host_path, socis_agent_home)
    ...
    # inside SOCIS_AGENT_HOME and therefore pass the containment check above.
            ...
            for item in cred_files:
                if isinstance(item, str) and item.strip():
                    ...
                    if os.path.isabs(rel):
                        ...
                        continue
                    host_path = socis_agent_home / rel
                    containment_error = validate_within_dir(host_path, socis_agent_home)
                    if containment_error:
                        logger.warning(
                    ...
                    if resolved_path.is_file():
                        container_path = f"/root/.socis-agent/{rel}"
                        result.append({
...
def _safe_skills_path(skills_dir: Path) -> str:
    ...
    safe_dir = Path(tempfile.mkdtemp(prefix="socis-skills-safe-"))
...
# Each tuple is (new_subpath, old_name) matching socis_agent_constants.get_socis_dir().
...
def get_cache_directory_mounts(
    ...
    ``get_socis_dir()`` for backward compatibility with old directory layouts.
    """
    from socis_agent_constants import get_socis_dir

    mounts: List[Dict[str, str]] = []
    for new_subpath, old_name in _CACHE_DIRS:
        host_dir = get_socis_dir(new_subpath, old_name)
        if not host_dir.is_dir():
            ...
            # mount costs the feature. get_socis_dir() already resolved
