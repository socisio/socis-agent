...
# Must match socis_cli.runtime_provider.RUNTIME_PROVIDER_TYPE_CUSTOM.
...
def _expand_parent_toolsets(parent_toolsets: set) -> set:
    ...
    When a parent uses a composite toolset like ``socis-cli`` (which bundles
...
def _blocked_toolsets_for_role(role: str) -> List[str]:
    ...
    mixed platform bundles such as ``socis-cli`` because those also contain
...
def _build_child_agent(
    ...
    if toolsets:
        ...
        # Expand composite toolsets (e.g. socis-cli) so that individual
    ...
    # Blocked tools also live inside mixed platform bundles (socis-cli,
    # socis-telegram, etc.) that _strip_blocked_tools must keep because they
    ...
    # child SOCIS/Qwen subagent onto the parent's Claude Messages wire (or the
    ...
    elif _effective_provider_norm in {"nous", "nous-portal", "socis"}:
...
def _dump_subagent_timeout_diagnostic(
    *,
    ...
    writes a dedicated log under ``~/.socis-agent/logs/subagent-<sid>-<ts>.log``
    ...
    try:
        ...
        logs_dir = socis_agent_home / "logs"
        try:
...
def _spill_summary_to_file(task_index: int, summary: str) -> Optional[str]:
    ...
    try:
        ...
        cache_dir = get_socis_dir("cache/delegation", "delegation_cache")
    ...
    if _subagent_id:
        if owner_session_id is None:
            try:
                ...
                owner_session_id = get_session_env("SOCIS_AGENT_UI_SESSION_ID", "") or None
            except Exception:
...
def _finalize_child_results(
    ...
    with _parent_finalization_lock(parent_agent):
        ...
        parent_session_id = getattr(parent_agent, "session_id", None)
        try:
            from socis_cli.plugins import invoke_hook as invoke_hook
        except Exception:
            invoke_hook = None
...
def delegate_task(
    ...
    # SOCIS_AGENT_SESSION_ID ContextVar and os.environ with the subagent's internal
    # id before the background-dispatch code below would read it. The
    # request-scoped chat_id binding (the raw X-SOCIS-Session-Id on
    ...
    try:
        ...
        _origin_ui_session_id = get_session_env("SOCIS_AGENT_UI_SESSION_ID", "")
    except Exception:
    ...
    if background:
        ...
        if not _async_ok:
            ...
            # with that id in X-SOCIS-Session-Id once the batch completes.
            ...
            # SOCIS_AGENT_SESSION_ID here would return the subagent's internal id.
        ...
        if not _async_ok:
            ...
            if isinstance(_sync_result, dict):
                _sync_result["note"] = (
                    ...
                    "one-shot runner such as `socis -z`, a cron job, a Kanban "
        ...
        try:
            ...
            _source = get_session_env("SOCIS_AGENT_SESSION_SOURCE", "")
            ...
            _origin_ui_session_id = (
                get_session_env("SOCIS_AGENT_UI_SESSION_ID", "") or _origin_ui_session_id
            )
        ...
        if not _session_key:
            ...
            # during gateway/TUI turns and SOCIS_AGENT_SESSION_KEY is not in the CLI
...
def _resolve_delegation_credentials(cfg: dict, parent_agent) -> dict:
    ...
    if not api_key:
        raise ValueError(
            ...
            f"Set the appropriate environment variable or run 'socis auth'."
        )
...
def _load_config() -> dict:
    """Load delegation config from the active SOCIS config.

    Prefer the shared persistent loader because it follows the active
    SOCIS_AGENT_HOME/profile. ``cli.CLI_CONFIG`` is a legacy fallback for entry
    ...
    ``SOCIS_AGENT_IGNORE_USER_CONFIG=1`` (``socis chat --ignore-user-config``) is
    ...
    """
    prefer_legacy = os.environ.get("SOCIS_AGENT_IGNORE_USER_CONFIG") == "1"
