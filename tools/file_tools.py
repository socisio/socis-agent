...
def _expand_tilde(path: str) -> str:
    ...
    mirrors ``socis_agent_constants.get_subprocess_home()`` so that ``~`` resolves
...
def _truncate_to_char_budget(content: str, max_chars: int) -> tuple[str, int, bool]:
    ...
    ``read_file``). Where socis previously hard-rejected an oversized read
...
def _terminal_env_type_for_task(task_id: str = "default") -> str:
    ...
    try:
        ...
        if env is not None:
            ...
            if "daytona" in name:
                return "daytona"
            stamped = getattr(env, "_socis_backend_name", None)
...
def _file_ops_uses_host_paths(file_ops) -> bool:
    """Return True when *file_ops* targets the same host filesystem as SOCIS.
...
_socis_config_resolved: str | None = None
_socis_config_resolved_loaded = False


def _get_socis_config_resolved() -> str | None:
    """Return the resolved absolute path of the SOCIS config file (cached)."""
    global _socis_config_resolved, _socis_config_resolved_loaded
    if _socis_config_resolved_loaded:
        return _socis_config_resolved
    _socis_config_resolved_loaded = True
    try:
        from socis_cli.config import get_config_path
        _socis_config_resolved = str(get_config_path().resolve())
    except Exception:
        try:
            _socis_config_resolved = str(Path(_expand_tilde("~/.socis-agent/config.yaml")).resolve())
        except Exception:
            _socis_config_resolved = None
    return _socis_config_resolved
...
def _check_sensitive_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    if resolved in _SENSITIVE_EXACT_PATHS or normalized in _SENSITIVE_EXACT_PATHS:
        return _err
    # Prevent agents from modifying the SOCIS config file directly.
    # approvals.mode and other security settings live here; a malicious or
    # prompt-injected agent could silently disable exec approval by writing to
    # this file.
    socis_config = _get_socis_config_resolved()
    if socis_config and (resolved == socis_config or normalized == socis_config):
        return (
            f"Refusing to write to SOCIS config file: {filepath}\n"
            "Agent cannot modify security-sensitive configuration. "
            "Edit ~/.socis-agent/config.yaml directly or use 'socis config' instead."
        )
    return None
...
_real_socis_agent_home_cached: str | None = None
_real_socis_agent_home_loaded = False


def _get_real_socis_agent_home() -> str | None:
    """Return the realpath of the authoritative SOCIS home (cached)."""
    global _real_socis_agent_home_cached, _real_socis_agent_home_loaded
    if _real_socis_agent_home_loaded:
        return _real_socis_agent_home_cached
    _real_socis_agent_home_loaded = True
    try:
        from socis_agent_constants import get_socis_agent_home
        _real_socis_agent_home_cached = os.path.realpath(str(get_socis_agent_home()))
    except Exception:
        try:
            _real_socis_agent_home_cached = os.path.realpath(_expand_tilde("~/.socis-agent"))
        except Exception:
            _real_socis_agent_home_cached = None
    return _real_socis_agent_home_cached
...
def _protected_instruction_reason(filepath: str, task_id: str = "default",
    ...
    for candidate in (normalized, resolved):
        ...
        if len(parts) >= 2 and parts[-2] == ".socis-agent":
            return candidate
    return None
...
def _get_container_mirror_prefix_for_task(task_id: str = "default") -> str | None:
    """Return the container-side SOCIS mirror prefix for Docker file tools."""
    try:
        from tools.terminal_tool import (
            _active_environments,
            _env_lock,
...
def _check_cross_profile_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    container's sandbox mirror of SOCIS state.

    Two detectors (both #32049): these catch writes that would be
    SILENTLY LOST — the host SOCIS process never reads the mirror, so
    ...
    Returns ``None`` when the write is in-scope or outside SOCIS scope.
    """
    try:
...
def read_file_tool(path: str, offset: int = 1, limit: int = 2000, task_id: str = "default") -> str:
    ...
    try:
        ...
        # ── SOCIS internal path guard ────────────────────────────────
        # Prevent prompt injection via catalog or hub metadata files,
        # and block credential stores under SOCIS_AGENT_HOME.  Pass the
        # already-resolved path so a relative-path read against
        # TERMINAL_CWD == SOCIS_AGENT_HOME (e.g. "auth.json") still hits the
...
def _handle_write_file(args, **kw):
    ...
    if "content" not in args:
        return tool_error(
            ...
            "payload, or use execute_code with socis_tools.write_file() for very "
            "large files."
        )
