#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the SOCIS Skills Hub.
...
Used by socis_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
def _skills_dir() -> Path:
    ...
    return Path(forced) if forced is not None else _socis_agent_home() / "skills"
...
_DYNAMIC_PATH_RESOLVERS = {
    "SOCIS_AGENT_HOME": _socis_agent_home,
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            socis_meta = metadata.get("socis", {})
            if isinstance(socis_meta, dict):
                tags = socis_meta.get("tags", [])
        if not tags:
...
class OptionalSkillSource(SkillSource):
    ...
    These skills are official (maintained by SOCIS) but not activated
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        # upstream project maintains (e.g. impeccable's SOCIS-native bundle
        # under pbakaus/impeccable:.socis-agent/skills/impeccable). Install pulls
    ...
    def _fetch_from_live_repo(self, rel: str) -> Optional[SkillBundle]:
        ...
        ``socis update``. Rather than telling them to update first, resolve
    ...
    def _upstream_pointer(self, skill_dir: Path) -> Optional[Dict[str, str]]:
        ...
        A stub declares ``metadata.socis.upstream`` in its SKILL.md
        frontmatter:
            ...
            metadata:
              socis:
                upstream:
                  ...
                  path: .socis-agent/skills/impeccable
    ...
    def _upstream_pointer_from_content(self, content: Union[str, bytes]) -> Optional[Dict[str, str]]:
        """Parse ``metadata.socis.upstream`` out of SKILL.md content."""
        ...
        if not isinstance(meta_block, dict):
            return None
        socis_meta = meta_block.get("socis")
        if not isinstance(socis_meta, dict):
            return None
        upstream = socis_meta.get("upstream")
...
def _category_skill_dirs(directory: Path) -> List[str]:
    ...
    ``socis_cli.skills_hub._existing_categories``.
    """
    skill_dirs: List[str] = []
...
# ---------------------------------------------------------------------------
# SOCIS centralized index source
# ---------------------------------------------------------------------------

SOCIS_AGENT_INDEX_URL = "https://agent.socis.io/docs/api/skills-index.json"
SOCIS_AGENT_INDEX_TTL = 6 * 3600  # 6 hours


def _socis_index_cache_file() -> Path:
    return _index_cache_dir() / "socis-index.json"


def _load_socis_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for SOCIS_AGENT_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    socis_index_cache_file = _socis_index_cache_file()
    if socis_index_cache_file.exists():
        try:
            age = time.time() - socis_index_cache_file.stat().st_mtime
            if age < SOCIS_AGENT_INDEX_TTL:
                return json.loads(socis_index_cache_file.read_text(encoding="utf-8"))
        except (OSError, json.JSONDecodeError):
            pass
    ...
    for accept_encoding in ("gzip, deflate", "identity"):
        try:
            resp = httpx.get(
                SOCIS_AGENT_INDEX_URL,
                timeout=15,
                follow_redirects=True,
                headers={"Accept-Encoding": accept_encoding},
            )
            if resp.status_code != 200:
                logger.debug("SOCIS index fetch returned %d", resp.status_code)
            ...
            logger.debug(
                "SOCIS index decode failed (Accept-Encoding=%s): %s",
                accept_encoding,
                e,
            )
            continue
        except (httpx.HTTPError, json.JSONDecodeError) as e:
            logger.debug("SOCIS index fetch failed: %s", e)
            return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        socis_index_cache_file.parent.mkdir(parents=True, exist_ok=True)
        socis_index_cache_file.write_text(json.dumps(data), encoding="utf-8")
    except OSError:
        pass
...
class SOCISIndexSource(SkillSource):
    """Skill source backed by the centralized SOCIS Skills Index.
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "socis-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "socis-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        SOCISIndexSource(auth=auth), # Centralized index (search + resolved install paths)
