...
def _voice_capture_install_hint() -> str:
    ...
    # If we're running inside a venv (e.g. the bundled SOCIS venv at
    # ~/.socis-agent/profiles/<name>/socis-agent/venv/), `pip install` on the
...
_TEMP_DIR = os.path.join(tempfile.gettempdir(), "socis_voice")
...
def _get_beep_volume() -> float:
    ...
    system can't be imported (e.g. broken ~/.socis-agent/config.yaml during a
...
class AudioRecorder:
    ...
    def _max_duration_reached(self, elapsed: float) -> bool:
        ...
        recording (see ``SOCISCLI._voice_start_recording``). A value <= 0
...
# playback-phase barge transcript is treated as a self-capture of SOCIS'
...
def is_tts_echo(
    ...
    SOCIS just spoke using a character-level similarity ratio, which works
    across languages without word-tokenization. A genuine user interjection
    is very unlikely to closely match SOCIS' own words, so a high ratio is
...
def _play_audio_file_impl(file_path: str) -> bool:
    ...
    # A unique suffix prevents concurrent SOCIS TTS calls from colliding on
        ...
        if _is_wsl2_env():
            try:
                ...
                if _win_tmp_wsl:
                    ...
                    _wsl_wav = os.path.join(_win_tmp_wsl, f"socis-tts-{_unique}.wav")
...
def _voice_debug_enabled() -> bool:
    return os.environ.get("SOCIS_AGENT_VOICE_DEBUG", "").strip() == "1"


def _vad_log(msg: str) -> None:
    """VAD decision-point diagnostic — always logger.debug, plus stderr when
    SOCIS_AGENT_VOICE_DEBUG=1 so live hardware tuning doesn't need a log tail."""
    logger.debug(msg)
