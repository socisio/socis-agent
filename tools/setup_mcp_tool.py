...
``socis mcp install <name>`` in the terminal.
"""
...
def setup_mcp_tool(
    ...
    if callback is None:
        return tool_error(
            "setup_mcp is only available in the SOCIS desktop app. Use the "
            "terminal instead: `socis mcp install <name>` for catalog entries, "
            "`socis mcp login <name>` for OAuth."
        )
...
SETUP_MCP_SCHEMA = {
    ...
    "description": (
        ...
        "unanswered, continue without it. Catalog names: `socis mcp "
