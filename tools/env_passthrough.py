...
def _is_socis_provider_credential(name: str) -> bool:
    """True if ``name`` is a SOCIS-managed provider credential (API key,
    token, or similar) per ``_SOCIS_AGENT_PROVIDER_ENV_BLOCKLIST``.
    ...
    Non-SOCIS API keys (TENOR_API_KEY, NOTION_TOKEN, etc.) are NOT
    ...
    let a skill tunnel a SOCIS credential into the execute_code child.
    """
    try:
        ...
        return True
    # Dynamically-generated SOCIS-internal secrets (AUXILIARY_*_API_KEY /
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    ...
    Variables that are SOCIS-managed provider credentials (from
    ``_SOCIS_AGENT_PROVIDER_ENV_BLOCKLIST``) are rejected here to preserve
    the ``execute_code`` sandbox's credential-scrubbing guarantee per
    GHSA-rhgp-j443-p4rf. A skill that needs to talk to a SOCIS-managed
    ...
    Non-SOCIS third-party API keys (TENOR_API_KEY, NOTION_TOKEN, etc.)
    ...
    for name in var_names:
        ...
        if not name:
            continue
        if _is_socis_provider_credential(name):
            logger.warning(
                "env passthrough: refusing to register SOCIS provider "
                "credential %r (blocked by _SOCIS_AGENT_PROVIDER_ENV_BLOCKLIST). "
    ...
    try:
        ...
        if isinstance(passthrough, list):
            for item in passthrough:
                ...
                # SOCIS-managed provider credentials must not be passed
                ...
                if _is_socis_provider_credential(name):
                    logger.warning(
                        "env passthrough: refusing to register SOCIS "
                        "provider credential %r from config.yaml (blocked "
                        "by _SOCIS_AGENT_PROVIDER_ENV_BLOCKLIST). Operator "
