...
_DEFAULT_TOOL_GATEWAY_DOMAIN = "socis.io"
...
def auth_json_path():
    """Return the SOCIS auth store path, respecting SOCIS_AGENT_HOME overrides."""
    return get_socis_agent_home() / "auth.json"
...
def peek_nous_access_token() -> Optional[str]:
    ...
    Availability scans (`socis tools`, banner/status paint, provider
...
# managed vendor's gateway URL is pinned: adding one is a SOCIS release, and
