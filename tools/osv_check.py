...
# repeated `socis mcp test` invocations re-run the preflight for the SAME
...
#
# The cache is also persisted to disk inside the SOCIS home so that separate
# `socis mcp test` processes (and gateway restarts) reuse a warm verdict
...
def _disk_cache_path() -> Optional[Path]:
    ...
    Uses ``socis_agent_constants.get_socis_agent_home()`` so the cache follows the
    active profile and is isolated across SOCIS homes. The cache directory
    is created on demand. Returns ``None`` when SOCIS home cannot be
...
def _query_osv(
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "socis-agent-osv-check/1.0",
        },
        method="POST",
    )
