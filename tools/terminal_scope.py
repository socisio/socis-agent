...
several SOCIS profiles from one process. Terminal settings were historically
...
_terminal_scope_var: ContextVar = ContextVar("socis_terminal_scope", default=None)
...
def build_profile_terminal_scope(socis_agent_home: "Any") -> Dict[str, str]:
    ...
    defaults.setdefault("sandbox_dir", "")    # tool derives SOCIS_AGENT_HOME path
    ...
    def _apply(cfg_key: str, value: Any) -> None:
        ...
        if cfg_key == "cwd" and str(value).strip() in {".", "auto", "cwd"}:
            return
        from socis_cli.config import TERMINAL_CONFIG_ENV_MAP
    ...
    #    SOCIS_AGENT_HOME override so the profile's own file is consulted; a
    ...
    override_token = None
    if get_socis_agent_home_override() != str(home):
        override_token = set_socis_agent_home_override(home)
    try:
        ...
        if config_path.exists():
            ...
            from socis_cli.config import fast_safe_load
        ...
        if override_token is not None:
            reset_socis_agent_home_override(override_token)

    return scope


def install_profile_terminal_scope(socis_agent_home: "Any") -> Token:
    ...
    """
    try:
        return set_terminal_scope(build_profile_terminal_scope(socis_agent_home))
...
def install_and_reset_profile_terminal_scope(
    ...
    """
    token = install_profile_terminal_scope(socis_agent_home)
    try:
        yield
    finally:
        reset_terminal_scope(token)
