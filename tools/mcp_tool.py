...
transport, discovers their tools, and registers them into the socis-agent
tool registry so the agent can call them like any built-in tool.

Configuration is read from ~/.socis-agent/config.yaml under the ``mcp_servers`` key.
...
# per-profile log file (~/.socis-agent/logs/mcp-stderr.log), tagged with the
...
def sdk_httpx():
    ...
    side by side with SOCIS' own pinned ``httpx``. Every object that crosses
...
def _check_logging_callback_support() -> bool:
    ...
    server emits, so server-side diagnostics never reach SOCIS' logs.
    """
    if not _MCP_AVAILABLE:
        return False
    try:
...
def _build_safe_env(user_env: Optional[dict]) -> dict:
    ...
    SOCIS explicitly tagged during dotenv loading, plus any variables
    ...
    an exception: users configured that backend specifically so SOCIS and
...
def _resolve_stdio_command(command: str, env: dict) -> tuple[str, dict]:
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            socis_agent_home = os.path.expanduser(
                os.getenv(
                    "SOCIS_AGENT_HOME", os.path.join(os.path.expanduser("~"), ".socis-agent")
                )
            )
            candidates = [
                os.path.join(socis_agent_home, "node", "bin", resolved_command),
                ...
                # image (which the SOCIS Docker image copies node + npm +
...
def _npx_bin_candidates(bin_dir: str, name: str, *, windows: Optional[bool] = None) -> list:
    ...
    the same precedence ``socis_agent_constants._candidate_node_command_names``
...
def _npx_cached_bin(args: list) -> Optional[tuple]:
    ...
    server — and it buys nothing here, because SOCIS already supervises the
...
# ---------------------------------------------------------------------------
# If this SOCIS process dies without running its cleanup path (kill -9, OOM,
...
# ---------------------------------------------------------------------------
# MCP ImageContent block → SOCIS MEDIA tag
# ---------------------------------------------------------------------------
...
def _cache_mcp_image_block(block) -> str:
    ...
    return a ``MEDIA:<path>`` tag that SOCIS gateways know how to render.
...
def _render_mcp_resource_block(block, server_name: str = "") -> str:
    ...
    - ``EmbeddedResource`` with blob contents → bytes are decoded (size-capped)
      and materialized into the SOCIS document cache; returns a marker with
...
def _unwrap_exception_group(exc: BaseException) -> BaseException:
    ...
    Adapted from :func:`socis_cli.mcp_config._unwrap_exception_group` with
...
def _resolve_identity_header(server_name: str, config: dict):
    ...
    mode resolves the value to the active SOCIS profile name once at
...
class ElicitationHandler:
    ...
    Form-mode elicitations are routed through SOCIS' existing approval
    ...
    async def __call__(self, context, params):
        ...
        # the agent's contextvars (SOCIS_AGENT_SESSION_PLATFORM etc.). When
...
class MCPServerTask:
    ...
    def __init__(self, name: str):
        ...
        # context doesn't inherit SOCIS_AGENT_SESSION_PLATFORM, so the
    ...
    def _make_logging_callback(self):
        ...
        server into SOCIS' logging (agent.log via socis_agent_logging), tagged
    ...
    async def _run_stdio(self, config: dict):
        ...
        if not _ensure_mcp_sdk():
            raise ImportError(
                ...
                "it is not installed. Run `socis setup` to install MCP support, "
                "then retry."
            )
        ...
        # SOCIS already supervises the child (shared death supervisor), so
        ...
        # ~/.socis-agent/logs/mcp-stderr.log.
        ...
        try:
            async with stdio_client(server_params, errlog=_errlog) as (
                ...
                if new_pids:
                    ...
                    for _pid in new_pids:
                        try:
                            from socis_cli.process_identity import register_child
    ...
    async def run(self, config: dict):
        ...
        # routes those requests through SOCIS' approval system.
        ...
        while True:
            try:
                ...
                # and the MCP server would stay dead until SOCIS is fully
                ...
                if not self._ever_connected:
                    if failure_class == "permanent":
                        ...
                        # re-authenticated with ``socis mcp login``. Parking
...
def _http_status_error_types() -> tuple:
    ...
    mcp >= 2.0) or by SOCIS' pinned ``httpx``, and the two define unrelated
...
def _handle_auth_error_and_retry(
    ...
    return tool_error(
        ...
        f"Run `socis mcp login {server_name}` (or delete the tokens "
        f"file under ~/.socis-agent/mcp-tokens/ and restart). Do NOT retry "
...
# ---------------------------------------------------------------------------
# Advisory file lock that prevents N concurrent SOCIS processes (e.g.
    ...
    try:
        ...
        if _MCP_DISCOVERY_LOCK_PATH is None:
            _MCP_DISCOVERY_LOCK_PATH = str(
                get_socis_agent_home() / ".mcp-discovery.lock"
            )
...
def _wrap_with_home_override(coro: "Coroutine") -> "Coroutine":
    """Carry the caller's context-local SOCIS_AGENT_HOME override into ``coro``.
    ...
    try:
        ...
        home_override = get_socis_agent_home_override()
    except Exception:
        return coro
    ...
    async def _scoped():
        token = set_socis_agent_home_override(home_override)
        try:
            return await coro
        finally:
...
def _run_on_mcp_loop(coro_or_factory, timeout: float = 30):
    ...
    # Propagate the context-local SOCIS_AGENT_HOME override onto the MCP loop.
    ...
    # vanish here: OAuth token stores and any other get_socis_agent_home()
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the SOCIS config file.
    ...
    ``os.environ`` (which includes ``~/.socis-agent/.env`` loaded at startup).
    """
    try:
...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        async def _call():
            ...
            # flow through SOCIS' MEDIA: tag convention and out to messaging
            ...
            # SOCIS' MEDIA tag + cache_image_from_bytes) was the cleaner of
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves SOCIS's historical behavior of converting hyphens to
...
# Native MCP tool-name prefix. SOCIS uses the ``mcp__<server>__<tool>``
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the SOCIS registry schema format.
...
def discover_mcp_tools(allowed_mcp_names: Optional[List[str]] = None) -> List[str]:
    ...
    Args:
        allowed_mcp_names: If provided, only spawn MCP servers whose names
            ...
            This is used by ``socis -z -t <toolsets>`` to skip cold-starting
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``socis tools`` interactive configuration — connects to each
    enabled server, grabs tool names and descriptions, then disconnects.
    Does NOT register tools in the SOCIS registry.
...
def _kill_orphaned_mcp_children(
    ...
    survivors, avoiding shared-resource collisions when multiple socis
