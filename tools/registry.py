"""Central registry for all socis-agent tools.
...
def _discovery_cache_path() -> Optional[Path]:
    ...
    try:
        ...
        # import time (socis_agent_constants itself is stdlib-only, so no cycle).
        from socis_agent_constants import get_socis_agent_home

        return Path(get_socis_agent_home()) / "cache" / "tool_discovery_cache.json"
    except Exception:
        return None
...
# timescales. Cache results for ~30 s so env-var flips via ``socis tools``
...
def check_fn_cache_scope() -> Optional[str]:
    ...
    cache. A multiplex gateway installs a SOCIS-home override for every
    ...
    try:
        ...
        browser_identity = (
            get_session_env("SOCIS_AGENT_SESSION_ID", ""),
            get_session_env("SOCIS_AGENT_BROWSER_CONTROL_PRINCIPAL", ""),
            get_session_env("SOCIS_AGENT_BROWSER_CONTROL_TRANSPORT_FAMILY", ""),
        )
...
def invalidate_check_fn_cache() -> None:
    ...
    affect tool availability (e.g. ``socis tools enable``)."""
    with _check_fn_cache_lock:
...
class ToolRegistry:
    ...
    def __init__(self):
        ...
        # Plugin registrations are overlays keyed by resolved SOCIS_AGENT_HOME. A
    ...
    def _plugin_namespace_of_module(
        ...
        if module_namespace.startswith("socis_plugins."):
    ...
    def deregister(self, name: str, *, scope: Optional[str] = None) -> None:
        ...
        with self._lock:
            ...
            if not entry.toolset.startswith("mcp-"):
                ...
                # (``socis_plugins.{name}``), not the exact module string.
                # A handler defined in ``socis_plugins.pkg.handlers`` is
                # still owned by the ``socis_plugins.pkg`` package — exact
    ...
    def get_definitions(self, tool_names: Set[str], quiet: bool = False) -> List[dict]:
        ...
        etc.); TTL chosen so env-var changes (``socis tools enable foo``)
