...
# SOCIS WebUI, NAS "update" flows that preserve an old entrypoint, etc.)
...
#
# Test hook: SOCIS_AGENT_TINI_SHIM_TARGET overrides the `/init` path (and
# SOCIS_AGENT_TINI_SHIM_WRAPPER the main-wrapper path) so unit tests can
# record argv without a real s6 tree.

set -e

INIT_TARGET="${SOCIS_AGENT_TINI_SHIM_TARGET:-/init}"
WRAPPER="${SOCIS_AGENT_TINI_SHIM_WRAPPER:-/opt/socis/docker/main-wrapper.sh}"
