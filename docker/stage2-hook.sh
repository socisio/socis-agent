...
#
# Wired into the image as /etc/cont-init.d/01-socis-setup by the
...
SOCIS_AGENT_HOME="${SOCIS_AGENT_HOME:-/opt/data}"
INSTALL_DIR="/opt/socis"

# Drop to socis via s6-setuidgid, but skip it when already non-root.
as_socis() { [ "$(id -u)" = 0 ] || { "$@"; return; }; s6-setuidgid socis "$@"; }
...
# starts non-root. The baked install tree under /opt/socis is intentionally
# root-owned and non-writable; mutable runtime state must live under
# $SOCIS_AGENT_HOME. An arbitrary `--user` UID therefore cannot repair or populate
...
# default) and pass SOCIS_AGENT_UID/SOCIS_AGENT_GID — or the PUID/PGID aliases — which the
...
# `id -u socis` is the unremapped build UID (10000) because no root-only remap
# could run. root starts (id -u = 0) and the normal supervised drop to the
# socis UID are both unaffected.
cur_uid="$(id -u)"
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u socis)" ]; then
    cat >&2 <<EOF
[stage2] ERROR: container started with --user $cur_uid (an arbitrary, non-socis UID).
...
and the baked /opt/socis install tree is intentionally root-owned and
...
Start the container as root (the default) and pass your host UID/GID instead:
    ...
    docker run -e SOCIS_AGENT_UID=\$(id -u) -e SOCIS_AGENT_GID=\$(id -g) ...

NAS users (Synology / unRAID / UGOS) can use the PUID/PGID aliases:

    docker run -e PUID=\$(id -u) -e PGID=\$(id -g) ...

The image remaps the socis user to that UID/GID at boot and chowns the data
...
# --- Bootstrap SOCIS_AGENT_HOME as root ---
...
# --- UID/GID remap ---
# Accept PUID/PGID as aliases for SOCIS_AGENT_UID/SOCIS_AGENT_GID.  NAS users (UGOS,
...
# UID 10000 leaves the runtime unable to read the volume.  SOCIS_AGENT_UID/
# SOCIS_AGENT_GID still win when both are set.  See #15290, salvages #25872.
SOCIS_AGENT_UID="${SOCIS_AGENT_UID:-${PUID:-}}"
SOCIS_AGENT_GID="${SOCIS_AGENT_GID:-${PGID:-}}"

if [ -n "${SOCIS_AGENT_UID:-}" ] && validate_uid_gid "$SOCIS_AGENT_UID" && [ "$SOCIS_AGENT_UID" != "$(id -u socis)" ]; then
    echo "[stage2] Changing socis UID to $SOCIS_AGENT_UID"
    usermod -u "$SOCIS_AGENT_UID" socis
fi
if [ -n "${SOCIS_AGENT_GID:-}" ] && validate_uid_gid "$SOCIS_AGENT_GID" && [ "$SOCIS_AGENT_GID" != "$(id -g socis)" ]; then
    echo "[stage2] Changing socis GID to $SOCIS_AGENT_GID"
    # -o allows non-unique GID (e.g. macOS GID 20 "staff" may already
    # exist as "dialout" in the Debian-based container image).
    groupmod -o -g "$SOCIS_AGENT_GID" socis 2>/dev/null || true
fi
...
# host's `docker` group (or root). The supervised socis user (UID 10000)
...
# socis process with `Groups: 10000` (998 gone); after this hook adds
...
# matching entry that includes socis. Idempotent across container
...
#   - socis is already a member of the right group (idempotent restart)
...
for sock in /var/run/docker.sock /run/docker.sock; do
    ...
    if id -G socis 2>/dev/null | tr ' ' '\n' | grep -qx "$sock_gid"; then
        echo "[stage2] socis already in group $sock_gid for $sock"
        break
    fi
    ...
    if usermod -aG "$sock_group" socis 2>/dev/null; then
        echo "[stage2] Added socis to group $sock_group (GID $sock_gid) for $sock"
    else
        echo "[stage2] Warning: usermod -aG $sock_group socis failed; docker backend may fail with EACCES"
    fi
    break
done

# --- Fix ownership of data volume ---
# When SOCIS_AGENT_UID is remapped or the top-level $SOCIS_AGENT_HOME isn't owned by
# the runtime socis UID, restore ownership to socis — but ONLY for the
# directories socis actually writes to. The full $SOCIS_AGENT_HOME may be a
# host-mounted bind containing unrelated user files; `chown -R` would
# silently destroy host ownership of those (see issue #19788).
#
# The canonical list of socis-owned subdirs is the same one the s6-setuidgid
# mkdir -p block below seeds. Keep them in sync if the seed list changes.
actual_socis_uid=$(id -u socis)

path_has_symlink_component() {
    path="$1"
    root="${2:-$SOCIS_AGENT_HOME}"
...
chown_socis_tree() {
    ...
    chown -R socis:socis "$target" 2>/dev/null || \
        echo "[stage2] Warning: chown $target failed (rootless container?) — continuing"
}

tree_has_non_socis_owner() {
    target="$1"
    find "$target" \( ! -user socis -o ! -group socis \) -print -quit 2>/dev/null | grep -q .
}

needs_chown=false
if [ "$(stat -c %u "$SOCIS_AGENT_HOME" 2>/dev/null)" != "$actual_socis_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $SOCIS_AGENT_HOME (targeted) to socis ($actual_socis_uid)"
    # In rootless Podman the container's "root" is mapped to an
    # unprivileged host UID — chown will fail. That's fine: the volume
    # is already owned by the mapped user on the host side.
    #
    # Top-level $SOCIS_AGENT_HOME: chown the directory itself (not its contents)
    # so socis can mkdir new subdirs but bind-mounted host files keep
    # their existing ownership.
    if refuse_symlinked_path "chown" "$SOCIS_AGENT_HOME"; then
        :
    else
        chown socis:socis "$SOCIS_AGENT_HOME" 2>/dev/null || \
            echo "[stage2] Warning: chown $SOCIS_AGENT_HOME failed (rootless container?) — continuing"
    fi
    # SOCIS-owned subdirs: recursive chown is safe here because these are
    # created and managed exclusively by socis (see the s6-setuidgid mkdir
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing lazy-packages; do
        if [ -e "$SOCIS_AGENT_HOME/$sub" ] && tree_has_non_socis_owner "$SOCIS_AGENT_HOME/$sub"; then
            chown_socis_tree "$SOCIS_AGENT_HOME/$sub"
        fi
    done
fi
...
# socis user. Hosted/container instances keep mutable state under
# $SOCIS_AGENT_HOME (/opt/data) and run with PYTHONDONTWRITEBYTECODE plus
# SOCIS_AGENT_DISABLE_LAZY_INSTALLS=1. Keeping /opt/socis root-owned and
...
# $SOCIS_AGENT_HOME/lazy-packages dir on the data volume (Dockerfile sets
# SOCIS_AGENT_LAZY_INSTALL_TARGET). That dir is appended to the END of sys.path,
...
# even though installs are re-enabled. The dir is seeded + chowned to socis
...
# Always reset ownership of $SOCIS_AGENT_HOME/profiles to socis on every
# boot. Profile dirs and files can land owned by root when commands
# are invoked via `docker exec <container> socis …` (which defaults
# to root unless `-u` is passed), and that breaks the cont-init
# reconciler (02-reconcile-profiles) which runs as socis and walks
...
if [ -d "$SOCIS_AGENT_HOME/profiles" ] && tree_has_non_socis_owner "$SOCIS_AGENT_HOME/profiles"; then
    chown_socis_tree "$SOCIS_AGENT_HOME/profiles"
fi

# Always reset ownership of $SOCIS_AGENT_HOME/cron on every boot for the same
# docker-exec/root-write reason as profiles/. The cron scheduler state
# (jobs.json) must stay readable by the unprivileged socis runtime even
...
# gate as profiles/).
if [ -d "$SOCIS_AGENT_HOME/cron" ] && tree_has_non_socis_owner "$SOCIS_AGENT_HOME/cron"; then
    chown_socis_tree "$SOCIS_AGENT_HOME/cron"
fi

# Always ensure logs/gateways is socis-owned (#45258). Formerly healed by
...
# top-level $SOCIS_AGENT_HOME is mis-owned, so a warm volume with socis-owned
# SOCIS_AGENT_HOME but root-owned logs/gateways would otherwise leave
# s6-setuidgid socis mkdir failing with Permission denied. Non-recursive:
# profile leaf dirs are each created/owned by their own log/run as socis.
if [ -d "$SOCIS_AGENT_HOME/logs/gateways" ]; then
    if refuse_symlinked_path "chown" "$SOCIS_AGENT_HOME/logs/gateways"; then
        :
    else
        chown socis:socis "$SOCIS_AGENT_HOME/logs/gateways" 2>/dev/null || true
    fi
fi
...
# socis pairing approve …` defaults to uid=0 and writes 0600 root-owned
# approval files that the unprivileged socis gateway cannot read,
# silently leaving the approved user unauthorized (#10270). The targeted
# data-volume chown above only runs when the top-level $SOCIS_AGENT_HOME is
...
# and cron/.
if [ -d "$SOCIS_AGENT_HOME/platforms/pairing" ] && tree_has_non_socis_owner "$SOCIS_AGENT_HOME/platforms/pairing"; then
    chown_socis_tree "$SOCIS_AGENT_HOME/platforms/pairing"
fi
# Legacy location (pre-consolidated layout).
if [ -d "$SOCIS_AGENT_HOME/pairing" ] && tree_has_non_socis_owner "$SOCIS_AGENT_HOME/pairing"; then
    chown_socis_tree "$SOCIS_AGENT_HOME/pairing"
fi

# Reset ownership of socis-owned top-level state files on every boot.
# The targeted data-volume chown above only covers socis-owned
# *subdirectories*; loose state files living directly under $SOCIS_AGENT_HOME
# are missed. When those files are created or rewritten by
# `docker exec <container> socis …` (root unless `-u` is passed) they
# land root-owned, and the unprivileged socis runtime then hits
...
# sweep so host-owned files in a bind-mounted $SOCIS_AGENT_HOME are never
...
# entries of socis_cli.profile_distribution.USER_OWNED_EXCLUDE plus the
# runtime lock files; keep them in sync if that set changes.
for f in \
    auth.json auth.lock .env \
    state.db state.db-shm state.db-wal \
    socis_agent_state.db \
    response_store.db response_store.db-shm response_store.db-wal \
    gateway.pid gateway.lock gateway_state.json processes.json \
    active_profile; do
    if [ -e "$SOCIS_AGENT_HOME/$f" ]; then
        if refuse_symlinked_path "chown" "$SOCIS_AGENT_HOME/$f"; then
            :
        else
            chown socis:socis "$SOCIS_AGENT_HOME/$f" 2>/dev/null || true
        fi
    fi
done

# --- config.yaml permissions ---
# Ensure config.yaml is readable by the socis runtime user even if it
# was edited on the host after initial ownership setup.
if [ -f "$SOCIS_AGENT_HOME/config.yaml" ]; then
    if refuse_symlinked_path "chown/chmod" "$SOCIS_AGENT_HOME/config.yaml"; then
        :
    else
        chown socis:socis "$SOCIS_AGENT_HOME/config.yaml" 2>/dev/null || true
        chmod 640 "$SOCIS_AGENT_HOME/config.yaml" 2>/dev/null || true
    fi
fi

# --- Seed directory structure as socis user ---
# Run as socis via s6-setuidgid so dirs end up owned correctly (matters
# under rootless Podman where chown back to root would fail).
#
# Use direct `mkdir -p` invocation (no `sh -c "..."` wrapper) so the
# shell isn't a second interpreter — defends against $SOCIS_AGENT_HOME values
# containing shell metacharacters. PR #30136 review item O2.
as_socis mkdir -p \
    "$SOCIS_AGENT_HOME/backups" \
    "$SOCIS_AGENT_HOME/cron" \
    "$SOCIS_AGENT_HOME/sessions" \
    "$SOCIS_AGENT_HOME/logs" \
    "$SOCIS_AGENT_HOME/logs/gateways" \
    "$SOCIS_AGENT_HOME/hooks" \
    "$SOCIS_AGENT_HOME/memories" \
    "$SOCIS_AGENT_HOME/skills" \
    "$SOCIS_AGENT_HOME/skins" \
    "$SOCIS_AGENT_HOME/plans" \
    "$SOCIS_AGENT_HOME/workspace" \
    "$SOCIS_AGENT_HOME/home" \
    "$SOCIS_AGENT_HOME/pairing" \
    "$SOCIS_AGENT_HOME/platforms/pairing" \
    "$SOCIS_AGENT_HOME/lazy-packages"

# --- Install-method stamp ---
# The 'docker' stamp is baked into the immutable install tree at
# /opt/socis/.install_method (see Dockerfile), NOT written here into
# $SOCIS_AGENT_HOME. detect_install_method() reads the code-scoped stamp first.
#
# Why we no longer stamp $SOCIS_AGENT_HOME: it is a shared DATA volume, commonly
# bind-mounted from the host (~/.socis-agent:/opt/data) and sometimes shared with a
...
# 'socis update'. To heal homes already poisoned by older images, remove a
# stale 'docker' stamp from $SOCIS_AGENT_HOME if one is present (the host install's
# own installer re-creates its code-scoped stamp; a genuine container relies on
# the baked /opt/socis stamp, so deleting the data-dir copy is safe).
if [ -f "$SOCIS_AGENT_HOME/.install_method" ]; then
    stamped="$(tr -d '[:space:]' < "$SOCIS_AGENT_HOME/.install_method" 2>/dev/null || true)"
    if [ "$stamped" = "docker" ]; then
        rm -f "$SOCIS_AGENT_HOME/.install_method" 2>/dev/null || true
    fi
fi
...
seed_one() {
    dest=$1
    src=$2
    if [ ! -f "$SOCIS_AGENT_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        if refuse_symlinked_path "seed" "$SOCIS_AGENT_HOME/$dest"; then
            :
        else
            as_socis cp "$INSTALL_DIR/$src" "$SOCIS_AGENT_HOME/$dest"
        fi
    fi
}
...
# the first-boot seed above depends on /opt/socis/.env.example being present
...
# not generate one. SOCIS loads $SOCIS_AGENT_HOME/.env with override=True, so
...
if [ -n "${API_SERVER_KEY:-}" ]; then
    if [ -f "$SOCIS_AGENT_HOME/.env" ] && grep -q '^API_SERVER_KEY=..*' "$SOCIS_AGENT_HOME/.env" 2>/dev/null; then
        echo "[stage2] Warning: API_SERVER_KEY is set in both the container environment and $SOCIS_AGENT_HOME/.env — the .env value wins at runtime (loaded with override=True)"
    else
        ...
        if [ -f "$SOCIS_AGENT_HOME/.env" ] && ! refuse_symlinked_path "clean" "$SOCIS_AGENT_HOME/.env"; then
            sed -i '/^API_SERVER_KEY=$/d' "$SOCIS_AGENT_HOME/.env" 2>/dev/null || true
        fi
        echo "[stage2] API_SERVER_KEY provided via container environment — skipping generation"
    fi
elif ! grep -q '^API_SERVER_KEY=..*' "$SOCIS_AGENT_HOME/.env" 2>/dev/null; then
    if refuse_symlinked_path "append" "$SOCIS_AGENT_HOME/.env"; then
        :
    else
        if [ ! -f "$SOCIS_AGENT_HOME/.env" ]; then
            ...
            (umask 077 && as_socis touch "$SOCIS_AGENT_HOME/.env") 2>/dev/null || true
        fi
        if [ -f "$SOCIS_AGENT_HOME/.env" ]; then
            ...
            if [ -n "$_gen_key" ]; then
                ...
                sed -i '/^API_SERVER_KEY=$/d' "$SOCIS_AGENT_HOME/.env" 2>/dev/null || true
                if printf 'API_SERVER_KEY=%s\n' "$_gen_key" >> "$SOCIS_AGENT_HOME/.env" 2>/dev/null; then
                    echo "[stage2] Generated API_SERVER_KEY for the loopback gateway api_server"
                else
                    echo "[stage2] Warning: could not write API_SERVER_KEY to $SOCIS_AGENT_HOME/.env (read-only volume?) — gateway api_server (cron fires) will be unavailable"
                fi
            fi
            unset _gen_key
        else
            echo "[stage2] Warning: could not create $SOCIS_AGENT_HOME/.env — gateway api_server (cron fires) will be unavailable"
        fi
    fi
fi
...
if [ -f "$SOCIS_AGENT_HOME/.env" ]; then
    if refuse_symlinked_path "chown/chmod" "$SOCIS_AGENT_HOME/.env"; then
        :
    else
        chown socis:socis "$SOCIS_AGENT_HOME/.env" 2>/dev/null || true
        chmod 600 "$SOCIS_AGENT_HOME/.env" 2>/dev/null || true
    fi
fi
...
# as the unprivileged `socis` user — without this it gets EACCES on every
...
# socis group (any group member could e.g. stop/suspend this machine), not
...
if [ -S /.fly/api ]; then
    if refuse_symlinked_path "chgrp/chmod" /.fly/api; then
        :
    elif chgrp socis /.fly/api 2>/dev/null && chmod g+w /.fly/api 2>/dev/null; then
        echo "[stage2] Granted socis group access to the Fly Machines API socket"
    else
...
# $SOCIS_AGENT_HOME on the mounted volume. Run the same safe, non-interactive
# config-schema migrations that `socis update` runs for non-Docker installs,
# after first-boot seeding and before supervised gateway services start.
# Set SOCIS_AGENT_SKIP_CONFIG_MIGRATION=1 for controlled/manual migrations.
if [ -f "$SOCIS_AGENT_HOME/config.yaml" ]; then
    s6-setuidgid socis "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/scripts/docker_config_migrate.py" \
...
if [ ! -f "$SOCIS_AGENT_HOME/auth.json" ] && [ -n "${SOCIS_AGENT_AUTH_JSON_BOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "seed" "$SOCIS_AGENT_HOME/auth.json"; then
        :
    else
        printf '%s' "$SOCIS_AGENT_AUTH_JSON_BOOTSTRAP" > "$SOCIS_AGENT_HOME/auth.json"
        chown socis:socis "$SOCIS_AGENT_HOME/auth.json" 2>/dev/null || true
        chmod 600 "$SOCIS_AGENT_HOME/auth.json"
    fi
fi
...
# supply a freshly-issued session via SOCIS_AGENT_AUTH_JSON_REBOOTSTRAP (distinct
...
if [ -f "$SOCIS_AGENT_HOME/auth.json" ] && [ -n "${SOCIS_AGENT_AUTH_JSON_REBOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "reseed" "$SOCIS_AGENT_HOME/auth.json"; then
        :
    else
        s6-setuidgid socis "$INSTALL_DIR/.venv/bin/python" \
            "$INSTALL_DIR/scripts/docker_rebootstrap_nous_session.py" \
            "$SOCIS_AGENT_HOME/auth.json" \
...
# can set SOCIS_AGENT_GATEWAY_BOOTSTRAP_STATE=running; we seed the state file
...
# SOCIS_AGENT_AUTH_JSON_BOOTSTRAP seeds auth.json. The [ ! -f ] guard is the
...
if [ ! -f "$SOCIS_AGENT_HOME/gateway_state.json" ] && \
        [ "${SOCIS_AGENT_GATEWAY_BOOTSTRAP_STATE:-}" = "running" ]; then
    if refuse_symlinked_path "seed" "$SOCIS_AGENT_HOME/gateway_state.json"; then
        :
    else
        printf '{"gateway_state":"running"}\n' > "$SOCIS_AGENT_HOME/gateway_state.json"
        chown socis:socis "$SOCIS_AGENT_HOME/gateway_state.json" 2>/dev/null || true
        chmod 644 "$SOCIS_AGENT_HOME/gateway_state.json"
    fi
fi
...
if [ -d "$INSTALL_DIR/skills" ]; then
    as_socis "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" \
        || echo "[stage2] Warning: skills_sync.py failed; continuing"
fi

# --- Discover agent-browser's Chromium binary ---
# The image's Dockerfile runs `npx playwright install chromium`, which
# populates ``$PLAYWRIGHT_BROWSERS_PATH`` (=/opt/socis/.playwright) with
# a ``chromium_headless_shell-<build>/chrome-headless-shell-linux64/``
# directory. agent-browser (the runtime CLI SOCIS spawns for the
...
# main-wrapper.sh propagates it into the supervised ``socis`` process
...
if [ -z "${AGENT_BROWSER_EXECUTABLE_PATH:-}" ] && \
    ...
    if [ -n "$browser_bin" ]; then
        ...
        # up for all supervised services (main-socis, dashboard, etc.).
