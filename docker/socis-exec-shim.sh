#!/bin/sh
# shellcheck shell=sh
# /opt/socis/bin/socis — `docker exec` privilege-drop shim.
#
# Background
# ----------
# The s6 image runs the supervised gateway/main process as the unprivileged
# `socis` user (UID 10000). When an operator runs `docker exec <c> socis ...`
# the default UID is root (0), and any file the command writes under
# $SOCIS_AGENT_HOME — auth.json, .env, config.yaml — ends up root-owned and
# unreadable to the supervised gateway. The most common manifestation: the
# user runs `docker exec <c> socis login`, this writes
# /opt/data/auth.json as root:root mode 0600, and from then on the gateway
# returns "Provider authentication failed: SOCIS is not logged into Nous
# Portal" on every incoming message — even though `docker exec <c> socis
...
# Fix
# ---
# This shim sits at /opt/socis/bin/socis and is placed earliest on PATH.
# When invoked as root, it drops to the socis user (via s6-setuidgid)
# before exec'ing the real venv binary, so anything that writes under
# $SOCIS_AGENT_HOME is uid-aligned with the supervised processes. When invoked
# as any non-root UID — including the supervised processes themselves,
# `docker exec --user socis`, kanban subagents, etc. — it short-circuits
...
# (/opt/socis/.venv/bin/socis), so the second hop cannot re-enter this
# shim regardless of PATH state. No sentinel env var needed.
#
# Opt-out: set SOCIS_AGENT_DOCKER_EXEC_AS_ROOT=1 (1/true/yes, case-insensitive)
...
# state via the socis CLI. Default is to drop.

set -e

REAL=/opt/socis/.venv/bin/socis

# Defensive: if the venv binary is missing (corrupted image, partial
# install), fail loudly rather than silently masking it.
if [ ! -x "$REAL" ]; then
    echo "socis-shim: $REAL not found or not executable" >&2
    exit 127
fi

# Already non-root? Just exec the real binary. This is the hot path for
# supervised processes (uid 10000) and for `docker exec --user socis`.
if [ "$(id -u)" != "0" ]; then
    exec "$REAL" "$@"
fi

# Root, with opt-out set? Honor it.
case "${SOCIS_AGENT_DOCKER_EXEC_AS_ROOT:-}" in
    1|true|TRUE|True|yes|YES|Yes)
        exec "$REAL" "$@"
        ;;
esac

# Root, no opt-out. Drop to the socis user.
#
...
if [ ! -x "$S6_SUID" ]; then
    ...
    echo "socis-shim: $S6_SUID not found; refusing to silently run as root." >&2
    echo "socis-shim: re-run with --user socis or set SOCIS_AGENT_DOCKER_EXEC_AS_ROOT=1." >&2
    exit 126
fi

# Reset HOME to the socis user's home before dropping privileges. Without
