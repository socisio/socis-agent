...
const BACKEND = process.env.SOCIS_AGENT_DASHBOARD_URL ?? "http://127.0.0.1:9119";

/**
 * In production the Python `socis dashboard` server injects a one-shot
 * session token into `index.html` (see `socis_cli/web_server.py`). The
 * Vite dev server serves its own `index.html`, so unless we forward that
 * token, every protected `/api/*` call 401s.
 *
 * This plugin fetches the running dashboard's `index.html` on each dev page
 * load, scrapes the `window.__SOCIS_AGENT_SESSION_TOKEN__` assignment, and
 * re-injects it into the dev HTML. No-op in production builds.
 */
function socisDevToken(): Plugin {
  const TOKEN_RE = /window\.__SOCIS_AGENT_SESSION_TOKEN__\s*=\s*"([^"]+)"/;
  const EMBEDDED_RE =
    /window\.__SOCIS_AGENT_DASHBOARD_EMBEDDED_CHAT__\s*=\s*(true|false)/;

  return {
    name: "socis:dev-session-token",
    apply: "serve",
    async transformIndexHtml() {
      try {
        ...
        if (!match) {
          console.warn(
            `[socis] Could not find session token in ${BACKEND} — ` +
              `is \`socis dashboard\` running? /api calls will 401.`,
          );
          return;
        }
        ...
        return [
          {
            ...
            injectTo: "head",
            children:
              `window.__SOCIS_AGENT_SESSION_TOKEN__="${match[1]}";` +
              `window.__SOCIS_AGENT_DASHBOARD_EMBEDDED_CHAT__=${embeddedJs};`,
          },
        ];
      } catch (err) {
        console.warn(
          `[socis] Dashboard at ${BACKEND} unreachable — ` +
            `start it with \`socis dashboard\` or set SOCIS_AGENT_DASHBOARD_URL. ` +
...
export default defineConfig({
  ...
  server: {
    proxy: {
      ...
      // Same host as `socis dashboard` must serve these; Vite has no
