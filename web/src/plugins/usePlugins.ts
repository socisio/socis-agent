...
import { api, SOCIS_AGENT_BASE_PATH } from "@/lib/api";
...
export const MANIFEST_CACHE_KEY = "socis:plugin-manifests";
...
export function usePlugins() {
  ...
  useEffect(() => {
    ...
    for (const manifest of manifests) {
      ...
      if (manifest.css) {
        const cssUrl = `${SOCIS_AGENT_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.css}`;
      ...
      const baseUrl = `${SOCIS_AGENT_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.entry}`;
      const scriptSrc = import.meta.env.DEV
        ? `${baseUrl}?socis_dv=${Date.now()}`
        : baseUrl;
      ...
      script.setAttribute("data-socis-plugin", manifest.name);
