...
interface SOCISConsoleModalProps {
  open: boolean;
  onClose: () => void;
}
...
export function SOCISConsoleModal({ open, onClose }: SOCISConsoleModalProps) {
  const modalRef = useModalBehavior({ open, onClose });
  const hostRef = useRef<HTMLDivElement | null>(null);
  const termRef = useRef<XtermTerminal | null>(null);
  const wsRef = useRef<WebSocket | null>(null);
  const lineRef = useRef("");
  const promptRef = useRef("socis> ");
  const inputPromptRef = useRef("socis> ");
  ...
  const handleFrame = useCallback(
    (frame: ConsoleFrame) => {
      ...
      if (frame.type === "ready") {
        const nextPrompt = frame.prompt || "socis> ";
  ...
  useEffect(() => {
    ...
    writeLine(term, "\x1b[2mConnecting to SOCIS Console...\x1b[0m");
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="socis-console-title"
    >
      <div
        className={cn(
          themedBody,
