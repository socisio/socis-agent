...
const SIDEBAR_COLLAPSED_KEY = "socis-sidebar-collapsed";
          ...
          >
            ...
            >
              ...
              {sidebarNav.pluginItems.length > 0 && (
                <div
                  aria-labelledby="socis-sidebar-plugin-nav-heading"
                ...
                >
                  <span
                    ...
                    id="socis-sidebar-plugin-nav-heading"
                  >
...
function SidebarSystemActions({
  ...
  const { activeAction, isBusy, isRunning, pendingAction, runAction } =
    useSystemActions();
  const canUpdateSOCIS = status?.can_update_socis === true;
  ...
  const updateConfirmDescription = useMemo(() => {
    if (updateConfirmInfo?.behind && updateConfirmInfo.behind > 0) {
      ...
      return `This will run 'socis update' (${cmd}) and pull ${n} new commit${n === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`;
    }
    const cmd = updateConfirmInfo?.update_command ?? "socis update";
    return (
      t.status.updateSOCISConfirmMessage ??
      `This will run 'socis update' (${cmd}) and restart the gateway when it finishes.`
    );
  }, [t.status.updateSOCISConfirmMessage, updateConfirmInfo]);
  ...
  if (canUpdateSOCIS) {
    items.push({
      action: "update",
      icon: Download,
      label: t.status.updateSOCIS,
      runningLabel: t.status.updatingSOCIS,
      spin: false,
    });
  }
  ...
  return (
    ...
    <ConfirmDialog
      ...
      description={
        ...
        "This restarts the SOCIS gateway process. Connected channels and active sessions will reconnect afterward."
      }
    ...
    <ConfirmDialog
      cancelLabel={t.common.cancel}
      confirmLabel={t.status.updateSOCISConfirmNow ?? "Update now"}
      description={
      ...
      title={t.status.updateSOCISConfirmTitle ?? `${t.status.updateSOCIS}?`}
    />
    </>
  );
}
