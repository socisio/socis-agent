/**
 * ChatPage — embeds `socis --tui` inside the dashboard.
 *
 ...
 *          ▼                                                              .
 *     FastAPI pty_ws  (socis_cli/web_server.py)                          .
 *          ▼                                                              .
...
const PTY_ATTACH_TOKEN_KEY = "socis.pty.token.chat";
...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  const [banner, setBanner] = useState<string | null>(() =>
    ...
    !window.__SOCIS_AGENT_SESSION_TOKEN__ &&
    !window.__SOCIS_AGENT_AUTH_REQUIRED__
      ? "Session token unavailable. Open this page through `socis dashboard`, not directly."
      : null,
  );
  ...
  const [chatPanelCollapsed, setChatPanelCollapsed] = useState(
    () => localStorage.getItem("socis-chat-panel-collapsed") === "1",
  );
  const toggleChatPanel = useCallback(() => {
    setChatPanelCollapsed((prev) => {
      const next = !prev;
      localStorage.setItem("socis-chat-panel-collapsed", next ? "1" : "0");
      return next;
    });
  }, []);
  ...
  useEffect(() => {
    ...
    const gated = !!window.__SOCIS_AGENT_AUTH_REQUIRED__;
    ...
    const term = new Terminal({
      ...
      // even when the inner SOCIS TUI has enabled xterm mouse-events
    ...
    //      payloads upload to SOCIS_AGENT_HOME/images then drive `/image`.
    //
    ...
    // must upload browser bytes to SOCIS_AGENT_HOME/images, then drive `/image`
    ...
    if (useWebgl) {
      try {
        ...
        console.warn(
          "[socis-chat] WebGL renderer unavailable; falling back to default",
          err,
        );
      }
    }
    ...
    // WebSocket. In gated mode (``window.__SOCIS_AGENT_AUTH_REQUIRED__``) this
    ...
    void (async () => {
      ...
      // Profile-scoped chat: the PTY child gets SOCIS_AGENT_HOME pointed at the
    ...
    // forwarding them into SOCIS. Keyboard input, paste, and resize still
    // behave normally.
    ...
    <div className="flex min-h-0 flex-1 flex-col gap-2">
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-2 lg:flex-row lg:gap-3">
        ...
        >
          <div
            ref={hostRef}
            className="socis-chat-xterm-host min-h-0 min-w-0 flex-1"
          />
