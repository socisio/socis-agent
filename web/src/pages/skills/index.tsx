        ...
        {expanded && (
          <div className={styles.cardDetail}>
            ...
            <div className={styles.installHint}>
              <code>{skill.installCmd || `socis skills install ${skill.name}`}</code>
              <CopyButton
                text={skill.installCmd || `socis skills install ${skill.name}`}
              />
            </div>
            {onPick ? (
              <button
...
export default function SkillsDashboard() {
  ...
  // app (SOCIS desktop's Bot Mode agent editor) as a skill PICKER. Site
  // chrome is hidden via a CSS class and every card gains an
  // "+ Add to this Agent" button that posts
  //   { type: 'socis-skill-pick', name, identifier, installCmd, source }
  ...
  const pickSkill = useCallback(
    (skill: Skill) => {
      ...
      window.parent.postMessage(
        {
          type: "socis-skill-pick",
          name: skill.name,
          identifier: skill.identifier || skill.name,
          installCmd: skill.installCmd || `socis skills install ${skill.name}`,
          source: skill.source,
        },
        "*"
      );
  ...
  return (
    <Layout
      title="Skills Hub"
      description="Browse all skills and plugins available for SOCIS Agent"
    >
      <div className={`${styles.page} ${pickerMode ? styles.pickerMode : ""}`}>
        <header className={styles.hero}>
          <div className={styles.heroGlow} />
          <div className={styles.heroContent}>
            <p className={styles.heroEyebrow}>SOCIS Agent</p>
