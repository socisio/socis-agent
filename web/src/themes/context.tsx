...
const STORAGE_KEY = "socis-dashboard-theme";
...
const FONT_STORAGE_KEY = "socis-dashboard-font";
...
const THEME_NAME_ALIASES: Record<string, string> = {
  // Renamed during the LENS_5I port + blue-accent theme rebrand (was
  // "Nous Blue", now "Cobalt" - the internal "nous-blue" key is kept
  // stable so anyone with it persisted doesn't silently fall back to
  // the default theme; only the user-facing label/description changed.
  "lens-5i": "nous-blue",
};
 ...
 *  `_THEME_NAMED_ASSET_KEYS` in `socis_cli/web_server.py`. */
...
const CUSTOM_CSS_STYLE_ID = "socis-theme-custom-css";
...
function applyCustomCSS(css: string | undefined) {
  ...
  if (!el) {
    ...
    el.setAttribute("data-socis-theme-css", "true");
...
function injectFontStylesheet(url: string | undefined) {
  ...
  link.rel = "stylesheet";
  link.href = url;
  link.setAttribute("data-socis-theme-font", "true");
...
function applyTheme(theme: DashboardTheme) {
  ...
  // (e.g. Cobalt) doesn't leave its values behind when the user
