...
import {
  ...
  buildSOCISWebSocketUrl,
  type ConnectionState,
  type GatewayEvent,
  type GatewayEventName,
} from "@socis/shared";

import { SOCIS_AGENT_BASE_PATH, buildWsAuthParam } from "@/lib/api";
...
export class GatewayClient extends JsonRpcGatewayClient {
  ...
  async connect(token?: string): Promise<void> {
    ...
    if (!authParam[1]) {
      throw new Error(
        "Session token not available — page must be served by the SOCIS dashboard server",
      );
    }
    ...
    await super.connect(
      buildSOCISWebSocketUrl({
        authParam,
        basePath: SOCIS_AGENT_BASE_PATH,
        path: "/api/ws",
      }),
    );
  }
