import { buildSOCISWebSocketUrl } from "@socis/shared";
...
function readBasePath(): string {
  ...
  const raw = window.__SOCIS_AGENT_BASE_PATH__ ?? "";
  if (!raw) return "";
...
export const SOCIS_AGENT_BASE_PATH = readBasePath();
const BASE = SOCIS_AGENT_BASE_PATH;
...
declare global {
  interface Window {
    __SOCIS_AGENT_SESSION_TOKEN__?: string;
    __SOCIS_AGENT_BASE_PATH__?: string;
    ...
    __SOCIS_AGENT_AUTH_REQUIRED__?: boolean;
  }
}
const SESSION_HEADER = "X-SOCIS-Session-Token";
...
export async function fetchJSON<T>(
  ...
  const token = window.__SOCIS_AGENT_SESSION_TOKEN__;
  if (token) {
  ...
  if (res.status === 401) {
    ...
    // (``socis update``, ``socis gateway restart``, etc.). A tab kept
    // open across the restart holds the OLD token in
    // ``window.__SOCIS_AGENT_SESSION_TOKEN__`` from the previous HTML render,
    ...
    if (!window.__SOCIS_AGENT_AUTH_REQUIRED__ && !options?.allowUnauthorized) {
  ...
  if (res.ok) {
    ...
    // current ``window.__SOCIS_AGENT_SESSION_TOKEN__`` is valid, so the next
 ...
 *
 * The dashboard's gated-mode WS auth (``socis_cli.web_server._ws_auth_ok``)
...
export async function buildWsAuthParam(): Promise<[string, string]> {
  if (window.__SOCIS_AGENT_AUTH_REQUIRED__) {
 ...
 *  - loopback / ``--insecure``: attach the ``X-SOCIS-Session-Token`` header.
 *  - gated OAuth: no token header (it's absent by design); the
 *    ``socis_session_at`` cookie rides along via ``credentials: 'include'``.
 *
 ...
 * ``window.__SOCIS_AGENT_SESSION_TOKEN__`` directly, so the gated-mode ticket
...
export const api = {
  ...
  updateSOCIS: () =>
    fetchJSON<ActionResponse>("/api/socis/update", { method: "POST" }),
  checkSOCISUpdate: (force = false) =>
    fetchJSON<UpdateCheckResponse>(
      `/api/socis/update/check${force ? "?force=true" : ""}`,
    ),
...
export interface SkillHubSource {
  ...
  /** socis-index only: whether the centralized index loaded. */
  available?: boolean;
}
...
export interface StatusResponse {
   ...
   * updates are handled by the outer launcher instead of ``socis update``. */
  can_update_socis?: boolean;
  config_path: string;
  ...
  /** NS-656: disk-usage rollup for the SOCIS_AGENT_HOME volume. Absent on
   * older gateways. */
 ...
 * sample of the SOCIS_AGENT_HOME volume — no staleness dimension, so no
 * sampled_at. */
