...
Extracted verbatim from socis_agent_state.py so the SessionDB mixin modules
(socis_agent_state_search / socis_agent_state_schema / socis_agent_state_portability) can
reference them without importing socis_agent_state (which would be a cycle).
socis_agent_state re-imports every name here for backward compatibility.
"""
...
# optimized via ``socis sessions optimize-storage``. A legacy DB sits at
...
--
-- A host-declared conversation key (X-SOCIS-Session-Key / build_session_key)
...
# `socis sessions optimize-storage` rebuilds it on a capable host.
...
# triggers repairable UNTIL the user opts into `socis db optimize`. This is
...
# service, the Desktop app's `socis serve` backend, interactive CLI sessions,
...
#
# Semantics mirror `socis_agent_state._cross_process_repair_lock` (the schema-
...
# socis_agent_state) because the search/schema mixins cannot import socis_agent_state
# (cycle).
#
...
def _describe_lock_holder(record) -> str:
    ...
    if not isinstance(record, dict) or "pid" not in record:
        return "unknown (no holder record; pre-fix writer or non-SOCIS)"
    pid = record.get("pid")
