...
2. An event-loop heartbeat file at ``<SOCIS_AGENT_HOME>/state/gateway.heartbeat`` so
...
def _process_socis_agent_home() -> Path:
    """SOCIS_AGENT_HOME for process-level identity files (ignore profile overrides)."""
...
def get_loop_heartbeat_path(home: Optional[Path] = None) -> Path:
    """Return ``<SOCIS_AGENT_HOME>/state/gateway.heartbeat``."""
...
def get_loop_tick_socket_path(
    ...
    ``<SOCIS_AGENT_HOME>/state/gateway.loop-tick.<pid>.sock`` — PID-suffixed so a
    ...
    *,
    ...
    def _watchdog() -> None:
        ...
        try:
            ...
            pass
        try:
            from socis_agent_logging import drain_log_queue
...
async def loop_heartbeat_forever(
    ...
    *initiated* by the loop, so a frozen loop still lets the file age.
    ...
    wedged; see ``socis_cli.gateway.probe_gateway_loop_liveness`` for the
