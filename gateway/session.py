...
# ``SOCIS_AGENT_AUTO_CONTINUE_FRESHNESS`` at startup.
...
def auto_continue_freshness_window() -> float:
    ...
    ``SOCIS_AGENT_AUTO_CONTINUE_FRESHNESS`` (bridged from ``config.yaml``
    ...
    """
    raw = os.environ.get("SOCIS_AGENT_AUTO_CONTINUE_FRESHNESS")
...
# ``sessions_dir / f"{session_id}.json"`` in socis_agent_state, request-dump
...
def _slack_tools_loaded() -> bool:
    ...
    Two independent paths grant Slack capability:
      1. Native `slack` toolset enabled via `socis tools` (opt-in, default
...
def _discord_tools_loaded() -> bool:
    ...
    Two conditions must hold:
      1. The `discord` or `discord_admin` toolset is enabled for the
         Discord platform via `socis tools` (opt-in, default OFF).
...
def build_session_context_prompt(
    ...
    *,
    ...
    if context.source.platform == Platform.SLACK:
        ...
        # `discord` / `discord_admin` via `socis tools` AND the bot
    ...
    lines.append(
        f"- `\"local\"` → Save to local files only ({display_socis_agent_home()}/cron/output/)"
    )
...
def build_channel_continuity_note(
    ...
    return (
        f"[System note: This {where} had an earlier SOCIS session "
...
class SessionStore:
    ...
    def __init__(self, sessions_dir: Path, config: GatewayConfig,
        ...
        # ``get_socis_agent_home()`` for the turn (its docstring lists "sessions"
        ...
        # profile name -> its SOCIS_AGENT_HOME (or None to use the ambient scope).
        ...
        try:
            ...
            self._routing_home: Optional[Path] = Path(get_socis_agent_home())
        except Exception:
    ...
    def _open_session_db_for_active_scope(self, db_path: Optional[Path] = None):
        ...
        time, and that helper follows the context-local SOCIS_AGENT_HOME override
        ...
        """
        from socis_agent_state import SessionDB, _default_db_path, get_shared_session_db
    ...
    def _profile_home_for_key(self, session_key: Optional[str]) -> Optional[Path]:
        """SOCIS_AGENT_HOME of the profile that owns *session_key*, or None.
        ...
        try:
            from socis_cli.profiles import get_profile_dir, profile_exists
    ...
    def _db_for_key(self, session_key: Optional[str]):
        ...
        ``_db`` follows the ambient SOCIS_AGENT_HOME, and only the inbound message
    ...
    def _save_sessions_json(self, data: Dict[str, Any]) -> None:
        ...
        data = {
            "_README": (
                ...
                "lives in the gateway_routing table in ~/.socis-agent/state.db). "
                "Maps messaging session keys (agent:main:<platform>:...) to "
                "active session IDs. This is NOT the session list. ALL "
                "sessions (CLI, TUI, and gateway) live in ~/.socis-agent/state.db "
                "and are shown by `socis sessions list` and `/sessions`. "
        ...
        while True:
            try:
                ...
                if isinstance(exc, (StateDbReplacedError, StateDbCorruptError)):
                    ...
                    try:
                        from socis_agent_state import divert_session_transcript_jsonl
