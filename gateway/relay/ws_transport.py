...
def _normalize_slack_parent_command(
    ...
    """Mirror native Slack ``/socis`` routing for authenticated relay text."""
    stripped = text.strip()
    parent_parts = stripped.split(maxsplit=1)
    if not parent_parts or parent_parts[0] != "/socis":
...
def _event_from_wire(raw: Dict[str, Any]) -> MessageEvent:
    ...
    source = SessionSource(
        ...
        # The SOCIS profile this event is routed to (multiplex mode). The
    ...
    if platform_enum == Platform.SLACK:
        ...
        # relay, bypassing SOCIS' native Slack command callback. Normalize at
        # the wire boundary so adapter-level active-session gates see the real
        # gateway command rather than the legacy `socis` parent name.
...
class PassthroughForward:
    ...
    body: bytes
    # The SOCIS profile this interaction is routed to (multiplex mode).
