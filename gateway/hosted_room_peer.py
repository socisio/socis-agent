...
def _gateway_room_grant_secret_for_home(home_value: str) -> bytes:
    ...
    return hmac.new(
        material,
        b"socis-hosted-room-installation-grant-v1",
        hashlib.sha256,
    ).digest()
...
def gateway_room_grant_secret(root: Path | str | None = None) -> bytes:
    ...
    if root is None:
        ...
        # Profile routing uses a context-local SOCIS_AGENT_HOME override. The process
        # environment retains the installation root and is the authority here.
        root = os.environ.get("SOCIS_AGENT_HOME") or get_socis_agent_home()
...
def catalog_mapping(
    *,
    ...
    persistent_process = bool(persistent_process and os.getenv("SOCIS_AGENT_DESKTOP") != "1")
    checked_policy = RoomExecutionPolicy.from_mapping(
        execution_policy
        or execution_policy_mapping(
            target_profile=(
                str(target_profile or "").strip()
                or (os.getenv("SOCIS_AGENT_PROFILE") or "default").strip()
                or "default"
            )
        )
    )
...
def _room_link_url_from_config(home: str) -> str | None:
    ...
    if str(get_socis_agent_home()) == home:
...
def _configured_room_link_url() -> str | None:
    ...
    override = os.getenv("SOCIS_AGENT_ROOM_LINK_URL")
