...
class PlatformEntry:
    ...
    # status displays (``socis setup``, ``socis status``, the dashboard
    ...
    # Env vars this platform needs (for ``socis setup`` display).
    ...
    # built-ins).  Used by ``socis gateway setup`` to auto-enable the
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # Plugin adapters are isolated per resolved SOCIS_AGENT_HOME and overlay the
        ...
        # plugin-discovery time added several seconds to *every* `socis`
        # invocation -- including plain `socis chat`, which never touches any
        ...
        # `socis setup`/`gateway status`, send_message).
    ...
    def _resolve_all(self) -> None:
        ...
        gateway startup, ``socis setup``/``gateway status``, channel
