...
``<SOCIS_AGENT_HOME>/state/gateway.lifecycle.json``:
...
def _process_socis_agent_home() -> Path:
    """SOCIS_AGENT_HOME for process-level identity files (ignore task overrides)."""
...
def get_lifecycle_sentinel_path(home: Optional[Path] = None) -> Path:
    """Return ``<SOCIS_AGENT_HOME>/state/gateway.lifecycle.json``."""
...
def check_state_db_integrity(home: Optional[Path] = None) -> str:
    ...
    ``_enforce_macos_synchronous_full`` in :mod:`socis_agent_state` — macOS
...
def read_prior_exit_label(profile_home: Path) -> str:
    ...
    ``socis_cli.container_boot`` to annotate ``container-boot.log``.
    """
    try:
