...
Design constraints (decisions.md):
  - Per-instance enable is gated SOLELY by the NAS "Labs" toggle, carried to the
    gateway as the ``SOCIS_AGENT_SCALE_TO_ZERO`` env stamp (D11/Q8=A). NOT a user
...
SCALE_TO_ZERO_ENV = "SOCIS_AGENT_SCALE_TO_ZERO"
...
def scale_to_zero_enabled(environ: Optional[dict] = None) -> bool:
    """Whether the per-instance Labs toggle is on (the SOCIS_AGENT_SCALE_TO_ZERO stamp).
...
def dashboard_client_heartbeat_path(socis_agent_home: Optional[os.PathLike | str] = None):
    """Path of the dashboard-client liveness marker under SOCIS_AGENT_HOME."""
    from pathlib import Path

    if socis_agent_home is None:
        from socis_agent_constants import get_socis_agent_home

        socis_agent_home = get_socis_agent_home()
    return Path(socis_agent_home) / DASHBOARD_CLIENT_HEARTBEAT_REL
