...
- POST /v1/chat/completions        — OpenAI Chat Completions format (stateless; opt-in session continuity via X-SOCIS-Session-Id header; opt-in long-term memory scoping via X-SOCIS-Session-Key header)
- POST /v1/responses               — OpenAI Responses API format (stateful via previous_response_id; X-SOCIS-Session-Key supported)
- GET  /v1/responses/{response_id} — Retrieve a stored response
- DELETE /v1/responses/{response_id} — Delete a stored response
- GET  /v1/models                  — lists socis-agent and any configured model_routes aliases
- GET  /v1/capabilities            — machine-readable API capabilities for external UIs
- GET  /api/sessions               — list client-visible SOCIS sessions
- POST /api/sessions               — create an empty SOCIS session
...
AnythingLLM, NextChat, ChatBox, etc.) can connect to socis-agent
...
_BROWSER_CONTROL_WS_PROTOCOL = "socis-browser-control-v1"
_BROWSER_CONTROL_TICKET_PROTOCOL_PREFIX = "socis-browser-control-ticket."
...
def _socis_version() -> str:
    """Return the canonical SOCIS Agent version string.

    ``socis_cli.__version__`` is the runtime source of truth used by the CLI,
    ...
    ``socis_agent-*.dist-info`` after a source update until the environment is
    reinstalled. Never raises — a version probe must not be able to break the
    health endpoint.
    """
    try:
        from socis_cli import __version__
...
# config.yaml accept (socis_agent_constants.VALID_REASONING_EFFORTS); wire-level
...
def _resolve_request_runtime_agent_kwargs(provider: str, target_model: Optional[str] = None) -> Dict[str, Any]:
    ...
    """
    from socis_cli.runtime_provider import resolve_runtime_provider, format_runtime_provider_error, _get_model_config
    ...
    max_tokens = None
    env_max_tokens = os.environ.get("SOCIS_AGENT_MAX_TOKENS")
    if env_max_tokens:
        try:
...
def _request_agent_overrides(
    ...
    *,
    ...
    ``/v1/models`` advertises a stable virtual model (usually ``socis-agent``)
    ...
    ``direct_model_requests`` config value here, while SOCIS-native
    endpoints (session chat, /v1/runs) always allow it.  A request that
    sends an explicit ``provider`` is unambiguously SOCIS-aware and is
    always honored.
    """
    ...
    def __init__(self, max_size: int = MAX_STORED_RESPONSES, db_path: str = None):
        ...
        if db_path is None:
            try:
                ...
                db_path = str(get_socis_agent_home() / "response_store.db")
            except Exception:
                db_path = ":memory:"
        ...
        # gracefully on NFS/SMB/FUSE-mounted SOCIS_AGENT_HOME (same filesystem
        # issue addressed for state.db/kanban.db — see
        # socis_agent_state._WAL_INCOMPAT_MARKERS).
...
def _derive_chat_session_id(
    ...
    the same SOCIS session (and therefore the same Docker container sandbox
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    and routes them through socis-agent's AIAgent.
    """
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send an explicit ``provider`` — and the SOCIS-native session-chat
    ...
    def _resolve_max_concurrent_runs() -> int:
        ...
        default = 10
        try:
            from socis_cli.config import cfg_get, load_config
    ...
    def _resolve_model_name(explicit: str) -> str:
        ...
        3. Fallback: "socis-agent"

        Delegates the tiered fallthrough to
        :func:`socis_cli.model_switch.resolve_effective_model` (the shared
        override > mid-tier > default precedence owner).
        """
        from socis_cli.model_switch import resolve_effective_model
        ...
        try:
            ...
            pass
        return resolve_effective_model(explicit, profile_name, "socis-agent")
    ...
    def _expected_api_key(self) -> str:
        ...
        try:
            ...
            from socis_cli.auth import has_usable_secret
    ...
    def _resolve_request_profile(self, request: "web.Request"):
        ...
        if not getattr(cfg, "multiplex_profiles", False):
            ...
            # `socis peer dm mini/researcher` answered by the mini's default
        ...
        if not profile:
            try:
                ...
                if is_multiplex_active():
                    ...
                    return _profile_runtime_scope(get_socis_agent_home())
            except Exception:
                pass
            return nullcontext()
        from gateway.run import _profile_runtime_scope
        from socis_cli.profiles import get_profile_dir
    ...
    def _declared_conversation_session(
        ...
        """Resolve the live session a client declared with ``X-SOCIS-Session-Key``.
        ...
        every conversation-affinity hint SOCIS sends off that id
    ...
    def _parse_session_key_header(
        ...
        """Extract and validate the ``X-SOCIS-Session-Key`` header.

        The session key is a stable per-channel identifier that scopes
        long-term memory (e.g. Honcho sessions) across transcripts.  It
        is independent of ``X-SOCIS-Session-Id``: callers may send
        ...
        """
        raw = request.headers.get("X-SOCIS-Session-Key", "").strip()
        if not raw:
            return None, None

        if not self._api_key:
            logger.warning(
                "X-SOCIS-Session-Key rejected: no API key configured. "
                "Set API_SERVER_KEY to enable long-term memory scoping."
            )
            return None, web.json_response(
                _openai_error(
                    "X-SOCIS-Session-Key requires API key authentication. "
    ...
    def _ensure_session_db(self):
        ...
        Sessions are persisted to ``state.db`` so that ``socis sessions list``
        shows API-server conversations alongside CLI and gateway ones.

        Under multiplex ``/p/<profile>/`` requests the profile runtime scope
        redirects ``get_socis_agent_home()``, so each profile gets its own DB —
        ...
        try:
            ...
            return self._open_and_cache_session_db(get_socis_agent_home())
    ...
    def _stored_session_model(self, session: Any) -> Optional[str]:
        ...
        The advertised virtual model (usually ``socis-agent``) means "use
        ...
        400s ("socis-agent is not a valid model ID") — the same filter
    ...
    def _session_runtime_request_from_body(self, body: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # The virtual model alias (self._model_name, e.g. "socis-agent") is
    ...
    def _normalize_session_source(value: Any) -> str:
        ...
        allowed = {"api_server", "socis_browser", "browser", "cli", "telegram", "discord", "slack", "desktop", "dashboard"}
        if text in allowed:
            return "socis_browser" if text == "browser" else text
        return "api_server"
    ...
    def _create_agent(
        ...
        gateway platforms), falling back to the socis-api-server default.

        ``gateway_session_key`` is a stable per-channel identifier supplied
        by the client (via ``X-SOCIS-Session-Key``).  Unlike ``session_id``
        ...
        # socis_cli.model_switch.resolve_effective_model (session /model
        ...
        # (e.g. user ran `socis auth add openai-codex` without `socis model`),
        ...
        if not model and runtime_kwargs.get("provider"):
            try:
                from socis_cli.models import get_default_model_for_provider
    ...
    async def _handle_health(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response(
            {"status": "ok", "platform": "socis-agent", "version": _socis_version()}
        )
    ...
    async def _handle_health_detailed(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({
            ...
            "readiness": readiness,
            "platform": "socis-agent",
            "version": _socis_version(),
    ...
    async def _handle_models(self, request: "web.Request") -> "web.Response":
        """GET /v1/models — list socis-agent and any configured model_routes aliases.
    ...
    async def _handle_model_options(self, request: "web.Request") -> "web.Response":
        """GET /api/model/options — return SOCIS provider/model inventory.
        ...
        SOCIS provider catalog instead of scraping the single OpenAI-compatible
        `/v1/models` alias.
        """
        auth_err = self._check_auth(request)
        if auth_err:
    ...
    async def _handle_capabilities(self, request: "web.Request") -> "web.Response":
        ...
        every SOCIS version exposes the same endpoints.
        """
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        ...
        return web.json_response({
            "object": "socis.api_server.capabilities",
            "platform": "socis-agent",
            ...
            "runtime": {
                ...
                "split_runtime": False,
                "description": (
                    "The API server creates a server-side SOCIS AIAgent; "
            ...
            "features": {
                ...
                "session_continuity_header": "X-SOCIS-Session-Id",
                "session_key_header": "X-SOCIS-Session-Key",
    ...
    def _artifact_store_for(self, profile: str) -> ArtifactStore:
        ...
        (``<SOCIS_AGENT_HOME>/plugin-data/.../artifacts``-style controlled root),
        ...
        try:
            ...
            # under the SOCIS home.
            try:
                from socis_agent_state import get_socis_agent_home

                root = Path(get_socis_agent_home()) / "artifacts" / "browser-control"
            except Exception:
    ...
    async def _handle_list_sessions(self, request: "web.Request") -> "web.Response":
        """GET /api/sessions — list persisted SOCIS sessions."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        ...
        # Exact-title lookup, used by `socis peer dm` to resolve a peer's
        ...
        if title_filter:
            ...
            if not sessions:
                ...
                # which would fail `socis peer dm` resolution and mint
    ...
    async def _handle_create_session(self, request: "web.Request") -> "web.Response":
        """POST /api/sessions -- create an empty SOCIS session row.
        ...
        # (e.g. "provider::socis-agent") are split, and the virtual model
        # alias (self._model_name, e.g. "socis-agent") is nulled out
        # there — a bare "socis-agent" is not a model_routes alias, so a
        ...
        return web.json_response({"object": "socis.session", "session": self._session_response(session)}, status=201)
    ...
    async def _handle_get_session(self, request: "web.Request") -> "web.Response":
        ...
        if err:
            return err
        return web.json_response({"object": "socis.session", "session": self._session_response(session)})
    ...
    async def _handle_delete_session(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"object": "socis.session.deleted", "id": session_id, "deleted": bool(deleted)})
    ...
    async def _handle_fork_session(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"object": "socis.session", "session": self._session_response(fork)}, status=201)
    ...
    async def _handle_session_chat(self, request: "web.Request") -> "web.Response":
        ...
        headers = {"X-SOCIS-Session-Id": effective_session_id or session_id}
        if gateway_session_key:
            headers["X-SOCIS-Session-Key"] = gateway_session_key
        runtime = {}
        ...
        return web.json_response(
            {
                "object": "socis.session.chat.completion",
    ...
    async def _handle_session_model_lock(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({
            "object": "socis.session.model_lock",
            "session_id": session_id,
            "runtime": runtime,
        })
    @_admit_api_agent_request
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        # stable per-channel identifier via X-SOCIS-Session-Key.  This
        # is independent of X-SOCIS-Session-Id: the key persists across
        ...
        # Allow caller to continue an existing session by passing X-SOCIS-Session-Id.
        ...
        provided_session_id = request.headers.get("X-SOCIS-Session-Id", "").strip()
        if provided_session_id:
            if not self._api_key:
                logger.warning(
                    "Session continuation via X-SOCIS-Session-Id rejected: "
            ...
            # conversation map to the same SOCIS session.  The first user
        ...
        if stream:
            ...
            def _on_tool_start(tool_call_id, function_name, function_args):
                """Emit ``socis.tool.progress`` with ``status: running``.
        ...
        response_headers = {
            "X-SOCIS-Session-Id": result.get("session_id", session_id),
        }
        if gateway_session_key:
            response_headers["X-SOCIS-Session-Key"] = gateway_session_key
        ...
        if not final_response and (is_failed or is_partial):
            ...
            err_body["error"]["socis"] = {
                "completed": completed,
                "partial": is_partial,
                "failed": is_failed,
            }
            response_headers["X-SOCIS-Completed"] = "false"
            response_headers["X-SOCIS-Partial"] = "true" if is_partial else "false"
            return web.json_response(err_body, status=502, headers=response_headers)

        # Soft-partial path: we have *some* text but the run did not complete
        # (e.g. truncation with partial buffered output). Still 200 but signal
        # truncation via finish_reason="length" + SOCIS-specific extras.
        response_data = {
            "id": completion_id,
        ...
        if is_partial or is_failed or not completed:
            ...
            response_headers["X-SOCIS-Partial"] = "true" if is_partial else "false"
            if err_msg:
                response_headers["X-SOCIS-Error"] = _redact_api_error_text(err_msg, limit=200)
    ...
    async def _write_sse_chat_completion(
        ...
        if session_id:
            sse_headers["X-SOCIS-Session-Id"] = session_id
        if gateway_session_key:
            sse_headers["X-SOCIS-Session-Key"] = gateway_session_key
        ...
        try:
            ...
            async def _emit(item):
                ...
                as a custom ``event: socis.tool.progress`` SSE event so
                frontends can display them without storing the markers in
                conversation history.  See #6972 for the original event,
                #16588 for the ``toolCallId``/``status`` lifecycle fields.
                """
                if isinstance(item, tuple) and len(item) == 2 and item[0] == "__tool_progress__":
                    await response.write(_sse_frame(item[1], event="socis.tool.progress"))
                else:
                    content_chunk = {
                        "id": completion_id, "object": "chat.completion.chunk",
                        "created": created, "model": model,
    ...
    async def _handle_responses(self, request: "web.Request") -> "web.Response":
        ...
        # the conversation it declared via ``X-SOCIS-Session-Key`` before
        ...
        # response and the X-SOCIS-Session-Id header.  Without this,
        ...
        response_headers = {"X-SOCIS-Session-Id": _effective_session_id}
        if gateway_session_key:
            response_headers["X-SOCIS-Session-Key"] = gateway_session_key
        return web.json_response(response_data, headers=response_headers)
    ...
    async def _handle_run_job(self, request: "web.Request") -> "web.Response":
        ...
        # `socis cron run` / cronjob(action='run', prompt=...) — same length
            ...
            if role == "assistant" and msg.get("tool_calls"):
                for tc in msg["tool_calls"]:
                    ...
                    items.append({
                        ...
                        # SOCIS agent; they are replayed for structured tool
        ...
        def _run():
            ...
            with self._profile_scope(request_profile):
                ...
                try:
                    ...
                    # (e.g. X-SOCIS-Session-Id header) can track compression-
                    ...
                    if include_runtime:
                        runtime = dict(getattr(agent, "_socis_api_runtime", {}) or {})
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            # Store the adapter after native routes are registered. Local SOCIS-Relay
            ...
            # that is the exact surface the socis-0day campaign abused to write
            # ~/.socis-agent/config.yaml and plant persistence. Sandboxing (Docker /
