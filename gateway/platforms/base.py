...
# Audio file extensions SOCIS recognizes for native audio delivery.
# Keep Telegram's narrower attachment/voice sets below separate: formats such
# as MPEG-2 Layer II are audio to SOCIS but unsupported by sendAudio/sendVoice.
_AUDIO_MIME_TYPES = {
    ".ogg": "audio/ogg",
    ".opus": "audio/opus",
    ".mp3": "audio/mpeg",
...
def _thread_metadata_for_source(source, reply_to_message_id: str | None = None) -> dict | None:
    ...
    value. Telegram private-chat topics created through SOCIS' DM-topic helper
...
def _reply_anchor_for_event(event) -> str | None:
    ...
    replying to the triggering message. SOCIS-created Telegram private-chat
    ...
    if (
        ...
        and raw_message.get("_socis_no_thread_response")
    ):
...
def build_auto_tts_output_path(platform) -> str:
    ...
    TTS tool's ``SOCIS_AGENT_SESSION_PLATFORM`` contextvar — that contextvar is
...
def gateway_trust_env() -> bool:
    ...
    """
    try:
        from socis_cli.config import load_config_readonly as _load_config
...
from socis_agent_constants import get_default_socis_root, get_socis_dir, get_socis_agent_home
...
GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    ...
    "Load this skill in the local CLI to be prompted, or add the key to ~/.socis-agent/.env manually."
)
...
IMAGE_CACHE_DIR = get_socis_dir("cache/images", "image_cache")


def _resolve_cache_dir(constant_name: str, new_subpath: str, old_name: str) -> Path:
    """Resolve fresh via get_socis_dir (active profile), unless a test has
    monkeypatched the constant away from its import-time default."""
    fresh = get_socis_dir(new_subpath, old_name)
        ...
        for attempt in range(retries + 1):
            try:
                async with client.stream(
                    ...
                    url,
                    headers={
                        "User-Agent": "Mozilla/5.0 (compatible; SOCISAgent/1.0)",
                        "Accept": "image/*,*/*;q=0.8",
                    },
                ) as response:
...
AUDIO_CACHE_DIR = get_socis_dir("cache/audio", "audio_cache")
...
VIDEO_CACHE_DIR = get_socis_dir("cache/videos", "video_cache")
...
DOCUMENT_CACHE_DIR = get_socis_dir("cache/documents", "document_cache")
SCREENSHOT_CACHE_DIR = get_socis_dir("cache/screenshots", "browser_screenshots")
...
_SOCIS_AGENT_ROOT = get_default_socis_root()
MEDIA_DELIVERY_ALLOW_DIRS_ENV = "SOCIS_AGENT_MEDIA_ALLOW_DIRS"
MEDIA_DELIVERY_TRUST_RECENT_ENV = "SOCIS_AGENT_MEDIA_TRUST_RECENT_FILES"
MEDIA_DELIVERY_TRUST_RECENT_SECONDS_ENV = "SOCIS_AGENT_MEDIA_TRUST_RECENT_SECONDS"
...
MEDIA_DELIVERY_STRICT_ENV = "SOCIS_AGENT_MEDIA_DELIVERY_STRICT"
MEDIA_DELIVERY_SAFE_ROOTS = (
    ...
    _SOCIS_AGENT_HOME / "image_cache",
    _SOCIS_AGENT_HOME / "audio_cache",
    _SOCIS_AGENT_HOME / "video_cache",
    _SOCIS_AGENT_HOME / "document_cache",
    _SOCIS_AGENT_HOME / "browser_screenshots",
    ...
    _SOCIS_AGENT_HOME / "cache" / "images",
    _SOCIS_AGENT_HOME / "cache" / "audio",
    _SOCIS_AGENT_HOME / "cache" / "videos",
    _SOCIS_AGENT_HOME / "cache" / "documents",
    _SOCIS_AGENT_HOME / "cache" / "screenshots",
)
...
def _profile_cache_roots() -> List[Path]:
    """Return per-profile canonical cache roots under the shared SOCIS root.

    Profile gateways write generated artifacts to
    ``<root>/profiles/<name>/cache/{images,audio,...}``. The static safe-roots
    list only covers the *active* SOCIS_AGENT_HOME's cache, so a gateway running at
    the root (e.g. ``SOCIS_AGENT_HOME=/opt/data``) while the model emits a
    ...
    is consulted (which otherwise wins when SOCIS_AGENT_HOME is symlinked under a
    denied prefix and $HOME is not that prefix). See issue #31733.
    """
    roots: List[Path] = []
    profiles_dir = _SOCIS_AGENT_ROOT / "profiles"
    try:
...
def _kanban_attachment_roots() -> List[Path]:
    ...
    home_override = os.environ.get("SOCIS_AGENT_KANBAN_HOME", "").strip()
    root = Path(home_override).expanduser() if home_override else _SOCIS_AGENT_ROOT
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    # The active SOCIS profile and shared SOCIS root both contain control
    ...
    # SOCIS_AGENT_HOME root. The set mirrors the canonical read guard in
    ...
    # tree, so skills/, logs/, and ad-hoc agent-written files under ~/.socis-agent
    ...
    for socis_root in (_SOCIS_AGENT_HOME, _SOCIS_AGENT_ROOT):
        for rel in _ROOT_CREDENTIAL_FILES:
            denied.append(socis_root / rel)
...
def _path_under_denied_prefix(resolved: Path) -> bool:
    ...
    sub-directories inside home (``~/.ssh``, ``~/.aws``, ...) and SOCIS
    secrets (``~/.socis-agent/.env``, ``auth.json``) are *separate, more-specific*
...
def _cache_dir_container_mounts() -> List[Tuple[Path, Path]]:
    """(host, container) pairs for the auto-mounted SOCIS cache dirs.

    The agent legitimately sees generated artifacts at ``/root/.socis-agent/...``
...
def _translate_docker_container_media_path(candidate: Path, session_key: str = "") -> Optional[Path]:
    ...
    auto-mounted SOCIS cache dirs (``/root/.socis-agent/...``), the session's
    ...
    # In-process gateways (Desktop backend, `socis serve`) may not have
    ...
    # are longer prefixes, so /root/.socis-agent/... still translates to the host
    # cache — this only catches stray home writes like /root/out.png.
    if not any(c.as_posix() == "/root" for _, c in mounts):
        # /root/.socis-agent/* that did NOT match a cache mount is the container's
...
def validate_media_delivery_path(path: str, session_key: str = "") -> Optional[str]:
    ...
    ``SOCIS_AGENT_MEDIA_DELIVERY_STRICT=1``): the file MUST live under a
    SOCIS-managed cache, under an operator-allowlisted root
    (``SOCIS_AGENT_MEDIA_ALLOW_DIRS``), or be freshly produced inside the
    ...
    # credential/secret stores under the SOCIS root (~/.socis-agent/.env,
    ...
    # ``MEDIA:~/.socis-agent/google_token.json``) remain rejected.
...
_PLAINTEXT_GATEWAY_RESTART_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    re.compile(r"^(?:please\s+)?restart\s+(?:the\s+)?socis\s+gateway[.!?\s]*$", re.IGNORECASE),
    re.compile(r"^(?:please\s+)?restart\s+socis[.!?\s]*$", re.IGNORECASE),
)
...
class BasePlatformAdapter(ABC):
    ...
    # into the ``SOCIS_AGENT_SESSION_ASYNC_DELIVERY`` contextvar at session-bind
    ...
    # SOCIS commands.  Default "/" (most platforms deliver "/approve" etc.
    ...
    def __init__(self, config: PlatformConfig, platform: Platform):
        ...
        # Cross-SOCIS_AGENT_HOME token takeover is armed by GatewayRunner only for
        ...
        self._busy_text_mode: str = (
            os.environ.get("SOCIS_AGENT_GATEWAY_BUSY_TEXT_MODE", "interrupt").strip().lower()
            or "interrupt"
        )
        self._busy_text_debounce_seconds: float = _float_env(
            "SOCIS_AGENT_GATEWAY_BUSY_TEXT_DEBOUNCE_SECONDS", 0.35
        )
        self._busy_text_hard_cap_seconds: float = _float_env(
            "SOCIS_AGENT_GATEWAY_BUSY_TEXT_HARD_CAP_SECONDS", 1.0
        )
    ...
    def prefers_fresh_final_streaming(
        ...
        implementation supports. Telegram is the motivating case: SOCIS sends
    ...
    def _acquire_platform_lock(self, scope: str, identity: str, resource_desc: str) -> bool:
        ...
        A live cross-SOCIS_AGENT_HOME holder may be replaced only when the runner
        ...
        if owner_profile:
            ...
            remedy = (
                ...
                f"(socis --profile {owner_profile} gateway stop)."
            )
        else:
            holder = f" (PID {owner_pid})" if owner_pid else ""
    ...
    def _wire_plugin_handlers(self, native: Any = None) -> None:
        ...
        try:
            from socis_cli.plugins import get_plugin_manager
    ...
    async def send_voice(
        ...
        **kwargs,
        ...
        host filesystem path that would leak the SOCIS home layout.
        """
    ...
    async def send_video(
        ...
        **kwargs,
        ...
        would leak the SOCIS home layout.
        """
    ...
    def _mask_json_string_media(content: str) -> str:
        ...
        Serialized tool results frequently embed a previous reply's text, e.g.::
            ...
            {"result": "MEDIA:/Users/x/.socis-agent/media/generated/stale.png"}
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        ...
        if session_key in self._active_sessions:
            ...
            from socis_cli.commands import (
    ...
    def _get_human_delay() -> float:
        ...
        Reads from env vars:
          SOCIS_AGENT_HUMAN_DELAY_MODE: "off" (default) | "natural" | "custom"
          SOCIS_AGENT_HUMAN_DELAY_MIN_MS: minimum delay in ms (default 800, custom mode)
          SOCIS_AGENT_HUMAN_DELAY_MAX_MS: maximum delay in ms (default 2500, custom mode)
        """
        mode = os.getenv("SOCIS_AGENT_HUMAN_DELAY_MODE", "off").lower()
        if mode == "off":
            return 0.0
        if mode == "natural":
        ...
        try:
            min_ms = int(os.getenv("SOCIS_AGENT_HUMAN_DELAY_MIN_MS", "800"))
        except (TypeError, ValueError):
            min_ms = 800
        try:
            max_ms = int(os.getenv("SOCIS_AGENT_HUMAN_DELAY_MAX_MS", "2500"))
        except (TypeError, ValueError):
            max_ms = 2500
        return random.uniform(min_ms / 1000.0, max_ms / 1000.0)
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        try:
            ...
            if response:
                ...
                if (self._should_auto_tts_for_chat(event.source.chat_id)
                        ...
                        and not self._streaming_tts_turn_completed(
                            session_key,
                            getattr(interrupt_event, "_socis_run_generation", None),
                            event=event,
                        )):
                    try:
                        ...
                        if check_tts_requirements():
                            ...
                            # SOCIS_AGENT_SESSION_PLATFORM contextvar the tool would
            ...
            # not at the top of this task.  _socis_run_generation is set on
    ...
    def build_source(
        ...
        per-profile SOCIS_AGENT_HOME isolation.
        """
