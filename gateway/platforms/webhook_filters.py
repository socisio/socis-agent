...
def _resolve_profile_path(path_value: Any) -> Optional[Path]:
    """Resolve a user path, mapping ~/.socis-agent to the active profile home."""
    ...
    if raw.startswith("~/.socis-agent/"):
        return socis_agent_home / raw.removeprefix("~/.socis-agent/")
    path = Path(raw).expanduser()
    if path.is_absolute():
        return path
    return socis_agent_home / path


def _resolve_script_path(script_value: Any) -> tuple[Optional[Path], Optional[str]]:
    """Resolve a route script under SOCIS_AGENT_HOME/scripts."""
    if not isinstance(script_value, str) or not script_value.strip():
        return None, "script path is empty"
    from socis_agent_constants import get_socis_agent_home

    scripts_root = (get_socis_agent_home() / "scripts").resolve()
    raw_text = os.path.expandvars(script_value.strip())
    if raw_text == "~/.socis-agent" or raw_text.startswith("~/.socis-agent/"):
...
class WebhookRouteProcessor:
    ...
    def run_route_script(self, script_value: Any, payload: dict) -> tuple[bool, Optional[dict]]:
        ...
        if (
            ...
            or transformed.get("__socis_ignore__") is True
        ):
            return False, None
