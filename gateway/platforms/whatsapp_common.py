...
class WhatsAppBehaviorMixin:
    ...
    DEFAULT_REPLY_PREFIX: str = "◆ *SOCIS Agent*\n────────────\n"
...
def resolve_whatsapp_bridge_dir() -> Path:
    """Resolve the WhatsApp bridge directory, mirroring to SOCIS_AGENT_HOME if needed.

    When the install tree is read-only (e.g., Docker /opt/socis), this function
    mirrors the bridge source to a writable SOCIS_AGENT_HOME location and returns that
    ...
    # Try SOCIS_AGENT_HOME location first
    socis_agent_home = get_socis_agent_home()
    socis_agent_home_bridge = socis_agent_home / "scripts" / "whatsapp-bridge"
    ...
    # Install dir is read-only, mirror to SOCIS_AGENT_HOME if needed
    if socis_agent_home_bridge.exists():
        return socis_agent_home_bridge

    # Mirror the bridge source to SOCIS_AGENT_HOME
    try:
        socis_agent_home_bridge.parent.mkdir(parents=True, exist_ok=True)
        shutil.copytree(
            install_bridge,
            socis_agent_home_bridge,
            dirs_exist_ok=False,
        )
        return socis_agent_home_bridge
    except Exception:
