...
There are two ways to add a platform to the SOCIS gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.socis-agent/plugins/` (or under `plugins/platforms/`
...
**zero changes to core SOCIS code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `socis gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `socis_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the SOCIS core.
...
And add it to the `socis-gateway` composite:

```python
"socis-gateway": {
    "includes": [..., "socis-your-platform"]
}
```
...
## 12. Status Display (`socis_cli/status.py`)
...
## 13. Gateway Setup Wizard (`socis_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ socis_cli/ toolsets.py \
  --include="*.py" -l | sort -u
