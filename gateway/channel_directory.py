...
~/.socis-agent/channel_directory.json.  The send_message tool reads this file for
...
def _directory_path() -> Path:
    return DIRECTORY_PATH or get_socis_agent_home() / "channel_directory.json"


def _aliases_path() -> Path:
    return CHANNEL_ALIASES_PATH or get_socis_agent_home() / "channel_aliases.json"
...
def _build_from_sessions_db(platform_name: str) -> List[Dict[str, str]]:
    ...
    try:
        from socis_agent_state import get_shared_session_db, release_or_close
...
def _build_from_sessions_json(platform_name: str) -> List[Dict[str, str]]:
    ...
    sessions_path = get_socis_agent_home() / "sessions" / "sessions.json"
...
def format_directory_for_display(platforms: Optional[Dict[str, Any]] = None) -> str:
    ...
    ``socis send --list`` to merge in configured-but-undiscovered
