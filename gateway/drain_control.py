...
Contract (presence-based, mirroring ``.restart_notify.json``):
  ...
  * begin-drain  → write ``{SOCIS_AGENT_HOME}/.drain_request.json`` with
...
Why the epoch (NS-570). ``SOCIS_AGENT_HOME`` is a **durable** store — on SOCIS
...
def current_instantiation_epoch() -> str:
    ...
    Together they discriminate every restart mode that matters:
      ...
      | host ``socis gateway restart``| same    | same(init) | SAME   | honour |
...
def drain_request_path(home: Optional[Path] = None) -> Path:
    """Absolute path to the drain-request marker, respecting SOCIS_AGENT_HOME."""
    base = home if home is not None else get_socis_agent_home()
...
def write_drain_request(
    *,
    ...
    later survives a machine restart on the durable SOCIS_AGENT_HOME volume can be
...
def drain_requested(*, home: Optional[Path] = None) -> bool:
    ...
    treated as absent: it survived a container/VM restart (SOCIS_AGENT_HOME is a
    durable Fly volume on SOCIS Cloud) and the lifecycle action that triggered
...
def drain_notification_suppressed(*, home: Optional[Path] = None) -> bool:
    ...
    SOCIS_AGENT_HOME volume, or an expired same-epoch orphan (#85433), is
