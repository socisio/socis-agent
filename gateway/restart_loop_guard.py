...
Defenses 1 and 2 (the ``_SOCIS_AGENT_GATEWAY`` guard on ``socis gateway
...
raw ``terminal("launchctl kickstart -k gui/<uid>/ai.socis.gateway")``,
...
State lives in ``<SOCIS_AGENT_HOME>/gateway/restart_loop.json`` so it is
...
def _state_path():
    return get_socis_agent_home() / "gateway" / "restart_loop.json"
