...
1. ``flush_pending_to_file()`` — called BEFORE ``_pending_messages.clear()``
   ...
   under ``<socis_agent_home>/pending_messages/``.
...
def _get_flush_dir():
    """Return the pending-messages flush directory under the active SOCIS_AGENT_HOME."""
    from socis_agent_constants import get_socis_agent_home

    flush_dir = get_socis_agent_home() / "pending_messages"
...
def spool_dropped_transcript_message(
    ...
    ``<socis_agent_home>/pending_messages/``), so a runtime cap rotation no
