...
def _normalize_multiplex_profile_allowlist(value: Any) -> Optional[List[str]]:
    ...
    from socis_cli.profiles import normalize_profile_name, validate_profile_name
...
# dashboard's pre-write mutation validation (socis_cli/web_server.py) so
...
class PlatformConfig:
    ...
    # "SOCIS is thinking…"). Platforms with textless indicators (Discord,
...
class GatewayConfig:
    ...
    # Storage paths
    sessions_dir: Path = field(default_factory=lambda: get_socis_agent_home() / "sessions")
    ...
    # gateway behaves exactly as before — single SOCIS_AGENT_HOME, no profile stamping.
    ...
    # advertise a route. SOCIS_AGENT_ROOM_LINK_URL remains the operator override.
...
def load_gateway_config() -> GatewayConfig:
    ...
    2. ~/.socis-agent/config.yaml (primary user-facing config)
    3. ~/.socis-agent/gateway.json (legacy — provides defaults under config.yaml)
    4. Built-in defaults
    """
    _home = get_socis_agent_home()
    gw_data: dict = {}
    ...
    try:
        ...
        if config_yaml_path.exists():
            ...
            # through socis_cli.config.load_config, so without this a managed
            ...
            # (e.g. via `socis config set gateway.<key> ...`, which naturally
            ...
            if isinstance(gateway_section, dict):
                if "multiplex_profiles" in gateway_section and "multiplex_profiles" not in gw_data:
                    # gateway.multiplex_profiles written by `socis config set gateway.multiplex_profiles true`
            ...
            if not isinstance(streaming_cfg, dict) and isinstance(gateway_section, dict):
                ...
                # ``socis config set gateway.streaming.*``
