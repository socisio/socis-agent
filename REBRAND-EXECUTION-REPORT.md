...
## Critical bug found via live testing: corrupted OAuth client ID (login completely broken)

Live use of `socis auth` surfaced a real, blocking bug that no amount of static analysis had caught: **`socis_cli/auth.py`'s `DEFAULT_NOUS_CLIENT_ID` constant had been corrupted from `"hermes-cli"` to `"socis-cli"`**, and a second fallback value at a token-refresh call site had the same corruption. This is the OAuth `client_id` sent to **Nous Portal's real authorization server** during the device-code login flow — client IDs are pre-registered on the OAuth server and cannot be renamed unilaterally by the client. Sending an unregistered client_id causes the server to correctly reject the request:

```
Login failed: Client error '400 Bad Request' for url
'https://portal.nousresearch.com/api/oauth/device/code'
```

This is the same category of bug as the earlier `com.nousresearch.hermes` bundle identifier and the `inference-api.nousresearch.com` domain — a real third-party system identifier that the bare "hermes"→"socis" substitution rule corrupted because it couldn't distinguish "our own product name" from "a token a different company's server expects verbatim." Both occurrences reverted to `"hermes-cli"`.

**Why this one wasn't caught by static review:** every other OAuth client ID in the codebase (MiniMax, OpenAI Codex, xAI, Qwen, GitHub Copilot) is an opaque UUID or app-specific token, immune to a text-based find/replace by construction. Nous Portal's was the one exception — a human-readable string — which is exactly why it got silently corrupted while the others didn't, and why nothing in earlier greps for "suspicious identifiers" flagged it: it looked like completely ordinary, correctly-formed code.

**Broader implication for verification:** this bug was invisible to every check performed so far — syntax checks, import checks, URL diffing, even the targeted "does this reference real Nous infrastructure" sweep — because `client_id` values aren't URLs or domains, they're just short strings passed as a POST parameter. The only way it surfaced was by actually attempting to log in. This suggests any other **runtime-only-visible** identifier (API scopes, protocol version strings, feature flags sent to third-party APIs) could have the same latent bug and would only surface through further live use of each provider integration (MiniMax OAuth, xAI OAuth, Qwen OAuth, GitHub Copilot, etc.) — worth exercising each one at least once before considering the third-party integration surface fully verified.

## Setup wizard default changed to Full Setup (product decision, not a bug fix)

Per explicit request, the first-run setup menu's default option was changed from "Quick Setup (Nous Portal)" to "Full Setup" (which already existed and already lists every provider — OpenAI, GLM, xAI, MiniMax, Nous Portal, etc. — with no Nous dependency). This does not and cannot make a `socis-cli` OAuth client ID work against Nous Portal's server (that still requires the real registered `hermes-cli` value, explained earlier in this report) — but it does mean new users are no longer funneled toward Nous Portal by default, and can pick any provider first without hitting Nous Portal's login at all if they don't want to.

Changed in `socis_cli/setup.py`: reordered the three-option menu (Full Setup now index 0/default, Quick Setup moved to index 1, Blank Slate unchanged at index 2), updated both option labels, and adjusted the `if setup_mode == ...` branches to match the new indices. Updated the corresponding description in `website/docs/getting-started/quickstart.md` to match. The Chinese translation of this page doesn't contain the three-mode explanation section at all (appears to predate it), so no change was needed there.

## Web dashboard's own theme system had separate, previously-unaudited branding (found via screenshot)

A screenshot of the actual running web dashboard revealed gold/amber colors and "Nous"-named content that the earlier TUI/CLI rebrand pass never touched, because `web/src/themes/presets.ts` and `web/src/index.css` are a completely separate theming system from `ui-tui/src/theme.ts` — the dashboard renders the TUI's live output through an embedded terminal (PTY) connection, but has its own independent color palette for the surrounding dashboard chrome (sidebar, charts, page backgrounds).

Found and fixed:
1. **The default dashboard theme** ("SOCIS Teal") used an amber/cream palette (`background: #041c1c`, `midground: #ffe6cb`, warm amber glow) inherited unchanged from the original project. Per explicit confirmation, replaced with coral/navy matching the real brand: `background: #070A0F`, `midground: #FFD9E1` (a light coral-tint computed via the same `liftForContrast`-style approach as the TUI fix — verified 15.35:1 contrast against the new navy background, exceeding the original's 14.67:1). The theme's internal `name`/`label` fields (`"default"` / `"SOCIS Teal"`) were deliberately left unchanged since they're a persisted user setting (saved theme choice) — renaming the key would silently reset anyone's saved preference back to a default they didn't choose. Only the actual color values and the description text changed.
2. **A whole theme literally named "Nous Blue"** (`nousBlueTheme`, `label: "Nous Blue"`, description "vivid Nous-blue accents") — a real miss the earlier pass never found since it lives in `web/src/themes/presets.ts`, not any of the previously-audited directories. Renamed the display label to "Cobalt" (matching the actual hue) and description text in both the frontend (`presets.ts`) and its synced backend copy (`socis_cli/web_server.py`'s `_BUILTIN_DASHBOARD_THEMES` list, which a code comment explicitly requires to stay in sync). The internal `name: "nous-blue"` key was kept unchanged for the same persisted-setting reason as above — confirmed by finding an existing `THEME_NAME_ALIASES` backward-compatibility table in `context.tsx` built for exactly this scenario (a previous rename, `"lens-5i"` → `"nous-blue"`, needed this same key-stability treatment).
3. **Hardcoded CSS variable defaults** in `web/src/index.css` (`--midground-base: #ffe6cb`, `--series-input-token: #ffe6cb`, `--background-base: #041c1c`) — these are the values that paint before the JS theme system finishes applying inline overrides, and the values used server-side for chart rendering. Updated to match the new coral/navy default theme.
4. **One incorrect rename caught and reverted during this same pass**: a code comment referencing "the Nous DS" was initially (incorrectly) changed to "the SOCIS DS" before realizing — and confirming via three other files (`types.ts`, `sdk.d.ts`, `registry.ts`) all using the identical phrase — that "Nous DS" refers to the real, external `@nous-research/ui` component library this dashboard is built on (the same package confirmed as a legitimate third-party dependency earlier in this rebrand). Reverted to "Nous DS" — this is accurate attribution to the real library, not leftover branding.

Verified: 0 TypeScript grammar errors across the full `web` workspace (via its own `tsconfig.json`), 0 Python syntax errors in `web_server.py`.

**Root cause of why this was missed for so long:** the desktop app screenshot that surfaced this was initially assumed to be showing a stale build (since the TUI banner/mascot were already fixed) — but the dashboard's *own* separate theme system had never been audited at all, since earlier passes focused on `website/`, `ui-tui/`, and static image assets, not `web/`'s runtime theme definitions. This is a reminder that "the same product surface, rendered through a different transport" (native terminal vs. browser-embedded terminal) can still have independently-styled chrome around it that needs its own check.
