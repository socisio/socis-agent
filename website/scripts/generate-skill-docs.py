...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/socisio/socis-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    socis_meta = (fm.get("metadata") or {}).get("socis") or {}
    tags = socis_meta.get("tags") or []
    related = socis_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `socis skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
    ...
    if related:
        ...
        for r in related:
            ...
            if target_meta is not None:
                href = (
                    # baseUrl-relative, NOT "/docs/...". Docusaurus prepends
                    # baseUrl to absolute links UNLESS the path already starts
                    # with it — so "/docs/x" happened to work while baseUrl was
                    # "/docs/", and silently doubled to "/socis-agent/docs/docs/x"
                    # the moment baseUrl changed for the GitHub Pages deploy.
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with SOCIS Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "SOCIS ships with a large built-in skill library copied into `~/.socis-agent/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "SOCIS also syncs bundled skills on `socis update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.socis-agent/skills/` tree, it is still shipped with SOCIS; restore it with `socis skills reset <name> --restore`.",
        "",
