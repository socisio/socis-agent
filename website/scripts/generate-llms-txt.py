#!/usr/bin/env python3
"""Generate llms.txt and llms-full.txt for the SOCIS docs site.
...
LLM reads to learn what SOCIS does.

Both publish at:
  https://agent.socis.io/docs/llms.txt
  https://agent.socis.io/docs/llms-full.txt
...
SITE_BASE = "https://agent.socis.io/docs"
...
SECTIONS: list[tuple[str, list[tuple[str, str, str | None]]]] = [
    ...
    ("Guides & Tutorials", [
        ...
        ("guides/python-library", "Use SOCIS as a Python Library", None),
        ("guides/use-mcp-with-socis", "Use MCP with SOCIS", None),
        ("guides/use-voice-mode-with-socis", "Use Voice Mode with SOCIS", None),
        ("guides/use-soul-with-socis", "Use SOUL.md with SOCIS", None),
    ...
    ("Reference", [
        ...
        ("reference/skills-catalog", "Bundled Skills Catalog", "Table of all ~90 skills bundled with SOCIS"),
...
def emit_llms_index() -> str:
    ...
    lines: list[str] = []
    lines.append("# SOCIS Agent")
    lines.append("")
    lines.append(
        "> The self-improving AI agent built by SOCIS. A terminal-native "
    ...
    lines.append("")
    lines.append(
        "Install: `curl -fsSL https://raw.githubusercontent.com/socisio/"
        "socis-agent/main/scripts/install.sh | bash`  "
        "(Linux, macOS, WSL2, Termux)"
    )
    lines.append("")
    lines.append("Repo: https://github.com/socisio/socis-agent")
    lines.append("")
...
def emit_llms_full() -> str:
    ...
    seen: set[Path] = set()
    chunks: list[str] = [
        "# SOCIS Agent — Full Documentation\n",
        (
            "This file is the entire SOCIS Agent documentation concatenated for LLM "
            "context ingestion. Section order reflects docs-site navigation: Getting "
            "Started, Using SOCIS, Features, Messaging, Integrations, Guides, "
            "Developer Guide, Reference, then everything else.\n"
        ),
        "Canonical site: https://agent.socis.io/docs\n",
        "Short index: https://agent.socis.io/docs/llms.txt\n",
        "\n---\n\n",
    ]
