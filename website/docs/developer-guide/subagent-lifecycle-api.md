...
Plugins can launch and supervise fresh SOCIS child sessions without importing
...
active agent turn fails closed with `No active SOCIS parent session`.
...
overrides, and per-launch timeouts are explicitly rejected until SOCIS can
support them without weakening isolation. Use `allowed_toolsets` to narrow a
child; SOCIS's existing unsafe-tool block remains enforced.
