...
sidebar_position: 2
title: "Adding Tools"
description: "How to add a new tool to SOCIS Agent — schemas, handlers, registration, and toolsets"
---
...
This page is for adding a **built-in SOCIS tool** to the repository itself.
If you want a personal, project-local, or otherwise custom tool without
modifying SOCIS core, use the plugin route instead:

- [Plugins](/user-guide/features/plugins)
- [Build a SOCIS Plugin](/developer-guide/plugins)
...
2. **`toolsets.py`** — add tool name to `_SOCIS_AGENT_CORE_TOOLS` (or a specific toolset)
...
If your tool requires an API key, add it to `socis_cli/config.py`:
...
- [ ] Optional: API key added to `OPTIONAL_ENV_VARS` in `socis_cli/config.py`
- [ ] Optional: Added to `toolset_distributions.py` for batch processing
- [ ] Tested with `socis chat -q "Use the weather tool for London"`
