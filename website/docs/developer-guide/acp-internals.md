...
The ACP adapter wraps SOCIS' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
socis acp / socis-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.socis-agent/.env
  -> configure stderr logging
  -> construct SOCISACPAgent
...
- `allow_once` -> SOCIS `once`
- `allow_always` -> SOCIS `always`
- reject options -> SOCIS `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps SOCIS tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["socis-acp"])
...
Instead it reuses SOCIS' runtime resolver:

- `acp_adapter/auth.py`
- `socis_cli/runtime_provider.py`

So ACP advertises and uses the currently configured SOCIS provider/credentials. It also always advertises a terminal setup auth method (`socis-setup`, args `--setup`) so first-run ACP clients can open SOCIS' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.socis-agent/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `socis-acp` toolset definition
- `socis_cli/main.py` — `socis acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `socis-acp` script
