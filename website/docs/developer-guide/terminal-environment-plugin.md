...
SOCIS runs shell commands through a pluggable set of **terminal backends**.
...
installed under `~/.socis-agent/plugins/`, registering a backend the user selects
...
| `socis setup` backend picker | `display_name`, `description`, `setup_instructions()`, `post_setup()` |
| Dashboard terminal-backend picker (probe status) | `probe()` |
| `socis status` / `socis doctor` | `doctor_checks()` |
...
```python title="~/.socis-agent/plugins/acmebox/__init__.py"
...
class AcmeBoxProvider(TerminalEnvironmentProvider):
    ...
    def cache_path_base(self):
        return "~/.socis-agent"    # where synced cache files land, or None
...
```yaml title="~/.socis-agent/plugins/acmebox/plugin.yaml"
name: acmebox
version: 0.1.0
...
```bash
socis plugins enable acmebox
socis config set terminal.backend acmebox
```
