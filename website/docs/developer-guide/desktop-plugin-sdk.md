...
title: "Desktop Plugin SDK (@socis/plugin-sdk)"
description: "Extend the native SOCIS Agent Desktop app — panes, pages, sidebar nav, status bar, palette commands, keybinds, themes, and a scoped backend namespace, with one import and no build step."
---

# Desktop Plugin SDK

The native [SOCIS Agent Desktop](/user-guide/desktop) app is contribution-driven: every
...
A **desktop plugin** is a single ESM file that default-exports a `SOCISPlugin`.
It imports one module — `@socis/plugin-sdk` — and gets everything: the app's
...
`$SOCIS_AGENT_HOME/desktop-plugins/<id>/plugin.js` and the app loads it within seconds
and hot-reloads every save.

:::warning This is not the web-dashboard plugin SDK
"Plugin" means several unrelated things across SOCIS. This page is the **native
desktop app** (`socis desktop`) SDK — the `@socis/plugin-sdk` module and
`$SOCIS_AGENT_HOME/desktop-plugins/`. The **web dashboard** (`socis dashboard`) has
its own, unrelated plugin system on `window.__SOCIS_AGENT_PLUGIN_SDK__` with a
`manifest.json` — documented at
[Extending the Dashboard](/user-guide/features/extending-the-dashboard). Python
CLI/gateway plugins are documented at [Build a SOCIS Plugin](/developer-guide/plugins).
...
|------|-------|-----|------------|
| **Disk** (recommended) | `$SOCIS_AGENT_HOME/desktop-plugins/<id>/plugin.js` | users, agents | none — plain ESM, loaded uncompiled |
| **Unified package** | `$SOCIS_AGENT_HOME/plugins/<id>/desktop/plugin.js` | plugins that also ship agent-side code | none — same disk pipeline |
| **Bundled** | `apps/desktop/src/plugins/<id>/plugin.tsx` | in-tree, shipped with the app | the app's own Vite build |

All three take the same `SOCISPlugin` contract, appear in **Settings → Plugins**,
...
Create `$SOCIS_AGENT_HOME/desktop-plugins/hello/plugin.js` (that's `~/.socis-agent/...`
by default, or `~/.socis-agent/profiles/<name>/...` under a named profile). The folder
name must equal the plugin `id`.

```javascript
// ~/.socis-agent/desktop-plugins/hello/plugin.js
import { host, haptic, useValue } from '@socis/plugin-sdk'
...
function HelloPane() {
  ...
  return jsxs('div', {
    ...
    children: [
      jsx('div', { className: 'font-medium', children: 'Hello, SOCIS' }),
      jsx('div', {
...
The only importable specifiers are `@socis/plugin-sdk`, `react`, and
`react/jsx-runtime` — everything else fails to resolve, on purpose.
:::

## The plugin contract

A plugin default-exports a `SOCISPlugin`:

```ts
interface SOCISPlugin {
...
interface PluginContext {
  ...
  os: PluginOs
  /** Plugin-scoped JSON persistence (keys live under `socis.plugin.<id>.`). */
...
```javascript
import { ROUTES_AREA, SIDEBAR_NAV_AREA } from '@socis/plugin-sdk'
...
```javascript
import { STATUSBAR_AREAS, TITLEBAR_AREAS } from '@socis/plugin-sdk'
...
```javascript
import { PALETTE_AREA, KEYBINDS_AREA } from '@socis/plugin-sdk'
...
```javascript
import { THEMES_AREA } from '@socis/plugin-sdk'
...
```javascript
import { Button, useTheme } from '@socis/plugin-sdk'
...
```javascript
import { host, requestTheme } from '@socis/plugin-sdk'
...
```javascript
import { TRANSCRIPT_DIRECTIVE_AREA } from '@socis/plugin-sdk'
...
`window.socis.send('get-price eth')` (or a declarative
`<button data-socis-send="get-price eth">` — no script needed) hands that
...
```javascript
import { Contribute, TITLEBAR_AREAS } from '@socis/plugin-sdk'
...
for requests; `targetProfile` is the backend SOCIS profile served by that route.
...
approval/turn alerts use. It fires only while the user is away from SOCIS
...
ctx.os.notify({
  ...
  // Body click → focus SOCIS + navigate. Same vocabulary as OS deep links:
  activate: 'socis://index-network/intent/1',
  ...
  actions: [
    { id: 'open', label: 'Open', activate: 'socis://index-network/intent/1' },
    { id: 'dismiss', label: 'Dismiss', onAction: () => dismiss('1') },
  ],
})
```

`activate` is deeplink-compatible: `socis://index-network/intent/1` and the
hash path `/index-network/intent/1` resolve to the same in-app route (and the
same `socis://…` URL works as an OS deep link). Action buttons only render on
...
```javascript
import { useQuery, useMutation, useQueryClient, atom, computed, useValue } from '@socis/plugin-sdk'
...
```javascript
import { queryClient } from '@socis/plugin-sdk'
...
desktop app also scans `$SOCIS_AGENT_HOME/plugins/<id>/` — the regular agent-plugin
root — for a `desktop/plugin.js`, and loads it through the exact same pipeline
as the standalone disk door (hot reload included):

```
~/.socis-agent/plugins/<id>/           # ONE installable folder
...
a package into `~/.socis-agent/plugins` is inert on every surface until the user
...
backend, the remote box's `~/.socis-agent/plugins` is not reachable as a filesystem —
...
the `socis://` scheme — a plain anchor on your website or README:
...
`dashboard/` subfolder of a regular SOCIS plugin and declare it in a
`manifest.json`:

```
~/.socis-agent/plugins/<id>/
└── dashboard/
    ├── manifest.json      # { "name": "<id>", "api": "plugin_api.py" }
...
socis-agent codebase directly (`socis_agent_state`, `socis_cli.config`, …). See
...
`config.yaml` (and not in `plugins.disabled`). Project plugins (`./.socis-agent/`)
...
(`socis.plugin.<id>.*`) so plugins can't read or clobber each other:
...
export a `SOCISPlugin`). It's discovered by `discoverBundledPlugins()` at boot —
...
1. It goes through the app's Vite build, so you can write **real JSX** and import
   the SDK by its `@socis/plugin-sdk` alias.
2. It's still lint-fenced to `@socis/plugin-sdk` + `react` only — no `@/…` app
   internals.
...
- **Only three specifiers resolve:** `@socis/plugin-sdk`, `react`,
...
| Plugin contract | `SOCISPlugin`, `PluginContext`, `PluginContribution`, `PluginStorage`, `PluginOs`, `PluginRestOptions`, `PluginNativeNotificationInput`, `PluginNotificationAction`, `SOCISOpenTarget`, `Contribution` |
...
### Agents: the `socis-desktop-plugins` skill

When an agent writes a desktop plugin, it should load the bundled
**`socis-desktop-plugins`** skill — it carries the same contract as this page in
...
`$SOCIS_AGENT_HOME/desktop-plugins/<id>/plugin.js` and the folder name matches the
export `id`. Run ⌘K → **Reload desktop plugins**. Check the app for an error
toast naming the failure, and tail `socis logs gui -f`.

**"unsupported import" on load.** A disk plugin may only import
`@socis/plugin-sdk`, `react`, and `react/jsx-runtime`. Remove any other import.
...
`~/.socis-agent/plugins/<id>/dashboard/manifest.json` has `"api": "plugin_api.py"`,
that the plugin is in `plugins.enabled` in `config.yaml`, and restart the gateway
(backend routes mount at startup). Tail `~/.socis-agent/logs/errors.log` for
