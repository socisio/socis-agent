...
sidebar_position: 4
title: "Contributing"
description: "How to contribute to SOCIS Agent — dev setup, code style, PR process"
---

# Contributing

Thank you for contributing to SOCIS Agent! This guide covers setting up your dev environment, understanding the codebase, and getting your PR merged.
...
- Building a custom/local tool without modifying SOCIS core? Start with [Build a SOCIS Plugin](../developer-guide/plugins/index.md)
- Building a new built-in core tool for SOCIS itself? Start with [Adding Tools](./adding-tools.md)
...
You can also run a fully isolated SOCIS instance (throwaway SOCIS_AGENT_HOME, separate Electron
userData, distinct Electron app name to avoid the single-instance lock):

```bash
scripts/dev-sandbox.sh python -m socis_cli.main
scripts/dev-sandbox.sh --persistent python -m socis_cli.main desktop  # state survives restarts, but lives in the worktree :)
```
...
echo 'OPENROUTER_API_KEY=sk-or-v1-your-key' >> ~/.socis-agent/.env
```
...
- **Profile-safe paths**: Never hardcode `~/.socis-agent` — use `get_socis_agent_home()` from `socis_agent_constants` for code paths and `display_socis_agent_home()` for user-facing messages. See [AGENTS.md](https://github.com/socisio/socis-agent/blob/main/AGENTS.md#profiles-multi-instance-support) for full rules.
...
Projects built on (or reviewed by) SOCIS can keep reviewer checklists inside the repository under `.agents/checks/`. Each file is a focused, plain-markdown checklist that an agent loads before reviewing a change touching the matching area:
...
When you ask SOCIS to review a PR in a repository that has `.agents/checks/`, tell it (or teach it via a skill) to read the relevant checklists first and report against them. This gives review agents the project-specific bar that generic review prompts miss.
...
By contributing, you agree that your contributions will be licensed under the [MIT License](https://github.com/socisio/socis-agent/blob/main/LICENSE).
