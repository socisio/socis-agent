...
title: "Build a SOCIS Plugin"
description: "Step-by-step guide to building a complete SOCIS plugin with tools, hooks, data files, and skills"
---

# Build a SOCIS Plugin

This guide walks through building a complete SOCIS plugin from scratch. By the end you'll have a working plugin with multiple tools, lifecycle hooks, shipped data files, and a bundled skill — everything the plugin system supports.

:::info Not sure which guide you need?
SOCIS has several distinct pluggable interfaces — some use Python `register_*` APIs, others are config-driven or drop-in directories. Use this map first:
...
| An **STT backend** (custom whisper / ASR CLI) | [Voice Message Transcription](/user-guide/features/tts#voice-message-transcription-stt) — set `SOCIS_AGENT_LOCAL_STT_COMMAND` to an argv-tokenized template |
...
| **Gateway event hooks** (fire on startup, session events, commands) | [Event Hooks](/user-guide/features/hooks#gateway-event-hooks) — drop `HOOK.yaml` + `handler.py` into `~/.socis-agent/hooks/<name>/` |
...
| **Additional skill sources** (custom GitHub repos, private skill indexes) | [Skills](/user-guide/features/skills) — `socis skills tap add <repo>` · [Publishing a tap](/user-guide/features/skills#publishing-a-custom-skill-tap) |
...
Plugins that integrate **someone else's product or project** — observability/metrics backends, vendor SaaS connectors, analytics dashboards, paid-service tie-ins — are built and distributed as **standalone plugin repos**, not merged into `SOCIS/socis-agent`. Users install them into `~/.socis-agent/plugins/` or via a pip entry point; everything in this guide works the same way from a standalone repo. This is a coupling-and-maintenance decision (the core moves fast and we don't own your backend), not a quality bar — a plugin can be excellent and still belong in its own repo. Promote it in the SOCIS Discord `#plugins-skills-and-skins` channel. See [CONTRIBUTING.md](https://github.com/socisio/socis-agent/blob/main/CONTRIBUTING.md) for the policy.
:::

## Portable Agent Plugins v1 packages

SOCIS can also install and load directory packages that target the Agent
Plugins v1.0.0 format. This is a compatibility adapter for the portable
components SOCIS already owns. It does not replace native `plugin.yaml` plus
...
```bash
socis plugins install owner/repository --no-enable
socis plugins list
socis plugins enable <plugin-name>
```
...
SOCIS validates `plugin.json`, Agent Skills frontmatter, fixed component
...
profile-scoped writable directory managed by SOCIS.
...
Portable `streamable-http` entries are routed through SOCIS' existing native
...
same full-trust posture as other installed SOCIS plugins.
...
records it as Published. SOCIS keys behavior on the canonical v1.0.0 schema
...
not by one global plugin API number. SOCIS does not expose a
...
continue to work after a normal SOCIS upgrade.
...
- **Hook payloads are keyword payloads.** New hook data is added as keyword
  ...
  SOCIS inspects callback signatures: a legacy callback receives the fields it
...
- **Manifests are open to additions.** Unknown `plugin.yaml` fields are ignored.
  Older SOCIS releases can therefore load a plugin whose manifest contains
...
SOCIS enforces this contract with frozen external-plugin fixtures discovered
from an isolated `SOCIS_AGENT_HOME`. Those tests load and invoke the plugin through
...
```bash
mkdir -p ~/.socis-agent/plugins/calculator
cd ~/.socis-agent/plugins/calculator
```

### Validate with Plugin Doctor

`socis plugins doctor [path-or-id]` runs the same directory discovery,
manifest parser, namespaced import, `register(ctx)`, hook registry, and tool
registry used by SOCIS itself. It reports invalid hook names, callbacks that do
...
```bash
socis plugins doctor . --ci
```

Doctor uses a temporary `SOCIS_AGENT_HOME`, restores plugin registration state after
...
This tells SOCIS: "I'm a plugin called calculator, I provide tools and hooks." The `provides_tools` and `provides_hooks` fields are lists of what the plugin registers.
...
`llm.task_override` (see `socis_cli/plugin_capabilities.py` for the
...
`socis_agent.plugin_capabilities` entry-point group. Each declaration is
named `<plugin-id>.<capability-id>` and points at the same object as your
`socis_agent.plugins` entry point:

```toml
[project.entry-points."socis_agent.plugins"]
calculator = "my_pkg:register"

[project.entry-points."socis_agent.plugin_capabilities"]
"calculator.tools.override" = "my_pkg:register"
```

SOCIS reads these from installed metadata without importing your code, so
`socis plugins capabilities` and the consent flow stay accurate for pip
installs.
...
this SOCIS understands still loads with a warning.
...
| `python_dependencies` | list of str | Declared pip requirements (e.g. `"requests>=2.0,<3"`). **Declaration seam only** — SOCIS validates them, and `socis plugins install` / `socis plugins doctor` surface missing ones with a `pip install` hint, but SOCIS **never auto-installs** them. Pin upper bounds. |
...
arbitrary packages into SOCIS' shared venv is a conflict and supply-chain
...
[#64165](https://github.com/socisio/socis-agent/issues/64165) and
[#15220](https://github.com/socisio/socis-agent/issues/15220). Plugin
...
4. **Accept `**kwargs`:** SOCIS may pass additional context in the future.
...
- `ctx.register_cli_command()` registers a CLI subcommand (e.g. `socis my-plugin <subcommand>`)
...
- If this function crashes, the plugin is disabled but SOCIS continues fine
...
Use plugin-relative config keys for user-visible behavior. SOCIS resolves them
...
`<SOCIS_AGENT_HOME>/plugin-data/`. Neither API exposes another plugin's namespace.
...
If your plugin doesn't show up — or shows up but isn't loading — set `SOCIS_AGENT_PLUGINS_DEBUG=1` to get verbose discovery logs on stderr:

```bash
SOCIS_AGENT_PLUGINS_DEBUG=1 socis plugins list
```
...
The same logs are always written to `~/.socis-agent/logs/agent.log` at WARNING level (failures only) and DEBUG level (everything) when the env var is set. So if you can't run with the env var (e.g. from inside the gateway), tail the log file instead:

```bash
socis logs --level WARNING | grep -i plugin
```

Common reasons a plugin doesn't appear:

- **Not enabled in config** — plugins are opt-in. Run `socis plugins enable <name>` (the name comes from the `plugins list` output, which can be `<category>/<plugin>` for nested layouts).
- **Wrong directory layout:** Native packages use `~/.socis-agent/plugins/<plugin-name>/plugin.yaml` (flat) or one category level. Portable packages use root `plugin.json` in the same locations. Anything deeper is ignored.
...
```
~/.socis-agent/plugins/calculator/
...
tree, and `socis plugins update` / `remove` git-pull or delete it — your
...
# <socis home>/plugin-data/<name>/ — created on first use
...
**Key properties:**
- Plugin skills are **read-only** — they don't enter `~/.socis-agent/skills/` and can't be edited via `skill_manage`.
...
:::tip Legacy pattern
The old `shutil.copy2` pattern (copying a skill into `~/.socis-agent/skills/`) still works but creates name collision risk with built-in skills. Prefer `ctx.register_skill()` for new plugins.
:::
...
When users run `socis plugins install`, they're **prompted interactively** for any missing `requires_env` variables. Values are saved to `.env` automatically.
...
| Rule | Why |
|---|---|
| Your feature key must appear in the in-tree `LAZY_DEPS` allowlist | Prevents a malicious config from coaxing SOCIS into installing arbitrary packages — only specs SOCIS itself ships are eligible |
...
auditable in `~/.socis-agent/logs/agent.log`. Plugins load after built-in
...
does not ship with SOCIS core (user, project, or pip source),
...
plugins:
  entries:
    my-plugin:                    # the plugin's registry key from `socis plugins list`
...
loader, the plugin is disabled and SOCIS continues. The gate exists
...
You normally never edit this key by hand. `socis plugins enable <name>`
...
This is the only hook whose return value matters. When a `pre_llm_call` callback returns a dict with a `"context"` key (or a plain string), SOCIS injects that text into the **current turn's user message**. This is the mechanism for memory plugins, RAG integrations, guardrails, and any plugin that needs to provide the model with additional context.

#### Return format

```python
# Dict with context key
return {"context": "Recalled memories:\n- User prefers dark mode\n- Last project: socis-agent"}
...
Per-hook context is capped at `10,000` characters by default. Anything above the cap is written to `$SOCIS_AGENT_HOME/hook_outputs/<session_id>/<uuid>.txt` and replaced with a head/tail preview plus the saved path. The model can read the full content via `read_file` or `terminal` if it genuinely needs it. This keeps a runaway plugin from inflating every subsequent turn's prompt and blowing out the prompt cache prefix. Tune in `config.yaml`:
...
hooks:
  output_spill:
    ...
    # directory: null      # default: $SOCIS_AGENT_HOME/hook_outputs
```
...
- **The system prompt is SOCIS's territory** — it contains model-specific guidance, tool enforcement rules, personality instructions, and cached skill content. Plugins contribute context alongside the user's input, not by altering the agent's core instructions.
...
The canonical list of kinds is `VALID_MIDDLEWARE` in `socis_cli/middleware.py`:
...
| `llm_request` | `request`, `original_request`, context kwargs | Return `{"request": {...}}` to replace the effective provider kwargs before SOCIS sends them. |
...
- Middleware payloads carry `middleware_schema_version` (`socis.middleware.v1`) alongside the observer telemetry fields.
- Unknown kinds register with a warning instead of failing, so a plugin written against a newer SOCIS still loads on an older one.

### Register CLI commands

Plugins can add their own `socis <plugin>` subcommand tree:

```python
def _my_command(args):
    """Handler for socis my-plugin <subcommand>."""
    ...
    if sub == "status":
        ...
        print("Usage: socis my-plugin <status|config>")

def _setup_argparse(subparser):
    """Build the argparse tree for socis my-plugin."""
...
After registration, users can run `socis my-plugin status`, `socis my-plugin config`, etc.
...
|---|---|---|
| Invoked as | `/name` in a session | `socis name` in a terminal |
...
`ctx._cli_ref` is only populated in an **interactive CLI** session. It is `None` in the gateway, in non-interactive `socis chat -q` runs, and in **kanban-spawned worker sessions** — so any plugin logic that reaches through `_cli_ref` silently no-ops in exactly those contexts. Two stable, session-agnostic APIs cover what hooks actually need:

- **`ctx.profile_name`** — the active profile name (e.g. `"default"`, or the assignee profile in a kanban worker). Derived from `SOCIS_AGENT_HOME`, so it works everywhere with no `_cli_ref` dependency.
...
For running a full `socis <subcommand>` (e.g. `socis kanban show`), shell out with the `terminal` tool via `ctx.dispatch_tool("terminal", {"command": "socis kanban show ..."})` — there is no in-process slash-command bridge for headless worker sessions, and tools are the supported way to drive SOCIS from a hook.
...
SOCIS has five specialized plugin types beyond the general surface. Each ships as a directory under `plugins/<category>/<name>/` (bundled) or `~/.socis-agent/plugins/<category>/<name>/` (user). The contract differs by category — pick the one you need, then read its full guide.
...
def register(ctx):
    ctx.register_platform(
        ...
        # show up in `socis gateway status` without SDK instantiation.
...
SOCIS also accepts extensions that aren't Python plugins at all. These are shown in the [Pluggable interfaces table](/user-guide/features/plugins#pluggable-interfaces--where-to-go-for-each); the sections below sketch each authoring style briefly.

### MCP servers — register external tools

Model Context Protocol (MCP) servers register their own tools into SOCIS without any Python plugin. Declare them in `~/.socis-agent/config.yaml`:
...
SOCIS connects to each server at startup, lists its tools, and registers them alongside built-ins. The LLM sees them exactly like any other tool. **Full guide:** [MCP](/user-guide/features/mcp).

### Gateway event hooks — fire on lifecycle events

Drop a manifest + handler into `~/.socis-agent/hooks/<name>/`:

```yaml
# ~/.socis-agent/hooks/long-task-alert/HOOK.yaml
...
```python
# ~/.socis-agent/hooks/long-task-alert/handler.py
...
socis skills search my-workflow --source myorg/skills-repo
socis skills install myorg/skills-repo/my-workflow
```
...
```bash
pip install socis-plugin-calculator
# Plugin auto-discovered on next socis startup
```
...
```python
# Wrong — will break if SOCIS passes extra context
def handler(args):
    ...
