...
sidebar_position: 1
title: "Architecture"
description: "SOCIS Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of SOCIS Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ socis_agent_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # SOCISCLI — interactive terminal UI (large file)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── socis_agent_state.py           # SQLite session/state database with FTS5
├── socis_agent_constants.py       # SOCIS_AGENT_HOME, profile-aware paths
...
│
├── socis_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `socis` subcommands (large file)
...
│   ├── skills_config.py      # socis skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # socis tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # socis gateway start/stop
│
...
```text
User input → SOCISCLI.process_input()
...
Three discovery sources: `~/.socis-agent/plugins/` (user), `.socis-agent/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `socis plugins` or `config.yaml`.
...
Exposes SOCIS as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`socis -p <name>`) gets its own SOCIS_AGENT_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
