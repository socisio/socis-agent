...
| Variable | Meaning |
|----------|---------|
| `SOCIS_AGENT_MAIN_CHECKOUT` | The deps checkout — where `node_modules` really lives, and whose `.venv/bin/python` runs the backend. |
| `SOCIS_AGENT_GUI_DEPS_CHECKOUT` | Where the desktop deps (`apps/desktop/node_modules`) live. Defaults to `SOCIS_AGENT_MAIN_CHECKOUT`; override only if you keep desktop deps elsewhere. |

Neither is read by SOCIS itself — they're private to these helpers. The variables SOCIS *does* read are covered in [Environment Variables](../reference/environment-variables.md).

## `htui` — TUI from the worktree

The Ink TUI has a dev path already: `socis --tui --dev` runs the TypeScript sources via `tsx` instead of the prebuilt bundle. `htui` is a one-liner over it that also points the run at the current worktree's `ui-tui/`:

```bash
htui() {
  local root
  root="$(_socis_root)" || { echo "htui: not in a SOCIS checkout" >&2; return 1; }
  ( cd "$root" && PYTHONPATH="$root" \
      "$SOCIS_AGENT_MAIN_CHECKOUT/.venv/bin/python" -m socis_cli.main --tui --dev "$@" )
}
```

`--dev` compiles from source, so it links `ui-tui/node_modules` from `SOCIS_AGENT_MAIN_CHECKOUT` when the root lockfile matches and installs locally otherwise (see [`_socis_root` / linking helpers](#shared-helpers)).

:::warning `--dev` and `SOCIS_AGENT_TUI_DIR` are mutually exclusive
`SOCIS_AGENT_TUI_DIR` points SOCIS at a *prebuilt* bundle (Nix, system packages), which has no source to hot-reload. If it's set in your shell, `socis --tui --dev` exits with an error. Run `unset SOCIS_AGENT_TUI_DIR` before `htui`.
:::
...
hgui() {
  ...
  root="$(_socis_root)" || { echo "hgui: not in a SOCIS checkout" >&2; return 1; }
  deps="${SOCIS_AGENT_GUI_DEPS_CHECKOUT:-$SOCIS_AGENT_MAIN_CHECKOUT}"
  ...
  if cmp -s "$root/package-lock.json" "$deps/package-lock.json"; then
    _socis_link_deps "$desktop" "$deps/apps/desktop"
    _socis_link_deps "$root" "$deps"
  else
  ...
  trap '_socis_gui_cleanup "$root"' INT TERM EXIT

  ( cd "$desktop"
    export PATH="$root/node_modules/.bin:$PATH"
    SOCIS_AGENT_DESKTOP_SOCIS_AGENT_ROOT="$root" \
    SOCIS_AGENT_DESKTOP_PYTHON="$SOCIS_AGENT_MAIN_CHECKOUT/.venv/bin/python" \
    SOCIS_AGENT_DESKTOP_IGNORE_EXISTING=1 \
    SOCIS_AGENT_DESKTOP_CWD="$root" \
    npm run dev )
}
```
...
|----------|----------------|
| `SOCIS_AGENT_DESKTOP_SOCIS_AGENT_ROOT` | Runs the backend from **this worktree**, not the packaged/PATH `socis`. |
| `SOCIS_AGENT_DESKTOP_PYTHON` | Reuses the deps checkout's venv instead of re-resolving a Python. |
| `SOCIS_AGENT_DESKTOP_IGNORE_EXISTING` | Ignores any `socis` on `PATH` so it can't shadow the worktree. |
| `SOCIS_AGENT_DESKTOP_CWD` | Opens the desktop chat rooted at the worktree. |
...
```bash
# The enclosing worktree, verified as a real SOCIS checkout.
_socis_root() {
  local root
  root="$(git rev-parse --show-toplevel 2>/dev/null)" || return 1
  [[ -f "$root/socis_cli/main.py" && -d "$root/ui-tui" ]] && print -r "$root"
}
...
_socis_gui_cleanup() {
  ...
  lsof -t -i:5174 >/dev/null 2>&1 && killport 5174
  pgrep -f 'socis_cli\.main.*dashboard.*--port 0' 2>/dev/null | xargs -r kill -TERM 2>/dev/null
}
```
...
- [TUI](../user-guide/tui.md) — `socis --tui --dev` and the `SOCIS_AGENT_TUI_DIR` prebuild path
- [Desktop App](../user-guide/desktop.md) — building from source and the backend resolution ladder
- [`apps/desktop/README.md`](https://github.com/socisio/socis-agent/blob/main/apps/desktop/README.md) — dev server, sandbox script, and packaging
- [Environment Variables](../reference/environment-variables.md) — every `SOCIS_AGENT_*` variable SOCIS reads
