...
Connect SOCIS to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
SOCIS supports two WeCom integration modes:
...
> Run `socis gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. SOCIS decrypts the message, queues it for the agent
...
(Use `socis gateway start` only after `socis gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in SOCIS and the token the
...
in `~/.socis-agent/.env` values around `=` will also break signature checks. After
fixing, restart `socis gateway run`.
...
Check `socis gateway run` logs for the bound host/port. If the adapter bound to
