...
description: Control your smart home with SOCIS Agent via Home Assistant integration.
...
SOCIS Agent integrates with [Home Assistant](https://www.home-assistant.io/) in two ways:
...
4. Click **Create Token**, give it a name like "SOCIS Agent"
5. Copy the token
...
SOCIS Agent registers four tools for smart home control:
...
Configure which events the agent sees in `~/.socis-agent/config.yaml` under the Home Assistant platform's `extra` section:
...
Outbound messages from the agent are delivered as **Home Assistant persistent notifications** (via `persistent_notification.create`). These appear in the HA notification panel with the title "SOCIS Agent".
...
The adapter reads credentials from `~/.socis-agent/.env` (auto-merged at startup) or
from `config.yaml`. Double-check the file lives under the active SOCIS profile
...
running SOCIS — `curl -H "Authorization: Bearer <token>" <url>/api/` should
