...
If you only run one SOCIS agent, you don't need this page — see
...
[Connecting Desktop to Many SOCIS Instances](./multi-connection-desktop.md).

## When to use this

You want this setup when you have two or more SOCIS agents that should all
...
(`ai.socis.gateway-<name>.plist`) or systemd user service
(`socis-gateway-<name>.service`). This guide adds the patterns for managing
them collectively.
...
socis profile create coder
socis profile create personal-bot
socis profile create research
...
```bash
socis config set gateway.multiplex_profiles true
socis gateway restart
```

Equivalently, in the default profile's `~/.socis-agent/config.yaml`:
...
You do **not** run `socis gateway start` for the secondary profiles — the
...
With a multiplexer running, a named-profile `socis gateway start` / `run` is a
...
- `/p/coder/...` API-server requests must use `API_SERVER_KEY` from
  `~/.socis-agent/profiles/coder/.env`; the default listener key is rejected.
...
home). `socis status` reports the multiplexer and the profiles it serves;
`socis status -p <name>` slices to one profile. Each profile still writes its
...
`~/.local/bin/socis-gateways` and `chmod +x` it:
...
usage() {
  echo "Usage: socis-gateways {start|stop|restart|status|list}"
}
...
run_for_profile() {
  ...
  if [ "$profile" = "default" ]; then
    socis gateway "$action"
  else
    socis -p "$profile" gateway "$action"
  fi
}
...
```bash
socis-gateways start      # start every configured profile
socis-gateways stop       # stop every configured profile
socis-gateways restart    # restart all
socis-gateways status     # status across all
socis-gateways list       # delegates to `socis gateway list`
```

:::tip
The `default` profile is targeted with `socis gateway <action>` (no `-p`),
not `socis -p default gateway <action>`. The wrapper above handles both forms.
:::
...
These are equivalent to `socis -p coder gateway <action>` — useful if a
...
| Platform | Path                                                              |
| -------- | ----------------------------------------------------------------- |
| macOS    | `~/Library/LaunchAgents/ai.socis.gateway-<profile>.plist`        |
| Linux    | `~/.config/systemd/user/socis-gateway-<profile>.service`         |

The default profile keeps the historical names: `ai.socis.gateway.plist` /
`socis-gateway.service`.
...
```bash
# Default profile
tail -f ~/.socis-agent/logs/gateway.log
tail -f ~/.socis-agent/logs/gateway.error.log

# Named profile
tail -f ~/.socis-agent/profiles/<name>/logs/gateway.log
tail -f ~/.socis-agent/profiles/<name>/logs/gateway.error.log
```

Stream every profile's log simultaneously:

```bash
tail -f ~/.socis-agent/logs/gateway.log ~/.socis-agent/profiles/*/logs/gateway.log
```

The CLI also has a structured log viewer:

```bash
socis logs -f                  # follow default profile
socis -p coder logs -f         # follow one profile
socis logs --help              # filters, levels, JSON output
```

## Identify what's actually running

```bash
socis profile list             # profiles + model + gateway state
socis-gateways status          # full status across every profile
launchctl list | grep socis    # macOS — PIDs and labels
systemctl --user list-units 'socis-gateway-*'   # Linux — units
```
...
```
~/.socis-agent/profiles/<name>/
├── .env              # API keys, bot tokens (chmod 600)
├── config.yaml       # model, provider, toolsets, gateway settings
└── SOUL.md           # personality / system prompt
```

The default profile uses `~/.socis-agent/` directly with the same three files.

Edit them with any editor or via the CLI:

```bash
socis config set model.model anthropic/claude-sonnet-4    # default profile
...
caffeinate -i -w $(cat ~/.socis-agent/gateway.pid) &   # awake while default gateway runs
...
systemd-inhibit --what=idle:sleep --who=socis --why="gateways running" \
  sleep infinity &
...
`socis-gateway-<profile>.service`) continue running across SSH disconnects
and reboots.
...
grep -H 'TELEGRAM_BOT_TOKEN\|DISCORD_BOT_TOKEN' \
     ~/.socis-agent/.env ~/.socis-agent/profiles/*/.env
```

## Updating the code

`socis update` pulls the latest code once and syncs new bundled skills into
every profile:
...
You ran `socis gateway start` after a previous `socis gateway stop`. The
...
```bash
ps -ef | grep "socis_cli.*-p <profile>"
cat ~/.socis-agent/profiles/<profile>/gateway.pid
...
```bash
# macOS
launchctl unload ~/Library/LaunchAgents/ai.socis.gateway-<profile>.plist
launchctl load   ~/Library/LaunchAgents/ai.socis.gateway-<profile>.plist

# Linux
systemctl --user restart socis-gateway-<profile>.service
```

### Health check

```bash
socis doctor                  # default profile
socis -p <profile> doctor     # one profile
```
