...
# Connecting Desktop to Many SOCIS Instances

Register every SOCIS backend you own — the local runtime, remote gateways on
your LAN or VPS, SSH hosts, and SOCIS Cloud instances — in one desktop app,
...
- **The sidebar profile rail** — the plug button at the right end of the rail
  (tooltip: **"Connect another SOCIS gateway…"**) deep-links straight to
...
list of SOCIS gateways. Its intro says it plainly: *"Manage this device and
every SOCIS gateway it can reach through remote, SSH, or Cloud connections."*
Each entry is a *connection*:

| Kind | What it is | Auth |
|---|---|---|
| **Local** | "The SOCIS runtime managed by this app." | automatic |
| **Remote gateway** | "A SOCIS gateway reachable over HTTP(S) — LAN, Tailscale, or the internet." | session token or OAuth |
| **SSH** | "A SOCIS install reached over SSH." The app opens the tunnel and starts the dashboard for you | SSH key + adopted token |
| **SOCIS Cloud** | "A hosted instance discovered through your SOCIS Cloud account." | portal sign-in |
...
- **Duplicates are rejected when you save**: there is only ever one **local**
  ...
  **SSH** entries are deduplicated on the normalized `user@host:port` plus
  remote profile.
- Cloud entries normally come from the SOCIS Cloud sign-in/discovery flow at
  the top of the Gateways page — the **SOCIS Cloud** kind in the add-connection
...
3. Pick the kind: **Local**, **SOCIS Cloud**, **Remote gateway**, or **SSH**.
   (**Local** is disabled while the app-managed local entry exists — which is
   almost always; **SOCIS Cloud** directs you to the cloud sign-in/discovery
   flow above.)
4. Fill the fields:
   - **Name** — required, unique; the "device name" shown everywhere this
     instance appears (placeholder: `Homelab`). Max 64 characters.
   - *Remote gateway only:*
     - **Gateway URL** — the base URL of a running `socis serve` backend,
...
Gateways are machines or hosted backends; profiles are isolated SOCIS agents
...
connection is registered) dispatches `socis update` to every eligible
...
- **SOCIS Cloud** instances are skipped with a *"Managed by SOCIS Cloud"*
...
panel, ⌘K **Update SOCIS**, the update-ready toast) run the same fan-out
...
- **"Connection test failed"** — the backend isn't reachable at that URL from
  this machine. Check that `socis serve` is running on the remote host, the
...
- **"Update SOCIS Agent Desktop to chat with agents on other connections"** — the
