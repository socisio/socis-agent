...
sidebar_position: 1
title: "CLI Interface"
description: "Master the SOCIS Agent terminal interface — commands, keybindings, personalities, and more"
---

# CLI Interface

SOCIS Agent's CLI is a full terminal user interface (TUI) — not a web UI. It features multiline editing, slash-command autocomplete, conversation history, interrupt-and-redirect, and streaming tool output. Built for people who live in the terminal.

:::tip First-time setup
One command — `socis setup --portal` — and you're ready to `socis chat`. See [Nous Portal](/integrations/nous-portal).
:::

:::tip
SOCIS also ships a modern TUI with modal overlays, mouse selection, and non-blocking input. Launch it with `socis --tui` — see the [TUI](tui.md) guide.
:::
...
socis chat --query-file prompt.txt
socis chat --query-file - < prompt.txt

# With a specific model
socis chat --model "anthropic/claude-sonnet-4"

# With a specific provider
socis chat --provider nous        # Use Nous Portal
socis chat --provider openrouter  # Force OpenRouter
...
socis chat -s github-pr-workflow -q "open a draft PR"

# Resume previous sessions
socis --continue             # Resume the most recent CLI session (-c)
socis --resume <session_id>  # Resume a specific session by ID (-r)
socis --resume latest        # Resume the most recent session (same as -c)
socis --resume latest --in ./dir  # Resume ./dir's latest session, staying in ./dir

# Verbose mode (debug output)
socis chat --verbose

# Isolated git worktree (for running multiple agents in parallel)
socis -w                         # Interactive mode in worktree
socis -w -z "Fix issue #123"     # Single query in worktree
```

### Worktree cleanup

`socis -w` sessions create disposable worktrees under `<repo>/.worktrees/`.
...
```bash
socis worktree list              # audit: age, size, verdict, reason per tree
socis worktree prune             # remove safe trees + delete merged branches
socis worktree prune --dry-run   # show the plan without changing anything
socis worktree prune --trees-only     # leave local branches alone
socis worktree prune --branches-only  # leave worktrees alone
```
...
- Trees **in use by a running socis session** are never touched.
- **Untracked-only scratch** (PR body drafts, notes) is archived to
  `~/.socis-agent/archive/worktree-prune/` before its tree is removed — never
  destroyed.
...
launches `socis -w` for days — no longer accumulate merged scratch trees
...
The `socis plugins` commands manage native SOCIS plugins and portable Agent
...
socis plugins disable <plugin-name>
socis plugins update <plugin-name>
socis plugins remove <plugin-name>
```

Portable packages remain disabled until explicitly enabled. SOCIS currently
...
<img className="docs-terminal-figure" src="/docs/img/docs/cli-layout.svg" alt="Stylized preview of the SOCIS CLI layout showing the banner, conversation area, and fixed input prompt." />
<p className="docs-figure-caption">The SOCIS CLI banner, conversation stream, and fixed input prompt rendered as a stable docs figure instead of fragile text art.</p>
...
| ⚠ YOLO | **YOLO mode warning** — shown whenever `SOCIS_AGENT_YOLO_MODE` is on (either `socis --yolo` at launch or `/yolo` toggled mid-session). Mirrors the banner-line warning so you can't forget you're in auto-approve mode. |
...
| `Ctrl+D` | Exit |
| `Ctrl+Z` | Suspend SOCIS to background (Unix only). Run `fg` in the shell to resume. |
...
- **Approvals still apply.** A dangerous command (`rm -rf`, writes to `~/.socis-agent/config.yaml`, etc.) goes through the same approval prompt the agent's `terminal` tool uses. `!` is a cost/latency shortcut, not a security bypass.
...
Type `/` to see the autocomplete dropdown. SOCIS supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
| `/voice tts` | Toggle spoken playback for SOCIS replies |
...
```bash
socis -s socis-agent-dev,github-auth
socis chat -s github-pr-workflow -s github-auth
```

SOCIS loads each named skill into the session prompt before the first turn. The same flag works in interactive mode and single-query mode.

## Skill Slash Commands

Every installed skill in `~/.socis-agent/skills/` is automatically registered as a slash command. The skill name becomes the command:
...
You can also define custom personalities in `~/.socis-agent/config.yaml`:

```yaml
personalities:
  helpful: "You are a helpful, friendly AI assistant."
  kawaii: "You are a kawaii assistant! Use cute expressions..."
  pirate: "Arrr! Ye be talkin' to Captain SOCIS..."
  # Add your own!
```
...
`Ctrl+J` and backslash continuation are enabled by default, matching Claude Code / Codex / OpenCode multiline shortcuts. On supported terminals such as iTerm2, SOCIS also requests extended key reporting so `Shift+Enter` arrives as a distinct newline key. If your terminal sends LF for plain `Enter` and you need the legacy `Ctrl+J`-as-submit fallback, opt out:
...
Most terminals send the same byte sequence for `Enter` and `Shift+Enter` by default, so applications cannot distinguish them. SOCIS recognises `Shift+Enter` only when the terminal sends a distinct sequence via the [Kitty keyboard protocol](https://sw.kovidgoyal.net/kitty/keyboard-protocol/) or xterm's `modifyOtherKeys` mode.
...
Where the terminal cannot distinguish them, `Alt+Enter` and `Ctrl+J` continue to work by default. **On Windows Terminal specifically, `Alt+Enter` is captured by the terminal (toggles fullscreen) and never reaches SOCIS — use `Ctrl+Enter` (delivered as `Ctrl+J`) or `Ctrl+J` directly for a newline.**
...
The first time you press Enter while SOCIS is working, SOCIS prints a one-line reminder explaining the `/busy` knob. It only fires once per install; `onboarding.seen.busy_input_prompt` in `config.yaml` records that it was shown. Delete that key to see the tip again.
:::

### Suspending to Background

On Unix systems, press **`Ctrl+Z`** to suspend SOCIS to the background — just like any terminal process. The shell prints a confirmation:

```
SOCIS Agent has been suspended. Run `fg` to bring SOCIS Agent back.
```
...
Resume this session with:
  socis --resume 20260225_143052_a1b2c3
...
```bash
socis --continue                          # Resume the most recent CLI session
socis -c                                  # Short form
socis -c "my project"                     # Resume a named session (latest in lineage)
socis --resume 20260225_143052_a1b2c3     # Resume a specific session by ID
socis --resume "refactoring auth"         # Resume by title
socis --resume latest                     # Resume the most recent session (same as -c)
socis --resume latest --in ./my-project   # Latest session for ./my-project's workspace
socis -r 20260225_143052_a1b2c3           # Short form
```

Resuming restores the full conversation history from SQLite. The agent sees all previous messages, tool calls, and responses — just as if you never left.

Use `/title My Session Name` inside a chat to name the current session, or `socis sessions rename <id> <title>` from the command line. Use `socis sessions list` to browse past sessions.

### Session Storage

CLI sessions are stored in SOCIS's SQLite state database under `~/.socis-agent/state.db`. The database keeps:
...
SOCIS immediately confirms the task and gives you back the prompt:
