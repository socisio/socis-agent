---
title: "Openclaw Migration — Import an OpenClaw setup (memories, skills) into SOCIS"
sidebar_label: "Openclaw Migration"
description: "Import an OpenClaw setup (memories, skills) into SOCIS"
---
...
Import an OpenClaw setup (memories, skills) into SOCIS.
...
| Tags | `Migration`, `OpenClaw`, `SOCIS`, `Memory`, `Persona`, `Import` |
...
# OpenClaw -> SOCIS Migration

Use this skill when a user wants to move their OpenClaw setup into SOCIS Agent with minimal manual cleanup.
...
```bash
socis claw migrate              # Full interactive migration
...
socis claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `socis setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_socis.py` to:

- import `SOUL.md` into the SOCIS home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into SOCIS memory entries
- merge OpenClaw command approval patterns into SOCIS `command_allowlist`
- migrate SOCIS-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map OpenClaw workspace settings to SOCIS working-directory configuration
- copy OpenClaw skills into `~/.socis-agent/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen SOCIS workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.socis-agent/tts/`
- archive non-secret docs that do not have a direct SOCIS destination
...
- `scripts/openclaw_to_socis.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.socis-agent/skills/migration/openclaw-migration/scripts/openclaw_to_socis.py`

Do not guess a shorter path like `~/.socis-agent/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.socis-agent/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of SOCIS-compatible secrets, currently:
...
SOCIS CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in SOCIS. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a SOCIS workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing SOCIS skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python ~/.socis-agent/skills/migration/openclaw-migration/scripts/openclaw_to_socis.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python /home/USER/.socis-agent/skills/migration/openclaw-migration/scripts/openclaw_to_socis.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python ~/.socis-agent/skills/migration/openclaw-migration/scripts/openclaw_to_socis.py --preset user-data
```

Execute a user-data migration:

```bash
python ~/.socis-agent/skills/migration/openclaw-migration/scripts/openclaw_to_socis.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python ~/.socis-agent/skills/migration/openclaw-migration/scripts/openclaw_to_socis.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python ~/.socis-agent/skills/migration/openclaw-migration/scripts/openclaw_to_socis.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of SOCIS unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty SOCIS targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean SOCIS destination. Unsupported auth blobs must still be reported as skipped.
...
- SOCIS persona state imported
- SOCIS memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.socis-agent/skills/openclaw-imports/`
