...
| | |
|---|---|
| Source | Optional — install with `socis skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${SOCIS_AGENT_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${SOCIS_AGENT_SKILL_DIR}/scripts/drive.sh` for private Drive storage
...
| `--client {name}`      | Agent name for attribution (e.g. `socis`)    |
