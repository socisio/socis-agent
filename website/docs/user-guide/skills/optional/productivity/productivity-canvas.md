...
| | |
|---|---|
| Source | Optional — install with `socis skills install official/productivity/canvas` |
...
4. Name the token (e.g., "SOCIS Agent"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `${SOCIS_AGENT_HOME:-~/.socis-agent}/.env`:
...
```bash
CANVAS="python $SOCIS_AGENT_HOME/skills/productivity/canvas/scripts/canvas_api.py"
