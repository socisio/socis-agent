...
This optional skill gives SOCIS practical phone capabilities while keeping telephony out of the core tool list.

It ships with a helper script, `scripts/telephony.py`, that can:
- save provider credentials into `${SOCIS_AGENT_HOME:-~/.socis-agent}/.env`
...
It does **not** turn SOCIS into a real-time inbound phone gateway. Inbound SMS is handled by polling the Twilio REST API. That is enough for many workflows, including notifications and some one-time-code retrieval, without adding core webhook infrastructure.
...
4. Treat third-party phone numbers as sensitive operational data:
   - do not save them to SOCIS memory
...
### 1) "I want SOCIS to own a real phone number"
Use **Twilio**.
...
- pairs well with SOCIS `text_to_speech` plus a public file host or tunnel

## Files and persistent state

The skill persists telephony state in two places:

### `${SOCIS_AGENT_HOME:-~/.socis-agent}/.env`
...
### `~/.socis-agent/telephony_state.json`
...
```bash
SCRIPT="$(find ~/.socis-agent/skills -path '*/telephony/scripts/telephony.py' -print -quit)"
```
...
```bash
socis skills search telephony
socis skills install official/productivity/telephony
```
...
Then save credentials into SOCIS:
...
3. Buy it and save it into `${SOCIS_AGENT_HOME:-~/.socis-agent}/.env` + state:
```bash
python "$SCRIPT" twilio-buy "+17025551234" --save-env
```
...
```bash
python "$SCRIPT" twilio-call "+15551230000" --message "Hello! This is SOCIS calling with your status update." --voice Polly.Joanna
```

### E. Call with a prerecorded / custom voice message

This is the main path for reusing SOCIS's existing `text_to_speech` support.

Use this when:
- you want the call to use SOCIS's configured TTS voice rather than Twilio `<Say>`
...
Recommended SOCIS TTS -> Twilio Play workflow:

1. Generate the audio with SOCIS `text_to_speech`.
2. Make the resulting MP3 publicly reachable.
3. Place the Twilio call with `--audio-url`.

Example agent flow:
- Ask SOCIS to create the message audio with `text_to_speech`
...
Important note:
- SOCIS TTS is great for prerecorded outbound messages
...
- SOCIS STT/TTS alone is not being used here as a full duplex phone conversation engine; that would require a much heavier streaming/webhook integration than this skill is trying to introduce
...
7. Summarize the outcome without persisting third-party numbers to SOCIS memory.
...
- Do not store arbitrary third-party phone numbers in SOCIS memory.
...
3. persist that number to `${SOCIS_AGENT_HOME:-~/.socis-agent}/.env`
