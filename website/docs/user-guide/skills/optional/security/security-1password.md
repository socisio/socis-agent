...
| | |
|---|---|
| Source | Optional — install with `socis skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by SOCIS Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during SOCIS terminal calls (desktop app flow only)
...
### Service Account (recommended for SOCIS)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${SOCIS_AGENT_HOME:-~/.socis-agent}/.env` (the skill will prompt for this on first load).
...
## SOCIS Execution Pattern (desktop app flow)

SOCIS terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/socis-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/socis-op.sock"
