...
| | |
|---|---|
| Source | Optional — install with `socis skills install official/creative/touchdesigner-mcp` |
...
```
SOCIS Agent -> MCP (Streamable HTTP) -> twozero.tox (port 40404) -> TD Python
```
...
3. Add `twozero_td` MCP server to SOCIS config (if missing)
...
3. **Restart SOCIS session** to pick up the new MCP server
