...
| | |
|---|---|
| Source | Optional — install with `socis skills install official/creative/unreal-mcp` |
...
Companion skill for the `unreal-engine` entry in the SOCIS MCP catalog. The
...
Two halves, in this order: the editor side must be up before SOCIS connects.
...
### One-time, SOCIS side
    ...
    socis mcp install unreal-engine
...
`~/.socis-agent/config.yaml` under `mcp_servers.unreal-engine` to match.

Do NOT use `ModelContextProtocol.GenerateClientConfig` for SOCIS — that
writes `.mcp.json`-style files for Claude Code/Cursor/etc. SOCIS connects
...
2. Start the SOCIS session. Tools register as `mcp_unreal_engine_*`. If
...
three meta-tools, and every real tool is reached through them. Through SOCIS
they appear as:
...
then happens at `socis mcp install`/`configure` time instead. Tool-search
...
3. **NEVER issue overlapping calls.** Do not batch multiple
   `mcp_unreal_engine_*` calls in one turn — SOCIS runs batched calls
...
- **Start order matters.** Editor + server up first, then the SOCIS
...
- **Timeouts on long operations.** SOCIS' per-call default is 120 s; asset
  imports, big level saves, and renders can exceed it. Raise
  `mcp_servers.unreal-engine.timeout` in `~/.socis-agent/config.yaml` for
