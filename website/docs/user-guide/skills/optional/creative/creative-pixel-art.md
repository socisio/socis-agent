...
| | |
|---|---|
| Source | Optional — install with `socis skills install official/creative/pixel-art` |
...
import sys
import os
sys.path.insert(0, os.path.expanduser("~/.socis-agent/skills/creative/pixel-art/scripts"))
...
```bash
cd ~/.socis-agent/skills/creative/pixel-art/scripts
...
- ffmpeg on PATH (only needed for video — SOCIS installs package this)
