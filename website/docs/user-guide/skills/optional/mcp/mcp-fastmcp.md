...
| | |
|---|---|
| Source | Optional — install with `socis skills install official/mcp/fastmcp` |
...
| Related skills | [`socis-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-socis-agent), [`mcporter`](/docs/user-guide/skills/optional/mcp/mcp-mcporter) |
...
- smoke-test a server with the FastMCP CLI before wiring it into SOCIS or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to SOCIS. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.socis-agent/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.socis-agent/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is SOCIS integration, either:

- configure the server in `~/.socis-agent/config.yaml` using the `native-mcp` skill, or
...
### SOCIS cannot see the deployed server

The server-building part may be correct while the SOCIS config is not. Load the `native-mcp` skill and configure the server in `~/.socis-agent/config.yaml`, then restart SOCIS.
