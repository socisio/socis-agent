...
| | |
|---|---|
| Source | Optional — install with `socis skills install official/autonomous-ai-agents/agent-merge-conflict-arbiter` |
...
- Both sides' intent sources: kanban completion summaries (`terminal` running
  `socis kanban show <task-id>`), PR bodies, or at minimum each branch's
  commit messages.
...
- Collect each side's intent: `socis kanban show <task-id>` for completion
