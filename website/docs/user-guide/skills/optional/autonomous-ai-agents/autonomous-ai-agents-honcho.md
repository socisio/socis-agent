---
title: "Honcho — Configure and troubleshoot Honcho memory for SOCIS"
sidebar_label: "Honcho"
description: "Configure and troubleshoot Honcho memory for SOCIS"
---
...
Configure and troubleshoot Honcho memory for SOCIS.
...
# Honcho Memory for SOCIS

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every SOCIS profile its own peer identity while sharing a unified view of the user.
...
```bash
socis honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this SOCIS profile's AI peer
...
Honcho models conversations as interactions between **peers**. SOCIS creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this SOCIS instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each SOCIS run |
| `global` | Single session across all directories |

Manual override: `socis honcho map my-project-name`
...
Each SOCIS profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block socis.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `socis.coder` host block in `honcho.json`
...
```bash
socis honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for SOCIS when Honcho memory is active.
...
Config file: `$SOCIS_AGENT_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `socis honcho setup`. Ensure `memory.provider: honcho` is in `~/.socis-agent/config.yaml`.

### Memory not persisting across sessions
Check `socis honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `socis profile create <name> --clone`. For existing profiles: `socis honcho sync`.
...
| Command | Description |
|---------|-------------|
| `socis honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `socis honcho status` | Show resolved config, connection test, peer info for active profile |
| `socis honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `socis honcho disable` | Disable Honcho for the active profile |
| `socis honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `socis honcho peers` | Show peer identities across all profiles |
| `socis honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `socis honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `socis honcho sessions` | List known directory-to-session-name mappings |
| `socis honcho map <name>` | Map current working directory to a Honcho session name |
| `socis honcho identity` | Seed AI peer identity or show both peer representations |
| `socis honcho sync` | Create host blocks for all SOCIS profiles that don't have one yet |
| `socis honcho migrate` | Step-by-step migration guide from OpenClaw native memory to SOCIS + Honcho |
| `socis memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `socis memory status` | Show active memory provider and config |
| `socis memory off` | Disable external memory provider |
