...
| | |
|---|---|
| Source | Optional — install with `socis skills install official/devops/watchers` |
...
All three live in `$SOCIS_AGENT_HOME/skills/devops/watchers/scripts/` once the skill is installed. Each reads `WATCHER_STATE_DIR` (defaults to `$SOCIS_AGENT_HOME/watcher-state/`) for its state file, keyed by the `--name` argument.
...
```bash
python $SOCIS_AGENT_HOME/skills/devops/watchers/scripts/watch_rss.py \
  --name hn --url https://news.ycombinator.com/rss --max 5
```

Watch a GitHub repo (set `GITHUB_TOKEN` in `${SOCIS_AGENT_HOME:-~/.socis-agent}/.env` to avoid the 60 req/hr anonymous rate limit):
...
```bash
python $SOCIS_AGENT_HOME/skills/devops/watchers/scripts/watch_http_json.py \
...
Every watcher writes `$SOCIS_AGENT_HOME/watcher-state/<name>.json`. Inspect:

```bash
cat $SOCIS_AGENT_HOME/watcher-state/hn.json
```

Force a replay (next run treated as first poll):

```bash
rm $SOCIS_AGENT_HOME/watcher-state/hn.json
```
...
4. **Putting the state dir where the agent's sandbox can't write.** `$SOCIS_AGENT_HOME/watcher-state/` is always writable. Docker/Modal backends may not see arbitrary host paths.
