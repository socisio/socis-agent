...
This skill is separate from the SOCIS Email gateway adapter. The gateway
adapter lets people email the agent and uses SOCIS' built-in IMAP/SMTP
...
To reply non-interactively from SOCIS, read the original message, compose a reply, and pipe it:
...
**Non-interactive (use this from SOCIS)** — pipe the message via stdin:
