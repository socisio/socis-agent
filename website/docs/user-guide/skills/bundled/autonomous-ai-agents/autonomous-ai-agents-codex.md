...
| Related skills | [`claude-code`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-claude-code), [`socis-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-socis-agent) |
...
Delegate coding tasks to [Codex](https://github.com/openai/codex) via the SOCIS terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For SOCIS itself, `model.provider: openai-codex` uses SOCIS-managed Codex
OAuth from `~/.socis-agent/auth.json` after `socis auth add openai-codex`. For the
...
## SOCIS Gateway Caveat

When invoking the Codex CLI from a SOCIS gateway/service context (for example,
