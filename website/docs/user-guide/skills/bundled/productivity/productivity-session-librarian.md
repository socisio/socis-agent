...
| List/filter by metadata (age, source, cost, tokens, workspace) | `socis sessions list` / `stats` via terminal |
| Rename | `socis sessions rename <session_id> <title...>` |
| Bulk soft-hide (reversible) | `socis sessions archive <filters>` |
| Delete (destructive) | `socis sessions delete` / `socis sessions prune <filters>` |
| Export before deleting anything valuable | `socis sessions export --session-id <id> --format md` |
...
`socis sessions list --source telegram --limit 50` instead.
...
- Before deleting anything with meaningful content, offer
  `socis sessions export --format md` as a backup.
...
After a cleanup pass, re-run the discovery query and `socis sessions list`
