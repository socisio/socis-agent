...
`~/.socis-agent/config.yaml`, `~/.socis-agent/.env`, and even the shell environment — for
...
Managed scope is read from a system-level directory, default `/etc/socis`:

```text
/etc/socis/
├── config.yaml     # managed config layer (wins over ~/.socis-agent/config.yaml)
└── .env            # managed env layer (wins over ~/.socis-agent/.env + shell)
```
...
The location can be relocated with the `SOCIS_AGENT_MANAGED_DIR` environment variable
(for containers or non-`/etc` deployments). This is a deployment/bootstrap path
knob — like `SOCIS_AGENT_HOME` — set by the same administrator who owns the managed
files. It is **never persisted** to any `.env` by SOCIS.

```bash
# Point managed scope at a custom directory (set by IT / the deployment, not the user)
export SOCIS_AGENT_MANAGED_DIR=/opt/org/socis-policy
```

:::warning
A user who can set `SOCIS_AGENT_MANAGED_DIR` can repoint managed scope at a directory
...
left user-settable. `socis doctor` reports the *resolved* managed directory so
...
|---|---|---|
| 1 | `/etc/socis/config.yaml` (managed) | `/etc/socis/.env` (managed) |
| 2 | `~/.socis-agent/config.yaml` (user) | `~/.socis-agent/.env` (user) |
...
```bash
socis config        # shows a header naming the managed source + the pinned keys
socis doctor        # reports the resolved managed dir + pinned key counts
```

If you try to change a managed value, SOCIS refuses and names the source:

```bash
$ socis config set model.default my/model
Cannot set 'model.default': it is managed by your administrator
(/etc/socis/config.yaml) and cannot be changed.
```

The same applies to managed secrets — `socis config set` / setup will not write
a user value for an env key pinned by the managed `.env`.

## Setting up a managed scope (administrators)

```bash
sudo mkdir -p /etc/socis

# Pin some config values for every user on this machine
sudo tee /etc/socis/config.yaml >/dev/null <<'YAML'
model:
  provider: nous
security:
  redact_secrets: true
...
sudo tee /etc/socis/.env >/dev/null <<'ENV'
OPENAI_API_BASE=https://inference.example.com/v1
ENV

sudo chmod 0755 /etc/socis
sudo chmod 0644 /etc/socis/config.yaml /etc/socis/.env
```

Changes take effect on the next SOCIS start (a malformed managed file is logged
loudly and ignored — it never blocks startup, but the admin should check
`socis doctor` to confirm the policy is being applied).

## Security model and limitations (v1)

- **Enforcement is filesystem permissions only.** If a user has write access to
  the managed directory (or runs SOCIS as `root`), managed scope is advisory.
