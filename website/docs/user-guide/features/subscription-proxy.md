...
chat completions — use your SOCIS-managed provider subscription as their
...
| Use case | "Use SOCIS as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. SOCIS stores
the refresh token in `~/.socis-agent/auth.json` — the same place all SOCIS
...
Model:      SOCIS-4-70B    # or SOCIS-4.3-36B, SOCIS-4-405B
```
...
interface in `socis_cli/proxy/adapters/`.
...
If you see `not logged in`, run `socis portal`. If you see
...
`socis_cli/proxy/adapters/<provider>.py` and registering it in
