...
SOCIS runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. SOCIS
...
1. SOCIS captures a baseline of current diagnostics for the file.
...
…). SOCIS auto-detects the binary on PATH or in
`<SOCIS_AGENT_HOME>/lsp/bin/`.
...
2. Download the latest release zip from
   ...
   and extract it.
3. Point SOCIS at the extracted bundle — the directory that contains
   `PowerShellEditorServices/Start-EditorServices.ps1`. Either:
   - set `lsp.servers.powershell.command: ["/path/to/bundle"]` in
     `config.yaml`, or
   - extract it to `<SOCIS_AGENT_HOME>/lsp/PowerShellEditorServices`, or
   - export `PSES_BUNDLE_PATH=/path/to/bundle`.

`socis lsp status` reports `installed` once `pwsh` is found; if the
...
`node_modules` tree — SOCIS installs both packages together when you
run `socis lsp install typescript` or auto-install fires on first
use.

## CLI

```
socis lsp status          # service state + per-server install status
socis lsp list            # registry, optionally --installed-only
socis lsp install <id>    # eagerly install one server
socis lsp install-all     # try every server with a known recipe
socis lsp restart         # tear down running clients
socis lsp which <id>      # print resolved binary path
```

`socis lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <SOCIS_AGENT_HOME>/lsp/bin
...
When `install_strategy: auto`, SOCIS installs binaries into
`<SOCIS_AGENT_HOME>/lsp/bin/`. NPM packages land in
`<SOCIS_AGENT_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully SOCIS-owned and is
...
**single process** per SOCIS process: the first Python project spawns
...
**`socis lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<SOCIS_AGENT_HOME>/lsp/bin/`. Run
`socis lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `socis lsp status`**
...
When `socis lsp status` shows a `Backend warnings` section, install
...
`~/.socis-agent/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.socis-agent/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `socis lsp restart` to clear the set;
