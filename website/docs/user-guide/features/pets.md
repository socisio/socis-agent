...
SOCIS can show an animated **pet** — a small mascot sprite that reacts to what
...
- Pets are installed into your profile's `pets/` directory
  (`<SOCIS_AGENT_HOME>/pets/<slug>/`), so each [profile](../profiles.md) keeps its
  own set.
...
In the terminal (CLI/TUI), SOCIS renders the sprite at full fidelity when your
...
socis pets install boba --select
...
| Goal | Command |
| --- | --- |
| Browse the gallery | `socis pets list [query] [--limit N]` |
| List installed pets | `socis pets list --installed` |
| Install a pet | `socis pets install <slug> [--select] [--force]` |
...
| Preview/animate | `socis pets show [slug] [--state <s>] [--cycle] [--once] [--mode <m>] [--scale <f>]` |
| Disable the pet | `socis pets off` |
| Remove an installed pet | `socis pets remove <slug>` |
| Diagnose setup | `socis pets doctor` |

`socis pets show` flags:
...
Beyond installing pre-made pets from the gallery, SOCIS can **generate a brand-new pet** from a text description — its own AI sprite-generation pipeline.

- CLI/TUI: `/hatch <description>` (alias `/generate-pet`), or `socis pets` → the generate flow.
...
- If no reference-capable backend is configured, generation surfaces an actionable error pointing you to `socis tools` → Image Generation. (Installing/adopting existing gallery pets needs no image backend.)
- Override the backend with the `SOCIS_AGENT_PET_IMAGE_PROVIDER` env var (e.g. `SOCIS_AGENT_PET_IMAGE_PROVIDER=openrouter`).
...
always-on-top desktop window. Out there it stays visible while SOCIS is
...
Run `socis pets doctor` — it reports:
...
- The [`socis-agent` skill](../skills/bundled/autonomous-ai-agents/autonomous-ai-agents-socis-agent.md)
