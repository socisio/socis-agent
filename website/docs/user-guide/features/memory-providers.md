...
SOCIS Agent ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time — the built-in memory is always active alongside it.

## Quick Start

```bash
socis memory setup      # interactive picker + configuration
socis memory status     # check what's active
socis memory off        # disable external provider
```

You can also select the active memory provider via `socis plugins` → Provider Plugins → Memory Provider.

Or set manually in `~/.socis-agent/config.yaml`:
...
When a memory provider is active, SOCIS automatically:
...
**Setup Wizard:**
```bash
socis memory setup        # select "honcho" — runs the Honcho-specific post-setup
```

The legacy `socis honcho setup` command still works (it now redirects to `socis memory setup`), but is only registered after Honcho is selected as the active memory provider.
...
**Config:** `$SOCIS_AGENT_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global). Resolution order: `$SOCIS_AGENT_HOME/honcho.json` > `~/.socis-agent/honcho.json` > `~/.honcho/config.json`. See the [config reference](https://github.com/socisio/socis-agent/blob/main/plugins/memory/honcho/README.md) and the [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/hermes).
...
:::tip Migrating from `socis honcho`
If you previously used `socis honcho setup`, your config and all server-side data are intact. Just re-enable through the setup wizard again or manually set `memory.provider: honcho` to reactivate via the new system.
:::

**Multi-peer setup:**

Honcho models conversations as peers exchanging messages — one user peer plus one AI peer per SOCIS profile, all sharing a workspace. The workspace is the shared environment: the user peer is global across profiles, each AI peer is its own identity. Every AI peer builds an independent representation / card from its own observations, so a `coder` profile stays code-oriented while a `writer` profile stays editorial against the same user.

The mapping:

| Concept | What it is |
|---------|-----------|
| **Workspace** | Shared environment. All SOCIS profiles under one workspace see the same user identity. |
| **User peer** (`peerName`) | The human. Shared across profiles in the workspace. |
| **AI peer** (`aiPeer`) | One per SOCIS profile. Host key `socis` → default; `socis.<profile>` for others. |
...
```bash
socis profile create coder --clone
```

`--clone` creates a `socis.coder` host block in `honcho.json` with `aiPeer: "coder"`, shared `workspace`, inherited `peerName`, `recallMode`, `writeFrequency`, `observation`, etc. The AI peer is eagerly created in Honcho so it exists before the first message.
...
Scans every SOCIS profile, creates host blocks for any profile without one, inherits settings from the default `socis` block, and creates the new AI peers eagerly. Idempotent — skips profiles that already have a host block.
...
Off-gateway these keys do nothing. `socis memory setup` only prompts for them when it detects a connected gateway platform. See the [Honcho page](./honcho.md#gateway-identity-mapping) for the resolver ladder and the setup flow.
...
See the [config reference](https://github.com/socisio/socis-agent/blob/main/plugins/memory/honcho/README.md) and [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/hermes).
...
socis memory setup    # select "openviking"
# Or manually:
socis config set memory.provider openviking
```

`socis memory setup` can reuse or copy connection values from
`~/.openviking/ovcli.conf`. Manual setup uses the active profile's `.env` file;
for the default profile that is `~/.socis-agent/.env`, and for named profiles use
`~/.socis-agent/profiles/<profile>/.env`.
...
Peer identity is optional. By default, SOCIS sends no peer ID and writes
...
Set `memory.openviking.agent: socis` to restore the old peer-scoped writes.
...
SOCIS sends `User-Agent: openviking-memory-socis/<version>` on OpenViking
...
**Setup (Platform):**
```bash
socis memory setup    # select "mem0" → "Platform"
# Or manually:
socis config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.socis-agent/.env
```

**Setup (OSS):**
```bash
socis memory setup    # select "mem0" → "Open Source (self-hosted)"
# Or via flags:
socis memory setup mem0 --mode oss --oss-llm openai --oss-llm-key sk-... --oss-vector qdrant
```

Preview without writing files:
```bash
socis memory setup mem0 --mode oss --oss-llm-key sk-... --dry-run
```

**Setup (Self-Hosted Dashboard):** connect to a Mem0 server you run via Docker (the dashboard's REST API):

```bash
socis memory setup    # select "mem0" → "Self-hosted server"
# Or via flags:
socis memory setup mem0 --mode selfhosted --host http://localhost:8888 --api-key your-admin-api-key
```

Or configure manually — either as env vars:

```bash
echo "MEM0_HOST=http://localhost:8888" >> ~/.socis-agent/.env
echo "MEM0_API_KEY=your-admin-api-key" >> ~/.socis-agent/.env
```
...
**Config:** `$SOCIS_AGENT_HOME/mem0.json` (behavioral settings). Only the secret `MEM0_API_KEY` belongs in `~/.socis-agent/.env`.
...
| `user_id` | `socis-user` | User identifier |
| `agent_id` | `socis` | Agent identifier |
...
**Switching modes:** Re-run `socis memory setup mem0 --mode <platform|selfhosted|oss>` or edit `mem0.json` directly.
...
**Setup:**
```bash
socis memory setup    # select "hindsight"
# Or manually:
socis config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.socis-agent/.env
```

The setup wizard installs dependencies automatically and only installs what's needed for the selected mode (`hindsight-client` for cloud, `hindsight-all` for local). Requires `hindsight-client >= 0.4.22` (auto-upgraded on session start if outdated).

**Local mode UI:** `hindsight-embed -p socis ui start`

**Config:** `$SOCIS_AGENT_HOME/hindsight/config.json`

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `cloud` | `cloud` or `local` |
| `bank_id` | `socis` | Memory bank identifier |
| `recall_budget` | `mid` | Recall thoroughness: `low` / `mid` / `high` |
| `memory_mode` | `hybrid` | `hybrid` (context + tools), `context` (auto-inject only), `tools` (tools only) |
| `auto_retain` | `true` | Automatically retain conversation turns |
| `auto_recall` | `true` | Automatically recall memories before each turn |
| `retain_async` | `true` | Process retain asynchronously on the server |
| `retain_context` | `conversation between SOCIS Agent and the User` | Context label for retained memories |
...
See [plugin README](https://github.com/socisio/socis-agent/blob/main/plugins/memory/hindsight/README.md) for the full configuration reference.
...
**Setup:**
```bash
socis memory setup    # select "holographic"
# Or manually:
socis config set memory.provider holographic
```

**Config:** `config.yaml` under `plugins.socis-memory-store`

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$SOCIS_AGENT_HOME/memory_store.db` | SQLite database path |
...
**Setup:**
```bash
socis memory setup    # select "retaindb"
# Or manually:
socis config set memory.provider retaindb
echo "RETAINDB_API_KEY=your-key" >> ~/.socis-agent/.env
```
...
# Then configure SOCIS
socis memory setup    # select "byterover"
# Or manually:
socis config set memory.provider byterover
```

**Key features:**
- Automatic pre-compression extraction (saves insights before context compression discards them)
- Knowledge tree stored at `$SOCIS_AGENT_HOME/byterover/` (profile-scoped)
...
**Setup:**
```bash
socis memory setup    # select "supermemory"
# Or manually:
socis config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.socis-agent/.env
```
...
Before running `socis memory setup`, set `base_url` in
`$SOCIS_AGENT_HOME/supermemory.json`:
...
Then run `socis memory setup` and enter the API key printed by the local
server. Configuring the endpoint first ensures the setup connection probe also
stays local.

**Config:** `$SOCIS_AGENT_HOME/supermemory.json`
...
| `container_tag` | `socis` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags. |
...
- **Profile-scoped containers** — use `{identity}` in `container_tag` (e.g. `socis-{identity}` → `socis-coder`) to isolate memories per SOCIS profile
...
| **Requires** | `pip install socis-memori` + `socis-memori install` + [Memori API key](https://app.memorilabs.ai/signup) |
...
**Setup:**
```bash
pip install socis-memori
socis-memori install
socis config set memory.provider memori
socis memory setup
```
...
| **Memori** | Cloud | Free/Paid | 5 | `socis-memori` | Tool-aware memory + structured recall |

## Profile Isolation

Each provider's data is isolated per [profile](/user-guide/profiles):

- **Local storage providers** (Holographic, ByteRover) use `$SOCIS_AGENT_HOME/` paths which differ per profile
- **Config file providers** (Honcho, Mem0, Hindsight, Supermemory) store config in `$SOCIS_AGENT_HOME/` so each profile has its own credentials
