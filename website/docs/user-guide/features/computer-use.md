...
SOCIS Agent can drive your desktop — clicking, typing, scrolling,
...
The built-in `computer_use` toolset is the recommended SOCIS integration. It
...
**Fresh installs already have the driver.** The SOCIS installer
...
- **`socis tools`** → pick `🖱️  Computer Use` — installs the driver
...
```
socis computer-use install
```

This fetches and runs the upstream cua-driver installer — `install.sh`
on macOS/Linux, `install.ps1` on Windows. Use `socis computer-use
status` to verify the install.

Already have cua-driver? SOCIS reuses it when it supports the 0.20 runtime
contract. During setup, toolset enablement, `socis update`, and the first
`computer_use` call of a session, SOCIS checks the local version and
...
selected with `SOCIS_AGENT_CUA_DRIVER_CMD` stays
under your control, so SOCIS reports the incompatibility and leaves it
unchanged.

If you install Cua Driver first, `cua-driver skills install` installs Cua's
skill pack under `~/.cua-driver/skills/cua-driver`. SOCIS autodetection is a
planned cua-driver follow-up, so currently point SOCIS at that directory or
...
interface. The built-in toolset provides SOCIS actions, configuration,
...
| Platform | Prereqs |
|---|---|
| **macOS** | System Settings → Privacy & Security → **Accessibility** + **Screen Recording**. Grant the identity named by `socis computer-use doctor`. Standard mode uses CuaDriver.app; bounded and unrestricted modes use the SOCIS host identity. |
...
```
socis -t computer_use chat
```

or add `computer_use` to your enabled toolsets in `~/.socis-agent/config.yaml`.

## Permission modes and logged-in browser profiles

SOCIS maps its existing approval UX onto cua-driver's immutable runtime
modes. Permission mode and capability manifest approval are launch settings.
They cannot change after the runtime starts:

| SOCIS session | cua-driver mode | Human intervention |
|---|---|---|
| Manual or smart approvals (default) | `standard` | Normal SOCIS approvals; Cua stops at its protected boundary |
...
| `--yolo`, `/yolo`, or `approvals.mode: off` | private `unrestricted` daemon | One explicit SOCIS risk acceptance; no runtime Cua prompts |
...
computer_use:
  ...
  capability_manifest: ~/.socis-agent/cua-manifest.yaml
```
...
for the format). SOCIS launches a private runtime with
...
identity instead of resetting with every SOCIS build), and SOCIS verifies
...
resetting or closing the SOCIS session, cancellation cleanup, or process exit
closes that transport session. SOCIS also stops private runtimes that it
launched for bounded or unrestricted access. One SOCIS
conversation cannot change another runtime's mode or grants. Bounded and
unrestricted modes use a private
embedded service under the SOCIS host identity.
...
## `socis computer-use doctor` — your first triage stop

`socis computer-use doctor` runs cua-driver's structured
`health_report` MCP tool and prints a per-check matrix. It's the single
fastest way to find out *why* an action isn't working.

```
$ socis computer-use doctor
...
SOCIS run declares a public cua-driver **session name** (something like
`socis-3a7b9c14d2e8`). The name labels cursor identity and related state, so
...
without it. SOCIS disables it automatically where it is a known failure
...
SOCIS keeps its wrapper skill (`skills/autonomous-ai-agents/computer-use/SKILL.md`)
focused on the SOCIS-side `computer_use` workflow and action vocabulary. For
...
The command installs the pack under `~/.cua-driver/skills/cua-driver`. SOCIS
autodetection is a planned cua-driver follow-up, so currently point SOCIS at
...
These are **platform deep dives, not duplicates of the SOCIS skill** —
...
Code, Codex, OpenCode, OpenClaw, and Antigravity; **SOCIS
...
image capture also saves a bounded, shareable copy under SOCIS' image cache
...
SOCIS applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.socis-agent/config.yaml` if you
want every action confirmed.

## Token efficiency

Screenshots are expensive. SOCIS applies four layers of optimisation:
...
- **Windows: elevated (admin) windows can't be driven from a normal
  ...
  SOCIS agent) cannot enumerate the UIA tree of, or inject mouse input
  ...
  run the SOCIS agent itself at High integrity (launch from an
...
- **Platform-specific deployment gotchas:**
  - **macOS** uses private SkyLight SPIs. Apple can change them in any
    OS update. SOCIS warns when the installed cua-driver is older than
    the version it was tested against.
  - **Windows** SSH sessions run in **Session 0**, which has no
    interactive desktop. Drive SOCIS from inside the RDP / console
...
On Linux, native Wayland support remains an explicit opt-in. SOCIS passes the
...
```
SOCIS_AGENT_CUA_DRIVER_CMD=/path/to/your/cua-driver
```

Swap the backend entirely (for testing):

```
SOCIS_AGENT_COMPUTER_USE_BACKEND=noop   # records calls, no side effects
```

### Telemetry

cua-driver ships with anonymous usage telemetry (PostHog) enabled by default
upstream. **SOCIS disables it for you** — on every cua-driver invocation
(the MCP backend, `status`, `doctor`, and install) SOCIS sets
...
When it's on, `socis computer-use doctor` reports `telemetry: enabled`;
...
unreleased fix — point SOCIS at a binary you built from source instead
of the published release. SOCIS resolves the driver with
`shutil.which("cua-driver")` and **does not enforce
`SOCIS_AGENT_CUA_DRIVER_VERSION`**, so a local build (reported as
...
- `-NoAutoStart` skips registering the `cua-driver-serve` logon daemon
  — you don't need it for SOCIS testing (see notes).
...
### Option B — point SOCIS straight at the built binary (fastest loop)

Skip the install ceremony entirely: `cargo build` and set
`SOCIS_AGENT_CUA_DRIVER_CMD` to the resulting binary. Best for rapid
edit/build/test.
...
```
# Windows (.env)
SOCIS_AGENT_CUA_DRIVER_CMD=C:\path\to\cua\libs\cua-driver\rust\target\debug\cua-driver.exe
# macOS / Linux (.env)
SOCIS_AGENT_CUA_DRIVER_CMD=/path/to/cua/libs/cua-driver/rust/target/debug/cua-driver
```

### Confirm SOCIS is using your build

- `socis computer-use status` prints the resolved binary path and
  version.
- `socis computer-use doctor` confirms the binary is reachable and
...
- **SOCIS spawns a `cua-driver mcp` stdio proxy.** In a normal session the
  proxy connects to (and may start) the standard machine daemon. In explicit
  SOCIS YOLO, SOCIS instead owns a private `cua-driver serve --embedded`
...
- **Rebuild loop.** After editing cua-driver source, re-run
  ...
  for Option A, or just re-`cargo build` for Option B — no SOCIS
  change needed either way.
- **Local builds skip the version check.** SOCIS warns when the
...
**First action when anything's off: run `socis computer-use doctor`.**
...
Run `socis computer-use install` to fetch the cua-driver binary, or
run `socis tools` and enable the Computer Use toolset.
...
Wayland without an XWayland bridge. `socis computer-use doctor` will
...
- **SOCIS-side skill** — `skills/autonomous-ai-agents/computer-use/SKILL.md` — teaches the
  SOCIS `computer_use` action vocabulary; this is what the agent loads.
- **cua-driver skill pack** — for platform-specific deep dives
  ...
  `LINUX.md` / `RECORDING.md` / `WEB_APPS.md`. SOCIS autodetection is a
  planned follow-up; currently point SOCIS at the installed pack directory
...
- **cua.ai/docs** — the cua-driver project's documentation:
  ...
  - [Connect your agent](https://cua.ai/docs/how-to-guides/driver/connect-your-agent) — register cua-driver with various harnesses (SOCIS among them)
