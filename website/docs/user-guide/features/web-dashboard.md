...
The web dashboard is a browser-based UI for managing your SOCIS Agent installation. Instead of editing YAML files or running CLI commands, you can configure settings, manage API keys, and monitor sessions from a clean web interface.

:::tip
Hosted-mode auth uses Nous Portal OAuth; if you also want the dashboard to talk to a real backend, `socis setup --portal` wires up the model and tool gateway too. See [Nous Portal](/integrations/nous-portal).
:::
...
```bash
# Custom port
socis dashboard --port 8080

# Bind to all interfaces (use with caution on shared networks)
socis dashboard --host 0.0.0.0

# Start without opening browser
socis dashboard --no-open
```
...
child with the selected profile's `SOCIS_AGENT_HOME`, so the conversation runs
...
processes (manage them via `socis -p <name> gateway …`), each profile's
session database, and cron schedulers (the Cron page already aggregates
across profiles with its own filter).

## Prerequisites

The default `socis-agent` install does not ship the HTTP stack or PTY helper — those are optional extras. The **web dashboard** needs FastAPI and Uvicorn (`web` extra). The **Chat** tab also needs `ptyprocess` to spawn the embedded TUI behind a pseudo-terminal (`pty` extra on POSIX). Install both with:

```bash
cd ~/.socis-agent/socis-agent && uv pip install -e ".[web,pty]"
```

The `web` extra pulls in FastAPI/Uvicorn; `pty` pulls in `ptyprocess` (POSIX) or `pywinpty` (native Windows — note that the embedded TUI itself still requires WSL). `cd ~/.socis-agent/socis-agent && uv pip install -e ".[all]"` includes both extras and is the easiest path if you also want messaging/voice/etc.

When you run `socis dashboard` without the dependencies, it will tell you what to install. If the frontend hasn't been built yet and `npm` is available, it builds automatically on first launch.

The Chat tab is part of every `socis dashboard` launch — the embedded browser chat pane (running the TUI over PTY/WebSocket) is always available, with no extra flag required.
...
- **Disk warnings** — the volume holding `~/.socis-agent` is nearly full
...
- Node.js (same requirement as `socis --tui`; the TUI bundle is built on first launch)
- `ptyprocess` — installed by the `pty` extra (`cd ~/.socis-agent/socis-agent && uv pip install -e ".[web,pty]"`, or `[all]` covers both)
...
```ini
[Service]
EnvironmentFile=%h/.socis-agent/.env
ExecStart=/path/to/venv/bin/python -m socis_cli.main dashboard \
    --host 0.0.0.0 --port 9119 --no-open
```

with `~/.socis-agent/.env` containing:

```bash
SOCIS_AGENT_DASHBOARD_BASIC_AUTH_USERNAME=admin
SOCIS_AGENT_DASHBOARD_BASIC_AUTH_PASSWORD=choose-a-strong-password
SOCIS_AGENT_DASHBOARD_BASIC_AUTH_SECRET=<32+ random bytes; openssl rand -base64 32>
```
...
:::tip
Config changes take effect on the next agent session or gateway restart. The web dashboard edits the same `config.yaml` file that `socis config set` and the gateway read from.
:::
...
Create and manage [profiles](../profiles.md) — isolated SOCIS instances with their own config, skills, and sessions.
...
- **Set as active** — flips the sticky default that **future CLI/gateway runs** pick up (same as `socis profile use`). This does *not* change what the dashboard manages — that's the profile switcher's job
- **Edit model / description / SOUL** — inline editors writing into that profile
- **Rename / Delete** — named profiles only

### Skills

Browse, search, and toggle installed skills and toolsets, and install new ones from the hub. Skills are loaded from `~/.socis-agent/skills/` and grouped by category.
...
block in `config.yaml` that `socis mcp` reads from.
...
Connect SOCIS to any messaging platform from the browser — full parity with
`socis setup gateway`. The page lists every supported channel (Telegram,
...
- **Host** — live system stats: OS / kernel, architecture, hostname, Python and SOCIS versions, CPU core count + utilization, memory, disk usage of the SOCIS home, uptime, and load average. (CPU/memory/disk come from `psutil` when installed; identity fields are always shown.) The SOCIS version shows an **update-status badge** (up to date / N commits behind) and a **Check for updates** button. When an update is available on a git install, an **Update now** button opens a confirmation dialog — showing how many commits you'll pull — before running `socis update` in the background. On Docker/Nix installs the dashboard can't apply the update in place, so it shows the correct out-of-band command instead.
- **Nous Portal** — login status, the active inference provider, and the Tool Gateway routing table (which tools run via the Portal vs. locally), with a link to manage your subscription. Read-only mirror of `socis portal`.
- **Skill curator** — the background skill-maintenance status (active / paused, interval, last run) with pause/resume and a run-now button. Mirrors `socis curator`.
...
This re-reads `~/.socis-agent/.env` into the running process's environment. Useful when you've added a new provider key via the dashboard and want to use it immediately.
...
`SOCIS_AGENT_HOME`. Omitted = the dashboard's own profile. Unknown profile names
...
- **`disk`** — a live `shutil.disk_usage()` sample of the volume holding
  `~/.socis-agent`. Fields: `pressure`, `free_mb`, `total_mb`, `used_percent`,
...
| `GET /api/socis/update/check` | Report update availability (commits behind, install method) without applying. For git installs that are behind, also returns a `commits` list (`sha`, `summary`, `author`, `at`) of what's changed. `?force=1` busts the 6h cache |
...
|-------|-----------|----------|
| `socis dashboard` (default — binds to `127.0.0.1`) | OFF | Local development |
| `socis dashboard --host 0.0.0.0` | **ON** | Remote / production — protect with the username/password provider or OAuth |
...
If the gate would engage but **no** `DashboardAuthProvider` is registered (no Nous plugin, no custom plugin), `socis dashboard` refuses to bind with an explicit error message. There is no "default-deny but accept everything" fallback — a misconfigured gated dashboard never starts.

When you run `socis dashboard --host 0.0.0.0` **interactively** (a real terminal) and no provider is configured yet, SOCIS doesn't just fail — it offers to set one up on the spot: pick **username & password** (writes `dashboard.basic_auth` to `config.yaml` and you're running in seconds) or **OAuth** (points you at `socis dashboard register`). Non-interactive callers — Docker/s6, CI, piped runs — skip the prompt and hit the fail-closed error above, so an unattended deploy still never starts without auth.

### Default provider: SOCIS
...
- **CLI — `socis dashboard register`.** Run it on the host where the dashboard lives. It resolves your existing Nous login (run `socis setup` first if you're not logged in), registers a self-hosted OAuth client with the Portal, and writes `SOCIS_AGENT_DASHBOARD_OAUTH_CLIENT_ID` into `~/.socis-agent/.env` for you. Optional flags: `--name` (a human-readable label, otherwise auto-generated) and `--redirect-uri` (a public HTTPS callback URL for an internet-facing host).

  ```bash
  socis dashboard register
  # ✓ Registered dashboard "swift_falcon"
  # …writes SOCIS_AGENT_DASHBOARD_OAUTH_CLIENT_ID to ~/.socis-agent/.env
  ```

- **GUI — the Local Dashboards page.** Open [`/local-dashboards`](https://portal.nousresearch.com/local-dashboards) in the Nous Portal to register, name, manage, and revoke self-hosted dashboards from the browser. Copy the resulting `agent:{id}` client ID into `SOCIS_AGENT_DASHBOARD_OAUTH_CLIENT_ID` (env) or `dashboard.oauth.client_id` (config.yaml). This is also where you revoke a dashboard registered via the CLI.
...
|---------|-----------|--------|----------------|
| `SOCIS_AGENT_DASHBOARD_OAUTH_CLIENT_ID` | `dashboard.oauth.client_id` | `agent:{instance_id}` | `socis dashboard register` |

Per the SOCIS Agent convention (`~/.socis-agent/.env` is for API keys / secrets only), **`config.yaml` is the recommended place to set these values** for local dev, on-prem, and any deployment you control directly. The environment-variable path exists so a hosting platform's secret injection can push per-deploy `client_id`s without anyone having to edit `config.yaml` inside the image — that's its primary purpose.
...
Bundled providers reported these issues:
  • nous: SOCIS_AGENT_DASHBOARD_OAUTH_CLIENT_ID is not set (and
...
Configure an auth provider before exposing the dashboard:
  ...
  • OAuth: run `socis dashboard register` (Nous Portal) or install a
...
#### Worked example: SOCIS

From a logged-in SOCIS install to a Nous-gated dashboard in three steps.

**1. Log in and register the dashboard.** `socis dashboard register` uses your existing Nous login to provision an OAuth client and writes `SOCIS_AGENT_DASHBOARD_OAUTH_CLIENT_ID` into `~/.socis-agent/.env` for you:

```bash
socis setup            # if you're not already logged into Nous Portal
socis dashboard register
# ✓ Registered dashboard "swift_falcon"
# …writes SOCIS_AGENT_DASHBOARD_OAUTH_CLIENT_ID to ~/.socis-agent/.env
```
...
```bash
socis dashboard --host 0.0.0.0 --port 9119 --no-open
```

**3. Log in.** Open `http://<host>:9119/`, you'll be bounced to `/login`. Click **Sign in with SOCIS** → authenticate at the Portal → land back on the authenticated dashboard. Verify the gate from any machine:
...
|---------|-----------|-------|
| `SOCIS_AGENT_DASHBOARD_BASIC_AUTH_USERNAME` | `dashboard.basic_auth.username` | required to activate |
| `SOCIS_AGENT_DASHBOARD_BASIC_AUTH_PASSWORD_HASH` | `dashboard.basic_auth.password_hash` | preferred (no plaintext at rest) |
| `SOCIS_AGENT_DASHBOARD_BASIC_AUTH_PASSWORD` | `dashboard.basic_auth.password` | plaintext; **wins over a config `password_hash`** so you can rotate via env |
| `SOCIS_AGENT_DASHBOARD_BASIC_AUTH_SECRET` | `dashboard.basic_auth.secret` | token-signing key |
| `SOCIS_AGENT_DASHBOARD_BASIC_AUTH_TTL_SECONDS` | `dashboard.basic_auth.session_ttl_seconds` | access-token lifetime |
...
**1. Set credentials in `~/.socis-agent/.env`.** Hash the password so no plaintext sits at rest, and set a stable signing secret so sessions survive restarts:
...
cat >> ~/.socis-agent/.env <<EOF
SOCIS_AGENT_DASHBOARD_BASIC_AUTH_USERNAME=admin
SOCIS_AGENT_DASHBOARD_BASIC_AUTH_PASSWORD_HASH=$HASH
SOCIS_AGENT_DASHBOARD_BASIC_AUTH_SECRET=$(openssl rand -base64 32)
EOF
chmod 600 ~/.socis-agent/.env
```
...
| Env var | Overrides | Notes |
|---------|-----------|-------|
| `SOCIS_AGENT_DASHBOARD_OIDC_ISSUER` | `dashboard.oauth.self_hosted.issuer` | OIDC issuer URL — required |
| `SOCIS_AGENT_DASHBOARD_OIDC_CLIENT_ID` | `dashboard.oauth.self_hosted.client_id` | Public client id — required |
| `SOCIS_AGENT_DASHBOARD_OIDC_SCOPES` | `dashboard.oauth.self_hosted.scopes` | Defaults to `openid profile email` |
...
**1. Run Keycloak with a pre-configured realm.** Save this realm export as `realm-socis.json` — it defines a `socis` realm, a **public PKCE client** (`socis-dashboard`), and a test user, all imported on boot so there's nothing to click in the admin UI:
...
```json
{
  ...
  "clients": [
    {
      "clientId": "socis-dashboard",
      "name": "SOCIS Agent Dashboard",
      "enabled": true,
      "publicClient": true,
...
docker run --rm -p 8080:8080 \
  ...
  -v "$PWD/realm-socis.json:/opt/keycloak/data/import/realm-socis.json:ro" \
  quay.io/keycloak/keycloak:26.0 \
  start-dev --import-realm
```

Once it's up, the realm advertises standard OIDC discovery at
`http://localhost:8080/realms/socis/.well-known/openid-configuration` (issuer
`http://localhost:8080/realms/socis`). The admin console is at
...
```bash
export SOCIS_AGENT_DASHBOARD_OIDC_ISSUER="http://localhost:8080/realms/socis"
export SOCIS_AGENT_DASHBOARD_OIDC_CLIENT_ID="socis-dashboard"
export SOCIS_AGENT_DASHBOARD_PUBLIC_URL="http://localhost:9119"
socis dashboard --host 0.0.0.0 --port 9119 --no-open
```

`SOCIS_AGENT_DASHBOARD_PUBLIC_URL` tells the dashboard its OAuth callback is
...
> admin console (Clients → socis-dashboard → Settings). The same pattern works
...
For deploys behind reverse proxies that don't reliably forward those headers (manual nginx setups, on-prem ingresses, custom-domain deploys with partial proxy chains), set `dashboard.public_url` (or `SOCIS_AGENT_DASHBOARD_PUBLIC_URL`) to the **complete public URL** the dashboard is reached at:
...
first; without one, SOCIS fails closed at startup. This prevents the local SPA
...
SOCIS always preserves loopback trust and rejects `*`, `0.0.0.0/0`, and
...
socis dashboard --host 127.0.0.1 --port 9119 --no-open
```
...
|---------|---------------|-------------|
| `dashboard.public_url` in `config.yaml` | `SOCIS_AGENT_DASHBOARD_PUBLIC_URL` | Local dev / on-prem (canonical) |
| `SOCIS_AGENT_DASHBOARD_PUBLIC_URL` env var | — | Hosting-platform secrets / CI |
...
The provider implements the [Nous Portal OAuth contract v1](https://github.com/SOCIS/nous-account-service/blob/main/docs/agent-dashboard-oauth-contract.md) — authorization-code grant with PKCE (S256):

1. User hits `/` without a session cookie → gate redirects to `/login`.
2. Login page shows a "Continue with SOCIS" button → `/auth/login?provider=nous`.
...
5. Server exchanges the code for an access token at `POST /api/oauth/token`, verifies the JWT signature against the Portal's JWKS (`/.well-known/jwks.json`), and sets the `socis_session_at` cookie.
...
| Name | Lifetime | Notes |
|------|----------|-------|
| `socis_session_at` | Token TTL (15 min) | HttpOnly, SameSite=Lax, Secure-when-HTTPS |
| `socis_session_pkce` | 10 min | HttpOnly; holds the PKCE verifier + provider hint during the round trip. SameSite=None + Secure over HTTPS (must survive the cross-site IDP redirect chain — Chromium drops SameSite=Lax cookies set on a 302 in a cross-site chain); SameSite=Lax on loopback HTTP |
| `socis_session_rt` | unused in v1 | Reserved for forward-compat; not written when `refresh_token` is empty |
...
Every login start, success, failure, and session-verify failure is written as a JSON line to `$SOCIS_AGENT_HOME/logs/dashboard-auth.log`. Sensitive fields (`access_token`, `refresh_token`, `code`, `code_verifier`, `state`, `Authorization` header) are redacted before logging.
...
```python
# ~/.socis-agent/plugins/dashboard-auth-myidp/__init__.py
from socis_cli.dashboard_auth import DashboardAuthProvider, Session, LoginStart
...
The bundled first consumer is the **drain** provider (`plugins/dashboard_auth/drain`): `nous-account-service` provisions a per-agent secret via `SOCIS_AGENT_DASHBOARD_DRAIN_SECRET`, and the provider verifies inbound bearer tokens against it with a constant-time compare, registering `/api/gateway/drain` as token-authable. It **fails closed** — a weak/short secret (< 256 bits) is rejected at registration and the endpoint stays disabled; it's a no-op when the env var is unset. Behavioural knobs (`scope`, `min_secret_chars`) live under `dashboard.drain_auth` in `config.yaml`.
...
```bash
# Quick env-var path.
SOCIS_AGENT_DASHBOARD_OAUTH_CLIENT_ID=agent:test \
  socis dashboard --host 0.0.0.0
...
SOCIS Agent Desktop can drive a SOCIS backend running on another machine (a VPS, a home server, a Mini behind Tailscale). In the app this lives under **Settings → Gateways → Remote gateway**, which asks for a **Remote URL** and a way to **Sign in**. (For the desktop app itself — install, settings, chat — see the [SOCIS Agent Desktop](/user-guide/desktop) page.)
...
```bash
# 1. Set the dashboard login credentials in ~/.socis-agent/.env (secrets file, 0600).
cat >> ~/.socis-agent/.env <<'EOF'
SOCIS_AGENT_DASHBOARD_BASIC_AUTH_USERNAME=admin
SOCIS_AGENT_DASHBOARD_BASIC_AUTH_PASSWORD=choose-a-strong-password
...
socis dashboard --no-open --host 0.0.0.0 --port 9119
```

Prefer no plaintext at rest? Use `SOCIS_AGENT_DASHBOARD_BASIC_AUTH_PASSWORD_HASH` with a scrypt hash instead — see [Username/password provider](#usernamepassword-provider-no-oauth-idp) for the full surface.

If you run the dashboard as a systemd service, `~/.socis-agent/.env` is picked up automatically when the unit has `EnvironmentFile=%h/.socis-agent/.env`, so the credentials are in the environment at boot.
...
### In SOCIS Agent Desktop

**Settings → Gateways → Remote gateway:**

- **Remote URL** — `http://<backend-host>:9119` (path prefixes like `/socis` are supported if you front it with a reverse proxy)
...
The session refreshes automatically and survives restarts when `SOCIS_AGENT_DASHBOARD_BASIC_AUTH_SECRET` is set on the backend.

### Environment-variable override

Instead of the in-app setting, you can point the desktop at a backend with an env var before launching it. When `SOCIS_AGENT_DESKTOP_REMOTE_URL` is set, it overrides the saved in-app URL (the Gateway settings panel shows an "env override" badge and disables editing); you still **Sign in** with your username and password from the panel.

| Env var | Value |
|---------|-------|
| `SOCIS_AGENT_DESKTOP_REMOTE_URL` | `http://<backend-host>:9119` |

### Troubleshooting

- **"Remote gateway incomplete"** — you haven't entered a remote URL.
- **Sign-in fails with 401 / "Invalid credentials"** — the username or password doesn't match the backend's `SOCIS_AGENT_DASHBOARD_BASIC_AUTH_USERNAME` / `SOCIS_AGENT_DASHBOARD_BASIC_AUTH_PASSWORD`. The backend returns the same generic error for unknown user and wrong password, so check both. Confirm the gate with `curl -s http://<host>:9119/api/status | jq '.auth_required, .auth_providers'` — it should report `true` and include `"basic"`.
...
- **Signed out on every restart** — set `SOCIS_AGENT_DASHBOARD_BASIC_AUTH_SECRET` to a stable value; otherwise the signing key is regenerated per boot.
...
The frontend is built with React 19, TypeScript, Tailwind CSS v4, and shadcn/ui-style components. Production builds output to `socis_cli/web_dist/` which the FastAPI server serves as a static SPA.

## Automatic Build on Update

When you run `socis update`, the web frontend is automatically rebuilt if `npm` is available. This keeps the dashboard in sync with code updates. If `npm` isn't installed, the update skips the frontend build and `socis dashboard` will build it on first launch.
...
| Theme | Character |
|-------|-----------|
| **SOCIS Teal** (`default`) | Dark teal + cream, system fonts, comfortable spacing |
| **SOCIS Teal (Large)** (`default-large`) | Same as default with 18px text and roomier spacing |
| **Cobalt** (`nous-blue`) | Vivid blue accents with airy spacing |
