...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of SOCIS's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple SOCIS instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
socis memory setup    # select "honcho" from the provider list
```
...
- `per-session` — each `socis` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — SOCIS syncs them back at session init.
...
The `socis honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `socis memory setup honcho` (or run `socis memory setup` and pick it from the list); the `socis honcho` subcommand then appears on the next invocation.

```bash
socis memory setup honcho    # Configure Honcho directly (works before activation)
socis honcho status          # Connection status, config, and key settings
socis honcho setup           # Redirects to `socis memory setup` (post-activation alias)
socis honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
socis honcho peer            # Show or update peer names + dialectic reasoning level
socis honcho mode            # Show or set recall mode (hybrid/context/tools)
socis honcho tokens          # Show or set token budget for context and dialectic
socis honcho identity        # Seed or show the AI peer's Honcho identity
socis honcho sync            # Sync Honcho config to all existing profiles
socis honcho peers           # Show peer identities across all profiles
socis honcho sessions        # List known Honcho session mappings
socis honcho map             # Map current directory to a Honcho session name
socis honcho enable          # Enable Honcho for the active profile
socis honcho disable         # Disable Honcho for the active profile
socis honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `socis honcho`

If you previously used the standalone `socis honcho setup`:
...
No re-login or re-setup needed. Run `socis memory setup` and select "honcho" — the wizard detects your existing config.
