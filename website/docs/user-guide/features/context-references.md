...
Type `@` followed by a reference to inject content directly into your message. SOCIS expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- SOCIS env: `$SOCIS_AGENT_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$SOCIS_AGENT_HOME/skills/.hub/`
