...
SOCIS has four hook systems that run custom code at key lifecycle points:

| System | Registered via | Runs in | Use case |
|--------|---------------|---------|----------|
| **[Gateway hooks](#gateway-event-hooks)** | `HOOK.yaml` + `handler.py` in `~/.socis-agent/hooks/` | Gateway only | Logging, alerts, webhooks |
...
| **[Shell hooks](#shell-hooks)** | `hooks:` block in `~/.socis-agent/config.yaml` pointing at shell scripts | CLI + Gateway | Drop-in scripts for blocking, auto-formatting, context injection |
| **[Outbound webhooks](#outbound-webhooks)** | `hooks.outbound:` list in `~/.socis-agent/config.yaml` | CLI + Gateway | Push signed lifecycle events to external HTTP endpoints — CI, dashboards, other agents |
...
Each hook is a directory under `~/.socis-agent/hooks/` containing two files:
...
LOG_FILE = Path.home() / ".socis-agent" / "hooks" / "my-hook" / "activity.log"
...
```yaml
# ~/.socis-agent/hooks/command-logger/HOOK.yaml
name: command-logger
...
```python
# ~/.socis-agent/hooks/command-logger/handler.py
import json
from datetime import datetime
from pathlib import Path

LOG = Path.home() / ".socis-agent" / "logs" / "command_usage.jsonl"
...
```yaml
# ~/.socis-agent/hooks/session-webhook/HOOK.yaml
...
A popular pattern from the community: drop a Markdown checklist at `~/.socis-agent/BOOT.md`, and have the agent run it once every time the gateway starts. Useful for "on every boot, check overnight cron failures and ping me on Discord if anything failed," or "summarize the last 24h of deploy.log and post it to Slack #ops."

This tutorial shows how to build it yourself as a user-defined hook. SOCIS does not ship a built-in BOOT.md hook — you wire up exactly the behavior you want.

#### What we're building

1. A file at `~/.socis-agent/BOOT.md` with natural-language startup instructions.
...
Create `~/.socis-agent/BOOT.md`. Write it as if you were giving instructions to a human assistant:

```markdown
# Startup Checklist

1. Run `socis cron list` and check if any scheduled jobs failed overnight.
...
```text
~/.socis-agent/hooks/boot-md/
├── HOOK.yaml
└── handler.py
```

**`~/.socis-agent/hooks/boot-md/HOOK.yaml`**

```yaml
name: boot-md
description: Run ~/.socis-agent/BOOT.md on gateway startup
events:
  - gateway:startup
```

**`~/.socis-agent/hooks/boot-md/handler.py`**

```python
"""Run ~/.socis-agent/BOOT.md on every gateway startup."""
...
BOOT_FILE = Path.home() / ".socis-agent" / "BOOT.md"
...
```bash
socis logs --follow --level INFO | grep boot-md
```
...
Delete `~/.socis-agent/BOOT.md` to disable the checklist — the hook stays loaded but silently skips when the file isn't there.
...
An earlier version of SOCIS shipped this as a built-in hook and silently spawned an agent with bare defaults on every gateway boot. That surprised users with custom endpoints and made the feature invisible to users who didn't know it was running. Keeping it as a documented pattern — built by you, in your hooks directory — means you see exactly what it does and opt in by writing the files.

### How It Works

1. On gateway startup, `HookRegistry.discover_and_load()` scans `~/.socis-agent/hooks/`
...
- Runtime event-name validity comes from `socis_cli.plugins.VALID_HOOKS`. `socis hooks list` lists configured shell/outbound hooks, not every available event; `socis hooks test <event>` reports the valid set only when an invalid event is supplied.
...
| `session_id` | `str` | Current SOCIS session id |
...
If a `pre_tool_call` callback exceeds `plugins.hook_callback_timeout` (or is still running from a previous timed-out fire), SOCIS **fails closed**: the tool is blocked with a timeout message rather than proceeding without a policy decision.
...
**Where context is injected:** Always the **user message**, never the system prompt. This preserves the prompt cache — the system prompt stays identical across turns, so cached tokens are reused. The system prompt is SOCIS's territory (model guidance, tool enforcement, personality, skills). Plugins contribute context alongside the user's input.

The clean user-message `content` remains unchanged. For replay and prompt-cache stability, SOCIS may persist the exact API-bound message, including plugin-injected context, in the row's `api_content` sidecar.
...
```python
VOCAB = "SOCIS, Teknium, SOCIS, kanban"
...
Declare shell-script hooks in your `~/.socis-agent/config.yaml` and SOCIS will run them as subprocesses whenever the corresponding plugin-hook event fires — in both CLI and gateway sessions. No Python plugin authoring required.
...
Shell hooks are registered by calling `agent.shell_hooks.register_from_config(cfg)` at both CLI startup (`socis_cli/main.py`) and gateway startup (`gateway/run.py`). They compose naturally with Python plugin hooks — both flow through the same dispatcher.

### Comparison at a glance

| Dimension | Shell hooks | [Plugin hooks](#plugin-hooks) | [Gateway hooks](#gateway-event-hooks) |
|-----------|-------------|-------------------------------|---------------------------------------|
| Declared in | `hooks:` block in `~/.socis-agent/config.yaml` | `register()` in a `plugin.yaml` plugin | `HOOK.yaml` + `handler.py` directory |
| Lives under | `~/.socis-agent/agent-hooks/` (by convention) | `~/.socis-agent/plugins/<name>/` | `~/.socis-agent/hooks/<name>/` |
...
Each time the event fires, SOCIS spawns a subprocess for every matching hook (matcher permitting), pipes a JSON payload to **stdin**, and reads **stdout** back as JSON.
...
{"action":   "block", "message": "Forbidden: rm -rf"}   // SOCIS-canonical

// Modify a pre_tool_call — rewrite tool args before dispatch:
{"action": "modify", "args": {"new_string": "fixed content"}}         // SOCIS-canonical
...
hooks:
  pre_tool_call:
    - matcher: "terminal|write_file|patch"
      command: "~/.socis-agent/agent-hooks/secret-scan.sh"
      timeout: 10
      fail_closed: true
```
...
`fail_closed` only applies to blocking-capable events (`pre_tool_call` today); setting it on any other event logs a warning at config-parse time and is ignored. `socis hooks test` reflects these semantics — the `parsed` line shows exactly the block shape the dispatcher would receive.
...
hooks:
  post_tool_call:
    - matcher: "write_file|patch"
      command: "~/.socis-agent/agent-hooks/auto-format.sh"
```

```bash
#!/usr/bin/env bash
# ~/.socis-agent/agent-hooks/auto-format.sh
payload="$(cat -)"
...
hooks:
  pre_tool_call:
    - matcher: "terminal"
      command: "~/.socis-agent/agent-hooks/block-rm-rf.sh"
      timeout: 5
```

```bash
#!/usr/bin/env bash
# ~/.socis-agent/agent-hooks/block-rm-rf.sh
payload="$(cat -)"
...
hooks:
  pre_llm_call:
    - command: "~/.socis-agent/agent-hooks/inject-cwd-context.sh"
```

```bash
#!/usr/bin/env bash
# ~/.socis-agent/agent-hooks/inject-cwd-context.sh
...
Claude Code's `UserPromptSubmit` event is intentionally not a separate SOCIS event — `pre_llm_call` fires at the same place and already supports context injection. Use it here.
...
hooks:
  subagent_stop:
    - command: "~/.socis-agent/agent-hooks/log-orchestration.sh"
```

```bash
#!/usr/bin/env bash
# ~/.socis-agent/agent-hooks/log-orchestration.sh
log=~/.socis-agent/logs/orchestration.log
...
Each unique `(event, command)` pair prompts the user for approval the first time SOCIS sees it, then persists the decision to `~/.socis-agent/shell-hooks-allowlist.json`. Subsequent runs (CLI or gateway) skip the prompt.

Three escape hatches bypass the interactive prompt — any one is sufficient:

1. `--accept-hooks` flag on the CLI (e.g. `socis --accept-hooks chat`)
2. `SOCIS_AGENT_ACCEPT_HOOKS=1` environment variable
3. `hooks_auto_accept: true` in `~/.socis-agent/config.yaml`
...
**Script edits are silently trusted.** The allowlist keys on the exact command string, not the script's hash, so editing the script on disk does not invalidate consent. `socis hooks doctor` flags mtime drift so you can spot edits and decide whether to re-approve.

#### Manual allowlisting

Manual allowlisting is useful for non-TTY or service-account deployments where an operator cannot answer the first-use prompt interactively. The allowlist file is `~/.socis-agent/shell-hooks-allowlist.json`, and the expected format is an `approvals` array. Each approval records the hook `event` and the exact `command` string:
...
```json
{
  "approvals": [
    {
      "event": "post_llm_call",
      "command": "/home/socis/.socis-agent/hooks/my-hook.py"
    }
  ]
}
```

The command string must match the configured hook command exactly. A path-keyed object with a `sha256` field is not the expected format and will not approve the hook. Verify manual entries with `socis hooks list`.

### The `socis hooks` CLI

| Command | What it does |
|---------|--------------|
| `socis hooks list` | Dump configured hooks with matcher, timeout, and consent status |
| `socis hooks test <event> [--for-tool X] [--payload-file F]` | Fire every matching hook against a synthetic payload and print the parsed response |
| `socis hooks revoke <command>` | Remove every allowlist entry matching `<command>` (takes effect on next restart) |
| `socis hooks doctor` | For every configured hook: check exec bit, allowlist status, mtime drift, JSON output validity, and rough execution time |
...
- Keep scripts inside `~/.socis-agent/agent-hooks/` so the path is easy to audit.
- Re-run `socis hooks doctor` after you pull a shared config to spot newly-added hooks before they register.
...
Outbound webhooks are the push-side mirror of the [inbound webhook platform](/user-guide/messaging/webhooks): inbound webhooks wake SOCIS when the world changes; outbound webhooks tell the world when SOCIS does something. Configure a list of HTTP endpoints and the lifecycle events they care about, and SOCIS POSTs a signed JSON payload to each endpoint whenever a matching event fires — no polling on the receiving end.
...
- Wake *another* SOCIS instance: point the URL at that instance's inbound webhook

### Configuration

Add a `hooks.outbound:` list to `~/.socis-agent/config.yaml`:
...
Secrets: prefer `secret_env` (the name of an environment variable, typically set in `~/.socis-agent/.env`) over an inline `secret:` literal, so the config file stays free of credentials. Entries without a secret are delivered unsigned (flagged as `UNSIGNED` by `socis hooks list`).

### Wire format

Each firing POSTs a JSON body with the same top-level shape as shell hooks' stdin, plus delivery metadata. `profile` names the SOCIS profile that emitted the event (`"default"` outside profiles), so receivers behind a multiplexed gateway can tell profiles apart:
...
| `X-SOCIS-Event` | The hook event name |
| `X-SOCIS-Delivery` | Unique id per delivery — same value as `delivery_id` in the body |
| `X-SOCIS-Signature-256` | `sha256=<hex>` — HMAC-SHA256 of the raw body, GitHub-style; only present when a secret is configured |
...
- **Dedupe** on `delivery_id` (or the matching `X-SOCIS-Delivery` header) — remember recently seen ids and skip duplicates. SOCIS retries failed deliveries once, so the same id can legitimately arrive twice.
...
- **No consent prompt.** Outbound targets execute no code on your machine — they receive data at a URL you configured. `SOCIS_AGENT_SAFE_MODE=1` still skips registration, same as plugins and shell hooks. Note that payloads include tool inputs and event metadata, so only point targets at endpoints you trust, and prefer `https://`.

`socis hooks list` shows configured outbound targets alongside shell hooks, including whether each target is signed.
