...
When SOCIS Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.socis-agent/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.socis-agent/config.yaml` (switchable per session
via `/personality`).
...
If you use SOCIS' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.socis-agent/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. SOCIS Agent's deliverable
