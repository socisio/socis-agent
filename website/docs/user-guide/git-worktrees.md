...
title: "Git Worktrees"
description: "Run multiple SOCIS agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

SOCIS Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with SOCIS so each session has a clean, isolated working directory.

## Why Use Worktrees with SOCIS?

SOCIS treats the **current working directory** as the project root:

- CLI: the directory where you run `socis` or `socis chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.socis-agent/config.yaml`
...
SOCIS creates `.worktrees/my-experiment/` inside the repo (branch
`socis/my-experiment`, based on the freshly-fetched remote tip unless
`worktree_sync: false`), and retargets the session's terminal and file tools
into it — no restart needed. Omit the name to get a random `socis-<id>`
...
git worktree add ../repo-feature feature/socis-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/socis-experiment` checked out in that directory

Now you can `cd` into the new worktree and run SOCIS there:

```bash
cd ../repo-feature

# Start SOCIS in the worktree
socis
```
...
git worktree add ../repo-experiment-a feature/socis-a
git worktree add ../repo-experiment-b feature/socis-b
```
...
- Works on its own branch (`feature/socis-a` vs `feature/socis-b`).
...
- SOCIS checkpoint data under `~/.socis-agent/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per SOCIS experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/socis-checkpoints-docs`, `feature/socis-refactor-tests`.
- **Commit frequently**
...
- **Avoid running SOCIS from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `socis -w` (Automatic Worktree Mode)

SOCIS has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `socis/socis-<hash>`).
...
```bash
socis -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `socis -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each SOCIS session its own clean checkout.
