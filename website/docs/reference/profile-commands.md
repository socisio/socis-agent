...
This page covers all commands related to [SOCIS profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `socis profile` without a subcommand shows help.
...
```bash
socis profile use <name>
```

Sets `<name>` as the active profile. All subsequent `socis` commands (without `-p`) will use this profile.
...
socis profile use default
```

## `socis profile create`

```bash
socis profile create <name> [options]
```
...
socis profile create work2 --clone-from work

# Clone everything from a specific profile
socis profile create work2-backup --clone-from work --clone-all
```

## `socis profile describe`

```bash
socis profile describe [<name>] [options]
```
...
socis profile describe researcher

# Set it explicitly
socis profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
socis profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
socis profile describe --all --auto
```

## `socis profile delete`

```bash
socis profile delete <name> [options]
```
...
```bash
socis profile delete mybot
socis profile delete mybot --yes
```

:::warning
This permanently deletes the profile's entire directory including all config, memories, sessions, and skills. The `default` profile (`~/.socis-agent`) cannot be deleted — use `socis uninstall` to remove everything.
:::

## `socis profile show`

```bash
socis profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's SOCIS home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ socis profile show work
Profile: work
Path:    ~/.socis-agent/profiles/work
...
```bash
socis profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your SOCIS installation.
...
```bash
socis profile alias work
# Creates/updates ~/.local/bin/work

socis profile alias work --name mywork
# Creates ~/.local/bin/mywork

socis profile alias work --remove
# Removes the wrapper script
```

## `socis profile rename`

```bash
socis profile rename <old-name> <new-name>
```
...
```bash
socis profile rename mybot assistant
# ~/.socis-agent/profiles/mybot → ~/.socis-agent/profiles/assistant
# ~/.local/bin/mybot → ~/.local/bin/assistant
```

## `socis profile export`

```bash
socis profile export <name> [options]
```
...
```bash
socis profile export work
# Creates work.tar.gz in the current directory

socis profile export work -o ./work-2026-03-29.tar.gz
```
...
## `socis profile import`

```bash
socis profile import <archive> [options]
```
...
```bash
socis profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

socis profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `socis -p telemetry`). |
...
socis profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
socis profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
socis profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
socis profile install ./telemetry/
```

### `socis profile update`

```bash
socis profile update <name> [--force-config] [--yes]
```
...
```bash
socis profile info <name>
```

Prints the profile's distribution manifest — name, version, required
SOCIS version, author, env var requirements, the source URL/path, and
...
`socis profile list` also shows the distribution name and version in a
`Distribution` column, and `socis profile show <name>` / `delete <name>`
...
socis profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
socis profile install https://github.com/your-org/internal-assistant.git
```
...
`socis_requires` supports `>=`, `<=`, `==`, `!=`, `>`, `<`, or a bare
version (treated as `>=`). Install fails with a clear error if the current
SOCIS version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host SOCIS can clone from.
3. Tell recipients to run `socis profile install <your-repo-url>`.
...
## `socis -p` / `socis --profile`

```bash
socis -p <name> <command> [options]
socis --profile <name> <command> [options]
```

Global flag to run any SOCIS command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
socis -p work chat -q "Check the server status"
socis --profile dev gateway start
socis -p personal skills list
socis -p work config edit
```

## `socis completion`

```bash
socis completion <shell>
```
...
- `socis profile <TAB>` — subcommands (list, use, create, etc.)
- `socis profile use <TAB>` — profile names
- `socis -p <TAB>` — profile names
