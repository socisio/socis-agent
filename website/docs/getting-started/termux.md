...
description: "Run SOCIS Agent directly on an Android phone with Termux"
---

# SOCIS on Android with Termux
...
SOCIS Agent can run directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop SOCIS from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.
...
APT-managed SOCIS installs are marked with install method `apt`. SOCIS therefore does not run its Git self-updater against package-owned files; use the package manager instead:

```bash
pkg update
pkg upgrade socis-agent
```

Packaging/repository/signing problems for this option should be reported to the community packaging repositories above. SOCIS runtime bugs can still be reported here, keeping in mind that Android/Termux support is best-effort.

---

## Option 1: One-line installer

SOCIS now ships a Termux-aware installer path:
...
- links `socis` into `$PREFIX/bin` so it stays on your Termux PATH
...
SOCIS requires **Python >=3.11,&lt;3.14**. Current Termux ships `python`
...
### 5. Put `socis` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/socis" "$PREFIX/bin/socis"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `socis` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.socis-agent/.env`.
...
### `socis doctor` says ripgrep or Node is missing
