...
description: "Install and deploy SOCIS Agent with Nix — from quick `nix run` to fully declarative NixOS module with container mode"
---
...
SOCIS Agent ships a Nix flake, a NixOS module, and a Home Manager module.
...
**For non-NixOS users**, this only changes the install step. Everything after (`socis setup`, `socis gateway install`, config editing) works identically to the standard install.

**For NixOS module users**, the entire lifecycle is different: configuration lives in `configuration.nix`, secrets go through sops-nix/agenix, the service is a systemd unit, and CLI config commands are blocked. You manage socis the same way you manage any other NixOS service.
:::
...
After `nix profile install`, `socis`, `socis-agent`, and `socis-acp` are on your PATH. From here, the workflow is identical to the [standard installation](./installation.md) — `socis setup` walks you through provider selection, `socis gateway install` sets up a launchd (macOS) or systemd user service, and config lives in `~/.socis-agent/`.

:::warning Messaging platforms (Discord, Telegram, Slack)
The default package includes ALL libraries socis-agent might need. if you want a smaller variant, check the other flake outputs. 
...
:::note
This module needs NixOS. SOCIS is an agent for one person. If you want an agent for one person and not a system service, use the [Home Manager module](#home-manager-module). That module runs on NixOS and on each other system that Home Manager supports.
:::
...
# /etc/nixos/flake.nix (or your system flake)
{
  ...
  outputs = { nixpkgs, socis-agent, ... }: {
    nixosConfigurations.your-host = nixpkgs.lib.nixosSystem {
      system = "x86_64-linux";
      modules = [
        socis-agent.nixosModules.default
        ./configuration.nix
      ];
    };
  };
...
That's it. `nixos-rebuild switch` creates the `socis` user, generates `config.yaml`, wires up secrets, and starts the gateway — a long-running service that connects the agent to messaging platforms (Telegram, Discord, etc.) and listens for incoming messages.
...
```bash
echo "OPENROUTER_API_KEY=sk-or-your-key" | sudo install -m 0600 -o socis /dev/stdin /var/lib/socis/env
```

```nix
services.socis-agent.environmentFiles = [ "/var/lib/socis/env" ];
```
:::

:::tip addToSystemPackages
Setting `addToSystemPackages = true` does two things: puts the `socis` CLI on your system PATH **and** sets `SOCIS_AGENT_HOME` system-wide so the interactive CLI shares state (sessions, skills, cron) with the gateway service. Without it, running `socis` in your shell creates a separate `~/.socis-agent/` directory.
:::

### Container-aware CLI

:::info
When `container.enable = true` and `addToSystemPackages = true`, **every** `socis` command on the host automatically routes into the managed container. This means your interactive CLI session runs inside the same environment as the gateway service — with access to all container-installed packages and tools.

- The routing is transparent: `socis chat`, `socis sessions list`, `socis --version`, etc. all exec into the container under the hood
- All CLI flags are forwarded as-is
- If the container isn't running, the CLI retries briefly (5s with a spinner for interactive use, 10s silently for scripts) then fails with a clear error — no silent fallback
- For developers working on the socis codebase, set `SOCIS_AGENT_DEV=1` to bypass container routing and run the local checkout directly

Set `container.hostUsers` to create a `~/.socis-agent` symlink to the service state directory, so the host CLI and the container share sessions, config, and memories:
...
Users listed in `hostUsers` are automatically added to the `socis` group for file permission access.
...
The CLI auto-detects when sudo is needed and uses it transparently. Without this, you'll need to run `sudo socis chat` manually.
:::
...
systemctl status socis-agent

# Watch logs (Ctrl+C to stop)
journalctl -u socis-agent -f

# If addToSystemPackages is true, test the CLI
socis --version
socis config       # shows the generated config
```
...
:::note Model naming
`settings.model.default` uses the model identifier your provider expects. With [OpenRouter](https://openrouter.ai) (the default), these look like `"anthropic/claude-sonnet-4"` or `"google/gemini-3-flash"`. If you're using a provider directly (Anthropic, OpenAI), set `settings.model.base_url` to point at their API and use their native model IDs (e.g., `"claude-sonnet-4-20250514"`). When no `base_url` is set, SOCIS defaults to OpenRouter.
:::
...
{ config, ... }: {
  services.socis-agent = {
    ...
    # ── Documents ──────────────────────────────────────────────────────
    # USER.md is memory, so it goes to SOCIS_AGENT_HOME. Workspace files use
...
```nix
services.socis-agent.configFile = /etc/socis/config.yaml;
```

This bypasses `settings` entirely — no merge, no generation. The file is copied as-is to `$SOCIS_AGENT_HOME/config.yaml` on each activation.
...
| Add API keys | `environmentFiles` | `[ config.sops.secrets."socis-env".path ]` |
| Give the agent an identity | `socisHomeFiles."SOUL.md"` | `"You are a terse ops assistant."` |
...
| Override the socis package | `package` | `inputs.socis-agent.packages.${system}.default.override { ... }` |
| Change state directory | `stateDir` | `"/opt/socis"` |
...
Both `environment` (non-secret vars) and `environmentFiles` (secret files) are merged into `$SOCIS_AGENT_HOME/.env` at activation time (`nixos-rebuild switch`). SOCIS reads this file on every startup, so changes take effect with a `systemctl restart socis-agent` — no container recreation needed.
...
{
  sops = {
    defaultSopsFile = ./secrets/socis.yaml;
    age.keyFile = "/home/user/.config/sops/age/keys.txt";
    secrets."socis-env" = { format = "yaml"; };
  };

  services.socis-agent.environmentFiles = [
    config.sops.secrets."socis-env".path
  ];
}
```
...
```yaml
# secrets/socis.yaml (encrypted with sops)
socis-env: |
...
```nix
{
  age.secrets.socis-env.file = ./secrets/socis-env.age;

  services.socis-agent.environmentFiles = [
    config.age.secrets.socis-env.path
  ];
}
```
...
{
  services.socis-agent = {
    authFile = config.sops.secrets."socis/auth.json".path;
...
SOCIS reads files from two directories. Thus there are two options. Use the option for the directory that the file must go into.
...
```nix
{
  services.socis-agent = {
    ...
    workingDirectory = "/var/lib/socis/workspace";
    documents = {
...
`socisHomeFiles` installs into **`SOCIS_AGENT_HOME`**. SOCIS reads the identity file and the memory files of the agent from that directory. `SOUL.md` and `memories/` work only from there. A `SOUL.md` in `documents` makes a workspace file. SOCIS does not load that file as the identity:

```nix
{
  services.socis-agent.socisHomeFiles = {
...
`socisHomeFiles` needs no `workingDirectory`, because the module owns the `SOCIS_AGENT_HOME` directory. Most users want `socisHomeFiles`.
...
```nix
{
  services.socis-agent.mcpServers = {
    filesystem = {
      command = "npx";
      args = [ "-y" "@modelcontextprotocol/server-filesystem" "/data/workspace" ];
    };
...
:::tip
Environment variables in `env` values are resolved from `$SOCIS_AGENT_HOME/.env` at runtime. Use `environmentFiles` to inject secrets — never put tokens directly in Nix config.
:::
...
```nix
{
  services.socis-agent.mcpServers.remote-api = {
...
Set `auth = "oauth"` for servers using OAuth 2.1. SOCIS implements the full PKCE flow — metadata discovery, dynamic client registration, token exchange, and automatic refresh.

```nix
{
  services.socis-agent.mcpServers.my-oauth-server = {
...
Tokens are stored in `$SOCIS_AGENT_HOME/mcp-tokens/<server-name>.json` and persist across restarts and rebuilds.

<details>
<summary><strong>Initial OAuth authorization on headless servers</strong></summary>

The first OAuth authorization requires a browser-based consent flow. In a headless deployment, SOCIS prints the authorization URL to stdout/logs instead of opening a browser.

**Option A: Interactive bootstrap** — run the flow once via `docker exec` (container) or `sudo -u socis` (native):

```bash
# Container mode
docker exec -it socis-agent \
...
```bash
socis mcp add my-oauth-server --url https://mcp.example.com/mcp --auth oauth
scp ~/.socis-agent/mcp-tokens/my-oauth-server{,.client}.json \
    server:/var/lib/socis/.socis-agent/mcp-tokens/
# Ensure: chown socis:socis, chmod 0600
```
...
```nix
{
  services.socis-agent.mcpServers.analysis = {
    command = "npx";
...
When socis runs via the NixOS module, the following CLI commands are **blocked** with a descriptive error pointing you to `configuration.nix`:

| Blocked command | Why |
|---|---|
| `socis setup` | Config is declarative — edit `settings` in your Nix config |
| `socis config edit` | Config is generated from `settings` |
| `socis config set <key> <value>` | Config is generated from `settings` |
| `socis gateway install` | The systemd service is managed by NixOS |
| `socis gateway uninstall` | The systemd service is managed by NixOS |

This prevents drift between what Nix declares and what's on disk. Detection uses two signals:

1. **The `SOCIS_AGENT_MANAGED` environment variable.** The service sets it, and the gateway process reads it.
2. **The `.managed` marker file** in `SOCIS_AGENT_HOME`. The activation script writes it, and an interactive shell reads it. Thus the CLI also blocks a command such as `docker exec -it socis-agent socis config set ...`.
...
The flake also exports `homeManagerModules.default`. SOCIS is an agent for one person. The credentials, the memory, the sessions and the cron jobs all belong to that person. Thus a user service is the correct shape on a personal machine. It runs on each distribution that Home Manager supports, and not only on NixOS.

The option set is the same set that the NixOS module uses. It is `services.socis-agent`, with the same `settings`, `environmentFiles`, `documents`, `mcpServers`, `extraPlugins` and `backend` options. Each example above works here without a change. Only the necessary parts are different:
...
`home-manager switch` makes `~/.socis-agent`, writes `config.yaml`, builds `.env` and starts the gateway as a user service.
...
`gateway.enable` runs the messaging gateway for Telegram, Discord, Slack and the other platforms. SOCIS Agent Desktop and the web dashboard connect to a *different* process, which is `socis serve` or `socis dashboard`. `backend.mode` runs that process with the gateway:
...
`serve` runs without a user interface. It gives the `/api/ws` and `/api/pty` sockets that SOCIS Agent Desktop connects to, and it does not build the web application. `dashboard` gives all of that, and also serves the browser admin panel. Both processes use one `SOCIS_AGENT_HOME` with the gateway. Thus the sessions, the skills, the memory and the cron jobs are the same for all of them. `backend.mode` works in the same way on the NixOS module, but not in container mode.
...
```bash
# Linux
systemctl --user status socis-agent
journalctl --user -u socis-agent -f

# macOS
launchctl list | grep socis
tail -f ~/Library/Logs/socis-agent.log

socis --version
socis config     # shows the configuration that Nix wrote
```
...
When container mode is enabled, socis runs inside a persistent Ubuntu container with the Nix-built binary bind-mounted read-only from the host:
...
The Nix-built binary works inside the Ubuntu container because `/nix/store` is bind-mounted — it brings its own interpreter and all dependencies, so there's no reliance on the container's system libraries. The container entrypoint resolves through a `current-package` symlink: `/data/current-package/bin/socis gateway run --replace`. On `nixos-rebuild switch`, only the symlink is updated — the container keeps running.

### What Persists Across What

| Event | Container recreated? | `/data` (state) | `/home/socis` | Writable layer (`apt`/`pip`/`npm`) |
|---|---|---|---|---|
| `systemctl restart socis-agent` | No | Persists | Persists | Persists |
...
The container is only recreated when its **identity hash** changes. The hash covers: schema version, image, `extraVolumes`, `extraOptions`, and the entrypoint script. Changes to environment variables, settings, documents, or the socis package itself do **not** trigger recreation.

:::warning Writable layer loss
When the identity hash changes (image upgrade, new volumes, new container options), the container is destroyed and recreated from a fresh pull of `container.image`. Any `apt install`, `pip install`, or `npm install` packages in the writable layer are lost. State in `/data` and `/home/socis` is preserved (these are bind mounts).

If the agent relies on specific packages, consider baking them into a custom image (`container.image = "my-registry/socis-base:latest"`) or scripting their installation in the agent's SOUL.md.
:::

### GC Root Protection

The `preStart` script creates a GC root at `${stateDir}/.gc-root` pointing to the current socis package. This prevents `nix-collect-garbage` from removing the running binary. If the GC root somehow breaks, restarting the service recreates it.

---

## Plugins

The NixOS module supports declarative plugin installation — no imperative `socis plugins install` needed.
...
Plugins are symlinked into `$SOCIS_AGENT_HOME/plugins/` at activation time. SOCIS discovers them via its normal directory scan. Removing a plugin from the list and running `nixos-rebuild switch` removes the symlink.
...
The package's `site-packages` is added to PYTHONPATH in the socis wrapper. `importlib.metadata` discovers the entry point at session start.

### Optional Dependency Groups (`extraDependencyGroups`)

For optional extras declared in socis-agent's `pyproject.toml`, use `extraDependencyGroups` to include them in the sealed venv at build time. This is required for any extra not in the default `[all]` set — on Nix, runtime installation into the read-only store is not possible.

```nix
# Enable Discord, Telegram, Slack
services.socis-agent.extraDependencyGroups = [ "messaging" ];
```
...
```nix
services.socis-agent.settings.plugins.enabled = [
  "socis-lcm"
  "rtk-rewrite"
];
```
...
:::note
A build-time collision check prevents plugin packages from shadowing core socis dependencies. If a plugin provides a package already in the sealed venv, `nixos-rebuild` fails with a clear error.
:::
...
nix build .#checks.x86_64-linux.managed-guard       # SOCIS_AGENT_MANAGED blocks mutation
...
| Check | What it tests |
|---|---|
| `package-contents` | `socis` and `socis-agent` binaries exist and `socis --version` runs |
| `entry-points-sync` | Every `[project.scripts]` entry in `pyproject.toml` has a wrapped binary in the Nix package |
| `cli-commands` | `socis --help` exposes `gateway` and `config` subcommands |
| `managed-guard` | `SOCIS_AGENT_MANAGED=true socis config set ...` prints the NixOS error |
| `bundled-skills` | Skills directory exists, contains SKILL.md files, `SOCIS_AGENT_BUNDLED_SKILLS` is set in wrapper |
...
|---|---|---|---|
| `enable` | `bool` | `false` | Enable the socis-agent service |
| `package` | `package` | `socis-agent` | The socis-agent package to use |
| `user` | `str` | `"socis"` | System user |
| `group` | `str` | `"socis"` | System group |
| `createUser` | `bool` | `true` | Auto-create user/group |
| `stateDir` | `str` | `"/var/lib/socis"` | State directory (`SOCIS_AGENT_HOME` parent) |
| `workingDirectory` | `str` | `"${stateDir}/workspace"` | Agent working directory |
| `addToSystemPackages` | `bool` | `false` | Add `socis` CLI to system PATH and set `SOCIS_AGENT_HOME` system-wide |
...
| Option | Type | Default | Description |
|---|---|---|---|
| `environmentFiles` | `listOf str` | `[]` | Paths to env files with secrets. Merged into `$SOCIS_AGENT_HOME/.env` at activation time |
...
| `socisHomeFiles` | `attrsOf (either str path)` | `{}` | Files that go into `SOCIS_AGENT_HOME`. `SOUL.md` and `memories/` must be here, or SOCIS does not load them. |
...
| Option | Type | Default | Description |
|---|---|---|---|
| `extraArgs` | `listOf str` | `[]` | Extra args for `socis gateway` |
| `extraPackages` | `listOf package` | `[]` | Extra packages available to the agent. Added to the socis user's per-user profile so terminal commands, skills, and cron jobs all see them |
| `extraPlugins` | `listOf package` | `[]` | Directory plugin packages to symlink into `$SOCIS_AGENT_HOME/plugins/`. Each must contain `plugin.yaml` |
...
### Backend (`socis serve` / `socis dashboard`)

This option runs the process that SOCIS Agent Desktop and the web dashboard connect to, with the gateway. You cannot use it with `container.enable`.
...
daemon". `services.socis-agent` keeps the state, the configuration and the
daemons. `programs.socis-agent` installs what you use, and reads
`socisHome` and the backend address from the services.

| Option | Type | Default | Description |
|---|---|---|---|
| `enable` | `bool` | `false` | Add the `socis` CLI to `home.packages`, and export `SOCIS_AGENT_HOME` for your shells |
| `package` | `package` | `services.socis-agent.package` | The package to install. The default applies `extraPythonPackages` and `extraDependencyGroups` from the services, so both are one build. |
| `desktop.enable` | `bool` | `false` | Add the SOCIS Agent Desktop application, with a launcher entry on Linux |
| `desktop.package` | `package` | `package.socisDesktop` | The desktop package. The default follows `package`, so the application and the services run one SOCIS runtime. |
...
services.socis-agent = {
  enable = true;
  backend.mode = "serve";
  backend.sessionTokenFile = config.sops.secrets."socis/desktop-token".path;
};
```

The launcher carries `SOCIS_AGENT_HOME` itself. A desktop menu reads no shell
profile, so the value that `programs.socis-agent.enable` exports with
`home.sessionVariables` reaches an interactive shell only. Without the
value in the launcher, the application opens `~/.socis-agent` while the
services use `socisHome`, and you see no sessions and no keys.
...
`services.socis-agent.installPackage` was removed by this split. A
...
| `container.hostUsers` | `listOf str` | `[]` | Interactive users who get a `~/.socis-agent` symlink to the service stateDir and are auto-added to the `socis` group |
...
```
/var/lib/socis/                     # stateDir (owned by socis:socis, 0750)
├── .socis-agent/                         # SOCIS_AGENT_HOME
│   ├── SOUL.md                      # from socisHomeFiles: the agent identity
...
```
~/.socis-agent/                           # socisHome (SOCIS_AGENT_HOME), 0700
├── SOUL.md                          # from socisHomeFiles
...
├── auth.json                        # OAuth credentials: seeded, then SOCIS owns it
...
|---|---|---|---|
| `/nix/store` | `/nix/store` | `ro` | SOCIS binary + all Nix deps |
| `/data` | `/var/lib/socis` | `rw` | All state, config, workspace |
| `/home/socis` | `${stateDir}/home` | `rw` | Persistent agent home — `pip install --user`, tool caches |
...
cd /etc/nixos && nix flake update socis-agent
...
```bash
systemctl status socis-agent
docker ps -a --filter name=socis-agent
docker inspect socis-agent --format='{{.State.Status}}'
docker exec -it socis-agent bash
docker exec socis-agent readlink /data/current-package
docker exec socis-agent cat /data/.container-identity
```
...
```bash
sudo systemctl stop socis-agent
docker rm -f socis-agent
sudo rm /var/lib/socis/.container-identity
sudo systemctl start socis-agent
```
...
```bash
# Native mode
sudo -u socis cat /var/lib/socis/.socis-agent/.env

# Container mode
docker exec socis-agent cat /data/.socis-agent/.env
```

### GC Root Verification

```bash
nix-store --query --roots $(docker exec socis-agent readlink /data/current-package)
```
...
| `No adapter available for discord` (or telegram/slack) | Messaging deps missing from the sealed Nix venv | Install `#messaging` variant: `nix profile install ...#messaging`. For NixOS module: `extraDependencyGroups = [ "messaging" ]`. Check `journalctl -u socis-agent` for `FeatureUnavailable` or `requirements not met` for the underlying error. |
| Container recreated unexpectedly | `extraVolumes`, `extraOptions`, or `image` changed | Expected — writable layer resets. Reinstall packages or use a custom image |
| `socis --version` shows old version | Container not restarted | `systemctl restart socis-agent` |
| Permission denied on `/var/lib/socis` | State dir is `0750 socis:socis` | Use `docker exec` or `sudo -u socis` |
| `nix-collect-garbage` removed socis | GC root missing | Restart the service (preStart recreates the GC root) |
| `no container with name or ID "socis-agent"` (Podman) | Podman rootful container not visible to regular user | Add passwordless sudo for podman (see [Container Mode](#container-mode) section) |
| `unable to find user socis` | Container still starting (entrypoint hasn't created user yet) | Wait a few seconds and retry — the CLI retries automatically |
| Tool added via `extraPackages` not found in terminal | Requires `nixos-rebuild switch` to update the per-user profile | Rebuild and restart: `nixos-rebuild switch && systemctl restart socis-agent` |
