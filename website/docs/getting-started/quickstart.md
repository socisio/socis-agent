---
sidebar_position: 1
title: "SOCIS Agent Quickstart"
description: "Your first conversation with SOCIS Agent — from install to chatting in under 5 minutes"
---

# SOCIS Agent Quickstart

This guide gets you from zero to a working SOCIS setup that survives real use. Install, choose a provider, verify a working chat, and know exactly what to do when something breaks.

## Prefer to watch?

**Onchain AI Garage** put together a Masterclass walkthrough of installation, setup, and basic commands — a good companion to this page if you'd rather follow along on video. For more, see the full [SOCIS Agent Tutorials & Use Cases](https://www.youtube.com/playlist?list=PLmpUb_PWAkDxewld5ZYyKifuHxgIbiq2d) playlist.
...
- Setting up SOCIS for a team, bot, or always-on workflow
...
|---|---|---|
| I just want SOCIS working on my machine | `socis setup` | Run a real chat and verify it responds |
| I already know my provider | `socis model` | Save the config, then start chatting |
| I want a bot or always-on setup | `socis gateway setup` after CLI works | Connect Telegram, Discord, Slack, or another platform |
| I want a local or self-hosted model | `socis model` → custom endpoint | Verify the endpoint, model name, and context length |
| I want multi-provider fallback | `socis model` first | Add routing and fallback only after the base chat works |

**Rule of thumb:** if SOCIS cannot complete a normal chat, do not add more features yet. Get one clean conversation working first, then layer on gateway, cron, skills, voice, or routing.

---

## 1. Install SOCIS Agent
### With the SOCIS Agent Desktop installer on macOS or Windows (recommended)
To easily install the command-line and desktop applications, [download the SOCIS Agent Desktop installer](https://agent.socis.io/) from our website and run it.

### Without SOCIS Agent Desktop:
For a command-line only install without SOCIS Agent Desktop, run:
...
The single most important setup step. Use `socis model` to walk through the choice interactively:
...
:::info Setup modes
On a fresh install, `socis setup` offers three modes:

- **Full Setup** — pick from every provider (OpenAI, GLM, xAI, MiniMax, Nous Portal, and more), bring your own keys or use OAuth where available. The recommended default path.
...
Blank Slate writes an explicit `platform_toolsets.cli` list plus `agent.disabled_toolsets`, so nothing you didn't choose ever loads — not even after `socis update`. Re-enable anything later with `socis tools`, seed skills with `socis skills opt-in --sync`, or tune settings with `socis setup agent`.
:::
...
|----------|-----------|---------------|
| **Nous Portal** | Subscription-based, zero-config | OAuth login via `socis model` |
| **OpenAI Codex** | ChatGPT or Codex subscription, uses Codex models | Device code auth via `socis model` → **ChatGPT or Codex Subscription** |
| **Anthropic** | Claude models directly — Max plan + extra usage credits (OAuth), or API key for pay-per-token | `socis model` → OAuth login (requires Max + extra credits), or an Anthropic API key |
...
| **MiniMax (OAuth)** | MiniMax frontier model via browser OAuth — no API key needed (model name in `socis_cli/models.py` may change between releases) | `socis model` → MiniMax (OAuth) |
...
| **xAI Grok OAuth** | SuperGrok / Premium+ subscription, no API key needed | `socis model` → xAI Grok OAuth |
...
| **Qwen OAuth** | Qwen Portal browser OAuth — no API key needed | `socis model` → Qwen OAuth |
...
| **GitHub Copilot** | GitHub Copilot subscription (GPT-5.x, Claude, Gemini, etc.) | OAuth via `socis model`, or `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` |
| **GitHub Copilot ACP** | Copilot ACP agent backend (spawns local `copilot` CLI) | `socis model` (requires `copilot` CLI + `copilot login`) |
...
SOCIS Agent requires a model with at least **64,000 tokens** of context. Models with smaller windows cannot maintain enough working memory for multi-step tool-calling workflows and will be rejected at startup. Most hosted models (Claude, GPT, Gemini, Qwen, DeepSeek) meet this easily. If you're running a local model, set its context size to at least 64K (e.g. `--ctx-size 65536` for llama.cpp or `-c 65536` for Ollama).
:::

:::tip
You can switch providers at any time with `socis model` — no lock-in. For a full list of all supported providers and setup details, see [AI Providers](../integrations/providers.md).
:::

### How settings are stored

SOCIS separates secrets from normal config:

- **Secrets and tokens** → `~/.socis-agent/.env`
- **Non-secret settings** → `~/.socis-agent/config.yaml`

The easiest way to set values correctly is through the CLI:

```bash
socis config set model anthropic/claude-opus-4.6
socis config set terminal.backend docker
socis config set OPENROUTER_API_KEY sk-or-...
```
...
```bash
socis            # classic CLI
socis --tui      # modern TUI (recommended)
```

You'll see a welcome banner with your model, available tools, and skills. Use a prompt that's specific and easy to verify:

:::tip Pick your interface
SOCIS ships with two terminal interfaces: the classic `prompt_toolkit` CLI and a newer [TUI](../user-guide/tui.md) with modal overlays, mouse selection, and non-blocking input. Both share the same sessions, slash commands, and config — try each with `socis` vs `socis --tui`.
:::
...
```bash
socis --continue    # Resume the most recent session
socis -c            # Short form
```
...
```bash
socis gateway setup    # Interactive platform configuration
```
...
- `socis tools` — tune tool access per platform
- `socis skills` — browse and install reusable workflows
...
```bash
socis config set terminal.backend docker    # Docker isolation
socis config set terminal.backend ssh       # Remote server
```

For Docker sandboxes, you can also enable the **egress credential-injection proxy** so the sandbox never sees your real API keys — only opaque proxy tokens that work exclusively from behind a local TLS-intercepting daemon. See [Egress proxy](../user-guide/egress/iron-proxy.md). Setup is `socis egress setup && socis egress start`; `socis setup terminal` also points Docker users at it. Modal, SSH, Daytona, and Singularity are not wired yet.

### Voice mode

```bash
# From the SOCIS install directory (the curl installer placed it at
# ~/.socis-agent/socis-agent on Linux/macOS or %LOCALAPPDATA%\socis\socis-agent on Windows):
cd ~/.socis-agent/socis-agent
...
Skills are on-demand instruction documents that teach SOCIS how to do a specific task — deploy to Kubernetes, open a GitHub PR, fine-tune a model, search for GIFs. Each is a `SKILL.md` file with a name, a description, and a step-by-step procedure. The agent reads the short descriptions for free and only loads a skill's full content when a task actually calls for it, so adding skills doesn't bloat every request.

SOCIS ships with a catalog of bundled skills already installed in `~/.socis-agent/skills/`. You can add more from the Skills Hub, or write your own.

**Browse and install from the hub:**

```bash
socis skills browse                      # list everything available
socis skills search kubernetes           # find skills by keyword
socis skills install openai/skills/k8s   # install one (runs a security scan first)
```

The install argument is a `source/path` slug from the hub — `openai/skills/k8s` means the `k8s` skill from OpenAI's catalog. `socis skills browse` shows the exact slugs to use.
...
/k8s                                       # load it and let SOCIS ask what you need
```
...
(If you installed without `[all]`, run `cd ~/.socis-agent/socis-agent && uv pip install -e ".[acp]"` first.)
...
|---|---|---|
| SOCIS opens but gives empty or broken replies | Provider auth or model selection is wrong | Run `socis model` again and confirm provider, model, and auth |
...
| Gateway starts but nobody can message it | Bot token, allowlist, or platform setup is incomplete | Re-run `socis gateway setup` and check `socis gateway status` |
| `socis --continue` can't find old session | Switched profiles or session never saved | Check `socis sessions list` and confirm you're in the right profile |
...
| `socis doctor` flags config problems | Config values are missing or stale | Fix the config, retest a plain chat before adding features |
...
2. `socis model`
3. `socis setup`
4. `socis sessions list`
5. `socis --continue`
6. `socis gateway status`
...
| `socis tools` | Configure which tools are enabled per platform |
| `socis setup` | Full setup wizard (configures everything at once) |
| `socis doctor` | Diagnose issues |
| `socis update` | Update to latest version |
| `socis gateway` | Start the messaging gateway |
| `socis --continue` | Resume last session |
...
- **[Moving to another machine](/reference/faq#exporting-socis-to-another-machine)** — `socis backup` migrates your whole setup (or [a single profile](/reference/faq#moving-a-single-profile-to-another-machine)); no need to rebuild from scratch
