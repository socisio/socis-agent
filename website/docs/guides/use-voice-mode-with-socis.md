---
sidebar_position: 8
title: "Use Voice Mode with SOCIS"
description: "A practical guide to setting up and using SOCIS voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with SOCIS
...
- you want SOCIS sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in SOCIS.
...
## Step 1: make sure normal SOCIS works first
...
SOCIS supports both local and cloud speech stacks.
...
### If you use `socis setup`

If you choose NeuTTS in the setup wizard, SOCIS checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. SOCIS transcribes and responds
...
- asking SOCIS to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full SOCIS loop.

## Tuning CLI behavior

### Silence threshold

If SOCIS starts/stops too aggressively, tune:
...
SOCIS stays a normal chat bot, but can speak replies.
...
- you want SOCIS to function like a portable research or ops assistant
...
SOCIS joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- SOCIS detects speech boundaries
- transcripts are posted in the associated text channel
- SOCIS responds in text and audio
...
1. get text SOCIS working
2. run `socis setup tts` to enable voice support
