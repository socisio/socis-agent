---
sidebar_position: 7
title: "Use SOUL.md with SOCIS"
description: "How to use SOUL.md to shape SOCIS Agent's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with SOCIS

`SOUL.md` is the **primary identity** for your SOCIS instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want SOCIS to feel like the same assistant every time you talk to it — or if you want to replace the SOCIS persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm SOCIS should be
- what SOCIS should avoid stylistically
- how SOCIS should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who SOCIS is and how SOCIS speaks
...
SOCIS now uses only the global SOUL file for the current instance:

```text
~/.socis-agent/SOUL.md
```

If you run SOCIS with a custom home directory, it becomes:
...
SOCIS automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, SOCIS does not overwrite it
- if the file exists but is empty, SOCIS adds nothing from it to the prompt

## How SOCIS uses it

When SOCIS starts a session, it reads `SOUL.md` from `SOCIS_AGENT_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how SOCIS feels.
...
SOCIS already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How SOCIS should sound.

# Avoid
What SOCIS should not do.

# Defaults
How SOCIS should behave when ambiguity appears.
```
...
```bash
nano ~/.socis-agent/SOUL.md
```

or

```bash
vim ~/.socis-agent/SOUL.md
```

Then restart SOCIS or start a new session.
...
### I edited SOUL.md but SOCIS still sounds the same

Check:
- you edited `~/.socis-agent/SOUL.md` or `$SOCIS_AGENT_HOME/SOUL.md`
...
### SOCIS is ignoring parts of my SOUL.md
