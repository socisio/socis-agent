...
```bash
socis teams-pipeline token-health
socis teams-pipeline token-health --force-refresh
```
...
```bash
socis teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
socis teams-pipeline maintain-subscriptions
socis teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: SOCIS cron (recommended if you already run the SOCIS gateway)

SOCIS ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.socis-agent/scripts/`. First create the script:

```bash
mkdir -p ~/.socis-agent/scripts
cat > ~/.socis-agent/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec socis teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.socis-agent/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
socis cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/socis-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=SOCIS Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=socis
EnvironmentFile=/etc/socis/env
ExecStart=/usr/local/bin/socis teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/socis-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run SOCIS Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now socis-teams-pipeline-maintain.timer
systemctl list-timers socis-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/socis teams-pipeline maintain-subscriptions >> /var/log/socis/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.socis-agent/.env` at the top of a wrapper script that crontab calls.
...
```bash
socis teams-pipeline subscriptions   # should show expirationDateTime advanced
socis teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
socis teams-pipeline list
socis teams-pipeline list --status failed
socis teams-pipeline show <job-id>
```

### Replay a stored job

```bash
socis teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
socis teams-pipeline fetch --meeting-id <meeting-id>
socis teams-pipeline fetch --join-web-url "<join-url>"
socis teams-pipeline fetch --join-web-url "<join-url>" --organizer-user-id <entra-user-id>
```
...
- run `socis teams-pipeline maintain-subscriptions --dry-run`
- inspect `socis teams-pipeline list --status failed`
...
- run `socis teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `socis teams-pipeline run`
...
- [ ] `socis teams-pipeline validate` returns an OK snapshot
- [ ] `socis teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (SOCIS cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
