...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `socis send`."
---

# Pipe Script Output to Messaging Platforms

`socis send` is a small, scriptable CLI that pushes a message to any
messaging platform SOCIS is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | socis send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `socis
...
socis send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
socis send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
socis send --list

# Filter by platform
socis send --list telegram
```
...
Any platform SOCIS ships adapters for works as a target:
...
`socis send` resolves the message body in this order:

1. **Positional argument** — `socis send --to telegram "hi"`
2. **`--file PATH`** — `socis send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | socis send --to telegram`

When stdin is a TTY (no pipe), SOCIS does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  socis send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `socis send` reuses your SOCIS config, the same script works on
any host where SOCIS is installed — no need to export bot tokens into
...
`socis send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  socis send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | socis send \
...
0 9 * * * /usr/local/bin/generate-metrics.sh \
  | /home/me/.socis-agent/bin/socis send \
...
```bash
./train.py --epochs 200 && \
  socis send --to telegram "training done" || \
  socis send --to telegram "training failed (exit $?)"
```
...
socis send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(socis send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `socis send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `socis send` calls
the platform's REST endpoint directly using credentials from
`~/.socis-agent/.env` and `~/.socis-agent/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `socis gateway start` and retry.
...
# Machine-readable
socis send --list --json
```

The listing is built from `~/.socis-agent/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`socis gateway
...
| Approach | Multi-platform | Reuses SOCIS creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `socis send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`socis send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `socis send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `socis send` shares with cron delivery.
