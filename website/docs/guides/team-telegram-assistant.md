...
This tutorial walks you through setting up a Telegram bot powered by SOCIS Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **SOCIS Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.socis-agent/.env`

:::tip
A $5/month VPS is plenty for running the gateway. SOCIS itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team SOCIS Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_socis_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by SOCIS Agent. DM me for help with code, research, debugging, and more.
   ```
...
Add these lines to `~/.socis-agent/.env`:
...
```
[Gateway] Starting SOCIS Gateway...
...
# View live logs
journalctl --user -u socis-gateway -f
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `socis gateway install` to update the plist.
:::
...
```bash
socis gateway stop && socis gateway start
```
...
socis pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.socis-agent/.env`:
...
Control how much detail the bot shows when using tools. In `~/.socis-agent/config.yaml`:
...
Customize how the bot communicates by editing `~/.socis-agent/SOUL.md`:

For a full guide, see [Use SOUL.md with SOCIS](/guides/use-soul-with-socis).
...
```bash
# From the CLI
socis cron list          # View all scheduled jobs
socis cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u socis-gateway` (Linux) or `~/.socis-agent/logs/gateway.log` (macOS) |
| Cron job output | `~/.socis-agent/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.socis-agent/cron/jobs.json` |
| Pairing data | `~/.socis-agent/pairing/` |
| Session history | `~/.socis-agent/sessions/` |
