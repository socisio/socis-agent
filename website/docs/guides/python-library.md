---
sidebar_position: 5
title: "Using SOCIS as a Python Library"
description: "Embed AIAgent in your own Python scripts, web apps, or automation pipelines — no CLI required"
---

# Using SOCIS as a Python Library

SOCIS isn't just a CLI tool. You can import `AIAgent` directly and use it programmatically in your own Python scripts, web applications, or automation pipelines. This guide shows you how.

---

## Installation

Clone SOCIS and create its supported editable development environment:
...
Run your application with `uv run python your_app.py` from that checkout. SOCIS does not publish a supported wheel or source distribution for `requirements.txt` installs.

:::tip
The same environment variables used by the CLI are required when using SOCIS as a library. At minimum, set `OPENROUTER_API_KEY` (or `OPENAI_API_KEY` / `ANTHROPIC_API_KEY` if using direct provider access).
:::
...
The simplest way to use SOCIS is the `chat()` method — pass a message, get a string back:
...
:::warning
Always set `quiet_mode=True` when embedding SOCIS in your own code. Without it, the agent prints CLI spinners, progress indicators, and other terminal output that will clutter your application's output.
:::
...
For running many prompts in parallel, SOCIS includes `batch_runner.py`. It manages concurrent `AIAgent` instances with proper resource isolation:
...
async def on_message(message):
    if message.author == client.user:
        return
    if message.content.startswith("!socis "):
...
|-----------|------|---------|-------------|
| `model` | `str` | `""` | Model in OpenRouter format (defaults to empty; resolved from your socis config at runtime) |
