...
description: "Diagnose and fix common SOCIS cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
If you're expecting jobs to fire automatically, you need a running gateway (`socis gateway` for foreground, or `socis gateway start` for the installed service). For one-off debugging, you can manually trigger a tick with `socis cron tick`.
...
```bash
date
socis cron list   # Compare next_run times with local time
```
...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.socis-agent/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.socis-agent/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.socis-agent/.env` |
...
| `local` | Write access to `~/.socis-agent/cron/output/` |
...
If delivery fails, the job still runs — it just won't send anywhere. Check `socis cron list` for updated `last_error` field (if available).
...
### Check 5: Relay-fronted platforms (SOCIS Cloud / Team Gateway)
...
Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `socis skills install <skill-name>` or via `/skills` in the CLI.

### Check 2: Check skill name vs. skill folder name

Skill names are case-sensitive and must match the installed skill's folder name. If your job specifies `ai-funding-report` but the skill folder is `ai-funding-daily-report`, confirm the exact name from `socis skills list`.
...
2. `~/.socis-agent/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
3. The job's `last_run` metadata via `socis cron list`

### Check 2: Common error patterns

**"No such file or directory" for scripts**
The `script` path must be an absolute path (or relative to the SOCIS config directory). Verify:
```bash
ls ~/.socis-agent/scripts/your-script.py   # Must exist
socis cron edit <job_id> --script ~/.socis-agent/scripts/your-script.py
```

**"Skill not found" at job execution**
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `socis skills install <skill-name>`.
...
Jobs are stored in `~/.socis-agent/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.socis-agent/cron/jobs.json
chmod 600 ~/.socis-agent/cron/jobs.json   # Your user should own it
```
...
```bash
socis cron list                    # Show all jobs, states, next_run times
socis cron run <job_id>            # Schedule for next tick (for testing)
socis cron edit <job_id>           # Fix configuration issues
socis logs                         # View recent SOCIS logs
socis skills list                  # Verify installed skills
```
...
1. Run the job with `socis cron run <job_id>` (fires on next gateway tick) and watch for errors in the chat output
2. Check `~/.socis-agent/logs/agent.log` for scheduler messages and `~/.socis-agent/logs/errors.log` for warnings
3. Open an issue at [github.com/socisio/socis-agent](https://github.com/socisio/socis-agent) with:
