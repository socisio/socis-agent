"""Windows UTF-8 bootstrap for SOCIS entry points.
...
should be imported at the very top of every SOCIS entry point
(``socis``, ``socis-agent``, ``socis-acp``, ``python -m gateway.run``,
...
def suppress_platform_ver_console() -> None:
    ...
    Mirrors ``socis_cli._subprocess_compat.suppress_platform_ver_console``
    ...
    ``socis_agent_bootstrap``, never ``socis_cli.main``.
    """
    if not _IS_WINDOWS:
        return
    try:
...
def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing SOCIS modules.

    SOCIS ships top-level modules with common names (``utils``, ``proxy``,
    ...
    We drop the relative forms outright, then force the real SOCIS source root
    ...
    """
    root = src_root or os.environ.get("SOCIS_AGENT_PYTHON_SRC_ROOT") or os.path.dirname(
...
def activate_durable_lazy_target() -> None:
    ...
    (``SOCIS_AGENT_LAZY_INSTALL_TARGET``, e.g. ``/opt/data/lazy-packages``).
    ...
    """
    if not os.environ.get("SOCIS_AGENT_LAZY_INSTALL_TARGET", "").strip():
        return
    try:
...
# Apply on import — entry points just need ``import socis_agent_bootstrap``
# (or ``from socis_agent_bootstrap import apply_windows_utf8_bootstrap``) at
