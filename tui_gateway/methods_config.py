    ...
    try:
        with _profile_db(params) as db:
            ...
            with pdb.connect_closing() as conn:
                ...
                # zero SOCIS sessions still surface. The desktop's native scan
...
def _(rid, params: dict) -> dict:
    ...
    if key == "provider":
        try:
            from socis_cli.models import list_available_providers, normalize_provider
    ...
    if key == "profile":
        ...
        return _ok(rid, {"home": str(_socis_agent_home), "display": display_socis_agent_home()})
    if key == "project":
    ...
    if key == "mtime":
        cfg_path = _socis_agent_home / "config.yaml"
        try:
...
def _readiness_profile_scope(params: dict):
    ...
    that profile's SOCIS_AGENT_HOME and ``.env`` secret scope (ContextVars, so
        ...
        if (
            ...
            return _ok(
                rid,
                {
                    ...
                    "source": source,
                    "error": "No SOCIS provider is configured.",
                    **scoped,
                },
            )
...
def _(rid, params: dict) -> dict:
    ...
    collection + force-redaction pipeline as ``socis debug share --nous``
