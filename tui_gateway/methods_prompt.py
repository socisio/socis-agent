...
def _(rid, params: dict) -> dict:
    from socis_cli.input_sanitize import sanitize_user_prompt_text
        ...
        if title.startswith("Group: "):
            ...
            if room_id:
                try:
                    ...
                    peer = False
                    if not hosted:
                        from socis_agent_constants import named_profile_home
                    ...
                    if hosted or peer:
                        return _err(
                            ...
                            + "Update SOCIS Agent Desktop to continue it.",
                        )
    ...
    # over another app when they are back in SOCIS.
        ...
        if (
            ...
            if not is_truthy_value(params.get("confirm_truncate")):
                ...
                return _err(
                    ...
                    "(update your SOCIS client if a rewind was intended)",
                )
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from socis_cli.clipboard import has_clipboard_image, save_clipboard_image
...
def _(rid, params: dict) -> dict:
    ...
    def run():
        ...
        try:
            ...
            # SOCIS_AGENT_HOME override (the ContextVar set on the session-create
            ...
            home_token = (
                set_socis_agent_home_override(_profile_home_str)
...
def _(rid, params: dict) -> dict:
    ...
    prompt = "\n".join(
        ...
        for line in [
            ...
            "Restart exactly the app intended for the Preview URL, not SOCIS Agent Desktop itself.",
            ...
            "First inspect what process, if any, owns the Preview URL port. If a stale server exists, inspect its cwd and prefer that cwd over the SOCIS/Desktop process cwd.",
            ...
            "Before declaring success, verify the Preview URL responds with the intended app, not SOCIS Agent Desktop. If it serves SOCIS/Desktop UI or another unrelated app, stop that process and report failure.",
    ...
    def run():
        ...
        try:
            ...
            # the session's SOCIS_AGENT_HOME override (the ContextVar set on the
...
def _(rid, params: dict) -> dict:
    # `text` is a JSON string describing the OS window underneath the SOCIS
