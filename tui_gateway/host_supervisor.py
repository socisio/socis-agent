...
def _default_registry_path() -> Path:
    return get_socis_agent_home() / "state" / _REGISTRY_NAME
...
class HostSupervisor:
    ...
    def __init__(
        ...
        *,
        ...
        expected_socis_agent_home: str | None = None,
        autostart: bool = True,
    ) -> None:
        ...
        self.expected_socis_agent_home = expected_socis_agent_home if expected_socis_agent_home is not None else str(get_socis_agent_home())
    ...
    def _spawn_locked(self, *, reason: str) -> None:
        ...
        if self.env:
            env.update(self.env)
        env["SOCIS_AGENT_COMPUTE_HOST_HEARTBEAT_SECS"] = str(self.heartbeat_secs)
    ...
    def _validate_hello(self) -> None:
        ...
        got_home = str(hello.get("socis_agent_home") or "")
        if got_home and got_home != self.expected_socis_agent_home:
            raise RuntimeError(f"compute host SOCIS_AGENT_HOME mismatch: {got_home} != {self.expected_socis_agent_home}")
