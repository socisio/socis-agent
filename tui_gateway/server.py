...
_socis_agent_home = get_socis_agent_home()
load_socis_dotenv(
    socis_agent_home=_socis_agent_home, project_env=Path(__file__).parent.parent / ".env"
)
...
# appends every unhandled exception to ~/.socis-agent/logs/tui_gateway_crash.log
...
_CRASH_LOG = os.path.join(_socis_agent_home, "logs", "tui_gateway_crash.log")
...
try:
    from socis_cli.banner import prefetch_update_check
...
try:
    _slash_timeout = float(os.environ.get("SOCIS_AGENT_TUI_SLASH_TIMEOUT_S") or "45")
...
def _resolve_ws_orphan_reap_grace() -> float:
    ...
    ``SOCIS_AGENT_TUI_WS_ORPHAN_REAP_GRACE_S`` env var is kept as an internal
    override for backward compatibility and wins when set.
    """
    raw = os.environ.get("SOCIS_AGENT_TUI_WS_ORPHAN_REAP_GRACE_S")
...
def _resolve_ws_orphan_activity_stale() -> float:
    ...
    ``SOCIS_AGENT_TUI_WS_ORPHAN_ACTIVITY_STALE_S`` env var is an internal
    ...
    """
    raw = os.environ.get("SOCIS_AGENT_TUI_WS_ORPHAN_ACTIVITY_STALE_S")
    if raw is None or not str(raw).strip():
        try:
            from socis_cli.config import load_config
...
try:
    _rpc_pool_workers = max(
        2, int(os.environ.get("SOCIS_AGENT_TUI_RPC_POOL_WORKERS") or "8")
    )
except (ValueError, TypeError):
    _rpc_pool_workers = 8
...
_current_runtime_session_record: contextvars.ContextVar[dict | None] = (
    contextvars.ContextVar("socis_gateway_runtime_session_record", default=None)
)
...
_current_rpc_method: contextvars.ContextVar[str] = contextvars.ContextVar(
    "socis_gateway_rpc_method", default=""
)
...
def _prepend_tool_paths(env: dict[str, str]) -> dict[str, str]:
    """Prepend SOCIS' managed bin, the venv bin dir, and the user-local
    bin dir to PATH so slash_worker child processes can resolve
    SOCIS-managed CLIs (browser-use, uvx, uv) even when the parent
    ...
    try:
        ...
        managed_bin = str(Path(get_socis_agent_home()) / "bin")
    except Exception:
        pass
...
class _SlashWorker:
    """Persistent SOCISCLI subprocess for slash commands."""
    ...
    def __init__(self, session_key: str, model: str, profile_home: str | None = None):
        ...
        self._closed = False
        from socis_cli._subprocess_compat import windows_hide_flags

        # slash_worker runs the SOCIS agent → needs provider credentials.
        ...
        # gateway's launch SOCIS_AGENT_HOME (#40677). The override goes through the
        # build_subprocess_env factory's `extra` (applied last, always wins)
        # instead of a hand-rolled env["SOCIS_AGENT_HOME"] assignment.
        from tools.environments.local import build_subprocess_env
        env = build_subprocess_env(
            socis_subprocess_env(inherit_credentials=True),
            scrub_secrets=False,
            inherit_profile_home=False,  # base already carries the HOME contract
            extra={"SOCIS_AGENT_HOME": str(profile_home)} if profile_home else None,
        )
        # Prepend the SOCIS venv bin dir and the user-local bin dir to PATH so
        # slash_worker child processes can resolve SOCIS-managed CLIs
...
def _notify_session_boundary(
    ...
    try:
        from socis_cli.lifecycle import finalize_session, invoke_hook
...
_SESSION_OWNERSHIP_UNAVAILABLE = (
    "SOCIS could not safely reserve this session. Try again."
)
...
def _claim_active_session_slot(
    ...
    *,
    ...
    try:
        from socis_cli.active_sessions import try_acquire_active_session
...
def _other_runtime_lease_guard(session_id: str, session: dict):
    ...
    lease = session.get("active_session_lease")
    try:
        from socis_cli.active_sessions import (
...
def _transfer_active_session_slot(
    ...
    *,
    ...
    if lease is None:
        return True
    try:
        from socis_cli.active_sessions import transfer_active_session
...
def _finalize_session(session: dict | None, end_reason: str = "tui_close") -> None:
    ...
    if agent is not None:
        try:
            from socis_cli.lifecycle import invoke_hook
        ...
        with _session_resume_lock:
            with _sessions_lock:
                ...
                if current.get("close_on_disconnect"):
                    ...
                    # `socis --tui` keeps real _stdio. UNLESS another window
...
try:
    _SESSION_TTL_S = float(os.environ.get("SOCIS_AGENT_TUI_SESSION_TTL_S") or 6 * 3600)
...
# A `socis serve` killed mid-update used to lose every un-flushed in-memory
...
try:
    _EXIT_FLUSH_BUDGET_S = float(
        os.environ.get("SOCIS_AGENT_TUI_EXIT_FLUSH_BUDGET_S") or 5.0
    )
except (TypeError, ValueError):
...
try:
    _INCREMENTAL_FLUSH_INTERVAL_S = float(
        os.environ.get("SOCIS_AGENT_TUI_SESSION_FLUSH_INTERVAL_S") or _REAPER_SCAN_S
    )
except (TypeError, ValueError):
...
def _flush_sessions_before_exit(budget_s: float | None = None) -> int:
    ...
    can never block exit longer than ``SOCIS_AGENT_TUI_EXIT_FLUSH_BUDGET_S``
    ...
    worker = threading.Thread(target=_run, daemon=True, name="socis-exit-flush")
    worker.start()
    worker.join(budget)
...
def install_exit_flush_signal_handlers() -> bool:
    ...
    Called by ``socis serve`` / dashboard startup before uvicorn takes over
...
def _transport_is_dead(transport) -> bool:
    ...
    # for a standalone `socis --tui`, so it must NOT count as dead here (doing
...
def _reap_idle_sessions() -> None:
    ...
    try:
        from socis_cli.mem_trim import trim_memory
...
def _reclaim_orphaned_leases() -> None:
    ...
    try:
        from socis_cli.active_sessions import release_orphaned_leases
...
def _max_live_sessions() -> int:
    try:
        from socis_cli.active_sessions import coerce_max_concurrent_sessions
...
def _sweep_orphaned_session_rows() -> list[str]:
    ...
    ago (``SOCIS_AGENT_TUI_SESSION_TTL_S``). A freshly created row that copied
...
_HEARTBEAT_REFRESH_S = float(
    os.environ.get("SOCIS_AGENT_GATEWAY_HEARTBEAT_REFRESH_S") or 60.0
)
...
def _schedule_startup_orphan_sweep() -> None:
    ...
    reads the DB. ``SOCIS_AGENT_TUI_WS_ORPHAN_REAP_GRACE_S=0`` (park forever)
    and ``SOCIS_AGENT_TUI_SESSION_TTL_S=0`` both suppress the sweep; so does
...
def _get_db():
    ...
    if _db is None:
        from socis_agent_state import get_shared_session_db
...
# ``profile`` on those calls; we open that profile's db and bind its SOCIS_AGENT_HOME
...
def _profile_home(profile: str | None) -> Path | None:
    ...
    if home.resolve() == Path(_socis_agent_home).resolve():
        return None
...
def _profile_scoped(handler):
    """Bind ``params['profile']``'s SOCIS_AGENT_HOME around a handler.

    Pets (config + sprites) and projects (projects.db, discovery policy) both
    resolve via ``get_socis_agent_home``. The desktop sends ``profile`` so a single
    ...
    def wrapper(rid, params):
        ...
        token = set_socis_agent_home_override(home)
        try:
            return handler(rid, params)
        finally:
...
def _profile_configured_cwd(profile_home: Path | None) -> str | None:
    ...
    if profile_home is None:
        return None
    try:
        from socis_cli.config import _expand_env_vars, read_user_config_raw
...
def _launch_configured_cwd() -> str | None:
    ...
    (wherever ``socis dashboard`` was launched) instead of the configured
...
def _inside_compute_host_child() -> bool:
    return os.environ.get("SOCIS_AGENT_COMPUTE_HOST_CHILD") == "1"
...
def _start_agent_build(sid: str, session: dict) -> None:
    ...
    Classic `socis` shows the prompt before constructing AIAgent; the TUI used
    ...
    def _build() -> None:
        ...
        try:
            ...
            # SOCIS_AGENT_HOME so config/skills/model resolve to it, and hand the
            # agent that profile's db so turns persist to the right state.db.
            session_db = None
            if profile_home:
                home_token = set_socis_agent_home_override(profile_home)
                try:
...
def _ensure_session_db_row(session: dict) -> bool:
    ...
    * A terminal session (``socis`` / ``socis --tui`` / CLI) is started from a
    ...
    if str(model_config.get("provider") or "").strip().lower() == "custom":
        try:
            from socis_cli.runtime_provider import canonical_custom_identity
    ...
    try:
        ...
        from socis_agent_state import is_disk_full_error
        ...
        if close_db:
            try:
                from socis_agent_state import release_or_close
                release_or_close(db)
            except Exception:
                pass
    return True
...
def _set_session_cwd(session: dict, cwd: str) -> str:
    from socis_agent_constants import translate_cwd_for_wsl_backend
...
def _load_dashboard_process_isolation_config(cfg: dict | None = None) -> dict[str, Any]:
    ...
    ``socis_cli.config.DEFAULT_CONFIG``. Keep
...
def _load_cfg_raw() -> dict:
    ...
    try:
        ...
        # launch profile's _socis_agent_home. Cache is keyed on the resolved path, so
        # profiles don't clobber each other.
        override = get_socis_agent_home_override()
        home = override if isinstance(override, str) and override else _socis_agent_home
...
def _load_cfg() -> dict:
    ...
    ``socis_cli.config.load_config_readonly`` — managed-scope overlay and
    ...
    try:
        from socis_cli.config import _expand_env_vars
...
def _apply_managed(cfg: dict) -> dict:
    ...
    socis_cli.config.load_config, so without this a managed skin / reasoning_effort
...
def _save_cfg(cfg: dict):
    ...
    override = get_socis_agent_home_override()
    home = Path(override) if isinstance(override, str) and override else _socis_agent_home
...
def _set_session_context(
    ...
    *,
    ...
    try:
        ...
        # can read SOCIS_AGENT_SESSION_ID. Without this, set_session_vars leaves the
        ...
        # SOCIS_AGENT_SESSION_ID even though agent_init set it via
...
def _enable_gateway_prompts() -> None:
    ...
    os.environ["SOCIS_AGENT_GATEWAY_SESSION"] = "1"
    os.environ["SOCIS_AGENT_EXEC_ASK"] = "1"
    os.environ["SOCIS_AGENT_INTERACTIVE"] = "1"
...
_TOUR_BRIDGE_UNAVAILABLE = json.dumps(
    {
        "success": False,
        "error": (
            "No SOCIS Agent Desktop window answered the tour request. The tour is "
            ...
            "nothing listening. Update the SOCIS Agent Desktop app and start a new "
...
def resolve_skin() -> dict:
    try:
        from socis_cli.skin_engine import init_skin_from_config, get_active_skin
...
def _broadcast_skin_if_changed() -> None:
    ...
    (``socis config set display.skin``) OR edited the active skin's colors in
...
def _watcher_home() -> Path:
    ...
    override = get_socis_agent_home_override()
    return Path(override if isinstance(override, str) and override else _socis_agent_home)
...
def _ensure_skin_watcher() -> None:
    ...
    SOCIS activates, a pet ``/pet`` adopts, a cron the scheduler fires, or a
    ...
    threading.Thread(target=_loop, name="socis-change-watcher", daemon=True).start()


def _resolve_model() -> str:
    env = (
        os.environ.get("SOCIS_AGENT_MODEL", "")
        or os.environ.get("SOCIS_AGENT_INFERENCE_MODEL", "")
    ).strip()
    if env:
        return env
    ...
    try:
        from socis_cli.models import get_preferred_silent_default_model
...
def _resolve_session_platform() -> str:
    ...
    Resolution:
      * ``SOCIS_AGENT_DESKTOP=1`` and ``SOCIS_AGENT_DESKTOP_TERMINAL`` unset → "desktop"
        (the chat-panel backend — a graphical React surface, not a terminal).
      * ``SOCIS_AGENT_DESKTOP_TERMINAL=1`` → "tui"
        (``socis --tui`` running in the desktop's embedded terminal pane;
        it IS a TUI, just embedded. The clarifier attached to the tui hint
        in system_prompt.py tells the agent about the embedding.)
      * neither set → "tui"
        (standalone ``socis --tui``.)
    """
    if is_truthy_value(os.environ.get("SOCIS_AGENT_DESKTOP")) and not is_truthy_value(
        os.environ.get("SOCIS_AGENT_DESKTOP_TERMINAL")
    ):
        return "desktop"
    return "tui"
...
def _config_model_target() -> tuple[str, str]:
    ...
    Unlike `_resolve_model()`, this never reads SOCIS_AGENT_MODEL /
    SOCIS_AGENT_INFERENCE_MODEL. Those env vars are a launch-scoped seed
    (`socis --tui -m <model>`, hosted-instance provisioning); if they
    ...
    # No fallback to _resolve_model() here: that reads SOCIS_AGENT_MODEL /
    # SOCIS_AGENT_INFERENCE_MODEL, which `socis --tui -m <model>` sets as a
...
def _resolve_startup_runtime() -> tuple[str, str | None]:
    ...
    explicit_provider = os.environ.get("SOCIS_AGENT_TUI_PROVIDER", "").strip()
    ...
    try:
        from socis_cli.models import detect_static_provider_for_model
        ...
        current_provider = (
            ...
            or os.environ.get("SOCIS_AGENT_INFERENCE_PROVIDER", "").strip().lower()
            or "auto"
        )
...
from socis_agent_state import _BARE_BILLING_PROVIDERS
...
def _overrides_have_routable_provider(overrides: dict) -> bool:
    ...
    if not provider:
        return False
    try:
        from socis_cli.runtime_provider import is_routable_provider
...
def _runtime_model_config(agent, existing: dict | None = None) -> dict:
    ...
    if provider:
        if provider.strip().lower() == "custom":
            ...
            try:
                from socis_cli.runtime_provider import (
...
def _persist_live_session_system_prompt(session: dict | None) -> None:
    ...
    # Re-bind SOCIS_AGENT_HOME to the session's profile so load_soul_md() and
    ...
    profile_home = session.get("profile_home")
    home_token = (
        set_socis_agent_home_override(profile_home) if profile_home else None
    )
...
def _load_approval_mode() -> str:
    ...
    canonical ``socis_cli.config.load_config`` path applies managed-scope
...
def _load_reasoning_config(model: str = "") -> dict | None:
    ...
    :func:`socis_agent_constants.resolve_reasoning_config` (per-model override >
    global ``agent.reasoning_effort``; YAML boolean False = disabled).
    Closes #21256.
    """
    from socis_agent_constants import resolve_reasoning_config
...
def _load_tool_progress_mode() -> str:
    env = os.environ.get("SOCIS_AGENT_TUI_TOOL_PROGRESS", "").strip().lower()
...
def _gui_surface_toolsets(platform: str) -> set[str]:
    ...
    Both entries are deliberately off ``_SOCIS_AGENT_CORE_TOOLS`` — every other
    ...
    paths run with ``SOCIS_AGENT_DESKTOP=1``. Keying GUI capability off that env var
    silently stripped every pane/browser tool from URL and cloud gateways while
    the same backend told the model it was "chatting inside the SOCIS desktop
...
def _load_enabled_toolsets(platform: str | None = None) -> list[str] | None:
    ...
    explicit = [
        item.strip()
        for item in os.environ.get("SOCIS_AGENT_TUI_TOOLSETS", "").split(",")
        if item.strip()
    ]
    cfg = None
    fallback_notice = None

    # Coding posture (base SOCIS): with no explicit pin, collapse to the
    # coding toolset (+ enabled MCP servers) when sitting in a code workspace.
    # The desktop app and `socis --tui` both land here. See
    ...
    if explicit and validate_toolset is not None:
        ...
        if any(name in {"all", "*"} for name in built_in):
            ...
            if ignored:
                print(
                    "[tui] SOCIS_AGENT_TUI_TOOLSETS=all enables every toolset; "
        ...
        try:
            ...
            from socis_cli.tools_config import _parse_enabled_flag
        ...
        if unknown:
            print(
                f"[tui] ignoring unknown SOCIS_AGENT_TUI_TOOLSETS entries: {', '.join(unknown)}",
                file=sys.stderr,
                flush=True,
            )
        if disabled:
            print(
                "[tui] ignoring disabled MCP servers in SOCIS_AGENT_TUI_TOOLSETS "
        ...
        fallback_notice = (
            "[tui] no valid SOCIS_AGENT_TUI_TOOLSETS entries; using configured CLI toolsets"
        )
    ...
    try:
        ...
        if not enabled:
            return None
        # The client-surface toolsets are off _SOCIS_AGENT_CORE_TOOLS (every other
        # platform would carry their schema for nothing), so the platform
        # recovery above — which keys off socis-cli's tool universe — can't
        ...
        if fallback_notice is not None:
            print(
                "[tui] no valid SOCIS_AGENT_TUI_TOOLSETS entries and configured CLI toolsets could not be loaded; enabling all toolsets",
                file=sys.stderr,
                flush=True,
            )
        return None
...
def _apply_model_switch(
    ...
    *,
    ...
    from socis_cli.model_switch import (
    ...
    from socis_cli.runtime_provider import resolve_runtime_provider
    ...
    cfg = None
    try:
        from socis_cli.config import get_compatible_custom_providers, load_config
    ...
    if agent:
        try:
            from socis_cli.context_switch_guard import merge_preflight_compression_warning
    ...
    #
    # We deliberately do NOT write process-global env vars (SOCIS_AGENT_MODEL /
    # SOCIS_AGENT_INFERENCE_MODEL / SOCIS_AGENT_TUI_PROVIDER / SOCIS_AGENT_INFERENCE_PROVIDER)
...
def _sync_agent_model_with_config(sid: str, session: dict) -> None:
    ...
    try:
        _apply_model_switch(
            ...
            # target the sync computed — the path that leaked `socis --tui -m`
...
def _compress_session_history(
    ...
    from socis_cli.partial_compress import (
...
def _get_usage(agent) -> dict:
    ...
    # SOCIS_AGENT_DEV_CREDITS so the payload stays clean when the flag is off.
    if is_truthy_value(os.environ.get("SOCIS_AGENT_DEV_CREDITS")):
        try:
...
def _probe_config_health(cfg: dict) -> str:
    ...
    if isinstance(display_cfg, dict):
        ...
        if personality and personality not in {"default", "none", "neutral"}:
            try:
                from socis_cli.personality import available_personalities
...
def _project_info_for_cwd(cwd: str) -> dict | None:
    ...
    if not str(cwd or "").strip():
        return None
    try:
        from socis_cli import projects_db as pdb
...
def _session_info(agent, session: dict | None = None) -> dict:
    ...
    if agent is not None and not (session or {}).get("_compute_host_active"):
        try:
            ...
            pass
        try:
            from socis_cli.banner import get_available_skills
    ...
    try:
        ...
        pass
    try:
        from socis_cli.banner import get_update_result
...
def _agent_cbs(sid: str) -> dict:
    callbacks = {
        ...
        # directly underneath the SOCIS window, and answers
...
def _wire_callbacks(sid: str):
    ...
    def secret_cb(env_var, prompt, metadata=None):
        ...
        from socis_cli.config import save_env_value_secure
...
def _render_personality_prompt(value) -> str:
    """Delegates to socis_cli.personality (single owner of rendering)."""
    from socis_cli.personality import render_personality_prompt

    return render_personality_prompt(value)


def _available_personalities(cfg: dict | None = None) -> dict:
    """Built-ins + user overrides, via socis_cli.personality (single owner)."""
    from socis_cli.personality import available_personalities
...
def _validate_personality(value: str, cfg: dict | None = None) -> tuple[str, str]:
    ...
    Same contract as socis_cli.personality.resolve_personality — (name,
    ...
    """
    from socis_cli.personality import normalize_personality_name
...
def _prompt_text(value) -> str:
    ...
    Delegates to socis_cli.personality (single owner).
    """
...
def _cfg_max_turns(cfg: dict, default: int) -> int:
    from socis_cli.config import resolve_turn_limit as _resolve_turn_limit
    # Env var override (highest priority)
    env_val = os.environ.get("SOCIS_AGENT_TUI_MAX_TURNS")
    if env_val:
...
def _parse_tui_skills_env() -> list[str]:
    raw = os.environ.get("SOCIS_AGENT_TUI_SKILLS", "")
    skills: list[str] = []
    seen: set[str] = set()
...
def _load_fallback_model():
    ...
    stays in parity with ``SOCISCLI.__init__`` and ``gateway/run.py``:
    ...
    """
    from socis_cli.fallback_config import get_fallback_chain
...
def _resolve_runtime_with_fallback(
    ...
    """
    from socis_cli.auth import AuthError
    from socis_cli.runtime_provider import resolve_runtime_provider
    ...
    try:
        ...
        for entry in fb_chain:
            ...
            if not fb_provider or not fb_model:
                continue
            try:
                from socis_cli.fallback_config import resolve_entry_api_key
...
def _make_agent(
    ...
    # block. Dashboard /api/ws uses socis_cli.mcp_startup; TUI stdio keeps
    # its existing tui_gateway.entry-owned thread.
    try:
        from socis_cli.mcp_startup import wait_for_mcp_discovery
    ...
    cfg = _load_cfg()
    from socis_cli.config import resolve_ephemeral_system_prompt_from_config
        ...
        if missing_skills:
            ...
            if loaded_skills:
                logger.warning(
                    ...
                    "Continuing with: %s. "
                    "List available skills with `socis skills list`.",
                    missing_display,
                    ", ".join(loaded_skills),
                )
            else:
    ...
    agent = AIAgent(
        ...
        checkpoints_enabled=is_truthy_value(os.environ.get("SOCIS_AGENT_TUI_CHECKPOINTS")),
        pass_session_id=is_truthy_value(os.environ.get("SOCIS_AGENT_TUI_PASS_SESSION_ID")),
        skip_context_files=is_truthy_value(os.environ.get("SOCIS_AGENT_IGNORE_RULES")),
        skip_memory=is_truthy_value(os.environ.get("SOCIS_AGENT_IGNORE_RULES")),
...
def _init_session(
    ...
    with _sessions_lock:
        _sessions[sid] = {
            ...
            "edit_snapshots": {},
            "tool_started_at": {},
            # Profile-scoped SOCIS_AGENT_HOME for app-global remote mode; None =
...
def _build_persist_message_with_image_refs(user_text: str, image_paths: list[str]) -> str:
    ...
    format the desktop client (directive-text.tsx / SOCIS_AGENT_DIRECTIVE_RE)
...
def _session_home(session: dict) -> Path:
    """The SOCIS_AGENT_HOME the session's durable state lives in (profile-aware)."""
    profile_home = session.get("profile_home")
    return Path(profile_home) if profile_home else Path(_socis_agent_home)
...
def _maybe_schedule_auto_continue(sid: str, session: dict, session_key: str) -> dict | None:
    ...
    def kickoff() -> None:
        ...
        # backends share one SOCIS_AGENT_HOME; B resumes S while A runs it and
...
def _finalize_superseded_runtimes(stale: list[tuple[str, dict]]) -> None:
    ...
    socis_agent_state_common._RECOVERABLE_END_REASONS so canonical Bot Chat
...
def _schedule_resume_hydration(
    ...
    def _run() -> None:
        ...
        try:
            ...
            from socis_agent_state import SessionResumeTooLargeError
...
def _pet_state_rows(spritesheet) -> list[str]:
    ...
    SOCIS has to support both the legacy 8-row petdex atlas and the current
...
def _pet_gen_root():
    ...
    root = get_socis_agent_home() / "cache" / "pet-gen"
...
try:
    _PET_REFERENCE_MAX_BYTES = max(
        1,
        int(os.environ.get("SOCIS_AGENT_PET_REFERENCE_MAX_BYTES") or str(16 * 1024 * 1024)),
    )
except (TypeError, ValueError):
...
#
# Layout:  $SOCIS_AGENT_HOME/spawn-trees/<session_id>/<timestamp>.json
...
def _spawn_trees_root():
    ...
    root = get_socis_agent_home() / "spawn-trees"
    root.mkdir(parents=True, exist_ok=True)
    return root
...
def _maybe_fire_tui_loop_tick(sid: str, session: dict) -> None:
    ...
    """
    try:
        from socis_cli.loops import LoopManager, goal_blocks_loop_tick
    except Exception:
        return
...
def _collect_kanban_notifications(session: dict) -> list:
    ...
    ``platform="tui"`` and ``chat_id=SOCIS_AGENT_SESSION_KEY`` (see
    ...
    if not session_key or session.get("_finalized"):
        return []
    try:
        from socis_cli import kanban_db as _kb
    except Exception:
        return []
    texts: list = []
    try:
    ...
    # DB when SOCIS_AGENT_KANBAN_DB pins the board path (same guard as the gateway
    # notifier).
...
def _wire_desktop_ui() -> None:
    ...
    Idempotent. The tool hands back the turn's ``SOCIS_AGENT_UI_SESSION_ID`` as
...
def _plan_goal_compression_recovery(
    ...
    *,
    ...
    from socis_cli.goals import GoalManager
...
def _run_prompt_submit(
    ...
    *,
    ...
    def run():
        ...
        approval_token = None
        session_tokens = []
        home_token = None  # per-turn SOCIS_AGENT_HOME override for a resumed remote profile
        secret_token = None
        ...
        try:
            ...
            if _profile_home_str:
                home_token = set_socis_agent_home_override(_profile_home_str)
            ...
            if images:
                try:
                    ...
                    from socis_cli.config import load_config as _tui_load_config
                ...
                try:
                    ...
                    def _thinking_should_play() -> bool:
                        if is_audio_output_active():
                            return False
                        try:
                            from socis_cli.voice import is_continuous_active
                ...
                try:
                    ...
                    if sid_key:
                        ...
                        if goal_mgr.is_active():
                            try:
                                from socis_cli.goals import gather_background_processes as _gather_bg
            ...
            if status == "complete":
                try:
                    from socis_cli.loops import LoopManager
            ...
            if (
                ...
                try:
                    ...
                    logger.warning("voice TTS skipped: socis_cli.voice unavailable")
                except Exception as e:
            ...
            # Run while any profile-specific SOCIS_AGENT_HOME override is still active
...
def _session_images_dir(session: dict) -> Path:
    ...
    run BEFORE ``prompt.submit`` installs the session's profile SOCIS_AGENT_HOME
    override, so ``get_socis_agent_home()`` here would return the gateway's launch
    ...
    """
    profile_home = session.get("profile_home")
    base = Path(profile_home) if profile_home else _socis_agent_home
    return base / "images"
...
def _desktop_attachment_dir(session: dict) -> Path:
    ...
    installs the session's profile SOCIS_AGENT_HOME override, while the docker/ssh
...
def _(rid, params: dict) -> dict:
    ...
    if key == "model":
        try:
            ...
            if session:
                from socis_cli.model_switch import parse_model_switch_args
    ...
    if key == "fast":
        ...
        overrides = None
        if nv == "fast":
            from socis_cli.models import resolve_fast_mode_overrides
    ...
    if key == "focus":
        ...
        from socis_cli.focus_view import (
    ...
    if key == "yolo":
        ...
        try:
            ...
            if session:
                ...
                current = is_truthy_value(os.environ.get("SOCIS_AGENT_YOLO_MODE"))
                enable = _resolve_toggle(current)
                if enable:
                    os.environ["SOCIS_AGENT_YOLO_MODE"] = "1"
                    nv = "1"
                else:
                    os.environ.pop("SOCIS_AGENT_YOLO_MODE", None)
                    nv = "0"
    ...
    if key == "reasoning":
        try:
            from socis_agent_constants import parse_reasoning_effort
    ...
    if key in {"prompt", "personality", "skin"}:
        try:
            ...
            if key == "prompt":
                ...
                # through socis_cli.personality (single owner) and never
                # touches the user-owned global system prompt.
                from socis_cli.personality import persist_personality
...
def _is_repo_junk(root: str) -> bool:
    ...
    :func:`_non_workspace_dirs`) or anything under SOCIS_AGENT_HOME (~/.socis-agent by
    ...
    socis_agent_home = os.path.realpath(str(get_socis_agent_home()))

    return (
        os.path.normcase(real) in _non_workspace_dirs()
        or real == socis_agent_home
        or real.startswith(socis_agent_home + os.sep)
    )
...
def _is_session_cwd_junk(cwd: str) -> bool:
    ...
    SOCIS_AGENT_HOME may be an intentional prose/data workspace. The pre-Projects
    desktop surfaced every such cwd, so exclude only the broad defaults that
    would create catch-all projects: SOCIS_AGENT_HOME itself and the dirs in
    ...
    socis_agent_home = os.path.normcase(os.path.realpath(str(get_socis_agent_home())))
    return real in _non_workspace_dirs() or real == socis_agent_home
...
def _scan_discovered_repos_remote(conn, policy: dict) -> bool:
    ...
    zero SOCIS sessions still appear in the sidebar (#81723). Mirrors the
...
def _discover_repos_payload(
    ...
    repos even with zero socis sessions. Session-derived roots cover repos
    outside the scan roots. Both are junk-filtered (socis home subtree + bare
    ...
    try:
        ...
        def _read(c) -> None:
            for entry in pdb.list_discovered_repos(c):
                ...
                # i.e. "just now" — so a git checkout with zero SOCIS sessions
...
def _rank_slash_completions(
    ...
    *,
    ...
    of skills someone invokes daily lead the ones that shipped with SOCIS
...
def _cli_exec_blocked(argv: list[str]) -> str | None:
    ...
    if not argv:
        return "bare `socis` is interactive — use `/socis chat -q …` or run `socis` in another terminal"
    a0 = argv[0].lower()
    if a0 == "setup":
        return "`socis setup` needs a full terminal — run it outside the TUI"
    if a0 == "gateway":
        return "`socis gateway` is long-running — run it in another terminal"
    if a0 == "sessions" and len(argv) > 1 and argv[1].lower() == "browse":
        return "`socis sessions browse` is interactive — use /resume here, or run browse in another terminal"
    if a0 == "config" and len(argv) > 1 and argv[1].lower() == "edit":
        return "`socis config edit` needs $EDITOR in a real terminal"
    return None


def _resolve_name(name: str) -> str:
    try:
        from socis_cli.commands import resolve_command
...
def _model_picker_context(agent):
    ...
    from socis_cli.inventory import load_picker_context
...
def _format_live_history_output(session: dict) -> str:
    ...
    for idx, message in enumerate(messages, start=1):
        ...
        label = "You" if role == "user" else "SOCIS" if role == "assistant" else role.title()
...
def _format_live_help_output() -> str:
    try:
        from socis_cli.commands import COMMANDS_BY_CATEGORY
...
def _voice_mode_enabled() -> bool:
    ...
    """
    return os.environ.get("SOCIS_AGENT_VOICE", "").strip() == "1"


def _voice_tts_enabled() -> bool:
    """Whether agent replies should be spoken back via TTS (runtime only)."""
    return os.environ.get("SOCIS_AGENT_VOICE_TTS", "").strip() == "1"
...
def _any_session_running() -> bool:
    ...
    Registered as the voice busy-probe (``socis_cli.voice.set_voice_busy_probe``)
        ...
        try:
            ...
            if text:
                ...
                if _is_stop:
                    ...
                    os.environ["SOCIS_AGENT_VOICE"] = "0"
                    os.environ["SOCIS_AGENT_VOICE_TTS"] = "0"
                    try:
                        from socis_cli.voice import stop_continuous
...
def _speak_text_with_barge(text: str) -> None:
    """Speak *text* via socis_cli.voice.speak_text with spoken barge-in.
    ...
    """
    from socis_cli.voice import speak_text
...
def _(rid, params: dict) -> dict:
    ...
    """
    from socis_cli.active_sessions import PER_SESSION_EXCLUSIVE_SUBMIT
...
def _(rid, params: dict) -> dict:
    ...
    * ``tts`` — toggle speech-output of agent replies. Requires mode on
    ...
    if action in {"on", "off"}:
        ...
        os.environ["SOCIS_AGENT_VOICE"] = "1" if enabled else "0"
    ...
    if action == "tts":
        ...
        os.environ["SOCIS_AGENT_VOICE_TTS"] = "1" if new_value else "0"
        if not new_value:
            _tts_stream_stop(user_barge=False)
...
def _(rid, params: dict) -> dict:
    ...
    try:
        if action == "start":
            ...
            from socis_cli.voice import start_continuous
            ...
            # are tolerated.
            try:
                from socis_cli.voice import set_voice_busy_probe
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        import socis_cli.voice  # noqa: F401
...
def _failure_messages(url: str, port: int, system: str) -> list[str]:
    from socis_cli.browser_connect import manual_chrome_debug_command
...
def _browser_connect(rid, params: dict) -> dict:
    ...
    from socis_cli.browser_connect import DEFAULT_BROWSER_CDP_URL
