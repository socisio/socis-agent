...
(local, remote URL, SSH, SOCIS Cloud, docker) must be able to find every
...
def _(rid, params: dict) -> dict:
    ...
    ``socis -p <profile> chat -c "Bot Chat"`` transport local DMs use and
    ...
    try:
        ...
        resolved = "default" if profile.lower() == "socis" else profile
        ...
        fd, tmp = tempfile.mkstemp(prefix="socis-relay-dm-", suffix=".txt", text=True)
        try:
