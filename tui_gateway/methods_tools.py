...
@method("reload.env")
def _(rid, params: dict) -> dict:
    """Re-read ``~/.socis-agent/.env`` into the gateway process via
    ``socis_cli.config.reload_env``, matching classic CLI's ``/reload``
    ...
    """
    try:
        from socis_cli.config import reload_env
...
@method("cli.exec")
def _(rid, params: dict) -> dict:
    """Run `python -m socis_cli.main` with argv; capture stdout/stderr (non-interactive only)."""
    ...
    try:
        ...
        r = subprocess.run(
            [sys.executable, "-m", "socis_cli.main", *argv],
            capture_output=True,
            text=True,
            ...
            cwd=os.getcwd(),
            # cli.exec runs `python -m socis_cli.main` (can drive the agent) →
            # needs provider credentials. Tier-1 secrets still stripped (#29157).
            env=socis_subprocess_env(inherit_credentials=True),
...
def _(rid, params: dict) -> dict:
    ...
    if name == "plan":
        ...
        # .socis-agent/plans/ via write_file. Works on any backend.
    ...
    if name == "moa":
        ...
        try:
            from socis_cli.moa_config import moa_usage, normalize_moa_config
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        # Re-bind SOCIS_AGENT_HOME to the session's profile so get_skill_commands()
        ...
        _home_token = (
            set_socis_agent_home_override(_profile_home) if _profile_home else None
        )
        try:
...
def _(rid, params: dict) -> dict:
    try:
        ...
        api_key = get_secret("SOCIS_AGENT_API_KEY", "") or cfg.get("api_key", "")
        masked = f"****{api_key[-4:]}" if len(api_key) > 4 else "(not set)"
        base_url = os.environ.get("SOCIS_AGENT_BASE_URL", "") or cfg.get("base_url", "")
        ...
        sections = [
            ...
            },
            {
                ...
                "rows": [
                    ...
                    ["Config File", str(_socis_agent_home / "config.yaml")],
                ],
            },
        ]
...
def _(rid, params: dict) -> dict:
    ...
    # Optional profile scoping: cronjob() keys off SOCIS_AGENT_HOME, so scoping the
    ...
    if profile:
        try:
            ...
            from socis_agent_constants import set_socis_agent_home_override
    ...
    try:
        ...
        if token is not None:
            try:
                from socis_agent_constants import reset_socis_agent_home_override
...
def _(rid, params: dict) -> dict:
    ...
    gateway. Shares its renderer with the ``socis journey`` CLI.
    """
    try:
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if action == "install":
            from socis_cli.skills_hub import do_install
        ...
        if action == "browse":
            from socis_cli.skills_hub import browse_skills
        ...
        if action == "inspect":
            from socis_cli.skills_hub import inspect_skill
...
def _(rid, params: dict) -> dict:
    ...
    keys], transport}]}`` — the same catalog `socis mcp` offers, so
    ...
    try:
        ...
        from socis_cli import mcp_catalog
...
# (socis_cli/web_routers/mcp.py) so a desktop plugin can manage MCP servers for
# ANY profile, not just the launch profile. Each accepts an optional ``profile``
# param that scopes SOCIS_AGENT_HOME via set_socis_agent_home_override (omitted/None = the
# launch profile) in a try/finally, exactly like ``skills.manage`` / ``mcp.catalog``.
# All persistence reuses socis_cli/mcp_config.py helpers — no logic is duplicated.
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from socis_cli.mcp_config import _get_mcp_servers
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from socis_cli.config import load_config, save_config, save_env_value
        from socis_cli.mcp_config import (
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from socis_cli.mcp_config import _remove_mcp_server
...
def _(rid, params: dict) -> dict:
    ...
    (``window.socisDesktop.openExternal``) and then polls
    ...
    the SAME interactive MCP OAuth machinery ``socis mcp login`` uses
    ...
    try:
        ...
        socis_agent_home = str(get_socis_agent_home().expanduser().resolve(strict=False))
        result = mcp_oauth_sessions.start_flow(
            socis_agent_home, name, cfg, client_redirect_uri=client_redirect_uri
        )
        return _ok(
            rid,
            {
...
def _(rid, params: dict) -> dict:
    ...
    primitives as ``socis plugins`` / the dashboard, so the three surfaces
    ...
    Actions:
      ...
      - ``install`` → git-clone into ``~/.socis-agent/plugins/`` (non-interactive).
    ...
    plugins live under each profile's SOCIS_AGENT_HOME, so a client can list or
                ...
                out.append(
                    {
                        ...
                        # skills/MCP format) vs a native SOCIS plugin.
