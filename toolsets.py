...
_SOCIS_AGENT_CORE_TOOLS = [
    ...
    # spawned as a kanban worker (SOCIS_AGENT_KANBAN_TASK env set) or the current
...
_SOCIS_AGENT_WEBHOOK_SAFE_TOOLS = [
    "web_search",
    "web_extract",
    "vision_analyze",
    "clarify",
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "or XAI_API_KEY). Off by default; enable in `socis tools` → "
            "X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (SOCIS_AGENT_KANBAN_TASK env "
    ...
    # Coding posture (base SOCIS — CLI/TUI/desktop/ACP). Auto-selected in a
    ...
    "coding": {
        ...
        # non-configurable-toolset recovery loop in socis_cli/tools_config.py).
        "posture": True,
    },
    
    # ==========================================================================
    # Full SOCIS toolsets (CLI + messaging platforms)
    #
    ...
    # the `socis send` CLI), not by the model deciding to send on its own.
    # ==========================================================================
    ...
    "socis-cli": {
        ...
        "tools": _SOCIS_AGENT_CORE_TOOLS,
        "includes": []
    },

    "socis-cron": {
        # Mirrors socis-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `socis tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as socis-cli; gated by `socis tools`",
        "tools": _SOCIS_AGENT_CORE_TOOLS,
        "includes": []
    },
    ...
    "socis-email": {
        "description": "Email bot toolset - interact with SOCIS via email (IMAP/SMTP)",
        "tools": _SOCIS_AGENT_CORE_TOOLS,
        "includes": []
    },
    ...
    "socis-sms": {
        "description": "SMS bot toolset - interact with SOCIS via SMS (Twilio)",
        "tools": _SOCIS_AGENT_CORE_TOOLS,
        "includes": []
    },
    ...
    "socis-webhook": {
        ...
        "tools": _SOCIS_AGENT_WEBHOOK_SAFE_TOOLS,
        "includes": []
    },
    ...
    "socis-gateway": {
        ...
        "tools": [],
        "includes": ["socis-telegram", "socis-discord", "socis-whatsapp", "socis-slack", "socis-signal", "socis-bluebubbles", "socis-homeassistant", "socis-email", "socis-sms", "socis-mattermost", "socis-matrix", "socis-dingtalk", "socis-feishu", "socis-wecom", "socis-wecom-callback", "socis-weixin", "socis-qqbot", "socis-webhook", "socis-yuanbao"]
    }
}
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """Return a ``socis-*`` bundle's platform-specific tools, excluding core.

    Platform bundles are defined as ``_SOCIS_AGENT_CORE_TOOLS + [platform extras]``.
    ...
    Bundle nesting is one level deep in practice (only ``socis-gateway``
    ...
    """
    core = set(_SOCIS_AGENT_CORE_TOOLS)
...
def resolve_toolset(name: str, visited: Set[str] = None, *, include_registry: bool = True) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (socis-<name>).
        # Gives them _SOCIS_AGENT_CORE_TOOLS plus any tools the plugin registered
        ...
        if include_registry and name.startswith("socis-"):
            platform_name = name[len("socis-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_SOCIS_AGENT_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
