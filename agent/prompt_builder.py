...
_SOCIS_AGENT_MD_NAMES = (".socis.md", "SOCIS.md")


def _find_socis_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.socis.md`` or ``SOCIS.md``.
    ...
    # could pick up a .socis.md planted in /tmp, /home, etc.
    search_dirs = [current, *current.parents] if stop_at else [current]

    for directory in search_dirs:
        for name in _SOCIS_AGENT_MD_NAMES:
...
SOCIS_AGENT_AGENT_HELP_GUIDANCE = (
    ...
    # precedence. Injected only when skill_view exists AND the socis-agent
    # skill is actually installed (see system_prompt.py slot resolution).
    "You run on SOCIS Agent (by SOCIS). When the user needs help with "
    "SOCIS itself — configuring, setting up, using, extending, or troubleshooting "
    "it — or when you need to understand your own features, tools, or capabilities, "
    "the documentation at https://agent.socis.io/docs is your "
    "authoritative reference and always holds the latest, most up-to-date "
    "information. The `socis-agent` skill has the actual commands and proven "
    "workflows — load it with skill_view(name='socis-agent') before configuring, "
    "modifying, or troubleshooting SOCIS so you don't guess or invent workarounds."
)
...
SOCIS_AGENT_AGENT_HELP_GUIDANCE_NO_SKILLS = (
    "You run on SOCIS Agent (by SOCIS). When the user needs help with "
    "SOCIS itself — configuring, setting up, using, extending, or troubleshooting "
    "it — or when you need to understand your own features, tools, or capabilities, "
    "the documentation at https://agent.socis.io/docs is the "
    "authoritative reference and always holds the latest, most up-to-date "
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.socis-agent/kanban.db`. Your task id is in "
    "`$SOCIS_AGENT_KANBAN_TASK`; your workspace is `$SOCIS_AGENT_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $SOCIS_AGENT_KANBAN_WORKSPACE` before "
    ...
    "\n"
    "- **Workspace.** `cd $SOCIS_AGENT_KANBAN_WORKSPACE` first. For a `worktree` kind "
    "with no `.git`, `git worktree add <path> "
    "${SOCIS_AGENT_KANBAN_BRANCH:-wt/$SOCIS_AGENT_KANBAN_TASK}` from the main repo, then "
    "cd there. For a project-linked task the workspace is a fresh "
    "`<repo>/.worktrees/<task-id>` and `$SOCIS_AGENT_KANBAN_BRANCH` a deterministic "
    ...
    "every assignee in a real profile (`socis profile list`, or ask the user), "
    "and express dependencies via `parents=[...]` on `kanban_create`, not prose.\n"
    "\n"
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `socis kanban <verb>` for board operations. Use "
...
#
# The socis-agent runtime already executes a batch of tool calls
...
# from Cline's TypeScript tool-surface guidance to socis-agent's Python
...
STEER_CHANNEL_NOTE = (
    ...
    "Mid-turn, the user can steer you: SOCIS appends their message to the "
...
def hud_surface_note(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    HUD mode is a strip of SOCIS floating over another application, so the
    user is rarely asking about SOCIS — they are asking about the thing behind
    ...
    sentences = [
        "[Note: this message came from HUD mode — a small floating SOCIS "
        ...
        "rather than anything inside SOCIS. read_window_below identifies "
        "that app.",
...
PLATFORM_HINTS = {
    ...
    "tui": (
        ...
        "You are in the SOCIS terminal UI (TUI). Files: there is no "
    ...
    "desktop": (
        ...
        "You are chatting inside the SOCIS desktop app, a graphical chat "
        ...
        "data-socis-send=\"prompt\" on any clickable element (or "
        "window.socis.send(\"prompt\")) sends that prompt as a hidden user "
...
# container / remote host rather than on the machine where SOCIS itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host SOCIS runs on.
    """
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where SOCIS itself is running. The host OS, home, and cwd "
                f"of the SOCIS process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where SOCIS "
    ...
    # Embedder-supplied environment description. Lets a host that wraps SOCIS
    ...
    extra = (os.getenv("SOCIS_AGENT_ENVIRONMENT_HINT") or "").strip()
    if not extra:
        try:
            from socis_cli.config import load_config_readonly
...
def _skills_prompt_snapshot_path() -> Path:
    return get_socis_agent_home() / ".skills_prompt_snapshot.json"
...
def _parse_skill_file(skill_file: Path) -> tuple[bool, dict, str]:
    ...
    try:
        ...
        # Fold triggers into the description string so the index-line code
        # below needs no change. Descriptions decide loading by lexical
        # overlap and miss the conceptual case; a declared trigger like
        # "is this a true positive" matches the question a description
        # phrased as "Triage a SOC alert" does not.
        _desc = extract_skill_description(frontmatter)
        _trig = extract_skill_triggers(frontmatter)
        if _trig:
            _desc = (_desc + "  ·  asks like: " + "; ".join(_trig)).strip()
        return True, frontmatter, _desc
    except Exception as e:
...
def _current_session_platform_hint() -> str:
    ...
    platform = os.environ.get("SOCIS_AGENT_PLATFORM") or os.environ.get("SOCIS_AGENT_SESSION_PLATFORM")
    if platform:
        return platform
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.socis-agent/skills/`` directory.  External dirs
    ...
    # SOCIS_AGENT_HOME would otherwise fall back to the launch (default) home and
    ...
    if skills_dir_override is not None:
        ...
        _home_token = set_socis_agent_home_override(str(skills_dir.parent))
    else:
        skills_dir = get_skills_dir()
        _home_token = None
    try:
        external_dirs = get_all_skills_dirs()[1:]  # skip local (index 0)
        # Trusted project-local dirs (./.socis-agent/skills, ./.agents/skills at
        ...
        if _home_token is not None:
            reset_socis_agent_home_override(_home_token)
...
def load_soul_md(
    ...
    """Load SOUL.md from SOCIS_AGENT_HOME and return its content, or None.
    ...
    ambient — which on a thread that lost the SOCIS_AGENT_HOME ContextVar falls
    back to the launch home and reads the wrong profile's SOUL.md (#50233,
    same class as the skills-index leak fixed in #86313).
    """
    try:
        from socis_cli.config import ensure_socis_agent_home
        ensure_socis_agent_home()
    except Exception as e:
        logger.debug("Could not ensure SOCIS_AGENT_HOME before loading SOUL.md: %s", e)

    _home = Path(home_override) if home_override is not None else get_socis_agent_home()
...
def _load_socis_md(cwd_path: Path, context_length: Optional[int] = None) -> str:
    """.socis.md / SOCIS.md — walk to git root."""
    socis_md_path = _find_socis_md(cwd_path)
    if not socis_md_path:
        return ""
    try:
        content = (_read_text_with_timeout(socis_md_path) or "").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = socis_md_path.name
        try:
            rel = str(socis_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(
            result, ".socis.md", context_length=context_length,
            read_path=str(socis_md_path),
        )
    except Exception as e:
        logger.debug("Could not read %s: %s", socis_md_path, e)
        return ""
...
def build_context_files_prompt(
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .socis.md / SOCIS.md  (walk to git root)
      2. AGENTS.md / agents.md   (merged chain: git root → cwd)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from SOCIS_AGENT_HOME is independent and always included when present.
    ...
    # Never let a FALLBACK-picked directory inside the SOCIS install/source
    ...
    # explicitly configured cwd is honored verbatim — the SOCIS tree is a
    ...
    # their launch dir IS the user's shell cwd (developing SOCIS in-tree).
    ...
    if (
        ...
        logger.warning(
            ...
            "fell back to the SOCIS install tree (%s) — set terminal.cwd to "
        ...
        project_context = (
            _load_socis_md(cwd_path, context_length)
    ...
    # SOUL.md from SOCIS_AGENT_HOME only — skip when already loaded as identity
    if not skip_soul:
