...
def build_turn_context(
    ...
    *,
    ...
    # Tag log records on this thread with the session ID for ``socis logs``.
    ...
    if (
        ...
        # SOCIS only initiates compaction in "socis" mode (#36801).
        _codex_native_auto = (
        ...
        if _preflight_deferred:
            ...
            logger.info(
                "Skipping SOCIS preflight compression for codex app-server "
                "(mode=%s); SOCIS will not start thread compaction here.",
    ...
    if agent._memory_manager:
        ...
        # to surface it. Rendered by SOCIS (via _emit_status), so it always
