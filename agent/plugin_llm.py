...
Plugins built on SOCIS Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing SOCIS
...
:class:`~socis_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors SOCIS' main config
...
handles every provider, fallback chain, and per-task override SOCIS
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`socis_cli.plugins.PluginContext`
