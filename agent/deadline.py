...
(https://github.com/socisio/socis-agent/issues/85125).
...
* :func:`resolve_timeout` — one config-first resolution path for timeout
  ...
  so new surfaces stop inventing ``SOCIS_AGENT_*_TIMEOUT`` env vars (".env is for
...
* ``None`` timeout means unbounded, and non-positive resolved values are
  ...
  ``SOCIS_AGENT_CONCURRENT_TOOL_TIMEOUT_S`` convention).
"""
...
class DeadlineExpired(TimeoutError):
    ...
    was SOCIS's own bound — error classification must not attribute it to
...
def clamp_timeout(timeout: Optional[float]) -> Optional[float]:
    ...
    * Non-positive values become ``None`` (unbounded) — matching the existing
      ``SOCIS_AGENT_CONCURRENT_TOOL_TIMEOUT_S`` "0 disables the bound" convention.
