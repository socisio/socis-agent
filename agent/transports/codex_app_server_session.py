...
Owns one Codex thread per SOCIS session. Drives `turn/start`, consumes
...
# SOCIS' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_SOCIS_AGENT_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse SOCIS/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per SOCIS session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _SOCIS_AGENT_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("SOCIS_AGENT_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="socis",
            client_title="SOCIS Agent",
            client_version=_get_socis_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but SOCIS' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into SOCIS' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own socis-tools callback we
            # auto-accept — the user already approved SOCIS' tools
    ...
    def _decide_exec_approval(self, params: dict) -> str:
        ...
        This is protocol-level routing only — it carries NO SOCIS
        approval-mode/timeout logic. The SOCIS-side resolution happens
    ...
    def _decide_apply_patch_approval(self, params: dict) -> str:
        ...
        Protocol-level routing only; SOCIS approval-mode/timeout
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map SOCIS approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    SOCIS returns 'once', 'session', 'always', or 'deny'.
    ...
    NOT in tools/approval.py: the SOCIS approval mode/timeout resolution
    ...
    # "prompt expired"; the SOCIS-side messaging already distinguishes them.
    return "decline"
...
def _get_socis_version() -> str:
    """Best-effort SOCIS version string for codex's userAgent line."""
    try:
