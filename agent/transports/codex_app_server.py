...
projection into SOCIS' display, approval bridging, transcript projection into
...
"codex_app_server"`. SOCIS' default tool dispatch is unchanged when this
...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        # `os.environ.copy()` also handed it every Tier-1 SOCIS secret — gateway
        ...
        spawn_env = socis_subprocess_env(inherit_credentials=True)
        if env:
        ...
        if spawn_env.get("SOCIS_AGENT_KANBAN_TASK"):
            kanban_db = spawn_env.get("SOCIS_AGENT_KANBAN_DB")
            kanban_root = (
                os.path.dirname(kanban_db)
                if kanban_db
                else spawn_env.get(
                    "SOCIS_AGENT_KANBAN_ROOT",
                    os.path.join(
                        spawn_env.get("SOCIS_AGENT_HOME", os.path.expanduser("~/.socis-agent")),
                        "kanban",
                    ),
                )
            )
    ...
    def initialize(
        self,
        client_name: str = "socis",
        client_title: str = "SOCIS Agent",
