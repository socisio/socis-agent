"""OpenAI-compatible shim that forwards SOCIS requests to `copilot --acp`.

This adapter lets SOCIS treat the GitHub Copilot ACP server as a chat-style
backend. Each request starts a short-lived ACP session, sends the formatted
conversation as a single prompt, collects text chunks, and converts the result
back into the minimal shape SOCIS expects from an OpenAI client.
"""
...
def _resolve_args() -> list[str]:
    raw = os.getenv("SOCIS_AGENT_COPILOT_ACP_ARGS", "").strip()
    if not raw:
        return ["--acp", "--stdio"]
...
def _acp_supported(command: str, args: list[str]) -> bool | None:
    ...
    SOCIS_AGENT_COPILOT_ACP_ARGS transport is the operator's business.
    """
    if "--acp" not in args:
        return True
...
def _resolve_home_dir() -> str:
    ...
    # subprocess with no HOME; callers can set SOCIS_AGENT_HOME explicitly if they
...
def _format_messages_as_prompt(
    ...
    sections: list[str] = [
        "You are being used as the active ACP agent backend for SOCIS.",
    ...
    # Copilot has no tools of its own that would collide with SOCIS', so it
...
class CopilotACPClient:
    ...
    SOCIS_AGENT_SKIP_TRANSPORT_WRAP = True
    SOCIS_AGENT_SKIP_ASYNC_WRAP = True
        ...
        *,
        ...
        if _acp_supported(self._acp_command, self._acp_args) is False:
            ...
            raise RuntimeError(
                ...
                f"SOCIS_AGENT_COPILOT_ACP_COMMAND / SOCIS_AGENT_COPILOT_ACP_ARGS "
                f"to a working pair."
            )

        # Note the model SOCIS selected; it is applied after session/new via
        ...
        try:
            ...
            raise RuntimeError(
                ...
                "Install GitHub Copilot CLI or set SOCIS_AGENT_COPILOT_ACP_COMMAND/COPILOT_CLI_PATH."
            ) from exc
        ...
        def _request(method: str, params: dict[str, Any], *, text_parts: list[str] | None = None, reasoning_parts: list[str] | None = None) -> Any:
            ...
            if proc.poll() is not None and stderr_text:
                if _is_gh_copilot_deprecation_message(stderr_text):
                    raise RuntimeError(
                        "SOCIS ACP mode requires the NEW GitHub Copilot CLI "
                        ...
                        "point SOCIS at it explicitly:\n"
                        "  export SOCIS_AGENT_COPILOT_ACP_COMMAND=/path/to/new/copilot\n\n"
                        "Alternative: use the `copilot` provider (no ACP, hits the Copilot API\n"
                        "directly with a Copilot subscription token) via `socis setup`.\n\n"
                        f"Original error:\n{stderr_text}"
                    )
        ...
        try:
            ...
            # Select the model SOCIS asked for. Prefer the stable ACP v1
        ...
        *,
        ...
        if method == "session/request_permission":
            ...
            response = _jsonrpc_error(
                message_id,
                -32601,
                f"ACP client method '{method}' is not supported by SOCIS yet.",
            )
