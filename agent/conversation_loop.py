...
# Must mirror _STALE_TOOL_CALL_MARKER_RE in socis_agent_state.py — kept local
# to avoid importing socis_agent_state at module load time (its module-level
# DEFAULT_DB_PATH = get_socis_agent_home() / "state.db" breaks tests that
# monkeypatch get_socis_agent_home to return a str).
...
from socis_agent_constants import PARTIAL_STREAM_STUB_ID
...
def _apply_active_turn_redirect(agent: Any, messages: List[Dict[str, Any]], text: str) -> None:
    ...
    mechanism as the ~/.socis-agent/prefill.json incident, 20/20 blocked with
...
def _ollama_context_limit_error(agent: Any, request_tokens: int) -> Optional[str]:
    ...
    logger.warning(
        "Ollama runtime context too small for SOCIS tool use: "
    ...
    return (
        ...
        f"context, but SOCIS needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens "
        "for reliable tool use.\n\n"
        "Increase the Ollama context for this model and restart/reload the "
        "model before trying again. A known-good starting point is 65,536 "
        "tokens. In SOCIS config, set `model.ollama_num_ctx: 65536` "
...
def _maybe_grow_local_window(agent: Any, compressor: Any,
    ...
    if "127.0.0.1" not in base_url and "localhost" not in base_url:
        return None
    try:
        from socis_cli.local_runtime.growth import maybe_grow_window
    ...
    if (provider or "").strip().lower() == "anthropic":
        ...
        if unverified:
            lines = [
                ...
                "Retry with a fresh credential state: `socis auth reset anthropic`. Until "
...
def _stored_prompt_matches_runtime(agent, prompt: str) -> bool:
    ...
    def host_info_value(label: str) -> str:
        ...
        occurrence, so only SOCIS' own emitted block can satisfy the read.
        """
        prefix = f"{label}:"
...
_CONTENT_POLICY_RECOVERY_HINT = (
    ...
    "adding a fallback provider with `socis fallback add`."
)
...
def run_conversation(
    ...
    if moa_config is None:
        try:
            from socis_cli.moa_config import decode_moa_turn
    ...
    # Adopt any ~/.socis-agent/.env credential/base-url edits made since the last
    ...
    # 'disk', or 'unknown' — see socis_agent_state.classify_persistence_error).
    ...
    # all run inside Codex). Default SOCIS path is bypassed entirely.
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        # cache prefix.  The system prompt is reserved for SOCIS internals.
        #
        # SOCIS invariant: the system prompt is built ONCE per session
        ...
        if _runtime_context_error:
            ...
            agent._emit_status("❌ Ollama runtime context is too small for SOCIS tool use")
            api_call_count -= 1
            agent._api_call_count = api_call_count
            try:
        ...
        while retry_count < max_retries:
            ...
            try:
                ...
                try:
                    from socis_cli.middleware import apply_llm_request_middleware
                ...
                try:
                    from socis_cli.lifecycle import (
                        has_hook,
                ...
                if env_var_enabled("SOCIS_AGENT_DUMP_REQUESTS"):
                ...
                from socis_cli.middleware import run_llm_execution_middleware
                ...
                if finish_reason == "content_filter":
                    ...
                    _refusal_response = (
                        ...
                        "(safety refusal — not a SOCIS/gateway failure).\n\n"
                        f"{_refusal_detail}\n\n"
                ...
                if (
                    ...
                    print(f"{agent.log_prefix}     • Re-authenticate: socis auth add nous")
                ...
                if (
                    ...
                    if is_token_provider(key):
                        ...
                        print(f"{agent.log_prefix}   Run `socis doctor` for credential-chain diagnostics, or")
                    ...
                    from socis_agent_constants import display_socis_agent_home as _dhh_fn
                    _dhh = _dhh_fn()
                    print(f"{agent.log_prefix}     • Check ANTHROPIC_TOKEN in {_dhh}/.env for SOCIS-managed OAuth/setup tokens")
                    ...
                    print(f"{agent.log_prefix}     • Legacy cleanup: socis config set ANTHROPIC_TOKEN \"\"")
                    print(f"{agent.log_prefix}     • Clear stale keys: socis config set ANTHROPIC_API_KEY \"\"")
                ...
                if (
                    ...
                    try:
                        from socis_cli.models import refresh_reasoning_caps_async
                ...
                # and omits the field, and SOCIS' local compression takes
                ...
                if getattr(api_error, "status_code", None) == 404:
                    try:
                        from socis_cli.model_normalize import suggest_prefixed_model_id
                    ...
                    if _suggestion:
                        ...
                        agent._buffer_vprint(
                            f"      Did you mean '{_suggestion}'?  Re-pick it with `socis model`."
                        )
                ...
                # providers (DeepSeek, Kimi, MiMo, SOCIS).  A 429 can
                ...
                # which SOCIS' system prompt + tool schemas alone exceed.
                ...
                if (
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      request at ~8K tokens. SOCIS' system prompt + tool schemas baseline",
                        force=True,
                    )
                    agent._vprint(
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      Use the `copilot` provider with a Copilot subscription token (`socis",
                        force=True,
                    )
                    agent._vprint(
                ...
                if is_context_length_error:
                    ...
                    if available_out is not None:
                        ...
                        # SOCIS may add API-only content not present in persisted
                    ...
                    # recovery arms on the TRUE request that overflowed. See socis-lcm engine
                ...
                if is_client_error:
                    ...
                    if classified.is_auth or classified.reason == FailoverReason.billing:
                        if classified.reason == FailoverReason.billing and _print_billing_or_entitlement_guidance(
                            ...
                            if _provider == "openai-codex":
                                ...
                                agent._vprint(f"{agent.log_prefix}      2. Then run `socis auth` to re-authenticate.", force=True)
                            elif _provider == "xai-oauth":
                                agent._vprint(f"{agent.log_prefix}   💡 xAI OAuth token was rejected (HTTP 401). To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      re-authenticate with xAI Grok OAuth (SuperGrok / Premium+) from `socis model`.", force=True)
                            else:  # nous
                                agent._vprint(f"{agent.log_prefix}   💡 Nous Portal OAuth token was rejected (HTTP 401). Your token may be", force=True)
                                agent._vprint(f"{agent.log_prefix}      expired, revoked, or your account may be out of credits. To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      1. Re-authenticate: socis portal", force=True)
                            ...
                            agent._vprint(f"{agent.log_prefix}      • Is the key valid? Run: socis setup", force=True)
                    ...
                    if classified.reason == FailoverReason.content_policy_blocked:
                        ...
                        agent._vprint(
                            f"{agent.log_prefix}        socis fallback add   (interactive picker — same as `socis model`)",
                            force=True,
                        )
                ...
                if retry_count >= max_retries:
                    ...
                    if _is_thinking_timeout:
                        ...
                        agent._vprint(
                            ...
                            f"in `~/.socis-agent/config.yaml` to extend the per-call "
                            f"timeout. (SOCIS's built-in floor is 600s for "
        ...
        try:
            ...
            # with the iterations SOCIS never saw. Appended before this turn's
            ...
            if assistant_message.tool_calls:
                ...
                try:
                    ...
                    # terminates SOCIS mid-turn, resume logic still sees the
                    ...
                    from socis_agent_state import classify_persistence_error
                ...
                # timeout (SOCIS_AGENT_AGENT_TIMEOUT, default 1800s) and the
                ...
                try:
                    ...
                    from socis_cli.plugins import get_pre_verify_continue_message
                ...
                if _kanban_nudge:
                    ...
                    logger.info(
                        ...
                        agent._kanban_stop_nudges,
                        os.environ.get("SOCIS_AGENT_KANBAN_TASK", ""),
                    )
                    agent._emit_status(
            ...
            if sys.is_finalizing() or _is_interpreter_shutdown_error(e):
                ...
                final_response = (
                    ...
                    "resumed with: socis --resume <session-id>"
                )
