...
from socis_agent_constants import get_config_path, get_skills_dir, is_termux
...
# Org mirrors live under ~/.socis-agent/skills/_org/<org_id>/. Resolution is
...
def _detect_environment(env: str) -> bool:
    ...
    SOCIS_AGENT_KANBAN_* vars without owning them), so caching it process-wide would
    ...
    if env == "kanban":
        ...
        # dispatcher sets ``SOCIS_AGENT_KANBAN_TASK`` / ``SOCIS_AGENT_KANBAN_BOARD`` in the
        ...
        # tool availability.
        if os.getenv("SOCIS_AGENT_KANBAN_TASK") or os.getenv("SOCIS_AGENT_KANBAN_BOARD"):
        ...
        # The SOCIS Docker image runs s6-overlay as PID 1 (/init). s6 plants
...
def _load_raw_config() -> Dict[str, Any]:
    ...
    This module intentionally avoids importing ``socis_cli.config`` on the
...
# `socis-agent` skill is the agent's own operating manual — it drives
# configuring, extending, and troubleshooting SOCIS itself, and the system
# prompt unconditionally points at it. Disabling it leaves the agent unable
# to help with SOCIS, so disable requests for these names are ignored
# everywhere the disabled list is consulted.
ESSENTIAL_SKILLS: frozenset = frozenset({"socis-agent"})
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``SOCIS_AGENT_PLATFORM`` or
            ``SOCIS_AGENT_SESSION_PLATFORM`` env vars.  Returns the global
    ...
    resolved_platform = (
        platform
        or os.getenv("SOCIS_AGENT_PLATFORM")
        or get_session_env("SOCIS_AGENT_SESSION_PLATFORM")
    )
...
def parse_config_string_list(value) -> List[str]:
    ...
    ``socis config set`` and JSON-mode editor saves store lists as quoted
...
# which becomes the dominant cost of ``socis`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.socis-agent/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``socis`` cold-start time
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against SOCIS_AGENT_HOME, not cwd
        if not p.is_absolute():
            p = (socis_agent_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_skill_create_dir() -> Optional[Path]:
    ...
    this directory instead of the profile-local ``~/.socis-agent/skills/``, and
    ...
    against SOCIS_AGENT_HOME.  A value that resolves to the local skills dir is
...
def display_skill_create_dir() -> str:
    ...
    if create_dir is None:
        return f"{display_socis_agent_home()}/skills/"
    try:
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.socis-agent/skills/`` first, then external.
    ...
    NOTE: trusted project-local dirs (``./.socis-agent/skills`` at the git root) are
...
#   <root>/.socis-agent/skills/   — SOCIS-native location
...
# a one-line "run `socis skills trust`" notice.
#
...
PROJECT_SKILLS_SUBDIRS = (
    os.path.join(".socis-agent", "skills"),
...
def _candidate_project_skills_dirs(root: Path) -> List[Path]:
    ...
    The exclusion matters when SOCIS_AGENT_HOME itself lives inside a git checkout:
    ``<root>/.socis-agent/skills`` would otherwise double as both the profile-local
...
def get_untrusted_project_skills_root() -> Optional[Tuple[Path, int]]:
    ...
    ``socis skills trust``. Returns None when there is nothing to notify
...
#
# Trust (`socis skills trust`) is a REPO-level decision made once; the repo's
...
#
# The scan cache lives under SOCIS_AGENT_HOME, never inside the repo (we don't
...
def _project_scan_cache_dir() -> Path:
    ...
    return get_socis_agent_home() / "cache" / "project_skill_scans"
...
def normalize_skill_lookup_name(identifier: str) -> str:
    ...
    under ``~/.socis-agent/skills/`` (including via symlinks) or configured
    ...
    # follows the live profile-scoped SOCIS_AGENT_HOME (the import-time
    ...
    # ~/.socis-agent/skills/<name> where <name> is a symlink to a checked-out
...
def is_external_skill_path(path) -> bool:
    ...
    ``skills.external_dirs`` are externally owned: SOCIS can discover and view
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    socis = metadata.get("socis") or {}
    if not isinstance(socis, dict):
        socis = {}
    return {
        "fallback_for_toolsets": socis.get("fallback_for_toolsets", []),
        "requires_toolsets": socis.get("requires_toolsets", []),
        "fallback_for_tools": socis.get("fallback_for_tools", []),
        "requires_tools": socis.get("requires_tools", []),
        ...
        "session_platforms": socis.get("session_platforms", []),
    }
...
def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    socis = metadata.get("socis")
    if not isinstance(socis, dict):
        return []
    raw = socis.get("config")
    if not raw:
        return []
...
def iter_skill_index_files(skills_dir: Path, filename: str):
    ...
    Excludes SOCIS metadata, VCS, virtualenv/dependency, cache, and skill
