...
- ``set_secret_scope(mapping)`` installs the active profile's secrets for the
  ...
  via ``copy_context()`` exactly like the SOCIS_AGENT_HOME override).
...
_GLOBAL_ENV_EXACT = frozenset({
    # SOCIS runtime / deployment
    "SOCIS_AGENT_HOME", "SOCIS_AGENT_PROFILE", "SOCIS_AGENT_GATEWAY_LOCK_DIR",
    "SOCIS_AGENT_MAX_ITERATIONS", "SOCIS_AGENT_MAX_TOKENS", "SOCIS_AGENT_API_TIMEOUT",
    "SOCIS_AGENT_REDACT_SECRETS", "SOCIS_AGENT_NOUS_TIMEOUT_SECONDS",
    "_SOCIS_AGENT_GATEWAY",
    # OS / interpreter
    "PATH", "HOME", "USER", "LANG", "LC_ALL", "TZ", "PWD", "SHELL", "TMPDIR",
    "VIRTUAL_ENV", "PYTHONPATH", "SSL_CERT_FILE",
    # Kanban paths (per-board, not per-profile-secret)
    "SOCIS_AGENT_KANBAN_DB", "SOCIS_AGENT_KANBAN_WORKSPACES_ROOT", "SOCIS_AGENT_KANBAN_BOARD",
...
_GLOBAL_ENV_PREFIXES = (
    "SOCIS_AGENT_KANBAN_",
    "SOCIS_AGENT_TELEGRAM_",   # tuning knobs (batch delays, fallback toggles) — NOT the token
...
def load_env_file(env_path: Path) -> Dict[str, str]:
    ...
    ``set_secret_scope``. Parses the small KEY=VALUE subset SOCIS writes
    ...
    ``socis_cli.config._parse_env_value``) but never mutates the process
    ...
    # Parse values with the canonical SOCIS parser: save_env_value
    ...
    from socis_cli.config import _parse_env_value
...
def build_profile_secret_scope(socis_agent_home: Path) -> Dict[str, str]:
    ...
    try:
        from socis_cli.env_loader import get_secret_source_values
