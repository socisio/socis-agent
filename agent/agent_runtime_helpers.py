...
def try_recover_primary_transport(
    ...
    if (
        provider_lower in {"nous", "nous-portal", "socis"}
...
def dump_api_request_debug(
    ...
    *,
    ...
    try:
        ...
        # originate from untrusted input (X-SOCIS-Session-Id header), and an
        ...
        if env_var_enabled("SOCIS_AGENT_DUMP_REQUEST_STDOUT"):
...
def anthropic_prompt_cache_policy(
    ...
    *,
    ...
    if eff_provider.strip().lower() == "moa":
        try:
            from socis_cli.config import load_config as _load_moa_cfg
    ...
    is_nous_portal = base_url_host_matches(eff_base_url, "socis.io")
    ...
    # that are canonicalized only after SOCIS sends the request. Honor its
    ...
    if not _supports_anthropic_cache_markers:
        ...
        from socis_cli.providers import custom_provider_aliases
    ...
    if _supports_anthropic_cache_markers and (
        ...
        try:
            from socis_cli.config import get_custom_provider_model_capability
...
def create_openai_client(agent, client_kwargs: dict, *, reason: str, shared: bool) -> Any:
    ...
    # built-in ladder so a profile registered from ~/.socis-agent/plugins/ or a pip
    ...
    # Delegate all rate-limit / 5xx retry to socis's outer conversation loop,
    ...
    if agent.provider == "opencode-free":
        from socis_cli.models import opencode_zen_free_headers
    ...
    # All primary construction and recovery paths must identify SOCIS to the
...
def _apply_switched_provider_request_overrides(agent, new_provider):
    ...
    if custom_providers is None:
        try:
            from socis_cli.config import load_config, get_compatible_custom_providers
...
def switch_model(
    ...
    try:
        ...
        from socis_cli.config import load_config as _sm_load_config
...
def invoke_tool(agent, function_name: str, function_args: dict, effective_task_id: str,
    ...
    try:
        from socis_cli.middleware import apply_tool_request_middleware
    ...
    if not pre_tool_block_checked:
        try:
            from socis_cli.plugins import _dispatch_pre_tool_call_hooks
    ...
    from socis_cli.middleware import run_tool_execution_middleware
...
def _session_id_for_heal_log() -> str:
    try:
        from socis_agent_logging import _session_context
...
def get_sanitizer_heal_stats() -> Dict[str, Dict[str, Any]]:
    ...
    Surfaced by diagnostics (``socis doctor`` / debug share callers) so
...
def _log_empty_non_final_heal(healed: int) -> None:
    ...
    / ``socis doctor`` — once per session, never re-armed by a new window.
    """
    key = _session_id_for_heal_log() or "-"
        ...
        if threshold > 0 and count >= threshold and not state["escalated"]:
            ...
            if key not in _empty_heal_user_notified:
                ...
                _empty_heal_pending_notice[key] = (
                    ...
                    "session's history — run /debug share or `socis "
...
def sanitize_api_messages(messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    #
    # We do NOT drop the call: socis' own dispatch loop intentionally keeps an
    ...
    # provider-agnostic: Gemini reaches SOCIS under many model strings and
...
def _iter_httpx_pools_with_owner(http_client: Any):
    ...
    SOCIS' keepalive client (#10324 / ``_build_keepalive_http_client``) and
...
def _iter_pool_sockets(client: Any):
    ...
    from agent.process_bootstrap import SOCIS_AGENT_TRANSPORT_OWNER_EXT
    ...
    for pool, owner in pools:
        ...
        for pool_req in list(getattr(pool, "_requests", None) or []):
            if owner is not None:
                ...
                if exts.get(SOCIS_AGENT_TRANSPORT_OWNER_EXT) != owner:
                    continue
