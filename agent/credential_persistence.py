...
``auth.json``.  They intentionally have no dependency on ``socis_cli.auth`` so
...
# Sources SOCIS owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and SOCIS-owned OAuth/device-code state)
