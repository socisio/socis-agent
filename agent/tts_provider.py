...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into SOCIS with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.socis-agent/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of SOCIS
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``socis tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``socis setup`` for
