...
See the ``socis-agent-dev`` skill (``references/self-improvement-loop.md``)
...
def _parent_can_emit_tool_calls(agent: Any) -> bool:
    ...
    calls. A provider that IS an autonomous agent reaches SOCIS through a client
    shim, and a shim that cannot carry SOCIS tool calls back turns the fork into
    ...
    exists so a shim that can't declares ``SUPPORTS_SOCIS_AGENT_TOOL_CALLS = False``
    ...
    for candidate in (client, type(client) if client is not None else None):
        if candidate is None:
            continue
        supported = getattr(candidate, "SUPPORTS_SOCIS_AGENT_TOOL_CALLS", None)
...
_SKILL_REVIEW_PROMPT = (
    ...
    "  • Bundled skills (shipped with SOCIS, e.g. 'socis-agent').\n"
    "  • Hub-installed skills (installed via 'socis skills install').\n"
    "  • Skills in skills.external_dirs (externally owned).\n"
    "  • PINNED skills (marked via 'socis curator pin'). You are an "
    ...
    "'socis curator adopt <name>' — do not try to patch it.\n"
...
_COMBINED_REVIEW_PROMPT = (
    ...
    "  • Hub-installed skills (installed via 'socis skills install').\n"
    "  • Skills in skills.external_dirs (externally owned).\n"
    "  • PINNED skills (marked via 'socis curator pin'). Pin blocks "
    ...
    "reply and recommend 'socis curator adopt <name>' instead.\n"
    "If the only skills that need updating are protected, say\n"
    "'Nothing to save.' and stop.\n\n"
...
def build_memory_write_metadata(
    ...
    *,
    ...
    metadata: Dict[str, Any] = {
        ...
        "platform": agent.platform or os.environ.get("SOCIS_AGENT_SESSION_SOURCE", "cli"),
        "tool_name": "memory",
    }
    if task_id:
...
def build_cache_parity_fork(
    ...
    *,
    ...
    # scratch (fresh _socis_now() timestamp, fresh
...
def _run_review_in_thread(
    ...
    # An agent-as-provider whose client can't carry SOCIS tool calls back would
    ...
    if not _parent_can_emit_tool_calls(agent) and not bool(
        ...
        logger.warning(
            "Background review skipped: provider %r cannot emit SOCIS tool calls, "
