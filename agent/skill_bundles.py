...
Storage
-------
Bundles live in ``~/.socis-agent/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``SOCIS_AGENT_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``socis bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under SOCIS_AGENT_HOME.

    Honors ``SOCIS_AGENT_BUNDLES_DIR`` for tests; falls back to
    ``<SOCIS_AGENT_HOME>/skill-bundles``.
    """
    override = os.environ.get("SOCIS_AGENT_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_socis_agent_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `socis bundles` CLI subcommand.
# ---------------------------------------------------------------------------
