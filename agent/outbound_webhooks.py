...
(``gateway/platforms/webhook.py``): inbound wakes SOCIS when the world
changes; outbound tells the world when SOCIS does something.
...
* Payloads are signed with HMAC-SHA256 (GitHub-style
  ``X-SOCIS-Signature-256: sha256=<hexdigest>`` over the raw body) when
...
* No consent prompt: unlike shell hooks, an outbound target executes no
  ...
  in config.  ``SOCIS_AGENT_SAFE_MODE=1`` still skips registration, matching
  plugins / MCP / shell hooks.
* Registration is idempotent — safe to invoke from both the CLI entry
  point and the gateway entry point.

Config schema (``~/.socis-agent/config.yaml``)::
    ...
    hooks:
      outbound:
        - url: https://ci.example.com/hermes-events
          ...
          name: ci-notify   # optional label for logs / `socis hooks list`
...
Headers::
    ...
    User-Agent:              SOCIS-Agent-Outbound-Webhook
    X-SOCIS-Event:          <hook event name>
    X-SOCIS-Delivery:       <delivery_id>
    X-SOCIS-Signature-256:  sha256=<hmac hexdigest>   # only when secret set
"""
...
# socis_cli.plugins.get_plugin_manager) — can register identical webhook
...
def register_from_config(cfg: Optional[Dict[str, Any]]) -> List[WebhookTarget]:
    ...
    if env_var_enabled("SOCIS_AGENT_SAFE_MODE"):
        logger.info("SOCIS_AGENT_SAFE_MODE=1 — outbound webhook registration skipped")
        return []
...
def _serialize_payload(
    ...
    ``delivery_id`` is shared with the ``X-SOCIS-Delivery`` header so
...
def _build_delivery(
    ...
    if target.secret:
        ...
        headers["X-SOCIS-Signature-256"] = f"sha256={digest}"
    return {
        "url": target.url,
        "label": target.label,
        "event": event,
