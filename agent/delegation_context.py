...
The parent SOCIS process may itself be a Kanban dispatcher worker with
SOCIS_AGENT_KANBAN_* variables in process env. delegate_task children run inside the
...
_DELEGATED_CHILD_CONTEXT: ContextVar[bool] = ContextVar(
    "socis_delegated_child_context",
    default=False,
)

# Set for any in-process execution that is NOT the dispatcher-owned worker even
# though the worker's SOCIS_AGENT_KANBAN_* vars are legitimately in os.environ (cron
...
_NON_DISPATCHER_OWNED_CONTEXT: ContextVar[bool] = ContextVar(
    "socis_non_dispatcher_owned_context",
    default=False,
)

DELEGATED_CHILD_ENV_MARKER = "SOCIS_AGENT_DELEGATED_CHILD_CONTEXT"
...
def non_dispatcher_owned_context() -> Iterator[None]:
    ...
    own process, where ``SOCIS_AGENT_KANBAN_TASK`` is legitimately set.  Without this
    ...
    ``kanban_complete`` defaults ``task_id`` to ``$SOCIS_AGENT_KANBAN_TASK`` — letting
...
def is_dispatcher_owned_worker_context() -> bool:
    ...
    The single predicate every ``SOCIS_AGENT_KANBAN_*`` identity gate should use
...
def delegated_child_subprocess_env(
    ...
    parent dispatcher ``SOCIS_AGENT_KANBAN_*`` vars while losing the ContextVar in
