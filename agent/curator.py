...
def _state_file() -> Path:
    return get_socis_agent_home() / "skills" / ".curator_state"
...
def _load_config() -> Dict[str, Any]:
    """Read curator.* config from ~/.socis-agent/config.yaml. Tolerates missing file."""
    try:
...
def get_prune_builtins() -> bool:
    ...
    suppression list keeping them archived across `socis update` re-seeds.
...
def get_consolidate() -> bool:
    ...
    The explicit ``socis curator run --consolidate`` flag overrides this for
...
def should_run_now(now: Optional[datetime] = None) -> bool:
    ...
    ``socis update``. On first observation we seed ``last_run_at`` to "now"
    and defer the first real pass by one full interval. Users who want to
    run it sooner can always invoke ``socis curator run`` (with or without
    ...
    if last is None:
        ...
        try:
            ...
            state["last_run_summary"] = (
                ...
                "interval; use `socis curator run --dry-run` to preview now"
            )
            save_state(state)
...
CURATOR_REVIEW_PROMPT = (
    "You are running as SOCIS' background skill CURATOR. This is an "
    ...
    "into ~/.socis-agent/skills/.archive/) is the maximum destructive action. "
    ...
    "to find: socis-config-*, socis-dashboard-*, gateway-*, codex-*, "
    ...
    "`socis curator rollback` restores a hollow skill (issue #96962).\n\n"
...
def _reports_root() -> Path:
    ...
    Lives under the profile-aware logs dir (``~/.socis-agent/logs/curator/``)
    ...
    authored skill data in ``~/.socis-agent/skills/``.

    ``ensure_socis_agent_home()`` pre-creates this dir on every CLI launch and
    ...
    that bypasses both.
    """
    root = get_socis_agent_home() / "logs" / "curator"
    try:
...
def _parse_structured_summary(
    ...
    # Prefer PyYAML when available — every socis install already has it
...
def _build_rename_summary(
    *,
    ...
    Format::
        ...
        full report: socis curator status
        keep an umbrella stable: socis curator pin document-tools
    ...
    lines.append("full report: socis curator status")
    ...
    if consolidated:
        ...
        if umbrellas:
            example = umbrellas[0]
            lines.append(
                f"keep an umbrella stable: socis curator pin {example}"
            )
    return "\n".join(lines)
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    # on disk still lives under ~/.socis-agent/skills/.archive/ (every removal is
    ...
    if consolidated:
        ...
        lines.append(
            ...
            "The original directory was moved to `~/.socis-agent/skills/.archive/` for "
            "safety and can be restored via `socis curator restore <name>` if the "
    ...
    if pruned:
        ...
        lines.append(
            ...
            "Directories live under `~/.socis-agent/skills/.archive/`. "
            "Restore any via `socis curator restore <name>`._\n"
        )
        SHOW = 50
    ...
    lines.append("- Restore an archived skill: `socis curator restore <name>`")
    lines.append("- All archives live under `~/.socis-agent/skills/.archive/` and are recoverable by `mv`")
...
def run_curator_review(
    ...
    *consolidate* gates the LLM umbrella-building pass. ``None`` (the default)
    ...
    ``socis curator run --consolidate`` flag. When consolidation is off, only
    ...
    *consolidate*: when consolidation is off, the preview only reports the
    ...
    # real pass out. We still record a summary so `socis curator status`
    ...
    def _llm_pass():
            ...
            if "No agent-created skills" in candidate_list:
                ...
                if get_prune_builtins():
                    builtins_note = (
                        ...
                        "delete). It will be restored on `socis update` only if "
        ...
        # recorded in state so `socis curator status` can point at it.
        try:
            after_report = skill_usage.curated_report()
        except Exception:
            after_report = []
...
def _resolve_review_model(cfg: Dict[str, Any]) -> tuple[str, str]:
    ...
    — so it participates in the canonical aux-model plumbing (``socis model`` →
...
def _run_llm_review(prompt: str) -> Dict[str, Any]:
    ...
    # (canonical aux-task slot, wired through `socis model` → auxiliary
    ...
    try:
        ...
        review_agent = AIAgent(
            ...
            **_agent_kwargs,
            ...
            # followed snapshotted an already-stripped package and ``socis
