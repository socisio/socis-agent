...
as ``kind: backend``) or ``~/.socis-agent/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        backend can't honor are not advertised), and by ``socis tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$SOCIS_AGENT_HOME/cache/videos/``, creating parents as needed."""
    from socis_agent_constants import get_socis_agent_home

    path = get_socis_agent_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$SOCIS_AGENT_HOME/cache/videos/``.
...
def save_url_video(
    ...
    *,
    ...
    """Download a video URL and write it under ``$SOCIS_AGENT_HOME/cache/videos/``.
