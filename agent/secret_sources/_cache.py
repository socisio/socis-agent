...
cache problem must never block SOCIS startup.
"""
...
def resolve_cache_home(home_path: Optional[Path] = None) -> Path:
    """Resolve the SOCIS home used for cache paths.

    ``home_path`` is whatever ``load_socis_dotenv()`` already resolved;
    falling back to ``$SOCIS_AGENT_HOME`` / ``~/.socis-agent`` keeps direct callers
    (and tests that don't thread a home through) working.
    """
    if home_path is None:
        from socis_agent_constants import get_socis_agent_home

        home_path = get_socis_agent_home()
    return home_path
...
class DiskCache(Generic[K]):
    ...
    One JSON object per backend lives at ``<socis_agent_home>/cache/<basename>``::
    ...
    def write(
        ...
        try:
            ...
            # metadata isn't exposed if SOCIS_AGENT_HOME is ever made traversable.
            try:
