...
Optional hooks (override to opt in):
  ...
  backup_paths() -> list[str]            — extra on-disk paths to include in `socis backup`
"""
...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - socis_agent_home (str): The active SOCIS_AGENT_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.socis-agent``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "socis").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'socis memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], socis_agent_home: str) -> None:
        ...
        Called by 'socis memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``socis_agent_home`` is the active SOCIS_AGENT_HOME directory path.
    ...
    def backup_paths(self) -> List[str]:
        """Return extra on-disk paths this provider stores OUTSIDE SOCIS_AGENT_HOME.

        ``socis backup`` only walks SOCIS_AGENT_HOME, so any provider state kept
        ...
        of the archive, and ``socis import`` restores them to their original
