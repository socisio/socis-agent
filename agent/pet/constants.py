...
row taxonomy is inferred from the atlas shape so SOCIS can render both legacy
...
# rows (1536x1872), while older SOCIS/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are SOCIS' activity state names. They are not always identical to the
...
# Legacy SOCIS/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical SOCIS activity names -> accepted row-name aliases in descending
