...
SOCIS_AGENT_TRANSPORT_OWNER_EXT = "socis_transport_owner"
...
def enable_happy_eyeballs_on_client(client) -> None:
    ...
    refresh / device-login / usage-probe clients in ``socis_cli.auth``.
...
class _SafeWriter:
    ...
    When socis-agent runs as a systemd service, Docker container, or headless
...
def _shared_transport_cls():
    ...
    class _SharedTransportImpl(httpx.BaseTransport):
        ...
        def handle_request(self, request: Any) -> Any:
            ...
            request.extensions[SOCIS_AGENT_TRANSPORT_OWNER_EXT] = id(self)
...
def build_keepalive_http_client(
    ...
    *,
    ...
    ``ssl_ca_cert`` / ``ssl_verify`` and ``SOCIS_AGENT_CA_BUNDLE`` settings the main
