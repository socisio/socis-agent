...
def _resolve_requests_verify(base_url: str = "") -> bool | str:
    ...
    3. Env vars ``SOCIS_AGENT_CA_BUNDLE`` / ``REQUESTS_CA_BUNDLE`` / ``SSL_CERT_FILE``
    ...
    for env_var in ("SOCIS_AGENT_CA_BUNDLE", "REQUESTS_CA_BUNDLE", "SSL_CERT_FILE"):
...
def _local_probe_disk_cache_path() -> Path:
    ...
    return get_socis_agent_home() / "cache" / "local_endpoint_probes.json"
...
def _get_model_metadata_cache_path() -> Path:
    ...
    from socis_agent_constants import get_socis_agent_home
    return get_socis_agent_home() / "cache" / "openrouter_model_metadata.json"
...
def _get_endpoint_metadata_cache_path() -> Path:
    ...
    from socis_agent_constants import get_socis_agent_home
    return get_socis_agent_home() / "cache" / "endpoint_model_metadata.json"
...
def _endpoint_disk_cache_get(normalized: str) -> Optional[Dict[str, Dict[str, Any]]]:
    ...
    (``socis -q``, cron, every Bot Mode DM hop) start cold and re-probed the
...
# Minimum context length required to run SOCIS Agent.  Models with fewer
...
DEFAULT_CONTEXT_LENGTHS = {
    ...
    # so these hardcoded fallbacks prevent SOCIS from probing-down to
...
_URL_TO_PROVIDER: Dict[str, str] = {
    ...
    # (often 8K) makes it unusable for SOCIS' system prompt, but mapping
...
def _maybe_cache_local_context_length(
    ...
    """Persist a locally probed context length only when it meets SOCIS minimum.
...
def _get_context_cache_path() -> Path:
    ...
    from socis_agent_constants import get_socis_agent_home
    return get_socis_agent_home() / "context_length_cache.yaml"
        ...
        with httpx.Client(timeout=3.0, headers=headers) as client:
            ...
            if server_type == "ollama":
                ...
                if resp.status_code == 200:
                    ...
                    # SOCIS grow conversations past the runtime limit and Ollama
...
# who never asked for it. The variant suffix is a SOCIS-side alias: it is
...
# SOCIS-side picker suffix that opts a Codex slug into the live-verified
...
def strip_codex_context_variant_suffix(model: Optional[str]) -> str:
    ...
    The suffix is a SOCIS picker alias (``gpt-5.6-sol-900k``); the Codex
...
def _resolve_codex_oauth_context_length_with_source(
    ...
    # ``-900k`` variants are SOCIS picker aliases — the Codex catalog only
...
def get_model_context_length(
    ...
    if effective_provider in {"copilot", "copilot-acp", "github-copilot"}:
        try:
            from socis_cli.models import get_copilot_model_context
    ...
    # ``SOCIS-3-Llama-3.1-70B`` substring-match ``llama`` (131072) even when
...
def estimate_request_tokens_rough(
    ...
    *,
    ...
    Includes the major payload buckets SOCIS sends to providers:
