...
def _socis_agent_home_path() -> Path:
    """Resolve the active SOCIS_AGENT_HOME (profile-aware) without circular imports."""
    try:
        from socis_agent_constants import get_socis_agent_home  # local import to avoid cycles
        return get_socis_agent_home()
    except Exception:
        return Path(os.path.expanduser("~/.socis-agent"))


def _socis_root_path() -> Path:
    """Resolve the SOCIS root dir (always the parent of any profile, never per-profile)."""
    try:
        from socis_agent_constants import get_default_socis_root  # local import to avoid cycles
...
def build_write_denied_paths(home: str) -> set[str]:
    ...
    socis_agent_home = _socis_agent_home_path()
    socis_root = _socis_root_path()
    return {
        os.path.realpath(p)
        for p in [
            ...
            str(socis_agent_home / ".env"),
            # Top-level .env, even when running under a profile — overwriting it
            # leaks credentials across every profile that inherits from root (#15981).
            str(socis_root / ".env"),
            # Active profile Anthropic PKCE credential store.
            str(socis_agent_home / ".anthropic_oauth.json"),
            # Top-level Anthropic PKCE credential store remains sensitive even
            # when a profile is active; default/non-profile sessions still read it.
            str(socis_root / ".anthropic_oauth.json"),
            # Bitwarden Secrets Manager encrypted disk cache.
            str(socis_agent_home / "cache" / "bws_cache.enc.json"),
            str(socis_root / "cache" / "bws_cache.enc.json"),
...
def get_safe_write_roots() -> set[str]:
    """Return resolved SOCIS_AGENT_WRITE_SAFE_ROOT paths. Supports multiple directories
    separated by ``os.pathsep`` (``:`` on Unix, ``;`` on Windows).
    E.g., ``/opt/data:/var/www/html`` on Unix, ``C:\\data;D:\\www`` on Windows."""
    env = os.getenv("SOCIS_AGENT_WRITE_SAFE_ROOT", "")
    if not env:
        return set()
    roots: set[str] = set()
...
def _classify_write_denial(path: str) -> Optional[str]:
    ...
    socis_dirs = []
    for base in (_socis_agent_home_path(), _socis_root_path()):
        try:
            real = os.path.realpath(base)
            if real not in socis_dirs:
                socis_dirs.append(real)
        except Exception:
            continue

    for base_real in socis_dirs:
...
def get_write_denied_error(path: str, *, verb: str = "Write") -> Optional[str]:
    ...
    if denial == "safe_root":
        ...
        return (
            f"{verb} denied: '{path}' is outside SOCIS_AGENT_WRITE_SAFE_ROOT "
...
def get_read_block_error(path: str) -> Optional[str]:
    """Return an error message when a read targets a denied SOCIS path.

    Three categories are blocked:

      * Internal SOCIS cache files under ``SOCIS_AGENT_HOME/skills/.hub`` —
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under SOCIS_AGENT_HOME and the global SOCIS
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    or ``cat ~/.socis-agent/.env`` and exfiltrate the file. The read-deny exists
    ...
    # Resolve BOTH the active SOCIS_AGENT_HOME (profile-aware) AND the global
    # SOCIS root so credential stores at <root>/auth.json etc. are also
    # blocked when running under a profile (SOCIS_AGENT_HOME points at
    # <root>/profiles/<name> in profile mode). Same shape as the write
    # deny widening (#15981, #14157).
    socis_dirs: list[Path] = []
    for base in (_socis_agent_home_path(), _socis_root_path()):
        try:
            real = base.resolve()
            if real not in socis_dirs:
    ...
    for hd in socis_dirs:
        ...
        for blocked in blocked_dirs:
            try:
                ...
                continue
            return (
                f"Access denied: {path} is an internal SOCIS cache file "
                "and cannot be read directly to prevent prompt injection. "
                "Use the skills_list or skill_view tools instead."
            )

    # Credential / secret stores. Exact-file matches under either
    # SOCIS_AGENT_HOME or <root>.
    ...
    for hd in socis_dirs:
        for name in credential_file_names:
            ...
            if resolved == blocked:
                return (
                    f"Access denied: {path} is a SOCIS credential store "
    ...
    for hd in socis_dirs:
        ...
        if resolved == mcp_tokens:
            return (
                f"Access denied: {path} is the SOCIS MCP token directory "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is a SOCIS MCP token file "
            "and cannot be read directly. (Defense-in-depth — not a "
            "security boundary; the terminal tool can still bypass.)"
        )
    ...
    for hd in socis_dirs:
        ...
        if resolved == browser_profile:
            return (
                f"Access denied: {path} is the SOCIS real-profile browser "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is inside the SOCIS real-profile browser "
...
def raise_if_read_blocked(path: str) -> None:
    """Raise ``ValueError`` if ``path`` is a denied SOCIS read (see
...
#
# SOCIS profiles are separate SOCIS_AGENT_HOME dirs under
...
#
# Reference: May 2026 incident where a socis-security profile session
# edited skills under both ``~/.socis-agent/profiles/socis-security/skills/``
# AND ``~/.socis-agent/skills/`` (the default profile's skills) without realizing
# the second path belonged to a different profile.
# ---------------------------------------------------------------------------

# Profile-scoped directories under SOCIS_AGENT_HOME / <root> / <root>/profiles/<X>/
...
def _resolve_active_profile_name() -> str:
    """Return the active profile name derived from SOCIS_AGENT_HOME.

    ``~/.socis-agent``              -> ``"default"``
    ``~/.socis-agent/profiles/X``  -> ``"X"``

    Falls back to ``"default"`` on any resolution failure so the guard
    never raises into the tool path.
    """
    try:
        home_real = _socis_agent_home_path().resolve()
        root_real = _socis_root_path().resolve()
...
def classify_cross_profile_target(path: str) -> Optional[dict]:
    ...
    Returns ``None`` when the target is outside SOCIS scope, or is inside
...
#
#   <SOCIS_AGENT_HOME>/profiles/<name>/sandboxes/<backend>/<task>/home/.socis-agent/...
#
...
# ``…/sandboxes/<backend>/<task>/home/.socis-agent/…`` segment and warns
# regardless of which SOCIS profile is active. It does NOT cover the
# inner-container case where the bind mount strips the ``sandboxes/`` prefix
# (the agent's view inside the container is plain ``/root/.socis-agent/...``);
...
def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    """Classify a write target as a sandbox-mirror of authoritative SOCIS state.
    ...
    SOCIS_AGENT_HOME resolution would be ambiguous.
    """
    try:
        target = Path(os.path.expanduser(str(path))).resolve()
    except (OSError, RuntimeError):
...
def get_sandbox_mirror_warning(path: str) -> Optional[str]:
    ...
    return (
        ...
        f"Writes here land on a copy that the host SOCIS process never "
        f"reads — the authoritative file is likely {info['inner_path']!r} "
        f"under the real SOCIS_AGENT_HOME. Use the host-side tool for "
...
# full ``…/sandboxes/<backend>/<task>/home/.socis-agent/…`` prefix on the host.
# But when file tools execute *inside* the container the bind-mount strips
# that prefix: the agent sees plain ``/root/.socis-agent/…``.  The root:root
...
def classify_container_mirror_target(
    ...
    under the mirror prefix. Otherwise returns:
      ...
      * ``inner_path``: portion under the mirror root (what the agent
        likely meant to address in the host SOCIS_AGENT_HOME)
    """
...
def get_container_mirror_warning(
    ...
    sandbox mirror of authoritative SOCIS state.
    ...
    return (
        ...
        f"bind-mounted home — a per-task mirror that the host SOCIS "
        f"process never reads. The authoritative file is "
        f"{info['inner_path']!r} under the real SOCIS_AGENT_HOME. Use the "
