...
Resolution order for text tasks (auto mode):
  ...
  3. Nous Portal (~/.socis-agent/auth.json active provider)
...
from socis_agent_constants import OPENROUTER_BASE_URL
...
def _resolve_aux_verify(base_url: Optional[str]) -> Any:
    ...
    ``ssl_ca_cert`` / ``ssl_verify`` config and the ``SOCIS_AGENT_CA_BUNDLE`` /
    ...
    try:
        ...
        if not _WARNED_KEEPALIVE_IMPORT_SKEW:
            ...
            logger.warning(
                ...
                "back to the SDK default HTTP client. Run `socis update` (or "
...
def _create_openai_client(*, api_key: str, base_url: str, **kwargs: Any) -> Any:
    ...
    # SOCIS owns auxiliary retry + provider/model fallback policy (the
    ...
    # SOCIS sees a single failure (issue #54465). Disable SDK-internal retries
    # by default and let SOCIS control the budget; explicit callers can still
    # override via kwargs.
...
def _run_protected_sync_provider_call(
    ...
    threading.Thread(
        ...
        name="socis-protected-aux-provider",
        daemon=True,
    ).start()
...
def _client_declares(client_obj: Any, flag: str) -> bool:
    ...
    without this module importing it. Mirrors ``SUPPORTS_SOCIS_AGENT_TOOL_CALLS`` in
...
def _compression_threshold_for_model(
    ...
    *,
    ...
    consumed before SOCIS triggers summarization.  Higher values delay
...
def _fast_model_from_catalog(provider_id: str) -> str:
    ...
    try:
        from socis_cli.auth import resolve_api_key_provider_credentials
        from socis_cli.models import fetch_models_with_pricing
        ...
        if not api_key and is_nous:
            ...
            try:
                from socis_cli.models import _resolve_nous_pricing_credentials
        ...
        _nous_kwargs = {}
        if is_nous:
            from socis_cli.models import _NOUS_CATALOG_TTL_SECONDS
...
def _nous_policy_blocks(model_id: str) -> bool:
    ...
    try:
        from socis_cli.models import nous_policy_allowed_ids, restrict_to_nous_policy
...
def build_or_headers(or_config: dict | None = None) -> dict:
    ...
    Environment variables:
        ``SOCIS_AGENT_OPENROUTER_CACHE`` — truthy (``1``/``true``/``yes``/``on``)
            enables caching; ``0``/``false``/``no``/``off`` disables.
            Overrides ``openrouter.response_cache`` in config.yaml.
        ``SOCIS_AGENT_OPENROUTER_CACHE_TTL`` — integer seconds (1-86400).
    ...
    *or_config* is the ``openrouter`` section from config.yaml.  When *None*,
    ...
    env_cache = os.environ.get("SOCIS_AGENT_OPENROUTER_CACHE", "").strip().lower()
    if env_cache:
    ...
    env_ttl = os.environ.get("SOCIS_AGENT_OPENROUTER_CACHE_TTL", "").strip()
    if env_ttl:
        if env_ttl.isdigit():
            ttl = int(env_ttl)
            if 1 <= ttl <= 86400:
...
_NVIDIA_NIM_CLOUD_HEADERS = {
    "X-BILLING-INVOKE-ORIGIN": "SOCISAgent",
}
...
from socis_cli import __version__ as _SOCIS_AGENT_VERSION

_AI_GATEWAY_HEADERS = {
    "HTTP-Referer": "https://agent.socis.io",
    "X-Title": "SOCIS Agent",
    "User-Agent": f"SOCISAgent/{_SOCIS_AGENT_VERSION}",
}
...
# in lockstep with socis_cli.__version__ across every Portal call site
...
def _nous_extra_body() -> dict:
    ...
    Computed at call time so a hot-reloaded ``socis_cli.__version__`` is
...
_AUTH_JSON_PATH = get_socis_agent_home() / "auth.json"
...
def _pool_runtime_base_url(entry: Any, fallback: str = "") -> str:
    ...
    if getattr(entry, "provider", None) == "nous":
        ...
        from socis_cli.auth import _nous_inference_env_override
...
def _nous_min_key_ttl_seconds() -> int:
    try:
        return max(60, int(os.getenv("SOCIS_AGENT_NOUS_MIN_KEY_TTL_SECONDS", "1800")))
...
class _CodexCompletionsAdapter:
    ...
    def create(self, **kwargs) -> Any:
        ...
        resp_kwargs: Dict[str, Any] = {
            # Strip the SOCIS-side ``-900k`` large-context picker suffix —
...
class _AnthropicCompletionsAdapter:
    ...
    def create(self, **kwargs) -> Any:
        ...
        #   - ``_``-prefixed keys: private SOCIS plumbing (_reasoning_config
...
def _endpoint_speaks_anthropic_messages(base_url: str) -> bool:
    ...
    Mirrors ``socis_cli.runtime_provider._detect_api_mode_for_url`` so the
...
def _maybe_wrap_anthropic(
    ...
    - It's already a complete client — an Anthropic/Codex wrapper, or any
      client declaring ``SOCIS_AGENT_SKIP_TRANSPORT_WRAP`` (the native and ACP
    ...
    if _client_declares(client_obj, "SOCIS_AGENT_SKIP_TRANSPORT_WRAP"):
        return client_obj
...
def _read_nous_auth() -> Optional[dict]:
    """Read and validate ~/.socis-agent/auth.json for an active Nous provider.
...
def _nous_api_key(provider: dict) -> str:
    ...
    from socis_cli.auth import _nous_invoke_jwt_is_usable
...
def _resolve_nous_pool_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    ...
    try:
        from socis_cli.auth import _agent_key_is_usable
...
def _resolve_nous_runtime_api(
    *, force_refresh: bool = False, stale_access_token: Optional[str] = None
    ...
    try:
        from socis_cli.auth import resolve_nous_runtime_credentials

        creds = resolve_nous_runtime_credentials(
            timeout_seconds=env_float("SOCIS_AGENT_NOUS_TIMEOUT_SECONDS", 15),
...
def _resolve_xai_oauth_for_aux() -> Optional[Tuple[str, str]]:
    ...
    compression report "no provider configured" even though ``socis auth
    status`` shows xAI OAuth as logged in.

    Falls back to ``socis_cli.auth``'s singleton runtime resolver for older
    ...
    try:
        from socis_cli.auth import resolve_xai_oauth_runtime_credentials
...
def _read_codex_access_token() -> Optional[str]:
    """Read a valid, non-expired Codex OAuth access token from SOCIS auth store.
    ...
    try:
        from socis_cli.auth import _read_codex_tokens
...
def _resolve_api_key_provider() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    """
    try:
        from socis_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    except ImportError:
    ...
    for provider_id, pconfig in PROVIDER_REGISTRY.items():
        ...
        if provider_id == "anthropic":
            ...
            try:
                from socis_cli.auth import is_provider_explicitly_configured
        ...
        if pool_present:
            ...
            if base_url_host_matches(base_url, "api.kimi.com"):
                ...
                from socis_cli.models import copilot_default_headers
...
def _try_nous(vision: bool = False) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    if not _aux_probe_active():
        ...
        try:
            from socis_cli.models import get_nous_recommended_aux_model
...
def _resolve_moa_aggregator(preset_name: Optional[str]) -> Tuple[Optional[str], Optional[str]]:
    ...
    try:
        from socis_cli.config import load_config
        from socis_cli.moa_config import resolve_moa_preset
...
def _validate_base_url(base_url: str) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Run `socis setup` or `socis model` and enter a valid http(s) base URL."
        ) from exc
...
def _build_xai_oauth_aux_client(model: str) -> Tuple[Optional[Any], Optional[str]]:
    ...
    real_client = _create_openai_client(
        api_key=api_key,
        base_url=base_url,
        default_headers=socis_xai_default_headers(),
    )
    return CodexAuxiliaryClient(real_client, model), model
...
def _try_azure_foundry(
    *,
    ...
    :func:`socis_cli.runtime_provider._resolve_azure_foundry_runtime` —
    ...
    * Non-default ``model.base_url`` overrides are honored.
    ...
    """
    try:
        from socis_cli.runtime_provider import _resolve_azure_foundry_runtime
...
# manual intervention.
#
# Failure isolation: the cache is in-process only. A second socis
...
def _reset_aux_unhealthy_cache() -> None:
    ...
    user trigger (e.g. ``socis config aux reset``)."""
...
def _nous_portal_account_has_fresh_paid_access() -> bool:
    ...
    try:
        from socis_cli.nous_account import get_nous_portal_account_info
...
def _refresh_provider_credentials(provider: str) -> bool:
    ...
    try:
        ...
        if normalized == "openai-codex":
            from socis_cli.auth import resolve_codex_runtime_credentials
...
def _fallback_destination(
    ...
    attached = getattr(fb_client, "_socis_fallback_destination", None)
...
def _fallback_entry_api_key(entry: Dict[str, Any]) -> Optional[str]:
    ...
    gateway multiplexing (see ``socis_cli.fallback_config.resolve_entry_api_key``).
    """
...
def _resolve_fallback_entry(entry: Dict[str, Any]) -> Tuple[Optional[Any], Optional[str]]:
    ...
    if client is not None:
        try:
            client._socis_fallback_destination = _fallback_destination_from_entry(
...
def _try_main_fallback_chain(
    ...
    main fallback policy before dropping into SOCIS' built-in discovery
...
def _resolve_auto_route(
    ...
    #    scenario where a user switches providers via `socis model` but the
    #    old OPENAI_BASE_URL lingers in ~/.socis-agent/.env. ──
    if not _stale_base_url_warned:
        ...
        if (_env_base and _cfg_provider
            ...
            logger.warning(
                ...
                "Run: socis model to reconfigure, or remove "
                "OPENAI_BASE_URL from ~/.socis-agent/.env",
    ...
    if (main_provider and main_model
        ...
        if runtime_base_url and main_provider == "custom":
            ...
            try:
                from socis_cli.runtime_provider import _get_named_custom_provider
...
def _tag_effective_provider(client: Any, provider: str) -> None:
    ...
    if client is None or not provider:
        return
    try:
        setattr(client, "_socis_aux_effective_provider", provider)
...
def _effective_provider_for_client(client: Any, fallback: str) -> str:
    ...
    effective_provider = getattr(client, "_socis_aux_effective_provider", "")
...
def _to_async_client(sync_client, model: str, is_vision: bool = False):
    ...
    if _client_declares(sync_client, "SOCIS_AGENT_SKIP_ASYNC_WRAP"):
        return sync_client, model
    ...
    if base_url_host_matches(sync_base_url, "openrouter.ai"):
        ...
        from socis_cli.copilot_auth import copilot_request_headers
        ...
        async_kwargs["default_headers"] = socis_xai_default_headers()
    else:
    ...
    # See _create_openai_client: disable SDK-internal retries so SOCIS owns
...
def resolve_provider_client(
    ...
    if provider == "nous":
        ...
        if client is None:
            logger.warning("resolve_provider_client: nous requested "
                           "but Nous Portal not configured (run: socis auth)")
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        ...
        if raw_codex:
            ...
            if not codex_token:
                logger.warning("resolve_provider_client: openai-codex requested "
                               "but no Codex OAuth token found (run: socis model)")
                return None, None
    ...
    if provider == "xai-oauth":
        ...
        if client is None:
            logger.warning(
                ...
                "OAuth token found (run: socis model -> xAI Grok OAuth — SuperGrok / Premium+)"
            )
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
    ...
    if provider == "azure-foundry":
        ...
        if client is None:
            logger.warning(
                ...
                "runtime resolution failed (run: socis doctor for "
                "diagnostics)"
            )
            return None, None
    ...
    try:
        ...
        logger.debug("socis_cli.auth not available for provider %s", provider)
        return None, None
    ...
    if pconfig.auth_type == "api_key":
        ...
        try:
            from socis_cli.models import opencode_zen_free_runtime as _oc_free_rt
        ...
        if base_url_host_matches(base_url, "api.kimi.com"):
            ...
            headers.update(socis_xai_default_headers())
        else:
            # Fall back to profile.default_headers for providers that declare
        ...
        if provider == "copilot" and final_model and not raw_codex:
            try:
                from socis_cli.models import _should_use_copilot_responses_api
...
def _refresh_nous_auxiliary_client(
    *,
    ...
    default ``"SOCIS-4-405B"``); it is stored as the entry's usable model and
    ...
    element ``"SOCIS-4-405B"`` vs the lookup's ``""``), leaving the expired
...
def _get_auxiliary_task_config(task: str) -> Dict[str, Any]:
    ...
    :meth:`socis_cli.plugins.PluginContext.register_auxiliary_task`) the
...
def resolve_compression_fast_lane(
    ...
    *,
    ...
    route is the one SOCIS will call. A requested model covers a compressor
...
def _build_call_kwargs(
    ...
    if max_tokens is not None:
        ...
        if _provider_norm in {"nous", "nous-portal", "socis"}:
            from socis_cli.providers import nous_api_mode
    ...
    if _provider_for_portal in {"nous", "nous-portal", "socis"}:
    ...
    # Anthropic Messages adapters translate SOCIS reasoning into native
    ...
    if reasoning_config and isinstance(reasoning_config, dict):
        ...
        effective_base = base_url or ""
        _nous_on_messages = False
        if provider_norm in {"nous", "nous-portal", "socis"}:
            from socis_cli.providers import nous_api_mode
...
def _managed_local_netloc() -> str:
    ...
    netloc = ""
    try:
        from socis_cli.local_runtime.supervisor import state_path
...
def _is_managed_local_endpoint(base_url: Optional[str]) -> bool:
    """True when *base_url* targets the llama-server this SOCIS manages."""
    if not base_url:
        return False
...
def _call_llm_impl(
    ...
    *,
    ...
    Args:
        ...
        reasoning_config: Optional SOCIS reasoning config for direct model calls
            ...
            if _explicit and _explicit not in {"auto", "openrouter", "custom"}:
                ...
                if fb_client is not None:
                    ...
                    raise RuntimeError(
                        ...
                        f"variable, or switch to a different provider with `socis model`."
                    )
