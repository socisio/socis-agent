...
``socis pause`` writes a sentinel file at ``$SOCIS_AGENT_HOME/ESTOP``;
``socis resume`` removes it. While the sentinel exists:
...
* new gateway turns get a brief "SOCIS is paused" reply instead of an
...
hold even if the file was created by ``touch ~/.socis-agent/ESTOP``.
...
def _socis_agent_home() -> Path:
    """Resolve the active SOCIS_AGENT_HOME (profile-aware) at call time."""
    try:
...
def _canonical_root() -> Path:
    """Fleet-wide SOCIS root, even when this process is a profile gateway.

    Profile gateways launch with SOCIS_AGENT_HOME=~/.socis-agent/profiles/<name>.
    ``socis pause`` from an operator seat writes ~/.socis-agent/ESTOP. If we
    ...
    try:
        ...
        return Path(get_default_socis_root())
    except Exception:
        return Path(os.path.expanduser("~/.socis-agent"))


def sentinel_path() -> Path:
    """Path of the ESTOP sentinel this process would write on `socis pause`."""
    return _socis_agent_home() / SENTINEL_NAME
...
def is_engaged() -> bool:
    ...
    Engaged if ANY candidate sentinel exists: the process SOCIS_AGENT_HOME
    (profile-local) or the fleet canonical root (~/.socis-agent). Fail SAFE on
...
def disengage() -> bool:
    ...
    ``socis resume`` from a profile gateway still clears an operator pause
    written at ~/.socis-agent/ESTOP.
    """
    lifted = False
    for path in _candidate_sentinel_paths():
        try:
...
def paused_reply() -> Optional[str]:
    ...
    if reason:
        return (
            f"⏸️ SOCIS is paused ({reason}). New work is on hold; "
            "run `socis resume` to pick things back up."
        )
    return (
        "⏸️ SOCIS is paused. New work is on hold; "
        "run `socis resume` to pick things back up."
    )
