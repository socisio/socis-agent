...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time SOCIS starts and finds ``~/.openclaw/``.

    Points users at ``socis claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``socis claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to SOCIS, run "
        "`socis claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `socis claw cleanup` (renames it to ~/.openclaw.pre-migration — "
