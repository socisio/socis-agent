...
A host that mints one physical ``session_id`` per RESPONSE (SOCIS Studio's
...
SOCIS sends — ``prompt_cache_key`` on both OpenAI-wire transports, plus the
...
SOCIS must not infer the logical conversation from the id's SYNTAX (that
...
``X-SOCIS-Session-Key`` header, which branching deliberately does NOT key
...
def _agent_source(
    ...
    :meth:`~socis_agent_state.SessionDB.declared_scope_identity` still takes.
    ...
    diverge whenever ``SOCIS_AGENT_SESSION_SOURCE`` overrides the platform, and the
...
def declared_conversation_scope(agent: Any) -> Optional[str]:
    ...
    Resolved from ``agent._gateway_session_key`` (the ``X-SOCIS-Session-Key``
