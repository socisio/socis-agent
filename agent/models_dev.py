...
Data resolution order:
  ...
  2. Disk cache (~/.socis-agent/models_dev_cache.json — any age; stale data is
...
# ---------------------------------------------------------------------------
# Provider ID mapping: SOCIS ↔ models.dev
# ---------------------------------------------------------------------------

# SOCIS provider names → models.dev provider IDs
PROVIDER_TO_MODELS_DEV: Dict[str, str] = {
    ...
    # socis_cli/models.py), and models.dev's "opencode" catalog is the one
    ...
    # under the "meta" provider id; SOCIS' provider is "meta-ai" (and the
...
# Reverse mapping: models.dev id → SOCIS ids (built lazily; many-to-one,
# e.g. both "meta" and "meta-ai" may map to the same models.dev id).
_MODELS_DEV_TO_PROVIDER: Optional[Dict[str, List[str]]] = None


def _models_dev_to_socis_ids(mdev_id: str) -> List[str]:
    """Return the SOCIS provider ids that map to *mdev_id* (may be [])."""
    global _MODELS_DEV_TO_PROVIDER
    if _MODELS_DEV_TO_PROVIDER is None:
        reverse: Dict[str, List[str]] = {}
        for socis_id, mapped in PROVIDER_TO_MODELS_DEV.items():
            reverse.setdefault(mapped, []).append(socis_id)
...
def _get_cache_path() -> Path:
    ...
    return get_socis_agent_home() / "models_dev_cache.json"


def _get_etag_path() -> Path:
    """Return path to the ETag sidecar file for conditional GET."""
    from socis_agent_constants import get_socis_agent_home
    return get_socis_agent_home() / "models_dev_cache.etag"
...
def fetch_models_dev(
    ...
    When ``force_refresh=True`` (used by ``socis config refresh``, the
...
def _provider_override_section(provider: str) -> Optional[Dict[str, Any]]:
    ...
    Accepts either the SOCIS provider id or the models.dev provider id as
    ...
    # Reverse: caller passed a models.dev id, config keyed by SOCIS id.
    for socis_id in _models_dev_to_socis_ids(provider_key):
        if socis_id != provider_key:
            candidates.append(socis_id)
...
def _get_provider_models(
    ...
    """Resolve a SOCIS provider ID to its models dict from models.dev.
...
def list_provider_models(
    ...
    picker (``socis model``), which is an interactive user-facing flow
...
# Gemma models whose TPM quotas are too small for normal SOCIS agent traffic.
...
def get_provider_info(
    ...
    Accepts either a SOCIS provider ID (e.g. "kilocode") or a models.dev
    ...
    ``allow_network=False``.
    """
    # Resolve SOCIS ID → models.dev ID
...
def get_model_info(
    ...
    Accepts SOCIS or models.dev provider ID.  Tries exact match then
