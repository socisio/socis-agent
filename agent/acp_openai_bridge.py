"""OpenAI-shape bridge shared by SOCIS' ACP clients.

An ACP agent (``copilot --acp``, and the ACP CLIs that reach SOCIS as
...
the agent's *own* tool notifications. SOCIS' agentic surface — ``memory``,
...
* :func:`completion_to_stream_chunks` — re-shape a one-shot ACP response as
  ...
  is inherently one-shot from SOCIS' perspective).
...
of its own (Copilot) forwards everything SOCIS offers; a CLI that is an
autonomous agent with its own read/edit/execute tools must forward only SOCIS'
agent-level tools, because re-offering the overlapping ones makes SOCIS re-run
...
class StreamChunks(list):
    ...
    SOCIS reads provider-level extras off the object returned by
    ``chat.completions.create`` (e.g. ``socis_projected_messages``, consumed by
...
def tool_specs_from_openai_tools(
    ...
    *,
    ...
    name is in it survive — that is how a client forwards just SOCIS'
