...
Every SOCIS request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to SOCIS Agent and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=socis-agent",
        "client=socis-client-v<__version__>",
    ]

The version is sourced live from ``socis_cli.__version__`` so it auto-aligns
...
``socis_cli.__version__`` is the canonical source of truth.
"""
...
_affinity_scope: ContextVar[Optional[str]] = ContextVar(
    "socis_affinity_scope", default=None
)
...
def _socis_version() -> str:
    """Return the current SOCIS release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``socis_cli`` cannot be imported (should
...
def socis_client_tag() -> str:
    """Return the ``client=...`` tag for Nous Portal requests.

    Format: ``client=socis-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=socis-client-v{_socis_version()}"


def conversation_tag(session_id: str) -> str:
    """Return the ``conversation=...`` tag for a SOCIS session/conversation.

    Format: ``conversation=<session_id>``. ``session_id`` is the canonical
    SOCIS conversation identifier (``AIAgent.session_id``) — the same value
    used for ``~/.socis-agent/sessions/`` storage, session logs, and lineage.
...
def nous_portal_tags(session_id: str | None = None) -> List[str]:
    ...
    appended so Portal usage can be attributed to a specific SOCIS
    ...
    """
    tags = ["product=socis-agent", socis_client_tag()]
