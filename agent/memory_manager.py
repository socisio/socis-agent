...
class MemoryManager:
    ...
    def add_provider(self, provider: MemoryProvider) -> None:
        ...
        _core_tool_names = set(_SOCIS_AGENT_CORE_TOOLS)
    ...
    def _strip_skill_scaffolding(text: str) -> Optional[str]:
        ...
        When a user invokes a /skill or /bundle, SOCIS expands the turn into
    ...
    def _prefetch_provider(
        ...
        # Propagate the caller's contextvars (profile SOCIS_AGENT_HOME override)
    ...
    def _submit_background(self, fn, *, kind: str = "write") -> None:
        ...
        dashboard, cron) is a ContextVar-scoped SOCIS_AGENT_HOME override, and
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        ...
        Automatically injects ``socis_agent_home`` into *kwargs* so that every
        provider can resolve profile-scoped storage paths without importing
        ``get_socis_agent_home()`` themselves.
        """
        if "socis_agent_home" not in kwargs:
            from socis_agent_constants import get_socis_agent_home
            kwargs["socis_agent_home"] = str(get_socis_agent_home())
        for provider in self._providers:
            try:
