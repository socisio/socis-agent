"""Anthropic Messages API adapter for SOCIS Agent.

Translates between SOCIS's internal OpenAI-style message format and
...
try:
    ...
    _SOCIS_AGENT_VERSION = str(_socis_cli.__version__)
except Exception:
    _SOCIS_AGENT_VERSION = "0.0.0"
...
# SOCIS effort → Anthropic adaptive-thinking effort (output_config.effort).
...
# Anthropic's OAuth billing classifier fingerprints certain SOCIS tool
...
def _build_anthropic_client_with_bearer_hook(
    ...
    *,
    ...
    kwargs = {
        ...
        # Delegate retry to socis's outer loop (honors Retry-After); the SDK
...
def build_anthropic_client(
    ...
    * a ``Callable[[], str]`` — an Entra ID bearer token provider from
    ...
    kwargs = {
        ...
        # Delegate all rate-limit / 5xx retry to socis's outer conversation
    ...
    if _is_kimi_coding_endpoint(base_url):
        ...
        # HTTP-Referer + X-Title + SOCISAgent User-Agent.
    ...
    if _is_opencode_endpoint(base_url):
        ...
        headers.setdefault("HTTP-Referer", "https://agent.socis.io")
        headers.setdefault("X-Title", "SOCIS Agent")
        headers.setdefault("User-Agent", f"SOCISAgent/{_SOCIS_AGENT_VERSION}")
        kwargs["default_headers"] = headers

    client = _anthropic_sdk.Anthropic(**kwargs)
    # Bearer-only construction leaves ``api_key`` unset, so the SDK fills it
    # from ``ANTHROPIC_API_KEY`` (SOCIS loads that into the process env from
    # ``~/.socis-agent/.env``). The result is dual auth —
...
def build_anthropic_kwargs(
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("SOCIS Agent", "Claude Code")
                text = text.replace("SOCIS agent", "Claude Code")
                text = text.replace("socis-agent", "claude-code")
                text = text.replace("SOCIS", "Anthropic")
        ...
        #      a) bare SOCIS-native tools (``read_file``)  -> ``mcp__read_file``
    ...
    # silently hides reasoning text that SOCIS surfaces in its CLI. We
