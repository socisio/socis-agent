"""Upload a SOCIS session transcript to Hugging Face as an agent trace.

SOCIS stores sessions in its own SQLite store (``socis_agent_state.SessionDB``),
...
* **Zero LLM turn.** This is a deterministic export — it never spends a
  model call. The ``socis trace upload`` subcommand calls
...
* **Private by default.** Traces can contain prompts, tool output, local
  ...
  is passed through SOCIS' secret redactor (``force=True``) unless the
...
DEFAULT_DATASET_NAME = "socis-traces"
_SOCIS_AGENT_VERSION = "socis-agent"
...
# ---------------------------------------------------------------------------
# Conversion: SOCIS OpenAI-format messages -> Claude Code JSONL
# ---------------------------------------------------------------------------
...
def _redact(text: Any, enabled: bool) -> Any:
    ...
    Non-strings pass through untouched. Uses SOCIS' shared redactor with
...
def build_trace_jsonl(
    ...
    *,
    ...
    """Render SOCIS conversation messages as Claude Code JSONL text.
...
_NO_TOKEN_MESSAGE = (
    ...
    "2. Add it to your environment as HF_TOKEN (e.g. in ~/.socis-agent/.env):\n"
    "     HF_TOKEN=hf_xxxxxxxxxxxxxxxxxxxx\n"
    "3. Run /upload-trace again (or `socis trace upload`)."
)
...
def upload_session_trace(
    ...
    *,
    ...
    the user's private ``{user}/socis-traces`` dataset. Returns a
