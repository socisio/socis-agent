...
Write path is the CLI (``socis cron notepad <job_id> set <key> <value>``),
...
def _current_notepad_file() -> Path:
    return NOTEPAD_FILE or (get_socis_agent_home().resolve() / "cron" / "notepad.db")
...
def render_notepad_section(job_id: str) -> str:
    ...
    return (
        ...
        f"`socis cron notepad {job_id} set <key> <value>` "
        f"(also: get/delete/list; `socis cron notepad {job_id} delete "
