...
Uses a file-based lock (~/.socis-agent/cron/.tick.lock) so only one tick
...
# Without this, standalone invocations (e.g. after `socis update` reloads
# the module) fail with ModuleNotFoundError for socis_agent_time et al.
...
from socis_agent_time import now as _socis_now
...
def _fallback_chain_phrase() -> str:
    ...
    if chain:
        return "Fallback chain was exhausted or unavailable."
    return (
        "No fallback chain configured — add one with `socis fallback add`, "
...
def _failure_streak_nudge(job: dict) -> str:
    ...
    return (
        ...
        f"Fix its prompt/config, or pause it with `socis cron pause {job_ref}` "
...
class CronTickYielded(RuntimeError):
    ...
    (``socis cron status`` must not show green while jobs only fire from the
...
def _summarize_cron_failure_for_delivery(job: dict, error: str | None) -> str:
    ...
    if "skipped to prevent unintended spend: global inference config drifted" in lower:
        if "finite one-shot job is consumed" in lower:
            ...
            remediation = (
                "On the host running SOCIS, pin it explicitly: "
                f"`socis cron edit {job_id} --provider <provider> "
                "--model <model>`."
            )
        return (
    ...
    # checkout was updated underneath it (interrupted `socis update`, manual
    ...
    if provider_reachable and re.search(
        ...
        if skew is not None:
            ...
            message += (
                ...
                "`socis gateway restart` to fix it."
            )
...
def _merge_mcp_into_per_job_toolsets(per_job: list[str], cfg: dict) -> list[str]:
    ...
    if "no_mcp" in per_job:
        return result
    # lazy import: avoid heavy socis_cli import at cron module load (matches
    # _resolve_cron_enabled_toolsets' fallback) and share one MCP-membership
    # computation with the gateway/CLI platform resolver.
    from socis_cli.tools_config import enabled_mcp_server_names
...
def _resolve_cron_enabled_toolsets(job: dict, cfg: dict) -> list[str] | None:
    ...
    2. Per-platform ``socis tools`` config for the ``cron`` platform.
    ...
    try:
        from socis_cli.tools_config import _get_platform_tools  # lazy: avoid heavy import at cron module load
...
def _resolve_job_reasoning_config(job: dict, cfg: dict, model: str) -> dict | None:
    ...
    """
    from socis_agent_constants import parse_reasoning_effort, resolve_reasoning_config
...
def _inflight_min_allowance_minutes() -> float:
    ...
    ``SOCIS_AGENT_CRON_INFLIGHT_MAX_MINUTES`` env var is kept as an internal
    escape hatch only.
    """
    try:
        ...
        pass
    raw = os.getenv("SOCIS_AGENT_CRON_INFLIGHT_MAX_MINUTES", "").strip()
    if raw:
        try:
            val = float(raw)
            if val > 0:
            ...
            logger.warning(
                "Invalid SOCIS_AGENT_CRON_INFLIGHT_MAX_MINUTES=%r; using default %s",
                raw,
...
def _record_forced_release(job_id: str, name: str, age_seconds: float, allowance_seconds: float) -> None:
    ...
    entry = {
        ...
        "at": _socis_now().isoformat(),
    }
    with _running_lock:
        _forced_releases.append(entry)
        del _forced_releases[:-_FORCED_RELEASE_HISTORY]
    try:
        path = _get_socis_agent_home() / "cron" / "inflight_forced_releases.jsonl"
    ...
    *,
    ...
    with _running_lock:
        ...
        if only_owners is None:
            active_fires.extend(
                (None, job_id, None, _get_socis_agent_home())
                for job_id in (
...
def _cron_inactivity_seconds() -> float:
    """Parse SOCIS_AGENT_CRON_TIMEOUT (seconds). 0 = unlimited; bad input = 600.
    ...
    """
    raw = os.getenv("SOCIS_AGENT_CRON_TIMEOUT", "").strip()
    if not raw:
        return 600.0
    try:
        return float(raw)
    except (ValueError, TypeError):
        logger.warning("Invalid SOCIS_AGENT_CRON_TIMEOUT=%r; using default 600s", raw)
        return 600.0
...
# Resolves through _get_socis_agent_home() so profile-scoped paths work correctly.
def _usage_audit_path() -> Path:
    return _get_socis_agent_home() / "cron" / "usage_audit.jsonl"
...
def _write_usage_audit(record: dict) -> None:
    """Append a single JSONL line to ~/.socis-agent/cron/usage_audit.jsonl.
...
def _interpreter_shutting_down(exc: Optional[BaseException] = None) -> bool:
    ...
    ``socis update`` / ``socis gateway stop`` / systemd restart, or an
...
_socis_agent_home: Path | None = None


def _get_socis_agent_home() -> Path:
    """Resolve SOCIS home dynamically while preserving test monkeypatch hooks.

    Cron is per-profile by design (#4707): the in-process ticker runs inside a
    profile-scoped gateway, so resolving the active SOCIS_AGENT_HOME at call time
    ...
    """
    return _socis_agent_home or get_socis_agent_home()


def _get_lock_paths() -> tuple[Path, Path]:
    """Resolve cron lock paths at call time so profile/env changes are honored."""
    socis_agent_home = _get_socis_agent_home()
    lock_dir = socis_agent_home / "cron"
...
def cron_delivery_targets() -> list[dict]:
    ...
    try:
        from socis_cli.profiles import list_profile_names
...
def _deliver_to_bot_chat(job: dict, content: str, profile: str) -> Optional[str]:
    ...
    Runs ``socis [-p <profile>] chat --in ~ -c "Bot Chat" --create-if-missing
    ...
    scheduler's SOCIS_AGENT_HOME) or a validated local profile name.  Returns None
    ...
    if socis_bin:
        ...
        try:
            ...
            if _ilu.find_spec("socis_cli") is not None:
                argv = [sys.executable, "-m", "socis_cli.main"]
            else:
                return "bot-chat delivery failed: socis CLI not resolvable"
        except Exception:
            return "bot-chat delivery failed: socis CLI not resolvable"
    ...
    if profile:
        argv += ["-p", profile]
        # -p owns profile resolution in the child; a leftover SOCIS_AGENT_HOME
    ...
    try:
        with tempfile.NamedTemporaryFile(
            "w", encoding="utf-8", suffix=".txt", prefix="socis-cron-botchat-",
            delete=False,
        ) as fh:
            fh.write(message)
            query_file = fh.name
...
def _resolve_bot_chat_target(job: dict, profile_arg: str) -> Optional[dict]:
    ...
    ``profile_arg`` is ``""`` for the job's own profile (the SOCIS_AGENT_HOME
    ...
    ``~/.socis-agent/profiles/`` tree, so same-named profiles on other gateways
    can never be targeted by accident.
    """
    if not profile_arg:
        # Own profile: chat subprocess inherits SOCIS_AGENT_HOME, no name needed.
        return {"platform": BOT_CHAT_PLATFORM, "chat_id": "", "thread_id": None}
    try:
        from socis_cli.profiles import normalize_profile_name, profile_exists
    ...
    for media_path, _is_voice in media_files:
        try:
            ...
            try:
                ...
                # SOCIS_AGENT_CRON_MEDIA_SEND_TIMEOUT env override).
...
def _deliver_result(
    ...
    # another job in-process (``socis cron run``), and that nested delivery
    # must not be keyed under the outer execution id.
    external_execution = os.environ.get("_SOCIS_AGENT_CRON_EXTERNAL_WORKER", "")
    if (
        external_execution
    ...
    # ``last_delivery_unverified`` so `socis cron list` shows the state
    ...
    # at boot; a standalone process (manual `socis cron run` from the CLI,
    ...
    for target in targets:
        ...
        if transport is not None:
            ...
            # (`socis cron run`) has no live relay adapter, so surface the
...
def _get_script_timeout() -> int:
    ...
    env_value = os.getenv("SOCIS_AGENT_CRON_SCRIPT_TIMEOUT", "").strip()
    if env_value:
        try:
            ...
            logger.warning("Invalid SOCIS_AGENT_CRON_SCRIPT_TIMEOUT=%r; using config/default", env_value)
...
def _get_media_send_timeout() -> int:
    ...
    SOCIS_AGENT_CRON_MEDIA_SEND_TIMEOUT env var wins, then
    ...
    """
    env_value = os.getenv("SOCIS_AGENT_CRON_MEDIA_SEND_TIMEOUT", "").strip()
    if env_value:
        try:
            timeout = int(float(env_value))
            if timeout > 0:
            ...
            logger.warning(
                "Invalid SOCIS_AGENT_CRON_MEDIA_SEND_TIMEOUT=%r; using config/default",
                env_value,
            )
...
def _get_session_db_timeout() -> float:
    ...
    Mirrors the ``script_timeout_seconds`` resolution pattern: the
    SOCIS_AGENT_CRON_SESSION_DB_TIMEOUT env var wins, then
    ...
    """
    env_value = os.getenv("SOCIS_AGENT_CRON_SESSION_DB_TIMEOUT", "").strip()
    if env_value:
        try:
            return float(env_value)
        except (ValueError, TypeError):
            logger.warning(
                "Invalid SOCIS_AGENT_CRON_SESSION_DB_TIMEOUT=%r; using config/default",
                env_value,
            )
...
def _run_job_script(
    ...
    Scripts must reside within SOCIS_AGENT_HOME/scripts/.  Both relative and
    ...
    * anything else — run with the current Python interpreter
    ...
    provider credentials and other SOCIS-managed secrets are not inherited
    (SECURITY.md §2.3), matching terminal and MCP child processes.

    Args:
        script_path: Path to the script.  Relative paths are resolved
            against SOCIS_AGENT_HOME/scripts/.  Absolute and ~-prefixed paths
    ...
    """
    scripts_dir = _get_socis_agent_home() / "scripts"
    ...
    # escape — scripts MUST reside within SOCIS_AGENT_HOME/scripts/.
    try:
...
def _block_and_pause_job(
    ...
    now_iso = _socis_now().strftime("%Y-%m-%d %H:%M:%S")
    doc = (
        f"# Cron Job: {job_name}\n\n"
        f"**Job ID:** {job_id}\n"
        f"**Run Time:** {now_iso}\n"
...
def _preflight_check_provider_key(job: dict, cfg: dict) -> Optional[str]:
    ...
    model = job.get("model") or os.getenv("SOCIS_AGENT_MODEL") or ""
    ...
    try:
        ...
        return (
            ...
            "Set the provider API key in .env (or `socis setup`), or pin a "
            "working provider via `socis cron edit "
...
def _primary_profile_routes_for_current_home() -> list:
    ...
    """
    try:
        from socis_agent_constants import get_default_socis_root, get_socis_agent_home

        primary_home = get_default_socis_root()
        current_home = Path(get_socis_agent_home())
        if (
        ...
        from socis_cli.profiles import profile_matches_home
    ...
    for platform_name in platform_parts:
        ...
        if platform_name.lower() not in connected:
            ...
            return (
                ...
                "`socis setup` or change the job's `deliver` target."
            )
    return None
...
def run_job(
    ...
    *,
    ...
    # AIAgent or spend tokens. Escape hatch: SOCIS_AGENT_IGNORE_USER_CONFIG=1.
    ...
    if job.get("no_agent"):
        ...
        # "no delivery target resolved". load_socis_dotenv does not override
        # already-set vars, so the gateway's in-process tick is unaffected.
        try:
            from socis_cli.env_loader import load_socis_dotenv

            load_socis_dotenv(socis_agent_home=_get_socis_agent_home())
        except Exception:
            logger.debug(
    ...
    if job_has_monitor(job):
        ...
        _mon_now = _socis_now().strftime("%Y-%m-%d %H:%M:%S")
        if not _mon.ok:
    ...
    if script_path:
        ...
        if _ran_ok and not _parse_wake_gate(_script_output):
            ...
            silent_doc = (
                f"# Cron Job: {job_name}\n\n"
                f"**Job ID:** {job_id}\n"
                f"**Run Time:** {_socis_now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
    ...
    _cron_session_id = f"cron_{job_id}_{_socis_now().strftime('%Y%m%d_%H%M%S')}"
    ...
    # gateway message. Do not seed SOCIS_AGENT_SESSION_* contextvars from the
    ...
    #     (notify_on_complete / watch_patterns) reads SOCIS_AGENT_SESSION_PLATFORM
    #     and SOCIS_AGENT_SESSION_CHAT_ID to populate watcher_platform / chat_id,
    #     which would route completion notifications to the origin chat
    #     instead of via SOCIS_AGENT_CRON_AUTO_DELIVER_* below.
    #   - tools/tts_tool.py: picks Opus vs MP3 based on
    #     SOCIS_AGENT_SESSION_PLATFORM == "telegram".
    ...
    #     SOCIS_AGENT_SESSION_PLATFORM.
    #   - tools/send_message_tool.py: mirror source labelling and the
    #     send_message gate read SOCIS_AGENT_SESSION_PLATFORM.
    # Cron output delivery itself reads job["origin"] directly via
    # _resolve_origin(job) and the SOCIS_AGENT_CRON_AUTO_DELIVER_* vars set
    # below, so clearing SOCIS_AGENT_SESSION_* here does not affect delivery.
    ...
    _ctx_tokens = set_session_vars(
        ...
        # SOCIS_AGENT_SESSION_* routing keys just below, so an async delegation's
        ...
        # os.environ SOCIS_AGENT_SESSION_KEY, which risks routing a cron subagent's output
    ...
    _cron_delivery_vars = (
        "SOCIS_AGENT_CRON_AUTO_DELIVER_PLATFORM",
        "SOCIS_AGENT_CRON_AUTO_DELIVER_CHAT_ID",
        "SOCIS_AGENT_CRON_AUTO_DELIVER_THREAD_ID",
    )
    ...
    _cron_session_var = _VAR_MAP["SOCIS_AGENT_CRON_SESSION"]
    ...
    try:
        ...
        #
        # A kanban worker is a normal `socis chat -q` CLI agent whose default
        # toolset includes `cronjob`, running with SOCIS_AGENT_KANBAN_TASK
        ...
        # and kanban_complete defaults task_id to $SOCIS_AGENT_KANBAN_TASK -- letting
        ...
        # load_socis_dotenv (not a bare load_dotenv) and reset the secret-
        # source cache first: startup already applied external secrets and
        # recorded this SOCIS_AGENT_HOME in _APPLIED_HOMES, so a naive reload would
        ...
        # forced re-pull off the network. load_socis_dotenv also handles the
        # utf-8/latin-1 encoding fallback internally.
        from socis_cli.env_loader import (
            load_socis_dotenv,
        ...
        if delivery_target:
            _VAR_MAP["SOCIS_AGENT_CRON_AUTO_DELIVER_PLATFORM"].set(delivery_target["platform"])
            _VAR_MAP["SOCIS_AGENT_CRON_AUTO_DELIVER_CHAT_ID"].set(str(delivery_target["chat_id"]))
            _VAR_MAP["SOCIS_AGENT_CRON_AUTO_DELIVER_THREAD_ID"].set(
                ""
        ...
        # cron-fleet default) > SOCIS_AGENT_MODEL env > config.yaml ``model:``
        # (string or ``{default: ...}``). The per-job value is intentionally
        # re-read from storage every tick so a ``socis cron edit --model``
        ...
        try:
            ...
            _cfg_path = str(_get_socis_agent_home() / "config.yaml")
        ...
        if not (isinstance(model, str) and model.strip()):
            raise RuntimeError(
                ...
                f"SOCIS_AGENT_MODEL={os.getenv('SOCIS_AGENT_MODEL', '')!r}, "
                "config.yaml model.default missing or empty). "
                f"Set a per-job model via "
                f"`socis cron edit {job_id} --model <name>` or set a "
                "default with `socis model <name>`."
            )

        # Apply IPv4 preference if configured.
        try:
            from socis_agent_constants import apply_ipv4_preference
        ...
        prefill_file = (
            os.getenv("SOCIS_AGENT_PREFILL_MESSAGES_FILE", "")
        ...
        if prefill_file:
            ...
            if not pfpath.is_absolute():
                pfpath = _get_socis_agent_home() / pfpath
            if pfpath.exists():
                try:
        ...
        try:
            # Do not inject SOCIS_AGENT_INFERENCE_PROVIDER here. resolve_runtime_provider()
        ...
        if cron_model_drift_guard_enabled(_cfg):
            ...
            if _drift:
                ...
                if _finite_oneshot:
                    ...
                    _remediation = (
                        "To run on the new config, on the host running SOCIS "
                        "pin it explicitly: "
                        f"`socis cron edit {job_id} --provider <provider> "
        ...
        #
        # Bounded with its own timeout (separate from SOCIS_AGENT_CRON_TIMEOUT,
        ...
        agent = AIAgent(
            ...
            # SOCIS_AGENT_HOME. When a workdir is configured, also inject project
        ...
        # override via SOCIS_AGENT_CRON_TIMEOUT env var.  0 = unlimited.
        #
        ...
        if final_response.strip() and turn_exit_reason:
            ...
            _explainer_variants = []
            try:
                from socis_agent_state import PERSISTENCE_ERROR_CAUSES as _causes
            except Exception:
...
**Job ID:** {job_id}
**Run Time:** {_socis_now().strftime('%Y-%m-%d %H:%M:%S')}
...
"""
        ...
        if _session_db:
            ...
            try:
                ...
                _cron_title = f"{_title_base} · {_socis_now().strftime('%b %d %H:%M')}"
            ...
            # socis_agent_state's session_lifecycle_statuses docstring — keep the
...
def run_one_job(
    ...
    *,
    ...
    external_owner = os.environ.get("_SOCIS_AGENT_CRON_EXTERNAL_WORKER") == execution_id
    ...
    if extra_prompt is None:
        # A gateway-forwarded manual run (`socis cron run --prompt` /
    ...
    profile_home = _get_socis_agent_home().resolve()
    with _running_lock:
    ...
    *,
    ...
    try:
        ...
        _scope_token = set_secret_scope(
            build_profile_secret_scope(_get_socis_agent_home())
        )
        ...
        if delivery_error:
            ...
            # exact signature via `socis cron incidents ack`.
...
def _launch_external_cron_worker(job: dict) -> bool:
    ...
    execution_id = str(job["execution_id"])
    job_id = str(job["id"])
    handoff_dir = _get_socis_agent_home() / "cron" / "external-workers"
    ...
    try:
        with os.fdopen(fd, "w", encoding="utf-8") as payload_file:
            json.dump(
                {
                    "job": job,
                    "profile_home": str(_get_socis_agent_home().resolve()),
    ...
    worker_env = build_subprocess_env(
        ...
        extra={"SOCIS_AGENT_HOME": str(_get_socis_agent_home().resolve())},
    )
    try:
        process = subprocess.Popen(
            scoped_command,
...
def _run_external_worker_payload(payload_path: Path, ack_path: Path) -> bool:
    ...
    from socis_cli.env_loader import hydrate_profile_secret_sources
    ...
    try:
        with use_cron_store(profile_home):
            ...
            try:
                ...
                return False
            old_external_execution = os.environ.get("_SOCIS_AGENT_CRON_EXTERNAL_WORKER")
            os.environ["_SOCIS_AGENT_CRON_EXTERNAL_WORKER"] = execution_id
            try:
                return run_one_job(job, adapters=None, loop=None, verbose=False)
            finally:
                if old_external_execution is None:
                    os.environ.pop("_SOCIS_AGENT_CRON_EXTERNAL_WORKER", None)
                else:
                    os.environ["_SOCIS_AGENT_CRON_EXTERNAL_WORKER"] = old_external_execution
    finally:
...
# `socis -w` launches, so on gateway-driven boxes (where sessions arrive via
...
def _worktree_maintenance_repos() -> List[str]:
    ...
    Candidates: the socis install checkout itself (where ``socis -w``
    ...
    # The socis source checkout (editable/git installs). Wheel installs have
...
def _maybe_run_worktree_maintenance() -> None:
    ...
    ``socis -w`` startup path uses — dirty/unpushed/live-locked trees are
...
def tick(
    ...
    *,
    ...
    # ``git pull``, interrupted ``socis update``) serves MIXED sys.modules:
    ...
    try:
        # Global emergency stop (`socis pause`): skip dispatch entirely while
        # the ESTOP sentinel exists. Never touches in-flight runs — due jobs
        # simply wait for the next tick after `socis resume`. Logged once per
        ...
        # scheduler STARTUP. A one-shot `socis cron run` that claimed a job
        ...
        # boxes never hit the `socis -w` startup pruner, so this is the only
        ...
        if not due_jobs:
            ...
            if verbose:
                logger.info("%s - No jobs due", _socis_now().strftime('%H:%M:%S'))
            try:
        ...
        if verbose:
            logger.info("%s - %s job(s) due", _socis_now().strftime('%H:%M:%S'), len(due_jobs))
        ...
        # Set SOCIS_AGENT_CRON_MAX_PARALLEL=1 to restore old serial behaviour.
        _max_workers: Optional[int] = None
        try:
            _env_par = os.getenv("SOCIS_AGENT_CRON_MAX_PARALLEL", "").strip()
            if _env_par:
                _max_workers = int(_env_par) or None
        except (ValueError, TypeError):
            logger.warning("Invalid SOCIS_AGENT_CRON_MAX_PARALLEL value; defaulting to unbounded")
...
if __name__ == "__main__":
    if "--external-worker-file" in sys.argv:
        ...
        try:
            from socis_agent_logging import setup_logging

            setup_logging(socis_agent_home=_get_socis_agent_home(), mode="cron")
        except Exception:
            pass
        raise SystemExit(
