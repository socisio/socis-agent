...
A *suggestion* is a ready-to-run cron job spec that SOCIS surfaces to the
...
Storage mirrors ``cron/jobs.py``: ``~/.socis-agent/cron/suggestions.json``, atomic
...
# profile's cron store. Anchor on get_socis_agent_home() (profile home), not the
...
# multiplexed profile ticks (set_socis_agent_home_override) cannot leak one
...
def _current_suggestions_file() -> Path:
    return SUGGESTIONS_FILE or (get_socis_agent_home().resolve() / "cron" / "suggestions.json")
...
def _save_raw(suggestions: List[Dict[str, Any]]) -> None:
    ...
    try:
        with os.fdopen(fd, "w", encoding="utf-8") as f:
            json.dump(
                {"suggestions": suggestions, "updated_at": _socis_now().isoformat()},
                f,
                indent=2,
            )
            f.flush()
    ...
    *,
    ...
    with _suggestions_lock:
        ...
        record = {
            ...
            "created_at": _socis_now().isoformat(),
        }
    ...
    with _suggestions_lock:
        ...
        for s in suggestions:
            if s.get("id") == suggestion_id:
                s["status"] = status
                s["resolved_at"] = _socis_now().isoformat()
                changed = True
                break
        if changed:
            _save_raw(suggestions)
