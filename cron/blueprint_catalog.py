...
renders natively:
  ...
  * Docs catalog          -> a copy-paste command + a ``socis://`` deep-link
...
def blueprint_deeplink(blueprint: AutomationBlueprint, values: Optional[Dict[str, Any]] = None) -> str:
    """Build the ``socis://blueprint/<key>?slot=val`` deep-link URL."""
    ...
    return f"socis://blueprint/{quote(blueprint.key)}{qs}"
