...
# 5. Symlinks the 'socis' CLI command into a user-facing bin dir
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/socis-uv-install.$$.log")"
...
if is_termux; then
    # SOCIS currently declares requires-python >=3.11,<3.14. Termux can expose
    ...
    if [ -z "${PYTHON_PATH:-}" ]; then
        if command -v python >/dev/null 2>&1; then
            ...
            echo -e "${RED}✗${NC} Termux Python $PYTHON_FOUND_VERSION is not supported; SOCIS requires Python >=3.11,<3.14"
...
# ============================================================================
# PATH setup — symlink socis into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up socis command..."

SOCIS_AGENT_BIN="$SCRIPT_DIR/venv/bin/socis"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$SOCIS_AGENT_BIN" "$COMMAND_LINK_DIR/socis"
echo -e "${GREEN}✓${NC} Symlinked socis → $COMMAND_LINK_DISPLAY_DIR/socis"
...
echo ""
echo "Syncing bundled skills to ~/.socis-agent/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$SOCIS_AGENT_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  socis status        # Check configuration"
if is_termux; then
    echo "  socis gateway       # Run gateway in foreground"
else
    echo "  socis gateway install # Install gateway service (messaging + cron)"
fi
echo "  socis cron list     # View scheduled jobs"
echo "  socis doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m socis_cli.main setup
fi
