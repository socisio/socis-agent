...
``socis_agent_state.SessionDB``. It defines no ``__init__`` and no state of its
...
``SessionDB.__init__``. It must never import socis_agent_state (cycle) — shared
module-level constants live in socis_agent_state_common.
"""
...
# Moved methods logged under the "socis_agent_state" logger before the split;
# keep that logger identity so log filtering/capture behavior is unchanged.
logger = logging.getLogger("socis_agent_state")
...
def schema_read_probe_statements() -> tuple:
    ...
    ``socis_cli/web_server.py``) run these probes right after a read-only
    ...
    served an empty session list after `socis update` until the user's
...
class SessionSchemaMixin:
    ...
    def _sqlite_supports_fts5(self, cursor: sqlite3.Cursor) -> bool:
        try:
            cursor.execute("CREATE VIRTUAL TABLE temp._socis_fts5_probe USING fts5(x)")
            cursor.execute("DROP TABLE temp._socis_fts5_probe")
            return True
        except sqlite3.OperationalError as exc:
    ...
    def _migrate_trigram_cron_exclusion(self, cursor: sqlite3.Cursor) -> bool:
        ...
        if self._db_has_trigram_tool_calls_projection(cursor):
            ...
            # ``socis sessions optimize-storage`` path (it recreates the
            ...
            if escalated:
                ...
                if foreign_holders:
                    logger.error(
                        ...
                        "`socis sessions optimize-storage` with the gateway stopped. "
                        "`socis doctor` reports this degraded state.",
                        attempts,
                        foreign_holders,
                    )
    ...
    def _parse_schema_columns(schema_sql: str) -> Dict[str, Dict[str, str]]:
        ...
        try:
            from socis_agent_constants import get_socis_agent_home
            cache_path = get_socis_agent_home() / "cache" / "schema_columns.json"
    ...
    def _init_schema(self):
        ...
        if row is None:
            ...
            if current_version < 23:
                ...
                # optimization is available. `socis sessions optimize-storage`
        ...
        if fts5_available:
            ...
            # not yet opted into `socis db optimize`) must keep its EXISTING
