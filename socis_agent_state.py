#!/usr/bin/env python3
"""
SQLite State Store for SOCIS Agent.
...
# transitive dependency via socis_agent_state_common). run_agent keeps its own
# predating copy — socis_agent_state cannot import run_agent (circular) — guarded
...
from socis_agent_startup_watchdog import report_startup_progress
from socis_cli.sqlite_runtime import (
    is_sqlite_wal_reset_vulnerable as _is_sqlite_wal_reset_vulnerable,
)
from typing import Any, Callable, Dict, Iterator, List, Optional, Set, Tuple, TypeVar, cast

import socis_agent_state_holders as _state_holders
from socis_agent_state_common import (  # noqa: F401  (re-exported for back-compat)
    _BRANCH_CHILD_SQL,
...
from socis_agent_state_portability import SessionPortabilityMixin
from socis_agent_state_schema import SessionSchemaMixin
from socis_agent_state_search import SessionSearchMixin
...
def _workspace_key_clause(key: str) -> Tuple[str, List[str]]:
    ...
    Used by ``socis -c``/``--resume`` to continue the most recent session in
...
DEFAULT_DB_PATH = get_socis_agent_home() / "state.db"
...
#
# The ceilings above bound SOCIS's SQLite descriptors, which is only ever part
...
def _default_db_path() -> Path:
    ...
    freezes the developer's real ``~/.socis-agent`` even when a test fixture later
    redirects ``SOCIS_AGENT_HOME`` — importing this module during collection was
    ...
    2. Otherwise resolve ``get_socis_agent_home()`` fresh so a runtime
       ``SOCIS_AGENT_HOME`` redirect takes effect regardless of import order.
    """
    if DEFAULT_DB_PATH != _IMPORT_DEFAULT_DB_PATH:
        return DEFAULT_DB_PATH
    return get_socis_agent_home() / "state.db"
...
# ~/.socis-agent/state.db accumulated pytest fixture rows — sessions with
...
# FTS write corruption").  The hermetic conftest redirects SOCIS_AGENT_HOME per
# test, but any escape (a session-scoped fixture running before the autouse
# fixture, a subprocess child launched without SOCIS_AGENT_HOME, a stale worktree
# without the re-pin, or a developer shell that exports SOCIS_AGENT_HOME to the
...
_STATE_DB_GUARD_BYPASS_ENV = "SOCIS_AGENT_STATE_DB_GUARD_BYPASS"

#: Additional production roots to refuse (beyond the platform default
#: ``~/.socis-agent``).  The test conftest injects the pre-sandbox production
#: root here so custom-``SOCIS_AGENT_HOME`` deployments are covered too.
_STATE_DB_GUARD_EXTRA_DENY_ROOTS: Tuple[Path, ...] = ()


def _real_platform_state_root() -> Optional[Path]:
    """Resolve the REAL platform-default SOCIS root for the guard.

    Deliberately avoids ``Path.home()`` / ``socis_agent_constants``: tests
    routinely monkeypatch ``Path.home`` to a tempdir, and ``socis_agent_state``
    ...
    try:
        if sys.platform == "win32":
            ...
            root = (
                Path(base) / "socis"
                if base
                else Path(os.path.expanduser("~")) / "AppData" / "Local" / "socis"
            )
        else:
            root = Path(os.path.expanduser("~")) / ".socis-agent"
        return root.resolve()
    except Exception:
        return None
...
#: redirects ``SOCIS_AGENT_HOME`` to the per-session tmp isolation root.  Its
...
#: is OURS: it declares "this process tree is running under SOCIS test
#: isolation", and it inherits into subprocess children by default — so a
#: child that received the patched ``SOCIS_AGENT_HOME`` also received the marker,
#: and a child that resolves a production DB while carrying it is, by
#: definition, an isolation escape (#82770).
_TEST_ISOLATION_MARKER_ENV = "SOCIS_AGENT_TEST_ISOLATION"
...
def _has_pytest_ancestor() -> bool:
    ...
    ``SOCIS_AGENT_HOME`` redirect: that child aims at the production DB *and*
...
def _in_test_context() -> bool:
    ...
    so a real ``socis`` invocation pays for at most one walk.
    """
...
def _is_production_state_db(resolved: Path, root: Path) -> bool:
    """True when *resolved* is a DB file of the real SOCIS home *root*.
    ...
    under ``~/.socis-agent/socis-agent/...``) so hermetic tests using unusual
...
def _ensure_test_isolation(db_path: Path) -> None:
    ...
    pytest and for hermetic (tmp ``SOCIS_AGENT_HOME``) paths.
    ...
    a rebuilt environment loses ``PYTEST_*`` and ``SOCIS_AGENT_HOME`` together,
    ...
    for root in _production_state_roots():
        if _is_production_state_db(resolved, root):
            raise RuntimeError(
                ...
                f"state.db at {resolved} (under real SOCIS root {root}). "
                "Tests must run against a temporary SOCIS_AGENT_HOME — pass an "
                "explicit tmp db_path or let the hermetic conftest redirect "
                "SOCIS_AGENT_HOME. If this test genuinely needs the live "
...
# socis_cli/kanban_db.py for ~30 call sites) would re-log the same
...
def _apply_wal_size_limit(conn: sqlite3.Connection) -> None:
    ...
    3.0 GB ``state.db``: ``socis sessions optimize`` (FTS merge + VACUUM)
    ...
    ``socis_cli/kanban_db.py`` already bounds its WAL growth with
...
def apply_wal_with_fallback(
    ...
    *,
    ...
    Shared by :class:`SessionDB` and ``socis_cli.kanban_db.connect`` so
...
def _wal_reset_repair_hint() -> str:
    ...
    ``socis update`` can actually do for this install (#75153).
    """
    try:
        ...
        if method in {"git", "unknown"}:
            return f"SOCIS-managed installs can repair the embedded runtime with `{cmd}`"
        if method == "docker":
    ...
    return (
        ...
        "(or backports 3.50.7 / 3.44.6) and restart SOCIS"
    )
...
def _log_wal_reset_bug_once(
    ...
    *,
    ...
    # Check whether this is a SOCIS-managed install (uv-managed venv)
    ...
    logger.warning(
        ...
        "%s. See `socis doctor`. This warning fires once per "
...
def _log_journal_mode_upgrade_once(db_label: str, previous_mode: str) -> None:
    ...
    ``socis_cli/managed_uv._default_live_venv``, which treats a database
...
def _log_wal_fallback_once(db_label: str, exc: Exception) -> None:
    ...
    connection on every operation — see socis_cli/kanban_db.py) would
...
def apply_database_pragmas(
    ...
    * ``synchronous`` — durability level: ``OFF``/``NORMAL``/``FULL``/``EXTRA``
    ...
    """
    try:
        # Local import avoids a circular import with socis_cli.config.
...
def classify_persistence_error(exc_or_str) -> str:
    ...
    * ``"corrupt"`` — the database file itself is structurally damaged
      ...
      (``socis doctor`` / automatic schema surgery).
...
# threads inside ONE interpreter, yet a normal SOCIS host runs several
# independent processes against the same ``state.db``: the gateway service,
# the Desktop app's own ``socis serve`` backend, interactive CLI sessions,
...
# included because rollback-journal (DELETE) mode — SOCIS's fallback on
...
def _db_fingerprint(db_path: Path) -> "Optional[str]":
    ...
    RESERVED lock (see ``socis_cli.sqlite_safe_read`` rule 1). This function
    ...
    try:
        ...
        try:
            ...
            # Scaffold/embed installs ship socis_agent_state without socis_cli. No
...
def _persistent_repair_exhausted_error(db_path: Path) -> str:
    ...
    return (
        ...
        "a backup, or salvage with `socis sessions recover --source "
        f"{db_path} --inspect-only`, then (if it reports recoverable) "
        f"`socis sessions recover --source {db_path} --output "
...
def _backup_db_file(db_path: Path) -> "Tuple[Optional[Path], Optional[str]]":
    ...
    connection's POSIX advisory locks (see ``socis_cli.sqlite_safe_read``).
        ...
        try:
            ...
            if usage.free - need < headroom:
                reason = (
                    ...
                    f"`socis sessions recover --source {db_path} "
                    "--inspect-only` first)."
                )
            ...
            reason = (
                ...
                f"with `socis sessions recover --source {db_path} "
                "--inspect-only` first)."
            )
            logger.error("Refusing forensic backup of %s: %s", db_path, reason)
            return None, reason
...
def preflight_db_writability(
    ...
    *,
    ...
    - **Repairs** permissions with ``chmod u+rw`` when the file lives inside
      the SOCIS home tree (``get_socis_agent_home()``) — the safe repair scope:
      SOCIS owns those files, and the OS makes ``chmod`` fail on files the
    ...
    to check). Shared by :class:`SessionDB` and ``socis_cli.kanban_db``.
    """
    raw = str(db_path)
    ...
    try:
        home: Optional[Path] = Path(get_socis_agent_home()).resolve()
    ...
    def _ensure_writable(p: Path, *, is_dir: bool = False) -> None:
        ...
        raise sqlite3.OperationalError(
            ...
            f"user. SOCIS needs read-write access to open the database. "
    ...
    try:
        ...
        for fts_table in ("messages_fts", "messages_fts_trigram", "messages_fts_cjk"):
            try:
                ...
                # (socis_agent_state.py:645-723). The supported degraded-runtime
                # path (SessionDB._is_fts5_unavailable_error + the
                # regression suite in tests/test_socis_agent_state.py:600-632)
        ...
        probe_session_id = f"_socis_fts_health_probe_{time.time_ns()}"
        try:
        ...
        if not holding_lock:
            ...
            # later BEGIN EXCLUSIVE acquisition.
                ...
                report["error"] = (
                    ...
                    "Stop the gateway (socis gateway stop) and retry."
                )
                logger.error("state.db repair skipped: %s", report["error"])
            else:
...
# (``~/.socis-agent/lib/libfts5_cjk.so``, built by ``native/fts5_cjk/build.sh``).
...
# next ``socis sessions optimize-storage`` on a capable host).
#
...
def fts5_cjk_so_path() -> Path:
    ...
    env = os.getenv("SOCIS_AGENT_FTS5_CJK_SO")
    if env:
        return Path(env).expanduser()
    return get_socis_agent_home() / "lib" / "libfts5_cjk.so"


def _cjk_fts_config_enabled() -> bool:
    """config.yaml ``sessions.cjk_fts`` (default on), via its env bridge."""
    return os.getenv("SOCIS_AGENT_CJK_FTS", "1").strip().lower() not in (
        "0", "false", "off", "no",
    )
...
_STATE_DB_CORRUPT_MSG = (
    ...
    "Stop the gateway, then run `socis sessions recover --source <state.db> "
...
def divert_session_transcript_jsonl(session_id: str, messages) -> "Optional[Path]":
    """Append pending messages as JSON lines under SOCIS_AGENT_HOME/sessions.
...
# (see socis_cli/sqlite_safe_read.py for the module built around this rule).
...
#
# The registry itself lives in socis_agent_state_registry.py — a bounded
# module owning acquisition, generation identity, refcounting,
# retirement, and teardown.  These re-exports keep the historical
# import path (``from socis_agent_state import get_shared_session_db``)
# working for every call site and test that imports from here.
#
# Routing rules (see socis_agent_state_registry for the full lifecycle):
...
from socis_agent_state_registry import (  # noqa: F401  (re-export)
...
def _connect_tracked_db(path, tracking_path=None, **kwargs):
    ...
    (scaffold/embed installs that ship socis_agent_state without socis_cli). A
    ...
    try:
        ...
        logger.debug(
            "socis_cli.sqlite_safe_read unavailable; opening %s untracked "
    ...
    # patch socis_agent_state.sqlite3.connect keep control of connection creation;
...
def is_zeroed_state_db(
    ...
    Prefer ``socis_cli.backup.is_zeroed_sqlite_file`` when available; this
    ...
    if size < 0:
        return False
    from socis_cli.sqlite_safe_read import has_live_connection, read_header_bytes_preopen
    ...
    with quarantine_cross_process_lock(path) as acquired:
        if not acquired:
            logger.error(
                ...
                "state-snapshots via `socis snapshot list` / "
                "`socis snapshot restore <id>`.",
                path,
            )
            return None
        return _do_quarantine()
...
# ── Read-only health/stats probes (socis doctor, dashboards) ──────────
...
class SessionDB(SessionSearchMixin, SessionSchemaMixin, SessionPortabilityMixin):
    ...
    # With multiple socis processes (gateway + CLI sessions + worktree agents)
    ...
    # legitimately held for multi-second stretches by sibling SOCIS
    ...
    # `socis update` leaves mixed-version processes sharing the DB until
    ...
    def __init__(self, db_path: Path = None, read_only: bool = False):
        ...
        try:
            ...
            def _handle_quarantine_if_zeroed(already_locked: bool = False):
                if (
                    ...
                    msg = (
                        ...
                        f"Restore from {snaps} via `socis snapshot list` / "
                        f"`socis snapshot restore <id>` if available. "
            ...
            # NOTE: the v23 FTS optimization is OPT-IN (`socis db optimize`),
            ...
            # ``socis_agent_state._set_last_init_error(None)`` explicitly.
    ...
    def _warn_fts5_unavailable(self, exc: sqlite3.OperationalError) -> None:
        ...
        logger.warning(
            ...
            "disabled. Run `socis update` to rebuild the venv with a "
                ...
                if cjk_present:
                    ...
                    if live:
                        ...
                        logger.warning(
                            ...
                            "run `socis sessions optimize-storage` on a host "
        ...
        while True:
            ...
            try:
                ...
                if "locked" in err_msg or "busy" in err_msg:
                    ...
                    raise sqlite3.OperationalError(
                        f"database is locked (another SOCIS process held the "
    ...
    def _db_file_was_replaced(self) -> bool:
        ...
        if recorded_app:
            ...
            # replace. A copied SOCIS DB that minted its own id is nonzero.
    ...
    def _halt_db_corrupt(self, exc: BaseException) -> None:
        ...
        logger.error(
            ...
            "gateway and run `socis sessions recover --source %s "
            "--inspect-only`.",
            self.db_path,
            exc,
            self.db_path,
    ...
    def _reap_inactive_orphan_desktop_holders(
        ...
        if not sys.platform.startswith("linux") or psutil is None:
            return []
        try:
            from socis_cli.dashboard_procs import _is_ephemeral_port_zero_backend
        except Exception:
            return []
    ...
    def close(self):
        ...
        if getattr(self, "_shared_registry_owned", False):
            from socis_agent_state_registry import release
            ...
            if self._conn:
                if self._db_corrupt:
                    ...
                    logger.warning(
                        ...
                        "then run `socis sessions recover --source %s "
                        "--inspect-only`.",
                        self.db_path,
                        self._db_corrupt_reason,
                        self.db_path,
    ...
    #
    # `optimize_fts_storage()` (the `socis sessions optimize-storage`
    ...
    # ── Opt-in v23 FTS storage optimization (`socis sessions optimize-storage`) ──
    #
    ...
    def _own_profile_name(self) -> Optional[str]:
        ...
        try:
            ...
            root = get_default_socis_root().resolve()
    ...
    # offline repair path behind ``socis sessions repair-routing``.
    ...
    def set_session_yolo(self, session_id: str, enabled: bool) -> None:
        ...
        ``--yolo`` launch — survives ``socis --resume`` into a fresh
    ...
    # of truth: socis_agent_state_common._RECOVERABLE_END_REASONS, interpolated
    ...
    def _rows_to_conversation(
        ...
        *,
        ...
        if repair_alternation and messages:
            # Lazy import: socis_agent_state already depends on agent.* (see
    ...
    def search_sessions(
        ...
        ``socis -c``/``--resume`` so the "last" session is the last one in
    ...
    def latest_conversation_boundary(
        ...
        uses — never the key alone.  ``X-SOCIS-Session-Key`` accepts any
    ...
    def delete_session_if_empty(
        ...
        and ``socis sessions list`` output. (Pattern ported from
    ...
    def retag_kanban_worker_sessions(self, workspaces_root: str) -> int:
        ...
        Workers used to spawn without ``SOCIS_AGENT_SESSION_SOURCE``, so their runs
    ...
    def apply_telegram_topic_migration(self) -> None:
        ...
        reconciliation. Operators must be able to upgrade SOCIS, keep the old
    ...
    def bind_telegram_topic(
        ...
        *,
        ...
        """Bind one Telegram DM topic thread to one SOCIS session.

        A SOCIS session may only be linked to one Telegram topic in MVP.
    ...
    def is_telegram_session_linked_to_topic(self, *, session_id: str) -> bool:
        """Return True if a SOCIS session is already bound to any Telegram DM topic.
    ...
    def vacuum(self) -> int:
        ...
        with self._lock:
            ...
            # TRUNCATE: a manual `socis sessions vacuum` runs in a transient
