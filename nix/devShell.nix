...
{ ... }:
{
  perSystem =
    ...
    let
      ...
      socisNpmLib = self'.packages.default.passthru.socisNpmLib;
      ...
      socisAgentDevShellHook = self'.packages.default.passthru.devShellHook;
    in
    {
      devShells.default = pkgs.mkShell {
        packages = with pkgs; [
          (pkgs.runCommand "socis" { } ''
            mkdir -p $out/bin
            install -Dm755 ${../socis} $out/bin/socis
          '')
        ...
        shellHook = ''
          ${socisAgentDevShellHook}
          ${socisNpmLib.mkNpmDevShellHook npmPackageJsonPaths}
          ...
          export SOCIS_AGENT_PYTHON_SRC_ROOT=$(git rev-parse --show-toplevel)
          ...
          echo "SOCIS Agent dev shell in $SOCIS_AGENT_PYTHON_SRC_ROOT"
          echo "Ready. Run 'socis' or 'sandbox socis' to start."
        '';
      };
    };
}
