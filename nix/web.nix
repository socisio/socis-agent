# nix/web.nix — SOCIS Web Dashboard (Vite/React) frontend build
{ socisNpmLib, ... }:
socisNpmLib.buildNpmPackage {
  dirs = [
    "web"

    # @socis/shared ships as a file: workspace dep of web, so its source
  ...
  buildPhase = ''
    ...
    # outDir in vite.config.ts points to ../socis_cli/web_dist for the
