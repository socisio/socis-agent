...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      socis-agent = self'.packages.default;
      socisVenv = socis-agent.socisVenv;
      ...
      evalHomeModule =
        ...
        inputs.home-manager.lib.homeManagerConfiguration {
          ...
          modules = [
            ...
            {
              home = {
                username = "socis-check";
                homeDirectory = "/home/socis-check";
                stateVersion = "24.11";
              };
            }
            { services.socis-agent = settings; }
      ...
      evalHomeSplit =
        ...
        inputs.home-manager.lib.homeManagerConfiguration {
          ...
          modules = [
            ...
            {
              programs.socis-agent = programs;
              services.socis-agent = services;
            }
          ];
        };

      # The option names that each module defines under
      # services.socis-agent. The internal names that the module system adds
      # are not in the list.
      moduleOptionNames =
        eval: lib.attrNames (lib.filterAttrs (n: _: !lib.hasPrefix "_" n) eval.options.services.socis-agent);
      ...
      configKeys = pkgs.runCommand "socis-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${socisVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "socis-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "socis-build-package" { } ''
          echo "PASS: package built at ${socis-agent}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "socis-build-devshell" { } ''
          ...
          let
            enabled = evalHomeSplit {
              ...
              services = {
                ...
                environment.SOCIS_AGENT_TEST = "1";
                environmentFiles = [ "/run/secrets/socis-env" ];
                socisHomeFiles."SOUL.md" = "test soul";
            ...
            processes =
              if pkgs.stdenv.hostPlatform.isDarwin then
                ...
                }) (lib.filterAttrs (n: _: lib.hasPrefix "socis" n) cfg.launchd.agents)
              else
                lib.mapAttrs (_: unit: {
                  argv = [ unit.Service.ExecStart ];
                  env = unit.Service.Environment;
                }) (lib.filterAttrs (n: _: lib.hasPrefix "socis" n) cfg.systemd.user.services);
            ...
            activation = cfg.home.activation.socisAgentSetup.data;

            failures =
              lib.optional (names != [
                "socis-agent"
                "socis-backend"
              ]) "expected socis-agent + socis-backend processes, got: ${toString names}"
              ++ lib.optional (
                !lib.hasInfix "bin/socis gateway" (argvOf "socis-agent")
              ) "gateway process does not run `socis gateway`: ${argvOf "socis-agent"}"
              ++ lib.optional (
                !lib.hasInfix "bin/socis serve" (argvOf "socis-backend")
              ) "backend process does not run `socis serve`: ${argvOf "socis-backend"}"
              ++ lib.optional (
                !lib.hasInfix "--no-open" (argvOf "socis-backend")
              ) "backend must pass --no-open so a service never opens a browser"
              ++ lib.optional (
                lib.any (n: !lib.hasInfix "/home/socis-check/.socis" (envOf n)) names
              ) "gateway and backend must share one SOCIS_AGENT_HOME"
              ++ lib.optional (
                cfg.home.sessionVariables.SOCIS_AGENT_HOME or null != "/home/socis-check/.socis"
              ) "programs.socis-agent.enable must export SOCIS_AGENT_HOME for interactive shells"
              ++ lib.optional (
                !lib.hasInfix "socis-config-merge" activation
              ) "activation must deep-merge config.yaml, not overwrite it"
              ++ lib.optional (
                !lib.hasInfix "/home/socis-check/.socis-agent/SOUL.md" activation
              ) "socisHomeFiles must install into SOCIS_AGENT_HOME"
              ++ lib.optional (
                !lib.hasInfix "/home/test-user/workspace/AGENTS.md" activation
              ) "documents must install into workingDirectory"
              # The CLI reads SOCIS_AGENT_MANAGED to name the rebuild command when
              # it refuses to write the configuration. A Home Manager install
              # has no nixos-rebuild command. Thus it must not report NixOS.
              ++ lib.optional (
                !lib.any (n: lib.hasInfix "SOCIS_AGENT_MANAGED=home-manager" (envOf n)) names
              ) "processes must report SOCIS_AGENT_MANAGED=home-manager"
              ++ lib.optional (
                !lib.hasInfix "socis-managed" activation
              ) "activation must write a .managed marker naming the managing system";
          in
          pkgs.runCommand "socis-home-manager-module" { } (
            if failures != [ ] then
        ...
        workspace-files-need-a-directory =
          let
            ...
            # the priority.
            sameAsDefault = "/home/socis-check";
            ...
            cases = [
              ...
              {
                name = "socisHomeFiles needs no directory";
                ok = accepts { socisHomeFiles."SOUL.md" = "x"; };
              }
              {
          ...
          in
          pkgs.runCommand "socis-workspace-files-need-a-directory" { } (
            if failed != [ ] then
        ...
        # ── The desktop application shares one SOCIS_AGENT_HOME ───────────────
        # `programs.enable` exports SOCIS_AGENT_HOME with home.sessionVariables,
        ...
        # no shell profile. Thus the desktop application would open ~/.socis-agent
        # while the services use the SOCIS_AGENT_HOME of the module, and the user
        ...
        home-manager-desktop =
          let
            tokenFile = "/run/secrets/socis-desktop-token";
            ...
            enabled = evalHomeSplit {
              ...
              services = {
                enable = true;
                socisHome = "/home/socis-check/.socis-work";
            ...
            desktopPackages = builtins.filter (p: (p.pname or "") == "socis-desktop") cfg.home.packages;
            ...
            # second SOCIS runtime beside the services is the fault that
            # `programs.enable` plus a plain desktop package would give.
            agentPackages = builtins.filter (p: (p.pname or "") == "socis-agent") cfg.home.packages;
            ...
            backendScript =
              let
                argv =
                  if pkgs.stdenv.hostPlatform.isDarwin then
                    cfg.launchd.agents.socis-backend.config.ProgramArguments
                  else
                    [ cfg.systemd.user.services.socis-backend.Service.ExecStart ];
            ...
            failures =
              ...
              ) "programs.desktop.enable must install exactly one socis-desktop package, got ${toString (lib.length desktopPackages)}"
              ++ lib.optional (
                setValue "SOCIS_AGENT_HOME" != "/home/socis-check/.socis-work"
              ) "the launcher must carry SOCIS_AGENT_HOME: a GUI launcher reads no shell profile, so home.sessionVariables never reaches it (got: ${toString (setValue "SOCIS_AGENT_HOME")})"
              ++ lib.optional (
                setValue "SOCIS_AGENT_MANAGED" != "home-manager"
              ) "the launcher must report SOCIS_AGENT_MANAGED=home-manager while the services own the configuration (got: ${toString (setValue "SOCIS_AGENT_MANAGED")})"
              ++ lib.optional (
                lib.length agentPackages == 1
                && setValue "SOCIS_AGENT_DESKTOP_SOCIS" != "${lib.head agentPackages}/bin/socis"
              ) "the launcher must pin the agent package that programs.enable installs, and not a second runtime: ${toString (setValue "SOCIS_AGENT_DESKTOP_SOCIS")}"

              # ── The application reaches the backend of the service ──────
              ++ lib.optional (
                setValue "SOCIS_AGENT_DESKTOP_REMOTE_URL" != "http://127.0.0.1:9231"
              ) "the launcher must name the backend of the service, or the application starts a second one (got: ${toString (setValue "SOCIS_AGENT_DESKTOP_REMOTE_URL")})"
              ++ lib.optional (
                !lib.hasInfix "SOCIS_AGENT_DESKTOP_REMOTE_TOKEN" wrapper
              ) "the launcher must give a token with the URL: the desktop resolver throws when the URL is set alone"
              ++ lib.optional (
                !lib.hasInfix "SOCIS_AGENT_DASHBOARD_SESSION_TOKEN" backendScript
              ...
              ++ lib.optional (
                setValue "SOCIS_AGENT_DESKTOP_REMOTE_TOKEN" != null
              ) "the token must never be a --set value: makeWrapper writes it into the world-readable Nix store";
          in
          pkgs.runCommand "socis-home-manager-desktop" { } (
            if failures != [ ] then
              throw "Home Manager desktop check failed:\n${lib.concatMapStringsSep "\n" (f: "  - ${f}") failures}"
            else
              ''
                echo "PASS: the desktop launcher shares SOCIS_AGENT_HOME, the runtime and the backend of the service"
                mkdir -p $out
                echo "ok" > $out/result
              ''
          );
        ...
        home-manager-desktop-standalone =
          let
            ...
            failures =
              ...
              ++ lib.optional (
                !lib.hasInfix "--set SOCIS_AGENT_HOME" wrapper
              ) "the launcher must carry SOCIS_AGENT_HOME even with no services"
              ++ lib.optional (
                lib.hasInfix "SOCIS_AGENT_MANAGED" wrapper
              ) "the launcher must not claim a managed install when no activation writes one"
              ++ lib.optional (
                lib.hasInfix "SOCIS_AGENT_DESKTOP_REMOTE_URL" wrapper
              ) "the launcher must not name a backend when the services run none"
              ++ lib.optional (
                cfg.systemd.user.services ? socis-backend || cfg.launchd.agents ? socis-backend
              ) "programs.enable alone must start no service";
          in
          pkgs.runCommand "socis-home-manager-desktop-standalone" { } (
            if failures != [ ] then
        ...
        # command line. A silent removal thus leaves them with no `socis`
        ...
        home-manager-install-package-removed =
          let
            ...
            cases = [
              {
                value = true;
                expect = "programs.socis-agent.enable = true;";
              }
              {
                value = false;
                expect = "programs.socis-agent.enable = false;";
              }
            ];
          ...
          in
          pkgs.runCommand "socis-home-manager-install-package-removed" { } (
            if failures != [ ] then
        ...
        module-option-parity =
          ...
          in
          pkgs.runCommand "socis-module-option-parity" { } (
            if failures != [ ] then
        ...
        nixos-module =
          let
            ...
            units = lib.filterAttrs (n: _: lib.hasPrefix "socis" n) cfg.systemd.services;
            names = lib.attrNames units;
            execOf = name: units.${name}.serviceConfig.ExecStart;
            activation = cfg.system.activationScripts."socis-agent-setup".text;

            failures =
              lib.optional (names != [
                "socis-agent"
                "socis-backend"
              ]) "expected socis-agent + socis-backend units, got: ${toString names}"
              ++ lib.optional (
                !lib.hasInfix "bin/socis gateway" (execOf "socis-agent")
              ) "gateway unit does not run `socis gateway`: ${execOf "socis-agent"}"
              ++ lib.optional (
                !lib.hasInfix "bin/socis dashboard" (execOf "socis-backend")
              ) "backend unit does not run `socis dashboard`: ${execOf "socis-backend"}"
              ++ lib.optional (
                units.socis-agent.environment.SOCIS_AGENT_HOME != units.socis-backend.environment.SOCIS_AGENT_HOME
              ) "gateway and backend must share one SOCIS_AGENT_HOME"
              ++ lib.optional (
                !lib.hasInfix "/var/lib/socis/.socis-agent/SOUL.md" activation
              ) "socisHomeFiles must install into SOCIS_AGENT_HOME";
          ...
          in
          pkgs.runCommand "socis-nixos-module" { } (
            if failures != [ ] then
        ...
        # polls and then execs socis, and the assertions reject a
        # configuration that cannot work.
        backend-bind-wait =
          let
            execOf =
              settings:
              (evalNixosModule ({ enable = true; } // settings)).config.systemd.services.socis-backend.serviceConfig.ExecStart;
            ...
            failures =
              ...
              lib.optional (!lib.hasInfix "bin/socis serve --host 127.0.0.1" direct)
                "without waitFor the backend must exec socis directly, got: ${direct}"
              ++ lib.optional (lib.hasInfix "socis-backend-launch" direct)
                "without waitFor the backend must not use the launcher"

              # The hostname mode polls the resolver, then binds the name.
              ++ lib.optional (!lib.hasInfix "socis-backend-launch" hostnameWait)
              ...
              ++ lib.optional (!lib.hasInfix "exec " hostnameScript)
                "the launcher must exec socis, so that it keeps the MainPID"
              ...
              ++ lib.optional (!lib.hasInfix "bin/socis dashboard" interfaceScript)
          ...
          in
          pkgs.runCommand "socis-backend-bind-wait" { } (
            if failures != [ ] then
        ...
        # $SOCIS_AGENT_HOME/.env. The important property is that a second run
        ...
        env-file-assembly =
          let
            envScript = (import ./moduleCommon.nix { inherit lib; }).mkEnvScript {
              inherit pkgs;
              environment = {
                SOCIS_AGENT_PUBLIC = "visible";
              };
            };
          in
          pkgs.runCommand "socis-env-file-assembly" { } ''
            set -e
            workdir=$(mktemp -d)
            ...
            grep -qx 'SOCIS_AGENT_PUBLIC=visible' "$workdir/.env" || \
        ...
        service-argv =
          let
            ...
            sentinel = "--socis-nix-argv-probe";
            probe = argv: lib.escapeShellArgs (argv ++ [ sentinel ]);
          in
          pkgs.runCommand "socis-service-argv" { } ''
            set -e
        ...
        package-contents = pkgs.runCommand "socis-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${socis-agent}/bin/socis || (echo "FAIL: socis binary missing"; exit 1)
          test -x ${socis-agent}/bin/socis-agent || (echo "FAIL: socis-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${socis-agent}/bin/socis --version 2>&1 | grep -qi "socis" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "socis-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in socis socis-agent socis-acp; do
            test -x ${socis-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "socis-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking socis --help ==="
          ${socis-agent}/bin/socis --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${socis-agent}/bin/socis --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "socis-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${socis-agent}/share/socis-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          # -L: skills/ is a symlink to the filtered source store path
          SKILL_COUNT=$(find -L ${socis-agent}/share/socis-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "SOCIS_AGENT_BUNDLED_SKILLS" ${socis-agent}/bin/socis || \
            (echo "FAIL: SOCIS_AGENT_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: SOCIS_AGENT_BUNDLED_SKILLS set in wrapper"

          # Optional skills ship via the wrapper too (pythonSrc excludes
          # them from the wheel, so the env var is the only path in nix).
          test -d ${socis-agent}/share/socis-agent/optional-skills || \
            (echo "FAIL: optional-skills directory missing"; exit 1)
          OPT_COUNT=$(find -L ${socis-agent}/share/socis-agent/optional-skills -name "SKILL.md" | wc -l)
          test "$OPT_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files in optional-skills"; exit 1)
          grep -q "SOCIS_AGENT_OPTIONAL_SKILLS" ${socis-agent}/bin/socis || \
            (echo "FAIL: SOCIS_AGENT_OPTIONAL_SKILLS not in wrapper"; exit 1)
          echo "PASS: $OPT_COUNT optional skills found, SOCIS_AGENT_OPTIONAL_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "socis-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${socis-agent}/share/socis-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${socis-agent}/share/socis-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "SOCIS_AGENT_BUNDLED_PLUGINS" ${socis-agent}/bin/socis || \
            (echo "FAIL: SOCIS_AGENT_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: SOCIS_AGENT_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-locales = pkgs.runCommand "socis-bundled-locales" { } ''
          set -e
          echo "=== Checking bundled locales ==="
          test -d ${socis-agent}/share/socis-agent/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          # -L: locales/ is a symlink to the source store path
          LOC_COUNT=$(find -L ${socis-agent}/share/socis-agent/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${socis-agent}/share/socis-agent/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "SOCIS_AGENT_BUNDLED_LOCALES" ${socis-agent}/bin/socis || \
            (echo "FAIL: SOCIS_AGENT_BUNDLED_LOCALES not in wrapper"; exit 1)
          echo "PASS: SOCIS_AGENT_BUNDLED_LOCALES set in wrapper"

          # locales/ is a bare data dir (no __init__.py), shipped via a
          # symlink + SOCIS_AGENT_BUNDLED_LOCALES (not via wheel data-files).
          # Verify the wrapper override resolves real strings.
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && SOCIS_AGENT_BUNDLED_LOCALES=${socis-agent}/share/socis-agent/locales \
            ${socisVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "rendered: $RENDERED"
          test "$RENDERED" != "gateway.reset.header_default" || (echo "FAIL: i18n returned the raw key with SOCIS_AGENT_BUNDLED_LOCALES set"; exit 1)
        ...
        # SOCIS_AGENT_OPTIONAL_MCPS (not via wheel data-files).
        bundled-mcps = pkgs.runCommand "socis-bundled-mcps" { } ''
          set -e
          echo "=== Checking bundled optional-mcps ==="
          test -d ${socis-agent}/share/socis-agent/optional-mcps || (echo "FAIL: optional-mcps directory missing"; exit 1)
          echo "PASS: optional-mcps directory exists"

          MANIFEST_COUNT=$(find -L ${socis-agent}/share/socis-agent/optional-mcps -name "manifest.yaml" | wc -l)
          test "$MANIFEST_COUNT" -gt 0 || (echo "FAIL: no manifest.yaml files found"; exit 1)
          echo "PASS: $MANIFEST_COUNT catalog manifests found"

          grep -q "SOCIS_AGENT_OPTIONAL_MCPS" ${socis-agent}/bin/socis || \
            (echo "FAIL: SOCIS_AGENT_OPTIONAL_MCPS not in wrapper"; exit 1)
          echo "PASS: SOCIS_AGENT_OPTIONAL_MCPS set in wrapper"

          export HOME=$(mktemp -d)
          CATALOG=$(cd "$HOME" && ${socis-agent}/bin/socis mcp catalog 2>/dev/null || true)
          echo "catalog output: $CATALOG"
          test -n "$CATALOG" || (echo "FAIL: socis mcp catalog returned empty"; exit 1)
        ...
        bundled-tui = pkgs.runCommand "socis-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${socis-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${socis-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "SOCIS_AGENT_TUI_DIR" ${socis-agent}/bin/socis || \
            (echo "FAIL: SOCIS_AGENT_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: SOCIS_AGENT_TUI_DIR set in wrapper"
        ...
        # Verify SOCIS_AGENT_NODE is set in wrapper and points to Node 26+
        # (SOCIS pins its toolchain to Node 26 everywhere)
        socis-node = pkgs.runCommand "socis-node-version" { } ''
          set -e
          echo "=== Checking SOCIS_AGENT_NODE in wrapper ==="
          grep -q "SOCIS_AGENT_NODE" ${socis-agent}/bin/socis || \
            (echo "FAIL: SOCIS_AGENT_NODE not set in wrapper"; exit 1)
          echo "PASS: SOCIS_AGENT_NODE present in wrapper"

          SOCIS_AGENT_NODE=$(sed -n "s/^export SOCIS_AGENT_NODE='\(.*\)'/\1/p" ${socis-agent}/bin/socis)
          test -x "$SOCIS_AGENT_NODE" || (echo "FAIL: SOCIS_AGENT_NODE=$SOCIS_AGENT_NODE not executable"; exit 1)
          echo "PASS: SOCIS_AGENT_NODE executable at $SOCIS_AGENT_NODE"

          NODE_MAJOR=$("$SOCIS_AGENT_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 26 || \
            (echo "FAIL: Node v$NODE_MAJOR < 26, SOCIS requires Node 26"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 26"

          echo "=== All SOCIS_AGENT_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify SOCIS_AGENT_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "socis-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(SOCIS_AGENT_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking SOCIS_AGENT_MANAGED guards ==="
          check_blocked "config set" ${socis-agent}/bin/socis config set model foo
          check_blocked "config edit" ${socis-agent}/bin/socis config edit
        ...
        extra-python-packages = let
          ...
          socisWithExtra = socis-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "socis-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${socisWithExtra}/bin/socis || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${socisWithExtra}/bin/socis || \
          ...
          if grep -q "PYTHONPATH" ${socis-agent}/bin/socis; then
        ...
        extra-dependency-groups = let
          socisWithGroups = socis-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "socis-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${socisWithGroups}"
          echo "venv: ${socisWithGroups.socisVenv}"
        ...
        messaging-variant = pkgs.runCommand "socis-messaging-variant" { } ''
          set -e
          echo "=== Checking discord.py importable from messaging variant ==="
          ${self'.packages.messaging.socisVenv}/bin/python3 -c \
        ...
        in pkgs.runCommand "socis-config-roundtrip" {
          ...
          merge_and_load() {
            local socis_agent_home="$1"
            export SOCIS_AGENT_HOME="$socis_agent_home"
            ${configMergeScript} ${nixSettings} "$socis_agent_home/config.yaml"
            ${socisVenv}/bin/python3 -c '
import json, sys
