# nix/desktop.nix — SOCIS Agent Desktop (Electron) app build + wrapper
#
# `socisAgent` is the fully-built `.#default` package — it ships the
# `socis` binary with the venv, runtime PATH, bundled skills/plugins, etc.
# already wired up.  We point the desktop at it via the existing
# `SOCIS_AGENT_DESKTOP_SOCIS` override env var, so the desktop's resolver
# uses our fully wrapped binary at step 4 ("existing SOCIS CLI").
# No reimplementation of the agent resolution in this wrapper.
{
  ...
  # SOCIS_AGENT_HOME and SOCIS_AGENT_MANAGED here, which gives the app the same state
...
let
  ...
  targetPlatform =
    if stdenv.hostPlatform.isDarwin then
      ...
      throw "socis-desktop: unsupported host platform for node-pty staging";
  ...
  targetArch =
    if stdenv.hostPlatform.isAarch64 then
      ...
      throw "socis-desktop: unsupported host arch for node-pty staging";

  # Build the renderer (dist/ + electron/ + package.json).
  renderer = socisNpmLib.buildNpmPackage {
    dirs = [
      "apps/desktop"
      "apps/shared"
    ];
    pname = "socis-desktop-renderer";
    ...
    buildPhase = ''
      ...
      pushd apps/desktop
        ...
        ${lib.getExe socisNpmLib.node-gyp} rebuild \
...
stdenv.mkDerivation {
  ...
  installPhase = ''
    ...
    mkdir -p $out/share/socis-desktop $out/bin
    cp -r ${renderer}/* $out/share/socis-desktop/
    ...
    substituteInPlace $out/share/socis-desktop/dist/electron-main.mjs \
      --replace-fail "process.resourcesPath" "'$out/share/socis-desktop'"

    # Wrap the nixpkgs electron binary to launch our app.  Set
    # SOCIS_AGENT_DESKTOP_SOCIS to the absolute path of the nix-built `socis`
    # binary so the desktop's resolver step 4 ("existing SOCIS CLI on
    # PATH") uses our fully wrapped binary — venv with all deps,
    # bundled skills/plugins, runtime PATH (ripgrep/git/ffmpeg/etc).
    # No reimplementation of the agent resolver in the wrapper.
    makeWrapper ${lib.getExe electron} $out/bin/socis-desktop \
      --add-flags "$out/share/socis-desktop" \
      --set SOCIS_AGENT_DESKTOP_SOCIS "${lib.getExe socisAgent}" \
    ...
    install -m 0644 ${../apps/desktop/assets/icon.png} \
      $out/share/icons/hicolor/1024x1024/apps/socis.png
    export PYTHONPATH=$(mktemp -d)
    cp ${../socis_cli/linux_desktop_entry.py} "$PYTHONPATH/linux_desktop_entry.py"
    export DESKTOP_EXEC="$out/bin/socis-desktop"
    export DESKTOP_ICON="$out/share/icons/hicolor/1024x1024/apps/socis.png"
    python3 -c 'import os; from linux_desktop_entry import render_desktop_entry; print(render_desktop_entry(os.environ["DESKTOP_EXEC"], os.environ["DESKTOP_ICON"]))' > $out/share/applications/socis.desktop
    runHook postInstall
  '';
  ...
  meta = with lib; {
    description = "Native Electron desktop shell for SOCIS Agent";
    homepage = "https://github.com/socisio/socis-agent";
    license = licenses.mit;
    platforms = platforms.unix;
    mainProgram = "socis-desktop";
  };
}
