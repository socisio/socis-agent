...
let
    ...
    (callPackage pyproject-nix.build.packages {
      ...
      (
        lib.composeManyExtensions [
          ...
          # SOCIS derivation. This is deliberately a derivation environment
          # variable, not a devShell variable: ``nix develop -c uv build``
          # must remain blocked.
          (final: prev: {
            socis-agent = prev.socis-agent.overrideAttrs (_old: {
              SOCIS_AGENT_NIX_BUILD = "1";
            });
          })
        ]
      );
  ...
  # $SOCIS_AGENT_PYTHON_SRC_ROOT at runtime.
  workspaceRoot = ./..;
  ...
  editableOverlay = editableWorkspace.mkEditablePyprojectOverlay {
    root = "$SOCIS_AGENT_PYTHON_SRC_ROOT"; # resolved at shellHook time
  };
  ...
  editableSet = pythonSet.overrideScope (
    lib.composeManyExtensions [
      editableOverlay
      (final: prev: {
        socis-agent = prev.socis-agent.overrideAttrs (old: {
...
in
{
  venv = pythonSet.mkVirtualEnv "socis-agent-env" {
    socis-agent = dependency-groups;
  };
  editableVenv = editableSet.mkVirtualEnv "socis-agent-editable-env" {
    socis-agent = dependency-groups;
  };
}
