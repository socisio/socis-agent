...
#
# `services.socis-agent` is the same option set on both modules. Both modules
...
#   homeManagerModules.nix  socisHome, programs.socis-agent (the CLI and
...
let
  ...
  deepConfigType = types.mkOptionType {
    name = "socis-config-attrs";
    description = "SOCIS YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    options = {
      ...
      auth = mkOption {
        ...
        description = ''
          ...
          (remote MCP servers). Tokens are stored in $SOCIS_AGENT_HOME/mcp-tokens/.
        '';
      };
  ...
  sharedOptions =
    ...
    {
      enable = lib.mkEnableOption "SOCIS Agent";
      ...
      package = mkOption {
        ...
        description = "The socis-agent package to use.";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          The SOCIS configuration, as an attribute set. The module joins the
          ...
          which includes the keys that `socis config set` and the settings
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          $SOCIS_AGENT_HOME/.env. SOCIS reads that file at each start, with
          load_socis_dotenv().

          Each activation writes .env again from the start. Thus a secret
          file cannot go into .env two times.
        '';
        example = literalExpression ''[ config.sops.secrets."socis/env".path ]'';
      };
      ...
      authFile = mkOption {
        ...
        description = ''
          ...
          does not exist. Thus a token that SOCIS refreshes at runtime stays
          after an activation.
        '';
      };
      ...
      documents = mkOption {
        ...
        description = ''
          ...
          SOCIS reads SOUL.md and memories/ from SOCIS_AGENT_HOME, so put those
          files in `socisHomeFiles`.
      ...
      socisHomeFiles = mkOption {
        type = documentsType;
        default = { };
        description = ''
          Files that the module installs into SOCIS_AGENT_HOME. Each key is a path
          relative to that directory, and the module makes the necessary
          subdirectories. Each value is a string or a path.

          SOCIS reads SOUL.md and the memory files from SOCIS_AGENT_HOME and not
          from the working directory. Declare those files here, or SOCIS
          does not load them.
        '';
        example = literalExpression ''
          {
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the socis plugins
          directory. Each package must contain a plugin.yaml and __init__.py
          at its root. SOCIS discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          socis_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as socis (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in socis-agent's
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `socis gateway`.";
      };
      ...
      # `socis serve` and `socis dashboard` are the same entry point,
      # socis_cli.main:cmd_dashboard, with one flag of difference. serve runs
      # without a user interface. dashboard also serves the web application.
      # Both give the /api/ws and /api/pty sockets that SOCIS Agent Desktop
      ...
      # controls an external gateway, with `socis gateway restart`. It does
      ...
      backend = {
        mode = mkOption {
          ...
          description = ''
            ...
            - "serve"     — the backend without a user interface. It gives
                            the /api/ws and /api/pty sockets that SOCIS
                            Desktop connects to.
        ...
        sessionTokenFile = mkOption {
          ...
          description = ''
            ...
            SOCIS_AGENT_DASHBOARD_SESSION_TOKEN. That token authorizes the /api
            routes and the /api/ws socket. SOCIS Agent Desktop presents the same
          ...
          '';
          example = literalExpression ''config.sops.secrets."socis/desktop-token".path'';
        };
      };
    };
  ...
  # removal leaves them with no `socis` on the PATH and no message. The
  ...
  installPackageRemovedMessage =
    value:
    ''
      services.socis-agent.installPackage was removed. SOCIS now
      separates the installation from the services, which is the
      Home Manager convention:

        programs.socis-agent.enable = ${lib.boolToString (value != false)};  # the socis CLI, and SOCIS_AGENT_HOME for your shells
        programs.socis-agent.desktop.enable = true;  # the desktop application

      `services.socis-agent` keeps the state, the configuration and
  ...
  mkConfigFiles =
    ...
    let
      generated = pkgs.writeText "socis-config.yaml" (
  ...
  mkDocumentTree =
    { pkgs, documents }:
    pkgs.runCommand "socis-documents" { } (
      ''
        mkdir -p $out
      ''
      + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (
          ...
          if builtins.isPath value || lib.isStorePath value then
            ...
            "${mkdir}\ncat > $out/${name} <<'SOCIS_AGENT_DOC_EOF'\n${value}\nSOCIS_AGENT_DOC_EOF"
        ) documents
      )
    );
  ...
  mkEnvScript =
    { pkgs, environment }:
    let
      base = pkgs.writeText "socis-env-base" (
        lib.concatStringsSep "\n" (lib.mapAttrsToList (k: v: "${k}=${v}") environment)
        + lib.optionalString (environment != { }) "\n"
      );
    in
    pkgs.writeShellScript "socis-env-merge" ''
      set -eu
      ...
      for file in "$@"; do
        if [ -r "$file" ]; then
          ...
          echo "socis-agent: WARNING cannot read environmentFile $file" >&2
        fi
      done
    '';
  ...
  mkStateScript =
    {
      ...
      # SOCIS_AGENT_MANAGED variable of the service. The value tells the shell
    ...
    in
    ''
      # Directories. The service units and SOCIS make most of these
      ...
      ${run}mkdir -p ${
        lib.escapeShellArgs (
          ...
          ++ map (d: "${socisHome}/${d}") stateDirs
        )
      }

      # config.yaml: merge the Nix settings into the file on disk. SOCIS
      ...
      ${
        if cfg.configFile != null then
          "${inst} -m ${modes.config} -D ${configFiles.effective} ${socisHome}/config.yaml"
        else
          ''
            ${run}${configFiles.mergeScript} ${configFiles.generated} ${socisHome}/config.yaml
            ${run}chmod ${modes.config} ${socisHome}/config.yaml
          ''
      }

      # The managed-mode marker. It makes an interactive shell also refuse to
      # change the configuration that Nix owns.
      ${inst} -m ${modes.managed} ${pkgs.writeText "socis-managed" managedSystem} ${socisHome}/.managed

      ${lib.optionalString (cfg.environment != { } || cfg.environmentFiles != [ ]) ''
        ${run}${envScript} ${socisHome}/.env ${modes.env} ${lib.escapeShellArgs cfg.environmentFiles}
        ${lib.optionalString (owner != null) "${run}chown ${owner} ${socisHome}/.env"}
      ''}

      ${lib.optionalString (cfg.authFile != null) (
        if cfg.authFileForceOverwrite then
          "${inst} -m ${modes.auth} ${cfg.authFile} ${socisHome}/auth.json"
        else
          ''
            if [ ! -e ${socisHome}/auth.json ]; then
              ${inst} -m ${modes.auth} ${cfg.authFile} ${socisHome}/auth.json
            fi
          ''
      )}
      ...
      ${installDocuments homeDocumentTree socisHome cfg.socisHomeFiles}
      ...
      ${run}find ${socisHome}/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
      ${lib.concatMapStringsSep "\n" (plugin: ''
        if [ ! -f ${plugin}/plugin.yaml ]; then
          echo "socis-agent: ERROR extraPlugins entry '${plugin}' has no plugin.yaml" >&2
          exit 1
        fi
        ${run}ln -sfn ${plugin} ${socisHome}/plugins/nix-managed-${lib.getName plugin}
      '') cfg.extraPlugins}
    '';
  ...
  gatewayArgv =
    cfg:
    [
      "${effectivePackage cfg}/bin/socis"
      "gateway"
    ]
    ++ cfg.extraArgs;
  ...
  #
  # `exec` on the last line keeps socis as the MainPID of the unit. No shell
  ...
  backendLauncher =
    ...
    pkgs.writeShellScript "socis-backend-launch" (
      builtins.replaceStrings [ "@HOST@" ] [ ''"$_target"'' ] ''
        ...
        ${lib.optionalString (cfg.backend.sessionTokenFile != null) ''
          ...
          if [ ! -r "$_token_file" ]; then
            echo "socis-backend: cannot read the session token file '$_token_file'. The unit stops." >&2
            echo "socis-backend: backend.sessionTokenFile must name a runtime path that this user can read." >&2
            exit 1
          fi

          SOCIS_AGENT_DASHBOARD_SESSION_TOKEN="$(${pkgs.coreutils}/bin/tr -d '\r\n' < "$_token_file")"
          export SOCIS_AGENT_DASHBOARD_SESSION_TOKEN

          if [ -z "$SOCIS_AGENT_DASHBOARD_SESSION_TOKEN" ]; then
            echo "socis-backend: the session token file '$_token_file' is empty. The unit stops." >&2
            exit 1
          fi
        ''}
        ${
          if cfg.backend.waitFor == null then
            ...
            ''
              ...
              while :; do
                ...
                if [ "$_waited" -ge "$_timeout" ]; then
                  echo "socis-backend: '$_target' did not resolve after ''${_timeout}s. The unit stops." >&2
                  exit 1
                fi

                if [ "$_waited" = 0 ]; then
                  echo "socis-backend: waits for '$_target' to resolve..." >&2
                fi
            ...
            ''
              ...
              while :; do
                ...
                if [ "$_waited" -ge "$_timeout" ]; then
                  echo "socis-backend: interface '$_iface' had no IPv4 address after ''${_timeout}s. The unit stops." >&2
                  echo "socis-backend: a fallback address can expose the backend more widely than you intend." >&2
                  exit 1
                fi

                if [ "$_waited" = 0 ]; then
                  echo "socis-backend: waits for an IPv4 address on '$_iface'..." >&2
                fi
                ${pkgs.coreutils}/bin/sleep 2
                _waited=$(( _waited + 2 ))
              done
        ...
        echo "socis-backend: binds to $_target:${toString cfg.backend.port} (from $_how)" >&2
  ...
  backendDescription =
    ...
    if cfg.backend.mode == "dashboard" then
      "SOCIS Agent web dashboard and desktop backend"
    else
      "SOCIS Agent backend for SOCIS Agent Desktop";

  # The environment that each SOCIS process needs, from either module.
  #
  # managedSystem gives the value of SOCIS_AGENT_MANAGED. The CLI reads that
  ...
  processEnvironment =
    ...
    }:
    {
      ...
      SOCIS_AGENT_MANAGED = managedSystem;
    };
    ...
    [
      {
        ...
        message = ''
          ...
          To give SOCIS an identity and a memory, use
          ${optionPath}.socisHomeFiles instead. Those files go to
          SOCIS_AGENT_HOME. SOCIS reads SOUL.md and memories/ only from there.
        '';
      }
    ];
  ...
  # The subdirectories of SOCIS_AGENT_HOME that both modules make.
  stateSubdirs = [
    "cron"
    "sessions"
    "logs"
