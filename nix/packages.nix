# nix/packages.nix — SOCIS Agent package built with uv2nix
{ inputs, ... }:
{
  perSystem =
    {
    ...
    let
      ...
      minimal = pkgs.callPackage ./socis-agent.nix {
    ...
    in
    {
      packages = {
        ...
        tui = full.socisTui;
        web = full.socisWeb;
        desktop = full.socisDesktop;

        update-npm-lockfile = full.socisNpmLib.updateNpmLockfile;
      };
    };
}
