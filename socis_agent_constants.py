"""Shared constants for SOCIS Agent.
...
_UNSET = object()
_SOCIS_AGENT_HOME_OVERRIDE: ContextVar[str | object] = ContextVar(
    "_SOCIS_AGENT_HOME_OVERRIDE", default=_UNSET
)
...
def set_socis_agent_home_override(path: str | Path | None) -> Token:
    """Set a context-local SOCIS home override and return its reset token.

    This is for in-process, per-task scoping.  It deliberately does not mutate
    ``os.environ`` because that is shared by every thread in the process.
    """
    value: str | object = _UNSET if path is None else str(path)
    return _SOCIS_AGENT_HOME_OVERRIDE.set(value)


def reset_socis_agent_home_override(token: Token) -> None:
    """Restore the previous context-local SOCIS home override."""
    _SOCIS_AGENT_HOME_OVERRIDE.reset(token)


def get_socis_agent_home_override() -> str | None:
    """Return the active context-local SOCIS home override, if any."""
    override = _SOCIS_AGENT_HOME_OVERRIDE.get()
...
def _get_platform_default_socis_agent_home() -> Path:
    ...
    if sys.platform == "win32":
        ...
        return base / "socis"
    return Path.home() / ".socis-agent"


def _socis_agent_home_from_env() -> Path:
    """Resolve SOCIS_AGENT_HOME from the process environment only.

    Reads the ``SOCIS_AGENT_HOME`` env var, falling back to the platform-native
    default.  Deliberately ignores the context-local override installed by
    :func:`set_socis_agent_home_override`, so this reflects the process/launch
    scope rather than a per-task profile.  Shared by :func:`get_socis_agent_home`
    and :func:`get_process_socis_agent_home` so the two never drift.
    """
    val = os.environ.get("SOCIS_AGENT_HOME", "").strip()
    if val:
        return Path(val)
    return _get_platform_default_socis_agent_home()


def _warn_profile_fallback_once() -> None:
    """Warn once when falling back to the default home while a profile is active.

    Guard: if a non-default profile is sticky-active but ``SOCIS_AGENT_HOME`` is
    ...
    if _profile_fallback_warned:
        return
    try:
        fallback_home = _get_platform_default_socis_agent_home()
    ...
    if active and active != "default":
        ...
        msg = (
            f"[SOCIS_AGENT_HOME fallback] SOCIS_AGENT_HOME is unset but active "
            f"profile is {active!r}. Falling back to {fallback_home}, which "
            f"is the DEFAULT profile — not {active!r}. Any data this "
            f"process writes will land in the wrong profile. The "
            f"subprocess spawner should pass SOCIS_AGENT_HOME explicitly "
            f"(see issue #18594)."
        )
        try:
...
def get_socis_agent_home() -> Path:
    """Return the SOCIS home directory (default: platform-native path).

    Resolution order: context-local override (see
    :func:`set_socis_agent_home_override`) → ``SOCIS_AGENT_HOME`` env var → the
    platform-native default.  This is the single source of truth — all other
    copies should import this.

    When ``SOCIS_AGENT_HOME`` is unset but an ``active_profile`` file indicates
    ...
    expected to propagate ``SOCIS_AGENT_HOME`` explicitly (see the systemd
    template in ``socis_cli/gateway.py`` and the kanban dispatcher in
    ``socis_cli/kanban_db.py``).  See https://github.com/socisio/socis-agent/issues/18594.
    """
    override = get_socis_agent_home_override()
    if override:
        return Path(override)

    if not os.environ.get("SOCIS_AGENT_HOME", "").strip():
        _warn_profile_fallback_once()

    return _socis_agent_home_from_env()
...
def socis_agent_home_key(path: str | Path | None = None) -> str:
    """Return a stable key for a SOCIS home/profile directory.
    ...
    out to be a symlink.
    """
    candidate = Path(path) if path is not None else get_socis_agent_home()
    raw = str(candidate)
...
def reset_socis_agent_home_key_cache() -> None:
...
def get_process_socis_agent_home() -> Path:
    """Return the SOCIS home for the running process, ignoring task overrides.

    Unlike :func:`get_socis_agent_home`, this never follows the context-local
    override set by :func:`set_socis_agent_home_override`.  It resolves only the
    process ``SOCIS_AGENT_HOME`` env var (falling back to the platform default),
...
# Process-level memo for get_default_socis_root(). The function resolves
# SOCIS_AGENT_HOME against the native home on every call (~80us of path
...
# Its result depends only on (SOCIS_AGENT_HOME, platform native home), which are
# compared for free on each call, so the memo is freshness-correct even if a
# test or plugin mutates SOCIS_AGENT_HOME mid-process.
_default_socis_root_memo: "tuple[str, str, Path] | None" = None


def get_default_socis_root() -> Path:
    """Return the root SOCIS directory for profile-level operations.

    In standard deployments this is the platform-native SOCIS home
    (``~/.socis-agent`` on POSIX, ``%LOCALAPPDATA%\\socis`` on native Windows).

    In Docker or custom deployments where ``SOCIS_AGENT_HOME`` points outside
    ``~/.socis-agent`` (e.g. ``/opt/data``), returns ``SOCIS_AGENT_HOME`` directly
    — that IS the root.

    In profile mode where ``SOCIS_AGENT_HOME`` is ``<root>/profiles/<name>``,
    returns ``<root>`` so that ``profile list`` can see all profiles.
    Works both for standard (``~/.socis-agent/profiles/coder``) and Docker
    (``/opt/data/profiles/coder``) layouts.

    Import-safe — no dependencies beyond stdlib.
    """
    global _default_socis_root_memo
    native_home = _get_platform_default_socis_agent_home()
    env_home = os.environ.get("SOCIS_AGENT_HOME", "")
    if _default_socis_root_memo is not None:
        memo_native, memo_env, memo_result = _default_socis_root_memo
    ...
    if not env_home:
        ...
        try:
            ...
            # SOCIS_AGENT_HOME is under ~/.socis-agent (normal or profile mode)
            result = native_home
        except ValueError:
            ...
            if env_path.parent.name == "profiles":
                ...
                # Not a profile path — SOCIS_AGENT_HOME itself is the root
                result = env_path
    _default_socis_root_memo = (str(native_home), env_home, result)
    return result
...
# Files whose presence marks a directory as a real SOCIS home. A fresh home
...
_SOCIS_AGENT_HOME_MARKERS = ("config.yaml", ".env", "state.db")


def _is_socis_profiles_root(profiles_dir: Path) -> bool:
    """Return True when *profiles_dir* is a canonical ``<socis-home>/profiles``.

    Anchors named-profile recognition so it only fires for directories that
    provably live under a SOCIS home: the classic ``~/.socis-agent`` layout, a
    root carrying SOCIS-home marker files (Docker/custom ``SOCIS_AGENT_HOME``
    like ``/opt/data``), a ``profiles/.deleted`` tombstone directory (only
    ever created by ``socis profile delete``), or the process's resolved
    default SOCIS root.
    """
    root = profiles_dir.parent
    if root.name == ".socis-agent":
        return True
    try:
        if (profiles_dir / _DELETED_PROFILES_DIR).is_dir():
            return True
        if any((root / marker).exists() for marker in _SOCIS_AGENT_HOME_MARKERS):
            return True
    except OSError:
        pass
    try:
        return root.resolve(strict=False) == get_default_socis_root().resolve(
            strict=False
        )
    except OSError:
        return False
...
def named_profile_home(path: str | Path) -> Path | None:
    ...
    for candidate in (current, *current.parents):
        if (
            ...
            and _is_socis_profiles_root(candidate.parent)
        ):
            return candidate
        # Stop at a default SOCIS home so a coincidental ``profiles/``
        # ancestor is not treated as a named-profile root.
        if candidate.name == ".socis-agent":
            return None
    return None
...
def assert_named_profile_home_live(path: str | Path) -> None:
    ...
    Default ``SOCIS_AGENT_HOME`` (not under ``profiles/``) is unchanged.
    """
...
def mkdir_under_socis_agent_home(path: str | Path) -> Path:
...
def get_optional_skills_dir(default: Path | None = None) -> Path:
    ...
    tree and expose it via ``SOCIS_AGENT_OPTIONAL_SKILLS``.
    """
    override = os.getenv("SOCIS_AGENT_OPTIONAL_SKILLS", "").strip()
    if override:
        return Path(override)
    if default is not None:
        return default
    return get_socis_agent_home() / "optional-skills"
...
def get_optional_mcps_dir(default: Path | None = None) -> Path:
    ...
    package tree and expose it via ``SOCIS_AGENT_OPTIONAL_MCPS``.
    """
    override = os.getenv("SOCIS_AGENT_OPTIONAL_MCPS", "").strip()
    if override:
        return Path(override)
    if default is not None:
        return default
    return get_socis_agent_home() / "optional-mcps"
...
def get_bundled_skills_dir(default: Path | None = None) -> Path:
    ...
    Resolution order:
        1. ``SOCIS_AGENT_BUNDLED_SKILLS`` env var (Nix wrapper / explicit override)
        2. Caller-supplied ``default`` (typically the source-checkout path)
        3. ``<SOCIS_AGENT_HOME>/skills`` last-resort
    """
    override = os.getenv("SOCIS_AGENT_BUNDLED_SKILLS", "").strip()
    if override:
        return Path(override)
    if default is not None:
        return default
    return get_socis_agent_home() / "skills"
...
def get_socis_dir(
    ...
    *,
    ...
    """Resolve a SOCIS subdirectory with backward compatibility.
    ...
    Args:
        new_subpath: Preferred path relative to SOCIS_AGENT_HOME (e.g. ``"cache/images"``).
        old_name: Legacy path relative to SOCIS_AGENT_HOME (e.g. ``"image_cache"``).
        home: Optional explicit SOCIS home. Profile-aware callers that manage
            more than one home in the same process use this instead of
            temporarily mutating the process or context-local SOCIS_AGENT_HOME.

    Returns:
        Absolute ``Path`` — legacy location if it exists with content,
        otherwise the new location.
    """
    home = home or get_socis_agent_home()
...
def iter_socis_node_dirs(home: Path | None = None) -> list[Path]:
    """Return SOCIS-managed Node.js directories in preferred lookup order.

    Windows installs from ``scripts/install.ps1`` unpack portable Node directly
    into ``%LOCALAPPDATA%\\socis\\node``. POSIX installs use
    ``$SOCIS_AGENT_HOME/node/bin``. Include both shapes on every platform so mixed
    or migrated installs still work.
    """
    root = home or get_socis_agent_home()
    dirs = [root / "node"]
    bin_dir = root / "node" / "bin"
    # NOTE: keep this ordering in sync with socisManagedNodePathEntries() in
...
_SOCIS_AGENT_NODE_TARGET_MAJOR = int(os.environ.get("SOCIS_AGENT_NODE_TARGET_MAJOR", "22"))
_managed_node_heal_attempted = False
_NODE_BOOTSTRAP_SCRIPT = Path(__file__).resolve().parent / "scripts" / "lib" / "node-bootstrap.sh"

# Install tree root (this file lives at <install_root>/socis_agent_constants.py).
# Used by secure_parent_dir() to skip chmod on the install dir — chmodding it
# 0700 breaks socis-user traversal in Docker (UID 10000). See #25821, #93050.
...
def node_tool_runnable(path: str | None) -> bool:
    ...
    SOCIS-managed Node trees live under ``$SOCIS_AGENT_HOME/node`` (or a profile's
    ``SOCIS_AGENT_HOME``). A partial upgrade or interrupted install can leave
    ``bin/npm`` behind while ``lib/cli.js`` is missing — the wrapper exists but
    immediately throws ``MODULE_NOT_FOUND``. ``find_socis_node_executable``
    used to trust file presence alone, so ``socis update`` would pick that
...
def socis_managed_node_tree_present(home: Path | None = None) -> bool:
    """Return True when any SOCIS-managed node/npm/npx shim exists on disk."""
    names = set()
    for command in ("node", "npm", "npx"):
        names.update(_candidate_node_command_names(command))
    for directory in iter_socis_node_dirs(home):
        for name in names:
...
def _print_managed_node_in_use_notice() -> None:
    ...
    print(
        "→ SOCIS-managed Node.js is in use by a running app; deferring its "
        "upgrade until the app is closed (re-run `socis update` afterwards).",
        flush=True,
    )


def _heal_managed_node_windows(home: Path | None = None) -> bool | None:
    """Redownload the portable Node zip into ``%SOCIS_AGENT_HOME%\\node`` on Windows.
    ...
    ``FILE_SHARE_DELETE`` — the same mechanism as the socis.exe quarantine);
    ...
    index_url = f"https://nodejs.org/dist/latest-v{_SOCIS_AGENT_NODE_TARGET_MAJOR}.x/"
    try:
    ...
    match = re.search(
        rf"node-v{_SOCIS_AGENT_NODE_TARGET_MAJOR}\.\d+\.\d+-win-{node_arch}\.zip",
        index_html,
    )
    if not match:
        return False
...
def _bootstrap_managed_node_posix() -> bool:
    """Install a fresh managed Node under ``$SOCIS_AGENT_HOME/node`` on POSIX.
    ...
    ``SOCIS_AGENT_NODE_SKIP_LINKS=1`` so the user's own node/npm on PATH is not
...
def bootstrap_socis_managed_node() -> str | None:
    """Install a SOCIS-managed Node tree and return its npm path.
    ...
    SOCIS never modifies a toolchain it does not own, so instead it provisions
    its own tree under ``$SOCIS_AGENT_HOME/node`` (the same tree a fresh install
    ...
    present.
    """
    existing = find_socis_node_executable("npm")
    if existing:
        return existing
    ...
    for directory in iter_socis_node_dirs():
...
def heal_socis_managed_node() -> bool:
    """Redownload SOCIS-managed Node when the tree exists but is broken.
    ...
    if _managed_node_heal_attempted:
        return False
    if not socis_managed_node_tree_present():
        return False
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "SOCIS_AGENT_HOME": str(get_socis_agent_home())},
            capture_output=True,
            timeout=300,
            check=False,
        )
...
def _managed_node_tree_outdated(home: Path | None = None) -> bool:
    ...
    same way a broken one does: :func:`find_socis_node_executable` triggers
    the once-per-process heal, which redownloads
    ``latest-v{_SOCIS_AGENT_NODE_TARGET_MAJOR}.x`` — so existing users are upgraded
    ...
    for directory in iter_socis_node_dirs(home):
        for name in _candidate_node_command_names("node"):
            ...
            if "-" in version:
                return True
            return major < _SOCIS_AGENT_NODE_TARGET_MAJOR
    return False


def find_socis_node_executable(command: str) -> str | None:
    """Return a SOCIS-managed Node/npm executable path, healing broken trees.

    Outdated trees (node major below ``_SOCIS_AGENT_NODE_TARGET_MAJOR``) heal the
    ...
    if needs_heal and heal_socis_managed_node():
...
def find_node_executable_on_path(command: str) -> str | None:
    ...
    directly, so prefer the launchable variants explicitly for SOCIS-owned
    subprocesses.
    """
    if sys.platform != "win32":
...
def find_node_executable(command: str) -> str | None:
    """Resolve a Node.js command, preferring healthy SOCIS-managed installs.

    This is for SOCIS-owned subprocesses that should not be broken by a bad,
    ...
    """
    managed = find_socis_node_executable(command)
    if managed:
        return managed
    if socis_managed_node_tree_present():
        return None
    return find_node_executable_on_path(command)


def with_socis_node_path(env: dict[str, str] | None = None) -> dict[str, str]:
    """Return *env* with SOCIS-managed Node directories prepended to PATH."""
    merged = dict(os.environ if env is None else env)
    existing = merged.get("PATH", "")
    parts = [p for p in existing.split(os.pathsep) if p]
    managed = [str(path) for path in iter_socis_node_dirs() if path.is_dir()]
...
def agent_browser_runnable(path: str | None) -> bool:
    ...
    next ``socis update`` — leaving a **dangling symlink** that ``which`` still
...
def display_socis_agent_home() -> str:
    """Return a user-friendly display string for the current SOCIS_AGENT_HOME.

    Uses ``~/`` shorthand for readability::

        default:  ``~/.socis-agent``
        profile:  ``~/.socis-agent/profiles/coder``
        custom:   ``/opt/socis-custom``

    Use this in **user-facing** print/log messages instead of hardcoding
    ``~/.socis-agent``.  For code that needs a real ``Path``, use
    :func:`get_socis_agent_home` instead.
    """
    ...
    try:
        ...
        # ``~/AppData\Local\socis/skills/`` once callers append
...
def secure_parent_dir(path: Path) -> None:
    ...
    prevent catastrophic host bricking when ``SOCIS_AGENT_HOME`` or other path
    env vars resolve to an unexpected location.

    Also refuses to chmod the socis-agent install tree (the directory this
    ...
    See https://github.com/socisio/socis-agent/issues/25821 and
    https://github.com/socisio/socis-agent/pull/93050.
    """
    parent = path.parent.resolve()
    # Refuse root and its direct children (/usr, /home, /var, /tmp, …).
    if parent == Path("/") or len(parent.parts) < 3:
        return
    # Refuse the install tree root. chmodding it 0700 breaks socis-user
    ...
    if parent == _INSTALL_ROOT or _INSTALL_ROOT in parent.parents:
        # A credential file inside the install tree usually means SOCIS_AGENT_HOME
        ...
        logging.getLogger(__name__).warning(
            ...
            "socis-agent install directory (%s). Credential files are "
            "normally stored under the socis home directory instead.",
            parent,
            _INSTALL_ROOT,
        )
        return
...
def _profile_home_path(env: dict[str, str] | None = None) -> str | None:
    """Return ``{SOCIS_AGENT_HOME}/home`` when the profile-home directory exists."""
    socis_agent_home = get_socis_agent_home_override() or (env or {}).get("SOCIS_AGENT_HOME") or os.getenv("SOCIS_AGENT_HOME")
    if not socis_agent_home:
        return None
    profile_home = os.path.join(socis_agent_home, "home")
...
def _iter_real_home_candidates(env: dict[str, str] | None = None) -> list[str]:
    ...
    env = env or {}
    candidates: list[str] = []
    explicit = str(env.get("SOCIS_AGENT_REAL_HOME") or os.getenv("SOCIS_AGENT_REAL_HOME", "")).strip()
    if explicit:
...
def get_real_home(env: dict[str, str] | None = None) -> str:
    """Return the OS user's real home directory, avoiding SOCIS profile HOME.

    ``SOCIS_AGENT_HOME`` scopes SOCIS state. ``HOME`` is reserved for the OS/user
    account and the many external CLIs that store credentials under ``~``.
    If a parent process is already running with ``HOME={SOCIS_AGENT_HOME}/home``,
...
def get_subprocess_home(env: dict[str, str] | None = None) -> str | None:
    ...
    * ``auto`` (default): host installs keep the real user HOME; containers use
      ``{SOCIS_AGENT_HOME}/home`` for persistent state. If a host parent already has
      HOME pointed at the profile home, repair subprocesses back to real HOME.
    * ``real``: always prefer the real OS-user HOME.
    * ``profile``: use ``{SOCIS_AGENT_HOME}/home`` when it exists, preserving the
...
def apply_subprocess_home_env(env: dict[str, str]) -> None:
    """Apply SOCIS' subprocess HOME contract to *env* in-place."""
    real_home = get_real_home(env)
    if real_home:
        env["SOCIS_AGENT_REAL_HOME"] = real_home
...
def translate_cwd_for_wsl_backend(cwd: str) -> str:
    """Normalize a cross-boundary cwd when SOCIS itself runs inside WSL.
...
def get_config_path() -> Path:
    """Return the path to ``config.yaml`` under SOCIS_AGENT_HOME.

    Replaces the ``get_socis_agent_home() / "config.yaml"`` pattern repeated
    in 7+ files (skill_utils.py, socis_agent_logging.py, socis_agent_time.py, etc.).
    """
    return get_socis_agent_home() / "config.yaml"


def get_skills_dir() -> Path:
    """Return the path to the skills directory under SOCIS_AGENT_HOME."""
    return get_socis_agent_home() / "skills"



def get_env_path() -> Path:
    """Return the path to the ``.env`` file under SOCIS_AGENT_HOME."""
...
def apply_ipv4_preference(force: bool = False) -> None:
    ...
    if getattr(socket.getaddrinfo, "_socis_ipv4_patched", False):
        return
    ...
    _ipv4_getaddrinfo._socis_ipv4_patched = True  # type: ignore[attr-defined]
...
def venv_bin_dir(venv_dir, *, windows: bool | None = None) -> Path:
    ...
    across four ``socis_cli`` modules using three different Windows
    ...
    A few sites outside ``socis_cli`` (``tools/code_execution_tool.py``,
    ...
    *windows* lets a caller pass its own platform verdict. Several callers
    ...
    Windows paths on Linux CI (``socis_cli.main._is_windows`` and friends);
...
# Top-level packages/modules that ship as part of SOCIS itself. An ImportError
...
# `socis_cli.update_cmd`'s post-update probe consumes this same set so the
...
def is_first_party_module(name: str | None) -> bool:
    """True when *name* is a module that ships with SOCIS.
    ...
    if not root:
        return False
    return root in FIRST_PARTY_MODULE_ROOTS or root.startswith("socis_")
...
def partial_update_hint(exc: BaseException) -> list[str]:
    ...
    rather than their config, is the problem — and `socis update` is exactly
    ...
    return [
        ...
        "    socis update",
        "If that also fails, reinstall: https://agent.socis.io",
    ]
