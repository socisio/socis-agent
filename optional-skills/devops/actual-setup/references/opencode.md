...
is typically only in `~/.socis-agent/.env`, not a shell profile, so the env form
...
```
OpenCode DOES use the `provider/model` slash form on the CLI (unlike SOCIS,
...
`socis-custom-providers` skill, pitfall 2). SOCIS hits this because it forwards
