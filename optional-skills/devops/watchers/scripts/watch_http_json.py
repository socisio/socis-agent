...
Usage (via cron with --no-agent):
    ...
    socis cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$SOCIS_AGENT_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "SOCIS-Watcher/1.0"})
