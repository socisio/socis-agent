...
## 5. SOCIS Configuration

### Config location

`$SOCIS_AGENT_HOME/config.yaml` (defaults to `~/.socis-agent/config.yaml` when `SOCIS_AGENT_HOME` is unset)
...
Restart the SOCIS session for changes to take effect. The MCP connection is
