...
# ── 3. Ensure SOCIS config has twozero_td MCP entry ──
if [[ ! -f "$SOCIS_AGENT_CFG" ]]; then
    echo -e " ${FAIL} SOCIS config not found at ${SOCIS_AGENT_CFG}"
    manual_steps+=("Create ${SOCIS_AGENT_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$SOCIS_AGENT_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in SOCIS config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to SOCIS config..."
    python3 -c "
import yaml, sys, copy

cfg_path = '$SOCIS_AGENT_CFG'
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${SOCIS_AGENT_CFG} manually"); }
    manual_steps+=("Restart SOCIS session to pick up config change")
fi
