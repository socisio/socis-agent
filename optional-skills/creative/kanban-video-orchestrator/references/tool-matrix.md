...
Maps each role archetype to the SOCIS skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public socis-agent
...
## SOCIS skills relevant to video production

### Visual / rendering skills (`socis-agent/skills/creative/`)
...
### Design / pre-production skills (`socis-agent/skills/creative/`)
...
### Audio / media skills (`socis-agent/skills/creative/` + `skills/media/`)
...
These are **not** SOCIS skills but external CLIs / APIs that profiles invoke.
...
## Built-in SOCIS tools for media review

These are native SOCIS tools — not invoked via terminal but through their own
...
key is present in `${SOCIS_AGENT_HOME:-~/.socis-agent}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every SOCIS profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${SOCIS_AGENT_HOME:-~/.socis-agent}/.env` → environment variable.
...
When a new SOCIS-public video skill ships:
