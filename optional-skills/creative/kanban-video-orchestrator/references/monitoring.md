...
socis kanban watch --tenant <project-slug>

# Snapshot of the board
socis kanban list --tenant <project-slug>
socis kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
socis kanban stats --tenant <project-slug>

# Visual dashboard (browser)
socis dashboard

# Inspect a specific task (includes comments + events)
socis kanban show <task-id>

# Follow a single task's event stream
socis kanban tail <task-id>
```

Verify available subcommands with `socis kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `socis kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `socis kanban block <id>` then `socis kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `socis kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `socis kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `socis profile list` (profile exists?), `socis status` (gateway/dispatcher up?), and verify tenant. |
...
socis kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
socis kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(socis kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
socis kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
socis kanban block <task-id>
socis kanban archive <task-id>

# Diagnose what happened
socis kanban show <task-id>      # task body, comments, recent events
socis kanban tail <task-id>      # follow the live event stream
socis kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `socis kanban create` for the director
...
while true; do
    clear
    socis kanban list --tenant <slug>
    echo "---"
    socis kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `socis kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `socis profile list` and start
