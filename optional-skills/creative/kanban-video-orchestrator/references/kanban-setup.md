...
the actual `setup.sh` that creates the project workspace, configures SOCIS
...
> [SOCIS/kanban-video-pipeline](https://github.com/SOCIS/kanban-video-pipeline).
...
2. **Create profiles** — `socis profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.socis-agent/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `socis kanban create` assigned to the director
...
```bash
socis profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.socis-agent/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.socis-agent/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most SOCIS
...
Each profile gets a `SOUL.md` at `~/.socis-agent/profiles/<name>/SOUL.md` that
...
```bash
socis kanban create "Direct production of <video title>" \
    --assignee director \
...
check_key() {
    ...
    echo "ERROR: ${var} not set in ${_socis_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY socis ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY socis OPENROUTER_API_KEY || exit 1
# ...
```
