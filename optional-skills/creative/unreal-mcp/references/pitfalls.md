...
### 1. Start order: editor first, SOCIS second

SOCIS probes MCP servers at session start. If the editor (and its server)
...
`LogModelContextProtocol` with the address), then open a NEW SOCIS session.
...
restart the editor, restart the SOCIS session.
...
server fails to bind (Output Log) or SOCIS' probe times out. Fix: change
Server Port Number in Editor Preferences > Model Context Protocol AND the
`url` in `~/.socis-agent/config.yaml` (`mcp_servers.unreal-engine`), then restart
...
### 5. GenerateClientConfig is not for SOCIS

`ModelContextProtocol.GenerateClientConfig` writes config files for Claude
Code/Cursor/VSCode/Gemini/Codex into the project root. SOCIS' connection
lives in `~/.socis-agent/config.yaml` via `socis mcp install unreal-engine`.
Running GenerateClientConfig neither helps nor harms SOCIS — just don't
mistake it for the SOCIS setup step.
...
explicitly warns against overlapping calls. SOCIS executes same-turn tool
...
### 9. Timeouts: SOCIS gives up before Unreal does

SOCIS' default per-call timeout is 120 s. Asset imports, first-shader
...
`<Project>/Saved/Screenshots/...`). If SOCIS runs elsewhere (SSH backend,
