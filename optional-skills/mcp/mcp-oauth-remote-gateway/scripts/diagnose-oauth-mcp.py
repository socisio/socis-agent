...
Usage:
  ...
  <server_name> matches the files in $SOCIS_AGENT_HOME/mcp-tokens/<server>.json etc.
...
def _socis_agent_home():
    # Prefer SOCIS' own resolver (profile-safe); fall back to env then ~/.socis-agent.
    try:
        from socis_agent_constants import get_socis_agent_home
        return str(get_socis_agent_home())
    except Exception:
        return os.environ.get("SOCIS_AGENT_HOME") or os.path.expanduser("~/.socis-agent")


def _tokens_dir():
    return os.path.join(_socis_agent_home(), "mcp-tokens")
    ...
    status, hdrs, body = _post(
        ...
        data={
            ...
            "params": {"protocolVersion": "2025-06-18", "capabilities": {},
                       "clientInfo": {"name": "socis-diag", "version": "1.0"}},
        },
        headers={
