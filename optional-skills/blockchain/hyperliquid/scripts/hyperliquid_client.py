#!/usr/bin/env python3
"""
Hyperliquid CLI Tool for SOCIS Agent
-------------------------------------
...
def _dotenv_paths() -> List[Path]:
    ...
    user_env = _socis_agent_home() / ".env"
    if user_env.exists():
...
def _resolve_user(user: Optional[str]) -> str:
    ...
    sys.exit(
        ...
        f"{DEFAULT_USER_ENV} in your environment or {_socis_agent_home() / '.env'}."
    )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Hyperliquid CLI Tool for SOCIS Agent")
