#!/usr/bin/env python3
"""Canvas LMS API CLI for SOCIS Agent.
...
def _check_config():
    ...
    if missing:
        socis_env = os.path.join(
            os.environ.get("SOCIS_AGENT_HOME", os.path.expanduser("~/.socis-agent")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {socis_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for SOCIS Agent"
    )
