...
$SOCIS_AGENT_OSINT_CACHE/icij/ (default: ~/.cache/socis-osint/icij/) and search
...
def _cache_dir() -> Path:
    base = os.environ.get("SOCIS_AGENT_OSINT_CACHE")
    if base:
        return Path(base) / "icij"
    return Path.home() / ".cache" / "socis-osint" / "icij"
...
def _download(dest: Path, force: bool = False) -> Path:
    ...
    req = urllib.request.Request(
        BULK_URL,
        headers={"User-Agent": "socis-agent osint-investigation skill"},
    )
...
def main() -> int:
    ...
    p.add_argument(
        ...
        type=Path,
        default=None,
        help="Override cache directory (default: $SOCIS_AGENT_OSINT_CACHE/icij or ~/.cache/socis-osint/icij)",
    )
    p.add_argument(
        "--force-refresh",
        action="store_true",
