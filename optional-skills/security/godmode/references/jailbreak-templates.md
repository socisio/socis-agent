...
## 5. GODMODE FAST — SOCIS 4 405B (Zero Refusal)

**Model:** `socis/socis-4-405b`
**Strategy:** SOCIS models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with SOCIS Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.socis-agent/config.yaml`:
