...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("SOCIS_AGENT_HOME", os.path.expanduser("~/.socis-agent")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("SOCIS_AGENT_HOME", Path.home() / ".socis-agent")) / "skills" / "red-teaming" / "godmode"
...
SOCIS_AGENT_HOME = Path(os.getenv("SOCIS_AGENT_HOME", Path.home() / ".socis-agent"))
CONFIG_PATH = SOCIS_AGENT_HOME / "config.yaml"
PREFILL_PATH = SOCIS_AGENT_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/SOCIS models — already uncensored, just needs clean prompt
    "socis": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "socis" in model_lower or "nous" in model_lower:
        return "socis"
...
def _get_current_model() -> tuple:
    """Read current model and provider from SOCIS config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.socis-agent/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart SOCIS for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart SOCIS for changes to take effect.")
