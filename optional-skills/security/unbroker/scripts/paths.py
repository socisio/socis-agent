...
All per-subject data lives under PDD_DATA_DIR (default: $SOCIS_AGENT_HOME/unbroker),
which is the same trust boundary SOCIS uses for .env and OAuth tokens.
"""
...
def socis_agent_home() -> Path:
    return Path(os.environ.get("SOCIS_AGENT_HOME") or (Path.home() / ".socis-agent"))


def data_dir() -> Path:
    override = os.environ.get("PDD_DATA_DIR")
    return Path(override) if override else socis_agent_home() / "unbroker"
