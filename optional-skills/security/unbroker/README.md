...
The design is **SOCIS-native**: a small deterministic Python CLI (`scripts/pdd.py`) owns the state
...
```bash
socis skills install official/security/unbroker
```

Then start a new SOCIS session and drive it (below). The skill works zero-config; a few optional
...
- **PII handling.** Dossiers live under the SOCIS home directory (`0600`, optionally
