"""Run the read-tool eval through the REAL SOCIS AIAgent.

For each task: fresh temp SOCIS_AGENT_HOME, fresh fixture workspace, real
...
def run_task(task, model: str, provider: str, timeout_mult: float,
    ...
    socis_agent_home = Path(tempfile.mkdtemp(prefix="readtool-home-")) / ".socis-agent"
    ...
    try:
        ...
        shutil.rmtree(socis_agent_home.parent, ignore_errors=True)
    return result
...
def main() -> int:
    ...
    if not os.environ.get("OPENROUTER_API_KEY"):
        raise SystemExit(
            ...
            "source ~/.socis-agent/.env; set +a  — then relaunch."
        )
