...
# --- hermetic env BEFORE any socis import -------------------------------
for var in list(os.environ):
    if var.endswith(("_API_KEY", "_TOKEN")) and var != "OPENROUTER_API_KEY":
        os.environ.pop(var, None)
os.environ.pop("FAL_KEY", None)
os.environ.pop("SOCIS_AGENT_PROFILE", None)

tmp_root = tempfile.mkdtemp(prefix=f"ab-{ARM}-{TASK_ID}-")
socis_agent_home = os.path.join(tmp_root, ".socis-agent")
workspace = os.path.join(tmp_root, "ws")
os.makedirs(socis_agent_home)
os.makedirs(workspace)
with open(os.path.join(socis_agent_home, "config.yaml"), "w", encoding="utf-8") as f:
    f.write("model:\n  provider: openrouter\n  model: %s\n" % MODEL)
...
PREAMBLE = ("You are running inside the SOCIS desktop app on the user's machine. "
...
ctx = {
    "workspace": workspace, "socis_agent_home": socis_agent_home,
