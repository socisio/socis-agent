...
ctx keys: workspace, socis_agent_home, events (desktop emit records),
...
def _grep_home(ctx, needle):
    """Binary-safe grep of the entire SOCIS_AGENT_HOME tree (sqlite included)."""
    nb = needle.encode()
    for root, _dirs, files in os.walk(ctx["socis_agent_home"]):
        for fn in files:
            try:
