"""
Timezone-aware clock for SOCIS.

Provides a single ``now()`` helper that returns a timezone-aware datetime
based on the user's configured IANA timezone (e.g. ``Asia/Kolkata``).

Resolution order:
  1. ``SOCIS_AGENT_TIMEZONE`` environment variable
  2. ``timezone`` key in ``~/.socis-agent/config.yaml``
  3. Falls back to the server's local time (``datetime.now().astimezone()``)

Invalid timezone values log a warning and fall back safely — SOCIS never
...
try:
    ...
    # Python 3.8 fallback (shouldn't be needed — SOCIS requires 3.9+)
    from backports.zoneinfo import ZoneInfo  # type: ignore[no-redef]

# Cached state, keyed to the active timezone source. This process can multiplex
# profiles by switching SOCIS_AGENT_HOME (context override or env), so a single
...
def _timezone_cache_identity() -> Tuple[str, str]:
    ...
    tz_env = os.getenv("SOCIS_AGENT_TIMEZONE", "").strip()
    if tz_env:
...
def get_timezone() -> Optional[ZoneInfo]:
    ...
    ``SOCIS_AGENT_TIMEZONE`` override or the active profile's config path — so a
...
def reset_cache() -> None:
    ...
    config edit or ``SOCIS_AGENT_TIMEZONE`` update) to force ``get_timezone()`` /
