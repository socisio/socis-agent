...
Registry and ABC for every inference provider SOCIS knows about.
...
`$SOCIS_AGENT_HOME/plugins/model-providers/<name>/` (per-user overrides). The
...
- `socis_cli/auth.py` extends `PROVIDER_REGISTRY` with every api-key
  profile it sees (skipping `copilot`, `kimi-coding`, `kimi-coding-cn`,
  `zai`, `openrouter`, `custom` — those need bespoke token resolution).
- `socis_cli/models.py` extends `CANONICAL_PROVIDERS` and calls
  `profile.fetch_models()` inside `provider_model_ids()`.
- `socis_cli/doctor.py` adds a `/models` health check for each
  `auth_type="api_key"` profile.
- `socis_cli/config.py` injects every `env_var` into
  `OPTIONAL_ENV_VARS` so the setup wizard knows about it.
- `socis_cli/runtime_provider.py` reads `profile.api_mode` as a fallback
...
under `$SOCIS_AGENT_HOME/plugins/model-providers/` for a private plugin).
