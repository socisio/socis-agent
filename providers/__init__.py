...
1. Bundled plugins: ``plugins/model-providers/<name>/`` (shipped with socis-agent)
2. User plugins: ``$SOCIS_AGENT_HOME/plugins/model-providers/<name>/``
3. Pip-installed plugins: distributions exposing a ``socis_agent.plugins``
...
def register_provider(profile: ProviderProfile) -> None:
    ...
    plugins under ``$SOCIS_AGENT_HOME/plugins/model-providers/`` can override
...
def _user_plugins_dir() -> Path | None:
    ...
    try:
        ...
        d = get_socis_agent_home() / "plugins" / "model-providers"
...
def _installed_plugins_dir() -> Path | None:
    """Return ``$SOCIS_AGENT_HOME/plugins/`` if it exists.

    This is where ``socis plugins install`` clones a plugin — flat, one
    ...
    try:
        ...
        d = get_socis_agent_home() / "plugins"
        return d if d.is_dir() else None
    except Exception:
        return None
...
def _import_plugin_dir(plugin_dir: Path, source: str) -> None:
    ...
    # multiple SOCIS_AGENT_HOME profiles don't alias each other.
    safe_name = plugin_dir.name.replace("-", "_")
    if source == "bundled":
        module_name = f"plugins.model_providers.{safe_name}"
    else:
        module_name = f"_socis_user_provider_{safe_name}"
...
def _discover_entry_point_providers() -> None:
    """Import pip-installed provider plugins via the ``socis_agent.plugins``
    ...
    * **Provider targets only.** The ``socis_agent.plugins`` group is shared
    ...
    first, so filesystem plugins (bundled + ``$SOCIS_AGENT_HOME``) keep their
    ...
    try:
        from socis_cli.plugins import _get_disabled_plugins, _get_enabled_plugins
...
def _discover_providers() -> None:
    ...
    Order:
      ...
      2. User plugins at ``$SOCIS_AGENT_HOME/plugins/model-providers/<name>/``
      2b. Plugins installed by ``socis plugins install`` at
          ``$SOCIS_AGENT_HOME/plugins/<name>/`` that declare ``kind: model-provider``
    ...
    # 0. Pip-installed plugins — entry points in the ``socis_agent.plugins``
    ...
    #    filesystem steps means a bundled or ``$SOCIS_AGENT_HOME`` profile of the
    ...
    # 2. User plugins — under $SOCIS_AGENT_HOME/plugins/model-providers/<name>/.
    ...
    # 2b. Plugins installed by ``socis plugins install`` / the plugin index.
    #     Those clone into $SOCIS_AGENT_HOME/plugins/<name>/ — flat, NOT under
