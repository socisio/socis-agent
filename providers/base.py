...
def _profile_user_agent() -> str:
    """Return a ``socis-cli/<version>`` UA string, with a stable fallback.
    ...
    """
    try:
        from socis_cli import __version__ as _ver  # lazy: avoid layer cycle at import time
        return f"socis-cli/{_ver}"
    except Exception:
        return "socis-cli"
...
class ProviderProfile:
    ...
    # describe how to launch it; socis_cli/auth.py's
    ...
    def create_client(self, **client_kwargs: Any) -> Any | None:
        ...
        a profile registered from ``~/.socis-agent/plugins/model-providers/`` or a
    ...
    def fetch_models(
        ...
        *,
        ...
        # socis-cli UA so the catalog endpoint is reachable.
