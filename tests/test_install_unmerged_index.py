...
The ``socis update`` Python path already clears the conflict with ``git reset``
...
def test_install_ps1_stops_venv_resident_processes_before_parking_venv() -> None:
    ...
    ``venv\\Scripts\\pythonw.exe -m socis_cli.main gateway run`` — image name
    ``pythonw``, not ``socis.exe`` — so the ``taskkill /IM socis.exe`` guard
    ...
    # The socis.exe tree-kill is preserved (kills spawned child processes too).
    assert 'taskkill /F /T /IM socis.exe' in text
