"""Tests for SOCISCLI initialization -- catches configuration bugs
...
def _make_cli(env_overrides=None, config_overrides=None, **kwargs):
    """Create a SOCISCLI instance with minimal mocking."""
    import importlib
    ...
    try:
        with patch.dict(sys.modules, prompt_toolkit_stubs), \
            ...
            with patch.object(_cli_mod, "get_tool_definitions", return_value=[]), \
                ...
                return _cli_mod.SOCISCLI(**kwargs)
    finally:
...
class TestFallbackChainInit:
    def test_merges_new_and_legacy_fallback_config(self):
        cli = _make_cli(config_overrides={
            ...
            "fallback_model": {"provider": "nous", "model": "SOCIS-4"},
        })
...
class TestHistoryDisplay:
    ...
    def test_resume_without_target_lists_recent_sessions(self, capsys):
        ...
        cli._session_db.list_sessions_rich.return_value = [
            ...
            },
            {
                ...
                "title": "Checking Running SOCIS Agent",
                "preview": "check running gateways for socis agent",
                "last_active": 0,
            },
        ]
        ...
        assert "Checking Running SOCIS Agent" in output
    ...
    def test_sessions_with_target_delegates_to_resume(self):
        ...
        with patch.object(cli, "_handle_resume_command") as mock_resume:
            cli.process_command("/sessions Checking Running SOCIS Agent")

        mock_resume.assert_called_once_with(
            "/resume Checking Running SOCIS Agent"
        )
...
class TestNestedDictModelDefaultPairing:
    ...
    model AND the nested provider, so ``SOCISCLI`` routes the model through
...
class TestRootLevelProviderOverride:
    ...
    def test_normalize_root_model_keys_moves_to_model(self):
        ...
        from socis_cli.config import _normalize_root_model_keys
