...
Regression: the old implementation wrote ``socis_conversation_<ts>.json``
to the current working directory (CWD). Users who ran /save expected the
file to be discoverable via ``socis sessions browse``, but CWD-resident
snapshots are not indexed in the state DB and are generally invisible.
The fix writes snapshots under ``~/.socis-agent/sessions/saved/`` and prints
...
def socis_agent_home(tmp_path, monkeypatch):
    ...
    # Clear any cached socis_agent_home computation
    import socis_agent_constants
    if hasattr(socis_agent_constants, "_socis_agent_home_cache"):
        socis_agent_constants._socis_agent_home_cache = None
    return home
...
def test_save_conversation_writes_under_socis_agent_home(socis_agent_home, tmp_path, monkeypatch, capsys):
    """Snapshot must land under ~/.socis-agent/sessions/saved/, not CWD."""
    # Change CWD to a different directory to prove the file does NOT go there.
    work = tmp_path / "somewhere-else"
    work.mkdir()
    monkeypatch.chdir(work)

    # Import fresh to pick up the SOCIS_AGENT_HOME fixture
    for mod in [m for m in sys.modules if m.startswith("cli") or m == "socis_agent_constants"]:
    ...
    cli.SOCISCLI.save_conversation(stub, "/save json")

    # File must NOT be in CWD
    cwd_leak = list(work.glob("socis_conversation_*.json"))
    assert not cwd_leak, f"snapshot leaked to CWD: {cwd_leak}"

    # File MUST be under ~/.socis-agent/sessions/saved/
    saved_dir = socis_agent_home / "sessions" / "saved"
    assert saved_dir.is_dir(), "expected saved/ subdirectory to be created"
    files = list(saved_dir.glob("socis_conversation_*.json"))
    ...
    assert "socis --resume 20260101_120000_abc123" in out, out


def test_save_conversation_empty_history_does_nothing(socis_agent_home, capsys):
    for mod in [m for m in sys.modules if m.startswith("cli") or m == "socis_agent_constants"]:
        sys.modules.pop(mod, None)
    import cli
...
def test_save_conversation_bare_shows_usage(socis_agent_home, capsys):
    ...
    cli.SOCISCLI.save_conversation(stub, "/save")
...
def test_save_conversation_bad_format_shows_usage(socis_agent_home, capsys):
    for mod in [m for m in sys.modules if m.startswith("cli") or m == "socis_agent_constants"]:
        sys.modules.pop(mod, None)
    import cli

    stub = _make_stub_cli([{"role": "user", "content": "hi"}])
    cli.SOCISCLI.save_conversation(stub, "/save pdf")
