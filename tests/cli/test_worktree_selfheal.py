...
Regression for the Aug 2026 `socis -w` timeout incident: 39 accumulated packs
...
(lock pid = the live socis process), poisoning every retry.
...
class TestCleanupFailedWorktreeAdd:
    def _simulate_timed_out_add(self, repo):
        ...
        wt = repo / ".worktrees" / "socis-dead00"
        _git(repo, "worktree", "add", str(wt), "-b", "socis/socis-dead00")
        # Live-pid lock, exactly what `socis -w` writes before the checkout.
        _git(repo, "worktree", "lock", str(wt), "--reason", "socis pid=999999")
    ...
    def test_retry_succeeds_after_cleanup(self, repo):
        ...
        result = _git(
            repo, "worktree", "add", str(wt), "-b", "socis/socis-dead00", check=False
        )
    ...
    def test_noop_when_nothing_exists(self, repo):
        ...
        _cleanup_failed_worktree_add(
            str(repo), repo / ".worktrees" / "never-existed", "socis/never-existed"
        )  # must not raise
