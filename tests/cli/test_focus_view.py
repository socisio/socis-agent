...
class TestStatusBarSegment:
    ...
    def test_text_renderer_includes_the_badge_at_every_width_tier(self, width):
        ...
        host = SOCISCLI.__new__(SOCISCLI)
        host.model = "opus"
        ...
        with patch.object(SOCISCLI, "_get_status_bar_snapshot", return_value=snapshot), \
             patch.object(SOCISCLI, "_is_session_yolo_active", return_value=False):
            text = SOCISCLI._build_status_bar_text(host, width=width)
