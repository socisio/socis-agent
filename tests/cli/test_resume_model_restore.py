...
SessionDB.session_gateway_runtime (socis_agent_state.py) — the round trip that
makes `socis --resume` reopen a session on the model/provider it actually
...
def _make_stub(**overrides):
    """Bare SOCISCLI the way resume paths see it (no __init__)."""
    stub = object.__new__(cli_mod.SOCISCLI)
