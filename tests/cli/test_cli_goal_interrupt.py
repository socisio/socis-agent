...
minimal ``SOCISCLI`` stub (pattern used elsewhere in tests/cli).
"""
...
def socis_agent_home(tmp_path, monkeypatch):
    """Isolated SOCIS_AGENT_HOME so SessionDB.state_meta writes stay hermetic."""
    home = tmp_path / ".socis-agent"
    home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    monkeypatch.setenv("SOCIS_AGENT_HOME", str(home))

    # Bust the goal module's DB cache so it re-resolves SOCIS_AGENT_HOME each test.
...
def _make_cli_with_goal(session_id: str, goal_text: str = "build a thing"):
    """Build a minimal SOCISCLI stub with an active goal wired in."""
...
class TestInterruptAutoPause:
    ...
    def test_interrupted_turn_is_resumable(self, socis_agent_home):
        ...
        with patch("socis_cli.goals.judge_goal"):
...
class TestHealthyTurnStillRuns:
    ...
    def test_clean_response_marks_done_when_judge_says_done(self, socis_agent_home):
...
class TestInterruptFlagLifecycle:
    def test_chat_resets_flag_at_entry(self, socis_agent_home):
        ...
        src = inspect.getsource(SOCISCLI.chat)
