...
def progress(tmp_path):
    ...
    status = tmp_path / "socis-update-status"
...
# Stands in for `socis update`, and reports the stage that was on screen
...
n="$(cat "$SOCIS_AGENT_TEST_CALLS" 2>/dev/null || echo 0)"; n=$((n + 1))
printf '%s' "$n" > "$SOCIS_AGENT_TEST_CALLS"
for f in "$TMPDIR"/socis-update-status.[0-9]*; do
  case "$f" in *.tmp) continue ;; esac
  cp "$f" "$SOCIS_AGENT_TEST_CAPTURE.$n" 2>/dev/null
done
exit "$(cat "$SOCIS_AGENT_TEST_EXITS.$n" 2>/dev/null || echo 0)"
"""
...
def _run_handoff(tmp_path, exits: dict[int, int]) -> list[dict]:
    ...
    install_root = tmp_path / "socis-agent"
    (install_root / "venv" / "bin").mkdir(parents=True)
    socis = install_root / "venv" / "bin" / "socis"
    socis.write_text(FAKE_SOCIS)
    socis.chmod(0o755)
    ...
    env = {
        **os.environ,
        "TMPDIR": str(tmp_path),
        "SOCIS_AGENT_TEST_CAPTURE": str(capture),
        "SOCIS_AGENT_TEST_CALLS": str(calls),
        "SOCIS_AGENT_TEST_EXITS": str(tmp_path / "exits"),
    }
    ...
    result = tmp_path / ".socis-update-result.json"
...
def test_update_gate_publishes_its_stage_before_running(tmp_path):
    """`socis update` is the longest wait in the hand-off and the one the
