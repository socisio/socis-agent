...
1. ``load_socis_dotenv()`` auto-loads ``~/.socis-agent/.op.env`` so the
...
def test_op_env_autoloads_bootstrap_token_in_cron_context(tmp_path, monkeypatch):
    ...
    env_loader.load_socis_dotenv(socis_agent_home=home)
...
def _seed_openrouter_token(monkeypatch, dotenv_value, environ_value):
    ...
    monkeypatch.setattr(
        "socis_cli.auth.is_source_suppressed", lambda _p, _s: False
    )
