...
def test_registered_observable_metric_names_cover_snapshot_metrics(monkeypatch):
    ...
    gateway_snapshot = type("S", (), {"metrics": [
        _M("socis.gateway.up"), _M("socis.gateway.active_agents"),
        _M("socis.gateway.busy"), _M("socis.gateway.drainable"),
        _M("socis.gateway.restart_requested"),
        _M("socis.platform.up"), _M("socis.platform.degraded"),
    ]})()
    cron_snapshot = type("S", (), {"metrics": [
        _M("socis.cron.scheduler.heartbeat_age_seconds"),
        _M("socis.cron.scheduler.last_success_age_seconds"),
        _M("socis.cron.scheduler.catch_up_occurrences"),
        _M("socis.cron.jobs.enabled"), _M("socis.cron.jobs.running"),
        _M("socis.cron.jobs.overdue"),
    ]})()
...
def test_monitoring_docs_distinguish_relay_health_scope_and_terminal_flush():
    ...
    assert "SOCIS Agent-owned Relay transport health" in text
