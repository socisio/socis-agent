...
class TestLoadMCPConfig:
    ...
    def test_valid_config_parsed(self):
        ...
        with patch("socis_cli.config.load_config", return_value={"mcp_servers": servers}):
    ...
    def test_mcp_servers_not_dict_returns_empty(self):
        ...
        with patch("socis_cli.config.load_config", return_value={"mcp_servers": "invalid"}):
            from tools.mcp_tool import _load_mcp_config
            result = _load_mcp_config()
            assert result == {}
    ...
    def test_portable_servers_merge_after_native_interpolation(self):
        ...
        with (
            patch("socis_cli.config.load_config", return_value={"mcp_servers": native}),
            patch("socis_cli.plugins.discover_plugins"),
            patch("socis_cli.plugins.get_plugin_manager", return_value=manager),
...
class TestSchemaConversion:
    def test_converts_mcp_tool_to_socis_schema(self):
...
class TestToolsetInjection:
    ...
    def test_partial_failure_retry_on_second_call(self):
        ...
        fake_toolsets = {
            "socis-cli": {"tools": [], "description": "CLI", "includes": []},
        }
