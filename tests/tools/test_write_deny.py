...
class TestWriteDenyExactPaths:
    ...
    def test_socis_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.socis-agent/.env`` (whose credentials are inherited by every profile)
        could be silently overwritten by ``write_file`` while a profile was
        active.
        """
        root = tmp_path / "socis_root"
        ...
        # Sanity check: SOCIS_AGENT_HOME does point to the profile dir, not the root.
        from socis_agent_constants import get_socis_agent_home, get_default_socis_root
...
class TestWriteAllowed:
    ...
    def test_socis_control_files_requested_writable(self):
