"""Tests for socis_subprocess_env() — the centralized credential-safe env
...
full credential environment. Two tiers:
  ...
  * Tier 2 (_SOCIS_AGENT_PROVIDER_ENV_BLOCKLIST): LLM provider/tool keys — stripped
...
_TIER1_SAMPLE = {
    ...
    "SOCIS_AGENT_DASHBOARD_SESSION_TOKEN": "dash-secret",
}
...
def _build(extra=None, *, inherit_credentials=False):
    ...
    with patch.dict(os.environ, env, clear=True):
        return socis_subprocess_env(inherit_credentials=inherit_credentials)
...
class TestStripByDefault:
    ...
    def test_buzz_platform_vars_stripped_by_default(self):
        ...
        for var in buzz_sample:
            assert var not in result, f"{var} leaked via socis_subprocess_env"
...
class TestDelegatedChildMarker:
    def test_delegated_child_context_scrubs_parent_kanban_keys_and_sets_marker(self):
        ...
        with patch.dict(
            ...
            {
                **_SAFE_SAMPLE,
                "SOCIS_AGENT_KANBAN_TASK": "t_parent",
                "SOCIS_AGENT_KANBAN_RUN_ID": "123",
                "SOCIS_AGENT_KANBAN_DB": "/tmp/parent-kanban.db",
                "SOCIS_AGENT_KANBAN_WORKSPACE": "/tmp/parent-workspace",
            },
            clear=True,
        ):
