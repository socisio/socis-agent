...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_tool._scrub_child_env — broad SOCIS_AGENT_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: SOCIS_AGENT_GATEWAY_SESSION set, a bound session
...
def test_env_scrub_socis_allowlist_and_secret_blocks():
    from tools.code_execution_tool import _scrub_child_env

    env = {
        # operational allowlist → kept
        "SOCIS_AGENT_HOME": "/h", "SOCIS_AGENT_PROFILE": "p",
        "SOCIS_AGENT_CONFIG": "/c.yaml", "SOCIS_AGENT_ENV": "/e",
        "SOCIS_AGENT_DELEGATED_CHILD_CONTEXT": "1",
        # other SOCIS_AGENT_* → dropped (broad prefix removed)
        "SOCIS_AGENT_BASE_URL": "https://x", "SOCIS_AGENT_INTERACTIVE": "1",
        "SOCIS_AGENT_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in (
        ...
        "SOCIS_AGENT_DELEGATED_CHILD_CONTEXT", "PATH",
    ):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "SOCIS_AGENT_BASE_URL", "SOCIS_AGENT_INTERACTIVE", "SOCIS_AGENT_KANBAN_DB",
...
def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped SOCIS_AGENT_* vars."""
    import logging
    ...
    with caplog.at_level(logging.DEBUG, logger="tools.code_execution_tool"):
        _scrub_child_env(
            {"SOCIS_AGENT_HOME": "/h", "PATH": "/usr/bin"},
