...
UDS socket lifecycle, socis_tools generation, timeout enforcement,
...
class TestSOCISToolsGeneration(unittest.TestCase):
    ...
    def test_file_transport_uses_tempfile_fallback_for_rpc_dir(self):
        src = generate_socis_tools_module(["terminal"], transport="file")
        self.assertIn("import json, os, shlex, tempfile, threading, time", src)
        self.assertIn("os.path.join(tempfile.gettempdir(), \"socis_rpc\")", src)
        self.assertNotIn('os.environ.get("SOCIS_AGENT_RPC_DIR", "/tmp/socis_rpc")', src)
    ...
    def test_uds_transport_serializes_concurrent_calls(self):
        ...
        src = generate_socis_tools_module(["terminal"], transport="uds")
...
class TestExecuteCodeRemoteTempDir(unittest.TestCase):
    def test_execute_remote_uses_backend_temp_dir_for_sandbox(self):
        ...
        mkdir_cmd = next(cmd for cmd, _, _ in env.commands
                         if "mkdir -p" in cmd and "socis_exec_" in cmd)
        run_cmd = next(cmd for cmd, _, _ in env.commands if "python3 script.py" in cmd)
        cleanup_cmd = next(cmd for cmd, _, _ in env.commands
                           if "rm -rf" in cmd and "socis_exec_" in cmd)
        self.assertIn("mkdir -p /data/data/com.termux/files/usr/tmp/socis_exec_", mkdir_cmd)
        self.assertIn("SOCIS_AGENT_RPC_DIR=/data/data/com.termux/files/usr/tmp/socis_exec_", run_cmd)
        self.assertIn("rm -rf /data/data/com.termux/files/usr/tmp/socis_exec_", cleanup_cmd)
        self.assertNotIn("mkdir -p /tmp/socis_exec_", mkdir_cmd)

    def test_timezone_shell_quoted_in_remote_execution(self):
        """SOCIS_AGENT_TIMEZONE must be shell-quoted in remote env_prefix to prevent injection."""
        class FakeEnv:
            def __init__(self):
                self.commands = []
        ...
        with patch("tools.code_execution_tool._load_config",
             ...
             patch.dict(os.environ, {"SOCIS_AGENT_TIMEZONE": malicious_tz}):
            result = json.loads(_execute_remote("print('hello')", "task-1", ["terminal"]))
...
class TestExecuteCode(unittest.TestCase):
    ...
    def test_repo_root_modules_are_importable(self):
        ...
        result = self._run('import socis_agent_constants; print(socis_agent_constants.__file__)')
        self.assertEqual(result["status"], "success")
        self.assertIn("socis_agent_constants.py", result["output"])
...
"""
    ...
    def test_shell_quote_helper(self):
        ...
        code = """
from socis_tools import shell_quote
...
"""
    ...
    def test_json_parse_helper_bom(self):
        ...
        code = """
from socis_tools import json_parse
...
class TestEnvVarFiltering(unittest.TestCase):
    ...
    def test_socis_rpc_socket_injected(self):
        child_env = self._get_child_env()
        self.assertIn("SOCIS_AGENT_RPC_SOCKET", child_env)
...
class TestRpcTokenAuthorization(unittest.TestCase):
    ...
    request without the matching SOCIS_AGENT_RPC_TOKEN must be rejected before
    ...
    def test_generated_module_sends_token(self):
        """The generated socis_tools module reads SOCIS_AGENT_RPC_TOKEN and sends it."""
        src = generate_socis_tools_module(["terminal"], transport="uds")
        self.assertIn("SOCIS_AGENT_RPC_TOKEN", src)
