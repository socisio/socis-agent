...
class TestDefaults:
    ...
    def test_get_limits_returns_defaults_when_load_config_raises(self):
        ...
        with patch("socis_cli.config.load_config", side_effect=_boom):
...
class TestDefaultConfigHasSection:
    """The DEFAULT_CONFIG in socis_cli.config must expose tool_output so
    that ``socis setup`` and default installs stay in sync with the
    helpers here."""
