...
SOCIS exposes ``terminal.temp_dir`` (mirrored to ``TERMINAL_TEMP_DIR``) so
...
def test_default_is_socis_cache_not_tmp(tmp_path, monkeypatch):
    """With no overrides at all, the default temp root is real storage under
    SOCIS_AGENT_HOME (cache/terminal), NOT tmpfs /tmp."""
    import socis_agent_constants  # noqa: F401 — resolves SOCIS_AGENT_HOME per call
    ...
    assert result == str(tmp_path / ".socis-agent" / "cache" / "terminal")
...
def test_cleanup_terminal_temp_cache(tmp_path, monkeypatch):
    ...
    assert not (root / "socis_bg_dead1.log").exists()
