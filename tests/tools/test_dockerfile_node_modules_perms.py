...
Older images made /opt/socis/ui-tui and /opt/socis/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV SOCIS_AGENT_TUI_DIR=/opt/socis/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R socis:socis /opt/socis/ui-tui" not in text
    assert "chown -R socis:socis /opt/socis/node_modules" not in text
