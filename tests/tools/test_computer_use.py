...
def _reset_backend():
    ...
    with patch.dict(os.environ, {"SOCIS_AGENT_COMPUTER_USE_BACKEND": "noop"}, clear=False):
        yield
    reset_backend_for_tests()
...
class TestRegistration:
    ...
    def test_cua_driver_cmd_env_override_is_resolved_dynamically(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("SOCIS_AGENT_CUA_DRIVER_CMD", str(driver))
...
class TestLazyMcpInstall:
    ...
    def test_start_reports_incompatible_existing_driver_before_mcp_setup(self):
        ...
        with patch.object(
            ...
            with pytest.raises(RuntimeError, match="socis computer-use install"):
                cua_backend.CuaDriverBackend().start()
...
class TestContractAutoRepair:
    ...
    installed driver (a state SOCIS' own version-floor bump created),
    ...
    def test_start_auto_repairs_incompatible_driver(self, monkeypatch):
        ...
        with patch.object(
             ...
             patch("socis_cli.tools_config.install_cua_driver",
    ...
    def test_explicit_override_is_never_repaired(self, monkeypatch):
        ...
        with patch.object(
             ...
             patch("socis_cli.tools_config.install_cua_driver") as installer, \
             patch("tools.lazy_deps.ensure"):
            with pytest.raises(RuntimeError, match="SOCIS_AGENT_CUA_DRIVER_CMD"):
                cua_backend.CuaDriverBackend().start()
...
class TestCuaDriverSessionReconnect:
    ...
    def test_reconnect_restores_public_label_before_replaying_read(self):
        ...
        bridge = FakeBridge()
        session = self._make_session(bridge)
        session._declared_session_id = "socis-label"
        ...
        assert bridge.calls == [
            ("call", "list_apps", {}),
            ("call", "start_session", {"session": "socis-label"}),
            ("call", "list_apps", {}),
        ]
...
class TestCuaEnvironmentScrubbing:
    ...
    def test_cua_session_sanitizes_provider_env_vars(self):
        ...
        The cua-driver MCP subprocess should not inherit SOCIS-managed
...
class TestMcpInvocationResolution:
    ...
    def test_falls_back_when_manifest_missing_command(self):
        ...
        resolved driver path (so SOCIS_AGENT_CUA_DRIVER_CMD still wins)."""
        from unittest.mock import patch
        from tools.computer_use.cua_backend import _resolve_mcp_invocation
...
class TestSessionLifecycle:
    """Surface gap (audit June 2026): SOCIS never declared a cua-driver
