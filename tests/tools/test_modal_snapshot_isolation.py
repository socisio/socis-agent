...
def _restore_tool_modules():
    original_socis_agent_home = os.environ.get("SOCIS_AGENT_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_socis_agent_home is None:
            os.environ.pop("SOCIS_AGENT_HOME", None)
        else:
            os.environ["SOCIS_AGENT_HOME"] = original_socis_agent_home
        _reset_modules(("tools", "socis_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    socis_cli = types.ModuleType("socis_cli")
    socis_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["socis_cli"] = socis_cli
    socis_agent_home = tmp_path / "socis-home"
    os.environ["SOCIS_AGENT_HOME"] = str(socis_agent_home)
    sys.modules["socis_cli.config"] = types.SimpleNamespace(
        get_socis_agent_home=lambda: socis_agent_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="socis-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": socis_agent_home / "modal_snapshots.json",
