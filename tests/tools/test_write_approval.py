...
and the shared slash-command handlers (socis_cli/write_approval_commands.py).
...
@pytest.fixture
def socis_agent_home(monkeypatch):
    d = tempfile.mkdtemp(prefix="socis_wa_test_")
    home = os.path.join(d, ".socis-agent")
    os.makedirs(home)
    monkeypatch.setenv("SOCIS_AGENT_HOME", home)
    yield home
...
def test_default_gate_is_off(socis_agent_home):
...
def test_invalid_subsystem_is_off(socis_agent_home):
    from tools import write_approval as wa
...
def test_memory_gate_off_allows_write(socis_agent_home):
...
def test_cli_memory_approve_without_live_agent_uses_fresh_store(socis_agent_home, capsys):
...
def test_load_on_disk_store_honors_configured_limits_and_permissions(socis_agent_home, monkeypatch):
    ...
    monkeypatch.setattr("socis_cli.config.load_config", _boom)
...
def test_handle_approve_all(socis_agent_home):
...
def test_handle_approval_on(socis_agent_home):
    from socis_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
    captured = {}
...
def test_handle_approval_off(socis_agent_home):
    from socis_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
    captured = {}
    out = handle_pending_subcommand(
...
def test_memory_inline_approve_writes(socis_agent_home, approval_callback_cleanup):
    from tools.memory_tool import memory_tool, MemoryStore
    from tools.terminal_tool import set_approval_callback
    from tools import write_approval as wa
    _set_approval("memory", True)
...
def test_memory_inline_deny_blocks(socis_agent_home, approval_callback_cleanup):
    from tools.memory_tool import memory_tool, MemoryStore
    from tools.terminal_tool import set_approval_callback
    from tools import write_approval as wa
    _set_approval("memory", True)
...
def test_memory_invalid_params_rejected_before_staging(socis_agent_home):
