...
def test_output_path_rejects_socis_oauth_store(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(file_safety, "_socis_agent_home_path", lambda: socis_agent_home)
    monkeypatch.setattr(file_safety, "_socis_root_path", lambda: socis_agent_home)

    target = socis_agent_home / ".anthropic_oauth.json"
    result = json.loads(text_to_speech_tool(
        text="hello",
...
def test_output_path_rejects_mcp_token_directory(tmp_path, monkeypatch):
    ...
    socis_agent_home = tmp_path / "socis-home"
    token_dir = socis_agent_home / "mcp-tokens"
