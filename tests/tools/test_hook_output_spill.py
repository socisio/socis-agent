...
class GetSpillConfigTests(unittest.TestCase):
    ...
    def test_load_config_exception_is_swallowed(self):
        with patch("socis_cli.config.load_config", side_effect=RuntimeError("bad")):
            cfg = hos.get_spill_config()
        self.assertEqual(cfg["max_chars"], hos.DEFAULT_MAX_CHARS)
        self.assertTrue(cfg["enabled"])
...
class SpillIfOversizedTests(unittest.TestCase):
    def setUp(self):
        self.tmpdir = tempfile.mkdtemp(prefix="socis-spill-test-")
    ...
    def test_default_directory_uses_socis_agent_home(self):
        """When no directory override, spill under SOCIS_AGENT_HOME/hook_outputs."""
        test_home = tempfile.mkdtemp(prefix="socis-home-")
        try:
            with patch.dict(os.environ, {"SOCIS_AGENT_HOME": test_home}):
                # Also patch get_socis_agent_home to the env var to mirror production.
            ...
            # ~/.socis-agent/hook_outputs depending on get_socis_agent_home behaviour.
            candidates = [
                Path(test_home) / "hook_outputs" / "sess",
                Path(os.path.expanduser("~/.socis-agent/hook_outputs/sess")),
            ]
