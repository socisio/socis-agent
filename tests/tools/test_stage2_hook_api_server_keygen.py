...
was gated on ``[ -f "$SOCIS_AGENT_HOME/.env" ]`` while the first-boot seed that
...
def _run_keygen(
    ...
    script = (
        ...
        "set -eu\n"
        f"{env_setup}"
        f'SOCIS_AGENT_HOME="{home}"\n'
        # In tests we run unprivileged; as_socis is a passthrough then.
        'as_socis() { "$@"; }\n'
...
def test_keygen_skips_when_container_env_provides_key(
    ...
    SOCIS loads $SOCIS_AGENT_HOME/.env with override=True, so a key generated
...
def test_dockerignore_keeps_env_example_template() -> None:
    """The first-boot seed copies /opt/socis/.env.example -> $SOCIS_AGENT_HOME/.env.
    ...
    assert verdict == "included", (
        ...
        "seeds $SOCIS_AGENT_HOME/.env from it on first boot"
    )
