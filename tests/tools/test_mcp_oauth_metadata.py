...
Covers:
- :class:`SOCISTokenStorage` ``.meta.json`` roundtrip (save / load / remove)
- The production manager provider
  (:class:`tools.mcp_oauth_manager.SOCISMCPOAuthProvider`) restoring metadata
...
from tools.mcp_oauth_manager import _SOCIS_AGENT_PROVIDER_CLS
...
# ---------------------------------------------------------------------------
# SOCISTokenStorage metadata roundtrip
# ---------------------------------------------------------------------------
...
class TestMetadataStorage:
    def test_save_and_load_roundtrip(self, tmp_path, monkeypatch):
        ...
        storage = SOCISTokenStorage("example-server")
    ...
    def test_remove_deletes_meta_file(self, tmp_path, monkeypatch):
        monkeypatch.setenv("SOCIS_AGENT_HOME", str(tmp_path))
        storage = SOCISTokenStorage("cleanup-server")
...
# ---------------------------------------------------------------------------
# Manager-path provider (SOCISMCPOAuthProvider) — production code path
# ---------------------------------------------------------------------------


def _manager_provider_with_context(storage: SOCISTokenStorage, **context_attrs):
    ...
    provider = _SOCIS_AGENT_PROVIDER_CLS.__new__(_SOCIS_AGENT_PROVIDER_CLS)
    provider._socis_server_name = context_attrs.get("server_name", "srv")
    context = MagicMock()
...
class TestManagerOAuthProviderMetadata:
    def test_initialize_restores_metadata_from_disk(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("SOCIS_AGENT_HOME", str(tmp_path))
        storage = SOCISTokenStorage("mgr-srv")
        ...
        with patch.object(
            _SOCIS_AGENT_PROVIDER_CLS.__bases__[0], "_initialize", new=AsyncMock()
        ):
    ...
    def test_async_auth_flow_persists_on_completion(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("SOCIS_AGENT_HOME", str(tmp_path))
        storage = SOCISTokenStorage("flow-srv")
        ...
        with patch.object(
            _SOCIS_AGENT_PROVIDER_CLS.__bases__[0],
            "async_auth_flow",
