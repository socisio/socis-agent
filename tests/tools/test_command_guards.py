...
def _mode_manual(monkeypatch):
    ...
    The test conftest redirects SOCIS_AGENT_HOME to an empty tempdir, so the
...
def _clean_state():
    ...
    saved = {}
    for k in ("SOCIS_AGENT_INTERACTIVE", "SOCIS_AGENT_GATEWAY_SESSION", "SOCIS_AGENT_EXEC_ASK", "SOCIS_AGENT_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["SOCIS_AGENT_INTERACTIVE"] = "1"
        session_key = os.getenv("SOCIS_AGENT_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No SOCIS_AGENT_INTERACTIVE or SOCIS_AGENT_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
