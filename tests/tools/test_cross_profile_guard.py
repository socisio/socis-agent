...
def fake_socis(tmp_path, monkeypatch):
    """Build a two-profile SOCIS layout and point SOCIS_AGENT_HOME at
    the socis-security profile (matching the original-incident shape).
    """
...
class TestWriteFileCrossProfileGuard:
    def test_in_profile_write_allowed(self, fake_socis):
        from tools.file_tools import write_file_tool
        target = fake_socis["sec_home"] / "skills" / "new-skill" / "SKILL.md"
    ...
    def test_cross_profile_write_allowed_guard_retired(self, fake_socis):
        ...
        from tools.file_tools import write_file_tool
        target = fake_socis["root"] / "skills" / "shared-skill" / "SKILL.md"
    ...
    def test_non_socis_path_unaffected(self, fake_socis, tmp_path):
        from tools.file_tools import write_file_tool
...
class TestPatchCrossProfileGuard:
    def test_cross_profile_patch_allowed_guard_retired(self, fake_socis):
    ...
    def test_cross_profile_patch_bypass(self, fake_socis):
        from tools.file_tools import patch_tool
        target = fake_socis["root"] / "skills" / "shared-skill" / "SKILL.md"
        result_json = patch_tool(
            mode="replace",
    ...
    def test_v4a_patch_writes_through_guard_retired(self, fake_socis):
...
class TestSkillManageCrossProfileErrorUX:
    ...
    def test_error_names_other_profile_when_skill_lives_there(
        self, fake_socis, monkeypatch
    ):
        """The original incident shape — model expects 'foo' in active
        profile, but 'foo' lives in default. Error must point at default."""
        self._make_skill_in_profile(fake_socis["root"], "default-only-skill")

        # Re-import the module so SKILLS_DIR picks up SOCIS_AGENT_HOME (set in
        ...
        assert "not found in active profile 'socis-security'" in err
...
class TestSystemPromptActiveProfile:
    def test_default_profile_line_in_prompt(self, tmp_path, monkeypatch):
        ...
        about ~/.socis-agent/profiles/<name>/."""
        # Don't set SOCIS_AGENT_HOME — falls back to default.
        import agent.file_safety as fs
        monkeypatch.setattr(fs, "_socis_agent_home_path", lambda: tmp_path / "fake")
        monkeypatch.setattr(fs, "_socis_root_path", lambda: tmp_path / "fake")
    ...
    def test_named_profile_line_in_prompt_text(self, fake_socis):
        """When active profile is 'socis-security', the prompt warns
        ...
        assert "Active SOCIS profile" in src
        assert "cross_profile=True" not in src  # guard retired
        assert "~/.socis-agent/profiles/" in src
        # Both branches present (default and named profile).
        assert "Active SOCIS profile: default" in src
        assert "Active SOCIS profile: {active_profile}" in src
