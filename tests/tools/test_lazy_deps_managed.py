...
A package-manager install (NixOS, and anything else shipping SOCIS from a
...
def _missing_and_installable(monkeypatch):
    ...
    depend on the host's ~/.socis-agent/config.yaml (a local
...
def test_nixos_install_fails_fast_without_touching_the_installer(monkeypatch):
    monkeypatch.setattr("socis_cli.config.get_managed_system", lambda: "nixos")
...
def test_durable_install_target_overrides_the_guard(monkeypatch, tmp_path):
    """The container deployment sets SOCIS_AGENT_MANAGED *and* a writable target.

    Dockerfile sets SOCIS_AGENT_LAZY_INSTALL_TARGET and the NixOS container module
    passes SOCIS_AGENT_MANAGED=true; blocking there would break that deployment.
    """
    monkeypatch.setattr("socis_cli.config.get_managed_system", lambda: "nixos")
...
def test_unreadable_config_fails_open(monkeypatch):
    ...
    monkeypatch.setattr("socis_cli.config.get_managed_system", _raise)
