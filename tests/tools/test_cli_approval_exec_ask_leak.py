...
When ``SOCIS_AGENT_EXEC_ASK`` leaks into a classic CLI process (historically via
...
class TestExecuteCodeGuardCliApprovalSurvivesExecAskLeak:
    ...
    def test_cli_callback_used_for_platform_marker_leak_without_exec_ask(
        ...
        ``SOCIS_AGENT_SESSION_PLATFORM`` is set, so the whole-script gate is
        reached with ``SOCIS_AGENT_EXEC_ASK`` entirely absent. That path must
...
class TestGatewayRunImportDoesNotSetExecAsk:
    def test_importing_gateway_run_does_not_set_exec_ask(self, tmp_path):
        ...
        script = r"""
import os, sys
os.environ.pop("SOCIS_AGENT_EXEC_ASK", None)
sys.path.insert(0, %r)
# Avoid starting the gateway; only import the module for _gateway_runner_ref
# style side imports.
import gateway.run  # noqa: F401
print("EXEC_ASK=" + repr(os.environ.get("SOCIS_AGENT_EXEC_ASK")))
""" % (str(REPO_ROOT),)
        socis_agent_home = tmp_path / "import-test-home"
        proc = subprocess.run(
            ...
            env={
                **os.environ,
                "SOCIS_AGENT_HOME": str(socis_agent_home),
            },
            timeout=60,
        )
