...
CASES = {
    ...
    "product python → nix": (["socis_cli/config.py"], _lanes(python=True, scan=True)),
    ...
    # files change — not on every socis_agent_state.py PR.
    ...
    "python source alone → no desktop_updater lane": (["socis_agent_state.py"], _lanes(python=True, scan=True)),
