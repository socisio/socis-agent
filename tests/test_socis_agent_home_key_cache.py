"""Tests for the remembered results in `socis_agent_home_key`.

`Path.resolve()` is a filesystem call. `socis_agent_home_key` sits under
...
def _uncached(path=None) -> str:
    ...
    candidate = Path(path) if path is not None else hc.get_socis_agent_home()
    return os.path.normcase(str(candidate.expanduser().resolve(strict=False)))


@pytest.fixture(autouse=True)
def _clear_cache():
    hc.reset_socis_agent_home_key_cache()
    yield
    hc.reset_socis_agent_home_key_cache()
...
class TestSameAnswerAsBefore:
    ...
    def test_matches_the_uncached_calculation(self, tmp_path, case):
        ...
        assert hc.socis_agent_home_key(target) == _uncached(target)

    def test_matches_for_the_default_home(self):
        assert hc.socis_agent_home_key() == _uncached()
    ...
    def test_accepts_a_path_object(self, tmp_path):
        (tmp_path / "real").mkdir()
        assert hc.socis_agent_home_key(tmp_path / "real") == _uncached(tmp_path / "real")

    def test_second_call_returns_the_same_string(self, tmp_path):
        (tmp_path / "real").mkdir()
        first = hc.socis_agent_home_key(str(tmp_path / "real"))
        second = hc.socis_agent_home_key(str(tmp_path / "real"))
...
class TestWhatGetsRemembered:
    def test_an_existing_path_is_remembered(self, tmp_path):
        (tmp_path / "real").mkdir()
        hc.socis_agent_home_key(str(tmp_path / "real"))
    ...
    def test_a_missing_path_is_not_remembered(self, tmp_path):
        ...
        hc.socis_agent_home_key(str(tmp_path / "not_there"))
        assert hc._HOME_KEY_CACHE == {}

    def test_a_path_created_later_picks_up_the_real_answer(self, tmp_path):
        later = tmp_path / "later"
        before = hc.socis_agent_home_key(str(later))
        later.mkdir()
        after = hc.socis_agent_home_key(str(later))
    ...
    def test_different_paths_get_their_own_entries(self, tmp_path):
        for name in ("a", "b", "c"):
            (tmp_path / name).mkdir()
            hc.socis_agent_home_key(str(tmp_path / name))
...
class TestHomeChanges:
    def test_pointing_socis_agent_home_somewhere_else_gives_a_new_key(
        ...
        key_one = hc.socis_agent_home_key()
        monkeypatch.setenv("SOCIS_AGENT_HOME", str(second))
        key_two = hc.socis_agent_home_key()
...
class TestSymlinks:
    def test_a_link_resolves_to_its_target(self, tmp_path):
        ...
        assert hc.socis_agent_home_key(str(link)) == _uncached(str(link))
        assert hc.socis_agent_home_key(str(link)) == hc.socis_agent_home_key(str(target))
