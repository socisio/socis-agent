...
Run from the PR worktree:
    ...
    cd ~/.socis-agent/socis-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.socis-agent/socis-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.socis-agent/socis-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a socis-agent checkout"
)
...
# Isolated SOCIS_AGENT_HOME for the config write.
