...
class TestPluginRegister:
    ...
    def clear_env(self, monkeypatch):
        for var in (
            ...
            "SOCIS_AGENT_DASHBOARD_OIDC_SCOPES",
            "SOCIS_AGENT_DASHBOARD_OIDC_CLIENT_SECRET",
        ):
    ...
    def test_skips_when_unconfigured(self, patch_config):
        ...
        assert "SOCIS_AGENT_DASHBOARD_OIDC_ISSUER" in oidc_plugin.LAST_SKIP_REASON
        assert "self_hosted" in oidc_plugin.LAST_SKIP_REASON


    def test_registers_from_env(self, patch_config, monkeypatch):
        patch_config(None)
        monkeypatch.setenv("SOCIS_AGENT_DASHBOARD_OIDC_ISSUER", _ISSUER)
        monkeypatch.setenv("SOCIS_AGENT_DASHBOARD_OIDC_CLIENT_ID", _CLIENT_ID)
        ctx = MagicMock()
        oidc_plugin.register(ctx)
    ...
    def test_config_load_failure_falls_through(self, monkeypatch):
        ...
        monkeypatch.setattr("socis_cli.config.load_config", _broken)
        ctx = MagicMock()
    ...
    def test_secret_from_env(self, patch_config, monkeypatch):
        ...
        monkeypatch.setenv("SOCIS_AGENT_DASHBOARD_OIDC_ISSUER", _ISSUER)
        monkeypatch.setenv("SOCIS_AGENT_DASHBOARD_OIDC_CLIENT_ID", _CLIENT_ID)
        monkeypatch.setenv("SOCIS_AGENT_DASHBOARD_OIDC_CLIENT_SECRET", "env-secret")
        ctx = MagicMock()
        oidc_plugin.register(ctx)
        registered = ctx.register_dashboard_auth_provider.call_args.args[0]
    ...
    def test_empty_env_secret_does_not_shadow_config(self, patch_config, monkeypatch):
        ...
        monkeypatch.setenv("SOCIS_AGENT_DASHBOARD_OIDC_CLIENT_SECRET", "")
        ctx = MagicMock()
        oidc_plugin.register(ctx)
        registered = ctx.register_dashboard_auth_provider.call_args.args[0]
