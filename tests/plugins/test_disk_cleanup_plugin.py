...
def _load_plugin_init():
    ...
    spec = importlib.util.spec_from_file_location(
        "socis_plugins.disk_cleanup",
        plugin_dir / "__init__.py",
        submodule_search_locations=[str(plugin_dir)],
    )
    # Ensure parent namespace package exists for the relative `. import disk_cleanup`
    import types
    if "socis_plugins" not in sys.modules:
        ns = types.ModuleType("socis_plugins")
        ns.__path__ = []
        sys.modules["socis_plugins"] = ns
    mod = importlib.util.module_from_spec(spec)
    mod.__package__ = "socis_plugins.disk_cleanup"
    mod.__path__ = [str(plugin_dir)]
    sys.modules["socis_plugins.disk_cleanup"] = mod
    spec.loader.exec_module(mod)
    return mod
...
class TestIsSafePath:
    def test_accepts_path_under_socis_agent_home(self, _isolate_env):
        dg = _load_lib()
    ...
    def test_rejects_outside_socis_agent_home(self, _isolate_env):
        dg = _load_lib()
...
class TestBundledDiscovery:
    def _write_enabled_config(self, socis_agent_home, names):
    ...
    def test_disk_cleanup_discovered_but_not_loaded_by_default(self, _isolate_env):
        ...
        from socis_cli import plugins as pmod
