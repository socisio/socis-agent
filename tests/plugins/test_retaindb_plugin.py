...
def _isolate_env(tmp_path, monkeypatch):
    """Ensure SOCIS_AGENT_HOME and RETAINDB vars are isolated."""
...
class TestRetainDBMemoryProvider:
    ...
    def test_initialize_creates_client_and_queue(self, tmp_path, monkeypatch):
        ...
        p.initialize("test-session", socis_agent_home=str(tmp_path / ".socis-agent"))
...
class TestPrefetch:
    ...
    def _make_initialized_provider(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("SOCIS_AGENT_HOME", str(socis_agent_home))
        p = RetainDBMemoryProvider()
        p.initialize("test-session", socis_agent_home=str(socis_agent_home))
        return p
