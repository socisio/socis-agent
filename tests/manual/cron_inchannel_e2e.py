...
Uses a throwaway SOCIS_AGENT_HOME so it never touches ~/.socis-agent. No real names.
"""
...
def _fresh_home():
    """Point SOCIS_AGENT_HOME at a throwaway dir BEFORE importing gateway modules
    (mirror.py binds _SESSIONS_INDEX from get_socis_agent_home() at import time)."""
    d = tempfile.mkdtemp(prefix="cron_inchannel_e2e_")
    os.environ["SOCIS_AGENT_HOME"] = d
    return Path(d)


HOME = _fresh_home()

# Import AFTER SOCIS_AGENT_HOME is set.
...
# a different get_socis_agent_home() at import if something imported it earlier).
...
def main():
    ...
    print(f"SOCIS_AGENT_HOME (throwaway): {HOME}")
