"""Shared fixtures for the socis-agent test suite.
...
2. **Isolated SOCIS_AGENT_HOME.** SOCIS_AGENT_HOME points to a per-test tempdir so
   code reading ``~/.socis-agent/*`` via ``get_socis_agent_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".socis-agent"`` instead of the canonical
   ``get_socis_agent_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No SOCIS_AGENT_SESSION_* inheritance** — the agent's current gateway
...
# ── Sandbox SOCIS_AGENT_HOME before ANY test module is imported ──────────────────
# `socis_cli/main.py` calls `setup_logging()` at MODULE level, which resolves
# `get_socis_agent_home()` and attaches rotating file handlers to the ROOT logger.
...
# real `~/.socis-agent/logs/agent.log` and `errors.log`.
#
# The `_isolate_env` fixture below also sandboxes SOCIS_AGENT_HOME, but fixtures run
...
# the REAL SOCIS root — capture it BEFORE the sandbox rewires SOCIS_AGENT_HOME,
# otherwise the deny-list would point at the throwaway tempdir and the guard
# would silently stop protecting the operator's actual ~/.socis-agent (#69385).
_PRE_SANDBOX_KANBAN_OVERRIDE = os.environ.get("SOCIS_AGENT_KANBAN_HOME", "").strip()
_PRE_SANDBOX_SOCIS_AGENT_HOME = os.environ.get("SOCIS_AGENT_HOME", "")


def _socis_agent_home_points_at_production(value: str) -> bool:
    """True when a pre-set SOCIS_AGENT_HOME resolves to the real production root.

    Gateway-launched shells (and developer shells that ``export
    SOCIS_AGENT_HOME=~/.socis-agent``) hand pytest the PRODUCTION home. Historically
    the session sandbox below honored any pre-set value, so collection-time
    imports (logging handlers, ``socis_agent_state.DEFAULT_DB_PATH``) froze paths
    inside the real ``~/.socis-agent`` — the escape vector that landed pytest
    ...
    SOCIS_AGENT_HOME is honored now.
    """
    if not value:
        return True
    try:
        resolved = Path(value).expanduser().resolve()
        real_root = (Path.home() / ".socis-agent").resolve()
    except Exception:
        return True
...
if _socis_agent_home_points_at_production(os.environ.get("SOCIS_AGENT_HOME", "")):
    _SESSION_SOCIS_AGENT_HOME = tempfile.mkdtemp(prefix="socis-test-home-")
    os.environ["SOCIS_AGENT_HOME"] = _SESSION_SOCIS_AGENT_HOME
    atexit.register(shutil.rmtree, _SESSION_SOCIS_AGENT_HOME, True)
...
# like a real CLI") — which used to disarm socis_agent_state's live-DB guard in
# the child at the same moment the child lost the SOCIS_AGENT_HOME redirect.
# SOCIS_AGENT_TEST_ISOLATION is OUR marker: exported here (before any test module
# imports), inherited by every child by default, and honored by
# socis_agent_state._running_under_pytest() as a test-context signal. A child
...
# open a real DB must export SOCIS_AGENT_STATE_DB_GUARD_BYPASS=1 in that child's
# env instead of stripping markers.
os.environ["SOCIS_AGENT_TEST_ISOLATION"] = os.environ.get("SOCIS_AGENT_HOME", "") or "1"

#: SOCIS_AGENT_HOME as it stood when conftest was imported - i.e. before any test
...
SOCIS_AGENT_HOME_AT_CONFTEST_IMPORT = os.environ.get("SOCIS_AGENT_HOME", "")
...
# SOCIS_AGENT_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_SOCIS_AGENT_BEHAVIORAL_VARS = frozenset({
    ...
    # final response text to ``socis_cli.voice.speak_text`` — real synthesis,
    ...
    "SOCIS_AGENT_VOICE",
    "SOCIS_AGENT_VOICE_TTS",
    "SOCIS_AGENT_YOLO_MODE",
    # Injected into subprocess envs by the terminal tool (_make_run_env), so
    # any test run launched FROM a SOCIS agent session inherits them and
    # socis_agent_constants home-resolution helpers prefer them over monkeypatched
    # HOME (test_subprocess_home_isolation red locally, green on CI).
    "SOCIS_AGENT_REAL_HOME",
    "TERMINAL_HOME_MODE",
    "SOCIS_AGENT_INTERACTIVE",
    "SOCIS_AGENT_QUIET",
    "SOCIS_AGENT_TOOL_PROGRESS",
    "SOCIS_AGENT_TOOL_PROGRESS_MODE",
    "SOCIS_AGENT_MAX_ITERATIONS",
    "SOCIS_AGENT_SESSION_PLATFORM",
    "SOCIS_AGENT_SESSION_CHAT_ID",
    "SOCIS_AGENT_SESSION_CHAT_NAME",
    "SOCIS_AGENT_SESSION_CHAT_TYPE",
    "SOCIS_AGENT_SESSION_THREAD_ID",
    "SOCIS_AGENT_SESSION_SOURCE",
    "SOCIS_AGENT_SESSION_KEY",
    "SOCIS_AGENT_GATEWAY_SESSION",
    "SOCIS_AGENT_CRON_SESSION",
    "_SOCIS_AGENT_GATEWAY",
    "SOCIS_AGENT_PLATFORM",
    "SOCIS_AGENT_MODEL",
    "SOCIS_AGENT_INFERENCE_MODEL",
    "SOCIS_AGENT_INFERENCE_PROVIDER",
    "SOCIS_AGENT_TUI_PROVIDER",
    "SOCIS_AGENT_MANAGED",
    "SOCIS_AGENT_MANAGED_DIR",
    "SOCIS_AGENT_DEV",
    "SOCIS_AGENT_CONTAINER",
    "SOCIS_AGENT_EPHEMERAL_SYSTEM_PROMPT",
    "SOCIS_AGENT_TIMEZONE",
    "SOCIS_AGENT_REDACT_SECRETS",
    "SOCIS_AGENT_BACKGROUND_NOTIFICATIONS",
    "SOCIS_AGENT_EXEC_ASK",
    "SOCIS_AGENT_HOME_MODE",
    "SOCIS_AGENT_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.socis-agent/kanban.db instead of the per-test SOCIS_AGENT_HOME.
    "SOCIS_AGENT_KANBAN_DB",
    "SOCIS_AGENT_KANBAN_BOARD",
    "SOCIS_AGENT_KANBAN_HOME",
    "SOCIS_AGENT_KANBAN_WORKSPACES_ROOT",
    "SOCIS_AGENT_KANBAN_LOGS_ROOT",
    "SOCIS_AGENT_KANBAN_TASK",
    "SOCIS_AGENT_KANBAN_WORKSPACE",
    "SOCIS_AGENT_KANBAN_RUN_ID",
    "SOCIS_AGENT_KANBAN_CLAIM_LOCK",
    "SOCIS_AGENT_KANBAN_DISPATCH_IN_GATEWAY",
    ...
    "SOCIS_AGENT_DELEGATED_CHILD_CONTEXT",
    "SOCIS_AGENT_TENANT",
    ...
    # otherwise-unrelated config tests away from the default "socis" host.
    "SOCIS_AGENT_HONCHO_HOST",
    ...
    "SOCIS_AGENT_DASHBOARD_OAUTH_CLIENT_ID",
    "SOCIS_AGENT_DASHBOARD_PORTAL_URL",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and SOCIS_AGENT_HOME to per-test tempdirs so code that
    reads ``~/.socis-agent/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral SOCIS_AGENT_* vars that could change test semantics.
    for name in _SOCIS_AGENT_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)
    ...
    monkeypatch.setenv("SOCIS_AGENT_HONCHO_HOST", "socis")

    # 3. Redirect SOCIS_AGENT_HOME to a per-test tempdir. Code that reads
    #    ``~/.socis-agent/*`` via ``get_socis_agent_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.socis-agent/*`` via
    #    ``Path.home() / ".socis-agent"`` instead of ``get_socis_agent_home()``
    #    is a bug to fix at the callsite.
    fake_socis_agent_home = tmp_path / "socis_test"
    fake_socis_agent_home.mkdir()
    (fake_socis_agent_home / "sessions").mkdir()
    (fake_socis_agent_home / "cron").mkdir()
    (fake_socis_agent_home / "memories").mkdir()
    (fake_socis_agent_home / "skills").mkdir()
    monkeypatch.setenv("SOCIS_AGENT_HOME", str(fake_socis_agent_home))
    ...
    # socis_agent_state's live-DB guard stays armed in them even when the test
    # strips pytest's own PYTEST_* vars from the child env.
    monkeypatch.setenv("SOCIS_AGENT_TEST_ISOLATION", str(fake_socis_agent_home))
    # And never let a developer-shell (or leaked child) bypass disarm the
    # guard for in-process code under test.
    monkeypatch.delenv("SOCIS_AGENT_STATE_DB_GUARD_BYPASS", raising=False)

    # 3b. socis_agent_state computes ``DEFAULT_DB_PATH = get_socis_agent_home() / "state.db"``
    #     at import time. When the module is first imported at collection (any
    #     test file with a top-level ``from socis_agent_state import ...``) that
    ...
    socis_agent_state_mod = sys.modules.get("socis_agent_state")
    if socis_agent_state_mod is not None and hasattr(socis_agent_state_mod, "DEFAULT_DB_PATH"):
        monkeypatch.setattr(
            socis_agent_state_mod, "DEFAULT_DB_PATH", fake_socis_agent_home / "state.db"
        )
    ...
    monkeypatch.setenv("SOCIS_AGENT_DISABLE_LAZY_INSTALLS", "1")

    # 5. Reset plugin singleton so tests don't leak plugins from
    #    ~/.socis-agent/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import socis_cli.plugins as _plugins_mod
        ...
        # previous test's tmp_path SOCIS_AGENT_HOME can't leak forward. Paths
...
@pytest.fixture(autouse=True)
def _isolate_socis_agent_home(_hermetic_environment):
...
def _neutralize_kanban_memory_guard(request, monkeypatch):
    ...
    if request.node.get_closest_marker("real_memory_guard"):
        return
    try:
        from socis_cli import kanban_db as _kb_mod
    except Exception:
        return
...
# invocation), kanban writes silently pollute the real ~/.socis-agent. This autouse
...
# allow-list because test-level fixtures legitimately move SOCIS_AGENT_HOME to
...
def _capture_real_kanban_root() -> Path:
    ...
    file (before the session SOCIS_AGENT_HOME sandbox rewired the env), so the
    deny-list keeps pointing at the operator's actual root. Mirrors
    ``kanban_db.kanban_home()`` resolution order:
    1. ``SOCIS_AGENT_KANBAN_HOME`` env var when set and non-empty
    2. the real (pre-sandbox) SOCIS root otherwise
    """
    if _PRE_SANDBOX_KANBAN_OVERRIDE:
        return Path(_PRE_SANDBOX_KANBAN_OVERRIDE).expanduser().resolve()
    if _PRE_SANDBOX_SOCIS_AGENT_HOME and not _socis_agent_home_points_at_production(
        _PRE_SANDBOX_SOCIS_AGENT_HOME
    ):
        # SOCIS_AGENT_HOME was genuinely set to a CUSTOM root before the sandbox
        ...
        return get_default_socis_root().resolve()
    # No pre-existing SOCIS_AGENT_HOME: the real root is the platform default,
    # NOT the sandbox tempdir now sitting in the env.
    return (Path.home() / ".socis-agent").resolve()
...
def _kanban_write_guard(_hermetic_environment, monkeypatch):
    ...
    ``~/.socis-agent`` captured at import time. Hermetic tests that legitimately
    move SOCIS_AGENT_HOME to sibling tempdirs are unaffected.

    Only patches when ``socis_cli.kanban_db`` is *already imported* — a
    ...
    """
    _kdb = sys.modules.get("socis_cli.kanban_db")
    if _kdb is None:
        return

    # The sys.modules probe can observe the module MID-IMPORT: a fixture
    # boundary firing while another test's lazy `import socis_cli.kanban_db`
    ...
    def _guarded_connect(db_path=None, *args, **kwargs):
        ...
        raise RuntimeError(
            ...
            f"to the real ~/.socis-agent. See #69283."
        )
...
# ``socis_agent_state._ensure_test_isolation`` (the single choke point every
# ``SessionDB()`` construction goes through) refuses, under pytest, any DB
# path that resolves inside the REAL SOCIS root. This fixture wires the
# test-side knobs:
...
#     installs where SOCIS_AGENT_HOME is not ~/.socis-agent) into the guard's
...
# so subprocess children that import socis_agent_state directly are covered even
# without this fixture.


@pytest.fixture(autouse=True)
def _state_db_write_guard(request, monkeypatch):
    _hs = sys.modules.get("socis_agent_state")
    ...
    if _PRE_SANDBOX_SOCIS_AGENT_HOME and not _socis_agent_home_points_at_production(
        ...
        extra_roots.append(
            Path(_PRE_SANDBOX_SOCIS_AGENT_HOME).expanduser().resolve()
        )
    monkeypatch.setattr(
...
def _reset_tui_gateway_server_state():
    ...
    # A leaked context-local SOCIS home override redirects every later
    # ``get_socis_agent_home()`` call (active-session registry, config paths)
    # to a stale per-test tmpdir. Force the main-thread ContextVar back
    # to its default.
    try:
        from socis_agent_constants import get_socis_agent_home_override, set_socis_agent_home_override

        if get_socis_agent_home_override() is not None:
            set_socis_agent_home_override(None)
    except Exception:
        pass
...
@pytest.fixture()
def mock_config():
    """Return a minimal socis config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``socis-gateway`` process
...
# correlated with a ``tests/socis_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> socis-gateway``
...
def _wal_is_usable() -> bool:
    """True when SOCIS will actually put a database into WAL mode here.

    SOCIS refuses journal_mode=WAL on SQLite builds carrying the upstream
    ...
    running SOCIS can link DIFFERENT SQLite versions: a repo ``.venv`` on
    3.50.4 (vulnerable → DELETE) alongside a SOCIS managed runtime on 3.53.1
    (fixed → WAL). The same test then passes in one and fails in the other.

    IMPORTANT: this must NOT import ``socis_agent_state``. That module computes
    ``DEFAULT_DB_PATH`` from ``get_socis_agent_home()`` at import time, so importing
    it during collection — before the per-test ``_isolate_socis_agent_home`` fixture
    redirects ``SOCIS_AGENT_HOME`` — permanently caches the DEVELOPER'S REAL
    ``~/.socis-agent/state.db`` for the whole session. Tests then read live
    production sessions instead of a tempdir. The version predicate is
    duplicated from ``socis_agent_state._is_sqlite_wal_reset_vulnerable`` (upstream
...
#      process environment: ``os.environ["SOCIS_AGENT_VOICE_TTS"] = "1"``. The
...
#      ``socis_cli.voice.speak_text(final_response)`` on a daemon thread.
...
# to mock it:
#
#  • ``socis_cli.voice.speak_text`` — the synth+playback entry point both
#    gateway call sites late-import, so patching the module attribute catches
#    them wherever they import it from.
#  • ``socis_cli.voice.play_audio_file`` — the module-level binding
#    ``speak_text`` actually plays through. Patching the binding inside
#    ``socis_cli.voice`` (not ``tools.voice_mode``) keeps the real function
...
# OS gating
#
# SOCIS runs on Linux, macOS and native Windows, and a lot of its behaviour
# genuinely differs per host: PTY vs pywinpty, taskkill vs SIGTERM, launchd
# vs systemd, Keychain vs libsecret, ``%LOCALAPPDATA%`` vs ``~/.socis-agent``.
#
...
def pytest_configure(config):  # noqa: D401 — pytest hook
    ...
    config.addinivalue_line(
        ...
        "SQLite WAL mode; skipped on builds where SOCIS falls back to "
...
def pytest_collection_modifyitems(config, items):  # noqa: D401 — pytest hook
    ...
    reason = (
        f"SQLite {sqlite3.sqlite_version} has the WAL-reset bug — SOCIS uses "
...
def _live_system_guard(request, monkeypatch):
    ...
    def _matches_socis_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _SOCIS_AGENT_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_socis_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in killer_tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch socis-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m socis_cli.main".
                if (
                    "socis" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting socis/python could hit the live gateway. "
                "Mark with @pytest.mark.live_system_guard_bypass if "
                "intentional."
            )
        # Block any subprocess that would run `socis update` (or the
        # equivalent `python -m socis_cli.main update`).  These commands
        ...
        if "update" in low and (
            # socis update / socis update --gateway / setsid bash -c ... socis update
            ("socis" in low and "update" in low.split())
            or
            # python -m socis_cli.main update --gateway
            ("socis_cli" in low and "update" in low.split())
            or
            # venv/bin/socis update  (absolute path variant used in tests)
            (".venv/bin/socis" in low and "update" in low)
        ):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}) — this command would run "
                "`socis update` against the real checkout, fetching "
...
def _audio_playback_guard(request, monkeypatch):
    ...
    depth behind ``_SOCIS_AGENT_BEHAVIORAL_VARS``: the env blanking stops the flag
    ...
    try:
        import socis_cli.voice as _voice
    except Exception:
