...
#
# Installs a real, earlier SOCIS the way a user does, applies ONE update route,
# and requires the checkout to land on this commit with a working `socis`.
#
...
# Keep sandbox state out of the default .socis-sandbox so a run never clobbers
...
SANDBOX_DIR_NAME=".socis-sandbox-e2e-$ROUTE"
export SOCIS_AGENT_DEV_SANDBOX_DIR="$SANDBOX_DIR_NAME"

SANDBOX_ROOT="$REPO_ROOT/$SANDBOX_DIR_NAME"
INSTALL_DIR="/home/socis/.socis-agent/socis-agent"   # user-level layout (sandbox default)
FAKE_REMOTE="/work/repos/socis-agent.git"
...
LOG_DIR="${SOCIS_AGENT_E2E_LOG_DIR:-$(mktemp -d -t socis-install-e2e-logs.XXXXXX)}"
...
# ── helpers ────────────────────────────────────────────────────────────────
# Does the INSTALLED socis accept FLAG on `socis update`?
#
...
# that silently rots. `socis update --help` is the same surface a user meets,
# and argparse prints every option it accepts.
update_supports() {
  local flag="$1"
  in_sandbox "socis update --help 2>&1" | grep -qF -- "$flag"
}
...
require_socis_works() {
  local when="$1" out
  out="$(in_sandbox "socis --version" 2>&1)" \
    || { printf '%s\n' "$out" >&2; fail "socis --version failed $when"; }
  printf '%s\n' "$out" | sed 's/^/    /'
  ok "socis runs $when"
}

# ── install the earlier SOCIS ─────────────────────────────────────────────
...
require_socis_works 'after install'

# ── apply exactly one update route ─────────────────────────────────────────
case "$ROUTE" in
  update)
    step 'ROUTE: socis update'
    ...
    # socis which it accepts; older ones read the prompt from stdin, so close it.
    if update_supports --yes; then
      update_cmd="socis update --yes"
    else
      update_cmd="socis update </dev/null"
    fi
    if ! in_sandbox "cd $INSTALL_DIR && $update_cmd"; then
      collect_sandbox_logs update
      fail "socis update failed ($update_cmd)"
    fi
    require_landed_on_target 'socis update'
    require_socis_works 'after socis update'
    ;;
  installer)
    ...
    require_socis_works 'after installer re-run'
    ;;
esac
