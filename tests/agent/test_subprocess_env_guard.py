...
``socis_subprocess_env`` / ``_sanitize_subprocess_env``, which the factory
...
``SOCIS_AGENT_HOME`` or secret-scrub propagation.
...
SCAN_DIRS = ("agent", "socis_cli", "tools", "gateway", "cron", "tui_gateway")
SCAN_ROOT_FILES = ("cli.py", "socis_agent_constants.py")
...
ALLOWED_RAW_SPAWN_ENV_FILES = {
    # THE owner module: _sanitize_subprocess_env / socis_subprocess_env /
