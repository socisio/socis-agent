"""Regression tests for the codex_app_server → SOCIS UI event bridge.
...
class TestCodexItemToToolName:
    ...
    def test_socis_tools_mcp_server_emits_bare_tool_name(self):
        """The socis-tools MCP server wraps SOCIS' own tools for codex;
        ...
        mcp.socis-tools.* namespacing (from #26541 by @simpolism)."""
        assert _codex_item_to_tool_name(
            {"type": "mcpToolCall", "server": "socis-tools", "tool": "web_search"}
        ) == "web_search"
        assert _codex_item_to_tool_name(
            {"type": "mcpToolCall", "server": "socis-tools", "tool": "browser_navigate"}
        ) == "browser_navigate"
...
class TestToolProgressDispatch:
    ...
    def test_web_search_builtin_fires_started_and_completed(self):
        ...
        assert calls[0].args[2] == "socis agent docs"
        assert calls[0].args[3] == {"query": "socis agent docs"}
