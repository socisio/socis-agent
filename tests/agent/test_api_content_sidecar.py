...
class TestPrologueStamping:
    ...
    def test_no_stamp_without_injections(self):
        agent = _FakeAgent()
        with patch("socis_cli.plugins.invoke_hook", return_value=[]):
            ctx = _build(agent)
...
@pytest.fixture()
def wire_env():
    """Mock provider + isolated SOCIS_AGENT_HOME + a shared SessionDB.
    ...
    test_home = tempfile.mkdtemp(prefix="socis_api_content_")
