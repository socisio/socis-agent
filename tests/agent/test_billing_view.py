...
def test_portal_base_url_default(monkeypatch):
    monkeypatch.delenv("SOCIS_AGENT_PORTAL_BASE_URL", raising=False)
...
# ---------------------------------------------------------------------------
# SOCIS_AGENT_DEV_BILLING_FIXTURE — offline card/scope state scaffolding (T0)
# ---------------------------------------------------------------------------
...
def test_billing_fixture_card_and_gate_invariants(monkeypatch, name, has_card, is_admin, billing_on):
    ...
    monkeypatch.setenv("SOCIS_AGENT_DEV_BILLING_FIXTURE", name)
    s = build_billing_state()
