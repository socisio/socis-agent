...
def curator_env(tmp_path, monkeypatch):
    """Isolated SOCIS_AGENT_HOME + freshly reloaded curator + skill_usage modules."""
    ...
    # save_state() resolves the state path from SOCIS_AGENT_HOME at write time, so a
    # straggler thread that outlives this test would write into whatever home
    # the *next* test has configured (or the default ~/.socis-agent once monkeypatch
    ...
    # thread here, while SOCIS_AGENT_HOME is still pinned to this test's tmp home
...
def test_cli_pin_refuses_bundled_skill(curator_env, capsys):
    from socis_cli import curator as cli
    skills_dir = curator_env["home"] / "skills"
...
# `socis model` → auxiliary picker, the dashboard Models tab, and the full
...
def test_curator_slot_is_canonical_aux_task():
    ...
    from socis_cli.main import _AUX_TASKS
    from socis_cli.web_server import _AUX_TASK_SLOTS
    ...
    # 2. socis_cli/main.py _AUX_TASKS — CLI picker
    aux_keys = {k for k, _name, _desc in _AUX_TASKS}
    assert "curator" in aux_keys, "curator missing from _AUX_TASKS (CLI picker)"

    # 3. socis_cli/web_server.py _AUX_TASK_SLOTS — REST API allowlist
...
def test_review_fork_restricts_toolsets_to_skills_only(curator_env, monkeypatch):
    ...
    ``socis curator rollback`` restored a hollow skill. Removing the toolset
...
def test_review_fork_toolset_surface_excludes_execution_tools():
    ...
    already-stripped package, and ``socis curator rollback`` restores a
...
def test_review_prompt_does_not_steer_terminal_writes():
    ...
    ``mkdir -p ~/.socis-agent/skills/<umbrella>/references/ && mv ...`` its
