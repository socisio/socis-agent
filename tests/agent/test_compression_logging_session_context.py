...
which moves ``SOCIS_AGENT_SESSION_ID`` env + ContextVar). The ``[session_id]`` tag on
log lines comes from a SEPARATE mechanism — ``socis_agent_logging._session_context``
...
def test_logging_session_context_follows_compression_rotation(tmp_path: Path) -> None:
    ...
    socis_agent_logging.set_session_context(parent_sid)
    try:
        ...
        current = getattr(socis_agent_logging._session_context, "session_id", None)
