...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate SOCIS_AGENT_HOME under tmp_path."""
    monkeypatch.setenv("SOCIS_AGENT_HOME", str(tmp_path / ".socis-agent"))
    (tmp_path / ".socis-agent").mkdir()

    # Force the constants/image cache helpers to re-read SOCIS_AGENT_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("socis_agent_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_socis_agent_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under SOCIS_AGENT_HOME — gateway cleanup
