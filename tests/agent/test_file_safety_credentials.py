"""Tests for SOCIS_AGENT_HOME credential-file read blocking in file_safety.

Regression for https://github.com/socisio/socis-agent/issues/17656 —
``read_file`` was previously only sandboxed against ``SOCIS_AGENT_HOME`` itself,
...
for the credential stores while leaving arbitrary ``SOCIS_AGENT_HOME`` files
...
def fake_home(tmp_path, monkeypatch):
    """Point ``_socis_agent_home_path()`` at a tmp dir for isolated checks."""
    import agent.file_safety as fs

    home = tmp_path / "socis_agent_home"
    home.mkdir()
    monkeypatch.setattr(fs, "_socis_agent_home_path", lambda: home)
    return home
...
def test_arbitrary_socis_agent_home_file_not_blocked(fake_home):
    """Non-credential files inside SOCIS_AGENT_HOME stay readable."""
...
def test_identically_named_socis_files_outside_home_not_blocked(
    fake_home, tmp_path
):
    """SOCIS-specific filenames (``auth.json``, ``mcp-tokens/``, ``google_oauth.json``)
    outside SOCIS_AGENT_HOME must remain readable — the gate is per-location for
    ...
    project.mkdir()
    # auth.json outside SOCIS_AGENT_HOME — readable (per-location gate).
    p = project / "auth.json"
    p.write_text("not secret here", encoding="utf-8")
    assert get_read_block_error(str(p)) is None, (
        "auth.json outside SOCIS_AGENT_HOME should NOT be blocked"
    )
...
def test_profile_mode_blocks_root_credentials(tmp_path, monkeypatch):
    """Under a profile, SOCIS_AGENT_HOME = <root>/profiles/<name>, but
    ...
    monkeypatch.setattr(fs, "_socis_agent_home_path", lambda: profile)
