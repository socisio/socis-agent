...
def socis_agent_home(tmp_path):
    """Create a minimal SOCIS_AGENT_HOME with config."""
...
class TestGetExternalSkillsDirs:
    def test_empty_config(self, socis_agent_home):
        (socis_agent_home / "config.yaml").write_text("skills:\n  external_dirs: []\n")
    ...
    def test_valid_dir_returned(self, socis_agent_home, external_skills_dir):
...
class TestGetAllSkillsDirs:
    def test_local_always_first(self, socis_agent_home, external_skills_dir):
        (socis_agent_home / "config.yaml").write_text(
            f"skills:\n  external_dirs:\n    - {external_skills_dir}\n"
        )
        with patch.dict(os.environ, {"SOCIS_AGENT_HOME": str(socis_agent_home)}):
        ...
        assert result[0] == socis_agent_home / "skills"
        assert result[1] == external_skills_dir.resolve()


class TestExternalSkillsInFindAll:
    def test_external_skills_found(self, socis_agent_home, external_skills_dir):
        (socis_agent_home / "config.yaml").write_text(
            f"skills:\n  external_dirs:\n    - {external_skills_dir}\n"
        )
        local_skills = socis_agent_home / "skills"
        with (
            patch.dict(os.environ, {"SOCIS_AGENT_HOME": str(socis_agent_home)}),
    ...
    def test_local_takes_precedence(self, socis_agent_home, external_skills_dir):
...
class TestExternalSkillView:
    def test_skill_view_finds_external(self, socis_agent_home, external_skills_dir):
        (socis_agent_home / "config.yaml").write_text(
            f"skills:\n  external_dirs:\n    - {external_skills_dir}\n"
        )
        local_skills = socis_agent_home / "skills"
