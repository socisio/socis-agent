...
class TestCacheFileReadBlocking:
    """Internal SOCIS cache files must remain blocked."""

    def test_hub_index_cache_blocked(self, tmp_path):
        """Hub index-cache reads are blocked."""
        socis_agent_home = tmp_path / ".socis-agent"
        cache = socis_agent_home / "skills" / ".hub" / "index-cache" / "data.json"
        cache.parent.mkdir(parents=True)
        cache.write_text("{}")

        with patch("agent.file_safety._socis_agent_home_path", return_value=socis_agent_home):
            error = get_read_block_error(str(cache))
            assert error is not None
            assert "internal SOCIS cache" in error

    def test_hub_directory_blocked(self, tmp_path):
        """Hub directory reads are blocked."""
        socis_agent_home = tmp_path / ".socis-agent"
        hub = socis_agent_home / "skills" / ".hub" / "metadata.json"
...
class TestCombinedGuards:
    ...
    def test_env_guard_works_regardless_of_socis_agent_home(self, tmp_path):
        """The env basename guard does not depend on SOCIS_AGENT_HOME resolution."""
        socis_agent_home = tmp_path / ".socis-agent"
    ...
    def test_cache_guard_still_works_with_env_guard(self, tmp_path):
        ...
        socis_agent_home = tmp_path / ".socis-agent"
        cache = socis_agent_home / "skills" / ".hub" / "index-cache" / "x"
        cache.parent.mkdir(parents=True)
        cache.write_text("")
