...
path via ``socis chat --query``) couldn't see what the aggregator actually
produced without joining to the session DB by hand.

These exercise the real ``consume_and_save_trace`` → ``save_moa_turn`` path
with real file I/O against a temp SOCIS_AGENT_HOME — no mocks on the write path.
"""
...
def _enable_traces(tmp_path, monkeypatch):
    """Point SOCIS_AGENT_HOME at a temp dir and turn moa.save_traces on."""
    socis_agent_home = tmp_path / ".socis-agent"
    socis_agent_home.mkdir()
    monkeypatch.setenv("SOCIS_AGENT_HOME", str(socis_agent_home))

    # save_moa_turn reads config via socis_cli.config.load_config; stub it to
    ...
    return socis_agent_home / "moa-traces"
