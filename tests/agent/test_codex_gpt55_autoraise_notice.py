...
2. The per-profile dedupe marker (#54432) — the notice must show at most once
   ...
   Discord etc. The gate persists a marker under ``$SOCIS_AGENT_HOME``
...
def test_marker_lives_under_socis_agent_home() -> None:
    marker = _codex_gpt55_autoraise_notice_marker()
    assert marker.parent == get_socis_agent_home()
