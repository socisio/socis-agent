...
def isolated_home(monkeypatch):
    """Temp SOCIS_AGENT_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="socis_test_31179_")
    socis_agent_home = os.path.join(test_home, ".socis-agent")
...
def _fresh_modules():
    """Drop cached socis modules so each test reloads against current env."""
    _drop_reload_targets()
