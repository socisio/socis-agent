...
conversation-affinity hint SOCIS sends — ``prompt_cache_key`` on both
...
bucket it warmed. SOCIS cannot infer the logical conversation from the id's
syntax (#79017's failure class), but it does not have to: the host declares
it through ``gateway_session_key`` (the ``X-SOCIS-Session-Key`` /
...
class TestDeclaredConversationScope:
    ...
    def test_scope_never_carries_the_raw_key(self, db):
        ...
        A session id is a SOCIS-internal token; a session KEY embeds the
...
class TestPeerIdentityIsSourceQualified:
    ...
    ``X-SOCIS-Session-Key`` accepts any authenticated caller-supplied string,
...
class TestSourceOverrideDomain:
    ...
    ``SOCIS_AGENT_SESSION_SOURCE``. Under an override both sides of a ``/new``
    ...
    def test_the_pre_row_source_matches_persistence(self, db, monkeypatch):
        ...
        monkeypatch.setenv("SOCIS_AGENT_SESSION_SOURCE", "override-src")
