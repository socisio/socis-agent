...
class TestGoalMigratesOnRotation:
    def test_goal_follows_compression_rotation(self, tmp_path: Path):
        ...
        with patch.dict(os.environ, {"SOCIS_AGENT_HOME": str(tmp_path / ".socis-agent")}):
            (tmp_path / ".socis-agent").mkdir(exist_ok=True)
            import socis_cli.goals as goals
...
class TestRotationChildFlushDedup:
    ...
    def test_adoption_divergence_merged_stamps_both_views_and_no_duplicate(
        ...
        # "born durable" (socis_agent_state stamps _DB_PERSISTED_MARKER on rows
