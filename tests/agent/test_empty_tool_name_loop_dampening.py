...
@pytest.fixture()
def agent_env():
    """Spin up the mock provider + an isolated SOCIS_AGENT_HOME, yield (agent, helpers)."""
    ...
    test_home = tempfile.mkdtemp(prefix="socis_e2e_47967_")
    os.makedirs(os.path.join(test_home, ".socis-agent"))
    prev_home = os.environ.get("SOCIS_AGENT_HOME")
    os.environ["SOCIS_AGENT_HOME"] = os.path.join(test_home, ".socis-agent")
    ...
    for mod in list(sys.modules):
        if mod == "run_agent" or mod.startswith("agent.") or mod.startswith("tools.") or mod.startswith("socis_"):
            del sys.modules[mod]
    ...
    try:
        ...
        if prev_home is None:
            ...
            os.environ["SOCIS_AGENT_HOME"] = prev_home
