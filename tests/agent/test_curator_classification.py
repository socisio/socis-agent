...
def test_reconcile_absorbed_into_beats_everything_else(curator_env):
    ...
    out = curator_env._reconcile_classification(
        ...
        destinations={"socis-agent-dev"},
        absorbed_declarations={
            "pr-review-format": {"into": "socis-agent-dev", "declared": True},
        },
    )
...
# `socis curator status`). The full data has always been in REPORT.md on
...
# ---------------------------------------------------------------------------
# Pin hint — surfaces `socis curator pin <umbrella>` in the rename block so
