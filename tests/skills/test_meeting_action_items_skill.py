...
def test_frontmatter_required_fields():
    ...
    socis = fm["metadata"]["socis"]
    assert socis["tags"]
    assert "related_skills" in socis
...
def test_author_credits_human_first():
    fm, _ = _frontmatter_and_body()
    assert not fm["author"].startswith("SOCIS Agent"), "human contributor must be credited first"
...
def test_related_skills_resolve_in_repo():
    ...
    for name in fm["metadata"]["socis"]["related_skills"]:
        hits = (
