...
def test_required_frontmatter_fields(p):
    ...
    socis = (fm.get("metadata") or {}).get("socis") or {}
    if not (socis.get("tags") or fm.get("tags")) and not _grandfathered(p, "tags"):
        pytest.fail(f"{_rel(p)}: no tags (metadata.socis.tags or top-level tags)")
...
def test_related_skills_resolve(p):
    ...
    socis = (fm.get("metadata") or {}).get("socis") or {}
    dangling = [
        rs for rs in (socis.get("related_skills") or []) if rs not in _all_names()
    ]
