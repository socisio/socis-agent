...
SESSION_REVOKED / REFRESH_DEAD), the SOCIS_AGENT_HOME resolution fallback, the
...
def _run_main(mod, tokens_dir, argv, responses):
    ...
    with patch.object(mod.os, "environ", dict(mod.os.environ, SOCIS_AGENT_HOME=str(tokens_dir.parent))), \
         patch.object(mod.urllib.request, "urlopen", side_effect=fake_urlopen), \
         patch.object(sys, "argv", ["diagnose-oauth-mcp.py", *argv]):
        # Force the env-var fallback path (ignore any importable socis_agent_constants).
        with patch.object(mod, "_socis_agent_home", lambda: str(tokens_dir.parent)):
            buf = io.StringIO()
...
def test_skill_md_frontmatter_invariants():
    ...
    assert fm["author"].split(",")[0].strip() != "SOCIS Agent"  # human credited first
