...
@pytest.mark.parametrize("host,allow_public,expected", [
    ...
    # bind (June 2026 socis-0day hardening). Non-loopback always requires auth.
    ("0.0.0.0",   True,  True),
    ("0.0.0.0",   False, True),
    ...
    ("socis-agent-prod-abc.fly.dev", False, True),
])
def test_should_require_auth_truth_table(host, allow_public, expected):
    from socis_cli.web_server import should_require_auth
...
def test_empty_provider_login_page_shows_supported_auth_paths():
    ...
    assert (
        'href="https://agent.socis.io/docs/'
...
def test_loopback_public_url_fail_closed_message_is_actionable(monkeypatch):
    ...
    assert "socis dashboard register" in msg
...
def test_should_require_dashboard_auth_truth_table(
    ...
    if public_url is None:
        monkeypatch.delenv("SOCIS_AGENT_DASHBOARD_PUBLIC_URL", raising=False)
        monkeypatch.setattr(
            web_server, "_dashboard_public_hosts", lambda: frozenset()
        )
    else:
        monkeypatch.setenv("SOCIS_AGENT_DASHBOARD_PUBLIC_URL", public_url)
