...
(remote → cache → seed), `socis plugins search --json`, and install-time
...
from socis_cli import plugin_index
...
SAMPLE = _index_doc(
    [
        ...
        },
        {
            ...
            "author": "SOCIS",
            "tags": ["example", "llm"],
            "repo": "SOCIS/socis-example-plugins",
...
def socis_agent_home(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(plugin_index, "get_socis_agent_home", lambda: tmp_path)
    return tmp_path
...
class TestParsing:
    def test_parses_object_form(self):
        ...
        assert [e.name for e in entries] == [
            "socis-media-studio",
            "socis-telegram-business",
            "plugin-llm-example",
        ]
        assert entries[2].subdir == "plugin-llm-example"
        assert entries[2].install_identifier == (
            "SOCIS/socis-example-plugins/plugin-llm-example"
        )
        assert entries[0].install_identifier == "SOCIS/socis-media-studio"
...
class TestSearch:
    ...
    def test_exact_name_ranks_first(self):
        results = search_index(self.entries, "socis-media-studio")
        assert results[0].name == "socis-media-studio"

    def test_matches_tags(self):
        results = search_index(self.entries, "telegram")
        assert results and results[0].name == "socis-telegram-business"

    def test_matches_description(self):
        results = search_index(self.entries, "secretary")
        assert [e.name for e in results] == ["socis-telegram-business"]

    def test_fuzzy_typo_tolerance(self):
        results = search_index(self.entries, "socis-media-studo")
        assert results and results[0].name == "socis-media-studio"
...
class TestLoadIndex:
    def test_fresh_cache_wins_without_network(self, socis_agent_home, monkeypatch):
        _write_cache(socis_agent_home, SAMPLE)
    ...
    def test_expired_cache_triggers_remote(self, socis_agent_home, monkeypatch):
        _write_cache(socis_agent_home, SAMPLE, age_seconds=plugin_index.INDEX_CACHE_TTL + 60)
    ...
    def test_remote_failure_falls_back_to_stale_cache(self, socis_agent_home, monkeypatch):
        _write_cache(socis_agent_home, SAMPLE, age_seconds=plugin_index.INDEX_CACHE_TTL + 60)
    ...
    def test_no_cache_no_remote_falls_back_to_seed(self, socis_agent_home, monkeypatch):
        monkeypatch.setattr(plugin_index, "_fetch_remote", lambda: None)
        entries, source = load_index()
        assert source == "seed"
        assert len(entries) >= 3

    def test_refresh_bypasses_fresh_cache(self, socis_agent_home, monkeypatch):
        _write_cache(socis_agent_home, SAMPLE)
    ...
    def test_offline_skips_network(self, socis_agent_home, monkeypatch):
    ...
    def test_corrupt_cache_ignored(self, socis_agent_home, monkeypatch):
        cache = socis_agent_home / "cache" / "plugin_index.json"
        cache.parent.mkdir(parents=True, exist_ok=True)
    ...
    def test_remote_fetch_writes_cache(self, socis_agent_home, monkeypatch):
...
class TestResolveName:
    ...
    def test_exact_unique(self):
        entry, candidates = resolve_name(self.entries, "socis-media-studio")
        assert entry is not None and entry.repo == "SOCIS/socis-media-studio"

    def test_case_insensitive(self):
        entry, _ = resolve_name(self.entries, "SOCIS-Media-Studio")
        assert entry is not None

    def test_unique_partial(self):
        entry, _ = resolve_name(self.entries, "telegram")
        assert entry is not None and entry.name == "socis-telegram-business"

    def test_ambiguous_partial(self):
        entry, candidates = resolve_name(self.entries, "socis")
...
class TestInstallResolution:
    def test_bare_name_detection(self):
        from socis_cli.plugins_cmd import _looks_like_bare_index_name

        assert _looks_like_bare_index_name("socis-media-studio")
    ...
    def test_install_resolves_name_and_pins_ref(self, socis_agent_home, monkeypatch):
        from socis_cli import plugins_cmd
        ...
        with pytest.raises(SystemExit):
            plugins_cmd.cmd_install("socis-media-studio", enable=False)
        assert captured["identifier"] == "SOCIS/socis-media-studio"
        assert captured["ref"] == "e" * 40

    def test_install_explicit_ref_beats_index_pin(self, socis_agent_home, monkeypatch):
        from socis_cli import plugins_cmd
        ...
        monkeypatch.setattr(plugins_cmd, "_install_plugin_core", fake_core)
        with pytest.raises(SystemExit):
            plugins_cmd.cmd_install("socis-media-studio", enable=False, ref="d" * 40)
        assert captured["ref"] == "d" * 40

    def test_install_ambiguous_name_lists_candidates_and_exits(
        self, socis_agent_home, monkeypatch, capsys
    ):
        from socis_cli import plugins_cmd
        ...
        with pytest.raises(SystemExit) as exc:
            plugins_cmd.cmd_install("socis", enable=False)
        assert exc.value.code == 1
        assert not called
        out = capsys.readouterr().out
        assert "ambiguous" in out
        assert "socis-media-studio" in out
        assert "socis-telegram-business" in out

    def test_install_unknown_name_exits(self, socis_agent_home, monkeypatch, capsys):
        from socis_cli import plugins_cmd
    ...
    def test_owner_repo_passthrough_skips_index(self, socis_agent_home, monkeypatch):
...
class TestCmdSearch:
    def test_json_output(self, socis_agent_home, monkeypatch, capsys):
        from socis_cli import plugins_cmd
        ...
        assert payload["results"][0]["name"] == "socis-telegram-business"
        assert payload["results"][0]["repo"] == "SOCIS/socis-telegram-business"
    ...
    def test_no_results_message(self, socis_agent_home, monkeypatch, capsys):
        from socis_cli import plugins_cmd
    ...
    def test_dispatch_routes_search(self, socis_agent_home, monkeypatch):
        from socis_cli import plugins_cmd
