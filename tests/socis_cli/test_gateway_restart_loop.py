...
Covers:
- Defense 1: gateway stop/restart refuse when _SOCIS_AGENT_GATEWAY=1
...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        ...
        "socis gateway stop",
        "socis gateway uninstall",
        "socis  gateway  restart",         # double spaces
        "Hermez Gateway Restart".lower().replace("z", "s"),  # case handled
        "SOCIS GATEWAY RESTART",           # uppercase
    ])
    def test_socis_gateway_commands(self, text):
    ...
    @pytest.mark.parametrize("text", [
        ...
        "launchctl submit -l ai.socis.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.socis-agent/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l socis-gateway-restart-helper -- /bin/sh helper.sh",
        # bootstrap loads an arbitrary plist — same laundering shape.
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.socis.gateway.restart-once.plist",
        "launchctl bootout gui/501/ai.socis.gateway",
        ...
        (
            "launchctl submit \\\n"
            "  -l ai.socis.gateway-hard-restart-no-photon-notice \\\n"
            "  -- /bin/sh ~/.socis-agent/scripts/hard_restart_gateway_no_photon_notice.sh"
        ),
    ])
    ...
    def test_label_defined_before_verb_is_caught(self):
        ...
        cmd = (
            "uid=$(id -u); for item in 'ai.socis.gateway-apollo:/a.plist' "
            "'ai.socis.gateway-cronus:/c.plist' 'ai.socis.gateway-plutus:/p.plist' "
            "'ai.socis.gateway:/Users/botuser/Library/LaunchAgents/ai.socis.gateway.plist'; "
    ...
    def test_line_continuation_does_not_bridge_unrelated_lines(self):
        ...
        text = (
            ...
            "unrelated socis note on the next line"
        )
    ...
    @pytest.mark.parametrize("text", [
        ...
        # `launchctl kick"start" ... ai.socis.gateway` executes exactly as
        ...
        'launchctl kick"start" -k gui/501/ai.socis.gateway',
        "launchctl kick'start' -k gui/501/ai.socis.gateway",
        "launchctl kick\\start -k gui/501/ai.socis.gateway",
        'launchctl "kickstart" -k gui/501/ai.socis.gateway',
        # Splices on the newer/legacy unload spellings this PR added.
        'launchctl boot"out" gui/501/ai.socis.gateway',
        "launchctl dis\\able gui/501/ai.socis.gateway",
        # The gateway identifier itself can be spliced just as easily.
        'launchctl bootout gui/501/ai.socis."gateway"',
        # Same class on the systemctl and socis-CLI branches.
        'systemctl re"start" socis-gateway',
        'socis gateway re"start"',
    ])
    ...
    def test_spliced_verb_inside_shell_c_payload_is_blocked(self):
        ...
        command = 'sh -c \'launchctl kick"start" -k gui/501/ai.socis.gateway\''
    ...
    @pytest.mark.parametrize("text", [
        ...
        'launchctl kick"start" -k gui/501/ai.socis.update-checker',
        'systemctl re"start" socis-meta.service',
    ...
    @pytest.mark.parametrize("text", [
        ...
        "socis cron list",
        "socis update",
        "socis config set model claude",
        "echo 'just a normal cron job'",
        "run the backup script",
        "gateway is running fine",
        # `socis gateway start` is benign — starting a gateway from inside a
        ...
        "socis gateway start",
        "socis gateway start --all",
        # Tightened launchctl/systemctl branches: ops on NON-gateway socis
        # services must not be falsely blocked (the old `.*socis` matched any
        # socis token).
        "launchctl unload ai.socis.update-checker.plist",
        "launchctl restart ai.socis.daemon",
        ...
        "systemctl restart socis-meta.service",
        "systemctl restart socis-cron-helper",
        ...
        "echo after the socis gateway restarted cleanly",
        "the socis gateway stopped responding, please investigate",
        # #92372 Branch D: `p?kill` without a leading \b matched the "kill"
        # tail of "skill".
        "socis skill view gateway-notes && echo socis gateway docs",
        # #77173/#77536: a file path with embedded spaces containing the
        # lifecycle words must not match — `socis` is a path component
        # there, not a command.
        "cat '/docs/socis gateway restart-notes.md'",
        "less /home/user/notes/socis gateway restart runbook.txt",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        "socis gateway restart",
        "socis gateway restart; echo done",
        "socis gateway stop && echo stopped",
        # #77173 command-position anchor must not weaken separator/subshell
        # forms either.
        "true;socis gateway restart",
        "true && socis gateway stop",
        "echo $(socis gateway restart)",
        "echo `socis gateway restart`",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        'import subprocess\nsubprocess.run(["launchctl", "bootout", "gui/501/ai.socis.gateway"])',
        'subprocess.run(["socis", "gateway", "restart"])',
        'os.system("launchctl kickstart -k gui/501/ai.socis.gateway")',
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        'data = ["socis", "notes"]  # unrelated list',
    ])
    ...
    def test_inert_heredoc_body_prose_not_blocked(self):
        ...
        text = (
            ...
            "If the box is wedged, a human can run: socis gateway restart\n"
            "EOF"
        )
        assert not _contains_gateway_lifecycle_command(text), f"Should NOT match: {text!r}"
...
class TestProfileFlagGatewayLifecycle:
    """#78028: `socis -p <profile> gateway restart|stop` bypasses Branch A's
    ...
    def _pin_profile_identity(self, monkeypatch):
        # The ambient test env may carry SOCIS_AGENT_HOME/SOCIS_AGENT_PROFILE; pin the
        # profile identity explicitly so every assertion is deterministic.
        monkeypatch.setenv("SOCIS_AGENT_PROFILE", "zeus")
        monkeypatch.delenv("SOCIS_AGENT_PROFILE_NAME", raising=False)

    @pytest.mark.parametrize("text", [
        "socis -p zeus gateway stop",
        "socis -p zeus gateway restart",
        "socis --profile zeus gateway restart",
        "socis --profile zeus gateway stop",
        "socis --profile=zeus gateway restart",
        # Global flags before/after the selector must not hide the shape.
        "socis -v -p zeus gateway restart",
        "socis -p zeus -v gateway restart",
        "socis --debug --profile zeus gateway stop",
    ...
    @pytest.mark.parametrize("text", [
        "socis -p venus gateway stop",
        "socis -p venus gateway restart",
        "socis --profile venus gateway restart",
        "socis --profile=venus gateway stop",
        "socis -p venus -v gateway restart",
    ])
    def test_sibling_allowed(self, text):
        assert not _contains_gateway_lifecycle_command(text), f"Should allow: {text!r}"

    @pytest.mark.parametrize("text", [
        "socis -p zeus gateway start",
        "socis -p zeus gateway start --all",
    ])
    ...
    def test_adjacent_form_still_blocked(self):
        ...
        assert _contains_gateway_lifecycle_command("socis gateway restart")
        assert _contains_gateway_lifecycle_command("socis gateway stop")

    def test_socis_agent_home_derived_profile(self, monkeypatch):
        # Without SOCIS_AGENT_PROFILE the guard falls back to the SOCIS_AGENT_HOME-
        # derived profile identity (get_active_profile_name) — the signal the
        # gateway process itself carries.
        monkeypatch.delenv("SOCIS_AGENT_PROFILE", raising=False)
        monkeypatch.delenv("SOCIS_AGENT_PROFILE_NAME", raising=False)
        import socis_cli.profiles as profiles_mod

        monkeypatch.setattr(profiles_mod, "get_active_profile_name", lambda: "zeus")
        assert _contains_gateway_lifecycle_command("socis -p zeus gateway restart")
        assert not _contains_gateway_lifecycle_command("socis -p venus gateway restart")
    ...
    def test_no_profile_context_conservative_allow(self, monkeypatch):
        ...
        assert not _contains_gateway_lifecycle_command("socis -p zeus gateway restart")
        assert _contains_gateway_lifecycle_command("socis gateway restart")
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_socis_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade socis then run socis gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_script_with_lifecycle_command(self, tmp_path, capsys, monkeypatch):
        ...
        # restart_socis_gateway_once.sh). The script must live under
        # SOCIS_AGENT_HOME/scripts so the scheduler — and the guard — resolve it.
        monkeypatch.setenv("SOCIS_AGENT_HOME", str(tmp_path / ".socis-agent"))
        scripts_dir = tmp_path / ".socis-agent" / "scripts"
...
class TestGatewaySelfTargetingGuard:
    ...
    def test_stop_allows_outside_gateway(self, monkeypatch):
        ...
        monkeypatch.delenv("_SOCIS_AGENT_GATEWAY", raising=False)
...
class TestTerminalToolGatewayLifecycleGuard:
    """terminal_tool must refuse gateway lifecycle commands when _SOCIS_AGENT_GATEWAY=1.

    Issue #37453: systemctl --user restart socis-gateway runs as a child of the
    ...
    @pytest.mark.parametrize("cmd", [
        "systemctl restart socis-gateway",
        "systemctl --user restart socis-gateway",
        "systemctl stop socis-gateway.service",
        "socis gateway restart",
        "socis gateway uninstall",
        "launchctl kickstart gui/501/ai.socis.gateway",
        "launchctl bootout gui/501/ai.socis.gateway",
        # #62891 exact reported shape and its bootstrap sibling.
        "launchctl submit -l ai.socis.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.socis-agent/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.socis.gateway.restart-once.plist",
        "pkill -f socis.*gateway",
    ])
    ...
    def test_force_true_cannot_bypass_block(self, monkeypatch):
        ...
        result = json.loads(tt.terminal_tool(
            command="systemctl restart socis-gateway", force=True
        ))
    ...
    def test_blocks_launchctl_submit_inside_gateway(self, monkeypatch, tmp_path):
        ...
        result = json.loads(tt.terminal_tool(
            command=(
                "launchctl submit -l ai.socis.delayed-ops -- "
                f"/bin/bash {script}"
            )
        ))
    ...
    @pytest.mark.parametrize("command", [
        # Neutral, non-socis label: label-independent detection is the point
        # (#62891 second reproduction used `ai.socis.svc-reload-tmp`).
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl submit -l ai.socis.svc-reload-tmp -- /bin/sh /tmp/h-svc-reload.sh",
    ...
    def test_cli_agent_session_not_blocked_by_inherited_env(
        ...
        """#92560: CLI/TUI agent sessions inherit _SOCIS_AGENT_GATEWAY=1 from the
        ...
        # Simulate a CLI agent session: _SOCIS_AGENT_GATEWAY=1 is in the
        ...
        self._patch_env(monkeypatch, _FakeEnv(), inside_gateway=False)
        monkeypatch.setenv("_SOCIS_AGENT_GATEWAY", "1")
        monkeypatch.setattr(
            tt, "_check_all_guards", lambda cmd, env, **kwargs: {"approved": True}
        )

        result = json.loads(tt.terminal_tool(command="socis gateway restart"))

        assert result["exit_code"] == 0
        assert calls == ["socis gateway restart"]
    ...
    def test_blocks_launchctl_submit_hidden_in_referenced_script(
        ...
        script.write_text(
            "#!/bin/bash\nlaunchctl submit -l ai.socis.loop -- /bin/true\n"
        )
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
    ...
    def test_launchctl_submit_parser_handles_shell_quoting(self, monkeypatch):
        ...
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
        result = json.loads(tt.terminal_tool(
            command="launchctl sub\"\"mit -l ai.socis.loop -- /bin/true"
        ))
    ...
    def test_safe_systemctl_commands_pass_through(self, monkeypatch):
        """Non-socis systemctl commands must not be blocked by this guard."""
        import tools.terminal_tool as tt
...
class TestLifecycleGuardModule:
    ...
    def test_prompt_with_command_raises(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked) as exc:
            check_gateway_lifecycle("please run socis gateway restart", None)
    ...
    def test_split_across_prompt_and_script_still_blocks(self, tmp_path):
        ...
        script.write_text("socis gateway stop\n", encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_binary_script_does_not_silently_bypass(self, tmp_path):
        ...
        script.write_bytes(b"\xfesocis gateway restart\xff")
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("", str(script))


    def test_relative_script_resolved_under_scripts_dir(self, tmp_path, monkeypatch):
        """A bare/relative script name resolves under SOCIS_AGENT_HOME/scripts (the
    ...
    def test_python_script_with_pathlib_division_not_blocked(self, tmp_path):
        ...
        ".socis-agent") must NOT be blocked.
    ...
    def test_python_script_with_literal_lifecycle_command_still_blocked(
        ...
        script.write_text('import os\nos.system("socis gateway restart")\n', encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("clean prompt", str(script))
    ...
    def test_read_referenced_script_tolerates_nul_in_path(self):
        ...
        text, unsafe = _read_referenced_script(Path("/tmp/socis\x00binary"))
        assert text is None
    ...
    def test_binary_from_remote_callback_never_false_positives(self):
        ...
        def _remote_read(_path: str):
            return "MZ\x00\x00\x90\x00 socis gateway restart \x00\x00junk"
    ...
    def test_cron_guard_total_when_home_unresolvable(self, monkeypatch):
        """`get_socis_agent_home()` falls back to Path.home(), which raises
        RuntimeError when neither SOCIS_AGENT_HOME nor HOME resolves
...
class TestRelativePathDoesNotDisableDataExemption:
    ...
    @pytest.mark.parametrize("command", [
        "grep -r 'systemctl restart socis-gateway' .",
        "grep -rn 'socis gateway restart' ./logs",
        "rg 'socis gateway restart' ../archive",
        "grep -c 'systemctl stop socis-gateway' ./var/log/syslog",
        "sqlite3 ./stats.db \"SELECT restart_reason FROM socis_gateway_restarts\"",
    ])
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 ./db ".shell socis gateway restart"',
        'sqlite3 ./db ".system systemctl restart socis-gateway"',
        'psql ./x -c "\\! systemctl restart socis-gateway"',
        "grep -r 'socis gateway restart' . | sh",
        "grep -r 'socis gateway restart' ./logs | bash",
        "grep -r 'socis gateway restart' . | sudo sh",
        "grep -r 'x' . ; socis gateway restart",
        "grep -r 'x' . && systemctl restart socis-gateway",
        'grep -r "$(socis gateway restart)" .',
        "rg 'x' ./logs | xargs systemctl restart socis-gateway",
    ])
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 db ".shell socis gateway restart"',
        'sqlite3 db ".system systemctl restart socis-gateway"',
        'psql -c "\\! systemctl restart socis-gateway"',
    ])
...
class TestCreateJobBlocksLifecycleCommands:
    ...
    socis_cli.cron.cron_create. Enforcing at create_job covers both."""
    ...
    def test_create_job_blocks_prompt_command(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked):
            create_job(prompt="then run socis gateway restart", schedule="30m")
    ...
    def test_cronjob_tool_surfaces_block_as_error(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("SOCIS_AGENT_HOME", str(tmp_path / ".socis-agent"))
        (tmp_path / ".socis-agent").mkdir(parents=True)
        from tools.cronjob_tools import cronjob
        result = json.loads(cronjob(
            action="create", schedule="0 9 * * *",
            prompt="please run socis gateway restart nightly",
        ))
...
class TestLifecycleGuardDataArgumentExemption:
    ...
    @pytest.mark.parametrize("command", [
        ...
        "'systemctl stop socis-gateway'\"",
        # grep/rg pattern arguments hunting for the lifecycle string.
        "grep -c 'systemctl restart socis-gateway' /var/log/syslog",
        "rg 'socis gateway restart' /home/user/.socis-agent/logs/",
        "journalctl -u socis-gateway --grep 'systemctl restart socis-gateway'",
        # SQL with stop/restart column/value words but no command shape.
        'sqlite3 stats.db "SELECT stop_time, restart_reason FROM '
        'socis_gateway_restarts"',
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 db ".shell socis gateway restart"',
        'psql -c "\\! systemctl restart socis-gateway"',
        "grep 'systemctl restart socis-gateway' cmds.txt | sh",
        "grep gateway f | xargs systemctl restart socis-gateway",
        'grep "$(systemctl restart socis-gateway)" f',
        "grep 'restart' log; systemctl restart socis-gateway",
        'sqlite3 db "SELECT 1"; socis gateway stop',
        # Plain lifecycle commands are unaffected by the exemption.
        "socis gateway restart",
        "sudo systemctl stop socis-gateway",
    ])
    ...
    def test_python_script_branch_gets_the_same_exemption(self, tmp_path):
        ...
        prompt = (
            ...
            "WHERE msg LIKE '%systemctl restart socis-gateway%'\""
        )
