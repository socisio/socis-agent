...
socis_cli.config.load_config, so the managed overlay has to be wired into each.
...
def test_timezone_honors_managed(homes, monkeypatch):
    home, managed = homes
    # socis_agent_time checks an env override first; ensure it's unset so config wins.
    ...
    assert socis_agent_time._resolve_timezone_name() == "Asia/Tokyo"
...
def test_gateway_env_bridge_honors_managed(homes, monkeypatch):
    ...
    every turn (SOCIS_AGENT_TIMEZONE, SOCIS_AGENT_REDACT_SECRETS, SOCIS_AGENT_MAX_ITERATIONS,
    ...
    # writing SOCIS_AGENT_TIMEZONE = cfg["timezone"]. Prove the overlay flips the value.
    import yaml
