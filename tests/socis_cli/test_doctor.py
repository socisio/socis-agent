...
import socis_cli.doctor as doctor
import socis_cli.gateway as gateway_cli
from socis_cli import doctor as doctor_mod
from socis_cli.doctor import _has_provider_env_config
...
class TestDoctorPlatformHints:
    ...
    def test_sqlite_upgrade_hint_recreates_docker_containers(self, monkeypatch):
        ...
        assert "docker pull socis/socis-agent:latest" in hint
        assert "recreate all SOCIS containers" in hint
        assert "socis update" not in hint

    def test_sqlite_upgrade_hint_keeps_git_runtime_repair(self):
        hint = doctor._sqlite_upgrade_hint("git")

        assert "run `socis update`" in hint

    def test_sqlite_upgrade_hint_uses_pkg_for_apt_managed_install(self):
        hint = doctor._sqlite_upgrade_hint("apt")

        assert "run `pkg upgrade socis-agent`" in hint
        assert "socis update" not in hint
...
class TestDoctorEnvFileEncoding:
    """Regression for #18637 (bug 3): `socis doctor` crashed on Windows
    ...
    def test_doctor_reads_env_as_utf8_even_when_locale_is_not_utf8(
        ...
        monkeypatch.setattr(doctor_mod, "SOCIS_AGENT_HOME", socis_agent_home)
...
class TestDoctorToolAvailabilityOverrides:
    ...
    def test_marks_kanban_available_only_when_missing_worker_env_gate(self, monkeypatch):
        ...
        monkeypatch.delenv("SOCIS_AGENT_KANBAN_TASK", raising=False)
    ...
    def test_leaves_kanban_unavailable_when_worker_env_is_set(self, monkeypatch):
        monkeypatch.setenv("SOCIS_AGENT_KANBAN_TASK", "probe")
...
class TestDoctorMemoryProviderSection:
    ...
    def _make_socis_agent_home(self, tmp_path, provider="", memory_config=None):
        """Create a minimal SOCIS_AGENT_HOME with config.yaml."""
    ...
    def _run_doctor_and_capture(
        ...
        *,
        ...
        home = self._make_socis_agent_home(tmp_path, provider, memory_config)
        ...
        monkeypatch.setattr(doctor_mod, "SOCIS_AGENT_HOME", home)
        ...
        try:
            from socis_cli import auth as _auth_mod
...
def test_run_doctor_accepts_socis_provider_ids_that_catalog_aliases(
...
def _doctor_env_for_agent_browser(monkeypatch, tmp_path):
    ...
    branch in run_doctor (socis_cli/doctor.py ~1557-1605)."""
    home = tmp_path / ".socis-agent"
    home.mkdir(parents=True, exist_ok=True)
    (home / "config.yaml").write_text("memory: {}\n", encoding="utf-8")
    project = tmp_path / "project"
...
def test_run_doctor_fix_warms_npx_cache_when_agent_browser_resolves_via_npx(
    ...
    """`socis doctor --fix` must actually call warm_agent_browser_npx_cache()
...
class TestGitHubTokenCheck:
    ...
    def _isolate_home(monkeypatch, home):
        """Point doctor at the temp SOCIS_AGENT_HOME.

        ``run_doctor`` reads the module-level ``SOCIS_AGENT_HOME`` constant (cached
        at import time), NOT the env var — so ``setenv("SOCIS_AGENT_HOME")`` alone
        leaves doctor probing the REAL ~/.socis-agent. On a dev machine with a
...
def test_has_healthy_oauth_fallback_returns_false_for_unknown_provider():
    from socis_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
...
class TestHasHealthyOauthFallbackForXai:
    ...
    def test_returns_false_when_xai_import_unavailable(self, monkeypatch):
        ...
        monkeypatch.delattr("socis_cli.auth.get_xai_oauth_auth_status", raising=False)
        # Force doctor module to re-import the function
        monkeypatch.delitem(sys.modules, "socis_cli.doctor", raising=False)
        from socis_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
...
class TestDoctorStaleMaxIterationsDrift:
    """Regression for #17534: a stale SOCIS_AGENT_MAX_ITERATIONS in .env shadows
    ...
    def _run_config_section(self, monkeypatch, tmp_path, *, fix, ghost, cfg_turns,
        ...
        if ghost is not None:
            env_lines.append(f"SOCIS_AGENT_MAX_ITERATIONS={ghost}\n")
        (socis_agent_home / ".env").write_text("".join(env_lines), encoding="utf-8")

        monkeypatch.setattr(doctor_mod, "SOCIS_AGENT_HOME", socis_agent_home)
        monkeypatch.setattr(doctor_mod, "get_socis_agent_home", lambda: socis_agent_home)
        ...
        if os_environ_value is not None:
            ...
            monkeypatch.setenv("SOCIS_AGENT_MAX_ITERATIONS", str(os_environ_value))
        else:
            monkeypatch.delenv("SOCIS_AGENT_MAX_ITERATIONS", raising=False)
    ...
    def test_fix_removes_ghost(self, monkeypatch, tmp_path):
        ...
        assert "Removed stale SOCIS_AGENT_MAX_ITERATIONS" in out
        env_after = (socis_agent_home / ".env").read_text(encoding="utf-8")
        assert "SOCIS_AGENT_MAX_ITERATIONS" not in env_after
...
class TestDoctorDeprecatedConfigAndEnv:
    ...
    def test_socis_tool_progress_warning_says_unsupported_since_floor(self):
        """SOCIS_AGENT_TOOL_PROGRESS lost its last consumer (the retired v3→4
        ...
        findings = dict(
            doctor_mod.collect_deprecated_env_vars({"SOCIS_AGENT_TOOL_PROGRESS": "true"})
        )
        assert "ignored/unsupported since config floor v12" in findings["SOCIS_AGENT_TOOL_PROGRESS"]
        # The MODE variant is still read by the gateway fallback → keeps the
        # plain deprecation wording.
        mode = dict(
            doctor_mod.collect_deprecated_env_vars({"SOCIS_AGENT_TOOL_PROGRESS_MODE": "all"})
        )
        assert mode["SOCIS_AGENT_TOOL_PROGRESS_MODE"] == "display.tool_progress in config.yaml"

    def _run_doctor_with_config(self, monkeypatch, tmp_path, *, config_yaml: str, env_text: str = ""):
        socis_agent_home = tmp_path / ".socis-agent"
        socis_agent_home.mkdir(parents=True)
        (socis_agent_home / "config.yaml").write_text(config_yaml, encoding="utf-8")
        env_body = env_text if env_text else "OPENAI_API_KEY=sk-test\n"
        (socis_agent_home / ".env").write_text(env_body, encoding="utf-8")
    ...
    def test_report_does_not_count_as_blocking_issue(self, monkeypatch, tmp_path, capsys):
        ...
        findings = doctor_mod.report_deprecated_config_and_env(
            ...
            {"SOCIS_AGENT_TOOL_PROGRESS_MODE": "verbose"},
        )
        out = capsys.readouterr().out
        assert len(findings) == 2
        assert "Deprecated: delegation.max_async_children" in out
        assert "Deprecated: SOCIS_AGENT_TOOL_PROGRESS_MODE" in out
...
class TestMacOSTCCGrants:
    ...
    def test_ok_on_certificate_anchored_dr(self, monkeypatch, capsys, tmp_path):
        """A cert-anchored DR (socis desktop --setup-tcc-identity, or a
