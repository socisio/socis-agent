"""Regression tests for #34107 — Docker UID/GID handling in ensure_socis_agent_home.

When SOCIS runs in Docker with ``SOCIS_AGENT_UID=1000`` / ``SOCIS_AGENT_GID=911``,
the entrypoint chowns the top-level ``SOCIS_AGENT_HOME`` once at startup. But
...
The fix is a ``_chown_to_socis_uid`` helper that reads the env vars and
...
# ---------------------------------------------------------------------------
# _resolve_socis_uid_gid
# ---------------------------------------------------------------------------


class TestResolveSOCISUidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):
        monkeypatch.setenv("SOCIS_AGENT_UID", "1000")
        monkeypatch.setenv("SOCIS_AGENT_GID", "911")
        from socis_cli.config import _resolve_socis_uid_gid
...
class TestChownToSOCISUid:
    ...
    def test_attributeerror_swallowed_for_windows_compat(self, tmp_path, monkeypatch):
        ...
        with patch.object(cfg.os, "chown", side_effect=AttributeError("no chown on this platform")):
            cfg._chown_to_socis_uid(d)  # must not raise
...
class TestSecureDirChown:
    ...
    def test_secure_dir_no_chown_when_env_unset(self, tmp_path, monkeypatch):
        monkeypatch.delenv("SOCIS_AGENT_UID", raising=False)
        monkeypatch.delenv("SOCIS_AGENT_GID", raising=False)
        from socis_cli import config as cfg
