...
def socis_agent_home(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("SOCIS_AGENT_ENABLE_PROJECT_PLUGINS", "0")
    monkeypatch.setenv(
        "SOCIS_AGENT_BUNDLED_PLUGINS", str(tmp_path / "empty-bundled")
    )
...
class TestV1Regression:
    def test_v1_manifest_parses_with_defaults(self, socis_agent_home):
        _write_plugin(socis_agent_home / "plugins", "oldie")
        _enable(socis_agent_home, ["oldie"])
        mgr = PluginManager()
    ...
    def test_v1_unknown_fields_do_not_warn_loudly(self, socis_agent_home, caplog):
        _write_plugin(
            socis_agent_home / "plugins", "oldie",
...
class TestV2Parsing:
    def test_v2_fields_parse(self, socis_agent_home):
        _write_plugin(
            socis_agent_home / "plugins", "modern",
            manifest_extra={
                "manifest_version": 2,
                "api_version": 1,
                "license": "MIT",
        ...
        _enable(socis_agent_home, ["modern"])
        mgr = PluginManager()
        mgr.discover_and_load()
    ...
    def test_unknown_field_in_v2_warns_but_loads(self, socis_agent_home, caplog):
        _write_plugin(
            socis_agent_home / "plugins", "modern",
    ...
    def test_future_manifest_version_warns_but_loads(self, socis_agent_home, caplog):
        _write_plugin(
            socis_agent_home / "plugins", "fromfuture",
            manifest_extra={
                "manifest_version": SUPPORTED_MANIFEST_VERSION + 5,
            },
        )
        _enable(socis_agent_home, ["fromfuture"])
        with caplog.at_level(logging.WARNING):
            mgr = PluginManager()
            mgr.discover_and_load()
        assert mgr._plugins["fromfuture"].enabled
        assert "newer than this SOCIS" in caplog.text

    def test_malformed_v2_fields_warn_and_degrade(self, socis_agent_home, caplog):
        _write_plugin(
            socis_agent_home / "plugins", "sloppy",
            manifest_extra={
                "manifest_version": 2,
                "api_version": "banana",
        ...
        _enable(socis_agent_home, ["sloppy"])
        with caplog.at_level(logging.WARNING):
            mgr = PluginManager()
            mgr.discover_and_load()
...
class TestDependencyOrder:
    def test_dep_registers_before_dependent(self, socis_agent_home):
        ...
        _write_plugin(
            socis_agent_home / "plugins", "aaa-consumer",
            manifest_extra={
                "manifest_version": 2,
        ...
        _write_plugin(
            socis_agent_home / "plugins", "zzz-base",
            register_body="import sys; sys._m2_order.append('zzz-base')",
        )
        _enable(socis_agent_home, ["aaa-consumer", "zzz-base"])
        import sys
    ...
    def test_missing_dep_warns_but_loads(self, socis_agent_home, caplog):
        _write_plugin(
            socis_agent_home / "plugins", "needy",
            manifest_extra={
                "manifest_version": 2,
                "requires_plugins": [{"id": "ghost-plugin"}],
            },
        )
        _enable(socis_agent_home, ["needy"])
        with caplog.at_level(logging.WARNING):
            mgr = PluginManager()
            mgr.discover_and_load()
    ...
    def test_cycle_warns_and_falls_back_alpha(self, socis_agent_home, caplog):
        _write_plugin(
            socis_agent_home / "plugins", "cyc-a",
            manifest_extra={
                "manifest_version": 2,
        ...
        _write_plugin(
            socis_agent_home / "plugins", "cyc-b",
            manifest_extra={
                "manifest_version": 2,
        ...
        _enable(socis_agent_home, ["cyc-a", "cyc-b"])
        import sys
...
class TestConfigSchema:
    def test_type_mismatch_warns_but_loads(self, socis_agent_home, caplog):
        _write_plugin(
            socis_agent_home / "plugins", "cfgd",
            manifest_extra={
                "manifest_version": 2,
                "config_schema": {
                    "api_url": {"type": "str"},
    ...
    def test_unknown_declared_type_skips_check(self, socis_agent_home, caplog):
        _write_plugin(
            socis_agent_home / "plugins", "weird",
            manifest_extra={
                "manifest_version": 2,
...
class TestPythonDependenciesSeam:
    def test_missing_pip_dep_surfaced_with_hint_not_installed(
        self, socis_agent_home, caplog, monkeypatch
    ):
        calls = []
        import subprocess
        ...
        _write_plugin(
            socis_agent_home / "plugins", "pipful",
            manifest_extra={
                "manifest_version": 2,
                "python_dependencies": [
        ...
        _enable(socis_agent_home, ["pipful"])
        with caplog.at_level(logging.WARNING):
            mgr = PluginManager()
            mgr.discover_and_load()
    ...
    def test_satisfied_pip_dep_is_quiet(self, socis_agent_home, caplog):
        _write_plugin(
            socis_agent_home / "plugins", "pipok",
            manifest_extra={
                "manifest_version": 2,
                "python_dependencies": ["pyyaml>=5,<7"],
            },
        )
        _enable(socis_agent_home, ["pipok"])
        with caplog.at_level(logging.WARNING):
            mgr = PluginManager()
            mgr.discover_and_load()
...
class TestCtxHasPlugin:
    def test_has_plugin_probe(self, socis_agent_home):
        _write_plugin(socis_agent_home / "plugins", "probe-target")
        _write_plugin(
            socis_agent_home / "plugins", "prober",
            manifest_extra={
                "manifest_version": 2,
        ...
        _enable(socis_agent_home, ["probe-target", "prober"])
        import sys
