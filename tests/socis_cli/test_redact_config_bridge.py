...
`SOCIS_AGENT_REDACT_SECRETS` at module-import time. `socis_cli/main.py` at
...
in .env), the toggle was silently ignored in both `socis chat` and
`socis gateway run`.

Fix: bridge `security.redact_secrets` from config.yaml → `SOCIS_AGENT_REDACT_SECRETS`
env var in `socis_cli/main.py` BEFORE the `setup_logging()` call.
"""
import os
import subprocess
import sys
...
def test_redact_secrets_false_in_config_yaml_is_honored(tmp_path):
    ...
    # Spawn a fresh Python process that imports socis_cli.main and checks
    # _REDACT_ENABLED. Must be a subprocess — we need a clean module state.
    probe = textwrap.dedent(
        """\
        import sys, os
        # Make absolutely sure the env var is not pre-set
        os.environ.pop("SOCIS_AGENT_REDACT_SECRETS", None)
        sys.path.insert(0, %r)
        import socis_cli.main  # triggers the bridge + setup_logging
        import agent.redact
        print(f"REDACT_ENABLED={agent.redact._REDACT_ENABLED}")
        print(f"ENV_VAR={os.environ.get('SOCIS_AGENT_REDACT_SECRETS', '<unset>')}")
        """
    ) % str(REPO_ROOT)

    env = dict(os.environ)
    env["SOCIS_AGENT_HOME"] = str(socis_agent_home)
    env.pop("SOCIS_AGENT_REDACT_SECRETS", None)
...
def test_redact_secrets_default_true_when_unset(tmp_path):
    ...
    `SOCIS_AGENT_REDACT_SECRETS=false`).
    """
    socis_agent_home = tmp_path / ".socis-agent"
    socis_agent_home.mkdir()
    (socis_agent_home / "config.yaml").write_text("{}\n")  # empty config
...
def test_dotenv_redact_secrets_beats_config_yaml(tmp_path):
    """.env SOCIS_AGENT_REDACT_SECRETS takes precedence over config.yaml."""
    socis_agent_home = tmp_path / ".socis-agent"
    socis_agent_home.mkdir()
    (socis_agent_home / "config.yaml").write_text(
        textwrap.dedent(
    ...
    (socis_agent_home / ".env").write_text("SOCIS_AGENT_REDACT_SECRETS=true\n")
