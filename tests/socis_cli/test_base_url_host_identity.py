...
substring check, missing case variants and matching lookalike URLs). SOCIS
...
from socis_cli.cli_agent_setup_mixin import CLIAgentSetupMixin
...
def test_nous_portal_host_detection():
    ...
    assert base_url_host_matches("https://inference-api.nousresearch.com/v1", "socis.io")
    assert base_url_host_matches("https://portal.nousresearch.com", "socis.io")
    assert not base_url_host_matches("https://socis.io.evil.io/v1", "socis.io")
    assert not base_url_host_matches("https://proxy.example/socis.io/v1", "socis.io")
