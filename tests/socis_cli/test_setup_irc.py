"""Tests for IRC gateway configuration via `socis setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh SOCIS install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new SOCIS install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#socis")
            monkeypatch.setenv("IRC_NICKNAME", "socis-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `socis setup gateway` experience with IRC present."""
