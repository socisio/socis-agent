"""`socis update` must self-heal the ``socis-acp`` launcher.

ACP hosts (Zed, JetBrains, Buzz Desktop) resolve the agent by the
``socis-acp`` command name on the login-shell PATH. Fresh installs get the
launcher from ``scripts/install.sh``; existing installs get it from
``_ensure_acp_launcher()`` during ``socis update``.
"""
...
from socis_cli.main import _ensure_acp_launcher
...
def test_does_not_follow_symlink_into_venv(fake_home, tmp_path):
    """#21454 failure mode: never write through a symlinked socis-acp."""
    (fake_home / "socis").write_text("#!/bin/sh\n", encoding="utf-8")
    ...
    (fake_home / "socis-acp").symlink_to(console_script)

    _ensure_acp_launcher()

    assert console_script.read_text(encoding="utf-8") == marker
    assert (fake_home / "socis-acp").is_symlink()
...
def test_unwritable_bin_dir_is_skipped(fake_home):
    ...
    if not hasattr(os, "geteuid"):
        ...
        _ensure_acp_launcher()
        assert not (fake_home / "socis-acp").exists()
        return
    if os.geteuid() == 0:
