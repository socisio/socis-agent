...
``socis*.exe`` shims aside (``socis.exe.old.<ms>``) before invoking the
...
quarantined shims stayed renamed aside and ``socis`` vanished from PATH
...
def _make_scripts_dir(tmp_path: Path) -> Path:
    ...
    for name in ("socis", "socis-agent", "socis-acp", "socis-gateway"):
...
# ---------------------------------------------------------------------------
# socis_cli.main._run_quarantined_install
# ---------------------------------------------------------------------------
...
def test_main_noop_success_restores_shims(tmp_path):
    ...
    assert "socis.exe" in names, "socis.exe must be restored after a no-op install"
    assert "socis-acp.exe" in names
    assert "socis-gateway.exe" in names
    assert (scripts / "socis.exe").read_bytes() == b"MZ-old-socis"
...
def test_main_rewriting_success_keeps_fresh_shims(tmp_path):
    ...
    assert (scripts / "socis.exe").read_bytes() == b"MZ-new-socis"
...
# ---------------------------------------------------------------------------
# socis_cli._install_repair._run_install_cmd (the deferred-recovery path)
# ---------------------------------------------------------------------------
...
def test_repair_noop_success_restores_shims(tmp_path):
    ...
    names = _shim_names(scripts)
    assert "socis.exe" in names, "socis.exe must be restored after a no-op recovery install"
    assert (scripts / "socis.exe").read_bytes() == b"MZ-old-socis"
...
def test_repair_rewriting_success_keeps_fresh_shims(tmp_path):
    ...
    def fake_run(cmd, cwd=None, check=None, env=None):
        (scripts / "socis.exe").write_bytes(b"MZ-new-socis")
