"""Tests for `socis memory status` CLI command.
...
def _run_cmd_status(capfd, mem_config=None, memory_tools=None):
    ...
    """
    from socis_cli.memory_setup import cmd_status

    config = {"memory": mem_config or {}}
    if memory_tools is None:
        memory_tools = {"memory"}

    with patch("socis_cli.config.load_config", return_value=config):
        with patch("socis_cli.memory_setup._get_available_providers", return_value=[]):
            with patch(
