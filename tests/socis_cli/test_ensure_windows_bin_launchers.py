"""``socis`` must survive git operations on the checkout (launcher layout).

The Windows ``socis`` command is a launcher derived from the venv console
script. Its canonical home is the managed binary dir ``SOCIS_AGENT_HOME\\bin`` —
OUTSIDE the git checkout — because the earlier in-checkout home
(``socis-agent\\bin``) was swept by ``socis update``'s autostash
(``git stash push --include-untracked``) and, with the desktop updater's
``--keep-stash``, never restored: ``socis`` stopped resolving in every new
...
existing install's PATH to the canonical layout from the ``socis update``
tail. Platform verdict, PATH values, and registry I/O are injected
parameters (same pattern as ``socis_agent_constants.venv_bin_dir``), so these
...
from socis_cli._install_repair import (
...
def _make_managed(tmp_path, monkeypatch, *, relocatable: bool = False):
    """Fake managed layout: SOCIS_AGENT_HOME/socis-agent/venv/Scripts + launchers."""
    home = tmp_path / "socis"
    root = home / "socis-agent"
...
def test_source_checkout_untouched(tmp_path, monkeypatch):
    """A checkout NOT under SOCIS_AGENT_HOME gains nothing anywhere."""
    home = tmp_path / "socis-home"
    home.mkdir()
    monkeypatch.setenv("SOCIS_AGENT_HOME", str(home))
    root = tmp_path / "src" / "socis-agent"
    scripts = root / "venv" / "Scripts"
    scripts.mkdir(parents=True)
    for name in _WINDOWS_BIN_LAUNCHERS:
...
def test_profile_session_still_heals_the_shared_bin(tmp_path, monkeypatch):
    """Under ``socis -p <name>`` SOCIS_AGENT_HOME points inside profiles/<name>;
    ...
    monkeypatch.setenv("SOCIS_AGENT_HOME", str(home / "profiles" / "work"))
...
# ---------------------------------------------------------------------------
# migrate_windows_bin_path — the `socis update` tail migration
# ---------------------------------------------------------------------------
...
def test_migration_moves_path_to_home_bin_and_strips_legacy(managed_install):
    ...
    (root / "bin").mkdir()
    (root / "bin" / "socis.exe").write_bytes(b"legacy copy")
    ...
    assert (root / "bin" / "socis.exe").read_bytes() == b"legacy copy"
...
def test_normalize_windows_path_equivalences():
    assert (
        _normalize_windows_path(r"C:\Users\Me\AppData\Local\socis\bin")
        == _normalize_windows_path("c:/users/me/appdata/local/SOCIS/BIN/")
    )
...
def test_repo_gitignores_the_legacy_bin_dir():
    ...
    launchers at ``<checkout>/bin``. ``socis update`` autostashes with
    ...
    result = subprocess.run(
        ["git", "-C", str(repo_root), "check-ignore", "-q", "bin/socis.exe"],
        capture_output=True,
    )
    assert result.returncode == 0, (
        "bin/socis.exe is not gitignored — socis update's autostash "
