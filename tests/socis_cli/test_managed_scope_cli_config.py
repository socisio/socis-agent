...
socis_cli.config._load_config_impl, so the managed-scope merge has to be
applied in BOTH places or the interactive CLI/TUI surface (skin, display prefs)
silently ignores administrator-pinned values while `socis config`/`doctor`
...
def _load_cli_config(home):
    ...
    cli.py binds ``_socis_agent_home = get_socis_agent_home()`` at import time (module
    singleton), so monkeypatching SOCIS_AGENT_HOME after import doesn't move it.
    ...
    this only matters for tests that swap SOCIS_AGENT_HOME.)
    """
    import cli

    cli._socis_agent_home = home
