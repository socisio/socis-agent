"""Tests for the stale-dashboard handling run at the end of ``socis update``.

``socis update`` detects ``socis dashboard`` processes left over from the
...
def _refresh_bindings_against_live_module():
    """Rebind module-level names to the *current* ``socis_cli.main``.

    Other tests in the suite (notably ``test_env_loader.py`` and
    ``test_skills_subparser.py``) reload or delete ``socis_cli.main`` from
    ``sys.modules``.  When that happens on the same xdist worker before we
    run, our top-of-file ``from socis_cli.main import ...`` bindings end
    up pointing at the *old* module object.  ``patch(\"socis_cli.main.X\")``
    ...
    live = sys.modules.get("socis_cli.main")
    if live is None:
        live = importlib.import_module("socis_cli.main")
...
def _write_valid_ssh_backend_lock(tmp_path, monkeypatch) -> int:
    ...
    (lock_dir / "backend.lock.json").write_text(json.dumps({
        ...
        "port": 46369,
        "profile": "default",
        "socisPath": "/opt/socis/bin/socis",
        "socisHome": str(tmp_path),
    ...
    monkeypatch.delenv("SOCIS_AGENT_DESKTOP_CHILD_PID", raising=False)
    return pid
...
def test_update_cleanup_spares_backend_owned_by_valid_ssh_lock(tmp_path, monkeypatch):
    ...
    with patch(
        "socis_cli.main._find_stale_dashboard_pids",
    ...
    def test_self_pid_excluded(self):
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout="\n".join([
                    _ps_line(os.getpid(), "python3 -m socis_cli.main dashboard"),
                    _ps_line(12345, "socis dashboard --port 9119"),
                ]) + "\n",
                stderr="",
            )
...
class TestKillStaleDashboardPosix:
    ...
    def test_sigterm_graceful_exit(self, capsys):
        ...
        with patch("socis_cli.main._find_stale_dashboard_pids",
    ...
    def test_user_scope_restart_never_falls_back_to_system_or_sudo(self, capsys):
        ...
        def fake_run(args, *a, **kw):
            ...
            if args == ["systemctl", "--user", "list-unit-files", "socis-dashboard.service", "--no-legend", "--no-pager"]:
                return MagicMock(returncode=0, stdout="socis-dashboard.service enabled enabled\n", stderr="")
            if args == ["systemctl", "--user", "is-active", "socis-dashboard.service"]:
                return MagicMock(returncode=0, stdout="active\n", stderr="")
            if args == ["systemctl", "--user", "is-enabled", "socis-dashboard.service"]:
                return MagicMock(returncode=0, stdout="enabled\n", stderr="")
            if args == ["systemctl", "--user", "restart", "socis-dashboard.service"]:
                return MagicMock(returncode=0, stdout="", stderr="")
            raise AssertionError(f"unexpected subprocess.run call: {args}")

        with patch("subprocess.run", side_effect=fake_run), \
             patch("socis_cli.main._find_stale_dashboard_pids", return_value=[]) as find_pids, \
             patch("os.kill") as kill:
            _kill_stale_dashboard_processes(restart_managed=True)

        assert calls == [
            ["systemctl", "--user", "list-unit-files", "socis-dashboard.service", "--no-legend", "--no-pager"],
            ["systemctl", "--user", "is-active", "socis-dashboard.service"],
            ["systemctl", "--user", "is-enabled", "socis-dashboard.service"],
            ["systemctl", "--user", "restart", "socis-dashboard.service"],
        ]
        ...
        assert "✓ restarted socis-dashboard.service" in capsys.readouterr().out
...
class TestKillStaleDashboardWindows:
    ...
    def test_taskkill_invoked_for_each_pid(self, capsys):
        ...
        with patch("socis_cli.main._find_stale_dashboard_pids",
             ...
             patch("socis_cli._subprocess_compat.pid_is_socis", return_value=True), \
...
class TestDashboardUpdateCleanup:
    ...
    def test_all_failed_stops_do_not_claim_the_dashboard_was_stopped(self, capsys):
        with patch(
            "socis_cli.main._kill_stale_dashboard_processes",
...
class TestWindowsWmicEncoding:
    ...
    `socis update` on non-UTF-8 system locales (e.g. cp936 on zh-CN).
    """
    ...
    def test_wmic_routed_through_bounded_probe_run_with_ignore_errors(self):
        ...
        with patch("sys.platform", "win32"), \
             patch("socis_cli._subprocess_compat.bounded_probe_run") as mock_probe:
            mock_probe.return_value = subprocess.CompletedProcess(
                args=["wmic"],
                returncode=0,
                stdout=(
                    "CommandLine=python -m socis_cli.main dashboard\n"
                    "ProcessId=12345\n"
                ),
                stderr="",
            )
    ...
    def test_probe_failure_fails_open_to_empty_list(self):
        ...
        with patch("sys.platform", "win32"), \
             patch("socis_cli._subprocess_compat.bounded_probe_run",
                   return_value=None):
            assert _find_stale_dashboard_pids() == []
...
class TestSupervisedBackendRestart:
    ...
    def _live(self):
        return sys.modules["socis_cli.main"]
    ...
    def test_supervised_pid_restarts_owning_unit(self, capsys):
        ...
        with patch.object(live, "_restart_managed_dashboard_service", return_value=False), \
             ...
             patch.object(live, "_get_pid_cgroup_path",
                          return_value="/system.slice/socis-serve.service"), \
             patch.object(live, "_get_systemd_service_for_pid",
                          return_value="socis-serve.service"), \
        ...
        restart.assert_called_once_with(
            "socis-serve.service", "/system.slice/socis-serve.service"
        )
        out = capsys.readouterr().out
        assert "✓ restarted systemd service socis-serve.service" in out
    ...
    def test_already_restarted_unit_is_left_untouched(self):
        """Review on #83595: socis update's systemd fleet-restart loop may
        ...
        with patch.object(live, "_restart_managed_dashboard_service", return_value=False), \
             ...
             patch("time.sleep"):
            result = _kill_stale_dashboard_processes(
                restart_managed=True, already_restarted_units={"socis-serve"}
            )
...
class TestManualBackendRespawn:
    ...
    def test_non_orphan_fixed_port_still_respawns(self, capsys):
        ...
        with patch.object(live, "_restart_managed_dashboard_service", return_value=False), \
             ...
             patch("socis_cli.dashboard_procs._socis_agent_home_for_pid", return_value=None), \
    ...
    def test_respawn_adds_no_open_to_dashboard_commands(self, tmp_path, monkeypatch):
        ...
        with patch.object(live.subprocess, "Popen", _FakePopen):
            failed = live._respawn_dashboard_processes([
                ["socis", "dashboard", "--port", "8300"],
                ["socis", "serve", "--host", "0.0.0.0"],
            ])

        assert failed == []
        assert spawned[0] == ["socis", "dashboard", "--port", "8300", "--no-open"]
        assert spawned[1] == ["socis", "serve", "--host", "0.0.0.0"]
    ...
    def test_respawn_failure_returned(self, tmp_path, monkeypatch, capsys):
        ...
        with patch.object(live.subprocess, "Popen", side_effect=OSError("no such file")):
            failed = live._respawn_dashboard_processes([["socis", "serve"]])

        assert failed == [["socis", "serve"]]
        out = capsys.readouterr().out
...
class TestFilterDashboardRespawnCandidates:
    ...
    def test_skips_serve_port_zero(self):
        ...
        assert _filter_dashboard_respawn_candidates([
            (42, argv, "/home/u/.socis-agent/profiles/mini-cat"),
        ]) == []
    ...
    def test_keeps_ppid1_fixed_port_for_repeat_update(self):
        ...
        argv = ["socis", "dashboard", "--port", "9119"]
        assert _filter_dashboard_respawn_candidates([(10, argv, None)]) == [argv]

    def test_dedupes_identical_normalized_cmdlines(self):
        from socis_cli.dashboard_procs import _filter_dashboard_respawn_candidates

        a = ["/usr/bin/python3", "-m", "socis_cli.main", "dashboard", "--port", "8300"]
        b = ["/other/python", "-m", "socis_cli.main", "dashboard", "--port", "8300"]
    ...
    def test_caps_one_per_profile(self):
        ...
        a = ["socis", "--profile", "coder", "dashboard", "--port", "8300"]
        b = ["socis", "--profile", "coder", "dashboard", "--port", "8301"]
        c = ["socis", "--profile", "writer", "dashboard", "--port", "8302"]
        out = _filter_dashboard_respawn_candidates([
            (1, a, None),
            (2, b, None),
            (3, c, None),
    ...
    def test_caps_one_per_socis_agent_home(self):
        from socis_cli.dashboard_procs import _filter_dashboard_respawn_candidates

        home = "/tmp/socis-home-a"
        a = ["socis", "dashboard", "--port", "8300"]
    ...
    def test_profile_flag_and_profiles_home_share_cap(self):
        ...
        out = _filter_dashboard_respawn_candidates([
            (1, a, None),
            (2, b, "/home/u/.socis-agent/profiles/coder"),
        ])
        assert out == [a]
    ...
    def test_foreign_home_backend_is_not_replayed(self):
        """A backend from another SOCIS_AGENT_HOME is never respawned (#94030).
    ...
    def test_skips_sidecar_fixed_port_serve_on_foreign_home(self):
        ...
        argv = [
            "/Users/u/.socis-sidecar/socis-agent/venv/bin/python",
            "-m", "socis_cli.main",
    ...
    def test_matching_socis_agent_home_is_kept(self):
        from socis_cli.dashboard_procs import _filter_dashboard_respawn_candidates
    ...
    def test_symlinked_socis_agent_home_compares_equal(self, tmp_path):
        from socis_cli.dashboard_procs import _filter_dashboard_respawn_candidates
    ...
    def test_unknown_home_stays_eligible(self):
        """Unreadable SOCIS_AGENT_HOME (env probe failed) keeps pre-#94030 behaviour."""
        from socis_cli.dashboard_procs import _filter_dashboard_respawn_candidates
    ...
    def test_own_home_defaults_to_get_socis_agent_home(self, monkeypatch):
    ...
    def test_keeps_fixed_port_serve(self):
        ...
        argv = ["socis", "serve", "--host", "0.0.0.0", "--port", "9119"]
        assert _filter_dashboard_respawn_candidates([
            (9, argv, None),
        ]) == [argv]
...
class TestCmdlineCapture:
    ...
    def test_reads_proc_cmdline_when_available(self, tmp_path, monkeypatch):
        ...
        proc_file.write_bytes(b"/usr/bin/python3\x00-m\x00socis_cli.main\x00serve\x00")
        ...
        assert argv == ["/usr/bin/python3", "-m", "socis_cli.main", "serve"]
...
class TestPostUpdateStaleModuleReload:
    ...
    ``socis update`` runs in the PRE-pull Python process. When the update
    adds a new symbol to ``socis_cli._subprocess_compat`` (as #87134 added
    ``bounded_probe_run``), the post-update dashboard cleanup's lazy
    ``from socis_cli._subprocess_compat import bounded_probe_run`` hits the
    ...
    def test_node_failures_skip_reload_and_kill(self):
        ...
        with patch.object(update_cmd, "_reload_process_scan_modules") as mock_reload, \
             patch("socis_cli.main._kill_stale_dashboard_processes") as mock_kill:
    ...
    def test_reload_restores_missing_symbol(self):
        ...
        import socis_cli._subprocess_compat as compat
        from socis_cli import update_cmd
        ...
        try:
            ...
            stale = sys.modules["socis_cli._subprocess_compat"]
            assert hasattr(stale, "bounded_probe_run")
        finally:
            importlib.reload(sys.modules["socis_cli._subprocess_compat"])
            importlib.reload(sys.modules["socis_cli.dashboard_procs"])
    ...
    def test_config_reload_list_includes_process_scan_modules(self):
        ...
        assert "socis_cli._subprocess_compat" in reloaded
        assert "socis_cli.dashboard_procs" in reloaded
