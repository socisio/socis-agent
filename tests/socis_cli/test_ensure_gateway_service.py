"""Tests for socis_cli.gateway.ensure_gateway_service — the zero-prompt
service install/start path used by `socis setup` and `socis import`.
...
def _patch_host(monkeypatch, *, container=False, systemd=True, macos=False, windows=False):
    monkeypatch.setattr("socis_agent_constants.is_container", lambda: container)
