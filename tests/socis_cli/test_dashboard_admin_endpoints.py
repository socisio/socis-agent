...
def _client():
    ...
    # Keep the state DB under the isolated SOCIS_AGENT_HOME for any handler that
    # touches it.
...
class TestMcpEndpoints:
    ...
    def _setup(self, _isolate_socis_agent_home):
    ...
    def test_http_bearer_auth_separates_secret_from_config(
        self, _isolate_socis_agent_home
    ):
        from socis_agent_constants import get_socis_agent_home
...
class TestCredentialPoolEndpoints:
    ...
    def test_env_seeded_delete_stays_deleted(self):
        ...
        load_pool() re-seeds from ~/.socis-agent/.env on every call, so removing
        just the pool row silently reverts on the next dashboard refresh.
        The endpoint must mirror `socis auth remove`: clean up the backing
    ...
    def test_post_readd_lifts_suppression(self):
        ...
        Mirrors `socis auth add`, which clears every suppression for the
...
class TestMemoryEndpoints:
    ...
    def _setup(self, _isolate_socis_agent_home):
        ...
        (get_socis_agent_home() / "memories").mkdir(parents=True, exist_ok=True)
    ...
    def test_reset_targets(self):
        ...
        mem = get_socis_agent_home() / "memories"
...
class TestPairingEndpoints:
    ...
    def test_pairing_is_isolated_per_profile(self):
        ...
        (get_socis_agent_home() / "profiles" / "work").mkdir(parents=True, exist_ok=True)
...
class TestWebhookEndpoints:
    ...
    def test_enable_platform_starts_gateway_restart(self, monkeypatch):
        ...
        monkeypatch.setattr(ws, "_spawn_socis_action", fake_spawn_action)
...
class TestSystemStatsEndpoint:
    ...
    def test_stats_shape(self):
        ...
        for key in ("os", "arch", "hostname", "python_version", "socis_version"):
    ...
    def test_sources_lists_configured_hubs(self, monkeypatch):
        ...
        class _Src:
            ...
            def search(self, q, limit=10):
                return [_FakeMeta("socis-index/featured-skill", "trusted")]

        def _fake_router():
            srcs = [_Src("official"), _Src("github")]
            # socis-index source advertises availability + featured search.
            idx = _Src("socis-index")
        ...
        assert {"official", "github", "socis-index"} <= ids
...
class TestOfficialSkillsCatalogEndpoint:
    ...
    def test_lists_full_catalog_with_installed_flags(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "socis_cli.web_server._installed_hub_identifiers",
...
class TestSkillsHubPreviewEndpoint:
    ...
    def test_preview_returns_skill_md_text(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "socis_cli.skills_hub._resolve_source_meta_and_bundle",
...
class TestUpdateCheckEndpoint:
    """``GET /api/socis/update/check`` reports availability without applying.

    Powers the dashboard's check-before-you-update flow: the System page
    shows the commit-behind count and asks the user to confirm before
    ``POST /api/socis/update`` runs ``socis update``.
    """
    ...
    def test_git_install_reports_behind_count(self, monkeypatch):
        ...
        import socis_cli.banner as banner

        monkeypatch.setattr(banner, "check_for_updates", lambda: 5)

        r = self.client.get("/api/socis/update/check")
        assert r.status_code == 200
        body = r.json()
        assert {
            "install_method",
    ...
    def test_managed_runtime_dashboard_is_not_applyable(self, monkeypatch):
        ...
        body = self.client.get("/api/socis/update/check").json()
...
class TestDebugShareEndpoint:
    ...
    def _setup(self, _isolate_socis_agent_home):
        ...
        logs = get_socis_agent_home() / "logs"
    ...
    def test_redact_false_is_honored(self, monkeypatch):
        ...
        monkeypatch.setattr("socis_cli.dump.run_dump", lambda a: None)
...
class TestToolsConfigEndpoints:
    ...
    the dashboard surface that replicates the `socis tools` configurator."""
...
# ---------------------------------------------------------------------------
# _spawn_socis_action env scrubbing (#52470)
# ---------------------------------------------------------------------------

def test_spawn_socis_action_scrubs_gateway_loop_guard_env(monkeypatch, tmp_path):
    """The dashboard runs inside the gateway, so os.environ has
    _SOCIS_AGENT_GATEWAY=1. Spawned actions (e.g. `gateway restart`) must NOT inherit
    ...
    # Isolate the module-global proc registry: _spawn_socis_action stores
    ...
    ws._spawn_socis_action(["gateway", "restart"], "gateway-restart")

    assert "_SOCIS_AGENT_GATEWAY" not in captured["env"]
    assert captured["env"]["SOCIS_AGENT_NONINTERACTIVE"] == "1"
...
def test_desktop_lifespan_reaps_orphan_gateways_on_startup(
    monkeypatch, _isolate_socis_agent_home
):
    ...
    startup under SOCIS_AGENT_DESKTOP=1 so the stale orphan is cleared first.
    """
    import socis_cli.web_server as ws
    ...
    # web_server imports the reaper lazily from socis_cli.gateway, so patch it
    # on that module.
    import socis_cli.gateway as g
