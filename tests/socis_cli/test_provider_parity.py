...
the SAME provider universe as ``socis model`` (the CLI/TUI picker).
...
def test_every_socis_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys ∪ accounts) ⊇ `socis model` universe."""
    ...
    assert not missing, (
        "providers shown in `socis model` but not configurable in the desktop "
