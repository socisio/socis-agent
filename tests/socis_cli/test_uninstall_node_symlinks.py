"""Tests for socis_cli.uninstall.remove_node_symlinks.

Regression for #34536: the POSIX installer drops node/npm/npx symlinks in
~/.local/bin pointing into $SOCIS_AGENT_HOME/node and prepends ~/.local/bin to
PATH, shadowing an existing nvm. Uninstall must remove those symlinks, but
only when they still resolve into the SOCIS-managed node dir.
"""
...
def _make_socis_node(socis_agent_home: Path) -> Path:
    """Create a fake $SOCIS_AGENT_HOME/node/bin/{node,npm,npx} tree."""
    node_bin = socis_agent_home / "node" / "bin"
...
def test_leaves_unrelated_symlinks_untouched(fake_home):
    ...
    socis_agent_home = fake_home / ".socis-agent"
    _make_socis_node(socis_agent_home)
    ...
    removed = uninstall.remove_node_symlinks(socis_agent_home)
...
def test_removes_fhs_symlinks_in_usr_local_bin(fake_home, tmp_path, monkeypatch):
    ...
    """
    socis_agent_home = fake_home / ".socis-agent"
    node_bin = _make_socis_node(socis_agent_home)
