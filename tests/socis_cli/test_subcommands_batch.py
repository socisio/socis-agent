...
from socis_cli.subcommands.dashboard import build_dashboard_parser
...
def test_config_get_unset_subcommands_parse():
    """`socis config get/unset` parse key args (and --json for get)."""
...
# ── deprecated `socis login` fails gracefully, not with argparse error ────
#
# `socis login` is a removed command; its handler (`login_command` in
# `socis_cli/auth.py`) prints a deprecation notice pointing at `socis auth` /
# `socis model` and exits 0.  Two behavior contracts guard the UX:
...
def test_login_subparser_help_is_suppressed():
    """The deprecated `login` row must not appear in `socis --help`.
