...
- Bare `socis setup` drops straight into the full reconfigure wizard
  (every prompt shows the current value as its default).
- `socis setup --quick` runs the narrower "fill in missing items" flow.
- `socis setup --reconfigure` is a backwards-compat alias for the
  bare-setup default.
...
def _enter_existing_install_patches(stack, **extra):
    ...
    for target, kwargs in [
        ("socis_cli.setup.ensure_socis_agent_home", {}),
        ("socis_cli.setup.is_interactive_stdin", {"return_value": True}),
        ("socis_cli.config.is_managed", {"return_value": False}),
        ("socis_cli.setup.load_config", {"return_value": {}}),
        ("socis_cli.setup.save_config", {}),
        ("socis_cli.setup.get_env_value", {"return_value": None}),
        ("socis_cli.auth.get_active_provider", {"return_value": "openrouter"}),
        ("socis_cli.setup._print_setup_summary", {}),
        ("socis_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
...
def _enter_fresh_install_patches(stack, **extra):
    for target, kwargs in [
        ...
        ("socis_cli.setup.load_config", {"return_value": {}}),
        ("socis_cli.setup.save_config", {}),
        ("socis_cli.auth.get_active_provider", {"return_value": None}),
        ("socis_cli.setup.get_env_value", {"return_value": None}),
        ("socis_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
        stack.enter_context(patch(target, **kwargs))
...
class TestExistingInstallDefault:
    """Bare `socis setup` on an existing install = full reconfigure wizard."""
    ...
    def test_bare_setup_runs_full_reconfigure_without_menu(self, existing_install):
        ...
        with ExitStack() as stack:
            m = _enter_existing_install_patches(
                stack,
                prompt_choice="socis_cli.setup.prompt_choice",
                quick="socis_cli.setup._run_quick_setup",
                model="socis_cli.setup.setup_model_provider",
                terminal="socis_cli.setup.setup_terminal_backend",
                agent="socis_cli.setup.setup_agent_settings",
                gateway="socis_cli.setup.setup_gateway",
                tools="socis_cli.setup.setup_tools",
            )
...
class TestQuickFlag:
    ...
    def test_quick_flag_runs_quick_setup_only(self, existing_install):
        ...
        with ExitStack() as stack:
            ...
            from socis_cli.setup import run_setup_wizard
            from socis_cli import setup as setup_mod
...
class TestFreshInstall:
    ...
    def test_reconfigure_on_fresh_install_falls_through(self, fresh_install):
        ...
        with ExitStack() as stack:
            m = _enter_fresh_install_patches(
                stack,
                prompt=("socis_cli.setup.prompt_choice", {"return_value": 0}),
                first="socis_cli.setup._run_first_time_quick_setup",
            )
            from socis_cli.setup import run_setup_wizard
            from socis_cli import setup as setup_mod
    ...
    def test_blank_slate_runs_inside_navigation_step(self, fresh_install):
        ...
        with ExitStack() as stack:
            m = _enter_fresh_install_patches(
                stack,
                prompt=("socis_cli.setup.prompt_choice", {"return_value": 2}),
                blank="socis_cli.setup._run_blank_slate_setup",
            )
            from socis_cli import setup as setup_mod
...
class TestArgparse:
    ...
    def test_reconfigure_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        captured = {}
        monkeypatch.setattr(
            "socis_cli.setup.run_setup_wizard",
            lambda args: captured.setdefault("args", args),
        )
        monkeypatch.setattr(sys, "argv", ["socis", "setup", "--reconfigure"])
        try:
            main()
        except SystemExit:
            pass
