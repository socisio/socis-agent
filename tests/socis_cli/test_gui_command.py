"""Tests for ``socis gui`` desktop launcher wiring."""
...
def _isolate_xdg_data_home(tmp_path, monkeypatch):
    ...
    call live, and on a Linux dev box it wrote a ``socis.desktop`` pointing
...
def _stable_keychain_detection(monkeypatch):
    ...
    monkeypatch.delenv("SOCIS_AGENT_DESKTOP_PASSWORD_STORE", raising=False)
...
def _make_packaged_executable(root: Path, monkeypatch) -> Path:
    ...
    if sys.platform == "darwin":
        exe = desktop_dir / "release" / "mac-arm64" / "SOCIS.app" / "Contents" / "MacOS" / "SOCIS"
    elif sys.platform == "win32":
        exe = desktop_dir / "release" / "win-unpacked" / "SOCIS.exe"
    else:
        exe = desktop_dir / "release" / "linux-unpacked" / "socis"
...
def _packaged_exe_rel() -> Path:
    ...
    if sys.platform == "darwin":
        return Path("mac-arm64") / "SOCIS.app" / "Contents" / "MacOS" / "SOCIS"
    if sys.platform == "win32":
        return Path("win-unpacked") / "SOCIS.exe"
    return Path("linux-unpacked") / "socis"
...
def test_gui_installs_packages_and_launches_desktop_app(tmp_path, monkeypatch):
    ...
    with patch("socis_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("socis_cli.main._run_npm_install_deterministic", return_value=install_ok) as mock_install, \
         patch("socis_cli.main._desktop_build_needed", return_value=True), \
         patch("socis_cli.main._write_desktop_build_stamp"), \
         patch("socis_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("socis_cli.main._desktop_linux_sandbox_fixup", return_value=True), \
         patch("socis_cli.main._register_linux_desktop_entry"), \
         patch("socis_cli.main.subprocess.run", side_effect=_pack_into_staging(root)) as mock_run, \
...
def test_gui_install_env_prepends_managed_node_on_bare_path(tmp_path, monkeypatch):
    ...
    the parent PATH is stripped, so the install env MUST carry the SOCIS-managed
    Node ahead of that bare PATH or the install dies with ``node: not found``.
    """
    import os

    from socis_agent_constants import iter_socis_node_dirs

    root = _make_desktop_tree(tmp_path)
    monkeypatch.setattr(cli_main, "PROJECT_ROOT", root)
    _make_packaged_executable(root, monkeypatch)

    # A managed Node tree on disk so with_socis_node_path() actually prepends it.
    home = tmp_path / "socis-home"
    ...
    install_ok = subprocess.CompletedProcess(["npm", "ci"], 0)
    launch_ok = subprocess.CompletedProcess(["socis"], 0)
    ...
    with patch("socis_cli.main._resolve_node_runtime_npm", return_value="/usr/bin/npm"), \
         patch("socis_cli.main._run_npm_install_deterministic", return_value=install_ok) as mock_install, \
         patch("socis_cli.main._desktop_build_needed", return_value=True), \
         patch("socis_cli.main._write_desktop_build_stamp"), \
         patch("socis_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("socis_cli.main._desktop_linux_sandbox_fixup", return_value=True), \
         patch("socis_cli.main.subprocess.run", return_value=launch_ok), \
         pytest.raises(SystemExit):
        cli_main.cmd_gui(_ns(skip_build=False))

    managed_dirs = [str(p) for p in iter_socis_node_dirs() if p.is_dir()]
...
def test_gui_does_not_retry_after_packaged_executable_exists(tmp_path, monkeypatch, capsys):
    ...
    SOCIS.app/Contents/MacOS/SOCIS in place. Re-downloading Electron can't
    ...
    with patch("socis_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("socis_cli.main._run_npm_install_deterministic", return_value=install_ok), \
         patch("socis_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("socis_cli.main._purge_electron_build_cache", return_value=[Path("/c/electron.zip")]) as mock_purge, \
         patch("socis_cli.main._redownload_electron_dist", return_value=True) as mock_dl, \
         patch("socis_cli.main.subprocess.run", side_effect=pack_fail) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns())
...
def _make_signable_app(desktop_dir: Path) -> Path:
    """Build a fake packaged SOCIS.app with the pieces the signer must find."""
    ...
    app = desktop_dir / "release" / "mac-arm64" / "SOCIS.app"
    _write_info_plist(app, "com.nousresearch.hermes")
    (app / "Contents" / "MacOS").mkdir(parents=True)
    (app / "Contents" / "MacOS" / "SOCIS").write_text("", encoding="utf-8")

    helper = app / "Contents" / "Frameworks" / "SOCIS Helper.app"
...
def test_desktop_macos_local_codesign_signs_native_binaries(tmp_path, monkeypatch):
    ...
    SOCIS.app component, silently skipping every .node/.dylib/crashpad
...
def test_setup_tcc_identity_creates_cert_imports_trusts_and_configures(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr("socis_cli.config.set_config_value", lambda key, value: None)
    ...
    assert not list(tmp_path.glob("socis-tcc-*"))
...
def test_setup_tcc_identity_fails_when_trust_step_fails(tmp_path, monkeypatch, capsys):
    ...
    assert cli_main._desktop_macos_setup_tcc_identity("SOCIS Local Signing") is False
...
def test_setup_tcc_identity_skips_generation_when_already_valid(tmp_path, monkeypatch, capsys):
    ...
    def fake_run(cmd, **kwargs):
        calls.append(list(cmd))
        if cmd[:4] == ["/usr/bin/security", "find-identity", "-v", "-p"]:
            return _fake_proc(cmd, stdout='  1) ABCD "SOCIS Local Signing"\n     1 valid identities found')
        return _fake_proc(cmd)
    ...
    assert cli_main._desktop_macos_setup_tcc_identity("SOCIS Local Signing") is True
...
def test_cmd_gui_setup_tcc_identity_exits_before_build(tmp_path, monkeypatch):
    """`socis desktop --setup-tcc-identity` calls the setup and exits 0/1
    without building or launching the app."""
    root = _make_desktop_tree(tmp_path)
    monkeypatch.setattr(cli_main, "PROJECT_ROOT", root)
    _make_packaged_executable(root, monkeypatch)

    with patch("socis_cli.main._desktop_macos_setup_tcc_identity", return_value=True) as mock_setup, \
         patch("socis_cli.main._run_npm_install_deterministic") as mock_install, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns(setup_tcc_identity=True, identity="SOCIS Local Signing"))

    assert exc.value.code == 0
    mock_setup.assert_called_once_with("SOCIS Local Signing")
...
def test_gui_registers_linux_desktop_entry_before_launch(tmp_path, monkeypatch):
    """`socis desktop` gives the app a launcher presence on Linux."""
    root = _make_desktop_tree(tmp_path)
    monkeypatch.setattr(cli_main, "PROJECT_ROOT", root)
    packaged_exe = _make_packaged_executable(root, monkeypatch)

    registered: list[Path] = []
    monkeypatch.setattr("socis_cli.linux_desktop_entry.is_supported", lambda: True)
    monkeypatch.setattr(
        "socis_cli.linux_desktop_entry.install_desktop_entry",
        lambda project_root: registered.append(project_root) or (tmp_path / "socis.desktop"),
    )

    launch_ok = subprocess.CompletedProcess([str(packaged_exe)], 0)

    with patch("socis_cli.main._desktop_build_needed", return_value=False), \
         patch("socis_cli.main._resolve_node_runtime_npm", return_value="/usr/bin/npm"), \
         patch("socis_cli.main._desktop_linux_sandbox_fixup", return_value=True), \
         patch("socis_cli.main.subprocess.run", return_value=launch_ok), \
         pytest.raises(SystemExit):
        cli_main.cmd_gui(_ns())
...
def test_gui_launches_even_when_desktop_entry_install_fails(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("socis_cli.linux_desktop_entry.is_supported", lambda: True)
    monkeypatch.setattr("socis_cli.linux_desktop_entry.install_desktop_entry", boom)
    ...
    with patch("socis_cli.main._desktop_build_needed", return_value=False), \
         patch("socis_cli.main._resolve_node_runtime_npm", return_value="/usr/bin/npm"), \
         patch("socis_cli.main._desktop_linux_sandbox_fixup", return_value=True), \
         patch("socis_cli.main.subprocess.run", return_value=launch_ok) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns())
...
def test_gui_skips_desktop_entry_off_linux(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("socis_cli.linux_desktop_entry.is_supported", lambda: False)

    def fail(_project_root):
        raise AssertionError("must not install a desktop entry off Linux")

    monkeypatch.setattr("socis_cli.linux_desktop_entry.install_desktop_entry", fail)
...
def test_desktop_launch_options_normalizes_password_store(raw, expected):
    ...
    with patch("socis_cli.config.load_config", return_value=cfg):
...
def test_desktop_launch_options_ozone_hint_defaults_auto():
    with patch("socis_cli.config.load_config", return_value={}):
...
def test_gui_bridges_ozone_hint_to_launch_env(tmp_path, monkeypatch):
    ...
    with patch("socis_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("socis_cli.main._run_npm_install_deterministic", return_value=ok), \
         patch("socis_cli.main._desktop_build_needed", return_value=True), \
         patch("socis_cli.main._write_desktop_build_stamp"), \
         patch("socis_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("socis_cli.main._desktop_linux_sandbox_fixup", return_value=True), \
         patch("socis_cli.config.load_config", return_value=cfg), \
         patch("socis_cli.linux_desktop_entry.install_desktop_entry", return_value=None), \
         patch("socis_cli.main.subprocess.run", side_effect=_pack_into_staging(root)) as mock_run, \
         pytest.raises(SystemExit):
        cli_main.cmd_gui(_ns())
    ...
    with patch("socis_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         ...
         patch("socis_cli.config.load_config", return_value=cfg), \
         patch("socis_cli.linux_desktop_entry.install_desktop_entry", return_value=None), \
         patch("socis_cli.main.subprocess.run", side_effect=_pack_into_staging(root)) as mock_run2, \
         pytest.raises(SystemExit):
        cli_main.cmd_gui(_ns())
...
def _clear_keychain_env(monkeypatch):
    for var in (
        ...
        "SOCIS_AGENT_DESKTOP_PASSWORD_STORE",
    ):
...
def test_detect_linux_password_store_via_dbus_secret_service(monkeypatch):
    ...
    with patch("socis_cli.main.subprocess.run", return_value=ping_ok) as mock_run:
        assert cli_main._detect_linux_password_store() == "gnome-libsecret"
...
def test_detect_linux_password_store_none_when_no_keychain(monkeypatch):
    ...
    with patch("socis_cli.main.subprocess.run", return_value=ping_fail):
        assert cli_main._detect_linux_password_store() is None
    with patch("socis_cli.main.subprocess.run", side_effect=FileNotFoundError):
        assert cli_main._detect_linux_password_store() is None
...
def test_gui_linux_packaged_launch_bridges_detected_password_store(tmp_path, monkeypatch):
    ...
    with patch("socis_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         ...
         patch("socis_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("socis_cli.main._desktop_linux_sandbox_fixup", return_value=True), \
         patch("socis_cli.config.load_config", return_value={}), \
         patch("socis_cli.linux_desktop_entry.install_desktop_entry", return_value=None), \
         patch("socis_cli.main._detect_linux_password_store", return_value="gnome-libsecret"), \
         patch("socis_cli.main.subprocess.run", side_effect=_pack_into_staging(root)) as mock_run, \
         pytest.raises(SystemExit):
        cli_main.cmd_gui(_ns())

    launch_env = mock_run.call_args_list[1].kwargs["env"]
    assert launch_env["SOCIS_AGENT_DESKTOP_PASSWORD_STORE"] == "gnome-libsecret"
...
def test_gui_linux_source_launch_bridges_detected_password_store(tmp_path, monkeypatch):
    ...
    with patch("socis_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         ...
         patch("socis_cli.config.load_config", return_value={}), \
         patch("socis_cli.linux_desktop_entry.install_desktop_entry", return_value=None), \
         patch("socis_cli.main._detect_linux_password_store", return_value="kwallet6"), \
         patch("socis_cli.main.subprocess.run", side_effect=_pack_into_staging(root)) as mock_run, \
         pytest.raises(SystemExit):
    ...
    launch_env = mock_run.call_args_list[1].kwargs["env"]
    assert launch_env["SOCIS_AGENT_DESKTOP_PASSWORD_STORE"] == "kwallet6"
...
def test_gui_config_password_store_skips_detection(tmp_path, monkeypatch):
    ...
    with patch("socis_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         ...
         patch("socis_cli.config.load_config", return_value=cfg), \
         patch("socis_cli.linux_desktop_entry.install_desktop_entry", return_value=None), \
         patch("socis_cli.main._detect_linux_password_store") as mock_detect, \
         patch("socis_cli.main.subprocess.run", side_effect=_pack_into_staging(root)) as mock_run, \
         pytest.raises(SystemExit):
        cli_main.cmd_gui(_ns())
...
def test_gui_explicit_password_store_env_wins_over_config_and_detection(tmp_path, monkeypatch):
    _clear_keychain_env(monkeypatch)
    monkeypatch.setenv("SOCIS_AGENT_DESKTOP_PASSWORD_STORE", "basic")
    root = _make_desktop_tree(tmp_path)
    monkeypatch.setattr(cli_main, "PROJECT_ROOT", root)
    _make_packaged_executable(root, monkeypatch)
    ...
    mock_detect.assert_not_called()
    launch_env = mock_run.call_args_list[1].kwargs["env"]
    assert launch_env["SOCIS_AGENT_DESKTOP_PASSWORD_STORE"] == "basic"
...
def test_gui_password_store_bridge_is_linux_only(tmp_path, monkeypatch):
    ...
    mock_detect.assert_not_called()
    launch_env = mock_run.call_args_list[1].kwargs["env"]
    assert "SOCIS_AGENT_DESKTOP_PASSWORD_STORE" not in launch_env
...
def _gui_build_patches(root: Path, run_side_effect):
    return [
        patch("socis_cli.main.shutil.which", return_value="/usr/bin/npm"),
        patch("socis_cli.main._run_npm_install_deterministic",
              return_value=subprocess.CompletedProcess(["npm", "ci"], 0)),
        patch("socis_cli.main._desktop_build_needed", return_value=True),
        patch("socis_cli.main._write_desktop_build_stamp"),
        patch("socis_cli.main._desktop_macos_relaunchable_fixup"),
        patch("socis_cli.main._register_linux_desktop_entry"),
        patch("socis_cli.main._stop_desktop_processes_locking_build", return_value=[]),
        patch("socis_cli.main._purge_electron_build_cache", return_value=[]),
        patch("socis_cli.main._redownload_electron_dist", return_value=False),
        patch("socis_cli.main.subprocess.run", side_effect=run_side_effect),
    ]
