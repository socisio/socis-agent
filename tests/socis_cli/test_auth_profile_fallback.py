...
When ``SOCIS_AGENT_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    ...
    * Profile     -> tmp_path/.socis-agent/profiles/coder   (active, SOCIS_AGENT_HOME points here)
    ...
    global_root = tmp_path / ".socis-agent"
    global_root.mkdir()
...
# ``_load_provider_state`` itself, those helpers raise ``"SOCIS is not
...
def classic_env(tmp_path, monkeypatch):
    """Classic single-root layout (SOCIS_AGENT_HOME != ~/.socis-agent, no profiles)."""
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    socis_agent_home = tmp_path / "classic"
