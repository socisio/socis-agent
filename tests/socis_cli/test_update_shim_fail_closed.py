...
The bug class: on Windows, when `socis.exe`/sibling shims could not be
...
import socis_cli._install_repair as ir
import socis_cli.update_cmd as update_cmd


def _make_shims(scripts_dir: Path, names=("socis", "socis-gateway")) -> list[Path]:
...
def test_strict_quarantine_refuses_before_install(windows, tmp_path, monkeypatch):
    ...
    # socis.exe cannot be renamed; socis-gateway.exe can
    real_rename = Path.rename

    def deny_socis(self, target):
        if self.name == "socis.exe":
            raise PermissionError(13, "held open")
        return real_rename(self, target)

    monkeypatch.setattr(Path, "rename", deny_socis)
    monkeypatch.setattr(cli_main, "_socis_exe_shims", lambda d: shims)
    ...
    assert install_ran == []
    assert "socis.exe" in exc_info.value.failed_shims
    # The successful rename (socis-gateway.exe) was rolled back.
    assert (scripts / "socis-gateway.exe").exists()
    assert not list(scripts.glob("socis-gateway.exe.old.*"))


def test_non_strict_keeps_warn_and_try(windows, tmp_path, monkeypatch):
    scripts = tmp_path / "venv" / "Scripts"
    shims = _make_shims(scripts, names=("socis",))
    monkeypatch.setattr(
        Path, "rename",
...
def test_recovery_install_cmd_fail_closed(windows, tmp_path, monkeypatch):
    ...
    _make_shims(scripts, names=("socis",))
