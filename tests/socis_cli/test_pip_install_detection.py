...
def test_code_scoped_stamp_wins_over_home_stamp(tmp_path):
    """The stamp next to the running code is authoritative over $SOCIS_AGENT_HOME.

    Models a host git install whose $SOCIS_AGENT_HOME is shared with (and stamped
    'docker' by) a co-located container. The code-scoped stamp must win so the
    host install is correctly identified as 'git' and 'socis update' works.
    """
    code = tmp_path / "code"
    home = tmp_path / "home"
    code.mkdir()
    ...
    with patch("socis_cli.config.get_managed_system", return_value=None), \
         patch("socis_cli.config.get_socis_agent_home", return_value=home):
...
def test_stamp_install_method_writes_code_scoped(tmp_path):
    """stamp_install_method writes next to the code, not into $SOCIS_AGENT_HOME."""
    code = tmp_path / "code"
    home = tmp_path / "home"
    code.mkdir()
    home.mkdir()
    with patch("socis_cli.config.get_socis_agent_home", return_value=home):
        from socis_cli.config import stamp_install_method
...
def test_container_without_stamp_is_not_docker(tmp_path):
    ...
    image, so ``socis update`` refused to run. With a ``.git`` checkout it
    must resolve to ``git``.
    """
    (tmp_path / ".git").mkdir()
    with patch("socis_cli.config.get_managed_system", return_value=None), \
         patch("socis_cli.config.get_socis_agent_home", return_value=tmp_path), \
         patch("socis_agent_constants.is_container", return_value=True):
        from socis_cli.config import detect_install_method
