...
#88251: ``socis update`` treated a failed desktop pack as non-fatal, printed
...
def desktop_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "socis_agent_constants.with_socis_node_path", lambda: {}, raising=False
    )
    monkeypatch.setattr(
        "socis_agent_constants.display_socis_agent_home", lambda: str(tmp_path), raising=False
    )
...
def test_gateway_exit_code_file_tracks_desktop_rebuild(tmp_path, monkeypatch):
    monkeypatch.setattr(update_cmd, "get_socis_agent_home", lambda: tmp_path)
