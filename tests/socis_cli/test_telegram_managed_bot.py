"""Tests for socis_cli.telegram_managed_bot — QR codes, deep links, pairing."""
...
class TestGenerateBotUsername:
    def test_secure_default_format(self):
        ...
        assert name.startswith("socis_")
        assert name.endswith("_bot")
        assert len(name) == len("socis_") + 16 + len("_bot")
        assert len(name) <= 32
...
class TestGenerateDeepLink:
    ...
    def test_name_url_encoded(self):
        link = generate_deep_link(
            ...
            suggested_name="SOCIS & Friends",
        )
        assert "SOCIS+%26+Friends" in link
...
class TestCreatePairing:
    def test_success(self):
        ...
        with patch(
            "socis_cli.telegram_managed_bot.httpx.post", return_value=mock_resp
        ) as post:
            pairing = create_pairing("https://api.example.com", bot_name="SOCIS Agent")
...
class TestPollForToken:
    ...
    def test_immediate_success(self):
        ...
        with patch(
            "socis_cli.telegram_managed_bot.httpx.get", return_value=mock_resp
        ) as get:
            with patch("socis_cli.telegram_managed_bot.time.sleep"):
    ...
    def test_eventual_success(self):
        ...
        with patch("socis_cli.telegram_managed_bot.httpx.get", side_effect=fake_get):
            with patch("socis_cli.telegram_managed_bot.time.sleep"):
                token = poll_for_token(
...
class TestSetupTelegramAuto:
    def test_setup_helper_exists(self):
        from socis_cli.setup import _setup_telegram_auto

        assert callable(_setup_telegram_auto)

    def test_setup_result_passes_profile_name_for_profile_home(self, monkeypatch, tmp_path):
        from socis_cli import setup

        seen = {}
        profile_home = tmp_path / ".socis-agent" / "profiles" / "oracle"
        profile_home.mkdir(parents=True)

        monkeypatch.setattr(setup, "get_socis_agent_home", lambda: profile_home)
        ...
        monkeypatch.setattr(
            "socis_cli.telegram_managed_bot.auto_setup_telegram_bot_result",
    ...
    def test_profile_name_from_home_path_handles_windows_separators(self):
        from socis_cli.setup import _profile_name_from_socis_agent_home

        assert (
            _profile_name_from_socis_agent_home(
                PureWindowsPath(r"C:\Users\test\AppData\Local\socis\profiles\oracle")
            )
            == "oracle"
        )
