"""
Tests for --yes / --force flag separation in `socis skills install`.
...
def test_cli_skills_install_yes_sets_skip_confirm(monkeypatch):
    ...
    monkeypatch.setattr("socis_cli.skills_hub.skills_command", fake_skills_command)
    monkeypatch.setattr(
        sys,
        "argv",
        ["socis", "skills", "install", "official/email/agentmail", "--yes"],
    )
...
def test_cli_skills_install_no_flags(monkeypatch):
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["socis", "skills", "install", "test/skill"],
    )
