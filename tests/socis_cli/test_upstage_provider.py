...
Regression guard for the bug where `socis model` saved `provider: upstage`
correctly but, on re-entry, showed a different provider as active. Root cause:
`socis_cli/providers.py` (the resolver behind `resolve_provider_full`) had no
...
class TestUpstageOverlay:
    def test_overlay_exists(self):
        ...
        assert "upstage" in SOCIS_AGENT_OVERLAYS
        overlay = SOCIS_AGENT_OVERLAYS["upstage"]
...
class TestUpstageConfigProviderWins:
    ...
    Mirrors the display logic in `socis_cli/main.py` (cmd_model): read
