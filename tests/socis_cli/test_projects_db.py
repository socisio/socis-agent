"""Tests for the per-profile Projects store (socis_cli/projects_db)."""
...
def test_create_get_list(conn):
    pid = pdb.create_project(conn, name="SOCIS Agent", folders=["/tmp/socis"])
    proj = pdb.get_project(conn, pid)

    assert proj is not None
    assert proj.slug == "socis-agent"
    assert proj.name == "SOCIS Agent"
    # First folder becomes primary.
    assert proj.primary_path == "/tmp/socis"
    assert [f.path for f in proj.folders] == ["/tmp/socis"]
    assert proj.folders[0].is_primary is True

    # Lookup by slug too.
    assert pdb.get_project(conn, "socis-agent").id == pid
...
def test_per_profile_isolation(tmp_path):
    # Two distinct DB paths stand in for two profiles' SOCIS_AGENT_HOME.
