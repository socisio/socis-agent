...
def test_update_notice_offers_v1_trigram_tool_calls_rebuild(tmp_path, monkeypatch, capsys):
    ...
    class FakeSessionDB:
        ...
        _db_needs_fts_storage_upgrade = staticmethod(
            socis_agent_state.SessionDB._db_needs_fts_storage_upgrade
        )

    monkeypatch.setattr(socis_agent_constants, "get_socis_agent_home", lambda: tmp_path)
    monkeypatch.setattr(socis_agent_state, "SessionDB", FakeSessionDB)
    ...
    assert "socis sessions optimize-storage" in capsys.readouterr().out
    conn.close()
