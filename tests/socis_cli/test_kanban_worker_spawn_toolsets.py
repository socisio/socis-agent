...
def test_default_spawn_pins_assignee_profile_cli_toolsets(monkeypatch, tmp_path):
    ...
    SOCIS_AGENT_KANBAN_TASK: the worker must not collapse to only kanban lifecycle
...
""".lstrip(),
    ...
    assert pid == 4242
    assert captured["env"]["SOCIS_AGENT_HOME"] == str(profile)
    assert captured["env"]["SOCIS_AGENT_KANBAN_TASK"] == "t_spawn_tools"
