"""Tests for ``socis doctor --live`` — opt-in bounded real-call tool-backend probes.
...
from socis_cli import doctor_live
from socis_cli.doctor_live import (
    ProbeResult,
...
class TestBrowserAvailableNpxRung:
    ...
    with. It must fall through to the same cascade `socis doctor` uses."""

    def _block_path_and_node_modules_checks(self, monkeypatch, tmp_path):
        monkeypatch.setattr("shutil.which", lambda *a, **k: None)
        monkeypatch.setattr("socis_cli.doctor.SOCIS_AGENT_HOME", tmp_path / "home")
        monkeypatch.setattr("socis_cli.doctor.PROJECT_ROOT", tmp_path / "root")
