...
def _stage(home_root, name):
    # Machine-scoped models dir (the shared root — tmp SOCIS_AGENT_HOME IS the root here).
...
def test_not_ours_returns_none(socis_agent_home):
    from socis_cli.local_runtime.capabilities import managed_model_supports_vision

    assert managed_model_supports_vision("gpt-4o") is None


def test_catalog_vision_model_with_projector_on_disk(socis_agent_home):
    ...
    _stage(socis_agent_home, variant.model_id)
    adir = assets_dir()
...
def test_catalog_vision_model_missing_projector_is_blind(socis_agent_home):
...
def test_live_props_beats_catalog(socis_agent_home, monkeypatch):
    """A running child's modalities report wins over the catalog: the
    server that will receive the image is the authority."""
    import socis_cli.local_runtime.capabilities as caps
...
def test_lookup_chain_consults_managed_runtime(socis_agent_home, monkeypatch):
    ...
    monkeypatch.setattr(
        "socis_cli.local_runtime.capabilities.managed_model_supports_vision",
        lambda mid: True)
...
def test_webp_transcodes_to_png_for_managed_provider(socis_agent_home, monkeypatch, tmp_path):
...
def test_webp_passes_through_for_cloud_providers(socis_agent_home, monkeypatch, tmp_path):
...
def test_vision_analyze_normalization_narrows_for_managed(socis_agent_home, monkeypatch, tmp_path):
