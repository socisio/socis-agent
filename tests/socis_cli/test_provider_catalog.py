"""Tests for the unified provider catalog (socis_cli.provider_catalog).

These are invariant tests, not snapshots: they assert the parity *contract*
between what ``socis model`` shows (``CANONICAL_PROVIDERS``) and what the
...
from socis_cli.provider_catalog import (
    ProviderDescriptor,
    provider_catalog,
