"""Tests for the top-level `./socis` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./socis` should use `socis_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "socis"
    called = []

    fake_main_module = types.ModuleType("socis_cli.main")

    def fake_main():
        called.append("socis_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "socis_cli.main", fake_main_module)
