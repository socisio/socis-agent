...
def test_installed_tags_newest_first(socis_agent_home):
    from socis_cli.local_runtime.binaries import installed_tags

    assert installed_tags() == []
    _install_fake_tag(socis_agent_home, "b10290")
    _install_fake_tag(socis_agent_home, "b10412")
    assert installed_tags() == ["b10412", "b10290"]


def test_default_tag_flows_from_default_config(socis_agent_home):
    """Unpinned users inherit the SOCIS-release default (deep-merge);
...
def test_update_available_requires_enabled_and_installed(socis_agent_home, monkeypatch):
    ...
    import socis_cli.web_routers.local_models as lm
...
def test_boot_never_downloads_missing_tag(socis_agent_home, monkeypatch):
...
def test_prune_keeps_n_minus_one(socis_agent_home):
    from socis_cli.local_runtime.binaries import installed_tags, prune_old_tags

    for tag in ("b10100", "b10200", "b10290"):
        _install_fake_tag(socis_agent_home, tag)
    prune_old_tags(["b10290", "b10200"])
    assert installed_tags() == ["b10290", "b10200"]
    # downloads/ cache dir must survive pruning when present.
    downloads = socis_agent_home / "runtimes" / "llamacpp" / "downloads"
