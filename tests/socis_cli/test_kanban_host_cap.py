...
1. The standalone daemon path (``socis kanban daemon --force`` /
   :func:`socis_cli.kanban_db.run_daemon`) never resolved
...
def test_run_daemon_resolves_and_passes_max_in_progress(
    ...
    ``socis kanban dispatch`` paths both resolved the memory-derived
    default.
    """
    captured: dict = {}
...
def test_configured_max_in_progress_parsing(monkeypatch):
    import socis_cli.config as cfgmod
...
def test_nonspawnable_review_does_not_tax_ready_budget(
    ...
    import socis_cli.config as cfgmod
    import socis_cli.profiles as profmod
