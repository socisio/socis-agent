"""Tests for kanban board export / import (``socis_cli.kanban_transfer``).
...
from socis_cli import kanban_transfer as kt
from socis_cli.archive_safe import normalize_archive_parts, safe_extract_targz
...
def kanban_root(tmp_path, monkeypatch):
    ...
    def _use(name: str) -> Path:
        ...
        monkeypatch.setenv("SOCIS_AGENT_KANBAN_HOME", str(root))
        for var in ("SOCIS_AGENT_KANBAN_DB", "SOCIS_AGENT_KANBAN_WORKSPACES_ROOT",
                    "SOCIS_AGENT_KANBAN_ATTACHMENTS_ROOT", "SOCIS_AGENT_KANBAN_BOARD"):
...
def test_import_rejects_a_future_format_version(kanban_root, tmp_path):
    ...
    kanban_root("target")
    with pytest.raises(ValueError, match="newer than this SOCIS"):
