...
Covers:
 - Normal path (tokens already in SOCIS auth store)
 - Claude Code fallback (tokens only in ~/.claude/.credentials.json)
 - Negative case (no credentials anywhere)

Note: auto-import from ~/.codex/auth.json was removed in #12360 — SOCIS
now owns its own openai-codex auth state, and users explicitly adopt
existing Codex CLI tokens via `socis auth openai-codex`. The old
...
@pytest.fixture()
def socis_auth_only_env(tmp_path, monkeypatch):
    """Tokens already in SOCIS auth store (no Codex CLI needed)."""
...
def test_normal_path_still_works(socis_auth_only_env):
    """openai-codex appears when tokens are already in SOCIS auth store."""
...
def claude_code_only_env(tmp_path, monkeypatch):
    ...
    ~/.claude/.credentials.json (Claude Code) — not in env vars or SOCIS
    auth store."""
    socis_agent_home = tmp_path / ".socis-agent"
    socis_agent_home.mkdir()
