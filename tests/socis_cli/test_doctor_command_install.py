"""Tests for the Command Installation check in socis doctor."""
...
import socis_cli.doctor as doctor_mod


def _setup_doctor_env(monkeypatch, tmp_path, venv_name="venv"):
    """Create a minimal SOCIS_AGENT_HOME + PROJECT_ROOT for doctor tests."""
    ...
    socis_bin = venv_bin_dir / "socis"
    socis_bin.write_text("#!/usr/bin/env python\n# entry point\n")
...
class TestDoctorCommandInstallation:
    ...
    def test_fix_repairs_wrong_symlink(self, monkeypatch, tmp_path):
        home, project, socis_bin = _setup_doctor_env(monkeypatch, tmp_path)

        # Create a symlink pointing to wrong target
        cmd_link_dir = tmp_path / ".local" / "bin"
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "socis"
        wrong_target = tmp_path / "wrong_socis"
        ...
        assert cmd_link.resolve() == socis_bin.resolve()
