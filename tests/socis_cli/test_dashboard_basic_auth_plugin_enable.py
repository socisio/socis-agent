...
from socis_cli.dashboard_auth import clear_providers, list_providers
from socis_cli.plugins import PluginManager, discover_plugins
...
class TestBasicProviderLoadsAfterUnblock:
    def test_disabled_basic_blocks_registration(self, socis_agent_home, monkeypatch):
    ...
    def test_unblock_then_rediscover_registers_provider(
        self, socis_agent_home, monkeypatch,
    ):
        password_hash = basic_plugin.hash_password("hunter2")
        cfg = {
            "dashboard": {
        ...
        _write_config(socis_agent_home, cfg)
