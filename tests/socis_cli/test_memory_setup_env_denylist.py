...
``socis_cli.memory_setup._write_env_vars`` persists provider plugin
credentials to ``~/.socis-agent/.env``. It previously called ``Path.write_text``
...
other subprocess-influencing or SOCIS-runtime-location name) could
otherwise plant a value into ``.env`` via the interactive memory-setup
wizard. The next SOCIS process would load it through the
...
from socis_cli.config import ensure_socis_agent_home, get_env_path, load_env
from socis_cli.memory_setup import _write_env_vars


def _env_file_keys() -> set[str]:
    """Parse ``~/.socis-agent/.env`` directly and return the set of keys present.
...
def _socis_agent_home(tmp_path, monkeypatch):
...
def test_denylisted_key_does_not_block_other_writes(capsys):
    ...
    _write_env_vars({
        ...
        "SOCIS_AGENT_LANGFUSE_PUBLIC_KEY": "pk-test-123",
        "OPENROUTER_API_KEY": "sk-or-test-456",
    })

    assert "LD_PRELOAD" not in _env_file_keys()
    env = load_env()
    assert env["SOCIS_AGENT_LANGFUSE_PUBLIC_KEY"] == "pk-test-123"
    assert env["OPENROUTER_API_KEY"] == "sk-or-test-456"


def test_legitimate_socis_integration_key_still_writable():
    """``SOCIS_AGENT_*`` overall is NOT blocked — only the four runtime
    location names (HOME/PROFILE/CONFIG/ENV). Integration credentials
    following the ``SOCIS_AGENT_*`` convention (SOCIS_AGENT_LANGFUSE_*,
    SOCIS_AGENT_SPOTIFY_*, SOCIS_AGENT_QWEN_BASE_URL, ...) must keep working or
    the memory-setup wizard regresses for every plugin that follows
    the convention."""
    _write_env_vars({
        "SOCIS_AGENT_LANGFUSE_PUBLIC_KEY": "pk-lf-789",
        "SOCIS_AGENT_QWEN_BASE_URL": "https://example.com/v1",
    })

    env = load_env()
    assert env["SOCIS_AGENT_LANGFUSE_PUBLIC_KEY"] == "pk-lf-789"
    assert env["SOCIS_AGENT_QWEN_BASE_URL"] == "https://example.com/v1"
...
def test_explicit_socis_agent_home_writes_to_that_env_file(tmp_path):
    """Plugin ``post_setup`` hooks (e.g. Supermemory) pass an explicit
    SOCIS home; keep that target while still routing through
    ``save_env_value`` validation instead of writing directly."""
    home = tmp_path / "plugin-home"

    _write_env_vars({"MEM0_API_KEY": "m0-test-key-abc"}, socis_agent_home=home)
