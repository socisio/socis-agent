...
A poisoned/contaminated ``gateway.pid`` inside one profile's SOCIS_AGENT_HOME can
truthfully name ANOTHER profile's live gateway (its ``socis_agent_home`` stamp
...
def _pid_record(proc: subprocess.Popen, script: Path, owner_home: Path) -> dict:
    ...
    return {
        ...
        "socis_agent_home": str(owner_home),
    }


class TestRecordedGatewayHomeConflicts:
    def test_conflicting_home_detected(self, tmp_path, monkeypatch):
        monkeypatch.setenv("SOCIS_AGENT_HOME", str(tmp_path / "profiles" / "tim"))
        record = {"pid": 1, "socis_agent_home": str(tmp_path)}
    ...
    def test_same_home_accepted(self, tmp_path, monkeypatch):
        ...
        record = {"pid": 1, "socis_agent_home": str(home)}
    ...
    def test_legacy_record_without_home_proves_nothing(self, tmp_path, monkeypatch):
        ...
        assert recorded_gateway_home_conflicts({"pid": 1, "socis_agent_home": "  "}) is False
...
class TestCrossProfileStopRefusal:
    def test_stop_profile_gateway_refuses_other_profiles_pid(
        ...
        monkeypatch.setenv("SOCIS_AGENT_HOME", str(tim_home))

        proc = _spawn_gateway_lookalike(
            tmp_path / "bin", tim_home / "gateway.lock"
        )
        try:
            record = _pid_record(proc, tmp_path / "bin" / "socis", root_home)
            (tim_home / "gateway.pid").write_text(json.dumps(record))

            from socis_cli import gateway as gateway_cli
    ...
    def test_stop_profile_gateway_still_stops_own_gateway(
        ...
        try:
            record = _pid_record(proc, tmp_path / "bin" / "socis", tim_home)
            (tim_home / "gateway.pid").write_text(json.dumps(record))
...
class TestProfileDeleteStopRefusal:
    def test_stop_gateway_process_refuses_other_profiles_pid(
        ...
        try:
            ...
            from socis_cli.profiles import _stop_gateway_process
