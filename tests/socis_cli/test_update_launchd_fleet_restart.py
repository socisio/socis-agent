"""Regression for #41403 — ``socis update`` must restart ALL macOS launchd gateways.
...
Sibling ``ai.socis.gateway-<profile>`` services kept running pre-update
...
class TestLaunchdGatewayLabelsForInstall:
    def test_labels_derive_from_this_installs_profiles(self, monkeypatch):
        ...
        the shared per-user LaunchAgents dir. A sandboxed SOCIS_AGENT_HOME (tests,
        ...
        assert launchd_gateway_labels_for_install() == [
            "ai.socis.gateway",
            "ai.socis.gateway-merit-ops",
            "ai.socis.gateway-tfl-wiki",
        ]

    def test_no_profiles_means_no_fleet(self, monkeypatch):
        monkeypatch.setattr(socis_cli.profiles, "list_profiles", lambda: [])
        assert launchd_gateway_labels_for_install() == []
...
class TestLocateLaunchdGatewayService:
    def test_domains_resolve_per_label_not_from_cache(self, monkeypatch):
        ...
        assert _locate_launchd_gateway_service("ai.socis.gateway-a") == (
            f"gui/{UID}",
            4242,
        )
        assert _locate_launchd_gateway_service("ai.socis.gateway-b") == (
            f"user/{UID}",
            4242,
        )
    ...
    def test_loaded_without_live_process(self, monkeypatch):
        ...
        assert _locate_launchd_gateway_service("ai.socis.gateway-x") == (
            f"gui/{UID}",
            None,
        )
    ...
    def test_not_loaded_in_either_domain(self, monkeypatch):
        ...
        assert _locate_launchd_gateway_service("ai.socis.gateway-x") == (None, None)
    ...
    def test_timeout_propagates_to_caller(self, monkeypatch):
        ...
        with pytest.raises(subprocess.TimeoutExpired):
            _locate_launchd_gateway_service("ai.socis.gateway-x")
...
class TestProbeLaunchdDomainForLabel:
    def test_unloaded_label_falls_back_to_managername(self, monkeypatch):
        ...
        monkeypatch.setattr(gw.subprocess, "run", fake_run)
        assert _probe_launchd_domain_for_label("ai.socis.gateway-x") == f"gui/{UID}"
    ...
    def test_unloaded_label_defaults_to_user_domain(self, monkeypatch):
        ...
        monkeypatch.setattr(gw.subprocess, "run", fake_run)
        assert _probe_launchd_domain_for_label("ai.socis.gateway-x") == f"user/{UID}"
...
class TestGetServicePidsScoping:
    def _wire(self, monkeypatch):
        ...
        monkeypatch.setattr(gw, "get_launchd_label", lambda: "ai.socis.gateway")
        monkeypatch.setattr(
            gw,
            "launchd_gateway_labels_for_install",
            lambda: ["ai.socis.gateway", "ai.socis.gateway-a", "ai.socis.gateway-b"],
        )
        located = {
            "ai.socis.gateway": (f"gui/{UID}", 100),
            "ai.socis.gateway-a": (f"gui/{UID}", 200),
            "ai.socis.gateway-b": (None, None),  # not bootstrapped
        }
        monkeypatch.setattr(
...
def _fleet(monkeypatch, tmp_path, *, current, labels, located,
    ...
    """Wire a fake launchd fleet through socis_cli.gateway seams.
...
class TestRestartMacosLaunchdGateways:
    def test_current_delegates_and_siblings_kickstart_in_own_domains(
        ...
        current = "ai.socis.gateway-merit-ops"
        rec = _fleet(
            monkeypatch,
            tmp_path,
            current=current,
            labels=["ai.socis.gateway", current, "ai.socis.gateway-user-scoped"],
            located={
                "ai.socis.gateway": (f"gui/{UID}", 100),
                current: (f"gui/{UID}", 200),
                "ai.socis.gateway-user-scoped": (f"user/{UID}", 300),
            },
        )
        restarted: list[str] = []
        failed: list[str] = []
        ...
        assert rec.kickstarts == [
            f"gui/{UID}/ai.socis.gateway",
            f"user/{UID}/ai.socis.gateway-user-scoped",
        ]
        assert rec.waits == [
            f"gui/{UID}/ai.socis.gateway",
            f"user/{UID}/ai.socis.gateway-user-scoped",
    ...
    def test_current_profile_without_plist_makes_no_launchctl_calls(
        ...
        still processed."""
        current = "ai.socis.gateway"
        rec = _fleet(
            monkeypatch,
            tmp_path,
            current=current,
            labels=[current, "ai.socis.gateway-a"],
            located={"ai.socis.gateway-a": (f"gui/{UID}", 200)},
            plist_exists=False,
        )
        restarted: list[str] = []
        failed: list[str] = []
        ...
        assert restarted == ["ai.socis.gateway-a"]
        assert failed == []
    ...
    def test_unbootstrapped_sibling_is_skipped_not_failed(
        ...
        rec = _fleet(
            ...
            tmp_path,
            current="ai.socis.gateway",
            labels=["ai.socis.gateway", "ai.socis.gateway-idle"],
            located={
                "ai.socis.gateway": (f"gui/{UID}", 100),
                "ai.socis.gateway-idle": (None, None),
            },
        )
        restarted: list[str] = []
        failed: list[str] = []
        ...
        assert rec.kickstarts == []
        assert restarted == ["ai.socis.gateway"]
        assert failed == []
    ...
    def test_loaded_but_not_running_sibling_is_kickstarted(
        ...
        rec = _fleet(
            ...
            tmp_path,
            current="ai.socis.gateway",
            labels=["ai.socis.gateway", "ai.socis.gateway-dormant"],
            located={
                "ai.socis.gateway": (f"gui/{UID}", 100),
                "ai.socis.gateway-dormant": (f"gui/{UID}", None),
            },
        )
        restarted: list[str] = []
        failed: list[str] = []
        ...
        assert rec.drains == []
        assert rec.kickstarts == [f"gui/{UID}/ai.socis.gateway-dormant"]
        assert restarted == ["ai.socis.gateway", "ai.socis.gateway-dormant"]
        assert failed == []
    ...
    def test_graceful_drain_with_keepalive_respawn_skips_kickstart(
        ...
        rec = _fleet(
            ...
            tmp_path,
            current="ai.socis.gateway",
            labels=["ai.socis.gateway", "ai.socis.gateway-a"],
            located={
                "ai.socis.gateway": (f"gui/{UID}", 100),
                "ai.socis.gateway-a": (f"gui/{UID}", 200),
            },
        ...
        assert rec.kickstarts == []
        assert rec.waits == [f"gui/{UID}/ai.socis.gateway-a"]
        assert restarted == ["ai.socis.gateway", "ai.socis.gateway-a"]
        assert failed == []
    ...
    def test_kickstart_failure_is_recorded_and_rest_continue(
        ...
        rec = _fleet(
            ...
            labels=[
                "ai.socis.gateway",
                "ai.socis.gateway-bad",
                "ai.socis.gateway-good",
            ],
            located={
                "ai.socis.gateway": (f"gui/{UID}", 100),
                "ai.socis.gateway-bad": (f"gui/{UID}", 200),
                "ai.socis.gateway-good": (f"gui/{UID}", 300),
            },
            kick_errors={
                "ai.socis.gateway-bad": subprocess.CalledProcessError(
        ...
        assert failed == ["ai.socis.gateway-bad"]
        assert rec.kickstarts == [f"gui/{UID}/ai.socis.gateway-good"]
        assert restarted == ["ai.socis.gateway", "ai.socis.gateway-good"]
    ...
    def test_timeout_during_discovery_is_failed_and_rest_continue(
        ...
        rec = _fleet(
            ...
            labels=[
                "ai.socis.gateway",
                "ai.socis.gateway-wedged",
                "ai.socis.gateway-after",
            ],
            located={
                "ai.socis.gateway": (f"gui/{UID}", 100),
                "ai.socis.gateway-wedged": subprocess.TimeoutExpired(
                    cmd=["launchctl", "print"], timeout=5
                ),
                "ai.socis.gateway-after": (f"gui/{UID}", 300),
            },
        )
        restarted: list[str] = []
        failed: list[str] = []
        ...
        assert failed == ["ai.socis.gateway-wedged"]
        assert rec.kickstarts == [f"gui/{UID}/ai.socis.gateway-after"]
        assert restarted == ["ai.socis.gateway", "ai.socis.gateway-after"]
    ...
    def test_timeout_during_kickstart_is_failed_and_rest_continue(
        ...
        rec = _fleet(
            ...
            located={
                "ai.socis.gateway": (f"gui/{UID}", 100),
                "ai.socis.gateway-wedged": (f"gui/{UID}", 200),
                "ai.socis.gateway-after": (f"gui/{UID}", 300),
            },
            kick_errors={
                "ai.socis.gateway-wedged": subprocess.TimeoutExpired(
    ...
    def test_sibling_that_never_comes_back_is_failed(self, monkeypatch, tmp_path):
        rec = _fleet(
            ...
            tmp_path,
            current="ai.socis.gateway",
            labels=["ai.socis.gateway", "ai.socis.gateway-zombie"],
            located={
                "ai.socis.gateway": (f"gui/{UID}", 100),
                "ai.socis.gateway-zombie": (f"gui/{UID}", 200),
            },
            wait_results={"ai.socis.gateway-zombie": False},
        )
        restarted: list[str] = []
        failed: list[str] = []
        ...
        assert restarted == ["ai.socis.gateway"]
        assert failed == ["ai.socis.gateway-zombie"]
...
class TestWaitForLaunchdServicePid:
    def test_returns_true_once_pid_changes(self, monkeypatch):
        ...
        assert gw._wait_for_launchd_service_pid(
            "ai.socis.gateway-x", old_pid=200, timeout=5.0, domain=f"gui/{UID}"
        )
    ...
    def test_returns_false_when_pid_never_changes(self, monkeypatch):
        ...
        assert not gw._wait_for_launchd_service_pid(
            "ai.socis.gateway-x", old_pid=200, timeout=3.0, domain=f"gui/{UID}"
        )
