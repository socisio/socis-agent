...
def profiles():
    ...
    Sibling test files reload ``socis_cli.profiles`` / ``socis_cli.main``;
    a top-level ``from socis_cli.profiles import ...`` here would bind the
    ...
    """
    return importlib.import_module("socis_cli.profiles")
...
def test_custom_socis_agent_home_inside_a_checkout_uses_a_sibling_store(
    tmp_path, monkeypatch, profiles
):
    ...
    monkeypatch.setattr(profiles, "_get_default_socis_agent_home", lambda: checkout)
...
def test_checkout_detection_does_not_depend_on_cwd(tmp_path, monkeypatch, profiles):
    """SOCIS_AGENT_HOME inside a checkout must be detected even when cwd is elsewhere.
    ...
    checkout.mkdir()
    (checkout / ".git").write_text("gitdir: ../git\n", encoding="utf-8")
    socis_agent_home = checkout / "data_home"
    socis_agent_home.mkdir()
    elsewhere = tmp_path / "elsewhere"
    elsewhere.mkdir()
    monkeypatch.chdir(elsewhere)  # cwd has no .git ancestor inside tmp_path
    monkeypatch.setattr(Path, "home", lambda: tmp_path / "home")
    monkeypatch.setattr(profiles, "_get_default_socis_agent_home", lambda: socis_agent_home)
...
def test_cli_export_default_does_not_write_into_the_current_checkout(
    ...
    monkeypatch.setattr(profiles, "_get_default_socis_agent_home", lambda: default_home)
    monkeypatch.setattr(
        "socis_agent_constants.get_default_socis_root", lambda: default_home
    )
...
def test_slash_export_uses_the_same_managed_destination(
    ...
    mixin_mod = importlib.import_module("socis_cli.cli_commands_mixin")
    default_home = tmp_path / ".socis-agent"
    default_home.mkdir()
    monkeypatch.setattr(profiles, "_get_default_socis_agent_home", lambda: default_home)
...
async def test_profile_export_api_uses_the_shared_managed_destination(
    ...
    from socis_cli.web_models import ProfileExport

    router_mod = importlib.import_module("socis_cli.web_routers.profiles")
...
def test_cwd_in_unrelated_checkout_does_not_prove_safety(
    ...
    of a SOCIS_AGENT_HOME inside checkout B."""
    ...
    monkeypatch.setattr(Path, "home", lambda: tmp_path / "home")
    monkeypatch.setattr(profiles, "_get_default_socis_agent_home", lambda: checkout_b)
