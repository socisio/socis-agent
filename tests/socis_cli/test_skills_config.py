"""Tests for socis_cli/skills_config.py and skills_tool disabled filtering."""
...
class TestSaveDisabledSkills:
    @patch("socis_cli.skills_config.save_config")
...
class TestIsSkillDisabled:
    ...
    @patch("socis_cli.config.load_config")
    ...
    @patch("socis_cli.config.load_config")
    @patch.dict("os.environ", {"SOCIS_AGENT_PLATFORM": "discord"})
...
class TestGetDisabledSkillNames:
    ...
    def test_session_platform_env_var(self, tmp_path, monkeypatch):
        """SOCIS_AGENT_SESSION_PLATFORM should be used when SOCIS_AGENT_PLATFORM is unset."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  disabled:\n"
    ...
    def test_socis_platform_takes_precedence(self, tmp_path, monkeypatch):
        """SOCIS_AGENT_PLATFORM should win over SOCIS_AGENT_SESSION_PLATFORM."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  platform_disabled:\n"
...
class TestGetCategories:
    def test_extracts_unique_categories(self):
        from socis_cli.skills_config import _get_categories
        skills = [
