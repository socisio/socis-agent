...
def _write_auth_store(tmp_path, payload: dict) -> None:
    ...
    (socis_agent_home / "auth.json").write_text(json.dumps(payload, indent=2))
...
def test_auth_add_api_key_persists_manual_entry(tmp_path, monkeypatch):
    monkeypatch.setenv("SOCIS_AGENT_HOME", str(tmp_path / "socis"))
    monkeypatch.delenv("OPENROUTER_API_KEY", raising=False)
    monkeypatch.delenv("OPENAI_API_KEY", raising=False)
    _write_auth_store(tmp_path, {"version": 1, "providers": {}})

    from socis_cli.auth_commands import auth_add_command
    ...
    payload = json.loads((tmp_path / "socis" / "auth.json").read_text())
...
def test_auth_add_configured_provider_uses_canonical_pool_key(tmp_path, monkeypatch):
    ...
    payload = json.loads((socis_agent_home / "auth.json").read_text(encoding="utf-8"))
...
def test_auth_add_migrates_display_name_derived_legacy_pool_key(
    ...
    with patch("socis_cli.models.clear_provider_models_cache") as clear_cache:
        auth_add_command(_Args())
...
def test_auth_list_includes_non_registry_configured_provider(
    ...
    from socis_cli.auth_commands import auth_list_command
...
def test_interactive_auth_add_accepts_non_registry_configured_provider(
    ...
    from socis_cli import auth_commands
...
def test_auth_add_nous_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "socis_cli.auth._nous_device_code_login",
        lambda **kwargs: {
    ...
    # `socis auth add nous` must also populate providers.nous so the
    ...
    # raises "SOCIS is not logged into Nous Portal" and the agent dies.
...
def test_auth_add_nous_oauth_honors_custom_label(tmp_path, monkeypatch):
    """`socis auth add nous --type oauth --label <name>` must preserve the
...
def test_auth_add_codex_oauth_keeps_distinct_pool_accounts(tmp_path, monkeypatch):
    """Two ``socis auth add openai-codex`` runs for different ChatGPT
    ...
    second independent one. ``socis auth list`` showed two labels sharing
    ...
    monkeypatch.setattr("socis_cli.auth._codex_device_code_login", lambda: next(logins))
...
def test_codex_auth_status_reports_pool_only_credential(tmp_path, monkeypatch):
    ...
    from socis_cli.auth import get_codex_auth_status
...
def test_codex_runtime_pool_only_rate_limit_is_not_missing_auth(tmp_path, monkeypatch):
    ...
    from socis_cli.auth import AuthError, CODEX_RATE_LIMITED_CODE, resolve_codex_runtime_credentials
...
def test_auth_add_xai_oauth_sets_active_provider(tmp_path, monkeypatch):
    """socis auth add xai-oauth must set active_provider and write a pool entry.
...
def test_auth_add_xai_oauth_keeps_distinct_pool_accounts(tmp_path, monkeypatch):
    """Two ``socis auth add xai-oauth`` runs must produce independent pool entries.
...
def test_auth_remove_codex_migrates_legacy_dict_suppression(tmp_path, monkeypatch):
    ...
    payload = json.loads((tmp_path / "socis" / "auth.json").read_text(encoding="utf-8"))
...
def test_logout_resets_codex_config_when_auth_state_already_cleared(tmp_path, monkeypatch, capsys):
    """`socis logout --provider openai-codex` must still clear model.provider.
    ...
    config_text = (socis_agent_home / "config.yaml").read_text()
...
def test_unsuppress_credential_source_clears_marker(tmp_path, monkeypatch):
    ...
    from socis_cli.auth import suppress_credential_source, unsuppress_credential_source, is_source_suppressed
...
# =============================================================================
# Unified credential-source stickiness — every source SOCIS reads from has a
...
def test_seed_from_singletons_respects_socis_pkce_suppression(tmp_path, monkeypatch):
    """anthropic socis_pkce must not re-seed from ~/.socis-agent/.anthropic_oauth.json when suppressed."""
    socis_agent_home = tmp_path / "socis"
    socis_agent_home.mkdir(parents=True, exist_ok=True)
    monkeypatch.setenv("SOCIS_AGENT_HOME", str(socis_agent_home))

    import yaml
    (socis_agent_home / "config.yaml").write_text(yaml.dump({"model": {"provider": "anthropic", "model": "claude"}}))
    (socis_agent_home / "auth.json").write_text(json.dumps({
        "version": 1,
        "providers": {},
        "suppressed_sources": {"anthropic": ["socis_pkce"]},
    }))

    # Stub the readers so only socis_pkce is "available"; claude_code returns None
    import agent.anthropic_adapter as aa
    monkeypatch.setattr(aa, "read_socis_oauth_credentials", lambda: {
    ...
    # socis_pkce suppressed, claude_code returns None → nothing should be seeded
    assert entries == []
    assert "socis_pkce" not in active
...
def test_credential_sources_registry_has_expected_steps():
    ...
    required = {
        ...
        "~/.socis-agent/.anthropic_oauth.json",
...
def test_auth_remove_copilot_suppresses_all_variants(tmp_path, monkeypatch):
    ...
    from types import SimpleNamespace
    from socis_cli.auth import is_source_suppressed
    from socis_cli.auth_commands import auth_remove_command

    with patch(
        "socis_cli.copilot_auth.resolve_copilot_token",
        return_value=("ghp_fake", "gh"),
    ), patch(
        "socis_cli.copilot_auth.get_copilot_api_token",
...
def test_auth_remove_env_seeded_dotenv_with_bom_no_shell_hint(tmp_path, monkeypatch, capsys):
    ...
    (socis_agent_home / ".env").write_bytes(
