"""Tests for ``socis debug`` CLI command and debug utilities."""
...
def socis_agent_home(tmp_path, monkeypatch):
    """Set up an isolated SOCIS_AGENT_HOME with minimal logs."""
    ...
    (logs_dir / "gui.log").write_text(
        "2026-04-12 17:00:12 INFO socis_cli.web_server: dashboard request\n"
    )
...
class TestUploadPasteRs:
    ...
    def test_upload_paste_rs_success(self):
        from socis_cli.debug import _upload_paste_rs
        ...
        with patch("socis_cli.debug.urllib.request.urlopen", return_value=mock_resp):
    ...
    def test_upload_paste_rs_network_error(self):
        ...
        with patch(
            "socis_cli.debug.urllib.request.urlopen",
...
class TestUploadToPastebin:
    ...
    def test_falls_back_to_dpaste_com(self):
        from socis_cli.debug import upload_to_pastebin

        with patch("socis_cli.debug._upload_paste_rs",
                    side_effect=Exception("down")), \
             patch("socis_cli.debug._upload_dpaste_com",
...
class TestCaptureLogSnapshot:
    ...
    def test_race_truncate_after_resolve_reports_empty(self, socis_agent_home, monkeypatch):
        """If the log is truncated between resolve and stat, say 'empty', not 'missing'."""
        log_path = socis_agent_home / "logs" / "agent.log"
        from socis_cli import debug
    ...
    def test_keeps_first_line_when_truncation_on_boundary(self, socis_agent_home):
        """When truncation lands on a line boundary, keep the first full line."""
        from socis_cli.debug import _capture_log_snapshot
        ...
        (socis_agent_home / "logs" / "agent.log").write_text(line * num_lines)
...
class TestMissingLogNote:
    ...
    `socis debug share` runs on the backend, so a desktop connected to a
    ...
    def test_backend_written_log_reports_plain_absence(self, socis_agent_home):
        from socis_cli.debug import _capture_log_snapshot

        (socis_agent_home / "logs" / "agent.log").unlink()

        snap = _capture_log_snapshot("agent", tail_lines=10)
        assert snap.full_text is None
        assert snap.tail_text == "(file not found)"

    def test_client_written_log_names_its_writer_and_path(self, socis_agent_home):
        from socis_cli.debug import _capture_log_snapshot

        (socis_agent_home / "logs" / "desktop.log").unlink()

        snap = _capture_log_snapshot("desktop", tail_lines=10)
        assert snap.full_text is None
        assert "not on this host" in snap.tail_text
        assert "SOCIS Agent Desktop" in snap.tail_text
        # The reader needs the path to collect by hand on the client machine.
        assert str(socis_agent_home / "logs" / "desktop.log") in snap.tail_text

    def test_present_client_log_is_captured_normally(self, socis_agent_home):
    ...
    def test_empty_client_log_is_empty_not_absent(self, socis_agent_home):
        """An empty file means the app ran and logged nothing — a different fact."""
        from socis_cli.debug import _capture_log_snapshot

        (socis_agent_home / "logs" / "desktop.log").write_text("")

        snap = _capture_log_snapshot("desktop", tail_lines=10)
        assert snap.tail_text == "(file empty)"

    def test_report_carries_the_note_for_a_remote_backend(self, socis_agent_home):
        """The uploaded report — what people paste into support — must explain it."""
        from socis_cli.debug import collect_debug_report
...
# ---------------------------------------------------------------------------
# Capture log redaction (force=True applies regardless of SOCIS_AGENT_REDACT_SECRETS)
# ---------------------------------------------------------------------------
...
class TestCaptureLogSnapshotRedaction:
    ...
    @pytest.fixture
    def socis_agent_home_with_secret(self, tmp_path, monkeypatch):
        """Isolated SOCIS_AGENT_HOME whose agent.log contains a vendor-prefixed token."""
        home = tmp_path / ".socis-agent"
        home.mkdir()
        monkeypatch.setenv("SOCIS_AGENT_HOME", str(home))
        ...
        monkeypatch.delenv("SOCIS_AGENT_REDACT_SECRETS", raising=False)
    ...
    def test_default_redacts_tail_and_full_text(self, socis_agent_home_with_secret):
        from socis_cli.debug import _capture_log_snapshot
    ...
    def test_redact_false_passes_through(self, socis_agent_home_with_secret):
        from socis_cli.debug import _capture_log_snapshot
    ...
    def test_force_true_works_when_redaction_disabled(
        self, socis_agent_home_with_secret, monkeypatch
    ):
        ...
        input unchanged when SOCIS_AGENT_REDACT_SECRETS=false, and the share-time
        ...
        monkeypatch.setenv("SOCIS_AGENT_REDACT_SECRETS", "false")

        from socis_cli.debug import _capture_log_snapshot

        assert os.environ.get("SOCIS_AGENT_REDACT_SECRETS", "") == "false"
    ...
    def test_default_redacts_email_addresses_for_public_share(
        self, socis_agent_home_with_secret
    ):
        from socis_cli.debug import _capture_log_snapshot

        log_path = socis_agent_home_with_secret / "logs" / "agent.log"
        log_path.write_text(
    ...
    def test_no_redact_preserves_email_addresses(self, socis_agent_home_with_secret):
        from socis_cli.debug import _capture_log_snapshot
    ...
    def test_capture_default_log_snapshots_threads_redact(
        ...
        from socis_cli.debug import _capture_default_log_snapshots
...
class TestCollectDebugReport:
    ...
    def test_report_includes_dump_output(self, socis_agent_home):
        from socis_cli.debug import collect_debug_report

        with patch("socis_cli.dump.run_dump") as mock_dump:
            mock_dump.side_effect = lambda args: print(
                "--- socis dump ---\nversion: 0.8.0\n--- end dump ---"
            )
            report = collect_debug_report(log_lines=50)

        assert "--- socis dump ---" in report
...
class TestRunDebugShare:
    ...
    def test_share_sweeps_expired_pastes(self, socis_agent_home, capsys):
        ...
        with patch("socis_cli.dump.run_dump"), \
             patch("socis_cli.debug._sweep_expired_pastes", return_value=(0, 0)) as mock_sweep, \
             patch("socis_cli.debug.upload_to_pastebin",
    ...
    def test_share_uploads_five_pastes(self, socis_agent_home, capsys):
        ...
        with patch("socis_cli.dump.run_dump") as mock_dump, \
            ...
            mock_dump.side_effect = lambda a: print("--- socis dump ---\nversion: test\n--- end dump ---")
            run_debug_share(args)
...
class TestRunDebugShareRedaction:
    ...
    def test_default_share_redacts_uploaded_content(
        self, socis_agent_home_with_secret, capsys
    ):
        ...
        with patch("socis_cli.dump.run_dump"), \
             patch("socis_cli.debug._sweep_expired_pastes", return_value=(0, 0)), \
             patch("socis_cli.debug.upload_to_pastebin", side_effect=fake_upload):
            run_debug_share(args)
...
class TestRunDebug:
    def test_no_subcommand_shows_usage(self, capsys):
        ...
        out = capsys.readouterr().out
        assert "socis debug" in out
        assert "share" in out
        assert "delete" in out

    def test_share_subcommand_routes(self, socis_agent_home):
        from socis_cli.debug import run_debug
...
class TestExtractPasteId:
    def test_paste_rs_url(self):
        from socis_cli.debug import _extract_paste_id
...
class TestDeletePaste:
    def test_delete_sends_delete_request(self):
        from socis_cli.debug import delete_paste
        ...
        with patch("socis_cli.debug.urllib.request.urlopen",
...
class TestScheduleAutoDelete:
    ...
    ``~/.socis-agent/pastes/pending.json`` and lets ``_sweep_expired_pastes``
    handle the DELETE requests synchronously on the next ``socis debug``
    invocation.
    """


    def test_records_pending_to_json(self, socis_agent_home):
        """Scheduled URLs are persisted to pending.json with expiration."""
        from socis_cli.debug import _schedule_auto_delete, _pending_file
        import json
    ...
    def test_dedupes_same_url(self, socis_agent_home):
        """Same URL recorded twice → one entry with the later expire_at."""
        from socis_cli.debug import _schedule_auto_delete, _load_pending
...
class TestSweepExpiredPastes:
    ...
    def test_sweep_deletes_expired_entries(self, socis_agent_home):
        from socis_cli.debug import (
        ...
        with patch("socis_cli.debug.delete_paste", side_effect=fake_delete):
    ...
    def test_sweep_leaves_future_entries_alone(self, socis_agent_home):
        from socis_cli.debug import _sweep_expired_pastes, _save_pending
        import time
        ...
        with patch("socis_cli.debug.delete_paste") as mock_delete:
            deleted, remaining = _sweep_expired_pastes()
    ...
    def test_sweep_survives_network_failure(self, socis_agent_home):
...
class TestRunDebugSweepsOnInvocation:
    ...
    def test_run_debug_calls_sweep(self, socis_agent_home):
        from socis_cli.debug import run_debug

        args = MagicMock()
        args.debug_command = None  # default → prints help

        with patch("socis_cli.debug._sweep_expired_pastes") as mock_sweep:
            run_debug(args)
...
class TestRunDebugDelete:
    ...
    def test_handles_delete_failure(self, capsys):
        from socis_cli.debug import run_debug_delete

        args = MagicMock()
        args.urls = ["https://paste.rs/abc"]

        with patch("socis_cli.debug.delete_paste",
...
class TestShareIncludesAutoDelete:
    ...
    def test_share_shows_privacy_notice(self, socis_agent_home, capsys):
        from socis_cli.debug import run_debug_share
        ...
        with patch("socis_cli.dump.run_dump"), \
             patch("socis_cli.debug.upload_to_pastebin",
                    return_value="https://paste.rs/test"), \
             patch("socis_cli.debug._schedule_auto_delete"):
            run_debug_share(args)
...
class TestBuildDebugShare:
    ...
    Backs both ``socis debug share`` (CLI) and ``POST /api/ops/debug-share``
    ...
    def test_redaction_keeps_secrets_out_of_payload(self, socis_agent_home):
        from socis_cli.debug import build_debug_share

        secret = "sk-proj-SUPERSECRETtoken1234567890"
        (socis_agent_home / "logs" / "agent.log").write_text(
        ...
        with patch("socis_cli.dump.run_dump"), patch(
            "socis_cli.debug.upload_to_pastebin", side_effect=_upload
        ), patch("socis_cli.debug._schedule_auto_delete"):
    ...
    def test_optional_log_failure_is_collected_not_raised(self, socis_agent_home):
        from socis_cli.debug import build_debug_share
...
class TestCollectShareBundle:
    ...
    def test_no_redact_omits_banner(self, socis_agent_home):
        from socis_cli.debug import collect_share_bundle
    ...
    def test_redaction_keeps_secrets_out(self, socis_agent_home):
        from socis_cli.debug import collect_share_bundle
...
class TestDebugSlashCommand:
    ...
    ``SOCISCLI.process_command`` → ``_handle_debug_command(cmd_original)``.
    """

    def _handler(self):
        from socis_cli.cli_commands_mixin import CLICommandsMixin
    ...
    def _captured(self, cmd_original):
        ...
        with patch("socis_cli.debug.run_debug_share", _fake_run):
    ...
    def test_local_is_parsed(self):
        assert self._captured("/debug local")["local"] is True

    def test_no_arg_default_keyword(self):
        # Calling with no cmd_original (legacy callers) must still work.
        assert self._captured("")["local"] is False

    def test_no_nous_attribute_is_passed_on(self):
        """The private Nous upload path is gone; nothing may request it."""
        assert "nous" not in self._captured("/debug")

    @pytest.mark.parametrize("cmd", ["/debug nous", "/debug NOUS", "/debug nous local"])
    def test_debug_nous_refuses_and_uploads_nothing(self, cmd, capsys):
        """REGRESSION GUARD. `/debug nous` used to upload privately to Nous.

        Once that path was removed, the word would otherwise fall through to
        the DEFAULT path — a PUBLIC paste — and the slash command skips the
        confirmation prompt. A user who asked for a private upload would have
        had their logs posted publicly with no prompt. It must refuse instead.
        """
        c = self._captured(cmd)
        assert "_called" not in c, "run_debug_share must not be called"
        out = capsys.readouterr().out
        assert "no longer available" in out
        assert "Nothing was uploaded" in out
        assert "github.com/socisio/socis-agent/issues" in out


class TestShareConsentGate:
    """`socis debug share` requires explicit consent before uploading.
    ...
    def test_non_interactive_requires_yes(self, socis_agent_home, capsys, monkeypatch):
        ...
        with patch("socis_cli.dump.run_dump"), \
             patch("socis_cli.debug.upload_to_pastebin") as mock_upload:
    ...
    def test_local_never_prompts(self, socis_agent_home, capsys, monkeypatch):
