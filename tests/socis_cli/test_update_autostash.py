...
def _patch_gateway_discovery():
    ...
    ``_purge_stale_socis_modules`` must also be stubbed: it evicts
    ``socis_cli.gateway`` from ``sys.modules`` mid-update, and the restart
    phase's fresh ``from socis_cli.gateway import ...`` then loads an
    ...
    with patch("socis_cli.gateway.find_gateway_pids", return_value=[]), \
         ...
         patch("socis_cli.update_inventory.collect_runtime_inventory", return_value=None), \
         patch("socis_cli.update_inventory.report_unaccounted_runtimes", return_value=False), \
         patch.object(socis_main, "_fleet_probe_expected_runtimes", lambda *a, **kw: False), \
         patch.object(socis_main, "_purge_stale_socis_modules", lambda *a, **kw: None), \
         patch("socis_cli.update_receipt.collect_fleet_versions", return_value=[]):
        yield
...
def _setup_update_mocks(monkeypatch, tmp_path):
    ...
    (tmp_path / ".git").mkdir()
    monkeypatch.setattr(socis_main, "PROJECT_ROOT", tmp_path)
    monkeypatch.setattr(socis_main, "_stash_local_changes_if_needed", lambda *a, **kw: None)
    monkeypatch.setattr(socis_main, "_restore_stashed_changes", lambda *a, **kw: True)
    monkeypatch.setattr(socis_config, "get_missing_env_vars", lambda required_only=True: [])
    monkeypatch.setattr(socis_config, "get_missing_config_fields", lambda: [])
    monkeypatch.setattr(socis_config, "check_config_version", lambda **_kwargs: (5, 5))
    monkeypatch.setattr(socis_config, "migrate_config", lambda **kw: {"env_added": [], "config_added": []})
    monkeypatch.setattr(socis_main, "_upgrade_pip_before_lazy_refresh", lambda *a, **kw: None)
    monkeypatch.setattr(socis_main, "_refresh_active_lazy_features", lambda *a, **kw: True)
...
def test_refresh_active_memory_provider_dependencies_reinstalls_active_provider(monkeypatch):
    ...
    monkeypatch.setattr(
        "socis_cli.config.load_config",
        lambda: {"memory": {"provider": "mem0"}},
    )
    monkeypatch.setattr(
        "socis_cli.memory_setup._install_dependencies",
        lambda provider_name, force=False: recorded.append((provider_name, force)),
    )

    socis_main._refresh_active_memory_provider_dependencies()
...
def test_reload_updated_runtime_modules_restores_new_socis_agent_constants_symbol(monkeypatch):
    """A pre-pull module object missing a new helper is repaired by reload."""
    import socis_agent_constants

    monkeypatch.delattr(socis_agent_constants, "apply_subprocess_home_env", raising=False)
    assert not hasattr(socis_agent_constants, "apply_subprocess_home_env")

    socis_main._reload_updated_runtime_modules()

    assert callable(socis_agent_constants.apply_subprocess_home_env)
...
def _make_update_side_effect(
    ...
    ``refs/socis-update-backups/orphan-<branch>-*`` (oldest first) so the
...
def test_cmd_update_skips_stash_restore_when_reset_fails(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(
        socis_main, "_stash_local_changes_if_needed",
        lambda *a, **kw: "abc123deadbeef",
    )
    restore_calls = []
    monkeypatch.setattr(
        socis_main, "_restore_stashed_changes",
        lambda *a, **kw: restore_calls.append(1) or True,
    )

    side_effect, _ = _make_update_side_effect(ff_only_fails=True, reset_fails=True)
    monkeypatch.setattr(socis_main.subprocess, "run", side_effect)

    with pytest.raises(SystemExit, match="1"):
        socis_main.cmd_update(SimpleNamespace())
...
def test_cmd_update_orphan_history_backs_up_before_reset(monkeypatch, tmp_path, capsys):
    ...
    ``refs/socis-update-backups/orphan-*`` ref before the reset proceeds."""
    _setup_update_mocks(monkeypatch, tmp_path)
    ...
    assert ref_name.startswith("refs/socis-update-backups/orphan-main-")
    ...
    assert f"expires after {socis_main._ORPHAN_RESCUE_REF_MAX_AGE_DAYS} days" in out
...
def test_cmd_update_orphan_rescue_refs_pruned_beyond_keep_limit(monkeypatch, tmp_path, capsys):
    ...
    total = socis_main._ORPHAN_RESCUE_REFS_TO_KEEP + 2
    stale_refs = [
        "refs/socis-update-backups/orphan-main-"
    ...
    assert len(delete_calls) == total - socis_main._ORPHAN_RESCUE_REFS_TO_KEEP
    deleted_refs = {c[c.index("-d") + 1] for c in delete_calls}
    assert deleted_refs == set(stale_refs[: total - socis_main._ORPHAN_RESCUE_REFS_TO_KEEP])
...
def test_cmd_update_orphan_rescue_refs_expired_by_age(monkeypatch, tmp_path, capsys):
    ...
    now = datetime.now(timezone.utc)
    old = now - timedelta(days=socis_main._ORPHAN_RESCUE_REF_MAX_AGE_DAYS + 5)
...
def test_prune_orphan_rescue_refs_leaves_unparseable_names_alone():
    ...
    weird = "refs/socis-update-backups/orphan-main-not-a-timestamp-xyz"
    calls = []
    ...
    with mock_patch.object(socis_main.subprocess, "run", side_effect=fake_run):
        socis_main._prune_orphan_rescue_refs(["git"], ".", "main")
...
def _setup_setting_test(monkeypatch, tmp_path, mode):
    ...
    monkeypatch.setattr(
        socis_main, "_discard_stashed_changes",
...
def _setup_keep_stash_test(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        socis_main, "_park_stashed_changes",
    ...
    monkeypatch.setattr(
        "socis_cli.gateway.find_gateway_pids", lambda **kw: [], raising=False
    )
...
def test_update_keep_stash_parks_instead_of_restoring(monkeypatch, tmp_path):
    ...
    socis_main.cmd_update(SimpleNamespace(yes=True, keep_stash=True))
...
def test_update_without_keep_stash_still_restores(monkeypatch, tmp_path):
    ...
    socis_main.cmd_update(SimpleNamespace(yes=True, keep_stash=False))
...
def test_bootstrap_marker_not_autostashed_by_update(tmp_path):
    ...
    ``socis update``'s ``git stash push --include-untracked`` does not sweep it
    ...
    repo_gitignore = Path(socis_main.__file__).resolve().parents[1] / ".gitignore"
    ...
    marker = tmp_path / ".socis-bootstrap-complete"
    marker.write_text("")

    # Exact flags used by socis update (socis_cli/main.py).
    git("stash", "push", "--include-untracked", "-m", "socis-update-autostash")

    assert marker.exists(), (
        ".socis-bootstrap-complete was swept into the update autostash — it must "
    ...
    assert ".socis-bootstrap-complete" not in status
...
def test_restore_rejects_invalid_python_and_keeps_clean_updated_tree(
    ...
    assert "made the SOCIS agent unexecutable" in output
...
def test_restore_allows_preexisting_import_time_failure(monkeypatch, tmp_path):
    ...
    assert socis_main._restore_stashed_changes(
        ["git"], tmp_path, stash_ref, prompt_user=False
    )
...
def test_prune_orphan_rescue_refs_with_real_git_unpins_objects(tmp_path):
    ...
    old_ref = "refs/socis-update-backups/orphan-main-20200101-000000-" + snap_sha[:12]
    ...
    socis_main._prune_orphan_rescue_refs(["git"], tmp_path, "main")
    remaining = git("for-each-ref", "refs/socis-update-backups/").stdout
