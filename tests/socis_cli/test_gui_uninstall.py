"""Tests for socis_cli.gui_uninstall — GUI-only uninstall + install discovery.
...
import socis_cli.gui_uninstall as gu


def _make_agent(socis_agent_home: Path) -> Path:
    """Create a fake agent install: source package + venv."""
    agent_root = socis_agent_home / "socis-agent"
    (agent_root / "socis_cli").mkdir(parents=True)
    (agent_root / "socis_cli" / "__init__.py").write_text("")
    (agent_root / "venv" / "bin").mkdir(parents=True)
    return agent_root


def _make_gui_build(socis_agent_home: Path) -> None:
    """Create the source-built GUI artifacts a `socis desktop` run produces."""
    desktop = socis_agent_home / "socis-agent" / "apps" / "desktop"
    ...
    (socis_agent_home / "socis-agent" / "node_modules").mkdir(parents=True)
    (socis_agent_home / "desktop-build-stamp.json").write_text("{}")


def _make_user_data(socis_agent_home: Path) -> None:
    (socis_agent_home / "config.yaml").write_text("x: 1\n")
    (socis_agent_home / ".env").write_text("KEY=secret\n")
    (socis_agent_home / "sessions").mkdir()
...
def test_gui_install_summary_shape(tmp_path, monkeypatch):
    ...
    _make_agent(socis_agent_home)
    _make_gui_build(socis_agent_home)
    monkeypatch.setattr(gu, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "none")

    summary = gu.gui_install_summary(socis_agent_home)
    ...
    assert summary["socis_agent_home"] == str(socis_agent_home)
...
def test_linux_discovery_includes_launcher_entry(tmp_path, monkeypatch):
    """The launcher entry that `socis desktop` installs is removable."""
...
def test_uninstall_removes_launcher_entry_and_refreshes_cache(tmp_path, monkeypatch):
    ...
    socis_agent_home = tmp_path / ".socis-agent"
    _make_agent(socis_agent_home)
    icon = lde.icon_path(socis_agent_home / "socis-agent")
    icon.parent.mkdir(parents=True, exist_ok=True)
    icon.write_bytes(b"\x89PNG")
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "none")

    removed = gu.uninstall_gui(socis_agent_home)
    ...
    assert lde.icon_path(socis_agent_home / "socis-agent").exists()
    # The agent itself survives a GUI uninstall.
    assert (socis_agent_home / "socis-agent" / "socis_cli").is_dir()
...
def test_uninstall_skips_cache_refresh_when_no_launcher_entry(tmp_path, monkeypatch):
    ...
    gu.uninstall_gui(tmp_path / ".socis-agent")
...
def test_uninstall_args_namespace_mode_mapping():
    ...
    import socis_cli.uninstall as uninstall
