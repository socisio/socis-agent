...
(release/win-unpacked/SOCIS.exe) exists only in the live tree. Swapping
...
def test_staged_apps_swap_preserves_live_release_dir(tmp_path, monkeypatch):
    ...
    # live tree: apps/desktop/release/win-unpacked/SOCIS.exe + old source
    root = tmp_path / "install"
    live_apps = root / "apps" / "desktop"
    (live_apps / "release" / "win-unpacked").mkdir(parents=True)
    (live_apps / "release" / "win-unpacked" / "SOCIS.exe").write_bytes(b"MZbuilt")
    ...
    monkeypatch.setattr(socis_main, "PROJECT_ROOT", root)
    ...
    exe = root / "apps" / "desktop" / "release" / "win-unpacked" / "SOCIS.exe"
