...
from socis_cli.backup import copy_db_and_verify, verify_sqlite_integrity
...
class TestPreUpdateBackupIntegrityGuard:
    ...
    SOCIS_AGENT_HOME whose state.db is corrupted mid-flight (#68474)."""
    ...
    def socis_agent_home(self, tmp_path, monkeypatch):
        ...
        for mod in list(sys.modules.keys()):
            if mod.startswith("socis_cli.config") or mod == "socis_agent_constants":
                del sys.modules[mod]
        return root

    def test_healthy_db_stays_quiet(self, socis_agent_home, capsys):
    ...
    def test_zeroed_db_after_snapshot_is_loud(self, socis_agent_home, capsys, monkeypatch):
        ...
        def create_then_zero(**kwargs):
            ...
            live = socis_agent_home / "state.db"
