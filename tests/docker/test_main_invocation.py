...
def test_no_args_starts_socis(built_image: str) -> None:
    """``docker run <image>`` should start socis cleanly.
...
def test_chat_subcommand_passthrough(built_image: str) -> None:
    """``docker run <image> chat --help`` should exec ``socis chat --help``.
...
def test_container_exit_code_matches_inner_exit(built_image: str) -> None:
    ...
    Critical for CI: ``docker run <image> socis batch ...`` returns a
