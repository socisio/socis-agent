"""Runtime smoke test for Docker $SOCIS_AGENT_HOME/logs/gateways seeding.

Build the real image and verify logs/ and logs/gateways/ exist and are
owned by the socis user after container boot.
...
registered later runs its log service as the dropped socis user and
...
def test_logs_gateways_seeded_and_socis_owned(
    built_image: str, container_name: str,
) -> None:
    """logs/ and logs/gateways/ must exist and be owned by socis after boot."""
    ...
    assert "logs=socis" in r.stdout, (
        f"logs/ not owned by socis: {r.stdout}"
    )
    assert "gateways=socis" in r.stdout, (
        f"logs/gateways/ not owned by socis: {r.stdout}"
    )
