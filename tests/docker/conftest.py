...
Override the image with ``SOCIS_AGENT_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("SOCIS_AGENT_TEST_IMAGE", "socis-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``SOCIS_AGENT_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("SOCIS_AGENT_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged socis user
# ---------------------------------------------------------------------------
#
# Background: every SOCIS runtime path inside the container drops to UID
# 10000 (the ``socis`` user) via ``s6-setuidgid socis``. ``docker exec``
...
# fails (PermissionError swallowed) for socis, so a test that ran as root
...
# helpers below run every probe under ``-u socis`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: socis).
