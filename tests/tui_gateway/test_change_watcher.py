...
Behavior contracts, exercised against a real temp SOCIS_AGENT_HOME (no mocks on the
...
def watcher_home(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(server, "_socis_agent_home", str(tmp_path))
...
def test_served_profile_store_move_broadcasts_sessions_changed(watcher_home, monkeypatch):
    ...
    monkeypatch.setattr("socis_cli.profiles.get_profile_dir", lambda name: home / "profiles" / name)
