...
def server():
    ...
    # e.g. socis_cli.active_sessions would bind the mocked get_socis_agent_home
    ...
    with patch.dict("sys.modules", {
        "socis_agent_constants": MagicMock(get_socis_agent_home=MagicMock(return_value="/tmp/socis_test")),
        "socis_cli.env_loader": MagicMock(),
        "socis_cli.banner": MagicMock(),
        "socis_agent_state": MagicMock(),
    }):
        import importlib
...
def test_disable_flush_env_var_actually_wires_to_module_constant(monkeypatch):
    """End-to-end: setting `SOCIS_AGENT_TUI_GATEWAY_NO_FLUSH=1` and importing
    ...
    monkeypatch.setenv("SOCIS_AGENT_TUI_GATEWAY_NO_FLUSH", "1")
    ...
    try:
        ...
        monkeypatch.delenv("SOCIS_AGENT_TUI_GATEWAY_NO_FLUSH", raising=False)
...
def test_config_roundtrip(server, tmp_path):
    server._socis_agent_home = tmp_path
...
def test_slash_exec_rejects_skill_commands(server):
    ...
    fake_skills = {"/socis-agent-dev": {"name": "socis-agent-dev", "description": "Dev workflow"}}

    with patch("agent.skill_commands.get_skill_commands", return_value=fake_skills):
        resp = server.handle_request({
            "id": "r1",
            "method": "slash.exec",
            "params": {"command": "socis-agent-dev", "session_id": sid},
        })
...
def test_slash_exec_scopes_skill_lookup_to_session_profile(server, tmp_path):
    ...
    profile_home rather than the gateway process's ambient SOCIS_AGENT_HOME
    ...
    # The gateway's own SOCIS_AGENT_HOME (the test-isolation tempdir, no
...
def test_skin_live_switch_end_to_end(server, tmp_path, monkeypatch):
    """Real config + skin files: activating a skin (as `socis config set` does)
    ...
    import socis_cli.skin_engine as skin_engine

    (tmp_path / "skins").mkdir()
    (tmp_path / "skins" / "midnight.yaml").write_text(
        "name: midnight\ndescription: t\ncolors:\n  banner_title: '#00ffcc'\n  background: '#001010'\n"
    )
    monkeypatch.setattr(skin_engine, "get_socis_agent_home", lambda: tmp_path)
    ...
    # Activate midnight, as `socis config set display.skin midnight` would.
