...
def socis_agent_home(tmp_path, monkeypatch):
    ...
    # Bust the goal-module DB cache so it re-resolves SOCIS_AGENT_HOME.
...
@pytest.fixture()
def server(socis_agent_home, monkeypatch):
    ...
    # Pin config resolution to the isolated SOCIS_AGENT_HOME. Sibling test
    ...
    # module-level ``_socis_agent_home = get_socis_agent_home()`` snapshot freezes
    ...
    monkeypatch.setattr(mod, "_socis_agent_home", socis_agent_home)
...
def test_moa_bare_returns_usage(server, session, socis_agent_home):
    _write_moa_config(socis_agent_home, """
moa:
