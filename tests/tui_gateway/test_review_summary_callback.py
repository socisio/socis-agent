    ...
    with patch.dict(
        "sys.modules",
        {
            "socis_agent_constants": MagicMock(
                get_socis_agent_home=MagicMock(return_value="/tmp/socis_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'socis-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'socis-release' patched"
    }
