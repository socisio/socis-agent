"""Guard: SOCIS-owned subprocesses must not resolve managed runtimes by bare PATH.

SOCIS installs runtimes for itself — ``uv`` at ``$SOCIS_AGENT_HOME/bin/uv``, Node at
``$SOCIS_AGENT_HOME/node``. Neither directory is on the ambient PATH of an arbitrary
process, so ``shutil.which("uv")`` / ``shutil.which("node")`` in SOCIS's own
...
* when a system copy also exists, the one SOCIS does not own wins — which is
...
``iter_socis_node_dirs()``, ``resolve_uv()``, or ``ensure_uv()``. This test is
...
# Runtimes SOCIS provisions into SOCIS_AGENT_HOME and must therefore resolve
# through a managed-aware helper rather than PATH.
_MANAGED_COMMANDS = frozenset({"uv", "node", "npm", "npx"})

# Directories that are not SOCIS-owned subprocess code: plugins ship their own
...
def _source_files() -> list[Path]:
    ...
    # (socis_agent-<version>/) gets created and deleted concurrently, and
...
def test_no_unreviewed_bare_managed_runtime_lookups():
    ...
    assert not unexpected, (
        "Bare PATH lookup for a SOCIS-managed runtime.\n\n"
        + "\n".join(f"  {rel}:{lineno}  which({cmd!r})" for rel, cmd, lineno in unexpected)
        + "\n\n$SOCIS_AGENT_HOME/bin (uv) and $SOCIS_AGENT_HOME/node are not on an "
        ...
        "  node/npm -> socis_agent_constants.find_node_executable()\n"
        "  PATH env -> socis_agent_constants.iter_socis_node_dirs()\n"
...
def test_managed_node_helpers_exist(helper):
    ...
    assert callable(getattr(socis_agent_constants, helper))


def test_managed_uv_helpers_exist():
    from socis_cli.managed_uv import ensure_uv, managed_uv_path, resolve_uv
