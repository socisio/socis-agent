...
from socis_cli.plugins import PluginManager, VALID_HOOKS


def _make_enabled_plugin(socis_agent_home: Path, name: str, register_body: str) -> Path:
    """Create a plugin under <socis_agent_home>/plugins/<name> and opt it in."""
    plugin_dir = socis_agent_home / "plugins" / name
...
def test_no_plugins_returns_empty_results(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("SOCIS_AGENT_HOME", str(tmp_path / "socis_empty"))
