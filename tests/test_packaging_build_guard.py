...
def _build_artifact(kind: str, tmp_path, *, nix_build: bool) -> subprocess.CompletedProcess[str]:
    ...
    if nix_build:
        env["SOCIS_AGENT_NIX_BUILD"] = "1"
    else:
        env.pop("SOCIS_AGENT_NIX_BUILD", None)
...
def test_artifact_build_rejects_nix_development_shell_environment(kind, tmp_path):
    ...
    assert "Building wheels or sdists for socis-agent is not supported" in result.stderr


@pytest.mark.parametrize(
    ("kind", "artifact_glob"),
    [("sdist", "socis_agent-*.tar.gz"), ("wheel", "socis_agent-*.whl")],
)
