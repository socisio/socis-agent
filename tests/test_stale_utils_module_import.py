...
Real incident (gateway session 1518671026962174144)::
    ...
    cannot import name 'env_float' from 'utils' (~/.socis-agent/socis-agent/utils.py)
...
2. ``socis update`` ran ``git pull``, updating ``utils.py`` (now defining
...
``socis_cli/main.py`` (the ``socis update`` flow, ~line 9326) already
acknowledges this exact hazard -- "source files on disk are newer than cached
Python modules in this process" -- and reloads ``socis_agent_constants`` after the
