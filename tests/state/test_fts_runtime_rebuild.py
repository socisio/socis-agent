...
import socis_agent_state_holders
import socis_agent_state_schema
...
class TestRuntimeFtsRebuild:
    def test_reap_candidates_exclude_uninspectable_holder_suspicions(
        ...
        assert _concrete_state_db_holder_pids(
            db_path,
            [
                (222, "uninspectable holder: python -m socis_cli.main serve --port 0"),
    ...
    @pytest.mark.parametrize(
        "argv",
        (
            ("journalctl", "-u", "socis-agent.service"),
            ("grep", "socis-agent", "/var/log/syslog"),
            (
                "/usr/sbin/tailscaled",
                "be-child",
                "ssh",
                "--cmd=python -m socis_cli.main gateway",
            ),
            ("tmux", "new-session", "/opt/socis-agent/.venv/bin/socis gateway"),
            ("python3", "/opt/socis-agent/tools/check_state.py"),
            ("socis-monitor", "gateway"),
            ("socisctl", "serve"),
            ("python3", "worker.py", "socis_cli.main"),
            ("python3", "-m", "other.module", "socis_cli.main"),
            ("python3", "-c", "socis_cli.main"),
            ("python3", "-Icprint('socis_cli.main')", "socis_cli/main.py"),
        ),
    )
    def test_uninspectable_non_socis_process_is_not_a_holder(self, argv):
        assert not socis_agent_state_holders._looks_like_socis(argv)

    @pytest.mark.parametrize(
        "argv",
        (
            ("/usr/local/bin/socis", "gateway"),
            ("/usr/local/bin/socis-agent", "serve"),
            ("/usr/local/bin/socis-acp", "--stdio"),
            ("/usr/bin/python3", "-m", "socis_cli.main", "gateway"),
            ("/usr/bin/python3", "-m", "acp_adapter"),
            ("/usr/bin/python3", "-Im", "socis_cli.main", "gateway"),
            ("/usr/bin/python3", "-msocis_cli.main", "gateway"),
            ("/usr/bin/python3", "-W", "ignore", "-m", "socis_cli.main"),
            ("/usr/bin/python3", "-Xdev", "-m", "socis_cli.main"),
            (
                "/opt/socis-agent/.venv/bin/python",
                "/opt/socis-agent/socis_cli/main.py",
                "gateway",
            ),
            ("python.exe", "--", "socis_cli/main.py", "gateway"),
            ("python3", "/opt/socis-agent/run_agent.py", "--query", "hello"),
        ),
    )
    def test_uninspectable_socis_process_remains_a_holder(self, argv):
        assert socis_agent_state_holders._looks_like_socis(argv)
    ...
    def test_foreign_holder_detection_proc_readlink_deleted_wal(
        ...
        monkeypatch.setattr(socis_agent_state_holders.os, "getpid", lambda: 111)
        real_listdir = os.listdir
        def _listdir(path):
            if isinstance(path, str):
                path = path.replace("/proc", str(proc_root))
            return real_listdir(path)
        monkeypatch.setattr(socis_agent_state_holders.os, "listdir", _listdir)
        real_readlink = os.readlink
        def _readlink(path):
            path = path.replace("/proc", str(proc_root))
            return real_readlink(path)
        monkeypatch.setattr(socis_agent_state_holders.os, "readlink", _readlink)
        real_stat = os.stat
        ...
        monkeypatch.setattr(socis_agent_state_holders.os, "stat", _stat)

        holders = socis_agent_state_holders.foreign_state_db_holders(db_path)
    ...
    def test_foreign_holder_ignores_same_path_with_different_file_identity(
        ...
        assert socis_agent_state_holders.foreign_state_db_holders(db_path) == []
    ...
    def test_foreign_holder_uninspectable_process_cmdline_fallback(
        ...
        flagged when /proc/<pid>/cmdline identifies it as a SOCIS process."""
        db_path = tmp_path / "state.db"
        ...
        # PID 222's cmdline is world-readable and looks like SOCIS
        cmdline_path = proc_root / "222" / "cmdline"
        cmdline_path.write_bytes(
            b"python3\x00-m\x00socis_cli.main\x00chat\x00"
        )
        ...
        def _fake_argv(pid):
            ...
            try:
                ...
                return None
        monkeypatch.setattr(socis_agent_state_holders, "_read_proc_argv", _fake_argv)

        holders = socis_agent_state_holders.foreign_state_db_holders(db_path)
        # Should include PID 222 with the cmdline info
        assert len(holders) == 1
        assert holders[0][0] == 222
        assert "socis_cli.main" in holders[0][1]
    ...
    def test_structural_corruption_propagates_without_live_fts_mutation(
        ...
        from socis_agent_state import StateDbCorruptError
    ...
    def test_repeated_deferrals_reap_inactive_orphan_then_rebuild(
        ...
        monkeypatch.setattr(socis_agent_state_schema.time, "time", lambda: 120.0)
...
class TestPhysicalCorruptionAcceptance:
    ...
    def test_canonical_btree_corruption_fails_closed(
        ...
        try:
            caplog.clear()
            with caplog.at_level("WARNING", logger="socis_agent_state"):
                with pytest.raises(sqlite3.DatabaseError) as caught:
