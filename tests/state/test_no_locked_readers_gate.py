...
The gate parses ``socis_agent_state.py`` with ``ast`` and flags any method
...
``SessionDB`` itself is declared in ``socis_agent_state.py`` as
``class SessionDB(SessionSearchMixin, SessionSchemaMixin,
SessionPortabilityMixin)`` — its actual methods live across four files.
A gate that only opens ``socis_agent_state.py`` never sees a locked reader
...
_STATE_PY = _REPO_ROOT / "socis_agent_state.py"

# SessionDB's own class body lives in socis_agent_state.py; the rest of its
...
_ALL_STATE_SOURCES: list[tuple[Path, str]] = [
    (_STATE_PY, "SessionDB"),
    (_REPO_ROOT / "socis_agent_state_search.py", "SessionSearchMixin"),
    (_REPO_ROOT / "socis_agent_state_schema.py", "SessionSchemaMixin"),
    (_REPO_ROOT / "socis_agent_state_portability.py", "SessionPortabilityMixin"),
]
...
def _scan_all_state_sources() -> list[str]:
    ...
    to ``SessionDB`` — the class body in ``socis_agent_state.py`` plus each mixin
...
class TestNoPureReadersUnderWriterLock:
    ...
    def test_scan_all_state_sources_visits_every_mixin_file(self, tmp_path):
        ...
        reader planted in a MIXIN file (not socis_agent_state.py) must still be
