...
work on a file several SOCIS processes share (gateway service, the Desktop
app's `socis serve` backend, CLI sessions, the TUI slash worker):

* `socis_agent_state_common.fts_rebuild_admission` — full structural FTS rebuilds
* `socis_agent_state._cross_process_repair_lock`   — writable_schema surgery / VACUUM
...
from socis_agent_state import SessionDB, repair_state_db_schema
...
def test_repair_lock_fails_closed_when_lock_file_is_unopenable(tmp_path):
    ...
    with socis_agent_state._cross_process_repair_lock(db_path) as holding:
