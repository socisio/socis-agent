"""Tests for socis_agent_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every SOCIS entry point
(socis, socis-agent, socis-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every SOCIS entry point imports socis_agent_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported socis_agent_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("socis_agent_bootstrap", None)
    import socis_agent_bootstrap  # noqa: WPS433
    return socis_agent_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the SOCIS CLI
...
class TestEntryPointsImportBootstrap:
    """Every SOCIS entry point must import socis_agent_bootstrap as its
    ...
    # Entry points that invoke SOCIS as a process.  Each one must
    # import socis_agent_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "socis_cli/main.py",   # socis CLI (console_script)
        "run_agent.py",          # socis-agent (console_script)
        "acp_adapter/entry.py",  # socis-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import socis_agent_bootstrap' and that
        ...
        half-finished ``socis update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``socis_agent_bootstrap`` as a top-level module) leaves socis
        ...
        """
        # Resolve relative to the socis-agent repo root.  Tests live
        # at tests/test_socis_agent_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # socis start even when socis_agent_bootstrap hasn't been
        ...
        assert first_import_name == "socis_agent_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'socis_agent_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import socis_agent_bootstrap' line to be the first import."
        )


class TestHardenImportPath:
    """harden_import_path() must keep a same-named package in the launch
    directory from shadowing SOCIS's own top-level modules — covering both
    the relative ('' / '.') and absolute-path forms the cwd can take on
    sys.path (issue #51286)."""

    def _run(self, hb, path_seed, env=None):
        original = sys.path[:]
        original_env = os.environ.get("SOCIS_AGENT_PYTHON_SRC_ROOT")
        try:
            sys.path[:] = path_seed
            if env is not None:
                os.environ["SOCIS_AGENT_PYTHON_SRC_ROOT"] = env
            elif "SOCIS_AGENT_PYTHON_SRC_ROOT" in os.environ:
                del os.environ["SOCIS_AGENT_PYTHON_SRC_ROOT"]
            hb.harden_import_path(src_root="/opt/socis")
            return sys.path[:]
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("SOCIS_AGENT_PYTHON_SRC_ROOT", None)
            else:
                os.environ["SOCIS_AGENT_PYTHON_SRC_ROOT"] = original_env

    def test_relative_cwd_forms_removed(self):
        hb = _fresh_import()
        result = self._run(hb, ["", ".", "/opt/socis", "/usr/lib/python"])
    ...
    def test_absolute_cwd_path_loses_to_src_root(self):
        ...
        # SOCIS root.  The guard must relocate SOCIS to the front.
        hb = _fresh_import()
        result = self._run(hb, ["/home/user/tg-ws-proxy", "/opt/socis"])
        assert result[0] == "/opt/socis"
        # The cwd absolute path may still appear (it can hold legit deps),
        # but only AFTER the SOCIS root.
        assert result.index("/opt/socis") < result.index("/home/user/tg-ws-proxy")
    ...
    def test_env_var_used_when_no_arg(self):
        ...
        try:
            ...
            os.environ["SOCIS_AGENT_PYTHON_SRC_ROOT"] = "/env/socis"
            hb.harden_import_path()
            assert sys.path[0] == "/env/socis"
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("SOCIS_AGENT_PYTHON_SRC_ROOT", None)
