...
def test_recover_closes_owned_db_when_unexpected_exception_escapes(
    ...
    db = InterruptingDB()
    monkeypatch.setattr("socis_agent_state.get_shared_session_db", lambda: db)
    monkeypatch.setattr(
        "socis_agent_state.release_or_close", lambda _: setattr(db, "released", True)
    )
...
def test_get_flush_dir_uses_get_socis_agent_home(tmp_path, monkeypatch):
    """Flush dir must use get_socis_agent_home(), not hardcoded Path.home()."""
    import gateway.shutdown_flush as mod

    captured = {}

    def fake_get_socis_agent_home():
        from pathlib import Path
        captured["called"] = True
        return tmp_path

    monkeypatch.setattr(
        "socis_agent_constants.get_socis_agent_home", fake_get_socis_agent_home
    )
