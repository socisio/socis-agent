...
class TestNtfyRequirements:
    ...
    def test_returns_false_when_httpx_unavailable(self, monkeypatch):
        monkeypatch.setenv("NTFY_TOPIC", "socis-test")
...
class TestNtfyAdapterInit:
    ...
    def test_publish_topic_uses_extra_value(self):
        config = PlatformConfig(
            enabled=True,
            extra={"topic": "socis-in", "publish_topic": "socis-out"},
        )
        adapter = NtfyAdapter(config)
        assert adapter._publish_topic == "socis-out"
...
class TestConnect:
    ...
    def test_connect_starts_stream_task(self, monkeypatch):
        ...
        config = PlatformConfig(enabled=True, extra={"topic": "socis-test"})
        adapter = NtfyAdapter(config)
...
class TestSend:
    ...
    def _make_adapter(self, topic="socis-in", publish_topic="", token="", markdown=False):
    ...
    def test_send_posts_to_publish_topic(self):
        adapter = self._make_adapter(topic="socis-in", publish_topic="socis-out")
        ...
        result = _run(adapter.send("socis-in", "Hello ntfy!"))
        assert result.success is True
        assert result.message_id == "abc123"

        posted_url = mock_client.post.call_args[0][0]
        assert posted_url.endswith("/socis-out")

    def test_send_falls_back_to_subscribe_topic(self):
        adapter = self._make_adapter(topic="socis-in")
        ...
        result = _run(adapter.send("socis-in", "Hello!"))
        assert result.success is True
        posted_url = mock_client.post.call_args[0][0]
        assert posted_url.endswith("/socis-in")
    ...
    def test_send_uses_metadata_publish_topic(self):
        ...
        result = _run(adapter.send(
            "socis-in", "Hi!", metadata={"publish_topic": "override-out"}
        ))
        assert result.success is True
        posted_url = mock_client.post.call_args[0][0]
    ...
    def test_get_chat_info_returns_dict(self):
        adapter = NtfyAdapter(PlatformConfig(enabled=True, extra={"topic": "t"}))
        info = _run(adapter.get_chat_info("socis-in"))
        assert info["name"] == "socis-in"
        assert info["type"] == "dm"
...
class TestOnMessage:
    ...
    def _make_adapter(self):
        return NtfyAdapter(PlatformConfig(enabled=True, extra={"topic": "socis-in"}))
    ...
    def test_duplicate_message_skipped(self):
        ...
        adapter.set_message_handler(handler)
        event = {"id": "dup-1", "event": "message", "topic": "socis-in", "message": "hi", "time": None}
        _run(adapter._on_message(event))
        _run(adapter._on_message(event))
        assert len(calls) == 1
...
class TestEnvEnablement:
    ...
    def test_markdown_truthy_values(self, monkeypatch):
        monkeypatch.setenv("NTFY_TOPIC", "socis-in")
...
class TestStandaloneSend:
    ...
    def test_emits_echo_tag_header(self, monkeypatch):
        ...
        with patch.object(_ntfy, "httpx") as mock_httpx:
            ...
            _run(_standalone_send(pconfig, "socis-in", "hi"))
