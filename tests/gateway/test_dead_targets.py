...
def isolate(tmp_path, monkeypatch):
    monkeypatch.setattr("gateway.delivery.get_socis_agent_home", lambda: tmp_path)
    monkeypatch.setattr("gateway.dead_targets.get_socis_agent_home", lambda: tmp_path)
    return tmp_path
...
class TestDeadTargetRegistry:
    ...
    def test_persists_across_instances(self, isolate):
        ...
        # New instance reads the same on-disk store under tmp SOCIS_AGENT_HOME.
