...
class TestDefaultInteractionDispatch:
    ...
    async def test_update_prompt_click_writes_response_file(self, tmp_path, monkeypatch):
        """update_prompt:y click writes 'y' to ~/.socis-agent/.update_response."""
        adapter = self._make_adapter()
        socis_agent_home = tmp_path / "socis_agent_home"
        socis_agent_home.mkdir()
        monkeypatch.setattr(
            "socis_agent_constants.get_socis_agent_home",
            lambda: socis_agent_home,
        )
        ...
        response = socis_agent_home / ".update_response"
