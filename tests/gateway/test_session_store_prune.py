...
def test_session_store_default_db_uses_runtime_socis_agent_home(tmp_path, monkeypatch):
    """SessionStore must honor runtime SOCIS_AGENT_HOME when opening the default DB.

    Regression for the import-time DEFAULT_DB_PATH freeze: importing
    socis_agent_state before a fixture redirected SOCIS_AGENT_HOME used to pin every
    default SessionDB() at the developer's real ~/.socis-agent/state.db.
    """
    config = GatewayConfig(default_reset_policy=SessionResetPolicy(mode="none"))
    fake_home = tmp_path / "alt_socis_agent_home"
    fake_home.mkdir()
    monkeypatch.setenv("SOCIS_AGENT_HOME", str(fake_home))
...
class TestReadmeSentinel:
    ...
    def test_save_writes_readme_sentinel_first(self, tmp_path):
        ...
        assert "socis sessions list" in raw["_README"]
