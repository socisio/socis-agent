...
SOCIS_AGENT_MAX_ITERATIONS, but a later per-turn load_dotenv(..., override=True)
can restore a stale .env SOCIS_AGENT_MAX_ITERATIONS value before the next turn.
"""
...
def test_reload_runtime_env_preserves_config_terminal_backend(
    ...
    ``socis config get terminal.backend`` still says local.
    """
    socis_agent_home = tmp_path / ".socis-agent"
    socis_agent_home.mkdir()
    (socis_agent_home / "config.yaml").write_text(
    ...
    (socis_agent_home / ".env").write_text("TERMINAL_ENV=docker\n", encoding="utf-8")
