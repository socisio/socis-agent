...
- The original implementation hardcoded an allow-list of known gateway
  ...
  sources (``acp``, ``webhook``, user-defined ``SOCIS_AGENT_SESSION_SOURCE``
  ...
  but exist in .socis-agent/sessions."
...
def test_session_list_surfaces_all_user_facing_sources(monkeypatch):
    ...
    assert "custom-1" in ids, "custom SOCIS_AGENT_SESSION_SOURCE values were being hidden"
