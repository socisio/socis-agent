...
``gateway run --replace`` only kills same-SOCIS_AGENT_HOME PID-file holders.
...
def test_lock_conflict_names_owning_profile(adapter):
    ...
    existing = {
        ...
        "socis_agent_home": "/opt/data/profiles/lead-gen-outreach",
    }
    ...
    assert adapter._fatal_error_message == (
        ...
        "(socis --profile lead-gen-outreach gateway stop)."
    )
    assert adapter._fatal_error_retryable is True
    assert adapter._fatal_error_code == "telegram-bot-token_lock"


def test_lock_conflict_infers_profile_from_legacy_socis_agent_home(adapter):
    """Locks written before the profile field existed still attribute via socis_agent_home."""
    existing = {
        "pid": 559,
        "start_time": 123,
        "socis_agent_home": "/opt/data/profiles/lead-gen-outreach",
