...
closure the PR changed, against a real temp ``SOCIS_AGENT_HOME``.
"""
...
def _fake_switch_result():
    ...
    from socis_cli.model_switch import ModelSwitchResult
...
def _stub_picker_dependencies(monkeypatch):
    ...
    monkeypatch.setattr(
        "socis_cli.model_switch.list_picker_providers",
    ...
    monkeypatch.setattr(
        "socis_cli.model_switch.resolve_display_context_length",
        lambda *a, **k: 272000,
    )
...
def _setup_isolated_home(tmp_path, monkeypatch, model_yaml_value):
    ...
    monkeypatch.setattr(gateway_run, "_socis_agent_home", socis_agent_home)
    _stub_picker_dependencies(monkeypatch)
    # save_config writes to ``get_socis_agent_home() / config.yaml`` — point it here.
    monkeypatch.setattr("socis_agent_constants.get_socis_agent_home", lambda: socis_agent_home)
    monkeypatch.setattr("socis_cli.config.get_socis_agent_home", lambda: socis_agent_home)
    return cfg_path
...
async def test_multiplex_picker_global_persists_only_named_profile(
    ...
    monkeypatch.setattr(gateway_run, "_socis_agent_home", default_home)
    _stub_picker_dependencies(monkeypatch)
