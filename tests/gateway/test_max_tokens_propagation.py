...
Precedence verified here:
    SOCIS_AGENT_MAX_TOKENS env  >  model.max_tokens  >  per-provider
...
def isolated_home(tmp_path, monkeypatch):
    """Isolated SOCIS_AGENT_HOME with a writable config.yaml and a clean module cache.

    These tests deliberately re-import ``socis_cli`` / ``gateway`` so each
    ...
    monkeypatch.delenv("SOCIS_AGENT_MAX_TOKENS", raising=False)

    _saved = {
        k: v
        for k, v in sys.modules.items()
        if k.startswith(("socis_cli", "gateway"))
    }

    def write_cfg(body: str) -> None:
        (socis_agent_home / "config.yaml").write_text(textwrap.dedent(body))

    def fresh_gateway():
        for mod in list(sys.modules.keys()):
            if mod.startswith(("socis_cli", "gateway")):
                del sys.modules[mod]
