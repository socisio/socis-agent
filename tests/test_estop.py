"""Global emergency stop (`socis pause` / `socis resume`) — agent/estop.py.
...
already in flight is never touched. Removing the sentinel (`socis resume`)
...
def socis_agent_home(tmp_path, monkeypatch):
    """Point SOCIS_AGENT_HOME at a temp dir and reset estop module log state."""
...
def test_engage_creates_sentinel_and_is_engaged(socis_agent_home):
    assert estop.is_engaged() is False
    estop.engage()
    assert (socis_agent_home / "ESTOP").exists()
    assert estop.is_engaged() is True


def test_disengage_removes_sentinel(socis_agent_home):
    estop.engage()
    assert estop.disengage() is True
    assert not (socis_agent_home / "ESTOP").exists()
    assert estop.is_engaged() is False
    # Disengaging when not engaged is a no-op that reports False.
    assert estop.disengage() is False


def test_reason_and_timestamp_stored(socis_agent_home):
    ...
    raw = json.loads((socis_agent_home / "ESTOP").read_text(encoding="utf-8"))
    assert raw["reason"] == "runaway cron fan-out"


def test_get_state_none_when_disengaged(socis_agent_home):
    assert estop.get_state() is None


def test_corrupt_sentinel_still_engages(socis_agent_home):
    """A hand-touched/corrupt ESTOP file must still pause (fail safe)."""
    (socis_agent_home / "ESTOP").write_text("not json", encoding="utf-8")
    assert estop.is_engaged() is True
    state = estop.get_state()
    assert state is not None
...
def test_paused_reply_none_when_disengaged(socis_agent_home):
    assert estop.paused_reply() is None


def test_paused_reply_surfaces_reason_and_resume_hint(socis_agent_home):
    ...
    assert "socis resume" in notice


def test_paused_reply_without_reason(socis_agent_home):
    estop.engage()
    notice = estop.paused_reply()
    assert notice is not None
    assert "paused" in notice.lower()
...
def test_check_paused_logs_once_per_engagement(socis_agent_home, caplog):
...
def test_cron_tick_skips_dispatch_when_engaged(socis_agent_home, monkeypatch):
...
def test_cron_tick_resumes_after_disengage(socis_agent_home, monkeypatch):
    from cron import scheduler
...
def test_kanban_dispatch_blocked_when_engaged(socis_agent_home):
...
@pytest.mark.asyncio
async def test_gateway_new_turn_gets_paused_reply(socis_agent_home):
...
@pytest.mark.asyncio
async def test_gateway_internal_events_bypass_estop(socis_agent_home):
...
# ── CLI: socis pause / socis resume ───────────────────────────────────────


def test_cli_pause_engages_with_reason(socis_agent_home, capsys):
    from socis_cli.subcommands.pause import cmd_pause
...
def test_cli_pause_idempotent(socis_agent_home, capsys):
    from socis_cli.subcommands.pause import cmd_pause
...
def test_cli_resume_disengages(socis_agent_home, capsys):
    from socis_cli.subcommands.pause import cmd_pause, cmd_resume
...
def test_cli_resume_when_not_paused(socis_agent_home, capsys):
    from socis_cli.subcommands.pause import cmd_resume
...
def test_builtin_subcommands_include_pause_resume():
    from socis_cli.main import _BUILTIN_SUBCOMMANDS

    assert "pause" in _BUILTIN_SUBCOMMANDS
    assert "resume" in _BUILTIN_SUBCOMMANDS


# ── socis status surfacing ─────────────────────────────────────────────────


def test_status_line_when_paused(socis_agent_home):
    from socis_cli.status import _estop_status_line
...
def test_is_engaged_fails_safe_on_stat_error(socis_agent_home, monkeypatch):
    """A stat failure must report ENGAGED (fail safe) — the pause has to
    hold even when SOCIS_AGENT_HOME is misbehaving, matching the module's
...
@pytest.mark.asyncio
async def test_gateway_slash_commands_bypass_estop(socis_agent_home):
    ...
    try:
        ...
        return
    assert reply is None or "socis is paused" not in (reply or "").lower()
...
@pytest.mark.asyncio
async def test_gateway_pause_command_engages_and_resumes(socis_agent_home):
    from gateway.run import GatewayRunner
...
def test_pause_command_registered_for_gateway():
    from socis_cli.commands import GATEWAY_KNOWN_COMMANDS, resolve_command
...
def test_profile_gateway_honors_canonical_root_estop(tmp_path, monkeypatch):
    """fleet-analyst-class: SOCIS_AGENT_HOME is a profile dir; pause lives at root.

    A process launched with SOCIS_AGENT_HOME=~/.socis-agent/profiles/fleet-analyst must
    still treat ~/.socis-agent/ESTOP as engaged. Otherwise `socis pause` is not
