...
def _make_agent():
    socis_agent_home = Path(tempfile.mkdtemp(prefix="socis-test-home-"))
    (socis_agent_home / "logs").mkdir(parents=True, exist_ok=True)
    with (
        patch(
        ...
        patch("run_agent._socis_agent_home", socis_agent_home),
