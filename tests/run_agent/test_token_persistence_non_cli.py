...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    socis_agent_state = ModuleType("socis_agent_state")
    socis_agent_state.SessionDB = FakeSessionDB
    socis_agent_state.get_shared_session_db = lambda db_path=None: sentinel_db
    monkeypatch.setitem(sys.modules, "socis_agent_state", socis_agent_state)
    ...
    result = json.loads(agent._invoke_tool(
        "session_search",
        {"query": "SOCIS", "detail": "full"},
        "task-id",
    ))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "SOCIS"
...
def test_sequential_session_search_forwards_detail(monkeypatch):
    ...
    tool_call = SimpleNamespace(
        ...
        function=SimpleNamespace(
            name="session_search",
            arguments=json.dumps({"query": "SOCIS", "detail": "full"}),
        ),
    )
