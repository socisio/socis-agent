...
def test_root_session_no_compression(db):
    ...
    assert prov["currentSOCISSessionId"] == "root1"
    assert prov["rootSOCISSessionId"] == "root1"
    assert prov["parentSOCISSessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_socis_session_id="old"
    )
    assert prov["sessionKind"] == "continuation"
    assert prov["parentSOCISSessionId"] == "old"
    assert prov["rootSOCISSessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousSOCISSessionId"] == "old"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootSOCISSessionId"] == "p"  # lineage root still walked
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"socis"}
    assert "sessionProvenance" in meta["socis"]
    assert meta["socis"]["sessionProvenance"]["currentSOCISSessionId"] == "root1"
