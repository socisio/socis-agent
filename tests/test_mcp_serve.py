"""
Tests for mcp_serve — SOCIS MCP server.
...
def _isolate_socis_agent_home(tmp_path, monkeypatch):
    """Redirect SOCIS_AGENT_HOME to a temp directory."""
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking socis_agent_state schema."""
...
class TestEventBridgePollE2E:
    ...
    def test_poll_picks_up_new_conversation_on_db_change(
        ...
        # _poll_once reads <SOCIS_AGENT_HOME>/state.db for its mtime gate; the autouse
        # fixture points SOCIS_AGENT_HOME at tmp_path.
        db_path = tmp_path / "state.db"
