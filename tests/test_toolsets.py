...
class TestToolsetConsistency:
    ...
    def test_socis_platforms_share_core_tools(self):
        """All socis-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on socis-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["socis-cli", "socis-telegram", "socis-discord", "socis-whatsapp", "socis-slack", "socis-signal", "socis-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_socis_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("socis-whatsapp")
...
class TestResolveToolsetMemo:
    ...
    def test_second_resolution_is_cached(self, monkeypatch):
        ...
        first = resolve_toolset("socis-cli")
        second = resolve_toolset("socis-cli")
        ...
        assert (
            "socis-cli", True, registry_id, generation
    ...
    def test_memo_result_matches_fresh_resolution(self):
        ...
        toolsets_mod._resolve_toolset_memo.clear()
        first = resolve_toolset("socis-cli", include_registry=False)
        second = resolve_toolset("socis-cli", include_registry=False)
        assert first == second
