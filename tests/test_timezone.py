"""
Tests for timezone support (socis_agent_time module + integration points).
...
import socis_agent_time


def _reset_socis_agent_time_cache():
    """Reset the socis_agent_time module cache."""
    socis_agent_time.reset_cache()


# =========================================================================
# socis_agent_time.now() — core helper
# =========================================================================

class TestSOCISTimeNow:
    """Test the timezone-aware now() helper."""

    def setup_method(self):
        _reset_socis_agent_time_cache()

    def teardown_method(self):
        _reset_socis_agent_time_cache()
        os.environ.pop("SOCIS_AGENT_TIMEZONE", None)

    def test_valid_timezone_applies(self):
        """With a valid IANA timezone, now() returns time in that zone."""
        os.environ["SOCIS_AGENT_TIMEZONE"] = "Asia/Kolkata"
        result = socis_agent_time.now()
    ...
    def test_utc_timezone(self):
        """UTC timezone works."""
        os.environ["SOCIS_AGENT_TIMEZONE"] = "UTC"
        result = socis_agent_time.now()
        assert result.utcoffset() == timedelta(0)

    def test_us_eastern(self):
        """US/Eastern timezone works (DST-aware zone)."""
        os.environ["SOCIS_AGENT_TIMEZONE"] = "America/New_York"
        result = socis_agent_time.now()
        assert result.tzinfo is not None
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["SOCIS_AGENT_TIMEZONE"] = "Europe/London"
        tz = socis_agent_time.get_timezone()
        assert isinstance(tz, ZoneInfo)
        assert str(tz) == "Europe/London"

    def test_cache_isolated_by_active_profile_config(self, tmp_path, monkeypatch):
        """Switching SOCIS_AGENT_HOME must not reuse another profile's timezone."""
        ...
        monkeypatch.delenv("SOCIS_AGENT_TIMEZONE", raising=False)

        monkeypatch.setenv("SOCIS_AGENT_HOME", str(first_home))
        assert str(socis_agent_time.get_timezone()) == "Asia/Tokyo"

        # Multiplexed profile runtime scopes switch SOCIS_AGENT_HOME in one process.
        monkeypatch.setenv("SOCIS_AGENT_HOME", str(second_home))
        assert str(socis_agent_time.get_timezone()) == "America/New_York"
    ...
    def test_concurrent_profile_resolution_never_mixes_zones(
        ...
        def worker(key: str) -> None:
            barrier.wait()
            for _ in range(200):
                token = set_socis_agent_home_override(str(homes[key]))
                try:
                    tz = socis_agent_time.get_timezone()
                    if str(tz) != zones[key]:
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When SOCIS_AGENT_TIMEZONE is set, child process sees TZ env var.
        ...
        #   2) SOCIS_AGENT_TIMEZONE itself does NOT leak into the child env
        probe = (
            'import os; '
            'print("TZ=" + os.environ.get("TZ", "NOT_SET")); '
            'print("SOCIS_AGENT_TIMEZONE=" + os.environ.get("SOCIS_AGENT_TIMEZONE", "NOT_SET"))'
        )
        ...
        assert "SOCIS_AGENT_TIMEZONE=NOT_SET" in result["output"], (
            "SOCIS_AGENT_TIMEZONE should not leak into child env (only TZ)"
        )

    def test_tz_not_injected_when_empty(self):
        """When SOCIS_AGENT_TIMEZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("SOCIS_AGENT_TIMEZONE", None)
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        Regression: the old code used replace(tzinfo=socis_tz) which shifted
        absolute time when system-local tz != SOCIS tz.  The fix interprets
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when SOCIS tz
        ...
        # Use a SOCIS timezone far behind UTC so that the numeric wall time
        # of the naive timestamp exceeds _socis_now's wall time — this would
        # have caused a false "not due" with the old replace(tzinfo=...) approach.
        os.environ["SOCIS_AGENT_TIMEZONE"] = "Pacific/Midway"  # UTC-11
        _reset_socis_agent_time_cache()
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of SOCIS timezone"
        )
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["SOCIS_AGENT_TIMEZONE"] = "US/Eastern"
        _reset_socis_agent_time_cache()
