...
``socis_agent_time`` under the process's own startup profile, then ticks OTHER
profiles' cron stores via ``set_socis_agent_home_override()`` + ``use_cron_store()``.
...
def test_foreign_process_tick_persists_owning_profile_offset(
    ...
    monkeypatch.setenv("SOCIS_AGENT_HOME", str(backend_home))

    # Backend process resolves its own timezone first (process startup).
    assert socis_agent_time.now().utcoffset() == datetime.now(
        ZoneInfo("UTC")
    ).utcoffset()
    ...
    token = set_socis_agent_home_override(str(profile_a))
    try:
