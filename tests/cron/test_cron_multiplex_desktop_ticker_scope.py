...
def test_standalone_fallback_pool_keeps_profile_scope(tmp_path, monkeypatch):
    ...
    from socis_agent_constants import get_socis_agent_home, set_socis_agent_home_override
    ...
    async def _inside_running_loop():
        ...
        set_socis_agent_home_override(str(sec_home))
...
def test_multiplex_ticker_profile_gate_skips_rejected_profile(tmp_path):
    ...
    def _tick(*args, **kwargs):
        ticked.append(str(get_socis_agent_home()))
        if len(ticked) >= 3:
            stop.set()
        return 0
...
def test_desktop_ticker_gates_on_profile_gateway_running(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "socis_cli.profiles.profiles_to_serve", lambda multiplex=False: list(homes)
    )
    monkeypatch.setattr(
        "socis_cli.profiles._check_gateway_running", lambda home: home.name == "ops"
    )
    captured = {}
    ...
    monkeypatch.setattr("socis_agent_logging.enable_profile_log_routing", lambda homes: None)
