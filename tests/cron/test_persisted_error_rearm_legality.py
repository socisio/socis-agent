...
def cron_store(tmp_path, monkeypatch):
    ...
    (socis_agent_home / "cron").mkdir(parents=True)
    monkeypatch.setenv("SOCIS_AGENT_HOME", str(socis_agent_home))
    monkeypatch.setattr(J, "SOCIS_AGENT_DIR", socis_agent_home)
    monkeypatch.setattr(J, "CRON_DIR", socis_agent_home / "cron")
    monkeypatch.setattr(J, "JOBS_FILE", socis_agent_home / "cron" / "jobs.json")
    monkeypatch.setattr(J, "OUTPUT_DIR", socis_agent_home / "cron" / "output")
...
class TestCronRearmRespectsScheduleLegality:
    def test_weekday_job_errored_friday_does_not_fire_on_saturday(self, cron_store):
        ...
        with mock.patch.object(J, "_socis_now", lambda: now):
            due = J.get_due_jobs()
