...
def test_run_job_cron_execute_code_deny_does_not_pollute_later_gateway_execute_code(
    ...
    monkeypatch.setenv("SOCIS_AGENT_MODEL", "test-model")
    monkeypatch.setattr(approval_module, "_YOLO_MODE_FROZEN", False)
    monkeypatch.setattr(approval_module, "_get_approval_mode", lambda: "manual")
    monkeypatch.setattr(approval_module, "_get_cron_approval_mode", lambda: "deny")
    monkeypatch.setattr("socis_agent_state.get_shared_session_db", _DummySessionDB)
    monkeypatch.setattr("run_agent.AIAgent", _FakeCronAgent)
    monkeypatch.setattr(
        "socis_agent_constants.resolve_reasoning_config", lambda *_args, **_kwargs: None
    )
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(cron_scheduler, "_get_socis_agent_home", lambda: tmp_path)
    ...
    assert os.environ.get("SOCIS_AGENT_CRON_SESSION") is None
    ...
    assert get_session_env("SOCIS_AGENT_CRON_SESSION") == "1"
    monkeypatch.delenv("SOCIS_AGENT_CRON_SESSION")
