...
the shared /blueprint command handler (socis_cli/blueprint_cmd.py), and
the docs generator. Uses an isolated SOCIS_AGENT_HOME for anything that touches the
cron job store.
"""
...
class TestRenderers:
    ...
    def test_deeplink_shape(self):
        ...
        assert url.startswith("socis://blueprint/morning-brief?")
