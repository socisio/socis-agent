...
binary download are exercised manually by `socis secrets bitwarden
...
def socis_agent_home(tmp_path, monkeypatch):
    """Point SOCIS at an isolated home directory."""
    home = tmp_path / ".socis-agent"
    home.mkdir()
    monkeypatch.setenv("SOCIS_AGENT_HOME", str(home))
    # Some modules cache get_socis_agent_home; clear if needed.
    import socis_agent_constants
    if hasattr(socis_agent_constants, "_SOCIS_AGENT_HOME_CACHE"):
        socis_agent_constants._SOCIS_AGENT_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_happy_path(socis_agent_home, monkeypatch):
...
def test_env_loader_calls_bsm_when_enabled(tmp_path, monkeypatch):
    ...
    from socis_cli.env_loader import _apply_external_secret_sources
