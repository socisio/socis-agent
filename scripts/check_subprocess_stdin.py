...
When SOCIS runs in TUI mode, the gateway child process communicates with
...
# ``get_socis_agent_home() / "plugins"`` (user) and ``./.socis-agent/plugins/`` (project,
# gated behind ``SOCIS_AGENT_ENABLE_PROJECT_PLUGINS``) — see
# ``socis_cli/plugins.py:10-12``.  The guard only checked the bundled
...
# this script runs as a standalone subprocess — ``socis_agent_constants`` isn't
...
def main() -> int:
    ...
    # Add repo root to sys.path so we can import socis_agent_constants (this script
    ...
    # plugins in get_socis_agent_home()/plugins/ and project plugins in
    # ./.socis-agent/plugins/, where code like ori/hooks.py can spawn
    # subprocesses with inherited stdin — #67639).
    plugin_roots: list[Path] = [get_socis_agent_home() / "plugins"]
    if os.environ.get("SOCIS_AGENT_ENABLE_PROJECT_PLUGINS"):
        plugin_roots.append(Path.cwd() / ".socis-agent" / "plugins")
