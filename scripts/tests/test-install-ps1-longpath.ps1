...
function Invoke-Normalization {
    ...
    try {
        ...
        # its diagnostics are all `[socis] `-prefixed, so the two separate
        ...
        $stderr = ($raw | Where-Object { $_ -like '`[socis`]*' }) -join "`n"
        $stdout = ($raw | Where-Object { $_ -notlike '`[socis`]*' }) -join "`n"
    } finally {
    ...
    return @{
        ...
        SOCISHome = $(if ($paths) { $paths.socis_agent_home } else { $null })
...
$expectedInstallDir = "$($longRoot)${sep}AppData${sep}Local" + '\socis\socis-agent'
...
$result = Invoke-Normalization -Environment @{ TEMP = $shortTemp; TMP = $shortTemp } `
    -ExtraArgs @('-InstallDir', (Join-Path $shortProfile 'custom-socis'))
Assert-Equal -Expected (Join-Path $longRoot 'custom-socis') -Actual $result.InstallDir -Label "explicit -InstallDir keeps the caller's directory, on the long root"
