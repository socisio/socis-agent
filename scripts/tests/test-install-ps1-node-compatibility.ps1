...
# changing PATH, installing software, or touching the user's SOCIS home.

$repoRoot = Split-Path -Parent (Split-Path -Parent (Split-Path -Parent $MyInvocation.MyCommand.Path))
$installScript = Join-Path $repoRoot 'scripts\install.ps1'
$testRoot = Join-Path $env:TEMP ("socis-node-compatibility-test-" + [Guid]::NewGuid().ToString('N'))
$SOCISHome = Join-Path $testRoot 'home'
$InstallDir = Join-Path $testRoot 'missing-checkout'
. $installScript -SOCISHome $SOCISHome -InstallDir $InstallDir
