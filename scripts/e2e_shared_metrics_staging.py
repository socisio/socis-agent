...
SOCIS_AGENT_HOME so the operator's own telemetry state is untouched.
...
def main() -> int:
    scratch = Path(tempfile.mkdtemp(prefix="socis-telemetry-e2e-"))
    os.environ["SOCIS_AGENT_HOME"] = str(scratch)
    ...
    from socis_cli.observability.shared_metrics_sender import SharedMetricsSender
    ...
    for index, metric_count in ((0, 3), (1, 140)):
        ...
        payload = {
            "schema_version": "socis.shared_metrics.v2",
    ...
    print(f"scratch SOCIS_AGENT_HOME : {scratch}")
    ...
    print("  aws s3 ls --recursive "
          "s3://socis-agent-telemetry-staging-767397871023-us-west-2-an/raw/ "
          "| tail -20")
    return 0
