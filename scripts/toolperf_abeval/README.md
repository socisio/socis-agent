...
batch (tracker: [#77056](https://github.com/socisio/socis-agent/issues/77056)).
...
- **Two arms, one variable.** `baseline` and `fixes` runs differ ONLY by
  ...
  SOCIS home, same model, same tasks, same reps.
...
1. Create a dedicated SOCIS home with credentials for the models under test:
   ...
   integration at it; no SOCIS observability plugin needs to be enabled.
