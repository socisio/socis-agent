...
measured in the production session DB) through `socis chat` twice — once per
...
Environment:
  ...
  ABEVAL_HOME    SOCIS_AGENT_HOME for runs   (default: $ABEVAL_ROOT/home)
                 Must be a configured SOCIS home with credentials for the
...
(tracker: socisio/socis-agent#77056).
"""
import json
import os
import re
...
""".strip(),
            ...
            env.update({
                ...
                "SOCIS_AGENT_NEMO_RELAY_PLUGINS_TOML": str(relay_config),
            })
            q = TASKS[name].replace("{WORK}", str(work))
            t0 = time.time()
            try:
                p = subprocess.run(
                    [sys.executable, "-m", "socis_cli.main", "chat", "--query", q,
