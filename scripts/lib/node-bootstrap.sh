...
# Redirect a SOCIS-managed Node's `npm install -g` to the command link dir
# (already on PATH) instead of the default $SOCIS_AGENT_HOME/node/bin, which is off
...
_nb_configure_npm_prefix() {
    ...
    printf 'prefix=%s\n' "$(dirname "$_link_dir")" > "$SOCIS_AGENT_HOME/node/etc/npmrc"
}
...
_nb_npm_range() {
    if [ -n "${SOCIS_AGENT_NPM_TARGET_RANGE:-}" ]; then
        printf '%s\n' "$SOCIS_AGENT_NPM_TARGET_RANGE"
        return 0
    fi
...
# The Python side recovers through socis_cli/npm_engine.py; the installer path
...
_nb_ensure_bundled_npm_range() {
    local npm_bin="$SOCIS_AGENT_HOME/node/bin/npm"
    [ -x "$npm_bin" ] || return 0
    ...
    _nb_warn "Fix manually: npm install -g --prefix \"$SOCIS_AGENT_HOME/node\" npm@\"$range\""
    return 1
}
...
_nb_have_modern_node() {
    ...
    [ "$(_nb_node_major)" -ge "$SOCIS_AGENT_NODE_MIN_VERSION" ]
}
...
_nb_try_fnm() {
    ...
    _nb_log "fnm detected — installing Node $SOCIS_AGENT_NODE_TARGET_MAJOR..."
    eval "$(fnm env 2>/dev/null)" || true
    fnm install "$SOCIS_AGENT_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    fnm use     "$SOCIS_AGENT_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
...
_nb_try_proto() {
    ...
    _nb_log "proto detected — installing Node $SOCIS_AGENT_NODE_TARGET_MAJOR..."
    proto install node "$SOCIS_AGENT_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    _nb_have_modern_node || return 1
...
_nb_try_nvm() {
    ...
    _nb_log "nvm detected — installing Node $SOCIS_AGENT_NODE_TARGET_MAJOR..."
    nvm install "$SOCIS_AGENT_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    nvm use     "$SOCIS_AGENT_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    _nb_have_modern_node || return 1
...
_nb_try_brew() {
    ...
    brew install "node@${SOCIS_AGENT_NODE_TARGET_MAJOR}" >/dev/null 2>&1 \
        || brew install node >/dev/null 2>&1 \
        || return 1
    brew link --overwrite --force "node@${SOCIS_AGENT_NODE_TARGET_MAJOR}" >/dev/null 2>&1 || true
    _nb_have_modern_node || return 1
...
_nb_install_bundled_node() {
    ...
    local index_url="https://nodejs.org/dist/latest-v${SOCIS_AGENT_NODE_TARGET_MAJOR}.x/"
    local tarball
    tarball=$(curl -fsSL "$index_url" \
        | grep -oE "node-v${SOCIS_AGENT_NODE_TARGET_MAJOR}\.[0-9]+\.[0-9]+-${node_os}-${node_arch}\.tar\.xz" \
        | head -1)
    if [ -z "$tarball" ]; then
        tarball=$(curl -fsSL "$index_url" \
            | grep -oE "node-v${SOCIS_AGENT_NODE_TARGET_MAJOR}\.[0-9]+\.[0-9]+-${node_os}-${node_arch}\.tar\.gz" \
            | head -1)
    fi
    if [ -z "$tarball" ]; then
        _nb_warn "Could not resolve Node $SOCIS_AGENT_NODE_TARGET_MAJOR binary for $node_os-$node_arch"
        return 1
    fi
    ...
    _nb_log "Extracting to $SOCIS_AGENT_HOME/node/..."
    ...
    mv "$extracted" "$SOCIS_AGENT_HOME/node"
    rm -rf "$tmp"

    local _link_dir
    _link_dir="$(_nb_get_link_dir)"
    # SOCIS_AGENT_NODE_SKIP_LINKS=1: the caller only wants the private managed tree
    ...
    if [ "${SOCIS_AGENT_NODE_SKIP_LINKS:-0}" != "1" ]; then
        mkdir -p "$_link_dir"
        ln -sf "$SOCIS_AGENT_HOME/node/bin/node" "$_link_dir/node"
        ln -sf "$SOCIS_AGENT_HOME/node/bin/npm"  "$_link_dir/npm"
        ln -sf "$SOCIS_AGENT_HOME/node/bin/npx"  "$_link_dir/npx"
    fi
    ...
    _nb_have_modern_node || return 1
    _nb_ok "Node $(node --version) installed to $SOCIS_AGENT_HOME/node/"
...
# ---------------------------------------------------------------------------
# Heal a broken SOCIS-managed Node tree (partial upgrade / missing lib/)
# ---------------------------------------------------------------------------
...
_nb_managed_tool_broken() {
    ...
    local probe
    for probe in \
        "$SOCIS_AGENT_HOME/node/bin/$tool" \
        "$SOCIS_AGENT_HOME/node/${tool}.exe" \
        "$SOCIS_AGENT_HOME/node/$tool"; do
...
# The managed node runs but is below SOCIS_AGENT_NODE_TARGET_MAJOR — an old tree
...
_nb_managed_node_outdated() {
    ...
    for probe in "$SOCIS_AGENT_HOME/node/bin/node" "$SOCIS_AGENT_HOME/node/node"; do
        [ -x "$probe" ] || continue
        ver="$("$probe" --version 2>/dev/null)" || return 1
        major="${ver#v}"; major="${major%%.*}"
        case "$major" in ''|*[!0-9]*) return 1 ;; esac
        [ "$major" -lt "$SOCIS_AGENT_NODE_TARGET_MAJOR" ] && return 0
        return 1
    done
    return 1
}
...
# absent. Used by socis_agent_constants.find_socis_node_executable() and safe
# to call from install reruns.
heal_managed_node() {
    [ -d "$SOCIS_AGENT_HOME/node" ] || return 1
    if ! _nb_managed_node_needs_heal; then
        return 0
    fi
    _nb_log "SOCIS-managed Node is broken — redownloading to $SOCIS_AGENT_HOME/node/..."
    _nb_install_bundled_node
}
...
ensure_node() {
    ...
    if _nb_have_modern_node; then
        ...
        SOCIS_AGENT_NODE_AVAILABLE=true
        return 0
    fi

    if [ -x "$SOCIS_AGENT_HOME/node/bin/node" ]; then
        export PATH="$SOCIS_AGENT_HOME/node/bin:$PATH"
        if _nb_have_modern_node; then
            _nb_ok "Node $(node --version) found (SOCIS-managed)"
            SOCIS_AGENT_NODE_AVAILABLE=true
    ...
    _nb_try_fnm   && { SOCIS_AGENT_NODE_AVAILABLE=true; return 0; }
    _nb_try_proto && { SOCIS_AGENT_NODE_AVAILABLE=true; return 0; }
    _nb_try_nvm   && { SOCIS_AGENT_NODE_AVAILABLE=true; return 0; }

    # Platform package managers.
    _nb_try_termux_pkg && { SOCIS_AGENT_NODE_AVAILABLE=true; return 0; }
    _nb_try_brew       && { SOCIS_AGENT_NODE_AVAILABLE=true; return 0; }

    # Last resort: pinned nodejs.org tarball.
    _nb_install_bundled_node && { SOCIS_AGENT_NODE_AVAILABLE=true; return 0; }

    _nb_warn "Node.js install failed — TUI and browser tools will be unavailable."
    _nb_warn "Install manually: https://nodejs.org/en/download/  (or: \`brew install node\`, \`fnm install $SOCIS_AGENT_NODE_TARGET_MAJOR\`, etc.)"
    return 1
}
