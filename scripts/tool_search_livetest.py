#!/usr/bin/env python3
"""Live test harness for SOCIS Agent's Tool Search feature.
...
# Force-isolate the test environment BEFORE any socis imports.
ORIGINAL_HOME = os.environ.get("SOCIS_AGENT_HOME")
ORIGINAL_AUTH = Path.home() / ".socis-agent" / "auth.json"
...
def setup_isolated_home(enabled: bool, listing: str = "off",
    ...
    """Create a fresh ~/.socis-agent/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.socis-agent/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="socis_ts_live_"))
    socis_agent_home = home_dir / ".socis-agent"
    socis_agent_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, socis_agent_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".socis-agent" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, socis_agent_home / ".env")
        ...
        load_socis_dotenv(socis_agent_home=str(Path.home() / ".socis-agent"))
    ...
    (socis_agent_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
...
def reset_module_state():
    """Drop cached modules so the new SOCIS_AGENT_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "socis_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def run_one_scenario(scenario: Dict[str, Any], enabled: bool, out_dir: Path) -> Dict[str, Any]:
    ...
    os.environ["SOCIS_AGENT_HOME"] = str(home)
...
def main():
    ...
    # Restore original SOCIS_AGENT_HOME
    if ORIGINAL_HOME is not None:
        os.environ["SOCIS_AGENT_HOME"] = ORIGINAL_HOME
    else:
