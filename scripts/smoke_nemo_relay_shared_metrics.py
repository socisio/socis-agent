"""Run a real SOCIS CLI turn and validate the Relay shared-metrics output."""
...
def _resolve_socis_executable(socis_repo: Path) -> Path:
    for relative_path in (
        Path(".venv") / "bin" / "socis",
        Path(".venv") / "Scripts" / "socis.exe",
    ):
        candidate = socis_repo / relative_path
        if candidate.is_file():
            return candidate
    discovered = shutil.which("socis")
    if discovered:
        return Path(discovered)
    raise SystemExit(
        "SOCIS executable not found in the repository virtual environment "
        "or on PATH"
    )
...
def _arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path.cwd(),
        help="SOCIS source checkout containing .venv/bin/socis",
    )
    parser.add_argument(
        "--relay-python",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Directory for the isolated SOCIS_AGENT_HOME and captured output",
    )
...
def _validate_store(database_path: Path) -> list[dict[str, Any]]:
    ...
    if by_name["socis.client.active"] != [
        {
        ...
        raise AssertionError(
            f"Unexpected client-active counter: {by_name['socis.client.active']}"
        )
    [model] = by_name["socis.model_route.count"]
    expected_model = {
        "name": "socis.model_route.count",
        "dimensions": {
            "model": MODEL_CANARY,
            "provider": "custom",
        },
    ...
    if model != expected_model:
        raise AssertionError(
            f"Unexpected model counter: {by_name['socis.model_route.count']}"
        )
    expected_start = {
    ...
    if by_name["socis.task_run.started"] != [expected_start]:
        raise AssertionError(
            f"Unexpected task start: {by_name['socis.task_run.started']}"
        )
    [terminal] = by_name["socis.task_run.finished"]
    ...
    [tool] = by_name["socis.tool_call.count"]
    ...
    lifecycle = by_name["socis.skill.lifecycle.count"]
    expected_actions = {
        "archived",
        "created",
        "edited",
    ...
    loads = by_name["socis.skill.load.count"]
...
def _validate_packages(
    ...
    try:
        ...
        raise RuntimeError(
            "The SOCIS development environment requires jsonschema"
        ) from exc
    ...
    if metrics["socis.client.active"] != [
        {
            "name": "socis.client.active",
            "type": "counter",
            "dimensions": {},
        ...
        raise AssertionError(
            f"Unexpected client-active metric: {metrics['socis.client.active']}"
        )
    [model] = metrics["socis.model_route.count"]
    if model["dimensions"] != {
        "model": MODEL_CANARY,
        "provider": "custom",
    } or model["value"] != 2:
        raise AssertionError(
            f"Unexpected model metric: {metrics['socis.model_route.count']}"
        )
    [terminal] = metrics["socis.task_run.finished"]
    ...
    [tool] = metrics["socis.tool_call.count"]
    if (
        tool["dimensions"]
        != {
            "approval_outcome": "not_required",
    ...
    lifecycle = metrics["socis.skill.lifecycle.count"]
    ...
    loads = metrics["socis.skill.load.count"]
    if {
        (
...
def main() -> int:
    args = _arguments()
    socis_repo = args.socis_repo.resolve()
    relay_python = args.relay_python.resolve() if args.relay_python else None
    socis = _resolve_socis_executable(socis_repo)
    ...
    if args.output_dir:
        ...
        root = Path(tempfile.mkdtemp(prefix="socis-relay-shared-metrics-"))
    home = root / "socis-home"
    ...
    (root / "socis.stdout.txt").write_text(result.stdout, encoding="utf-8")
    (root / "socis.stderr.txt").write_text(result.stderr, encoding="utf-8")
    if result.returncode != 0:
        raise AssertionError(
            f"SOCIS exited with {result.returncode}\n"
            f"stdout:\n{result.stdout}\nstderr:\n{result.stderr}"
        )
    ...
    if PROMPT_CANARY not in json.dumps(request.get("messages", [])):
        raise AssertionError("SOCIS model request did not contain the prompt canary")
    ...
    if TOOL_CALL_CANARY not in follow_up or TOOL_RESULT_CANARY not in follow_up:
        raise AssertionError("SOCIS did not return the tool result to the model")
    if RESPONSE_CANARY not in result.stdout:
        raise AssertionError("SOCIS did not print the mock model response")
    ...
    skill_result = subprocess.run(
        [
            ...
            "-c",
            "\n".join([
                "from socis_cli.observability import relay_shared_metrics",
    ...
    package_paths, packages = _validate_packages(
        ...
        / "observability"
        / "schemas"
        / "socis.shared_metrics.v2.schema.json",
    )

    print("SOCIS -> NeMo Relay shared-metrics smoke test passed")
