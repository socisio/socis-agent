...
# Python-driven tool session (e.g. SOCIS terminal tool). A pre-set PYTHONPATH
...
if [ -n "${SOCIS_AGENT_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$SOCIS_AGENT_INSTALL_DIR"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   write \$SOCIS_AGENT_HOME/.no-bundled-skills so future"
            echo "                   'socis update' runs never inject bundled skills either"
            ...
            echo "  --include-desktop  Also build the desktop app (apps/desktop -> SOCIS.app)"
            echo "  --dir PATH     Installation directory"
            echo "                   default (non-root):  ~/.socis-agent/socis-agent"
            echo "                   default (root, Linux): /usr/local/lib/socis-agent"
            echo "  --socis-home PATH  Data directory (default: ~/.socis-agent, or \$SOCIS_AGENT_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, SOCIS installs the code under"
...
print_banner() {
    ...
    # SOCIS AGENT wordmark, matching the logo and the dashboard banner.
    # Falls back to the mark alone when the terminal is too narrow.
...
# intentional, but they leave the checkout dirty — which forces `socis update`
...
emit_manifest() {
    ...
    # install.ps1: the signed bootstrap installer (SOCIS-Setup) passes it so
    ...
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"System prerequisites","category":"runtime","needs_user_input":false},{"name":"repository","title":"Download SOCIS Agent","category":"runtime","needs_user_input":false},{"name":"venv","title":"Create Python virtual environment","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"Install Python dependencies","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"Install browser-tool dependencies","category":"runtime","needs_user_input":false},{"name":"path","title":"Install socis command","category":"runtime","needs_user_input":false},{"name":"config","title":"Prepare config and skills","category":"configuration","needs_user_input":false},{"name":"setup","title":"Configure API keys and settings","category":"configuration","needs_user_input":true},{"name":"gateway","title":"Configure gateway service","category":"configuration","needs_user_input":true},'"$desktop_stage"'{"name":"complete","title":"Finish install","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
# Decide where the repo checkout + venv live, and where the `socis` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $SOCIS_AGENT_HOME/socis-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $SOCIS_AGENT_HOME/socis-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/socis-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $SOCIS_AGENT_HOME/socis-agent — then preserve it)
#
# Always no-op when the user set --dir or $SOCIS_AGENT_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in SOCIS_AGENT_HOME.
    if is_termux; then
        INSTALL_DIR="$SOCIS_AGENT_HOME/socis-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$SOCIS_AGENT_HOME/socis-agent/.git" ]; then
            INSTALL_DIR="$SOCIS_AGENT_HOME/socis-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/socis-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/socis-agent"
        ...
        # /usr/local/bin/socis wrapper unable to exec the bad-interpreter venv
        # python.  See #21457.
        ...
        log_info "  Command: /usr/local/bin/socis"
        log_info "  Data:    $SOCIS_AGENT_HOME (unchanged)"
...
# Point a SOCIS-managed Node's `npm install -g` at a directory that is on
# PATH. npm's default global prefix for a bundled Node is the Node dir itself,
# so global package binaries land in $SOCIS_AGENT_HOME/node/bin — which is NOT on
...
# SOCIS's own global installs pass an explicit --prefix and are unaffected.
# Idempotent and a no-op when there is no SOCIS-managed npm, so calling it on
# every install run repairs pre-existing installs, not just fresh ones.
configure_managed_node_npm_prefix() {
    [ -x "$SOCIS_AGENT_HOME/node/bin/npm" ] || return 0
    local link_dir
    link_dir="$(get_command_link_dir)"
    mkdir -p "$SOCIS_AGENT_HOME/node/etc"
    printf 'prefix=%s\n' "$(dirname "$link_dir")" > "$SOCIS_AGENT_HOME/node/etc/npmrc"
}

get_socis_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/socis" ]; then
        echo "$link_dir/socis"
    else
        echo "socis"
    fi
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://agent.socis.io/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    # SOCIS owns its own uv at $SOCIS_AGENT_HOME/bin/uv.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (socis_cli/managed_uv.py) looks in the same
    # place, so install.sh and `socis update` stay in sync.
    ...
    log_info "Installing managed uv into $SOCIS_AGENT_HOME/bin ..."
    mkdir -p "$SOCIS_AGENT_HOME/bin"

    # Two-stage: download the installer, then run it.  Piping
    # `curl | sh` masks curl failures (sh exits 0 on empty stdin)
    # and conflates network errors with installer errors.
    local _uv_install_log _uv_installer
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/socis-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/socis-uv-installer.$$.sh")"
    ...
    # directly into $SOCIS_AGENT_HOME/bin instead of ~/.local/bin.
    if UV_UNMANAGED_INSTALL="$SOCIS_AGENT_HOME/bin" sh "$_uv_installer" >>"$_uv_install_log" 2>&1; then
        rm -f "$_uv_installer"
        if [ -x "$_managed_uv" ]; then
            UV_CMD="$_managed_uv"
        else
...
check_python() {
    if [ "$DISTRO" = "termux" ]; then
        ...
        # SOCIS currently declares requires-python >=3.11,<3.14.  Termux can
        ...
        log_error "Termux Python $PYTHON_FOUND_VERSION is not supported; SOCIS requires Python >=3.11,<3.14"
...
# replaced with the SOCIS-managed Node $NODE_VERSION.
...
check_node() {
    ...
    # Repair pre-existing SOCIS-managed installs where `npm install -g` lands
    ...
    if command -v node &> /dev/null && command -v npm &> /dev/null \
        ...
        log_warn "min-release-age-exclude) — installing SOCIS-managed Node $NODE_VERSION instead..."
        install_node
        return
    fi

    # Prefer a SOCIS-managed Node from a previous run over a too-old system one.
    if [ -x "$SOCIS_AGENT_HOME/node/bin/node" ] && [ -x "$SOCIS_AGENT_HOME/node/bin/npm" ] \
        && node_satisfies_build "$("$SOCIS_AGENT_HOME/node/bin/node" --version)"; then
        export PATH="$SOCIS_AGENT_HOME/node/bin:$PATH"
        log_success "Node.js $("$SOCIS_AGENT_HOME/node/bin/node" --version) found (SOCIS-managed)"
        HAS_NODE=true
        return 0
    fi

    if command -v node &> /dev/null && ! command -v npm &> /dev/null; then
        log_warn "node found but npm is not on PATH (stray node symlink?) — installing SOCIS-managed Node $NODE_VERSION LTS..."
    elif command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is unsupported (SOCIS requires Node 22.22+, 24.11+, or 26+) — installing SOCIS-managed Node $NODE_VERSION..."
...
# Download and adopt one Node release line (e.g. 26) into ~/.socis-agent/node/.
#
...
install_node_line() {
    ...
    log_info "Extracting to ~/.socis-agent/node/..."
    ...
    # Place into ~/.socis-agent/node/ and symlink binaries into the same bin dir
    # the socis command uses (get_command_link_dir): /usr/local/bin for root
    # FHS installs, $PREFIX/bin on Termux, ~/.local/bin otherwise.
    rm -rf "$SOCIS_AGENT_HOME/node"
    mkdir -p "$SOCIS_AGENT_HOME"
    mv "$extracted_dir" "$SOCIS_AGENT_HOME/node"
    rm -rf "$tmp_dir"
    ...
    ln -sf "$SOCIS_AGENT_HOME/node/bin/node" "$node_link_dir/node"
    ln -sf "$SOCIS_AGENT_HOME/node/bin/npm"  "$node_link_dir/npm"
    ln -sf "$SOCIS_AGENT_HOME/node/bin/npx"  "$node_link_dir/npx"
    ...
    local installed_ver
    if ! installed_ver=$("$SOCIS_AGENT_HOME/node/bin/node" --version 2>&1); then
    ...
    log_success "Node.js $installed_ver installed to ~/.socis-agent/node/"
    HAS_NODE=true
    return 0
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. SOCIS install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "SOCIS Agent itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                ...
                # index-level conflict state. Mirrors the `socis update` path
                # (#4735).
                ...
                local stash_name
                stash_name="socis-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            # cannot succeed — mirror ``socis update`` and reset to the
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if [ "$restore_ok" = "yes" ] && [ -z "$conflicted_files" ]; then
                        ...
                        log_warn "Review git diff / git status if SOCIS behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up socis command..."

    if [ "$USE_VENV" = true ]; then
        SOCIS_AGENT_BIN="$INSTALL_DIR/venv/bin/python"
        SOCIS_AGENT_ENTRYPOINT="$INSTALL_DIR/socis"
    else
        SOCIS_AGENT_BIN="$(which socis 2>/dev/null || echo "")"
        if [ -z "$SOCIS_AGENT_BIN" ]; then
            log_warn "socis not found on PATH after install"
            return 0
        fi
    fi

    # Verify the interpreter and the checked-in entrypoint needed by the launcher.
    if [ ! -x "$SOCIS_AGENT_BIN" ] || { [ "$USE_VENV" = true ] && [ ! -f "$SOCIS_AGENT_ENTRYPOINT" ]; }; then
        log_warn "SOCIS launcher prerequisites not found"
    ...
    # Create a user-facing shim for the socis command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $SOCIS_AGENT_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$SOCIS_AGENT_BIN"` self-recurse. (#21454)
    rm -f "$command_link_dir/socis"
    if [ "$USE_VENV" = true ]; then
        ...
        cat > "$command_link_dir/socis" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$SOCIS_AGENT_BIN" "$SOCIS_AGENT_ENTRYPOINT" "\$@"
EOF
    else
        cat > "$command_link_dir/socis" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$SOCIS_AGENT_BIN" "\$@"
EOF
    fi
    chmod +x "$command_link_dir/socis"
    log_success "Installed socis launcher → $command_link_display_dir/socis"

    # Also expose `socis-agent`. The `socis-agent` console script declared in
    ...
    rm -f "$command_link_dir/socis-agent"
    if [ "$USE_VENV" = true ]; then
        cat > "$command_link_dir/socis-agent" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$SOCIS_AGENT_BIN" "$INSTALL_DIR/run_agent.py" "\$@"
EOF
    else
        cat > "$command_link_dir/socis-agent" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$SOCIS_AGENT_BIN" run_agent.py "\$@"
EOF
    fi
    chmod +x "$command_link_dir/socis-agent"
    log_success "Installed socis-agent launcher → $command_link_display_dir/socis-agent"

    # Also expose `socis-acp`. ACP hosts (Zed, JetBrains, Buzz) resolve the
    # agent by command name on the login-shell PATH, and the `socis-acp`
    # console script lives inside the venv, which is not on that PATH. Without
    # this launcher those hosts report SOCIS as not installed. (#21454 applies
    # here too: clear the path first so `cat >` cannot follow an old symlink
    # into the venv and overwrite the console script.)
    rm -f "$command_link_dir/socis-acp"
    if [ "$USE_VENV" = true ]; then
        cat > "$command_link_dir/socis-acp" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$SOCIS_AGENT_BIN" "$SOCIS_AGENT_ENTRYPOINT" acp "\$@"
EOF
    else
        cat > "$command_link_dir/socis-acp" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$SOCIS_AGENT_BIN" acp "\$@"
EOF
    fi
    chmod +x "$command_link_dir/socis-acp"
    log_success "Installed socis-acp launcher → $command_link_display_dir/socis-acp"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "socis command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v socis' \
                >/dev/null 2>&1; then
        ...
        log_info "socis not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# SOCIS Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# SOCIS Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# SOCIS Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so socis works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.socis-agent directory structure (config at top level, code in subdir)
    mkdir -p "$SOCIS_AGENT_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.socis-agent/.env (top level, easy to find)
    if [ ! -f "$SOCIS_AGENT_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$SOCIS_AGENT_HOME/.env"
            log_success "Created ~/.socis-agent/.env from template"
        else
            touch "$SOCIS_AGENT_HOME/.env"
            log_success "Created ~/.socis-agent/.env"
        fi
    else
        log_info "~/.socis-agent/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$SOCIS_AGENT_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.socis-agent/config.yaml (top level, easy to find)
    if [ ! -f "$SOCIS_AGENT_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$SOCIS_AGENT_HOME/config.yaml"
            log_success "Created ~/.socis-agent/config.yaml from template"
        fi
    else
        log_info "~/.socis-agent/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file).
    # This MUST match DEFAULT_SOUL_MD in socis_cli/default_soul.py — the
    ...
    if [ ! -f "$SOCIS_AGENT_HOME/SOUL.md" ]; then
        cat > "$SOCIS_AGENT_HOME/SOUL.md" << 'SOUL_EOF'
...
SOUL_EOF
        log_success "Created ~/.socis-agent/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.socis-agent/"

    # Seed bundled skills into ~/.socis-agent/skills/ (manifest-based, one-time per skill)
    if [ "$NO_SKILLS" = true ]; then
        # Blank-slate install: write the opt-out marker and skip seeding.
        # skills_sync.py and `socis update` both honor this marker, so the
        ...
        printf '%s\n' \
            ...
            "Delete this file to re-enable sync on the next 'socis update'." \
            > "$SOCIS_AGENT_HOME/.no-bundled-skills" 2>/dev/null || true
        log_info "Skipping bundled skills (--no-skills). Wrote $SOCIS_AGENT_HOME/.no-bundled-skills"
        log_info "  Future 'socis update' runs will not inject bundled skills. Delete the marker to opt back in."
    else
        log_info "Syncing bundled skills to ~/.socis-agent/skills/ ..."
        if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
            log_success "Skills synced to ~/.socis-agent/skills/"
        else
            # Fallback: simple directory copy if Python sync fails
            if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$SOCIS_AGENT_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
                cp -r "$INSTALL_DIR/skills/"* "$SOCIS_AGENT_HOME/skills/" 2>/dev/null || true
                log_success "Skills copied to ~/.socis-agent/skills/"
            fi
        fi
    fi
}
...
strip_snap_browser_override() {
    ...
    local env_file="$SOCIS_AGENT_HOME/.env"
    ...
    local tmp
    tmp="$(mktemp)" || return 0
    if grep -Ev '^AGENT_BROWSER_EXECUTABLE_PATH=/snap/|^# SOCIS Agent browser tools' "$env_file" > "$tmp"; then
        mv "$tmp" "$env_file"
        log_warn "Removed stale Snap browser override (AGENT_BROWSER_EXECUTABLE_PATH=/snap/...) from $env_file"
        log_info "SOCIS will use the bundled Chromium instead."
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# SOCIS Agent browser tools — explicit browser override."
...
# install — the same closure `socis update` installs
# (socis_cli/main.py::_update_node_dependencies). Prebuilt/partial checkouts
...
install_node_deps() {
    ...
    # Keep the checkout clean so `socis update` doesn't autostash every run.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
install_browser_use_cli() {
    ...
    # `socis tools` can install it later.
    if [ "$SKIP_BROWSER" = true ]; then
    ...
    if [ -x "$SOCIS_AGENT_HOME/bin/browser-use" ]; then
        log_success "Browser Use CLI already installed"
        return 0
    fi

    log_info "Installing Browser Use CLI (default browser backend)..."
    # UV_TOOL_BIN_DIR keeps the binary inside SOCIS' managed bin dir, where
    # the browser tool resolves it — no reliance on the user's PATH.
    if run_with_timeout 600 env UV_NO_CONFIG=1 UV_TOOL_BIN_DIR="$SOCIS_AGENT_HOME/bin" \
        ...
        log_info "Install later with: $UV_CMD tool install browser-use  (or via 'socis tools')"
    fi
}
...
install_computer_use_driver() {
    ...
    # `socis tools`, the dashboard, or the desktop app — is a config flip,
    # not a surprise multi-minute binary fetch (the confusion this fixes:
    # users had to discover `socis computer-use install` on their own).
    ...
    # Same upstream installer `socis computer-use install` runs; time-boxed
    # so a stalled GitHub download can't hang the SOCIS install. The
    # upstream installer serializes with its own lock (600s stale window),
    # so give it a ceiling above that — matching SOCIS'
    ...
    if run_with_timeout 660 /bin/bash -c \
        ...
        >"$cua_log" 2>&1; then
        log_success "Computer Use driver installed (enable via 'socis tools' → Computer Use)"
    else
        log_warn "Computer Use driver install failed — it will install on demand when you enable the tool."
        log_info "Install later with: socis computer-use install"
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'socis setup' after install."
        return 0
    fi
    ...
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m socis_cli.main setup < /dev/tty
    else
        python -m socis_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$SOCIS_AGENT_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for SOCIS to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$SOCIS_AGENT_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'socis whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                SOCIS_AGENT_CMD="$(get_socis_command_path)"
                $SOCIS_AGENT_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'socis whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'socis gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $SOCIS_AGENT_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $SOCIS_AGENT_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: socis gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: socis gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $SOCIS_AGENT_CMD gateway > "$SOCIS_AGENT_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.socis-agent/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: socis gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: socis gateway"
    fi
}

write_bootstrap_marker() {
    # Writes $INSTALL_DIR/.socis-bootstrap-complete, which tells the SOCIS
    ...
    local marker_path="$INSTALL_DIR/.socis-bootstrap-complete"
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $SOCIS_AGENT_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $SOCIS_AGENT_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $SOCIS_AGENT_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}socis${NC}              Start chatting"
    echo -e "   ${GREEN}socis setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}socis config${NC}       View/edit configuration"
    echo -e "   ${GREEN}socis config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}socis gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}socis update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'socis' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'socis' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'socis' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$SOCIS_AGENT_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$SOCIS_AGENT_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$SOCIS_AGENT_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    if ! run_with_timeout "$NODE_DEPS_TIMEOUT" "$npm_bin" install -g --prefix "$SOCIS_AGENT_HOME/node" --silent --ignore-scripts \
...
# extract a tree MISSING the electron binary, so the `electron`->`SOCIS` rename
...
# _purge_electron_build_cache() used by `socis desktop`; install.sh was the only
...
install_desktop() {
    ...
    # supported Node lines and prepends the SOCIS-managed Node to PATH, so
    ...
    #         retry (matches install.ps1 / `socis desktop`),
    ...
    local app=""
    if [ "$OS" = "linux" ]; then
        if [ -x "$desktop_dir/release/linux-unpacked/SOCIS" ]; then
            app="$desktop_dir/release/linux-unpacked/SOCIS"
        elif [ -x "$desktop_dir/release/linux-unpacked/socis" ]; then
            app="$desktop_dir/release/linux-unpacked/socis"
        fi
    else
        local cand
        for cand in \
            "$desktop_dir/release/mac-arm64/SOCIS.app" \
            "$desktop_dir/release/mac/SOCIS.app"; do
            if [ -d "$cand" ]; then
                app="$cand"
                break
            fi
    ...
    # `socis desktop`, so install/repair and self-update agree about the app's
    ...
    # importing Python cannot reverse it by loading SOCIS_AGENT_HOME/.env. If the
    ...
    if [ "$OS" = "macos" ] && [ -z "${CSC_LINK:-}" ] && [ -z "${APPLE_SIGNING_IDENTITY:-}" ] && command -v codesign >/dev/null 2>&1; then
        ...
        if [ -x "$config_python" ]; then
            if SOCIS_AGENT_HOME="$SOCIS_AGENT_HOME" "$config_python" - "$desktop_dir" <<'PYEOF'
import sys
from pathlib import Path
from socis_cli.main import _desktop_macos_relaunchable_fixup
...
PYEOF
    ...
    # checkout stays clean for the next `socis update`.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            resolve_install_layout
            require_install_dir
            # Each stage runs in its own process, so the SOCIS-managed Node
            # provisioned during prerequisites/node-deps (at $SOCIS_AGENT_HOME/node/bin)
        ...
        complete)
            ...
            # $SOCIS_AGENT_HOME. $SOCIS_AGENT_HOME is a shared data dir (it can be
            # bind-mounted into a Docker gateway too), so a stamp there gets
            # clobbered by the container's 'docker' stamp and wrongly blocks
            # 'socis update' on this host install. See detect_install_method().
...
main() {
    ...
    # Code-scoped stamp: write next to the install tree, not into $SOCIS_AGENT_HOME.
    # $SOCIS_AGENT_HOME is a shared data dir (it can be bind-mounted into a Docker
    # gateway too), so a stamp there gets clobbered by the container's 'docker'
    # stamp and wrongly blocks 'socis update' on this host install.
