# Behavioral test for install.ps1's socis launcher staging (PR #92092,
...
#
# The test lifts the real Install-SOCISCommandLaunchers function from the
...
# the caller (Set-PathVariable passes $SOCISHome\bin -- the managed binary
...
$fn = $ast.Find({
    ...
    $n.Name -eq 'Install-SOCISCommandLaunchers'
}, $true)

if (-not $fn) {
    throw "Install-SOCISCommandLaunchers not found in $installPs1"
}
...
$caseRoot = [System.IO.Path]::GetFullPath((Join-Path $tempBase (
    'socis-cli-launcher-test-' + [guid]::NewGuid().ToString('N')
)))
...
try {
    $installRoot = Join-Path $caseRoot 'socis-agent'
    ...
    $missingThrew = $false
    try {
        Install-SOCISCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    } catch {
        $missingThrew = $_.Exception.Message -like '*required launcher not found*'
    }
    Assert-True $missingThrew 'missing socis.exe fails the launcher stage'
    ...
    $socisV1 = [byte[]](77, 90, 1)
    $socisV2 = [byte[]](77, 90, 2)
    $acp = [byte[]](77, 90, 3)
    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'socis.exe'), $socisV1)
    Set-Content -Path (Join-Path $installRoot 'venv\pyvenv.cfg') `
        -Value "home = X" -Encoding Ascii

    $staged = Install-SOCISCommandLaunchers -Root $installRoot -Destination $binDir
    Assert-True ($staged -eq $binDir) 'returns the destination it staged into'
    Assert-BytesEqual $socisV1 `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'socis.exe'))) `
        'normal venv: exe copy lands in the destination'
    Assert-True (-not (Test-Path -LiteralPath (Join-Path $binDir 'socis-acp.exe'))) `
        'optional ACP launcher may be absent'

    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'socis.exe'), $socisV2)
    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'socis-acp.exe'), $acp)
    Install-SOCISCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    Assert-BytesEqual $socisV2 `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'socis.exe'))) `
        'installer refreshes an existing SOCIS launcher'
    Assert-BytesEqual $acp `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'socis-acp.exe'))) `
    ...
    Install-SOCISCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    Assert-True (Test-Path -LiteralPath (Join-Path $binDir 'socis.cmd')) `
        'relocatable venv: .cmd delegator staged'
    Assert-True (-not (Test-Path -LiteralPath (Join-Path $binDir 'socis.exe'))) `
        'relocatable venv: stale exe copy removed'
    $cmdBody = [System.IO.File]::ReadAllText((Join-Path $binDir 'socis.cmd'))
    Assert-True ($cmdBody.Contains((Join-Path $scriptsDir 'socis.exe')) -and $cmdBody.Contains('%*')) `
