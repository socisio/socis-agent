...
# tree generates llms.txt — the index every LLM (SOCIS included, via the
# socis-agent skill) reads to learn what SOCIS can do — and
...
_MCP_CATALOG_FILES = {"socis_cli/mcp_catalog.py"}
...
# Rust crates — currently just the Tauri bootstrap installer (SOCIS-Setup).
...
def _py_test_only(p: str) -> bool:
    ...
    Product jobs (Desktop E2E's ``socis serve`` backend, the Docker image)
