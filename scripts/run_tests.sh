#!/usr/bin/env bash
# Canonical test runner for socis-agent. Run this instead of calling
...
#   * Proper venv activation (probes .venv, venv, then ~/.socis-agent/...)
#
# Usage:
...
# (SOCIS_AGENT_PYTHON is exported by the devShell hook and ships [dev] extras:
# pytest, pytest-asyncio, pytest-timeout, ruff, ty).
#
# A candidate must have pytest INSTALLED, not merely exist. The release venv
# at ~/.socis-agent/socis-agent/venv has bin/activate but no pytest, so an
...
VENV_PYTHON=""
SKIPPED_VENVS=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.socis-agent/socis-agent/venv"; do
...
if [ -n "$VENV" ]; then
  PYTHON="$VENV_PYTHON"
elif [ -n "${SOCIS_AGENT_PYTHON:-}" ] && [ -x "$SOCIS_AGENT_PYTHON" ] \
    && "$SOCIS_AGENT_PYTHON" -c 'import pytest' 2>/dev/null; then
  # Guard with an import check: SOCIS_AGENT_PYTHON may point at the RELEASE
  # venv (no pytest) when inherited from a wrapped `socis` binary rather
  # than the devShell hook.
  PYTHON="$SOCIS_AGENT_PYTHON"
  echo "▶ no local venv — using Nix dev venv via SOCIS_AGENT_PYTHON: $PYTHON"
else
  echo "error: no virtualenv with pytest found in $REPO_ROOT/.venv or $REPO_ROOT/venv," >&2
  echo "       and SOCIS_AGENT_PYTHON is not a python with pytest (enter the Nix devShell or create a venv)" >&2
  if [ -n "$SKIPPED_VENVS" ]; then
...
#
#   * SOCIS_AGENT_TEST_WORKERS / PATHS / FILE_TIMEOUT / FILE_RETRIES / SLICE are
#     read by run_tests_parallel.py at argparse-default time — inside the
#     stripped environment.
#   * SOCIS_AGENT_TEST_IMAGE is read by tests/docker/conftest.py to skip its
...
#     SOCIS_AGENT_GIT_SHA build-arg the workflow bakes in).
#
# These are test-infrastructure knobs, not credentials — same class as the
# SOCIS_AGENT_RUN_SLOW_PET_TESTS / SOCIS_AGENT_E2E_BROWSER opt-ins already forwarded.
# Keep this an explicit allowlist (no SOCIS_AGENT_TEST_* glob) so the "no
# credential can leak" property stays auditable at a glance.
TEST_ENV=()
for _test_var in SOCIS_AGENT_TEST_IMAGE SOCIS_AGENT_TEST_WORKERS SOCIS_AGENT_TEST_PATHS \
  SOCIS_AGENT_TEST_FILE_TIMEOUT SOCIS_AGENT_TEST_FILE_RETRIES SOCIS_AGENT_TEST_SLICE; do
  if [ -n "${!_test_var:-}" ]; then
...
exec env -i \
  ...
  PYTHONHASHSEED=0 \
  PYTHONUTF8=1 \
  ${SOCIS_AGENT_RUN_SLOW_PET_TESTS:+SOCIS_AGENT_RUN_SLOW_PET_TESTS="$SOCIS_AGENT_RUN_SLOW_PET_TESTS"} \
  ${SOCIS_AGENT_E2E_BROWSER:+SOCIS_AGENT_E2E_BROWSER="$SOCIS_AGENT_E2E_BROWSER"} \
