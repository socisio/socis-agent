...
unprivileged `socis` user, giving the layout most people have —
$SOCIS_AGENT_HOME/socis-agent plus a ~/.local/bin launcher. Pass --root for the FHS
...
Environment:
  SOCIS_AGENT_DEV_SANDBOX_DIR    Sandbox directory name, relative to the repo root
                            (default: .socis-sandbox).

Examples:
  # create a sandbox, install this branch as `main`, and then drop to a shell,
  # skipping `socis setup` & the browser tools for speed.
...
INSTALL_REF=""
UPSTREAM_URL="${SOCIS_AGENT_DEV_SANDBOX_UPSTREAM:-https://github.com/socisio/socis-agent.git}"
...
GIT_ROOT="${SOCIS_AGENT_SANDBOX_SOURCE_ROOT:-$(git rev-parse --show-toplevel)}"
...
SANDBOX_DIR_NAME="${SOCIS_AGENT_DEV_SANDBOX_DIR:-.socis-sandbox}"
...
if [ "$PERSISTENT" = true ]; then
  ...
  SANDBOX_ROOT="$(mktemp -d -t socis-sandbox.XXXXXX)"
...
if [ -n "$INSTALL_REF" ]; then
  ...
  UPSTREAM_REPO="$(mktemp -d -t socis-sandbox-upstream.XXXXXX)"
...
if [ "$INSTALL_SHORTCUT" = true ]; then
  mkdir -p "$SANDBOX_ROOT/root/http/agent.socis.io"
  if [ -n "$INSTALL_REF" ]; then
    git -C "$UPSTREAM_REPO" show "$UPSTREAM_COMMIT:scripts/install.sh" \
      > "$SANDBOX_ROOT/root/http/agent.socis.io/install.sh"
  else
    cp -a "$INSTALLER_PATH" "$SANDBOX_ROOT/root/http/agent.socis.io/install.sh"
  fi
  set -- bash -c '
    set +e
    curl -fsSL https://agent.socis.io/install.sh | bash -s -- "$@"
    install_status=$?
    if [ "$install_status" -eq 0 ] && [ -f /work/promote-main ]; then
      next_main=$(cat /work/promote-main)
      if git --git-dir=/work/repos/socis-agent.git update-ref refs/heads/main "$next_main"; then
...
# A user-level install writes the `socis` launcher to ~/.local/bin and the
# checkout to $SOCIS_AGENT_HOME; both live under the sandbox HOME, which is bound
...
SOURCE_REF="$COMMIT"
SNAPSHOT_REPO=""
FAKE_REPO="$SANDBOX_ROOT/root/repos/socis-agent.git"
git -C "$SANDBOX_ROOT/root/repos" init --bare -q socis-agent.git
if [ -n "$INSTALL_REF" ]; then
...
if [ -n "$(git -C "$GIT_ROOT" status --porcelain)" ]; then
  ...
  SNAPSHOT_REPO="$(mktemp -d -t socis-sandbox-snapshot.XXXXXX)"
  git -C "$SNAPSHOT_REPO" init -q
  git -C "$SNAPSHOT_REPO" fetch -q "$GIT_ROOT" "$COMMIT"
  git -C "$SNAPSHOT_REPO" config user.name 'SOCIS sandbox'
...
if [ -n "$INSTALL_REF" ]; then
  git --git-dir="$FAKE_REPO" fetch -q --force "$SOURCE_REPO" \
    "$SOURCE_REF:refs/socis-sandbox/next"
