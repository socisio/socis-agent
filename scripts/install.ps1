# ============================================================================
# SOCIS Agent Installer for Windows
# ============================================================================
...
param(
    ...
    [switch]$ForceCommit,
    [string]$Tag = "",
    [string]$SOCISHome = $(if ($env:SOCIS_AGENT_HOME) { $env:SOCIS_AGENT_HOME } else { "$env:LOCALAPPDATA\socis" }),
    [string]$InstallDir = $(if ($env:SOCIS_AGENT_HOME) { "$env:SOCIS_AGENT_HOME\socis-agent" } else { "$env:LOCALAPPDATA\socis\socis-agent" }),
    ...
    # builds apps/desktop into a launchable SOCIS.exe.
    #
    # Why opt-in:
    #   * SOCIS-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched SOCIS.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live SOCIS.exe
    ...
    #     `socis desktop` already builds on demand.
...
# SOCIS_AGENT_HOME and InstallDir -- in that short form:
...
function Write-PathDiag {
    ...
    [Console]::Error.WriteLine("[socis] $Message")
}
...
function ConvertTo-LongPath {
    ...
    try {
        if (-not ([System.Management.Automation.PSTypeName]'SOCISInstall.LongPath').Type) {
            Add-Type -Namespace 'SOCISInstall' -Name 'LongPath' -MemberDefinition @'
...
'@
        ...
        $length = [SOCISInstall.LongPath]::GetLongPathNameW($Path, $buffer, $buffer.Capacity)
...
# long. An explicitly passed -SOCISHome / -InstallDir is normalized in place
...
if ($PSBoundParameters.ContainsKey('SOCISHome')) {
    $SOCISHome = ConvertTo-LongPath $SOCISHome
} else {
    $SOCISHome = ConvertTo-LongPath $(
        if ($env:SOCIS_AGENT_HOME) { $env:SOCIS_AGENT_HOME } else { "$env:LOCALAPPDATA\socis" }
    )
}
if ($PSBoundParameters.ContainsKey('InstallDir')) {
    ...
    $InstallDir = ConvertTo-LongPath $(
        if ($env:SOCIS_AGENT_HOME) { "$env:SOCIS_AGENT_HOME\socis-agent" } else { "$env:LOCALAPPDATA\socis\socis-agent" }
    )
}
if ($script:NormalizedProfilePaths) {
    ...
    Write-PathDiag "resolved install paths: SOCISHome=$SOCISHome InstallDir=$InstallDir"
}
...
$RepoUrlSsh = "git@github.com:socisio/socis-agent.git"
$RepoUrlHttps = "https://github.com/socisio/socis-agent.git"
$PythonVersion = "3.11"
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $SOCISHome)) {
        New-Item -ItemType Directory -Force -Path $SOCISHome | Out-Null
    }
    $envFile = Join-Path $SOCISHome ".env"
...
function Install-AgentBrowser {
    ...
    # which every consumer (tools/browser_tool.py, `socis update`'s npx
    ...
    $prefixDir = Join-Path $SOCISHome "node"
...
function Install-Uv {
    # SOCIS owns its own uv at $SOCISHome\bin\uv.exe.  Always install there --
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (socis_cli/managed_uv.py) looks in the same
    # place, so install.ps1 and `socis update` stay in sync.
    $managedUv = Join-Path $SOCISHome "bin\uv.exe"
    ...
    Write-Info "Installing managed uv into $SOCISHome\bin ..."
    New-Item -ItemType Directory -Path (Join-Path $SOCISHome "bin") -Force | Out-Null

    # UV_INSTALL_DIR tells the astral installer to place the binary
    # directly into $SOCISHome\bin instead of ~/.local/bin.
    $prevEAP = $ErrorActionPreference
    try {
        $ErrorActionPreference = "Continue"
        $env:UV_INSTALL_DIR = Join-Path $SOCISHome "bin"
        ...
        # (socis_cli/managed_uv.py looks only at $SOCISHome\bin\uv.exe).
...
# Put the SOCIS-managed Node dir at the FRONT of the persisted User PATH.
#
...
# environment (a standalone socis-setup.exe run, a user typing `npm`) silently
...
# Upgrade the SOCIS-managed Node tree's bundled npm into $NpmRange when
...
function Update-ManagedNpm {
    ...
    if (Test-ManagedNodeInUse $NodeDir) {
        Write-Warn "SOCIS-managed Node.js is in use by a running app; skipping the bundled npm upgrade (applies on a later update with the app closed)."
        return $false
    }

    Write-Info "Upgrading bundled npm to satisfy $range ..."

    $tmpCwd = Join-Path $env:TEMP ("socis-npm-upgrade-" + [Guid]::NewGuid().ToString("N"))
...
function Resolve-UvCmd {
    ...
    # sibling process and SOCIS_AGENT_HOME wasn't propagated).
...
function Initialize-ManagedPythonEnvironment {
    # Python used by SOCIS belongs to the checkout, never to another
    # application or a user-level uv configuration. Keep this aligned with
    # socis_cli.managed_uv.managed_python_env(), which owns the update path.
    foreach ($name in @(
    ...
    $managedRoot = Join-Path $InstallDir ".socis-runtime\python"
...
function Resolve-AvailablePythonVersion {
    # Return the path and minor version of the first SOCIS-managed interpreter
    ...
    #
    # Under SOCIS-Setup.exe each stage runs in a fresh powershell.exe. The
    ...
    foreach ($ver in $candidates) {
        ...
        try {
            ...
            throw "Failed to resolve SOCIS-managed Python $ver`: $_"
        } finally {
...
function New-GitBashAslrFailureReason {
    ...
    return @(
        ...
        "Then rerun SOCIS setup. If the override is blocked or later re-applied, ask your Windows administrator to allow this per-program exception."
...
function Install-Git {
    ...
    which SOCIS uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\socis\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    SOCIS needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the SOCIS copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\socis\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``SOCIS_AGENT_GIT_BASH_PATH`` (User scope) so SOCIS can find it in a fresh
    ...
    if (Get-Command git -ErrorAction SilentlyContinue) {
        ...
        Write-Info "Trying a SOCIS-managed PortableGit install instead..."
    }

    # Download PortableGit into $SOCISHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $SOCISHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent SOCIS features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  SOCIS needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``SOCIS_AGENT_GIT_BASH_PATH`` (User env scope) so SOCIS can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $SOCISHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $SOCISHome\git\usr\bin\bash.exe
    $candidates += "$SOCISHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$SOCISHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("SOCIS_AGENT_GIT_BASH_PATH", $candidate, "User")
            $env:SOCIS_AGENT_GIT_BASH_PATH = $candidate
            $script:GitBashPath = $candidate
            Write-Info "Set SOCIS_AGENT_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- SOCIS may not find Git Bash."
    Write-Info "If needed, set SOCIS_AGENT_GIT_BASH_PATH manually to your bash.exe path."
}
...
# range used to provision the SOCIS-managed tree. Keeping this probe separate
...
function Test-SystemNodeReady {
    ...
    if (Test-NodeVersionOk $version) {
        ...
        Write-Warn "Node.js $version is unsupported (SOCIS requires Node 22.22+, 24.11+, or 26+)"
        return $false
    }
    ...
    if ($npmVersion) {
        Write-Warn "Node.js $version uses npm $npmVersion, which does not satisfy SOCIS requirement $npmRange"
    } else {
...
function Test-Node {
    ...
    Write-Info "Using a SOCIS-managed Node.js installation instead..."

    # Prefer a SOCIS-managed Node from a previous run over a too-old system one.
    $managedNode = "$SOCISHome\node\node.exe"
    if ((Test-Path $managedNode) -and (Test-NodeVersionOk (& $managedNode --version))) {
        $version = & $managedNode --version
        $env:Path = "$SOCISHome\node;$env:Path"
        Set-ManagedNodeFirstOnUserPath "$SOCISHome\node"
        Write-Success "Node.js $version found (SOCIS-managed)"
        ...
        Update-ManagedNpm "$SOCISHome\node" | Out-Null
        $script:HasNode = $true
        return $true
    }

    Write-Info "Installing SOCIS-managed Node.js $NodeVersion LTS..."
    ...
    # The portable zip path drops node.exe + npm into $SOCISHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $SOCISHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\socis-node-extract"
            ...
            if ($extractedDir) {
                ...
                Get-ChildItem "$SOCISHome" -Directory -Filter "node.old-*" -ErrorAction SilentlyContinue |
                    Where-Object { $_.LastWriteTime -lt (Get-Date).AddMinutes(-10) } |
                    Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
                Get-ChildItem "$SOCISHome" -Directory -Filter "node.new-*" -ErrorAction SilentlyContinue |
                    Where-Object { $_.LastWriteTime -lt (Get-Date).AddMinutes(-10) } |
                    Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
                ...
                if (Test-Path "$SOCISHome\node") {
                    try {
                        Rename-Item "$SOCISHome\node" $backup -ErrorAction Stop
                    } catch {
                        Write-Warn "SOCIS-managed Node.js is in use by a running app; deferring its upgrade. Close the app and re-run the update."
                ...
                $version = & "$SOCISHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $SOCISHome\node\ (portable, user-scoped)"
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\socis-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
function Install-Repository {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    ...
                    # the index conflict state is dropped. Mirrors the `socis
                    # update` path (#4735).
                    ...
                    $stashName = "socis-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
                ...
                if ($Commit) {
                    ...
                    # BACKWARDS. socis-setup.exe bakes its build-time commit
                    ...
                    # ff-only pull cannot succeed -- mirror ``socis update`` and
                ...
                if ($autostashRef) {
                    ...
                    if ($restoreNow) {
                        ...
                        if (($restoreExit -eq 0) -and ($conflictedFiles.Count -eq 0)) {
                            ...
                            Write-Warn "Review git diff / git status if SOCIS behaves unexpectedly."
                        } else {
            ...
            try {
                ...
                Write-Info "terminals, running socis processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/socisio/socis-agent/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/socisio/socis-agent/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/socisio/socis-agent/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                $zipPath = "$env:TEMP\socis-agent-$zipLabel.zip"
                $extractPath = "$env:TEMP\socis-agent-extract"
                ...
                if ($extractedDir) {
                    ...
                    # `socis update` (see the notes at the shared clone-path
    ...
    # next `socis update` checkout aborts on a "dirty" tree the user never
...
function Install-Venv {
    ...
    # Re-resolve the interpreter before creating the venv.  Under SOCIS-Setup.exe
    ...
    if (-not $resolvedPython) {
        throw "SOCIS-managed Python is unavailable. Run install.ps1 -Stage python first."
    }
    ...
    if (Test-Path -LiteralPath "venv") {
        ...
        # speedups.pyd) are loaded as DLLs by any running socis process.
        ...
        if ($env:OS -eq "Windows_NT") {
            $myPid = $PID
            Write-Info "Stopping any running socis processes before recreating venv..."
            ...
            try {
                schtasks /Query /FO CSV 2>$null | ConvertFrom-Csv | Where-Object { $_.TaskName -like '*SOCIS_Gateway*' } | ForEach-Object {
                    $tn = $_.TaskName
            ...
            # The launcher CLI (socis.exe) plus its child tree.
            & taskkill /F /T /IM socis.exe /FI "PID ne $myPid" 2>$null | Out-Null
            # taskkill /IM socis.exe is NOT enough: the gateway/agent that a
            # scheduled task or watchdog autostarts runs as
            # `pythonw.exe -m socis_cli.main gateway run` straight out of
            # venv\Scripts\, so its image name is python/pythonw, not socis.exe.
            ...
            # SOCIS children re-exec through .socis-runtime, so killing only
            ...
            # user (see socis_cli/gateway_windows.py), so the installer always
        ...
        try {
            ...
            throw (
                ...
                "A process still has the install directory open (often a non-SOCIS " +
    ...
    $env:UV_PYTHON = $venvPythonExe
        ...
        # the task fires normally on next logon and `socis update` / the
...
function Restore-VenvBackup {
    ...
    # venv so SOCIS (and the venv-blocker probe) stay usable (#83149).
    $backupName = Get-PendingVenvBackup
    if (-not $backupName) { return }
    try {
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `socis.exe` exists
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install socis-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and socis.exe broken with
    ...
    # here, not 30 seconds later when the user runs `socis`.
    if (-not $NoVenv) {
        $venvPython = "$InstallDir\venv\Scripts\python.exe"
        if (-not (Test-Path $venvPython)) {
            throw "Install reported success but $venvPython does not exist. The dependency sync likely landed in a sibling .venv\ directory. Re-run the installer; if it persists, close SOCIS processes and preserve existing venv directories before retrying. Do not delete venv in place."
        }
        ...
        if ($importExitCode -ne 0) {
            ...
            $hint = if (Test-Path $sibling) {
                "Detected sibling .venv\ at $sibling -- uv synced there instead of venv\. Close SOCIS processes, preserve the existing venv, and rerun the installer so the transactional recovery path can move directories safely."
            } else {
    ...
    Complete-VenvTransaction
        ...
        # a failed update leaves SOCIS and its blocker probe usable.
        Restore-VenvBackup
        Pop-Location
        throw
    }
    ...
    if (-not $NoVenv) {
        # uv on Windows can register socis.exe in dist-info/RECORD but fail to
        ...
        # `socis` command while socis-agent.exe / socis-acp.exe exist
...
"@ 2>$null
            if ($LASTEXITCODE -eq 0 -and $scriptNames) {
                ...
                if ($missing.Count -gt 0) {
                    ...
                    if ($stillMissing.Count -gt 0) {
                        ...
                        Write-Info "Workaround: `"$pythonExe`" -m socis_cli.main <command>"
                    } else {
    ...
    # users hit and lazy-import errors from `socis dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        try {
            & $pythonExe -m py_compile "$InstallDir\socis_cli\web_server.py" 2>&1 | Out-Null
            if ($LASTEXITCODE -eq 0) { $webServerSyntaxOk = $true }
        } catch { }
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `socis dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `socis dashboard` should now work."
            } else {
        ...
        if (-not $webServerSyntaxOk) {
            throw "dashboard backend source failed syntax check: socis_cli/web_server.py"
        }
    }
...
function Install-SOCISCommandLaunchers {
    param(
        [Parameter(Mandatory=$true)] [string]$Root,
        [Parameter(Mandatory=$true)] [string]$Destination
    )

    # Expose ONLY the socis launchers on PATH -- never the whole
    # venv\Scripts directory, which contains python.exe / pip.exe and
    # silently hijacks the `python` command in every terminal (#83797).
    # Requiring socis.exe before creating the destination keeps the PATH
    # stage from reporting success with an unusable command (PR #92092).
    $scriptsDir = Join-Path $Root "venv\Scripts"
    $requiredSource = Join-Path $scriptsDir "socis.exe"
    if (-not (Test-Path -LiteralPath $requiredSource -PathType Leaf)) {
        throw "Cannot set up the socis command: required launcher not found: $requiredSource"
    }

    New-Item -ItemType Directory -Force -Path $Destination | Out-Null

    # Launcher form depends on the venv (keep in lockstep with
    # socis_cli/_install_repair.py): a normal venv's exe trampoline
    ...
    foreach ($launcher in @("socis", "socis-acp")) {
    ...
    $requiredExe = Join-Path $Destination "socis.exe"
    $requiredCmd = Join-Path $Destination "socis.cmd"
    if (-not ((Test-Path -LiteralPath $requiredExe -PathType Leaf) -or
              (Test-Path -LiteralPath $requiredCmd -PathType Leaf))) {
        throw "Cannot set up the socis command: launcher was not installed: $requiredExe"
    }
    return $Destination
}

function Set-PathVariable {
    Write-Info "Setting up socis command..."
    
    if ($NoVenv) {
        $socisBin = "$InstallDir"
    } else {
        # $SOCISHome\bin is the managed binary dir (shared with the managed
        # uv), OUTSIDE the git checkout: `socis update`'s autostash
        ...
        # installer staged under socis-agent\bin. No git operation can ever
        # touch this dir. Staging and verification live in
        # Install-SOCISCommandLaunchers, which throws BEFORE any PATH
        # mutation when the launchers cannot be staged.
        $socisBin = "$SOCISHome\bin"
        Install-SOCISCommandLaunchers -Root $InstallDir -Destination $socisBin | Out-Null
    }
    ...
    #   socis-agent\bin -- lived inside the git checkout, where the update
    #                       autostash could sweep the launchers off disk
    # The socis-agent\bin FILES are left in place on purpose: editor/ACP
    ...
    if (-not $NoVenv) {
        ...
        if ($cleaned.Count -ne $items.Count) {
            ...
            Write-Info "Removed legacy launcher entries from user PATH (kept socis via $socisBin)"
        }
    }
    
    if ($currentPath -notlike "*$socisBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$socisBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $socisBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set SOCIS_AGENT_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\socis instead
    # of the Unix default ~/.socis-agent
    $currentSOCISHome = [Environment]::GetEnvironmentVariable("SOCIS_AGENT_HOME", "User")
    if (-not $currentSOCISHome -or $currentSOCISHome -ne $SOCISHome) {
        [Environment]::SetEnvironmentVariable("SOCIS_AGENT_HOME", $SOCISHome, "User")
        Write-Success "Set SOCIS_AGENT_HOME=$SOCISHome"
    }
    $env:SOCIS_AGENT_HOME = $SOCISHome
    
    # Update current session
    $env:Path = "$socisBin;$env:Path"
    
    Write-Success "socis command ready"
}

function Write-BootstrapMarker {
    # Writes $InstallDir\.socis-bootstrap-complete which tells the SOCIS
    ...
    $markerPath = Join-Path $InstallDir ".socis-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    # Create the SOCIS_AGENT_HOME directory structure ($SOCISHome, default %LOCALAPPDATA%\socis)
    New-Item -ItemType Directory -Force -Path "$SOCISHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SOCISHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SOCISHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SOCISHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SOCISHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SOCISHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SOCISHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SOCISHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$SOCISHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$SOCISHome\.env"
    ...
    # Create config.yaml
    $configPath = "$SOCISHome\config.yaml"
    ...
    # (the default PS5 behaviour), and SOCIS's prompt-injection scanner
    ...
    # PowerShell version.
    $soulPath = "$SOCISHome\SOUL.md"
    if (-not (Test-Path $soulPath)) {
        # MUST match DEFAULT_SOUL_MD in socis_cli/default_soul.py. The runtime
...
"@
    ...
    Write-Success "Configuration directory ready: $SOCISHome"
    
    # Seed bundled skills into $SOCISHome\skills (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to $SOCISHome\skills ..."
    ...
    if (Test-Path $pythonExe) {
        try {
            ...
            Write-Success "Skills synced to $SOCISHome\skills"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$SOCISHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to $SOCISHome\skills"
            }
        }
    }
}
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (SOCIS-Setup.exe runs each -Stage NAME
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'socis setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\socis-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\socis-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\socis-npm-tui-$(Get-Random).log"
...
# failure is non-fatal (browser_exec can still run via uvx, and `socis tools`
...
function Install-BrowserUseCli {
    ...
    $managedBin = Join-Path $SOCISHome "bin"
    $managedBu = Join-Path $managedBin "browser-use.exe"
    # MANAGED-FIRST: only SOCIS' managed copy short-circuits. A browser-use
    ...
    $ErrorActionPreference = "Continue"
    try {
        # UV_TOOL_BIN_DIR keeps the binary inside SOCIS' managed bin dir,
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-Info "Install later with: uv tool install browser-use  (or via 'socis tools')"
        }
    } catch {
...
# Provision it at install time so enabling the tool later -- via `socis
...
# paths still lazy-install via install_cua_driver() (socis_cli/tools_config)
...
function Install-CuaDriver {
    ...
    $ErrorActionPreference = "Continue"
    try {
        # Same upstream installer `socis computer-use install` runs. Bounded
        ...
        # SOCIS' _CUA_INSTALLER_TIMEOUT (660s).
        ...
        if (Wait-Job $job -Timeout 660) {
            ...
            if ($installedCuaDriver -and (Test-CuaDriverRuntimeContract -DriverPath $installedCuaDriver.Source)) {
                Write-Success "Computer Use driver installed (enable via 'socis tools' -> Computer Use)"
            } else {
                Write-Warn "Computer Use driver install did not produce a compatible runtime -- repair it before enabling the tool."
                Write-Info "Install later with: socis computer-use install"
            }
        } else {
...
# the final `electron` -> `SOCIS` rename dies with ENOENT and every re-run
...
function Install-Desktop {
    # Build apps/desktop into a launchable SOCIS.exe. Only called from
    ...
    #
    # The Tauri bootstrap installer's launch_socis_desktop command
    # resolves apps/desktop/release/win-unpacked/SOCIS.exe directly,
    ...
    # enforces the supported Node lines and prepends the SOCIS-managed Node to
    ...
    # The --dir mode produces an unpacked SOCIS.exe in
    ...
    # are NOT trying to work around). The SOCIS icon + product name are
    # stamped onto SOCIS.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $buildLog = "$env:TEMP\socis-desktop-build-$(Get-Random).log"
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\SOCIS.exe",
        "$desktopDir\release\win-arm64-unpacked\SOCIS.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no SOCIS.exe was found under $desktopDir\release\*-unpacked\"
    }

    # 3b. The SOCIS icon + identity are stamped onto SOCIS.exe by the
    ...
    #     SIDs under %LOCALAPPDATA% (electron/electron#51761, socis-agent#38216).
    ...
    #    SOCIS.exe. We deliberately do NOT point them at `socis desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'SOCIS.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'SOCIS.lnk')
        )
        ...
        # with the SOCIS icon, but without this the shortcut can keep drawing
...
function Install-PlatformSdks {
    ...
    # ~/.socis-agent/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'socis setup'."
        return
    }
    ...
    # Run socis setup using the venv Python directly (no activation needed)
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m socis_cli.main setup
    } else {
        python -m socis_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $socisCmd = "$InstallDir\venv\Scripts\socis.exe"
    if (-not (Test-Path $socisCmd)) {
        $socisCmd = "socis"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$SOCISHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'socis whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: socis gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$SOCISHome\logs\gateway.log"
            Start-Process -FilePath $socisCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$SOCISHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: socis gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: socis gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$SOCISHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$SOCISHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$SOCISHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$SOCISHome\socis-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   socis              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   socis setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   socis config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   socis config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   socis gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   socis update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning SOCIS repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
    # Managed Python lives under $InstallDir\.socis-runtime, so the checkout
...
if ($IncludeDesktop) {
    ...
    # (SOCIS-Setup.exe), never via the irm|iex CLI one-liner.
    $InstallStages += @{ Name = "desktop"; Title = "Building desktop app"; Category = "install"; NeedsUserInput = $false; Worker = "Stage-Desktop" }
}
$InstallStages += @(
    @{ Name = "path";             Title = "Adding SOCIS to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
function Write-SigmaBackendHint {
    <#
    .SYNOPSIS
    Install Sigma SIEM backends. sigma-cli ships with none.

    .DESCRIPTION
    Without a backend, `sigma convert -t splunk` fails with an error that never
    mentions why, so this installs them rather than printing instructions.

    There is no "install all": roughly 8 of the ~19 published backends are
    marked `Compatible? no` against current pySigma at any moment and
    `sigma plugin install` errors on them. That set moves with every pySigma
    release, so each install is allowed to fail independently.
    #>
    $backends = @(
    ...
    # cortexxdr is DELIBERATELY ABSENT. It installs cleanly then breaks the
    # sigma CLI outright: its pipeline imports ConditionTransformation from
    # sigma.processing.transformations, which pySigma 3.1.0 removed. Because
    # autodiscover imports every backend at startup, that one bad import
    # disables sigma for ALL backends. `sigma plugin list` reports it
    # Compatible? yes, so the metadata does not catch it. The rollback below
    # would recover, but there is no reason to install a known-broken backend
    # on every machine. Re-add once upstream updates it:
    #   https://github.com/SigmaHQ/pySigma-backend-cortexxdr

    if (-not (Get-Command sigma -ErrorAction SilentlyContinue)) {
        Write-Warn "sigma not on PATH - skipping backend install."
        return
    }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://agent.socis.io/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
