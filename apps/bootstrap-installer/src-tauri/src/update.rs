...
//!
//! Driven when the installer is launched as `SOCIS-Setup.exe --update` (see
//! `AppMode` in lib.rs). The desktop app hands off to us — it exits, then we:
//!
//!   1. wait for the old SOCIS desktop process to fully exit (so both the
//!      venv shim and packaged app.asar are free; otherwise `socis update`
//!      or repair bootstrap can race locked files),
//!   2. run `socis update --yes --gateway` (Python/repo update; this does NOT
//!      rebuild apps/desktop by design — see cmd_update in socis_cli/main.py),
//!   3. run `socis desktop --build-only` (the rebuild step update skips),
...
//!
//! Cross-platform note: `socis update` already handles macOS/Linux (git/pip).
//! The only OS-specific bits here are the venv shim path (resolve_socis) and
...
/// `socis update` exit code meaning "another socis process is holding the
/// venv shim open / dirty precondition" — see _cmd_update_impl in
/// socis_cli/main.py (sys.exit(2)). We surface a targeted message for this.
const UPDATE_EXIT_CONCURRENT: i32 = 2;

/// How long to wait for the old desktop process to release files under the
/// install tree before giving up and letting `socis update`'s own guard decide.
...
///
/// The marker is also the cross-process update lock: `socis update` claims
/// the same file (see `socis_cli/update_lock.py`) so a dashboard-spawned
...
/// overwriting — the pre-fix clobber is what let a dashboard `socis update`
...
/// UPDATE_MARKER_MAX_AGE_SECONDS in socis_cli/update_lock.py — all three read
...
/// a dashboard-spawned `socis update`) still blocks.
...
/// the marker name our PID — because a `socis update` child that
...
async fn run_update(app: AppHandle) -> Result<()> {
    let socis_agent_home = crate::paths::socis_agent_home();
    let install_root = socis_agent_home.join("socis-agent");
    ...
    // it, that backend re-locks the venv shim, our `force_kill_other_socis`
    ...
    //
    // The same marker is the cross-process update lock (socis_cli/
    // update_lock.py claims it too), so a live foreign owner means another
    // updater — most often a dashboard-spawned `socis update` — is already
    ...
    let _update_marker = match UpdateMarkerGuard::acquire(
        ...
        Err(owner) => {
            ...
            let msg = format!(
                "Another SOCIS update is already running (PID {}, started {} ago). \
    ...
    let socis = resolve_socis(&install_root).ok_or_else(|| {
        let msg = format!(
            "Could not find the socis CLI under {}. Is SOCIS installed? \
    ...
    // async on Windows. If it still holds the venv shim, `socis update`
    ...
    // ---- stage 2: socis update -----------------------------------------
    // Pass --branch so `socis update` targets the branch this installer was
    ...
    // without --branch, `socis update` switches the checkout to `main` (a
    ...
    // --force skips `socis update`'s Windows running-exe guard (which would
    ...
    // time `socis update` runs there is no legitimate socis.exe to protect,
    // and the guard would only produce a false "SOCIS is still running" stop.
    //
    ...
    // probe only checks the socis.exe shim and app.asar, so an external venv
    ...
    // below surfaces the correct "close all SOCIS windows" message.
    ...
    // Retry-once for the update-boundary crash. `socis update` lazily imports
    ...
    // second `socis update` runs clean because the now-current module is loaded
    ...
    // for the concurrent-instance guard (exit 2) — that's a "close SOCIS" state
    // a retry can't fix.
    ...
    // checkout predating the SOCIS_AGENT_UPDATE_HANDOFF_PID env fix (8c76fe19f)
    ...
    match update.exit_code {
        ...
        Some(code) if code == UPDATE_EXIT_CONCURRENT => {
            let msg = "SOCIS is still running. Close all SOCIS windows and try \
                       the update again."
                .to_string();
            emit_stage(
                &app,
        ...
        other => {
            let msg = format!(
                "socis update failed (exit {:?}). See {} for details.",
                other,
                crate::paths::socis_agent_home()
                    .join("logs")
                    .join("update.log")
                    .display()
            );
    ...
    // ---- stage 3: socis desktop --build-only ----------------------------
    // `socis update` deliberately does NOT build apps/desktop (it installs
    ...
    // retry-once `socis update` already does above, and `socis update`'s own
    ...
    if rebuild.exit_code != Some(0) {
        let msg = format!(
            "Rebuilding the desktop app failed (exit {:?}). The update was \
             applied but the app could not be rebuilt; run `socis desktop` \
    ...
    if let Some(target_app) = launch_target {
        if let Err(err) = launch_macos_app_and_exit(&app, &target_app).await {
            emit_log(
                ...
                None,
                LogStream::Stderr,
                &format!("[update] could not auto-launch desktop: {err}. Launch SOCIS manually."),
            );
        }
    } else if let Err(err) =
        crate::bootstrap::launch_socis_desktop(app.clone(), install_root.to_string_lossy().into_owned()).await
    {
...
pub(crate) async fn wait_for_install_locks_free(install_root: &Path, app: &AppHandle, stage: &str) {
    ...
    emit_log(app, Some(stage), LogStream::Stdout, "[handoff] waiting for SOCIS to exit…");
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            ...
            // SOCIS.exe, so an image-name kill would tear down the app itself.
            emit_log(
                app,
                Some(stage),
                LogStream::Stdout,
                &format!(
                    "[handoff] SOCIS still holding install files ({}); locating backend shims…",
                    format_locked_paths(&locked)
                ),
            );
            let shim = venv_socis(install_root);
...
fn install_lock_probe_paths(install_root: &Path) -> Vec<PathBuf> {
    let mut paths = vec![venv_socis(install_root)];
...
fn desktop_app_payload_paths(install_root: &Path) -> Vec<PathBuf> {
    ...
    if cfg!(target_os = "windows") {
        ...
        vec![
            release.join("mac").join("SOCIS.app").join("Contents").join("Resources").join("app.asar"),
            release.join("mac-arm64").join("SOCIS.app").join("Contents").join("Resources").join("app.asar"),
        ]
    } else {
...
/// Find processes running the exact `venv\Scripts\socis.exe` shim for this
/// installation. Windows image names are case-insensitive and the desktop is
/// also SOCIS.exe, so matching by image name alone is unsafe.
#[cfg(windows)]
...
/// Spawn `socis <args>` from `cwd`, stream stdout/stderr as Log events on the
...
async fn run_streamed(
    ...
    // the same rule about pipe EOF: `socis update` is precisely the shape that
...
/// Path to the venv socis shim under an install root, regardless of existence.
fn venv_socis(install_root: &Path) -> PathBuf {
    if cfg!(target_os = "windows") {
        install_root.join("venv").join("Scripts").join("socis.exe")
    } else {
        install_root.join("venv").join("bin").join("socis")
    }
}

/// Resolve the socis CLI to drive. Prefer the venv shim in the install we
/// just updated; fall back to `socis` on PATH.
fn resolve_socis(install_root: &Path) -> Option<PathBuf> {
    let shim = venv_socis(install_root);
    if shim.exists() {
        return Some(shim);
    }
    // PATH fallback. which-style probe via env, kept dependency-free.
    let exe = if cfg!(target_os = "windows") { "socis.exe" } else { "socis" };
...
fn update_child_env(install_root: &Path) -> Vec<(String, OsString)> {
    let socis_agent_home = crate::paths::socis_agent_home();
    let mut envs = vec![(
        "SOCIS_AGENT_HOME".to_string(),
        socis_agent_home.as_os_str().to_os_string(),
    )];
    // `socis update` is a Python CLI writing to a pipe here, so CPython
    ...
    // `socis update` child claims that SAME lock (socis_cli/update_lock.py).
    ...
    // refuses its parent's marker with exit 2 ("SOCIS is still running")
    // and no number of retries can ever succeed. Keep the variable name in
    // sync with HANDOFF_PID_ENV in socis_cli/update_lock.py.
    envs.push((
        "SOCIS_AGENT_UPDATE_HANDOFF_PID".to_string(),
        OsString::from(std::process::id().to_string()),
    ));
    if let Some(path) = path_with_prepended_entries(&[
        socis_agent_home.join("node").join("bin"),
...
async fn install_macos_app_update(
    ...
    let rebuilt_app = crate::bootstrap::resolve_socis_desktop_app(install_root).ok_or_else(|| {
        anyhow!(
            "desktop rebuild succeeded but no SOCIS.app was found under {}",
    ...
    let tmp = PathBuf::from(format!("{}.socis-update-new", target_app.display()));
    let old = PathBuf::from(format!("{}.socis-update-old", target_app.display()));
    ...
    let ditto = Command::new("/usr/bin/ditto")
        .arg(&rebuilt_app)
        .arg(&tmp)
        .current_dir(crate::paths::socis_agent_home())
        .status()
        .await
...
mod tests {
    ...
    #[test]
    fn venv_socis_is_under_install_root() {
        let root = Path::new("/x/socis-agent");
        let shim = venv_socis(root);
    ...
    fn update_child_env_forces_unbuffered_python() {
        let envs = update_child_env(Path::new("/x/socis-agent"));
        assert!(
            envs.iter()
    ...
    fn update_child_env_names_our_pid_for_the_lock_handoff() {
        let envs = update_child_env(Path::new("/x/socis-agent"));
        assert!(
            envs.iter().any(|(k, v)| k == "SOCIS_AGENT_UPDATE_HANDOFF_PID"
                && v.to_str() == Some(std::process::id().to_string().as_str())),
            "the socis update child claims the same marker we hold; without our pid \
    ...
    fn lock_probe_paths_include_desktop_app_payload() {
        ...
        assert!(
            probes.iter().any(|p| p == &venv_socis(root)),
    ...
    fn locked_paths_ignores_missing_payloads() {
        let root = Path::new("/nonexistent/socis-agent");
        let probes = install_lock_probe_paths(root);
    ...
    fn same_windows_path_accepts_case_only_difference() {
        assert!(same_windows_path(
            Path::new(r"C:\Users\tester\.socis\socis-agent\venv\scripts\SOCIS.EXE"),
            Path::new(r"c:\users\tester\.socis\socis-agent\venv\Scripts\socis.exe"),
        ));
    }
    ...
    fn same_windows_path_rejects_desktop_binary() {
        assert!(!same_windows_path(
            Path::new(r"C:\Users\tester\.socis\socis-agent\apps\desktop\SOCIS.exe"),
            Path::new(r"C:\Users\tester\.socis\socis-agent\venv\Scripts\socis.exe"),
        ));
    }
    ...
    fn update_marker_guard_writes_then_removes_on_drop() {
        ...
        let marker = dir.join(".socis-update-in-progress");
    ...
    fn acquire_refuses_while_a_live_updater_owns_the_marker() {
        ...
        // dashboard `socis update` and install-mode bootstrap mutate one tree
    ...
    fn acquire_adopts_a_marker_prewritten_with_our_own_pid() {
        // #74761: desktop writeUpdateMarker(socisHome, child.pid) races ahead
    ...
    // checkout's `socis update` reads it as a live foreign update and exits
    ...
    fn missing_or_garbage_marker_never_heals() {
        ...
        let garbage = dir.join(".socis-update-in-progress");
    ...
    fn heal_end_to_end_marker_lifecycle() {
        ...
        // own exit 2, e.g. a genuinely still-running SOCIS, stays terminal).
        assert!(!should_heal_self_marker_refusal(
            Some(UPDATE_EXIT_CONCURRENT),
            &marker
        ));
    ...
    fn parses_only_app_targets() {
        assert_eq!(
            target_app_from_args(["--update", "--target-app", "/Applications/SOCIS.app"]),
            Some(PathBuf::from("/Applications/SOCIS.app"))
        );
    ...
    async fn swap_installs_new_bundle_and_cleans_up() {
        ...
        let target = base.join("SOCIS.app");
        let tmp = base.join("SOCIS.app.socis-update-new");
        let old = base.join("SOCIS.app.socis-update-old");
    ...
    async fn swap_failure_never_leaves_target_missing() {
        ...
        let target = base.join("SOCIS.app");
        let tmp = base.join("SOCIS.app.socis-update-new"); // intentionally absent
        let old = base.join("SOCIS.app.socis-update-old");
        write_marker(&target, "OLD");
    ...
    async fn swap_rolls_back_when_install_step_fails() {
        ...
        let target = base.join("SOCIS.app");
        let tmp = base.join("SOCIS.app.socis-update-new"); // absent
        let old = base.join("SOCIS.app.socis-update-old");
        write_marker(&target, "OLD");
