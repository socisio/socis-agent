...
/// read side does not see EOF until the last of them closes it. `socis update`
...
/// stranded with it.
///
/// Same bound `Invoke-SOCISStep` grew in `scripts/desktop-update/windows.ps1`
...
///
/// `socis_agent_home_override` propagates to the child as $SOCIS_AGENT_HOME so the
...
pub async fn run_script(
    ...
    if let Some(cwd) = stable_script_cwd(script_path, socis_agent_home_override) {
        cmd.current_dir(cwd);
    }

    if let Some(home) = socis_agent_home_override {
        cmd.env("SOCIS_AGENT_HOME", home);
    }
...
fn stable_script_cwd<'a>(script_path: &'a Path, socis_agent_home_override: Option<&'a str>) -> Option<&'a Path> {
    if let Some(home) = socis_agent_home_override {
...
"#;
    ...
    #[test]
    fn stable_script_cwd_prefers_existing_socis_agent_home() {
    ...
    /// child. `socis update` is this shape -- the Electron build alone is
    /// megabytes.
    #[cfg(unix)]
    #[tokio::test]
