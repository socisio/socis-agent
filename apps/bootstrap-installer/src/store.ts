...
export const $socisHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  if (fake) {
    unlisten = () => {}
    $logPath.set('~/.socis-agent/logs/bootstrap-installer.log')
    $socisHome.set('~/.socis-agent')
  ...
  try {
    const [logPath, socisHome, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_socis_agent_home'),
      invoke<AppMode>('get_mode')
    ])

    $logPath.set(logPath)
    $socisHome.set(socisHome)
    $mode.set(mode)
  } catch (err) {
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch SOCIS" success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  ...
  // Update is driven by the desktop handing off (SOCIS-Setup.exe --update);
...
export async function launchSOCISDesktop(): Promise<void> {
  ...
  await invoke('launch_socis_desktop', { installRoot })
}
...
const FAKE_INSTALL_STAGES: FakeStage[] = [
  ...
  { name: 'repo', title: 'SOCIS repository' },
