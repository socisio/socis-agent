 ...
 * product name (`SOCIS`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/SOCIS'
 *
 * This is a hard failure with no obvious cause for the user — `socis desktop`
 ...
 * (the mac-unpacked SOCIS.app bundle) and Windows (win-unpacked), so we
 ...
 * SOCIS.exe that Windows can't load (truncated PE from a corrupt cached
 * Electron zip, wrong arch), the updater's integrity gate in
 * `socis desktop --build-only` (socis_cli/main.py
 ...
 */
export function preserveRollbackBackup(appOutDir, productExeName = 'SOCIS.exe') {
...
export default async function beforePack(context) {
  ...
  try {
    ...
    // rename fails.
    const productExe = `${(context && context.packager?.appInfo?.productFilename) || 'SOCIS'}.exe`
