...
//   · its own SOCIS_AGENT_HOME      → its own backend + sessions, no shared state.
//   · its own --remote-debugging-port → a private CDP endpoint.
//   · SOCIS_AGENT_DESKTOP_BOOT_FAKE=1 → deterministic boot overlay.
...
// Seed an isolated SOCIS_AGENT_HOME with just enough config (NOT sessions) so the
// spawned instance reaches an empty chat view instead of the onboarding wizard.
// A separate SOCIS_AGENT_HOME dir means a separate gateway lock — no collision with
...
export async function startIsolatedInstance({
  ...
  const home = socisHome ?? mkTemp('socis-perf-home-')
  const userData = userDataDir ?? mkTemp('socis-perf-ud-')
  const devUrl = prod ? null : `http://127.0.0.1:${devPort}`

  if (seedConfig && !socisHome) {
    seedConfigFrom(join(homedir(), '.socis-agent'), home)
  }
  ...
  try {
    ...
    // SOCIS_AGENT_HOME (backend + sessions). No DEV_SERVER env in prod → dist load.
    const electronBin = require('electron')
    // NB: do NOT set SOCIS_AGENT_DESKTOP_BOOT_FAKE here — it injects artificial
    ...
    const env = {
      ...
      SOCIS_AGENT_DESKTOP_CDP_PORT: String(port),
      XCURSOR_SIZE: '24'
    }

    if (devUrl) {
      env.SOCIS_AGENT_DESKTOP_DEV_SERVER = devUrl
    }
...
export async function coldStartSamples({ runs = 3, port = 9222, devPort = 5174, prod = false, warm = true } = {}) {
  ...
  if (warm) {
    ...
    const home = mkdtempSync(join(tmpdir(), 'socis-perf-cold-home-'))
    const userDataDir = mkdtempSync(join(tmpdir(), 'socis-perf-cold-ud-'))
    seedConfigFrom(join(homedir(), '.socis-agent'), home)
