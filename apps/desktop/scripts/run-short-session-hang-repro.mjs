...
const UPSTREAM_URL = 'https://github.com/socisio/socis-agent.git'
...
const LEGACY_STORAGE_KEY = 'socis.desktop.inflightTurnJournal.v1'
const LEGACY_MIGRATION_KEY = 'socis.desktop.inflightTurnJournal.v2.migrated'
...
async function verifyInteractiveSurfaces(cdp, timed, measure, label) {
  ...
  const version = await timed(`version.ipc.${label}`, () => cdp.eval('window.socisDesktop.getVersion()'))
...
async function withTemporarySandbox(label, body) {
  const sandbox = mkdtempSync(join(tmpdir(), `socis-short-session-${label}-`))
...
async function executeRunInSandboxAttempt(target, index, warmup, mock, output, sandbox, attempt) {
  ...
  const socisHome = join(sandbox, 'socis-home')
  const userData = join(sandbox, 'electron-user-data')
  const desktopLog = join(socisHome, 'logs', 'desktop.log')
  ...
  writeSandboxConfig(socisHome, mock.url)
  ...
  const child = spawn(
    ...
    [target.targetDesktop, `--user-data-dir=${userData}`, `--remote-debugging-port=${port}`],
    {
      ...
      env: sanitizedEnv({
        SOCIS_AGENT_DESKTOP_APP_NAME: `SOCISShortSession-${target.label}-${process.pid}-${index}-${warmup ? 'w' : 'm'}-${attempt}`,
        SOCIS_AGENT_DESKTOP_SOCIS_AGENT_ROOT: target.targetRoot,
        SOCIS_AGENT_DESKTOP_IGNORE_EXISTING: '1',
        SOCIS_AGENT_DESKTOP_USER_DATA_DIR: userData,
...
async function main() {
  ...
  const ephemeralRoot = mkdtempSync(join(tmpdir(), 'socis-short-session-ab-'))
  const prepared = []
