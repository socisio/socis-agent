#!/usr/bin/env node
// set-exe-identity.mjs — stamp the SOCIS icon + version metadata onto the
// built SOCIS.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked SOCIS.exe keeps the stock Electron icon and
...
// so EVERY packed build — first install, `socis desktop`, the installer's
...
// path (which rebuilds via `socis desktop --build-only`, never install.ps1)
// shipped a stock "Electron" exe. Keeping it in afterPack closes that gap.
//
// Also runnable standalone for ad-hoc re-stamping:
//   node scripts/set-exe-identity.mjs <path-to-SOCIS.exe>
//
...
// Stamp the SOCIS icon + identity onto `exe`. Resolves on success, throws on
...
async function stampExeIdentity(exe, desktopRoot = resolve(import.meta.dirname, '..')) {
  ...
  await rcedit(exe, {
    ...
    'version-string': {
      ProductName: 'SOCIS',
      FileDescription: 'SOCIS',
      CompanyName: 'SOCIS',
      LegalCopyright: 'Copyright (c) 2026 SOCIS'
    }
  })

  console.log('[set-exe-identity] done — SOCIS icon + identity stamped')
}
