...
// no longer bundles the SOCIS Agent Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'SOCIS.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'SOCIS'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'SOCIS.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'SOCIS'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
// Default SOCIS_AGENT_HOME for non-sandboxed runs -- matches main.ts's
// resolveSOCISHome(). On Windows it's %LOCALAPPDATA%\socis; elsewhere
// it's ~/.socis-agent. The fresh-install sandbox launchFresh() sets its own
// SOCIS_AGENT_HOME and never touches this.
const DEFAULT_SOCIS_AGENT_HOME = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'socis')
  }
  return path.join(os.homedir(), '.socis-agent')
})()
const VENV_ROOT = path.join(DEFAULT_SOCIS_AGENT_HOME, 'socis-agent', 'venv')
const FRESH_SANDBOX_ROOT = path.join(os.tmpdir(), 'socis-desktop-fresh-install')
...
function ensurePackagedApp() {
  if (process.env.SOCIS_AGENT_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `SOCIS-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `SOCIS-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `SOCIS-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'SOCIS-${version}-${os}-${arch}.${ext}'
...
function ensureDmg() {
  ...
  if (process.env.SOCIS_AGENT_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.SOCIS_AGENT_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
function launchFresh() {
  ...
  const socisHome = path.join(sandbox, 'socis-home')
  ...
  env.SOCIS_AGENT_DESKTOP_CWD = cwd
  env.SOCIS_AGENT_DESKTOP_IGNORE_EXISTING = '1'
  env.SOCIS_AGENT_DESKTOP_TEST_MODE = 'fresh-install'
  env.SOCIS_AGENT_DESKTOP_USER_DATA_DIR = userDataDir
  env.SOCIS_AGENT_HOME = socisHome
  delete env.SOCIS_AGENT_DESKTOP_SOCIS
  delete env.SOCIS_AGENT_DESKTOP_SOCIS_AGENT_ROOT
  ...
  console.log(`  SOCIS_AGENT_HOME: ${socisHome}`)
  console.log(`  cwd: ${cwd}`)

  return { runtimeRoot: path.join(socisHome, 'socis-agent', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The SOCIS Agent Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of socis_cli sneaks back in we want
  // to fail loudly rather than re-introduce the 400MB delta we just removed.
  const staleFactoryMarker = path.join(APP.resourcesPath, 'socis-agent', 'socis_cli', 'main.py')
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing SOCIS
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + SOCIS_AGENT_HOME
...
Fast rerun (skip rebuild if the packaged app already exists):
  SOCIS_AGENT_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
`)
}
