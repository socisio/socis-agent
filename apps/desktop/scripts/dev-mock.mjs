...
function createSandbox() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `socis-dev-mock-${Date.now()}`))
  ...
  return { root, socisHome, userDataDir, cleanup: () => fs.rmSync(root, { recursive: true, force: true }) }
}

function writeMockConfig(socisHome, mockUrl) {
  fs.writeFileSync(
    path.join(socisHome, 'config.yaml'),
...
`,
  ...
  fs.writeFileSync(path.join(socisHome, '.env'), 'MOCK_API_KEY=e2e-mock-key\n', 'utf8')
}
...
async function main() {
  ...
  writeMockConfig(sandbox.socisHome, mock.url)
  console.log(`  SOCIS_AGENT_HOME: ${sandbox.socisHome}`)
  ...
  const env = {
    ...
    SOCIS_AGENT_DESKTOP_APP_NAME: `SOCISDevMock-${Date.now()}`,
  }
