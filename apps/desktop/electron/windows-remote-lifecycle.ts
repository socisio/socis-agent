...
const READY_RE = /^SOCIS_AGENT_(?:BACKEND|DASHBOARD)_READY port=(\d+)/gm
...
async function probeWindowsRemote(ssh, explicitSOCISPath = '') {
  const explicit = psLiteral(explicitSOCISPath)
  ...
  const script = [
    ...
    '$socisHome=$env:SOCIS_AGENT_HOME',
    'if(-not $socisHome){$socisHome=Join-Path $env:LOCALAPPDATA "socis"}',
    'Assert-NoReparse $socisHome $true',
    '$candidate=[IO.Path]::Combine($socisHome, "socis-agent\\venv\\Scripts\\socis.exe")',
    ...
    '$profileCandidate=[IO.Path]::Combine($HOME, "socis-agent\\.venv\\Scripts\\socis.exe")',
    ...
    '$fallbackHomeCandidate=Join-Path $socisHome "socis-agent\\venv\\Scripts\\socis.exe"',
    '$fallbackProfileCandidate=Join-Path $HOME "socis-agent\\.venv\\Scripts\\socis.exe"',
    '$candidates=@()',
    'if($explicit){$candidates+=$explicit}',
    '$cmd=Get-Command socis.exe -ErrorAction SilentlyContinue',
    ...
    '$socis=$null',
    'foreach($candidate in $candidates){Assert-NoReparse $candidate $true;$candidatePython=[IO.Path]::Combine([IO.Path]::GetDirectoryName($candidate), "python.exe");Assert-NoReparse $candidatePython $true;try{$item=Get-Item -LiteralPath $candidate -Force -ErrorAction Stop;if(($item.Attributes -band [IO.FileAttributes]::ReparsePoint) -eq 0 -and -not $item.PSIsContainer){$socis=$item.FullName;break}}catch [Management.Automation.ItemNotFoundException]{continue}}',
    'if(-not $socis){throw "SOCIS is not installed on the remote Windows host."}',
    'Assert-NoReparse $socis $false',
    'if($explicit -and $socis -ne $explicit){throw "The configured SOCIS path is not an executable file."}',
    '$python=[IO.Path]::Combine([IO.Path]::GetDirectoryName($socis), "python.exe")',
    'Assert-NoReparse $python $false',
    '[ordered]@{os="Windows";arch=$env:PROCESSOR_ARCHITECTURE;socisHome=$socisHome;socisPath=$socis;python=$python}|ConvertTo-Json -Compress'
  ].join(';')

  return JSON.parse((await ssh.exec(powerShellCommand(script))).trim())
}

function windowsUpdateMarkerProbeCommand(socisHome) {
  const script = [
    '$ErrorActionPreference="Stop"',
...
public static class SOCISMarkerNoFollow {
...
`,
    ...
    '}',
    '}',
    `$home=${psLiteral(socisHome)}`,
    '$installRoot=$home',
    '$parent=Split-Path -Parent $home',
    'if((Split-Path -Leaf $parent) -ieq "profiles"){$installRoot=Split-Path -Parent $parent}',
    '$marker=Join-Path $installRoot ".socis-update-in-progress"',
    '$result="UNCERTAIN"',
    '$stream=$null;$memory=$null',
    'try{',
    'Assert-NoReparse $marker $true',
    'if(-not (Test-Path -LiteralPath $marker -PathType Leaf)){$result="CLEAR"}else{$stream=[SOCISMarkerNoFollow]::OpenRead($marker)',
 ...
 */
async function assertWindowsRemoteInstallUpdateClear(ssh, socisHome) {
  let observation = ''

  try {
    observation =
      String(await ssh.exec(windowsUpdateMarkerProbeCommand(socisHome)))
        .replace(/^\uFEFF/, '')
        .trim()
        .split(/\r?\n/)
        .pop() || ''
...
async function detectRemotePlatform(ssh, explicitSOCISPath = '') {
  try {
  ...
  try {
    return await probeWindowsRemote(ssh, explicitSOCISPath)
  } catch (cause: any) {
...
function helperCommand(runtime, operation, args = []) {
  const argv = [runtime.python, '-m', 'socis_cli.windows_ssh_runtime', operation, ...args]
...
function atomicWindowsSpawnCommand(runtime, reservation: any = {}) {
  const argv = [runtime.python, '-m', 'socis_cli.windows_ssh_runtime', 'spawn']
  const helper = operation => [runtime.python, '-m', 'socis_cli.windows_ssh_runtime', operation]

  const script = [
    '$ErrorActionPreference="Stop"',
    `$home=${psLiteral(runtime.socisHome)}`,
    '$installRoot=$home',
    '$parent=Split-Path -Parent $home',
    'if((Split-Path -Leaf $parent) -ieq "profiles"){$installRoot=Split-Path -Parent $parent}',
    '$marker=Join-Path $installRoot ".socis-update-in-progress"',
    ...
    reservation.ownershipId
      ...
      : '  if($LASTEXITCODE -ne 0){exit $LASTEXITCODE}',
    reservation.ownershipId
      ? `  $spawned=$spawnLines[-1]|ConvertFrom-Json; $lock=[ordered]@{schemaVersion=2;protocolVersion=1;ownershipId=${psLiteral(reservation.ownershipId)};spawnNonce=${psLiteral(reservation.spawnNonce)};pid=[int]$spawned.pid;creationTimeNs=[string]$spawned.creationTimeNs;port=0;profile=${psLiteral(reservation.profile)};socisPath=${psLiteral(reservation.socisPath)};socisHome=${psLiteral(reservation.socisHome)};tokenFingerprint=${psLiteral(reservation.tokenFingerprint)};startedAt=${psLiteral(reservation.startedAt)}}|ConvertTo-Json -Compress; ` +
...
function validLock(lock, ownershipId) {
  ...
  return Boolean(
    ...
    typeof lock.socisPath === 'string' &&
    typeof lock.socisHome === 'string'
  )
}
...
function reusableWindowsLock(lock, state, profile, reuseToken, runtime) {
  return Boolean(
    ...
    lock.socisPath === runtime.socisPath &&
    lock.socisHome === runtime.socisHome
  )
}
...
async function connectWindowsRemote(deps) {
  ...
  assertBootstrapNotSuperseded(signal)
  const runtime = await probeWindowsRemote(ssh, remoteSOCISPath)
  await assertWindowsRemoteInstallUpdateClear(ssh, runtime.socisHome)
  const inspection = await helper(ssh, runtime, 'inspect', [runtime.socisPath])

  if (!inspection.supported) {
    const error: any = new Error('Update SOCIS on the remote Windows host before connecting with Desktop SSH.')
    error.kind = 'update-required'
    throw error
  }

  runtime.socisPath = inspection.path
  const socisVersion = inspection.version || ''
  rememberLog(`[ssh-lifecycle] remote platform Windows/${runtime.arch}`)
  rememberLog(`[ssh-lifecycle] located socis at ${runtime.socisPath}`)
  ...
  try {
    ...
    spawned = await atomicWindowsSpawn(
      ssh,
      runtime,
      JSON.stringify({ ownershipId, spawnNonce, profile, socisPath: runtime.socisPath }),
      {
        ownershipId,
        spawnNonce,
        profile,
  ...
  try {
    ...
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForSOCIS(baseUrl, token)
    assertBootstrapNotSuperseded(signal)
...
function buildWindowsInteractiveCommand(remoteCwd = '') {
  ...
  script.push('$host.UI.RawUI.WindowTitle="SOCIS SSH"', 'powershell.exe -NoLogo')
