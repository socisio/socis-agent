...
export function resolveDesktopRemoteRoute({
  ...
  if (envUrl) {
    ...
    if (!envToken) {
      throw new Error(
        'SOCIS_AGENT_DESKTOP_REMOTE_URL is set but SOCIS_AGENT_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote SOCIS backend.'
      )
    }
  ...
  if (!modeIsRemoteLike(config.mode)) {
    ...
    // resolves local and spawns a loopback `socis serve` the desktop never
