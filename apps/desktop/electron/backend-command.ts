// Backend subcommand routing for the desktop-managed SOCIS process.
//
// The desktop app launches its own headless backend via `socis serve` — it
...
// app hasn't updated yet, or an older `socis` resolved from PATH) only knows
 ...
 * @param {string} [profile] optional SOCIS profile to pin via `--profile`.
 */
 ...
 * `-m socis_cli.main` and any `--profile <name>`). Returns a copy; if there is
...
/**
 * True when a runtime's `socis_cli/subcommands/dashboard.py` source registers
