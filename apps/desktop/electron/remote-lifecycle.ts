 ...
 * remote-lifecycle.ts
 *
 * Pure, electron-free remote SOCIS dashboard lifecycle over SSH for Desktop
 ...
 *
 *   - locate the SOCIS install on the remote (login-shell probe),
 ...
 *     `SOCIS_AGENT_DASHBOARD_READY port=<n>` readiness line,
 ...
 * the real SshConnection, fetch, adoptServedDashboardToken, and waitForSOCIS in.
 *
 * The minted SOCIS_AGENT_DASHBOARD_SESSION_TOKEN is the SPAWN credential. After
...
const READY_RE = /^SOCIS_AGENT_(?:BACKEND|DASHBOARD)_READY port=(\d+)/m
const REMOTE_LOCK_DIR = '~/.socis-agent/desktop-ssh'
...
// Resolve the remote socis executable. An EXPLICIT path is honored strictly
...
async function locateSOCIS(ssh, remoteSOCISPath) {
  const resolveLauncher = async (candidate: string) => {
    // Return the candidate path directly. The socis binary or wrapper script
    ...
    //     the SOCIS version, and
    //   - capability probing: `<python> serve --help` failed entirely.
    // See https://github.com/socisio/socis-agent/issues/74411
    return candidate
  }
  ...
  if (remoteSOCISPath) {
    if (await isExecutable(remoteSOCISPath)) {
      return resolveLauncher(remoteSOCISPath)
    }

    const err: any = new Error(
      `The SOCIS path you set is not an executable on the remote host: "${remoteSOCISPath}". ` +
        'Check the path (it must be the full path to the `socis` binary on the remote, e.g. ' +
        '~/socis-agent/.venv/bin/socis), or clear it to auto-detect.'
    )

    err.kind = 'socis-not-found'
    throw err
  }
  ...
  try {
    const found = (await ssh.exec(`bash -lc ${shq('command -v socis')}`)).trim()
  ...
  candidates.push('~/.local/bin/socis')
  candidates.push('/usr/local/bin/socis')
  candidates.push('~/.socis-agent/socis-agent/venv/bin/socis')
  ...
  const err: any = new Error(
    'SOCIS is not installed on the remote host (could not find a `socis` executable). ' +
      'Install it on the remote with:  curl -fsSL https://agent.socis.io/install.sh | sh  ' +
      '— or set the SOCIS path explicitly in the SSH connection settings.'
  )
...
// Probe the resolved binary's version string (first line of `<socis> --version`,
// e.g. "SOCIS Agent v0.18.2 ..."), or '' on failure. Surfaces WHICH socis a
// connection uses, so a stale/unexpected install is visible.
async function probeSOCISVersion(ssh, socisPath) {
  try {
    const out = (await ssh.exec(`${expandRemotePath(socisPath)} --version 2>&1`)).trim()
...
async function probeRemotePlatform(ssh) {
  ...
  if (!SUPPORTED_REMOTE_OS.has(osName)) {
    const err: any = new Error(
      `Unsupported remote platform "${osName || 'unknown'}". SOCIS Agent Desktop SSH mode supports Linux, macOS, and Windows remote hosts.`
    )
...
// The SOCIS_AGENT_HOME the remote dashboard will use (explicit env wins, else
// ~/.socis-agent). Recorded in the lockfile so a future reuse can tell it's the same
...
async function probeRemoteSOCISHome(ssh) {
  try {
    ...
    const error: any = new Error('Could not resolve the remote SOCIS home.')
...
marker=home/'.socis-update-in-progress'
try:
 ...
 * bytes; it never imports or executes code from the changing SOCIS checkout.
 ...
 */
async function assertRemoteInstallUpdateClear(ssh, socisHome) {
  const home = assertSafeRemoteHome(socisHome)
  let observation = ''
  ...
  try {
    ...
    const error: any = new Error('Could not prove that the remote SOCIS install is clear for SSH startup.')
  ...
  const error: any = new Error(
    live
      ? `Remote SOCIS update process ${live[1]} is still running; SSH startup is paused.`
      : 'The remote SOCIS update marker is unreadable or malformed; refusing SSH startup.'
  )
...
async function listRemoteSOCISProfiles(ssh) {
  const home = assertSafeRemoteHome(await probeRemoteSOCISHome(ssh))
  ...
  try {
    ...
    const error: any = new Error('Could not list remote SOCIS profiles.')
    error.kind = 'transient-transport-error'
    error.cause = cause
    throw error
  }
...
function assertSafeRemoteHome(home) {
  ...
  if (!/^(\/|~\/)[A-Za-z0-9._/+-]+$/.test(value) || value.includes('..')) {
    const error: any = new Error('Unsafe remote SOCIS home.')
...
async function readLockfile(ssh, ownershipId) {
  ...
  for (const field of ['profile', 'socisPath', 'socisHome', 'logPath', 'startedAt']) {
...
async function pidIsOurDashboard(
  ...
  if (!pid || !/^[0-9a-f]{16}$/.test(String(spawnNonce || '')) || !socisPath) {
    return false
  }
  ...
  try {
    const script =
      ...
      `pid=${Number(pid)}\n` +
      `expected=os.path.expanduser(${shq(socisPath)})\n` +
      // The installer-facing launcher is intentionally preserved for invocation
      // (#74411), but it may `exec python <install-dir>/socis`, leaving neither
      // launcher nor SOCIS_AGENT_HOME-derived entrypoint in argv. The ownership-scoped
      // token path + random nonce + exact profile below are the alternative proof.
      `socis_agent_home=os.path.expanduser(${shq(socisHome)}) if ${shq(socisHome)} else ""\n` +
      'expected_entries={expected}\n' +
      'if socis_agent_home:\n' +
      ' expected_entries.add(os.path.join(socis_agent_home,"socis-agent","venv","bin","socis"))\n' +
...
function buildOwnedStaleTerminationCommand(lock, ownershipId) {
  ...
  const expectedPath = expandRemotePath(lock.socisPath)
  const expectedHome = lock.socisHome ? expandRemotePath(lock.socisHome) : "''"
  ...
  const executableMatch = lock.socisHome
...
function lockMatchesManagedUpdateScope(lock, expected) {
  return Boolean(
    ...
    lock.socisPath === expected.socisPath &&
    lock.socisHome === expected.socisHome
  )
}
...
expected_path=os.path.expanduser(${py(lock.socisPath)})
socis_agent_home=os.path.expanduser(${py(lock.socisHome)})
expected_entries={expected_path,os.path.join(socis_agent_home,"socis-agent","venv","bin","socis")}
...
result=None
try:
 result=subprocess.run(["sh","-c",payload,"socis-update-mutex",str(fd)],pass_fds=(fd,),check=False)
finally:
 os.close(fd)
...
function buildSpawnCommand(socisPath, profile, opts: any = {}) {
  const socis = expandRemotePath(socisPath)
  ...
  const marker = expandRemotePath(`${remoteInstallRoot(opts.socisHome || '~/.socis-agent')}/.socis-update-in-progress`)

  const updateMutex = expandRemotePath(
    `${remoteInstallRoot(opts.socisHome || '~/.socis-agent')}/.socis-update-in-progress.mutex`
  )
  ...
  const dashCmd =
    ...
    `exec env SOCIS_AGENT_DESKTOP=1 ${socis} ${profileArgs}${subCmd}`

  const detachedShell = `eval "exec $1>&-"; ${dashCmd} </dev/null >> ${logPath} 2>&1 & echo $!`
  const detachedSpawn = `child=$("$(command -v setsid || echo nohup)" sh -c ${shq(detachedShell)} socis-update-child "$1" & echo $!)`
...
async function remoteSupportsSshOwnership(ssh, socisPath) {
  const socis = expandRemotePath(socisPath)

  const out = await ssh.exec(
    `help="$(${socis} serve --help 2>&1)"; ` +
...
async function spawnRemoteDashboard(
  ssh,
  { socisPath, profile, token, ownershipId, socisHome = '~/.socis-agent', assertInstallClear = async () => {} }
) {
  if (!(await remoteSupportsSshOwnership(ssh, socisPath))) {
    const err: any = new Error(
      'The remote SOCIS install does not support --ssh-session-token-file and --ssh-owner-nonce. ' +
        'Update SOCIS on the remote host to continue using Desktop SSH mode.'
    )
  ...
  try {
    ...
    out = await ssh.exec(
      buildSpawnCommand(socisPath, profile, {
        spawnNonce,
        tokenFilePath,
        logPath,
        socisHome,
 ...
 * opened SshConnection, forward/pickLocalPort/waitForSOCIS, a token-gated
...
async function connect(deps) {
  ...
  const socisHome = await probeRemoteSOCISHome(ssh)
  await assertRemoteInstallUpdateClear(ssh, socisHome)
  const socisPath = await locateSOCIS(ssh, remoteSOCISPath)
  log(`located socis at ${socisPath}`)
  const socisVersion = await probeSOCISVersion(ssh, socisPath)

  if (socisVersion) {
    log(`remote socis version: ${socisVersion}`)
  }
  ...
  if (isLockfileSkew(lock)) {
    ...
    const error: any = new Error(
      `The remote ownership record ${lpath} does not match this SOCIS Agent Desktop build (${lock.reason}). ` +
  ...
  if (lock) {
    ...
    const reusable =
      ...
      lock.socisPath === socisPath &&
      lock.socisHome === socisHome
  ...
  const spawned = await spawnRemoteDashboard(ssh, {
    ...
    ownershipId,
    socisHome,
    assertInstallClear: () => assertRemoteInstallUpdateClear(ssh, socisHome)
  })
  ...
  try {
    ...
    assertBootstrapNotSuperseded(signal)
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForSOCIS(baseUrl, spawnToken)
    assertBootstrapNotSuperseded(signal)
