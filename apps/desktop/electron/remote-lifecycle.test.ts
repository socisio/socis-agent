...
function ownedLock(over: any = {}) {
  return {
    ...
    port: 40000,
    profile: '',
    socisPath: '~/.local/bin/socis',
    socisHome: '~/.socis-agent',
...
function fakeSsh(rules: any[] = []) {
  ...
  return {
    ...
    async exec(cmd) {
      ...
      if (cmd.includes('.socis-update-in-progress') && !cmd.includes('marker_clear()') && !/setsid|nohup/.test(cmd)) {
        return 'CLEAR'
      }
...
test('POSIX relaunch gate refuses live and uncertain install markers without executing SOCIS', async () => {
...
test('POSIX relaunch gate permits absent/dead markers and normalizes named-profile homes install-wide', async () => {
  ...
  await assertRemoteInstallUpdateClear(ssh, '/home/alice/.socis-agent/profiles/research')
  assert.match(commands[0], /home\.parent\.name/)
  assert.match(commands[0], /profiles/)
  assert.match(commands[0], /\.socis-update-in-progress/)
})
...
test('listRemoteSOCISProfiles inventories Mini-style profile dirs without spawning a dashboard', async () => {
  ...
  assert.deepEqual(await listRemoteSOCISProfiles(ssh), ['default', 'bob', 'dixie', 'goose', 'rambo'])
  assert.equal(
...
test('listRemoteSOCISProfiles rejects a hostile SOCIS_AGENT_HOME', async () => {
  const ssh = fakeSsh([[/SOCIS_AGENT_HOME/, '/tmp/x; echo pwned\n']])

  await assert.rejects(
    () => listRemoteSOCISProfiles(ssh),
    (err: any) => {
...
test('locateSOCIS prefers the explicit profile path when executable', async () => {
  const ssh = fakeSsh([[/\[ -x .*\/opt\/socis/, 'OK']])
  assert.equal(await locateSOCIS(ssh, '/opt/socis'), '/opt/socis')
})

test('locateSOCIS throws (no silent fallback) when an EXPLICIT path is not executable', async () => {
  // command -v WOULD find a different install, but an explicit path must not
  // silently fall back to it — that is the "connected to the wrong socis" bug.
  const ssh = fakeSsh([
    [/command -v socis/, '/home/u/.local/bin/socis\n'],
    [/\[ -x .*\.local\/bin\/socis/, 'OK']
  ])

  await assert.rejects(
    () => locateSOCIS(ssh, '/bad/path/socis'),
    (err: any) => {
      assert.equal(err.kind, 'socis-not-found')
      assert.match(err.message, /\/bad\/path\/socis/)
...
test('locateSOCIS falls back to the login-shell command -v probe', async () => {
  const ssh = fakeSsh([
    [/command -v socis/, '/home/u/.local/bin/socis\n'],
    [/\[ -x .*\.local\/bin\/socis/, 'OK']
  ])

  assert.equal(await locateSOCIS(ssh, ''), '/home/u/.local/bin/socis')
})

test('locateSOCIS preserves an installer wrapper instead of resolving its interpreter', async () => {
  // install.sh venv mode writes: exec "$SOCIS_AGENT_BIN" "$SOCIS_AGENT_ENTRYPOINT" "$@",
  // where $SOCIS_AGENT_BIN is the venv python. The old canonicalization returned
  ...
  const ssh = fakeSsh([
    ...
    [/python3 -c/, '/home/u/.socis-agent/socis-agent/venv/bin/python\n']
  ])

  assert.equal(await locateSOCIS(ssh, ''), '/home/u/.local/bin/socis')
  assert.ok(
    !ssh.calls.some(cmd => cmd.includes('python3 -c')),
    'locateSOCIS must not shell out to a python3 parser to rewrite the launcher'
  )
})

test('locateSOCIS returns an explicit remoteSOCISPath unchanged', async () => {
  // The override half of #74411: an explicit remoteSOCISPath pointing at a
  // wrapper was also canonicalized to its interpreter, so overriding to
  // ~/.local/bin/socis changed nothing for affected users.
  const ssh = fakeSsh([
    [/\[ -x .*\.local\/bin\/socis/, 'OK'],
    [/python3 -c/, '/home/u/.socis-agent/socis-agent/venv/bin/python\n']
  ])

  assert.equal(await locateSOCIS(ssh, '~/.local/bin/socis'), '~/.local/bin/socis')
  assert.ok(!ssh.calls.some(cmd => cmd.includes('python3 -c')), 'an explicit remoteSOCISPath must never be rewritten')
})

test('locateSOCIS falls back to ~/.local/bin/socis when the login-shell probe misses', async () => {
  ...
  assert.equal(await locateSOCIS(ssh, ''), '~/.local/bin/socis')
})

test('locateSOCIS tries the conventional venv path last', async () => {
  const ssh = fakeSsh([[/\[ -x .*venv\/bin\/socis/, 'OK']])
  assert.equal(await locateSOCIS(ssh, ''), '~/.socis-agent/socis-agent/venv/bin/socis')
})

test('locateSOCIS throws a socis-not-found error with an install hint', async () => {
...
test('locateSOCIS uses a login shell for the command -v probe', async () => {
  const ssh = fakeSsh([
    [/command -v socis/, '/x/socis'],
    [/\[ -x/, 'OK']
  ])

  await locateSOCIS(ssh, '')
  assert.ok(
...
test('ownership paths are isolated by ownership ID and spawn nonce', () => {
  assert.equal(ownershipDirectory(OWNERSHIP_ID), `~/.socis-agent/desktop-ssh/${OWNERSHIP_ID}`)
  assert.equal(lockfilePath(OWNERSHIP_ID), `~/.socis-agent/desktop-ssh/${OWNERSHIP_ID}/backend.lock.json`)
  assert.equal(spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE), `~/.socis-agent/desktop-ssh/${OWNERSHIP_ID}/${SPAWN_NONCE}.log`)
})
...
test('metadata and process proof transport failures remain indeterminate', async () => {
  ...
  await assert.rejects(
    () => pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, failure]]), 5, SPAWN_NONCE, '/x/socis'),
    (error: any) => error.kind === 'transient-transport-error'
  )
})

test('pidIsOurDashboard requires the exact serve ownership nonce', async () => {
  const ours = `/x/socis serve --isolated --ssh-owner-nonce ${SPAWN_NONCE}`
  assert.equal(await pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, 'OWNED\n']]), 5, SPAWN_NONCE, '/x/socis'), true)
  assert.equal(
...
test.skipIf(process.platform === 'win32')(
  ...
  async () => {
    const temp = await mkdtemp(path.join(os.tmpdir(), 'socis wrapper ownership '))
    ...
    const entrypoint = path.join(installDir, 'socis')
    const launcher = path.join(temp, 'socis launcher')
    ...
    try {
      ...
      assert.equal(
        await pidIsOurDashboard(ssh, child.pid, SPAWN_NONCE, launcher, '/unrelated/socis-home', OWNERSHIP_ID, 'ops'),
        true
      )
      assert.equal(
        await pidIsOurDashboard(
...
test('buildSpawnCommand is headless serve, detached, token not in argv', () => {
  const cmd = buildSpawnCommand('/x/socis', 'work', { logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE) })
  ...
  assert.ok(!cmd.includes('SOCIS_AGENT_DASHBOARD_SESSION_TOKEN'), 'token env var must not appear')
})
...
test('buildSpawnCommand atomically reserves the ownership slot through spawn and lock publication', () => {
  const cmd = buildSpawnCommand('/x/socis', 'work', {
    socisHome: '~/.socis-agent',
    logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE),
    ownershipId: OWNERSHIP_ID,
  ...
  assert.match(
    cmd,
    /subprocess\.run\(\["sh","-c",payload,"socis-update-mutex",str\(fd\)\],pass_fds=\(fd,\),check=False\)/
  )
  assert.doesNotMatch(cmd, /os\.set_inheritable\(fd,True\)/)
  assert.match(cmd, /socis-update-child "\$1"/)
...
test.skipIf(process.platform === 'win32')('detached backend does not inherit the update mutex descriptor', async () => {
  const directory = await mkdtemp(path.join(os.tmpdir(), 'socis-update-mutex-'))
  const socisPath = path.join(directory, 'socis')
...
for fd in /proc/$$/fd/*; do
  ...
  case "$target" in
    *socis-update-in-progress.mutex) printf '%s\\n' "$target" >> ${reportPath} ;;
  esac
done
`,
      { mode: 0o700 }
    ...
    const command = buildSpawnCommand(socisPath, '', {
      socisHome: path.join(directory, 'home'),
      logPath
    })
...
test('spawnRemoteDashboard always spawns serve (legacy dashboard path removed)', async () => {
  ...
  await spawnRemoteDashboard(ssh, { socisPath: '/x/socis', profile: '', token: 'tk', ownershipId: OWNERSHIP_ID })
...
test('READY_RE accepts both serve and dashboard sentinels', () => {
  assert.equal(READY_RE.exec('SOCIS_AGENT_BACKEND_READY port=4321')?.[1], '4321')
  assert.equal(READY_RE.exec('SOCIS_AGENT_DASHBOARD_READY port=8765')?.[1], '8765')
})
...
test('spawnRemoteDashboard rejects when no pid is returned', async () => {
  ...
  await assert.rejects(
    () => spawnRemoteDashboard(ssh, { socisPath: '/x/socis', profile: '', token: 't', ownershipId: OWNERSHIP_ID }),
    (err: any) => {
...
test('scrapeReadyPort reads only the named spawn log', async () => {
  ...
  const ssh = fakeSsh([[/cat/, 'some noise\nSOCIS_AGENT_DASHBOARD_READY port=51234\n']])
...
test('connect() spawns fresh when there is no lockfile, adopts the served token', async () => {
  const ssh = fakeSsh([
    ...
    [/setsid/, '777\n'],
    [/kill -0 777/, 'ALIVE'],
    [/cat .*\.log/, 'SOCIS_AGENT_DASHBOARD_READY port=51999\n']
  ])
...
test('managed SSH maps a local scope to a different non-default remote profile', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '778\n'],
    [/kill -0 778/, 'ALIVE'],
    [/cat .*\.log/, 'SOCIS_AGENT_BACKEND_READY port=52000\n']
  ])
...
test('connect() respawns when the requested remote profile differs from the lockfile profile', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/kill 333/, ''],
    [/--version/, 'SOCIS Agent v0.18.2\n'],
    [/grep -q ssh-session-token-file/, 'YES\n'],
    [/python3 -c/, ''],
    [/setsid/, '890\n'],
    [/kill -0 890/, 'ALIVE'],
    [/cat .*\.log/, 'SOCIS_AGENT_DASHBOARD_READY port=52050\n']
  ])
...
test('connect() respawns when the lockfile socisPath differs from the resolved path', async () => {
  const reuseToken = 'stored-token'
  const lock = ownedLock({ socisPath: '/old/stale/socis', tokenFingerprint: fingerprintToken(reuseToken) })
  ...
  const result = await connect(
    connectDeps(ssh, { reuseToken, remoteSOCISPath: '/new/socis', adoptServedToken: async () => 'fresh' })
  )
...
test('connect() respawns when the lockfile protocolVersion is incompatible', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '901\n'],
    [/kill -0 901/, 'ALIVE'],
    [/cat .*\.log/, 'SOCIS_AGENT_DASHBOARD_READY port=44100\n']
  ])
...
test('connect() fresh spawn writes socisHome + protocolVersion into the lockfile', async () => {
  const writes: string[] = []
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '700\n'],
    [/kill -0 700/, 'ALIVE'],
    [/cat .*\.log/, 'SOCIS_AGENT_DASHBOARD_READY port=45500\n'],
    [
      /printf '%s' '/,
      c => {
        writes.push(c)
...
test('connect() respawns when the lockfile pid is dead (killed dashboard)', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '888\n'],
    [/kill -0 888/, 'ALIVE'],
    [/cat .*\.log/, 'SOCIS_AGENT_DASHBOARD_READY port=42000\n']
  ])
...
test('connect() respawns when the dashboard is wedged (alive pid, probe fails)', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '999\n'],
    [/kill -0 999/, 'ALIVE'],
    [/cat .*\.log/, 'SOCIS_AGENT_DASHBOARD_READY port=43000\n']
  ])
...
test('validateRemotePath accepts absolute POSIX paths', () => {
  assert.doesNotThrow(() => validateRemotePath('/usr/bin/socis'))
  assert.doesNotThrow(() => validateRemotePath('/home/user/.socis-agent/socis-agent/venv/bin/socis'))
})

test('validateRemotePath accepts ~/ prefix paths', () => {
  assert.doesNotThrow(() => validateRemotePath('~/bin/socis'))
  assert.doesNotThrow(() => validateRemotePath('~/.socis-agent/logs/desktop-ssh.log'))
  assert.doesNotThrow(() => validateRemotePath('~'))
})

test('validateRemotePath accepts paths with spaces and quotes', () => {
  assert.doesNotThrow(() => validateRemotePath('/home/user/my project/socis'))
...
test('validateRemotePath rejects relative paths', () => {
  assert.throws(() => validateRemotePath('socis'), /absolute|relative/i)
  assert.throws(() => validateRemotePath('./bin/socis'), /absolute|relative/i)
  assert.throws(() => validateRemotePath('../etc/passwd'), /absolute|relative/i)
})

test('validateRemotePath rejects NUL and newline', () => {
  assert.throws(() => validateRemotePath('/usr/bin/socis\x00'), /unsafe/i)
  assert.throws(() => validateRemotePath('/usr/bin/socis\n'), /unsafe/i)
  assert.throws(() => validateRemotePath('/usr/bin/socis\r'), /unsafe/i)
})

test('validateRemotePath preserves shell metacharacters as path data', () => {
  for (const p of ['/usr/$(whoami)/socis', '/usr/`id`/socis', '/usr/a;b|c&d<e>f']) {
...
test('expandRemotePath expands ~/ to "$HOME"/', () => {
  const result = expandRemotePath('~/.socis-agent/logs/desktop-ssh.log')
...
test('expandRemotePath returns quoted absolute paths unchanged', () => {
  const result = expandRemotePath('/usr/local/bin/socis')
  assert.ok(result.includes('/usr/local/bin/socis'))
  assert.ok(!result.includes('eval'))
})

test('expandRemotePath preserves spaces as data', () => {
  const result = expandRemotePath('/home/user/my project/socis')
...
test('buildSpawnCommand does not embed the token in the command string', () => {
  ...
  assert.ok(!cmd.includes('SOCIS_AGENT_DASHBOARD_SESSION_TOKEN'), 'env var name must not appear')
})

test('buildSpawnCommand includes --ssh-session-token-file when tokenFilePath is provided', () => {
  const cmd = buildSpawnCommand('/x/socis', 'work', {
    tokenFilePath: `~/.socis-agent/desktop-ssh/${OWNERSHIP_ID}/${SPAWN_NONCE}.token`,
    logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE),
    spawnNonce: SPAWN_NONCE
  })
...
test('buildSpawnCommand always uses serve, never dashboard', () => {
  const cmd = buildSpawnCommand('/x/socis', '', { logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE) })
  assert.match(cmd, /serve --isolated/)
  assert.doesNotMatch(cmd, /\bdashboard\b/)
  assert.doesNotMatch(cmd, /--skip-build/)
...
test('buildSpawnCommand raises the SSH child file limit before execing SOCIS', () => {
  const cmd = buildSpawnCommand('/x/socis', '', { logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE) })
  assert.match(cmd, /ulimit -n 65536 2>\/dev\/null \|\| true; exec env SOCIS_AGENT_DESKTOP=1/)
...
test('spawnRemoteDashboard removes a token file when upload reporting fails', async () => {
  ...
  await assert.rejects(
    () => spawnRemoteDashboard(ssh, { socisPath: '/x/socis', profile: '', token: 'tok', ownershipId: OWNERSHIP_ID }),
    /channel closed/
  )
...
test('spawnRemoteDashboard fails with update-required when remote lacks --ssh-session-token-file', async () => {
  ...
  await assert.rejects(
    () => spawnRemoteDashboard(ssh, { socisPath: '/x/socis', profile: '', token: 'tk', ownershipId: OWNERSHIP_ID }),
    (err: any) => {
...
test('readLockfile treats a log path outside the exact ownership and spawn path as skew', async () => {
  const lock = ownedLock({ logPath: '~/.socis-agent/desktop-ssh/other.log' })
...
test('cleanupStale never deletes a lock-supplied unexpected log path', async () => {
  ...
  await cleanupStale(ssh, OWNERSHIP_ID, ownedLock({ logPath: '~/.socis-agent/unrelated.log' }))
...
test('pidIsOurDashboard requires an exact nonce option value', async () => {
  const prefix = `/x/socis serve --isolated --ssh-owner-nonce ${SPAWN_NONCE}ff`
  const suffix = `/x/socis serve --isolated --ssh-owner-nonce xx${SPAWN_NONCE}`
  assert.equal(await pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, 'FOREIGN\n']]), 5, SPAWN_NONCE, '/x/socis'), false)
  assert.equal(await pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, 'FOREIGN\n']]), 5, SPAWN_NONCE, '/x/socis'), false)
})
...
test('remote SSH ownership capability requires both secure bootstrap flags', async () => {
  ...
  assert.equal(await remoteSupportsSshOwnership(supported, '/x/socis'), true)
  ...
  assert.equal(await remoteSupportsSshOwnership(unsupported, '/x/socis'), false)
})
