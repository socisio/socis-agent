 ...
 *     `X-SOCIS-Session-Token` header; WS uses `?token=`.
...
// socis_cli/dashboard_auth/cookies.py.
//
// Two cookies are in play (see that module):
//   - socis_session_at: the OAuth access token. Short-lived (~15 min); its
//     Max-Age tracks the access-token TTL, so the cookie jar drops it the
//     instant the AT expires.
//   - socis_session_rt: the OAuth refresh token. Long-lived (24h rotating,
//     reuse-detected — Portal NAS #293 / socis #37247). When the AT cookie
...
const AT_COOKIE_VARIANTS = ['__Host-socis_session_at', '__Secure-socis_session_at', 'socis_session_at']
const RT_COOKIE_VARIANTS = ['__Host-socis_session_rt', '__Secure-socis_session_rt', 'socis_session_rt']

// The Nous portal (NAS) does NOT use SOCIS gateway session cookies — it is a
...
// Keep this aligned with socis_cli.profiles.validate_profile_name(). `default`
// is the built-in root alias; these names cannot be created as profiles.
const RESERVED_REMOTE_PROFILES = new Set(['socis', 'test', 'tmp', 'root', 'sudo'])
...
/** Which SOCIS profile the remote SSH dashboard should actually run as.
...
// SOCIS Cloud connection (cloud-auto-discovery Q3/Q6): it carries a
...
function normalizeSshConfig(entry) {
  ...
  const remoteSOCISPath = String(entry.remoteSOCISPath || '').trim()

  if (remoteSOCISPath) {
    out.remoteSOCISPath = remoteSOCISPath
  }

  // A Desktop profile can be a local routing label rather than the profile
  // name used by the remote SOCIS installation. Preserve an explicit mapping
  // when it is a valid SOCIS profile identifier; otherwise fall back to the
...
function localPrimaryRequestScope(opts: ProfileRouteOptions): boolean | null {
  ...
  // spawned them: `_spawn_socis_action` registers the (often dynamic, e.g.
 ...
 *     `--profile`, so its `SOCIS_AGENT_HOME` scopes it.
 *
...
function resolveProfileBackendRoute(profile, opts: ProfileRouteOptions = {}): ProfileBackendRoute {
  ...
  if (scopedProfile === primaryProfile) {
    ...
    // the wire: the dashboard's process SOCIS_AGENT_HOME can belong to a different
 ...
 * `socis:api` handler — both routes reach a backend whose namespace is the
...
/**
 * Resolve the two decisions made by the `socis:api` IPC handler from the same
...
/**
 * True if any cookie in `cookies` is a socis session ACCESS-token cookie
 ...
 * the SOCIS gateway session cookies, so cloud sign-in / discovery liveness
