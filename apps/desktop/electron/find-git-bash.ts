 ...
 *   1. SOCIS_AGENT_GIT_BASH_PATH env var override
 *   2. PortableGit under %LOCALAPPDATA%\socis\git\ (install.ps1)
...
export function findGitBash(opts: GitBashOptions): string | null {
  ...
  // Respect SOCIS_AGENT_GIT_BASH_PATH if set (mirrors tools/environments/local.py:_find_bash).
  const gitBashPath = env.SOCIS_AGENT_GIT_BASH_PATH
  ...
  if (localAppData) {
    candidates.push(joinWin(localAppData, 'socis', 'git', 'bin', 'bash.exe'))
    candidates.push(joinWin(localAppData, 'socis', 'git', 'usr', 'bin', 'bash.exe'))
  }
