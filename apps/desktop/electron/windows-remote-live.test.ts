...
//   SOCIS_AGENT_WIN_SSH_HOST   ssh alias/host of the Windows box
//   SOCIS_AGENT_WIN_SSH_USER   remote user
//   SOCIS_AGENT_WIN_SSH_SOCIS absolute path to the remote socis.exe under test
const liveHost = process.env.SOCIS_AGENT_WIN_SSH_HOST || ''
const liveUser = process.env.SOCIS_AGENT_WIN_SSH_USER || ''
const configuredSOCIS = process.env.SOCIS_AGENT_WIN_SSH_SOCIS || ''
const ownershipId = '89abcdef0123456789abcdef01234567'

function fetchJson(url, token, path) {
  return fetch(`${url}${path}`, { headers: { 'X-SOCIS-Session-Token': token } }).then(async response => {
    if (!response.ok) {
...
test.skipIf(!liveHost || !liveUser || !configuredSOCIS)(
  ...
  async () => {
    ...
    try {
      ...
      const runtimeScript = `& '${configuredSOCIS.replace('socis.exe', 'python.exe')}' -m socis_cli.windows_ssh_runtime read-lock '${ownershipId}'`
      ...
      if (lock) {
        const python = configuredSOCIS.replace('socis.exe', 'python.exe')
        const terminate = `& '${python}' -m socis_cli.windows_ssh_runtime terminate '${lock.pid}' '${lock.creationTimeNs}' '${lock.socisPath}' '${lock.spawnNonce}'`
        await ssh.exec(`powershell.exe -NoProfile -NonInteractive -Command "${terminate}"`)
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m socis_cli.windows_ssh_runtime remove-lock '${ownershipId}'"`
        )
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m socis_cli.windows_ssh_runtime remove-log '${ownershipId}' '${lock.spawnNonce}'"`
        )
      }
