 ...
 * runtime, remote gateways, SOCIS Cloud instances, SSH hosts) that are all
 ...
 * apps/shared/src/backend-scope.ts (`@socis/shared`) — tsconfig project
...
export async function reuseMatchingPrimarySshBackend({
  ...
  if (
    ...
    String(source.remoteSOCISPath || '').trim() !== String(activeSsh.remoteSOCISPath || '').trim() ||
...
/** Turn `ls ~/.socis-agent/profiles` output into roster names. Always includes
...
/**
 * Whether "Update all instances" may drive this connection. SOCIS Cloud
 * instances are platform-managed — we never run `socis update` against them.
...
export function normalizeConnectionInput(input: ConnectionInput, registry: ConnectionRegistry): RegistryConnection {
  ...
  if (kind === 'ssh') {
    const ssh = normalizeSshConfig({
      ...
      remoteSOCISPath: input.remoteSOCISPath,
  ...
  if (kind === 'remote' || kind === 'cloud') {
    ...
    // transport-/SOCIS-managed names; an empty result stores nothing.
 ...
 * `remoteSOCISPath`/`remoteProfile`. Only fields the payload explicitly
...
export function migrateV1ToRegistry(v1: unknown): ConnectionRegistry {
  ...
  const addRemoteLike = (block: Record<string, any>, kind: 'cloud' | 'remote'): null | RegistryConnection => {
    ...
    const label = uniqueLabel(
      hostLabelFromBaseUrl(url) || (kind === 'cloud' ? 'SOCIS Cloud' : 'Remote gateway'),
