...
function mkTmpHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'socis-bootstrap-test-'))
}
...
test('runBootstrap bails immediately when the signal is already aborted', async () => {
  ...
  const result = await runBootstrap({
    installStamp: null,
    activeRoot: '/tmp/socis-runner-test',
    sourceRepoRoot: null,
    socisHome: '/tmp/socis-runner-test',
    logRoot: '/tmp/socis-runner-test',
...
test('installedAgentInstallScript resolves the installer in the agent checkout', () => {
  ...
  try {
    ...
    const scriptsDir = path.join(home, 'socis-agent', 'scripts')
...
test('existing checkout detection requires git metadata', () => {
  ...
  try {
    const activeRoot = path.join(home, 'socis-agent')
...
test('fresh bootstrap args include the packaged commit pin', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp,
      activeRoot: '/tmp/socis-agent',
      socisHome: '/tmp/socis'
    }),
    ['--dir', '/tmp/socis-agent', '--socis-home', '/tmp/socis', '--branch', 'main', '--commit', installStamp.commit]
  )
})
...
test('existing-checkout bootstrap args keep branch but skip the packaged commit pin', () => {
  ...
  assert.deepEqual(
    ...
    ['--dir', '/tmp/socis-agent', '--socis-home', '/tmp/socis', '--branch', 'main']
  )
})
...
test('fallback install stamps use an unpinned branch ref', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp: stamp,
      activeRoot: '/tmp/socis',
      socisHome: '/tmp/home'
    }),
    ['--dir', '/tmp/socis', '--socis-home', '/tmp/home', '--branch', 'main']
  )
})
