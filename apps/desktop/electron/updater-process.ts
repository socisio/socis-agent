 ...
 *
 * The staged Tauri `socis-setup.exe` has no self-update path, so every
 ...
 * checkout instead: every `socis update` refreshes the code that drives the
 ...
 * and QUITS, the script waits it out, runs `socis update`, swaps/relaunches
 * the app, and writes .socis-update-result.json. With the app gone before
 * the update starts, the SOCIS_AGENT_DESKTOP_CHILD_PID reaper-exclusion dance is
 ...
 * `socis update` card — old checkouts pull the script on their next update).
 */
 ...
 *
 * The Tauri installer self-copies into SOCIS_AGENT_HOME on *every* platform
 * (`socis-setup.exe` on Windows, `socis-setup` elsewhere — see
 * apps/bootstrap-installer `paths::installer_dest` and
 * `bootstrap::copy_self_to_socis_agent_home`), so finding that binary on macOS or
 ...
 * than the hand-off protocol holds the update marker, spawns `socis update`,
...
export function resolveStagedUpdaterBinary(
  ...
  const candidate = path.join(socisHome, 'socis-setup.exe')
 ...
 *
 * `copy_self_to_socis_agent_home` deliberately no-ops during `--update`
 ...
 * "Another SOCIS update is already running (PID <itself>, started 1s ago)" —
 * the observed infinite "Install didn't finish" loop. Skipping the pre-write
 * for those binaries lets them acquire cleanly and run `socis update`, which
 ...
 * the installer is written to SOCIS_AGENT_HOME at install/repair time, making mtime
