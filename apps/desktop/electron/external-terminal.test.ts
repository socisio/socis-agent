...
test('buildTerminalScript execs the resolved runtime with its env', () => {
  const script = buildTerminalScript({
    args: ['-m', 'socis_cli.main', '--tui', '--resume', 'sess'],
    command: '/home/b/.socis-agent/socis-agent/venv/bin/python',
    cwd: "/home/b/o'brien",
    env: { PYTHONPATH: '/home/b/.socis-agent/socis-agent' },
    platform: 'darwin'
  })
  ...
  assert.equal(
    ...
    [
      ...
      `export PYTHONPATH='/home/b/.socis-agent/socis-agent'`,
      `exec '/home/b/.socis-agent/socis-agent/venv/bin/python' '-m' 'socis_cli.main' '--tui' '--resume' 'sess'`,
      ''
    ].join('\n')
  )
})
...
test('buildTerminalScript emits a cmd script on Windows', () => {
  const script = buildTerminalScript({
    ...
    command: 'C:\\socis\\venv\\Scripts\\socis.exe',
    cwd: 'C:\\Users\\b',
    env: { PYTHONUTF8: '1' },
    platform: 'win32'
  })
  ...
  assert.deepEqual(script.split('\r\n'), [
    ...
    'cd /d "C:\\Users\\b"',
    'set "PYTHONUTF8=1"',
    '"C:\\socis\\venv\\Scripts\\socis.exe" "--tui" "--resume" "sess"',
    ''
  ])
})
