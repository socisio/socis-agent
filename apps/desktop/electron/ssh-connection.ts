...
const _REDACTIONS: Array<[RegExp, string]> = [
  [/(SOCIS_AGENT_DASHBOARD_SESSION_TOKEN=)(\S+)/g, '$1<redacted>'],
  [/(X-SOCIS-Session-Token["']?\s*[:=]\s*["']?)([^\s"'&]+)/gi, '$1<redacted>'],
...
// root under a short per-user base (`~/.socis-agent/desktop-ssh`) so even worst case
...
function defaultControlDir() {
  ...
  if (process.platform === 'win32') {
    return path.join(os.tmpdir(), 'socis-desktop-ssh')
  }

  return path.join(os.homedir(), '.socis-agent', 'desktop-ssh')
}
