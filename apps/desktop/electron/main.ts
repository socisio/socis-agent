...
import { buildDesktopBackendEnv, socisManagedNodePathEntries, normalizeSOCISHomeRoot } from './backend-env'
import {
...
import { isSOCISOwnedVenvDaemon } from './venv-holder-select'
...
import {
  ...
  resolveVenvSOCISCommand
} from './windows-socis-path'
import {
...
const USER_DATA_OVERRIDE = process.env.SOCIS_AGENT_DESKTOP_USER_DATA_DIR
...
const DEV_SERVER = process.env.SOCIS_AGENT_DESKTOP_DEV_SERVER
const IS_PACKAGED = app.isPackaged || Boolean(process.env.SOCIS_AGENT_DESKTOP_IS_PACKAGED)
...
// switches only apply pre-launch. Override with SOCIS_AGENT_DESKTOP_DISABLE_GPU
...
if (REMOTE_DISPLAY_REASON) {
  ...
  console.log(
    `[socis] remote display detected (${REMOTE_DISPLAY_REASON}); disabling GPU hardware acceleration to prevent flicker`
  )
}
...
if (DEV_CDP.port) {
  ...
  console.log(
    `[socis] renderer debugging on http://127.0.0.1:${DEV_CDP.port} — anything that can reach it ` +
      'can run code in the renderer. SOCIS_AGENT_DESKTOP_CDP_PORT=off to disable.'
  )
} else {
  const why = describeDevCdpDecision(DEV_CDP)

  if (why) {
    console.warn(`[socis] ${why}`)
  }
}
...
if (IS_WSL && !REMOTE_DISPLAY_REASON && fs.existsSync('/dev/dxg')) {
  ...
  console.log('[socis] WSL GPU passthrough (/dev/dxg) detected; enabling GPU acceleration')
}
...
// it). The value arrives via SOCIS_AGENT_DESKTOP_PASSWORD_STORE, bridged by the
// `socis desktop` launcher from detection or `desktop.password_store` in
...
if (PASSWORD_STORE.warning) {
  console.warn(`[socis] ${PASSWORD_STORE.warning}`)
}

if (PASSWORD_STORE.store) {
  app.commandLine.appendSwitch('password-store', PASSWORD_STORE.store)
  console.log(`[socis] using password-store backend: ${PASSWORD_STORE.store}`)
}
...
// never go through `socis desktop`; it is version-scoped so an app update
...
if (IS_WINDOWS) {
  ...
  // expose SOCIS sessions/config to every packaged app on the machine.
  if (shouldAttemptAclRepair(priorMarker)) {
    const exeDir = path.dirname(process.execPath)
    const acl = grantAllApplicationPackagesAcl(exeDir, { execFileSync })

    if (acl.ok) {
      console.log(`[socis] granted ALL APPLICATION PACKAGES RX on ${exeDir} (#38216)`)
    } else if (acl.error && acl.error !== 'missing-target-or-exec') {
      console.warn(`[socis] AppContainer ACL grant failed on ${exeDir}: ${acl.error}`)
    }
  }
  ...
  if (sandboxDecision.enable && sandboxDecision.reason !== 'already-enabled') {
    ...
    console.log(
      `[socis] Windows sandbox fallback enabled (${sandboxDecision.reason}); launching with --no-sandbox (#38216)`
    )
  }
  ...
  app.on('child-process-gone', (_event, details) => {
    ...
    console.warn(
      `[socis] Windows GPU sandbox crashed (exit=${details?.exitCode}); relaunching once with --no-sandbox (#38216)`
    )
    ...
    try {
      ...
      console.error(`[socis] --no-sandbox relaunch failed: ${error?.message || error}`)
    }
  })
}

ipcMain.handle('socis:get-remote-display-reason', () => REMOTE_DISPLAY_REASON)
...
// always-on timers. A completely idle, minimized SOCIS burned ~20% CPU
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && typeof parsed.commit === 'string' && parsed.commit.length >= 7) {
        if (parsed.schemaVersion !== INSTALL_STAMP_SCHEMA_VERSION) {
          console.warn(
            `[socis] install-stamp.json schemaVersion ${parsed.schemaVersion} != expected ${INSTALL_STAMP_SCHEMA_VERSION}; ignoring`
          )
      ...
      console.warn(`[socis] install-stamp.json found at ${p} , but parsing failed with ${e}`)
...
if (INSTALL_STAMP) {
  console.log(
    `[socis] install stamp: ${INSTALL_STAMP.commit.slice(0, 12)}${INSTALL_STAMP.branch ? ` (${INSTALL_STAMP.branch})` : ''}${INSTALL_STAMP.dirty ? ' [DIRTY]' : ''} from ${INSTALL_STAMP.source || 'unknown'}`
  )
} else if (IS_PACKAGED) {
  ...
  console.error(
    '[socis] WARNING: no install-stamp.json found in packaged build. First-launch bootstrap will not have a pinned ref to install.'
  )
}

// SOCIS_AGENT_HOME — the user-facing root for everything SOCIS-related. Mirrors
// scripts/install.ps1's $SOCISHome and scripts/install.sh's $SOCIS_AGENT_HOME.
//
// Defaults:
//   Windows: %LOCALAPPDATA%\socis (matches install.ps1)
//   macOS / Linux: ~/.socis-agent (matches install.sh)
//
// Special case for Windows: if the user has a legacy ~/.socis-agent directory
// (e.g., from a prior pip install or a manual setup) AND no
// %LOCALAPPDATA%\socis yet, prefer the legacy path so we don't orphan their
// existing config / sessions / .env. New installs go to %LOCALAPPDATA%.
//
// SOCIS_AGENT_DESKTOP_USER_DATA_DIR (used by test:desktop:fresh) puts the sandbox
// SOCIS_AGENT_HOME beneath the throwaway userData dir so a fresh-install run never
// touches the user's real ~/.socis-agent / %LOCALAPPDATA%\socis.
function resolveSOCISHome() {
  if (process.env.SOCIS_AGENT_HOME) {
    return normalizeSOCISHomeRoot(process.env.SOCIS_AGENT_HOME)
  }

  if (USER_DATA_OVERRIDE) {
    return path.join(path.resolve(USER_DATA_OVERRIDE), 'socis-home')
  }

  if (IS_WINDOWS) {
    // A GUI app launched from Explorer inherits the environment block captured
    // at login, so a SOCIS_AGENT_HOME set via `setx` AFTER login is invisible in
    // process.env even though the CLI (a fresh shell) sees it. Without this the
    // backend silently falls back to %LOCALAPPDATA%\socis and reports "No
    // inference provider configured" despite a valid configured home (#45471).
    // Consult the live User-scoped registry value before the default below.
    const fromRegistry = readWindowsUserEnvVar('SOCIS_AGENT_HOME')

    if (fromRegistry) {
      return normalizeSOCISHomeRoot(fromRegistry)
    }
  }

  if (IS_WINDOWS && process.env.LOCALAPPDATA) {
    const localappdata = path.join(process.env.LOCALAPPDATA, 'socis')
    const legacy = path.join(app.getPath('home'), '.socis-agent')

    // Migrate transparently to LOCALAPPDATA, but honour an existing legacy
    // ~/.socis-agent setup (no LOCALAPPDATA install yet) so users don't lose state.
  ...
  return path.join(app.getPath('home'), '.socis-agent')
}

const SOCIS_AGENT_HOME = resolveSOCISHome()

function pathWithSOCISManagedNode(...entries) {
  const managed = socisManagedNodePathEntries(SOCIS_AGENT_HOME).filter(directoryExists)

  return [...managed, ...entries, process.env.PATH].filter(Boolean).join(path.delimiter)
}

// ACTIVE_SOCIS_AGENT_ROOT — the canonical mutable SOCIS install. Same path
// install.ps1 / install.sh use, so a desktop-only user and a CLI-only user end
// up with identical layouts and can share one install.
const ACTIVE_SOCIS_AGENT_ROOT = path.join(SOCIS_AGENT_HOME, 'socis-agent')
// VENV_ROOT — venv lives inside the repo, exactly like install.ps1 does it.
const VENV_ROOT = path.join(ACTIVE_SOCIS_AGENT_ROOT, 'venv')
...
// boots, going straight to `resolveSOCISBackend()`. Missing or stale marker
...
//
// We deliberately put the marker INSIDE ACTIVE_SOCIS_AGENT_ROOT (not alongside)
...
// active-profile.json records which SOCIS profile the desktop launches its
// local backend as. When set, startSOCIS() passes `socis --profile <name>
// dashboard …`, which deterministically pins SOCIS_AGENT_HOME (see
// _apply_profile_override in socis_cli/main.py) and bypasses the sticky
// ~/.socis-agent/active_profile file. Unset (null) preserves the legacy behavior:
// no --profile flag, so the backend honors active_profile / default.
const DESKTOP_PROFILE_CONFIG_PATH = path.join(app.getPath('userData'), 'active-profile.json')
// Mirrors socis_cli.profiles._PROFILE_ID_RE so we never hand the backend a
// value its profile resolver would reject and exit on.
const PROFILE_NAME_RE = /^[a-z0-9][a-z0-9_-]{0,63}$/
// Branch we track for self-update. The GUI work has merged to main, so this
// tracks main. User can also override at runtime via
// socisDesktop.updates.setBranch().
const DEFAULT_UPDATE_BRANCH = 'main'
// desktop.log lives under SOCIS_AGENT_HOME/logs/ so it sits next to agent.log,
// errors.log, gateway.log produced by socis_agent_logging.setup_logging — one log
// directory per user, regardless of which UI surface produced the line.
const DESKTOP_LOG_PATH = path.join(SOCIS_AGENT_HOME, 'logs', 'desktop.log')
...
//
// Mirror the Python logs (socis_agent_logging.py RotatingFileHandler, maxBytes x
...
const BOOT_FAKE_MODE = process.env.SOCIS_AGENT_DESKTOP_BOOT_FAKE === '1'
const BOOT_FAKE_ERROR = process.env.SOCIS_AGENT_DESKTOP_BOOT_FAKE_ERROR || ''
...
const SKIP_QUIT_CONFIRM = process.env.SOCIS_AGENT_DESKTOP_SKIP_QUIT_CONFIRM === '1'

const BOOT_FAKE_STEP_MS = (() => {
  const raw = Number.parseInt(String(process.env.SOCIS_AGENT_DESKTOP_BOOT_FAKE_STEP_MS || ''), 10)
...
const APP_NAME = process.env.SOCIS_AGENT_DESKTOP_APP_NAME || 'SOCIS'
...
// 'socis:native-theme' ('dark' | 'light' | 'system'); we pin
...
// Seed the native About panel with the live SOCIS version. This is refreshed
// on every open via the explicit "About" menu handler (refreshAboutPanel), so
// an in-place `socis update` mid-session is reflected without an app restart;
// the seed here just covers the first open and any non-menu invocation path.
app.setAboutPanelOptions({
  applicationName: APP_NAME,
  applicationVersion: resolveSOCISVersion(),
  copyright: 'Copyright © 2026 SOCIS'
})
...
// startSOCIS(); this pool only holds EXTRA profile
...
// The legacy SOCIS_AGENT_DESKTOP_POOL_* env vars remain the initial-value fallback
...
function readPersistedPoolLimits() {
  try {
    ...
    const fromEnv = clampPoolLimits({
      maxBackends: Number(process.env.SOCIS_AGENT_DESKTOP_POOL_MAX) || undefined,
      idleMs: Number(process.env.SOCIS_AGENT_DESKTOP_POOL_IDLE_MS) || undefined
    })
...
//   * WSL2 IPC stall  = the renderer's `socis:backend:touch` roundtrips
...
const POOL_KEEPALIVE_FRESH_MS = Math.max(
  120_000,
  Number(process.env.SOCIS_AGENT_DESKTOP_POOL_KEEPALIVE_FRESH_MS) || 4 * 60_000
)
...
// onto the error so subsequent startSOCIS() calls (e.g. the renderer's
// ensureGatewayOpen retrying after the WS won't open) return the same error
// instead of re-running install.ps1 in a hot loop. Cleared explicitly by
// the renderer's "Reload and retry" path or by quitting the app.
let bootstrapFailure = null
// Latched non-bootstrap backend spawn failure — stops getConnection() from
// respawning socis serve backend children in a tight loop while boot is broken.
...
let bootProgressState = {
  ...
  message: 'Waiting to start SOCIS backend',
  phase: 'idle',
  progress: 0,
  retryable: false,
  running: false,
...
function rememberLog(chunk) {
  ...
  if (socisLog.length > 300) {
    socisLog.splice(0, socisLog.length - 300)
  }
...
function ensureWslWindowsFonts() {
  ...
  try {
    ...
    const confPath = path.join(confDir, '99-socis-wsl-windows-fonts.conf')
    let existing = ''
...
function broadcastBootProgress() {
  ...
  webContents.send('socis:boot-progress', bootProgressState)
}
...
//
// The snapshot is queryable via the socis:bootstrap:get IPC handler so a
...
function broadcastBootstrapEvent(ev) {
  ...
  webContents.send('socis:bootstrap:event', ev)
}
...
function promptFirstRunSetupChoice(backend) {
  broadcastBootstrapEvent({
    ...
    activeRoot: backend.activeRoot || ACTIVE_SOCIS_AGENT_ROOT
  })
}
...
// The Tauri updater writes SOCIS_AGENT_HOME/.socis-update-in-progress for the whole
...
// (`force_kill_other_socis`, taskkill /IM socis.exe) kills it, the launch
...
function updateGateDeps() {
  return {
    hasLiveMarker: () => Boolean(readLiveUpdateMarker(SOCIS_AGENT_HOME)),
...
const BUNDLE_SWAP_RELAUNCH_FLAG = '--socis-bundle-swap-relaunched'
...
// The detached updater swaps the packaged bundle on disk AFTER `socis update`
...
async function waitForUpdateToFinish() {
  ...
  const outcome = await waitForUpdateClearance(updateGateDeps(), {
    onWaitTick: async reason => {
      ...
      await advanceBootProgress(
        'backend.update-wait',
        'An update is finishing — SOCIS will start automatically when it completes…',
        12
      )
    },
  ...
  // "nothing happened").
  try {
    const result = readAndConsumeHandoffResult(SOCIS_AGENT_HOME)
    ...
    if (result && result.ok && result.manual) {
      ...
      dialog.showErrorBox(
        'SOCIS update did not finish',
        `${result.message}\n\nDetails: ${path.join(SOCIS_AGENT_HOME, 'logs', 'desktop-update-handoff.log')}`
      )
    }
  } catch (err) {
  ...
  if (outcome === 'timeout') {
    ...
    await advanceBootProgress('backend.update-restart', 'Restarting SOCIS to load the updated app…', 14)
...
function findOnPath(command) {
  ...
  // shell-script shim named `socis` — must not shadow `socis.cmd`/`socis.exe`.
...
function unwrapWindowsVenvSOCISCommand(command, backendArgs) {
  return resolveVenvSOCISCommand(command, backendArgs, {
    isWindows: IS_WINDOWS,
    isCommandScript,
    fileExists,
    directoryExists,
...
// spawns `socis serve`; runtimes older than serve only have `dashboard`. We
...
// (covers a bare `socis` resolved from PATH with no known source root). Result
...
function backendSupportsServe(backend) {
  ...
  if (backend.root) {
    try {
      const src = fs.readFileSync(path.join(backend.root, 'socis_cli', 'subcommands', 'dashboard.py'), 'utf8')
  ...
  if (supported === null) {
    try {
      ...
      // `socis --version` (~10.5s measured cold), and a false negative here
      ...
      execProbeSync(backend.command, [...prefix, 'serve', '--help'], {
        ...
        env: { ...process.env, SOCIS_AGENT_HOME, ...(backend.env || {}) },
        timeout: PROBE_TIMEOUT_MS,
        stdio: 'ignore',
        // `.cmd`/`.bat` shim backends carry shell: true in their descriptor
        // (see resolveSOCISBackend step 4); execFileSync of a .cmd without
...
function isSOCISSourceRoot(root) {
  return directoryExists(root) && fileExists(path.join(root, 'socis_cli', 'main.py'))
}

function findPythonForRoot(root) {
  const override = process.env.SOCIS_AGENT_DESKTOP_PYTHON
...
function findSystemPython() {
  ...
  // CPython but several SOCIS deps (notably pywinpty's Rust-built
      ...
      try {
        const out = execFileSync(
          ...
          hiddenWindowsChildOptions({
            ...
            // Bare interpreter startup — much lighter than the socis-import
...
// findGitBash — locate bash.exe on Windows. Resolves SOCIS_AGENT_GIT_BASH_PATH
...
// normal SOCIS_AGENT_DASHBOARD_READY stdout line and no ready-file side channel is
// needed.
...
// install only has PortableGit under %LOCALAPPDATA%\socis\git (never on
...
function resolveGitBinary() {
  ...
  if (localAppData) {
    candidates.push(path.join(localAppData, 'socis', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'socis', 'git', 'bin', 'git.exe'))
  }
...
function recentSOCISLog() {
  return socisLog.slice(-20).join('\n')
}

// ─── Self-update (git-pull against the running backend's socis root) ──────
...
// Dev → SOURCE_REPO_ROOT. Packaged/CLI install → ACTIVE_SOCIS_AGENT_ROOT.
// SOCIS_AGENT_DESKTOP_SOCIS_AGENT_ROOT always wins so devs can pin a worktree.
function resolveUpdateRoot() {
  const candidates = [
    process.env.SOCIS_AGENT_DESKTOP_SOCIS_AGENT_ROOT && path.resolve(process.env.SOCIS_AGENT_DESKTOP_SOCIS_AGENT_ROOT),
    !IS_PACKAGED && isSOCISSourceRoot(SOURCE_REPO_ROOT) ? SOURCE_REPO_ROOT : null,
    isSOCISSourceRoot(ACTIVE_SOCIS_AGENT_ROOT) ? ACTIVE_SOCIS_AGENT_ROOT : null
  ].filter(Boolean)

  return candidates.find(c => directoryExists(path.join(c, '.git'))) || candidates[0] || ACTIVE_SOCIS_AGENT_ROOT
}
...
function emitUpdateProgress(payload) {
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    window.webContents.send('socis:updates:progress', merged)
  }
}
      ...
      const req = https.get(
        url,
        {
          headers: {
            ...
            'User-Agent': 'socis-desktop-update-check'
          },
          timeout: 10_000
        },
        res => {
...
// Windows that binary owns ALL repo mutation — running `socis update` +
...
function resolveUpdaterBinary() {
  return resolveStagedUpdaterBinary(SOCIS_AGENT_HOME, { fileExists, isWindows: IS_WINDOWS })
}
...
// Path to the venv shim whose lock decides whether `socis update` can write
// fresh entry points. On Windows this is the file the running backend
// `socis.exe` holds open; on POSIX it's never mandatory-locked.
function venvSOCISShimPath(updateRoot) {
  return IS_WINDOWS
    ? path.join(updateRoot, 'venv', 'Scripts', 'socis.exe')
    : path.join(updateRoot, 'venv', 'bin', 'socis')
}
...
// Kill only SOCIS-OWNED venv daemons (the memory plugin's hindsight daemon:
...
// venv files mapped. External holders (a user terminal running `socis`,
...
function killSOCISOwnedVenvDaemons(updateRoot) {
  if (!IS_WINDOWS) {
    return
  }
  ...
  try {
    ...
    holders = (Array.isArray(parsed) ? parsed : [parsed]).filter(p =>
      isSOCISOwnedVenvDaemon(p?.ExecutablePath, p?.CommandLine, scriptsDir)
    )
  } catch {
  ...
  for (const holder of holders) {
    ...
    if (Number.isInteger(pid) && pid > 0) {
      rememberLog(`[updates] stopping SOCIS-owned venv daemon (hindsight) PID ${pid} before hand-off`)
...
// only signals the direct child, so on Windows a backend `socis.exe` that
// spawned its own grandchildren (a `socis` REPL, a pty terminal session, the
...
async function claimBackendChild(child, command, profile, nonce, outputTail: BackendOutputTail | null = null) {
  ...
  if (decision.action === 'fail') {
    ...
    throw new Error(
      `SOCIS backend (PID ${child.pid}) died before its identity could be recorded: ${decision.reason}${outputTail?.describe() ?? ''}`
    )
  }
  ...
  if (decision.action === 'degrade') {
    ...
    rememberLog(
      `WARNING: process start marker probe failed for live SOCIS backend PID ${child.pid}; ` +
  ...
  try {
    ...
    child.socisBackendIdentity = identity
    ...
    await waitForBackendExit(child)
    throw new Error(
      `Could not persist ownership for the SOCIS backend: ${error.message}${outputTail?.describe() ?? ''}`
    )
  }
}

function releaseBackendChild(child) {
  const identity = child?.socisBackendIdentity
...
// `socisProcess.kill('SIGTERM')` + `app.quit()` fire-and-forget: SIGTERM on
// Windows doesn't reap the backend's grandchildren, and quit didn't wait for
// teardown, so the updater raced a still-locked `socis.exe`, the quarantine
...
// problem: the desktop's backend (and the grandchildren IT spawned — a socis
// REPL, a pty terminal, the gateway) keep `socis.exe` and other files in the
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  const socisProcess = backendConnectionState.getProcess()
  ...
  // unlock — the shim probe only covers venv\Scripts\socis.exe, but the
  // backend is `python.exe -m socis_cli.main serve`, which need not hold
  // the shim at all (#74805 first-attempt race).
  const initialPids = []

  if (socisProcess && Number.isInteger(socisProcess.pid)) {
    initialPids.push(socisProcess.pid)
  }
  ...
  stopBackendTreesForUpdate(socisProcess, {
  ...
  // `socis gateway stop --all`: the CLI discovers every profile's gateway
  ...
  stopGatewayBeforeUpdate(venvSOCISShimPath(updateRoot), SOCIS_AGENT_HOME)

  // Reap SOCIS-OWNED venv daemons the tree-kill above cannot reach: the
  ...
  killSOCISOwnedVenvDaemons(updateRoot)

  const shim = venvSOCISShimPath(updateRoot)
  ...
  const gate = await waitForBackendRelease(
    initialPids,
    {
      ...
      collectStragglerPids: () => {
        ...
        const currentSOCISProcess = backendConnectionState.getProcess()

        if (currentSOCISProcess && Number.isInteger(currentSOCISProcess.pid)) {
          stragglers.push(currentSOCISProcess.pid)
        }
...
// `socis update`). Instead we spawn the staged SOCIS-Setup binary with
// --update and quit, so it can run `socis update` (which refuses while we
...
async function applyUpdates(opts: { stopSafeBlockers?: boolean } = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      ...
      // Windows (quit → detached orchestrator → `socis update` → relaunch),
      ...
      // in: the SOCIS_AGENT_DESKTOP_CHILD_PID reaper-exclusion dance (#37532),
      ...
      // get the manual `socis update` card once; their next update pulls it.
      return await applyUpdatesPosixHandoff(opts)
    }

    if (!updater) {
      // No staged updater binary — this is a CLI-installed user (they ran
      // `socis desktop`, never the Tauri installer that self-copies
      // socis-setup.exe into SOCIS_AGENT_HOME). On Windows the repo hand-off
      ...
      if (!resolveUpdateScriptHandoff(updateRoot)) {
        // They DO have a working `socis` on PATH / in the venv, so the
        ...
        try {
          ...
          if (head.code === 0 && current && current !== 'HEAD') {
            ...
            if (branch !== 'main') {
              command = `socis update --branch ${branch}`
            }
          }
        } catch {
          // Best-effort: fall back to bare `socis update` if branch detection fails.
        }

        rememberLog(`[updates] no staged updater; surfacing manual \`${command}\` for CLI install at ${updateRoot}`)
        emitUpdateProgress({ stage: 'manual', message: command, percent: null })

        return { ok: true, manual: true, command, socisRoot: updateRoot }
      }

      rememberLog('[updates] no staged updater; using repo hand-off script for CLI install')
    }

    const handoffConflict = updateHandoffConflict(SOCIS_AGENT_HOME)
    ...
    emitUpdateProgress({
      stage: 'restart',
      message:
        'Updating SOCIS — this window will close and the updater will open. Don’t reopen SOCIS yourself; it restarts automatically when the update finishes.',
      percent: 100
    })
    ...
    preflightStateDb(SOCIS_AGENT_HOME, rememberLog)

    // Stop our own backend(s) and wait for the venv shim to unlock BEFORE we
    // spawn the updater. Without this the updater races a still-locked
    // socis.exe (held by the backend child / its grandchildren) and the update
    ...
    if (!lock.unlocked) {
      ...
      // terminal running socis, an unkillable child). Handing off anyway
      ...
      const message =
        'Update aborted: another process is holding the SOCIS install open ' +
        '(a second SOCIS window or a terminal running socis?). Close it and retry.'
      ...
      if (IS_WINDOWS) {
        ...
        startGatewaysAfterUpdateAbort(venvSOCISShimPath(updateRoot))
      }
    ...
    // SOCIS processes running from this venv.  The updater normally refuses
    ...
    // `socis update` will run (the venv shim is locked while we live).
    //
    ...
    // checkout, so each `socis update` refreshes the code that drives the
    ...
    if (scriptHandoff) {
      ...
      child = spawnUpdaterProcess(wrapped.command, wrapped.args, {
        cwd: SOCIS_AGENT_HOME,
        env: {
          ...process.env,
          SOCIS_AGENT_HOME,
          SOCIS_AGENT_UPDATE_STARTED_AT: String(updateStartedAt),
          PATH: pathWithSOCISManagedNode(venvBin)
        },
        detached: true,
        stdio: 'ignore'
      })
      ...
      // The `socis update` child adopts the SCRIPT's claim via
      // update_lock.py's process-ancestry rule; no mtime heuristics needed.
      if (Number.isInteger(child.pid)) {
        writeUpdateMarker(SOCIS_AGENT_HOME, child.pid, { startedAt: updateStartedAt })
      }
      ...
      // abort with "Another SOCIS update is already running (PID <itself>)" —
      ...
      if (Number.isInteger(child.pid) && stagedUpdaterSupportsPrewrittenMarker(updater)) {
        writeUpdateMarker(SOCIS_AGENT_HOME, child.pid)
    ...
    if (!handoffOutcome.ok) {
      const message = `Update failed to start: ${handoffOutcome.message}. SOCIS will keep running — try again, or run \`socis update\` from a terminal.`
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  const venvBin = path.join(updateRoot, 'venv', IS_WINDOWS ? 'Scripts' : 'bin')
  const venvSOCIS = path.join(venvBin, IS_WINDOWS ? 'socis.exe' : 'socis')
  const venvPython = path.join(venvBin, IS_WINDOWS ? 'python.exe' : 'python')

  // The updater invokes the venv's SOCIS launcher, which in turn requires the
  ...
  // marker-only install through --update dead-ends at "Could not find the socis
  ...
  const updaterArgs = chooseUpdaterArgs(
    {
      hasBootstrapMarker: fileExists(path.join(updateRoot, '.socis-bootstrap-complete')),
      hasVenvSOCIS: fileExists(venvSOCIS),
...
// Python-level pre-update snapshot inside `socis update`.
function preflightStateDb(socisHome, rememberLog) {
  const stateDbPath = path.join(socisHome, 'state.db')
  ...
  try {
    ...
    if (stat.size > 100) {
      ...
      const emergencyPath = path.join(socisHome, `state.db.pre-update-emergency-${ts}.bak`)
      ...
      try {
        ...
        try {
          const homeDir = fs.readdirSync(socisHome)
          ...
          for (const old of backups.slice(2)) {
            try {
              fs.unlinkSync(path.join(socisHome, old))
            } catch {
              void 0
            }
          }
...
// out, runs `socis update`, swaps/relaunches the app bundle, and writes
// .socis-update-result.json for the relaunched Desktop to surface. It shows
...
async function applyUpdatesPosixHandoff(opts: any) {
  ...
  if (!handoff) {
    emitUpdateProgress({ stage: 'manual', message: 'socis update', percent: null })

    return { ok: true, manual: true, command: 'socis update', socisRoot: updateRoot }
  }
  ...
  const child = spawnUpdaterProcess(handoff.command, args, {
    ...
    env: {
      ...
      SOCIS_AGENT_HOME,
      SOCIS_AGENT_UPDATE_STARTED_AT: String(updateStartedAt),
      PATH: pathWithSOCISManagedNode(path.join(updateRoot, 'venv', 'bin'))
    },
    detached: true,
    stdio: 'ignore'
  })
  ...
  emitUpdateProgress({
    stage: 'restart',
    message:
      'Updating SOCIS — this window will close. Don’t reopen SOCIS yourself; it restarts automatically when the update finishes.',
    percent: 100
  })
...
// Marker-independent: is the canonical install at ACTIVE_SOCIS_AGENT_ROOT actually
...
function isActiveRuntimeUsable() {
  ...
  return (
    isSOCISSourceRoot(ACTIVE_SOCIS_AGENT_ROOT) &&
    fileExists(venvPython) &&
    canImportSOCISCli(venvPython, {
      env: {
        PYTHONPATH: [ACTIVE_SOCIS_AGENT_ROOT, process.env.PYTHONPATH].filter(Boolean).join(path.delimiter)
      }
    })
  )
}
...
function activeRuntimeState() {
  ...
  // pinned commit -- users update via the in-app update path or `socis
...
function resolveWebDist() {
  const override = process.env.SOCIS_AGENT_DESKTOP_WEB_DIST
  ...
  if (IS_PACKAGED && /app\.asar(?=$|[\\/])/.test(fallback) && !directoryExists(fallback)) {
    rememberLog(
      `[web-dist] dashboard frontend dir resolved to an asar-internal path that ` +
        ...
        `Ensure dist/** is unpacked (asarUnpack) or set SOCIS_AGENT_DESKTOP_WEB_DIST.`
    )
  }
...
function resolveRendererIndex() {
  ...
  if (present.length > 0) {
    ...
    rememberLog(
      `[renderer] every renderer bundle is incomplete (${present.join(', ')}). ` +
        ...
        `Repair with: socis desktop --force-build`
    )
  ...
  rememberLog(
    `[renderer] index.html not found — the desktop app was packaged without a ` +
      ...
      `Rebuild with: socis desktop --force-build`
  )
...
function resolveSOCISCwd() {
  // In a packaged build, `process.cwd()` resolves to the install root (e.g.
  // `…/win-unpacked` on Windows or `/Applications/SOCIS.app/Contents/...`
  ...
  const candidates = [
    ...
    process.env.SOCIS_AGENT_DESKTOP_CWD,
...
function sanitizeWorkspaceCwd(cwd) {
  ...
  if (!trimmed || isPackagedInstallPath(trimmed)) {
    return { cwd: resolveSOCISCwd(), sanitized: Boolean(trimmed) }
  }
...
// createActiveBackend — build a backend pointing at ACTIVE_SOCIS_AGENT_ROOT, the
...
function createActiveBackend(backendArgs) {
  ...
  return {
    kind: 'python',
    label: `SOCIS at ${ACTIVE_SOCIS_AGENT_ROOT}`,
    command,
    args: ['-m', 'socis_cli.main', ...backendArgs],
    env: buildDesktopBackendEnv({
      socisHome: SOCIS_AGENT_HOME,
      pythonPathEntries: [ACTIVE_SOCIS_AGENT_ROOT, ...getVenvSitePackagesEntries(VENV_ROOT)],
      venvRoot: VENV_ROOT
    }),
    root: ACTIVE_SOCIS_AGENT_ROOT,
    bootstrap: true,
    shell: false
  }
}

function resolveSOCISBackend(backendArgs) {
  // 1. Explicit override -- SOCIS_AGENT_DESKTOP_SOCIS_AGENT_ROOT points at a developer
  //    checkout. Honour it as-is (no bootstrap; the user is driving).
  const overrideRoot = process.env.SOCIS_AGENT_DESKTOP_SOCIS_AGENT_ROOT && path.resolve(process.env.SOCIS_AGENT_DESKTOP_SOCIS_AGENT_ROOT)

  if (overrideRoot && isSOCISSourceRoot(overrideRoot)) {
    const backend = createPythonBackend(overrideRoot, `SOCIS source at ${overrideRoot}`, backendArgs)
  ...
  //    installed `socis` on PATH so local Python edits are actually exercised.
  //    (In dev with no checkout, SOURCE_REPO_ROOT won't pass isSOCISSourceRoot.)
  if (!IS_PACKAGED && isSOCISSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `SOCIS source at ${SOURCE_REPO_ROOT}`, backendArgs)
  ...
  // 3. ACTIVE_SOCIS_AGENT_ROOT — the canonical install at
  //    %LOCALAPPDATA%\\socis\\socis-agent (Windows) or ~/.socis-agent/socis-agent.
  ...
  if (activeRuntime.shouldUseActiveRuntime && !bootstrapRepairRequested) {
    if (!activeRuntime.hasValidMarker) {
      rememberLog(
        `[bootstrap] Active SOCIS runtime at ${ACTIVE_SOCIS_AGENT_ROOT} is usable but the bootstrap marker is missing or stale; skipping first-run bootstrap.`
      )
    }
  ...
  // 4. Existing `socis` on PATH -- installed via install.ps1 / install.sh from
  //    a previous tool-only setup, or pip-installed system-wide. Use it but
  //    do NOT write a bootstrap marker; the user did this themselves and we
  //    don't want to take ownership of an install we didn't perform.
  //    SOCIS_AGENT_DESKTOP_IGNORE_EXISTING=1 forces the bootstrap path for testing.
  if (process.env.SOCIS_AGENT_DESKTOP_IGNORE_EXISTING !== '1') {
    let socisCommand = null
    const socisOverride = process.env.SOCIS_AGENT_DESKTOP_SOCIS

    if (socisOverride) {
      const resolvedOverride = findOnPath(socisOverride)

      if (resolvedOverride) {
        socisCommand = resolvedOverride
      } else if (!isWindowsBinaryPathInWsl(socisOverride, { isWsl: IS_WSL })) {
        socisCommand = socisOverride
      } else {
        rememberLog(`Ignoring Windows SOCIS override under WSL: ${socisOverride}`)
      }
    } else {
      socisCommand = findOnPath('socis')
    }

    if (socisCommand) {
      if (looksLikeDesktopAppBinary(socisCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving SOCIS CLI: ${socisCommand}`)
        socisCommand = null
      }
    }

    if (socisCommand) {
      const unwrapped = unwrapWindowsVenvSOCISCommand(socisCommand, backendArgs)

      if (unwrapped) {
        return unwrapped
      }

      // Smoke-test the candidate before trusting it. A `socis` shim
      ...
      const shellForProbe = isCommandScript(socisCommand)

      // SOCIS_AGENT_DESKTOP_SOCIS is an explicit deployment override (used by
      ...
      if (shouldTrustSOCISOverride(socisOverride) || verifySOCISCli(socisCommand, { shell: shellForProbe })) {
        ...
        // unwrapWindowsVenvSOCISCommand here: the second call repeats the
        // same un-memoized import probe, costing up to another full probe
        // timeout on the boot path for an answer we already have.
        return {
          label: `existing SOCIS CLI at ${socisCommand}`,
          command: socisCommand,
          args: backendArgs,
          bootstrap: false,
          env: {},
          kind: 'command',
      ...
      rememberLog(
        `Ignoring existing SOCIS CLI at ${socisCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }

  // 5. Last-ditch: pip-installed socis_cli module via system Python.
  ...
  if (python) {
    ...
    // having socis_cli installed -- common on dev boxes that have
    ...
    // a uv-managed 3.11 into %LOCALAPPDATA%\socis\socis-agent\venv.
    if (canImportSOCISCli(python)) {
      return {
        kind: 'python',
        label: `installed socis_cli module via ${python}`,
        command: python,
        args: ['-m', 'socis_cli.main', ...backendArgs],
        bootstrap: false,
        env: {},
    ...
    rememberLog(`Ignoring system Python ${python}: socis_cli is not importable; falling through to bootstrap.`)
  }
  ...
  //    resolveSOCISBackend was the old "no payload" path and forced the
  //    user into a dead end. With the bootstrap protocol, "no install yet"
  //    is a recoverable state the GUI can drive through.
  return {
    kind: 'bootstrap-needed',
    label: 'SOCIS Agent not installed yet; bootstrap required',
    command: null,
    args: backendArgs,
    bootstrap: true,
    env: {},
    ...
    activeRoot: ACTIVE_SOCIS_AGENT_ROOT,
...
async function ensureRuntime(backend) {
  ...
  // backend.kind === 'bootstrap-needed' means resolveSOCISBackend couldn't
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no SOCIS install found; starting first-launch bootstrap')

    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError: Error & { isBootstrapFailure?: boolean; bootstrapHandedOff?: boolean } = new Error(
        'SOCIS recovery was handed off to SOCIS Setup. The desktop will restart when recovery completes.'
      )
    ...
    const bootstrapResult = await runBootstrap({
      ...
      socisHome: SOCIS_AGENT_HOME,
      logRoot: path.join(SOCIS_AGENT_HOME, 'logs'),
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('SOCIS install was cancelled.') as any
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `SOCIS bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +
          `${bootstrapResult.error || 'unknown error'}. ` +
          `Check ${path.join(SOCIS_AGENT_HOME, 'logs', 'desktop.log')} for the full transcript.`
      ) as any

      bootstrapError.isBootstrapFailure = true
      bootstrapError.failedStage = bootstrapResult.failedStage || null
      // Latch the failure so subsequent startSOCIS() calls return this
      // same error without re-running install.ps1.  Cleared by the
      // socis:bootstrap:reset IPC (renderer's "Reload and retry").
    ...
    return ensureRuntime(resolveSOCISBackend(backend.args))
  }
  ...
  if (!isSOCISSourceRoot(ACTIVE_SOCIS_AGENT_ROOT)) {
    throw new Error(
      `SOCIS install at ${ACTIVE_SOCIS_AGENT_ROOT} is missing or incomplete. ` +
        'Reinstall via the desktop installer or scripts/install.ps1.'
    )
  }

  // On Windows, preflight Git Bash. SOCIS' terminal tool calls bash.exe
  ...
  // %LOCALAPPDATA%\socis\git\, which findGitBash() picks up, so for any
  // user who completed the bootstrap this is a no-op. For users who got
  // here via an external `socis` on PATH, this check still helps.
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      'Git for Windows is required for SOCIS on Windows (provides Git Bash, ' +
  ...
  if (!fileExists(venvPython)) {
    ...
    // normal flow because activeRuntimeState() requires isSOCISSourceRoot()
    // plus an importable socis_cli before it hands back the active runtime.
    // If we hit this, the user (or a deleted venv) broke the invariant; tell
    // them to re-run the install.
    throw new Error(
      `SOCIS venv missing at ${VENV_ROOT}. Re-run the desktop installer or ` + '`scripts/install.ps1` to rebuild it.'
    )
  }

  backend.command = getVenvPython(VENV_ROOT)
  backend.label = `SOCIS at ${ACTIVE_SOCIS_AGENT_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'SOCIS runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
...
function multipartBody(upload) {
  const boundary = `----socis-${crypto.randomBytes(12).toString('hex')}`
    ...
    (requestState: any) =>
      new Promise((resolve, reject) => {
        ...
        if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
          reject(new Error(`Unsupported SOCIS backend URL protocol: ${parsed.protocol}`))
                ...
                reject(
                  new Error(
                    `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                      'The endpoint is likely missing on the SOCIS backend.'
                  )
                )
        ...
        req.setTimeout(timeoutMs, () => {
          req.destroy(new Error(`Timed out connecting to SOCIS backend after ${timeoutMs}ms`))
        })
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = client.request(
      parsed,
      {
        agent,
        method: 'GET',
        headers: options.bearer ? { Authorization: `Bearer ${options.bearer}` } : { 'X-SOCIS-Session-Token': token }
      },
      res => {
...
function fetchPublicJson(url, options: any = {}) {
  ...
  // NO ``X-SOCIS-Session-Token`` header — used by the auth-mode probe before
...
function getLinkTitleSession() {
  ...
  linkTitleSession = session.fromPartition('socis:link-titles', { cache: false })
...
async function previewFileTarget(rawTarget, baseDir) {
  ...
  const base = baseDir ? path.resolve(expandUserPath(baseDir)) : resolveSOCISCwd()
...
function sendPreviewFileChanged(payload) {
  ...
  webContents.send('socis:preview-file-changed', payload)
}
...
// waitForSOCISReady how to read a 401 — rejected session vs gated route.
...
async function waitForSOCIS(baseUrl, token, signal?, authMode?, headers = {}) {
  const { probeHealth, probeIsCredentialed } = await buildReadinessHealthProbe(baseUrl, authMode, token)

  return waitForSOCISReady(baseUrl, {
    token,
    signal,
    fetchPublicJson,
...
function sendBackendExit(payload) {
  ...
  webContents.send('socis:backend-exit', payload)
}
...
function sendClosePreviewRequested() {
  ...
  webContents.send('socis:close-preview-requested')
}
...
function sendPreviewNavCommand(command: 'back' | 'forward' | 'reload') {
  ...
  webContents.send('socis:preview-nav', command)
}
...
function sendOpenFolderRequested() {
  ...
  webContents.send('socis:open-folder-requested')
}

// Tell the renderer the machine just woke. Sleep silently drops the
// renderer's WebSocket to the local backend; the renderer reconnects on this
// signal so the chat composer doesn't stay stuck on "Starting SOCIS...".
function sendPowerResume() {
  ...
  webContents.send('socis:power-resume')
}
...
// 'socis:power-battery' push below.
ipcMain.handle('socis:power-battery:get', () => onBatteryPower === true)
...
function broadcastBatteryState(next: boolean) {
  ...
  for (const win of BrowserWindow.getAllWindows()) {
    ...
    if (webContents && !webContents.isDestroyed()) {
      webContents.send('socis:power-battery', next)
    }
  }
}
...
function sendOpenUpdatesRequested() {
  ...
  webContents.send('socis:open-updates')
...
function sendWindowStateChanged(nextIsFullscreen?: boolean, target = mainWindow) {
  ...
  webContents.send('socis:window-state-changed', state)
}
...
function installContextMenuBridge(window: BrowserWindow) {
  window.webContents.on('context-menu', (_event, params) => {
    ...
    if (params.isEditable && params.misspelledWord) {
      window.webContents.send('socis:context-menu-spellcheck', {
...
//
// Hosted SOCIS gateways gate the dashboard behind an OAuth provider (e.g.
// SOCIS) instead of a static session token. The auth model is
// fundamentally different from the token path:
//
//   * REST is authed by HttpOnly session cookies (``socis_session_at``),
...
//     alongside the ~15-min access token (Portal NAS #293 / socis #37247).
//     Both are set as HttpOnly cookies (``socis_session_at`` ~15 min,
//     ``socis_session_rt`` 24h). When the AT cookie lapses but the RT cookie
...
// "not signed in", which on the initial boot path (startSOCIS → the renderer's
// single-shot boot() with no retry) surfaced as the "SOCIS couldn't start"
...
async function hasLiveOauthSession(baseUrl) {
  ...
  // for a signed-in user — the exact race that produced the transient "SOCIS
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: silent ? 'Connecting to SOCIS Cloud agent…' : 'Sign in to SOCIS gateway',
...
function fetchJsonViaOauthSession(url, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to SOCIS backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
// feature; the server half lives in socis_cli/dashboard_auth/native_flow.py.
// ---------------------------------------------------------------------------
...
// --- SOCIS Cloud discovery + silent per-agent sign-in (cloud-auto-discovery
// Phase 3) ---------------------------------------------------------------
//
...
// portal session cookie living in `persist:socis-remote-oauth`:
...
// convention (socis_cli/auth.py DEFAULT_NOUS_PORTAL_URL + the same env names)
// so a single override flips every SOCIS surface to the same portal.
const DEFAULT_NOUS_PORTAL_URL = 'https://portal.nousresearch.com'

function resolvePortalBaseUrl() {
  const raw = process.env.SOCIS_AGENT_PORTAL_BASE_URL || process.env.NOUS_PORTAL_BASE_URL || DEFAULT_NOUS_PORTAL_URL
...
// authenticates via PRIVY, not the SOCIS gateway session cookies, so this
// checks for the `privy-token` cookie on the portal host (NOT
// hasLiveOauthSession, which looks for socis_session_at/rt that the portal
    ...
    return await new Promise<boolean>(resolve => {
      ...
      try {
        win = new BrowserWindow({
          ...
          height: 720,
          show: false,
          title: 'Renewing SOCIS Cloud session…',
          autoHideMenuBar: true,
          webPreferences: {
            contextIsolation: true,
            nodeIntegration: false,
...
function openPortalLoginWindow() {
  ...
  return new Promise((resolve, reject) => {
    if (!app.isReady()) {
      reject(new Error('Desktop is not ready to start a SOCIS Cloud sign-in.'))
...
// Discover the hosted (SOCIS Cloud) agents the signed-in user can see. Calls
...
async function discoverCloudAgents(org?: string) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error(
      'You are not signed in to SOCIS Cloud. Open Settings → Gateway, choose SOCIS Cloud, and sign in.'
    ) as any
  ...
  try {
    ...
    if (body === undefined) {
      ...
      if (error && error.statusCode === 401) {
        const err = new Error(
          'Your SOCIS Cloud session has expired. Open Settings → Gateway and sign in again.'
        ) as any
...
async function cloudAgentSilentSignIn(dashboardUrl) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error('Your SOCIS Cloud session has expired. Sign in to SOCIS Cloud again.') as any
    err.needsCloudLogin = true
    throw err
  }
...
function sanitizeConnectionProfiles(raw: Record<string, any>) {
  ...
  for (const [name, entry] of Object.entries(raw)) {
    ...
    // Preserve the SOCIS Cloud org tag on cloud-mode entries so Settings can
...
function readDesktopConnectionsRegistry() {
  ...
  if (mtime === null) {
    ...
    try {
      ...
      // hard-failing every socis:connections:* call.
 ...
 * editor doesn't carry — cloud `org`, ssh `remoteSOCISPath`/`remoteProfile` —
...
// a valid stored value (pins the root SOCIS_AGENT_HOME explicitly); null means "no
...
// contains "socis", avoiding false positives from stale gateway.pid files
// whose PID was recycled by the OS to an unrelated process.
function isSOCISProcess(pid) {
  try {
...
//   1. Legacy ~/.socis-agent/active_profile (explicit CLI choice via socis profile use)
//   2. Running gateway (gateway.pid with verified liveness + socis identity)
...
function migrateActiveProfileIfMissing() {
  migrateActiveProfileIfMissingPure(DESKTOP_PROFILE_CONFIG_PATH, {
    legacyActivePath: path.join(SOCIS_AGENT_HOME, 'active_profile'),
    socisHome: SOCIS_AGENT_HOME,
    profilesRoot: path.join(SOCIS_AGENT_HOME, 'profiles'),
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  const envOverride = key ? false : Boolean(process.env.SOCIS_AGENT_DESKTOP_REMOTE_URL)
  ...
  const remoteUrl = envOverride ? String(process.env.SOCIS_AGENT_DESKTOP_REMOTE_URL || '') : String(block.url || '')
  ...
  return {
    ...
    remoteUrl,
    // The persisted SOCIS Cloud org (slug/id) for a cloud connection, or '' for
    ...
    sshRemoteSOCISPath: (ssh || savedSsh)?.remoteSOCISPath || '',
    sshRemoteProfile: (ssh || savedSsh)?.remoteProfile || '',
    // The env override only forces the global/primary connection; a per-profile
    // scope is never overridden by SOCIS_AGENT_DESKTOP_REMOTE_URL.
    envOverride
  }
}
...
// `org` (optional) is the SOCIS Cloud org slug/id the instance was discovered
...
function coerceDesktopConnectionConfig(input: any = {}, existing = readDesktopConnectionConfig(), options: any = {}) {
  ...
  // describe a discovered SOCIS Cloud instance, NOT a user-owned remote gateway,
...
function buildSshBlock(input: any, existingBlock: any = {}) {
  ...
  const merged = normalizeSshConfig({
    ...
    remoteSOCISPath: input.sshRemoteSOCISPath ?? existingBlock.remoteSOCISPath,
...
async function buildRemoteConnection(
  ...
  if (authMode === 'oauth') {
    ...
    // refresh token (socis #37247), and the gateway middleware transparently
    ...
    try {
      ...
      throw gatewayTicketFailure(
        ...
        'Could not reach the remote SOCIS gateway while refreshing its WebSocket ticket. Try reconnecting.'
      )
    }
  ...
  if (!token) {
    throw new Error(
      'Remote SOCIS gateway is selected, but no session token is saved. ' +
...
function activeSshTerminalTarget(webContentsId?: number) {
  ...
  const route = resolveDesktopRemoteRoute({
    config,
    env: {
      token: process.env.SOCIS_AGENT_DESKTOP_REMOTE_TOKEN,
      url: process.env.SOCIS_AGENT_DESKTOP_REMOTE_URL
    },
    profile,
    registry: readDesktopConnectionsRegistry()
  })
...
async function rollbackSshBootstrapResult(ssh, result, profile, sshConfig, boundaryError) {
  ...
  try {
    const expected = {
      ...
      socisPath: result.socisPath,
      socisHome: result.socisHome,
    ...
    if (result.platform?.os === 'Windows') {
      await terminateOwnedWindowsDashboardForUpdate(
        ssh,
        { socisPath: result.socisPath, socisHome: result.socisHome, python: result.pythonPath },
        expected
      )
...
async function bootstrapSshConnectionInner(profile, sshConfig, reuseToken, source, metadata, fingerprint, lease) {
  ...
  try {
    ...
    const platform = await detectRemotePlatform(ssh, sshConfig.remoteSOCISPath || '')
    const lifecycle = platform.os === 'Windows' ? connectWindowsRemote : remoteLifecycle.connect
    result = await lifecycle({
      ssh,
      profile: resolveRemoteSshDashboardProfile(sshConfig.remoteProfile, profile),
      remoteSOCISPath: sshConfig.remoteSOCISPath || '',
      ownershipId: sshOwnershipKey(profile),
      ...
      pickLocalPort,
      waitForSOCIS: (baseUrl, token) => waitForSOCIS(baseUrl, token, lease.signal, 'token'),
  ...
  await fenceManagedSshBootstrapPublication({
    ...
    publish: () => {
      ...
      sshConnections.set(scope, {
        ...
        host: sshConfig.host,
        hostLabel,
        socisVersion: result.socisVersion || '',
        ...
        // per-profile pools also use bare keys. Only startSOCIS' explicit call
  ...
  sshRememberLog(
    `[ssh] connection ${result.reused ? 'REUSED' : 'spawned'} dashboard: ` +
      `${result.socisVersion || 'socis (version unknown)'} at ${result.socisPath || '?'}`
  )
  ...
  return {
    ...connection,
    remoteSOCISVersion: result.socisVersion || '',
    ssh: {
      effectiveConfigFingerprint: sshConfig.effectiveConfigFingerprint,
      host: sshConfig.host,
...
//   2. env override (SOCIS_AGENT_DESKTOP_REMOTE_URL/_TOKEN) — applies app-wide
...
function globalRemoteActive() {
  if (process.env.SOCIS_AGENT_DESKTOP_REMOTE_URL) {
    return true
  }
...
// env → global) so a startSOCIS() failure can be classified as remote (never
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every SOCIS
  ...
  if (authRequired) {
    ...
    // button can read "Sign in with SOCIS" instead of a generic
...
async function testDesktopConnectionConfig(input: any = {}) {
  if (input.mode === 'ssh') {
    const sshConfig = normalizeSshConfig({
      ...
      remoteSOCISPath: input.sshRemoteSOCISPath
    })
    ...
    try {
      ...
      for (;;) {
        try {
          await ssh.open()
          const platform: any = await detectRemotePlatform(ssh, sshConfig.remoteSOCISPath || '')
          let socisPath
          let socisVersion
          let supported
          ...
          if (platform.os === 'Windows') {
            ...
            socisPath = runtime.socisPath
            const inspection = await helper(ssh, runtime, 'inspect', [runtime.socisPath])
            socisVersion = inspection.version
            supported = inspection.supported
          } else {
            socisPath = await remoteLifecycle.locateSOCIS(ssh, sshConfig.remoteSOCISPath || '')
            socisVersion = await remoteLifecycle.probeSOCISVersion(ssh, socisPath)
            supported = await remoteLifecycle.remoteSupportsSshOwnership(ssh, socisPath)
          }
          ...
          if (!supported) {
            return {
              ...
              error: 'Update SOCIS on the remote host before connecting with Desktop SSH.'
            }
          }
          ...
          return {
            ...
            remoteSOCISPath: socisPath,
            remoteSOCISVersion: socisVersion,
  ...
  if (wantRemote && block?.url) {
    ...
    const remote = (await resolveRemoteBackend(key)) || (await startSOCIS())
  ...
  // connect to SOCIS gateway". Mirror the renderer's connect here so the test
...
function resetSOCISConnection({ soft = false } = {}) {
  backendStartFailure = null
  remoteReauthFailure = null
  remoteLiveness.clear()
  const socisProcess = backendConnectionState.invalidate()
  stopBackendChild(socisProcess)
...
// startSOCIS() spawns fresh instead of racing the dying one. Shared by the
// connection-config and profile switch flows.
async function teardownPrimaryBackendAndWait({ soft = false } = {}) {
  // Capture the reference before resetSOCISConnection() invalidates it.
  const socisProcess = backendConnectionState.getProcess()
  const dying = socisProcess && !socisProcess.killed ? socisProcess : null
...
function sendConnectionApplied() {
  ...
  webContents.send('socis:connection:applied')
}
...
function broadcastConnectionsChanged(payload: { connectionId: string; reason: 'removed' | 'saved' | 'updated' }) {
  for (const win of BrowserWindow.getAllWindows()) {
    ...
    if (webContents && !webContents.isDestroyed()) {
      webContents.send('socis:connections:changed', payload)
    }
  }
}
...
async function ensureBackend(profile) {
  ...
  if (route.backend === 'primary') {
    const connection = await startSOCIS()
  ...
  // spawning, so two children never share one profile's SOCIS_AGENT_HOME.
  ...
  entry.connectionPromise = spawnPoolBackend(key, entry).catch(async error => {
    ...
    rememberLog(
      `SOCIS backend for profile "${key}" failed to start: ${error instanceof Error ? error.message : String(error)}`
    )
...
async function ensureRegistryBackend(connectionId, profile, managedUpdateCorrelation = '') {
  ...
  const resolveRegistrySshConfig = () => {
    ...
    if (!resolvedRegistrySshConfig) {
      resolvedRegistrySshConfig = normalizeSshConfig({
        ...
        remoteSOCISPath: source.remoteSOCISPath,
  ...
  if (source.kind === 'local') {
    ...
    localEntry.connectionPromise = spawnPoolBackend(profileKey, localEntry, {
      ...
      rememberLog(
        `SOCIS backend for profile "${profileKey}" (forced-local) failed to start: ${error instanceof Error ? error.message : String(error)}`
      )
...
async function connectRegistryBackend(
  ...
  if (source.kind === 'ssh') {
    ...
    return {
      ...
      // is only the routing label. socis:api uses it to translate explicit
      // self-profile query filters into the backend's namespace.
      remoteProfile: sshConfig.remoteProfile || '',
      logs: socisLog.slice(-80),
      ...getWindowState()
    }
  }
  ...
  await waitForSOCIS(connection.baseUrl, connection.token, undefined, connection.authMode, connection.headers)
  poolEntry.remoteBaseUrl = connection.baseUrl
...
function remoteUpdateTargetFromState(state): RemoteUpdateTarget {
  if (!state?.ssh || !state?.socisPath || !state?.socisHome) {
  ...
  return {
    ...
    socisPath: state.socisPath,
    socisHome: state.socisHome,
...
async function openManagedSshUpdateTransport(
  ...
  try {
    const platform: any = await detectRemotePlatform(ssh, config.remoteSOCISPath || '')

    if (platform.os === 'Windows') {
      const runtime = platform.socisPath ? platform : await probeWindowsRemote(ssh, config.remoteSOCISPath || '')
      ...
      return {
        ...
        target: {
          ...
          platform: 'Windows',
          socisPath: runtime.socisPath,
          socisHome: runtime.socisHome,
          pythonPath: runtime.python
        }
      }
    }

    const socisPath = await remoteLifecycle.locateSOCIS(ssh, config.remoteSOCISPath || '')
    const socisHome = await remoteLifecycle.probeRemoteSOCISHome(ssh)

    return {
      close: () => ssh.close(),
      target: { ssh, platform: platform.os, socisPath, socisHome }
    }
  } catch (error) {
    await ssh.close()
    throw error
...
async function drainManagedSshScope(scope) {
  ...
  try {
    ...
    if (state.remotePlatform === 'Windows') {
      await terminateOwnedWindowsDashboardForUpdate(
        state.ssh,
        { socisPath: state.socisPath, socisHome: state.socisHome, python: state.pythonPath },
        expected
      )
    ...
    if (state && state.localPort && state.remotePort && scope.reuseToken) {
      try {
        ...
        if (!forwardClosed) {
          await waitForSOCIS(`http://127.0.0.1:${state.localPort}`, scope.reuseToken, undefined, 'token')
        } else {
...
// local-spawn portion of startSOCIS() but without the boot-progress UI,
...
async function spawnPoolBackend(profile, entry, opts: { forceLocal?: boolean; poolKey?: string } = {}) {
  ...
  if (remote) {
    await waitForSOCIS(remote.baseUrl, remote.token, undefined, remote.authMode, remote.headers)
  ...
  // --profile wins over the inherited SOCIS_AGENT_HOME env (see _apply_profile_override
  // step 3 in socis_cli/main.py), so the child re-homes to this profile.
  // --port 0: the OS assigns an ephemeral port; the child announces it on stdout.
  const backendArgs = ['--profile', profile, 'serve', '--host', '127.0.0.1', '--port', '0']
  const backend = await ensureRuntime(resolveSOCISBackend(backendArgs))
  // Route old runtimes (no `serve`) through the legacy `dashboard --no-open`.
  backend.args = getBackendArgsForRuntime(backend)
  const socisCwd = resolveSOCISCwd()
  ...
  assertLocalProfileCanStart(profile, profileDeletionGate, key =>
    directoryExists(path.join(SOCIS_AGENT_HOME, 'profiles', key))
  )
  rememberLog(`Starting SOCIS backend for profile "${profile}" via ${backend.label}`)
  ...
  const child = spawn(
    ...
    hiddenWindowsChildOptions({
      ...
      env: {
        ...
        TERMINAL_CWD: socisCwd,
        SOCIS_AGENT_DASHBOARD_SESSION_TOKEN: token,
        // Marks this dashboard backend as desktop-spawned so it runs the cron
        // scheduler tick loop (the gateway isn't running under the app).
        SOCIS_AGENT_DESKTOP: '1',
        ...
        ...parentIdentityEnv,
        SOCIS_AGENT_WEB_DIST: webDist,
        ...(readyFile ? { SOCIS_AGENT_DESKTOP_READY_FILE: readyFile } : {})
      },
      shell: backend.shell,
      stdio: ['ignore', 'pipe', 'pipe']
    })
  ...
  child.once('error', error => {
    rememberLog(`SOCIS backend for profile "${profile}" failed to start: ${error.message}`)
    void teardownFailedLocalBackend(poolKey, entry).catch(cleanupError => {
      rememberLog(
        `SOCIS backend for profile "${profile}" cleanup failed: ${cleanupError instanceof Error ? cleanupError.message : String(cleanupError)}`
      )
    })
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(`SOCIS backend for profile "${profile}" exited (${signal || code})`)
    releaseLocalBackendSlot(entry)
    releaseBackendChild(child)
    ...
    if (!ready) {
      rejectStart?.(
        new Error(
          `SOCIS backend for profile "${profile}" exited before it became ready (${signal || code}).${outputTail.describe()}`
        )
      )
    }
  })
  ...
  await Promise.race([waitForSOCIS(baseUrl, token), startFailed])
  ready = true

  const authToken = await adoptServedDashboardToken(baseUrl, token, {
    childAlive: () => child.exitCode === null && !child.killed,
    label: `SOCIS backend for profile "${profile}"`,
    rememberLog
  })
  ...
  if (!wsProbe.ok) {
    throw new Error(
      `SOCIS backend for profile "${profile}" is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
    )
  }
...
// backend whose ensure_socis_agent_home() recreates the deleted profile directory.
//
...
async function startSOCIS() {
  ...
  if (!isPrimaryInstance) {
    ...
    throw new Error('SOCIS Agent Desktop is already running in another window.')
  }

  await reapOrphanedBackendsOnce()

  // Latched-failure short-circuit: once bootstrap has failed in this
  // process, every subsequent startSOCIS() call re-throws the same error
  ...
  if (BOOT_FAKE_ERROR) {
    await advanceBootProgress('backend.resolve', 'Resolving SOCIS backend', 8)
  ...
  const connectionPromise = (async () => {
    const connectRemote = async remote => {
      ...
      if (!backendConnectionState.isCurrentAttempt(connectionAttempt)) {
        throw new Error('SOCIS backend start was superseded by a newer connection attempt.')
      }

      await advanceBootProgress('backend.remote', `Connecting to remote SOCIS backend at ${remote.baseUrl}`, 24)
      await waitForSOCIS(remote.baseUrl, remote.token, undefined, remote.authMode, remote.headers)
      ...
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote SOCIS backend is ready',
        progress: 94,
        running: true,
        error: null
      })

      return createPrimaryRemoteConnection(remote, socisLog.slice(-80), getWindowState())
    }
    ...
    // deterministic (it wins over the sticky ~/.socis-agent/active_profile file) and
    // resolves SOCIS_AGENT_HOME the same way `socis -p <name>` does on the CLI. An
    ...
    const setup = await runPrimaryBackendStartup({
      ...
      prepareLocalBackend: async () => {
        await advanceBootProgress('backend.runtime', 'Resolving SOCIS runtime', 28)

        return resolveSOCISBackend(backendArgs)
      },
      resolveRemote: () => {
    ...
    await advanceBootProgress('backend.spawn', `Starting SOCIS backend via ${backend.label}`, 84)
    rememberLog(`Starting SOCIS backend via ${backend.label}`)
    ...
    const socisProcess = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Explicitly pin SOCIS_AGENT_HOME for the child so Python's get_socis_agent_home()
          // resolves to the SAME location our resolveSOCISHome() picked. Without
          // this pin, Python falls back to ~/.socis-agent on every platform — fine on
          // mac/linux (where our default matches), but on Windows our default is
          // %LOCALAPPDATA%\socis, which differs from C:\Users\<u>\.socis.
          // Mismatch would split config / sessions / .env / logs across two
          // directories. install.ps1 sets SOCIS_AGENT_HOME via setx; the desktop
    ...
    primaryOutputTail.attach(socisProcess)
    ...
    const portAnnouncement = waitForDashboardPortAnnouncement(socisProcess, {
    ...
    const processOwner = backendConnectionState.attachProcess(connectionAttempt, socisProcess)

    if (!processOwner) {
      stopBackendChild(socisProcess)
      await waitForBackendExit(socisProcess)
      releaseBackendChild(socisProcess)
      throw new Error('SOCIS backend start was superseded by a newer connection attempt.')
    }

    socisProcess.stdout.on('data', rememberLog)
    socisProcess.stderr.on('data', rememberLog)
    ...
    socisProcess.once('error', error => {
      releaseBackendChild(socisProcess)

      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale SOCIS backend error: ${error.message}`)
        rejectBackendStart?.(new Error('SOCIS backend start was superseded by a newer connection attempt.'))

        return
      }

      rememberLog(`SOCIS backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `SOCIS backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    socisProcess.once('exit', (code, signal) => {
      releaseBackendChild(socisProcess)

      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale SOCIS backend exit (${signal || code})`)
      ...
      rememberLog(`SOCIS backend exited (${signal || code})`)
      sendBackendExit({ code, signal })

      if (!backendReady) {
        const message = `SOCIS backend exited before it became ready (${signal || code}).${primaryOutputTail.describe()}`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `SOCIS backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentSOCISLog()}`
          )
        )
      }
    })

    await advanceBootProgress('backend.port', 'Waiting for SOCIS backend to launch', 86)
    ...
    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for SOCIS backend to become ready', 90)
    await Promise.race([waitForSOCIS(baseUrl, token), backendStartFailed])
    backendReady = true
    backendStartFailure = null

    const authToken = await adoptServedDashboardToken(baseUrl, token, {
      childAlive: () => socisProcess.exitCode === null && !socisProcess.killed,
      rememberLog
    })
    ...
    if (!wsProbe.ok) {
      throw new Error(
        `Local SOCIS backend is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
      )
    }

    updateBootProgress({
      phase: 'backend.ready',
      message: 'SOCIS backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
...
function notifyBrowserPopoutClosed(tabId) {
  ...
  for (const other of BrowserWindow.getAllWindows()) {
    if (!other.isDestroyed()) {
      other.webContents.send('socis:browser-popout:closed', tabId)
    }
  }
}
...
// here so it can leave the app's bounds and stay visible while SOCIS is
...
// pushes pet state over IPC (socis:pet-overlay:state); the overlay just renders
// it. Control flows back (pop-in, composer submit) via socis:pet-overlay:control.
...
function spawnPetOverlayWindow(bounds) {
  const win = new BrowserWindow({
    ...
    // instead, leaving the main SOCIS app as the Dock/cmd-tab anchor.
    skipTaskbar: !IS_MAC,
    hasShadow: false,
    alwaysOnTop: true,
    ...
    // anchor and the SOCIS icon drops out of cmd/alt-tab — especially when the
    // main window is minimized. We flip this on only while the composer needs
    // the keyboard (see socis:pet-overlay:set-focusable).
    focusable: false,
    show: false,
  ...
  try {
    ...
    // which briefly hides the Dock/cmd-tab presence. Keep SOCIS in the normal
  ...
  win.on('closed', () => {
    ...
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send('socis:pet-overlay:control', { type: 'pop-in' })
    }
  })
...
// window showing only the composer and its scrollback, so SOCIS can be driven
...
function startHudCursorFeed(win: BrowserWindow) {
  ...
  const timer = setInterval(() => {
    ...
    last = key
    win.webContents.send('socis:hud:cursor', point)
  }, HUD_CURSOR_POLL_MS)
...
function startHudGameOverlayFeed(win: BrowserWindow) {
  ...
  const push = (state: { active: boolean; app: string }) => {
    if (!win.isDestroyed()) {
      win.webContents.send('socis:hud:game-overlay', state)
    }
  }
  ...
  const enumerate = async () => {
    ...
    if (!reported) {
      reported = true
      console.warn(`[socis] HUD cannot enumerate windows: ${windows.reason}`)
    }
...
function broadcastHudState(open) {
  ...
  for (const win of BrowserWindow.getAllWindows()) {
    if (!win.isDestroyed()) {
      win.webContents.send('socis:hud:changed', payload)
    }
  }
}
...
function spawnHudWindow(sessionId, profile) {
  const win = new BrowserWindow({
    ...
    // `socis:hud:set-bounds`, which flips resizable on for the call — the
  ...
  // grab, socis:hud:workspace-transfer temporarily makes the X11 window
...
function openHudWindow(sessionId, profile) {
  ...
  if (hudWindow && !hudWindow.isDestroyed()) {
    ...
    if (sessionId && sessionId !== hudSessionId) {
      ...
      hudWindow.webContents.send('socis:hud:goto', sessionId)
...
function spawnQuickEntryWindow() {
  ...
  win.webContents.on('did-finish-load', () => {
    if (!win.isDestroyed() && quickEntryLastState) {
      win.webContents.send('socis:quick-entry:state', quickEntryLastState)
    }
  })
...
function showQuickEntryWindow() {
  ...
  quickEntryWindow.webContents.send('socis:quick-entry:shown')
}
...
function createWindow() {
    ...
    callbacks: {
      ...
      onFailedLoadBudgetExhausted: details => {
        ...
        void loadRendererLoadErrorPage(mainWindow, {
          ...
          repairHint: 'socis desktop --force-build',
  ...
  startSOCIS().catch(error => rememberLog(error.stack || error.message))
...
ipcMain.handle('socis:connection', async (_event, profile) => {
...
ipcMain.handle('socis:connection:for', async (_event, payload) => {
  const { connectionId, profile } = payload && typeof payload === 'object' ? (payload as any) : ({} as any)
  const registry = readDesktopConnectionsRegistry()
  const id = String(connectionId || '').trim() || registry.primary
  // Same single-owner claim as 'socis:connection', keyed by the composite
...
ipcMain.on('socis:connection:active-route', (event, route) => {
...
// "Starting SOCIS…". Before the renderer's backoff loop reconnects, it asks us
// to confirm the cached PRIMARY backend is still reachable; if a remote one is
// not, we drop the cache so the next getConnection() rebuilds it. Local backends
// self-heal via their child 'exit' handler, so we never touch them here.
ipcMain.handle('socis:connection:revalidate', async () => {
  ...
  return remoteRevalidation.run(connectionPromise, async () => {
    const [result] = await Promise.all([
      revalidateRemoteConnection({
        ...
        resetConnection: () => resetSOCISConnection({ soft: true }),
...
ipcMain.handle('socis:backend:touch', async (_event, profile) => {
...
ipcMain.handle('socis:pool-limits:get', async () => ({ ...poolLimits }))
ipcMain.handle('socis:pool-limits:set', async (_event, raw) => {
...
ipcMain.handle('socis:gateway:ws-url', async (_event, profile) => {
  return gatewayWsUrlIpcResult(() => freshGatewayWsUrl(profile))
})
ipcMain.handle('socis:window:openSession', async (_event, sessionId, opts) => {
...
ipcMain.handle('socis:window:openInstance', async () => {
  createInstanceWindow()

  return { ok: true }
})
ipcMain.handle('socis:window:openBrowser', async (_event, tabId) => {
...
ipcMain.handle('socis:window:openInTerminal', async (_event, sessionId, opts) => {
  ...
  try {
    ...
    const backend = resolveSOCISBackend(tuiResumeArgs(sessionId.trim(), profile || undefined))

    if (!backend.command) {
      return { ok: false, error: 'SOCIS is not installed yet' }
    }
    ...
    const scriptPath = path.join(
      scriptDir,
      `socis-${crypto.randomBytes(6).toString('hex')}${terminalScriptExtension()}`
    )
    ...
    fs.writeFileSync(
      ...
      buildTerminalScript({
        ...
        cwd,
        env: terminalScriptEnv(backend.env, SOCIS_AGENT_HOME)
      }),
      { mode: 0o700 }
    )
...
ipcMain.handle('socis:wake-indicator:get', () => wakeIndicatorController.getState())
ipcMain.on('socis:wake-indicator:set', (_event, state) => {
...
ipcMain.handle('socis:zoom:get', event => {
...
ipcMain.on('socis:zoom:set-percent', (event, percent) => {
  const window = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('socis:backend:recycle', async (_event, profile) => {
...
ipcMain.handle('socis:bootstrap:reset', async () => {
  // Renderer's "Reload and retry" path. Clear the latched failure and
  // reset connection state so the next startSOCIS() call restarts the
...
ipcMain.handle('socis:bootstrap:repair', async () => {
  // Forceful repair: force the next startSOCIS() through the full installer
  ...
  // "bypass active runtime" path inside startSOCIS() and fall through
...
ipcMain.handle('socis:boot-progress:get', async () => bootProgressState)
ipcMain.handle('socis:bootstrap:get', async () => getBootstrapState())
ipcMain.handle('socis:connection-config:get', async (_event, profile) =>
  sanitizeDesktopConnectionConfig(readDesktopConnectionConfig(), profile)
)
ipcMain.handle('socis:plugin-profile-routes', async (_event, rawProfileNames) => {
...
ipcMain.handle('socis:ssh-config:hosts', async () => ({ hosts: collectSshConfigHosts() }))
ipcMain.handle('socis:ssh-config:resolve', async (_event, host) => {
...
ipcMain.handle('socis:connection-config:test', async (_event, payload) => testDesktopConnectionConfig(payload))
...
ipcMain.handle('socis:secret-storage:get', async () => ({ on: secretStoragePolicy().on }))
ipcMain.handle('socis:secret-storage:set', async (_event: any, on: any) => applySecretStorageEncryption(on === true))
...
ipcMain.handle('socis:connections:list', async () => sanitizeConnectionsRegistry())
ipcMain.handle('socis:connections:save', async (_event, payload) => {
  const saved = await saveRegistryConnection(payload)

  return { ok: true, connection: saved, registry: sanitizeConnectionsRegistry() }
})
ipcMain.handle('socis:connections:remove', async (_event, id) => {
  const key = String(id || '')
...
ipcMain.handle('socis:connections:set-primary', async (_event, id) => {
...
ipcMain.handle('socis:connections:set-launch-mode', async (_event, mode) => {
  assertCanMutateManagedPrimaryRouting()
...
ipcMain.handle('socis:connections:set-last-used', async (_event, id) => {
  const registry = setLastUsedConnection(readDesktopConnectionsRegistry(), String(id || ''))
  writeDesktopConnectionsRegistry(registry)

  return { ok: true, registry: sanitizeConnectionsRegistry(registry) }
})
ipcMain.handle('socis:connections:test', async (_event, id) => {
  const registry = readDesktopConnectionsRegistry()
  ...
  if (entry.kind === 'ssh') {
    const result = await testDesktopConnectionConfig({
      ...
      sshRemoteSOCISPath: entry.remoteSOCISPath
    })
...
async function probeSshProfileInventory(connection) {
  ...
  const sshConfig = normalizeSshConfig({
    ...
    remoteSOCISPath: connection.remoteSOCISPath
  })
  ...
  try {
    await ssh.open()
    const profiles = await remoteLifecycle.listRemoteSOCISProfiles(ssh)
    ...
    registry.connections.map(async connection => {
      ...
      try {
        ...
        if (connection.kind === 'ssh') {
          ...
          // The root SOCIS_AGENT_HOME is an agent too; enumerations that omit it
...
ipcMain.handle('socis:agents:roster', async () => {
  const registry = readDesktopConnectionsRegistry()
  const enumerations = await enumerateRegistryAgentSources(registry)
...
// socis:gateway:ws-url. Same single-use-ticket discipline for OAuth sources.
...
ipcMain.handle('socis:gateway:ws-url-for', async (_event, payload) => {
...
ipcMain.handle('socis:connections:update-managed', async (_event, rawId) => requestManagedSshUpdate(rawId))

// Fan out `socis update` to every eligible registered connection at once.
...
ipcMain.handle('socis:connections:update-all', async (_event, payload) => {
  const registry = readDesktopConnectionsRegistry()
    ...
    registry.connections
      ...
      .map(async connection => {
        ...
        try {
          ...
          const body: any = await postJsonForBackend(descriptor, '/api/socis/update', {}, { timeoutMs: 15_000 })
...
// analogue of the socis:api handler's own auth split: OAuth backends prefer a
...
ipcMain.handle('socis:connection-config:probe', async (_event, rawUrl) => probeRemoteAuthMode(rawUrl))
ipcMain.handle('socis:connection-config:oauth-login', async (_event, rawUrl) => {
  ...
  if (strategy === 'native') {
    try {
      ...
      // startSOCIS() re-dials instead of replaying the stale rejection.
      remoteReauthFailure = null
...
ipcMain.handle('socis:connection-config:oauth-logout', async (_event, rawUrl) => {
  const baseUrl = normalizeRemoteBaseUrl(rawUrl)
...
// --- SOCIS Cloud (cloud-auto-discovery Phase 3) ---
...
ipcMain.handle('socis:cloud:agent-sign-in', async (_event, dashboardUrl) => {
...
ipcMain.handle('socis:connection-config:save', async (_event, payload) => {
  assertCanMutateManagedPrimaryRouting()
...
ipcMain.handle('socis:connection-config:apply', async (_event, payload) => {
  assertCanMutateManagedPrimaryRouting()
...
ipcMain.handle('socis:profile:get', async () => ({ profile: readActiveDesktopProfile() }))
// Persistence-only sibling of socis:profile:set: records the profile the
...
// the gateway (#79886).
ipcMain.handle('socis:profile:remember', async (_event, name) => ({
  profile: writeActiveDesktopProfile(name)
}))
ipcMain.handle('socis:profile:set', async (_event, name) => {
  assertCanMutateManagedPrimaryRouting()
  const next = writeActiveDesktopProfile(name)

  // Switching profiles is a backend re-home: relaunch the dashboard under the
  // new SOCIS_AGENT_HOME. Pool backends keep their own homes, so only the primary
  // is torn down.
  await teardownPrimaryBackendAndWait()
  mainWindow?.reload()
...
ipcMain.on('socis:previewShortcutActive', (_event, active) => {
  previewShortcutActive = Boolean(active)
})

ipcMain.handle('socis:requestMicrophoneAccess', async () => {
...
ipcMain.handle('socis:window:readBelow', async event => {
...
async function handleSOCISApiRequest(request) {
  ...
  // backend calls ensure_socis_agent_home() which recreates the profile directory,
...
ipcMain.handle('socis:api', async (_event, request) => {
  ...
  // respawn the old-name backend and recreate its SOCIS_AGENT_HOME (#45474).
  ...
  if (!mutatingProfile) {
    return handleSOCISApiRequest(request)
  }

  const releaseProfileDeletion = profileDeletionGate.acquire(mutatingProfile)

  return handleSOCISApiRequest(request).finally(releaseProfileDeletion)
})
...
ipcMain.handle('socis:ambient:claim', (_event, key) => !claimedAmbientCue(String(key ?? '')))

ipcMain.handle('socis:notify', (_event, payload) => {
  ...
  notification.on('click', () => {
    ...
    if (payload?.sessionId) {
      mainWindow.webContents.send('socis:focus-session', payload.sessionId)
    }

    // Plugin / session-less activation — serializable path (+ optional notifyId
    // for renderer callbacks). Same vocabulary as socis://index-network/….
    if (payload?.activate || payload?.notifyId) {
      mainWindow.webContents.send('socis:notification-activate', {
  ...
  notification.on('action', (_actionEvent, index) => {
    ...
    if (payload?.sessionId && !payload?.notifyId && !payload?.activate) {
      mainWindow.webContents.send('socis:notification-action', { sessionId: payload.sessionId, actionId: action.id })
...
ipcMain.handle('socis:data-url-read-max:get', () => ({
...
ipcMain.handle('socis:data-url-read-max:set', (_event, maxMb) => {
...
ipcMain.handle('socis:readFileDataUrl', async (_event, filePath) => {
...
ipcMain.handle('socis:readFileDataUrlForAttach', async (_event, filePath) => {
  return readFileDataUrlForIpc(filePath, {
...
ipcMain.handle('socis:readFileText', async (_event, filePath) => {
...
// `socis:readFileText` is the *preview* read and silently truncates at
...
ipcMain.handle('socis:readPluginSource', async (_event: unknown, filePath: unknown) => {
  const { resolvedPath, stat } = await resolveReadableFileForIpc(filePath, {
...
ipcMain.handle('socis:selectPaths', async (_event, options: any = {}) => {
...
ipcMain.handle('socis:writeClipboard', (_event, text) => {
...
ipcMain.handle('socis:selectSavePath', async (_event, options: any = {}) => {
  const result = await dialog.showSaveDialog(mainWindow, {
...
ipcMain.handle('socis:readClipboard', () => clipboard.readText())

ipcMain.handle('socis:saveGatewayFile', (_event, payload) => saveGatewayFile(payload))

ipcMain.handle('socis:saveImageFromUrl', (_event, url) => saveImageFromUrl(String(url || '')))
...
ipcMain.handle('socis:context-menu:edit', (event, command) => {
...
ipcMain.handle('socis:context-menu:copy-image', event => {
...
ipcMain.handle('socis:context-menu:spellcheck', (event, action) => {
...
ipcMain.handle('socis:context-menu:guest-add-word', (_event, payload) => {
...
ipcMain.handle('socis:capturePreview', async (_event, payload) => {
  const guest = electronWebContents.fromId(Number(payload?.webContentsId))

  return capturePreviewContents(guest, payload?.rect, payload?.viewport)
})

ipcMain.handle('socis:saveImageBuffer', async (_event, payload) => {
...
ipcMain.handle('socis:saveClipboardImage', async () => {
...
ipcMain.handle('socis:normalizePreviewTarget', (_event, target, baseDir) =>
  normalizePreviewTarget(String(target || ''), baseDir ? String(baseDir) : '')
)

ipcMain.handle('socis:watchPreviewFile', (_event, url) => watchPreviewFile(String(url || '')))

ipcMain.handle('socis:watchDirectory', (_event, dir) => watchDirectory(String(dir || '')))

ipcMain.handle('socis:stopPreviewFileWatch', (_event, id) => stopPreviewFileWatch(String(id || '')))
...
ipcMain.on('socis:active-work', (event, payload) => {
  const id = event.sender.id
...
ipcMain.on('socis:titlebar-theme', (_event, payload) => {
...
ipcMain.on('socis:native-theme', (_event, mode) => {
...
ipcMain.on('socis:translucency:support', event => {
...
ipcMain.on('socis:keep-awake', (_event, on) => {
...
ipcMain.handle('socis:quick-entry:settings:get', async () => {
...
ipcMain.handle('socis:quick-entry:settings:set', async (_event, patch) => {
...
ipcMain.on('socis:quick-entry:submit', (_event, payload) => {
  hideQuickEntryWindow()
  ...
  mainWindow.webContents.send('socis:quick-entry:submit', {
...
ipcMain.on('socis:quick-entry:state', (_event, payload) => {
  quickEntryLastState = payload ?? null

  if (quickEntryWindow && !quickEntryWindow.isDestroyed()) {
    quickEntryWindow.webContents.send('socis:quick-entry:state', payload)
  }
})

ipcMain.on('socis:quick-entry:dismiss', () => hideQuickEntryWindow())
...
ipcMain.on('socis:devtools:disable-f12', (_event, on) => {
...
ipcMain.handle('socis:openExternal', (_event, url) => {
...
// per-session secondary windows spawned via `socis:window:openSession`).
...
ipcMain.handle('socis:find-in-page', async (event, query, options) => {
  const win = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('socis:stop-find-in-page', event => {
  const win = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('socis:preview:reach', async (event, url) => reachablePreviewUrl(event.sender.id, String(url || '')))

ipcMain.handle('socis:openPreviewInBrowser', async (_event, url) => {
...
// it via writeDefaultProjectDir so resolveSOCISCwd picks it up on the next
// session spawn (no app restart needed).
ipcMain.handle('socis:setting:defaultProjectDir:get', async () => ({
  dir: readDefaultProjectDir(),
  defaultLabel: app.getPath('home'),
  resolvedCwd: resolveSOCISCwd()
}))

ipcMain.handle('socis:workspace:sanitize', async (_event, cwd) => sanitizeWorkspaceCwd(cwd))

ipcMain.handle('socis:setting:defaultProjectDir:set', async (_event, dir) => {
...
ipcMain.handle('socis:setting:defaultProjectDir:pick', async () => {
...
ipcMain.handle('socis:fetchLinkTitle', (_event, url) => fetchLinkTitle(url))

ipcMain.handle('socis:resolveFavicon', (_event, url) => resolveFaviconCached(url))

ipcMain.handle('socis:logs:reveal', async () => {
  try {
    await fs.promises.mkdir(path.dirname(DESKTOP_LOG_PATH), { recursive: true })
...
ipcMain.handle('socis:logs:recent', async () => ({ path: DESKTOP_LOG_PATH, lines: socisLog.slice(-200) }))
...
ipcMain.on('socis:logs:renderer-error', (_event, report) => {
...
// Embedded terminal PTY host (socis:terminal:*) — see terminal-ipc.ts.
...
ipcMain.handle('socis:updates:check', async () =>
...
ipcMain.handle('socis:updates:apply', async (_event, payload) =>
...
ipcMain.handle('socis:updates:branch:get', async () => readDesktopUpdateConfig())

ipcMain.handle('socis:updates:branch:set', async (_event, name) => {
...
// Resolve the canonical SOCIS version (the one `release.py` bumps in
// socis_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real SOCIS version instead of the Electron app's own package.json version,
// which historically drifted (stuck at 0.0.2). Falls back to app.getVersion()
// when the source tree can't be read (e.g. a packaged build without the repo).
function resolveSOCISVersion() {
  try {
    const root = resolveUpdateRoot()
    const initPath = path.join(root, 'socis_cli', '__init__.py')
...
// Renderer-bundle skew: `socis update` moves the SOURCE TREE, but the UI
...
// Re-resolve the live SOCIS version and push it into the native About panel
// just before showing it, so an in-place `socis update` is reflected without
  ...
  void detectRendererSkew().then(skew => {
    app.setAboutPanelOptions({
      ...
      applicationVersion: skew.outOfSync
        ? `${resolveSOCISVersion()} — app build out of date, update the desktop app`
        : resolveSOCISVersion(),
      copyright: 'Copyright © 2026 SOCIS'
    })
    app.showAboutPanel()
...
ipcMain.handle('socis:version', async () => {
  const skew = await detectRendererSkew()

  return {
    appVersion: resolveSOCISVersion(),
    electronVersion: process.versions.electron,
    nodeVersion: process.versions.node,
    platform: process.platform,
    socisRoot: resolveUpdateRoot(),
...
// The About page's "Restart SOCIS" button (shown when bundleSwapPending):
...
ipcMain.handle('socis:app:relaunch', async () => {
...
// the actual removal via `socis uninstall …` so the cross-platform PATH /
// registry / service / node-symlink cleanup all lives in one place
// (socis_cli/uninstall.py + socis_cli/gui_uninstall.py).
//
...
async function getUninstallSummary() {
  ...
  const agentRoot = ACTIVE_SOCIS_AGENT_ROOT
  ...
  const fallback = () => ({
    ...
    agent_installed: isSOCISSourceRoot(agentRoot) && fileExists(py),
    gui_installed: true,
  ...
  return new Promise(resolve => {
    ...
    try {
      const child = spawn(
        py,
        ['-m', 'socis_cli.main', 'uninstall', '--gui-summary'],
        hiddenWindowsChildOptions({
          cwd: agentRoot,
          env: { ...process.env, SOCIS_AGENT_HOME, NO_COLOR: '1' },
          stdio: ['ignore', 'pipe', 'ignore']
        })
      )
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no SOCIS agent venv at ${VENV_ROOT}.`
    }
  }
  ...
  // Python with PYTHONPATH=<agentRoot> so `import socis_cli` resolves from
  ...
  if (modeRemovesAgent(mode)) {
    ...
    if (sysPy) {
      py = sysPy
      pythonPath = ACTIVE_SOCIS_AGENT_ROOT
    } else if (IS_WINDOWS) {
      rememberLog(
  ...
  try {
    await releaseBackendLock(ACTIVE_SOCIS_AGENT_ROOT, 'uninstall')
  } catch (error) {
  ...
  const scriptArgs = {
    ...
    pythonExe: py,
    pythonPath,
    agentRoot: ACTIVE_SOCIS_AGENT_ROOT,
    uninstallArgs,
  ...
  try {
    if (IS_WINDOWS) {
      scriptPath = path.join(app.getPath('temp'), `socis-uninstall-${Date.now()}.cmd`)
      ...
      scriptPath = path.join(app.getPath('temp'), `socis-uninstall-${Date.now()}.sh`)
...
ipcMain.handle('socis:uninstall:summary', async () => getUninstallSummary())
ipcMain.handle('socis:uninstall:run', async (_event, payload) => {
...
ipcMain.handle('socis:vscode-theme:fetch', async (_event, id) => fetchMarketplaceThemes(String(id || '')))

// Search the Marketplace for color-theme extensions (empty query = top installs).
ipcMain.handle('socis:vscode-theme:search', async (_event, query) => searchMarketplaceThemes(String(query || ''), 20))

// ---------------------------------------------------------------------------
// socis:// deep links (e.g. socis://blueprint/morning-brief?time=08:00,
// socis://mcp/install?name=NAME&config=B64 — the vendor "Add to SOCIS"
// button, or socis://plugin/install?repo=owner/repo). Dev
// (`SOCIS_AGENT_DESKTOP_DEV_SERVER`) registers socis-dev:// instead — bare
// Electron or a stale OS handler often owns socis:// on dev machines.
...
// ---------------------------------------------------------------------------
const SOCIS_AGENT_PROTOCOL = DEV_SERVER ? 'socis-dev' : 'socis'
/** Schemes accepted when parsing inbound URLs (dev accepts both). */
const DEEPLINK_SCHEMES = DEV_SERVER ? ['socis-dev', 'socis'] : ['socis']
...
function handleDeepLink(url) {
  ...
  // socis://blueprint/<key>?slot=val  -> host="blueprint", path="/<key>"
  ...
  try {
    ...
    mainWindow.focus()
    mainWindow.webContents.send('socis:deep-link', payload)
...
ipcMain.handle('socis:deep-link-ready', () => {
  ...
  if (_pendingDeepLink) {
    ...
    handleDeepLink(
      `${SOCIS_AGENT_PROTOCOL}://${queued.kind}/${encodeURIComponent(queued.name)}` +
...
function registerDeepLinkProtocol() {
  try {
    if (process.defaultApp && process.argv.length >= 2) {
      ...
      app.setAsDefaultProtocolClient(SOCIS_AGENT_PROTOCOL, process.execPath, [entry])
    } else {
      app.setAsDefaultProtocolClient(SOCIS_AGENT_PROTOCOL)
    }

    rememberLog(`[deeplink] registered ${SOCIS_AGENT_PROTOCOL}:// handler`)
  } catch (err) {
...
// second-instance argv. Without the lock a second `socis://` launch spawns a
...
if (!isPrimaryInstance) {
  ...
  // startup (shortcut registration, createWindow → startSOCIS), whose
...
app.whenReady().then(() => {
  ...
  // startSOCIS() finishes resolving the configured backend.
  const primaryProfile = primaryProfileKey()
  ...
  // Win/Linux cold start: the launching socis:// URL is in our own argv.
...
app.on('before-quit', event => {
  ...
  // quitting SOCIS never keeps another app's chord hostage.
