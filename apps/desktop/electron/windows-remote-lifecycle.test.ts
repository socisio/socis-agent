...
test('Windows relaunch gate refuses live and uncertain markers before executing the remote runtime', async () => {
  for (const observation of ['LIVE:4242', 'UNCERTAIN']) {
    ...
    const ssh = sshWith(async command => {
      ...
      if (script.includes('Get-Command socis.exe')) {
        return JSON.stringify({
          os: 'Windows',
          arch: 'AMD64',
          socisHome: 'C:\\Users\\alice\\.socis',
          socisPath: 'C:\\SOCIS\\socis.exe',
          python: 'C:\\SOCIS\\python.exe'
        })
      }

      if (script.includes('.socis-update-in-progress')) {
        return observation
      }
    ...
    assert.equal(
      scripts.some(script => script.includes('socis_cli.windows_ssh_runtime')),
      false
    )
  }
})
...
test('Windows relaunch gate uses strict install-wide marker parsing and fail-closed PID probing', async () => {
  ...
  await assertWindowsRemoteInstallUpdateClear(ssh, 'C:\\Users\\alice\\.socis\\profiles\\research')
  assert.match(script, /\.socis-update-in-progress/)
...
test('Windows probe validates SOCIS and Python topology before selection', async () => {
  let script = ''
  ...
  const fallbackJoin = script.indexOf('Join-Path $socisHome')
  ...
  await assert.rejects(
    detectRemotePlatform(
      sshWith(async command => {
        ...
        throw new Error('SOCIS is not installed on the remote Windows host.')
      })
    ),
    (err: any) => err.kind === 'unsupported-platform' && /SOCIS is not installed/.test(err.message)
  )
})

test('helper command uses the fixed remote Python entry point and quotes path data', () => {
  const command = helperCommand({ python: "C:\\Program Files\\SOCIS's\\python.exe" }, 'inspect', [
    'C:\\x y\\socis.exe'
  ])

  const encoded = command.split(' ').pop()!
  const script = Buffer.from(encoded, 'base64').toString('utf16le')
  assert.match(script, /-m' 'socis_cli\.windows_ssh_runtime' 'inspect'/)
  assert.match(script, /SOCIS''s/)
  assert.match(script, /C:\\x y\\socis\.exe/)
})
...
test('Windows SSH reuse requires the requested remote profile to match the lock', () => {
  ...
  const runtime = { socisPath: lock.socisPath, socisHome: lock.socisHome }
