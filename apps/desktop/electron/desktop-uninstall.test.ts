...
test('uninstallArgsForMode maps each mode to the module-runner argv', () => {
  assert.deepEqual(uninstallArgsForMode('gui'), ['-m', 'socis_cli.uninstall', '--mode', 'gui'])
  assert.deepEqual(uninstallArgsForMode('lite'), ['-m', 'socis_cli.uninstall', '--mode', 'lite'])
  assert.deepEqual(uninstallArgsForMode('full'), ['-m', 'socis_cli.uninstall', '--mode', 'full'])
})
...
test('resolveRemovableAppPath finds the .app bundle on macOS', () => {
  assert.equal(
    resolveRemovableAppPath('/Applications/SOCIS.app/Contents/MacOS/SOCIS', 'darwin'),
    '/Applications/SOCIS.app'
  )
  assert.equal(
    resolveRemovableAppPath('/Users/x/Applications/SOCIS.app/Contents/MacOS/SOCIS', 'darwin'),
    '/Users/x/Applications/SOCIS.app'
  )
})
...
test('resolveRemovableAppPath finds the install dir on Windows', () => {
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\Programs\\SOCIS\\SOCIS.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\Programs\\SOCIS'
  )
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\socis-desktop\\SOCIS.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\socis-desktop'
  )
})

test('resolveRemovableAppPath returns null for an unrecognized Windows dir', () => {
  assert.equal(resolveRemovableAppPath('C:\\Temp\\foo\\SOCIS.exe', 'win32'), null)
})

test('resolveRemovableAppPath uses APPIMAGE on Linux when set', () => {
  assert.equal(
    resolveRemovableAppPath('/tmp/.mount_SOCISXXXX/socis', 'linux', { APPIMAGE: '/home/x/Apps/SOCIS.AppImage' }),
    '/home/x/Apps/SOCIS.AppImage'
  )
})

test('resolveRemovableAppPath finds the unpacked dir on Linux', () => {
  assert.equal(resolveRemovableAppPath('/opt/socis/linux-unpacked/socis', 'linux', {}), '/opt/socis/linux-unpacked')
  // A system-package install (/usr/bin) → null, left to apt/dnf.
  assert.equal(resolveRemovableAppPath('/usr/bin/socis', 'linux', {}), null)
})
...
test('shouldRemoveAppBundle requires packaged AND a resolved path', () => {
  assert.equal(shouldRemoveAppBundle(true, '/Applications/SOCIS.app'), true)
  assert.equal(shouldRemoveAppBundle(false, '/Applications/SOCIS.app'), false)
...
test('buildPosixCleanupScript waits for the PID, runs the uninstall module, removes bundle', () => {
  const script = buildPosixCleanupScript({
    desktopPid: 4321,
    pythonExe: '/home/x/.socis-agent/socis-agent/venv/bin/python',
    pythonPath: null,
...
test('buildPosixCleanupScript exports PYTHONPATH when pythonPath is set (lite/full)', () => {
  const script = buildPosixCleanupScript({
    ...
    pythonPath: '/home/x/.socis-agent/socis-agent',
    agentRoot: '/home/x/.socis-agent/socis-agent',
...
test('buildPosixCleanupScript omits the bundle rm when appPath is null', () => {
  const script = buildPosixCleanupScript({
    ...
    pythonPath: null,
    agentRoot: '/a',
    uninstallArgs: ['-m', 'socis_cli.uninstall', '--mode', 'lite'],
    appPath: null,
    socisHome: '/h'
  })
  ...
  assert.match(script, /'-m' 'socis_cli\.uninstall' '--mode' 'lite'/)
})
...
test('buildWindowsCleanupScript waits (bounded) for PID, runs uninstall, rmdir bundle', () => {
  const script = buildWindowsCleanupScript({
    ...
    agentRoot: 'C:\\socis',
    uninstallArgs: ['-m', 'socis_cli.uninstall', '--mode', 'full'],
    appPath: 'C:\\Users\\x\\AppData\\Local\\Programs\\SOCIS',
    socisHome: 'C:\\Users\\x\\AppData\\Local\\socis'
  })

  assert.match(script, /@echo off/)
  assert.match(script, /set "PID=9988"/)
  // PYTHONPATH set so a system python can import socis_cli from source.
  assert.match(script, /set "PYTHONPATH=C:\\socis;%PYTHONPATH%"/)
  assert.match(script, /"C:\\Python313\\python.exe" "-m" "socis_cli\.uninstall" "--mode" "full"/)
  ...
  assert.match(script, /rmdir \/s \/q "C:\\Users\\x\\AppData\\Local\\Programs\\SOCIS" >nul 2>&1/)
