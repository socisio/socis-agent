// Launching the SOCIS TUI in the user's OWN terminal emulator.
//
...
// `socis --tui --resume <id>` there. Two problems have to be solved for that
// to work anywhere:
//
//  1. WHAT to run. The desktop's SOCIS runtime is often a venv Python invoked
//     as `python -m socis_cli.main`, not a `socis` on PATH — so the command
 ...
 * headless child) is the wrong answer for an interactive terminal. The SOCIS
...
export function terminalScriptEnv(
  ...
  if (socisHome) {
    out.SOCIS_AGENT_HOME = socisHome
  }
