...
export function registerHudIpc({
  ...
  ipcMain.on('socis:hud:native-drag', event => {
    event.returnValue = hudWindowing().move === 'native-drag'
  })

  ipcMain.on('socis:hud:windowing', event => {
  ...
  ipcMain.on('socis:hud:workspace-transfer', (event, transferring) => {
  ...
  ipcMain.handle('socis:hud:open', async (_event, request) => {
    openHudWindow(
  ...
  ipcMain.handle('socis:hud:frost', (_event, showing) => {
  ...
  // reaches the bar.
  ipcMain.on('socis:hud:ignore-mouse', (_event, ignore) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('socis:hud:begin-move', event => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('socis:hud:end-move', event => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('socis:hud:move-by', (event, delta) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('socis:hud:set-bounds', (event, bounds) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.handle('socis:hud:reset-layout', event => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('socis:hud:session', (event, sessionId) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.handle('socis:hud:close', async () => {
    closeHudWindow()
