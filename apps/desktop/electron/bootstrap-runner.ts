 ...
 * bootstrap-runner.ts
 *
 * Drives apps/desktop's first-launch install of SOCIS Agent by spawning
...
function readExistingPinnedCommit(activeRoot: string | null | undefined): string | null {
  ...
  try {
    const raw = fs.readFileSync(path.join(activeRoot, '.socis-bootstrap-complete'), 'utf8')
...
function bootstrapCacheDir(socisHome) {
  return path.join(socisHome, 'bootstrap-cache')
}

// The install.sh / install.ps1 that ships inside the already-installed agent
// checkout under ~/.socis-agent/socis-agent. Used as a last-resort fallback when
// the pinned commit can't be fetched from GitHub (e.g. a locally-built desktop
// app stamped to an unpushed HEAD).
function installedAgentInstallScript(socisHome) {
  if (!socisHome) {
    return null
  }

  const candidate = path.join(socisHome, 'socis-agent', 'scripts', installScriptName())
...
function cachedScriptPath(socisHome, commit) {
  return path.join(bootstrapCacheDir(socisHome), `install-${commit}.${process.platform === 'win32' ? 'ps1' : 'sh'}`)
}
...
function downloadInstallScript(ref, destPath) {
  ...
  const url = `https://raw.githubusercontent.com/socisio/socis-agent/${ref}/scripts/${scriptName}`
...
async function resolveInstallScript({
  ...
  const cached = cachedScriptPath(socisHome, installRef.cacheKey)
  ...
  try {
    ...
    const installed = installedAgentInstallScript(socisHome)
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, socisHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
    ...
    const child = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Pass SOCIS_AGENT_HOME through so install.ps1 respects the caller's
          // choice rather than re-computing the default.
          SOCIS_AGENT_HOME: socisHome || process.env.SOCIS_AGENT_HOME || ''
        }
      })
    )
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, socisHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
...
function buildPosixPinArgs({ installStamp, activeRoot, socisHome, pinCommit = true }) {
  const args = ['--dir', activeRoot, '--socis-home', socisHome]
...
async function fetchManifest({ scriptPath, installerKind, emit, socisHome, activeRoot, installStamp, pinCommit }) {
  const isPosix = installerKind === 'posix'

  const args = isPosix
    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, socisHome, pinCommit })]
...
async function runStage({
  ...
  const args = isPosix
    ? [
        ...
        '--non-interactive',
        '--json',
        ...buildPosixPinArgs({ installStamp, activeRoot, socisHome, pinCommit })
      ]
...
async function runBootstrap(opts) {
  ...
  const runLog = openRunLog(logRoot || path.join(socisHome, 'logs'))
  ...
  try {
    ...
    const scriptInfo = await resolveInstallScript({ installStamp, sourceRepoRoot, socisHome, emit })
