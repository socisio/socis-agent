 ...
 *   - the browser sees only a minimal "return to SOCIS" page.
 */
...
const DONE_HTML =
  ...
  '<p>You can close this window and return to SOCIS.</p>' +
...
export function registerMcpOauthCallbackIpc() {
  ...
  ipcMain.handle('socis:mcp-oauth:listen', async () => {
  ...
  ipcMain.handle('socis:mcp-oauth:wait', async (_event, id, timeoutMs) => {
  ...
  ipcMain.handle('socis:mcp-oauth:cancel', (_event, id) => {
    dispose(String(id || ''))
