...
test('desktop backend PATH adds SOCIS-managed bins and missing POSIX sane entries', () => {
  ...
  assert.deepEqual(entries.slice(0, 3), [
    '/Users/test/.socis-agent/node/bin',
    '/Users/test/.socis-agent/node',
    '/Users/test/.socis-agent/socis-agent/venv/bin'
  ])
...
test('managed Node dirs lead with the platform-native layout but always offer both', () => {
  ...
  const windows = socisManagedNodePathEntries('C:\\Users\\test\\AppData\\Local\\socis', {
    platform: 'win32',
  ...
  assert.deepEqual(posix, ['/Users/test/.socis-agent/node/bin', '/Users/test/.socis-agent/node'])
...
test('managed Node dirs are empty without a SOCIS home', () => {
  assert.deepEqual(socisManagedNodePathEntries(undefined, { platform: 'darwin', pathModule: path.posix }), [])
  assert.deepEqual(socisManagedNodePathEntries('', { platform: 'win32', pathModule: path.win32 }), [])
})
...
test('every managed Node dir outranks the inherited PATH on both platforms', () => {
  for (const [platform, pathModule, home, inherited, delimiter] of [
    ...
    const managed = socisManagedNodePathEntries(home, { platform, pathModule })
...
test('buildDesktopBackendEnv extends PYTHONPATH and backend PATH together', () => {
  ...
  assert.equal(env.PYTHONPATH, '/repo/socis-agent:/existing/pythonpath')
  assert.ok(
    env.PATH.startsWith(
      '/Users/test/.socis-agent/node/bin:/Users/test/.socis-agent/node:/Users/test/.socis-agent/socis-agent/venv/bin:'
    )
  )
...
test('Windows PATH casing and delimiter are preserved without POSIX sane entries', () => {
  const env = buildDesktopBackendEnv({
    socisHome: 'C:\\Users\\test\\AppData\\Local\\socis',
    pythonPathEntries: ['C:\\repo\\socis-agent'],
    venvRoot: 'C:\\Users\\test\\AppData\\Local\\socis\\socis-agent\\venv',
    currentEnv: {
  ...
  assert.ok(
    env.Path.startsWith(
      'C:\\Users\\test\\AppData\\Local\\socis\\node;C:\\Users\\test\\AppData\\Local\\socis\\node\\bin;'
    )
  )
