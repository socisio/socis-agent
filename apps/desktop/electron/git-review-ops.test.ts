...
function makeRepo() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'socis-desktop-git-status-'))

  tempDirs.push(dir)
  execFileSync('git', ['init', '-q'], { cwd: dir })
  execFileSync('git', ['config', 'user.email', 'socis-test@example.com'], { cwd: dir })
  execFileSync('git', ['config', 'user.name', 'SOCIS Test'], { cwd: dir })
