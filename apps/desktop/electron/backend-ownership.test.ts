...
function ownershipEntry(overrides: Partial<BackendIdentity> = {}) {
  return { command: 'socis serve --port 0', ...identity(overrides) }
}
...
test('backend identity check matches only serve and dashboard invocation shapes', () => {
  assert.equal(backendCommandMatches('/venv/bin/socis serve --port 0'), true)
  assert.equal(backendCommandMatches('python -m socis_cli.main dashboard --no-open'), true)
  assert.equal(backendCommandMatches('/venv/bin/socis --profile work serve --port 0'), true)
  assert.equal(backendCommandMatches('"C:\\SOCIS Runtime\\socis.exe" dashboard --no-open'), true)
  assert.equal(backendCommandMatches('socis chat --query serve'), false)
