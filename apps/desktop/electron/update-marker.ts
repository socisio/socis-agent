...
export function markerPath(socisHome) {
  return path.join(socisHome, '.socis-update-in-progress')
}
...
export function readLiveUpdateMarker(
  ...
  const file = markerPath(socisHome)
  let raw
 ...
 *
 * The Tauri-based socis-setup.exe takes several seconds to initialise its
...
export function writeUpdateMarker(
  ...
  const file = markerPath(socisHome)
  const nowMs = now()
  const owner = readLiveUpdateMarker(socisHome, { kill, maxAgeMs, now: () => nowMs })
 ...
 * SOCIS to exit…") clobbers that still-running updater's claim: the
...
export function updateHandoffConflict(
  ...
  const owner = readLiveUpdateMarker(socisHome, opts)
