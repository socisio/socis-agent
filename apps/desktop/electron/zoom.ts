...
export const ZOOM_STORAGE_KEY = 'socis:desktop:zoomLevel'
 ...
 * but forgot to emit 'socis:zoom:changed', leaving the control stuck at 100%.
...
export function applyZoomLevel(webContents, level) {
  ...
  webContents.send('socis:zoom:changed', { level: clamped, percent: zoomLevelToPercent(clamped) })
 ...
 * only learns of changes through `socis:zoom:changed`, which never fired —
