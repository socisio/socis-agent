...
const CRED_GATEWAY = 'https://alice:supersecret@gw.example.com/socis'
...
test('a decryption failure logs the gateway host and path but not its credentials', () => {
  ...
  assert.match(locked.logs[0], /failed to decrypt stored tokens for https:\/\/gw\.example\.com\/socis/)
  assert.match(locked.logs[0], /keeping stored entry for retry/)
...
test('a parsing failure logs the gateway host and path but not its credentials', () => {
  ...
  assert.match(disk.logs[0], /failed to load stored tokens for https:\/\/gw\.example\.com\/socis/)
