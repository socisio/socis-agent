...
export function registerPetOverlayIpc({
  ...
  ipcMain.handle('socis:pet-overlay:open', async (_event, request) => {
  ...
  ipcMain.handle('socis:pet-overlay:close', async () => {
    closePetOverlay()
  ...
  ipcMain.on('socis:pet-overlay:set-bounds', (_event, bounds) => {
  ...
  ipcMain.on('socis:pet-overlay:ignore-mouse', (_event, ignore) => {
    const petOverlayWindow = getPetOverlayWindow()
  ...
  ipcMain.on('socis:pet-overlay:set-focusable', (_event, focusable) => {
    const petOverlayWindow = getPetOverlayWindow()
  ...
  ipcMain.on('socis:pet-overlay:state', (_event, payload) => {
    const petOverlayWindow = getPetOverlayWindow()

    if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
      petOverlayWindow.webContents.send('socis:pet-overlay:state', payload)
    }
  })
  // Overlay → main renderer: control messages (pop back in, composer submit).
  ipcMain.on('socis:pet-overlay:control', (_event, payload) => {
    const mainWindow = getMainWindow()
    ...
    mainWindow.webContents.send('socis:pet-overlay:control', payload)
  })
}
