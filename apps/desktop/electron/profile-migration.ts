...
export interface MigrationDeps {
  ...
  /** Default profile home (`~/.socis-agent`). Default's state.db and gateway.pid live here. */
  socisHome: string
  /** Named-profile root (`~/.socis-agent/profiles`). Does not contain `default`. */
  profilesRoot: string
  ...
  isSOCISProcess: (pid: number) => boolean
  now: () => number
...
/**
 * Production layout: default IS `socisHome`; named profiles are children of
 * `profilesRoot`. There is no `profiles/default` directory on a normal install.
 */
export function profileStateDbPath(name: string, socisHome: string, profilesRoot: string): string {
  return name === 'default' ? `${socisHome}/state.db` : `${profilesRoot}/${name}/state.db`
}

export function profileGatewayPidPath(name: string, socisHome: string, profilesRoot: string): string {
  return name === 'default' ? `${socisHome}/gateway.pid` : `${profilesRoot}/${name}/gateway.pid`
}

function resolveSOCISHome(profilesRoot: string, socisHome?: string): string {
  if (socisHome) {
    return socisHome
  }

  // Tests that predate socisHome pass only profilesRoot.
...
/**
 * Return the profile names whose gateway.pid file points to a live socis process.
 ...
 *
 * `socisHome` is optional so existing call sites that only pass `profilesRoot`
 * still work: it is derived as the parent of `…/profiles`.
 */
export function findRunningGatewayProfiles(
  profilesRoot: string,
  allProfiles: string[],
  deps: Pick<MigrationDeps, 'existsSync' | 'readFileSync' | 'isSOCISProcess'> & { socisHome?: string }
): string[] {
  const socisHome = resolveSOCISHome(profilesRoot, deps.socisHome)
  const running: string[] = []

  for (const name of allProfiles) {
    const pidFile = profileGatewayPidPath(name, socisHome, profilesRoot)
...
export function decideMigration(
  ...
  for (const name of candidates) {
    const s = score(profileStateDbPath(name, deps.socisHome, deps.profilesRoot))
...
/** Default is always a candidate; it is `$SOCIS_AGENT_HOME`, not `$SOCIS_AGENT_HOME/profiles/default`. */
...
export function migrateActiveProfileIfMissing(desktopProfileConfigPath: string, deps: MigrationDeps): boolean {
  ...
  // launches `socis --profile default` and is worse than writing nothing
