...
function createTempGitRepo() {
  const cwd = fs.mkdtempSync(path.join(os.tmpdir(), 'socis-update-count-'))
  ...
  try {
    ...
    git('config', 'user.name', 'SOCIS Test')
    git('config', 'user.email', 'socis@example.invalid')
...
test('compareApiUrl handles SSH origin forms', () => {
  for (const originUrl of [
    ...
    'ssh://git@github.com/socisio/socis-agent.git',
