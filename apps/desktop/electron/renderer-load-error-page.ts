 ...
 *
 * 1. TORN BUNDLE after an update (#95575): `socis update` replaces the app
...
export interface RendererLoadErrorDetails {
  ...
  /** Repair command hint, e.g. `socis desktop --force-build`. */
  repairHint?: string
  /**
...
export function buildRendererLoadErrorPage(details: RendererLoadErrorDetails = {}): string {
  ...
  const title = 'SOCIS couldn\u2019t start the desktop UI'
  ...
  const repair = details.repairHint ? `<p>Repair with: <code>socis desktop --force-build</code></p>` : ''
...
<main>
  ...
  <code>socis desktop --force-build</code>, then restart the app.</p>
