...
// registry, and the socis:terminal:* IPC surface. Extracted from main.ts; the
...
export function registerTerminalIpc({
  ...
  function terminalShellCommand() {
    // SOCIS_AGENT_DESKTOP_SHELL is the cross-platform escape hatch (a path or a bare
    ...
    const override = (process.env.SOCIS_AGENT_DESKTOP_SHELL || (isWindows ? '' : process.env.SHELL) || '').trim()
  ...
  function terminalShellEnv() {
    ...
    // /TERM=dumb; some terminals set COLORFGBG which would flip SOCIS' TUI into
    ...
    env.TERM_PROGRAM = 'SOCIS'
    env.TERM_PROGRAM_VERSION = app.getVersion()

    // Let a socis/--tui launched in this pane know it's embedded in the desktop
    // GUI (build_environment_hints surfaces this). Distinct from SOCIS_AGENT_DESKTOP,
    // which marks the agent *backend* and gates cron/gateway behavior.
    env.SOCIS_AGENT_DESKTOP_TERMINAL = '1'

    return env
  }

  function terminalChannel(id, suffix) {
    return `socis:terminal:${id}:${suffix}`
  }
  ...
  ipcMain.handle('socis:terminal:start', async (event, payload = {}) => {
  ...
  ipcMain.handle('socis:terminal:attach', (event, id) => {
  ...
  ipcMain.handle('socis:terminal:write', (_event, id, data) => {
    const sessionInfo = terminalSessions.get(String(id || ''))
  ...
  ipcMain.handle('socis:terminal:resize', (_event, id, size = {}) => {
    const sessionInfo = terminalSessions.get(String(id || ''))
  ...
  ipcMain.handle('socis:terminal:cwd', async (_event, id) => {
    const sessionInfo = terminalSessions.get(String(id || ''))
  ...
  ipcMain.handle('socis:terminal:dispose', (_event, id) => disposeTerminalSession(String(id || '')))
