// Profile-delete routing logic for the `socis:api` IPC handler.
//
...
// be rejected: spawning a fresh backend would call ensure_socis_agent_home() and
...
/** Parse a `socis:api` request into the profile name a DELETE targets. */
 ...
 * still removing the profile and recreate its SOCIS_AGENT_HOME.
 *
...
/**
 * Route the next `socis:api` request away from the primary/window backend
 ...
 * ensure_socis_agent_home() recreates the directory the delete just removed.
 */
