...
test('POSIX managed launcher is detached, correlation-scoped, and never publishes handoff exit 75', () => {
  ...
  assert.match(command, new RegExp(`SOCIS_AGENT_UPDATE_CORRELATION_ID=.*${CORRELATION}`))
...
test('Windows managed launcher starts a hidden child and leaves exit 75 to the external coordinator', () => {
  ...
  assert.match(wrapper, /SOCIS_AGENT_UPDATE_WINDOWS_DETACHED/)
  assert.match(wrapper, /SOCIS_AGENT_UPDATE_TAURI_READY_PATH/)
  assert.match(wrapper, /SOCIS_AGENT_UPDATE_TAURI_OUTCOME_PATH/)
...
test('POSIX observer reads the exact correlation receipt and terminal marker from disk', async () => {
  const home = await mkdtemp(path.join(os.tmpdir(), 'socis-managed-update-'))
...
test('managed observer unwraps a named profile home for the install-wide marker', async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), 'socis-managed-profile-marker-'))
  const profileHome = path.join(root, 'profiles', 'research')

  try {
    await mkdir(profileHome, { recursive: true })
    await writeFile(path.join(root, '.socis-update-in-progress'), `${process.pid}\n1\n`)
