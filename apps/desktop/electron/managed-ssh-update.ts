 ...
 * wire protocol:
 *
 *   gate dials -> drain every captured scope -> detached `socis update`
...
function buildPosixManagedUpdateLaunch(target: RemoteUpdateTarget, correlationId: string): string {
  ...
  const home = validateRemoteValue(target.socisHome, 'SOCIS home')
  const socisPath = validateRemoteValue(target.socisPath, 'launcher path')
  ...
  const launcherWord = expandRemotePath(socisPath)

  const updateCommand =
    `env SOCIS_AGENT_HOME=${homeWord} ` +
    `SOCIS_AGENT_UPDATE_CORRELATION_ID=${shq(correlation)} ` +
    'SOCIS_AGENT_UPDATE_ORIGIN_PROFILE=default ' +
    `SOCIS_AGENT_UPDATE_ORIGIN_HOME=${homeWord} ` +
    `SOCIS_AGENT_UPDATE_OUTPUT_PATH=${outputWord} ` +
...
function buildWindowsManagedUpdateLaunch(target: RemoteUpdateTarget, correlationId: string): string {
  ...
  const wrapper = [
    ...
    `$env:SOCIS_AGENT_HOME=${psLiteral(home)}`,
    `$env:SOCIS_AGENT_UPDATE_CORRELATION_ID=${psLiteral(correlation)}`,
    '$env:SOCIS_AGENT_UPDATE_ORIGIN_PROFILE="default"',
    `$env:SOCIS_AGENT_UPDATE_ORIGIN_HOME=${psLiteral(home)}`,
    `$env:SOCIS_AGENT_UPDATE_OUTPUT_PATH=${psLiteral(outputPath)}`,
    ...
    `$env:SOCIS_AGENT_UPDATE_WINDOWS_DETACHED=${psLiteral(correlation)}`,
    `$env:SOCIS_AGENT_UPDATE_TAURI_OUTCOME_PATH=${psLiteral(statusPath)}`,
    `$env:SOCIS_AGENT_UPDATE_TAURI_READY_PATH=${psLiteral(readyPath)}`,
    ...
    `& ${psLiteral(socisPath)} update --yes *>> ${psLiteral(outputPath)}`,
    '$rc=$LASTEXITCODE',
...
marker_path=install_root/'.socis-update-in-progress'
...
async function runManagedSshUpdate<TScope extends ManagedSshScope>(
  ...
  return {
    ...
    message:
      outcome === 'updated'
        ? 'Remote SOCIS updated and every managed SSH profile is ready.'
        : restoreOk
