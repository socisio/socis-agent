 ...
 *
 * So we delegate to `socis gateway stop --all`: the CLI discovers every
 ...
 * force-kills survivors — the same logic `socis update`'s
...
export function stopGatewayBeforeUpdate(
  ...
  return runGatewayLifecycleCommand(socisCliPath, ['gateway', 'stop', '--all'], deps)
}
 ...
 */
export function startGatewaysAfterUpdateAbort(socisCliPath: string, deps: StopGatewayBeforeUpdateDeps = {}): boolean {
  return runGatewayLifecycleCommand(socisCliPath, ['gateway', 'start', '--all'], deps)
}

function runGatewayLifecycleCommand(socisCliPath: string, args: string[], deps: StopGatewayBeforeUpdateDeps): boolean {
  ...
  if (deps.spy) {
    deps.spy(socisCliPath, args)
  }

  if (!existsSync(socisCliPath)) {
    return false
  }

  try {
    exec(socisCliPath, args, {
