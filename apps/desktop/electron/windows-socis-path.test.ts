// Unit tests for the pure Windows `socis` resolution helpers extracted from
// main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
// unwrapWindowsVenvSOCISCommand(). These pin the two Windows resolution bugs
// that caused desktop reinstall loops:
//   1. buildPathExtCandidates() — PATHEXT extensions must be tried BEFORE the
//      empty extension, or an extensionless Git-Bash `socis` shim shadows
//      the real socis.cmd/socis.exe.
...
//   3. resolveVenvSOCISCommand() — must probe the venv python via
//      canImportSOCISCli() before trusting it, or a broken venv gets
...
test('chooseUpdaterArgs: gentle --update when both updater runtime files exist', () => {
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvSOCIS: true, hasVenvPython: true }, 'main'), [
    '--update',
    '--branch',
    'main'
  ])
...
test('chooseUpdaterArgs: marker-only install uses --repair when the venv is gone', () => {
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvSOCIS: false, hasVenvPython: false }, 'main'),
    ['--repair', '--branch', 'main']
  )
})

test('chooseUpdaterArgs: partial updater runtimes use --repair', () => {
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvSOCIS: false, hasVenvPython: true }, 'main'), [
    '--repair',
    '--branch',
    'main'
  ])
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvSOCIS: true, hasVenvPython: false }, 'main'), [
    '--repair',
    '--branch',
    'main'
  ])
...
test('chooseUpdaterArgs: passes the branch through unchanged in both modes', () => {
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: false, hasVenvSOCIS: true, hasVenvPython: true }, 'release/1.2'),
    ['--update', '--branch', 'release/1.2']
  )
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: false, hasVenvSOCIS: false, hasVenvPython: false }, 'release/1.2'),
    ['--repair', '--branch', 'release/1.2']
  )
})

function makeDeps(overrides: Partial<Parameters<typeof resolveVenvSOCISCommand>[2]> = {}) {
  return {
    isWindows: true,
    isCommandScript: () => false,
    fileExists: () => true,
    directoryExists: () => false,
    canImportSOCISCli: () => true,
    getVenvPython: (venvRoot: string) => `${venvRoot}/Scripts/python.exe`,
    getVenvSitePackagesEntries: () => [],
    buildDesktopBackendEnv: () => ({ FAKE_ENV: '1' }),
    socisHome: '/fake/socis-home',
...
test('resolveVenvSOCISCommand: returns null off Windows', () => {
  const deps = makeDeps({ isWindows: false })

  assert.equal(resolveVenvSOCISCommand('/root/venv/Scripts/socis.exe', [], deps), null)
})

test('resolveVenvSOCISCommand: returns null for a .cmd/.bat script command', () => {
  const deps = makeDeps({ isCommandScript: () => true })

  assert.equal(resolveVenvSOCISCommand('/root/venv/Scripts/socis.cmd', [], deps), null)
})

test('resolveVenvSOCISCommand: returns null when the basename is not socis/socis.exe', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvSOCISCommand('/root/venv/Scripts/python.exe', [], deps), null)
})

test('resolveVenvSOCISCommand: returns null when the parent dir is not Scripts', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvSOCISCommand('/root/venv/bin/socis.exe', [], deps), null)
})

test('resolveVenvSOCISCommand: returns null when the venv python does not exist on disk', () => {
  const deps = makeDeps({ fileExists: () => false })

  assert.equal(resolveVenvSOCISCommand('/root/venv/Scripts/socis.exe', [], deps), null)
})

test('resolveVenvSOCISCommand: probes the venv python before trusting it (returns null on failed probe)', () => {
  let probed = false

  const deps = makeDeps({
    canImportSOCISCli: (python: string) => {
      probed = true
  ...
  const result = resolveVenvSOCISCommand('/root/venv/Scripts/socis.exe', ['serve'], deps)
...
test('resolveVenvSOCISCommand: returns the resolved python backend descriptor when the probe passes', () => {
  const deps = makeDeps()
  const result = resolveVenvSOCISCommand('/root/venv/Scripts/socis.exe', ['serve', '--port', '0'], deps)
  ...
  assert.deepEqual(result.args, ['-m', 'socis_cli.main', 'serve', '--port', '0'])
...
test('resolveVenvSOCISCommand: is case-insensitive on socis.exe and the Scripts dir name', () => {
  const deps = makeDeps()

  assert.ok(resolveVenvSOCISCommand('/root/venv/Scripts/SOCIS.EXE', [], deps))
  assert.ok(resolveVenvSOCISCommand('/root/venv/SCRIPTS/socis.exe', [], deps))
})
