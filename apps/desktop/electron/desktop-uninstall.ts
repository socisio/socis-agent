 ...
 * the three user-facing uninstall modes to the `socis uninstall` CLI flags,
...
/**
 * Map an uninstall mode to the `python -m socis_cli.uninstall` argv (after the
 * python executable). Uses the dedicated lightweight module entrypoint (not
 * `socis_cli.main`) so it can run under a system Python OUTSIDE the venv that
...
function uninstallArgsForMode(mode) {
  ...
  return ['-m', 'socis_cli.uninstall', '--mode', mode]
}
 ...
 *
 *   macOS:   …/SOCIS.app/Contents/MacOS/SOCIS  → …/SOCIS.app
 *   Windows: …\SOCIS\SOCIS.exe                 → …\SOCIS  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env: any = {}) {
  ...
  if (platform === 'darwin') {
    // …/SOCIS.app/Contents/MacOS/SOCIS → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/SOCIS.app
  ...
  if (platform === 'win32') {
    // NSIS per-user installs SOCIS.exe directly in the install dir.
    const dir = p.dirname(exe)

    if (/[\\/]SOCIS$/i.test(dir) || /[\\/]socis-desktop$/i.test(dir)) {
      return dir
    }
  ...
  // Unpacked electron-builder tree: …/linux-unpacked/socis
  const dir = p.dirname(exe)
 ...
 * being deleted); `pythonPath` is prepended to PYTHONPATH so `import socis_cli`
 ...
 */
function buildPosixCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, socisHome }) {
  ...
  const lines = [
    ...
    '  done',
    'fi',
    `export SOCIS_AGENT_HOME=${q(socisHome)}`
  ]
 ...
 * modes + `pythonPath`=agentRoot so `import socis_cli` resolves from source
...
function buildWindowsCleanupScript({
  ...
  // a problem, but SOCIS install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`

  const lines = [
    '@echo off',
    'setlocal enableextensions',
    `set "SOCIS_AGENT_HOME=${String(socisHome).replace(/"/g, '')}"`,
    `set "PID=${pid}"`
  ]
