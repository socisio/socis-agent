 ...
 * console. It writes SOCIS_AGENT_HOME/.socis-update-result.json on every exit
 ...
 * ever surfaces, and the user may not reopen SOCIS within 30 minutes.
...
export function handoffResultPath(socisHome: string): string {
  return path.join(socisHome, '.socis-update-result.json')
}
...
export function readAndConsumeHandoffResult(
  ...
  const file = handoffResultPath(socisHome)
  let raw: string
