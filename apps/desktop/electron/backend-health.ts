...
export interface SOCISReadyOptions {
   ...
   * two very different meanings of a 401 (see `waitForSOCISReady`).
   */
...
export const REMOTE_UNSIGNED_OAUTH_MESSAGE =
  'Remote SOCIS gateway uses OAuth, but you are not signed in. ' +
 ...
 *  - readiness-probe exhaustion in waitForSOCISReady().
 *
...
export function makeNousCloudBackendDownError(baseUrl: string, error: unknown): Error | null {
  ...
  const err = new Error(
    `Nous Cloud agent ${hostname} is down ` +
      ...
      'You can also reach out on Discord at discord.gg/SOCIS ' +
 ...
 * `isReauthRequired` so startSOCIS latches; `needsOauthLogin` alone only
...
export async function waitForSOCISReady(baseUrl: string, options: SOCISReadyOptions): Promise<void> {
  ...
  throw new Error(`SOCIS backend did not become ready: ${detail}`)
}
