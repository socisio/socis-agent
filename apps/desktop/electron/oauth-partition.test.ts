...
// evict each other's `socis_session*` cookies when they ride the single
// shared `persist:socis-remote-oauth` partition — and, worse, gateway A's
...
describe('resolveOauthPartition (#92183 per-connection cookie jars)', () => {
  ...
  it('keeps cloud connections on the legacy partition (silent portal cascade needs the shared jar)', () => {
    const reg = registry('local', [
      { id: 'cloud-1', kind: 'cloud', url: 'https://agent.socis.io', authMode: 'oauth' }
    ])

    expect(resolveOauthPartition('https://agent.socis.io/api/status', { registry: reg })).toBe(
      LEGACY_OAUTH_PARTITION
    )
  })
