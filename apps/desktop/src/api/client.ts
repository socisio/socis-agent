import { JsonRpcGatewayClient } from '@socis/shared'

import type { SOCISApiRequest } from '@/global'
...
// "Timed out connecting to SOCIS backend" that hangs the UI (#48504).
//
...
export class SOCISGateway extends JsonRpcGatewayClient {
  constructor() {
    super({
      closedErrorMessage: 'SOCIS gateway connection closed',
      connectErrorMessage: 'Could not connect to SOCIS gateway',
      createRequestId: nextId => nextId,
      notConnectedErrorMessage: 'SOCIS gateway is not connected',
...
// process to route them to that host (socis:api's registry branch). Null
...
export function socisApi<T>(request: SOCISApiRequest): Promise<T> {
  return window.socisDesktop.api<T>({ ...connectionScoped(), ...request })
}
