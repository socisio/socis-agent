...
const SKIN_KEY = 'socis-desktop-theme-v2'
const MODE_KEY = 'socis-desktop-mode-v1'
...
const PROFILE_SKINS_KEY = 'socis-desktop-profile-themes-v1'
const PROFILE_MODES_KEY = 'socis-desktop-profile-modes-v1'
// Last active profile, recorded so the boot-time paint can pick that profile's
// theme before the gateway reports which profile actually launched.
const LAST_PROFILE_KEY = 'socis-desktop-active-profile-v1'
...
function applyTheme(theme: DesktopTheme, mode: 'light' | 'dark') {
  ...
  root.dataset.socisTheme = skinName
  root.dataset.socisMode = rendered
  ...
  window.socisDesktop?.setTitleBarTheme?.({
    background: chromeBg,
  ...
  try {
    window.localStorage.setItem('socis-boot-background', chromeBg)
    window.localStorage.setItem('socis-boot-color-scheme', rendered)
  } catch {
  ...
  if (typo.fontUrl && !INJECTED_FONT_URLS.has(typo.fontUrl)) {
    ...
    link.dataset.socisThemeFont = 'true'
...
const syncNativeTheme = (pref: ThemeMode, rendered: 'light' | 'dark') =>
  window.socisDesktop?.setNativeTheme?.(pref === 'system' ? 'system' : rendered)
...
export function ThemeProvider({ children }: { children: ReactNode }) {
  ...
  // Drain a backend-driven skin switch (SOCIS authoring/activating a skin from a
