/**
 * Live skin sync from the SOCIS backend.
 *
 * The backend resolves the active skin (built-in or `$SOCIS_AGENT_HOME/skins/*.yaml`)
 ...
 * stomps the user's persisted desktop theme; only a genuine name change (SOCIS
 * authoring/activating a skin from a prompt, or `/skin` elsewhere) repaints.
 */

import type { SOCISSkin } from '@socis/shared/skin'
 ...
 */
export function ingestBackendSkin(skin: SOCISSkin | undefined | null, { apply }: { apply: boolean }): void {
  ...
  if (name !== 'default' && !BUILTIN_THEMES[name]) {
    const theme = skinToDesktopTheme(skin as SOCISSkin)
