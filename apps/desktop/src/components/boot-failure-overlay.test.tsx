...
function failBoot() {
  $desktopBoot.set({
    error: 'Could not connect to SOCIS gateway',
    fakeMode: false,
    message: 'boot failed',
...
function stubDesktop(config: Record<string, unknown>, overrides: Record<string, unknown> = {}) {
  const original = window.socisDesktop
  Object.defineProperty(window, 'socisDesktop', {
    configurable: true,
    value: { getRecentLogs: async () => ({ lines: [] }), getConnectionConfig: async () => config, ...overrides }
  })

  return () => Object.defineProperty(window, 'socisDesktop', { configurable: true, value: original })
}
