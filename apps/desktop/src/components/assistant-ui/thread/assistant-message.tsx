...
const ErrorRecoveryActions: FC = () => {
  ...
  // Open Logs reveals the LOCAL Electron profile's SOCIS_AGENT_HOME/logs. On a
  ...
  const openLogs = useCallback(async () => {
    try {
      const root = await window.socisDesktop?.logsRoot?.()
      ...
      const result = await window.socisDesktop?.openDir?.(root)
