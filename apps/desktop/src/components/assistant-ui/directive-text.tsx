...
const SOCIS_AGENT_REF_TYPES = WIRE_REFERENCE_KINDS
type SOCISRefType = (typeof SOCIS_AGENT_REF_TYPES)[number]
...
const SOCIS_AGENT_DIRECTIVE_RE = referenceRe()
...
export const socisDirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(SOCIS_AGENT_DIRECTIVE_RE)).map(match => {
...
function safeDirectiveSegments(text: string): Unstable_DirectiveSegment[] {
  try {
    return [...socisDirectiveFormatter.parse(text)]
  } catch {
...
/**
 * Renders text containing SOCIS directives (`@file:...`, `@image:...`) as
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.socisDesktop && isRemoteGateway() ? gatewayMediaDataUrl(id) : window.socisDesktop?.readFileDataUrl(id)
