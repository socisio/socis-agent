 ...
 * own design.
 *
 * WIDGETS TALK BACK OFF-SCREEN. `window.socis.send(prompt)` (or declarative
 * `data-socis-send` on any clickable element) routes the prompt through the
...
const SIZE_MESSAGE_TYPE = 'socis-inline-preview-size'
const INTENT_MESSAGE_TYPE = 'socis-inline-preview-intent'
...
/** The script that gives the widget its ONE voice: `socis.send(prompt)`.
 ...
 *  wires `data-socis-send` so declarative HTML works with zero script:
 *  `<button data-socis-send="get-price eth">ETH</button>`. */
export function intentScript(token: string): string {
  return (
    ...
    ')},"*");return true}' +
    'window.socis={send:send};' +
    'addEventListener("click",function(e){var el=e.target&&e.target.closest?' +
    'e.target.closest("[data-socis-send]"):null;' +
    'if(el)send(el.getAttribute("data-socis-send")||"")},true)})()</script>'
  )
}
...
export function InlinePreviewDirective({
  ...
  // URL connection — including a same-machine `socis serve` — renders live.)
