import type { GatewayWsUrlResult } from '@socis/shared'
import type { TranslucencyState } from '@socis/shared/translucency'
...
declare global {
  interface Window {
    socisDesktop: {
      ...
      getConnection: (profile?: string | null) => Promise<SOCISConnection>
      ...
      // Resume this session in the user's own terminal emulator (`socis --tui
      ...
      // SOCIS Cloud: one portal login powers discovery + silent per-agent
      ...
      profile: {
        ...
        // backend under the new SOCIS_AGENT_HOME (reloads the window). Pass null to
        // clear the preference.
        set: (name: string | null) => Promise<DesktopActiveProfile>
      }
      api: <T>(request: SOCISApiRequest) => Promise<T>
      notify: (payload: SOCISNotification) => Promise<boolean>
      ...
      readFileText: (filePath: string) => Promise<SOCISReadFileTextResult>
      ...
      readPluginSource?: (filePath: string) => Promise<SOCISReadFileTextResult>
      selectPaths: (options?: SOCISSelectPathsOptions) => Promise<string[]>
      ...
      normalizePreviewTarget: (target: string, baseDir?: string) => Promise<SOCISPreviewTarget | null>
      watchPreviewFile: (url: string) => Promise<SOCISPreviewWatch>
      ...
      watchDirectory?: (dir: string) => Promise<SOCISPreviewWatch>
      stopPreviewFileWatch: (id: string) => Promise<boolean>
      setActiveWork?: (payload: SOCISActiveWork) => void
      setTitleBarTheme?: (payload: SOCISTitleBarTheme) => void
      ...
      readDir: (path: string) => Promise<SOCISReadDirResult>
      ...
      // Local Desktop runtime-plugin root (<SOCIS_AGENT_HOME>/desktop-plugins),
      ...
      /** LOCAL `<SOCIS_AGENT_HOME>/logs` (profile-aware) — error card "Open Logs". */
      logsRoot?: () => Promise<string>
      // Local AGENT-plugin root (<SOCIS_AGENT_HOME>/plugins), same Electron-local
      ...
      git?: {
        worktreeList: (repoPath: string) => Promise<SOCISGitWorktree[]>
        worktreeAdd: (
          repoPath: string,
        ...
        branchList: (repoPath: string) => Promise<SOCISGitBranch[]>
        ...
        baseBranchList: (repoPath: string) => Promise<SOCISGitBaseBranch[]>
        ...
        repoStatus: (repoPath: string) => Promise<SOCISRepoStatus | null>
        ...
        review: {
          list: (repoPath: string, scope: SOCISReviewScope, baseRef?: null | string) => Promise<SOCISReviewList>
          diff: (
            repoPath: string,
            filePath: string,
            scope: SOCISReviewScope,
            baseRef?: null | string,
            staged?: boolean
          ) => Promise<string>
          ...
          shipInfo: (repoPath: string) => Promise<SOCISReviewShipInfo>
          ...
          prList: (repoPath: string, branches: string[], numbers?: number[]) => Promise<SOCISRepoPullRequests>
          ...
          fetchPrComment: (repoPath: string, url: string) => Promise<SOCISPrComment | null>
      ...
      terminal: {
        ...
        onExit: (id: string, callback: (payload: SOCISTerminalExit) => void) => () => void
        resize: (id: string, size: { cols: number; rows: number }) => Promise<boolean>
        start: (options?: { cols?: number; cwd?: string; rows?: number }) => Promise<SOCISTerminalSession>
      ...
      onWindowStateChanged?: (callback: (payload: SOCISWindowState) => void) => () => void
      ...
      onPreviewFileChanged: (callback: (payload: SOCISPreviewFileChanged) => void) => () => void
...
export interface SOCISTerminalSession {
  cwd: string
  id: string
  shell: string
}

export interface SOCISTerminalExit {
  code: number | null
  signal: string | null
}
...
export interface DesktopUpdateApplyResult {
   ...
   *  `socis update` themselves. `command` is the exact line to run. */
  manual?: boolean
  command?: string
  socisRoot?: string
...
export interface SOCISConnection {
  baseUrl: string
  darwinMajor?: number
...
export interface SOCISTitleBarTheme {
  background: string
  foreground: string
}

/** Turns in flight, so the main process can confirm before a quit kills them. */
export interface SOCISActiveWork {
  count: number
  titles: string[]
}

export interface SOCISWindowState {
  darwinMajor?: number
  isFullscreen: boolean
  isMinimized?: boolean
  isVisible?: boolean
...
export interface DesktopConnectionConfig {
  envOverride: boolean
  // The saved connection mode. 'cloud' is a SOCIS Cloud connection: it carries
  ...
  remoteUrl: string
  // For a 'cloud' connection: the persisted SOCIS Cloud org (slug or id) the
...
export interface DesktopConnectionConfigInput {
  ...
  remoteUrl?: string
  // For a 'cloud' connection: the selected SOCIS Cloud org (slug or id) to
...
export interface DesktopCloudStatus {
  ...
  // the socis gateway session cookies. See cookiesHavePrivySession.
  signedIn: boolean
}

// A discovered SOCIS Cloud agent — the trimmed DTO from NAS GET /api/agents.
...
export interface SOCISApiRequest {
  path: string
  method?: string
  body?: unknown
...
export interface SOCISNotification {
  title?: string
  body?: string
  silent?: boolean
  kind?: string
...
export interface SOCISPreviewTarget {
  binary?: boolean
  byteSize?: number
  kind: 'file' | 'url'
  label: string
...
export interface SOCISReadFileTextResult {
  binary?: boolean
  byteSize?: number
  language?: string
  mimeType?: string
...
export interface SOCISPreviewWatch {
  id: string
  path: string
}
...
export interface SOCISGitWorktree {
  path: string
  branch: null | string
  isMain: boolean
  detached: boolean
...
export interface SOCISGitBranch {
  name: string
  checkedOut: boolean
  isDefault: boolean
  isRemote: boolean
...
// preselect it.
export interface SOCISGitBaseBranch {
  name: string
  isRemote: boolean
  isDefault: boolean
}
...
export interface SOCISRepoStatusFile {
  path: string
  staged: boolean
  unstaged: boolean
  untracked: boolean
...
export interface SOCISRepoStatus {
  branch: null | string
...
// turn began.
export type SOCISReviewScope = 'branch' | 'lastTurn' | 'uncommitted'

// One changed file in the review pane (status letter, +/- lines, staged flag).
export interface SOCISReviewFile {
  path: string
  added: number
  removed: number
...
export interface SOCISReviewList {
  files: SOCISReviewFile[]
...
export interface SOCISReviewPr {
  url: string
  state: string
  number: number
}
...
export interface SOCISBranchPullRequest {
  branch: string
  draft: boolean
  number: number
...
export interface SOCISRepoPullRequests {
  ghReady: boolean
  prs: SOCISBranchPullRequest[]
}
...
export interface SOCISPrComment {
  author: string
  body: string
  diffHunk: string
  kind: 'issue' | 'review'
...
export interface SOCISReviewShipInfo {
  ghReady: boolean
  pr: SOCISReviewPr | null
}

export interface SOCISReadDirEntry {
  name: string
  path: string
  isDirectory: boolean
}

export interface SOCISReadDirResult {
  entries: SOCISReadDirEntry[]
  error?: string
}

export interface SOCISPreviewFileChanged {
  id: string
  path: string
  url: string
}

export interface SOCISSelectPathsOptions {
  title?: string
  defaultPath?: string
  directories?: boolean
  multiple?: boolean
