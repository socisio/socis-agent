...
import type { SessionInfo } from '@/socis'
...
// zero SOCIS sessions, and those arrive continuously — so once the user dragged
...
export function useRepoWorktreeMap(
  repoPaths: string[],
  enabled: boolean
): [Record<string, SOCISGitWorktree[]>, boolean] {
  const [map, setMap] = useState<Record<string, SOCISGitWorktree[]>>({})
