        ...
        <>
          <a
            ...
            onClick={event => {
              ...
              void window.socisDesktop?.openExternal(error.url)
            }}
          >
...
export function PreviewPane({ embedded = false, onRestartServer, reloadRequest = 0, tabId, target }: PreviewPaneProps) {
  ...
  const annotateGuest = useCallback((): null | PreviewAnnotateGuest => {
    ...
    return {
      capture: async rect => {
        ...
        const dataUrl = await window.socisDesktop.capturePreview?.({ rect, viewport, webContentsId })
  ...
  const restartServer = useCallback(async () => {
    ...
    // "Ask SOCIS to restart the server" looked like it did nothing —
  ...
  useEffect(() => {
    if (
      ...
      !window.socisDesktop?.watchPreviewFile ||
      !window.socisDesktop?.onPreviewFileChanged
    ) {
      return
    }
    ...
    const unsubscribe = window.socisDesktop.onPreviewFileChanged(payload => {
    ...
    void window.socisDesktop
      ...
      .then(watch => {
        if (!active) {
          void window.socisDesktop?.stopPreviewFileWatch?.(watch.id)
    ...
    return () => {
      ...
      if (watchId) {
        void window.socisDesktop?.stopPreviewFileWatch?.(watchId)
      }
    }
  ...
  useEffect(() => {
    ...
    webview.setAttribute('partition', 'persist:socis-preview')
    ...
    const onGuestContextMenu = (event: Event) => {
      ...
      const zoom = window.socisDesktop?.zoom?.factor?.() || 1
      ...
      openGuestContextMenu(
        ...
        },
        {
          addToDictionary: (word: string) => {
            ...
            if (typeof webContentsId === 'number') {
              void window.socisDesktop?.contextMenuGuestAddWord?.({ webContentsId, word })
            }
          },
          copyImage: () => void window.socisDesktop?.contextMenuCopyImage?.(),
          ...
          <PreviewBrowserBar
            ...
            onOpenExternal={
              !isBrowserWindow() && !canOpenBrowserWindow()
                ? () => void window.socisDesktop?.openExternal(currentUrl)
                : undefined
            }
