...
function buildTourScript(action: TourAction): string {
  return `(function () {
  var w = window;
  if (!w.__socisTourEngine) {
    ${driverIife}
    w.__socisTourHolder = {};
    w.__socisTourCollect = (${collectTourTargets.toString()});
    w.__socisTourEngine = (${runTourEngine.toString()});
  }
  if (!document.getElementById('__socis-tour-style')) {
    var style = document.createElement('style');
    style.id = '__socis-tour-style';
    style.textContent = ${JSON.stringify(driverCss)};
    (document.head || document.documentElement).appendChild(style);
  }
  return JSON.stringify(w.__socisTourEngine(
    w.driver.js.driver,
    w.__socisTourHolder,
    ${JSON.stringify(action)},
    w.__socisTourCollect,
    document
  ));
})()`
}
