...
vi.mock('@/socis', () => ({
  saveSOCISConfigRecord: (config: Record<string, unknown>, profile?: unknown) => mocks.save(config, profile)
}))

const promptCopy = {
  title: 'Stay signed in to your sites',
  body: 'Let SOCIS browse with a snapshot of your default browser profile.',
...
vi.mock('../../hooks/use-config-record', () => ({
  socisConfigCacheWriter: () => (config: Record<string, unknown>) => mocks.cache(config),
  useSOCISConfigRecord: () => ({ data: mocks.loadedConfig })
}))
