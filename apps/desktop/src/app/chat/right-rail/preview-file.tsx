...
import { SOCIS_AGENT_PATHS_MIME } from '@/app/chat/hooks/use-composer-actions'
...
async function readTextPreview(filePath: string) {
  try {
    ...
    if (!message.includes("No handler registered for 'socis:readFileText'")) {
      throw error
    }
  }
  ...
  const dataUrl = await window.socisDesktop.readFileDataUrl(filePath)
...
function startLineDrag(event: ReactDragEvent<HTMLElement>, filePath: string, { end, start }: LineSelection) {
  ...
  event.dataTransfer.setData(SOCIS_AGENT_PATHS_MIME, JSON.stringify([{ line: start, lineEnd, path: filePath }]))
