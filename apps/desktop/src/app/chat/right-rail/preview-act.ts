...
const preamble = () => `  var w = window;
  ...
  w.__socisActHolder = w.__socisActHolder || {};
  w.__socisAct = ${actEngineSource()};
  w.__socisWatch_fn = (${watchInPage.toString()});
  w.__socisWatchTag = ${WATCH_TAG};
  var holder = w.__socisActHolder;
  var act = function (a) { return w.__socisAct(document, holder, a); };
  var watch = function (stage, label) {
    try { w.__socisWatch_fn(document, holder, stage, label); } catch (err) {}
  };
...
${preamble()}
  ...
  document.addEventListener('pointerdown', function (e) {
    w.__socisHit = { tag: e.target ? e.target.tagName : '?', trusted: e.isTrusted === true };
...
function hitNote(hit?: { tag: string; trusted: boolean } | null): string | undefined {
  return hit && hit.tag === 'SOCIS-WATCH' ? 'The action overlay intercepted the click instead of the page.' : undefined
}
