...
vi.mock('@/socis', async importOriginal => {
  const actual = await importOriginal<typeof SOCISModule>()

  class FakeSOCISGateway {
  ...
  return { ...actual, SOCISGateway: FakeSOCISGateway }
})

import type * as SOCISModule from '@/socis'
import type { SOCISGateway } from '@/socis'
import {
  $gateway,
...
const fakeGateway = { connectionState: 'open' } as unknown as SOCISGateway
...
async function activateRemoteGateway() {
  ...
  setPrimaryGateway(primary as unknown as SOCISGateway, 'default')
  $gateway.set(primary as unknown as SOCISGateway)
...
describe('useGatewayRequest', () => {
  ...
  it('surfaces a missing optional scoped mint bridge without falling back to a stale ticket or local lookup', async () => {
    const { desktop, gateway } = await activateRemoteGateway()
    Reflect.deleteProperty(window.socisDesktop, 'getGatewayWsUrlFor')
  ...
  it('rejects instead of hanging forever when the reconnect getConnection() wedges (#93454)', async () => {
    ...
    } as unknown as SOCISGateway

    const getConnection = vi.fn(() => new Promise(() => undefined))

    ;(window as unknown as { socisDesktop: unknown }).socisDesktop = { getConnection }
    $gateway.set(dropped)
