import { isGatewayReauthRequired, JsonRpcGatewayError, resolveGatewayWsUrl } from '@socis/shared'
import { useEffect, useRef } from 'react'

import { shouldApplyPostBootProgressError } from '@/components/boot-failure-reauth'
import type { SOCISConnection } from '@/global'
import { SOCISGateway } from '@/socis'
...
import type { RpcEvent } from '@/types/socis'
...
export function primaryRuntimeConnectionId(connection: Pick<SOCISConnection, 'connectionId' | 'mode'>): null | string {
...
interface GatewayBootOptions {
  ...
  onConnectionReady: (
    connection: Awaited<ReturnType<NonNullable<typeof window.socisDesktop>['getConnection']>> | null
  ) => void
  onGatewayReady: (gateway: SOCISGateway | null) => void
  refreshSOCISConfig: (force?: boolean, shouldPublish?: () => boolean) => Promise<void>
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    const publish = (next: SOCISConnection | null) => {
    ...
    // composer stayed disabled forever on "Starting SOCIS...". Once the
    ...
    const attemptReconnect = async () => {
      ...
      try {
        ...
        // "Starting SOCIS…". The probe is a no-op for a healthy or local backend.
        ...
        const conn = await withTimeout(
          ...
          RECONNECT_ATTEMPT_TIMEOUT_MS,
          'Timed out reconnecting to SOCIS backend'
        )
        ...
        // as an opaque "Could not connect to SOCIS gateway". resolveGatewayWsUrl
        ...
        await callbacksRef.current.refreshSOCISConfig().catch(() => undefined)
    ...
    const softSwitch = async () => {
      ...
      try {
        ...
        await Promise.all([
          ...
          callbacksRef.current.refreshSOCISConfig(false, ownsSwitch).catch(() => undefined),
    ...
    const offBootProgress = desktop.onBootProgress(payload => {
      // Soft switch / post-boot startSOCIS re-emits progress — ignore so the
    ...
    const gateway = adoptedFromHmr ? survivor!.gateway : new SOCISGateway()
    ...
    async function boot() {
      try {
        ...
        // hanging "Starting SOCIS…" forever.
        const wsUrl = await withTimeout(
          resolveGatewayWsUrl(desktop, conn),
          RECONNECT_ATTEMPT_TIMEOUT_MS,
        ...
        await Promise.all([
          ...
          callbacksRef.current.refreshSOCISConfig(),
          ...
          // retry it), not brick boot behind the "SOCIS couldn't start"
