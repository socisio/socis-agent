import { isGatewayReauthRequired, resolveGatewayWsUrl } from '@socis/shared'
...
export function useGatewayRequest() {
  ...
  const gateway = useStore($gateway) as SOCISGateway | null
  const gatewayRef = useRef<SOCISGateway | null>(null)

  const connectionRef = useRef<Awaited<ReturnType<NonNullable<typeof window.socisDesktop>['getConnection']>> | null>(
    null
  )

  const gatewayStateRef = useRef(gatewayState)
  const reconnectingRef = useRef<Promise<SOCISGateway | null> | null>(null)
  ...
  useEffect(
    () =>
      $gateway.subscribe(gateway => {
        gatewayRef.current = gateway as SOCISGateway | null
      }),
    []
  )
