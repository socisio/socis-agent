...
function setWorkspaceTransfer(transferring: boolean): void {
  window.socisDesktop?.hud?.setWorkspaceTransfer?.(transferring)
}

function moveHud(state: PressState): void {
  window.socisDesktop?.hud?.moveBy?.({
    width: state.originW,
...
function armGrab(state: PressState, workspaceTransfer: boolean): void {
  ...
  window.socisDesktop?.hud?.beginMove?.()
}
 ...
 * (see socis:hud:move-by — a transparent frameless window drifts wider on
...
export function useHudComposerDrag(
  ...
  const reset = useCallback(() => {
    ...
    if (state) {
      if (state.armed) {
        window.socisDesktop?.hud?.endMove?.()
      }
