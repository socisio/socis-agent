...
import { getElevenLabsVoices, getSOCISConfigSchema, saveSOCISConfig } from '@/socis'
...
import type { ConfigFieldSchema, SOCISConfigRecord } from '@/types/socis'

import { socisConfigCacheWriter, useSOCISConfigRecord } from '../hooks/use-config-record'
...
function ConfigSettingsInner({
  ...
  const [config, setConfig] = useState<SOCISConfigRecord | null>(null)
  const { data: loadedConfig, isError: configLoadFailed, refetch: refetchConfig } = useSOCISConfigRecord(scopeProfile)
  ...
  const writeConfigCache = useMemo(() => socisConfigCacheWriter(scopeProfile), [scopeProfile])
  ...
  const {
    ...
    queryKey:
      scopeProfile == null ? ['socis-config-schema'] : ['socis-config-schema', normalizeProfileKey(scopeProfile)],
    queryFn: () => getSOCISConfigSchema(scopeProfile),
    staleTime: 5 * 60 * 1000
  })
  ...
  // never touched — possibly changed out-of-band by `socis config set`
  // while this page sat open — is never resent with its stale value.
  const configBaselineRef = useRef<SOCISConfigRecord | null>(null)
    ...
    const t = window.setTimeout(() => {
      ...
      saveQueueRef.current = saveQueueRef.current.then(async () => {
        try {
          ...
          const result = await saveSOCISConfig(patch, scopeProfile)
  ...
  const applyConfig = (next: SOCISConfigRecord) => {
    saveVersionRef.current += 1
    setConfig(next)
    setSaveVersion(saveVersionRef.current)
  }

  const updateConfig = (next: SOCISConfigRecord) => {
