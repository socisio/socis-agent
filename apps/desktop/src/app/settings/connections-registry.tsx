 ...
 * agent sources (local runtime + any number of remote gateways / SOCIS Cloud
...
export function ConnectionsRegistrySection() {
  ...
  useEffect(() => {
    if (!editorWantsOauth || !editorUrl || !window.socisDesktop?.probeConnectionConfig) {
      setAuthProbe(null)
  ...
  const signInOauth = useCallback(async () => {
    ...
    try {
      const result = await window.socisDesktop.oauthLoginConnectionConfig(editorUrl)
  ...
  // Fan out `socis update` to every eligible source; per-connection results
