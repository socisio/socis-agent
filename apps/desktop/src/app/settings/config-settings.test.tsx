...
const getSOCISConfigSchema = vi.fn()
const saveSOCISConfig = vi.fn()
const getElevenLabsVoices = vi.fn()

vi.mock('@/socis', () => ({
  getSOCISConfigRecord: () => getSOCISConfigRecord(),
  getSOCISConfigSchema: () => getSOCISConfigSchema(),
  saveSOCISConfig: (config: unknown, profile?: string) => saveSOCISConfig(config, profile),
...
beforeEach(() => {
  ...
  getSOCISConfigSchema.mockResolvedValue({ fields: {} })
...
describe('ConfigSettings autosave', () => {
  it('sends a later revert instead of diffing it away against the stale page-load baseline', async () => {
    getSOCISConfigRecord.mockResolvedValue({ checkpoints: { enabled: false }, other: 'untouched' })
    ...
    try {
      ...
      await waitFor(() => expect(saveSOCISConfig).toHaveBeenCalledTimes(1))
      expect(saveSOCISConfig.mock.calls[0][0]).toEqual({ checkpoints: { enabled: true } })

      // Revert: flip it back to its original value and let autosave fire again.
      toggle.click()
      await vi.advanceTimersByTimeAsync(700)

      await waitFor(() => expect(saveSOCISConfig).toHaveBeenCalledTimes(2))
      ...
      expect(saveSOCISConfig.mock.calls[1][0]).toEqual({ checkpoints: { enabled: false } })
    } finally {
      vi.useRealTimers()
    }
  })
