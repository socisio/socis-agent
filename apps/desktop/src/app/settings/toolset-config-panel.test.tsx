...
import type { ToolsetConfig } from '@/types/socis'
...
beforeEach(() => {
  ...
  getSOCISConfigRecord.mockResolvedValue({
    tts: {
      provider: 'edge',
  ...
  getSOCISConfigSchema.mockResolvedValue({ fields: {}, category_order: [] })
...
describe('ToolsetConfigPanel', () => {
  it('renders inline voice/model fields for a TTS provider row carrying tts_provider', async () => {
    ...
    await waitFor(() => expect(saveSOCISConfig).toHaveBeenCalled(), { timeout: 3000 })
    const saved = saveSOCISConfig.mock.calls.at(-1)?.[0] as Record<string, Record<string, Record<string, string>>>
