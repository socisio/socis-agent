...
import { type ProfileScope, saveSOCISConfigRecord } from '@/socis'
 ...
 * the default browser's profile (cookies/logins) into a SOCIS-owned copy;
...
export function BrowserRealProfilePanel({ profile }: BrowserRealProfilePanelProps) {
  ...
  const { data: config } = useSOCISConfigRecord(profile)
  ...
  const toggle = useCallback(
    async (on: boolean) => {
      ...
      try {
        await saveSOCISConfigRecord(next, profile)
        notify({
          kind: 'info',
