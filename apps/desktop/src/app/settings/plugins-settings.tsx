...
// User-installed plugins first — mirrors `socis plugins list --user`.
...
function reveal(file: string) {
  void window.socisDesktop?.revealPath?.(file)?.catch(() => undefined)
}

async function revealPluginsDir() {
  try {
    // Electron owns the local plugin root — deriving it from the backend's
    // socis_agent_home breaks against a remote backend (#66899).
    const dir = await window.socisDesktop?.desktopPluginsRoot?.()
    ...
    const result = await window.socisDesktop?.openDir?.(dir)
...
// Agent plugins live under the BACKEND's socis home (profile-aware), so the
// path comes from the gateway — not from the renderer's local SOCIS_AGENT_HOME.
...
async function revealAgentPluginsDir(request: GatewayRequest) {
  try {
    ...
    const opened = await window.socisDesktop?.openDir?.(`${home}/plugins`)
            ...
            <SelectContent>
              {profiles.map(profile => (
                <SelectItem key={profile.name} value={profile.name}>
                  {profile.is_default ? 'SOCIS (default)' : profile.name}
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
