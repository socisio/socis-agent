...
import { saveSOCISConfig } from '@/socis'
import { useI18n } from '@/i18n'
import { notifyError } from '@/store/notifications'
import type { SOCISConfigRecord } from '@/types/socis'

import { setSOCISConfigCache, useSOCISConfigRecord } from '../hooks/use-config-record'
...
function fontFamilyFromConfig(config: SOCISConfigRecord): string {
...
export function TerminalFontSetting() {
  ...
  const { data: loadedConfig } = useSOCISConfigRecord()
  ...
  const [staleConfig, setStaleConfig] = useState<SOCISConfigRecord | null>(null)
  ...
  useEffect(() => {
    ...
    // holds — successful saves write it back via setSOCISConfigCache, so
        ...
        <div className="mt-3 space-y-2">
          <div className="flex items-center gap-3">
            <Input
              ...
              className="flex-1"
              disabled={draft === null}
              list="socis-terminal-font-families"
          ...
          <datalist id="socis-terminal-font-families">
