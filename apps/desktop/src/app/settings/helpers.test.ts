...
import type { SOCISConfigRecord } from '@/types/socis'
...
describe('settings helpers', () => {
  ...
  it('reads and writes nested config paths', () => {
    const config: SOCISConfigRecord = { display: { theme: 'mono' } }
  ...
  it('rejects prototype-polluting config paths', () => {
    const config: SOCISConfigRecord = {}
  ...
  describe('providerGroup', () => {
    ...
    it('prefers the longest matching prefix so CN/regional buckets win', () => {
      ...
      // SOCIS_AGENT_QWEN_ shares the SOCIS_AGENT_ stem with other integrations.
      expect(providerGroup('SOCIS_AGENT_QWEN_BASE_URL')).toBe('DashScope (Qwen)')
  ...
  describe('enumOptionsFor — backend selector dropdowns', () => {
    ...
    it('surfaces user-defined command-type TTS providers (canonical providers nesting + legacy)', () => {
      const withCustom: SOCISConfigRecord = {
        tts: {
          provider: 'neutts',
    ...
    it('never offers a built-in name as a command provider, even one absent from the dropdown list', () => {
      const shadowing: SOCISConfigRecord = {
        tts: {
          provider: 'edge',
          providers: {
  ...
  describe('sectionFieldEntries', () => {
    it('renders memory.provider from config even when the backend schema omits it', () => {
      ...
      const config: SOCISConfigRecord = { memory: { memory_enabled: true, provider: '' } }
    ...
    it('infers the field type from the config value when the schema omits the key', () => {
      const config: SOCISConfigRecord = { memory: { provider: '', memory_enabled: true, memory_char_limit: 2200 } }
    ...
    it('prefers the backend schema entry over inference when both exist', () => {
      ...
      const config: SOCISConfigRecord = { memory: { provider: 'honcho' } }
  ...
  describe('clearsEnabledToolsets', () => {
    it('flags a non-empty → empty transition', () => {
      const prev: SOCISConfigRecord = { toolsets: ['memory', 'terminal', 'web_search'] }
      const next: SOCISConfigRecord = { toolsets: [] }
    ...
    it('does not flag a non-empty → missing transition (deep-merge preserves the key)', () => {
      ...
      const prev: SOCISConfigRecord = { toolsets: ['memory'] }
      const next: SOCISConfigRecord = {}

      expect(clearsEnabledToolsets(prev, next)).toBe(false)
    })

    it('does not flag when at least one toolset remains', () => {
      const prev: SOCISConfigRecord = { toolsets: ['memory', 'terminal'] }
      const next: SOCISConfigRecord = { toolsets: ['memory'] }

      expect(clearsEnabledToolsets(prev, next)).toBe(false)
    })

    it('does not flag when the list was already empty', () => {
      const prev: SOCISConfigRecord = { toolsets: [] }
      const next: SOCISConfigRecord = { toolsets: [] }
    ...
    it('does not flag an unrelated edit that never touched toolsets', () => {
      const prev: SOCISConfigRecord = { model: 'a', toolsets: ['memory'] }
      const next: SOCISConfigRecord = { model: 'b', toolsets: ['memory'] }
  ...
  describe('diffConfig', () => {
    it('omits a top-level key the draft never touched', () => {
      ...
      // an agent set via `socis config set` while the page sat open must not
      // come back in the patch just because it's still present in the draft.
      const baseline: SOCISConfigRecord = { fallback_providers: ['nara1'], timezone: 'UTC' }
      const draft: SOCISConfigRecord = { fallback_providers: ['nara1'], timezone: 'America/New_York' }
    ...
    it('includes a nested key only when it actually changed, leaving siblings out', () => {
      const baseline: SOCISConfigRecord = { display: { personality: 'default', show_reasoning: true } }
      const draft: SOCISConfigRecord = { display: { personality: 'default', show_reasoning: false } }
    ...
    it('sends a new key that was absent from the baseline', () => {
      const baseline: SOCISConfigRecord = {}
      const draft: SOCISConfigRecord = { timezone: 'UTC' }
    ...
    it('returns an empty object when the draft matches the baseline exactly', () => {
      const baseline: SOCISConfigRecord = { toolsets: ['memory'], display: { personality: 'default' } }
      const draft: SOCISConfigRecord = { toolsets: ['memory'], display: { personality: 'default' } }

      expect(diffConfig(baseline, draft)).toEqual({})
    })

    it('treats an array as a whole value, not diffed element by element', () => {
      const baseline: SOCISConfigRecord = { toolsets: ['memory', 'terminal'] }
      const draft: SOCISConfigRecord = { toolsets: ['memory'] }
