...
import { disconnectOAuthProvider, listOAuthProviders } from '@/socis'
...
import type { EnvVarInfo, OAuthProvider } from '@/types/socis'
...
const canRunInTerminal = () => typeof window !== 'undefined' && Boolean(window.socisDesktop?.terminal)
...
//      in socis_cli/provider_catalog.py) — the SAME provider identity
//      `socis model` uses. This is authoritative: a provider tagged by the
...
function ConnectedProviderRow({
  ...
  // SOCIS can clear this provider's creds via the API.
  const canDisconnect = provider.disconnectable ?? provider.flow !== 'external'
  // External (CLI-managed) provider SOCIS can't clear via the API, but ships a
...
export function ProvidersSettings({
  ...
  // SOCIS never deletes creds another tool owns behind a silent API call.
