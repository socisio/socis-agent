...
import { getSOCISConfigRecord, saveMcpServers } from '@/socis'
...
import { setSOCISConfigCache } from '../hooks/use-config-record'

/**
 * Explicit-confirm gate for `socis://mcp/install` deep links. The payload is
 ...
 * confirming lets SOCIS spawn that local process. An existing server name is
...
export function McpInstallDeepLinkDialog() {
  ...
  const confirm = async () => {
    ...
    try {
      ...
      const current = getServers(await getSOCISConfigRecord())
      ...
      setSOCISConfigCache(previous => (previous ? { ...previous, mcp_servers: nextServers } : previous))
