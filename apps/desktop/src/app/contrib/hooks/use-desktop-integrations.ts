...
import { pathFromSOCISDeepLink, resolveSOCISOpenPath } from '@/lib/socis-open-target'
...
export function useDesktopIntegrations({
  ...
  useEffect(() => {
    ...
    // "About SOCIS" — it is the OS-standard affordance for updating THIS app,
    ...
    const unsubscribe = window.socisDesktop?.onOpenUpdatesRequested?.(() => openUpdatesWindow('client'))
  ...
  useEffect(() => {
    window.socisDesktop?.setPreviewShortcutActive?.(true)
  }, [])
  ...
  useEffect(() => {
    const unsubscribe = window.socisDesktop?.onFocusSession?.(sessionId => {
      if (sessionId) {
  ...
  useEffect(() => {
    const unsubscribe = window.socisDesktop?.onNotificationAction?.(({ actionId, sessionId }) => {
  ...
  // socis://index-network/intent/1 vocabulary with deep links.
  useEffect(() => {
    const unsubscribe = window.socisDesktop?.onNotificationActivate?.(payload => {
      if (!payload) {
        return
      }
      ...
      if (payload.activate) {
        ...
        // the pre-IPC validation — any future socisDesktop.notify caller gets
        // funneled through the same resolver.
        const path = resolveSOCISOpenPath(payload.activate)
  ...
  useEffect(() => {
    const unsubscribe = window.socisDesktop?.onDeepLink?.(payload => {
      if (!payload?.kind) {
        return
      }
      ...
      // link (socis://index-network/intent/1, socis://open/…). The resolver
      // rejects reserved kinds and unsafe paths.
      const path = pathFromSOCISDeepLink(payload.kind, payload.name || '', payload.params || {})
    ...
    void window.socisDesktop?.signalDeepLinkReady?.()
  ...
  useEffect(() => {
    const unsubscribe = window.socisDesktop?.onClosePreviewRequested?.(
  ...
  useEffect(() => {
    const unsubscribe = window.socisDesktop?.onPreviewNav?.(command => {
  ...
  useEffect(() => {
    const unsubscribe = window.socisDesktop?.onOpenFolderRequested?.(() => void openFolderAsProject())
