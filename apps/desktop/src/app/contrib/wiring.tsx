...
import { getLatestSessionMessages } from '@/socis'
...
import { useSOCISConfigRecord } from '../hooks/use-config-record'
...
import { useSOCISConfig } from '../session/hooks/use-socis-config'
...
export function ContribWiring({ children }: { children: ReactNode }) {
  ...
  const { refreshSOCISConfig, sttEnabled, voiceMaxRecordingSeconds } = useSOCISConfig({ activeSessionIdRef })
  ...
  useEffect(() => {
    ...
    window.addEventListener('socis:open-keybinds', onOpenKeybinds)

    return () => window.removeEventListener('socis:open-keybinds', onOpenKeybinds)
  }, [navigate])
  ...
  useEffect(() => {
    ...
    void refreshSOCISConfig(true)
    void refreshActiveProfile()
    resetProjectTreeState()
  }, [gatewayScope, refreshCurrentModel, refreshSOCISConfig])
  ...
  const configRecord = useSOCISConfigRecord()
