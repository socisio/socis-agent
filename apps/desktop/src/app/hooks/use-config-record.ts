...
import { getSOCISConfigRecord, type ProfileScope, profileScopeKey } from '@/socis'
...
//
// Distinct from session/hooks/use-socis-config.ts, which is side-effecting —
// it pushes personality/cwd/voice/… into the session stores for live chat.
export const SOCIS_AGENT_CONFIG_KEY = ['socis-config-record'] as const
...
// share a cache row.
export const socisConfigKey = (profile?: ProfileScope) =>
  profile == null ? SOCIS_AGENT_CONFIG_KEY : ([...SOCIS_AGENT_CONFIG_KEY, profileScopeKey(profile)] as const)
...
export const useSOCISConfigRecord = (profile?: ProfileScope) =>
  useQuery({
    queryKey: socisConfigKey(profile),
    ...
    queryFn: () => getSOCISConfigRecord(profile ?? undefined),
    staleTime: 0
  })

// setSOCISConfigCache writes the app-wide (base-key) record. Pass a profile to
...
export const setSOCISConfigCache = writeCache<SOCISConfigRecord>(SOCIS_AGENT_CONFIG_KEY)
export const socisConfigCacheWriter = (profile?: ProfileScope) =>
  writeCache<SOCISConfigRecord>(socisConfigKey(profile))

export const invalidateSOCISConfig = (profile?: ProfileScope) =>
  queryClient.invalidateQueries({ queryKey: socisConfigKey(profile) })
