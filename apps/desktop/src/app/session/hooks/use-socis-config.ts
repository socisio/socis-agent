...
import { getSOCISConfig, getSOCISConfigDefaults } from '@/socis'
...
interface SOCISConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
}

export function useSOCISConfig({ activeSessionIdRef }: SOCISConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)
  const profileRefreshEpochRef = useRef(0)

  const refreshSOCISConfig = useCallback(
    async (force = false, shouldPublish: () => boolean = () => true) => {
      ...
      try {
        const [config, defaults] = await Promise.all([getSOCISConfig(), getSOCISConfigDefaults().catch(() => ({}))])
        ...
        // rendering/applying SOCIS' built-in medium over the user's config.
        if (!canPublish()) {
          return
        }
  ...
  return { refreshSOCISConfig, sttEnabled, voiceMaxRecordingSeconds }
}
