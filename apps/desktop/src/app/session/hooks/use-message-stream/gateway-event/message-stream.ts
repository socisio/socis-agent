import type { BillingBlock } from '@socis/shared'
 ...
 * billing-specific toast — never the generic "SOCIS error" — with a smart CTA
  ...
  if (event.type === 'message.complete') {
    ...
    const failure =
      payload?.status === 'error'
        ? {
            error: coerceGatewayText(payload.error).trim() || finalText || 'SOCIS reported an error',
