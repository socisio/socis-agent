import { registryBackendScopeKey } from '@socis/shared'
...
export function useGatewayEventHandler(deps: GatewayEventDeps) {
  const { activeSessionIdRef, compactedTurnRef, refreshSOCISConfig, sessionStateByRuntimeIdRef } = deps
  ...
  // refreshSOCISConfig — two REST calls (config + defaults) per event, per
