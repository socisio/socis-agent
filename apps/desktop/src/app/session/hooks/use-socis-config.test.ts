...
import { getSOCISConfig } from '@/socis'
...
import { useSOCISConfig } from './use-socis-config'

vi.mock('@/socis', () => ({
  getSOCISConfig: vi.fn(),
  getSOCISConfigDefaults: vi.fn().mockResolvedValue({})
}))

const WORKSPACE_CWD_KEY = 'socis.desktop.workspace-cwd'

const mockConfig = (config: Record<string, unknown>) =>
  vi.mocked(getSOCISConfig).mockResolvedValue(config as Awaited<ReturnType<typeof getSOCISConfig>>)

describe('useSOCISConfig refreshSOCISConfig', () => {
  beforeEach(() => {
  ...
  // `agent.reasoning_effort: high` to SOCIS' built-in medium.
  it('publishes the profile default effort even when a manual pick blocks the composer reseed', async () => {
    ...
    const { result } = renderHook(() => useSOCISConfig({ activeSessionIdRef: { current: null } }))

    await act(async () => {
      await result.current.refreshSOCISConfig()
    })
  ...
  it('does not let terminal.cwd replace an active session workspace', async () => {
    ...
    mockConfig({ terminal: { cwd: '/Users/example/new-workspace' } })
    const { result } = renderHook(() => useSOCISConfig({ activeSessionIdRef: { current: 'session-1' } }))
  ...
  it('does not let a stale forced config refresh overwrite newer draft selector intent', async () => {
    const profileConfig = deferred<Awaited<ReturnType<typeof getSOCISConfig>>>()
    vi.mocked(getSOCISConfig).mockReturnValueOnce(profileConfig.promise)

    const { result } = renderHook(() => useSOCISConfig({ activeSessionIdRef: { current: null } }))

    let pendingRefresh!: Promise<void>
    act(() => {
      pendingRefresh = result.current.refreshSOCISConfig(true)
    })
    expect(getSOCISConfig).toHaveBeenCalled()
    ...
    } as Awaited<ReturnType<typeof getSOCISConfig>>)
  ...
  it('does not publish config after its switch loses ownership', async () => {
    const staleConfig = deferred<Awaited<ReturnType<typeof getSOCISConfig>>>()
    vi.mocked(getSOCISConfig).mockReturnValueOnce(staleConfig.promise)
    const { result } = renderHook(() => useSOCISConfig({ activeSessionIdRef: { current: null } }))
    let ownsSwitch = true

    let refresh!: Promise<void>
    act(() => {
      refresh = result.current.refreshSOCISConfig(false, () => ownsSwitch)
    })
  ...
  it('does not let an older profile config overwrite a newer profile', async () => {
    const profileB = deferred<Awaited<ReturnType<typeof getSOCISConfig>>>()
    const profileC = deferred<Awaited<ReturnType<typeof getSOCISConfig>>>()
    vi.mocked(getSOCISConfig).mockReturnValueOnce(profileB.promise).mockReturnValueOnce(profileC.promise)
    ...
    act(() => {
      refreshB = result.current.refreshSOCISConfig(true)
      refreshC = result.current.refreshSOCISConfig(true)
    })
