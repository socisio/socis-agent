...
import type * as SOCISApi from '@/socis'
...
vi.mock('@/socis', async importOriginal => ({
  ...(await importOriginal<typeof SOCISApi>()),
...
describe('SkillsView toolset management', { timeout: 60_000 }, () => {
    ...
    await act(async () => {
      window.dispatchEvent(
        new MessageEvent('message', {
          data: { type: 'socis-skill-pick', name: 'web-research', identifier: 'web-research' },
          origin: 'https://agent.socis.io'
        })
      )
    })
  ...
  it('offers (connection, profile) scope rows on multi-connection desktops', async () => {
    ...
    ;(window as { socisDesktop?: unknown }).socisDesktop = { connections, getAgentRoster }
