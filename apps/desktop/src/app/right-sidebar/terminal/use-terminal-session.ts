...
const SOCIS_AGENT_PATHS_MIME = 'application/x-socis-paths'
...
function transferHasDropCandidates(t: DataTransfer): boolean {
  if (t.types?.includes(SOCIS_AGENT_PATHS_MIME)) {
    return true
  }
...
function collectDroppedPaths(t: DataTransfer): string[] {
  ...
  try {
    const raw = t.getData(SOCIS_AGENT_PATHS_MIME)
  ...
  const getPath = window.socisDesktop?.getPathForFile
...
export function useTerminalSession({
  ...
  useEffect(() => {
    ...
    const terminalApi = window.socisDesktop?.terminal
    ...
    const term = new Terminal({
      ...
      // Full-screen TUIs (socis --tui, vim) grab the mouse, so a plain drag
    ...
    term.attachCustomKeyEventHandler(event => {
      ...
      void (async () => {
        const text = (await window.socisDesktop?.readClipboard?.()) ?? ''
    ...
    const mount = () => {
      ...
      try {
        ...
        console.warn('[socis-terminal] WebGL unavailable; falling back to DOM', err)
      }
  ...
  useEffect(() => {
    ...
    return $terminalInjection.subscribe(command => {
      ...
      hasSessionActivityRef.current = true
      void window.socisDesktop?.terminal?.write(sessionId, `${command}\r`)
