...
import type { SOCISReadDirEntry, SOCISReadDirResult } from '@/global'
...
export type ProjectTreeEntry = SOCISReadDirEntry
...
function ignoredBy(rules: GitignoreRule[], entry: SOCISReadDirEntry) {
...
async function filterIgnored(entries: SOCISReadDirEntry[], rootPath: string, dirPath: string) {
...
export async function readProjectDir(dirPath: string, rootPath = dirPath): Promise<SOCISReadDirResult> {
  if (!window.socisDesktop) {
