...
beforeEach(() => {
  ...
  readDir.mockReset()
  ;(window as unknown as { socisDesktop: { readDir: typeof readDir } }).socisDesktop = { readDir }
})
...
function ok(entries: { name: string; path: string; isDirectory: boolean }[]): SOCISReadDirResult {
  return { entries }
}
...
describe('useProjectTree', () => {
  ...
  it('reads gitignore from the real path while caching per connection', async () => {
    ...
    ;(window as unknown as { socisDesktop: unknown }).socisDesktop = { gitRoot, readDir, readFileDataUrl }
  ...
  it('dedupes concurrent loadChildren calls for the same id', async () => {
    ...
    let resolveChildren: ((value: SOCISReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<SOCISReadDirResult>(resolve => {
  ...
  it('discards a stale live refresh after the active registered connection changes', async () => {
    let resolveRefreshFromA: ((result: SOCISReadDirResult) => void) | undefined
  ...
  it('discards a stale child read after the active registered connection changes', async () => {
    let resolveChildFromA: ((result: SOCISReadDirResult) => void) | undefined
  ...
  it('discards a stale root read after the active registered connection changes', async () => {
    let resolveFirst: ((result: SOCISReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<SOCISReadDirResult>(resolve => {
  ...
  it('returns no-bridge gracefully when window.socisDesktop is missing', async () => {
    delete (window as unknown as { socisDesktop?: unknown }).socisDesktop
  ...
  it('keeps an unreadable root on screen while it re-probes', async () => {
    ...
    let releaseProbe: ((value: SOCISReadDirResult) => void) | undefined
  ...
  it('keeps loaded rows on screen while the root refreshes', async () => {
    ...
    let releaseRefresh: ((value: SOCISReadDirResult) => void) | undefined
  ...
  it('clears the rows when the same path is re-read from another backend', async () => {
    ...
    let releaseFromB: ((value: SOCISReadDirResult) => void) | undefined
