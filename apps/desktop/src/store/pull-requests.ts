...
import type { SOCISBranchPullRequest } from '@/global'
import { scanSessionPullRequests, type SessionInfo } from '@/socis'
...
export const $pullRequestsByBranch = atom<Record<string, SOCISBranchPullRequest>>({})
...
export const $prBranchBySession = persistentAtom<Record<string, string>>(
  'socis.desktop.prBranchBySession',
  {},
  Codecs.stringRecord
)
...
const $prScannedSessions = persistentAtom<string[]>('socis.desktop.prScannedSessions', [], Codecs.stringArray)
...
export function pullRequestBucket(pr: SOCISBranchPullRequest | undefined): PullRequestBucket {
  if (!pr) {
    return 'none'
  }
