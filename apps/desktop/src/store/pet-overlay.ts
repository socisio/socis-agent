 ...
 * app's bounds and stays visible while SOCIS is minimized. That window carries
...
const OVERLAY_ACTIVE_KEY = 'socis.desktop.pet-overlay-active.v1'
const OVERLAY_BOUNDS_KEY = 'socis.desktop.pet-overlay-bounds.v1'
...
function pushNow(): void {
  window.socisDesktop?.petOverlay?.pushState(currentPayload())
}
...
function openOverlay(request: PetOverlayOpenRequest): void {
  const api = window.socisDesktop?.petOverlay
...
export function restorePetOverlay(): void {
  if (!window.socisDesktop?.petOverlay || !$petOverlayActive.get() || stateUnsubs.length) {
    return
  }
...
export function popInPet(): void {
  ...
  stateUnsubs = []
  $petOverlayActive.set(false)
  void window.socisDesktop?.petOverlay?.close()
}
