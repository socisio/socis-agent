...
import { getSOCISConfigRecord, saveSOCISConfig } from '@/socis'
...
export async function setAutoSpeakReplies(enabled: boolean): Promise<void> {
  ...
  try {
    const record = await getSOCISConfigRecord()
    const voice = record.voice && typeof record.voice === 'object' ? (record.voice as Record<string, unknown>) : {}

    await saveSOCISConfig({ ...record, voice: { ...voice, auto_tts: enabled } })
  } catch (error) {
