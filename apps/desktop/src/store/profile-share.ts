 ...
 *
 * The archive is the CLI's own `socis profile export` tar.gz (config, skills,
...
import { exportProfileArchive, importProfileArchive } from '@/socis'
...
import type { ProfileDesktopOverlay } from '@/types/socis'
...
const ARCHIVE_FILTERS = [{ extensions: ['tar.gz', 'tgz'], name: 'SOCIS profile' }]
...
export async function runExportProfileFlow(profile?: string): Promise<null | string> {
  ...
  const pick = window.socisDesktop?.selectSavePath
...
export async function runImportProfileFlow(): Promise<null | string> {
  const paths = await window.socisDesktop?.selectPaths?.({
