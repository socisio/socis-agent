...
const { getProfiles } = await import('@/socis')
...
const remoteConn = (over: Partial<SOCISConnection> = {}): SOCISConnection =>
  ({ baseUrl: 'https://hermes-roy.tail.ts.net', mode: 'remote', profile: 'vps-remote', ...over }) as SOCISConnection
...
beforeEach(() => {
  ...
  $profiles.set([])
  vi.stubGlobal('window', { socisDesktop: { getConnection } })
