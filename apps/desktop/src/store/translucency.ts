 ...
 * `@socis/shared/translucency`, which owns the mapping both this store and the
 * main process read.
 *
 ...
 * material underneath the web contents to read (see the `[data-socis-glass]`
 * block in styles.css).
 */
...
} from '@socis/shared/translucency'
 ...
 * sets `socisDesktop.glassSupported` from `os.release()` so Win10 cannot
...
export const GLASS_SUPPORTED =
  typeof window !== 'undefined' && typeof window.socisDesktop?.glassSupported === 'boolean'
    ? window.socisDesktop.glassSupported
...
export const TRANSLUCENCY_SUPPORTED =
  typeof window !== 'undefined' && typeof window.socisDesktop?.translucencySupported === 'boolean'
    ? window.socisDesktop.translucencySupported
    : isMacPlatform() || isWindowsPlatform()
...
/* Peek: while the user is actively adjusting translucency from Settings, the
   ...
   [data-socis-translucency-peek] rules in styles.css). A counter rather
   than a boolean: a held slider drag and a timed pulse from a picker click
   can overlap. */
const PEEK_ATTR = 'data-socis-translucency-peek'
...
const applyGlassSurfaces = ({ intensity, mode, scope }: TranslucencyState): void => {
  ...
  root.toggleAttribute('data-socis-glass-on', glassLive)
  root.toggleAttribute('data-socis-glass', glassOn)
  root.toggleAttribute('data-socis-clear', clearOn)
  ...
  if (glassOn) {
    root.setAttribute('data-socis-glass-scope', scope)
  } else {
    root.removeAttribute('data-socis-glass-scope')
  }
...
if (typeof window !== 'undefined') {
  ...
  $translucency.subscribe(state => {
    ...
    window.socisDesktop?.setTranslucency?.(state)
  })
