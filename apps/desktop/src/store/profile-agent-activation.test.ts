...
const agentConn = (over: Partial<SOCISConnection> = {}): SOCISConnection =>
  ({ baseUrl: 'https://homelab.invalid', mode: 'remote', profile: 'research', ...over }) as SOCISConnection

const localConn = (over: Partial<SOCISConnection> = {}): SOCISConnection =>
  ({ baseUrl: '', mode: 'local', profile: 'default', ...over }) as SOCISConnection

const getConnection = vi.fn<(profile?: string | null) => Promise<SOCISConnection>>()

const getConnectionFor =
  vi.fn<(payload: { connectionId?: null | string; profile?: null | string }) => Promise<SOCISConnection>>()
...
beforeEach(() => {
  ...
  vi.stubGlobal('window', { socisDesktop: { getConnection, getConnectionFor }, localStorage })
...
describe('ensureGatewayAgent commit hook (beforeActivate) — the Sessions switcher door (#93937)', () => {
  ...
  it('a descriptor lookup that never answers is bounded, fails open, and releases the mutex', async () => {
    ...
    try {
      getConnectionFor.mockImplementationOnce(() => new Promise<SOCISConnection>(() => undefined))
