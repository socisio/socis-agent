...
/**
 * Feature store for backend (agent) plugins — the native SOCIS plugins plus
 ...
 * primitives `socis plugins` and the dashboard use, so all surfaces agree on
...
export interface AgentPluginRow {
  ...
  /** Agent Plugins v1 package (portable skills/MCP format) vs native SOCIS. */
  portable?: boolean
}
 ...
 *  SOCIS_AGENT_HOME. Always refetches (it's a cheap local disk scan on the
