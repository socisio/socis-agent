 ...
 * composer with the reply scrolling above it, so SOCIS can be driven while
...
export const canUseHud = (): boolean =>
  typeof window !== 'undefined' && typeof window.socisDesktop?.hud?.open === 'function'

export function openHud(sessionId?: null | string): void {
  const api = window.socisDesktop?.hud
...
export function resetHudLayout(): void {
  void window.socisDesktop?.hud?.resetLayout?.()
}
...
export const reportHudSession = (sessionId: null | string): void => window.socisDesktop?.hud?.setSession?.(sessionId)
...
export function watchHudState(onClosed?: (sessionId: null | string) => void): () => void {
  const off = window.socisDesktop?.hud?.onChanged?.(({ open, sessionId }) => {
    $hudActive.set(open)
