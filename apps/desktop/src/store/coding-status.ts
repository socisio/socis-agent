...
import type { SOCISGitWorktree, SOCISRepoStatus } from '@/global'
...
const EMPTY_WORKTREES: SOCISGitWorktree[] = []
...
export const $repoStatusByCwd = atom<Record<string, SOCISRepoStatus | null>>({})
export const $repoWorktreesByCwd = atom<Record<string, SOCISGitWorktree[]>>({})
...
export const $repoStatus: ReadableAtom<SOCISRepoStatus | null> = computed(
...
export const $repoWorktrees: ReadableAtom<SOCISGitWorktree[]> = computed(
...
const statusAtomByCwd = new Map<string, ReadableAtom<SOCISRepoStatus | null>>()
const worktreesAtomByCwd = new Map<string, ReadableAtom<SOCISGitWorktree[]>>()
const $noRepoStatus = atom<SOCISRepoStatus | null>(null)
const $noWorktrees = atom<SOCISGitWorktree[]>(EMPTY_WORKTREES)

/** Reactive status for one repo cwd (a tile's worktree). Stable per cwd. */
export function repoStatusForCwd(cwd?: null | string): ReadableAtom<SOCISRepoStatus | null> {
...
export function repoWorktreesForCwd(cwd?: null | string): ReadableAtom<SOCISGitWorktree[]> {
  const key = normalizeCwd(cwd)
...
function setRepoStatusEntry(target: string, status: SOCISRepoStatus | null): void {
...
function setRepoWorktreesEntry(target: string, worktrees: SOCISGitWorktree[]): void {
...
interface RepoStatusRefreshRequest {
  probe: (cwd: string) => Promise<SOCISRepoStatus | null>
  seq: number
  target: string
}
