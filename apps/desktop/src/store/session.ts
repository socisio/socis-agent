import type { ConnectionState } from '@socis/shared'
...
import type { SessionInfo, UsageStats } from '@/types/socis'
...
const COMPOSER_MODEL_KEY = 'socis.desktop.composer.model'
const COMPOSER_PROVIDER_KEY = 'socis.desktop.composer.provider'
const COMPOSER_MODEL_SOURCE_KEY = 'socis.desktop.composer.model-source'
const COMPOSER_EFFORT_KEY = 'socis.desktop.composer.reasoning-effort'
const COMPOSER_FAST_KEY = 'socis.desktop.composer.fast'
...
function composerScopeForConnection(connection: SOCISConnection | null): string | null {
  if (!connection) {
    return null
  }
...
const LAST_SESSION_KEY = 'socis.desktop.lastSessionId'
const LAST_ROUTE_KEY = 'socis.desktop.lastRoute'
...
function workspaceCwdKey(connection: SOCISConnection | null = $connection.get()): string {
...
export async function syncConfiguredDefaultProjectDir(shouldPublish: () => boolean = () => true): Promise<string> {
  const settings = window.socisDesktop?.settings?.getDefaultProjectDir
...
export async function ensureDefaultWorkspaceCwd(shouldPublish: () => boolean = () => true): Promise<void> {
  const sanitize = window.socisDesktop?.sanitizeWorkspaceCwd
...
export const $connection = atom<SOCISConnection | null>(null)
...
const SESSION_OWNER_HINTS_KEY = 'socis.desktop.sessionOwnerHints.v1'
 ...
 * SOCISConnection lookup is unavailable. */
...
export const setConnection = (next: Updater<SOCISConnection | null>) => {
...
// until config loads, and re-seeded on every profile switch by useSOCISConfig.
