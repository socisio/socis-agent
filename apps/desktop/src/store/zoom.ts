...
export function setZoomPercent(percent: number): void {
  window.socisDesktop?.zoom?.setPercent(percent)
}

if (typeof window !== 'undefined' && window.socisDesktop?.zoom) {
  void window.socisDesktop.zoom.get().then(({ percent }) => $zoomPercent.set(percent))
  window.socisDesktop.zoom.onChanged(({ percent }) => $zoomPercent.set(percent))
}
