 ...
 *  Working in `~/www/socis-agent`, the draft says "socis-agent" constantly
 *  — a "Use skill: socis-agent" pill on every mention is noise (the reported
 *  annoyance that prompted this guard). Boundary containment, not exact
 *  segment equality, so worktree dirs (`socis-agent-suggest`) suppress too. */
 ...
 *  segment of a qualified form (`github/socis-agent-dev`, `plugin:skill`). */
