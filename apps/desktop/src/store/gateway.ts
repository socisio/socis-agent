import { type ConnectionState, type GatewayEvent, registryBackendScopeKey, resolveGatewayWsUrl } from '@socis/shared'
import { atom } from 'nanostores'

import type { SOCISConnection } from '@/global'
import { SOCISGateway, setApiRequestConnection } from '@/socis'
...
const isOpen = (gateway: SOCISGateway | null): boolean => gateway?.connectionState === 'open'
...
interface RegistryConfig {
  ...
  onActiveConnectionChanged?: (connection: SOCISConnection) => void
  /**
...
interface Secondary {
  ...
  connection: SOCISConnection | null
  gateway: SOCISGateway
...
interface GatewayRegistryState {
  ...
  primaryGateway: SOCISGateway | null
  ...
  $gateway: ReturnType<typeof atom<SOCISGateway | null>>
  $activeProfile: ReturnType<typeof atom<string>>
}

const STATE_KEY = Symbol.for('socis.desktop.gatewayRegistryState')
...
function createRegistryState(): GatewayRegistryState {
  return {
    ...
    $gateway: atom<SOCISGateway | null>(null),
...
export function setPrimaryGateway(gateway: SOCISGateway | null, profile = 'default'): void {
...
export function setPrimaryGatewayConnection(connection: Pick<SOCISConnection, 'connectionId'> | null): void {
...
export function activeGateway(): SOCISGateway | null {
  if (g.activeKey === g.primaryProfile) {
...
function applyActive(profile: string, activationEpoch: number): boolean {
  ...
  // Push the active scope's registry connection into the socis module (null
...
function publishActiveConnection(connection: SOCISConnection): void {
...
function createSecondary(profile: string, connectionId: null | string = null): Secondary {
  const gateway = new SOCISGateway()
...
async function gatewayForProfile(
  profile: string,
  leaseRequest = false
): Promise<{ gateway: SOCISGateway | null; key: string; release: () => void; scopeProfile: boolean }> {
  const key = normKey(profile)
...
export async function requestGatewayForProfile<T>(
  ...
  try {
    if (!route.gateway) {
      throw new Error(`SOCIS gateway unavailable for profile "${route.key}"`)
    }
...
export async function requestGatewayForAgent<T>(
  ...
  if (!window.socisDesktop?.getConnectionFor) {
    throw new Error('This Desktop build cannot dial registry connections. Update SOCIS Agent Desktop.')
  }
...
export async function ensureActiveGatewayOpen(): Promise<SOCISGateway | null> {
  if (g.activeKey === g.primaryProfile) {
    return g.primaryGateway
  }
  ...
  if (!isOpen(entry.gateway)) {
    ...
    // "SOCIS gateway is not connected" on the Sessions `+` action (#88880).
