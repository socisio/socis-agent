 ...
 *
 * Local models ship on main behind `--local` (either `socis desktop --local`
 * or the flag on SOCIS.exe itself). The flag is strict: without it the GUI
...
export const $localModelsEnabled = atom<boolean>(
  typeof window !== 'undefined' && window.socisDesktop?.localModelsEnabled === true
)
