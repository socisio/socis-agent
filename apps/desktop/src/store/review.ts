...
import type { SOCISReviewFile, SOCISReviewShipInfo } from '@/global'
...
//
// Scope is always "uncommitted" — SOCIS' flow is agent edits you review BEFORE
...
const OPEN_KEY = 'socis.desktop.reviewOpen'
const COMMIT_DEFAULT_KEY = 'socis.desktop.reviewCommitDefault'
const TREE_MODE_KEY = 'socis.desktop.reviewTreeMode'
const SELECTED_KEY = 'socis.desktop.reviewSelectedPath'
...
export const $reviewFiles = atom<SOCISReviewFile[]>([])
...
export const $reviewShipInfo = atom<SOCISReviewShipInfo>({ ghReady: false, pr: null })
...
type ReviewBridge = NonNullable<NonNullable<NonNullable<Window['socisDesktop']>['git']>['review']>
...
export async function selectReviewFile(file: SOCISReviewFile): Promise<void> {
...
function matchReviewFile(files: readonly SOCISReviewFile[], path: string): SOCISReviewFile | undefined {
...
export async function createOrOpenPr(): Promise<void> {
  ...
  if (existing?.url) {
    void window.socisDesktop?.openExternal?.(existing.url)
  ...
  await runShip(async () => {
    ...
    if (url) {
      void window.socisDesktop?.openExternal?.(url)
    }
