...
import type { SOCISGitBaseBranch, SOCISGitBranch } from '@/global'
import { getSOCISConfig, socisApi, type SOCISGateway } from '@/socis'
...
import type { ProjectInfo, ProjectsPayload } from '@/types/socis'
...
const PROJECT_SCOPE_KEY = 'socis.desktop.projectScope'
...
async function gatewayRequest<T>(method: string, params: Record<string, unknown> = {}): Promise<T> {
  ...
  if (!gateway) {
    throw new Error('SOCIS gateway is not connected')
  }
...
async function activeProjectsContext(): Promise<ActiveProjectsContext> {
  ...
  if (!gateway || gateway !== activeGateway() || profile !== projectProfile()) {
    throw new Error('Active SOCIS profile changed while connecting')
  }
...
async function refreshProjectTreeAcrossProfiles(): Promise<void> {
  ...
  try {
    const res = await socisApi<ProjectTreePayload>({
...
const repoScanStates = new WeakMap<SOCISGateway, RepoScanState>()
const scanningGatewayGenerations = new WeakMap<SOCISGateway, number>()
...
export async function scanAndRecordRepos(force = false): Promise<void> {
  if (isDesktopFsRemoteMode()) {
    ...
    // with `scan: true` — added in #81723) so repos with zero SOCIS
  ...
  try {
    const policy = repoDiscoveryPolicyFromConfig(await getSOCISConfig(context.profile))
...
// repo, returning where SOCIS should start working. Git is the source of
...
export async function listRepoBranches(repoPath: string): Promise<SOCISGitBranch[]> {
  const git = desktopGit()
...
export async function listBaseBranches(repoPath: string): Promise<SOCISGitBaseBranch[]> {
  const git = desktopGit()
...
export async function revealPath(path: null | string): Promise<void> {
  if (path) {
    await window.socisDesktop?.revealPath?.(path)
  }
}
...
export async function copyPath(path: null | string): Promise<void> {
  if (path) {
    await window.socisDesktop?.writeClipboard?.(path)
  }
}
