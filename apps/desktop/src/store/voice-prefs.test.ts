...
vi.mock('@/socis', () => ({
  getSOCISConfigRecord: vi.fn(async () => ({})),
  saveSOCISConfig: vi.fn(async () => undefined)
}))
...
describe('applyVoiceStopPhraseFromConfig', () => {
  ...
  it('uses the first configured phrase so a custom phrase renders correctly', () => {
    applyVoiceStopPhraseFromConfig({ voice: { stop_phrases: ['goodbye socis', 'stop'] } })
    expect($voiceStopPhrase.get()).toBe('goodbye socis')
  })
