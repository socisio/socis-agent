import { LOCAL_CONNECTION_ID } from '@socis/shared'
import { atom, batch, computed } from 'nanostores'

import type { SOCISConnection } from '@/global'
import { getProfiles, socisApi, setApiRequestProfile, STARTUP_REQUEST_TIMEOUT_MS } from '@/socis'
...
import type { ProfileInfo } from '@/types/socis'
...
// /api/profiles/active `current`). "default" is the root ~/.socis-agent. This is the
...
const PROFILE_ORDER_STORAGE_KEY = 'socis.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'socis.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  ...
  try {
    const res = await socisApi<ActiveProfileResponse>({
...
// Persist the choice and relaunch the backend under the new SOCIS_AGENT_HOME. The
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.socisDesktop.profile.set(name)
}
...
async function resolveConnectionForProfile(profile: string): Promise<SOCISConnection | null> {
  const getConnection = window.socisDesktop?.getConnection
...
// previous descriptor.
async function resolveConnectionForAgent(connectionId: string, profile: string): Promise<SOCISConnection | null> {
  const getConnectionFor = window.socisDesktop?.getConnectionFor
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'socis.desktop.showAllProfiles'
...
export function selectProfile(name: string): void {
  ...
  void Promise.all([activateOnCurrentSource(target), shouldRememberStartupProfile])
    .then(([, shouldRemember]) => {
      if (shouldRemember) {
        return window.socisDesktop?.profile?.remember(target)
      }
...
async function isLocalDesktopProfile(target: string): Promise<boolean> {
  const getConnectionConfig = window.socisDesktop?.getConnectionConfig
...
// Switch to the default (root ~/.socis-agent) profile — bound to ⌘1.
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.socisDesktop?.touchBackend?.(target).catch(() => undefined)
}
