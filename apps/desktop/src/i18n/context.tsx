...
import { getSOCISConfigRecord, type SOCISConfigRecord, saveSOCISConfig } from '@/socis'
...
export interface I18nConfigClient {
  getConfig: () => Promise<SOCISConfigRecord>
  saveConfig: (config: SOCISConfigRecord) => Promise<{ ok: boolean }>
}

const defaultConfigClient: I18nConfigClient = {
  getConfig: () => {
    if (typeof window === 'undefined' || !window.socisDesktop?.api) {
      return Promise.resolve({})
    }

    return getSOCISConfigRecord()
  },
  saveConfig: config => {
    if (typeof window === 'undefined' || !window.socisDesktop?.api) {
...
export function getConfigDisplayLanguage(config: SOCISConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: SOCISConfigRecord, locale: Locale): SOCISConfigRecord {
