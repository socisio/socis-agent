 ...
 * `SOCISPlugin`; it never touches the registry directly — it receives a
...
import { pluginRest, type PluginRestOptions, pluginSocket } from '@/socis'
...
export type { PluginRestOptions } from '@/socis'
 ...
 *  under `socis.plugin.<id>.` — plugins can't read or clobber each other. */
 ...
 *  in one attributed namespace instead of the raw `window.socisDesktop`
...
export interface PluginOs {
   ...
   *  the user is away from SOCIS — use `host.notify` for the in-app toast.
   *  Throttled per plugin; reserve it for genuinely notable events.
   *  Supports `icon`, `activate` (e.g. `socis://index-network/intent/1`),
...
function createPluginStorage(pluginId: string): PluginStorage {
  const scoped = (key: string) => `socis.plugin.${pluginId}.${key}`
...
function createPluginOs(pluginId: string): PluginOs {
  const attempt = async (run: (bridge: NonNullable<typeof window.socisDesktop>) => Promise<boolean>) => {
    const bridge = typeof window === 'undefined' ? undefined : window.socisDesktop
  ...
  const attemptPath = async (run: (bridge: NonNullable<typeof window.socisDesktop>) => Promise<null | string>) => {
    const bridge = typeof window === 'undefined' ? undefined : window.socisDesktop
