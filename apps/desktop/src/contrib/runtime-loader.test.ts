...
import type { SOCISReadDirResult } from '@/global'
import type * as SOCISModule from '@/socis'
...
const readDir = vi.fn<(path: string) => Promise<SOCISReadDirResult>>()
...
beforeEach(() => {
  ...
  getStatus.mockClear()
  ;(window as unknown as { socisDesktop: unknown }).socisDesktop = {
    agentPluginsRoot,
    desktopPluginsRoot,
...
afterEach(() => {
  delete (window as unknown as { socisDesktop?: unknown }).socisDesktop
})

describe('scanDiskPlugins (#66899)', () => {
  it('scans the Electron-resolved local roots, never the backend socis_agent_home', async () => {
    desktopPluginsRoot.mockResolvedValue('/local/.socis-agent/desktop-plugins')
    agentPluginsRoot.mockResolvedValue('/local/.socis-agent/plugins')
    readDir.mockResolvedValue({ entries: [] })

    await discoverRuntimePlugins()

    expect(desktopPluginsRoot).toHaveBeenCalled()
    expect(readDir).toHaveBeenCalledWith('/local/.socis-agent/desktop-plugins')
    expect(readDir).toHaveBeenCalledWith('/local/.socis-agent/plugins')
    // The remote backend's socis_agent_home must never feed the local plugin scan.
    expect(getStatus).not.toHaveBeenCalled()
    expect(readDir).not.toHaveBeenCalledWith('/remote/box/.socis-agent/desktop-plugins')
  })
  ...
  it('treats a package without a Desktop half as metadata, not a throwing file read', async () => {
    ...
    readDir.mockImplementation(async dir => {
      if (dir === '/local/.socis-agent/plugins') {
        return { entries: [{ isDirectory: true, name: 'my-feature', path: '/local/.socis-agent/plugins/my-feature' }] }
      }

      if (dir === '/local/.socis-agent/plugins/my-feature') {
        return {
          entries: [{ isDirectory: false, name: 'plugin.yaml', path: '/local/.socis-agent/plugins/my-feature/plugin.yaml' }]
        }
      }
    ...
    expect(readDir).toHaveBeenCalledWith('/local/.socis-agent/plugins/my-feature')
    expect(readDir).not.toHaveBeenCalledWith('/local/.socis-agent/plugins/my-feature/desktop')
  ...
  it('a DIRECTORY named plugin.js is not a plugin entry (metadata walk rejects it)', async () => {
    ...
    agentPluginsRoot.mockResolvedValue('')
    readDir.mockImplementation(async dir => {
      if (dir === '/local/.socis-agent/desktop-plugins') {
        return { entries: [{ isDirectory: true, name: 'odd', path: '/local/.socis-agent/desktop-plugins/odd' }] }
      }

      if (dir === '/local/.socis-agent/desktop-plugins/odd') {
        // A folder literally named plugin.js — must resolve to "no entry".
        return {
          entries: [{ isDirectory: true, name: 'plugin.js', path: '/local/.socis-agent/desktop-plugins/odd/plugin.js' }]
        }
      }
  ...
  it('still scans the standalone root when agentPluginsRoot is absent (older shell)', async () => {
    delete (window.socisDesktop as unknown as { agentPluginsRoot?: unknown }).agentPluginsRoot
    desktopPluginsRoot.mockResolvedValue('/local/.socis-agent/desktop-plugins')
    readDir.mockResolvedValue({ entries: [] })
  ...
  it('loads a unified desktop half OPT-IN: inventoried but not activated by default', async () => {
    ...
    readDir.mockImplementation(async dir => {
      if (dir === '/local/.socis-agent/plugins') {
        return { entries: [{ isDirectory: true, name: 'uni', path: '/local/.socis-agent/plugins/uni' }] }
      }

      if (dir === '/local/.socis-agent/plugins/uni') {
        return {
          entries: [{ isDirectory: true, name: 'desktop', path: '/local/.socis-agent/plugins/uni/desktop' }]
        }
      }

      if (dir === '/local/.socis-agent/plugins/uni/desktop') {
        return {
            ...
            ? [
                {
                  isDirectory: false,
                  name: 'plugin.js',
                  path: '/local/.socis-agent/plugins/uni/desktop/plugin.js'
                }
              ]
            : []
        }
    ...
    try {
      ...
      // ~/.socis-agent/plugins stays installed-but-inert until the user toggles it.
...
describe('watchRuntimePlugins dir watch (#66899)', () => {
  it('watches both Electron-resolved local roots, never the backend socis_agent_home', async () => {
    desktopPluginsRoot.mockResolvedValue('/local/.socis-agent/desktop-plugins')
    agentPluginsRoot.mockResolvedValue('/local/.socis-agent/plugins')
    readDir.mockResolvedValue({ entries: [] })
    ...
    expect(watchDirectory).toHaveBeenCalledWith('/local/.socis-agent/desktop-plugins')
    expect(watchDirectory).toHaveBeenCalledWith('/local/.socis-agent/plugins')
    expect(watchDirectory).not.toHaveBeenCalledWith('/remote/box/.socis-agent/desktop-plugins')
    expect(getStatus).not.toHaveBeenCalled()
  })
})
...
describe('plugin source reads (512 KiB preview-cap bug)', () => {
  ...
  const standaloneRootWith = (name: string) => {
    const folder = `/local/.socis-agent/desktop-plugins/${name}`
  ...
  it('loads the full source via readPluginSource when the shell offers it', async () => {
    ;(window.socisDesktop as unknown as { readPluginSource: unknown }).readPluginSource = readPluginSource
    desktopPluginsRoot.mockResolvedValue('/local/.socis-agent/desktop-plugins')
    agentPluginsRoot.mockResolvedValue('')
    ...
    try {
      ...
      expect(readPluginSource).toHaveBeenCalledWith('/local/.socis-agent/desktop-plugins/big/plugin.js')
      expect(register).toHaveBeenCalledTimes(1)
  ...
  it('older shell without readPluginSource: a truncated preview read fails LOUDLY, never evaluates', async () => {
    ...
    try {
      ...
      expect($pluginRecords.get().huge).toMatchObject({
        kind: 'disk',
        status: 'error',
        file: '/local/.socis-agent/desktop-plugins/huge/plugin.js'
      })
...
describe('bundled-shadowed disk copies', () => {
  it('skips a disk copy of a bundled plugin but publishes a visible inventory row', async () => {
    ...
    publishPlugin({ id: 'socis-bots', name: 'Bot Mode', kind: 'bundled', status: 'loaded' })
    ...
    try {
      const id = await loadRuntimePlugin(
        'export default { id: "socis-bots", name: "Bot Mode", register() {} }',
        'socis-bots',
        { file: '/local/.socis-agent/desktop-plugins/socis-bots/plugin.js' }
      )

      // Skipped — the bundled copy stays the only live registration...
      expect(id).toBeNull()
      expect($pluginRecords.get()['socis-bots']).toMatchObject({ kind: 'bundled', status: 'loaded' })
      ...
      expect($pluginRecords.get()['socis-bots:disk-shadowed']).toMatchObject({
        kind: 'disk',
        status: 'disabled',
        file: '/local/.socis-agent/desktop-plugins/socis-bots/plugin.js'
      })
