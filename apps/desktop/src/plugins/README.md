...
Drop a `<name>/plugin.{ts,tsx}` here that default-exports a `SOCISPlugin` and
...
`$SOCIS_AGENT_HOME/desktop-plugins/<name>/plugin.js` (the disk door) — see the
`socis-desktop-plugins` skill.
