...
import { atom, host } from '@socis/plugin-sdk'
...
const GROUP_CHAT_SYNC_META_KEY = 'socis-bots-groups'
...
/** The v3 envelope stored under the default profile's `socis-bots-groups`
 ...
 *  clients (SOCIS Go, headless backends) see rooms regardless of which
 ...
 *  renamed bot never shows up as its raw profile id or a stale "SOCIS"
 *  (community report, Aug 21 2026: renamed default still read "SOCIS is
 *  thinking…" in group rooms). The untitled primary profile is literally
 *  named "default" — render it as SOCIS (matching displayName and the
 *  @socis handle) so the main agent never loses its name in rooms. */
export function groupSpeakerLabel(name?: null | string) {
  ...
  // Core profile display_name (`socis profile rename …` / dashboard) from
  ...
  return trimmed.toLowerCase() === 'default' ? 'SOCIS' : trimmed
}
