...
import * as sdk from '@socis/plugin-sdk'
import { host } from '@socis/plugin-sdk'
...
// grandfather, or recover. Stored-id pins (ui_meta['socis-bots'].chat) were
 ...
 *  click path's tile-staleness probe (socis-agent#90102), which must
...
async function openStoredBotChat(
  ...
  if (!storedId || typeof host.openSession !== 'function') {
    throw new Error('This SOCIS Agent Desktop version cannot open stored sessions')
  }
  ...
  // even though the session is fine (socis-agent#89617) — clicking Retry
  ...
  // until an app restart (socis-agent#93604). A resume is cheap and
  ...
  if (route && typeof host.requestProfile !== 'function') {
    throw new Error(
      getPluginCtx()?.i18n?.t('bot.remoteConnectionsUnsupported') ??
        'Update SOCIS Agent Desktop to chat with bots on other connections.'
    )
  }
...
export async function ensureBotMetadata(bot: RosterRow): Promise<BotMeta> {
  ...
  const server = row?.ui_meta?.['socis-bots']
