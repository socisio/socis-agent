...
export function displayName(bot: Partial<RosterRow>, meta?: BotMeta | null): string {
  ...
  // "SOCIS". Annotated active rows carry sourceScoped too, and keying this
  ...
  // Core-profile display name (profile.yaml, set via `socis profile rename
  ...
  // never generic "SOCIS" or a hostname-derived label.
  if (alias) {
  ...
  // that reads like nobody bothered. Present it as SOCIS (the agent it is)
