/**
 * Routines: the SOCIS cron jobs scoped to the bot you're chatting with — the
...
// socis -p <bot> chat delegation wrapper so the run reaches that bot's
...
export function routinePrompt(
  ...
  return (
    ...
    `socis -p ${shellQuote(bot)} chat -c ${shellQuote(`Routine: ${title}`)} -q ${shellQuote(`[Scheduled routine] ${instruction}`)}\n\n` +
...
// SOCIS-native schedule string; Advanced exposes it raw.
...
/** Compose the SOCIS schedule string from picker state. */
