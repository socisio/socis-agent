...
function published(room: Room) {
  ...
  return (configure?.params.ui_meta as Record<string, Record<string, unknown>>)?.['socis-bots-groups']
}
...
describe('speaker labels', () => {
  it('the default profile speaks as SOCIS in transcripts, not @default', async () => {
    ...
    expect(line).toBe('SOCIS: hello room')
  ...
  it('honor friendly identity: Bot Mode title, then display_name, never a stale SOCIS', async () => {
    const { chat, rounds } = await loadRoom()
    const data = await import('./data')

    // A renamed default (core display_name via `socis profile rename`) must
    // read as its new name — the community report was "Lucy" still showing
    // "SOCIS is thinking…" in group rooms.
    ...
    // Untitled rows keep today's behavior: default → SOCIS, others verbatim.
    data.$botMeta.set({})

    expect(chat.groupSpeakerLabel('default')).toBe('SOCIS')
...
describe('sync worker', () => {
  it('retries a gateway CAS conflict and publishes the merged room', async () => {
    ...
    await drain(() => (room.gateway.uiMetaRevisions['socis-bots-groups'] || 0) < 2, 60)

    const stored = room.gateway.uiMeta['socis-bots-groups'] as {
      rooms: Record<string, { log: GroupMessage[] }>
    }

    expect(room.gateway.uiMetaRevisions['socis-bots-groups']).toBe(2)
  ...
  it('fans a room write out to every reachable default-profile gateway', async () => {
    ...
    host.requestProfile = async (route: { connectionId: string }, method: string) => {
      ...
      if (method === 'profiles.configure') {
        return { applied: { ui_meta: true, ui_meta_revisions: { 'socis-bots-groups': 1 } } }
      }
