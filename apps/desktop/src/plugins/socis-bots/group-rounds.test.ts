...
describe('routing', () => {
  ...
  it('resolves @socis to the default member', async () => {
    const { rounds } = await loadRoom()
    ...
    const parsed = rounds.parseGroupChatMentions('@socis take a look', members)
...
describe('turn prompt', () => {
  it('addresses the default profile as @socis', async () => {
    const { rounds } = await loadRoom()
    ...
    expect(own).toMatch(/You are @socis,/)
    ...
    expect(peer).toMatch(/group chat with @socis/)
  })
