...
import { forgetSessionUnread, host, queryClient } from '@socis/plugin-sdk'
...
export function mergeServerMeta(roster: RosterRow[], fetchedAt = 0) {
  ...
  for (const bot of roster) {
    const server = bot.ui_meta?.['socis-bots']
...
/** Permanently delete a bot's SOCIS profile, then remove plugin-local state
 ...
 * directory (socis-agent#52279). That is the "can't delete a bot" error. */
