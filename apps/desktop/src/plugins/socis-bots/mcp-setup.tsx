...
import { Button, host, Input, useI18n } from '@socis/plugin-sdk'
...
// and the row falls back to the "run socis mcp / Settings" hint. profile is
...
export function McpSetupButton({ profile, entry, onDone, ensureProfile }: McpSetupButtonProps) {
  ...
  const beginOAuth = async () => {
    ...
    const mcpOauthBridge =
      typeof window !== 'undefined' && window.socisDesktop && window.socisDesktop.mcpOauth
        ? window.socisDesktop.mcpOauth
        : null
    ...
    try {
      ...
      } else if (typeof window !== 'undefined' && window.socisDesktop && window.socisDesktop.openExternal) {
        window.socisDesktop.openExternal(authUrl)
      } else {
