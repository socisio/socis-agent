...
vi.mock('@/socis', () => ({ deleteProfile: vi.fn(), getLogs: vi.fn(), getStatus: vi.fn(), socisApi: vi.fn() }))
...
const { deleteProfile, socisApi } = await import('@/socis')
...
describe('connection-aware plugin host APIs', () => {
  it('retires a profile gateway before deleting it', async () => {
    ...
    // profile's backend, re-creating its SOCIS_AGENT_HOME (#94235).
  ...
  it('refreshes the profile inventory before asking Electron for routes', async () => {
    ...
    ;(window as unknown as { socisDesktop: unknown }).socisDesktop = { getProfileRoutes }
  ...
  it('reads and hides persisted sessions through the source primary without activating the profile', async () => {
    ...
    expect(socisApi).toHaveBeenNthCalledWith(
      1,
    ...
    expect(socisApi).toHaveBeenNthCalledWith(2, {
      connectionId: 'source-a',
      path: '/api/sessions/bot-chat',
      method: 'PATCH',
      body: { hidden: true, profile: 'backend-worker' }
    })
    expect(vi.mocked(socisApi).mock.calls.every(([request]) => !('profile' in request))).toBe(true)
