/**
 * @socis/plugin-sdk — THE plugin language. The vscode-module model: plugin
 ...
 *    object as `window.__SOCIS_AGENT_PLUGIN_SDK__` and maps the import to it, so a
...
import { deleteProfile, getLogs, getStatus, socisApi, type SOCISGateway } from '@/socis'
import {
  $gateway,
...
import type { PaginatedSessions, UsageStats } from '@/types/socis'
...
export interface PluginProfileRoute {
  ...
  profile: string
  /** Backend SOCIS profile served by that route. */
...
async function requestPluginProfile<T>(
  ...
  const getAgentRoster = window.socisDesktop?.getAgentRoster
...
async function pluginRouteStillRegistered(route: PluginProfileRoute): Promise<boolean> {
  const getProfileRoutes = window.socisDesktop?.getProfileRoutes
...
//
// ensureGatewayProfile awaits the store's dial, and SOCISGateway.connect() has
...
export const host = {
  ...
  deleteProfile: async (profile: string | PluginProfileRoute): Promise<void> => {
    ...
    // restores on relaunch and re-creates the deleted profile (socis-agent#94235).
    dropTilesForProfile(
  ...
  connections: async () => {
    const bridge = window.socisDesktop?.connections

    if (!bridge) {
      throw new Error('This Desktop build has no connection registry. Update SOCIS Agent Desktop.')
    }
  ...
  agents: async () => {
    const roster = window.socisDesktop?.getAgentRoster

    if (!roster) {
      throw new Error('This Desktop build cannot enumerate multi-source agents. Update SOCIS Agent Desktop.')
    }
  ...
  newChat: (profile?: null | string | PluginProfileRoute, options: PluginNewChatOptions = {}): void => {
    if (options.workspaceMode === 'bots') {
      ...
      if (!openTab) {
        notify({ kind: 'error', message: 'Update SOCIS Agent Desktop to open another Bot chat.' })
   ...
   *
   *  `isStaleTile` (socis-agent#90102): the caller's reconciliation probe
  ...
  profileRoutes: async () => {
    const desktop = window.socisDesktop
    const getProfileRoutes = desktop?.getProfileRoutes

    if (!getProfileRoutes) {
      throw new Error('SOCIS Agent Desktop connection routing unavailable')
    }
  ...
  listPersistedSessions: async (
    ...
    return socisApi<PaginatedSessions>({
  ...
  setPersistedSessionHidden: async (
    ...
    return socisApi<{ ok: boolean; hidden: boolean }>({
      ...(route ? { connectionId: route.connectionId } : {}),
  ...
  request: async <T>(method: string, params: Record<string, unknown> = {}): Promise<T> => {
    ...
    if (!gateway) {
      throw new Error('SOCIS gateway unavailable')
    }
 ...
 *  "SOCIS AGENT" with it, a `chat.empty` contribution writes its own name. */
...
export type { SOCISGateway } from '@/socis'
...
export type { SOCISOpenTarget } from '@/lib/socis-open-target'
...
/** SOCIS' reasoning levels + their compact labels, so a plugin surfacing a
...
export type { RpcEvent, StatusResponse } from '@/types/socis'
