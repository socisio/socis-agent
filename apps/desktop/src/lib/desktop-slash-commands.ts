...
/** Why a known SOCIS command has no desktop UI surface. */
...
const DESKTOP_COMMAND_SPECS: readonly DesktopCommandSpec[] = [
  ...
  { name: '/profile', description: 'Switch the active SOCIS profile', surface: action('profile') },
  {
    name: '/skin',
...
function isKnownSOCISSlashCommand(command: string): boolean {
  const normalized = normalizeCommand(command)
 ...
 * SOCIS' built-in slash commands — i.e. skill commands (`/gif-search`,
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownSOCISSlashCommand(normalized)
}
 ...
 * activity — the ones that ship with SOCIS and were never asked for. It is
