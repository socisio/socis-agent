import { socisApi } from '@/api/client'
import type {
  SOCISConnection,
  SOCISReadDirResult,
  SOCISReadFileTextResult,
  SOCISSelectPathsOptions
} from '@/global'
...
function connectionCacheKey(connection: SOCISConnection | null) {
  if (!connection) {
    return 'local:'
  }
...
export function desktopFsCacheKey(connection: SOCISConnection | null = $connection.get()) {
...
export async function readDesktopDir(path: string): Promise<SOCISReadDirResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readDir(path)
  }

  return remoteFsApi<SOCISReadDirResult>(fsPath('list', path))
}

export async function readDesktopFileText(path: string): Promise<SOCISReadFileTextResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readFileText(path)
  }

  return remoteFsApi<SOCISReadFileTextResult>(fsPath('read-text', path))
}
...
export async function readDesktopFileDataUrlLocalFirst(path: string): Promise<string> {
  try {
    const local = await window.socisDesktop?.readFileDataUrl?.(path)
...
export async function selectDesktopPaths(options?: SOCISSelectPathsOptions): Promise<string[]> {
  const desktop = bridge()
