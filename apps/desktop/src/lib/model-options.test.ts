...
import { getGlobalModelOptions } from '@/socis'
...
const globalOptions = { model: 'socis-4', provider: 'nous', providers: [] }
...
describe('requestModelOptions', () => {
  ...
  it('recovers an empty gateway catalog through profile-scoped REST without replacing the session selection', async () => {
    const gatewayPayload = { model: 'socis-local', provider: 'socis-local' }

    const restPayload = {
      model: 'profile-default',
      provider: 'openai-codex',
      providers: [{ models: ['socis-local'], name: 'SOCIS Local vLLM', slug: 'socis-local' }]
    }
  ...
  it('keeps the gateway result when both catalog paths have no selectable models', async () => {
    const gatewayPayload = { model: 'socis-local', provider: 'socis-local', providers: [] }
  ...
  it('keeps an empty owner-routed catalog instead of replacing it from ambient REST', async () => {
    const ownerPayload = { model: 'berry-local', provider: 'socis-local', providers: [] }
...
describe('manualPickRemoved', () => {
  ...
  it('never clobbers when the provider has an empty model list (re-auth)', () => {
    expect(manualPickRemoved(providers, 'nous', 'socis-4')).toBe(false)
  })
