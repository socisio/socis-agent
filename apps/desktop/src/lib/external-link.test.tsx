...
function installDesktopBridge(partial: Partial<Window['socisDesktop']> = {}) {
...
function installTitleBridge(title: string) {
  ...
  installDesktopBridge({ fetchLinkTitle: bridge as unknown as Window['socisDesktop']['fetchLinkTitle'] })
...
describe('external link helpers', () => {
  ...
  it('opens a web link in the in-app browser', async () => {
    ...
    installDesktopBridge({ openExternal: openExternal as unknown as Window['socisDesktop']['openExternal'] })
  ...
  it('prefixes a pretty link to a known host with its brand glyph', () => {
    ...
    const url = 'https://github.com/socisio/socis-agent/pull/123'
