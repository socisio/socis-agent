...
//
// When someone is in a hands-free "Hey SOCIS" voice chat, the natural way to
...
// phrase (optionally addressed to SOCIS), so a real turn that merely contains
...
// Optional address prefixes so "socis stop" / "ok stop" / "hey socis, stop"
// still count. Stripped before matching the core phrase.
const ADDRESS_PREFIXES: readonly string[] = ['hey socis', 'hey socis,', 'socis', 'socis,', 'ok', 'okay', 'hey']
...
function stripAddress(text: string): string {
  for (const prefix of ADDRESS_PREFIXES) {
    if (text === prefix) {
      // Bare address ("socis") is not a stop command on its own.
      continue
    }
 ...
 * to SOCIS). Returns false for anything that merely contains "stop" as part of
