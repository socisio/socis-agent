...
describe('performScopedFind', () => {
  ...
  it('does not re-wrap when the query differs only in case (fast path, triage #81778)', () => {
    ...
    const surface = plantSurface('surface', '<p>SOCIS SOCIS</p>')
    performScopedFind(surface, 'socis', { forward: true, findNext: false })

    const before = [...surface.querySelectorAll('mark.find-hit')]
    const result = performScopedFind(surface, 'socis', { forward: true, findNext: true })
  ...
  it('re-wraps when a stale mark from an earlier query survives (fast path, #81778 review)', () => {
    ...
    const surface = plantSurface('surface', '<p>socis</p>')
    performScopedFind(surface, 'socis', { forward: true, findNext: false })
    ...
    surface.insertAdjacentHTML('beforeend', '<mark class="find-hit">SOCIS</mark>')
    surface.insertAdjacentHTML('beforeend', '<p>socis</p>')
    ...
    expect(marks.every(mark => mark.textContent.toLowerCase() === 'socis')).toBe(true)
  })
