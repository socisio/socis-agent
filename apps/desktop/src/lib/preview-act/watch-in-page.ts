...
export function watchInPage(doc: Document, holder: WatchHolder, stage: WatchStage, label?: string): void {
  const win = doc.defaultView as (Window & { __socisWatch?: Record<string, unknown> }) | null
  ...
  const busy = 25
  const state = win.__socisWatch || (win.__socisWatch = {})
  ...
  const build = () => {
    ...
    const stale = doc.querySelectorAll('socis-watch')

    for (let i = 0; i < stale.length; i++) {
      stale[i].remove()
    }

    const host = doc.createElement('socis-watch')
  ...
  // this up" in dev.
  const stamp = (win as unknown as { __socisWatchTag?: number }).__socisWatchTag
