...
// SOCIS is rendering, and when the two are opposite schemes it takes over
...
function renderedMode(doc: Document): RenderedMode {
  const attr = doc.documentElement.dataset.socisMode
 ...
 * the only carriers of SOCIS' palette — are dropped so the paste target's
