...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when SOCIS doesn't know how to remove it. */
...
export interface EnvVarInfo {
  ...
  // in socis_cli/provider_catalog.py). When present, the Keys tab groups by
  // this provider identity — the SAME one `socis model` uses — instead of
...
export type SOCISConfigRecord = Record<string, unknown>
...
export interface ModelOptionProvider {
   ...
   *  the `socis model` CLI / onboarding OAuth flow. */
  auth_type?: string
...
// non-desktop) SOCIS simply carries none of it. See store/profile-share.ts.
export interface ProfileDesktopOverlay {
  ...
  /** Layout tree (socis.desktop.layoutTree.v2 shape). */
  layoutTree?: unknown
...
// folders. Mirrors socis_cli/projects_db.Project.to_dict().
...
export interface SkillInfo {
  ...
  usage?: number
  /** 'agent' = learned/local (editable), 'bundled' = ships with SOCIS, 'hub' = installed. */
  provenance?: 'agent' | 'bundled' | 'hub'
}

/** One entry of the built-in optional-skills catalog (optional-skills/ in the
 *  repo) — official skills that ship with SOCIS but install on demand. */
...
export interface ToolProvider {
   ...
   *  CLI `socis tools` active-provider detection). */
  is_active: boolean
 ...
 *  not SOCIS); elsewhere, driver health from `cua-driver doctor`. `null`
...
export interface ActionStatusResponse {
  ...
  pid: number | null
  running: boolean
  /** socis-update only: durable completion identity recovered from update.log. */
  action_id?: string
  /** socis-update only: summary of the durable update receipt (#91277 bullet 3) —
...
/** Shape of `GET /api/socis/update/check` — the backend's own update state.
