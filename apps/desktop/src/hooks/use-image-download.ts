...
function isMissingIpcHandler(error: unknown): boolean {
  ...
  return message.includes("No handler registered for 'socis:saveImageFromUrl'")
}
...
export function useImageDownload(src?: string) {
  ...
  const download = useCallback(async () => {
    ...
    try {
      if (window.socisDesktop?.saveImageFromUrl) {
        if (await window.socisDesktop.saveImageFromUrl(src)) {
