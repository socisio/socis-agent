...
How to build SOCIS Agent Desktop well. This is a judgment guide, not an inventory —
...
- The **backend** is authoritative for anything another SOCIS surface can also
...
- A **runtime home change** (switching the underlying `SOCIS_AGENT_HOME` profile) is
...
its shape. "Plugin" means several unrelated things across SOCIS — do not assume
