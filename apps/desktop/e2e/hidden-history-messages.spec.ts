...
async function setupSeededMockBackend(): Promise<MockBackendFixture> {
  ...
  fs.appendFileSync(
    path.join(sandbox.socisHome, 'config.yaml'),
    '\ncompression:\n  threshold_tokens: 1\n',
    'utf8',
  )
  writeEnvFile(sandbox.socisHome)
  const builder = await RealSessionBuilder.start(sandbox.socisHome)
...
test('live verify-on-stop continuations stay out of the transcript', async ({}, testInfo) => {
  ...
  writeMockProviderConfig(sandbox.socisHome, mock.url)
  fs.appendFileSync(path.join(sandbox.socisHome, 'config.yaml'), '\nagent:\n  verify_on_stop: true\n', 'utf8')
