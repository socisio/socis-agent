 ...
 * second gateway, "Homelab", is a REAL second `socis serve` this spec spawns
 * with its own SOCIS_AGENT_HOME, profiles and session token, registered in the v2
...
function findSOCISBinary(): string {
  const venv = path.join(REPO_ROOT, '.venv', 'bin', 'socis')

  if (fs.existsSync(venv)) {
    return venv
  }

  const result = spawnSync('which', ['socis'], { encoding: 'utf8' })

  if (result.status === 0 && result.stdout.trim()) {
    return result.stdout.trim()
  }

  throw new Error('socis binary not found: create the repo venv (uv sync) or put socis on PATH')
}
...
/**
 * Spawn a second, fully real `socis serve` as the remote gateway. Its
 * session token is pinned through SOCIS_AGENT_DASHBOARD_SESSION_TOKEN so the
...
async function startRemoteGateway(root: string, mockUrl: string, profiles: string[]): Promise<RemoteGateway> {
  ...
  const child: ChildProcess = spawn(
    ...
    ['serve', '--host', '127.0.0.1', '--port', String(port), '--skip-build'],
    {
      ...
      env: {
        ...
        SOCIS_AGENT_DASHBOARD_SESSION_TOKEN: REMOTE_TOKEN,
      },
  ...
  while (Date.now() < deadline) {
    if (child.exitCode !== null) {
      throw new Error(`remote socis serve exited early (${child.exitCode}):\n${log}`)
    }

    try {
      const response = await fetch(`${url}/api/status`, {
        headers: { 'X-SOCIS-Session-Token': REMOTE_TOKEN },
      })
  ...
  if (Date.now() >= deadline) {
    throw new Error(`remote socis serve never became ready:\n${log}`)
  }
...
test.describe('fleet profile rail — two registered gateways', () => {
  ...
  test.beforeAll(async () => {
    ...
    writeMockProviderConfig(sandbox.socisHome, mock.url)
    ...
    seedProfiles(sandbox.socisHome, ['research'])
