...
function createGitRepo(root: string): string {
  ...
  execFileSync('git', ['config', 'user.name', 'SOCIS E2E'], { cwd: repo })
...
function configureRepoCwd(socisHome: string, mockUrl: string, repo: string): void {
  writeMockProviderConfig(socisHome, mockUrl)
  fs.appendFileSync(path.join(socisHome, 'config.yaml'), `\nterminal:\n  cwd: ${repo}\n`, 'utf8')
  writeEnvFile(socisHome)
}
...
test.beforeAll(async () => {
  ...
  configureRepoCwd(sandbox.socisHome, mock.url, repo)
