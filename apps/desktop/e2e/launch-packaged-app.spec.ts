...
/**
 * E2E smoke tests for the packaged SOCIS desktop app.
 *
 ...
 * (credential stripping, isolated SOCIS_AGENT_HOME + userData, unique app name).
 *
...
test('window opens with the SOCIS title', async () => {
...
test('HUD composer remains fully inside the transparent window', async () => {
  ...
  await fixture!.page.evaluate(() =>
    (window as typeof window & {
      socisDesktop?: { hud?: { open: (options: { sessionId: null }) => Promise<void> } }
    }).socisDesktop?.hud?.open({ sessionId: null })
  )
