/**
 * Shared E2E fixtures for the SOCIS desktop Playwright suite.
 *
 * Two fixture modes:
 *
 ...
 *     (electron → socis serve → provider → inference → renderer) is
 ...
 * `electron-builder --dir` step and matches `socis desktop --source`. The
...
export function createSandbox(prefix: string): Sandbox {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `socis-e2e-${prefix}-${Math.random()}`))
  const socisHome = path.join(root, 'socis-home')
  const userDataDir = path.join(root, 'electron-user-data')

  fs.mkdirSync(socisHome, { recursive: true })
...
export function writeMockProviderConfig(
  ...
  const configPath = path.join(socisHome, 'config.yaml')
 ...
 */
export function writeEnvFile(socisHome: string, apiKey = 'e2e-mock-key'): void {
  const envPath = path.join(socisHome, '.env')
 ...
 */
function writeEmptyConfig(socisHome: string): void {
  const configPath = path.join(socisHome, 'config.yaml')
 ...
 *
 * Key env vars:
 *  - SOCIS_AGENT_HOME → sandbox socis-home (isolated config/sessions)
 *  - SOCIS_AGENT_DESKTOP_USER_DATA_DIR → sandbox electron-user-data
 *  - SOCIS_AGENT_DESKTOP_IGNORE_EXISTING=1 → don't pick up `socis` from PATH
 *    (we want the dev checkout at REPO_ROOT)
 *  - SOCIS_AGENT_DESKTOP_SOCIS_AGENT_ROOT → REPO_ROOT (dev checkout resolution)
 *  - SOCIS_AGENT_DESKTOP_APP_NAME → unique-ish per test (avoids single-instance lock)
...
export function buildAppEnv(sandbox: Sandbox, extra: Record<string, string> = {}): Record<string, string> {
  ...
  return {
    ...clean,
    SOCIS_AGENT_HOME: sandbox.socisHome,
    SOCIS_AGENT_DESKTOP_USER_DATA_DIR: sandbox.userDataDir,
    SOCIS_AGENT_DESKTOP_IGNORE_EXISTING: '1',
    SOCIS_AGENT_DESKTOP_SOCIS_AGENT_ROOT: REPO_ROOT,
    SOCIS_AGENT_DESKTOP_APP_NAME: `SOCISE2E-${Date.now()}`,
    ...
    SOCIS_AGENT_DESKTOP_SKIP_QUIT_CONFIRM: '1',
 ...
 *
 * @param sandbox  - isolated SOCIS_AGENT_HOME + userData
 * @param env      - the process environment (already has SOCIS_AGENT_HOME etc.)
...
export async function setupMockBackend(options: MockBackendOptions = {}): Promise<MockBackendFixture> {
  ...
  writeEnvFile(sandbox.socisHome)
...
export async function setupNoProvider(): Promise<NoProviderFixture> {
  ...
  writeEmptyConfig(sandbox.socisHome)
...
export interface DeadBackendOptions {
  /**
   * When true, inject a fake boot error via SOCIS_AGENT_DESKTOP_BOOT_FAKE_ERROR
 ...
 * nothing listens on). By default the backend still boots (`socis serve`
...
export async function setupDeadBackend(options: DeadBackendOptions = {}): Promise<DeadBackendFixture> {
  ...
  const configPath = path.join(sandbox.socisHome, 'config.yaml')
  fs.writeFileSync(
    configPath,
...
`,
  ...
  const env = buildAppEnv(sandbox, options.fakeError ? { SOCIS_AGENT_DESKTOP_BOOT_FAKE_ERROR: 'Failed to connect to SOCIS backend: connection refused' } : {})
  const { app, page } = await launchDesktop(env)
...
function resolvePackagedBinaryPath(): string {
  if (process.platform === 'win32') {
    return path.join(RELEASE_ROOT, 'win-unpacked', 'SOCIS.exe')
  }

  if (process.platform === 'darwin') {
    const arch = process.arch === 'arm64' ? 'arm64' : 'x64'

    return path.join(RELEASE_ROOT, `mac-${arch}`, 'SOCIS.app', 'Contents', 'MacOS', 'SOCIS')
  }

  return path.join(RELEASE_ROOT, 'linux-unpacked', 'socis')
}
 ...
 * progress without spawning a real SOCIS backend.
 *
 * Uses the same sandbox isolation (credential stripping, isolated
 * SOCIS_AGENT_HOME + userData, unique app name) as the dev-mode fixtures.
 *
...
export async function setupPackagedApp(): Promise<PackagedAppFixture> {
  ...
  const env = buildAppEnv(sandbox, {
    ...
    SOCIS_AGENT_DESKTOP_BOOT_FAKE: '1',
    SOCIS_AGENT_DESKTOP_BOOT_FAKE_STEP_MS: '120',
  })

  // Clear dev-server + socis-root overrides — the packaged binary
  // should use its own bundled renderer, not the dev checkout.
  delete (env as Record<string, string | undefined>).SOCIS_AGENT_DESKTOP_DEV_SERVER
  delete (env as Record<string, string | undefined>).SOCIS_AGENT_DESKTOP_SOCIS
  delete (env as Record<string, string | undefined>).SOCIS_AGENT_DESKTOP_SOCIS_AGENT_ROOT
