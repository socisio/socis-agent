 ...
 * AIAgent loop, using the E2E mock provider configured in `socisHome`.
 *
...
export class RealSessionBuilder {
  ...
  private constructor(socisHome: string) {
  ...
  static async start(socisHome: string): Promise<RealSessionBuilder> {
    const builder = new RealSessionBuilder(socisHome)
