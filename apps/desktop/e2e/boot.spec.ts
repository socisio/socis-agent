 ...
 * packaged binary) with a real `socis serve` backend pointed at a mock
 * inference server. The full chain is exercised:
 *
 *   electron → socis serve (python) → mock provider → renderer
 *
...
test.describe('dev-mode boot with mock backend', () => {
  test('window opens with SOCIS title', async () => {
    const title = await fixture!.page.title()
    expect(title).toContain('SOCIS')
  })
  ...
  test('the preload bridge reaches the renderer', async () => {
    const bridge = await fixture!.page.evaluate(() => {
      const desktop = (window as unknown as { socisDesktop?: Record<string, unknown> }).socisDesktop
  ...
  test('backend boots and app becomes ready', async () => {
    ...
    // electron starts → socis serve is spawned → WS connects → config
