 ...
 * dir (macOS `~/Library/Application Support/SOCIS/connection.json`, Windows
 * `AppData\Roaming\SOCIS\connection.json`). Anything that can read the file
 ...
 *      or in SOCIS_AGENT_HOME (logs included).
 ...
 * `X-SOCIS-Session-Token` header it receives — a dropped or mangled token
 ...
 * gateway token: `51c68d4ab` ("Add SOCIS desktop app (#20059)"), the commit
 ...
 */
const SENTINEL_TOKEN = 'socis-e2e-at-rest-sentinel-Zq7Z4hV9nX2pL8sK3tB6wR1yM5jD0fG'
 ...
 */
const STABLE_APP_NAME = 'SOCISE2EAtRestStorage'

// ─── Fake gateway ───────────────────────────────────────────────────────

interface FakeGateway {
  url: string
  /** Every `X-SOCIS-Session-Token` value the app has sent us. */
  sessionTokens: string[]
  close: () => Promise<void>
}

/**
 * A minimal stand-in for a remote SOCIS gateway. It serves the public
...
async function startFakeGateway(): Promise<FakeGateway> {
  ...
  const server = http.createServer((req, res) => {
    const token = req.headers['x-socis-session-token']
 ...
 * backend, so we skip spawning `socis serve` (no Python needed, ~3s launch,
 ...
 * this state. BOOT_FAKE_ERROR short-circuits startSOCIS() *before* remote
...
async function launchAgainst(sandbox: Sandbox): Promise<{ app: ElectronApplication; page: Page }> {
  const env = buildAppEnv(sandbox, {
    SOCIS_AGENT_DESKTOP_APP_NAME: STABLE_APP_NAME,
    SOCIS_AGENT_DESKTOP_BOOT_FAKE_ERROR: 'E2E at-rest storage spec: local backend intentionally not started',
  })
  ...
  await page.waitForFunction(
    () => Boolean((window as unknown as { socisDesktop?: Record<string, unknown> }).socisDesktop?.saveConnectionConfig),
    undefined,
    { timeout: 60_000 },
  )
 ...
 * posture SOCIS should take there (refuse to save vs. accept basic_text) is
...
async function saveRemoteToken(page: Page, remoteUrl: string, remoteToken?: string): Promise<SaveOutcome> {
  return page.evaluate(
    async ([url, token]) => {
      const desktop = (window as unknown as { socisDesktop: any }).socisDesktop
...
test.describe('remote gateway session token at rest', () => {
  test('with keychain encryption opted IN, a newly configured token is never written to userData in plaintext, and still works after restart', async () => {
    ...
    // SOCIS_AGENT_HOME (desktop.log lives there).
    expect(
      scanTreeForSecret(userDataDir, needles),
      'the gateway session token leaked into a userData file',
    ).toEqual([])
    expect(
      scanTreeForSecret(sandbox.socisHome, needles),
      'the gateway session token leaked into a SOCIS_AGENT_HOME file (logs included)',
    ).toEqual([])
