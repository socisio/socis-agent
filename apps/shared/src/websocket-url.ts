...
export async function resolveGatewayWsUrl(deps: ResolveGatewayWsUrlDeps, conn: GatewayWsConnection): Promise<string> {
  ...
  if (conn.authMode === 'oauth') {
    if (!mint) {
      throw new Error('This Desktop build cannot refresh OAuth WebSocket tickets. Update SOCIS Agent Desktop and try again.')
    }
...
export interface SOCISWebSocketUrlOptions {
...
export function buildSOCISWebSocketUrl(options: SOCISWebSocketUrlOptions): string {
