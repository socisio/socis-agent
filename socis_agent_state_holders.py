...
an unlinked SQLite DB/WAL/SHM generation.  ``socis_agent_state`` supplies only the
...
_SOCIS_AGENT_EXECUTABLES = frozenset({"socis", "socis-agent", "socis-acp"})
_SOCIS_AGENT_PYTHON_MODULES = frozenset({"acp_adapter", "socis_cli.main"})
_SOCIS_AGENT_PYTHON_SCRIPTS = frozenset({"socis_cli/main.py", "run_agent.py"})
...
def _looks_like_socis(argv: Sequence[str]) -> bool:
    """Return whether argv identifies a supported SOCIS execution target."""
    if not argv:
        return False
    program = os.path.basename(argv[0]).lower().removesuffix(".exe")
    if program in _SOCIS_AGENT_EXECUTABLES:
        return True
    ...
    if kind == "module":
        return normalized in _SOCIS_AGENT_PYTHON_MODULES
    return any(
        normalized == script or normalized.endswith(f"/{script}")
        for script in _SOCIS_AGENT_PYTHON_SCRIPTS
    )
        ...
        try:
            ...
            for pid_str in os.listdir("/proc"):
                ...
                try:
                    ...
                    if argv is not None and _looks_like_socis(argv):
                        cmdline = " ".join(argv)
