"""Shell completion script generation for socis CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_socis_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# SOCIS Agent bash completion
# Add to ~/.bashrc:
#   eval "$(socis completion bash)"

_socis_profiles() {{
    local profiles_dir="$HOME/.socis-agent/profiles"
...
_socis_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_socis_profiles)" -- "$cur"))
        return
    fi
...
complete -F _socis_completion socis
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef socis
# SOCIS Agent zsh completion
# Add to ~/.zshrc:
...
_socis_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.socis-agent/profiles" ]]; then
        profiles+=($HOME/.socis-agent/profiles/*(N/:t))
    fi
...
_socis() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_socis_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'socis command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# SOCIS Agent fish completion",
        "# Add to your config:",
        "#   socis completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __socis_profiles",
        "    echo default",
        "    if test -d $HOME/.socis-agent/profiles",
        "        for d in $HOME/.socis-agent/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c socis -f -s p -l profile"
        " -d 'Profile name' -xa '(__socis_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__socis_profiles)' -d 'Profile name'"
                )
