...
from socis_agent_constants import agent_browser_runnable, find_node_executable
from tools.environments.local import socis_subprocess_env

_IS_WINDOWS = platform.system() == "Windows"

_DEP_CHECKS = {
    # find_node_executable() rather than a bare which(): $SOCIS_AGENT_HOME/node is
...
def _has_socis_agent_browser() -> bool:
    ...
    # install.sh installs globally into $SOCIS_AGENT_HOME/node/bin/ via npm -g --prefix
...
def ensure_dependency(
    ...
    if shell == "powershell":
        ...
        cmd = [
            ...
            "-File", str(script),
            "-Ensure", dep,
            "-SOCISHome", str(get_socis_agent_home()),
        ]
    else:
        cmd = ["bash", str(script), "--ensure", dep]

    run_env = socis_subprocess_env(inherit_credentials=False)
