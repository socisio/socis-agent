"""`socis checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.socis-agent/checkpoints/``.  Actions:

    socis checkpoints               # same as `status`
    socis checkpoints status        # total size, project count, breakdown
    socis checkpoints list          # per-project checkpoint counts + workdir
    socis checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    socis checkpoints clear [-f]    # nuke the entire base (asks first)
    socis checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    socis checkpoints
    socis checkpoints prune --retention-days 3 --max-size-mb 200
    socis checkpoints clear -f
...
from socis_cli.sizefmt import format_bytes as _fmt_bytes
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: socis checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``socis checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `socis checkpoints` → status
