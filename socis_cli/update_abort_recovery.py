...
``socis update`` performs its fleet restart in the interpreter that started
...
itself lives in :mod:`socis_cli.update_restart_recovery`.
"""
...
def _serve_unit_recovery_available() -> bool:
    """Can a fresh process restart ``socis-serve*`` units on this host?"""
...
def _qualified_serve_skips(skip_units) -> list[dict]:
    ...
    ``socis-serve.service`` can exist in BOTH the user and the system
...
def _recover_gateway_restart_after_abort(
    ...
    *,
    ...
    ``socis update`` normally performs the fleet restart in the interpreter
    ...
    The same child additionally restarts active ``socis-serve*`` systemd
    units (#92145).  ``socis serve`` hosts ``tui_gateway.server`` and is
    ...
    """
    from socis_cli.update_cmd import _gateway_recovery_partition
    ...
    command = [
        sys.executable,
        "-m",
        "socis_cli.update_restart_recovery",
        "--stdin",
    ]
    env = os.environ.copy()
    env["SOCIS_AGENT_UPDATE_RESTART_RECOVERY"] = "1"
    for marker in ("_SOCIS_AGENT_GATEWAY", "SOCIS_AGENT_GATEWAY", "SOCIS_AGENT_GATEWAY_MODE"):
        env.pop(marker, None)
...
def _warn_stale_serve_runtimes(rows) -> None:
    ...
    terminal naming the responsible process. ``socis serve`` hosts
    ...
    print(
        "    Restart them before using SOCIS again, e.g."
        " `systemctl --user restart socis-serve.service`"
        " or by relaunching `socis serve` / the Desktop app."
    )
...
def _abort_recovery_is_complete(
    *,
    ...
    ``gateway_state.json`` / control socket — so a ``socis serve`` still
    ...
    * no ``socis-serve*`` unit failed to produce a verified replacement; and
