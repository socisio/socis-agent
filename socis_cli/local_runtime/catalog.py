...
_CATALOG_URL = ("https://raw.githubusercontent.com/socisio/socis-agent"
                "/main/socis_cli/local_runtime/catalog.json")
_SCHEMA_VERSION = 1
...
def _packaged_catalog() -> "tuple[CatalogEntry, ...]":
    ...
    raw = files("socis_cli.local_runtime").joinpath("catalog.json").read_text(
        encoding="utf-8")
...
def refresh_catalog(force: bool = False) -> bool:
    ...
    try:
        req = urllib.request.Request(
            _CATALOG_URL, headers={"User-Agent": "socis-local-runtime"})
