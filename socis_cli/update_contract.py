...
``socis update`` mutation (CLI apply, CLI --check, dashboard update
endpoint). The decision layers:

1. **Baked provenance marker** (``/etc/socis/image-provenance.json``,
   written by the image build — see :mod:`socis_cli.image_provenance`):
