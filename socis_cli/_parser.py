"""
Top-level argparse construction for the socis CLI.
...
# argparse runs (it sets ``SOCIS_AGENT_HOME`` and strips itself from ``sys.argv``),
...
# tests/socis_cli/test_top_level_value_flags_parity.py fails CI if the parser
...
def top_level_value_flag_sets() -> tuple[frozenset[str], frozenset[str]]:
    ...
    the exact drift that made ``socis --reasoning high chat …`` misread
...
def _inherited_flag(parser, *args, **kwargs):
    """Register a flag that ``socis_cli.relaunch`` should carry over when
...
_EPILOGUE = """
Examples:
    socis                        Start interactive chat
    socis chat -q "Hello"        Single query mode
    socis --tui                  Launch the modern TUI (or set display.interface: tui)
    socis --cli                  Force the classic REPL (overrides display.interface: tui)
    socis -c                     Resume the most recent session
    socis -c "my project"        Resume a session by name (latest in lineage)
    socis --resume <session_id>  Resume a specific session by ID
    socis --resume latest        Resume the most recent session (same as -c)
    socis --tui --resume latest --in ./dir   Resume ./dir's latest session in the TUI
    socis setup                  Run setup wizard
    socis logout                 Clear stored authentication
    socis auth add <provider>    Add a pooled credential
    socis auth list              List pooled credentials
    socis auth remove <p> <t>    Remove pooled credential by index, id, or label
    socis auth reset <provider>  Clear exhaustion status for a provider
    socis model                  Select default model
    socis fallback [list]        Show fallback provider chain
    socis fallback add           Add a fallback provider (same picker as `socis model`)
    socis fallback remove        Remove a fallback provider from the chain
    socis config                 View configuration
    socis config edit            Edit config in $EDITOR
    socis config set model gpt-4 Set a config value
    socis gateway                Run messaging gateway
    socis -s socis-agent-dev,github-auth
    socis -w                     Start in isolated git worktree
    socis gateway install        Install gateway background service
    socis sessions list          List past sessions
    socis sessions browse        Interactive session picker
    socis sessions rename ID T   Rename/title a session
    socis logs                   View agent.log (last 50 lines)
    socis logs -f                Follow agent.log in real time
    socis logs errors            View errors.log
    socis logs --since 1h        Lines from the last hour
    socis debug share             Upload debug report for support
    socis console                Open the safe SOCIS command console
    socis update                 Update to latest version
    socis dashboard              Start web UI dashboard (port 9119)
    socis dashboard --stop       Stop running dashboard processes
    socis dashboard --status     List running dashboard processes
...
def build_top_level_parser():
    ...
    parser = argparse.ArgumentParser(
        prog="socis",
        description="SOCIS Agent - AI assistant with tool-calling capabilities",
    ...
    # Mirrors `socis chat --model ... --provider ...` semantics.
    _inherited_flag(
        parser,
        "-m",
        "--model",
        ...
        help=(
            ...
            "Applies to -z/--oneshot and --tui. Also settable via SOCIS_AGENT_INFERENCE_MODEL env var."
        ),
    )
    _inherited_flag(
        parser,
        ...
        help=(
            ...
            "under model.provider — use `socis setup` or edit the file to change it."
        ),
    )
    _inherited_flag(
        parser,
    ...
    _inherited_flag(
        ...
        help=(
            ...
            "without a TTY prompt.  Equivalent to SOCIS_AGENT_ACCEPT_HOOKS=1 or "
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=False,
        help="Ignore ~/.socis-agent/config.yaml and fall back to built-in defaults (credentials in .env are still loaded)",
    )
    _inherited_flag(
        parser,
        "--ignore-rules",
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with SOCIS Agent",
    )
    ...
    # top-level parser: when the user writes `socis -m foo chat`, argparse
    ...
    # `-w/--worktree`, `--yolo`, etc. (see tests/socis_cli/
    ...
    _inherited_flag(
        ...
        help=(
            "Auto-approve any unseen shell hooks declared in config.yaml "
            "without a TTY prompt (see also SOCIS_AGENT_ACCEPT_HOOKS env var and "
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=argparse.SUPPRESS,
        help="Ignore ~/.socis-agent/config.yaml and fall back to built-in defaults (credentials in .env are still loaded). Useful for isolated CI runs, reproduction, and third-party integrations.",
    )
    _inherited_flag(
        chat_parser,
        "--ignore-rules",
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=argparse.SUPPRESS,
        help="Troubleshooting mode: disable ALL customizations — user config, AGENTS.md/memory injection, plugins, and MCP servers (implies --ignore-user-config and --ignore-rules). Use to isolate whether a problem comes from your setup or from SOCIS itself.",
    )
    chat_parser.add_argument(
        "--source",
        default=None,
