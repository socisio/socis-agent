...
1. **Update receipt** — a machine-readable JSON record of what one
   ``socis update`` run discovered, did, skipped (and why), written to
   ``<SOCIS_AGENT_HOME>/logs/update_receipts/``. Silent-failure classes this
...
``socis_cli.build_info.get_code_identity()``: an image build reports
...
# Module-level current receipt. ``socis update`` is a single-threaded CLI
...
class UpdateReceipt:
    """Collects the observable facts of one ``socis update`` run."""
    ...
    def gateway_restart_result(
        ...
        *,
        ...
        if fresh_recovery is not None:
            ...
            # Serve/dashboard coverage (#92145). ``socis serve`` hosts
...
def _receipt_dir() -> Path:
    ...
    return get_socis_agent_home() / "logs" / _RECEIPT_DIR_NAME
...
def finalize_pending_update_receipt(
    ...
    ``socis update`` has many early-termination paths (Windows
