"""Safe SOCIS Console command engine.

This module backs ``socis console`` and is intentionally narrower than the
full SOCIS CLI. It exposes a curated set of native adapters that can later be
...
class ConsoleCommand:
    ...
    usage: str
    summary: str
    handler: Callable[["SOCISConsoleEngine", list[str]], str]
    mutating: bool = False
    confirmation: str = ""
...
def _strip_console_status_footer(text: str) -> str:
    ...
    if not (
        prev.startswith("Run 'socis doctor'")
        and last.startswith("Run 'socis setup'")
    ):
        return text.rstrip()
...
def _split_line(line: str) -> list[str]:
    try:
        ...
        from socis_cli._subprocess_compat import split_command_line
...
def _parser_root() -> tuple[_ArgumentParser, argparse._SubParsersAction]:
    parser = _ArgumentParser(prog="socis", add_help=False)
...
def _clean_summary(text: str | None) -> str:
    ...
    if not summary:
        return ""
    if summary.startswith("Run `socis "):
        return ""
    return summary
...
# dashboard opens a fresh SOCISConsoleEngine per /api/console connection, so
...
def _dispatch_extracted_subcommand(
    *,
    ...
    parser, subparsers = _parser_root()
    module = importlib.import_module(module_name)
    main_module = importlib.import_module("socis_cli.main")
    builder = getattr(module, builder_name)
...
def _extracted_handler(
    ...
    main_handler_name: str,
    namespace_update: Callable[[argparse.Namespace], None] | None = None,
) -> Callable[["SOCISConsoleEngine", list[str]], str]:
    def handler(_engine: SOCISConsoleEngine, args: list[str]) -> str:
...
def _register_command_family(
    engine: "SOCISConsoleEngine",
    *,
    root: str,
    paths: Iterable[Sequence[str]],
    handler_factory: Callable[[Sequence[str]], Callable[["SOCISConsoleEngine", list[str]], str]],
    ...
    for child_path in paths:
        ...
        command_summary = summary or (summaries or {}).get(full_path) or f"Run `socis {usage}`."
        engine.register(
            full_path,
            usage,
            command_summary,
            ...
            confirmation=confirmation or f"Run `socis {usage}`?",
        )


class SOCISConsoleEngine:
    """Curated line-command executor for SOCIS Console."""
        ...
        try:
            ...
            if _contains_shell_syntax(raw_line, tokens):
                raise ConsoleCommandError(
                    "SOCIS Console does not run shell syntax. Use one supported "
                    "SOCIS command at a time."
                )
    ...
    def _register_defaults(self) -> None:
        self.register(("status",), "status", "Show SOCIS component status.", _status)
        self.register(("version",), "version", "Show SOCIS version information.", _version)
        self.register(("doctor",), "doctor", "Run diagnostics without auto-fix.", _doctor)
        self.register(("logs",), "logs [name] [-n N]", "Show recent SOCIS logs.", _logs)
        ...
        self.register(
            ...
            _config_set,
            mutating=True,
            confirmation="Update SOCIS configuration?",
        )
    ...
    def _register_broad_cli_surface(self) -> None:
        """Register non-admin CLI commands that are safe for SOCIS Console."""

        extracted = {
            "dump": (
                "socis_cli.subcommands.dump",
                "build_dump_parser",
                "cmd_dump",
                [()],
                set(),
            ...
            "prompt-size": (
                "socis_cli.subcommands.prompt_size",
            ...
            "insights": (
                "socis_cli.subcommands.insights",
            ...
            "security": (
                "socis_cli.subcommands.security",
            ...
            "import": (
                "socis_cli.subcommands.import_cmd",
        ...
        self.register(
            ...
            _config_migrate,
            mutating=True,
            confirmation="Update SOCIS configuration with missing defaults?",
        )
        self.register(
            ("sessions", "export"),
        ...
        self.register(
            ...
            _adder_handler("send", (), "socis_cli.send_cmd", "register_send_subparser"),
            mutating=True,
        ...
        _register_command_family(
            ...
            root="portal",
            paths=portal_paths,
            summaries=_adder_summaries("socis_cli.portal_cli", "add_parser"),
            handler_factory=lambda fixed: _adder_handler(
                "portal",
                fixed,
                "socis_cli.portal_cli",
                "add_parser",
            ),
        )
        ...
        _register_command_family(
            ...
            summaries=_builder_summaries("socis_cli.projects_cmd", "build_parser"),
            mutating=[
                ("create",),
                ("add-folder",),
                ("remove-folder",),
        ...
        _register_command_family(
            ...
            summaries=_builder_summaries("socis_cli.kanban", "build_parser"),
            mutating=[
                ("init",),
                ("boards", "create"),
                ("boards", "rm"),
    ...
    def _resolve_command(self, tokens: Sequence[str]) -> tuple[ConsoleCommand, list[str]]:
        ...
        raise ConsoleCommandError(f"Unsupported SOCIS Console command: {probe}.{suffix}")

    def _rejection_for(self, tokens: Sequence[str]) -> str:
        first = tokens[0]
        if first.startswith("-"):
            return f"{first} is not available in SOCIS Console."
        blocked_top = {
            "acp",
            "chat",
            "claw",
        ...
        if first in blocked_top:
            return f"`socis {first}` is not available in SOCIS Console."
        blocked_pairs = {
            ("config", "edit"): "`config edit` opens an editor and is not available in SOCIS Console.",
            ("mcp", "serve"): "`mcp serve` starts a server and is not available in SOCIS Console.",
            ("profile", "alias"): "`profile alias` creates shell wrappers and is not available in SOCIS Console.",
            ("skills", "config"): "`skills config` is interactive and is not available in SOCIS Console.",
            ("skills", "publish"): "`skills publish` is not available in SOCIS Console.",
            ("portal", "login"): "`portal login` is interactive and is not available in SOCIS Console.",
            ("portal", "open"): "`portal open` opens a browser and is not available in SOCIS Console.",
            ("kanban", "tail"): "`kanban tail` streams output and is not available in SOCIS Console.",
            ("kanban", "watch"): "`kanban watch` streams output and is not available in SOCIS Console.",
            ("kanban", "daemon"): "`kanban daemon` starts a service and is not available in SOCIS Console.",
            ("kanban", "dispatcher"): "`kanban dispatcher` starts a worker and is not available in SOCIS Console.",
            ("kanban", "swarm"): "`kanban swarm` starts agent work and is not available in SOCIS Console.",
            ("kanban", "decompose"): "`kanban decompose` starts agent work and is not available in SOCIS Console.",
            ("kanban", "specify"): "`kanban specify` starts agent work and is not available in SOCIS Console.",
            ("kanban", "gc"): "`kanban gc` is not available in SOCIS Console.",
        }
        if len(tokens) >= 2:
        ...
        if tuple(tokens[:2]) in {("sessions", "delete"), ("sessions", "prune")}:
            return "`sessions delete` and `sessions prune` are not available in SOCIS Console."
        return ""
...
def _apply_confirmed_defaults(args: argparse.Namespace) -> None:
    ...
    if getattr(args, "auth_action", None) == "add":
        ...
        if auth_type in {"api-key", "api_key"} and not getattr(args, "api_key", None):
            raise ConsoleCommandError("auth add --type api-key requires --api-key in SOCIS Console.")
...
def _version(_engine: SOCISConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "version")
    from socis_cli._startup_fast import print_fast_version_info

    return _capture_output(lambda: print_fast_version_info(check_updates=True))


def _status(_engine: SOCISConsoleEngine, args: list[str]) -> str:
...
def _doctor(_engine: SOCISConsoleEngine, args: list[str]) -> str:
...
def _logs(_engine: SOCISConsoleEngine, args: list[str]) -> str:
    if "-f" in args or "--follow" in args:
        raise ConsoleCommandError("`logs -f` is not available in SOCIS Console.")
    ...
    from socis_cli.logs import list_logs, tail_log
...
def _sessions_list(_engine: SOCISConsoleEngine, args: list[str]) -> str:
...
def _sessions_stats(_engine: SOCISConsoleEngine, args: list[str]) -> str:
...
def _config_show(_engine: SOCISConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "config show")
    from socis_cli.config import show_config

    return _capture_output(show_config)


def _config_path(_engine: SOCISConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "config path")
    from socis_cli.config import get_config_path

    return str(get_config_path())


def _config_set(_engine: SOCISConsoleEngine, args: list[str]) -> str:
    if len(args) < 2:
...
def _config_migrate(_engine: SOCISConsoleEngine, args: list[str]) -> str:
...
def _sessions_export(_engine: SOCISConsoleEngine, args: list[str]) -> str:
...
def _sessions_rename(_engine: SOCISConsoleEngine, args: list[str]) -> str:
...
def _sessions_optimize(_engine: SOCISConsoleEngine, args: list[str]) -> str:
...
def _sessions_repair(_engine: SOCISConsoleEngine, args: list[str]) -> str:
    ...
    def _run() -> None:
        from socis_agent_state import DEFAULT_DB_PATH, _db_opens_cleanly, repair_state_db_schema
...
def _profile_status(_engine: SOCISConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "profile")
    return _dispatch_extracted_subcommand(
        root="profile",
        fixed=(),
        args=(),
        module_name="socis_cli.subcommands.profile",
        builder_name="build_profile_parser",
        main_handler_name="cmd_profile",
    )


def _cron_list(_engine: SOCISConsoleEngine, args: list[str]) -> str:
    parser = _ArgumentParser(prog="cron list", add_help=False)
    parser.add_argument("--all", action="store_true")
    ns = parser.parse_args(args)
    from socis_cli.cron import cron_list

    return _capture_output(lambda: cron_list(show_all=ns.all))


def _cron_status(_engine: SOCISConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "cron status")
    from socis_cli.cron import cron_status

    return _capture_output(cron_status)


def _cron_pause(_engine: SOCISConsoleEngine, args: list[str]) -> str:
    if len(args) != 1:
        raise ConsoleCommandError("Usage: cron pause <job>")
    from cron.jobs import AmbiguousJobReference, pause_job

    try:
        job = pause_job(args[0], reason="paused from socis console")
...
def _cron_resume(_engine: SOCISConsoleEngine, args: list[str]) -> str:
    ...
    from cron.jobs import AmbiguousJobReference, _socis_now, rearm_oneshot, resume_job

    try:
        if ns.at or ns.run_now:
            job = rearm_oneshot(ns.job, _socis_now().isoformat() if ns.run_now else ns.at)
        else:
...
def _cron_run(_engine: SOCISConsoleEngine, args: list[str]) -> str:
    if len(args) != 1:
...
def run_console_repl(
    *,
    ...
    """Run the local ``socis console`` REPL."""
    ...
    engine = SOCISConsoleEngine()
    if interactive:
        print("SOCIS Console. Type `help` for commands, `exit` to quit.", file=stdout)

    while True:
        if interactive:
            print("socis> ", end="", file=stdout, flush=True)
        line = stdin.readline()
        if line == "":
            if interactive:
                print(file=stdout)
