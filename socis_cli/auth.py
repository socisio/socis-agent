"""
Multi-provider authentication system for SOCIS Agent.

Supports OAuth device code flows (Nous Portal, future: OpenAI Codex) and
traditional API key providers (OpenRouter, custom endpoints). Auth state
is persisted in ~/.socis-agent/auth.json with cross-process file locking.
...
# httpx is imported lazily: it costs ~30ms at import time and socis_cli.auth
...
if TYPE_CHECKING:
    ...
    class _LazyHttpx:
        ...
        # ("socis_cli.auth.httpx.Client", ...) keeps working in tests.
...
from socis_agent_constants import OPENROUTER_BASE_URL, secure_parent_dir
...
DEFAULT_NOUS_CLIENT_ID = "socis-cli"
...
try:  # Version tag for the Codex token-endpoint User-Agent; fall back if unavailable.
    from socis_cli import __version__ as _SOCIS_AGENT_CLI_VERSION
except Exception:  # pragma: no cover - version import should always succeed
    _SOCIS_AGENT_CLI_VERSION = "unknown"
CODEX_OAUTH_USER_AGENT = f"socis-cli/{_SOCIS_AGENT_CLI_VERSION}"
...
SPOTIFY_DOCS_URL = "https://agent.socis.io/docs/user-guide/features/spotify"
SPOTIFY_DASHBOARD_URL = "https://developer.spotify.com/dashboard"
SPOTIFY_ACCESS_TOKEN_REFRESH_SKEW_SECONDS = 120

OAUTH_OVER_SSH_DOCS_URL = "https://agent.socis.io/docs/guides/oauth-over-ssh"
...
def normalize_actual_base_url(base_url: str) -> str:
    ...
    surface for SOCIS' Responses transport.
    """
...
PROVIDER_REGISTRY: Dict[str, ProviderConfig] = {
    ...
    "anthropic": ProviderConfig(
        ...
        # from it), so removing it would stop SOCIS finding a setup-token
    ...
    "opencode-free": ProviderConfig(
        ...
        # secret to configure. Select via `socis model` / `/model free`.
        api_key_env_vars=(),
    ),
...
try:
    ...
    for _pp in _list_providers_for_registry():
        ...
        if _pp.auth_type == "external_process":
            ...
            # without it, `socis -m <that provider>` dies with
...
def get_anthropic_key() -> str:
    ...
    ``~/.socis-agent/.env`` so a deliberate key rotation isn't shadowed by a stale
    ...
    """
    from socis_cli.config import get_env_value_prefer_dotenv
...
def _resolve_api_key_provider_secret(
    ...
    if provider_id == "copilot":
        ...
        try:
            from socis_cli.copilot_auth import resolve_copilot_token, get_copilot_api_token
    ...
    for env_var in pconfig.api_key_env_vars:
        # Prefer ~/.socis-agent/.env over os.environ so a deliberate key rotation
...
def is_rate_limited_auth_error(error: Exception) -> bool:
    ...
    instead of prompting the operator to run ``socis auth``.
    """
    return (
...
def format_auth_error(error: Exception) -> str:
    ...
    if error.relogin_required:
        return f"{error} Run `socis model` to re-authenticate."
...
def _format_nous_entitlement_auth_error(error: AuthError) -> str:
    try:
        from socis_cli.nous_account import (
...
def _oauth_trace_enabled() -> bool:
    raw = os.getenv("SOCIS_AGENT_OAUTH_TRACE", "").strip().lower()
...
# =============================================================================
# Auth Store — persistence layer for ~/.socis-agent/auth.json
# =============================================================================

def _auth_file_path() -> Path:
    path = get_socis_agent_home() / "auth.json"
    # Seat belt: if pytest is running and SOCIS_AGENT_HOME resolves to the real
    # user's auth store, refuse rather than silently corrupt it. This catches
    # tests that forgot to monkeypatch SOCIS_AGENT_HOME, tests invoked without the
    # hermetic conftest, or sandbox escapes via threads/subprocesses. In
    # production (no PYTEST_CURRENT_TEST) this is a single dict lookup.
    if os.environ.get("PYTEST_CURRENT_TEST"):
        real_home_auth = (Path.home() / ".socis-agent" / "auth.json").resolve(strict=False)
        try:
        ...
        if resolved == real_home_auth:
            raise RuntimeError(
                ...
                "Set SOCIS_AGENT_HOME to a tmp_path in your test fixture, or run "
...
def _global_auth_file_path() -> Optional[Path]:
    ...
    directory (classic mode, or custom SOCIS_AGENT_HOME that is not a profile).
    ...
    try:
        ...
        global_root = get_default_socis_root()
    except Exception:
        return None
    profile_home = get_socis_agent_home()
    try:
...
def _load_global_auth_store() -> Dict[str, Any]:
    ...
    (``_global_auth_file_path()`` -> ``get_default_socis_root()``) + JSON
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        ...
        if real_home_env:
            real_root = Path(real_home_env) / ".socis-agent" / "auth.json"
            try:
...
def _file_lock(
    ...
    # A stress test with 20 concurrent SOCIS processes reproduced this
...
def _save_auth_store(auth_store: Dict[str, Any], target_path: Optional[Path] = None) -> Path:
    ...
    # socis-agent install tree (#25821, #93050).
...
def _load_provider_state(auth_store: Dict[str, Any], provider_id: str) -> Optional[Dict[str, Any]]:
    ...
    global scope. Once the user runs ``socis auth login <provider>`` inside
...
def mark_provider_active_if_unset(provider_id: str) -> None:
    ...
    Used by ``socis auth add`` OAuth paths that create credential-pool
...
def strip_cloned_single_use_oauth_grants(profile_dir: Path) -> Dict[str, Any]:
    ...
    into another (``socis profile create --clone-all``, the dashboard/TUI
...
# (an independent ``socis -p <p> auth add`` grant, or the only surviving
...
def consume_oauth_heal_notices() -> List[str]:
    ...
    ``socis auth list`` / ``socis auth status`` print them so the user sees
...
def _find_root_counterpart(
    ...
    Anthropic ``socis_pkce`` grant uses one client id and carries no claims,
    ...
    (an independent ``socis -p <p> auth add`` login for another account).
    """
...
def _heal_forked_single_use_oauth_grants(provider_id: str) -> Optional[Dict[str, Any]]:
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        ...
        # user's ~/.socis-agent/auth.json from a test that forgot to isolate HOME.
        real_home_env = os.environ.get("HOME", "")
        if real_home_env and _same_path(root_path, Path(real_home_env) / ".socis-agent" / "auth.json"):
            return None
    ...
    with _auth_store_lock():
        ...
        with _auth_store_lock(target_path=root_path):
            ...
            for row in p_rows:
                ...
                # .anthropic_oauth.json (the ``socis auth`` PKCE shape); a
                # profile socis_pkce-family row is that grant's copy.
                is_pkce = str(row.get("source") or "").endswith("socis_pkce")
                    ...
                    if root_singleton_row is not None:
                        ...
                        idx = next(
                            (i for i, r in enumerate(r_rows)
                             ...
                             and str(r.get("source") or "").endswith("socis_pkce")),
                            None,
                        )
            ...
            if summary["adopted"] and root_singleton is not None and root_singleton_row is not None:
                # Keep root's singleton and its ``socis_pkce``-seeded pool row
                ...
                pkce_idx = next(
                    (i for i, r in enumerate(r_rows)
                     if _is_oauth_pool_payload(r) and r.get("source") == "socis_pkce"),
                    None,
                )
            ...
            if summary["adopted"] and root_singleton is not None and root_singleton_row is not None:
                from agent.anthropic_credentials import _write_socis_oauth_credentials
                _write_socis_oauth_credentials(
...
def read_credential_pool(provider_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    ``socis auth add <provider>`` inside the profile, profile entries
...
def is_provider_explicitly_configured(provider_id: str) -> bool:
    ...
    try:
        ...
        # ``provider: anthropic`` as a MoA advisor/aggregator has opted SOCIS
    ...
    # not by the user explicitly configuring anthropic in SOCIS.
    ...
    if pconfig is None:
        from socis_cli.providers import get_provider
    ...
    # the user initiated inside SOCIS (manual add / device-code / PKCE), plus
    ...
    try:
        for entry in read_credential_pool(normalized):
            ...
            if source.startswith("env:"):
                ...
                continue
            if (
                source in {"device_code", "loopback_pkce", "socis_pkce", "manual"}
    ...
    # explicit for users who never set SOCIS up for it. Only SOCIS-scoped
...
def clear_provider_auth(provider_id: Optional[str] = None) -> bool:
    """
    Clear auth state for a provider. Used by `socis logout`.
...
def _get_config_hint_for_unknown_provider(provider_name: str) -> str:
    ...
    """
    try:
        from socis_cli.config import validate_config_structure
        issues = validate_config_structure()
        if not issues:
            return ""

        lines = ["Config issue detected — run 'socis doctor' for full diagnostics:"]
        for ci in issues:
...
def _refuse_env_adoption_if_config_corrupt() -> None:
    ...
    When ``~/.socis-agent/config.yaml`` EXISTS but fails to parse, ``load_config()``
    ...
    """
    try:
        from socis_cli.config import get_active_config_parse_failure, get_config_path
    ...
    raise AuthError(
        ...
        f"was saved next to it) or run socis setup.",
        code="corrupt_config",
    )
    ...
    *,
    ...
    if normalized != "auto":
        ...
        if _config_hint:
            ...
            msg += " Check 'socis model' for available providers, or run 'socis doctor' to diagnose config issues."
    ...
    # Auto-detect an OpenRouter credential added via `socis auth add openrouter`
    ...
    # `socis auth list` showing the credential while requests go out with no
    ...
    for pid, pconfig in PROVIDER_REGISTRY.items():
        ...
        for env_var in pconfig.api_key_env_vars:
            if has_usable_secret(_scoped_key_env(env_var)):
                ...
                # OAuth but has a stale key in ~/.socis-agent/.env isn't silently
    ...
    raise AuthError(
        "No inference provider configured. Run 'socis model' to choose a "
        "provider and model, or set an API key (OPENROUTER_API_KEY, "
        "OPENAI_API_KEY, etc.) in ~/.socis-agent/.env.",
...
def _nous_portal_env_override() -> Optional[str]:
    ...
    Mirrors ``_nous_inference_env_override()``: ``SOCIS_AGENT_PORTAL_BASE_URL`` /
    ...
    return _optional_base_url(
        os.getenv("SOCIS_AGENT_PORTAL_BASE_URL") or os.getenv("NOUS_PORTAL_BASE_URL")
    )
...
def _assert_nous_inference_jwt_usable(
    ...
    *,
    ...
    raise AuthError(
        ...
        f"({reason}). Re-authenticate with: socis auth add nous",
        provider="nous",
        code=reason,
...
def resolve_qwen_runtime_credentials(
    *,
    ...
    base_url = os.getenv("SOCIS_AGENT_QWEN_BASE_URL", "").strip().rstrip("/") or DEFAULT_QWEN_BASE_URL
    return {
        "provider": "qwen-oauth",
        "base_url": base_url,
...
def get_qwen_auth_status() -> Dict[str, Any]:
    ...
    try:
        ...
        # and `socis model` walks users into a broken Qwen setup flow.
...
# =============================================================================
# Spotify auth — PKCE tokens stored in ~/.socis-agent/auth.json
# =============================================================================
...
def _spotify_client_id(
    ...
    candidates = (
        explicit,
        get_env_value("SOCIS_AGENT_SPOTIFY_CLIENT_ID"),
    ...
    for candidate in candidates:
        ...
        if cleaned:
            return cleaned
    raise AuthError(
        "Spotify client_id is required. Set SOCIS_AGENT_SPOTIFY_CLIENT_ID or pass --client-id.",
        provider="spotify",
...
def _spotify_redirect_uri(
    ...
    candidates = (
        explicit,
        get_env_value("SOCIS_AGENT_SPOTIFY_REDIRECT_URI"),
...
def _spotify_api_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("SOCIS_AGENT_SPOTIFY_API_BASE_URL"),
...
def _spotify_accounts_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("SOCIS_AGENT_SPOTIFY_ACCOUNTS_BASE_URL"),
...
def _refresh_spotify_oauth_state(
    ...
    *,
    ...
    if not refresh_token:
        raise AuthError(
            "Spotify refresh token missing. Run `socis auth spotify` again.",
            provider="spotify",
    ...
    if response.status_code >= 400:
        detail = response.text.strip()
        raise AuthError(
            "Spotify token refresh failed. Run `socis auth spotify` again."
            + (f" Response: {detail}" if detail else ""),
            provider="spotify",
            code="spotify_refresh_failed",
            relogin_required=True,
...
def resolve_spotify_runtime_credentials(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError(
                "Spotify is not authenticated. Run `socis auth spotify` first.",
                provider="spotify",
    ...
    if not access_token:
        raise AuthError(
            "Spotify access token missing. Run `socis auth spotify` again.",
            provider="spotify",
...
def _spotify_interactive_setup(redirect_uri_hint: str) -> str:
    ...
    resulting client_id to ~/.socis-agent/.env, and return it.
    ...
    print("       App name:     anything (e.g. socis-agent)")
    ...
    # Persist so subsequent `socis auth spotify` runs skip the wizard.
    save_env_value("SOCIS_AGENT_SPOTIFY_CLIENT_ID", raw)
    ...
    if redirect_uri_hint and redirect_uri_hint != DEFAULT_SPOTIFY_REDIRECT_URI:
        save_env_value("SOCIS_AGENT_SPOTIFY_REDIRECT_URI", redirect_uri_hint)

    print()
    print("Saved SOCIS_AGENT_SPOTIFY_CLIENT_ID to ~/.socis-agent/.env")
    print()
    return raw
...
def login_spotify_command(args) -> None:
    ...
    # with "SOCIS_AGENT_SPOTIFY_CLIENT_ID is required".
    ...
    print()
    print("Open this URL to authorize SOCIS:")
    print(authorize_url)
    print()
...
def _print_loopback_ssh_hint(redirect_uri: str, *, docs_url: str | None = None) -> None:
    ...
    print(divider)
    print(f"SOCIS is waiting for the OAuth callback on {redirect_uri}")
...
# =============================================================================
# OpenAI Codex auth — tokens stored in ~/.socis-agent/auth.json (not ~/.codex/)
#
# SOCIS maintains its own Codex OAuth session separate from the Codex CLI
# and VS Code extension. This prevents refresh token rotation conflicts
# where one app's refresh invalidates the other's session.
# =============================================================================

def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from SOCIS auth store (~/.socis-agent/auth.json).
    ...
    if not state:
        raise AuthError(
            "No Codex credentials stored. Run `socis auth` to authenticate.",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "Codex auth state is missing tokens. Run `socis auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(access_token, str) or not access_token.strip():
        raise AuthError(
            "Codex auth is missing access_token. Run `socis auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "Codex auth is missing refresh_token. Run `socis auth` to re-authenticate.",
            provider="openai-codex",
...
def _sync_codex_pool_entries(
    ...
    * ``device_code`` — the singleton-seeded entry written by the device-code
      OAuth flow when the user logged in via ``socis setup`` / the model
      picker.  Always synced with the fresh tokens.
    * ``manual:device_code`` — entries created by ``socis auth add openai-codex``
      ...
      ``socis auth add openai-codex`` and must not be overwritten by a
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None, label: str = None) -> None:
    """Save Codex OAuth tokens to SOCIS auth store (~/.socis-agent/auth.json)."""
    ...
    with _auth_store_lock():
        ...
        # ``socis auth add openai-codex`` created (which must not be
...
def _recover_codex_tokens_from_cli(reason: str) -> Optional[Dict[str, str]]:
    """Adopt a valid Codex CLI token pair into SOCIS auth, if available."""
...
def refresh_codex_oauth_pure(
    ...
    *,
    ...
    """Refresh Codex OAuth tokens without mutating SOCIS auth state."""
    ...
    if response.status_code == 429:
        ...
        # "run socis auth" prompt (see issue #32790).
    ...
    if response.status_code != 200:
        ...
        if code == "refresh_token_reused":
            message = (
                ...
                "then run `socis auth` to re-authenticate."
            )
            relogin_required = True
...
def _refresh_codex_auth_tokens(
    ...
    Saves the new tokens to SOCIS auth store automatically.
    """
    try:
        ...
        # Self-heal cross-store refresh_token rotation. SOCIS keeps its OWN
        ...
        # the Codex CLI (or another SOCIS process) rotates the shared token,
...
def resolve_codex_runtime_credentials(
    *,
    ...
    """Resolve runtime credentials from SOCIS's own Codex token store.
    ...
    if data is None:
        ...
        if pool_token:
            base_url = (
                os.getenv("SOCIS_AGENT_CODEX_BASE_URL", "").strip().rstrip("/")
    ...
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = env_float("SOCIS_AGENT_CODEX_REFRESH_TIMEOUT_SECONDS", 20)

    should_refresh = bool(force_refresh)
    if (not should_refresh) and refresh_if_expiring:
        should_refresh = _codex_access_token_is_expiring(access_token, refresh_skew_seconds)
    if should_refresh:
        # Re-read under lock to avoid racing with other SOCIS processes
...
def _probe_codex_quota_restored(
    ...
    *,
    ...
    SOCIS persists a Codex 429's ``reset_at`` locally and freezes the
...
# =============================================================================
# xAI Grok OAuth — tokens stored in ~/.socis-agent/auth.json
# =============================================================================
...
def _read_xai_oauth_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    ...
    if not state:
        raise AuthError(
            "No xAI OAuth credentials stored. Select xAI Grok OAuth (SuperGrok / Premium+) in `socis model`.",
            provider="xai-oauth",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "xAI OAuth state is missing tokens. Re-authenticate with `socis model`.",
            provider="xai-oauth",
    ...
    if not access_token:
        raise AuthError(
            "xAI OAuth state is missing access_token. Re-authenticate with `socis model`.",
            provider="xai-oauth",
    ...
    if not refresh_token:
        raise AuthError(
            "xAI OAuth state is missing refresh_token. Re-authenticate with `socis model`.",
            provider="xai-oauth",
...
def _write_through_xai_oauth_to_global_root(state: Dict[str, Any]) -> None:
    ...
    # ~/.socis-agent/auth.json even when SOCIS_AGENT_HOME points at a profile path
...
def _xai_proactive_refresh_skew_seconds(access_token: str) -> int:
    ...
    resolution (chat turn, Imagine tool call, ``socis auth status``, …),
...
def _xai_validate_oauth_endpoint(url: str, *, field: str) -> str:
    ...
    output is cached in ``~/.socis-agent/auth.json``. A single MITM during initial
    ...
    if host != "x.ai" and not host.endswith(".x.ai"):
        raise AuthError(
            ...
            f"discovery; re-authenticate with `socis model` to re-fetch.",
            provider="xai-oauth",
            code="xai_discovery_invalid",
        )
    return url
...
def _xai_validate_inference_base_url(value: str, *, fallback: str) -> str:
    ...
    the user's SuperGrok subscription. ``XAI_BASE_URL`` / ``SOCIS_AGENT_XAI_BASE_URL``
...
def refresh_xai_oauth_pure(
    ...
    *,
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "xAI OAuth is missing refresh_token. Re-authenticate with `socis model`.",
            provider="xai-oauth",
            code="xai_auth_missing_refresh_token",
            relogin_required=True,
        )
    ...
    # written by an older SOCIS (or hand-edited) may carry a non-xAI
    ...
    # with a clear error so the user can re-run `socis model` to refetch.
    ...
    if response.status_code != 200:
        ...
        # on the allowlist for API access).  Re-running ``socis model``
...
def resolve_xai_oauth_runtime_credentials(
    *,
    ...
    tokens = dict(data["tokens"])
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = env_float("SOCIS_AGENT_XAI_REFRESH_TIMEOUT_SECONDS", 20)
    ...
    base_url = _xai_validate_inference_base_url(
        os.getenv("SOCIS_AGENT_XAI_BASE_URL", "").strip().rstrip("/")
...
def _resolve_verify(
    *,
    ...
    effective_ca = (
        ...
        or os.getenv("SOCIS_AGENT_CA_BUNDLE")
...
# so a new `socis --profile <name> auth add nous --type oauth` can one-tap
# import instead of running the full device-code flow every time.
#
# File lives at ${SOCIS_AGENT_SHARED_AUTH_DIR}/nous_auth.json, defaulting to
# ``<socis-root>/shared/nous_auth.json`` where ``<socis-root>`` is what
# ``get_default_socis_root()`` returns — ``~/.socis-agent`` on Linux/macOS,
# ``%LOCALAPPDATA%\socis`` on native Windows, or the Docker/custom root.
# It is OUTSIDE any named profile's SOCIS_AGENT_HOME so named profiles (which
# typically live under ``<socis-root>/profiles/<name>/``) all see the
# same file.
#
...
def _nous_shared_auth_dir() -> Path:
    ...
    Honors ``SOCIS_AGENT_SHARED_AUTH_DIR`` so tests can redirect it to a tmp
    path without touching the real user's home. Defaults to
    ``<socis-root>/shared/``, where ``<socis-root>`` is what
    :func:`socis_agent_constants.get_default_socis_root` returns — so
    Linux/macOS classic installs land at ``~/.socis-agent/shared/``, native
    Windows installs at ``%LOCALAPPDATA%\\socis\\shared\\``, and
    Docker / custom ``SOCIS_AGENT_HOME`` deployments at
    ``<SOCIS_AGENT_HOME>/shared/``. Sits outside any named profile so all
    profiles under the same root share the store.
    """
    override = os.getenv("SOCIS_AGENT_SHARED_AUTH_DIR", "").strip()
    if override:
        return Path(override).expanduser()
    from socis_agent_constants import get_default_socis_root
    return get_default_socis_root() / "shared"


def _nous_shared_store_path() -> Path:
    path = _nous_shared_auth_dir() / NOUS_SHARED_STORE_FILENAME
    # Seat belt: if pytest is running and this resolves to a path under the
    # real user's SOCIS root, refuse rather than silently corrupt cross-profile
    # state. Tests must set SOCIS_AGENT_SHARED_AUTH_DIR to a tmp_path (conftest
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        from socis_agent_constants import get_default_socis_root
        real_home_shared = (
            get_default_socis_root() / "shared" / NOUS_SHARED_STORE_FILENAME
        ...
        if resolved == real_home_shared:
            raise RuntimeError(
                ...
                f"{path}. Set SOCIS_AGENT_SHARED_AUTH_DIR to a tmp_path in your test fixture."
            )
    return path
...
def _nous_shared_store_lock(timeout_seconds: float = AUTH_LOCK_TIMEOUT_SECONDS):
    ...
    try:
        ...
        # No SOCIS_AGENT_HOME yet (pre-setup): fall through without locking.
        yield
        return
...
def _quarantine_nous_oauth_state(
    ...
    *,
    ...
    # sizes/booleans. NEVER pass a raw token/agent_key into the log call — SOCIS
...
def _refresh_access_token(
    *,
    ...
    # called POST /api/oauth/token with SOCIS's refresh_token without
    # persisting the rotated token back to auth.json — the server then
    # retires the original RT, SOCIS's next refresh uses it, and the whole
    ...
    if code == "refresh_token_reused" or "reuse" in lowered or "reuse detected" in lowered:
        description = (
            ...
            "custom self-heal hook, or another SOCIS install sharing "
            "~/.socis-agent/auth.json) called POST /api/oauth/token with SOCIS's "
            "refresh token without persisting the rotated token back.\n"
            "Nous refresh tokens are single-use — only SOCIS may call the "
            "refresh endpoint. For health checks, use `socis auth status` "
            "instead.\n"
            "Re-authenticate with: socis auth add nous"
        )
        relogin = True
    ...
    *,
    ...
    for item in data:
        ...
        if isinstance(model_id, str) and model_id.strip():
            mid = model_id.strip()
            # Skip SOCIS models — they're not reliable for agentic tool-calling
            if "socis" in mid.lower():
                continue
...
def resolve_nous_access_token(
    *,
    ...
    with _provider_state_transaction("nous") as (
        ...
        if not state:
            raise AuthError(
                "SOCIS is not logged into Nous Portal.",
                provider="nous",
                relogin_required=True,
            )

        # SOCIS_AGENT_PORTAL_BASE_URL / NOUS_PORTAL_BASE_URL is the trusted
...
def persist_nous_credentials(
    ...
    *,
    ...
    Historically ``socis auth add nous`` wrote a ``manual:device_code`` pool
    ...
    ``socis auth add nous --label <name>``).  It gets embedded in the
    ...
    # these credentials via `socis auth add nous --type oauth`. Best-
...
def resolve_nous_runtime_credentials(
    *,
    ...
    with _provider_state_transaction("nous") as (
        ...
        if not state:
            raise AuthError("SOCIS is not logged into Nous Portal.",
        ...
        def _resolve_effective_routing_metadata() -> tuple[str, str, str, str]:
            ...
            portal_url = (
                _optional_base_url(state.get("portal_base_url"))
                or os.getenv("SOCIS_AGENT_PORTAL_BASE_URL")
...
# ~350ms even on the failure path, and read-only UI surfaces (`socis tools`, status panels,
# subscription-feature checks) call it many times per render — `socis tools` → "All Platforms"
...
# `socis auth login/logout/add/remove` invalidate naturally on the next call.
...
def get_codex_auth_status() -> Dict[str, Any]:
    ...
    Checks the credential pool first (where `socis auth` stores credentials),
    then falls back to the legacy provider state.
    """
    # Check credential pool first — this is where `socis auth` and
    # `socis model` store device_code tokens.
    try:
        from agent.credential_pool import load_pool
...
def get_api_key_provider_status(provider_id: str) -> Dict[str, Any]:
    ...
    # the SOCISOverlay keyless flag — the same source the provider catalog
    # and GUI contract tests use.
    try:
        from socis_cli.providers import SOCIS_AGENT_OVERLAYS
        _overlay = SOCIS_AGENT_OVERLAYS.get(provider_id)
    except Exception:
        _overlay = None
...
def _external_process_auth_evidence(provider_id: str) -> tuple[bool, Optional[str]]:
    ...
    CLI may hold its session in an OS keychain SOCIS can't read. Callers
    ...
    try:
        from socis_cli.copilot_auth import COPILOT_ENV_VARS, validate_copilot_token
...
def get_external_process_provider_status(provider_id: str) -> Dict[str, Any]:
    ...
    SOCIS can actually see some — absence of evidence is not absence of auth.
    """
    pconfig = PROVIDER_REGISTRY.get(provider_id)
    ...
    command = (
        os.getenv("SOCIS_AGENT_COPILOT_ACP_COMMAND", "").strip()
        or os.getenv("COPILOT_CLI_PATH", "").strip()
        or "copilot"
    )
    raw_args = os.getenv("SOCIS_AGENT_COPILOT_ACP_ARGS", "").strip()
...
def _get_azure_foundry_auth_status() -> Dict[str, Any]:
    ...
    checks:
      ...
      * ``auth_mode == "entra_id"`` AND ``azure-identity`` is importable
        (we do NOT mint a token here; ``socis doctor`` runs the live
    ...
    try:
        from socis_cli.config import load_config, get_env_value_prefer_dotenv
        cfg = load_config()
    except Exception:
        cfg = {}
        ...
        try:
            ...
            if not installed:
                info["hint"] = (
                    ...
                    "pip install azure-identity  (or rely on SOCIS' "
                    "lazy-install at first use)."
                )
            else:
                info["hint"] = (
                    "azure-identity is installed; live credential validation "
                    "is skipped here. Run `socis doctor` to verify token acquisition."
                )
            return info
        except Exception as exc:
...
def resolve_api_key_provider_credentials(provider_id: str) -> Dict[str, Any]:
    ...
    if provider_id in {"kimi-coding", "kimi-coding-cn"}:
        ...
        try:
            from socis_cli.copilot_auth import (
...
def resolve_external_process_provider_credentials(provider_id: str) -> Dict[str, Any]:
    ...
    # SOCIS_AGENT_COPILOT_ACP_COMMAND / COPILOT_CLI_PATH / SOCIS_AGENT_COPILOT_ACP_ARGS
...
def _update_config_for_provider(
    ...
    # model.api_key.
    from socis_cli.config import clear_model_endpoint_credentials
...
def _logout_default_provider_from_config() -> Optional[str]:
    ...
    `socis logout` historically keyed off auth.json.active_provider only.
...
def _confirm_selection_guards(
    ...
    *,
    ...
    :mod:`socis_cli.model_selection_guards` and shows one [y/N] confirm with
...
def _prompt_model_selection(
    ...
    def _confirmed_selection(mid: str) -> Optional[str]:
        if not mid:
            return None
        # Unified guard registry (socis_cli.model_selection_guards): the cost
    ...
    try:
        ...
        from socis_cli.model_search import model_search_text
    ...
    from socis_cli.curses_ui import format_radio_item_ansi
...
def _save_model_choice(model_id: str) -> None:
    ...
    """
    from socis_cli.config import save_config, load_config
...
def login_command(args) -> None:
    """Deprecated: use 'socis model' or 'socis setup' instead."""
    print("The 'socis login' command has been removed.")
    print("Use 'socis auth' to manage credentials,")
    print("'socis model' to select a provider, or 'socis setup' for full setup.")
    raise SystemExit(0)
...
def _login_openai_codex(
    ...
    *,
    ...
    """OpenAI Codex login via device code flow. Tokens stored in ~/.socis-agent/auth.json."""

    del args, pconfig  # kept for parity with other provider login helpers

    # Check for existing SOCIS-owned credentials
    if not force_new_login:
        try:
            ...
            if isinstance(_resolved_key, str) and _resolved_key and not _codex_access_token_is_expiring(_resolved_key, 60):
                print("Existing Codex credentials found in SOCIS auth store.")
                try:
    ...
    if not force_new_login:
        ...
        if cli_tokens:
            ...
            print("SOCIS will create its own session to avoid conflicts with Codex CLI / VS Code.")
            try:
            ...
            if do_import in {"y", "yes"}:
                ...
                base_url = os.getenv("SOCIS_AGENT_CODEX_BASE_URL", "").strip().rstrip("/") or DEFAULT_CODEX_BASE_URL
                config_path = _update_config_for_provider("openai-codex", base_url)
                print()
                print("Credentials imported. Note: if Codex CLI refreshes its token,")
                print("SOCIS will keep working independently with its own session.")
                print(f"  Config updated: {config_path} (model.provider=openai-codex)")
                return

    # Run a fresh device code flow — SOCIS gets its own OAuth session
    print()
    print("Signing in to OpenAI Codex...")
    print("(SOCIS creates its own session — won't affect Codex CLI or VS Code)")
    print()

    creds = _codex_device_code_login()

    # Save tokens to SOCIS auth store
...
def _login_xai_oauth(
    ...
    *,
    ...
    if not force_new_login:
        try:
            ...
            if isinstance(api_key, str) and api_key and not _xai_access_token_is_expiring(api_key, 60):
                print("Existing xAI OAuth credentials found in SOCIS auth store.")
                try:
                    reuse = input("Use existing credentials? [Y/n]: ").strip().lower()
                except (EOFError, KeyboardInterrupt):
                    reuse = "y"
    ...
    print("(SOCIS creates its own local OAuth session)")
    print()
    ...
    # xAI credential re-enabled. ``socis auth remove xai-oauth`` leaves a
    # ``device_code`` suppression marker that otherwise stops the singleton
    # seed from re-creating the pool entry, so ``socis auth list`` would show
...
def _minimax_save_auth_state(auth_state: Dict[str, Any]) -> None:
    """Persist MiniMax OAuth state to SOCIS auth store (~/.socis-agent/auth.json)."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
...
def _minimax_oauth_login(
    *, region: str = "global", open_browser: bool = True,
    ...
    print(f"Starting SOCIS login via MiniMax ({region}) OAuth...")
...
def build_minimax_oauth_token_provider() -> Callable[[], str]:
    ...
    def _provide() -> str:
        ...
        if not state or not state.get("access_token"):
            raise AuthError(
                "Not logged into MiniMax OAuth. Run `socis model` and select "
                "MiniMax (OAuth).",
...
def resolve_minimax_oauth_runtime_credentials(
    *, min_token_ttl_seconds: int = MINIMAX_OAUTH_REFRESH_SKEW_SECONDS,
    ...
    diagnostic call sites like ``socis status`` that just want to know
...
def _nous_device_code_login(
    *,
    ...
    print(f"Starting SOCIS login via {pconfig.name}...")
    print(f"Portal: {portal_base_url}")
    if insecure:
    ...
    try:
        ...
        if exc.code == "subscription_required":
            ...
            print()
            print("After subscribing, run `socis model` again to finish setup.")
            raise SystemExit(1)
        raise
...
def step_up_nous_billing_scope(
    *,
    ...
    targets the same deployment (incl. a preview via ``SOCIS_AGENT_PORTAL_BASE_URL`` set
...
def _login_nous(args, pconfig: ProviderConfig) -> None:
    ...
    try:
        ...
        try:
            ...
            if model_ids:
                ...
                if free_tier:
                    ...
                    # as free so users on older SOCIS builds still see
            ...
            if model_ids:
                from socis_cli.nous_account import nous_policy_notice
        ...
        if not selected_model:
            ...
            print("  Run `socis model` again to switch to Nous Portal.")
            return
...
def logout_command(args) -> None:
    ...
    if clear_provider_auth(target) or should_reset_config:
        ...
        if should_reset_config and os.getenv("OPENROUTER_API_KEY"):
            print("SOCIS will use OpenRouter for inference.")
        elif should_reset_config:
            print("Run `socis model` or configure an API key to use SOCIS.")
        else:
