"""MCP picker — interactive `socis mcp picker` (also the default `socis mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``socis mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `socis plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from socis_cli.cli_output import prompt_yes_no
from socis_cli.curses_ui import curses_single_select
from socis_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new SOCIS session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from socis_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `socis mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: socis mcp install <name>    Picker: socis mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their SOCIS is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer SOCIS — run `socis update` "
...
def show_catalog() -> None:
    """`socis mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`socis mcp picker` (and default `socis mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`socis mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `socis mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
