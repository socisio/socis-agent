"""Pydantic request/response models for the SOCIS dashboard web server.

Extracted verbatim from ``socis_cli/web_server.py`` (pure schema move).
``web_server`` re-exports every name here, so existing imports like
``from socis_cli.web_server import ConfigUpdate`` keep working.
"""
...
class ModelAssignment(BaseModel):
    ...
    # ``socis model`` custom flow collects.
    api_key: str = ""
...
class ImportRequest(BaseModel):
    archive: str
    # Pass --force to `socis import`. The spawned action runs with
...
class ProfileCreate(BaseModel):
    ...
    # via a subprocess scoped to the profile (`socis -p <name> skills install`)
    # because skills_hub.SKILLS_DIR is import-time-bound and the SOCIS_AGENT_HOME
...
class ProfileExport(BaseModel):
    ...
    # Where to write the archive. Empty → a staging path under SOCIS_AGENT_HOME.
    output: str = ""
