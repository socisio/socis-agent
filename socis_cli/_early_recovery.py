"""Dependency-light venv recovery that runs BEFORE socis_cli.main's imports.

The ``socis`` console entry point is ``socis_cli.main:main``.  Importing
``socis_cli.main`` pulls in third-party packages at module level (``dotenv``
via ``socis_cli.env_loader``, ``yaml`` via ``socis_cli.config``, ...).  In
...
a corrupted venv.  ``socis_cli.main`` imports and calls
:func:`recover_if_needed` at the very top of its module body, before any
third-party import.

Scope: this early pass only repairs enough for ``socis_cli.main`` to become
...
#
# ``socis update`` renames the live ``socis*.exe`` shims aside
# (``socis.exe.old.<unix-ms>``) so uv can write replacements. Putting them BACK
# is the safety-critical direction: losing that rename leaves the install with
# no ``socis`` on PATH, and the command that would repair it IS ``socis
# update`` (#75584).
#
...
def restore_quarantined_shims(
    ...
    *,
    ...
    Messages go to stderr by default -- the startup sweep runs on EVERY socis
    invocation, and ``socis acp`` speaks JSON-RPC on stdout.
    """
    if stream is None:
        stream = sys.stderr
...
def _find_uv_binary() -> str | None:
    ...
    the only sanctioned installer is uv itself, which SOCIS already vendors
    (``~/.socis-agent/bin/uv.exe``) or the user has on PATH.  Stdlib-only.
    """
    exe = "uv.exe" if sys.platform == "win32" else "uv"
    candidates = [
        Path.home() / ".socis-agent" / "bin" / exe,
...
def _run_repair_install(specs: list[str], project_root: Path) -> bool:
    ...
    Streams nothing to stdout (``socis acp`` speaks JSON-RPC on stdout);
...
def _pytest_owns_live_checkout(root: Path) -> bool:
    ...
    Lifecycle tests spawn real subprocesses that import ``socis_cli.main``
...
def recover_if_needed(
    ...
    """Repair wiped core packages so ``socis_cli.main`` can import at all.

    Fast path (no marker present) is two ``lstat`` calls.  Only acts when a
    recovery marker from a prior ``socis update`` exists AND an import probe
    ...
    try:
        ...
        # launch is itself `socis update`: CLI and Desktop retries preserve
...
# every `socis` launch: past this many attempts the early pass hands the
...
def _complete_pending_core_install(root: Path, core_marker: Path) -> bool:
    ...
    a prior ``socis update`` (or the self-lock preflight, #83569) left the
    ...
    ``socis_cli.main``'s third-party imports, maps nothing yet, so the
    ...
    failure does not block every launch (``socis acp`` included).
    ...
    """
    try:
        from socis_cli import _install_repair as ir
        ...
        try:
            ...
            print(
                "  The next launch will retry; socis will keep working from "
