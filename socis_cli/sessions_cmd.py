"""``socis sessions`` command — extracted from ``socis_cli/main.py``.
...
Helpers that stay in ``socis_cli.main`` (``get_socis_agent_home``,
...
``socis_cli.main.<name>`` keep reaching this code path, and so imports stay
...
def get_socis_agent_home():
    return _m().get_socis_agent_home()
...
#: Default age floor for `socis sessions prune --never-active`.  Deliberately
...
def _prune_never_active_keyed(db, args):
    """`socis sessions prune --never-active` — drop leaked/dead keyed rows.
    ...
    """
    from socis_cli.session_filters import format_epoch, parse_duration_seconds
...
def cmd_sessions(args, sessions_parser=None):
    ...
    if action == "repair":
        ...
        if report.get("repaired"):
            ...
            print(f"    socis sessions recover --source {source_hint} \\")
            print("        --inspect-only")
    ...
    if action == "recover":
        ...
        from socis_cli.session_recovery import (
    ...
    if action == "import":
        from socis_cli.foreign_sessions import run_sessions_import
    ...
    if action == "list":
        from socis_agent_state import workspace_key as _ws_key
        ...
        from socis_cli.session_filters import (
            build_prune_filters,
            describe_filters,
        )
        ...
        if args.format == "html":
            if not args.output or args.output == "-":
                ...
                return
            from socis_cli.session_export_html import (
            ...
            try:
                if len(ids) == 1:
                    ...
                    out_dir = (
                        ...
                        else get_socis_agent_home() / "session-exports"
                    )
        ...
        # Markdown / QMD export
        from socis_cli.session_export_md import (
        ...
        if args.output == "-":
            ...
            db.close()
            return
        output_dir = Path(args.output).expanduser() if args.output else get_socis_agent_home() / "session-exports"
        ...
        # `socis sessions prune`: no time window and no filters at all
        ...
        if not _include_pinned and callable(_count_matches):
            ...
            if _pinned_skipped:
                ...
                _optin = (
                    ...
                    "first with `socis sessions unpin <id>`."
                    if action == "prune"
                    else "Unpin first with `socis sessions unpin <id>` to include them."
                )
                print(
        ...
        if skipped_open:
            ...
            print(
                ...
                "sessions. Use `socis sessions delete <id>` "
        ...
        if not pinned_rows:
            print(
                "No pinned sessions. Pin one with: socis sessions pin <session_id>"
            )
            return
        ...
        # Launch socis --resume <id> by replacing the current process
        ...
        if result.get("vacuumed") is False:
            print("  (VACUUM was skipped or failed — run "
                  "`socis sessions optimize` later to reclaim freed space.)")
