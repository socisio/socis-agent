...
Usage:
    socis pairing list              # Show all pending + approved users
    socis pairing approve <platform> <request-id|code>  # Approve a pairing request
    socis pairing revoke <platform> <user_id> # Revoke user access
    socis pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle socis pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: socis pairing {list|approve|revoke|clear-pending}")
        print("Run 'socis pairing --help' for details.")
...
def _cmd_list(store):
    ...
    if pending:
        ...
        print("\n  Approve with: socis pairing approve <platform> <request-id>")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(
            ...
            "~/.socis-agent/platforms/pairing/_rate_limits.json\n".format(platform)
        )
    else:
        print(f"\n  Pairing request or code '{code}' not found or expired for platform '{platform}'.")
        print("  Run 'socis pairing list' to see pending requests.\n")
