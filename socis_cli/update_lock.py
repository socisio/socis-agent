"""Cross-process mutual exclusion for in-flight SOCIS updates.

Three different surfaces can start an update of the same install tree:

* ``socis update`` from a terminal,
* the dashboard's Update button (``POST /api/socis/update`` →
  ``_spawn_socis_action(["update"])``, detached),
* the desktop's Update button, which hands off to the Tauri
  ``socis-setup --update`` and, on its failure screen, to install-mode
...
once — so a dashboard-spawned ``socis update`` and an installer-driven
...
unchanged and remain byte-compatible with the Rust and Electron readers:
    ...
    <SOCIS_AGENT_HOME>/.socis-update-in-progress   body: "<pid>\\n<started_at_unix>"
...
then spawns ``socis update`` as a child stage. Without a handoff the child
sees its own parent's live marker and refuses — the GUI update deadlocks
against itself on every attempt ("SOCIS is still running", retry forever).
...
* A live holder that is a *process ancestor* of ours is likewise our own
  ...
  ``socis-setup`` binary under ``~/.socis-agent`` is only refreshed by a full
  installer run (``copy_self_to_socis_agent_home`` deliberately no-ops during
  ...
  ancestry check those users get exit 2 ("SOCIS is still running") on every
  GUI update forever, with no SOCIS process actually running.
"""
...
MARKER_NAME = ".socis-update-in-progress"

# Set by an orchestrating updater (the Tauri `socis-setup --update` flow) to
# its own pid before spawning `socis update` as a child stage. The parent
...
HANDOFF_PID_ENV = "SOCIS_AGENT_UPDATE_HANDOFF_PID"
...
# to show "SOCIS is still running" instead of a generic failure. Naming it
...
def update_marker_path() -> Path:
    ...
    Uses the *process* SOCIS home (never the context-local profile override):
    the Rust updater resolves ``$SOCIS_AGENT_HOME`` or the platform default, and the
    ...
    return get_process_socis_agent_home() / MARKER_NAME
...
def _is_ancestor_pid(pid: int) -> bool:
    ...
    The orchestrating updater spawns ``socis update`` as a (grand)child, so a
    ...
    parent chain. This heals the fleet of staged ``socis-setup`` binaries
...
def describe_holder(holder: UpdateHolder) -> str:
    ...
    return (
        f"✗ Another SOCIS update is already running (PID {holder.pid}, "
...
class UpdateLock:
    ...
    def acquire(self) -> bool:
        ...
        updater spawning `socis update` as a stage): we run under ITS claim
