"""Windows ConPTY bridge for the `socis dashboard` chat tab.

Drop-in counterpart to ``socis_cli.pty_bridge.PtyBridge`` for native
Windows. Mirrors the exact public surface the ``/api/pty`` WebSocket
handler in ``socis_cli.web_server`` consumes: ``spawn``, ``read``,
