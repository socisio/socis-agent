...
Add, remove, or reorder entries here — both `socis setup` and
`socis` provider-selection will pick up the change automatically.
"""
...
from socis_cli.urllib_security import open_credentialed_url, url_origin
...
_SOCIS_AGENT_USER_AGENT = f"socis-cli/{_SOCIS_AGENT_VERSION}"
...
def _custom_provider_ssl_context(base_url: str):
    ...
    if not base_url:
        return None
    try:
        from socis_cli.config import get_custom_provider_tls_settings
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `socis model`
    flow without maintaining a separate static list.
    """
    from socis_cli.codex_models import DEFAULT_CODEX_MODELS, _finalize_codex_models
...
# $SOCIS_AGENT_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_merge_curated_extras(ids: list[str]) -> list[str]:
    """Append SOCIS-curated xAI models that are missing from models.dev."""
    out = list(ids)
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $SOCIS_AGENT_HOME/models_dev_cache.json directly (no network). Falls
...
_PROVIDER_MODELS: dict[str, list[str]] = {
    ...
    # endpoint expects (see socis_cli/model_setup_flows.py).
...
def union_with_portal_free_recommendations(
    ...
    *,
    ...
    older SOCIS that doesn't ship that model in its hardcoded curated
...
def union_with_portal_paid_recommendations(
    ...
    *,
    ...
    picker even if the user is running an older SOCIS that doesn't ship
...
def _nous_recommended_disk_path() -> "Path":
    ...
    return get_socis_agent_home() / "cache" / "nous_recommended_cache.json"
...
def fetch_nous_recommended_models(
    ...
    *,
    ...
    (``$SOCIS_AGENT_HOME/cache/nous_recommended_cache.json``) as last-known-good.
...
# this list:  socis model, /model, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `socis model` interactive picker
# ---------------------------------------------------------------------------

class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `socis model` TUI
...
#
# Some vendors expose several SOCIS provider slugs (one per endpoint /
# auth method: global API, China API, OAuth coding plan, ...). Listing every
# slug as a top-level row in the interactive `socis model` / setup wizard /
...
def group_providers(slugs):
    ...
    DISPLAY ONLY. Used by every interactive picker (``socis model``, the
...
# In-repo fallback for the model SOCIS silently lands on when the user never
...
def get_preferred_silent_default_model(provider: str = "openrouter") -> str:
    ...
    """
    try:
        from socis_cli.model_catalog import get_default_model_from_cache
...
# ``socis model``) uses the richer free/paid-tier-aware resolver — see
# ``get_recommended_default_model`` in socis_cli/web_server.py and
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model was ever selected (e.g. ``socis auth add openai-codex`` without
    ``socis model``, or a profile that sets ``provider`` with no ``model``).

    For most providers this is the first entry in ``_PROVIDER_MODELS`` — the
    same model the ``socis model`` picker offers first. For metered aggregators
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    socis-agent is tool-calling-first — every provider path assumes the model
...
# copy, `socis -p`, a cron job, or a freshly booted gateway answers
...
def _reasoning_caps_disk_path() -> Path:
    from socis_agent_constants import get_socis_agent_home
    return get_socis_agent_home() / "cache" / "reasoning_caps.json"
...
def _fetch_reasoning_caps_catalog(
    ...
    """
    headers = {"Accept": "application/json", "User-Agent": _SOCIS_AGENT_USER_AGENT}
    try:
...
def refresh_reasoning_caps_async(provider: Optional[str]) -> None:
    ...
    refresh = {
        ...
        "socis": _refresh_nous_reasoning_caps,
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from socis_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — socis-agent
        ...
        if preferred_id == silent_default:
            ...
            # picker shows which model SOCIS lands on when none is selected.
            desc = "default"
        else:
...
def get_curated_nous_model_ids() -> list[str]:
    ...
    """
    try:
        from socis_cli.model_catalog import get_curated_nous_models
...
def fetch_ai_gateway_models(
    ...
    *,
    ...
    from socis_agent_constants import AI_GATEWAY_BASE_URL
...
def fetch_ai_gateway_pricing(
    ...
    *,
    ...
    """Fetch Vercel AI Gateway /v1/models and return socis-shaped pricing.

    Vercel uses ``input`` / ``output`` field names; socis's picker expects
...
def nous_policy_allowed_ids(*, force_refresh: bool = False) -> Optional[set[str]]:
    ...
    """
    try:
        from socis_cli.nous_account import nous_policy_present
...
def _configured_custom_provider_ids() -> set[str]:
    ...
    try:
        ...
        from socis_cli.providers import custom_provider_slug
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``socis model``, ``/model``, etc.).
    """
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from socis_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def probe_ollama_local_models(
    ...
    try:
        ...
        request_headers = {"User-Agent": _SOCIS_AGENT_USER_AGENT}
...
def _resolve_static_model_alias(
    ...
    try:
        from socis_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def _resolve_provider_prefix(model_name: str) -> Optional[tuple[str, str]]:
    ...
    ``TestDenormalizeProviderSwitch`` in tests/socis_cli/test_web_server.py).
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to SOCIS' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``socis_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether SOCIS should expose the /fast toggle for this model."""
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``socis auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``~/.socis-agent/.env``.
...
def _model_dedup_key(model_id: str) -> str:
    ...
    try:
        from socis_cli.model_search import model_alias_canonical
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    on the platform appear in ``/model`` without a SOCIS release.
    """
    requested = str(provider or "").strip().lower()
    if requested == "ollama":
        if force_refresh:
    ...
    if normalized == "openai-codex":
        from socis_cli.codex_models import get_codex_model_ids
        ...
        # a SOCIS release). Falls back to the hardcoded catalog if no token
    ...
    if normalized == "nous":
        ...
        try:
            from socis_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
            creds = resolve_nous_runtime_credentials()
            if creds:
        ...
        # added Portal models still surface without a SOCIS release.
    ...
    if normalized in ("openai", "openai-api"):
        ...
        if api_key:
            ...
            # ``socis model`` shows.
            from socis_cli.providers import is_official_openai_host
...
#
# Cache strategy:
#   - One JSON file at $SOCIS_AGENT_HOME/provider_models_cache.json
...
def _provider_models_cache_path() -> Path:
    from socis_agent_constants import get_socis_agent_home
    return get_socis_agent_home() / "provider_models_cache.json"
...
def _credential_fingerprint(provider: str) -> str:
    ...
    relevant tokens live in ``$SOCIS_AGENT_HOME/auth.json`` and external
    ...
    # env var, so it must change the fingerprint too or `socis config set
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    headers = {"User-Agent": _SOCIS_AGENT_USER_AGENT}
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: SOCIS' default Claude IDs elsewhere use
...
def copilot_model_api_mode(
    ...
    *,
    ...
    # endpoint, not through SOCIS' native Anthropic adapter. The live catalog may
...
def _fetch_opencode_free_models(
    ...
    from socis_cli.urllib_security import open_credentialed_url
...
def probe_api_models(
    ...
    tried: list[str] = []
    headers: dict[str, str] = {"User-Agent": _SOCIS_AGENT_USER_AGENT}
    if urllib.parse.urlparse(normalized).hostname == "generativelanguage.googleapis.com":
        headers["X-Goog-Api-Client"] = f"socis-agent/{_SOCIS_AGENT_VERSION}"
...
_DEEPINFRA_MODELS_QUERY = "filter=true&sort_by=socis"
...
def _ollama_cloud_cache_path() -> Path:
    ...
    from socis_agent_constants import get_socis_agent_home
    return get_socis_agent_home() / "ollama_cloud_models_cache.json"
    ...
    *,
    ...
    if normalized == "moa":
        try:
            ...
            return {
                ...
                "message": f"MoA preset `{requested}` was not found. Run `socis moa list`.",
            }
        except Exception as exc:
            return {
                "accepted": False, "persist": False, "recognized": False,
        ...
        if ollama_models is None:
            return {
                ...
                "message": (
                    ...
                    "SOCIS will save the model name, but local Ollama model discovery could not verify it."
                ),
            }
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"SOCIS will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
    ...
    if normalized in {"openai-codex", "xai-oauth"}:
        ...
        # BEFORE the hidden-slug soft-accept below: the suffix is a SOCIS
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so SOCIS cannot verify the model name."
    ...
    if api_models is not None:
        ...
        if _variant_base is not None and _variant_base in set(api_models):
            ...
            # fetch) so we only accept models SOCIS actually ships.  (#46850)
            #
            ...
            # `socis chat` already accepts these models via
