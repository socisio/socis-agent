...
SOCIS is developed on Linux / macOS and tested natively on Windows too.
...
_WINDOWS_GATEWAY_BREAKAWAY_ENV = "_SOCIS_AGENT_GATEWAY_BREAKAWAY"
...
# Electron exits, so the gateway never gets respawned after a `socis
...
def suppress_platform_ver_console() -> None:
    ...
    *visible* console: one flashing ``cmd`` window per process, triggered by
    ...
    during a dependency's import, not from SOCIS' own code.
    """
    if not IS_WINDOWS:
        return
    try:
...
def noninteractive_git_env(
    ...
    SOCIS shells out to git from many non-interactive contexts — MCP catalog
    ...
    * isolated git config — inherited ``GIT_CONFIG_*`` overrides, global/system
      ...
      hang or mutate SOCIS's internal plumbing calls.
...
def _text_names_socis(text: str) -> bool:
    """True when *text* names SOCIS at a path-segment / token boundary.

    A bare ``"socis" in text`` substring test would also match unrelated
    processes whose paths merely contain the letters (``...\\ssocisa\\...``),
...
def _process_command_is_socis(pid: int) -> bool:
    """Best-effort check that *pid* currently runs SOCIS code."""
    try:
        import psutil
        ...
        return _text_names_socis(f"{command} {executable}")
    except Exception:
        return False
...
def pid_is_socis(
    ...
    *,
    ...
    The PID must be valid, currently exist, and identify a SOCIS process. When
    ...
    if (
        ...
        return False
    try:
        return _process_command_is_socis(pid)
    except Exception:
        return False
...
def _legacy_kill_process_tree(proc: "subprocess.Popen") -> None:
    ...
    if IS_WINDOWS:
        ...
        # ``pid_is_socis`` guard is for BARE pids from state files or process
...
def bounded_probe_run(
    ...
    *,
    ...
    scan hit exactly this during ``socis update`` on slow-WMI machines
