"""Install and remove the Linux desktop entry (``socis.desktop``).

``socis desktop`` builds and launches the Electron app. On Linux, a
...
DESKTOP_ENTRY_NAME = "socis.desktop"
...
def desktop_entry_path() -> Path:
    """Where the ``socis.desktop`` entry lives."""
...
def _can_import_socis_cli(interpreter: Path) -> bool:
    """Whether *interpreter* can import ``socis_cli.main`` unaided.
...
def _running_interpreter_fallback() -> str:
    ...
    The RUNNING interpreter by definition has ``socis_cli`` importable
...
def resolve_exec_command(project_root: Optional[Path] = None) -> str:
    """Build the absolute ``Exec=`` command line for ``socis desktop``.

    Prefer the real ``socis`` executable (argv[0] or PATH). When SOCIS
    ...
    bytes. ``resolve_socis_bin()`` prefers ``sys.argv[0]``, which differs
    ...
    """
    from socis_cli.relaunch import resolve_socis_bin

    bin_path = _resolve_socis_bin_for_desktop_entry(
        resolve_socis_bin, checkout_root=project_root
    )
    interpreter = _running_interpreter()
    if not _can_import_socis_cli(Path(interpreter)):
        # The candidate interpreter cannot actually import socis_cli.main
    ...
    if bin_path:
        ...
        if _needs_interpreter(resolved):
            ...
            # a NON-venv interpreter (e.g. the repo's `socis` script with
            ...
            # sys.executable is the interpreter actually running SOCIS (the
...
def _resolve_socis_bin_for_desktop_entry(
    resolve_fn=None,
    checkout_root: Optional[Path] = None,
) -> Optional[str]:
    """Resolve the launcher binary for the persisted ``.desktop`` entry.

    Wraps :func:`socis_cli.relaunch.resolve_socis_bin` with one
    desktop-entry-specific rule: an ``argv[0]`` that points inside this
    checkout is a launch-context artifact (the repo ``socis`` script the
    ...
    if resolve_fn is None:
        from socis_cli.relaunch import resolve_socis_bin as resolve_fn
    ...
    def _inside_checkout(candidate: str) -> bool:
        try:
            ...
            return False
        # The repo `socis` script and anything else shipped in the tree is
        ...
        for root in {checkout_root, resolved_root}:
            if root is not None and (path == root or root in path.parents):
                return True
        # The `python -m socis_cli.main` relaunch context surfaces the
    ...
    # interpreter is an external launcher (e.g. /opt/.../bin/socis from
    ...
    if rerouted is None and primary:
        # argv[0] was checkout-internal AND PATH had no `socis` — common
...
def _wrapper_targets_checkout(wrapper: Path, checkout_root: Path) -> bool:
    ...
    ``<checkout>/venv/bin/python <checkout>/socis``; a venv console
    ...
    for root in roots:
        ...
        # <root>/socis …): a path-continuation boundary is also a match.
        if root + "/" in text:
            return True
    return False
...
def _known_wrapper_candidates():
    ...
    (``$PREFIX/bin``). The wrapper is always named ``socis``.
    """
    candidates = []
    home = Path.home()
    prefix = os.environ.get("PREFIX")
    if prefix:
        candidates.append(Path(prefix) / "bin" / "socis")
    if hasattr(os, "geteuid") and os.geteuid() == 0:
        candidates.append(Path("/usr/local/bin/socis"))
    candidates.append(home / ".local" / "bin" / "socis")
    return candidates


def _project_root() -> Path:
    """This file lives at ``<checkout>/socis_cli/linux_desktop_entry.py``.
...
def _needs_interpreter(bin_path: Path) -> bool:
    ...
    ``sys.executable`` to see SOCIS' venv (rather than its own shebang)."""
    try:
...
def render_desktop_entry(exec_command: str, icon: str) -> str:
    return (
        ...
        "Type=Application\n"
        "Name=SOCIS\n"
        "GenericName=SOCIS Agent Desktop\n"
        "Comment=Launch SOCIS Agent Desktop\n"
        f"Exec={exec_command}\n"
        f"Icon={icon}\n"
        "Terminal=false\n"
        "Categories=Utility;\n"
        "StartupNotify=true\n"
        "StartupWMClass=SOCIS\n"
    )
...
def _hicolor_icon_dest(subdir: str) -> Path:
    return _xdg_data_home() / "icons" / "hicolor" / subdir / "apps" / "socis.png"
...
def _install_icon_to_hicolor(icon: Path) -> bool:
    ...
    The freedesktop icon lookup finds an installed ``apps/socis.png``
    by the unqualified name ``socis``, so the entry can reference the
...
def install_desktop_entry(project_root: Path) -> Optional[Path]:
    """Write (or refresh) the SOCIS desktop entry. Return its path.
    ...
    icon_value = str(icon) if icon.is_file() else "socis"
