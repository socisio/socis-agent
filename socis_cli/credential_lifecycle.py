"""Unified provider-credential lifecycle across every store SOCIS reads.

A provider API key can live in up to THREE stores at once:

    1. ``~/.socis-agent/.env``                     — the canonical secret store
    2. ``~/.socis-agent/auth.json`` →
       ``credential_pool.<provider>[*]``      — env-seeded pool entries
       (``source == "env:<VAR>"``) persisted by the pool loader
    3. ``~/.socis-agent/config.yaml``              — inline mirrors written by the
...
def purge_env_credential_references(
    ...
    # Make the removal sticky the same way `socis auth remove` does: a
...
def save_provider_env_credential(env_var: str, value: str) -> Dict[str, Any]:
    ...
    re-add through the UI behaves like ``socis auth add``.
    ...
    ran ``socis auth add <provider> --type api-key`` separately. This makes
    the Desktop save's effect on disk match what ``socis auth add`` does.
    """
    from socis_cli.config import load_env, save_env_value
    ...
    # that reads this var.
    try:
        from socis_cli.auth import unsuppress_credential_source
...
def remove_provider_env_credential(env_var: str) -> Dict[str, Any]:
    ...
    """
    from socis_cli.config import load_env, remove_env_value
