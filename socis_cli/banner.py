...
Pure display functions with no SOCISCLI state dependency.
"""
import json
import logging
import os
...
_CORAL = "\033[1;38;2;255;51;102m"  # True-color #FF3366 bold
_BOLD = "\033[1m"
_DIM = "\033[2m"
_RST = "\033[0m"
...
def cprint(text: str):
    ...
    try:
        ...
        # absent stdout (pythonw.exe, CI, `socis ... > file`) raises
...
from socis_cli import __version__ as VERSION, __release_date__ as RELEASE_DATE
...
# (e.g. nix-built socis — no local git history to count against).
UPDATE_AVAILABLE_NO_COUNT = -1

_UPSTREAM_REPO_URL = "https://github.com/socisio/socis-agent.git"
_OFFICIAL_REPO_CANONICAL = "github.com/socisio/socis-agent"
    ...
    try:
        ...
        req = urllib.request.Request(
            ...
            headers={
                ...
                "User-Agent": "socis-cli-update-check",
            },
        )
...
def _check_via_local_git(repo_dir: Path) -> Optional[int]:
    ...
    if _is_official_ssh_remote(origin_url):
        ...
        # ahead checkout as behind nudges the user into `socis update`,
...
def check_for_updates() -> Optional[int]:
    """Check whether a SOCIS update is available.

    Two paths: if ``SOCIS_AGENT_REVISION`` is set (nix builds embed it), compare
    ...
    cache_file = socis_agent_home / ".update_check"
    embedded_rev = os.environ.get("SOCIS_AGENT_REVISION") or None

    # Docker images have no working tree to count commits against — the
    # published image excludes `.git` (see .dockerignore) and sets no
    # SOCIS_AGENT_REVISION (that's nix-only). Returning None makes both the Rich
    # banner (build_welcome_banner) and the Ink badge (branding.tsx, guarded
    # on `typeof === 'number' && > 0`) show nothing. The dashboard's REST
    # `/api/socis/update/check` endpoint short-circuits docker the same way
    # (web_server.py); mirror that here so the banner/TUI surfaces agree.
    try:
        from socis_cli.config import detect_install_method, get_project_root
    ...
    if embedded_rev:
        ...
        # $SOCIS_AGENT_HOME/socis-agent/ may be a stale copy from --clone-all;
        # Path(__file__) always resolves to the actual installed checkout.
        repo_dir = Path(__file__).parent.parent.resolve()
        if not (repo_dir / ".git").exists():
            repo_dir = socis_agent_home / "socis-agent"
        if not (repo_dir / ".git").exists():
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active SOCIS git checkout, or None if this isn't a git install.

    Prefers the running code's location over the profile-scoped path
    because ``$SOCIS_AGENT_HOME/socis-agent/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``socis_cli/build_info.py``) and return it as a frozen
...
def _compute_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    if repo_dir is None:
        ...
        try:
            from socis_cli.build_info import get_build_sha
...
_RELEASE_URL_BASE = "https://github.com/socisio/socis-agent/releases/tag"
...
def format_banner_version_label() -> str:
    ...
    base = f"SOCIS Agent v{VERSION} ({RELEASE_DATE})"
...
def _format_update_notice(behind: int) -> str:
    ...
    from socis_cli.config import get_managed_update_command, recommended_update_command
    if behind > 0:
    ...
    # UPDATE_AVAILABLE_NO_COUNT: nix-built socis; we know an update
...
def _banner_snapshot_path() -> Path:
    return get_socis_agent_home() / "cache" / "banner_snapshot.json"
...
def banner_snapshot_fingerprint() -> Optional[str]:
    ...
    try:
        ...
        for p in (get_config_path(), get_socis_agent_home() / ".env"):
            try:
                st = p.stat()
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    accent = _skin_color("banner_accent", "#F04162")
    dim = _skin_color("banner_dim", "#A81D3E")
    text = _skin_color("banner_text", "#FFD9E1")
    ...
    try:
        ...
        _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else SOCIS_AGENT_CADUCEUS
    except Exception:
        _bskin = None
        _hero = SOCIS_AGENT_CADUCEUS
    left_lines = ["", _hero, ""]
    if (provider or "").strip().lower() == "moa":
        ...
        left_lines.append(f"[{accent}]MoA: {preset_name}[/]{agg_str}{ctx_str} [dim {dim}]·[/] [dim {dim}]SOCIS[/]")
    else:
        if not (model or "").strip() or (model or "").strip().lower() == "unknown":
            ...
            left_lines.append(
                ...
                f"[dim {dim}]— run /model or socis setup[/]"
            )
        else:
            ...
            ctx_str = f" [dim {dim}]·[/] [dim {dim}]{_format_context_length(context_length)} context[/]" if context_length else ""
            left_lines.append(f"[{accent}]{model_short}[/]{ctx_str} [dim {dim}]·[/] [dim {dim}]SOCIS[/]")

    if os.getenv("SOCIS_AGENT_YOLO_MODE"):
    ...
    if not _has_native_mcp:
        try:
            from socis_cli.plugins import get_portable_mcp_server_names_nowait
    ...
    try:
        from socis_cli.codex_runtime_switch import get_current_runtime
        from socis_cli.config import load_config as _load_cfg
    ...
    title_color = _skin_color("banner_title", "#FF3366")
    border_color = _skin_color("banner_border", "#C42248")
    ...
    if term_width >= 95:
        _logo = _bskin.banner_logo if _bskin and hasattr(_bskin, 'banner_logo') and _bskin.banner_logo else SOCIS_AGENT_AGENT_LOGO
        console.print(_logo)
        console.print()
