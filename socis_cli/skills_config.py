"""
Skills configuration for SOCIS Agent.
`socis skills` enters this module.

Toggle individual skills or categories on/off, globally or per-platform.
Config stored in ~/.socis-agent/config.yaml under:
...
from socis_cli.config import cfg_get, load_config, save_config
from socis_cli.colors import Colors, color
from socis_cli.platforms import PLATFORMS as _PLATFORMS
...
def save_disabled_skills(config: dict, disabled: Set[str], platform: Optional[str] = None):
    ...
    Essential skills (e.g. ``socis-agent``) are silently dropped from the
