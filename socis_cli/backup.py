"""
Backup and import commands for socis CLI.

`socis backup` creates a zip archive of the entire ~/.socis-agent/ directory
(excluding the socis-agent repo and transient files).

`socis import` restores from a backup zip, overlaying onto the current
SOCIS_AGENT_HOME root.
"""
...
from socis_cli.sizefmt import format_bytes as _format_size
...
# Where ``socis backup --quick`` / ``/snapshot`` / the pre-update safety net
...
# ``socis-agent`` is special-cased to root level only in ``_should_exclude``
# so that skill directories like ``skills/autonomous-ai-agents/socis-agent/``
...
# SOCIS_AGENT_HOME gets walked file-by-file, ballooning a backup to hundreds of
...
_EXCLUDED_DIRS = {
    "socis-agent",     # the codebase repo — re-clone instead
    ...
    "browser-profile",
    # Python dependency trees (plugin / MCP-server venvs under SOCIS_AGENT_HOME) —
...
# SOCIS-managed runtime downloads that only exist at the top of a profile
...
# ONLY at the root of SOCIS_AGENT_HOME and at ``profiles/<name>/`` — a deeper
...
def _in_excluded_root_dir(rel_path: Path) -> bool:
    """True when *rel_path* (relative to SOCIS_AGENT_HOME) is, or sits inside, a
    SOCIS-managed runtime tree at the top of a profile home."""
...
# ``state.db.pre-update-emergency-<timestamp>.bak`` at the SOCIS_AGENT_HOME root
...
# File names that ``socis import`` must never overwrite, matched by basename so
...
# Reserved archive subtree for provider state that lives OUTSIDE SOCIS_AGENT_HOME
...
class BackupInProgressError(RuntimeError):
    """Raised when another process already owns the SOCIS backup slot."""
...
@contextmanager
def _backup_operation_lock(socis_agent_home: Path, timeout_seconds: float = 0.25):
    """Acquire one cross-process backup slot for full and quick snapshots."""
    lock_path = socis_agent_home / ".backup.lock"
            ...
            while True:
                try:
                    ...
                    if time.monotonic() >= deadline:
                        raise BackupInProgressError("another SOCIS backup is already running")
                    time.sleep(0.05)
        else:
            import fcntl
...
def _collect_memory_provider_external_paths() -> List[Path]:
    ...
    outside SOCIS_AGENT_HOME, resolved from config only (no network, no init).
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to socis root) should be skipped."""
    parts = rel_path.parts
    ...
    for part in parts:
        if part not in _EXCLUDED_DIRS:
            continue
        # ``socis-agent`` only matches at the root level (first component).
        # Nested directories with the same name — e.g.
        # ``skills/autonomous-ai-agents/socis-agent/`` — must be preserved.
        if part == "socis-agent" and part != parts[0]:
            continue
        return True
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside SOCIS_AGENT_HOME.
...
def _iter_backup_files(
    ...
    never descends a multi-GB excluded tree), the root-only ``socis-agent``
    carve-out, profile-home-root runtime trees, and the per-file exclusion
    rules — shared by the manual ``socis backup`` path and the automatic
    ...
    """
    for dirpath, dirnames, filenames in os.walk(socis_root, followlinks=False):
        rel_dir = Path(dirpath).relative_to(socis_root)

        # ``socis-agent`` is only pruned at the root level; nested dirs
        ...
        dirnames[:] = [
            d for d in dirnames
            if (d not in _EXCLUDED_DIRS or (d == "socis-agent" and not is_root))
...
# to the user as a hung `socis update` (#70553 follow-up). Sessions databases
...
def verify_sqlite_integrity(
    ...
    *,
    ...
    if check_header:
        ...
        # socis_cli.sqlite_safe_read). Verification targets snapshots and
    ...
    try:
        ...
        try:
            ...
            logger.error(
                ...
                "database handles (or restart SOCIS) and retry.",
                dst, exc2,
            )
            return False
...
def run_backup(args) -> None:
    """Create a zip backup of the SOCIS home directory."""
    socis_root = get_default_socis_root()

    if not socis_root.is_dir():
        print(f"Error: SOCIS home directory not found at {socis_root}")
        sys.exit(1)

    try:
        with _backup_operation_lock(socis_root):
            _run_backup_locked(args, socis_root)
    except BackupInProgressError as exc:
        print(f"Error: {exc}")
        raise SystemExit(2) from exc


def _run_backup_locked(args, socis_root: Path) -> None:
    ...
    try:
        if args.output:
            ...
            if out_path.is_dir():
                ...
                out_path = out_path / f"socis-backup-{stamp}.zip"
        else:
            stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
            out_path = Path.home() / f"socis-backup-{stamp}.zip"
    ...
    print(f"Scanning {display_socis_agent_home()} ...")
    skipped_dirs: set = set()
    files_to_add: list[tuple[Path, Path]] = list(
        _iter_backup_files(socis_root, out_path, skipped_dirs)
    )

    # External memory-provider state (e.g. ~/.honcho, ~/.hindsight) lives
    # outside SOCIS_AGENT_HOME, so the walk above never sees it. Ask the active
    ...
    if external_to_add:
        print(
            ...
            f"stored outside {display_socis_agent_home()}."
        )
    ...
    if not errors:
        print(f"\nRestore with: socis import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a SOCIS backup.
    ...
    # Look for telltale files that a socis home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a SOCIS backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.socis-agent/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a socis dir name
        if prefix in {".socis-agent", "socis"}:
            return prefix + "/"
...
def _extract_member_atomically(
    ...
    with nothing behind it — during ``socis import``, which is the
    ...
    so without the chown a ``sudo socis import`` would silently re-own every
    ...
    if mode is None:
        ...
        # take over an existing setuid/setgid file, so ``socis import`` would
        # hand whoever produced the zip the identity that file runs as.  Nothing
        # constrains that to SOCIS' own state either: the ``_external/`` branch
...
def _count_session_rows(path: Path) -> Optional[Tuple[int, int]]:
    ...
    database that is not a SOCIS session store, or one that cannot be read.
...
def _import_db_member(
    ...
    ``socis import`` is the disaster-recovery path, so that failure mode lands
...
def run_import(args) -> None:
    ...
    # same path printed as "Target:" via display_socis_agent_home(). Resolving
    # through get_default_socis_root() instead maps a profile home
    # (<root>/profiles/<name>) back to <root>, silently retargeting the
    # restore at the live root while the profile directory stays empty.
    socis_root = get_socis_agent_home()
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_socis_agent_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (socis_root / "config.yaml").exists()
        has_env = (socis_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has SOCIS configuration.")
        ...
        for member in members:
            ...
            # location (e.g. ~/.honcho/config.json), NOT under SOCIS_AGENT_HOME.
            ...
            target = socis_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(socis_root.resolve())
            except ValueError:
        ...
        if restored_external:
            print(
                ...
                f"their original location(s) outside {display_socis_agent_home()}."
            )
        ...
        profiles_dir = socis_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # socis_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print("\n  Profiles detected but aliases could not be created.")
                    print("  Run: socis profile list  (after installing socis)")

        # Guidance
        print()
        if not (socis_root / "socis-agent").is_dir():
            print("Note: The socis-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: socis update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  socis -p {pname} gateway install")
        ...
        native_default = _get_platform_default_socis_agent_home()
        ...
        if socis_root != native_default and default_has_install:
            print(
                "\nRestored into a non-default home; leaving the gateway service "
                "alone to avoid clashing with the install at "
                f"{native_default}."
            )
            print("To start a gateway for this home, run:  socis gateway install")
        else:
            try:
                from socis_cli.gateway import ensure_gateway_service, _is_service_running
        ...
        print("Done. Your SOCIS configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and socis backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to SOCIS_AGENT_HOME).
...
# — `socis update` snapshots this set before pulling so approved-user lists
...
_QUICK_STATE_FILES = (
    ...
    # are at $SOCIS_AGENT_HOME/<name> for the default/root profile; on non-root
    # profiles the real path is outside SOCIS_AGENT_HOME and the entry is silently
...
def _quick_snapshot_root(socis_agent_home: Optional[Path] = None) -> Path:
...
def _create_quick_snapshot_locked(
    ...
    if failed_dbs:
        ...
        print(
            ...
            f"{root} and run: socis snapshot list"
        )
        logger.error(
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(socis_agent_home)
    if not root.exists():
        return []
...
# Relative path of the cron job database inside SOCIS_AGENT_HOME. Kept in sync with
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``socis update``.
    ...
    Args:
        ...
        socis_agent_home: Override for the SOCIS home directory (tests).
...
def create_pre_update_snapshots_all_profiles(
    ...
    home = invoking_home or get_socis_agent_home()
...
def restore_config_model_settings_if_rewritten(
    ...
    """Safety net for silent config.yaml model/MoA loss across ``socis update``.
    ...
    Args:
        snapshot_id: The pre-update quick-snapshot id (from
            :func:`create_quick_snapshot`).
        socis_agent_home: Override for the SOCIS home directory (tests/siblings).
    ...
    for name, snap_id in profile_snapshots.items():
        ...
        try:
            result = restore_config_model_settings_if_rewritten(
                snap_id, socis_agent_home=profile_home
            )
        except Exception as exc:
            logger.debug(
...
def restore_cron_jobs_all_profiles(
    ...
    for name, snap_id in profile_snapshots.items():
        ...
        if profile_home is None:
            continue
        try:
            result = restore_cron_jobs_if_emptied(snap_id, socis_agent_home=profile_home)
        except Exception as exc:
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(socis_agent_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for socis backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_socis_agent_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, socis_root: Path) -> Optional[Path]:
    """Single-flight wrapper for automatic full zip backups."""
    try:
        with _backup_operation_lock(socis_root):
            return _write_full_zip_backup_locked(out_path, socis_root)
    except BackupInProgressError as exc:
        logger.warning("Full-zip backup skipped: %s", exc)
        return None


def _write_full_zip_backup_locked(out_path: Path, socis_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``socis_root`` to ``out_path``.
    ...
    try:
        files_to_add = list(_iter_backup_files(socis_root, out_path))
    except OSError as exc:
...
def _pre_update_backup_dir(socis_agent_home: Optional[Path] = None) -> Path:
    home = socis_agent_home or get_socis_agent_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of SOCIS_AGENT_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<SOCIS_AGENT_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``socis update``) should continue even if the backup fails.
    """
    socis_root = socis_agent_home or get_default_socis_root()
    if not socis_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(socis_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, socis_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `socis claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of SOCIS_AGENT_HOME under ``backups/`` before a
    ``socis claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``socis import <archive>``.  Writes to
    ``<SOCIS_AGENT_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `socis import` and the
