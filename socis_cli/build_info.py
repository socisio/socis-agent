"""
Baked-in build metadata for SOCIS Agent.

Source installs report their git revision live via ``git rev-parse`` (see
``socis_cli/dump.py`` and ``socis_cli/banner.py``).  That doesn't work inside
...
To make ``socis dump`` and the startup banner identify the exact commit the
image was built from, the Docker build writes the build-time ``$SOCIS_AGENT_GIT_SHA``
arg into ``<project_root>/.socis_build_sha``.  This module is the single
...
- Returns ``None`` when the file is absent.  Source installs and dev images
  built without the ``SOCIS_AGENT_GIT_SHA`` build-arg fall through to live-git
...
_BUILD_SHA_FILE = Path(__file__).parent.parent / ".socis_build_sha"
...
def get_code_identity(refresh: bool = False) -> dict:
    ...
    rev-parse`` for source installs, the baked ``.socis_build_sha`` for
...
def get_build_sha(short: int = 8) -> Optional[str]:
    ...
    Reads ``<project_root>/.socis_build_sha`` if present.  The file is
    written by the Dockerfile's ``SOCIS_AGENT_GIT_SHA`` build-arg and contains
