"""``socis worktree`` — audit and reclaim accumulated git worktrees/branches.

Attended counterpart of the silent startup pruner (see
``socis_cli/worktree_gc.py`` for the policy and shared invariants). Usage:

    socis worktree list             # audit: verdict + reason per tree
    socis worktree prune            # reap safe trees + merged branches
    socis worktree prune --dry-run  # show the plan, change nothing
    socis worktree prune --trees-only / --branches-only
"""
...
def cmd_worktree(args) -> int:
    ...
    if action == "list":
        ...
        print(
            ...
            f"{_fmt_size(reapable_mb)} reclaimable now via `socis worktree prune`."
        )
