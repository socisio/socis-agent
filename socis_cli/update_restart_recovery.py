...
- **gateway profiles**, relaunched through the existing per-profile
  ``socis_cli.main -p <profile> gateway restart`` command; and
- **``socis-serve*`` systemd units**, restarted directly through
  ``systemctl``.  ``socis serve`` is not a gateway profile and has no
  ...
  inventory, so a manually launched or Desktop-owned ``socis serve`` — which
  has no relaunch authority — can never enter this path.

Serve-unit identity is always ``<scope>/<unit>`` (``user/socis-serve``,
``system/socis-serve``).  The two managers can each own a unit of the same
...
_RECOVERY_ENV = "SOCIS_AGENT_UPDATE_RESTART_RECOVERY"
_GATEWAY_MARKERS = ("_SOCIS_AGENT_GATEWAY", "SOCIS_AGENT_GATEWAY", "SOCIS_AGENT_GATEWAY_MODE")
_PROFILE_RESTART_TIMEOUT = 90
_VERIFY_TIMEOUT = 15
_PROFILE_ID_RE = re.compile(r"^[a-z0-9][a-z0-9_-]{0,63}$")
_SUPERVISOR_RE = re.compile(r"^[a-z0-9][a-z0-9_-]{0,31}$")
_UNIT_RE = re.compile(r"^socis-serve(-[a-z0-9][a-z0-9_-]{0,63})?\.service$")
_SERVE_UNIT_PATTERN = "socis-serve*"
...
def _systemd_unit_candidates(profile: str) -> tuple[str, ...]:
    ...
    return (
        f"socis-gateway-{profile}.service",
...
def _systemctl_scopes() -> list[tuple[str, list[str]]]:
    ...
    any SOCIS platform helper — importing the freshly pulled tree is exactly
    what aborted the phase that called us.

    Each scope is returned with its label because ``socis-serve.service`` in
    the user manager and ``socis-serve.service`` in the system manager are
...
def _listed_serve_units(scope: list[str], *, run: Callable[..., Any]) -> list[str]:
    ...
    for line in (getattr(result, "stdout", "") or "").splitlines():
        ...
        if not parts:
            continue
        # The glob is a systemd pattern, not a name gate: `socis-serve*` also
        # matches the unrelated `socis-server.service`. Require the exact
...
def _normalized_skips(
    ...
    Qualified entries (``{"scope": "user", "unit": "socis-serve"}`` or the
    equivalent ``"user/socis-serve"`` string) suppress exactly one process.

    A bare ``"socis-serve"`` carries no scope and therefore cannot say WHICH
...
def restart_serve_units(
    *,
    ...
    """Restart every active ``socis-serve*`` systemd unit from this process.

    ``socis serve`` hosts ``tui_gateway.server`` and is restarted by the
    ...
    part-way — systemd lists ``socis-gateway.service`` before
    ``socis-serve.service``, so the gateway is typically already done — the
    ...
    or Desktop-owned ``socis serve`` owns no unit and therefore cannot be
    touched here.

    Every identity in and out of this function is scope-qualified. The user
    manager and the system manager can each own a ``socis-serve.service``,
...
def _parse_payload(stream) -> tuple[list[str], dict[str, str], bool, list[str]]:
    ...
    if raw_serve is not None:
        ...
        for entry in raw_skip:
            ...
            # shape carries the systemd scope, because `socis-serve.service`
