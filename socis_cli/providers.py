"""
Single source of truth for provider identity in SOCIS Agent.
...
2. **SOCIS overlays** — transport type, auth patterns, aggregator flags,
...
# -- SOCIS overlay ----------------------------------------------------------
# SOCIS-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class SOCISOverlay:
    """SOCIS-specific provider metadata layered on top of models.dev."""
...
SOCIS_AGENT_OVERLAYS: Dict[str, SOCISOverlay] = {
    "moa": SOCISOverlay(
        transport="openai_chat",
        auth_type="virtual",
        base_url_override="moa://local",
    ),
    "openrouter": SOCISOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "openai-codex": SOCISOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "openai-api": SOCISOverlay(
        transport="codex_responses",
        base_url_override="https://api.openai.com/v1",
        base_url_env_var="OPENAI_BASE_URL",
    ),
    "xai-oauth": SOCISOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://api.x.ai/v1",
        base_url_env_var="XAI_BASE_URL",
    ),
    "qwen-oauth": SOCISOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="SOCIS_AGENT_QWEN_BASE_URL",
    ),
    "lmstudio": SOCISOverlay(
        transport="openai_chat",
        auth_type="api_key",
    ...
    "copilot-acp": SOCISOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": SOCISOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": SOCISOverlay(
    ...
    "kimi-for-coding": SOCISOverlay(
        transport="openai_chat",
        base_url_env_var="KIMI_BASE_URL",
    ),
    "stepfun": SOCISOverlay(
        transport="openai_chat",
        extra_env_vars=("STEPFUN_API_KEY",),
        base_url_override="https://api.stepfun.ai/step_plan/v1",
        base_url_env_var="STEPFUN_BASE_URL",
    ),
    "minimax": SOCISOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_BASE_URL",
    ),
    "minimax-oauth": SOCISOverlay(
        transport="anthropic_messages",
        auth_type="oauth_external",
        base_url_override="https://api.minimax.io/anthropic",
    ),
    "minimax-cn": SOCISOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": SOCISOverlay(
        transport="openai_chat",
        base_url_env_var="DEEPSEEK_BASE_URL",
    ),
    "alibaba": SOCISOverlay(
        transport="openai_chat",
        base_url_env_var="DASHSCOPE_BASE_URL",
    ),
    "alibaba-coding-plan": SOCISOverlay(
        transport="openai_chat",
    ...
    "opencode": SOCISOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": SOCISOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_GO_BASE_URL",
    ),
    "opencode-free": SOCISOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "tencent-tokenhub": SOCISOverlay(
        transport="openai_chat",
        base_url_env_var="TOKENHUB_BASE_URL",
    ),
    "tencent-tokenplan": SOCISOverlay(
        transport="anthropic_messages",
    ...
    "azure-foundry": SOCISOverlay(
        transport="openai_chat",  # default; overridden by api_mode in config
        base_url_env_var="AZURE_FOUNDRY_BASE_URL",
    ),
    "bedrock": SOCISOverlay(
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "socis", "user-config"
...
def get_provider(name: str, *, allow_network: bool = True) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. SOCIS overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + SOCIS overlay
    ...
    overlay = SOCIS_AGENT_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + socis extra
    ...
    if overlay is not None:
        # SOCIS-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
    ...
    # tencent-tokenhub) are absent from models.dev and SOCIS_AGENT_OVERLAYS, so
...
def determine_api_mode(provider: str, base_url: str = "", model: str = "") -> str:
    ...
    *model* is optional but required for dual-wire providers (Nous) whose
    ...
    # chat_completions. The SOCIS overlay still advertises openai_chat
    ...
    # Direct provider checks for providers not in SOCIS_AGENT_OVERLAYS
    if provider == "bedrock":
        return "bedrock_converse"
...
def resolve_provider_full(
    ...
    # 0.5 Exact SOCIS provider IDs must win over LOSSY alias collapsing.
    ...
    if canonical != raw:
        try:
            from socis_cli.auth import PROVIDER_REGISTRY as _AUTH_PROVIDER_REGISTRY
