...
Pure logic: walks a SOCIS config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `socis doctor` and a future `socis migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a SOCIS config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a SOCIS config and return retirement issues.
...
def apply_migration(
    ...
    # None``, so the next `socis migrate xai` reports nothing to migrate
