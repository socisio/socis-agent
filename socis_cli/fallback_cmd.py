"""
socis fallback — manage the fallback provider chain.
...
Subcommands:
  socis fallback [list]   Show the current fallback chain (default when no subcommand)
  socis fallback add      Pick provider + model via the same picker as `socis model`,
                           then append the selection to the chain
  socis fallback remove   Pick an entry to delete from the chain
  socis fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.socis-agent/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from socis_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `socis model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  socis fallback add")
        print()
        return
    ...
    print("  Docs: https://agent.socis.io/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `socis model`, then append the selection to the chain."""
    from socis_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `socis model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `socis fallback list` to view, or `socis fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``socis fallback [subcommand]``."""
