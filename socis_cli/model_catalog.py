...
The SOCIS docs site hosts a JSON manifest of curated models for providers
...
1. ``get_catalog()`` — returns a parsed manifest dict.
   ...
   - Reads disk cache at ``~/.socis-agent/cache/model_catalog.json``.
...
DEFAULT_CATALOG_FALLBACK_URLS: tuple[str, ...] = (
    "https://raw.githubusercontent.com/socisio/socis-agent/main/website/static/api/model-catalog.json",
)
...
def _cache_path() -> Path:
    ...
    return get_socis_agent_home() / "cache" / "model_catalog.json"
...
def refresh_catalogs() -> bool:
    ...
    try:
        from socis_cli.models import fetch_nous_recommended_models, fetch_openrouter_models
...
def get_default_model_from_cache(provider: str) -> str | None:
    ...
    ``"default": true``; that entry is the model SOCIS silently lands on when
    ...
    network-free. The cache is kept fresh by the picker/`socis update` paths;
...
def seed_cache_from_checkout(project_root: "Path | str") -> bool:
    ...
    ``socis update`` pulls the latest repo, so the freshly-pulled
    ...
    to ``~/.socis-agent/cache/model_catalog.json`` via the same atomic writer the
...
def reset_cache() -> None:
    """Clear the in-process cache. Used by tests and ``socis model --refresh``."""
    global _catalog_cache, _catalog_cache_source_mtime
    _catalog_cache = None
