...
``socis sessions recover --allow-partial`` normally copies rows through SQL,
...
rows back into a fresh current-schema SOCIS session database.
...
# SOCIS session ids are timestamps: 20260812_135332_ab12cd. This is the
...
# 3.50.7 and 3.44.6 — the same version gate socis_agent_state applies to the
...
# caught exactly this shell converting a live SOCIS state.db into two
# generations. A salvage shell must therefore be version-gated, not just
# capability-gated, before it is pointed at (a copy of) a SOCIS database.
#
# The predicate lives in socis_cli.sqlite_runtime (stdlib-only, shared with
# the installer/update gates) so the embedded runtime and the salvage shell
# can never disagree about which versions are safe.
from socis_cli.sqlite_runtime import is_sqlite_wal_reset_vulnerable as _wal_reset_vulnerable  # noqa: E502

_WAL_RESET_VULNERABLE_GUIDANCE = (
    "salvage against a SOCIS database with the WAL-reset bug "
...
def _cli_supports_recover(binary: str) -> bool:
    ...
    scratch_dir = tempfile.mkdtemp(prefix="socis-recover-probe-")
