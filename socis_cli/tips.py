...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or socis -c.",
    ...
    "Ctrl+Z suspends SOCIS to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "socis -c resumes your most recent CLI session. socis -c \"project name\" resumes by title.",
    "socis -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "socis -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "socis chat -t web,terminal enables only specific toolsets for a focused session.",
    "socis chat -s github-pr-workflow preloads a skill at launch.",
    "socis chat -q \"query\" runs a single non-interactive query and exits.",
    "socis chat --max-turns 1000 overrides the default 500-iteration limit per turn.",
    "socis chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "socis --yolo bypasses all dangerous command approval prompts for the entire session.",
    "socis chat --source telegram tags the session for filtering in socis sessions list.",
    "socis -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "socis doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "socis dump outputs a compact setup summary — great for bug reports.",
    "socis config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "socis config edit opens config.yaml in your default editor.",
    "socis config check scans for missing or stale configuration options.",
    "socis sessions browse opens an interactive session picker with search.",
    "socis sessions stats shows session counts by platform and database size.",
    "socis sessions prune --older-than 30 cleans up old sessions.",
    "socis skills search react --source skills-sh searches the skills.sh public directory.",
    "socis skills check scans installed hub skills for upstream updates.",
    "socis skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "socis skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "socis mcp add github --command npx adds MCP servers from the command line.",
    "socis mcp serve runs SOCIS itself as an MCP server for other agents.",
    "socis auth add lets you add multiple API keys for credential pool rotation.",
    "socis completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "socis logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "socis backup creates a zip backup of your entire SOCIS home directory.",
    "socis profile create coder creates an isolated profile that becomes its own command.",
    "socis profile create work --clone copies your current config and keys to a new profile.",
    "socis update syncs new bundled skills to ALL profiles automatically.",
    "socis gateway install sets up SOCIS as a system service (systemd/launchd).",
    "socis memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "socis webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: socis tools disables unused tools, socis skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "socis models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as socis -w).",
    ...
    "SOCIS_AGENT_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call SOCIS tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'socis profile create coder' creates the 'coder' command.",
    "socis profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, SOCIS prints a resume command with session ID and stats.",
    "socis sessions export backup.jsonl exports all sessions for backup or analysis.",
    "socis -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "socis skills install official/security/1password installs optional skills from the repo.",
    ...
    "/plan writes a markdown implementation plan to .socis-agent/plans/ without executing anything.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: socis cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "SOCIS runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "socis gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to SOCIS via a local macOS server.",
    ...
    "Context files (.socis.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "SOCIS loads project context from .socis.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "socis mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "socis mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "socis mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — socis tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.socis-agent/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "socis chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "socis plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "socis acp runs SOCIS as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "SOCIS_AGENT_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "socis auth add nous or socis auth add openai-codex sets up OAuth-based providers.",
    ...
    "socis status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.socis-agent/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make SOCIS your own.",
    ...
    "Before a gateway session reset, SOCIS auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.socis-agent/skills/.hub/audit.log tracks every install and removal operation.",
    ...
    "Profiles scope SOCIS state via SOCIS_AGENT_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    "SOCIS_AGENT_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in SOCIS_AGENT_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.socis-agent/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'socis profile create ls' would be rejected.",
    "socis profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "socis claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "SOCIS_AGENT_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via socis tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.socis-agent/.env.",
    "Skills from trusted repos (SOCIS) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.socis-agent/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — SOCIS auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of SOCIS config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while SOCIS is working.',
    ...
    '/reload reloads ~/.socis-agent/.env into the running session — pick up new API keys without restarting.',
    ...
    "SOCIS_AGENT_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.socis-agent/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.socis-agent/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'socis curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "socis curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'socis curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'socis auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    # --- TUI & Dashboard ---
    'SOCIS_AGENT_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "SOCIS_AGENT_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'socis dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'socis dashboard embeds the full SOCIS TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.socis-agent/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.socis-agent/dashboard-plugins/ — no npm build required.',
    ...
    'SOCIS_AGENT_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs concise/all/error/off).',
    'SOCIS_AGENT_WRITE_SAFE_ROOT restricts write_file/patch to directory prefixes; multiple paths via os.pathsep (: or ;).',
    'SOCIS_AGENT_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'SOCIS_AGENT_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.socis-agent/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.socis-agent/skills/ so drop-in skills appear without restarting the session.',
    ...
    'socis -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'socis chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'socis chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'socis chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "socis chat --source tool tags programmatic chats so they don't clutter socis sessions list.",
    'socis dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'socis sessions rename <ID> "new title" renames any past session; socis sessions delete <ID> removes one.',
    'socis import restores a full SOCIS backup zip; session JSON/JSONL exports import from the dashboard Sessions page.',
    'socis fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'socis pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'socis setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'socis status --deep runs the full health sweep across every component; plain socis status is the quick view.',

    # --- Agent Behavior Env Vars ---
    'SOCIS_AGENT_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'SOCIS_AGENT_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.socis-agent/plugins/ — trust-gated by design.',
    "SOCIS_AGENT_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'SOCIS_AGENT_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'SOCIS_AGENT_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'SOCIS_AGENT_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'SOCIS_AGENT_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'SOCIS_AGENT_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'SOCIS_AGENT_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'SOCIS_AGENT_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'SOCIS_AGENT_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'SOCIS_AGENT_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'SOCIS_AGENT_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes SOCIS block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from socis sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.socis-agent/dashboard-plugins/.',
]
...
# any project or none — SOCIS is not a coding-only agent.
# ---------------------------------------------------------------------------
