"""``socis slack ...`` CLI subcommands.

Today only ``socis slack manifest`` is implemented — it generates the
...
Typical workflow::
    ...
    $ socis slack manifest > slack-manifest.json
    # or:
...
def _build_full_manifest(
    ...
    it stays in sync with the rest of SOCIS. Other manifest sections
    (display info, OAuth scopes, socket mode) are set to sensible defaults
    for a SOCIS deployment — users can tweak them in the Slack UI after
    pasting.

    By default, this keeps SOCIS on Slack's older Assistant messaging
    ...
    """
    from socis_cli.commands import slack_app_manifest
    ...
    if messaging_experience == "assistant":
        features["assistant_view"] = {
            "assistant_description": "Chat with SOCIS in threads and DMs.",
        }
        ...
        features["agent_view"] = {
            "agent_description": "Chat with SOCIS in Slack Messages.",
        }
        bot_scopes.append("assistant:write")
    ...
    display_information = {
        "name": bot_name[:35],
        "description": (bot_description or "Your SOCIS agent on Slack")[:140],
...
def slack_manifest_command(args) -> int:
    ...
    Flags (all parsed in ``socis_cli/main.py``):
      --write [PATH]  Write to file instead of stdout (default path:
                      ``$SOCIS_AGENT_HOME/slack-manifest.json``)
      --name NAME     Override the bot display name (default: "SOCIS")
    ...
    """
    name = getattr(args, "name", None) or "SOCIS"
    description = getattr(args, "description", None) or "Your SOCIS agent on Slack"
    ...
    if getattr(args, "slashes_only", False) and (
        ...
        print(
            "socis slack manifest: long description options cannot be used "
            "with --slashes-only",
            file=sys.stderr,
        )
        return 2
    if long_description_file is not None:
        ...
        try:
            ...
            print(
                f"socis slack manifest: cannot read long description from "
                f"{source_arg}: {exc}",
                file=sys.stderr,
            )
            return 2
    if (
        ...
        print(
            "socis slack manifest: long description must be at least "
    ...
    if (
        ...
        print(
            "socis slack manifest: long description must be at most "
    ...
    if write_target is not None:
        ...
        print(
            "\nNext steps:\n"
            "  1. Open https://api.slack.com/apps and pick your SOCIS app\n"
