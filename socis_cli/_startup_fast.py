...
This module is imported by ``socis_cli/main.py`` BEFORE its heavy import
wall (config, argparse tree, logging, providers). Everything here must stay
**stdlib-only and cheap** (os/sys file probes; no yaml, no socis_cli.config,
...
``socis_cli/config.py``'s ``get_container_exec_info()`` reads the same
...
def active_profile_may_override_home(socis_root: str) -> bool:
    """Cheap probe: does an active non-default profile redirect SOCIS_AGENT_HOME?"""
    active_profile = os.path.join(socis_root, "active_profile")
    try:
...
def _resolved_home() -> str:
    socis_agent_home = os.environ.get("SOCIS_AGENT_HOME", "").strip()
    if socis_agent_home:
        return socis_agent_home
    return os.path.join(os.path.expanduser("~"), ".socis-agent")
...
def container_mode_may_be_active() -> bool:
    ...
    socis_agent_home = os.environ.get("SOCIS_AGENT_HOME", "").strip()
    if socis_agent_home:
        if os.path.exists(os.path.join(socis_agent_home, ".container-mode")):
            return True
        parent_name = os.path.basename(os.path.dirname(os.path.normpath(socis_agent_home)))
        return (
            parent_name != "profiles"
            and active_profile_may_override_home(socis_agent_home)
        )

    default_home = os.path.join(os.path.expanduser("~"), ".socis-agent")
...
def read_install_method() -> str | None:
    ...
    non-default profile might redirect SOCIS_AGENT_HOME.
    """
...
def print_fast_version_info(*, check_updates: bool = True) -> None:
    """THE canonical ``socis --version`` output (also used by /version).
    ...
    ``socis version`` subcommand. Every lazy block degrades gracefully —
    ...
    try:
        from socis_cli.banner import format_banner_version_label

        print(format_banner_version_label())
    except Exception:
        from socis_cli import __release_date__, __version__

        print(f"SOCIS Agent v{__version__} ({__release_date__})")
    ...
    try:
        ...
        from socis_cli.config import detect_install_method
    ...
    try:
        from socis_cli.banner import UPDATE_AVAILABLE_NO_COUNT, check_for_updates
...
def try_fast_version(argv: list[str] | None = None) -> bool:
    """Handle ``socis --version`` before the heavy import wall.
    ...
    container. Termux keeps the SOCIS_AGENT_TERMUX_DISABLE_FAST_CLI escape hatch.
    """
    if argv is None:
        argv = sys.argv[1:]
    is_termux = is_termux_env()
    if is_termux and os.environ.get("SOCIS_AGENT_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
    if is_termux:
