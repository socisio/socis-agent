...
on Japanese locales, etc.).  SOCIS's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``socis_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. SOCIS is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``SOCIS_AGENT_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("SOCIS_AGENT_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the SOCIS-managed Git install directories so
    ...
    # already-running shells don't see the change — which means socis
    ...
    # install without SOCIS-managed PortableGit).
    ...
    # stdin is re-configured for completeness; SOCIS's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and SOCIS's
    ``socis config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching SOCIS (User env var in Windows Settings, or
    export in a PowerShell profile) and SOCIS picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known SOCIS-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\socis\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``socis`` from right after install) retain their old PATH.

    Any subprocess SOCIS spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known SOCIS-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "socis", "git", "cmd"),
        os.path.join(local_appdata, "socis", "git", "bin"),
        os.path.join(local_appdata, "socis", "git", "usr", "bin"),
        # SOCIS venv Scripts directory — host of the socis.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes socis, but harmless to include.
        os.path.join(local_appdata, "socis", "socis-agent", "venv", "Scripts"),
