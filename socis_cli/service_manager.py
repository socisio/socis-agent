...
the existing module-level functions in socis_cli.gateway and
socis_cli.gateway_windows directly. This protocol is a thin facade
...
``socis gateway start/stop/restart`` when running inside a container.
"""
...
def detect_service_manager() -> ServiceManagerKind:
    ...
    in ``socis gateway start/stop/restart``).
    """
    ...
    # `socis gateway start/stop/restart` fell through to host code that spawns
...
def _s6_running() -> bool:
    ...
    Detection has to work for **both** root and the unprivileged socis
    ...
    detection always fails. Since every SOCIS runtime call inside the
    container drops to socis via ``s6-setuidgid``, that silent failure
...
# in ``socis_cli.gateway`` (systemd/launchd) and ``socis_cli.gateway_windows``
...
# active (set via the ``socis -p <profile>`` flag before the call). This
...
class SystemdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``systemd_*`` functions in socis_cli.gateway.
    ...
    def start(self, name: str) -> None:
        from socis_cli.gateway import systemd_start
        systemd_start()

    def stop(self, name: str) -> None:
        from socis_cli.gateway import systemd_stop
        systemd_stop()

    def restart(self, name: str) -> None:
        from socis_cli.gateway import systemd_restart
        systemd_restart()

    def is_running(self, name: str) -> bool:
        from socis_cli.gateway import _probe_systemd_service_running
        _, running = _probe_systemd_service_running()
        return running


class LaunchdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``launchd_*`` functions in socis_cli.gateway."""

    kind: ServiceManagerKind = "launchd"

    def start(self, name: str) -> None:
        from socis_cli.gateway import launchd_start
        launchd_start()

    def stop(self, name: str) -> None:
        from socis_cli.gateway import launchd_stop
        launchd_stop()

    def restart(self, name: str) -> None:
        from socis_cli.gateway import launchd_restart
        launchd_restart()

    def is_running(self, name: str) -> bool:
        from socis_cli.gateway import _probe_launchd_service_running
        return _probe_launchd_service_running()


class WindowsServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around ``socis_cli.gateway_windows`` (Scheduled Task /
...
#
# Per-profile gateways are registered dynamically when `socis profile create`
# runs inside the container (Phase 4). Static services (main-socis, dashboard)
...
def _profile_dir_for_gateway_service(name: str) -> Path:
    ...
    ``gateway stop --all``. Do not write the caller's SOCIS_AGENT_HOME blindly;
    derive the shared profile root from the current SOCIS_AGENT_HOME and map the
    ...
    socis_agent_home = Path(os.environ.get("SOCIS_AGENT_HOME", "/opt/data"))
    if socis_agent_home.parent.name == "profiles":
        root = socis_agent_home.parent.parent
    else:
        root = socis_agent_home
...
# UID/GID of the in-image ``socis`` user. Hardcoded to match what
...
def _seed_supervise_skeleton(svc_dir: Path) -> None:
    ...
    (the ``socis`` user — UID 10000 — running every SOCIS runtime
    ...
    is dispatched as the socis user.
    ...
    with socis ownership before triggering ``s6-svscanctl -a``,
    s6-supervise inherits our layout and never touches it.

    Layout produced
    ---------------
    ``svc_dir/``                           socis:socis, 0755 (parent must already exist)
    ``svc_dir/event/``                     socis:socis, 03730   (setgid + g+rwx + sticky)
    ``svc_dir/supervise/``                 socis:socis, 0755
    ``svc_dir/supervise/event/``           socis:socis, 03730
    ``svc_dir/supervise/control``          socis:socis, 0660    (FIFO)
    ...
    slot's supervise/ was socis-owned.
    ...
    def _mkdir_owned(path: Path, mode: int) -> None:
        ...
        path.chmod(mode)
        try:
            os.chown(path, _SOCIS_AGENT_UID, _SOCIS_AGENT_GID)
        except PermissionError:
            # Running as the socis user already — directory is socis-
    ...
    if not control.exists():
        ...
        control.chmod(0o660)
        try:
            os.chown(control, _SOCIS_AGENT_UID, _SOCIS_AGENT_GID)
        except PermissionError:
            pass
    ...
    # when the parent slot's supervise/ is socis-owned.
    log_dir = svc_dir / "log"
    if log_dir.is_dir():
        ...
        if not log_control.exists():
            ...
            try:
                os.chown(log_control, _SOCIS_AGENT_UID, _SOCIS_AGENT_GID)
            except PermissionError:
                pass
...
class GatewayNotRegisteredError(S6Error):
    ...
    Most commonly: ``socis -p typo gateway start`` when no profile
    ...
    def __init__(self, profile: str) -> None:
        ...
        super().__init__(
            ...
            f"`socis profile create {profile}` first, or pass "
...
class S6ServiceManager:
    ...
    ``S6_DYNAMIC_SCANDIR``. Static services (main-socis, dashboard)
    ...
    def _render_run_script(
        ...
        The script:
          1. Sources SOCIS_AGENT_HOME (and any extra env) via with-contenv —
             so e.g. ``-e SOCIS_AGENT_HOME=/data/socis`` is honored at run
          ...
          4. Drops to the socis user and exec's
             ``socis -p <profile> gateway run`` (or just ``socis
             gateway run`` for the default profile — see below).

        Special case: ``profile == "default"`` emits ``socis gateway
        run`` with **no** ``-p`` flag. This is the sentinel for "the
        root SOCIS_AGENT_HOME profile" (the implicit profile that exists at
        the top of $SOCIS_AGENT_HOME, not under profiles/). It must be
        spelled this way because ``_profile_suffix()`` returns the
        empty string for the root profile, and the dispatcher in
        ``socis_cli.gateway`` maps that empty string to the
        ``gateway-default`` service slot. Passing ``-p default`` here
        would instead look up ``$SOCIS_AGENT_HOME/profiles/default/`` — a
        ...
        each supervised profile gateway loads its own ``SOCIS_AGENT_HOME``,
        ...
        allocator in ``socis_cli.profiles._allocate_gateway_port``).
        ...
        # guard.
        lines.append("export SOCIS_AGENT_S6_SUPERVISED_CHILD=1")
        # Generalized supervisor marker (#74872) — same meaning for the
        # profile-redirect guard in socis_cli.main._apply_profile_override,
        # kept alongside the s6-specific sentinel for back-compat.
        lines.append("export SOCIS_AGENT_SUPERVISED_CHILD=1")
        # ``--replace`` makes the supervised gateway authoritative for its
        # profile's SOCIS_AGENT_HOME. Without it, a gateway started OUTSIDE s6
        # (a stray ``socis gateway run`` from a shell, an agent action, or
        # the Open WebUI helper) grabs the per-SOCIS_AGENT_HOME PID lock first;
        ...
        # gateway/run.py) so s6 always wins. The SOCIS_AGENT_S6_SUPERVISED_CHILD
        # sentinel above prevents the run→start→run redirect recursion.
        # Each profile is scoped to its own SOCIS_AGENT_HOME and s6 guarantees a
        # single supervised instance per slot, so there is no legitimate
        # supervised sibling for ``--replace`` to clobber.
        if profile == "default":
            gateway_cmd = "socis gateway run --replace"
        else:
            gateway_cmd = f"socis -p {shlex.quote(profile)} gateway run --replace"
        # Skip the drop when already non-root (setgroups() lacks CAP_SETGID →
        # s6 boot-loop).
        lines.append(f'[ "$(id -u)" = 0 ] || exec {gateway_cmd}')
        lines.append(f"exec s6-setuidgid socis {gateway_cmd}")
        return "\n".join(lines) + "\n"
    ...
    def _render_log_run(profile: str) -> str:
        ...
        OQ8-C: persist to ``${SOCIS_AGENT_HOME}/logs/gateways/<profile>/``.
        CRITICAL: the SOCIS_AGENT_HOME path is sourced from the runtime env
        via with-contenv — NOT Python-substituted at registration time
        — so a container started with ``-e SOCIS_AGENT_HOME=/data/socis``
        gets its logs under /data/socis/logs/..., not the build-time
        default.
        ...
        per line, in order:
          ...
          2. ``T <log_dir>`` — also write a timestamped copy to the
             ...
             files). This is what ``socis logs`` reads and what
        ...
        return (
            ...
            f': "${{SOCIS_AGENT_HOME:=/opt/data}}"\n'
            f'log_dir="$SOCIS_AGENT_HOME/logs/gateways/{prof}"\n'
            # Create the leaf and clear a stale s6-log lock as socis when
            # this script starts as root. Never chown or unlink socis-writable
            # volume paths from this restartable root-context script:
            # log/supervise/control is socis-owned, so an unprivileged user
            # can race a pathname op through a symlink swap (CWE-59 /
            # CWE-367). Parent logs/gateways is seeded socis-owned at stage2
            # boot (#45258; tests/docker/test_log_dir_seed.py).
            f'if [ "$(id -u)" = 0 ]; then\n'
            f'  s6-setuidgid socis mkdir -p "$log_dir"\n'
            f'  s6-setuidgid socis rm -f "$log_dir/lock"\n'
            f'else\n'
            f'  mkdir -p "$log_dir"\n'
            f'  rm -f "$log_dir/lock"\n'
            f'fi\n'
            ...
            f'exec s6-setuidgid socis s6-log 1 n10 s1000000 T "$log_dir"\n'
        )
    ...
    def stop(self, name: str) -> None:
        ...
        intent). Without the marker, an intentional ``socis gateway
    ...
    def register_profile_gateway(
        ...
        *,
        ...
        user explicitly runs ``socis -p <profile> gateway start``.
        ...
        try:
            ...
            # Pre-create the supervise/ skeleton with socis ownership
            ...
            # the socis user) won't hit EACCES on root-owned 0700
    ...
    def unregister_profile_gateway(self, profile: str) -> None:
        ...
        **before** ``rmtree`` so s6-svscan reaps the supervise child
        ...
        # is socis-owned (see ``_seed_supervise_skeleton``), and on
