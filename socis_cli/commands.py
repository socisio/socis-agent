"""Slash command definitions and autocomplete for the SOCIS CLI.
...
from socis_agent_constants import INDICATOR_STYLES
...
class CommandDef:
    ...
    # Names a key in ``socis_cli.slash_exec.EXECUTORS`` — a pure formatter
    # producing the canonical, surface-independent core text.  Surfaces
    # resolve it via ``socis_cli.slash_exec.run_execute`` and apply only
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("snapshot", "Create or restore state snapshots of SOCIS config/state", "Session",
    ...
    CommandDef("goal", "Set a standing goal SOCIS works on across turns until achieved", "Session",
    ...
    CommandDef("plan", "Write a markdown implementation plan to .socis-agent/plans/ without executing anything", "Session",
               args_hint="[task]"),
    ...
    CommandDef("wake", "Toggle the 'Hey SOCIS' wake word listener", "Configuration",
               cli_only=True, args_hint="[on|off|status]",
               subcommands=("on", "off", "status")),
    CommandDef("busy", "Control how messages behave while SOCIS is working", "Configuration",
    ...
    CommandDef("reload-skills", "Re-scan ~/.socis-agent/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update SOCIS Agent to the latest version", "Info",
               busy_policy="dispatch", desktop="terminal"),
    CommandDef("version", "Show SOCIS Agent version", "Info", aliases=("v",),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`socis_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/socis`` subcommand mapping, and
    ...
    """
    try:
        from socis_cli.plugins import get_plugin_commands
    except Exception:
        return []
    try:
...
# Telegram allows up to 100 BotCommands. SOCIS ships ~50 built-in commands;
...
Telegram only displays a small BotCommand menu in practice.  The full SOCIS
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``socis skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``socis skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``socis skills list`` and
...
# unrelated command landed. These claim their slots right after /socis,
# ahead of both canonical names and the rest of the aliases. Anything not
# listed here still degrades gracefully (reachable via /socis <command>).
...
# native slot, the alias spelling stays reachable via /socis reset).
...
# ``/socis <command>`` on Slack only. They remain native on every other
# surface (CLI, TUI, Telegram, Discord). Keep this list TIGHT and intentional —
# the telegram-parity test reads it so an entry here is a deliberate
# "Slack-via-/socis" decision, not a silent clamp.
#   - topup: the billing/balance surface; reached via /socis topup on Slack.
#     (the rehaul folded the old /credits + /billing surfaces into /topup.)
#   - moa: high-cost slash mode, available through /socis moa to avoid
#     displacing existing native Slack slash commands at the 50-command cap.
#   - debug: the log/report upload surface; reached via /socis debug on Slack.
#   - egress: Docker-only proxy status; reachable as /socis egress on Slack.
#   - init: repo-scan AGENTS.md bootstrap — a cwd-centric dev command that is
#     rare from Slack; reachable as /socis init. Without this entry, adding
#     /init clamps /version off the native list and breaks Telegram parity.
#   - version: low-frequency info command; reachable as /socis version on
...
#   - diff: git working-tree diff; reached via /socis diff on Slack so it
...
#     /socis update on Slack. Demoted to free the native slot /approvals now
...
#   - heartbeat: session heartbeat management; reached via /socis heartbeat
#     on Slack. Added at the 50-cap — a native slot would clamp /insights.
#   - refine: on-demand memory/skill review; reached via /socis refine on
#     Slack. Added at the 50-cap — a native slot would clamp an existing
#     native slash.
#   - pause: global emergency stop; reached via /socis pause [off] on
#     Slack. Added at the 50-cap — a native slot would clamp /platform.
#   - whoami: one-off identity lookup; reached via /socis whoami on Slack.
...
#     /socis platform on Slack. Demoted when /save became gateway-available
#     (session export is an interactive surface; platform is a rare
#     informational lookup) — without this entry /save tips the registry
#     past the 50-cap and silently clamps /platform, breaking parity.
_SLACK_VIA_SOCIS_AGENT_ONLY = frozenset({"topup", "moa", "debug", "egress", "init", "version", "diff", "update", "heartbeat", "refine", "review", "pause", "whoami", "platform", "insights"})
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/socis <verb>`` subcommand.
    ...
    can still reach them via ``/socis <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/socis`` is always reserved as the first entry so the
    legacy ``/socis <subcommand>`` form keeps working for anything that
    ...
    # Reserve /socis as the catch-all top-level command.
    entries.append(("socis", "Talk to SOCIS or run a subcommand", "[subcommand] [args]"))
    seen.add("socis")
    ...
    def _add(name: str, desc: str, hint: str) -> None:
        ...
        if slack_name in _SLACK_RESERVED_COMMANDS:
            return
        if slack_name in _SLACK_VIA_SOCIS_AGENT_ONLY:
            # Intentionally Slack-via-/socis only (see _SLACK_VIA_SOCIS_AGENT_ONLY).
            return
    ...
    # everything except /socis, so a new canonical command can never silently
...
def slack_app_manifest(request_url: str = "https://socis-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /socis handler.

    Maps both canonical names and aliases so /socis bg do stuff works
    the same as /socis bg do stuff.

    Plugin-registered slash commands are included so ``/socis <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from socis_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
    ...
    def _personality_completions(sub_text: str, sub_lower: str):
        """Yield completions for /personality via socis_cli.personality."""
        try:
