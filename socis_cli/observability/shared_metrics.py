"""Durable aggregation and local export for SOCIS shared metrics."""
...
_PACKAGE_SCHEMA_VERSION = "socis.shared_metrics.v2"
...
class SharedMetricsStore:
    ...
    def __init__(
        ...
        root = get_socis_agent_home() / "telemetry" / "shared_metrics"
        ...
        return [
            {
                ...
                "resource": {
                    "socis_version": row["socis_version"],
    ...
    def _ensure_schema(self) -> None:
        with self._connection(busy_timeout_ms=_SCHEMA_BUSY_TIMEOUT_MS) as connection:
            # Serialize first-run creation and upgrades across SOCIS processes.
            with write_txn(connection):
    ...
    def _add_send_columns(connection: sqlite3.Connection) -> None:
        ...
        branch, so bumping would make an older SOCIS — a second profile on an
    ...
    def _create_package_in_transaction(
        ...
        resource = {
            "socis_version": period_row["socis_version"],
