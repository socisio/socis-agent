"""Bounded product contract for the first SOCIS shared-metrics slice."""
...
SCHEMA_KEY = "socis.metrics.schema_version"
SCHEMA_VERSION = "socis.metrics.event.v2"
MODEL_CALL_SCOPE = "socis.model_call"
MODEL_CALL_PROFILE_MODEL = "unknown"
TASK_SCOPE = "socis.task_run"
TOOL_CALL_SCOPE = "socis.tool_call"
CLIENT_ACTIVE_MARK = "socis.client.active"
TOOL_APPROVAL_MARK = "socis.tool_approval"
SKILL_LIFECYCLE_MARK = "socis.skill.lifecycle"
SKILL_LOAD_MARK = "socis.skill.load"
SUBSCRIBER_NAME = "socis.nemo_relay.shared_metrics"
CLIENT_ACTIVE_METRIC = "socis.client.active"
LEGACY_MODEL_CALL_METRIC = "socis.model_call.count"
MODEL_ROUTE_METRIC = "socis.model_route.count"
TASK_STARTED_METRIC = "socis.task_run.started"
TASK_FINISHED_METRIC = "socis.task_run.finished"
TOOL_CALL_METRIC = "socis.tool_call.count"
TOOL_APPROVAL_METRIC = "socis.tool_approval.count"
SKILL_LIFECYCLE_METRIC = "socis.skill.lifecycle.count"
SKILL_LOAD_METRIC = "socis.skill.load.count"
...
def client_install_method(value: Any) -> str:
    """Return an allowlisted SOCIS installation method."""
    normalized = str(value or "").strip().lower()
    if normalized == "nix":
        return "nixos"
...
def client_resource(
    ...
    *,
    ...
    normalized_version = str(socis_version or "").strip()
    ...
    return {
        ...
        "socis_version": normalized_version,
...
def client_resource_is_valid(resource: Any) -> bool:
    ...
    if not isinstance(resource, dict) or set(resource) != CLIENT_RESOURCE_KEYS:
        return False
    version = resource.get("socis_version")
    return (
...
def _auxiliary_model_call_dimensions(event: Any) -> dict[str, str] | None:
    """Project a terminal auxiliary route from its SOCIS logical scope."""
    metadata = getattr(event, "metadata", None)
    if (
    ...
    if relay_metadata - {"otel.status_code"} or metadata.get(
        ...
        return None
    call_role = metadata.get("socis.call_role")
...
def execution_surface(kwargs: dict[str, Any]) -> str:
    ...
    try:
        from socis_cli.platforms import get_all_platforms
...
def task_terminal_state(kwargs: dict[str, Any]) -> tuple[str, str, str]:
    """Map SOCIS terminal state to bounded task outcome dimensions."""
...
def tool_category(kwargs: dict[str, Any]) -> str:
    """Map SOCIS registry toolset metadata to a low-cardinality category."""
    ...
    if toolset.startswith(
        ("discord", "email", "feishu", "socis-yuanbao", "slack", "sms")
    ):
...
def tool_outcome(kwargs: dict[str, Any]) -> str:
    """Normalize the terminal SOCIS tool status without inspecting its result."""
...
def model_call_fields(kwargs: dict[str, Any]) -> dict[str, str]:
    """Return the terminal model identity and provider route known to SOCIS."""
