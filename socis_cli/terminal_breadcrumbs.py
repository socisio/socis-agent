"""Per-terminal session breadcrumbs for ``socis -c`` / ``--continue``.

Each CLI session writes a tiny breadcrumb file
``$SOCIS_AGENT_HOME/terminal-sessions/<terminal-id>`` containing
``{"session_id": ..., "cwd": ..., "ts": ...}``.  A bare ``socis -c`` then
...
def _breadcrumbs_dir() -> Path:
    ...
    return get_socis_agent_home() / "terminal-sessions"
