...
Backs ``socis kanban export|import``, the matching ``/boards/{slug}/export``
...
ARCHIVE_FORMAT = "socis-kanban-board"
    ...
    *,
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        manifest = {
            ...
            "socis_version": str(socis_version),
            "includes": {
...
def _read_manifest(root: Path) -> dict[str, Any]:
    ...
    if not path.exists():
        raise ValueError(
            "archive is not a SOCIS kanban board export (no manifest.json)"
        )
    try:
    ...
    if not isinstance(manifest, dict) or manifest.get("format") != ARCHIVE_FORMAT:
        raise ValueError(
            "archive is not a SOCIS kanban board export "
    ...
    if not isinstance(version, int) or version > ARCHIVE_FORMAT_VERSION:
        raise ValueError(
            f"archive format version {version!r} is newer than this SOCIS "
            f"understands (max {ARCHIVE_FORMAT_VERSION}) — update SOCIS and retry"
        )
    return manifest
    ...
    *,
    ...
    return {
        ...
        "source": {
            ...
            "socis_version": manifest.get("socis_version"),
        },
        "counts": counts,
