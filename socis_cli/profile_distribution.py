"""Profile distributions — shareable, packaged SOCIS profiles via git.

A distribution is a SOCIS profile published as a git repository (or
...
* ``socis profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``socis skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``socis profile``, not a parallel tree):

    socis profile install <source> [--name N] [--alias] [--force] [--yes]
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "socis_agent_state.db", "response_store.db",
    "response_store.db-shm", "response_store.db-wal",
    "gateway.pid", "gateway_state.json", "processes.json",
    "auth.lock", "active_profile", ".update_check",
    "errors.log", ".socis_history",
    # User data
    ...
    "backups", "cache",
    # Infrastructure
    "socis-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            socis_requires=str(data.get("socis_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.socis_requires:
            out["socis_requires"] = self.socis_requires
        if self.author:
...
def check_socis_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires SOCIS {op}{target}, "
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``socis profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a SOCIS profile distribution."
            )
...
def plan_install(
    ...
    from socis_cli import __version__ as socis_version
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a SOCIS distribution."
        )

    # Version check up-front so we fail fast
    check_socis_requires(manifest.socis_requires, socis_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.socis-agent).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="socis_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `socis profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`socis profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="socis_dist_update_") as tmp:
        plan = plan_install(
