"""``socis logs`` — view and filter SOCIS log files.

Supports tailing, following, session filtering, level filtering,
component filtering, and relative time ranges.  All log files live
under ``~/.socis-agent/logs/``.

Usage examples::

    socis logs                    # last 50 lines of agent.log
    socis logs -f                 # follow agent.log in real time
    socis logs errors             # last 50 lines of errors.log
    socis logs gateway -n 100    # last 100 lines of gateway.log
    socis logs gui -f            # follow gui.log (dashboard/pty/ws)
    socis logs desktop -f        # follow desktop.log (Electron app boot/backend)
    socis logs --level WARNING    # only WARNING+ lines
    socis logs --session abc123   # filter by session ID substring
    socis logs --component tools  # only tool-related lines
    socis logs --since 1h         # lines from the last hour
    socis logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_socis_agent_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print("(Logs are created when SOCIS runs — try 'socis chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_socis_agent_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_socis_agent_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_socis_agent_home()}/logs/")
        return

    print(f"Log files in {display_socis_agent_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'socis chat' to generate logs)")
