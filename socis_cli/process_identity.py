...
Three layers that make every long-lived SOCIS process positively
identifiable, so reapers (``socis update``, Desktop startup sweeps) never
have to guess lineage from PPID archaeology or cmdline pattern-matching:

1. **Spawn tag** (``SOCIS_AGENT_SPAWN`` env var): every spawner stamps its children
...
2. **Spawn ledger** (``spawn-ledger.json`` at the machine SOCIS root): every
...
SPAWN_ENV_VAR = "SOCIS_AGENT_SPAWN"
_TAG_VERSION = "v1"
...
def install_id(project_root: Optional[Path] = None) -> str:
    ...
    Lets a reaper reject processes from a different SOCIS install on the
    same machine without path comparisons at scan time.
    """
    if project_root is None:
        try:
            from socis_agent_constants import PROJECT_ROOT as _root
...
def build_spawn_tag(purpose: str, *, project_root: Optional[Path] = None) -> str:
    """Value for the child's ``SOCIS_AGENT_SPAWN`` env var, stamped by the spawner."""
...
def parse_spawn_tag(raw: object) -> Optional[SpawnTag]:
    """Parse a ``SOCIS_AGENT_SPAWN`` value; ``None`` for anything malformed."""
...
def _ledger_path() -> Path:
    ...
    try:
        ...
        return Path(get_default_socis_root()) / LEDGER_FILENAME
    except Exception:
        from socis_cli.config import get_socis_agent_home

        return Path(get_socis_agent_home()) / LEDGER_FILENAME
...
def register_self(
    ...
    *,
    ...
    if spawner_pid is None:
        ...
        # SOCIS_AGENT_PARENT_PID (+ optional `winms:<ms>` start marker) for its
        # parent-death watchdog. Reuse it as spawner identity so ledger
        # lineage works with every Desktop version, no TS change needed.
        try:
            raw = int(os.environ.get("SOCIS_AGENT_PARENT_PID", ""))
            if raw > 0:
                spawner_pid = raw
        except (TypeError, ValueError):
            pass
        marker = os.environ.get("SOCIS_AGENT_PARENT_START_MARKER", "")
    ...
    try:
        ...
        # 10 tokens (was 6): enough for `socis serve --host X --port N
...
def register_child(
    ...
    *,
    ...
    ``npx``/binary servers never import SOCIS code). The entry records the
