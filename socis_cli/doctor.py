"""
Doctor command for socis CLI.

Diagnoses issues with SOCIS Agent setup.
"""
...
from socis_agent_constants import agent_browser_runnable

PROJECT_ROOT = get_project_root()
SOCIS_AGENT_HOME = get_socis_agent_home()
_DHH = display_socis_agent_home()  # user-facing display path (e.g. ~/.socis-agent or ~/.socis-agent/profiles/coder)

# Load environment variables from ~/.socis-agent/.env so API key checks work
_env_path = get_env_path()
load_socis_dotenv(socis_agent_home=_env_path.parent, project_env=PROJECT_ROOT / ".env")

from socis_cli.colors import Colors, color
from socis_cli.models import _SOCIS_AGENT_USER_AGENT
from socis_cli.vercel_auth import describe_vercel_auth
from socis_agent_constants import OPENROUTER_MODELS_URL
from socis_agent_state_common import FTS_STORAGE_VERSION
...
from socis_agent_constants import is_termux as _is_termux
...
def _sqlite_upgrade_hint(install_method: str | None = None) -> str:
    ...
    if method == "docker":
        ...
        action = f"run `{command}`, then recreate all SOCIS containers"
...
def _socis_database_paths(socis_agent_home: Path) -> list[tuple[str, Path]]:
    """Return (display name, path) pairs for SOCIS-managed SQLite databases."""
    # backup.py owns the canonical list of per-profile stores; reuse it.
    from socis_cli.backup import _QUICK_STATE_FILES

    entries = [
        (name, socis_agent_home / name)
        for name in _QUICK_STATE_FILES
        if name.endswith(".db")
    ]
    # Non-default kanban boards each keep their own kanban.db.
    for board_db in sorted((socis_agent_home / "kanban" / "boards").glob("*/kanban.db")):
        entries.append((str(board_db.relative_to(socis_agent_home)), board_db))
    return entries
...
def _read_journal_mode(db_path: Path) -> tuple[str | None, str | None]:
    ...
    locks a live connection is holding (see ``socis_cli.sqlite_safe_read``).
    ...
    """
    from socis_cli.sqlite_safe_read import (
        has_live_connection,
...
def _report_database_journal_modes(
    ...
    from socis_agent_state import _wal_reset_repair_hint, is_sqlite_wal_reset_vulnerable

    vulnerable = is_sqlite_wal_reset_vulnerable(version_info)
    home = socis_agent_home if socis_agent_home is not None else SOCIS_AGENT_HOME
    try:
        databases = _socis_database_paths(home)
    except Exception as exc:
        check_warn(f"Could not list SOCIS databases: {exc}")
        return
    exposed = []
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.socis-agent/.env contains provider auth/base URL settings."""
...
def _is_kanban_worker_env_gate(item: dict) -> bool:
    ...
    if item.get("name") != "kanban":
        return False
    if os.environ.get("SOCIS_AGENT_KANBAN_TASK"):
        return False
...
def _doctor_tool_availability_detail(toolset: str) -> str:
    ...
    if toolset == "kanban" and not os.environ.get("SOCIS_AGENT_KANBAN_TASK"):
...
def _has_healthy_oauth_fallback_for_apikey_provider(provider_label: str) -> bool:
    ...
    if normalized == "minimax":
        try:
            from socis_cli.auth import get_minimax_oauth_auth_status
...
def _doctor_memory_config(socis_agent_home: Path | None = None) -> dict:
...
from socis_cli.sizefmt import format_bytes as _human_bytes
...
def _render_state_db_stats(stats: dict, holders=None) -> list:
    ...
    if isinstance(deferral, dict):
        ...
        lines.append((
            ...
            "(stop the listed processes, then run 'socis sessions "
    ...
    if logical is not None and logical > STATE_DB_SIZE_WARN_BYTES:
        ...
        if stats.get("fts_rebuild_pending") or stale_trigram:
            detail += (
                "; run 'socis sessions optimize-storage' offline "
...
_DEPRECATED_ENV_VARS: tuple[tuple[str, str], ...] = (
    ...
    # ignored. SOCIS_AGENT_TOOL_PROGRESS_MODE is still read by the gateway as a
    # back-compat fallback but remains deprecated.
    ("SOCIS_AGENT_TOOL_PROGRESS", "display.tool_progress in config.yaml — ignored/unsupported since config floor v12"),
    ("SOCIS_AGENT_TOOL_PROGRESS_MODE", "display.tool_progress in config.yaml"),
...
def collect_relay_plugin_cutover_findings(
    ...
    """Return actionable findings for the removed SOCIS Relay plugin."""
    from socis_cli.relay_plugin_cutover import (
...
def _check_version_consistency(issues: list[str]) -> None:
    """Verify pyproject.toml version matches socis_cli.__version__.

    A git conflict resolution (reset/merge) can revert one file without the
    other, leaving ``socis --version`` reporting a stale version while
    ...
    """
    try:
        from socis_cli import __version__ as init_version
    except Exception:
        return
    ...
    if pyproject_version == init_version:
        ...
        _fail_and_issue(
            ...
            f"(pyproject.toml {pyproject_version} != socis_cli/__init__.py {init_version})",
            "Re-sync version files (e.g. run 'socis update', or set "
            "socis_cli/__init__.py __version__ to match pyproject.toml)",
            issues,
        )
...
def _check_s6_supervision(issues: list[str]) -> None:
    ...
    Reports:
      - Whether the main-socis and dashboard static services are up
    ...
    for static in ("main-socis", "dashboard"):
    ...
    if not profiles:
        check_info("No per-profile gateways registered yet — create one with `socis profile create <name>`")
        return
...
def check_certificates(should_fix: bool = False, issues: "list | None" = None) -> None:
    ...
    if not should_fix:
        ...
        if issues is not None:
            issues.append(
                "Repair the CA bundle: run `socis doctor --fix`, or "
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        try:
            from socis_cli.providers import normalize_provider as _normalize_provider
...
def managed_scope_check() -> None:
    ...
    resolved from the SOCIS_AGENT_MANAGED_DIR override (rather than the system
    ...
    if os.environ.get("SOCIS_AGENT_MANAGED_DIR", "").strip():
        check_info(f"managed dir set via SOCIS_AGENT_MANAGED_DIR={managed_dir}")
...
def check_macos_tcc_grants() -> None:
    ...
    if "cdhash" in dr.lower():
        check_warn(
            ...
            "Run `socis update` to get the stable identifier-pinned signing "
            "identity, then re-grant permissions once.",
        )
        return
    if "certificate" in dr.lower():
        # Certificate-anchored DR (socis desktop --setup-tcc-identity, or a
        ...
        check_ok(
            ...
            "anchor, see `socis desktop --setup-tcc-identity`)",
        )
    check_info(
        ...
        "fully quit & relaunch SOCIS once."
    )
...
def _desktop_app_bundle() -> Path | None:
    ...
    (``apps/desktop/release/mac-<arch>/SOCIS.app``) — the only layout whose
    ...
    ``_desktop_packaged_executable``'s selection. ``/Applications/SOCIS.app``
    is deliberately not probed: it is the separately-signed SOCIS-Setup
    ...
    candidates = [p for p in release_dir.glob("mac*/SOCIS.app") if p.is_dir()]
    if not candidates:
        return None
...
def check_macos_tcc_anchor(should_fix: bool = False) -> None:
    ...
    """
    try:
        from socis_cli import macos_tcc_anchor as tcc
...
def check_macos_full_disk_access() -> None:
    ...
    check_info(
        ...
        "app Full Disk Access and SOCIS will never trip per-folder dialogs "
        ...
        "    then enable your terminal (and SOCIS.app if you use Desktop), "
        "and restart them once. With SOCIS' stable signing identities the "
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `socis`.
    os.environ.setdefault("SOCIS_AGENT_INTERACTIVE", "1")

    # Handle `socis doctor --ack <id>` as a fast path. Persist the ack and
    # return without running the rest of the diagnostics — the user has
    # already seen the advisory and just wants to silence it.
    if ack_target:
        from socis_cli.security_advisories import (
            ADVISORIES,
            ack_advisory,
        )
        ...
        if ack_advisory(ack_target):
            ...
            print(color(
                ...
                f"Check ~/.socis-agent/config.yaml is writable.",
                Colors.RED,
            ))
            sys.exit(1)
        return
    ...
    print(color("│                 🩺 SOCIS Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
        ...
        if fresh_hits:
            for hit in fresh_hits:
                ...
                manual_issues.append(
                    ...
                    f"`socis doctor --ack {hit.advisory.id}`."
                )
    ...
    try:
        import sqlite3
        from socis_agent_state import is_sqlite_wal_reset_vulnerable, sqlite_source_id
        ...
        if is_sqlite_wal_reset_vulnerable():
            # Warn-only: SOCIS already refuses to enable WAL on fresh DBs.
    ...
    # Detect drift between pyproject.toml and socis_cli/__init__.py versions
    ...
    managed_scope_check()
    # Check ~/.socis-agent/.env (primary location for user config)
    env_path = SOCIS_AGENT_HOME / '.env'
    if env_path.exists():
        ...
        # matches socis_cli.env_loader._load_dotenv_with_fallback.
        try:
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'socis setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'socis setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'socis setup' to create one")
                issues.append("Run 'socis setup' to create .env")
    
    # Check ~/.socis-agent/config.yaml (primary) or project cli-config.yaml (fallback)
    ...
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from socis_cli.config import get_compatible_custom_providers as _compatible_custom_providers
            ...
            if isinstance(user_providers, dict):
                from socis_cli.config import is_provider_enabled
                ...
                if catalog_provider is None or (
                    ...
                    _fail_and_issue(
                        ...
                        (
                            ...
                            f"Fix: run 'socis config set model.provider <valid_provider>'"
                        ),
                        issues,
                    )
            ...
            if runtime_provider and runtime_provider not in ("auto", "custom"):
                try:
                    if runtime_provider == "openrouter":
                        ...
                        from socis_cli.auth import PROVIDER_REGISTRY, get_auth_status
                    ...
                    if not configured:
                        _fail_and_issue(
                            ...
                            "(check ~/.socis-agent/.env or run 'socis setup')",
                            (
                                f"No credentials found for provider '{runtime_provider}'. "
                                f"Run 'socis setup' or set the provider's API key in {_DHH}/.env, "
                                f"or switch providers with 'socis config set model.provider <name>'"
                            ),
                            issues,
                        )
                except Exception:
        ...
        if fallback_config.exists():
            ...
            if should_fix:
                ...
                if example_config.exists():
                    ...
                    from socis_cli.config import DEFAULT_CONFIG, save_config
    ...
    if config_path.exists():
        try:
            from socis_cli.config import check_config_version, migrate_config
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'socis setup' to migrate config")
                else:
                    issues.append("Run 'socis doctor --fix' or 'socis setup' to migrate config")
            else:
                ...
                if should_fix:
                    ...
                    for k in stale_root_keys:
                        if not model_section.get(k):
                            ...
                            raw_config.pop(k)
                    from socis_cli.config import atomic_config_write
                    atomic_config_write(config_path, raw_config)
                    check_ok("Migrated stale root-level keys into model section")
                    fixed_count += 1
                else:
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'socis doctor --fix'")
        except Exception:
            pass

        # Detect stale SOCIS_AGENT_MAX_ITERATIONS ghost in .env shadowing
        ...
        # bridge normally derives SOCIS_AGENT_MAX_ITERATIONS from agent.max_turns
        ...
        try:
            from socis_cli.config import load_env, read_user_config_raw, remove_env_value
            ...
            env_ghost = load_env().get("SOCIS_AGENT_MAX_ITERATIONS")
            drift = (
            ...
            if drift:
                check_warn(
                    f"SOCIS_AGENT_MAX_ITERATIONS={env_ghost} in .env shadows "
                    f"agent.max_turns={cfg_max_turns} in config.yaml",
                    "(stale ghost from an earlier `socis setup` run)",
                )
                if should_fix:
                    if remove_env_value("SOCIS_AGENT_MAX_ITERATIONS"):
                        check_ok(
                            "Removed stale SOCIS_AGENT_MAX_ITERATIONS from .env "
                            f"(config.yaml agent.max_turns={cfg_max_turns} is now authoritative)"
                        )
                        fixed_count += 1
                    else:
                        check_warn("Could not remove SOCIS_AGENT_MAX_ITERATIONS from .env")
                        manual_issues.append(
                            "Manually delete the SOCIS_AGENT_MAX_ITERATIONS line from "
                            f"{_DHH}/.env — config.yaml agent.max_turns is authoritative."
                        )
                else:
                    issues.append(
                        "Stale SOCIS_AGENT_MAX_ITERATIONS in .env shadows config.yaml — "
                        "run 'socis doctor --fix'"
                    )
        except Exception:
            pass
        ...
        try:
            from socis_cli.config import load_env as _load_env_depr
            from socis_cli.config import read_user_config_raw as _read_raw_depr
    ...
    try:
        ...
        if codex_status.get("logged_in"):
            ...
            # Native OAuth uses SOCIS' own device-code flow — the Codex CLI is
    ...
    _memory_config = _doctor_memory_config(socis_agent_home)
    ...
    for subdir_name in expected_subdirs:
        subdir_path = socis_agent_home / subdir_name
    ...
    soul_path = socis_agent_home / "SOUL.md"
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give SOCIS a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# SOCIS Agent Persona\n\n"
                "<!-- Edit this file to customize how SOCIS communicates. -->\n\n"
                "You are SOCIS, a helpful AI assistant.\n",
                encoding="utf-8",
            )
    ...
    memories_dir = socis_agent_home / "memories"
    ...
    state_db_path = socis_agent_home / "state.db"
    if state_db_path.exists():
        try:
            ...
            from socis_agent_state import _db_opens_cleanly, repair_state_db_schema
            ...
            if _write_reason is not None:
                ...
                if should_fix:
                    ...
                    issues.append(
                        "state.db FTS write corruption — run 'socis doctor --fix' "
                        "(or 'socis sessions repair') to rebuild the FTS index"
                    )
        except Exception as e:
            from socis_agent_state import is_malformed_db_error, repair_state_db_schema
            ...
            if is_malformed_db_error(e):
                ...
                if should_fix:
                    ...
                    issues.append(
                        "state.db schema malformed — run 'socis doctor --fix' "
                        "(or 'socis sessions repair') to recover hidden sessions"
                    )
            else:
                check_warn(f"{_DHH}/state.db exists but has issues: {e}")

        # Health/stats snapshot (#statedb-visibility): a multi-GB state.db
        # with a runaway WAL was previously invisible to every SOCIS
        ...
        try:
            from socis_agent_state import collect_state_db_stats, count_db_holders
                ...
                if _kind == "warn":
                    ...
                    if "auto_prune" in _detail:
                        issues.append(
                            ...
                            "in config.yaml"
                            + (
                                " and run 'socis sessions optimize-storage' "
    ...
    wal_path = socis_agent_home / "state.db-wal"
    if wal_path.exists():
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'socis doctor --fix' to checkpoint")
    ...
    if sys.platform != "win32":
        ...
        for _venv_name in ("venv", ".venv"):
            _candidate = PROJECT_ROOT / _venv_name / "bin" / "socis"
        ...
        _cmd_link = _cmd_link_dir / "socis"

        if _venv_bin is None:
            check_warn(
                "Venv entry point not found",
                "(socis not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')"
            )
            manual_issues.append(
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    check_ok(f"{_cmd_link_display}/socis → correct target")
                else:
                    check_warn(
                        f"{_cmd_link_display}/socis points to wrong target",
                        f"(→ {_target}, expected → {_expected})"
                    )
                    if should_fix:
                        _cmd_link.unlink()
                        _cmd_link.symlink_to(_venv_bin)
                        check_ok(f"Fixed symlink: {_cmd_link_display}/socis → {_venv_bin}")
                        fixed_count += 1
                    else:
                        issues.append(f"Broken symlink at {_cmd_link_display}/socis — run 'socis doctor --fix'")
            elif _cmd_link.exists():
                # It's a regular file, not a symlink — possibly a wrapper script
                check_ok(f"{_cmd_link_display}/socis exists (non-symlink)")
            else:
                check_fail(
                    f"{_cmd_link_display}/socis not found",
                    "(socis command may not work outside the venv)"
                )
                if should_fix:
                    _cmd_link_dir.mkdir(parents=True, exist_ok=True)
                    _cmd_link.symlink_to(_venv_bin)
                    check_ok(f"Created symlink: {_cmd_link_display}/socis → {_venv_bin}")
                    fixed_count += 1
                    ...
                    issues.append(f"Missing {_cmd_link_display}/socis symlink — run 'socis doctor --fix'")
    ...
    try:
        from socis_agent_constants import is_container as _is_container
    ...
    if terminal_env == "vercel_sandbox":
        ...
        if importlib.util.find_spec("vercel") is not None:
            ...
            _fail_and_issue(
                ...
                "Install the Vercel optional dependency: pip install 'socis-agent[vercel]'",
                issues,
            )
    ...
    if _safe_which("node"):
        ...
        # — it resolves lazily via npx (or a global/SOCIS-managed install)
        ...
        if _resolved_ab and _is_npx_agent_browser_sentinel(_resolved_ab):
            ...
            if should_fix:
                ...
                # `socis update` does, so a session's first browser call
            ...
            # `socis update` wiped node_modules (issue #48521).
            check_warn(
        ...
        if agent_browser_ok and not _is_termux():
            try:
                ...
                # to eagerly load in every `socis doctor` invocation.
    ...
    try:
        ...
        if _using_lightpanda_engine():
            ...
            if not _lp_used:
                ...
                check_info(
                    "Fix: pick Lightpanda in `socis tools` → Browser Automation, "
    ...
    if _npm_bin:
        ...
        # The WhatsApp bridge may live under a writable SOCIS_AGENT_HOME mirror
        ...
        npm_audit_targets = [
            (PROJECT_ROOT, "root workspace (build tooling)", ["--workspaces=false"]),
            ...
            try:
                ...
                if total == 0:
                    ...
                    if audit_extra and audit_extra[0] == "--workspace":
                        ...
                        # tree — in that case it is an npm bug, not a SOCIS one.
                        check_info(
            ...
            if r.status_code == 402:
                return _ConnectivityResult(
                    ...
                    ["OpenRouter account has insufficient credits. "
                     "Fix: run 'socis config set model.provider <provider>' "
    ...
    def _probe_apikey_provider(pname, env_vars, default_url, base_env,
        ...
        try:
            ...
            headers = {
                ...
                "User-Agent": _SOCIS_AGENT_USER_AGENT,
            }
            ...
            # GOOGLE_API_KEY/GEMINI_API_KEY always shows red in ``socis doctor``.
    ...
    # never the legitimate source for `socis doctor`.
    ...
    try:
        ...
        if api_disabled or web_not_ready:
            issues.append("Run 'socis setup' to configure missing API keys for full tool access")
    except Exception as e:
        check_warn("Could not check tool availability", f"({e})")
    
    _section("Skills Hub")
    hub_dir = SOCIS_AGENT_HOME / "skills" / ".hub"
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: socis skills list)")
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                ...
                if hcfg.api_key or hcfg.base_url:
                    ...
                    check_warn("Honcho config not found", "run: socis memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                _fail_and_issue(
                    "Honcho API key or base URL not set",
                    "run: socis memory setup",
                    "No Honcho API key — run 'socis memory setup'",
                    issues,
                )
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                _fail_and_issue(
                    ...
                    "(set MEM0_API_KEY in .env or run socis memory setup)",
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: socis memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: socis memory setup")
        except Exception as _e:
            check_warn(f"{_active_memory_provider} check failed", str(_e))

    try:
        from socis_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
            ...
            if wrapper_dir.is_dir():
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "socis -p" in content:
                            _m = _re.search(r"socis -p (\S+)", content)
    ...
    # `socis doctor --live` (real network calls; bounded + read-only).
    try:
        from socis_cli.doctor_live import maybe_run_live_checks
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'socis doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
