"""Migrate SOCIS' MCP server config and Codex's installed curated plugins
...
module:
  ...
  1. Reads SOCIS' YAML and writes equivalent [mcp_servers.<name>]
...
What translates (MCP servers):
  SOCIS mcp_servers.<n>.command/args/env  → codex stdio transport
  SOCIS mcp_servers.<n>.url/headers       → codex streamable_http transport
  SOCIS mcp_servers.<n>.timeout           → codex tool_timeout_sec
  SOCIS mcp_servers.<n>.connect_timeout   → codex startup_timeout_sec

What does NOT translate (warned + skipped):
  SOCIS-specific keys (sampling, etc.) — codex's MCP client has no
  equivalent. Listed in the per-server skipped[] field of the report.

What's NOT migrated (intentional):
  AGENTS.md — codex respects this file natively in its cwd. SOCIS' own
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in SOCIS config.")
...
_KEYS_DROPPED_WITH_WARNING = {
    # SOCIS' sampling subsection — codex MCP has no equivalent
    "sampling",
}


def _translate_one_server(
    name: str, socis_cfg: dict
) -> tuple[Optional[dict], list[str]]:
    """Translate one SOCIS MCP server config to the codex inline-table dict
    ...
    if not isinstance(socis_cfg, dict):
        return None, []

    skipped: list[str] = []
    out: dict[str, Any] = {}

    has_command = bool(socis_cfg.get("command"))
    has_url = bool(socis_cfg.get("url"))
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (SOCIS_AGENT_HOME, PYTHONPATH) could
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out.append("# (no MCP servers, plugins, or permissions configured by SOCIS)")
...
def _insert_managed_block_at_top_level(user_text: str, managed_block: str) -> str:
    """Insert SOCIS' managed Codex TOML block while keeping root keys root-scoped.
...
def _strip_unmanaged_plugin_tables(toml_text: str) -> str:
    ...
    directly (i.e. before SOCIS' migrate has ever touched the file). When we
...
def _strip_existing_managed_block(toml_text: str) -> str:
    ...
    this code wrote so re-runs don't break configs from prior SOCIS
    versions."""
    lines = toml_text.splitlines(keepends=True)
    out: list[str] = []
    in_managed = False
...
def _query_codex_plugins(
    ...
    try:
        with CodexAppServerClient(
            ...
            client.initialize(client_name="socis-migration")
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    what pytest's tempdir factory uses by default. If a SOCIS_AGENT_HOME pointing
    at one of those paths is burned into ``~/.codex/config.toml``, every
    codex-routed socis-tools call fails silently once the directory is GC'd.

    We err on the side of refusing — losing a (very unlikely) real
    ``~/.socis-agent`` symlink that happens to live under ``/private/var/folders``
...
def _build_socis_tools_mcp_entry() -> dict:
    """Build the codex stdio-transport entry that launches SOCIS' own
    ...
    so a socis installed under /opt/, /usr/local/, or a venv all work.
    SOCIS_AGENT_HOME and PYTHONPATH are passed through so the spawned process
    sees the same config + module layout the user is running."""
    import sys

    env: dict[str, str] = {}
    # SOCIS_AGENT_HOME passes through IF SET so the MCP subprocess sees the same
    # config / auth / sessions DB as the parent CLI. Read from os.environ
    # (not get_socis_agent_home()) on purpose: when the env var is unset we want
    # codex's subprocess to inherit whatever SOCIS_AGENT_HOME its launcher sets
    ...
    # — that would override the launcher's SOCIS_AGENT_HOME and pin the subprocess
    # to the wrong profile.
    #
    # The pytest-tempdir guard below catches the issue #26250 Bug C scenario:
    # a sibling test's monkeypatch.setenv("SOCIS_AGENT_HOME", tmp_path) would
    ...
    socis_agent_home = os.environ.get("SOCIS_AGENT_HOME") or ""
    if socis_agent_home and _looks_like_test_tempdir(socis_agent_home):
        socis_agent_home = ""
    if socis_agent_home:
        env["SOCIS_AGENT_HOME"] = socis_agent_home
    # PYTHONPATH passes through so a worktree-launched socis finds the
    # branch's modules instead of the installed package.
    pythonpath = os.environ.get("PYTHONPATH")
    if pythonpath:
        env["PYTHONPATH"] = pythonpath
    # Quiet mode + redaction defaults so the MCP wire stays clean.
    env["SOCIS_AGENT_QUIET"] = "1"
    env["SOCIS_AGENT_REDACT_SECRETS"] = env.get("SOCIS_AGENT_REDACT_SECRETS", "true")
...
def migrate(
    ...
    *,
    ...
    expose_socis_tools: bool = True,
) -> MigrationReport:
    """Translate SOCIS mcp_servers config + Codex curated plugins into
    ~/.codex/config.toml.

    Args:
        socis_config: full ~/.socis-agent/config.yaml dict
        ...
        expose_socis_tools: when True (default), register SOCIS' own
            tool surface (web_search, browser_*, delegate_task, vision,
            memory, skills, etc.) as an MCP server in ~/.codex/config.toml
            so the codex subprocess can call back into SOCIS for tools
    ...
    socis_servers = (socis_config or {}).get("mcp_servers") or {}
    if not isinstance(socis_servers, dict):
        report.errors.append(
            "mcp_servers in SOCIS config is not a dict; cannot migrate."
        )
        return report

    translated: dict[str, dict] = {}
    for name, cfg in socis_servers.items():
    ...
    # Inject SOCIS' own tool surface as an MCP server so the spawned
    # codex subprocess can call back into SOCIS for the tools codex
    # doesn't ship with — web_search, browser_*, delegate_task, vision,
    # memory, skills, session_search, image_generate, text_to_speech.
    # The server itself is agent/transports/socis_tools_mcp_server.py
    # and is launched on demand by codex (stdio MCP).
    if expose_socis_tools:
        translated["socis-tools"] = _build_socis_tools_mcp_entry()
        if "socis-tools" not in report.migrated:
            report.migrated.append("socis-tools")
