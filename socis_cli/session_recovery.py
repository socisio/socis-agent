"""Offline, non-destructive recovery for a damaged SOCIS session database.
...
def _copy_source_bundle(source: Path, snapshot_dir: Path) -> tuple[Path, list[str]]:
    ...
    locks -- the failure class ``socis_cli.sqlite_safe_read`` exists to
    ...
    keeps this path consistent with ``socis_agent_state._backup_db_file``.
    """
    ...
    try:
        ...
        if before != after:
            raise SessionRecoverySafetyError(
                ...
                "Stop every SOCIS process using this profile and retry. "
                "This includes the interactive `socis` CLI session this "
                ...
                "SOCIS process even after the gateway is stopped. Run the "
                "recovery from a fresh shell with no `socis` session open, "
...
def _lost_and_found_plausibility_errors(
    ...
    """
    from socis_cli.session_lost_and_found import _EPOCH_LOW, STUB_TITLE_PREFIX
