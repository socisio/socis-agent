"""SOCIS-managed uv and Python runtime repair.

SOCIS owns its own uv binary at ``$SOCIS_AGENT_HOME/bin/uv`` (or ``uv.exe`` on
Windows).  Every code path that needs uv resolves it from that single location.
If the binary is missing, ``ensure_uv()`` bootstraps it via the official
standalone installer with ``UV_UNMANAGED_INSTALL`` / ``UV_INSTALL_DIR`` pointed
at ``$SOCIS_AGENT_HOME/bin`` so the installer writes directly there — no PATH
probing, no conda guards, no multi-location resolution chains.

The Python backing the install is different: it is shared by every SOCIS
profile because the checkout's ``venv`` is shared.  Runtime repair therefore
uses an install-scoped store under ``<checkout>/.socis-runtime/python``. A
...
from socis_cli.sqlite_runtime import SQLiteRuntimeInfo, probe_sqlite_runtime

logger = logging.getLogger(__name__)

_PROJECT_ROOT = Path(__file__).resolve().parents[1]
_RUNTIME_DIR_NAME = ".socis-runtime"
_VENV_NAME = "venv"
_ALT_VENV_NAME = ".venv"
...
def managed_uv_path() -> Path:
    """Return the path where SOCIS keeps *its* uv binary.

    ``$SOCIS_AGENT_HOME/bin/uv`` on POSIX, ``$SOCIS_AGENT_HOME\\bin\\uv.exe`` on
...
def managed_python_env(
    ...
    *,
    ...
    """Return a sanitized environment for SOCIS-private uv Python commands."""
    target = (
        Path(install_dir)
...
def _macos_sign_managed_python(python: Path) -> bool:
    ...
    TCC with a cdhash-only identity that changes whenever SOCIS provisions a
...
def _report_runtime_repair_failure(repair: RuntimeRepairResult) -> None:
    if repair.backup_venv is None:
        ...
        print(
            "    Sessions stay protected meanwhile: SOCIS keeps databases "
            "out of WAL mode on this SQLite build. The next `socis update` "
            "will retry."
        )
        return
...
class _UvResult(str):
    ...
    ``(path, fresh_bootstrap)`` tuple across releases. ``socis update`` runs
    the call site from the *old*, already-imported ``socis_cli.main`` against
...
def _uv_self_update_is_fresh(now: float | None = None) -> bool:
    ...
    uv releases roughly weekly while many users run ``socis update`` daily;
    re-running a blocking network self-update on every invocation is waste
    and, offline, an unbounded hang risk. A stamp file under SOCIS_AGENT_HOME
    caches the last successful self-update time.
    """
    try:
        from socis_agent_constants import get_socis_agent_home

        stamp = get_socis_agent_home() / "cache" / ".uv_self_update_stamp"
...
def update_managed_uv(
    *,
    ...
    Call this during ``socis update`` so the managed copy stays current.
...
def _reload_socis_agent_constants():
    """Re-execute ``socis_agent_constants`` from disk and return the fresh module.

    ``socis update`` imports ``socis_agent_constants`` from the OLD checkout,
    ...
    return importlib.reload(socis_agent_constants)
...
def _attempt_install_generation(
    ...
    *,
    ...
    try:
        ...
        logger.warning("uv resolved Python outside the SOCIS generation: %s", python)
        _remove_tree(generation, boundary=python_root)
        return None
...
def _install_safe_python_generation(
    ...
    *,
    ...
    # user isn't stuck on every `socis update` with no path to a fixed
...
def _windows_runtime_holders() -> tuple[bool, str]:
    if platform.system() != "Windows":
        return False, ""
    main_module = sys.modules.get("socis_cli.main")
    ...
    if holders:
        ...
        return True, f"other SOCIS processes still hold the venv (PID {pids})"
    return False, ""
...
def _windows_runtime_self_lock(live: Path) -> tuple[bool, str]:
    ...
    ancestors on purpose — a CLI ``socis update`` itself runs from the
    ...
    updater's own ``python.exe`` (or a waiting ``socis.exe`` launcher
    ...
    # Belt-and-braces: the venv\Scripts\socis.exe launcher stays mapped
...
def _refresh_managed_uv_catalog(uv_bin: str) -> bool:
    ...
    unmanaged installs.  Only the SOCIS-managed binary is ever refreshed;
...
def _default_live_venv(root: Path) -> Path:
    ...
    ``not-applicable`` on every ``socis update`` and stayed on
    journal_mode=DELETE forever — even though the WAL fallback warning
    promises that ``socis update`` repairs the runtime (issue class:
...
def repair_vulnerable_runtime(
    ...
    *,
    ...
    if self_locked:
        ...
        print(
            "    Retrying `socis update` from inside this venv cannot help: "
        ...
        print(f"      cd {root}")
        print("      <system Python> -m socis_cli.main update")
        print(
            "    Sessions stay protected meanwhile: SOCIS keeps databases "
...
def _install_uv(target: Path) -> None:
    ...
    ``$SOCIS_AGENT_HOME/bin/`` instead of ``~/.local/bin/``.
    """
