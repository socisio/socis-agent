"""Image-authored deployment provenance for immutable SOCIS runtimes.

The published image bakes ``/etc/socis/image-provenance.json`` outside both
``$SOCIS_AGENT_HOME`` and the mutable checkout.  A bind-mounted checkout (including
...
IMAGE_PROVENANCE_PATH = Path("/etc/socis/image-provenance.json")
