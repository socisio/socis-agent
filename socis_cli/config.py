"""
Configuration management for SOCIS Agent.

Config files are stored in ~/.socis-agent/ for easy access:
- ~/.socis-agent/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.socis-agent/.env         - API keys and secrets

This module provides:
- socis config          - Show current configuration
- socis config edit     - Open config in editor
- socis config get      - Print a resolved configuration value
- socis config set      - Set a specific value
- socis config unset    - Remove a user configuration value
- socis config wizard   - Re-run setup wizard
"""
...
from socis_cli.route_identity import normalize_route_base_url
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``socis config set`` (which rewrites
    ...
    ``config.yaml`` in place: socis never silently mutates the user's config,
...
def _warn_config_parse_failure(
    ...
    A YAML parse error in ``~/.socis-agent/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``socis logs``
    ...
    if fallback == "last-known-good":
        ...
        msg = (
            ...
            f"Fix the YAML (socis config edit) and retry."
        )
    else:
        msg = (
            f"Failed to parse {config_path}: {exc}. "
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  socis config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
def get_active_config_parse_failure() -> Optional[str]:
    ...
    Consumers: ``socis_cli.auth.resolve_provider`` uses this to refuse
...
#   the next ``subprocess.run([...])`` SOCIS makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. SOCIS itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``socis update``, the TUI build.
...
#   on every plugin install / ``socis update``.
...
# * ``SOCIS_AGENT_HOME`` / ``SOCIS_AGENT_PROFILE`` / ``SOCIS_AGENT_CONFIG`` /
#   ``SOCIS_AGENT_ENV`` — SOCIS runtime location flags. Writing these into
...
#
# IMPORTANT: ``SOCIS_AGENT_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (SOCIS_AGENT_LANGFUSE_PUBLIC_KEY,
# SOCIS_AGENT_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # SOCIS runtime location — never via dashboard env writer.
    # NOT a SOCIS_AGENT_* blanket: integration credentials (SOCIS_AGENT_GEMINI_*,
    # SOCIS_AGENT_LANGFUSE_*, SOCIS_AGENT_SPOTIFY_*, ...) ARE allowed.
    "SOCIS_AGENT_HOME", "SOCIS_AGENT_PROFILE", "SOCIS_AGENT_CONFIG", "SOCIS_AGENT_ENV",
    "SOCIS_AGENT_CONFIG_PATH", "SOCIS_AGENT_ENV_PATH",
    ...
    "SOCIS_AGENT_COPILOT_ACP_COMMAND", "SOCIS_AGENT_COPILOT_ACP_ARGS",
    # SOCIS security policy / approval-routing context. These remain available
    ...
    "SOCIS_AGENT_YOLO_MODE", "SOCIS_AGENT_ACCEPT_HOOKS", "SOCIS_AGENT_REDACT_SECRETS",
    "SOCIS_AGENT_INTERACTIVE", "SOCIS_AGENT_EXEC_ASK", "SOCIS_AGENT_GATEWAY_SESSION",
    "SOCIS_AGENT_CRON_SESSION", "SOCIS_AGENT_SINGLE_QUERY_SESSION",
    "SOCIS_AGENT_SESSION_KEY", "SOCIS_AGENT_SESSION_PLATFORM",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if _env_var_policy_name(key) in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or SOCIS runtime location "
            "and security policy (SOCIS_AGENT_HOME, SOCIS_AGENT_YOLO_MODE, ...) "
            "cannot be persisted via "
            "the env writer. If you really need this, edit "
            "~/.socis-agent/.env directly."
        )
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # SOCIS_AGENT_TOOL_PROGRESS_MODE is deprecated (replaced by display.tool_progress
    ...
    # Activation is via plugins.enabled (opt-in through `socis plugins enable
    # observability/langfuse` or `socis tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "SOCIS_AGENT_LANGFUSE_ENV",
    "SOCIS_AGENT_LANGFUSE_RELEASE",
    "SOCIS_AGENT_LANGFUSE_SAMPLE_RATE",
    "SOCIS_AGENT_LANGFUSE_MAX_CHARS",
    "SOCIS_AGENT_LANGFUSE_CAPTURE",
    "SOCIS_AGENT_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
    # ACP (Agent Client Protocol) keys — profile-isolable so different
    # profiles can use different ACP backends without cross-leak.
    "SOCIS_AGENT_ACP_AUTH_METHOD",
    "SOCIS_AGENT_ACP_AUTO_APPROVE",
...
from socis_cli.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
...
# from `nix run` / `nix profile install` (which don't set SOCIS_AGENT_MANAGED).
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("SOCIS_AGENT_MANAGED", "").strip()
    marker = None
    if raw:
        marker = raw.lower()
    else:
        managed_marker = get_socis_agent_home() / ".managed"
        # An interactive shell reads the marker, because it does not see the
        # SOCIS_AGENT_MANAGED variable of the service. A marker with content
...
def is_managed() -> bool:
    """Check if SOCIS is running in package-manager-managed mode.

    Two signals: the SOCIS_AGENT_MANAGED env var (set by the systemd service),
    or a .managed marker file in SOCIS_AGENT_HOME (set by the NixOS activation
...
_NIX_UPDATE_MSG = (
    "Update SOCIS through the Nix source that installed it "
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``socis_cli/`` — i.e. the git checkout for source
    installs, ``/opt/socis`` inside the published image. It is a property of
    the running interpreter, NOT of ``$SOCIS_AGENT_HOME``, which is why a
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how SOCIS was installed: 'apt', 'docker', 'nix', 'nixos',
    ...
    2. Legacy home-scoped stamp ``$SOCIS_AGENT_HOME/.install_method`` — read for
       backward compatibility, but a ``docker`` value is IGNORED when we are
       not actually running inside a container (see below).
    3. SOCIS_AGENT_MANAGED env / .managed marker (NixOS managed mode)
    4. /nix/store/ path detection -> 'nix' (nix run / nix profile install)
    5. .git directory presence -> 'git'
    6. Fallback -> 'unknown'

    Why the stamp is code-scoped, not home-scoped (issue: shared ``~/.socis-agent``)
    --------------------------------------------------------------------------
    The install method describes *the binary that is running*, but
    ``$SOCIS_AGENT_HOME`` is a shared DATA directory — the Docker docs deliberately
    bind-mount it (``~/.socis-agent:/opt/data``) so config/sessions/memory persist
    and can be shared with a host-side Desktop/CLI install. When a
    containerised gateway and a host install share one ``$SOCIS_AGENT_HOME``, a
    ...
    ``docker`` and ``socis update`` refuses to run ("doesn't apply inside the
    ...
    The supported installs self-identify via the code-scoped stamp:
      ...
      - the published ``socis/socis-agent`` image bakes a ``docker``
        stamp into ``/opt/socis`` at build time.
    ...
    # 1. Code-scoped stamp — authoritative, immune to shared $SOCIS_AGENT_HOME.
    try:
    ...
    #    host install whose shared $SOCIS_AGENT_HOME was stamped by a co-located
    #    container, and honouring it wrongly blocks ``socis update``.
    try:
        method = (
            (get_socis_agent_home() / ".install_method")
    ...
    # detect Nix installs that don't set SOCIS_AGENT_MANAGED (e.g. ``nix run``,
...
def _running_in_container() -> bool:
    """Thin wrapper around ``socis_agent_constants.is_container`` (import-safe)."""
    try:
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$SOCIS_AGENT_HOME``, so that two installs sharing one data directory
    ...
    ``/opt/socis`` in the published image, which instead bakes the stamp at
...
# Long-form text for ``socis update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# socis_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side SOCIS, it doesn't update
...
✗ ``socis update`` doesn't apply inside the Docker container.

SOCIS Agent runs as a published image (socis/socis-agent), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull socis/socis-agent:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate socis-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm socis/socis-agent:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/socis/socis-agent/tags
  • Your config and session history live under ``$SOCIS_AGENT_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``socis update`` inside Docker.
...
def format_managed_message(action: str = "modify this SOCIS installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    return (
        f"Cannot {action}: this SOCIS installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall SOCIS."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from SOCIS_AGENT_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, socis_bin
    or None if container mode is not active, we're already inside the
    container, or SOCIS_AGENT_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("SOCIS_AGENT_DEV") == "1":
        return None
    ...
    container_mode_file = get_socis_agent_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "socis-agent")
    exec_user = info.get("exec_user", "socis")
    socis_bin = info.get("socis_bin", "/data/current-package/bin/socis")
...
# Re-export from socis_agent_constants — canonical definition lives there.
from socis_agent_constants import get_socis_agent_home, get_process_socis_agent_home  # noqa: F811,E402
from utils import atomic_replace, fast_safe_load

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_socis_agent_home() / "config.yaml"
...
def require_parseable_user_config(*, ignore_user_config: bool = False) -> None:
    ...
    """
    if ignore_user_config or os.environ.get("SOCIS_AGENT_IGNORE_USER_CONFIG") == "1":
        return
...
def get_env_path() -> Path:
    ...
    return get_socis_agent_home() / ".env"

def get_project_root() -> Path:
    """Get the project installation directory."""
    return Path(__file__).parent.parent.resolve()

def _resolve_socis_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the SOCIS_AGENT_UID / SOCIS_AGENT_GID env vars set by Docker deployments.

    Docker containers running SOCIS commonly set these to map the in-container
    user to a host user so volume-mounted state files end up with the right
    ownership. The entrypoint chowns the top-level SOCIS_AGENT_HOME once, but
    subdirectories created at runtime by ``ensure_socis_agent_home()`` (especially
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("SOCIS_AGENT_UID", "").strip()
    gid_str = os.environ.get("SOCIS_AGENT_GID", "").strip()
    try:
...
def _chown_to_socis_uid(path) -> None:
    """Chown ``path`` to ``SOCIS_AGENT_UID:SOCIS_AGENT_GID`` if those env vars are set.
    ...
    directories created by :func:`ensure_socis_agent_home` on Docker deployments.
    See #34107.
    """
    uid, gid = _resolve_socis_uid_gid()
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the socis group can
    share state with the gateway service.

    The mode can be overridden via the SOCIS_AGENT_HOME_MODE environment variable
    (e.g. SOCIS_AGENT_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse SOCIS_AGENT_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``SOCIS_AGENT_UID``/``SOCIS_AGENT_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("SOCIS_AGENT_HOME_MODE", "").strip()
    ...
    try:
        ...
        pass
    _chown_to_socis_uid(path)


def _is_container() -> bool:
    """Detect if we're running inside a Docker/Podman/LXC container.

    When SOCIS runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("SOCIS_AGENT_CONTAINER") or os.environ.get("SOCIS_AGENT_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set SOCIS_AGENT_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into SOCIS_AGENT_HOME, upgrading legacy empty templates.
...
# ensure_socis_agent_home(). Only successful passes are recorded, so a raised
# managed-mode/missing-profile error keeps re-checking on later loads.
_SOCIS_AGENT_HOME_ENSURED: set = set()


def ensure_socis_agent_home():
    """Ensure ~/.socis-agent directory structure exists with secure permissions.
    ...
    After the first successful pass for a given ``SOCIS_AGENT_HOME`` we only re-run
    ...
    ``get_socis_agent_home()`` and therefore re-run for the new path.
    """
    home = get_socis_agent_home()
    key = str(home)

    # Named profiles must be created explicitly (e.g. ``socis profile create``).
    # Check tombstones before the memo so a stale empty shell cannot skip
    # the deleted-profile guard.
    from socis_agent_constants import assert_named_profile_home_live
    assert_named_profile_home_live(home)
    if key in _SOCIS_AGENT_HOME_ENSURED and home.is_dir():
        return
    if is_managed():
        old_umask = os.umask(0o007)
        try:
            _ensure_socis_agent_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
    ...
    _SOCIS_AGENT_HOME_ENSURED.add(key)


def _ensure_socis_agent_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"SOCIS_AGENT_HOME {home} does not exist."
        )
...
from socis_cli.config_defaults import DEFAULT_CONFIG, OPTIONAL_ENV_VARS  # noqa: F401
...
def _split_key_path(key: str) -> list[str]:
    ...
    ``socis config set`` uses ``.`` as the nesting separator, so a key that
...
def _is_env_config_key(key: str) -> bool:
    """Return whether `socis config set` routes this key to .env."""
    if "." in key:
        return False
    key_upper = key.upper()
    api_keys = [
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.socis.config`` entries, then checks
    ...
    try:
        ...
        # should never break `socis update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    _KNOWN_KEYS = {
        ...
        # unused here, but SOCIS' own config writer has historically emitted it
    ...
    # SOCIS (``_save_discovered_models_to_config``), not hand-curated.
    # Older SOCIS versions wrote an in-mapping ``__discovered_model_catalog__``
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older SOCIS versions) may write
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``socis_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
_EXTRA_KNOWN_ROOT_KEYS = {
    ...
    "plugins",           # plugin enable/disable lists (socis_cli/plugins_cmd.py)
    "smart_model_routing",   # written by the setup wizard (socis_cli/setup.py)
    "platform_toolsets",     # written by the setup wizard (socis_cli/setup.py)
    "known_plugin_toolsets", # written/read by socis_cli/tools_config.py toolset-save flow
    "known_builtin_toolsets",  # ditto — which builtin toolsets a platform's checklist has offered
    "tool_gateway_declined_tools",  # per-tool Tool Gateway offer declines (socis_cli/nous_subscription.py, #92647)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'socis setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — SOCIS won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
    ...
    # scalars are bridged into os.environ (gateway/run.py, socis send) so
            ...
            for _key in ("backend", "search_backend", "extract_backend"):
                ...
                if note:
                    issues.append(ConfigIssue(
                        ...
                        "Run 'socis tools' and pick a different Web Search & Extract provider",
                    ))
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'socis doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars() -> None:
    ...
    if lines:
        ...
        hint_path = display_socis_agent_home()
...
def _persist_migration(config: Dict[str, Any]) -> None:
    ...
    DEFAULT_CONFIG dumps (the "socis update / socis -p blows up my config"
    reports).
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    from socis_cli.config_migrations import (
    ...
    if floor_refused:
        ...
        sys.stderr.write(f"⚠ socis config: {msg}\n")
        if not quiet:
            print(f"  ⚠ {msg}")
    else:
        # ── Versioned migration ladder (table-driven) ──
        # The per-version steps live in socis_cli.config_migrations as a
    ...
    if isinstance(raw_mcp_servers, dict):
        try:
            from socis_cli.mcp_security import validate_mcp_server_entry as _validate_mcp_server_entry
        except Exception:
    ...
    try:
        ...
        from socis_cli.toolset_validation import validate_platform_toolsets
        from socis_cli.toolset_scope import toolset_allowed_for_platform
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: socis config set <key> <value>")
    ...
    # `socis update`, but only the version bump is persisted.
    ...
    # metadata.socis.config in their SKILL.md frontmatter.
    ...
    * ``${env:VAR}`` — Cursor-style SecretRef, same resolution after the
    ...
    if inner.startswith("env:"):
        ...
        if val is not None:
            return val
        logger.warning(
            "Config ref %r: %s is not set (check ~/.socis-agent/.env); "
...
def _env_ref_snapshot(obj, snapshot=None):
    ...
    ``load_socis_dotenv()`` populated the process env, or an env var
...
def split_model_config_default(raw_default: Any) -> tuple[str, str]:
    ...
    together through ``SOCISCLI`` construction: the model becomes a plain
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    intuitive name OpenAI-SDK / LiteLLM users reach for, and ``socis config set``
    ...
    backends) — while display paths (``socis status``/``dump``) read ``name``
...
#   - survives the ``str() → int()`` round-trip through the SOCIS_AGENT_MAX_ITERATIONS
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _socis_agent_home / "config.yaml"`` local variable
...
# Back-compat alias — canonical set lives in socis_cli.personality.
from socis_cli.personality import NEUTRAL_PERSONALITY_NAMES as _NEUTRAL_PERSONALITY_NAMES  # noqa: F401


def _prompt_text(value: Any) -> str:
    """Normalize config prompt values from YAML before handing them to AIAgent.

    Delegates to :mod:`socis_cli.personality` — the single owner of
    personality/overlay semantics. Kept as a re-export for existing importers.
    """
    from socis_cli.personality import prompt_text
...
def render_personality_prompt(value: Any) -> str:
    ...
    from socis_cli.personality import render_personality_prompt as _render
...
def resolve_ephemeral_system_prompt_from_config(cfg: Optional[Dict[str, Any]]) -> str:
    ...
    still prefer ``SOCIS_AGENT_EPHEMERAL_SYSTEM_PROMPT`` when that env var is set.

    Delegates to :mod:`socis_cli.personality` (single owner).
    """
    from socis_cli.personality import resolve_ephemeral_system_prompt

    return resolve_ephemeral_system_prompt(cfg)


def read_raw_config() -> Dict[str, Any]:
    """Read ~/.socis-agent/config.yaml as-is, without merging defaults or migrating.
...
def read_user_config_raw(config_path: Optional[Path] = None) -> Dict[str, Any]:
    ...
    ``_socis_agent_home``, tui profile override, multi-profile probes).
    """
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.socis-agent/config.yaml.
    ...
    (which change ``SOCIS_AGENT_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
def _is_ssh_remote_tilde_cwd(backend: str, cwd: str) -> bool:
    ...
    Expanding ``~`` on the SOCIS host rewrites it to the host or container
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        # cache signature so editing /etc/socis/config.yaml invalidates the
        ...
        if cached is not None and cache_sig is not None and cached[:4] == cache_sig:
            ...
            # Without this, a load_config() that ran before load_socis_dotenv()
...
#   openai-codex (OAuth — socis auth) — OpenAI Codex
#   nous         (OAuth — socis auth) — Nous Portal
...
def save_config(
    ...
    *,
    ...
    """Save configuration to ~/.socis-agent/config.yaml.\n
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset SOCIS writes itself."""
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.socis-agent/.env.
    ...
    interactive menu render (`socis tools`, `socis setup`, status
    panels). Sanitisation is O(lines), so re-parsing the
    same file on every call was burning ~300ms of CPU per `socis tools`
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.socis-agent/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.socis-agent/.env."""
    if is_managed():
...
def custom_endpoint_key_env(identity: str) -> str:
    ...
    - The fixed ``SOCIS_AGENT_CUSTOM_`` prefix keeps the result a valid POSIX name
    ...
    return f"SOCIS_AGENT_CUSTOM_{slug}_API_KEY" if slug else "SOCIS_AGENT_CUSTOM_API_KEY"


def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.socis-agent/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.socis-agent/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to SOCIS — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known SOCIS vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ``os.environ`` or ``~/.socis-agent/.env``, scope-aware.
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a credential env value, preferring ``~/.socis-agent/.env`` over ``os.environ``.

    Used for SOCIS-managed credentials where a deliberate edit to ``.env``
...
def show_config():
    ...
    print(color("│              ◆ SOCIS Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    from socis_cli.auth import get_anthropic_key
    ...
    # Warn on stale SOCIS_AGENT_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("SOCIS_AGENT_MAX_ITERATIONS")
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                f"                ⚠ .env has stale SOCIS_AGENT_MAX_ITERATIONS={_env_ghost} "
                f"(run 'socis doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
    ...
    try:
        from socis_cli.personality import active_personality_name
    ...
    print(color("  socis config edit     # Edit config file", Colors.DIM))
    print(color("  socis config set <key> <value>", Colors.DIM))
    print(color("  socis setup           # Run setup wizard", Colors.DIM))
    print()
...
def resolve_cron_model_drift_defaults(
    ...
    *,
    ...
    wins ``SOCIS_AGENT_MODEL``; the environment is only a fallback. Per-job and cron
    ...
    provider = ""
    model = _model_assignment_text(env.get("SOCIS_AGENT_MODEL", ""))
...
def warn_unpinned_cron_jobs_after_model_config_change(
    ...
    print(
        ...
        "changed model/provider. Inspect with `socis cron list`, then pin the "
        "intended values with `socis cron edit <job_id> --provider <provider> "
        "--model <model>`."
    )
...
_SCHEMA_DEFINED_DICT_KEYS = frozenset({
    ...
    # (socis_cli/plugins_cmd.py, socis_cli/plugin_index.py). Absent from
    # DEFAULT_CONFIG (written only when used), so listed here for
    # `socis config set plugins.index_url ...` validation.
    "plugins",
})
...
def _known_top_level_keys() -> set[str]:
    ...
    to decide whether a ``socis config set`` invocation is targeting a
    known shape.
    """
...
def _suggest_closest_key(key: str, candidates: set[str], cutoff: float = 0.6) -> Optional[str]:
    ...
    similar enough to ``key``, else None.  Used by ``socis config set``
...
def _redirect_platform_display_key(key: str) -> tuple[str, Optional[str]]:
    ...
    ``socis config set platforms.telegram.streaming false`` landed on a key
...
def set_config_value(key: str, value: str, force: bool = False):
    ...
    Args:
        ...
        force: When True, skip the unknown-key warning — useful for scripted
            ...
            CLI exposes this via ``socis config set --force``.
    """
    if is_managed():
    ...
    if not isinstance(_default_value_for_key(key), str):
        ...
        if _lower in {'true', 'yes', 'on'}:
            ...
            #   socis config set platform_toolsets.line '["file","web"]'
            # or a multi-line YAML block:
            #   socis config set custom_providers '- name: foo
            #     base_url: https://...'
    ...
    # ``socis config set model.provider openai`` doesn't silently
    ...
    # (e.g. ``socis config set model gpt-5.6-sol`` when model already
    ...
    if "." not in key:
        ...
        if isinstance(_existing, dict):
            if key == "model":
                ...
                print(
                    f"    socis config set {key}.<sub-key> <value>",
                    file=sys.stderr,
                )
                print(
                ...
                print(
                    f"    socis config set --force {key} {value!r}",
                    file=sys.stderr,
                )
                sys.exit(1)
    try:
    ...
    # so a fresh `socis config set model.api_base ...` lands on the canonical
    ...
    if key == "display.skin" and isinstance(value, str) and value:
        try:
            skin_file = get_socis_agent_home() / "skins" / f"{value}.yaml"
            if skin_file.exists():
                skin_file.touch()
        except Exception:
    ...
    # — e.g. `socis config set model.api_key cfut_...` routes to config.yaml
...
def unset_config_value(key: str):
    ...
    if _is_env_config_key(key):
        ...
        # model-cache rows too, so `socis config unset <KEY>` fully removes
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key:
            print("Usage: socis config get <key> [--json]")
            print()
            print("Examples:")
            print("  socis config get model")
            print("  socis config get terminal.backend")
            print("  socis config get skills.config --json")
            sys.exit(1)
        ...
        if not key or value is None:
            print("Usage: socis config set [--force] <key> <value>")
            print()
            print("Examples:")
            print("  socis config set model anthropic/claude-sonnet-4")
            print("  socis config set terminal.backend docker")
            print("  socis config set OPENROUTER_API_KEY sk-or-...")
            print()
        ...
        key = getattr(args, 'key', None)
        if not key:
            print("Usage: socis config unset <key>")
            print()
            print("Examples:")
            print("  socis config unset model")
            print("  socis config unset terminal.backend")
            print("  socis config unset OPENROUTER_API_KEY")
            sys.exit(1)
        try:
        ...
        if missing_config:
            ...
            print("    Run 'socis config migrate' to add them")
        ...
        print("  socis config           Show current configuration")
        print("  socis config edit      Open config in editor")
        print("  socis config get <key>          Print a resolved config value")
        print("  socis config set <key> <value>   Set a config value")
        print("  socis config unset <key>        Remove a config value")
        print("  socis config check     Check for missing/outdated config")
        print("  socis config migrate   Update config with new options")
        print("  socis config path      Show config file path")
        print("  socis config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``socis config`` UI so users
