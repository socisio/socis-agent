"""``socis debug`` debug tools for SOCIS Agent.

Currently supports:
    socis debug share    Upload debug report (system info + logs) to a
                          ...
                          ``~/.socis-agent/logs/*.log`` are not leaked into
                          the public paste service. Pass ``--no-redact``
                          to disable.

For help, open an issue at https://github.com/socisio/socis-agent/issues.
SOCIS is MIT-licensed; there is no private support upload channel. An
upstream ``--nous`` option that sent the bundle to Nous's internal storage
(viewable by Nous staff and their Discord moderators) was removed.
"""
...
_REDACTION_BANNER = (
    "[socis debug share: log content redacted at upload time. "
...
def _pending_file() -> Path:
    """Path to ``~/.socis-agent/pastes/pending.json``.
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``socis debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``socis debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `socis debug share` "
...
def delete_paste(url: str) -> bool:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "socis-agent/debug-share"},
    )
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``socis debug share`` invocation added ~20 MB of resident Python
    interpreters that never exited until the sleep completed.

    The replacement is stateless: we append to ``~/.socis-agent/pastes/pending.json``
    and the gateway's cron ticker sweeps expired entries once per hour.
    ``socis debug share`` also runs an opportunistic sweep as a fallback
...
def _upload_paste_rs(content: str) -> str:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "socis-agent/debug-share",
        },
    )
    with urllib.request.urlopen(req, timeout=30) as resp:
...
def _upload_dpaste_com(content: str, expiry_days: int = 7) -> str:
    ...
    """
    boundary = "----SOCISDebugBoundary9f3c"
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from socis_cli.logs import LOG_FILES

    filename = LOG_FILES.get(log_name)
    return (get_socis_agent_home() / "logs" / filename) if filename else None


# Logs written by a client process rather than by this backend. When the
# desktop app talks to a remote/docker/SSH backend, `socis debug share` runs
...
_CLIENT_SIDE_LOGS = {
    "desktop": (
        "written by SOCIS Agent Desktop on the machine running the app, not by this "
...
def _capture_dump() -> str:
    """Run ``socis dump`` and return its stdout as a string."""
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``socis dump`` is run
        internally.
...
# ---------------------------------------------------------------------------
# Shared bundle collection (used by the paste upload and --local paths)
# ---------------------------------------------------------------------------
...
def collect_share_bundle(
    ...
    This is the single source of collection + redaction shared by the paste
    upload (:func:`build_debug_share`) and ``--local``, so both produce the
    *same* force-redacted snapshots — redaction is the safety boundary.
...
def build_debug_share(
    *,
    ...
    This is the shared core behind ``socis debug share`` (CLI) and the
    ...
    # shared collector so the uploaded and --local outputs build identical,
    ...
    if redact:
        logger.info(
            "socis debug share: applied force-mode redaction to log snapshots before upload"
        )
...
def run_debug_share(args):
    ...
    try:
        ...
        print("\nRun `socis debug share --local` to print the report instead.\n")
        sys.exit(1)
    ...
    print("To delete now:  socis debug delete <url>")

    print(
        "\nTo get help, open an issue at "
        "https://github.com/socisio/socis-agent/issues\n"
        "and include these links. Review them first: they are public."
    )
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: socis debug delete <url> [<url> ...]")
        print("  Deletes paste.rs pastes uploaded by 'socis debug share'.")
        return
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``socis debug`` call.
    ...
    # best-effort — any failure is swallowed so ``socis debug`` stays
    ...
    if subcmd == "share":
        ...
        # Default: show help
        print("Usage: socis debug <command>")
        print()
        print("Commands:")
