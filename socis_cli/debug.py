...
_NOUS_BUNDLE_FORMAT = "socis-debug-share/1"
...
def run_debug_share(args):
    ...
    print("\nShare these links with the SOCIS team for support.")
...
⚠️  --nous: This uploads your debug bundle to Nous-INTERNAL storage (AWS S3),
  ...
  • System info (OS, Python/SOCIS version, provider, which API keys are
...
def _run_debug_share_nous(args, *, log_lines: int, redact: bool) -> None:
    """Handle ``socis debug share --nous``: upload the bundle to Nous-S3.
    ...
    suggests ``--local``.
    """
    from socis_cli.diagnostics_upload import share_to_nous
    ...
    if redact:
        logger.info(
            "socis debug share --nous: applied force-mode redaction before upload"
        )
    ...
    try:
        ...
        print(
            ...
            "provisioned.\n"
            "Run `socis debug share --local` to print the report instead, "
            "or `socis debug share` to upload to a public paste service.\n",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    print(
        ...
        "  GitHub Issues        https://github.com/socisio/socis-agent/issues\n"
