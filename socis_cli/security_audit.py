"""On-demand supply-chain audit for SOCIS Agent installs.

Scans three surfaces a SOCIS user actually controls and we can map to
upstream advisories without auth or extra binaries:

1. The SOCIS venv (every PyPI dist via ``importlib.metadata``).
2. Python deps declared by user-installed plugins under ``~/.socis-agent/plugins``
...
def _discover_plugins(socis_agent_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.socis-agent/plugins``.
...
def _discover_components(
    *,
    ...
    home = socis_agent_home or Path(get_socis_agent_home())
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `socis security audit`."""
    ...
    components = _discover_components(
        skip_venv=skip_venv, skip_plugins=skip_plugins, skip_mcp=skip_mcp, socis_agent_home=home
    )
