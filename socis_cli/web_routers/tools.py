...
late-binding seam in :mod:`socis_cli.web_deps` (``late`` for callables,
...
from socis_cli.web_deps import late, LateState
from socis_cli.web_models import (
...
_log = logging.getLogger("socis_cli.web_server")
...
_spawn_socis_action = late("_spawn_socis_action")
...
async def get_toolsets(profile: Optional[str] = None):
    ...
    from socis_cli.platforms import platform_label
...
async def toggle_toolset(name: str, body: ToolsetToggle, profile: Optional[str] = None):
    ...
    # same background install `socis tools` runs interactively. Without this,
    ...
    # dead-end that forced users to discover `socis computer-use install`
...
async def get_toolset_config(name: str, profile: Optional[str] = None):
    ...
    Surfaces the same provider rows the CLI ``socis tools`` picker shows
...
async def get_toolset_models(
    ...
    The GUI counterpart of the model picker `socis tools` runs after a
...
async def select_toolset_provider(
    ...
    core extracted from the CLI configurator — so the GUI and ``socis tools``
...
async def save_toolset_env(name: str, body: ToolsetEnvUpdate, profile: Optional[str] = None):
    ...
    Writes each ``key: value`` to ``~/.socis-agent/.env`` via ``save_env_value`` —
    the same store ``socis tools`` writes when it prompts for keys. Keys are
...
async def run_toolset_post_setup(
    ...
    ``socis tools post-setup <key>`` and the frontend tails the log via
    ...
    ``profile`` spawns the hook as ``socis -p <profile> tools post-setup``.
    ...
    write per-profile state must see the same SOCIS_AGENT_HOME the rest of the
...
async def grant_computer_use_permissions(profile: Optional[str] = None):
    """Spawn ``socis computer-use permissions grant`` as a background action.
