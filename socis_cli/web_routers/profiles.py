...
late-binding seam in :mod:`socis_cli.web_deps` so tests that
...
def get_profiles_projects_tree(preview_limit: int = 3, session_limit: int = 2000):
    ...
    the repo-scan policy, the SOCIS_AGENT_HOME junk filters — through the
...
async def create_profile_endpoint(body: ProfileCreate):
    ...
    # profile's SOCIS_AGENT_HOME at import). Returns PIDs for the UI to poll.
...
async def get_active_profile_endpoint():
    ...
    ``active`` is the sticky default written by ``socis profile use`` —
    the profile new CLI invocations pick up. ``current`` is the profile
    the running dashboard/gateway is scoped to (derived from SOCIS_AGENT_HOME).
    """
    from socis_cli import profiles as profiles_mod
    ...
    def _run():
        ...
        # SOCIS_AGENT_HOME against the profiles root. Batched into one hop so the
...
async def set_active_profile_endpoint(body: ProfileActiveUpdate):
    """Set the sticky active profile (mirrors ``socis profile use``).
...
async def update_profile_soul(name: str, body: ProfileSoulUpdate):
    ...
    def _run():
        ...
        # profiles seed SOUL.md at the umask default (socis_cli.profiles
...
async def update_profile_model_endpoint(name: str, body: ProfileModelUpdate):
    ...
    to the named profile via the SOCIS_AGENT_HOME override.
    """
    profile_dir = _resolve_profile_dir(name)
    provider = (body.provider or "").strip()
    model = (body.model or "").strip()
...
async def describe_profile_auto_endpoint(name: str, body: ProfileDescribeAuto):
    ...
    (``auxiliary.profile_describer``). Mirrors ``socis profile describe
    <name> --auto``.
    ...
    def _run():
        from socis_cli import profile_describer
...
# ── Export / Import ──────────────────────────────────────────────────────────
# Profile sharing for the desktop: wraps socis_cli.profiles.export_profile /
# import_profile (the same machinery behind `socis profile export|import`).
