...
the late-binding seam in :mod:`socis_cli.web_deps` so tests that
``monkeypatch.setattr(web_server, "_spawn_socis_action", ...)`` keep
working.
"""
...
async def preview_skill_hub(identifier: str = "", profile: Optional[str] = None):
    ...
    def _run():
        from socis_cli.skills_hub import _resolve_source_meta_and_bundle
        from tools.skills_hub import create_source_router
