"""Default configuration data for SOCIS Agent.

Pure-data leaf module: DEFAULT_CONFIG and OPTIONAL_ENV_VARS, extracted
verbatim from socis_cli/config.py. Must not import from socis_cli.config.
"""
...
DEFAULT_CONFIG = {
    ...
    "toolsets": ["socis-cli"],
    # SQLite journal mode used by every SOCIS database opener. WAL is the
    ...
    # Soft file-descriptor limit for long-running SOCIS server processes.
    ...
    "session": {
        # Per-terminal `socis -c`: each CLI session drops a breadcrumb file
        # under $SOCIS_AGENT_HOME/terminal-sessions/<terminal-id>, and a bare
    ...
    "agent": {
        ...
        # all normalized by socis_cli.config.resolve_turn_limit.
        "max_turns": None,
        ...
        # budget. CLI one-shot equivalent: `socis chat --run-budget N`.
        ...
        # an interactive `socis gateway restart` must never block for hours
        ...
        # the SOCIS-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps SOCIS
        ...
        # identity slot (SOUL.md). Empty by default. The SOCIS_AGENT_ENVIRONMENT_HINT
        # env var overrides this (build-time/container mechanism).
        "environment_hint": "",
        # Coding posture — on interactive coding surfaces (CLI, TUI, desktop
        # app, ACP) in a code workspace, SOCIS adds a coding operating brief
        ...
        # `socis doctor`. Delivered via the status channel only —
        ...
        # `socis status` / fleet monitoring). Retries never stop — this is a
        ...
        # ``SOCIS_AGENT_LOCAL_STREAM_STALE_TIMEOUT`` overrides for escape-hatch use.
    ...
    "terminal": {
        ...
        # Root directory for SOCIS' terminal session temp files (background
        # logs/pid/exit files, code-execution sandboxes, etc.). When empty,
        # SOCIS uses TMPDIR/TMP/TEMP if set, otherwise a managed dir on real
        # storage at SOCIS_AGENT_HOME/cache/terminal (auto-pruned after 72h) — NOT
        ...
        # bot_relay from a short-lived `socis -p <bot> chat -Q` recipient
        ...
        #             SOCIS_AGENT_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force SOCIS_AGENT_HOME/home when it exists (old strict
        ...
        # skipped. When empty, SOCIS auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), SOCIS sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when SOCIS
        ...
        # bundled SOCIS image, which drops to the `socis` user via
    ...
    "web": {
        ...
        # `socis tools`, not a ring member). Set by the `socis tools`
    ...
    "browser": {
        ...
        #   Browser Use mode (default) — "lightpanda" makes SOCIS spawn
        ...
        # `socis doctor` say so. Also settable via AGENT_BROWSER_ENGINE.
        "engine": "auto",
        ...
        # browser tools) runs on a SOCIS-managed SNAPSHOT of the user's
        ...
        # fresh session launches — driven by SOCIS' packaged Chromium. Only
        ...
        # deletes the snapshot store (~/.socis-agent/browser-profile/) so copied
        ...
        # approval does it run `socis browser close-profile` (terminates the
        ...
        "camofox": {
            # When true, SOCIS sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and SOCIS should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``socis chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.socis-agent/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: socis sweeps the checkpoint base at startup
        ...
        # ``socis checkpoints prune`` command (add ``--keep-orphans`` to
    ...
    # AGENTS.md, CLAUDE.md, .socis.md, or .cursorrules before SOCIS applies
    ...
    # Single-query (``socis -q/-z "..."``) variant of mcp_discovery_timeout.
    ...
    # single read_file page exceeds these limits, SOCIS truncates the
    ...
    "compression": {
        ...
        "codex_app_server_auto": "native",  # Codex app-server (codex CLI runtime) thread
                                      ...
                                      # thread context, so SOCIS' summarizer cannot
                                      # shrink it (#36801). native = codex decides when
                                      # to compact its own thread (default); socis =
                                      # SOCIS' compression threshold triggers
        ...
        "codex_responses_native": False,  # Opt in to OpenAI's server-side compaction
                                      ...
                                      # route/model is unaffected. SOCIS' local
    ...
    "auxiliary": {
        ...
        # Invoked by ``socis kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``socis kanban decompose`` and the kanban
        ...
        # ``socis profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `socis model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `socis` (and `socis chat`) launches by default:
        ...
        # REPL and `--tui` (or SOCIS_AGENT_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `socis --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `socis -c` muscle memory.  Default off so existing
        # users aren't surprised.  SOCIS_AGENT_TUI_RESUME=<id> always wins.
        ...
        # When true (default), `socis --tui` drops a one-time hint
        ...
        # model — see socis_cli/focus_view.py.
        "focus_view": False,
        ...
        # can set `path`/`paths` to SOCIS_AGENT_HOME-relative YAML files/directories
        ...
        # e.g. `model · 68% · ~/projects/socis`. Per-platform overrides go under
        ...
        # CLI, TUI, and desktop app. Manage with `socis pets`. Disabled until
        ...
        "pet": {
            ...
            # get_socis_agent_home()/pets/. Empty → first installed pet.
            "slug": "",
    ...
    "dashboard": {
        ...
        # protocol ping (see socis_cli/web_server.py — an event-loop stall
        ...
        # SOCIS_AGENT_TUI_WS_ORPHAN_REAP_GRACE_S env var remains an internal
        ...
        # (SOCIS_AGENT_TUI_SESSION_TTL_S, default 6h) are closed with
        ...
        # ``SOCIS_AGENT_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``SOCIS_AGENT_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # (``SOCIS_AGENT_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # ``SOCIS_AGENT_DASHBOARD_DRAIN_SECRET`` env var (the .env-is-for-secrets
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; SOCIS appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.socis-agent/cache/piper-voices/
    ...
    # "Hey SOCIS" hands-free wake word. Always-on, on-device hotword
    ...
    "wake_word": {
        ...
        "phrase": "hey socis",       # for "sherpa" this IS the detected phrase (any text works); for other engines it's a cosmetic label — detection is keyed by the model/keyword below
        ...
        "openwakeword": {
            # "hey_socis" (the bundled, works-out-of-the-box default) OR a
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.socis-agent/plugins/.
    "context": {
        "engine": "compressor",
    ...
    "delegation": {
        ...
        # trim, the full text is spilled to ~/.socis-agent/cache/delegation/
    ...
    # response. If not, SOCIS feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before SOCIS auto-pauses the goal and
    ...
    "moa": {
        ...
        # <socis_agent_home>/moa-traces/<session_id>.jsonl. Off by default — turn it
    ...
    # goes to ~/.socis-agent/skills/ unless create_dir (below) redirects it.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Where agent-created skills (skill_manage action=create) are written.
        # Empty = the profile-local skills dir (~/.socis-agent/skills/). When set,
        ...
        # against SOCIS_AGENT_HOME. The directory is scanned for skills alongside
        # the local dir. e.g. "/opt/brain/skills"
        "create_dir": "",
        # Project-local skill discovery: when a session starts inside a git
        # checkout, ``<root>/.socis-agent/skills/`` and ``<root>/.agents/skills/``
        ...
        # with ``socis skills trust`` (run from inside it). Set to false to
        ...
        # Managed by ``socis skills trust`` / ``socis skills untrust``.
        "trusted_project_dirs": [],
        # Substitute ${SOCIS_AGENT_SKILL_DIR} and ${SOCIS_AGENT_SESSION_ID} in SKILL.md
        ...
        # (`socis skills install`). Runs ALONGSIDE the built-in skills
        ...
        # ~/.socis-agent/skills/.curator_ledger.jsonl with before/after file
        # hashes; file contents are stored content-addressed (deduped) under
        # ~/.socis-agent/.curator_backups/blobs/. Enables `socis curator ledger`
        # and single-mutation `socis curator rollback <entry-id>`.
    ...
    # cron daemon.
    #
    # See `socis curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `socis curator run --consolidate` overrides
        ...
        # normally restored on every `socis update`, so pruning them only
        ...
        # skills are kept forever. When > 0, `socis curator purge` deletes
        ...
        # skipped), snapshot ~/.socis-agent/skills/ into
        # ~/.socis-agent/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `socis curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for socis-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "◆ *SOCIS Agent*" header.
    ...
    # dangerous command. -q runs export SOCIS_AGENT_INTERACTIVE=1 (for interactive
    ...
    "approvals": {
        ...
        # modal; SOCIS_AGENT_TUI_NO_CONFIRM=1 force-skips that modal regardless of
    ...
    # replace SOCIS' built-in platform hint for a single messaging platform
    ...
    # ``socis plugins enable|disable`` and intentionally omitted here so an
    ...
    # stored approval from ~/.socis-agent/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or SOCIS_AGENT_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `socis doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``socis_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow SOCIS to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # $SOCIS_AGENT_HOME/plugins/<name>/) to relocate the trigger — e.g. "chronos",
        ...
        # Also overridable via SOCIS_AGENT_CRON_MAX_PARALLEL env var.
        ...
        # SOCIS_AGENT_CRON_SCRIPT_TIMEOUT. Keep this in sync with
        ...
        # SOCIS_AGENT_CRON_SESSION_DB_TIMEOUT env var. 0 = unlimited (skip the bound).
        ...
        # overridable via SOCIS_AGENT_CRON_MEDIA_SEND_TIMEOUT env var. Keep in
    ...
    # todos to ready, and fires `socis -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `socis` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `socis kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     socis-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core SOCIS tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.socis-agent/logs/.
    ...
    # update model picker lists without shipping a socis-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://agent.socis.io/docs/api/model-catalog.json",
        ...
        # /model or `socis model` invocation once the cache is older than
    ...
    # #8731).
    #
    # Provider keys accept the SOCIS provider id (as used elsewhere in
    ...
    "gateway": {
        ...
        # internal SOCIS_AGENT_GATEWAY_PLATFORM_CONNECT_TIMEOUT env var, which still
        ...
        # bridges these keys to the internal SOCIS_AGENT_STARTUP_WATCHDOG /
        # SOCIS_AGENT_STARTUP_WATCHDOG_TIMEOUT_S env vars AND applies them to the
        ...
        # producing ~/.socis-agent/sessions/sessions.json entirely.
        ...
        # the SOCIS_AGENT_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent
        ...
        # `launchctl kickstart ai.socis.gateway` that defenses 1-2 don't
        ...
        # `socis update` hang because it can never drain the gateway.
        "restart_loop_guard": {
            "max_restarts": 3,
            "window_seconds": 60,
            "max_gap_seconds": 300,
        ...
        # ``SOCIS_AGENT_GATEWAY_MAX_STARTS`` / ``SOCIS_AGENT_GATEWAY_START_WINDOW_S``
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.socis-agent/.env,
        ...
        # behavior: files must live under the SOCIS cache, under
        ...
        # user. Bridged to SOCIS_AGENT_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the SOCIS
        # cache (~/.socis-agent/cache/{documents,images,audio,video,screenshots})
        ...
        # to SOCIS_AGENT_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # SOCIS_AGENT_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to SOCIS_AGENT_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.socis-agent/state.db.
    ...
    "sessions": {
        ...
        # the default of ``socis sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.socis-agent/sessions/session_{sid}.json`` on every turn
        ...
        # user runs `socis sessions optimize-storage`, because the rebuild is
        # disk-heavy and long on large DBs (see that command's disk preflight).
        #
        #   "advise" (default): `socis update` prints a one-line notice with
        ...
        # ~/.socis-agent/lib/libfts5_cjk.so), 1-2 char CJK terms (일본, 项目, ...)
        ...
        # serve the cjk index. Bridged to SOCIS_AGENT_CJK_FTS (internal carrier).
        "cjk_fts": True,
        ...
        # SOCIS_AGENT_SEARCH_SLOW_MS (internal carrier).
        "search_slow_ms": 1000,
        ...
        # (non-streaming) export such as `socis sessions export`. Checked
    ...
    # ``socis doctor`` behaviour.
    "doctor": {
        # Per-probe timeout (seconds) for the opt-in `socis doctor --live`
        # real-call backend probes (Firecrawl/FAL/browser/MCP/TTS/STT).
        "live_probe_timeout": 10,
    },

    # ``socis update`` behaviour.
    "updates": {
        ...
        #     DBs) into <SOCIS_AGENT_HOME>/state-snapshots/ before the update.
        ...
        #     jobs) safety net.
        #   full — the quick snapshot PLUS a full ``socis backup``-style zip
        #     of SOCIS_AGENT_HOME into <SOCIS_AGENT_HOME>/backups/, restorable with
        #     ``socis import``. Can add minutes on large homes. This is the
        ...
        "backup_keep": 5,
        # What `socis update` does with uncommitted local changes to the
        ...
        # When `socis update` finds the source checkout parked on a feature
        ...
        #                         merge. `socis update --switch-branch`
    ...
    "lsp": {
        ...
        #              ``<SOCIS_AGENT_HOME>/lsp/bin/`` on first use.
    ...
    # `socis tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.socis-agent/.env.
    "secrets": {
        ...
        "bitwarden": {
            ...
            # in ~/.socis-agent/.env (or your shell) and never in config.yaml.
            ...
            # material under ~/.socis-agent/cache/. If a later startup cannot reach
            # Bitwarden due to NETWORK/TIMEOUT, SOCIS may use this encrypted
            ...
            # ~/.socis-agent/bin/ on first use.  When False you must install
        ...
        "onepassword": {
            ...
            # for headless auth.  Sourced from ~/.socis-agent/.env (or the shell)
    ...
    "computer_use": {
        ...
        # by default upstream. SOCIS disables it for our users unless they
        # explicitly opt in here. When false (default), SOCIS sets
        ...
        # cua-driver ≥ 0.6.x supports --no-overlay; SOCIS also calls
        ...
        "no_overlay": None,
        # cua-driver permission mode for each SOCIS computer-use runtime.
        ...
        # manifest used when permission_mode is "bounded". SOCIS passes the
    ...
    #
    # Configure with `socis egress setup`.  Disabled by default — the rest of
    # SOCIS works exactly as before with `enabled: false`.
    "proxy": {
        ...
        "tunnel_port": 9090,
        # Auto-download the pinned iron-proxy binary into ~/.socis-agent/bin/ on
    ...
    # SOCIS Agent Desktop (Electron app) launch options. These only affect
    # `socis desktop`; they do not touch the CLI/gateway.
    "desktop": {
        ...
        # Bridged to the SOCIS_AGENT_DESKTOP_DISABLE_GPU env var the Electron app reads.
        "disable_gpu": "auto",
        ...
        # Ignored on macOS/Windows. Bridged to the SOCIS_AGENT_DESKTOP_PASSWORD_STORE
    ...
    # SOCIS downloads official llama.cpp release binaries, then spawns and
    ...
    "local_runtime": {
        ...
        # (SOCIS still finds an external llama-server you run yourself).
        "enabled": False,
        # Pinned llama.cpp release tag (rolling bNNNN). Bumped by SOCIS
...
OPTIONAL_ENV_VARS = {
    ...
    "VERTEX_CREDENTIALS_PATH": {
        "description": "Path to a Google Cloud service account JSON for Vertex AI (Gemini). "
                       ...
                       "credentials SOCIS mints short-lived tokens from. Falls back to "
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'socis model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_DOMAIN": {
        "description": "Shared tool-gateway domain suffix for Nous Subscribers only, used to derive vendor hosts, e.g. socis.io -> firecrawl-gateway.nousresearch.com",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the SOCIS auth store)",
    ...
    "PORCUPINE_ACCESS_KEY": {
        "description": "Picovoice access key for the Porcupine 'Hey SOCIS' wake word engine (optional; openWakeWord is the free default)",
    ...
    "SOCIS_AGENT_LANGFUSE_PUBLIC_KEY": {
    ...
    "SOCIS_AGENT_LANGFUSE_SECRET_KEY": {
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use SOCIS, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @socis:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. SOCIS_AGENT_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #socis)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: socis-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'socis-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote SOCIS API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote SOCIS API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote SOCIS API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # SOCIS_AGENT_TOOL_PROGRESS_MODE is deprecated — tool progress is configured via
    # display.tool_progress in config.yaml (off|new|all|verbose|log). The
    # gateway still falls back to SOCIS_AGENT_TOOL_PROGRESS_MODE for backward
    ...
    # SOCIS_AGENT_TOOL_PROGRESS is fully unsupported since the v12 config support
    # floor retired its only consumer (the v3→4 migration).
    "SOCIS_AGENT_PREFILL_MESSAGES_FILE": {
    ...
    "SOCIS_AGENT_EPHEMERAL_SYSTEM_PROMPT": {
