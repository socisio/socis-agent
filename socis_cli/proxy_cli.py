...
The top-level command is ``socis egress``.  Note that the inbound OAuth
reverse-proxy command (``socis proxy``) lives elsewhere in
``socis_cli/main.py`` — different direction, different purpose.
"""
...
# ---------------------------------------------------------------------------
# Argparse wiring — called from socis_cli.main
# ---------------------------------------------------------------------------
...
def register_cli(parent_parser: argparse.ArgumentParser) -> None:
    ...
    Called from ``socis_cli.main`` as part of building the top-level
    ``socis egress`` parser.
    """

    # dest='egress_command' — keeps this subparser tree disjoint from the
    # inbound OAuth ``socis proxy`` subparser (which uses dest='proxy_command').
    ...
    setup.add_argument(
        ...
        help="Do not restart a running daemon after setup; you'll need to run "
             "`socis egress restart` yourself for changes to take effect.",
    )
...
def cmd_setup(args: argparse.Namespace) -> int:
    ...
    if args.from_bitwarden:
        ...
        if not bw_cfg.get("enabled"):
            ...
            console.print(
                "  Run `socis secrets bitwarden setup` first, or omit "
                "--from-bitwarden."
            )
            return 1
        try:
        ...
        # provider keys only in ~/.socis-agent/.env (loaded automatically when the
        # agent runs, but NOT exported into an interactive shell).  Fall back
        # to loading that file so `socis egress setup` finds the same keys the
        ...
        if loaded:
            console.print(
                ...
                f"~/.socis-agent/.env for discovery.[/dim]"
            )
    ...
    # explicitly requested rotation.  This prevents re-running `socis
    ...
    if rotate and existing:
        ...
        if _sys.stdin.isatty():
            console.print(
                ...
                "tokens in every running SOCIS sandbox.  They will start "
    ...
    # `socis egress reload` apply future ruleset changes without a
    ...
    # rotation), running `socis egress setup` again WITHOUT
    ...
    if do_restart:
        try:
            ...
            console.print(
                "  Run [cyan]socis egress start[/cyan] manually before "
        ...
        console.print(
            ...
            "changed.  Run [cyan]socis egress restart[/cyan] (or "
    ...
    console.print(
        "  Start:   [cyan]socis egress start[/cyan]\n"
        "  Restart: [cyan]socis egress restart[/cyan]  (after any re-setup)\n"
        "  Reload:  [cyan]socis egress reload[/cyan]   (apply ruleset edits "
        "in-place, no restart)\n"
        "  Status:  [cyan]socis egress status[/cyan]\n"
        "  Stop:    [cyan]socis egress stop[/cyan]\n"
        "  Disable: [cyan]socis egress disable[/cyan]"
    )
    return 0
...
def cmd_start(args: argparse.Namespace) -> int:
    ...
    if not proxy_cfg.get("enabled"):
        console.print(
            "[yellow]proxy.enabled is false — run `socis egress setup` "
            "first.[/yellow]"
        )
        return 1
    ...
    if credential_source == "bitwarden" and not refresh_bw:
        if bool(proxy_cfg.get("allow_env_fallback", False)):
            ...
            console.print(
                ...
                "back to env credentials with `socis egress setup "
    ...
    if refresh_bw:
        ...
        if not os.environ.get(bw_access_env, "").strip():
            ...
            console.print(
                ...
                "`socis egress setup --no-bitwarden` to switch back to "
        ...
        if not (bw_cfg or {}).get("project_id"):
            ...
            console.print(
                "  Run `socis secrets bitwarden setup` to configure the "
                "project, or switch back via `socis egress setup "
                "--no-bitwarden`."
            )
            return 1
...
def cmd_reload(args: argparse.Namespace) -> int:
    ...
    ``socis egress restart`` instead: the daemon reads real credentials
    ...
    console.print(
        ...
        "still need `socis egress restart` — the daemon reads real "
...
def format_status_text(*, show_tokens: bool = False) -> str:
    ...
    if bool(proxy_cfg.get("enabled")) and not status.configured:
        lines.extend(["", "Next: run `socis egress setup` to mint tokens and write proxy.yaml."])
    elif bool(proxy_cfg.get("enabled")) and not (status.pid and status.listening):
        lines.extend(["", "Next: run `socis egress start` before launching Docker sandboxes."])
...
def cmd_disable(args: argparse.Namespace) -> int:
    ...
    if ip.get_status().pid is not None:
        console.print(
            ...
            "[cyan]socis egress stop[/cyan] if you want it down too."
        )
    return 0
...
def cmd_config(args: argparse.Namespace) -> int:
    ...
    if status.config_path is None:
        console.print(
            "[yellow](no config generated — run `socis egress setup`)[/yellow]"
        )
        return 1
...
def _load_env_file_into_environ() -> int:
    """Backfill provider keys from ``~/.socis-agent/.env`` into ``os.environ``.

    ``socis egress setup`` discovers providers by reading ``os.environ``, but
    many operators keep their keys ONLY in ``~/.socis-agent/.env`` (which the agent
