"""
Unified self-relaunch for SOCIS CLI.

Preserves critical flags (--tui, --dev, --profile, --model, etc.) across
process replacement so that ``socis sessions browse`` or post-setup relaunch
doesn't silently drop the user's UI mode or other preferences.

Also works when ``socis`` is not on PATH (e.g. ``nix run`` or ``python -m``).
"""
...
def _extract_inherited_flags(argv: Sequence[str]) -> list[str]:
    """Pull out flags that should carry over into a self-relaunched socis."""
    flags: list[str] = []
    i = 0
    while i < len(argv):
        arg = argv[i]
...
def resolve_socis_bin() -> Optional[str]:
    """Find the socis entry point.

    Priority:
      1. ``sys.argv[0]`` if it resolves to a real executable.
      2. ``shutil.which("socis")`` on PATH.
      3. ``None`` → caller should fall back to ``python -m socis_cli.main``.
    ...
    ``socis.exe`` on PATH or the ``sys.executable -m socis_cli.main``
    fallback.
    """
    argv0 = sys.argv[0]
...
def build_relaunch_argv(
    ...
    *,
    ...
    """Construct an argv list for replacing the current process with socis.
...
def relaunch(
    ...
    *,
    ...
    """Replace the current process with a fresh socis invocation.
    ...
    relaunch contract wants: "run socis again as if the user had typed
    the new argv".
    ...
    *emulates* exec by spawning the child and exiting the parent, but
    ...
    is usually ``socis.exe`` (a Python console-script shim that wraps
    ``python -m socis_cli.main``) or a ``.cmd`` batch file, and both
    ...
    That's functionally equivalent — the user sees "socis exited, then
    new socis started" — just with two PIDs in play instead of one.
    """
    ...
    if sys.platform == "win32":
        ...
        try:
            ...
            # cryptic.  Common causes: ``socis`` not on PATH yet (install
            # hasn't propagated User PATH into this shell) or a stale shim.
            print(
                f"\nSOCIS relaunch failed: {exc}\n"
                f"Command: {' '.join(new_argv)}\n"
                f"Fix: open a new terminal so PATH picks up, then re-run socis.",
                file=sys.stderr,
            )
            sys.exit(1)
    else:
