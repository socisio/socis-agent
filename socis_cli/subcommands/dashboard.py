"""``socis dashboard`` / ``socis serve`` subcommand parsers.
...
``socis_cli/main.py:main()`` (god-file Phase 2); handler injected to avoid
importing ``main``.
"""
...
def _add_server_runtime_args(parser) -> None:
    ...
    # `socis dashboard` / `socis serve` cmdlines and SIGTERM them directly —
    # the same path `socis update` uses to clean up stale servers.
    parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running SOCIS web server processes and exit",
    )
    parser.add_argument(
        "--status",
        action="store_true",
        help="List running SOCIS web server processes and exit",
    )
...
def build_serve_parser(
    *,
    ...
    parser = argparse.ArgumentParser(
        prog="socis serve",
        description=(
            "Run the SOCIS backend server - the JSON-RPC/WebSocket gateway the "
...
def build_dashboard_parser(
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the SOCIS Agent web dashboard for managing config, API keys, and sessions",
    )
    _add_server_runtime_args(dashboard_parser)
    dashboard_parser.add_argument(
        "--no-open", action="store_true", help="Don't open browser automatically"
    )
    # Backward-compat shim: older SOCIS desktop app shells (<= 0.15.x) spawn the
    # backend as `socis dashboard --no-open --tui --host ... --port ...`. The
    ...
    # showed "SOCIS couldn't start" with no actionable cause. Accept and silently
    ...
    # browser. It exists so the SOCIS Agent Desktop app (and headless remote
    ...
    serve_parser = subparsers.add_parser(
        "serve",
        help="Start the SOCIS backend server (headless; powers the desktop app and remote backends)",
        description=(
            "Run the SOCIS backend server — the JSON-RPC/WebSocket gateway the "
            "desktop app and remote clients connect to. Headless: it never opens "
            "a browser UI."
    ...
    # `socis dashboard register` — register a self-hosted dashboard OAuth
    # client with Nous Portal and write the client_id into ~/.socis-agent/.env.
    # Nested subparser so bare `socis dashboard` keeps launching the server
    ...
    dashboard_register_parser = dashboard_subparsers.add_parser(
        ...
        description=(
            ...
            "SOCIS_AGENT_DASHBOARD_OAUTH_CLIENT_ID into ~/.socis-agent/.env, and prints "
            "how to engage the login gate. Requires being logged in (socis setup)."
        ),
    )
    ...
    dashboard_register_parser.add_argument(
        ...
        help=(
            ...
            "https://socis.example.com/auth/callback. Omit for localhost-only use."
        ),
    )
    dashboard_register_parser.add_argument(
        "--portal-url",
        ...
        help=(
            ...
            "portal. Also settable via SOCIS_AGENT_DASHBOARD_PORTAL_URL. Mainly for "
