...
def build_login_parser(subparsers, *, cmd_login: Callable) -> None:
    ...
    ``socis login`` was removed in favor of ``socis auth`` / ``socis model``
    (the runtime handler in ``socis_cli/auth.py::login_command`` just prints a
    deprecation message and exits).  The subparser is kept registered so that
    old scripts/aliases invoking ``socis login [--flags]`` still receive the
    ...
    - The subparser is registered WITHOUT a ``help=`` kwarg so the row is
      omitted from ``socis --help`` (argparse only lists subcommands that
    ...
    - ``--provider`` accepts ANY value (no ``choices=``) so that, e.g.,
      ``socis login --provider anthropic`` reaches the deprecation handler and
      gets pointed at ``socis model`` instead of crashing in argparse with
      ``invalid choice: 'anthropic'`` before the handler can run.
    """
    login_parser = subparsers.add_parser(
        "login",
        description=(
            "Deprecated. Use `socis auth` to manage credentials, "
            "`socis model` to select a provider, or `socis setup` for full setup."
        ),
    )
    ...
    login_parser.add_argument(
        "--provider",
        default=None,
        help="(deprecated) Provider name; ignored — see `socis model`",
    )
    login_parser.add_argument(
    ...
    login_parser.add_argument(
        "--client-id", default=None, help="OAuth client id to use (default: socis-cli)"
    )
