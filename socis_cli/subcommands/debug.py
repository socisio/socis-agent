...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for SOCIS Agent. Use 'socis debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    socis debug share              Upload debug report (asks for confirmation)
    socis debug share --yes        Skip confirmation (for scripts/CI)
    socis debug share --lines 500  Include more log lines
    socis debug share --expire 30  Keep paste for 30 days
    socis debug share --local      Print report locally (no upload)
    socis debug share --no-redact  Disable upload-time secret redaction
    socis debug share --nous       Upload to Nous-internal storage (private)
    socis debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'socis debug share'",
    )
