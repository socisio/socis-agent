"""``socis sync`` subcommand parser — Skill Sync.

Cloned from ``socis_cli/subcommands/cron.py`` — same injected-handler shape
...
Skill Sync covers two surfaces, both under this one command for launch:
  ...
  Personal — your own skills, across your own devices:
    socis sync status                 show gate/opt-in/head state
    socis sync pull                   pull and materialize opted-in skills
    socis sync push                   push opted-in skills
    socis sync now                    reconcile: pull then push
    socis sync enable <skill>         opt a skill into sync
    socis sync disable <skill>        opt a skill out of sync
    socis sync device [--name]        show or set this device's label

  Organisation — skills shared with your team:
    socis sync propose <skill>        share a skill with your organisation
...
def build_sync_parser(subparsers, *, cmd_sync: Callable) -> None:
    ...
    sync_parser = subparsers.add_parser(
        ...
        epilog=(
            "Examples:\n"
            "  socis sync status            what is synced, and from where\n"
            "  socis sync enable my-skill   include a skill in your sync\n"
            "  socis sync now               pull, then push\n"
            "  socis sync propose my-skill  share a skill with your team\n"
        ),
