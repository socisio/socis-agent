...
Three cookies in play:
  ...
  - socis_session_pkce: short-lived PKCE state + CSRF nonce + provider
...
https://datatracker.ietf.org/doc/html/draft-west-cookie-prefixes):
  ...
  * Gated HTTPS, direct deploy (Path=/) — ``__Host-`` prefix. Binds the
    ...
    guarantee.
  * Gated HTTPS, behind a reverse-proxy prefix (Path=/socis) —
    ``__Secure-`` prefix. ``__Host-`` is disallowed when Path != "/";
    ``__Secure-`` keeps the Secure-required hardening without the
    Path constraint, and the explicit ``Path=/socis`` covers
...
setter wrote ``__Secure-socis_session_at`` would never find the value.
...
SESSION_AT_COOKIE = "socis_session_at"
SESSION_RT_COOKIE = "socis_session_rt"
...
SESSION_PROVIDER_COOKIE = "socis_session_provider"
PKCE_COOKIE = "socis_session_pkce"
...
SSO_ATTEMPT_COOKIE = "socis_sso_attempt"
...
def _cookie_path(prefix: str) -> str:
    ...
    Under ``X-Forwarded-Prefix: /socis`` we want ``Path=/socis`` so:
...
def set_session_cookies(
    ...
    *,
    ...
    issues a 24h rotating refresh token (socis #37247); a provider that
    ...
    ``prefix`` is the normalised X-Forwarded-Prefix value (e.g. ``/socis``)
