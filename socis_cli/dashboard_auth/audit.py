...
Profile-aware location: ``$SOCIS_AGENT_HOME/logs/dashboard-auth.log``.
...
from ``socis_agent_constants`` or other socis_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$SOCIS_AGENT_HOME/logs/dashboard-auth.log``.

    Uses ``socis_agent_constants.get_socis_agent_home()`` (a leaf module — no import
    ...
    return get_socis_agent_home() / "logs" / "dashboard-auth.log"
