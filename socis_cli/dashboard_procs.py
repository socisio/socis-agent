"""Dashboard process-hygiene helpers — extracted from ``socis_cli/main.py``.
...
``_detect_concurrent_socis_instances``) are lifted verbatim. References to
helpers that STAY in ``socis_cli.main`` (``_find_stale_dashboard_pids``,
``_respawn_dashboard_processes``, ``_is_windows``, ...) are routed through a
lazy ``_m()`` main reference so existing test monkeypatches on
``socis_cli.main.<name>`` keep reaching this code path, and imports stay
...
patches on ``socis_cli.main`` resolve unchanged.
"""
...
def _m():
    """Lazy ``socis_cli.main`` reference (call-time; keeps patches working)."""
...
def _scan_dashboard_processes(
    *,
    ...
    ``socis dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``socis update`` replaces files on disk, the running
    ...
    ``socis-dashboard.service`` systemd unit.  Managed units are restarted
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    This is used by the SOCIS Agent Desktop Electron app to protect its own
    backend child process: when the desktop spawns ``socis serve`` as
    ...
    environment variable ``SOCIS_AGENT_DESKTOP_CHILD_PID`` on the spawned
    ...
    patterns = [
        ...
        # The headless backend (`socis serve`) is the same long-lived server
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "socis.*dashboard"`) keeps us consistent
            # with `socis_cli.gateway._scan_gateway_pids` and avoids the
    ...
    # miss profiled launches — `socis --profile p serve --host <ip>` contains
    # neither "socis serve" nor "socis_cli.main serve". Every serve/
    ...
    # truncated view.
    try:
        from socis_cli.process_identity import ledger_entries
...
def _socis_agent_home_for_pid(pid: int) -> str | None:
    """Best-effort ``SOCIS_AGENT_HOME`` from *pid*'s environment."""
    try:
        import psutil

        home = psutil.Process(pid).environ().get("SOCIS_AGENT_HOME")
        if home:
            return home
    except Exception:
        pass
    ...
    for part in raw.split(b"\x00"):
        if part.startswith(b"SOCIS_AGENT_HOME="):
...
def _is_ephemeral_port_zero_backend(argv: list[str]) -> bool:
    ...
    Ephemeral-port backends are owned by SOCIS Agent Desktop (or become PPID-1
    orphans after a prior update respawn).  Replaying them after
    ``socis update`` multiplies listening backends because ``--port 0``
...
def _profile_key_for_respawn(
    argv: list[str], socis_agent_home: str | None = None
) -> str:
    """Stable owner key: ``SOCIS_AGENT_HOME`` when known, else ``--profile`` / ``-p``.
    ...
    if socis_agent_home:
        try:
            home_path = Path(socis_agent_home).resolve()
        except (OSError, RuntimeError, ValueError):
            home_path = Path(socis_agent_home)
        ...
        try:
            ...
            return f"home:{os.path.normcase(socis_agent_home)}"
...
def _filter_dashboard_respawn_candidates(
    ...
    *,
    ...
    """Select which killed manual backends to respawn after ``socis update``.

    Each candidate is ``(pid, argv, socis_agent_home)``.  *own_home* is the
    updating install's home; it defaults to this process's
    ``get_socis_agent_home()`` and exists as a parameter so tests can pin it.

    Rules (#78821, #94030):
    1. Never resurrect Desktop ephemeral ``serve|dashboard --port 0``
       backends — Desktop (``SOCIS_AGENT_DESKTOP_CHILD_PID``) owns their
       lifecycle.  These are also the PPID-1 orphans that previously
       multiplied across updates because ``--port 0`` always binds a
       fresh free port.
    2. Never replay a backend from a **foreign** ``SOCIS_AGENT_HOME``.  The
    ...
    4. Cap at most one managed backend per profile / ``SOCIS_AGENT_HOME``.

    Intentionally does **not** blanket-skip every PPID-1 process: a prior
    ``socis update`` respawn detaches with ``start_new_session=True``, so
    ...
    if own_home is None:
        try:
            ...
            own_home = str(get_socis_agent_home())
        except Exception:
            own_home = ""
    ...
    for _pid, argv, socis_agent_home in candidates:
        if not argv:
            continue
        if _is_ephemeral_port_zero_backend(argv):
            continue
        if own_key and socis_agent_home and _normalized_home_for_compare(socis_agent_home) != own_key:
            continue
        norm = _normalize_dashboard_cmdline(argv)
        if norm in seen_cmdlines:
            continue
        profile_key = _profile_key_for_respawn(argv, socis_agent_home)
...
def _kill_stale_dashboard_processes(
    ...
    *,
    ...
    """Kill running ``socis dashboard`` / ``socis serve`` processes.

    Called at the end of ``socis update`` (default ``reason``) and also
    from ``socis dashboard --stop`` (which overrides ``reason``).  The
    ...
    When ``restart_managed`` is true (the ``socis update`` path), a detected
    ``socis-dashboard.service`` is restarted through systemd; any OTHER
    killed PID that was supervised by a systemd unit (custom unit names —
    e.g. a remote backend's ``socis-serve.service``) has its owning unit
    ...
    *already_restarted_units* names units (no ``.service`` suffix) the
    caller already restarted directly — e.g. ``socis update``'s systemd
    fleet-restart loop, which restarts ``socis-serve*`` units before this
    ...
    if restart_managed and _m()._restart_managed_dashboard_service(reason):
        ...
        # ``socis-dashboard.service`` and ``socis-serve.service`` -- the
        ...
        _dash_unit = getattr(
            _m(), "_DASHBOARD_SYSTEMD_UNIT", "socis-dashboard.service"
        )
        already_restarted_units = set(already_restarted_units or ()) | {
            str(_dash_unit).removesuffix(".service")
        }

    # When the SOCIS Agent Desktop Electron app spawns this dashboard as a
    # backend child, it sets SOCIS_AGENT_DESKTOP_CHILD_PID so that the update
    # path can skip killing the desktop-managed process.  (#37532)
    exclude: set[int] = set()
    raw_pid = os.environ.get("SOCIS_AGENT_DESKTOP_CHILD_PID")
    if raw_pid:
    ...
    # caller asked for restarts (the `socis update` path) — `--stop` must
    ...
    if restart_managed and sys.platform != "win32":
        for pid in pids:
            ...
            if not pid_service[pid]:
                ...
                # Snapshot SOCIS_AGENT_HOME before the kill so per-profile caps
                # still work after the process is gone (#78821).
                cmdline = _m()._dashboard_cmdline_for_pid(pid)
                if cmdline:
                    pid_cmdline[pid] = cmdline
                    pid_home[pid] = _socis_agent_home_for_pid(pid)

        if already_restarted_units:
            # Already handled directly by the caller (e.g. socis update's
        ...
        for pid in pids:
            try:
                ...
                if not pid_is_socis(
                    ...
                    failed.append((pid, "not socis-owned or process identity changed"))
                    continue
                result = subprocess.run(
    ...
    #    remote backend (socis serve) under Restart=on-failure never comes
...
def _detect_concurrent_socis_instances(
    ...
    The SOCIS Agent Desktop Electron app spawns ``socis.EXE`` as a backend child,
    so during ``socis update`` the user-invoked process and the desktop's
    ...
    shims (``socis.exe`` / ``socis-gateway.exe``) and returns ``(pid,
    process_name)`` pairs. The caller's own PID and its entire ancestor
    chain are excluded so the running ``socis update`` invocation never
    reports itself — this matters on Windows where the setuptools .exe
    launcher (``socis.exe``) is a separate process from the Python
    ...
    for shim in _m()._socis_exe_shims(scripts_dir):
        try:
    ...
    # setuptools-generated socis.exe launcher is a separate native process
    ...
    # file lock) is the parent. Without excluding it, every ``socis update``
    ...
    #      socis.exe sitting *under* a non-SOCIS parent (e.g. a SOCIS
...
def _is_desktop_local_serve_cmdline(command: str) -> bool:
    ...
    Desktop primary/pool backends launch as::
        ...
        socis serve --host 127.0.0.1 --port 0
        socis serve --isolated --host 127.0.0.1 --port 0 ...
    ...
    if "serve" not in cmd:
        return False
    if "socis" not in cmd and "socis_cli" not in cmd:
        return False
...
def _exclude_pids_from_env() -> set[int]:
    """PIDs Desktop marks as live backends (SOCIS_AGENT_DESKTOP_CHILD_PID)."""
    raw = os.environ.get("SOCIS_AGENT_DESKTOP_CHILD_PID", "")
    out: set[int] = set()
...
# on the *remote* host for every ``socis serve`` backend it spawns over SSH
# (see apps/desktop/electron/remote-lifecycle.ts). A backend started from
# another client/machine — e.g. a MacBook driving a ``socis serve`` on a Mac
...
def _socis_agent_home_dir() -> Path:
    """Resolved SOCIS home (SOCIS_AGENT_HOME override or ~/.socis-agent)."""
    override = os.environ.get("SOCIS_AGENT_HOME", "").strip()
    if override:
...
def _valid_lockfile_payload(parsed: object, ownership_id: str) -> bool:
    ...
    *adds* lock-owned PIDs to its spare-set.
    ...
    for field in ("profile", "socisPath", "socisHome", "logPath", "startedAt"):
        value = parsed.get(field)
        if not isinstance(value, str) or len(value) > 1024:
            return False
    # logPath is written as ``{lock_root}/{ownershipId}/{spawnNonce}.log``. We
    # only check the suffix so a relocated SOCIS_AGENT_HOME (different leading path)
...
def _lock_owned_serve_pids(base_dir: Path | None = None) -> set[int]:
    ...
    Scans ``{socis_agent_home}/desktop-ssh/<ownershipId>/backend.lock.json`` (the
    ...
    root = base_dir if base_dir is not None else (
        _socis_agent_home_dir() / _REMOTE_LOCK_SUBDIR
    )
    owned: set[int] = set()
    if not root.is_dir():
        return owned
...
def _reap_orphaned_desktop_local_serves(
    *,
    reason: str = "orphaned desktop-local socis serve",
    signal_term=None,
    signal_kill=None,
    sleep_fn=None,
    ...
    """Kill leftover Desktop-local ``socis serve`` backends with no parent.
    ...
    running under SOCIS_AGENT_PARENT_PID; this helper clears *already* orphaned
    ...
    - never self / never SOCIS_AGENT_DESKTOP_CHILD_PID entries
    ...
    - never a candidate younger than ``_REAP_MIN_AGE_SECONDS`` (or whose age
      ...
      only after the backend reports SOCIS_AGENT_BACKEND_READY, so during
    ...
    for pid, cmd in scanned:
        ...
        # SOCIS_AGENT_BACKEND_READY, so a sibling spawned seconds ago is not yet
