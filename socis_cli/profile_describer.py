...
Used by ``socis profile describe <name> --auto`` and the dashboard's
...
Design notes
------------
- Mirrors the shape of ``socis_cli/kanban_specify.py``: lazy aux
...
_SYSTEM_PROMPT = """You are a profile-describer for the SOCIS Agent kanban board.
...
Rules:
  ...
  - Never write "SOCIS Agent profile" or other meta-narration.
    ...
    *,
    ...
    try:
        if canon == "default":
            ...
            profile_dir = Path(get_socis_agent_home())
        else:
