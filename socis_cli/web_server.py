"""
SOCIS Agent — Web UI server.

Provides a FastAPI backend serving the Vite/React frontend and REST API
endpoints for managing configuration, environment variables, and sessions.

Usage:
    python -m socis_cli.main web          # Start on http://127.0.0.1:9119
    python -m socis_cli.main web --port 8080
"""
...
from socis_cli._subprocess_compat import windows_detach_flags, windows_hide_flags
from socis_cli.install_identity import get_install_id as _shared_get_install_id
...
from socis_cli.config import (
    ...
    get_config_path,
    get_env_path,
    get_socis_agent_home,
    get_process_socis_agent_home,
    load_config,
    load_env,
    read_raw_config,
...
try:
    ...
    # running `socis dashboard` needs fastapi+uvicorn; lazy install keeps
...
WEB_DIST = Path(os.environ["SOCIS_AGENT_WEB_DIST"]) if "SOCIS_AGENT_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    The scheduler tick loop normally lives in ``socis gateway run`` — but the
    desktop app spawns a ``socis dashboard`` backend, not a gateway, so a cron
    ...
    if isinstance(provider, InProcessCronScheduler):
        try:
            from socis_cli.profiles import profiles_to_serve
            ...
            if len(profile_homes) > 1:
                ...
                from socis_cli.profiles import _check_gateway_running

                start_kwargs["profile_gate"] = (
                    lambda _name, home: not _check_gateway_running(Path(home))
                )
                from socis_agent_logging import enable_profile_log_routing
...
def _warm_gateway_module() -> None:
    ...
    ``socis_cli.gateway``. But the first WS connection and its initial
    ...
    for mod in (
        ...
        "socis_cli.auth",
        "socis_cli.copilot_auth",
        "socis_cli.runtime_provider",
...
def _resolve_restart_drain_timeout() -> float:
    try:
        from socis_cli.gateway import _get_restart_drain_timeout
...
def _eager_reconcile_own_session_db() -> None:
    ...
    bringing a store left behind by `socis update` current before the
    ...
    """
    try:
        from socis_agent_state import SessionDB, _default_db_path
...
async def _lifespan(app: "FastAPI"):
    ...
    # so a store left behind by `socis update` kept 500ing every
    ...
    # Import socis_cli.gateway eagerly *before* the lifespan yield so the
    ...
    # model picker) can detect when `socis update` replaced the code
    ...
    # Desktop-spawned backends (SOCIS_AGENT_DESKTOP=1) fire cron jobs themselves,
    # since the app has no gateway running the scheduler. Server `socis
    # dashboard` is unaffected — it relies on its own gateway.
    cron_stop: "threading.Event | None" = None
    cron_thread: "threading.Thread | None" = None
    if os.getenv("SOCIS_AGENT_DESKTOP") == "1":
        ...
        # a healthy standalone gateway (launched via `socis gateway run`,
        ...
        try:
            from socis_cli.gateway import _reap_unsupervised_gateway_orphans
    ...
    def _boot_local_runtime():
        try:
            ...
            from socis_cli.local_runtime.bootstrap import ensure_local_runtime
    ...
    try:
        ...
        try:
            from socis_cli.local_runtime.bootstrap import shutdown_local_runtime
...
app = FastAPI(title="SOCIS Agent", version=__version__, lifespan=_lifespan)


# Memory-provider OAuth connect routes live in the memory layer, not here.
from socis_cli.memory_oauth import router as _memory_oauth_router  # noqa: E402
...
# SOCIS_AGENT_DASHBOARD_SESSION_TOKEN so its main process can authenticate the
...
def _resolve_session_token() -> str:
    return os.environ.get("SOCIS_AGENT_DASHBOARD_SESSION_TOKEN") or secrets.token_urlsafe(32)


_SESSION_TOKEN = _resolve_session_token()
_SESSION_HEADER_NAME = "X-SOCIS-Session-Token"
...
def _apply_ssh_owner_nonce(nonce: Optional[str]) -> None:
    ...
    if nonce:
        ...
        try:
            marker = os.path.join(purelib, f".socis-ssh-runtime-{nonce}")
...
#
# This list is defined in ``socis_cli.dashboard_auth.public_paths`` so the
...
# ---------------------------------------------------------------------------
from socis_cli.dashboard_auth.public_paths import (
...
def _require_token(request: Request) -> None:
    ...
    * **Loopback / ``--insecure`` mode** (``auth_required`` False): the
      ...
      back via ``X-SOCIS-Session-Token`` (or the legacy ``Bearer`` header).
      Validate it here.
...
def _dashboard_public_hosts() -> frozenset[str]:
    ...
    ``public_url`` is already SOCIS' canonical browser-facing URL behind a
    ...
    """
    from socis_cli.dashboard_auth.prefix import resolve_public_url
...
def should_require_auth(host: str, allow_public: bool = False) -> bool:
    ...
    unauthenticated-public-dashboard hole behind the June 2026 ``socis-0day``
...
def _desktop_loopback_auth_exempt(
    ...
    for EVERY ``socis serve`` on the machine — including the private loopback
    backends the Desktop app spawns for itself. Those backends authenticate
    with the per-spawn session token (injected via
    ``SOCIS_AGENT_DASHBOARD_SESSION_TOKEN`` for local spawns, ``--ssh-session-token
    ...
    Exemption requires ALL of: loopback bind, ``SOCIS_AGENT_DESKTOP=1`` (set by
    ...
    ``socis serve`` with ``SOCIS_AGENT_DESKTOP=1`` exported but no credential is
    NOT exempt.
    """
    ...
    if os.environ.get("SOCIS_AGENT_DESKTOP") != "1":
        return False
    return bool(
        os.environ.get("SOCIS_AGENT_DASHBOARD_SESSION_TOKEN")
    ...
    if path.startswith("/api/plugins/"):
        ...
        if _authed:
            ...
            if len(parts) >= 4:
                ...
                if plugin_name:
                    try:
                        from socis_cli.plugins_cmd import (
                            _get_enabled_set,
                            _get_disabled_set,
                        )
...
async def _dashboard_auth_gate(request: Request, call_next):
    from socis_cli.dashboard_auth.middleware import gated_auth_middleware
...
async def _token_auth_seam(request: Request, call_next):
    ...
    through untouched.
    """
    from socis_cli.dashboard_auth.token_auth import token_auth_middleware
...
_SCHEMA_OVERRIDES: Dict[str, Dict[str, Any]] = {
    ...
    "proxy.enabled": {
        "type": "boolean",
        "description": (
            "Docker-only egress credential firewall. Requires `socis egress setup` "
            "and `socis egress start`; Modal/SSH/Daytona are not wired yet."
        ),
        "category": "security",
    },
    ...
    "updates.non_interactive_local_changes": {
        "type": "select",
        "description": (
            "When the chat app / gateway updates SOCIS (no terminal prompt), "
    ...
    "updates.refresh_cua_driver": {
        "type": "boolean",
        "description": (
            "Refresh an already-installed cua-driver during socis update. "
...
from socis_cli.web_models import (  # noqa: F401
    ConfigUpdate,
    EnvVarUpdate,
    EnvVarDelete,
    EnvVarReveal,
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    - The "Change" picker sends a real SOCIS provider slug — fine.
    ...
    1. Vendor-name → SOCIS-provider mapping: when the provider string is
       not a known SOCIS provider/alias (e.g. ``moonshotai``, ``x-ai`` is
    ...
    """
    from socis_cli.config import get_compatible_custom_providers
    from socis_cli.models import _KNOWN_PROVIDER_NAMES, normalize_provider
    from socis_cli.model_normalize import normalize_model_for_provider
    from socis_cli.providers import resolve_custom_provider, resolve_user_provider
    ...
    # not be treated as a stray vendor prefix -- it isn't a known SOCIS
    ...
    if (
        ...
        try:
            ...
            cur_provider = ""
        from socis_cli.models import _AGGREGATOR_PROVIDERS
...
def _count_status_active_sessions() -> int:
    ...
    SOCIS process is using it.
    """
    from socis_agent_state import _default_db_path
...
_MANAGED_FILES_ROOT_ENV = "SOCIS_AGENT_DASHBOARD_FILES_ROOT"
...
# SOCIS_AGENT_HOME itself, at which point every one of these basenames is a live
...
# without needing to resolve them relative to SOCIS_AGENT_HOME.
...
def _is_sensitive_filename(name: str) -> bool:
    ...
    canonical SOCIS credential-store basenames (see
...
def _is_sensitive_path(path: Path) -> bool:
    ...
    and needs no SOCIS_AGENT_HOME resolution, so it blocks these trees wherever
...
def _default_socis_root_is_opt_data() -> bool:
    raw = os.environ.get("SOCIS_AGENT_HOME", "").strip()
    if not raw:
        return False
    try:
        from socis_agent_constants import get_default_socis_root

        root = get_default_socis_root().expanduser().resolve(strict=False)
    except (OSError, RuntimeError):
...
def _dashboard_local_update_managed_externally() -> bool:
    """Return true when the dashboard should not offer ``socis update``.
    ...
    a container — e.g. the socis-webui image sharing the SOCIS source tree),
    the dashboard's ``socis update`` button is the correct update path and
    ...
    """
    if _default_socis_root_is_opt_data():
        return True
    try:
        from socis_agent_constants import is_container
...
def _managed_files_policy(request: Request, *, create_root: bool = True) -> ManagedFilesPolicy:
    ...
    # to /opt/data only when the installation's SOCIS root is actually /opt/data
    # (the container/hosted layout) or when SOCIS_AGENT_DASHBOARD_FILES_ROOT is set.
    if _default_socis_root_is_opt_data():
...
async def upload_chat_image(payload: ChatImageUpload, profile: Optional[str] = None):
    ...
    The dashboard /chat page runs SOCIS inside an xterm.js PTY. Browser
...
async def fs_write_text(payload: FsWriteText):
    ...
    Mirrors the local Electron ``socis:fs:writeText`` hardening: the path is
    ...
    tmp = target.with_name(f".{target.name}.socis-tmp-{os.getpid()}")
    try:
...
# gate + path hardening as /api/fs). Logic lives in ``socis_cli.web_git``;
# these are thin, executor-offloaded wrappers (git/gh can block).
# ---------------------------------------------------------------------------

from socis_cli import web_git as _web_git  # noqa: E402
...
from socis_cli.web_routers import git as _git_routes  # noqa: E402

app.include_router(_git_routes.router)

from socis_cli.web_routers import local_models as _local_models_routes  # noqa: E402

app.include_router(_local_models_routes.router)
from socis_cli.web_routers.git import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    git_status_route,
    git_worktrees_route,
    git_branches_route,
...
def _profile_platform_ports(profile_home: Path, runtime: Optional[dict]) -> Dict[str, int]:
    ...
    try:
        ...
        from socis_cli.config import read_user_config_raw
...
def _collect_profile_gateway_topology() -> Dict[str, Any]:
    ...
    * ``profile_platforms`` — ``{profile: platforms}`` runtime platform maps
    ...
    """
    try:
        from socis_cli.profiles import _check_gateway_running, profiles_to_serve
...
# install, minted on first read and persisted under the ROOT SOCIS home
# (get_default_socis_root()) — NOT the profile-scoped SOCIS_AGENT_HOME — so every
...
_PROFILE_PLATFORM_STATUS_KEY_RE = re.compile(
    # Profile segment mirrors socis_cli.profiles._PROFILE_ID_RE.  Platform
...
async def get_status(profile: Optional[str] = None):
    ...
    # across that await. Status only resolves get_socis_agent_home() at call time
    ...
    try:
        ...
        # SOCIS_AGENT_HOME (see issue #69143). Plain /api/status keeps the exact
        ...
        # Windows install the first import of socis_cli.gateway blocks the
        ...
        # and which providers are registered so ``socis status`` and the
        # SPA's StatusPage can show "OAuth gate ON via SOCIS" or
        ...
        auth_flows: list[str] = []
        try:
            from socis_cli.dashboard_auth import (
        ...
        try:
            from socis_cli.auth import get_nous_session_validity
        ...
        status = {
            ...
            "can_update_socis": not _dashboard_local_update_managed_externally(),
            "gateway_running": gateway_running,
            "gateway_state": gateway_state,
        ...
        try:
            ...
            storage_check = await run_in_threadpool(_probe_state_db, get_socis_agent_home())
        ...
        try:
            ...
            status["memory"] = await run_in_threadpool(
                ...
                profile_dir if profile_dir else get_socis_agent_home(),
            )
        except Exception:
            status["memory"] = {"pressure": "unknown"}

        # Disk-usage rollup (NS-656, same lineage as OOF-2/OOF-107 fleet
        # disk-exhaustion incidents). One statvfs call on SOCIS_AGENT_HOME's
        ...
        try:
            from socis_agent_state import SessionDB as _SDB
            from socis_agent_constants import get_socis_agent_home as _ghh
        ...
        # ``gateway_mode`` are low-sensitivity PRODUCT surface — SOCIS Cloud
...
async def get_system_stats():
    ...
    and non-sensitive (no env values, no paths beyond the socis home root).
    """
    ...
    try:
        ...
        try:
            du = psutil.disk_usage(str(get_socis_agent_home()))
            info["disk"] = {
                "total": du.total,
                "used": du.used,
                "free": du.free,
...
async def run_curator():
    ...
    try:
        proc = _spawn_socis_action(["curator", "run"], "curator-run")
    except Exception as exc:
...
def _get_portal_status_sync():
    ...
    auth: Dict[str, Any] = {}
    try:
        from socis_cli.auth import get_nous_auth_status_local
    ...
    features = []
    try:
        from socis_cli.nous_subscription import get_nous_subscription_features
...
async def run_prompt_size():
    try:
        proc = _spawn_socis_action(["prompt-size"], "prompt-size")
    except Exception as exc:
...
async def run_dump():
    try:
        proc = _spawn_socis_action(["dump"], "dump")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
async def run_config_migrate():
    try:
        proc = _spawn_socis_action(["config", "migrate"], "config-migrate")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
async def run_debug_share_endpoint(body: DebugShareRequest | None = None):
    ...
    *synchronous*: the whole point of ``debug share`` is the set of shareable
    ...
    """
    from socis_cli.debug import build_debug_share
...
# streamed to a per-action log file under ``~/.socis-agent/logs/<action>.log`` so
# the dashboard can tail them back to the user.
# ---------------------------------------------------------------------------

_ACTION_LOG_DIR: Path = get_socis_agent_home() / "logs"
...
_UPDATE_ACTION_COMPLETED_RE = re.compile(
    r"^=== socis-update completed ([0-9a-f]{32}) ===$"
)
...
def _dashboard_spawn_executable() -> str:
    ...
    runpy.run_module('socis_cli.main', ...)"``) — so ``sys.executable`` is
...
def _spawn_socis_action(
    subcommand: List[str],
    name: str,
    *,
    env_overrides: Optional[Dict[str, str]] = None,
) -> subprocess.Popen:
    """Spawn ``socis <subcommand>`` detached and record the Popen handle.

    Uses the running interpreter's ``socis_cli.main`` module so the action
    ...
    cmd = [_dashboard_spawn_executable(), "-m", "socis_cli.main", *subcommand]

    # The dashboard runs *inside* the gateway process, so os.environ carries
    # _SOCIS_AGENT_GATEWAY=1. Inheriting it makes a spawned `socis gateway restart`
    ...
    action_env = {**os.environ, "SOCIS_AGENT_NONINTERACTIVE": "1"}
    action_env.pop("_SOCIS_AGENT_GATEWAY", None)
    ...
    action_id = (env_overrides or {}).get("SOCIS_AGENT_ACTION_ID")
    if action_id:
...
def _durable_completed_update_action_id(lines: List[str]) -> Optional[str]:
    ...
    for index, line in enumerate(lines):
        if line.startswith("=== socis update started "):
            last_start = index
...
def _gateway_display_command(profile: Optional[str], verb: str) -> str:
    return " ".join(["socis", *_gateway_subcommand(profile, verb)])
...
def _spawn_gateway_restart(profile: Optional[str] = None) -> Tuple[subprocess.Popen, bool]:
    """Spawn ``socis gateway restart``, reusing an in-flight restart.
    ...
    concurrent ``socis gateway restart`` children race each other on the
    ...
    proc = _spawn_socis_action(subcommand, "gateway-restart")
...
async def restart_gateway(profile: Optional[str] = None):
    """Kick off a ``socis gateway restart`` in the background."""
    try:
...
async def gateway_drain(request: Request):
    ...
    active (no ``SOCIS_AGENT_DASHBOARD_DRAIN_SECRET``), the route is NOT a token
...
@app.post("/api/socis/update")
async def update_socis():
    """Kick off ``socis update`` in the background."""
    if _dashboard_local_update_managed_externally():
        message = (
            "SOCIS updates are managed outside this dashboard in "
            "containerized environments. The built-in local updater is "
            "disabled here."
        )
        _record_completed_action("socis-update", message, exit_code=1)
        return {
            "ok": False,
            "pid": None,
            "name": "socis-update",
    ...
    if refusal is not None:
        _record_completed_action("socis-update", refusal.message, exit_code=1)
    ...
    existing = _ACTION_PROCS.get("socis-update")
    if existing is not None and existing.poll() is None:
        response = {
            "ok": True,
            "pid": existing.pid,
        ...
        action_id = _ACTION_IDS.get("socis-update")
        if action_id:
    ...
    try:
        proc = _spawn_socis_action(
            ["update"],
            "socis-update",
            env_overrides={"SOCIS_AGENT_ACTION_ID": action_id},
        )
    except Exception as exc:
        _log.exception("Failed to spawn socis update")
...
@app.get("/api/socis/update/check")
async def check_socis_update(force: bool = False):
    """Report whether a SOCIS update is available, without applying it.

    Powers the dashboard's "check before you update" flow: the System page
    shows the commit-behind count and asks the user to confirm before
    ``POST /api/socis/update`` actually runs ``socis update``.

    Returns:
        install_method: 'apt' | 'git' | 'docker' | 'nix' | 'nixos' | 'unknown'
        current_version: installed SOCIS version string
    ...
    if install_method == "apt":
        payload["message"] = (
            "SOCIS is managed by Termux APT; run `pkg upgrade socis-agent`."
        )
        return payload
    ...
    try:
        from socis_cli.banner import check_for_updates

        if force:
            try:
                (get_socis_agent_home() / ".update_check").unlink()
            except OSError:
                pass
...
async def speak_text(payload: TTSSpeakRequest, profile: Optional[str] = None):
    ...
    configured in ``~/.socis-agent/config.yaml`` under ``tts.``.
    """
...
async def get_action_status(name: str, lines: int = 200):
    ...
    if name == "socis-update":
        ...
        if durable_update_action_id:
            marker = f"=== socis-update completed {durable_update_action_id} ==="
...
def _latest_update_receipt_summary() -> Optional[Dict[str, Any]]:
    ...
    Phase-1 bullet 3 (#91277): the receipt (written by EVERY ``socis
    ...
    """
    try:
        from socis_cli.update_receipt import read_latest_receipt
...
@app.get("/api/socis/update/receipt")
async def get_update_receipt():
    ...
    if not receipt:
        raise HTTPException(
            status_code=404,
            detail="No update receipt found (no `socis update` run recorded).",
        )
...
from socis_cli.web_routers import sessions as _sessions_routes  # noqa: E402

app.include_router(_sessions_routes.list_router)
from socis_cli.web_routers.sessions import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    get_sessions,
)


from socis_cli.web_routers import profiles as _profiles_routes  # noqa: E402

app.include_router(_profiles_routes.sessions_router)
from socis_cli.web_routers.profiles import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    SOCIS supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
def _flat_json_path(provider: ProviderConfigSchema) -> Path:
    return get_socis_agent_home() / provider.name / "config.json"
...
def _install_memory_provider_pip_dependencies(dependencies: List[str]) -> List[Dict[str, Any]]:
    ...
    # under /opt/socis is sealed read-only, and installs must be redirected
    # to the writable durable target on the data volume
    # (SOCIS_AGENT_LAZY_INSTALL_TARGET, e.g. /opt/data/lazy-packages) — the same
...
def _read_memory_provider_existing_values(name: str) -> Dict[str, Any]:
    ...
    for path in (
        socis_agent_home / f"{name}.json",
        socis_agent_home / name / "config.json",
    ):
...
def _save_memory_provider_native_config(name: str, provider: Any, values: Dict[str, Any]) -> None:
    if provider is not None and hasattr(provider, "save_config"):
        try:
            ...
            provider.save_config(values, str(get_socis_agent_home()))
            return
...
async def get_egress_status():
    ...
    from socis_cli.proxy_cli import format_status_text
...
# in socis_cli/config.py — listed here for deterministic ordering in the UI.
...
def _dashboard_code_skew_guard() -> Optional[str]:
    ...
    The dashboard and Desktop-owned ``socis serve`` are long-lived; their
    ``sys.modules`` is frozen at boot.  When ``socis update`` (or a manual
...
def _dashboard_skew_restart_hint() -> str:
    ...
    ``socis serve --isolated`` (local or SSH). Hardcoding a systemd unit
    misleads macOS/launchd hosts and Desktop SSH backends, which have no
    ``socis-dashboard`` unit (#97046).
    """
    if os.environ.get("SOCIS_AGENT_SERVE_HEADLESS") == "1":
        return (
            "restart the Desktop-owned backend to load the new code "
            "(use Restart backend in SOCIS Agent Desktop, or quit and reopen the app)"
        )
    return (
        "restart this SOCIS process to load the new code "
        "(socis dashboard --port <port>, or the equivalent service restart for this install)"
    )
...
async def get_model_options(
    ...
    try:
        ...
        from socis_cli.inventory import build_model_options_payload, load_picker_context
...
def get_recommended_default_model(provider: str = ""):
    ...
    Mirrors the model-curation `socis model` does so GUI onboarding lands on a
    ...
    if slug == "nous":
        try:
            ...
            from socis_cli.auth import get_provider_auth_state
    ...
    try:
        from socis_cli.inventory import build_models_payload, load_picker_context
        from socis_cli.models import pick_silent_default_model
...
def get_moa_models(profile: Optional[str] = None):
    ...
    try:
        from socis_cli.moa_config import normalize_moa_config
...
def set_moa_models(body: MoaConfigPayload, profile: Optional[str] = None):
    ...
    try:
        from socis_cli.moa_config import normalize_moa_config, validate_moa_payload
...
async def set_model_assignment(body: ModelAssignment, profile: Optional[str] = None):
    ...
    Writes to ``~/.socis-agent/config.yaml`` — applies to **new** sessions only.
    ...
    try:
        ...
        if model and not body.confirm_expensive_model:
            try:
                from socis_cli.model_selection_guards import combined_selection_warning
...
def _apply_model_assignment_sync(
    ...
    if scope == "main":
        ...
        # post-model-selection behaviour (socis_cli/main.py
        ...
        if provider.strip().lower() == "nous":
            try:
                from socis_cli.nous_subscription import apply_nous_managed_defaults
        ...
        # endpoint, mirroring the ``socis model`` custom flow
        ...
        if provider.strip().lower() in {"custom", "local"} and base_url:
            try:
                from socis_cli.main import _auto_provider_name, _save_custom_provider
...
async def update_config(body: ConfigUpdate, profile: Optional[str] = None):
    def _run():
        ...
        # different SOCIS_AGENT_HOME than this process's gateway sessions.
...
def _is_other_profile(profile: Optional[str]) -> bool:
    ...
    try:
        ...
        return True
    return target.resolve() != get_process_socis_agent_home().resolve()
...
def _catalog_provider_env_metadata() -> dict:
    ...
    (i.e. the ``socis model`` universe). This is what lets the desktop Keys tab
    ...
    """
    try:
        from socis_cli.provider_catalog import provider_catalog
    except Exception:
        return {}
    ...
    try:
        from socis_cli.config import OPTIONAL_ENV_VARS as _OPT
    except Exception:
        _OPT = {}
    _non_provider_keys = {
    ...
    for d in provider_catalog():
        ...
        # Keys tab (otherwise Bedrock — a `socis model` provider — would be
        ...
        # Vertex — a `socis model` provider — would be invisible in the desktop
...
def _get_env_vars_sync(profile: Optional[str] = None):
    ...
    def _row(var_name: str, info: dict, *, custom: bool = False) -> dict:
        ...
        return {
            ...
            # CLI `socis model` picker uses (not desktop-only prefix guesses).
...
async def set_env_var(body: EnvVarUpdate, profile: Optional[str] = None):
    def _run():
        with _profile_scope(body.profile or profile):
            ...
            from socis_cli.credential_lifecycle import save_provider_env_credential
...
def list_custom_endpoints(profile: Optional[str] = None):
    ...
    the process-level SOCIS_AGENT_HOME. Mirrors ``/api/config``'s profile scoping.
    """
    try:
        with _config_profile_scope(profile):
...
async def remove_env_var(body: EnvVarDelete, profile: Optional[str] = None):
    def _run():
        with _profile_scope(body.profile or profile):
            ...
            from socis_cli.credential_lifecycle import remove_provider_env_credential
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    "telegram": {
        "name": "Telegram",
        "description": "Run SOCIS from Telegram DMs, groups, and topics.",
    ...
    "discord": {
        "name": "Discord",
        "description": "Connect SOCIS to Discord DMs, channels, and threads.",
    ...
    "slack": {
        "name": "Slack",
        "description": "Use SOCIS from Slack via Socket Mode. Add allowed Slack member IDs so connected bots can respond.",
    ...
    "mattermost": {
        "name": "Mattermost",
        "description": "Connect SOCIS to Mattermost channels and direct messages.",
    ...
    "matrix": {
        "name": "Matrix",
        "description": "Use SOCIS in Matrix rooms and direct messages.",
    ...
    "whatsapp": {
        "name": "WhatsApp",
        "description": "Use SOCIS through the bundled WhatsApp bridge with QR-based auth.",
    ...
    "homeassistant": {
        "name": "Home Assistant",
        "description": "Control your smart home from SOCIS via Home Assistant.",
    ...
    "email": {
        "name": "Email",
        "description": "Talk to SOCIS through an IMAP/SMTP mailbox.",
        "docs_url": "https://agent.socis.io/docs/user-guide/messaging/",
        "env_vars": (
            "EMAIL_ADDRESS",
            "EMAIL_PASSWORD",
            "EMAIL_IMAP_HOST",
    ...
    "dingtalk": {
        "name": "DingTalk",
        "description": "Connect SOCIS to DingTalk groups (钉钉).",
    ...
    "feishu": {
        "name": "Feishu / Lark",
        "description": "Use SOCIS inside Feishu / Lark.",
    ...
    "google_chat": {
        "name": "Google Chat",
        "description": "Connect SOCIS to Google Chat via Cloud Pub/Sub.",
        "docs_url": "https://agent.socis.io/docs/user-guide/messaging/google_chat",
    },
    "wecom": {
    ...
    "weixin": {
        ...
        "docs_url": "https://agent.socis.io/docs/user-guide/messaging/weixin/",
        "env_vars": ("WEIXIN_ACCOUNT_ID", "WEIXIN_TOKEN", "WEIXIN_BASE_URL"),
        "required_env": ("WEIXIN_ACCOUNT_ID", "WEIXIN_TOKEN"),
    },
    "bluebubbles": {
        "name": "BlueBubbles (iMessage)",
        "description": "Use SOCIS through iMessage via a BlueBubbles server.",
    ...
    "qqbot": {
        "name": "QQ Bot",
        "description": "Connect SOCIS to a QQ Bot from the QQ Open Platform.",
    ...
    "teams": {
        "description": "Connect SOCIS to Microsoft Teams chats via the Bot Framework.",
        "docs_url": "https://agent.socis.io/docs/user-guide/messaging/teams",
    },
    ...
    "irc": {
        "description": "Relay messages between an IRC channel (or DMs) and SOCIS.",
        "docs_url": "https://agent.socis.io/docs/user-guide/messaging/irc",
    },
    "line": {
        "description": "Use SOCIS from LINE via the LINE Messaging API webhook.",
        "docs_url": "https://agent.socis.io/docs/user-guide/messaging/line",
    },
    "ntfy": {
        "description": "Chat with SOCIS over ntfy push topics (ntfy.sh or self-hosted).",
        "docs_url": "https://agent.socis.io/docs/user-guide/messaging/ntfy",
    },
    "photon": {
        "description": "Use SOCIS through iMessage via Photon's managed Spectrum platform.",
        "docs_url": "https://agent.socis.io/docs/user-guide/messaging/photon",
    },
    "raft": {
        "description": "Join a Raft workspace as an external agent.",
        "docs_url": "https://agent.socis.io/docs/user-guide/messaging/raft",
    },
    "simplex": {
        "description": "Talk to SOCIS over SimpleX Chat via a local simplex-chat daemon.",
        "docs_url": "https://agent.socis.io/docs/user-guide/messaging/simplex",
    },
    "yuanbao": {
        "name": "Yuanbao (元宝)",
        "description": "Connect SOCIS to Tencent Yuanbao.",
        "docs_url": "",
        "required_env": (),
    },
    "api_server": {
        "name": "API server",
        "description": "Expose SOCIS as an OpenAI-compatible HTTP API for tools like Open WebUI.",
        "docs_url": "https://agent.socis.io/docs/user-guide/messaging/",
        "env_vars": (
            "API_SERVER_ENABLED",
            "API_SERVER_KEY",
    ...
    "webhook": {
        ...
        "docs_url": "https://agent.socis.io/docs/user-guide/messaging/webhooks/",
    ...
    "msgraph_webhook": {
        ...
        "docs_url": "https://agent.socis.io/docs/user-guide/messaging/msgraph-webhook",
        "required_env": (),
    },
    "whatsapp_cloud": {
        "name": "WhatsApp Cloud API",
        "description": "Use SOCIS via Meta's hosted WhatsApp Cloud API (no local bridge).",
        "docs_url": "https://agent.socis.io/docs/user-guide/messaging/whatsapp-cloud",
    },
    "relay": {
        "name": "Relay (experimental)",
        "description": "Generic relay adapter fronted by the SOCIS Relay connector.",
        "docs_url": "",
        "required_env": (),
    },
}
...
_MESSAGING_ENV_FALLBACKS: dict[str, dict[str, Any]] = {
    ...
    "WEIXIN_ACCOUNT_ID": {
        "description": "iLink Bot account ID obtained through QR login in socis gateway setup",
        "prompt": "iLink Bot account ID",
    },
    "WEIXIN_TOKEN": {
        "description": "iLink Bot token obtained through QR login in socis gateway setup",
...
# socis_cli/setup_hidden_env.py. Shared with the `socis setup gateway`
# wizard so the surfaces ask for the same things.
from socis_cli.setup_hidden_env import (  # noqa: E402
...
def _messaging_platform_payload(
    ...
    # SOCIS_AGENT_HOME contextvar override (#56986 / #69143), so the profile's
...
def _whatsapp_session_path() -> Path:
    from socis_agent_constants import get_socis_dir

    return get_socis_dir("platforms/whatsapp/session", "whatsapp/session")
...
_TELEGRAM_ONBOARDING_USER_AGENT = f"SOCISDashboard/{__version__}"
@dataclass
...
async def start_telegram_onboarding(body: TelegramOnboardingStart):
    bot_name = (body.bot_name or "SOCIS Agent").strip() or "SOCIS Agent"
...
def _restart_gateway_after_telegram_onboarding(profile: Optional[str] = None) -> dict[str, Any]:
    ...
    saved token waits on a separate dashboard restart click, SOCIS appears
...
async def get_messaging_platforms(profile: Optional[str] = None):
    ...
    # load_env() honors the SOCIS_AGENT_HOME contextvar override; the gateway
...
def _multiplex_port_binding_conflict(
    ...
    *enabling* is blocked; disabling/clearing stays allowed so users can
    ...
    if not requested or requested.lower() == "current":
        ...
        # The dashboard's own profile. "custom" (an unrecognized SOCIS_AGENT_HOME)
...
# the canonical ``socis auth add <provider>`` command so the dashboard can
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    1. ``~/.socis-agent/.anthropic_oauth.json`` — SOCIS-managed terminal PKCE
    ...
    2. ``ANTHROPIC_API_KEY`` → ``ANTHROPIC_TOKEN`` → ``CLAUDE_CODE_OAUTH_TOKEN``
       ...
       env during ``load_socis_dotenv()``, so the same check covers them)
    ...
    try:
        from agent.anthropic_adapter import (
            read_socis_oauth_credentials,
            _get_socis_oauth_file,
        )
    except ImportError:
        read_socis_oauth_credentials = None  # type: ignore
        _get_socis_oauth_file = None  # type: ignore

    socis_creds = None
    if read_socis_oauth_credentials:
        try:
            socis_creds = read_socis_oauth_credentials()
        except Exception:
            socis_creds = None
    if socis_creds and socis_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "socis_pkce",
            "source_label": f"SOCIS PKCE ({_get_socis_oauth_file() if _get_socis_oauth_file else None})",
            "token_preview": _truncate_token(socis_creds.get("accessToken")),
            "expires_at": socis_creds.get("expiresAt"),
            "has_refresh_token": bool(socis_creds.get("refreshToken")),
        }
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into SOCIS even
    when they also have a separate SOCIS-managed PKCE login.
    """
    try:
...
def _copilot_acp_status() -> Dict[str, Any]:
    ...
    hold its session in an OS keychain SOCIS can't read, so the unverified
    state is presented as "managed by the Copilot CLI" — never as signed out.
    """
    try:
        from socis_cli.auth import get_external_process_provider_status
...
def _external_process_cli_command(provider_id: str, default: str) -> str:
    ...
    SOCIS at a custom binary (``SOCIS_AGENT_COPILOT_ACP_COMMAND`` /
    ``COPILOT_CLI_PATH``) would otherwise be told to run a command that isn't
    the one SOCIS spawns. Non-external-process providers get ``default`` back
    untouched.
    """
    try:
        from socis_cli.auth import PROVIDER_REGISTRY, get_external_process_provider_status
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "name": "Nous Portal",
        "flow": "device_code",
        "cli_command": "socis auth add nous",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "socis auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "socis auth add qwen-oauth",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "socis auth add minimax-oauth",
    ...
    },
    {
        ...
        # 127.0.0.1 callback.
        "flow": "device_code",
        "cli_command": "socis auth add xai-oauth",
        "docs_url": "https://agent.socis.io/docs/guides/xai-grok-oauth",
    ...
    # server). SOCIS previously reimplemented that subscription-OAuth PKCE
    ...
    # credentials. Login still works via the terminal (`socis auth add
    # anthropic`, unaffected by this change) or a plain API key below.
    {
        "id": "anthropic",
        "name": "Anthropic API Key",
        "flow": "external",
        "cli_command": "socis auth add anthropic",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from socis_cli import auth as hauth
        if provider_id == "nous":
...
def _oauth_provider_disconnect_command(provider: Dict[str, Any]) -> Optional[str]:
    ...
    External providers store their credentials outside SOCIS, so the disconnect
    ...
    user sees exactly what executes, and SOCIS then stops resolving the token.
...
def _oauth_provider_disconnect_hint(provider: Dict[str, Any], status: Dict[str, Any]) -> Optional[str]:
    ...
    # catalog entry) but, unlike other external providers, SOCIS still OWNS
    # the credential it can show here: the SOCIS-managed PKCE file
    # (~/.socis-agent/.anthropic_oauth.json) and its credential-pool entry, both
    # written by `socis auth add anthropic` in the terminal. Those are ours
    ...
    if provider.get("flow") == "external" and provider.get("id") != "anthropic":
        if _oauth_provider_disconnect_command(provider):
            ...
            return "Managed outside SOCIS — run the disconnect command to remove it."
...
def _build_oauth_catalog() -> list[Dict[str, Any]]:
    ...
    MEMBERSHIP is the union of:
      ...
      2. every accounts-tab provider in the unified ``provider_catalog()`` (the
         ``socis model`` universe) — so any OAuth/external provider added as a
    ...
    #    in lockstep with the `socis model` universe (zero-edit for new plugins).
    try:
        from socis_cli.provider_catalog import provider_catalog
        for d in provider_catalog():
            ...
            rows.append({
                ...
                "name": d.label,
                "flow": "external",
                "cli_command": f"socis auth add {d.slug}",
...
async def list_oauth_providers(profile: Optional[str] = None):
    ...
    Response shape (per provider):
        ...
        status:
          ...
          source           short slug ("socis_pkce", "claude_code", ...)
    ...
    sync with the `socis model` picker; _OAUTH_OVERRIDES supplies per-provider
...
async def disconnect_oauth_provider(
    ...
    def _run():
        with _profile_scope(profile):
            ...
            # Anthropic clears only the SOCIS-managed PKCE file and auth-store entry.
            ...
            if provider_id == "anthropic":
                cleared = False
                try:
                    from agent.anthropic_adapter import _get_socis_oauth_file
                    oauth_file = _get_socis_oauth_file()
                ...
                try:
                    from socis_cli.auth import clear_provider_auth
            ...
            try:
                from socis_cli.auth import clear_provider_auth, invalidate_nous_auth_status_cache
...
# points at `socis auth add anthropic` (terminal PKCE, unaffected) instead.
#
...
async def _start_device_code_flow(
    ...
    if provider_id == "nous":
        ...
        portal_base_url = (
            os.getenv("SOCIS_AGENT_PORTAL_BASE_URL")
    ...
    if provider_id == "xai-oauth":
        from socis_cli.auth import _xai_oauth_request_device_code
        import httpx
...
def _nous_poller(session_id: str) -> None:
    ...
    try:
        ...
        with _profile_scope(_oauth_session_profile(session_id)):
            ...
            from socis_cli.auth import persist_nous_credentials
...
def _xai_device_poller(session_id: str) -> None:
    ...
    try:
        ...
        with _profile_scope(_oauth_session_profile(session_id)):
            ...
            # Mirror `socis auth add xai-oauth`: first credential may become
            ...
            # prior ``socis auth remove xai-oauth`` (mirrors auth_add_command
            # and the ``socis model`` re-login path in _login_xai_oauth).
...
def _codex_device_code_start_error(resp: Any) -> str:
    ...
    if "device" in lower and ("authori" in lower or "enable" in lower):
        message = (
            ...
            "account may need device-code authorization enabled before SOCIS "
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        from socis_cli.auth import _save_codex_tokens
...
def _session_db_read_probe_statements() -> tuple:
    ...
    after `socis update` until the first message forced a writable open.
    """
    from socis_agent_state_schema import schema_read_probe_statements
...
def _open_session_db_at_path(db_path: Path, *, read_only: bool):
    ...
    from socis_agent_state import SessionDB, is_malformed_schema_error
...
def _maybe_auto_archive_for_profile(profile: Optional[str]) -> None:
    ...
    The Desktop backend is spawned as ``socis serve`` — it runs neither the
    ...
    try:
        ...
        from socis_cli.config import load_config as _load_full_config
...
def _prune_sessions(body: SessionPrune):
    """Delete ended sessions matching filters (mirrors `socis sessions prune`)."""
    has_window = (
    ...
    profile_home = _cron_profile_home(body.profile)[1] if body.profile else get_socis_agent_home()
...
async def get_logs(
    ...
    from socis_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_socis_agent_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from socis_agent_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _cron_profile_dicts() -> List[Dict[str, Any]]:
    ...
    """
    from socis_cli import profiles as profiles_mod
    try:
        return [
            {
                "name": name,
...
def _cron_default_profile() -> str:
    ...
    A desktop pool backend runs one process per profile (SOCIS_AGENT_HOME already
    ...
    fallback would write a non-default profile's job into ``~/.socis-agent``.
    Resolve the process's own profile instead. ``custom`` (an unrecognized
    SOCIS_AGENT_HOME outside the profiles tree) has no profile-dir equivalent, so
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, SOCIS_AGENT_HOME)."""
    from socis_cli import profiles as profiles_mod
...
def _annotate_cron_job(job: Dict[str, Any], profile: str, home: Path) -> Dict[str, Any]:
    ...
    annotated["socis_agent_home"] = str(home)
...
def _call_cron_for_profile(target_profile: Optional[str], func_name: str, *args, **kwargs):
    ...
    from socis_agent_constants import (
        reset_socis_agent_home_override,
        set_socis_agent_home_override,
    )

    token = set_socis_agent_home_override(str(home))
    try:
...
from socis_cli.web_routers import cron as _cron_routes  # noqa: E402

app.include_router(_cron_routes.router)
from socis_cli.web_routers.cron import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    list_cron_jobs,
    get_cron_job,
    list_cron_job_runs,
    create_cron_job,
...
def _gateway_fire_endpoint(profile: str, home: Path) -> str:
    ...
    if multiplex and profile != "default":
        ...
        listener_profile, listener_home = "default", get_default_socis_root()
        _log.info(
    ...
    try:
        ...
        # yaml.safe_load of config.yaml (tests/socis_cli/
        # test_config_read_guard.py). The SOCIS_AGENT_HOME override scopes
        ...
        token = set_socis_agent_home_override(str(listener_home))
        try:
...
def _gateway_intentionally_stopped(profile: Optional[str]) -> bool:
    ...
    commands (``socis gateway stop`` persists ``"stopped"``; start and
...
#
# Wraps the same config data layer the CLI uses (socis_cli.mcp_config), so
# servers managed here show up under `socis mcp list` and vice versa.  Secrets
...
def _normalize_mcp_server_create(
    ...
    """
    from socis_cli.mcp_config import (
        _bearer_auth_headers,
        _strip_bearer_prefix,
    )
    from socis_cli.mcp_security import validate_mcp_server_entry
...
from socis_cli.web_routers import mcp as _mcp_routes  # noqa: E402

app.include_router(_mcp_routes.router)
from socis_cli.web_routers.mcp import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    list_mcp_servers,
    add_mcp_server,
    replace_mcp_servers,
    remove_mcp_server,
...
def _mcp_oauth_callback_url(request: Request, server_name: str) -> str:
    ...
    from socis_cli.dashboard_auth.prefix import prefix_from_request, resolve_public_url
...
def _mcp_oauth_transaction(flow) -> threading.Lock:
    key = (flow.socis_agent_home, flow.server_name)
...
def _run_dashboard_mcp_oauth(flow, cfg: dict) -> None:
    ...
    try:
        ...
        from tools.mcp_oauth import SOCISTokenStorage, force_interactive_oauth
        from tools.mcp_oauth_manager import get_manager

        home_token = set_socis_agent_home_override(flow.socis_agent_home)
        secret_token = set_secret_scope(build_profile_secret_scope(Path(flow.socis_agent_home)))
        try:
            transaction = _mcp_oauth_transaction(flow)
            with transaction, force_interactive_oauth(), dashboard_oauth_flow(flow):
                manager = get_manager()
                storage = SOCISTokenStorage(flow.server_name)
                backup = storage.snapshot()
                previous_entry = None
                try:
                    previous_entry = manager.remove(
                        flow.server_name,
                        socis_agent_home=flow.socis_agent_home,
                    )
            ...
            reset_socis_agent_home_override(home_token)
    except Exception as exc:
        msg = str(exc)
...
#
# Wraps the same JSON store the CLI uses (socis_cli.webhook); the webhook
...
async def list_webhooks():
    import socis_cli.webhook as wh
...
# spawn the real `socis gateway <verb>` so behaviour matches the CLI exactly.
...
async def start_gateway(profile: Optional[str] = None):
    try:
        proc = _spawn_socis_action(_gateway_subcommand(profile, "start"), "gateway-start")
    except HTTPException:
        raise
    except Exception as exc:
...
async def stop_gateway(profile: Optional[str] = None):
    try:
        proc = _spawn_socis_action(_gateway_subcommand(profile, "stop"), "gateway-stop")
    except HTTPException:
        raise
    except Exception as exc:
...
async def list_credential_pool():
    ...
    from socis_cli.auth import read_credential_pool
...
async def add_credential_pool_entry(body: CredentialPoolAdd):
    ...
    def _run():
        try:
            ...
            # (via DELETE below or `socis auth add`) can seed again.
            # Mirrors the `socis auth add` behaviour in auth_commands.py.
...
async def remove_credential_pool_entry(provider: str, index: int):
    ...
    registry the CLI ``socis auth remove`` uses: each source cleans up its
    ...
    from socis_cli.auth import suppress_credential_source
...
async def run_doctor():
    try:
        proc = _spawn_socis_action(["doctor"], "doctor")
    except Exception as exc:
...
async def run_security_audit():
    try:
        proc = _spawn_socis_action(["security", "audit"], "security-audit")
    except Exception as exc:
...
def _dashboard_backup_dir() -> Path:
    return get_socis_agent_home() / "backups"


def _new_dashboard_backup_path() -> Path:
    stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
    return _dashboard_backup_dir() / f"socis-backup-{stamp}-{secrets.token_hex(4)}.zip"
...
async def run_backup(body: BackupRequest):
    ...
    try:
        proc = _spawn_socis_action(args, "backup")
    except Exception as exc:
...
async def run_import(body: ImportRequest):
    ...
    if body.force:
        args.append("--force")
    try:
        proc = _spawn_socis_action(args, "import")
    except Exception as exc:
...
async def list_hooks():
    ...
    """
    def _run():
        from socis_cli.config import load_config as _load_config
        from agent import shell_hooks

        try:
            from socis_cli.plugins import VALID_HOOKS
...
async def list_checkpoints():
    ...
    # Checkpoints live under <socis_agent_home>/checkpoints/.  Surface a count +
    ...
    cp_dir = get_socis_agent_home() / "checkpoints"
    sessions = []
    total_bytes = 0
    if cp_dir.is_dir():
...
async def prune_checkpoints():
    try:
        proc = _spawn_socis_action(["checkpoints", "prune"], "checkpoints-prune")
    except Exception as exc:
...
def _profile_cli_args(profile: Optional[str]) -> List[str]:
    ...
    Hub install/uninstall/update run in a fresh ``socis`` subprocess, and
...
def _hub_action_name(verb: str, key: str) -> str:
    ...
    ``_spawn_socis_action`` tracks one process/log per name, so a shared
...
from socis_cli.web_routers import skills as _skills_routes  # noqa: E402

app.include_router(_skills_routes.hub_router)
from socis_cli.web_routers.skills import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    install_skill_hub,
    uninstall_skill_hub,
    update_skills_hub,
...
def _fallback_profile_dicts(profiles_mod) -> List[Dict[str, Any]]:
    ...
    default_home = profiles_mod._get_default_socis_agent_home()
...
def _profile_setup_command(name: str) -> str:
    ...
    return "socis setup" if name == "default" else f"{name} setup"
...
def _write_profile_model(profile_dir: Path, provider: str, model: str) -> None:
    ...
    context-local SOCIS_AGENT_HOME override so the write lands in the target
    ...
    """
    from socis_agent_constants import set_socis_agent_home_override, reset_socis_agent_home_override

    token = set_socis_agent_home_override(str(profile_dir))
    try:
        provider, model = _normalize_main_model_assignment(provider, model)
        cfg = load_config()
...
def _write_profile_mcp_servers(profile_dir: Path, servers: List["MCPServerCreate"]) -> int:
    ...
    Scopes ``load_config``/``save_config`` to ``profile_dir`` via the
    context-local SOCIS_AGENT_HOME override (same mechanism as
    ...
    """
    from socis_agent_constants import set_socis_agent_home_override, reset_socis_agent_home_override
    from socis_cli.mcp_config import _save_bearer_auth_token
...
def _disable_unselected_skills(profile_dir: Path, keep: List[str]) -> int:
    ...
    install.) Scoped to the profile via the SOCIS_AGENT_HOME override. Returns the
    number of skills newly disabled.
    """
    from socis_agent_constants import set_socis_agent_home_override, reset_socis_agent_home_override
    from socis_cli.skills_config import get_disabled_skills, save_disabled_skills
...
def _profile_scope(profile: Optional[str]):
    ...
    1. ``load_config``/``save_config`` resolve ``get_socis_agent_home()`` at call
       time — the context-local override from ``set_socis_agent_home_override``
    ...
    retargeted to the *current* ``get_socis_agent_home()`` so writes land in the
    live home even when the import-time binding is stale (e.g. the process
    imported the modules before a SOCIS_AGENT_HOME override, or under test
    isolation).
    """
    requested = (profile or "").strip()
...
def _config_profile_scope(profile: Optional[str]):
    ...
    ``set_socis_agent_home_override`` contextvar — it does NOT swap the
    ...
    which is all endpoints that resolve ``get_socis_agent_home()`` at call time
...
from socis_cli.web_routers import tools as _tools_routes  # noqa: E402

app.include_router(_tools_routes.router)
from socis_cli.web_routers.tools import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    get_toolsets,
    toggle_toolset,
    get_toolset_config,
    get_toolset_models,
...
# (which attach to cua-driver's OWN identity, com.trycua.driver — not SOCIS,
...
#
# The endpoint spawns the same ``socis --tui`` binary the CLI uses, behind
...
if sys.platform.startswith("win"):
    try:
        from socis_cli.win_pty_bridge import WinPtyBridge as PtyBridge, PtyUnavailableError
    ...
    try:
        from socis_cli.pty_bridge import PtyBridge, PtyUnavailableError
        _PTY_BRIDGE_AVAILABLE = True
...
from socis_cli.pty_session import PtySessionRegistry, RegistryFull, run_reaper  # noqa: E402
...
_GATEWAY_WS_PROTOCOL = "socis-gateway-v1"
_GATEWAY_WS_TICKET_PROTOCOL_PREFIX = "socis-gateway-ticket."
...
def _ws_auth_reason(ws: "WebSocket") -> tuple[Optional[str], str]:
    ...
    * ``?internal=<process-credential>`` — the process-lifetime internal
    ...
    if auth_required:
        ...
        from socis_cli.dashboard_auth.audit import AuditEvent, audit_log
        from socis_cli.dashboard_auth.ws_tickets import (
            TicketInvalid,
        ...
        try:
            ...
            if protocol_ticket:
                ...
                ws._socis_ws_subprotocol = _GATEWAY_WS_PROTOCOL
...
def _resolve_chat_argv(
    ...
    Default: whatever ``socis --tui`` would run.  Tests monkeypatch this
    function to inject a tiny fake command (``cat``, ``sh -c 'printf …'``)
    so nothing has to build Node or the TUI bundle.

    Session resume is propagated via the ``SOCIS_AGENT_TUI_RESUME`` env var —
    matching what ``socis_cli.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``ui-tui`` does
    not parse its argv.

    ``SOCIS_AGENT_TUI_GATEWAY_URL`` is injected so the PTY child can attach to
    this process's in-memory ``tui_gateway`` instance instead of spawning
    its own Python gateway subprocess.

    `sidecar_url` (when set) is forwarded as ``SOCIS_AGENT_TUI_SIDECAR_URL`` so
    the spawned ``tui_gateway.entry`` can mirror dispatcher emits to the
    dashboard's ``/api/pub`` endpoint (see :func:`pub_ws`).

    `active_session_file` (when set) is forwarded as
    ``SOCIS_AGENT_TUI_ACTIVE_SESSION_FILE``. The TUI writes the current session id
    ...
    ``SOCIS_AGENT_HOME`` at the profile dir in the child env. Every spawned
    process (the TUI and the ``tui_gateway.entry`` it launches) resolves
    ``get_socis_agent_home()`` from that env var at its own import, so the child
    binds the profile's config, skills, memory, and state.db from the start
    — the same propagation ``socis -p <name>`` performs. The in-process
    ``SOCIS_AGENT_TUI_GATEWAY_URL`` attach is SKIPPED for scoped chats: the
    ...
    """
    from socis_cli.main import PROJECT_ROOT, _apply_tui_python_env, _make_tui_argv
    ...
    # SOCIS TUI child: build via the single spawn-env factory (profile-home
    # contract applied; secrets kept — the spawned agent needs provider creds).
    # An explicit profile scope still overrides SOCIS_AGENT_HOME before config is
    # bridged into the child environment.
    from tools.environments.local import build_subprocess_env
    env = build_subprocess_env(scrub_secrets=False, inherit_profile_home=True)
    if profile_dir is not None:
        env["SOCIS_AGENT_HOME"] = str(profile_dir)
    try:
        from socis_cli.config import (
    ...
    env.setdefault("SOCIS_AGENT_TUI_DISABLE_MOUSE", "1")
    env.setdefault("SOCIS_AGENT_TUI_INLINE", "1")
    ...
    env["SOCIS_AGENT_TUI_DASHBOARD"] = "1"
    ...
    if resume:
        ...
        env["SOCIS_AGENT_TUI_RESUME"] = resume

    if sidecar_url:
        env["SOCIS_AGENT_TUI_SIDECAR_URL"] = sidecar_url
    ...
    # inherits the profile SOCIS_AGENT_HOME set above.
    if profile_dir is None:
        if gateway_ws_url := _build_gateway_ws_url():
            env["SOCIS_AGENT_TUI_GATEWAY_URL"] = gateway_ws_url
...
def _resolve_client_ws_host() -> Optional[str]:
    ...
    1. Explicit ``SOCIS_AGENT_DASHBOARD_WS_HOST`` env var — wins always. Operators
    ...
    """
    explicit = os.environ.get("SOCIS_AGENT_DASHBOARD_WS_HOST", "").strip()
    if explicit:
        return explicit
...
def _build_gateway_ws_url() -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        from socis_cli.dashboard_auth.ws_tickets import internal_ws_credential
...
def _active_session_file_for_channel(app: "FastAPI", channel: str) -> Path:
    ...
    fd, raw_path = tempfile.mkstemp(prefix="socis-pty-active-", suffix=".json")
    os.close(fd)
    path = Path(raw_path)
    files[channel] = path
    return path
...
# ---------------------------------------------------------------------------
# /api/console — safe SOCIS Console command WebSocket.
#
# Unlike /api/pty, this endpoint never spawns a PTY, shell, or full SOCIS CLI
        ...
        with _console_executor_lock:
            if _console_executor is None:
                _console_executor = concurrent.futures.ThreadPoolExecutor(
                    ...
                    thread_name_prefix="socis-console",
                )
...
async def console_ws(ws: WebSocket) -> None:
    ...
    try:
        from socis_cli.console_engine import SOCISConsoleEngine

        engine = SOCISConsoleEngine(output_limit=_CONSOLE_OUTPUT_LIMIT)
            ...
            if command_id == command_generation:
                ...
                await _console_send(
                    ...
                    send_lock,
                    {
                        ...
                        "id": command_id,
                        "message": (
                            "Command timed out. SOCIS Console returned to the prompt."
                        ),
                        "command": line,
                    },
                )
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            ...
            "\x1b[33mInstall SOCIS inside WSL2 to use the dashboard's /chat "
    ...
    if raw_resume and env:
        registry_resume = env.get("SOCIS_AGENT_TUI_RESUME") or raw_resume
...
async def gateway_ws(ws: WebSocket) -> None:
    ...
    await handle_ws(
        ws,
        auth_identity=getattr(ws, "_socis_auth_identity", None),
        subprotocol=getattr(ws, "_socis_ws_subprotocol", None),
    )
...
# SOCIS_AGENT_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Thin re-export of :func:`socis_cli.dashboard_auth.prefix.normalise_prefix`
    ...
    """
    from socis_cli.dashboard_auth.prefix import normalise_prefix
...
def _render_active_theme_bootstrap_css() -> str:
    ...
    default SOCIS Teal canvas before the SPA flips the configured user
    theme into place.
    ...
    try:
        ...
        for theme in _discover_user_themes():
            ...
            return (
                '<style id="socis-theme-bootstrap">'
                ":root{"
...
def mount_spa(application: FastAPI):
    ...
    ``mission-control.tilos.com/socis/*`` -> local Caddy -> :9119), the
    proxy injects ``X-Forwarded-Prefix: /socis`` on every request. We
    rewrite the served ``index.html`` so absolute asset URLs (``/assets/...``)
    and the SPA's runtime ``__SOCIS_AGENT_BASE_PATH__`` honour that prefix
    without rebuilding the bundle.
    """
    # `socis serve` is the headless backend: it must NEVER serve the browser
    ...
    _headless = os.environ.get("SOCIS_AGENT_SERVE_HEADLESS") == "1"
    if _headless:
        _msg = (
            "Headless backend (socis serve): web UI disabled — use "
            "`socis dashboard` for the browser UI."
        )

        @application.get("/{full_path:path}")
        async def no_frontend(full_path: str):
            # Desktop token handshake (#94227): the Electron shell boots by
            # fetching `/` and extracting ``window.__SOCIS_AGENT_SESSION_TOKEN__``
            ...
            # `socis update` replaced the backend) had no way to adopt the
            ...
            if full_path == "" and not gated:
                ...
                return HTMLResponse(
                    ...
                    f"window.__SOCIS_AGENT_SESSION_TOKEN__={token_js};"
                    "window.__SOCIS_AGENT_AUTH_REQUIRED__=false;"
                    "</script></head><body>"
                    "Headless backend (socis serve): web UI disabled — use "
                    "`socis dashboard` for the browser UI."
                    "</body></html>",
    ...
    # (#82614): a long-lived `socis dashboard --skip-build` process that
    ...
    def _serve_index(prefix: str = ""):
        ...
        ``prefix`` is the normalised ``X-Forwarded-Prefix`` (e.g. ``/socis``)
        ...
        ``__SOCIS_AGENT_AUTH_REQUIRED__`` flag lets the SPA pick the right
        ...
        if gated:
            bootstrap_script = (
                f"<script>"
                f"window.__SOCIS_AGENT_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__SOCIS_AGENT_BASE_PATH__="{prefix}";'
                f"window.__SOCIS_AGENT_AUTH_REQUIRED__={gated_js};"
                f"</script>"
            )
        else:
            bootstrap_script = (
                f'<script>window.__SOCIS_AGENT_SESSION_TOKEN__="{_SESSION_TOKEN}";'
                f"window.__SOCIS_AGENT_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__SOCIS_AGENT_BASE_PATH__="{prefix}";'
                f"window.__SOCIS_AGENT_AUTH_REQUIRED__={gated_js};"
                f"</script>"
        ...
        # (``SOCIS_AGENT_HOME/dashboard-themes/<name>.yaml``), inject a minimal
        # critical-CSS block so the first paint uses the target palette.
        # Without this the SPA paints the default SOCIS Teal canvas, then
    ...
    # under ``/socis`` they'd hit ``mission-control.tilos.com/fonts/...``
    # (the MC Pages app), not the SOCIS backend. Intercept CSS asset
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.socis-agent/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.socis-agent/dashboard-themes/*.yaml for user-created themes.

    Returns a list of fully-normalised theme definitions ready to ship
    to the frontend, so the client can apply them without a secondary
    round-trip or a built-in stub.

    Uses the dashboard process launch home, not ``get_socis_agent_home()``, so a
    transient profile override from embedded chat does not hide themes that
    live under the server's own ``SOCIS_AGENT_HOME``.
    """
    themes_dir = get_process_socis_agent_home() / "dashboard-themes"
...
async def get_dashboard_themes():
    ...
    from `~/.socis-agent/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as socis_cli.plugins):
    1. User plugins:    ~/.socis-agent/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.socis-agent/plugins/  (only if SOCIS_AGENT_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()

    from socis_cli.plugins import get_bundled_plugins_dir
    ...
    # SOCIS_AGENT_HOME override (e.g. embedded /chat under --open-profile).
    #
    # #87197: when the process itself is profile-scoped (``--profile <name>``
    # sets ``SOCIS_AGENT_HOME=<root>/profiles/<name>``), the launch home is the
    # profile directory, which has no ``plugins/`` — user plugins are
    # installed in the socis root (``~/.socis-agent/plugins``). Scan the default
    # root as well (``get_default_socis_root()`` unwraps
    # ``<root>/profiles/<name>`` → ``<root>`` and returns a custom
    # ``SOCIS_AGENT_HOME`` unchanged when it *is* the root), mirroring how
    # ``socis_cli.plugins`` resolves plugin install locations. The
    # ``seen_names`` dedupe below keeps profile-local plugins (if any)
    # authoritative over same-named root plugins.
    from socis_agent_constants import get_default_socis_root

    user_plugin_roots = [get_process_socis_agent_home() / "plugins"]
    root_plugins = get_default_socis_root() / "plugins"
    ...
    # ``socis_cli/plugins.py`` and the documented user contract.
    if env_var_enabled("SOCIS_AGENT_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".socis-agent" / "plugins", "project"))
...
async def get_dashboard_plugins():
    ...
    def _run():
        ...
        try:
            from socis_cli.plugins_cmd import _get_enabled_set, _get_disabled_set
            enabled_set = _get_enabled_set()
            disabled_set = _get_disabled_set()
        except Exception:
            enabled_set = set()
...
def _merged_plugins_hub(force_refresh: bool = False) -> Dict[str, Any]:
    ...
    plugins_root_resolved = (get_socis_agent_home() / "plugins").resolve()
    rows: List[Dict[str, Any]] = []
            ...
            try:
                ...
                for tname in provides_tools:
                    ...
                    if cached_result is False:
                        auth_required = True
                        auth_command = f"socis auth {name}"
                        break
            except Exception:
                pass
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from socis_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from socis_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from socis_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from socis_cli.plugins_cmd import dashboard_remove_user_plugin
...
def _mount_plugin_api_routes():
    ...
    Project plugins (``./.socis-agent/plugins/``) ship with the CWD and are
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if plugin.get("source") == "project":
            _log.warning(
                ...
                "~/.socis-agent/plugins/ if you trust it)",
        ...
        if not api_path.exists():
            ...
            continue
        try:
            module_name = f"socis_dashboard_plugin_{plugin['name']}"
...
from socis_cli.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
...
def _write_dashboard_ready_file(actual_port: int) -> None:
    ...
    so Electron passes ``SOCIS_AGENT_DESKTOP_READY_FILE`` and waits for this JSON.
    Normal CLI/dashboard launches still use the stdout READY line below.
    """
    target = os.environ.get("SOCIS_AGENT_DESKTOP_READY_FILE")
    if not target:
        return
...
def _is_serve_orphaned(
    ...
    *,
    ...
    ``SOCIS_AGENT_PARENT_PID`` is the Electron Desktop PID, not necessarily this
    Python process's immediate PPID. On Windows the venv ``socis.exe`` launcher
...
def _start_parent_death_watchdog() -> None:
    ...
    Desktop versions that provide only ``SOCIS_AGENT_PARENT_PID`` retain PID-only
    tracking.
    """
    raw_pid = os.environ.get("SOCIS_AGENT_PARENT_PID")
    start_marker = os.environ.get("SOCIS_AGENT_PARENT_START_MARKER")
    nonce = os.environ.get("SOCIS_AGENT_PARENT_NONCE")
    ...
    try:
        poll = max(0.5, float(os.environ.get("SOCIS_AGENT_SERVE_WATCHDOG_POLL_S", "2.0")))
    except (TypeError, ValueError):
        poll = 2.0
...
# spawn (and any script wrapping ``socis serve``) cannot tell "port occupied"
...
# SOCIS_AGENT_BACKEND_READY sentinel (which is NOT changed by any of this).
...
def _write_machine_sentinel_line(line: str) -> None:
    ...
    SOCIS_AGENT_DESKTOP_READY_FILE channel) for port discovery instead. Never
...
def _report_port_in_use(host: str, port: int) -> None:
    ...
    print(
        ...
        "'socis serve' / 'socis dashboard' backend or the SOCIS gateway. "
...
def start_server(
    ...
    build and no SPA mount (mount_spa() honours ``SOCIS_AGENT_SERVE_HEADLESS``), so
    ...
    try:
        from socis_cli.nous_auth_keepalive import start_nous_auth_keepalive
    ...
    # the socis-0day MCP-persistence campaign abused unauthenticated public
    ...
    if app.state.auth_required:
        ...
        if not list_providers():
            ...
            # to register (e.g. missing SOCIS_AGENT_DASHBOARD_OAUTH_CLIENT_ID).
            # Each provider plugin that ships with SOCIS Agent exposes a
            ...
            if host in _LOOPBACK_HOST_VALUES:
                _public_url_for_msg = ""
                try:
                    from socis_cli.dashboard_auth.prefix import (
                ...
                _local_only_hint = (
                    ...
                    "(and unset SOCIS_AGENT_DASHBOARD_PUBLIC_URL) to restore the "
            ...
            _fix_hint = (
                ...
                "  • OAuth: run `socis dashboard register` (Nous Portal) or "
            ...
            # (#54489).
            try:
                from socis_cli.config import load_config as _load_cfg
                from socis_cli.plugins_cmd import _BASIC_AUTH_PLUGIN_KEYS
                ...
                if _has_creds and (set(_disabled) & _BASIC_AUTH_PLUGIN_KEYS):
                    _fix_hint = (
                        ...
                        "`socis plugins enable basic`), then restart the "
                        "dashboard.\n\n"
                    ) + _fix_hint
            except Exception:
                pass
    ...
    # SOCIS_AGENT_DASHBOARD_READY, open the browser, *then* serve.
    #
    ...
    async def _serve():
        ...
        with server.capture_signals():
            ...
            if os.getenv("SOCIS_AGENT_DESKTOP") == "1":
                try:
                    from socis_cli.dashboard_procs import (
            ...
            try:
                from socis_cli.process_identity import reap_orphaned_mcp_helpers
            ...
            # for standalone `socis serve` (no SOCIS_AGENT_PARENT_PID env).
            ...
            # is what lets `socis update` relaunch a manually-started serve
            # on its real endpoint instead of dropping it (#63206).
            try:
                from socis_cli.process_identity import (
            ...
            ready_token = "SOCIS_AGENT_BACKEND_READY" if headless else "SOCIS_AGENT_DASHBOARD_READY"
            ...
            if headless:
                ...
                print(f"  SOCIS backend listening on {host}:{actual_port}", flush=True)
            else:
                print(f"  SOCIS Web UI → http://{host}:{actual_port}")
            ...
            if start_mcp_discovery_after_bind:
                ...
                try:
                    from socis_cli.mcp_startup import defer_background_mcp_discovery
