...
- ``socis_cli.providers``        -- canonical provider identity + overlays
- ``socis_cli.model_normalize``  -- per-provider name formatting
...
from socis_cli.model_normalize import (
...
def _declared_model_ids(value: Any) -> list[str]:
    ...
    if isinstance(value, dict):
        for model_id in value:
            # Backward compat: pre-fix SOCIS wrote sentinel keys inside the
...
def _entry_models_discovered(entry: Any) -> bool:
    """True when the entry's ``models`` mapping was auto-discovered by SOCIS.

    The current shape is an entry-level ``models_discovered: true`` sibling of
    ``models``. Older SOCIS versions wrote an in-mapping
...
def _models_config_is_allowlist(value: Any, discovered: bool = False) -> bool:
    ...
    written by ``_save_custom_provider`` / the ``socis model`` wizard — not a
    ...
    while ``socis model`` still live-probed the full ``/v1/models`` list.
    ...
    ``_entry_models_discovered``): a catalog SOCIS itself persisted after a
    ...
    *,
    ...
    try:
        ...
        for entry in providers:
            ...
            # A mapping SOCIS itself discovered (``models_discovered: true``
...
_NOUS_HERMES_MODEL_WARNING = (
    "Nous Hermes 3 & 4 models are NOT agentic and are not designed "
    "for use with SOCIS Agent. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Nous Hermes 3 / Hermes 4 chat families (a third-party
# model family published by Nous Research, unrelated to this project's own
# branding — this regex must match the REAL external model identifier, not
# our own name).
...
def _check_socis_model_warning(model_name: str) -> str:
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``socis config set model.aliases.xxx``
...
# switch (SOCIS_AGENT_HOME moves, so the path moves) and a config/key rotation
...
def _direct_alias_source_identity() -> Optional[tuple]:
    ...
    """
    try:
        from socis_agent_constants import get_config_path
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from socis_cli.model_switch import
...
def resolve_startup_model_route(
    ...
    *,
    ...
    ``SOCISCLI`` is constructed before the interactive ``/model`` pipeline
    ...
    if current_provider:
        try:
            ...
            if _is_routing_agg(_norm_prov(current_provider)):
                from socis_cli.models import _find_openrouter_slug
    ...
    try:
        from socis_cli.models import normalize_provider
...
def resolve_display_context_length(
    ...
    if config_context_length is not None and (
        ...
        try:
            from socis_cli.route_identity import should_clear_context_pin
...
async def resolve_display_context_length_async(
    ...
    ``socis_cli.route_identity.should_clear_context_pin_async``, which
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'socis model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'socis doctor' — config issues detected:"
        ...
        from socis_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
        from socis_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
        ...
        if not new_model:
            if pdef.base_url:
                from socis_cli.runtime_provider import _auto_detect_local_model
    ...
    # =================================================================
        ...
        try:
            from socis_cli.config import load_config
            from socis_cli.moa_config import exact_moa_preset_name, normalize_moa_config
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from socis_cli.providers import resolve_user_provider as _ruser
        ...
        if current_provider == "custom" and current_base_url:
            try:
                from socis_cli.models import should_use_ollama_native_catalog
    ...
    if target_provider in {"nous", "nous-portal", "socis"}:
        from socis_cli.providers import nous_api_mode
    ...
    # socis_cli.runtime_provider.resolve_runtime_provider; without the strip,
    ...
    # kimi) the same way.
    from socis_cli.models import opencode_provider_family as _oc_family_fn
    if _oc_family_fn(target_provider) is not None and isinstance(base_url, str):
        from socis_cli.models import normalize_opencode_base_url
    ...
    socis_warn = _check_socis_model_warning(new_model)
    if socis_warn:
        warnings.append(socis_warn)
...
def _extra_headers_from_config(entry: Any) -> dict[str, str]:
    if not isinstance(entry, dict):
        return {}
    from socis_cli.config import normalize_extra_headers
...
def _prefetch_provider_models_parallel(provider_slugs: list[str]) -> None:
    ...
    :param provider_slugs: SOCIS provider IDs to prefetch (e.g. ``["openrouter",
        "anthropic", "deepseek"]``).  Unknown providers are silently skipped.
    """
    from socis_cli.models import cached_provider_model_ids
    ...
    def _fetch_one(slug: str) -> None:
        try:
            ...
            if models:
                from socis_cli.models import update_provider_cache_entry
...
def _collect_authed_provider_slugs(
    ...
    from socis_cli.auth import PROVIDER_REGISTRY, _load_auth_store
    from socis_cli.providers import SOCIS_AGENT_OVERLAYS, ALIASES as _PROVIDER_ALIAS_TABLE
    from socis_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS, CANONICAL_PROVIDERS

    _excluded_set = {str(p).strip().lower() for p in excluded if p}
    slugs: list[str] = []
    seen: set[str] = set()

    # --- Section 1: SOCIS-mapped providers (PROVIDER_TO_MODELS_DEV) ---
    for socis_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        _alias_target = _PROVIDER_ALIAS_TABLE.get(socis_id)
        if (
            _alias_target
            and _alias_target != socis_id
        ...
        try:
            ...
            pass
        if _canonical != socis_id:
            continue
        if socis_id.lower() in seen:
            continue
        if socis_id.lower() in _excluded_set or mdev_id.lower() in _excluded_set:
            continue
        pdata = models_dev_data.get(mdev_id)
        if not isinstance(pdata, dict):
            continue
        pconfig = PROVIDER_REGISTRY.get(socis_id)
        if pconfig and pconfig.auth_type != "api_key":
            continue
        from socis_cli.auth import is_runtime_provider_routable
        if not is_runtime_provider_routable(socis_id):
            continue
        ...
        if not has_creds:
            try:
                ...
                raw_pool_present = bool(
                    store and store.get("credential_pool", {}).get(socis_id)
                )
                if raw_pool_present:
                    has_creds = _credential_pool_is_usable(
                        socis_id, raw_pool_present=True
                    )
            except Exception:
                pass
        if has_creds:
            slugs.append(socis_id)
            seen.add(socis_id.lower())

    # --- Section 2: SOCIS-only providers (SOCIS_AGENT_OVERLAYS) ---
    ...
    for pid, overlay in SOCIS_AGENT_OVERLAYS.items():
        if pid.lower() in seen:
            continue
        socis_slug = _mdev_to_socis.get(pid, pid)
        if socis_slug.lower() in seen:
            continue
        if pid.lower() in _excluded_set or socis_slug.lower() in _excluded_set:
            continue
        has_creds = False
        ...
        if not has_creds:
            try:
                ...
                if pid in providers_store or socis_slug in providers_store:
                    has_creds = True
            except Exception:
                pass
        if not has_creds:
            try:
                if _credential_pool_is_usable(socis_slug):
                    has_creds = True
            except Exception:
                pass
        if has_creds:
            slugs.append(socis_slug)
            seen.add(pid.lower())
            seen.add(socis_slug.lower())
...
def list_authenticated_providers(
    ...
    *,
    ...
    Uses the curated model lists from socis_cli/models.py (OPENROUTER_MODELS,
    ...
    from socis_cli.config import coerce_provider_id, stringify_provider_map
    ...
    # Compared against socis_id / mdev_id (section 1), pid / socis_slug
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from socis_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by socis provider ID
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # "nous" pulls from the remote model-catalog manifest published at
    # https://agent.socis.io/docs/api/model-catalog.json so
    # newly added Portal models surface in the /model picker without
    # requiring a SOCIS release. Falls back to the in-repo
    # _PROVIDER_MODELS["nous"] snapshot when the manifest is unreachable.
    curated["nous"] = get_curated_nous_model_ids()
    # Ollama Cloud uses dynamic discovery (no static curated list)
    if "ollama-cloud" not in curated:
        from socis_cli.models import fetch_ollama_cloud_models
    ...
    if "lmstudio" not in curated and (
        ...
        from socis_cli.models import fetch_lmstudio_models
    ...
    # --- 1. Check SOCIS-mapped providers ---
    from socis_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
    from socis_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    for socis_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        if (
            ...
            continue
        # Resolve the canonical provider profile name.  Skip socis_ids
        # that are mere aliases resolving to a different canonical profile
        # (e.g. "kimi" and "moonshot" both → "kimi-coding").  Only process
        # entries whose socis_id matches the canonical profile name so
        ...
        # same socis_id).  Distinct canonical profiles that share a
        ...
        if slug.lower() in seen_slugs:
            continue
        if socis_id.lower() in _excluded or mdev_id.lower() in _excluded:
            continue
        ...
        # section 2 (SOCIS_AGENT_OVERLAYS) with proper auth store checking.
        if pconfig and pconfig.auth_type != "api_key":
            continue
        # models.dev catalogs include providers SOCIS may not route yet.
        ...
        # /model picker sees the SAME list `socis model` would build, with
        # disk caching to keep the picker open snappy. Falls back to the
        # curated static list when the live fetcher returns nothing.
        model_ids = cached_provider_model_ids(socis_id)
        if not model_ids:
            model_ids = curated.get(socis_id, [])
            if socis_id in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(socis_id, model_ids)
    ...
    # --- 2. Check SOCIS-only providers (nous, openai-codex, copilot, opencode-go) ---
    from socis_cli.providers import SOCIS_AGENT_OVERLAYS
    from socis_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → SOCIS provider ID.
    # SOCIS_AGENT_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    ...
    for pid, overlay in SOCIS_AGENT_OVERLAYS.items():
        ...
        socis_slug = _mdev_to_socis.get(pid, pid)
        if socis_slug.lower() in seen_slugs:
            continue
        if pid.lower() in _excluded or socis_slug.lower() in _excluded:
            continue
        ...
        if getattr(overlay, "keyless", False):
            ...
            has_creds = _has_aws_sdk_creds_for_listing(socis_slug)
        elif overlay.auth_type == "vertex":
        ...
        if not has_creds and overlay.auth_type == "external_process":
            try:
                ...
                _ext_status = get_auth_status(socis_slug) or {}
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or socis_slug in providers_store):
                    has_creds = True
            except Exception as exc:
        ...
        if not has_creds:
            try:
                if _credential_pool_is_usable(socis_slug):
                    ...
                    try:
                        from agent.credential_pool import load_pool
                        _pool = load_pool(socis_slug)
                        if _pool.has_credentials():
                            has_creds = True
                    except Exception:
                        pass
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", socis_slug, exc)
        ...
        # configured.
        if not has_creds and socis_slug == "anthropic":
            try:
                ...
                if (socis_creds and socis_creds.get("accessToken")) or \
        ...
        if socis_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = cached_provider_model_ids(socis_slug)
        # For aws_sdk providers (bedrock), use live discovery so the list
        # reflects the active region (eu.*, ap.*) not the static us.* list.
        elif overlay.auth_type == "aws_sdk":
            try:
                _ids = cached_provider_model_ids(socis_slug)
                model_ids = _ids if _ids else (curated.get(socis_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(socis_slug, []) or curated.get(pid, [])
        elif socis_slug == "nous":
            # Nous serves a large live /v1/models catalog (vendor-prefixed
            # models from many providers, returned alphabetically). The
            # `socis model` picker deliberately shows ONLY the curated agentic
            ...
            try:
                ...
                from socis_cli.auth import get_provider_auth_state as _nous_state
            ...
            if not model_ids:
                model_ids = curated.get(socis_slug, []) or curated.get(pid, [])
                if socis_slug in _MODELS_DEV_PREFERRED:
                    model_ids = _merge_with_models_dev(socis_slug, model_ids)
        total = len(model_ids)
        if socis_slug in _UNCAPPED_PICKER_PROVIDERS:
            top = model_ids  # Aggregator: show full catalog regardless of max_models
        else:
            top = model_ids[:max_models] if max_models is not None else model_ids
        ...
        results.append({
            "slug": socis_slug,
            "name": get_label(socis_slug),
            "is_current": socis_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "socis",
        ...
        seen_slugs.add(socis_slug.lower())
        _record_builtin_endpoint(socis_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or SOCIS_AGENT_OVERLAYS (keeps /model in sync
    # with `socis model`).
    try:
        from socis_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        ...
        for ep_name, ep_cfg in user_providers.items():
            ...
            # SOCIS writes ``models:`` as a dict keyed by model id, but older
            ...
            if group_key not in ep_groups:
                ...
                # SOCIS's own writer uses (mirrors section-4 grouping).
            ...
            # ``socis model``, not an allowlist — see
        ...
        for grp in ep_groups.values():
            ...
            #   Ollama/llama.cpp match ``socis model``. Pin with
            ...
            if _probe_live:
                ...
                try:
                    from socis_cli.models import cached_fetch_api_models
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            # active model. SOCIS's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, socis_cli/config.py) already consume that dict.
        ...
        for grp in groups.values():
            ...
            #   ``socis model``. Pin a dict catalog with
...
def _prepend_moa_picker_provider(providers: List[dict], current_provider: str = "") -> List[dict]:
    ...
    """
    try:
        from socis_cli.inventory import _moa_provider_row
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`socis_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from socis_cli.models import fetch_openrouter_models
