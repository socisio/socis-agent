"""
SOCIS Agent Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``socis desktop`` / ``socis gui``)
     Built inside the agent checkout under ``$SOCIS_AGENT_HOME/socis-agent/``:
       ...
       - ``$SOCIS_AGENT_HOME/desktop-build-stamp.json`` (the build freshness stamp)
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/SOCIS.app`` or ``~/Applications/SOCIS.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\SOCIS`` (NSIS per-user)
...
the app name ("SOCIS"), separate from ``$SOCIS_AGENT_HOME``:
  - macOS:   ``~/Library/Application Support/SOCIS``
  - Windows: ``%APPDATA%\\SOCIS``
  - Linux:   ``$XDG_CONFIG_HOME/SOCIS`` (default ``~/.config/SOCIS``)
...
``socis uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def _agent_root(socis_agent_home: Path) -> Path:
    """The agent checkout root — same layout install.sh / install.ps1 use."""
    return socis_agent_home / "socis-agent"


def desktop_userdata_dir() -> Path:
    """Return the Electron ``userData`` directory for the desktop app.

    Mirrors Electron's ``app.getPath('userData')`` for an app named "SOCIS"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "SOCIS"
...
def source_built_gui_artifacts(socis_agent_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``socis desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``socis-agent/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(socis_agent_home)
    ...
    return [
        ...
        socis_agent_home / "desktop-build-stamp.json",
    ]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "SOCIS" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/SOCIS.app"),
            home / "Applications" / "SOCIS.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\SOCIS).
            local_base / "Programs" / "SOCIS",
            # Older / alternate layout some builds used.
            local_base / "socis-desktop",
        ]
        program_files = os.environ.get("ProgramFiles")
        if program_files:
            # NSIS per-machine fallback (needs admin to remove).
            paths.append(Path(program_files) / "SOCIS")
    else:
        ...
        # ``uninstall_gui``.
        from socis_cli.linux_desktop_entry import desktop_entry_path

        data = os.environ.get("XDG_DATA_HOME")
        data_base = Path(data) if data else (home / ".local" / "share")
        paths += [
            # The launcher entry `socis desktop` installs. Its icon is
            ...
            data_base / "applications" / "SOCIS.desktop",
            data_base / "icons" / "hicolor" / "scalable" / "apps" / "socis.png",
        ]
        ...
        for size in ("24x24", "32x32", "48x48", "256x256", "512x512", "1024x1024"):
            paths.append(data_base / "icons" / "hicolor" / size / "apps" / "socis.png")
    return paths


def agent_is_installed(socis_agent_home: Path) -> bool:
    """Return True when a usable Python agent install exists under SOCIS_AGENT_HOME.
    ...
    if (agent_root / "socis_cli").is_dir():
        return True
...
def gui_is_installed(socis_agent_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(socis_agent_home):
        if p.exists():
            return True
...
def gui_install_summary(socis_agent_home: "Path | None" = None) -> dict:
    ...
    """
    home: Path = socis_agent_home if socis_agent_home is not None else get_socis_agent_home()
...
def uninstall_gui(
    socis_agent_home: "Path | None" = None, *, remove_userdata: bool = True
) -> "list[Path]":
    ...
    Never touches ``socis-agent/socis_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$SOCIS_AGENT_HOME``.
    ...
    if sys.platform.startswith("linux"):
        ...
        # Reindex so SOCIS disappears from the launcher.
        try:
            from socis_cli.linux_desktop_entry import (
                desktop_entry_path,
        ...
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove socis' or "
            "'sudo dnf remove socis'). AppImage builds are a single file you "
