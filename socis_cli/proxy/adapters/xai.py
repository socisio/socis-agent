...
# xAI's public API is OpenAI-compatible for the endpoints SOCIS commonly
# uses. The Responses endpoint is included because SOCIS' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via SOCIS-managed OAuth credentials."""

    auth_hint = "socis auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`socis auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`socis auth reset xai-oauth` or re-authenticate with "
                    "`socis auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `socis auth add xai-oauth --type oauth`."
            )
