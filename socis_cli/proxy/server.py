...
:mod:`socis_cli.proxy.sse_done`, issue #90848).
...
from socis_cli.proxy.sse_done import (
    DONE_SSE_FRAME,
    SseDoneTracker,
    content_type_is_sse,
)
...
def create_app(adapter: UpstreamAdapter) -> "web.Application":
    ...
    if not AIOHTTP_AVAILABLE:
        raise RuntimeError(
            "aiohttp is required for `socis proxy`. Run `socis setup` to install it."
        )
    ...
    async def handle_health(request: "web.Request") -> "web.Response":
        ...
        # a concurrent ``socis auth`` command.
