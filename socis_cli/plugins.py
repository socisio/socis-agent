...
1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with socis-agent;
   ``memory/`` and ``context_engine/`` subdirs are excluded — they have their
   own discovery paths)
2. **User plugins**   – ``~/.socis-agent/plugins/<name>/``
3. **Project plugins** – ``./.socis-agent/plugins/<name>/`` (opt-in via
   ``SOCIS_AGENT_ENABLE_PROJECT_PLUGINS``)
4. **Pip plugins**     – packages that expose the ``socis_agent.plugins``
   entry-point group.
...
from socis_agent_constants import (
    ...
    socis_agent_home_key,
    reset_socis_agent_home_override,
    set_socis_agent_home_override,
)
from registration_lifecycle import replacement_coordinator
from utils import env_var_enabled, fast_safe_load
from socis_cli.config import cfg_get, load_config_readonly
from socis_cli.middleware import OBSERVER_SCHEMA_VERSION, VALID_MIDDLEWARE
from socis_cli.plugin_capabilities import (  # noqa: F401 — re-exported
    CAPABILITY_REGISTRY,
    VALID_CAPABILITY_IDS,
    plugin_capability_granted,
)
from socis_cli.plugin_capabilities import (
...
def get_bundled_plugins_dir() -> Path:
    ...
    Honours ``SOCIS_AGENT_BUNDLED_PLUGINS`` (set by the Nix wrapper / packaged
    installs) so read-only store paths are consulted first.  Falls back to
    the in-repo path used during development.
    """
    env_override = os.getenv("SOCIS_AGENT_BUNDLED_PLUGINS")
    if env_override:
...
# ---------------------------------------------------------------------------
#
# Set ``SOCIS_AGENT_PLUGINS_DEBUG=1`` to surface verbose plugin-discovery logs to
# stderr in addition to ~/.socis-agent/logs/agent.log. Aimed at plugin authors
...
_PLUGINS_DEBUG = os.getenv("SOCIS_AGENT_PLUGINS_DEBUG", "").strip().lower() in {
    "1", "true", "yes", "on",
}
...
def _install_plugin_debug_handler(force: bool = False) -> None:
    """When SOCIS_AGENT_PLUGINS_DEBUG is on, tee plugin logs to stderr at DEBUG.

    Idempotent: only attaches the handler once per process unless ``force``
    is passed. Does not touch the root logger or other SOCIS loggers.
    """
    ...
    logger.debug(
        "SOCIS_AGENT_PLUGINS_DEBUG=1 — verbose plugin discovery logging enabled"
    )
...
VALID_HOOKS: Set[str] = {
    ...
    # finish. SOCIS' shipped guidance lives in the evidence-based
    ...
    "pre_transcription",
    # Kanban task lifecycle hooks. Fired by socis_cli.kanban_db when a task
    ...
    # separate `socis -p <profile> chat -q` subprocesses:
    #   - kanban_task_claimed   -> the DISPATCHER process (gateway-embedded
    #                              dispatcher or `socis kanban dispatch`),
    ...
    # ``socis kanban dispatch``); on_kanban_task_updated fires in whichever
    ...
    #   result: socis_cli.kanban_db.DispatchResult (spawned, reclaimed,
...
ENTRY_POINTS_GROUP = "socis_agent.plugins"
ENTRY_POINT_CAPABILITIES_GROUP = "socis_agent.plugin_capabilities"
...
def discover_entrypoint_manifests() -> List["PluginManifest"]:
    ...
    * **Capability declarations** — read from the companion
      ``socis_agent.plugin_capabilities`` entry-point group (declarations
...
PLUGIN_SECTIONS_START = "<!-- socis-plugin-sections:start -->"
PLUGIN_SECTIONS_END = "<!-- socis-plugin-sections:end -->"
...
def format_system_prompt_section(section_id: str, content: str) -> str:
    ...
    return (
        ...
        f"<!-- socis-plugin-section-chars:{len(content)} -->\n\n"
        f"{content}"
    )
...
# Reserved event namespace prefix — only core may publish ``socis:<event>``.
SOCIS_AGENT_EVENT_NAMESPACE = "socis"
...
def _plugin_home_scope(home: Path):
    """Bind discovery and loading to the manager's immutable SOCIS home."""
...
_KNOWN_MANIFEST_FIELDS: Set[str] = {
    ...
    "capabilities", "emits", "listens", "socis", "depends",
}

# Highest manifest schema version this SOCIS understands.
...
def _parse_manifest_v2_fields(data: Mapping, key: str) -> Dict[str, Any]:
    ...
    if mv > SUPPORTED_MANIFEST_VERSION:
        logger.warning(
            "Plugin %s: manifest_version %d is newer than this SOCIS "
        ...
        for dep in manifests[k].requires_plugins:
            ...
            if resolved is None:
                logger.warning(
                    ...
                    "via ctx.has_plugin). Run `socis plugins enable %s` if "
                    "it is installed.",
                    k, dep_id, dep_id,
                )
                continue
...
class PluginManifest:
    ...
    #              in ~/.socis-agent/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
    # Registry key — path-derived, used by ``plugins.enabled``/``disabled``
    # lookups and by ``socis plugins list``. For a flat plugin at
    ...
    # ``socis_cli.plugin_capabilities.CAPABILITY_REGISTRY``. Declaration is
    ...
    # Declared pip dependencies. VALIDATED AND SURFACED ONLY — SOCIS never
    ...
    # (``socis plugins show``); a plugin may emit/subscribe without declaring.
...
def _plugin_relative_segments(key: str) -> tuple[str, ...]:
    ...
    ``retry.policy``).  Full SOCIS paths, traversal syntax, and the security-
...
class PluginState:
    ...
    def data_dir(self) -> Path:
        ...
        return get_socis_agent_home() / "plugin-data" / self._data_namespace
...
class PluginContext:
    ...
    def set_config(self, key: str, value: Any) -> None:
        ...
        # threads. Other SOCIS config writers still retain their existing
    ...
    def platform_actions(self):
        ...
        if self._platform_actions is None:
            from socis_cli.platform_actions import PlatformActions
    ...
    def profile_name(self) -> str:
        """Return the active SOCIS profile name (e.g. ``"default"``).

        Derived from ``SOCIS_AGENT_HOME`` via
        :func:`socis_cli.profiles.get_active_profile_name`, so it works in
        ...
        running under ``~/.socis-agent/profiles/<name>``, or ``"custom"`` when
        ``SOCIS_AGENT_HOME`` points somewhere unrecognized.
        """
        try:
    ...
    def _tool_override_allowed(self, tool_name: str) -> bool:
        ...
        Bundled plugins (shipped with SOCIS core) are trusted by default —
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``socis honcho ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``socis <subcommand>``
    ...
    def register_dashboard_auth_provider(self, provider) -> Optional[PluginRegistration]:
        ...
        ``provider`` must be an instance of
        :class:`socis_cli.dashboard_auth.DashboardAuthProvider`. Used by
        ...
        """
        from socis_cli.dashboard_auth import DashboardAuthProvider
        from socis_cli.dashboard_auth.registry import (
    ...
    def register_secret_source(self, source) -> Optional[PluginRegistration]:
        ...
        sources run during ``load_socis_dotenv()`` startup — after
        ``~/.socis-agent/.env`` loads, before SOCIS reads credentials — when
        ...
        NOTE ON TIMING: ``load_socis_dotenv()`` usually runs at import
        *before* plugin discovery.  After discovery completes, the plugin
        ...
        + ``load_socis_dotenv``) so the first process sees them (#64177).
    ...
    def register_slack_action_handler(
        ...
        SOCIS' Slack adapter wires registered handlers into its
    ...
    def register_telegram_handler(self, factory: Callable) -> None:
        ...
        SOCIS' Telegram adapter invokes registered factories at ``connect()``
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        core files. After registration, the task:
          ...
          - Appears in the ``socis model → Configure auxiliary models`` picker
        ...
        # Lazy import to avoid circular: socis_cli.main imports plugins indirectly
        from socis_cli.main import _AUX_TASKS as _BUILTIN_AUX_TASKS
    ...
    def emit(self, event: str, payload: Optional[dict] = None) -> int:
        ...
        including ``socis:x`` or a foreign ``other:x``) is rejected with a
        ``ValueError`` and a logged warning — fail-closed. The ``socis:``
        ...
        if ":" in event:
            logger.warning(
                ...
                plugin_key, event, plugin_key, SOCIS_AGENT_EVENT_NAMESPACE,
            )
            raise ValueError(
                f"Plugin '{plugin_key}' may not emit '{event}': emit only the "
                f"bare event name; the namespace is forced to '{plugin_key}:' "
                f"and the '{SOCIS_AGENT_EVENT_NAMESPACE}:' prefix is reserved for core"
            )
    ...
    def subscribe(self, event: str, callback: Callable) -> None:
        ...
        *event* is the full ``<plugin_key>:<event>`` name (or ``socis:<event>``
    ...
    def register_skill(
        ...
        ``~/.socis-agent/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def __init__(self, scope_key: Optional[str] = None) -> None:
        ...
        # original scope.
        self.scope_key = scope_key or socis_agent_home_key()
        ...
        # by resolved socis home). The ledger is therefore keyed per manager
        # — i.e. per (socis_agent_home, plugin_id) — and every release/restore
        ...
        # contributed, so `socis plugins list` still attributes them once the
        # full plugin loads.
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        with self._discovery_lock, _plugin_home_scope(self.home_path):
            ...
            if force:
                ...
                self.unload()
            if env_var_enabled("SOCIS_AGENT_SAFE_MODE"):
                logger.info("SOCIS_AGENT_SAFE_MODE=1 — plugin discovery skipped")
            ...
            try:
                ...
                # load_socis_dotenv() already ran at import time. Re-pull so the
    ...
    def _refresh_secret_sources_after_discovery(self) -> None:
        ...
        try:
            ...
            from socis_cli.env_loader import load_socis_dotenv, reset_secret_source_cache
        except Exception:
            return
        try:
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        if stale_relay_keys:
            logger.warning(
                "Removed SOCIS plugin %s is still listed in plugins.enabled; "
        ...
        for manifest in winners.values():
            ...
            # Relay lifecycle ownership now lives in the SOCIS core. Loading
            ...
            if (
                ...
                loaded.error = (
                    "removed — Relay lifecycle is owned by SOCIS core; configure "
                    f"{RELAY_PLUGINS_CONFIG_ENV} instead"
                )
                self._plugins[lookup_key] = loaded
                logger.warning(
                    "Refusing to load removed SOCIS Relay plugin '%s'; %s",
                    lookup_key,
                    loaded.error,
                )
                continue
            ...
            # Built-in backends auto-load — they ship with socis and must
            ...
            # all ~20 of them added several seconds to every `socis`
            # invocation — including plain `socis chat`, which never touches a
            ...
            # path actually asks for that platform. Every platform SOCIS ships
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `socis plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
    ...
    def register_approval_transport(
        ...
        *,
        ...
        from socis_cli.approval_transport import RegisteredApprovalTransport
        ...
        self._approval_transports[clean] = RegisteredApprovalTransport(
            ...
            present=present_fn,
            plugin_id=plugin_id,
            profile_home=str(get_socis_agent_home().resolve()),
        )
    ...
    def get_approval_transport(self, name: str):
        ...
        if registered is None:
            return None
        if registered.profile_home != str(get_socis_agent_home().resolve()):
            return None
        return registered
    ...
    def _collect_directory_manifests(self) -> List[PluginManifest]:
        ...
        # 2. User plugins (~/.socis-agent/plugins/)
        user_dir = get_socis_agent_home() / "plugins"
        logger.debug("Scanning user plugins: %s", user_dir)
        user_manifests = self._scan_directory(user_dir, source="user")
        logger.debug("  user: %d manifest(s)", len(user_manifests))
        manifests.extend(user_manifests)

        # 3. Project plugins (./.socis-agent/plugins/), only when explicitly opted
        # in. This must match the full discovery gate exactly.
        if _env_enabled("SOCIS_AGENT_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".socis-agent" / "plugins"
            ...
            logger.debug(
                "Project plugins disabled (set SOCIS_AGENT_ENABLE_PROJECT_PLUGINS=1 to enable)"
            )
    ...
    def has_enabled_portable_mcp(self, raw_config: Mapping[str, Any]) -> bool:
        ...
        """
        if _env_enabled("SOCIS_AGENT_SAFE_MODE"):
            return False
        ...
        for manifest in winners.values():
            ...
            if lookup_key not in enabled and manifest.name not in enabled:
                continue
            try:
                from socis_cli.agent_plugins import _discover_mcp
        ...
        *,
        ...
        for child in sorted(path.iterdir()):
            ...
            if portable_file.exists() or portable_file.is_symlink():
                try:
                    from socis_cli.agent_plugins import read_agent_plugin_manifest
    ...
    def _classify_entrypoint_kind(self, ep) -> str:
        ...
        module's import cost in every SOCIS process (e.g. a pip
    ...
    def _scan_entry_points(self) -> List[PluginManifest]:
        ...
        declarations from the ``socis_agent.plugin_capabilities`` group.
    ...
    def _register_deferred_platform(self, manifest: PluginManifest) -> None:
        ...
        ``socis plugins list`` still shows the platform as available, and we
        hand the registry a loader that runs the normal eager-load path.
        """
        lookup_key = manifest.key or manifest.name
        platform_name = self._platform_name_from_manifest(manifest)

        # Record an enabled placeholder for introspection (`socis plugins
    ...
    def _register_deferred_platform_tools(
        ...
        ``socis tools`` checklist, and even an explicit ``platform_toolsets``
        ...
        try:
            ...
            # `socis plugins list` under-reports what the process is actually
    ...
    def _warn_python_dependencies(self, manifest: PluginManifest) -> None:
        ...
        python_dependencies is a declaration seam ONLY: SOCIS validates and
        ...
        if missing:
            logger.warning(
                ...
                "installed: %s. SOCIS does not install plugin dependencies "
    ...
    def _load_plugin_scoped(self, manifest: PluginManifest) -> None:
        ...
        try:
            ...
            if register_fn is None:
                ...
                # above cannot see them and `socis plugins list` would
    ...
    def _load_portable_plugin(
        ...
        lookup_key = manifest.key or manifest.name
        try:
            from socis_cli.agent_plugins import load_agent_plugin

            package = load_agent_plugin(
                Path(manifest.path),
                get_socis_agent_home() / "plugin-data" / manifest.skill_namespace,
            )
            ctx = PluginContext(manifest, self)
    ...
    def _load_directory_module(
        ...
        *,
        ...
        """Import a directory-based plugin as ``socis_plugins.<slug>``.

        The module slug is derived from ``manifest.key`` so category-namespaced
        plugins (``image_gen/openai``) import as
        ``socis_plugins.image_gen__openai`` without colliding with any
        future ``tts/openai``.
        """
        ...
        # from a different SOCIS home (profile switch reusing a slug
        # like "socis-lcm") or from an earlier force=True reload in the
            ...
            try:
                if use_timeout:
                    ...
                    thread = threading.Thread(
                        target=_runner,
                        name=f"socis-hook-{callback_name}"[:40],
                        daemon=True,
                    )
                    thread.start()
...
# Keyed cache: resolved SOCIS home -> PluginManager. SOCIS supports
# multiple profiles via different SOCIS_AGENT_HOME directories, and a single
# long-lived process (gateway multiplexer, test session, embedder) can
# switch between them via ``set_socis_agent_home_override()`` — which is a
# ContextVar and deliberately does NOT touch os.environ (see
# socis_agent_constants.set_socis_agent_home_override). A process-wide single-slot
...
def _plugin_home_key() -> Path:
    ...
    Plugins are discovered from ``get_socis_agent_home() / "plugins"`` and some
    plugins (notably context engines such as socis-lcm) capture that home
    at registration time for profile-scoped storage. A long-lived process
    can temporarily switch SOCIS home (env var *or* the context-local
    ``set_socis_agent_home_override()``) while serving another profile, so the
    plugin manager must be scoped to the active SOCIS home instead of
    being one process-wide singleton.
    """
    try:
        return get_socis_agent_home().expanduser().resolve()
    except Exception:
        return get_socis_agent_home().expanduser()
...
def _clear_plugin_submodules(manager: Optional[PluginManager]) -> None:
    ...
    ``socis_plugins.<slug>`` and registers that top-level module in
    ...
    *relative* import (``from . import foo``, ``from .sub import bar``)
    ...
    ``socis_plugins.<slug>.<submodule>``. When we swap in a fresh manager
...
def get_plugin_manager() -> PluginManager:
    """Return the plugin manager for the active SOCIS profile/home.
    ...
    profile switch — via ``SOCIS_AGENT_HOME`` or the context-local
    ``set_socis_agent_home_override()`` — gets its own manager with its own
    ...
    with _plugin_managers_lock:
        ...
        if manager is None:
            manager = PluginManager(scope_key=socis_agent_home_key(current_home))
...
def _plugin_toolset_keys_cache_path():
    ...
    return get_socis_agent_home() / "cache" / "plugin_toolset_keys.json"
...
def _get_pre_tool_call_directive_details(
    ...
    from socis_cli.lifecycle import invoke_hook as invoke_lifecycle_hook
...
def resolve_plugin_command_result(result: Any) -> Any:
    ...
    thread = threading.Thread(
        target=_runner,
        name="socis-plugin-command-await",
        daemon=True,
    )
    thread.start()
...
def get_plugin_subscriptions() -> Dict[str, List[Callable]]:
    ...
    (``<plugin_key>:<event>`` or ``socis:<event>``) to subscriber callbacks in
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``socis tools`` TUI so plugin-provided toolsets appear
