"""SOCIS update pipeline — extracted from ``socis_cli/main.py``.
...
change is that references to helpers/constants that STAY in ``socis_cli.main``
(and to moved-but-test-patched siblings) are routed through ``_m()`` — a lazy
``socis_cli.main`` reference — so existing call sites and test monkeypatches
that target ``socis_cli.main.<name>`` (``PROJECT_ROOT``, ``_is_windows``,
...
the test-patch surface still resolve on ``socis_cli.main``.
...
Imports are one-way: ``socis_cli.main`` imports this module, never the reverse
...
from socis_agent_constants import get_default_socis_root, venv_python_path

logger = logging.getLogger(__name__)


def _m():
    """Lazy ``socis_cli.main`` reference.

    Lets callers keep patching ``socis_cli.main.<helper>`` (the historical
    test surface) and have those patches reach this code path, and defers the
    import so ``socis_cli.main`` -> ``socis_cli.update_cmd`` stays one-way
    at import time.
    """
    from socis_cli import main
...
#: ``_purge_stale_socis_modules`` so any LATER import chain resolves against
...
_STALE_PURGE_PROTECTED = frozenset(
    {
        ...
        "socis_cli.socis_agent_logging",
    }
)


def _purge_stale_socis_modules() -> None:
    """Evict every cached SOCIS module after the checkout changed in-place.

    ``socis update`` keeps running in the pre-pull Python process. The
    gateway auto-restart phase that follows does function-level
    ``from socis_cli.gateway import ...`` — executing NEW source inside an
    ...
    field failure: freshly-pulled ``socis_cli.gateway`` does
    ``from socis_cli.cli_output import line_input``, but ``cli_output`` was
    ...
    export. This is the class fix: drop EVERY cached module under the SOCIS
    ...
    try:
        ...
        if purged:
            logger.debug(
                "Purged %d stale SOCIS module(s) after checkout update", len(purged)
            )
    except Exception as exc:
        logger.debug("Could not purge stale SOCIS modules: %s", exc)


def _reload_updated_runtime_modules() -> None:
    """Reload update-sensitive modules after the checkout changes in-place.

    ``socis update`` keeps running in the pre-pull Python process. After a
...
def _reload_config_modules() -> None:
    ...
    ``socis update`` runs in the PRE-pull Python process. After ``git pull``
    ...
    This function force-reloads ``socis_cli.config_defaults``,
    ``socis_cli.config``, and ``socis_cli.config_migrations`` from disk
    so subsequent imports read the UPDATED code.

    It also reloads ``socis_cli._subprocess_compat`` and
    ``socis_cli.dashboard_procs`` so that post-update dashboard cleanup
    ...
    importlib.invalidate_caches()
    for mod_name in (
        "socis_cli.config_defaults",
        "socis_cli.config",
        "socis_cli.config_migrations",
        "socis_cli._subprocess_compat",
...
def _run_config_check_fresh() -> tuple:
    ...
    from socis_cli.config import check_config_version
...
def _run_migrate_config_fresh(*, interactive: bool = False, quiet: bool = False) -> dict:
    ...
    """
    _reload_config_modules()
    from socis_cli.config import migrate_config
...
def _migrate_sibling_profile_configs() -> list[tuple[str, int, int]]:
    ...
    shared checkout serves every profile, but ``socis update`` historically
    ...
    caller): scope config reads/writes via the context-local SOCIS_AGENT_HOME
    ...
    try:
        ...
        from socis_cli.profiles import _get_profiles_root, _PROFILE_ID_RE

        active_home = get_process_socis_agent_home()
        ...
        for entry in sorted(root.iterdir()):
            ...
            token = set_socis_agent_home_override(entry)
            try:
...
def _check_and_apply_config_migration(
    *,
    ...
    try:
        ...
        print("     Run 'socis config migrate' to check manually.")
        return
    ...
    if version_bump_only:
        ...
        try:
            ...
            print("     Run 'socis config migrate' to retry.")
        ...
        if assume_yes:
            ...
            try:
                ...
                print(
                    ...
                    "Run 'socis config migrate' manually to configure."
                )
                response = "n"
        ...
        if response in {"", "y", "yes", "auto"}:
            ...
            if (gateway_mode or assume_yes or response == "auto") and missing_env:
                print("  ℹ API keys require manual entry: socis config migrate")
        else:
            print()
            print("Skipped. Run 'socis config migrate' later to configure.")
    else:
    ...
    # context-local SOCIS_AGENT_HOME override (never os.environ — other
    ...
    # it and warn loudly.
    try:
        from socis_cli.backup import restore_cron_jobs_if_emptied
    ...
    try:
        from socis_cli.backup import restore_config_model_settings_if_rewritten
    ...
    try:
        from socis_cli.backup import restore_cron_jobs_all_profiles
    ...
    try:
        from socis_cli.backup import restore_config_model_settings_all_profiles
...
# Critical files that SOCIS must be able to import immediately after an
...
_UPDATE_CRITICAL_FILES = (
    "socis_cli/main.py",
    "socis_cli/config.py",
    "socis_cli/__init__.py",
    "socis_cli/web_server.py",
    "cli.py",
    "run_agent.py",
    "model_tools.py",
    "toolsets.py",
    "socis_agent_constants.py",
)
...
def _prune_orphan_rescue_refs(
    ...
    ``refs/socis-update-backups/orphan-<branch>-<ts>-<sha>``. A rescue ref
    ...
    try:
        list_result = subprocess.run(
            git_cmd + [
                ...
                "--format=%(refname)",
                "--sort=refname",
                f"refs/socis-update-backups/orphan-{branch}-*",
            ],
            cwd=cwd,
            capture_output=True,
            text=True, encoding="utf-8", errors="replace",
        ...
        if max_age_days > 0:
            ...
            prefix = f"refs/socis-update-backups/orphan-{branch}-"
            for ref in refs:
...
def _editable_install_is_current(git_cmd, cwd, pre_pull_sha: str | None) -> bool:
    ...
    On Windows that rewrite is the only reason the running ``socis.exe`` has
    ...
    Skipping is safe because SOCIS pins its editable finder to a *static*
...
def _validate_python_files_syntax(
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="socis-syntax-check-") as tmpdir:
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``socis`` startup; if any of them
...
def _critical_module_import_failures(
    ...
    ``_venv_core_imports_healthy``): ``socis update`` can be driven by a
    ...
    a later one.
    """
    from socis_agent_constants import FIRST_PARTY_MODULE_ROOTS

    import secrets

    marker = f"__SOCIS_AGENT_IMPORT_HEALTH_{secrets.token_hex(16)}__"
    probe = (
        ...
        # The root set is injected from socis_agent_constants so this can't drift
        # from the hint the user is shown (they disagreed once already).
        "        missing = (getattr(exc, 'name', '') or '').split('.')[0]\n"
        "        if missing in %r or missing.startswith('socis_') or %r:\n"
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``socis update --gateway`` so interactive prompts (stash restore,
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `socis update`.
    ...
    print("  Preview now:  socis curator run --dry-run")
    print("  Pause it:     socis curator pause")
    print(
        "  Docs:         https://agent.socis.io/docs/user-guide/features/curator"
    )

def _print_fts_optimize_available_notice() -> None:
    """Advertise the opt-in v23 search-index optimization after `socis update`.
    ...
    try:
        ...
        return
    db_path = get_socis_agent_home() / "state.db"
    ...
    if interrupted:
        ...
        print(
            "  A previous `socis sessions optimize-storage` run was "
            "interrupted. Search still works; re-run the command to resume "
            "and finish reclaiming disk:"
        )
        print("    socis sessions optimize-storage")
        return
    ...
    print("  Run when convenient:  socis sessions optimize-storage")
    print(
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``socis update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``socis update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: socis curator status)"
    )
...
def _reload_process_scan_modules() -> None:
    ...
    ``from socis_cli._subprocess_compat import bounded_probe_run``. If the
...
def _finish_dashboard_update_cleanup(
    ...
    *already_restarted_units* forwards the systemd unit names (no
    ...
    print("    socis dashboard --port <port>")
...
def _atomic_replace_dir(src: str, dst: str) -> None:
    ...
    ``socis_cli.main`` re-export surface and guards the #49145 regression.
    """
...
def _stage_replacement(src: str, dst: str) -> str:
    ...
    untouched.
    """
    staging = f"{dst}.socis-update-staging"
    backup = f"{dst}.socis-update-old"
...
def _discard_staged(staged) -> None:
    ...
    the tree. The user then follows the "re-run `socis update`" advice with
...
def _commit_staged_replacements(staged) -> None:
    ...
    first-party modules (``run_agent.py``, ``cli.py``, ``socis_agent_constants.py``
...
def _assess_parked_branch_switch(
    ...
    stale feature branch left behind by earlier tooling; ``socis update``
...
def _print_parked_branch_skip_warning(
    ...
    print(f"    git -C {cwd} checkout {target_branch} && socis update")
    print(
...
def _print_update_completion(message: str) -> None:
    ...
    action_id = os.environ.get("SOCIS_AGENT_ACTION_ID", "")
    if len(action_id) == 32 and all(char in "0123456789abcdef" for char in action_id):
        print(f"=== socis-update completed {action_id} ===")
...
def _refuse_update_for_contended_shims(exc: BaseException) -> None:
    ...
    """
    print("✗ Cannot continue the update: live SOCIS launcher(s) could not be")
    print("  moved aside:")
    for name in getattr(exc, "failed_shims", []) or ["socis.exe"]:
        print(f"    {name}")
    print("  Another process is holding this install's venv — typically SOCIS")
    print("  Desktop, a gateway, or another socis REPL — and mutating the venv")
    print("  now would strand it half-updated.")
    print("  The dependency install has been deferred: close the process(es)")
    print("  above, then run any `socis` command to finish it automatically.")
...
def _should_zip_fallback_on_update_error(exc: BaseException) -> bool:
    ...
    A dependency-install failure (locked ``socis.exe`` / ``uv pip install``
...
def _zip_overlay_block_reason(
    ...
    two-phase replace creates ``*.socis-update-staging`` siblings inside the
...
_ZIP_STAGING_ARTIFACT_SUFFIXES = (".socis-update-staging", ".socis-update-old")
...
def _abort_zip_update_if_dirty_tree() -> None:
    ...
    print("  Stash or commit your changes, then rerun `socis update`.")
...
def _post_update_sqlite_runtime_status():
    ...
    from socis_cli.sqlite_runtime import probe_sqlite_runtime
...
def _print_verified_update_completion(message: str) -> bool:
    """Print a success completion only after probing the next SOCIS runtime."""
    ...
    print(
        "  Rebuild the SOCIS venv with a uv-managed Python, restart SOCIS, "
        "then verify with `socis doctor`."
    )
    return False
...
def _clear_stale_sqlite_sidecars(db_path: Path) -> None:
    ...
    database (a crashed writer, or a second SOCIS process the updater's drain
...
def _print_update_summary(
    *,
    ...
    if node_failures or not desktop_build_ok or not sqlite_runtime_ok:
        ...
        if not desktop_build_ok:
            print("  Run `socis desktop` to retry the desktop rebuild.")
        if not sqlite_runtime_ok:
            print(
                "  The Python runtime remediation did not complete. Run `socis "
                "update` again; if SQLite is unchanged, rebuild the SOCIS venv "
                "with a uv-managed Python, restart SOCIS, then verify with "
                "`socis doctor`."
            )
    else:
...
def _write_gateway_update_exit_code(ok: bool) -> None:
    path = get_socis_agent_home() / ".update_exit_code"
    try:
...
def _restore_state_db_from_snapshot(state_path: Path, snap_state: Path) -> bool:
    ...
    """
    from socis_cli.backup import _foreign_db_holder_pids, verify_sqlite_integrity
    from socis_cli.sqlite_safe_read import LiveConnectionError, offline_file_access

    holders = _foreign_db_holder_pids(state_path)
    if holders:
        print(
            f"  ✗ Auto-restore refused: process(es) {holders} still hold "
            "state.db or its WAL open. Stop them (socis gateway stop), "
    ...
    try:
        ...
        print(
            ...
            "handles (or restart SOCIS) and retry."
        )
        return False
...
def _verify_and_restore_one_state_db(home: Path, *, label: str) -> None:
    ...
    """
    try:
        from socis_cli.backup import _quick_snapshot_root, verify_sqlite_integrity
...
def _verify_and_restore_state_dbs_post_update() -> None:
    ...
    try:
        from socis_cli.backup import _sibling_profile_homes
...
def _update_via_zip(args, *, had_desktop_app_before_update: bool = False) -> bool:
    """Update SOCIS Agent by downloading a ZIP archive.
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `socis update "
            f"--branch {branch}`, or update against main with `socis update`."
        )
        _m().sys.exit(1)
    _abort_zip_update_if_dirty_tree()
    zip_url = (
        f"https://github.com/socisio/socis-agent/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    tmp_dir = tempfile.mkdtemp(prefix="socis-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"socis-agent-{branch}.zip")
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for socis-agent itself
        ...
        # GitHub ZIPs extract to socis-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"socis-agent-{branch}")
        ...
        try:
            for item in entries:
                ...
                # SOCIS.exe) exists only in the LIVE tree. Swapping `apps`
        ...
        print(
            "  Re-run `socis update` to retry; if the agent won't start, "
            "reinstall from https://agent.socis.io"
        )
        _m().sys.exit(1)
    finally:
    ...
    from socis_cli.managed_uv import ensure_uv, update_managed_uv
    ...
    if uv_bin:
        ...
        from socis_cli.managed_uv import managed_python_env
    ...
    if not import_ok:
        ...
        print("  Re-run `socis update` to complete it.")
        _m().sys.exit(1)
    ...
    try:
        ...
        if result.get("user_modified"):
            ...
            print(
                "    → see them: socis skills list-modified  "
    ...
    try:
        from socis_cli.model_catalog import seed_cache_from_checkout
    ...
    try:
        from socis_cli.update_receipt import finalize_update_receipt
...
_AUTOSTASH_NAME_PREFIX = "socis-update-autostash-"

#: Age past which a leftover ``socis-update-autostash-*`` entry is called out
...
def _warn_orphaned_update_autostashes(git_cmd: list[str], cwd: Path) -> int:
    ...
    SOCIS never drops one automatically.
...
def _park_stashed_changes(stash_ref: str) -> None:
    ...
    Used by ``socis update --keep-stash`` (the desktop updater's mode): the
...
def _reject_unsafe_stash_restore(
    ...
    print()
    print("✗ Restored local changes made the SOCIS agent unexecutable.")
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if SOCIS behaves unexpectedly.")
    ...
    if restore.returncode != 0 and not has_conflicts and (
        ...
        # files makes socis completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but SOCIS couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but SOCIS couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if SOCIS behaves unexpectedly.")
    return True
...
def _discard_stashed_changes(
    ...
    if stash_selector is None:
        print(
            ...
            "but SOCIS couldn't find the stash entry to drop."
        )
        _print_stash_cleanup_guidance(stash_ref)
        return False
    ...
    if drop.returncode != 0:
        print(
            "⚠ Configured to discard local changes, but SOCIS couldn't drop "
...
OFFICIAL_REPO_URLS = {
    "https://github.com/socisio/socis-agent.git",
    "git@github.com:socisio/socis-agent.git",
    "https://github.com/socisio/socis-agent",
    "git@github.com:socisio/socis-agent",
}

OFFICIAL_REPO_URL = "https://github.com/socisio/socis-agent.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_socis_agent_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_socis_agent_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
    ...
    *,
    ...
    if not has_upstream:
        ...
        if assume_yes or (
            ...
            print(
                "  Add it later with: git remote add upstream https://github.com/socisio/socis-agent.git"
            )
            return False
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/socisio/socis-agent.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/socisio/socis-agent.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``socis update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from socis_agent_constants import get_default_socis_root

    default_home = get_default_socis_root()
...
# ``fleet_restart_pending`` lives under SOCIS_AGENT_HOME (not next to the venv).
...
def _fleet_restart_pending_marker_path() -> Path:
    """SOCIS_AGENT_HOME breadcrumb for a pull that has not yet restarted the fleet."""
    return get_socis_agent_home() / _FLEET_RESTART_PENDING_NAME
...
def _current_checkout_sha() -> str | None:
    ...
    try:
        from socis_cli.build_info import get_code_identity
...
def _warn_pending_fleet_restart(*, startup: bool = False) -> None:
    ...
    print(
        "⚠ A previous `socis update` pulled new code but did not "
    ...
    if startup:
        print(
            "  Run `socis update` or `socis gateway restart`.",
            file=stream,
        )
...
def _restart_systemd_gateway_units_best_effort(failed: list) -> None:
    """Best-effort ``systemctl restart`` of every socis-gateway/serve unit."""
...
def _apply_pending_fleet_restart_catchup() -> None:
    """On an already-up-to-date ``socis update``, finish a skipped restart.
    ...
    if _run_pending_fleet_restart():
        ...
        return
    print("  ⚠ Fleet restart incomplete. Recover with: socis gateway restart")
    sys.exit(1)
...
def _format_concurrent_instances_message(
    ...
    shim = scripts_dir / "socis.exe"
    lines = ["✗ Another socis.exe is running:"]
    ...
    lines.append("")
    lines.append("  Close SOCIS Agent Desktop, exit any open `socis` REPLs, and")
    lines.append("  stop the gateway (`socis gateway stop`) before retrying.")
    lines.append("")
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `socis update --force` if you've already")
...
def _classify_concurrent_instance(pid: int) -> str:
    ...
    from socis_cli._scan_venv_blockers import _is_pausable_gateway  # noqa: PLC0415
...
def _filter_non_gateway_concurrent_instances(
    ...
    ``socis update``. If every concurrent instance is a gateway, the pause
    ...
    TUI shell, a SOCIS Agent Desktop backend child, an unrelated ``socis`` REPL)
...
def _capture_active_tool_dependencies() -> list[str]:
    """Snapshot Python dependencies installed explicitly through ``socis tools``."""
    try:
        from socis_cli import tools_config

        return tools_config.active_restorable_python_tool_dependencies()
    except Exception as exc:
        logger.debug("Could not snapshot active SOCIS Tools dependencies: %s", exc)
        return []
...
def _restore_active_tool_dependencies(
    ...
    *,
    ...
    """Restore allowlisted ``socis tools`` dependencies into a rebuilt venv.
    ...
    try:
        ...
        logger.debug("SOCIS Tools dependency restore skipped (import failed): %s", exc)
        return
    ...
    print()
    print(f"→ Restoring {len(missing)} SOCIS Tools dependency set(s)...")
    restored: list[str] = []
...
def _refresh_active_lazy_features(
    ...
    *,
    ...
    first use. ``socis update`` runs ``uv pip install -e .[all]`` which
    ...
    if install_cmd_prefix is None:
        print("  ⚠ Lazy refresh failed; rerun `socis update` once resolved.")
        return False
    ...
    if status == "healthy":
        ...
        print("  Rerun `socis update` once the upstream issue is resolved.")
        return True
...
def _refresh_active_memory_provider_dependencies() -> None:
    ...
    ``hindsight-all``), NOT in SOCIS' editable-install extras or
    ...
    try:
        from socis_cli.memory_setup import _install_dependencies
    except Exception as exc:
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from socis_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    standalone uv into ``$SOCIS_AGENT_HOME/bin/uv``, but on Termux the official
    ...
    """
    from socis_cli.managed_uv import resolve_uv
...
def _npm_manifest_paths() -> tuple[Path, ...]:
    ...
    lockfile is then unchanged but `socis update` is exactly the step
...
def _npm_lockfile_changed(socis_root: Path) -> bool:
    ...
    # landed must NOT skip the reinstall — otherwise every later `socis
    ...
    try:
        ...
        cache_file = socis_root / f".npm_lock_hash_{cache_key}"
...
def _record_npm_lockfile_hash(socis_root: Path) -> None:
...
def _repair_node_deps_on_current_checkout(
    ...
    *,
    ...
    socis update" — but the early return never reached the Node refresh,
    ...
    if node_failures:
        ...
        print("    Fix npm and re-run `socis update`.")
        print_completion(
...
def _update_node_dependencies() -> list[str]:
    ...
    if not npm:
        ...
        from socis_agent_constants import is_wsl
        ...
        if is_wsl() and path_npm and _m()._is_windows_npm_path(path_npm):
            ...
            print("    package manager), then re-run `socis update`.")
            failed = []
            if any(
    ...
    # SOCIS profile using this checkout. Keep one per-checkout cache under the
    # shared SOCIS root rather than rerunning npm once per named profile.
    shared_socis_root = get_default_socis_root()
    ...
    # `socis update` case. Synchronous and can block ~11s on a true cold
    ...
    if not _m()._npm_lockfile_changed(shared_socis_root):
    ...
    def _partial_update_failure(*labels: str) -> list[str]:
        ...
        print("    deps). Fix npm and re-run `socis update`.")
        return list(labels)
    ...
    nixos_env = with_socis_node_path(_m()._nixos_build_env())
    ...
    # `socis update` comes from the *desktop* build, not this step; that
    ...
    if result.returncode == 0:
        _record_npm_lockfile_hash(shared_socis_root)
...
def _log_only_write(text: str) -> None:
    """Write ``text`` to ``~/.socis-agent/logs/update.log`` only, never the terminal.

    During ``socis update`` ``sys.stdout`` is an ``_UpdateOutputStream`` that
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``socis update --check``: fetch and report without installing.
    ...
    from socis_cli.gitlock import clear_stale_git_locks, clear_stale_tmp_packs
    ...
    if is_shallow:
        ...
        if head_sha and target_sha and head_sha == target_sha:
            ...
            from socis_cli.banner import _github_compare_behind
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``socis update`` without requiring a reinstall.  The
    ...
    ``socis`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/socis.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v socis'`` already resolves.  Idempotent.
    """
    ...
    # /usr/local/bin/socis, code at /usr/local/lib/socis-agent).
    fhs_link = Path("/usr/local/bin/socis")
    ...
    path_comment = (
        "# SOCIS Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _ensure_acp_launcher() -> None:
    r"""Self-heal: install a ``socis-acp`` launcher next to the ``socis`` one.

    Mirrors the launcher block in ``scripts/install.sh`` so existing installs
    gain the ACP command on ``socis update`` without a reinstall.  ACP hosts
    (Zed, JetBrains, Buzz Desktop) spawn the agent by resolving the
    ``socis-acp`` command name against the login-shell PATH; the console
    script of that name lives inside the install's venv, which is not on that
    PATH, so those hosts report SOCIS as not installed even when it is.

    The shim simply delegates to the sibling ``socis`` launcher with the
    ...
    No-op on Windows (install.ps1 stages the ``socis`` / ``socis-acp``
    launchers into the managed binary dir ``$SOCISHome\bin`` and puts THAT
    ...
    ``socis_cli._install_repair.ensure_windows_bin_launchers`` re-stages
    them) and wherever a ``socis-acp`` is already present next to the
    ``socis`` command.  Unwritable directories (e.g. ``/usr/local/bin`` as
    ...
    for bin_dir in (Path.home() / ".local" / "bin", Path("/usr/local/bin")):
        socis_cmd = bin_dir / "socis"
        acp_cmd = bin_dir / "socis-acp"
        try:
            if not (socis_cmd.is_file() or socis_cmd.is_symlink()):
                continue
...
def _run_pre_update_backup(args) -> Optional[str]:
    ...
    - ``full``  — the quick snapshot PLUS a full zip of SOCIS_AGENT_HOME under
      ``backups/`` (restorable via ``socis import``; the #48200 wrong-path
    ...
    snapshot_id = None
    try:
        from socis_cli.backup import (
            _quick_snapshot_root,
            create_quick_snapshot,
            verify_sqlite_integrity,
        )

        # NOTE: this function later does `from socis_agent_constants import
        # get_socis_agent_home`, which makes the name function-local — the
        # module-level import is shadowed and unbound here. Alias explicitly.
        from socis_cli.config import get_socis_agent_home as _get_home
        ...
        try:
            from socis_cli.backup import create_pre_update_snapshots_all_profiles
            ...
            if _sibling_snaps:
                ...
                try:
                    from socis_cli.update_receipt import record_step
    ...
    try:
        from socis_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Human-readable size
    from socis_cli.sizefmt import format_bytes

    size_str = format_bytes(size_bytes)

    # Render path using display_socis_agent_home so the user sees ~/.socis-agent/...
    try:
        from socis_agent_constants import get_socis_agent_home, display_socis_agent_home

        home = get_socis_agent_home()
        try:
            display_path = f"{display_socis_agent_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  socis import {out_path}")
...
def _venv_core_imports_healthy() -> tuple[bool, str]:
    ...
    ``socis update`` may be driven by a different Python). Catches the
    ...
    Without this probe, ``socis update`` on a current checkout prints
    ...
    if not venv_python.exists():
        ...
        # dev may run socis from any interpreter), so report healthy to
        # avoid forcing reinstalls. But on a MANAGED install (the Windows
        # installer / desktop bootstrap stamps `.socis-bootstrap-complete`,
        ...
        managed_markers = (
            _m().PROJECT_ROOT / ".socis-bootstrap-complete",
...
def _detect_venv_python_processes(
    *, exclude_pids: set[int] | None = None
    ...
    The socis.exe shim guard misses the biggest lock-holder class on
    Windows: the Desktop app's backend (``python.exe -m socis_cli.main
    ...
    calling process and its ancestors are always excluded (a CLI ``socis
    ...
    try:
        ...
        # fetch cmdline/cwd lazily for plausible Python/uv/SOCIS candidates.
    ...
    for proc in proc_iter:
        ...
        if not is_holder and not (
            ...
            or name_low in {"uv.exe", "uvx.exe", "socis.exe"}
        ):
            continue
        ...
        # the `-m socis_cli.main gateway run` argv, so autostarted gateways
...
# ``cryptography`` is the canonical case: ``socis_cli.main`` used to import
...
def _abort_dependency_sync_if_self_locked(gateway_resume=None) -> None:
    ...
    - The ``socis.exe`` console shim we were launched from (#88838, #89599).
      The marker cannot help here — every future ``socis`` launch is also the
...
def _defer_update_for_self_lock(loaded: list[str]) -> None:
    ...
    print("  sync has been deferred: the next `socis` launch will complete it")
...
def _socis_holder_subcommand(cmdline: str) -> str | None:
    """The actual SOCIS SUBCOMMAND a venv-holder argv runs, or None.

    Token-based, never substring (#90778: ``kanban --preserve-cache``
    contained \"serve\" and got labeled as the Desktop backend). Finds the
    ``socis_cli.main`` / ``socis(.exe)`` entry token, then returns the
    ...
    for i, token in enumerate(tokens):
        ...
        if low.endswith("socis_cli.main") and i > 0 and tokens[i - 1] == "-m":
            entry_idx = i
            break
        base = low.rsplit("\\", 1)[-1].rsplit("/", 1)[-1]
        if base in ("socis", "socis.exe"):
            entry_idx = i
            break
...
def _format_venv_python_holders_message(matches: list[tuple[int, str, str]]) -> str:
    ...
    (#90778): a standalone ``socis dashboard`` must not be labeled as the
    ...
    """
    lines = [
        "✗ Other SOCIS processes are running from this install's venv:",
    ]
    hint_by_subcommand = {
        "serve": "  ← SOCIS backend (if the Desktop app is open, close it)",
        "dashboard": "  ← socis dashboard (stop it: socis dashboard stop, or close that terminal)",
        "gateway": "  ← gateway",
    }
    for pid, name, cmdline in matches[:6]:
        sub = _socis_holder_subcommand(cmdline)
    ...
    lines.append(
        "  Close the SOCIS desktop app / other SOCIS terminals, then re-run:"
    )
    lines.append("    socis update")
    lines.append("  (or use `socis update --force-venv` to proceed anyway at your own risk)")
    return "\n".join(lines)
...
def _venv_launcher_ancestors(pids: list[int]) -> list[int]:
    ...
    # Never return ourselves or our own ancestry: a CLI ``socis update``
...
def _leftover_pausable_gateway_pids(
    ...
    """
    from socis_cli._scan_venv_blockers import _is_pausable_gateway
...
def _refuse_gateway_ancestor_tree_kill(
    ...
    A chat agent can launch plain ``socis update`` through its terminal tool.
    ...
    try:
        from socis_cli.gateway import _is_pid_ancestor_of_current_process
    ...
    print("  Otherwise, run `socis update` from a separate terminal.")
    return True
...
def _ledger_manual_serve_holders(
    ...
    SOCIS spawner). Returns the full ledger entries so the relauncher can
    rebuild the launch command from structured host/port/profile instead of
    parsing argv.
    """
    try:
        from socis_cli.process_identity import ledger_entries, spawner_is_dead
    except Exception:
        return []
...
def _relaunch_stopped_serves(token: dict) -> None:
    ...
    if skipped or failed:
        print(
            ...
            "restart them manually (socis serve --host <ip> --port <port>)."
        )
    try:
        from socis_cli.update_receipt import record_step
...
def _orphaned_desktop_backend_pids(
    ...
    shim before spawning socis-setup, and the update-in-progress marker
    ...
    the update with "SOCIS is still running" while the user stares at zero
    open windows (ryanc's 2026-08-09 01:59/02:17 failures).

    A holder qualifies only when BOTH hold:

    - its cmdline is a SOCIS backend (``socis_cli.main`` + ``serve`` /
      ``dashboard``), and
    ...
    managed-runtime descendants (the ``.socis-runtime`` interpreter child)
    ...
    def _is_backend(argv_low: str) -> bool:
        return "socis_cli.main" in argv_low and (
    ...
    for pid, _name, cmdline in matches:
        ...
        try:
            ...
            # value round-trips through pid_is_socis at kill time. (Reading
...
def _ledger_reapable_backend_pids(
    ...
    (``socis_cli.process_identity``). A holder qualifies when ALL of:
...
def _handoff_reapable_backend_pids(
    ...
    """PIDs of SOCIS ``serve``/``dashboard`` backends safe to reap during a
    ...
    run AND no live Desktop shim (``socis.exe``) is open, NOTHING legitimate
    ...
    - Only SOCIS backends (``socis_cli.main`` + ``serve``/``dashboard``)
    ...
    - Only runs when the CALLER has confirmed the hand-off context
      ...
      ``socis.exe`` shim) — outside that gate this function is never called
...
def _stop_process_trees(
    ...
    ``.socis-runtime`` interpreter child alive and holding the install open
    (#70026). Best effort; never raises.
    """
    from gateway.status import get_process_start_time
    from socis_cli._subprocess_compat import pid_is_socis, windows_hide_flags
    ...
    for entry in pids:
        ...
        try:
            ...
            if not pid_is_socis(
                ...
                logger.debug(
                    "Skipping taskkill of non-SOCIS or changed PID %s",
                    pid,
                )
                continue
            subprocess.run(
...
def _looks_like_desktop_control_plane(cmdline: str) -> bool:
    """True for this-install ``socis serve`` / ``socis dashboard`` argv.
    ...
    """
    if "socis_cli.main" not in (cmdline or "").lower():
        return False
    return _socis_holder_subcommand(cmdline) in ("serve", "dashboard")
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    socis.exe concurrent-instance guard does not see them. They still import
    ...
    print("→ Stopping Windows gateway process(es) before updating SOCIS...")
    try:
    ...
    # — e.g. a Windows Scheduled Task running ``pythonw.exe -m socis_cli.main
    ...
    if unmapped_pids:
        ...
        if respawnable < len(unmapped_pids):
            ...
            print("    Restart manually after update: socis gateway run")
...
def _cold_start_windows_gateway_after_update() -> bool:
    ...
    ``socis gateway start`` uses (``gateway_windows._spawn_detached``).
    ...
    try:
        from socis_cli import gateway_windows
        from socis_cli.gateway import find_gateway_pids
    except Exception as exc:
        raise RuntimeError(
...
def _for_each_systemd_gateway_unit(
    ...
    *,
    ...
    """Process each ``socis-gateway*.service``/``socis-serve*.service`` unit
    ...
    for line in (list_units_stdout or "").strip().splitlines():
        ...
        # ``unit.startswith("socis-serve")`` alone would also accept the
        # unrelated ``socis-server.service`` — require the exact base unit
        # or the hyphenated profile family instead (review on #83595).
        if not (
            unit == "socis-gateway.service"
            or unit.startswith("socis-gateway-")
            or unit == "socis-serve.service"
            or unit.startswith("socis-serve-")
        ):
            continue
...
def _service_unit_supports_graceful_sigusr1_restart(svc_name: str) -> bool:
    ...
    Only ``socis-gateway*`` units run ``gateway/run.py``, which installs the
    SIGUSR1 handler. ``socis-serve*`` units (#83438) don't, so sending them
    ...
    (``socis-gateway-helper`` is fine — profile units are
    ``socis-gateway-<profile>`` — but ``socis-gatewayd``-style names are
    not) can't be sent a SIGUSR1 it doesn't handle.
    """
    return svc_name == "socis-gateway" or svc_name.startswith("socis-gateway-")
...
def _warn_incomplete_gateway_fleet_restart(failed_units: list) -> None:
    ...
    from socis_cli.gateway import is_macos
    ...
    print("    socis gateway status")
    if any(not name.startswith("ai.socis.") for name in ordered):
        print("    systemctl --user restart <unit>   # user-scope")
        print("    sudo systemctl restart <unit>     # system-scope")
    if any(name.startswith("ai.socis.") for name in ordered):
...
def _restart_launchd_gateway_after_update(
    *, supervision_verify: bool = True
    ...
    try:
        ...
        try:
            ...
            print(
                ...
                "Recover manually: socis gateway restart"
            )
    ...
    print(
        ...
        "    Check logs, then: socis gateway restart"
    )
    return [], [current_label]
...
def _restart_macos_launchd_gateways(
    ...
    ``ai.socis.gateway*`` LaunchAgent must reload it — restarting only the
...
def _surviving_gateway_pids_after_failed_restart():
    ...
    when ``socis_cli.gateway`` itself no longer imports, which is one of the
...
def _gateway_service_matches_profile(profile: str, service: object) -> bool:
    ...
    that ``socis-gateway-foobar.service`` was already restarted.  These are
    ...
    return name in {
        f"socis-gateway-{profile}",
        f"ai.socis.gateway-{profile}",
        f"gateway-{profile}",
    }


# Abort recovery lives in its own bounded module (review on #96235). Re-exported
# here because `socis_cli.main` and the update flow below address these names
# through `update_cmd`.
from socis_cli.update_abort_recovery import (  # noqa: E402
...
def _gateway_recovery_partition(
    ...
    only safe source here: re-importing ``socis_cli.gateway`` in the failing
    ...
    unrecoverable.  The fresh child runs a separate ``socis-serve*`` systemd
    ...
    ``socis serve``: it records no spawner, so it reads as ``manual-serve``.
        ...
        for runtime in getattr(plan, "runtimes", ()) or ():
            ...
            if kind == "gateway":
                ...
                if supervisor == "desktop":
                    ...
                    # `socis serve` sets neither SOCIS_AGENT_SPAWN nor
                    # SOCIS_AGENT_PARENT_PID, so it lands here as "manual-serve".
                    # Unit-backed serve runtimes are recovered by the
                    # fresh child's systemd pass, which enumerates
                    # `socis-serve*` from systemd instead of trusting this
                    ...
                    reason = (
                        ...
                        " unit pass when it owns a socis-serve* unit, else"
...
def _warn_gateway_restart_phase_aborted(exc: BaseException, pids) -> None:
    ...
    (e.g. importing ``socis_cli.gateway`` from the freshly pulled checkout)
...
def _refresh_windows_gateway_launchers() -> None:
    ...
    ``socis update`` never touched them, so installs created before the
...
def _refresh_bootstrap_cache_scripts(branch: str = "main") -> None:
    ...
    The Desktop GUI updater (``socis-setup.exe``) executes
    ``$SOCIS_AGENT_HOME/bootstrap-cache/install-<ref>.ps1`` (or ``.sh``) for its
    ...
    try:
        ...
        cache_dir = Path(_m().get_socis_agent_home()) / "bootstrap-cache"
...
def _resume_windows_gateways_after_update(token: dict | None) -> None:
    ...
    if relaunched or unmapped_relaunched:
        ...
        if not ready_pids:
            ...
            print("    Recover with: socis gateway restart")
            raise RuntimeError(
...
def _portable_git_candidates() -> list:
    ...
    The SOCIS-managed PortableGit tree lives under the SHARED root
    (``<root>/git/...``), not the profile-scoped SOCIS_AGENT_HOME
    (``<root>/profiles/<name>``), so a profile-scoped ``socis update`` must
    ...
    candidates = []
    try:
        for root in (get_default_socis_root(), Path(get_socis_agent_home())):
            candidates.append(
...
def _locate_real_git() -> Optional[Path]:
    ...
    SOCIS-managed PortableGit copy; accept the first candidate that runs and
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``socis update`` sees a clean tree instead of autostashing every run.
...
def _normalize_managed_eol(git_cmd, repo_root):
    ...
    ``install.ps1`` forever — so ``socis update``, which ships with the checkout
...
def _rebuild_desktop_after_update(
    ...
    # `socis desktop --build-only` subprocess re-imports the whole CLI stack
    ...
    desktop_build_cmd = [sys.executable, "-m", "socis_cli.main", "desktop", "--build-only"]
    ...
    #
    # Start the build subprocess with the SOCIS-managed Node on PATH: when
    # `socis update` runs inside the desktop updater chain (Desktop →
    # socis-setup → socis update), the shell PATH customizations are lost,
    # so a bare-PATH child would fail with `node: not found` before cmd_gui can
    # self-heal.
    from socis_agent_constants import with_socis_node_path

    build_env = with_socis_node_path()
    ...
    if build_result.returncode != 0:
        print("  ⚠ Desktop build failed (run `socis desktop` to retry)")
        tail = "\n".join((build_result.stdout or "").strip().splitlines()[-15:])
        if tail:
            print(tail)
        from socis_agent_constants import display_socis_agent_home as _dhh
...
def _venv_foreign_owned_paths(venv_root, limit: int = 5) -> list:
    ...
    A venv that was ever touched by ``sudo pip`` / ``sudo socis`` contains
    root-owned files (classically ``*.dist-info/INSTALLER``). A later normal
    ``socis update`` then dies mid-mutation inside ``uv pip install -e .``
    ("Permission denied (os error 13)") with ``venv/bin/socis`` already
...
def _refuse_update_if_venv_foreign_owned(project_root) -> None:
    ...
    print("  Updating now would fail midway (Permission denied) and leave SOCIS broken.")
    print("  This usually happens after running socis or pip with sudo. Offending paths:")
...
def _drain_or_signal_gateway_for_update(
    ...
    """Decide how ``socis update`` hands a running gateway over to new code.
    ...
    1. **Gateway is an ancestor of this process** — THREE-WAY DEADLOCK BREAK
       (#100179). When ``socis update`` runs INSIDE the gateway's own
       process tree (the socis-auto-update cron job is the canonical case),
       waiting for that gateway to exit is a circular wait::

           gateway  waits on all in-flight work units (#77184)
             └─ cron agent session waits on the `socis update` process
                  └─ `socis update` waits on the gateway to exit  ← back to A
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        from socis_cli.update_receipt import begin_update_receipt
    ...
    # running SOCIS runtime, its supervisor, and its running code version —
    ...
    # On Windows, abort early if another socis.exe is holding the venv shim
    ...
    if _m()._is_windows() and not getattr(args, "force", False):
        scripts_dir = _m()._venv_scripts_dir()
        if scripts_dir is not None:
            concurrent = _m()._detect_concurrent_socis_instances(scripts_dir)
            if concurrent:
    ...
    # (most commonly the Desktop app's `socis serve` backend) will keep .pyd
    ...
    # the socis.exe shim guard above, but its lock probe only checks the shim
    ...
    if _m()._is_windows() and not getattr(args, "force_venv", False):
        ...
        if _venv_holders:
            ...
            # the spawn ledger proves are orphaned SOCIS backends — the
            ...
            if _ledger_backends:
                print(
                    ...
                    "SOCIS backend process(es) hold the venv; stopping their trees"
                )
        ...
        if _venv_holders:
            ...
            if _orphan_backends:
                ...
                # its backend (and any .socis-runtime child) holding the
                # venv. Nothing will respawn an orphan, so reap the tree and
                # re-check instead of dead-ending with "SOCIS is still
        ...
        if _venv_holders:
            ...
            # `socis serve --host <ip>` powering a REMOTE Desktop holds the
        ...
        if _venv_holders:
            ...
            # the hand-off context those surviving SOCIS backends are leaks,
            ...
            try:
                ...
                if _scripts_dir is not None:
                    _no_live_shim = not _m()._detect_concurrent_socis_instances(_scripts_dir)
            except Exception:
                _no_live_shim = False
            if _handoff and _no_live_shim:
                ...
                if _handoff_backends:
                    print(
                        f"  ⚠ {len(_handoff_backends)} SOCIS backend process(es) "
    ...
    # process by design (a CLI `socis update` IS the venv python), and an
    ...
    try:
        ...
        if commit_count == 0:
            ...
            if handed_off_sync:
                print("→ Finishing the dependency install handed off by socis.exe...")
            elif not healthy:
            ...
            if handed_off_sync or not healthy:
                ...
                if healthy_after:
                    ...
                    print("  Close all SOCIS windows/gateways and re-run: socis update")
            else:
            ...
            if runtime_repaired is not None and not _m()._is_windows():
                ...
                print(
                    "  Any running SOCIS gateways, Desktop backends, or other "
        ...
        # orphan merge-conflict markers in socis_cli/config.py bricked
        # every user who ran ``socis update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``socis
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``socis update`` again later once a fix lands.")
                    else:
        ...
        # Before this guard, ``socis update`` printed "✓ Code updated!" and
        # reinstalled deps + rebuilt the desktop app against the stale tree —
        # no error, no warning, ``socis doctor`` healthy. Compare pre-pull
        ...
        if pre_pull_sha and post_pull_sha == pre_pull_sha:
            ...
            print(
                ...
                f"git -C {_m().PROJECT_ROOT} checkout {branch} && socis update"
            )
            _m()._resume_windows_gateways_after_update(_windows_gateway_resume)
            sys.exit(1)
        ...
        # ``socis update`` can catch up even when git is already up to date.
        ...
        # the old bytecode (e.g. get_socis_agent_home added to socis_agent_constants).
        removed = _m()._clear_bytecode_cache(_m().PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # marker survives and the next ``socis`` launch finishes the install
        ...
        # symbols in socis_agent_constants or lazy_deps. The dependency install
        ...
        if lazy_ok:
            ...
            print(
                "  ⚠ Lazy-refresh recovery incomplete — run `socis` again "
        ...
        if not import_ok:
            ...
            print("    Run `socis update` again — if it persists, reinstall:")
            print("    https://agent.socis.io")
        ...
        if sys.platform == "darwin" and had_desktop_app_before_update:
            print()
            print(
                "  ℹ macOS: if SOCIS re-prompts for permissions you already "
        ...
        try:
            from socis_cli.macos_tcc_anchor import ensure_tcc_anchor
        ...
        # current — so copy it straight over ~/.socis-agent/cache/model_catalog.json
        ...
        # seed_profile_skills() uses subprocess with an explicit SOCIS_AGENT_HOME so
        # it is not affected by sync_skills()'s module-level SOCIS_AGENT_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's SOCIS_AGENT_HOME env var points at the default or a named profile.
        try:
        ...
        try:
            from socis_cli.profiles import backfill_profile_envs
        ...
        # check `socis curator status`. Self-stamps after printing so it
        ...
        # Self-heal the socis-acp launcher for installs that predate it, so
        # ACP hosts (Zed, JetBrains, Buzz) can resolve SOCIS on PATH without
        # a reinstall.  No-op on Windows (the launcher migration below owns
        # that) and when already present.
        try:
            _ensure_acp_launcher()
        except Exception as e:
            logger.debug("socis-acp launcher self-heal failed: %s", e)

        # Migrate the Windows socis launchers to the managed binary dir
        # (the default SOCIS root's bin, next to the managed uv) and repair
        # them if they are missing. Earlier layouts put them inside the git
        # checkout (socis-agent\bin) or put venv\Scripts itself on PATH; the
        # in-checkout copies were swept by this command's own pre-update
        # autostash (git stash push --include-untracked) and, with
        # --keep-stash (the desktop updater), never restored — `socis`
        ...
        # the managed clone under the default SOCIS root).
        try:
            from socis_cli._install_repair import migrate_windows_bin_path
        ...
        # don't have it. Tying the refresh to ``socis update`` gives users a
        ...
        try:
            ...
            if (
                ...
                from socis_cli.tools_config import install_cua_driver
                ...
                # driver) keeps the installed version — `socis update`
                # must stay fast; `socis computer-use install --upgrade`
        ...
        # When running as ``socis update --gateway`` (spawned by the gateway's
        ...
        # ``user/socis-serve`` and ``system/socis-serve`` are two different
        ...
        #
        # Purge stale cached SOCIS modules FIRST: the import below pulls
        ...
        try:
            ...
            def _resolve_manage_cmd(scope_: str, scope_cmd_: list, svc_name_: str):
                ...
                ``systemctl ... socis-gateway*`` also qualifies
                ...
                if (
                    ...
                    try:
                        ...
                        if not sudo_ok:
                            ...
                            # (NOPASSWD for systemctl ... socis-gateway*)
            ...
            try:
                from socis_cli.gateway import _get_restart_exit_wait_budget
            ...
            # Discover all socis-gateway* units (default + profiles) plus
            # socis-serve* units (the Desktop app's backend, #83438).
            if supports_systemd_services():
                try:
                    _ensure_user_systemd_env()
                except Exception:
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        print(
                            ...
                            f"Check the gateway with: socis gateway status"
                        )
                        continue
                    ...
                    def _restart_one_systemd_gateway_unit(svc_name: str) -> None:
                        ...
                        # socis-serve has no such handler (it isn't
                        ...
                        if _manage_cmd is None:
                            ...
                            print(
                                ...
                                f"    To let `socis update` restart it automatically, allow\n"
            ...
            # Restart EVERY ai.socis.gateway* LaunchAgent, not only the
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: socis gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: socis -p <profile> gateway run  for each profile)"
                        )

            if failed_or_stale_units:
                gateway_fleet_restart_incomplete = True
                if gateway_mode:
                    _exit_code_path = get_socis_agent_home() / ".update_exit_code"
                    try:
            ...
            try:
                from socis_cli.update_receipt import record_gateway_restart
            ...
            # the reporter hit through tui_gateway (hosted by `socis serve`,
        ...
        # Warn if legacy SOCIS gateway unit files are still installed.
        # When both socis.service (from a pre-rename install) and the
        # current socis-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `socis update` surfaces the issue until the user migrates.
        try:
            from socis_cli.gateway import (
                has_legacy_socis_units,
                _find_legacy_socis_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_socis_units():
                print()
                print("⚠ Legacy SOCIS gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_socis_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (socis.service) fight the current")
                print("  socis-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    socis gateway migrate-legacy")
                print()
        ...
        #
        # Forward the systemd units restarted above (includes socis-serve*,
        ...
        # `socis serve` (no unit) is left running its pre-update
        ...
        print("  socis model              # Select provider and model")
        ...
        try:
            from socis_cli.update_receipt import (
        ...
        try:
            if _pre_update_plan is not None and _pre_update_plan.runtimes:
                ...
                if report_unaccounted_runtimes(_runtime_outcomes):
                    gateway_fleet_restart_incomplete = True
                try:
                    import socis_cli.update_receipt as _ur
        ...
        if gateway_fleet_restart_incomplete:
            ...
            # ``socis update`` still runs the catch-up restart.
            sys.exit(1)
        _clear_fleet_restart_pending_marker()
            ...
            if _called_process_error_is_python_dep_install(e):
                ...
                if _m()._is_windows():
                    ...
                    print(
                        ...
                        '"from socis_cli.main import main; main()" update --yes'
                    )
            try:
                from socis_cli.update_receipt import finalize_update_receipt
...
def _restart_phase_failure_is_incomplete(surviving, pre_restart_pids) -> bool:
    ...
    * ``surviving is None`` — the survivor probe could not determine state
      (typically the freshly-pulled ``socis_cli.gateway`` no longer imports,
