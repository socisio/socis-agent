"""
Profile management for multiple isolated SOCIS instances.

Each profile is a fully independent SOCIS_AGENT_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.socis-agent/profiles/<name>/`` by default.

The "default" profile is ``~/.socis-agent`` itself — backward compatible,
zero migration needed.

Usage::

    socis profile create coder          # fresh profile + bundled skills
    socis profile create coder --clone  # also copy config, .env, SOUL.md, skills
    socis profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    socis -p coder chat                 # or via flag
    socis profile use coder             # set as sticky default
    socis profile delete coder          # remove profile + alias + service
"""
...
from socis_cli.archive_safe import (
    archive_root_dirs,
    make_targz,
...
_PROFILE_DIRS = [
    ...
    # See socis_agent_constants.get_subprocess_home().
    "home",
]
...
# default profile (``~/.socis-agent``).  Named profiles never contain these
# directories at root, so the exclusion is gated to avoid silently dropping
# user data from a named-profile source.
#
# Rationale per item:
#   socis-agent  — git repo checkout (~84 MB source + ~3 GB venv)
...
# Marker file written by `socis profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `socis
...
# `socis skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Three categories:
      ...
      2. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         SOCIS infrastructure directories that only the default profile
         (``~/.socis-agent``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_socis_agent_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.socis-agent) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "socis-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
# Allow-list for ``export_profile("default")``: when SOCIS_AGENT_HOME equals the
...
# into the export. The set below identifies the *known SOCIS profile
# artifacts* at the root of SOCIS_AGENT_HOME; everything else is excluded.
...
# (#58394). Add new artifacts here when introduced in ``socis_agent_constants``.
...
_RESERVED_NAMES = frozenset({
    "socis", "default", "test", "tmp", "root", "sudo",
})

# SOCIS subcommands that cannot be used as profile names/aliases
_SOCIS_AGENT_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the socis root, NOT to the current SOCIS_AGENT_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where SOCIS_AGENT_HOME points outside
    ``~/.socis-agent``, profiles live under ``SOCIS_AGENT_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_socis_agent_home() / "profiles"


def _get_default_socis_agent_home() -> Path:
    """Return the default (pre-profile) SOCIS_AGENT_HOME path.

    In standard deployments this is ``~/.socis-agent``.
    In Docker/custom deployments where SOCIS_AGENT_HOME is outside ``~/.socis-agent``
    (e.g. ``/opt/data``), returns SOCIS_AGENT_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_socis_agent_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the SOCIS installation itself or a common system binary.  "
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its SOCIS_AGENT_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_socis_agent_home()
...
def profile_matches_home(name: str, home: "Path | None" = None) -> bool:
    ...
    ``home`` defaults to the process's current SOCIS home
    (:func:`socis_agent_constants.get_socis_agent_home`).  Used by single-profile
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: alias-name validity, reserved names, socis subcommands, existing
    binaries in PATH.
    """
    canon = normalize_profile_name(name)
    try:
    ...
    if canon in _SOCIS_AGENT_SUBCOMMANDS:
        return f"'{canon}' conflicts with a socis subcommand"
...
def create_wrapper_script(name: str, target: Optional[str] = None) -> Optional[Path]:
    ...
    if is_windows:
        ...
        try:
            socis_exe = shutil.which("socis") or "socis"
            wrapper_path.write_text(f'#!/bin/sh\nexec {shlex.quote(socis_exe)} -p {profile} "$@"\n', encoding="utf-8")
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    ``_config_version``) or that is simply older than the running SOCIS. If we
    ...
    try:
        ...
        # not be migrated. The next `socis doctor --fix` can still surface the
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``socis -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``socis profile alias
...
# Real wrappers are a few hundred bytes of shell; the needle (``socis -p X``)
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``socis profile list`` for an unrelated profile.
    """
...
def _seed_model_config(profile_dir: Path) -> None:
    ...
    try:
        ...
        source = get_socis_agent_home() / "config.yaml"
        ...
        # Creation must not fail over this; `socis model` still sets it later.
        pass
...
def _check_gateway_running(profile_dir: Path) -> bool:
    ...
    *this* reader — which is exactly the case for a dashboard process that is
    ...
    agree.  Parameterized by ``profile_dir`` so it never mutates ``SOCIS_AGENT_HOME``.
    """
    try:
...
def _served_by_running_multiplexer(profile_name: str) -> bool:
    ...
    """
    try:
        from socis_cli.gateway import named_profile_served_by_running_multiplexer
...
# ``config.yaml``. ``config.yaml`` is the user-facing SOCIS config
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_socis_agent_home()
...
def profiles_to_serve(
    ...
    """Return the ``(profile_name, socis_agent_home)`` pairs a gateway should serve.
    ...
    - ``multiplex=True``: returns the default profile plus every valid named
      profile under ``profiles/``, each paired with its own SOCIS_AGENT_HOME. When
    ...
    The returned ``socis_agent_home`` is the path to pass to
    ``set_socis_agent_home_override`` when scoping a turn to that profile.
    """
    active = get_active_profile_name() or "default"
    if not multiplex:
        return [(active, get_profile_dir(active))]

    serve: List[Tuple[str, Path]] = [("default", _get_default_socis_agent_home())]
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``socis update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.socis-agent)."
        )
    ...
    if clone_from is not None or clone_all or clone_config:
        if clone_from is None:
            ...
            from socis_agent_constants import get_socis_agent_home
            source_dir = get_socis_agent_home()
        else:
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.socis-agent/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
            symlinks=True,
        ...
        from socis_cli.auth import strip_cloned_single_use_oauth_grants
    ...
    # Keys pages, `socis -p <name> auth add`) had no file until first
    ...
    if not env_path.exists():
        try:
            env_path.write_text(
                "# Per-profile secrets for this SOCIS profile.\n"
    ...
    if not soul_path.exists():
        try:
            from socis_cli.default_soul import DEFAULT_SOUL_MD
            soul_path.write_text(DEFAULT_SOUL_MD, encoding="utf-8")
        except Exception:
            pass  # best-effort — don't fail profile creation over this

    # Write the opt-out marker so seed_profile_skills() and `socis update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`socis profile create --no-skills`).\n"
    ...
    # Cloned configs can be older than the running SOCIS (or predate schema
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `socis profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `socis -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches SOCIS_AGENT_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``socis profile create
    ...
    only the essential skills (e.g. ``socis-agent``), reporting
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "SOCIS_AGENT_HOME": str(profile_dir)},
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    ...
    default_env = _get_default_socis_agent_home() / ".env"
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    """PIDs of running SOCIS *backends* bound to this profile.
    ...
    names it, so find it by inspection: a SOCIS backend subcommand
    (``serve``/``dashboard``/``gateway``) that is bound to *this* profile either
    by a ``--profile <canon>`` / ``-p <canon>`` selector or by a ``SOCIS_AGENT_HOME``
    ...
    # Never terminate ourselves or a parent (e.g. `socis -p <canon> profile
    ...
    socis_markers = ("socis_cli.main", "socis-gateway", "tui_gateway")
    ...
    # something (e.g. Electron's `findOnPath('socis')` resolution) spawns the
    ...
    # interpreter, not "socis", so the checks below would otherwise miss it.
    ...
    # interpreter.
    _SOCIS_AGENT_CONSOLE_SCRIPT_NAMES = frozenset({"socis", "socis-agent", "socis-acp"})
    pids: list[int] = []
    ...
    for proc in psutil.process_iter(["pid", "name", "username", "cmdline"]):
        try:
            ...
            # Must be a SOCIS process: either an entrypoint marker in argv, a
            # resolved executable named `socis`, or a python interpreter
            # directly exec'ing a `socis`-named console-script shim (argv[0]
            # is the interpreter, argv[1] is the shim's path).
            joined = " ".join(argv)
            exe_name = os.path.basename(argv[0]).lower()
            is_socis = (
                any(marker in joined for marker in socis_markers)
                or exe_name == "socis"
                or exe_name.startswith("socis")
            )
            if not is_socis and len(argv) >= 2 and _python_interpreter_re.match(exe_name):
                # Match against the actual known console-script entry points
                # (pyproject.toml [project.scripts]: socis, socis-agent,
                # socis-acp) rather than a bare `startswith("socis")` --
                ...
                # "socis-notes.py" or "socis-unrelated-tool" as the shim,
                ...
                is_socis = script_stem in _SOCIS_AGENT_CONSOLE_SCRIPT_NAMES
            if not is_socis:
                continue
            ...
            # ...or by SOCIS_AGENT_HOME env pointing at this profile dir.
            if not bound:
                try:
                    env_home = (proc.environ() or {}).get("SOCIS_AGENT_HOME", "")
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.socis-agent).\n"
            "To remove everything, use: socis uninstall"
        )
    ...
    # the rmtree below fail with ENOTEMPTY and — before the ensure_socis_agent_home
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``SOCIS_AGENT_HOME`` and binds the port resolved by ``gateway/config.py``
    ...
    try:
        ...
        from socis_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set SOCIS_AGENT_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("SOCIS_AGENT_HOME")
    try:
        os.environ["SOCIS_AGENT_HOME"] = str(profile_dir)
        from socis_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["SOCIS_AGENT_HOME"] = old_home
        elif "SOCIS_AGENT_HOME" in os.environ:
            del os.environ["SOCIS_AGENT_HOME"]
...
def _stop_gateway_process(profile_dir: Path) -> None:
    ...
    try:
        ...
        pid = int(data["pid"])
        # Cross-profile kill refusal (#89315): the record's socis_agent_home stamp
        ...
        if recorded_gateway_home_conflicts(data, expected_home=profile_dir):
            print(
                f"✗ Refusing to stop PID {pid}: its recorded SOCIS_AGENT_HOME "
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.socis-agent/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: socis profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from SOCIS_AGENT_HOME.

    Returns ``"default"`` if SOCIS_AGENT_HOME is not set or points to ``~/.socis-agent``.
    Returns the profile name if SOCIS_AGENT_HOME points into ``~/.socis-agent/profiles/<name>``.
    Returns ``"custom"`` if SOCIS_AGENT_HOME is set to an unrecognized path.
    """
    from socis_agent_constants import get_socis_agent_home
    socis_agent_home = get_socis_agent_home()
    resolved = socis_agent_home.resolve()

    default_resolved = _get_default_socis_agent_home().resolve()
...
def _inside_git_checkout(path: Path) -> bool:
    ...
    ``SOCIS_AGENT_HOME`` points inside a checkout but the process runs from
...
def _profile_export_directory() -> Path:
    ...
    export_dir = _get_default_socis_agent_home() / "profile-exports"
    if not _inside_git_checkout(export_dir):
        return export_dir

    # A custom deployment may point SOCIS_AGENT_HOME at its source checkout.  Do
    ...
    # Per-uid temp name: a fixed /tmp/socis-profile-exports is a predictable
    # shared path another local user could pre-create (or symlink) before us.
    uid_suffix = f"-{os.getuid()}" if hasattr(os, "getuid") else ""
    candidates = (
        Path.home() / ".socis-profile-exports",
        Path(tempfile.gettempdir()) / f"socis-profile-exports{uid_suffix}",
    )
...
def _default_export_ignore(root_dir: Path):
    ...
    * **Root-level allow-list** — only entries whose name appears in
      ...
      SOCIS_AGENT_HOME equals the cwd) is excluded. Blacklisting was tried
      first and proved unable to anticipate every non-SOCIS file the
      user may have lying alongside SOCIS_AGENT_HOME (#58394).
...
def _scrub_export_secrets(staged: Path) -> None:
    ...
    ``socis sessions export --redact``. Runs on the *staged copy only* so the
    live profile is never rewritten. ``force=True`` ignores
    ``security.redact_secrets`` / ``SOCIS_AGENT_REDACT_SECRETS`` — share archives
...
def export_profile(name: str, output_path: str, extra_files: Optional[Dict[str, str]] = None) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.socis-agent itself — its parent is ~/ and its
        # directory name is ".socis-agent", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: socis profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.socis-agent itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.socis-agent). "
            "Specify a different name: socis profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="socis_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"socis_{old_name}"
    legacy_old_host = f"socis.{old_name}"
    new_host = f"socis_{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_socis_agent_home() / "honcho.json",
    ...
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            if source_host.startswith("socis_"):
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a SOCIS_AGENT_HOME path string.

    Called early in the CLI entry point, before any socis modules
    are imported, to set the SOCIS_AGENT_HOME environment variable.

    When SOCIS_AGENT_HOME is already set, the configured spelling IS the
