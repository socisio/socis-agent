"""
Dump command for socis CLI.

Outputs a compact, plain-text summary of the user's SOCIS setup
...
from socis_cli.config import get_socis_agent_home, get_env_path, get_project_root, load_config
...
def _dotenv_key_names() -> set[str]:
    """Return the set of env-var names assigned a non-empty value in ~/.socis-agent/.env.
    ...
    exports. ``socis debug share`` runs in a terminal, so ``os.getenv`` reflects
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.socis_build_sha`` by the Dockerfile's
    ``SOCIS_AGENT_GIT_SHA`` build-arg (see ``socis_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``socis dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from socis_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(socis_agent_home: Path) -> int:
...
def _cron_summary(socis_agent_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = socis_agent_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    env_path = get_env_path()
    load_socis_dotenv(
        socis_agent_home=env_path.parent,
    ...
    lines.append(f"socis_agent_home:      {display_socis_agent_home()}")
    ...
    for env_var, label in api_keys:
        ...
        # Set in this (shell) process but absent from ~/.socis-agent/.env: a managed
        ...
        # A credential added via `socis auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `socis auth list` shows it (#42130).
    ...
    toolsets = config.get("toolsets", ["socis-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(socis_agent_home)}")
    lines.append(f"  skills:             {_count_skills(socis_agent_home)}")
